...
Public Class rptVSPCustomerReco
...
#End Region
    ...
    Private Sub rptVSPCustomerReco_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub Print(ByVal IsPrint As Exporter, Optional ByVal BulkExport As Integer = 0)
        Try
            ...
            If txtVendor.arrValueMember IsNot Nothing AndAlso txtVendor.arrValueMember.Count > 0 Then
                obj.Vendor_Code = txtVendor.arrValueMember
            End If
            ...
            'End If
            strRunQuery = "Select *,2  as RI from (" + clsCustomerMaster.GetCustomerBaseQry(False, False, "", False, ConvRate, strcustomerfilter, False, strFromDate, strToDate, False, False, chkIncludeApplyDocument.Checked) + ")aaa " & Environment.NewLine &
            " Union All 
             Select  *,2 as RI from (" + clsPurchaseInvoiceHead.GetVendorRecoQry(obj) + ")aaa "
            ...
            If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Account Wise") = CompairStringResult.Equal Then
                ...
                strRunQuery = "select xxxxx.*,isnull( JEOther.OpeningDRAmount,0)-isnull( JEOther.OpeningCRAmount ,0)  as OtherOpeningAmount,isnull( JEOther.DRAmount,0)  as OtherDRAmount," + Environment.NewLine +
                    " isnull( JEOther.CRAmount,0)  as OtherCRAmount,(isnull( JEOther.DRAmount,0)-isnull(JEOther.CRAmount,0)) as OtherNetAmount," + Environment.NewLine +
                    " ((isnull( JEOther.OpeningDRAmount,0)-isnull( JEOther.OpeningCRAmount ,0) )+(isnull( JEOther.DRAmount,0)-isnull(JEOther.CRAmount,0))) as TotalTrialNet," + Environment.NewLine +
                    " ((isnull( JEOther.OpeningDRAmount,0)-isnull( JEOther.OpeningCRAmount ,0) )+(isnull( JEOther.DRAmount,0)-isnull(JEOther.CRAmount,0)))+GLCLBal as TotalTrialCL," + Environment.NewLine +
                    " ( xxxxx.CLBal -(((isnull( JEOther.OpeningDRAmount,0)-isnull( JEOther.OpeningCRAmount ,0) )+(isnull( JEOther.DRAmount,0)-isnull(JEOther.CRAmount,0)))+GLCLBal) ) as VendorNetTotalDiff from (" + Environment.NewLine +
                "select Account_code,max(Account_Desc) as Account_Desc" + Environment.NewLine +
                ",sum(case when RI=1 then  DRAmount-CRAmount else 0 end) as OPBal" + Environment.NewLine +
                ",sum(case when RI=2 then  DRAmount else 0 end) as DRAmount" + Environment.NewLine +
                ",sum(case when RI=2  then CRAmount else 0 end ) as CRAmount" + Environment.NewLine +
                ",sum(case when RI=2 then NetAmount else 0 end ) as NetAmount" + Environment.NewLine +
                ",sum(case when RI=2 then NetAmount when RI=1 then  DRAmount-CRAmount else 0 end) as CLBal" + Environment.NewLine +
                ...
                ",sum(case when RI=2 then DiffAmount else 0 end ) as DiffAmountNet " + Environment.NewLine +
                ",(sum(case when RI=2 then NetAmount when RI=1 then  DRAmount-CRAmount else 0 end)-sum(case when RI=2 then GLNetAmount  when RI=1 then GLDRAmount-GLCRAmount else 0 end)) as DiffAmountCL " + Environment.NewLine +
                "from (" + strRunQuery + ")Final group by Account_code" + Environment.NewLine +
                ")xxxxx  " + Environment.NewLine +
                "left outer join (select z.Account_code,sum(z.DRAmount) as DRAmount,sum(z.CRAmount) as CRAmount,sum(z.OpeningDRAmount) as OpeningDRAmount,sum(z.OpeningCRAmount) as OpeningCRAmount from (select TSPL_JOURNAL_DETAILS.Account_code,sum(TSPL_JOURNAL_DETAILS.Amount * case when TSPL_JOURNAL_DETAILS.Amount>0  then 1 else 0 end   ) as DRAmount ," + Environment.NewLine +
                "sum(TSPL_JOURNAL_DETAILS.Amount * case when TSPL_JOURNAL_DETAILS.Amount<0 then -1 else 0 end  ) as CRAmount ,0 as OpeningDRAmount,0 as OpeningCRAmount" + Environment.NewLine +
                ...
                End If
                strRunQuery += ") and TSPL_JOURNAL_MASTER.Authorized='A' " + Environment.NewLine +
                    " and CAST(TSPL_JOURNAL_MASTER.Voucher_Date AS DATE) >= '" + clsCommon.GetPrintDate(obj.From_Date, "dd-MMM-yyyy") + " ' " + Environment.NewLine +
                ...
                If chkOPAndClosing.Checked = True Then
                    strRunQuery += " Union All " & Environment.NewLine &
                    "select TSPL_JOURNAL_DETAILS.Account_code,0 as DRAmount,0 as CRAmount,sum(TSPL_JOURNAL_DETAILS.Amount * case when TSPL_JOURNAL_DETAILS.Amount>0  then 1 else 0 end   ) as OpeningDRAmount ," + Environment.NewLine +
                    "sum(TSPL_JOURNAL_DETAILS.Amount * case when TSPL_JOURNAL_DETAILS.Amount<0 then -1 else 0 end  ) as OpeningCRAmount " + Environment.NewLine +
                    " from TSPL_JOURNAL_DETAILS " + Environment.NewLine +
                    " left outer join TSPL_JOURNAL_MASTER on TSPL_JOURNAL_MASTER.Voucher_No=TSPL_JOURNAL_DETAILS.Voucher_No " + Environment.NewLine +
                    " where (isnull(TSPL_JOURNAL_DETAILS.Reco_Control_Account,'')<>'V' "
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Vendor And Account Wise") = CompairStringResult.Equal Then
                strRunQuery = "select  max(Vendor_Group_Code) as Vendor_Group_Code,max(Group_Desc) as Group_Desc,max(Vendor_Account) as Vendor_Account,Vendor_Code,max(Vendor_Name) as Vendor_Name, Account_code,max(Account_Desc) as Account_Desc" + Environment.NewLine +
                    ",sum(case when RI=1 then  DRAmount-CRAmount else 0 end) as OPBal" + Environment.NewLine +
                    ",sum(case when RI=2 then  DRAmount else 0 end) as DRAmount" + Environment.NewLine +
                    ",sum(case when RI=2  then CRAmount else 0 end ) as CRAmount" + Environment.NewLine +
                    ",sum(case when RI=2 then NetAmount else 0 end ) as NetAmount" + Environment.NewLine +
    ...
    Sub OtherDataPrint(ByVal IsPrint As Exporter, Optional ByVal BulkExport As Integer = 0)
        Try
            ...
            strRunQuery = " select TSPL_JOURNAL_MASTER.Voucher_No ,TSPL_JOURNAL_MASTER.Voucher_Date  ,TSPL_JOURNAL_MASTER.Source_Doc_No ,TSPL_JOURNAL_MASTER.Source_Doc_Date ,TSPL_JOURNAL_MASTER.CustVend_Code as [Customer/Vendor Code],TSPL_JOURNAL_MASTER.CustVend_Name as [Customer/Vendor Name]  ," + Environment.NewLine +
            " TSPL_JOURNAL_DETAILS.Account_code,case when TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_JOURNAL_MASTER .CustVend_Code then 'Customer'  when TSPL_VENDOR_MASTER.Vendor_Code  =TSPL_JOURNAL_MASTER .CustVend_Code then 'Vendor'" + Environment.NewLine +
            " when isnull(TSPL_JOURNAL_MASTER .CustVend_Code,'')='' then 'JE Other' end as [Journal Entry Type]," + Environment.NewLine +
            "  (TSPL_JOURNAL_DETAILS.Amount * case when TSPL_JOURNAL_DETAILS.Amount>0  then 1 else 0 end   ) as DRAmount ," + Environment.NewLine +
            " (TSPL_JOURNAL_DETAILS.Amount * case when TSPL_JOURNAL_DETAILS.Amount<0 then -1 else 0 end  ) as CRAmount " + Environment.NewLine +
            " from TSPL_JOURNAL_DETAILS " + Environment.NewLine +
            " left outer join TSPL_JOURNAL_MASTER on TSPL_JOURNAL_MASTER.Voucher_No=TSPL_JOURNAL_DETAILS.Voucher_No " + Environment.NewLine +
    ...
    Private Sub btnClose_Click(sender As Object, e As EventArgs) Handles btnClose.Click
        ...
        Try
            qry = "CREATE NONCLUSTERED INDEX [TSPL_JOURNAL_MASTER_Authorized_Voucher_Date]" +
        ...
        Try
            qry = "CREATE NONCLUSTERED INDEX [TSPL_VENDOR_INVOICE_HEAD_Posting_Date]" +
            "ON [dbo].[TSPL_VENDOR_INVOICE_HEAD] ([Posting_Date])" +
    ...
    Private Sub txtTransaction__My_Click(sender As Object, e As EventArgs) Handles txtTransaction._My_Click
        Dim qry As String = " Select xxx.Code,  xxx.Name From (" &
                                 ...
                                 " Union  Select distinct 'MCC' As Code,    'Milk Receipt' As Name from TSPL_MILK_RECEIPT_HEAD " &
...
select Cust_Account as [Code],Cust_Acct_Desc as Name,Receivable_Control_acct as [Payable Account],Receipts_Discount_acct as [Discount Account],Advance_Acct as [Advance Account],Created_By as [Created By],Created_Date as [Created Date],Modify_By as [Modify By],Modify_Date as [Modify Date],Comp_Code as [Company Code],CURRENCY_CODE as [Currency Code],EXCHANGE_LOSS_ACCOUNT as [Exchange Loss Account],EXCHANGE_GAIN_ACCOUNT as [Exchange Gain Account] from TSPL_CUSTOMER_ACCOUNT_SET"
    ...
    Sub DrillDown()
        Try
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Vendor And Account Wise") = CompairStringResult.Equal Then
                ...
                arrCustomer = txtVendor.arrValueMember
                Dim tmp As New ArrayList()
                tmp.Add(clsCommon.myCstr(Gv1.CurrentRow.Cells("Vendor_Code").Value))
                txtVendor.arrValueMember = tmp
                Print(Exporter.Refresh)
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Detail") = CompairStringResult.Equal Then
    ...
    Private Sub txtCustomerGroup__My_Click(sender As Object, e As EventArgs) Handles txtCustomerGroup._My_Click
        Dim strQry As String = "Select Cust_Group_Code as Code, Cust_Group_Desc as Name from TSPL_CUSTOMER_GROUP_MASTER where ShowGroupOnReport=1"
    ...
    Private Sub txtVendor__My_Click(sender As Object, e As EventArgs) Handles txtVendor._My_Click
        Dim strQry As String = "Select TSPL_VENDOR_MASTER.Vendor_Code as Code,TSPL_VENDOR_MASTER.Vendor_Name as Name from TSPL_VENDOR_MASTER Left Outer join TSPL_CUSTOMER_VENDOR_MAPPING On TSPL_CUSTOMER_VENDOR_MAPPING.Vendor_Code =TSPL_VENDOR_MASTER.Vendor_Code where 1=1 "
    ...
    Private Sub txtcustomer__My_Click_1(sender As Object, e As EventArgs) Handles txtcustomer._My_Click
