...
,VirtualCategoryTabel.[FINISHFOOD],VirtualCategoryTabel.[RAWMATERIAL],VirtualCategoryTabel.[OTHER],VirtualCategoryTabel.[PACKINGMAT],VirtualCategoryTabel.[FIXEDASST],VirtualCategoryTabel.[MAKE],VirtualCategoryTabel.[KV],VirtualCategoryTabel.[FINISHFOODDESC],VirtualCategoryTabel.[RAWMATERIALDESC],VirtualCategoryTabel.[OTHERDESC],VirtualCategoryTabel.[PACKINGMATDESC],VirtualCategoryTabel.[FIXEDASSTDESC],VirtualCategoryTabel.[MAKEDESC],VirtualCategoryTabel.[KVDESC] ,TSPL_ITEM_MASTER.Item_Type,VirtualTableItemType.Name as Item_Type_Name,TSPL_INVENTORY_SOURCE_CODE.In_Category,TSPL_INVENTORY_SOURCE_CODE.Out_Category,TSPL_INVENTORY_SOURCE_CODE.Code,(case when ISNULL(InventroyMovement.Location_Code,'')='' then InventroyMovement.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) as PrimaryLocation, Structure_Desc,TSPL_ITEM_MASTER.RACK_NO  from ( "
            If chkPhV.Checked = False Then
                qry += "  select Fat_Amt,SNF_Amt,0 AS Fat_Rate,0 AS SNF_Rate ,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_INVENTORY_MOVEMENT where 2=2 "
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_INVENTORY_MOVEMENT.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_INVENTORY_MOVEMENT.Location_Code in (" + clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) + ")  "
                End If
                qry += "  union all 
select Fat_Amt,SNF_Amt,ISNULL(Fat_Rate,0) AS Fat_Rate,ISNULL(SNF_Rate,0) AS SNF_Rate,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,1 as IsFromMilk,Fat_Per as MilkFatPer ,SNF_Per as MilkSNFPer,Fat_KG as MilkFATKG,SNF_KG as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,isnull(Custom_UOM,'') as Custom_UOM,isnull(Custom_Coversion_Factor,0) as Custom_Coversion_Factor from TSPL_INVENTORY_MOVEMENT_NEW where 2=2 "
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_INVENTORY_MOVEMENT_NEW.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_INVENTORY_MOVEMENT_NEW.Location_Code in (" + clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) + ")  "
                End If
            Else
...
from TSPL_INVENTORY_MOVEMENT  where 2=2 "
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_INVENTORY_MOVEMENT.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                ...
                qry += "union all 
select 0 as Fat_Amt,0 as SNF_Amt,0 AS Fat_Rate,0 AS SNF_Rate ,'' as Trans_Id,'' as Trans_Type,'' as Source_Doc_No,TSPL_PO_WEIGHTMENT_HEAD.Weighment_Date as Punching_Date,'I' as InOut,TSPL_GRN_HEAD.Bill_To_Location,TSPL_PO_WEIGHTMENT_DETAIL.Item_Code,TSPL_PO_WEIGHTMENT_DETAIL.UOM, 0 as MRP,TSPL_PO_WEIGHTMENT_DETAIL.UOM As Stock_UOM,(TSPL_PO_WEIGHTMENT_DETAIL.Net_Weight*TSPL_ITEM_UOM_DETAIL.Conversion_Factor ) As Stock_Qty, 0 as FIFO_Cost,0 as LIFO_Cost,0 as Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 AS MilkSNFKG,0 as MilkFATQuintal,0 as MilkSNFQuintal,'' as SourceCode,'' as SourceName, '' as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  
...
And  TSPL_GRN_HEAD.GRN_No Not In (Select Against_GRN from TSPL_SRN_HEAD Where TSPL_SRN_HEAD.status=1 and  Against_GRN=TSPL_GRN_HEAD.GRN_No)
and convert(date,TSPL_PO_WEIGHTMENT_HEAD.Weighment_Date ,103) < '" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "' 
...
And TSPL_PO_WEIGHTMENT_HEAD.Status=1"
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_PO_WEIGHTMENT_DETAIL.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_GRN_HEAD.Bill_To_Location in (" + clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) + ")  "
                End If

                qry += "  union all
select 0 as Fat_Amt,0 as SNF_Amt,0 AS Fat_Rate,0 AS SNF_Rate ,'' as Trans_Id,'' as Trans_Type,'' as Source_Doc_No,MRN_Date as Punching_Date,'I' as InOut,TSPL_MRN_HEAD.Bill_To_Location,TSPL_MRN_DETAIL.Item_Code,TSPL_MRN_DETAIL.Unit_code UOM, 0 as MRP,TSPL_MRN_DETAIL.Unit_code Stock_UOM,TSPL_MRN_DETAIL.MRN_Qty Stock_Qty,0 as FIFO_Cost,0 as LIFO_Cost,0 as Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 AS MilkSNFKG,0 as MilkFATQuintal,0 as MilkSNFQuintal,'' as SourceCode,'' as SourceName, '' as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  
from TSPL_MRN_HEAD 
...
where TSPL_MRN_HEAD.MRN_No not in (select Against_MRN from TSPL_SRN_HEAD) and Location IN (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") and convert(date,MRN_Date ,103) < '" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") & "'   
and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and (VisualQCStatus <>2 or VisualQCStatusSecond<>2) "
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_MRN_DETAIL.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_MRN_HEAD.Bill_To_Location in (" + clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) + ")  "
                End If
...
left join (select Conversion_factor as cf,TSPL_ITEM_UOM_DETAIL.Item_code from TSPL_ITEM_UOM_DETAIL where UOM_code in (select UOM_Code  from TSPL_item_uom_detail where Item_Code = TSPL_ITEM_UOM_DETAIL.Item_code) And TSPL_ITEM_UOM_DETAIL.UOM_Code='" + clsCommon.myCstr(cmbUnit.SelectedValue) + "' ) as ITEMDETAIL1 on ITEMDETAIL1.Item_code=TSPL_SRN_DETAIL.Item_Code 
where TSPL_SRN_HEAD.Status=0 and Location IN (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") and convert(date,SRN_Date ,103) < '" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "'   
and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and TSPL_NIR_QC.qc_status=1  and (VisualQCStatus <>2 or VisualQCStatusSecond<>2)"
                ...
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_SRN_HEAD.Bill_To_Location in (" + clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) + ")  "
                End If
            End If
...
select * from ( 
select TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code ,TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code+'DESC' as Item_Category_CodeDesc,TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values ,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION as Category_Value_Desc 
...
Left Outer Join TSPL_ITEM_UOM_DETAIL On TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_PO_WEIGHTMENT_DETAIL.Item_Code And TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_PO_WEIGHTMENT_DETAIL.UOM "
                ...
                qry += " where  1=1 And TSPL_GRN_HEAD.Bill_To_Location In (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") 
And  TSPL_GRN_HEAD.GRN_No Not In (Select Against_GRN from TSPL_SRN_HEAD Where TSPL_SRN_HEAD.status=1 and Against_GRN=TSPL_GRN_HEAD.GRN_No)
and convert(date,TSPL_PO_WEIGHTMENT_HEAD.Weighment_Date ,103) between '" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "'  and '" + clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") + "'   
and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and (VisualQCStatus <>2 or VisualQCStatusSecond<>2)  
And TSPL_PO_WEIGHTMENT_HEAD.Status=1 "
...
where TSPL_MRN_HEAD.MRN_No not in (select Against_MRN from TSPL_SRN_HEAD) and Location In (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") and convert(date,MRN_Date ,103) between '" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "'  and '" + clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") + "'   
and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and (VisualQCStatus <>2 or VisualQCStatusSecond<>2)"
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_MRN_DETAIL.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                ...
                End If
                qry += "   union all
select 0 as Fat_Amt,0 as SNF_Amt,0 AS Fat_Rate,0 AS SNF_Rate ,'' as Trans_Id,'SRN' as Trans_Type,'' as Source_Doc_No,SRN_Date as Punching_Date,'I' as InOut,TSPL_SRN_HEAD.Bill_To_Location,TSPL_SRN_DETAIL.Item_Code,TSPL_SRN_DETAIL.Unit_code UOM, TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor as MRP ,ITEMDETAIL2.UOM_Code Stock_UOM, TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor Stock_Qty,(TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor)*(TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor) as FIFO_Cost,(TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor)*(TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor) as LIFO_Cost,(TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor)*(TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor) as Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 AS MilkSNFKG,0 as MilkFATQuintal,0 as MilkSNFQuintal,'' as SourceCode,'' as SourceName, '' as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  
from TSPL_SRN_HEAD 
left outer join TSPL_SRN_DETAIL on TSPL_SRN_DETAIL.SRN_No=TSPL_SRN_HEAD.SRN_No
left outer join TSPL_MRN_HEAD on TSPL_MRN_HEAD.MRN_No=TSPL_SRN_HEAD.Against_MRN
left outer join TSPL_GRN_HEAD on TSPL_GRN_HEAD.GRN_No=TSPL_MRN_HEAD.Against_GRN
left outer join TSPL_NIR_QC on TSPL_NIR_QC.mrn_no=TSPL_MRN_HEAD.mrn_no
left  join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_SRN_DETAIL.Item_Code And  TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_SRN_DETAIL.Unit_code
left join ( select UOM_Code,Item_Code,Conversion_Factor from TSPL_ITEM_UOM_DETAIL where  Stocking_Unit='Y') as ITEMDETAIL2 on ITEMDETAIL2.Item_code=TSPL_SRN_DETAIL.Item_Code where TSPL_SRN_HEAD.Status=0 and Location IN (" & clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) & ") and convert(date,SRN_Date ,103)  between '" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "'  and '" + clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") + "'  and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and TSPL_NIR_QC.qc_status=1  and (VisualQCStatus <>2 or VisualQCStatusSecond<>2)"
                If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                    qry += " and TSPL_SRN_DETAIL.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
                End If
                If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
...
union  all 
select Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,Punching_Date,InOutView,InOut,Location_Code,[Loc Desp],[LocAddress],SourceCode,SourceName,SourceType ,Item_Type, Item_Type_Name,Item_Group,Group_Description,null as [FINISHFOOD],null as [RAWMATERIAL],null as [OTHER],null as [PACKINGMAT],null as [FIXEDASST],null as [MAKE],null as [KV],null as [FINISHFOODDESC],null as [RAWMATERIALDESC],null as [OTHERDESC],null as [PACKINGMATDESC],null as [FIXEDASSTDESC],null as [MAKEDESC],null as [KVDESC],Items.Item_Code,Item_Desc, Item_Category_Struct_Code,  Items.Stock_UOM ,itf_code ,Stock_Qty,Balance_QTYKG,Rate,Cost,Balance_FAT, Balance_SNF ,0 as FATAmount,0 as SNFAmount ,In_Category,Out_Category,Structure_Desc,RACK_NO  from (SELECT 0 as Trans_Id,null as Trans_Type,null as Trans_Type_Name, null as Source_Doc_No, thedate as Punching_Date,'In' as InOutView,'I' as InOut,TSPL_LOCATION_MASTER.Location_Code as Location_Code,TSPL_LOCATION_MASTER.Location_Desc as [Loc Desp],null as [LocAddress],null as SourceCode,null as SourceName,null as SourceType ,TSPL_ITEM_MASTER.Item_Type,null as Item_Type_Name,null as Item_Group,null as Group_Description,null as [FINISHFOOD],null as [RAWMATERIAL],null as [OTHER],null as [PACKINGMAT],null as [FIXEDASST],null as [MAKE],null as [KV],null as [FINISHFOODDESC],null as [RAWMATERIALDESC],null as [OTHERDESC],null as [PACKINGMATDESC],null as [FIXEDASSTDESC],null as [MAKEDESC],null as [KVDESC],TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER.Item_Desc,null as Item_Category_Struct_Code "
            If clsCommon.myLen(cmbUnit.SelectedValue) > 0 Then
                qry += ",'" + clsCommon.myCstr(cmbUnit.SelectedValue) + "' as Stock_UOM "
            Else
                qry += ",TSPL_ITEM_UOM_DETAIL.UOM_Code as Stock_UOM "
            End If
            qry += " ,null as  itf_code ,0 as Stock_Qty,0 as Balance_QTYKG,0 as Rate,0 as Cost,0 as Balance_FAT, 0 as Balance_SNF ,null as In_Category,null as Out_Category,TSPL_ITEM_MASTER.Product_Type,Structure_Desc,TSPL_ITEM_MASTER.RACK_NO   FROM ExplodeDates( '" + clsCommon.GetPrintDate((txtFromDate.Value), "dd/MMM/yyyy") + "','" + clsCommon.GetPrintDate((txtToDate.Value), "dd/MMM/yyyy") + "') as d,TSPL_ITEM_MASTER,TSPL_LOCATION_MASTER,TSPL_ITEM_UOM_DETAIL where 2=2 " + Item_Type + " "

            If TxtItem.arrValueMember IsNot Nothing AndAlso TxtItem.arrValueMember.Count > 0 Then
                qry += " and TSPL_ITEM_MASTER.Item_Code in (" + clsCommon.GetMulcallString(TxtItem.arrValueMember) + ")  "
            End If
            If txtBillToLocation.arrValueMember IsNot Nothing AndAlso txtBillToLocation.arrValueMember.Count > 0 Then
                qry += " and TSPL_LOCATION_MASTER.Location_Code in (" + clsCommon.GetMulcallString(txtBillToLocation.arrValueMember) + ")  "
            End If


            qry += " and TSPL_ITEM_UOM_DETAIL.Stocking_Unit='Y' and TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code  
) Items 
left join (select TSPL_ITEM_QC_PARAMETER_MASTER.Item_Code,TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range as Fat_Per  
from TSPL_ITEM_QC_PARAMETER_MASTER  
left join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  
where TSPL_PARAMETER_MASTER.Type='FAT') Fat on Items.Item_Code=Fat.Item_Code  
left join  (select TSPL_ITEM_QC_PARAMETER_MASTER.Item_Code,TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range as SNF_Per  
from TSPL_ITEM_QC_PARAMETER_MASTER  
left join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  
where TSPL_PARAMETER_MASTER.Type='SNF') as SNF on Items.Item_Code=SNF.Item_Code where 2=2  
) xxxxxx Group by  Item_Code,Location_Code,Punching_Date 
) xxxxxxx left outer join tspl_location_master on tspl_location_master.Location_Code=xxxxxxx.Location_code where Punching_Date is not null )x "
            If clsCommon.myLen(cmbUnit.SelectedValue) > 0 Then
            ...
            Else
                qry += " where Location_Code IN (" & arrLoc & ") "
            End If
    ...
    Sub FormatGrid()
        ...
        If chkPhV.Checked = True Then
            Gv1.Columns("StockType").Width = 100
            Gv1.Columns("StockType").IsVisible = True
            Gv1.Columns("StockType").HeaderText = "Stock Type"
            Gv1.Columns("StockType").FormatString = "{0:n2}"
        End If
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Else
                qry += "isnull((InventroyMovement.Stock_Qty * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end))  ,0) as QtyKG,'" + clsCommon.myCstr(cmbUnit.SelectedValue) + "' as Stock_UOM,cast((InventroyMovement.Stock_Qty /  (case when InventroyMovement.Custom_UOM=TSPL_ITEM_UOM_DETAIL.UOM_Code and InventroyMovement.Custom_Coversion_Factor>0 then  InventroyMovement.Custom_Coversion_Factor else TSPL_ITEM_UOM_DETAIL.Conversion_Factor end)) as decimal(18,10)) as Stock_Qty,"
            End If
            qry += "isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end))
                    from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) as FatPer, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) as SNFPer, (case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=3 then InventroyMovement.FIFO_Cost else case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=2 then InventroyMovement.LIFO_Cost else InventroyMovement.Avg_Cost end end ) as Cost,TSPL_ITEM_MASTER.Item_Category_Struct_Code 
                   ,VirtualCategoryTabel.[FINISHFOOD],VirtualCategoryTabel.[RAWMATERIAL],VirtualCategoryTabel.[OTHER],VirtualCategoryTabel.[PACKINGMAT],VirtualCategoryTabel.[FIXEDASST],VirtualCategoryTabel.[MAKE],VirtualCategoryTabel.[KV],VirtualCategoryTabel.[FINISHFOODDESC],VirtualCategoryTabel.[RAWMATERIALDESC],VirtualCategoryTabel.[OTHERDESC],VirtualCategoryTabel.[PACKINGMATDESC],VirtualCategoryTabel.[FIXEDASSTDESC],VirtualCategoryTabel.[MAKEDESC],VirtualCategoryTabel.[KVDESC] ,TSPL_ITEM_MASTER.Item_Type,VirtualTableItemType.Name as Item_Type_Name,TSPL_INVENTORY_SOURCE_CODE.In_Category,TSPL_INVENTORY_SOURCE_CODE.Out_Category,TSPL_INVENTORY_SOURCE_CODE.Code,(case when ISNULL(InventroyMovement.Location_Code,'')='' then InventroyMovement.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) as PrimaryLocation, Structure_Desc,TSPL_ITEM_MASTER.RACK_NO  from ( "
            ...
            Else
	            ...
	            TSPL_MRN_DETAIL.MRN_Qty Stock_Qty,0 as FIFO_Cost,0 as LIFO_Cost,0 as Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 AS MilkSNFKG,0 as MilkFATQuintal,0 as MilkSNFQuintal,'' as SourceCode,'' as SourceName, '' as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_MRN_HEAD 
	             ...
	             where TSPL_MRN_HEAD.MRN_No not in (select Against_MRN from TSPL_SRN_HEAD) and Location IN (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") and convert(date,MRN_Date ,103) between '" + clsCommon.GetPrintDate(txtFromDate.Value) + "'  and  '" + clsCommon.GetPrintDate(txtToDate.Value) + "'   
                 and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and (VisualQCStatus <>2 or VisualQCStatusSecond<>2)  
	             union all
	              select 0 as Fat_Amt,0 as SNF_Amt,0 AS Fat_Rate,0 AS SNF_Rate ,'' as Trans_Id,'' as Trans_Type,'' as Source_Doc_No,SRN_Date as Punching_Date,'I' as InOut,TSPL_SRN_HEAD.Bill_To_Location,TSPL_SRN_DETAIL.Item_Code,TSPL_SRN_DETAIL.Unit_code UOM, 0 as MRP,TSPL_SRN_DETAIL.Unit_code Stock_UOM,TSPL_SRN_DETAIL.SRN_Qty Stock_Qty,0 as FIFO_Cost,0 as LIFO_Cost,0 as Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 AS MilkSNFKG,0 as MilkFATQuintal,0 as MilkSNFQuintal,'' as SourceCode,'' as SourceName, '' as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_SRN_HEAD 
	              left outer join TSPL_SRN_DETAIL on TSPL_SRN_DETAIL.SRN_No=TSPL_SRN_HEAD.SRN_No
                 ...
                 left outer join TSPL_GRN_HEAD on TSPL_GRN_HEAD.GRN_No=TSPL_MRN_HEAD.Against_GRN
				 left outer join TSPL_NIR_QC on TSPL_NIR_QC.mrn_no=TSPL_MRN_HEAD.mrn_no
	              left join (select Conversion_factor as cf,TSPL_ITEM_UOM_DETAIL.Item_code from TSPL_ITEM_UOM_DETAIL where UOM_code in (select UOM_Code  from TSPL_item_uom_detail where Item_Code = TSPL_ITEM_UOM_DETAIL.Item_code) And TSPL_ITEM_UOM_DETAIL.UOM_Code=(select Unit_Code  from TSPL_UNIT_MASTER where Unit_Desc=  '" + clsCommon.myCstr(cmbUnit.SelectedValue) + "') ) as ITEMDETAIL1 on ITEMDETAIL1.Item_code=TSPL_SRN_DETAIL.Item_Code where TSPL_SRN_HEAD.Status=0 and Location IN (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") and convert(date,SRN_Date ,103) between '" + clsCommon.GetPrintDate(txtFromDate.Value) + "'  and '" + clsCommon.GetPrintDate(txtToDate.Value) + "'  
                 and TSPL_GRN_HEAD.IsCancel=0 and TSPL_GRN_HEAD.Status=1 and TSPL_NIR_QC.qc_status=1  and (VisualQCStatus <>2 or VisualQCStatusSecond<>2)"
            End If
            qry += "   ) InventroyMovement 
                     left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=InventroyMovement.Item_Code
                     ...
                     left outer join TSPL_ADJUSTMENT_HEADER ON TSPL_ADJUSTMENT_HEADER.Adjustment_No=InventroyMovement.Source_Doc_No  "
            If clsCommon.myLen(cmbUnit.SelectedValue) > 0 Then
                qry += "  inner Join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=InventroyMovement.Item_Code And TSPL_ITEM_UOM_DETAIL.UOM_Code=(select Unit_Code  from TSPL_UNIT_MASTER where Unit_Desc= '" + clsCommon.myCstr(cmbUnit.SelectedValue) + "') "
            End If
            qry += "  left outer join (select Item_Code,max([FINISHFOOD]) as [FINISHFOOD],max([RAWMATERIAL]) as [RAWMATERIAL],max([OTHER]) as [OTHER],max([PACKINGMAT]) as [PACKINGMAT],max([FIXEDASST]) as [FIXEDASST],max([MAKE]) as [MAKE],max([KV]) as [KV],max([FINISHFOODDESC]) as [FINISHFOODDESC],max([RAWMATERIALDESC]) as [RAWMATERIALDESC],max([OTHERDESC]) as [OTHERDESC],max([PACKINGMATDESC]) as [PACKINGMATDESC],max([FIXEDASSTDESC]) as [FIXEDASSTDESC],max([MAKEDESC]) as [MAKEDESC],max([KVDESC]) as [KVDESC]  from (
                     select * from ( 
                     select TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code 
	 ...
	 union all
	   select 0 as Fat_Amt,0 as SNF_Amt,0 AS Fat_Rate,0 AS SNF_Rate ,'' as Trans_Id,'SRN' as Trans_Type,'' as Source_Doc_No,SRN_Date as Punching_Date,'I' as InOut,TSPL_SRN_HEAD.Bill_To_Location,TSPL_SRN_DETAIL.Item_Code,TSPL_SRN_DETAIL.Unit_code UOM, TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor as MRP ,ITEMDETAIL2.UOM_Code Stock_UOM, TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor Stock_Qty,(TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor)*(TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor) as FIFO_Cost,(TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor)*(TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor) as LIFO_Cost,(TSPL_SRN_DETAIL.Item_Cost/TSPL_ITEM_UOM_DETAIL.Conversion_Factor*ITEMDETAIL2.Conversion_Factor)*(TSPL_SRN_DETAIL.MRN_Qty*TSPL_ITEM_UOM_DETAIL.Conversion_Factor/ITEMDETAIL2.Conversion_Factor) as Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 AS MilkSNFKG,0 as MilkFATQuintal,0 as MilkSNFQuintal,'' as SourceCode,'' as SourceName, '' as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_SRN_HEAD 
	            ...
	            left  join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_SRN_DETAIL.Item_Code And  
	              ...
	              left join (select UOM_Code,Item_Code,Conversion_Factor from TSPL_ITEM_UOM_DETAIL where  Stocking_Unit='Y') as ITEMDETAIL2 on ITEMDETAIL2.Item_code=TSPL_SRN_DETAIL.Item_Code where TSPL_SRN_HEAD.Status=0 and Location IN (" & IIf(txtBillToLocation.arrValueMember IsNot Nothing, clsCommon.GetMulcallString(txtBillToLocation.arrValueMember), arrLoc) & ") and convert(date,SRN_Date ,103)  between '" + clsCommon.GetPrintDate(txtFromDate.Value) + "'  and '" + clsCommon.GetPrintDate(txtToDate.Value) + "'  
                   and TSPL_GRN_HEAD.IsCancel=0  and TSPL_GRN_HEAD.Status=1 and TSPL_NIR_QC.qc_status=1  and (VisualQCStatus <>2 or VisualQCStatusSecond<>2) "
            End If
            qry += ") InventroyMovement 
                     left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=InventroyMovement.Item_Code
