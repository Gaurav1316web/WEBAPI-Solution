...
Public Class clsERPFuncationality
    ...
    'Private Shared Function GetQryOFDOCPrefix(ByVal tran As SqlTransaction, ByVal dtDocDate As Date, ByVal strDocType As String, ByVal strTransType As String, ByVal strLocatinSegmentCode As String, ByVal IntFiscalYear As Integer, ByVal isTakingTopOne As Boolean) As String
    '    Dim qry As String = "select " + IIf(isTakingTopOne, " Top 1", "") + " Doc_Prfeix,Fin_Year,Next_Number,Separator,Is_Change_monthly,Curr_Month,Is_Change_Daily,Curr_Date,dontDisplayYearInSeries,Short_Fiscal_Year,MinSizeofSeries,Year_Separator from TSPL_DOCPREFIX_MASTER  WITH ( HOLDLOCK ) "
    '    Dim whr As String = "Doc_Type='" + strDocType + "' and  isnull(Doc_Trans_Type,'')='" + strTransType + "' and isnull(Location_Code,'')='" + strLocatinSegmentCode + "' and Fin_Year='" + clsCommon.myCstr(IntFiscalYear) + "'"
    '    Dim Order As String = ""
    '    If isTakingTopOne Then
    '        Order = " order by case when Is_Change_Daily=1 then  CONVERT(varchar, Curr_Date,112) else  CONVERT(varchar, Curr_Month ) end desc"
    '    Else
...
"        and con.parent_object_id = col.object_id" + Environment.NewLine +
    ...
    'sanjay
    Public Shared Function GetReportID(ByVal strForm_ID As String, ByVal ddlReportType As RadDropDownList) As String
        Dim StrReportId As String = ""
        Try
            StrReportId = strForm_ID
            'Dim txtArray As String() = ddlReportType.Text.Split(" ")
            'Dim str As String = ""

            'For Each s As String In txtArray
            '    str += s.Substring(0, 1)
            'Next

            StrReportId = StrReportId + clsCommon.myCstr(ddlReportType.Text)
    ...
    Public Shared Function GetReportID(ByVal strForm_ID As String, ByVal StrReportType As String) As String
        ...
        Try
            StrReportId = strForm_ID
            'Dim txtArray As String() = StrReportType.Split(" ")
            'Dim str As String = ""
    ...
    Public Shared Function GetForeignKey_Name(ByVal strTableName As String, ByVal strColumnName As String) As String
        ...
        Try
            Dim qry As String = "select fk_constraint_name from (" + Environment.NewLine + _
"select " + Environment.NewLine + _
"fk_tab.name as TabName," + Environment.NewLine + _
"schema_name(fk_tab.schema_id) + '.' + fk_tab.name as foreign_table," + Environment.NewLine + _
"                '>-' as rel," + Environment.NewLine + _
"    schema_name(pk_tab.schema_id) + '.' + pk_tab.name as primary_table," + Environment.NewLine + _
"    fk_cols.constraint_column_id as no, " + Environment.NewLine + _
"    fk_col.name as fk_column_name," + Environment.NewLine + _
"                ' = ' as [join]," + Environment.NewLine + _
"    pk_col.name as pk_column_name," + Environment.NewLine + _
"    fk.name as fk_constraint_name" + Environment.NewLine + _
"from sys.foreign_keys fk" + Environment.NewLine + _
"    inner join sys.tables fk_tab" + Environment.NewLine + _
"        on fk_tab.object_id = fk.parent_object_id" + Environment.NewLine + _
"    inner join sys.tables pk_tab" + Environment.NewLine + _
"        on pk_tab.object_id = fk.referenced_object_id" + Environment.NewLine + _
"    inner join sys.foreign_key_columns fk_cols" + Environment.NewLine + _
"        on fk_cols.constraint_object_id = fk.object_id" + Environment.NewLine + _
"    inner join sys.columns fk_col" + Environment.NewLine + _
"        on fk_col.column_id = fk_cols.parent_column_id" + Environment.NewLine + _
"        and fk_col.object_id = fk_tab.object_id" + Environment.NewLine + _
"    inner join sys.columns pk_col" + Environment.NewLine + _
"        on pk_col.column_id = fk_cols.referenced_column_id" + Environment.NewLine + _
"        and pk_col.object_id = pk_tab.object_id" + Environment.NewLine + _
"	)xx where TabName='" + strTableName + "' and fk_column_name='" + strColumnName + "'"
    ...
    Public Shared Function ReportingMailIdandPhone(ByVal user_Code As String, ByRef arrMobileNo As List(Of String), ByVal trans As SqlTransaction) As List(Of String)
        ...
        Dim Sql As String = ""
        Try
            Sql = "With Hierarchy(ChildId, ParentId, Parents)" & _
             " AS(SELECT User_Code, Level4_Code, CAST('' AS VARCHAR(MAX))" & _
                  " FROM TSPL_USER_MASTER AS FirtGeneration" & _
                  " WHERE Level4_Code IS NULL OR Level4_Code='' " & _
              " UNION ALL " & _
              " SELECT NextGeneration.User_Code, Parent.ChildId," & _
              " CAST(CASE WHEN Parent.Parents = ''" & _
               " THEN(CAST(''''+NextGeneration.Level4_Code+'''' AS VARCHAR(MAX)))" & _
               " ELSE(Parent.Parents + ',''' + CAST(NextGeneration.Level4_Code+'''' AS VARCHAR(MAX)))" & _
            " END AS VARCHAR(MAX))" & _
               " FROM TSPL_USER_MASTER AS NextGeneration" & _
               " INNER JOIN Hierarchy AS Parent ON NextGeneration.Level4_Code = Parent.ChildId  " & _
                    ")" & _
                    " SELECT Parents FROM Hierarchy WHERE ChildId='" & user_Code & "'" & _
