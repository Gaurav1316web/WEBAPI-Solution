...
Public Class clsPostCreateTable
    ...
    '        End Try
    '    End If
    '    ''richa agarwal for deleting unique key from qc table of bulk sale for pavitra BM00000007898
    '    If clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select Description from TSPL_FIXED_PARAMETER WHERE CODE='" & clsFixedParameterCode.BulkQCTableHavingUniqueKey & "' AND TYPE='" & clsFixedParameterType.BulkQCTableHavingUniqueKey & "'")), "0") = CompairStringResult.Equal Then
    '        trans = clsDBFuncationality.GetTransactin()
    '        Try
    '            qry = "select CONSTRAINT_NAME  from INFORMATION_SCHEMA.TABLE_CONSTRAINTS where CONSTRAINT_TYPE='UNIQUE' and TABLE_NAME ='TSPL_Quality_Check_BulkSale'"
    '            dt = clsDBFuncationality.GetDataTable(qry, trans)

    '            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    '                qry = "alter table TSPL_Quality_Check_BulkSale drop constraint [" + clsCommon.myCstr(dt.Rows(0)("CONSTRAINT_NAME")) + "]"
    '                clsDBFuncationality.ExecuteNonQuery(qry, trans)
    '            End If
    '            trans.Commit()
    '        Catch ex As Exception
    ...
    '            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    '            Else
    '                qry = "alter table TSPL_Quality_Check_BulkSale add Unique(LoadingTanker_No) "
    '                clsDBFuncationality.ExecuteNonQuery(qry, trans)
    '            End If
    '            trans.Commit()
    '        Catch ex As Exception
    ...
    '        If (clsCommon.CompairString("5.1.6.70", exeVersion) = CompairStringResult.Greater Or clsCommon.CompairString("5.1.6.70", exeVersion) = CompairStringResult.Equal) Then
    '            trans = clsDBFuncationality.GetTransactin()
    '            Try
    '                qry = " delete from tspl_bulk_price_detail_item_wise where Price_Code  in ( select TSPL_BULK_PRICE_DETAIL_ITEM_WISE.Price_Code from TSPL_BULK_PRICE_DETAIL_ITEM_WISE where  TSPL_BULK_PRICE_DETAIL_ITEM_WISE.Price_Code not in (SELECT TSPL_Bulk_Price_MASTER.Price_code FROM TSPL_Bulk_Price_MASTER) ) "
    '                clsDBFuncationality.ExecuteNonQuery(qry, trans)
    '                trans.Commit()
    '            Catch ex As Exception
    '                trans.Rollback()
    ...
    '    ''richa 14 Dec,2018
    '    If (clsCommon.CompairString("5.1.6.92", exeVersion) = CompairStringResult.Greater Or clsCommon.CompairString("5.1.6.92", exeVersion) = CompairStringResult.Equal) Then
    '        trans = clsDBFuncationality.GetTransactin()
    '        Try
    '            qry = "select COUNT(*) from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME='TSPL_ROUTE_MASTER' and COLUMN_NAME='Route_Time'"
    '            check = clsDBFuncationality.getSingleValue(qry, trans)
    '            check = CheckColumnExist("TSPL_ROUTE_MASTER", "Route_Time", DBDataType.time_Type, 0, 0, trans)
    '            If check > 0 Then
    '                qry = "ALTER TABLE TSPL_ROUTE_MASTER alter COLUMN Route_Time datetime null "
    '                clsDBFuncationality.ExecuteNonQuery(qry, trans)
    '            End If
    ...
    '    ''richa 31 July,2019
    '    If (clsCommon.CompairString("5.1.8.60", exeVersion) = CompairStringResult.Greater Or clsCommon.CompairString("5.1.8.60", exeVersion) = CompairStringResult.Equal) Then
    '        trans = clsDBFuncationality.GetTransactin()
    '        Try
    '            qry = "update tspl_fixed_parameter set Specification ='0:Off, 1:On; Bulk Procurement Chamber wise' where type='Gate Entry tanker From Master' and Code ='Gate Entry tanker From Master'"
    '            clsDBFuncationality.ExecuteNonQuery(qry, trans)

    '            qry = "update tspl_fixed_parameter set Specification ='1-ON;0-OFF Applicable for Route Wise Vendor in Bulk Procurement' where type='Allow Tanker Based on Vendor of GE' and Code ='Allow Tanker Based on Vendor of GE'"
    '            clsDBFuncationality.ExecuteNonQuery(qry, trans)

    '            qry = "update tspl_fixed_parameter set Specification ='Do Not Show JobWork Type Tanker in Tanker status Report' where type='Apply Not Show JobWork Type Tanker' and Code ='Apply Not Show JobWork Type Tanker'"
    '            clsDBFuncationality.ExecuteNonQuery(qry, trans)

    '            qry = "update tspl_fixed_parameter set Specification ='Option to enter Manual Transfer Rate On Tanker Dispatch Chamber wise' where type='ApplyMaTransferRateOnMultilpeChamberTankerDispatch' and Code ='ApplyMaTransferRateOnMultilpeChamberTankerDispatch'"
    '            clsDBFuncationality.ExecuteNonQuery(qry, trans)

    '            qry = "update tspl_fixed_parameter set Specification ='Tanker of JobWork (Bulk Procurement) will reflect on Gate Out After General Weighment' where type='Gate Out Tanker No After General Weighment' and Code ='Gate Out Tanker No After General Weighment'"
    '            clsDBFuncationality.ExecuteNonQuery(qry, trans)
