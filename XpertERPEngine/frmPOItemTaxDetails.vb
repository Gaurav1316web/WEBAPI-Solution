...
#Region "Varibales"
    ...
    Public colTaxOnBaseAmt As String = "colTaxOnBaseAmt"
    ...
    Const colIsRateChange As String = "colIsRateChange"

    Public PurchaseModulePickFixTaxRate As Boolean = False
    Public IsTaxableItem As Boolean = False
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        repoIsTaxable = New GridViewCheckBoxColumn()
        repoIsTaxable.HeaderText = "Tax On Base Amount"
        repoIsTaxable.Name = colTaxOnBaseAmt
        repoIsTaxable.Width = 80
        repoIsTaxable.ReadOnly = True
        repoIsTaxable.IsVisible = False
        repoIsTaxable.TextAlignment = System.Drawing.ContentAlignment.MiddleCenter
        ...
        repoIsTaxable = New GridViewCheckBoxColumn()
        repoIsTaxable.HeaderText = "Rate Change"
        repoIsTaxable.Name = colIsRateChange
        repoIsTaxable.Width = 80
        repoIsTaxable.ReadOnly = True
        repoIsTaxable.IsVisible = False
        repoIsTaxable.TextAlignment = System.Drawing.ContentAlignment.MiddleCenter
    ...
    Sub LoadData()
        If ArrIn IsNot Nothing AndAlso ArrIn.Count > 0 Then
            For Each obj As clsTempItemTaxDetails In ArrIn
                ...
                gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxOnBaseAmt).Value = obj.TaxOnBaseAmount
                ...
                gv1.Rows(gv1.Rows.Count - 1).Cells(colIsRateChange).Value = clsTaxMaster.GetTaxRateChangable(obj.AuthorityCode, Nothing)
            Next
        End If
    End Sub
    ...
    Private Sub gv1_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gv1.DoubleClick
        Try
            If clsCommon.CompairString(gv1.CurrentCell.ColumnInfo.Name, colTaxRate) = CompairStringResult.Equal Then
                If Not PurchaseModulePickFixTaxRate OrElse Not IsTaxableItem Or clsCommon.myCBool(gv1.CurrentRow.Cells(colIsRateChange).Value) Then
                    Dim isCancelButtonClicked As Boolean = False
                    Dim dblNewRate As Double = 0
                    If clsCommon.CompairString(strTaxType, "T") = CompairStringResult.Equal Then
                        dblNewRate = clsLocationWiseTax.FinderForTaxRateForTransfer(isCancelButtonClicked, strTransLocation, strTaxGroup, clsCommon.myCstr(gv1.CurrentRow.Cells(colTaxAutCode).Value), strVendorCustomerCode, strTaxType)
                    Else
                        dblNewRate = clsLocationWiseTax.FinderForTaxRate(isCancelButtonClicked, strTransLocation, strTaxGroup, clsCommon.myCstr(gv1.CurrentRow.Cells(colTaxAutCode).Value), strVendorCustomerCode, strTaxType, Without_State_Condition, IslocationSegment)
                    End If
                    If Not isCancelButtonClicked Then
    ...
    Private Sub UpdateCurrentRow()
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            ...
            Dim IsTaxOnBaseAmt As Boolean = clsCommon.myCBool(gv1.Rows(ii).Cells(colTaxOnBaseAmt).Value)
            ...
            Else
                ...
                'If IsTaxable Then ''Becuase it is in Purchase Invoice 
    ...
    Private Sub btnOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnOk.Click
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            ...
            obj.TaxOnBaseAmount = clsCommon.myCBool(gv1.Rows(ii).Cells(colTaxOnBaseAmt).Value)
