...
Public Class clsProspectHead
#Region "Variables"
    Public Cust_PO_No As String = Nothing
    Public Price_Group_Code As String = Nothing

    Public State As String = Nothing
    Public CustID As String = Nothing
    Public Existing_Cust_Active As String = Nothing
    Public Zone As String = Nothing
    Public Location As String = Nothing
    Public Prospect_Category As String = Nothing
    ...
    Public QueryRec As String = Nothing
    Public Query_Value As String = Nothing
    Public Email As String = Nothing
    Public PhoneNo As String = Nothing
    Public Internal_Desc As String = Nothing
    Public Category_Desc As String = Nothing
    Public Item_Category As String = Nothing
    Public Internal_Chk As String = Nothing
    Public Cust_Source As String = Nothing
    Public Source_Value As String = Nothing
    Public Close_Status As String = Nothing
    Public Close_Query As String = Nothing
    Public Close_Query_Value As String = Nothing
    Public Forward_Status As String = Nothing
    Public Forward_User_Query As String = Nothing
    Public Forward_Vendor As String = Nothing
    Public Forward_Query_Active As String = Nothing
    Public lblForward_User_Query As String = Nothing
    Public lblForward_Vendor As String = Nothing
    ...
    Public Arr As List(Of clsProspectDetail) = Nothing
    ...
    Public Comm_Type As String = Nothing
    Public ArrLoc As List(Of clsProspectLocation) = Nothing
    Public ArrComm As List(Of clsProspectCommunication) = Nothing
#End Region

    ''Note Very Important If any change mad in SO Head or SO Detail table allso update it's History table.

    Public Function SaveData(ByVal obj As clsProspectHead, ByVal isNewEntry As Boolean, ByVal isMakeAbandomentNo As Boolean) As Boolean
        ...
        Try
            ...
            Dim qry As String = "delete from TSPL_PROSPECT_DETAIL where Document_Code='" + obj.Document_Code + "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_PROSPECT_COMMUNICATION where Document_Code='" + obj.Document_Code + "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
            Dim strDocNo As String = ""
            If isNewEntry Then
                obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.ProspectDetail, clsDocType.ProspectDetail, obj.Bill_To_Location)
            ...
            clsCommon.AddColumnsForChange(coll, "QueryRecBy", obj.QueryRec)
            clsCommon.AddColumnsForChange(coll, "Query_Value", obj.Query_Value)
            clsCommon.AddColumnsForChange(coll, "Prospect_Category", obj.Prospect_Category)
            clsCommon.AddColumnsForChange(coll, "Email", obj.Email)
            clsCommon.AddColumnsForChange(coll, "PhoneNo", obj.PhoneNo)
            clsCommon.AddColumnsForChange(coll, "Internal_Desc", obj.Internal_Desc)
            clsCommon.AddColumnsForChange(coll, "Internal_Check", obj.Internal_Chk)
            clsCommon.AddColumnsForChange(coll, "Item_Category", obj.Item_Category)
            clsCommon.AddColumnsForChange(coll, "Category_Desc", obj.Category_Desc)
            clsCommon.AddColumnsForChange(coll, "Cust_Source", obj.Cust_Source)
            clsCommon.AddColumnsForChange(coll, "Source_Value", obj.Source_Value)
            clsCommon.AddColumnsForChange(coll, "Close_Status", obj.Close_Status)
            clsCommon.AddColumnsForChange(coll, "Close_Query", obj.Close_Query)
            clsCommon.AddColumnsForChange(coll, "Close_Query_Value", obj.Close_Query_Value)
            clsCommon.AddColumnsForChange(coll, "Forward_Status", obj.Forward_Status)
            clsCommon.AddColumnsForChange(coll, "Forward_User_Query", obj.Forward_User_Query)
            clsCommon.AddColumnsForChange(coll, "Forward_Vendor", obj.Forward_Vendor)
            clsCommon.AddColumnsForChange(coll, "Forward_Query_Active", obj.Forward_Query_Active)
            clsCommon.AddColumnsForChange(coll, "CustID", obj.CustID)
            clsCommon.AddColumnsForChange(coll, "Exisiting_Cust_Active", obj.Existing_Cust_Active)
            ...
            clsCommon.AddColumnsForChange(coll, "Comm_Type", obj.Comm_Type)
            ...
            If isNewEntry Then
                ...
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_PROSPECT_HEAD", OMInsertOrUpdate.Insert, "", trans)
            Else
                If isMakeAbandomentNo Then
                    obj.Abandonment_No = obj.Abandonment_No + 1
                    clsCommon.AddColumnsForChange(coll, "Abandonment_No", obj.Abandonment_No)
                End If
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_PROSPECT_HEAD", OMInsertOrUpdate.Update, "TSPL_PROSPECT_HEAD.Document_Code='" + obj.Document_Code + "'", trans)

            End If
            isSaved = isSaved AndAlso clsProspectDetail.SaveData(obj.Document_Code, Arr, trans)
            isSaved = isSaved AndAlso clsProspectCommunication.SaveData(obj.Document_Code, ArrComm, trans)
            isSaved = isSaved AndAlso clsCustomFieldValues.SaveData(obj.Form_ID, obj.Document_Code, obj.arrCustomFields, trans)
            isSaved = isSaved AndAlso clsProspectLocation.SaveData(obj.Document_Code, isNewEntry, ArrLoc, trans)
            isSaved = isSaved AndAlso clsApprovalScreen.SaveApprovalAtTransLevel(obj.Form_ID, "Document_Code", obj.Document_Code, "TSPL_PROSPECT_HEAD", trans)
            If isSaved Then
                trans.Commit()
            End If
        ...
        Return isSaved
    End Function
    Public Shared Function CheckCommTypeSeq(ByVal Doc_Code As String, Optional ByVal trans As SqlTransaction = Nothing) As Integer
        Dim qry As String = "select coalesce(max(comm_type_Seq),1) as seq from TSPL_PROSPECT_COMMUNICATION where Document_Code='" & Doc_Code & "' "
        Return clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry, trans))
    End Function

    Public Shared Function GetCommTypeTable() As DataTable
        ...
        Return DT
    End Function
    Public Shared Function GetQueryreceivedbyTable() As DataTable
        Dim DT As DataTable = New DataTable
        DT.Columns.Add("Code", GetType(String))
        DT.Columns.Add("Name", GetType(String))
        ...
        DR = DT.NewRow()
        DR("Name") = "Online business portal"
        DR("Code") = "Online business portal"
        DT.Rows.Add(DR)
    ...
    Public Shared Function GetProspectCategoryTable() As DataTable
        Dim DT As DataTable = New DataTable
        DT.Columns.Add("Code", GetType(String))
        DT.Columns.Add("Name", GetType(String))
    ...
    Public Shared Function GetCustomerSourceTable() As DataTable
        Dim DT As DataTable = New DataTable
        DT.Columns.Add("Code", GetType(String))
        DT.Columns.Add("Name", GetType(String))

        Dim DR As DataRow = DT.NewRow()
        DR("Name") = "Existing customer"
        DR("Code") = "Existing customer"
        DT.Rows.Add(DR)
        ...
        DR = DT.NewRow()
        DR("Name") = "Employee ref"
        DR("Code") = "Employee ref"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "Manufacturer referencer"
        DR("Code") = "Manufacturer reference"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "Other reference"
        DR("Code") = "Other reference"
        DT.Rows.Add(DR)
    ...
    Public Shared Function GetCloseQueryTable() As DataTable
        Dim DT As DataTable = New DataTable
        DT.Columns.Add("Code", GetType(String))
        DT.Columns.Add("Name", GetType(String))
        ...
        DR = DT.NewRow()
        DR("Name") = "Price variation"
        DR("Code") = "Price variation"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "Non avaliabliity of product"
        DR("Code") = "Non avaliabliity of product"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "Out of stock"
        DR("Code") = "Out of stock"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "followup failure"
        DR("Code") = "followup failure"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "No respond from customer"
        DR("Code") = "No respond from customer"
        DT.Rows.Add(DR)

        DR = DT.NewRow()
        DR("Name") = "Not a genuine customer"
        DR("Code") = "Not a genuine customer"
        DT.Rows.Add(DR)
        ...
        Try
            ...
            For Each dr As DataRow In dt.Rows
                ...
                If clsCommon.CompairString(dr("Criteria"), "Credit days") = CompairStringResult.Equal Then
                    qry = "Select COUNT(*) from TSPL_PROSPECT_HEAD WHERE Status<>1 AND Customer_Code='" + strCustCode + "' AND CONVERT(Date, Due_Date,103)<'" + clsCommon.GetPrintDate(strDate, "dd/MMM/yyyy") + "'"
                ...
                ElseIf clsCommon.CompairString(dr("Criteria"), "Credit Amount") = CompairStringResult.Equal Then
                    qry = "Select SUM(Total_Amt) from TSPL_PROSPECT_HEAD WHERE Status<>1 AND Customer_Code='" + strCustCode + "'"
    ...
    Public Shared Function GetData(ByVal strDocumentNo As String, ByVal NavType As NavigatorType) As clsProspectHead
        Return GetData(strDocumentNo, NavType, Nothing)
    End Function

    Public Shared Function GetData(ByVal strPONo As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction) As clsProspectHead
        Dim obj As clsProspectHead = Nothing
        Dim qry As String = "SELECT TSPL_PROSPECT_HEAD.Cust_PO_No,TSPL_PROSPECT_HEAD.price_group_code, TSPL_PROSPECT_HEAD.Route_No,TSPL_PROSPECT_HEAD.Route_Desc,TSPL_PROSPECT_HEAD.Price_Code,TSPL_PROSPECT_HEAD.Document_Code,TSPL_PROSPECT_HEAD.SalesOrder_Type ,TSPL_PROSPECT_HEAD.Document_Date, " & _
        " TSPL_PROSPECT_HEAD.Customer_Name,TSPL_PROSPECT_HEAD.QueryRecBy,TSPL_PROSPECT_HEAD.Close_Status,TSPL_PROSPECT_HEAD.exisiting_cust_active,TSPL_PROSPECT_HEAD.CustID,TSPL_VENDOR_MASTER.Vendor_Name,TSPL_USER_MASTER.User_Name,TSPL_PROSPECT_HEAD.Forward_Status,TSPL_PROSPECT_HEAD.Forward_Query_Active,TSPL_PROSPECT_HEAD.Forward_User_Query,TSPL_PROSPECT_HEAD.Forward_Vendor,TSPL_PROSPECT_HEAD.Close_Query, TSPL_PROSPECT_HEAD.Close_Query_Value, TSPL_PROSPECT_HEAD.Query_Value, TSPL_PROSPECT_HEAD.Item_Category, TSPL_PROSPECT_HEAD.Source_Value, TSPL_PROSPECT_HEAD.Cust_Source, TSPL_PROSPECT_HEAD.Category_Desc, TSPL_PROSPECT_HEAD.Internal_Check, TSPL_PROSPECT_HEAD.Internal_Desc, TSPL_PROSPECT_HEAD.Prospect_Category, TSPL_PROSPECT_HEAD.Email, TSPL_PROSPECT_HEAD.PhoneNo, TSPL_PROSPECT_HEAD.Status, " & _
        " TSPL_PROSPECT_HEAD.On_Hold,TSPL_PROSPECT_HEAD.Ref_No,TSPL_PROSPECT_HEAD.Description,TSPL_PROSPECT_HEAD.Remarks, " & _
        " TSPL_PROSPECT_HEAD.Tax_Group,TSPL_PROSPECT_HEAD.Bill_To_Location,TSPL_PROSPECT_HEAD.Ship_To_Location, " & _
        " TSPL_PROSPECT_HEAD.TAX1,TSPL_PROSPECT_HEAD.TAX1_Rate,TSPL_PROSPECT_HEAD.TAX1_Amt,TSPL_PROSPECT_HEAD.TAX1_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX2,TSPL_PROSPECT_HEAD.TAX2_Rate,TSPL_PROSPECT_HEAD.TAX2_Amt,TSPL_PROSPECT_HEAD.TAX2_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX3,TSPL_PROSPECT_HEAD.TAX3_Rate,TSPL_PROSPECT_HEAD.TAX3_Amt,TSPL_PROSPECT_HEAD.TAX3_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX4,TSPL_PROSPECT_HEAD.TAX4_Rate,TSPL_PROSPECT_HEAD.TAX4_Amt,TSPL_PROSPECT_HEAD.TAX4_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX5,TSPL_PROSPECT_HEAD.TAX5_Rate,TSPL_PROSPECT_HEAD.TAX5_Amt,TSPL_PROSPECT_HEAD.TAX5_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX6,TSPL_PROSPECT_HEAD.TAX6_Rate,TSPL_PROSPECT_HEAD.TAX6_Amt,TSPL_PROSPECT_HEAD.TAX6_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX7,TSPL_PROSPECT_HEAD.TAX7_Rate,TSPL_PROSPECT_HEAD.TAX7_Amt,TSPL_PROSPECT_HEAD.TAX7_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX8,TSPL_PROSPECT_HEAD.TAX8_Rate,TSPL_PROSPECT_HEAD.TAX8_Amt,TSPL_PROSPECT_HEAD.TAX8_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX9,TSPL_PROSPECT_HEAD.TAX9_Rate,TSPL_PROSPECT_HEAD.TAX9_Amt,TSPL_PROSPECT_HEAD.TAX9_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.TAX10,TSPL_PROSPECT_HEAD.TAX10_Rate,TSPL_PROSPECT_HEAD.TAX10_Amt,TSPL_PROSPECT_HEAD.TAX10_Base_Amt, " & _
        " TSPL_PROSPECT_HEAD.Discount_Base,TSPL_PROSPECT_HEAD.Discount_Amt,TSPL_PROSPECT_HEAD.Amount_Less_Discount, " & _
        " TSPL_PROSPECT_HEAD.Total_Tax_Amt,TSPL_PROSPECT_HEAD.Total_Amt,TSPL_PROSPECT_HEAD.Mode_Of_Transport," & _
        " TSPL_PROSPECT_HEAD.Comments,TSPL_PROSPECT_HEAD.Comp_Code,TSPL_PROSPECT_HEAD.Terms_Code,TSPL_PROSPECT_HEAD.Due_Date , " & _
        " TSPL_LOCATION_MASTER.Location_Desc as BillToLocationName,TSPL_SHIP_TO_LOCATION.Ship_To_Desc as ShipToLocationName, " & _
        " TSPL_TAX_GROUP_MASTER.Tax_Group_Desc as TaxGroupName,TSPL_TERMS_MASTER.Terms_Desc as TermsName,TSPL_PROSPECT_HEAD.Posting_Date, " & _
        " TSPL_PROSPECT_HEAD.Delivery_date,TSPL_PROSPECT_HEAD.Dept,TSPL_PROSPECT_HEAD.Dept_Desc,TSPL_PROSPECT_HEAD.Item_Type, " & _
        " TSPL_PROSPECT_HEAD.Modify_By,TSPL_PROSPECT_HEAD.Modify_Date,TSPL_PROSPECT_HEAD.Created_By," & _
        " TSPL_PROSPECT_HEAD.Created_Date,TSPL_PROSPECT_HEAD.Abandonment_No,TSPL_PROSPECT_HEAD.Against_Quotation_No, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code1,TSPL_PROSPECT_HEAD.Add_Charge_Name1,TSPL_PROSPECT_HEAD.Add_Charge_Amt1, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code2,TSPL_PROSPECT_HEAD.Add_Charge_Name2,TSPL_PROSPECT_HEAD.Add_Charge_Amt2, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code3,TSPL_PROSPECT_HEAD.Add_Charge_Name3,TSPL_PROSPECT_HEAD.Add_Charge_Amt3, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code4,TSPL_PROSPECT_HEAD.Add_Charge_Name4,TSPL_PROSPECT_HEAD.Add_Charge_Amt4, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code5,TSPL_PROSPECT_HEAD.Add_Charge_Name5,TSPL_PROSPECT_HEAD.Add_Charge_Amt5, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code6,TSPL_PROSPECT_HEAD.Add_Charge_Name6,TSPL_PROSPECT_HEAD.Add_Charge_Amt6, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code7,TSPL_PROSPECT_HEAD.Add_Charge_Name7,TSPL_PROSPECT_HEAD.Add_Charge_Amt7, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code8,TSPL_PROSPECT_HEAD.Add_Charge_Name8,TSPL_PROSPECT_HEAD.Add_Charge_Amt8, " & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code9 ,TSPL_PROSPECT_HEAD.Add_Charge_Name9,TSPL_PROSPECT_HEAD.Add_Charge_Amt9 ," & _
        " TSPL_PROSPECT_HEAD.Add_Charge_Code10 ,TSPL_PROSPECT_HEAD.Add_Charge_Name10,TSPL_PROSPECT_HEAD.Add_Charge_Amt10, " & _
        " TSPL_PROSPECT_HEAD.Total_Add_Charge,TSPL_PROSPECT_HEAD.Salesman_Code,TSPL_PROSPECT_HEAD.Salesman_Name, " & _
        " TSPL_PROSPECT_HEAD.CURRENCY_CODE,TSPL_PROSPECT_HEAD.CONVRATE,TSPL_PROSPECT_HEAD.APPLICABLEFROM ,TSPL_PROSPECT_HEAD.PROJECT_ID,TSPL_PROSPECT_HEAD.Approvel_Required,TSPL_PROSPECT_HEAD.Is_Approved" & _
        " ,TSPL_PROSPECT_HEAD.HeadDisc_Per,TSPL_PROSPECT_HEAD.HeadDisc_Amt,TSPL_PROSPECT_HEAD.TotCashDiscAmt,TSPL_PROSPECT_HEAD.Comm_Type,TSPL_Prospect_Location.Address,TSPL_Prospect_Location.Location,TSPL_Prospect_Location.State_Code,TSPL_Prospect_Location.Zone_Code FROM TSPL_PROSPECT_HEAD left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=TSPL_PROSPECT_HEAD.Bill_To_Location " & _
        " left outer join TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code=TSPL_PROSPECT_HEAD.Ship_To_Location left outer join  " & _
        " TSPL_TAX_GROUP_MASTER on TSPL_TAX_GROUP_MASTER.Tax_Group_Code= TSPL_PROSPECT_HEAD.Tax_Group left " & _
        " outer join TSPL_TERMS_MASTER on TSPL_TERMS_MASTER.Terms_Code=TSPL_PROSPECT_HEAD.Terms_Code left " & _
        " outer join TSPL_USER_MASTER ON TSPL_USER_MASTER.User_Code=TSPL_PROSPECT_HEAD.Forward_User_Query LEFT " & _
        " OUTER JOIN TSPL_VENDOR_MASTER ON TSPL_VENDOR_MASTER.Vendor_Code=TSPL_PROSPECT_HEAD.Forward_Vendor left " & _
        " outer join TSPL_PROSPECT_LOCATION on TSPL_PROSPECT_LOCATION.Document_Code=TSPL_PROSPECT_HEAD.Document_Code  " & _
        " where 2=2"
        '" left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_PROSPECT_HEAD.Customer_Code " & _
        ...
        ElseIf clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            ...
            '    whrClas = " AND TSPL_PROSPECT_HEAD.Customer_Code in (" + strwherecls + ")"
        End If
        '-----------------------------------------------------
        Select Case NavType
            Case NavigatorType.First
                qry += " and TSPL_PROSPECT_HEAD.Document_Code = (select MIN(Document_Code) from TSPL_PROSPECT_HEAD where 1=1 " + whrClas + ")"
            Case NavigatorType.Last
                qry += " and TSPL_PROSPECT_HEAD.Document_Code = (select Max(Document_Code) from TSPL_PROSPECT_HEAD where 1=1 " + whrClas + ")"
            Case NavigatorType.Next
                qry += " and TSPL_PROSPECT_HEAD.Document_Code = (select Min(Document_Code) from TSPL_PROSPECT_HEAD where Document_Code>'" + strPONo + "' " + whrClas + ")"
            Case NavigatorType.Previous
                qry += " and TSPL_PROSPECT_HEAD.Document_Code = (select Max(Document_Code) from TSPL_PROSPECT_HEAD where Document_Code<'" + strPONo + "' " + whrClas + ")"
            Case NavigatorType.Current
                qry += " and TSPL_PROSPECT_HEAD.Document_Code = '" + strPONo + "'"
        End Select
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)

        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            obj = New clsProspectHead()
            ...
            obj.QueryRec = clsCommon.myCstr(dt.Rows(0)("QueryRecBy"))
            obj.Query_Value = clsCommon.myCstr(dt.Rows(0)("Query_Value"))
            obj.State = clsCommon.myCstr(dt.Rows(0)("State_Code"))
            obj.Location = clsCommon.myCstr(dt.Rows(0)("Location"))
            obj.Address = clsCommon.myCstr(dt.Rows(0)("Address"))
            obj.Zone = clsCommon.myCstr(dt.Rows(0)("Zone_Code"))
            obj.Prospect_Category = clsCommon.myCstr(dt.Rows(0)("Prospect_Category"))
            obj.Email = clsCommon.myCstr(dt.Rows(0)("Email"))
            obj.PhoneNo = clsCommon.myCstr(dt.Rows(0)("PhoneNo"))
            obj.CustID = clsCommon.myCstr(dt.Rows(0)("CustID"))
            obj.Existing_Cust_Active = clsCommon.myCdbl(dt.Rows(0)("exisiting_cust_active"))
            obj.Internal_Desc = clsCommon.myCstr(dt.Rows(0)("Internal_Desc"))
            obj.Internal_Chk = clsCommon.myCstr(dt.Rows(0)("Internal_Check"))
            obj.Item_Category = clsCommon.myCstr(dt.Rows(0)("Item_Category"))
            obj.Category_Desc = clsCommon.myCstr(dt.Rows(0)("Category_Desc"))
            obj.Cust_Source = clsCommon.myCstr(dt.Rows(0)("Cust_Source"))
            obj.Source_Value = clsCommon.myCstr(dt.Rows(0)("Source_Value"))
            obj.Close_Status = clsCommon.myCstr(dt.Rows(0)("Close_Status"))
            obj.Close_Query = clsCommon.myCstr(dt.Rows(0)("Close_Query"))
            obj.Close_Query_Value = clsCommon.myCstr(dt.Rows(0)("Close_Query_Value"))
            obj.Forward_Status = clsCommon.myCstr(dt.Rows(0)("Forward_Status"))
            obj.Forward_Query_Active = clsCommon.myCstr(dt.Rows(0)("Forward_Query_Active"))
            obj.Forward_User_Query = clsCommon.myCstr(dt.Rows(0)("Forward_User_Query"))
            obj.Forward_Vendor = clsCommon.myCstr(dt.Rows(0)("Forward_Vendor"))
            obj.lblForward_Vendor = clsCommon.myCstr(dt.Rows(0)("Vendor_Name"))
            obj.lblForward_User_Query = clsCommon.myCstr(dt.Rows(0)("User_Name"))
            ...
            obj.Comm_Type = clsCommon.myCstr(dt.Rows(0)("Comm_Type"))
            qry = "SELECT  TSPL_PROSPECT_DETAIL.Document_Code,TSPL_PROSPECT_DETAIL.Line_No,TSPL_PROSPECT_DETAIL.Status, " & _
            "TSPL_PROSPECT_DETAIL.Row_Type,TSPL_PROSPECT_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_PROSPECT_DETAIL.Qty, " & _
            "TSPL_PROSPECT_DETAIL.Balance_Qty,TSPL_PROSPECT_DETAIL.Unit_code,TSPL_PROSPECT_DETAIL.Cost, " & _
            "TSPL_PROSPECT_DETAIL.Location,TSPL_PROSPECT_DETAIL.Item_Cost,TSPL_PROSPECT_DETAIL.TAX1, " & _
            "TSPL_PROSPECT_DETAIL.TAX1_Rate,TSPL_PROSPECT_DETAIL.TAX1_Amt,TSPL_PROSPECT_DETAIL.TAX2, " & _
            "TSPL_PROSPECT_DETAIL.TAX2_Rate,TSPL_PROSPECT_DETAIL.TAX2_Amt,TSPL_PROSPECT_DETAIL.TAX3, " & _
            "TSPL_PROSPECT_DETAIL.TAX3_Rate,TSPL_PROSPECT_DETAIL.TAX3_Amt,TSPL_PROSPECT_DETAIL.TAX4, " & _
            "TSPL_PROSPECT_DETAIL.TAX4_Rate,TSPL_PROSPECT_DETAIL.TAX4_Amt,TSPL_PROSPECT_DETAIL.TAX5, " & _
            "TSPL_PROSPECT_DETAIL.TAX5_Rate,TSPL_PROSPECT_DETAIL.TAX5_Amt,TSPL_PROSPECT_DETAIL.TAX6, " & _
            "TSPL_PROSPECT_DETAIL.TAX6_Rate,TSPL_PROSPECT_DETAIL.TAX6_Amt,TSPL_PROSPECT_DETAIL.TAX7, " & _
            "TSPL_PROSPECT_DETAIL.TAX7_Rate,TSPL_PROSPECT_DETAIL.TAX7_Amt,TSPL_PROSPECT_DETAIL.TAX8, " & _
            "TSPL_PROSPECT_DETAIL.TAX8_Rate,TSPL_PROSPECT_DETAIL.TAX8_Amt,TSPL_PROSPECT_DETAIL.TAX9, " & _
            "TSPL_PROSPECT_DETAIL.TAX9_Rate,TSPL_PROSPECT_DETAIL.TAX9_Amt,TSPL_PROSPECT_DETAIL.TAX10, " & _
            "TSPL_PROSPECT_DETAIL.TAX10_Rate,TSPL_PROSPECT_DETAIL.TAX10_Amt,TSPL_PROSPECT_DETAIL.Amount, " & _
            "TSPL_PROSPECT_DETAIL.Disc_Per,TSPL_PROSPECT_DETAIL.Disc_Amt,TSPL_PROSPECT_DETAIL.Amt_Less_Discount, " & _
            "TSPL_PROSPECT_DETAIL.Total_Tax_Amt,TSPL_PROSPECT_DETAIL.Item_Net_Amt,TSPL_LOCATION_MASTER.Location_Desc as LocationName, " & _
            "TSPL_PROSPECT_DETAIL.TAX1_Base_Amt,TSPL_PROSPECT_DETAIL.TAX2_Base_Amt,TSPL_PROSPECT_DETAIL.TAX3_Base_Amt, " & _
            "TSPL_PROSPECT_DETAIL.TAX4_Base_Amt,TSPL_PROSPECT_DETAIL.TAX5_Base_Amt,TSPL_PROSPECT_DETAIL.TAX6_Base_Amt, " & _
            "TSPL_PROSPECT_DETAIL.TAX7_Base_Amt,TSPL_PROSPECT_DETAIL.TAX8_Base_Amt,TSPL_PROSPECT_DETAIL.TAX9_Base_Amt, " & _
            "TSPL_PROSPECT_DETAIL.TAX10_Base_Amt,TSPL_PROSPECT_DETAIL.Specification, " & _
            "TSPL_PROSPECT_DETAIL.Remarks,TSPL_PROSPECT_DETAIL.MRP,TSPL_PROSPECT_DETAIL.Assessable, " & _
            " TSPL_PROSPECT_DETAIL.MRP,TSPL_PROSPECT_DETAIL.Scheme_Applicable,TSPL_PROSPECT_DETAIL.Scheme_Code, " & _
            "TSPL_PROSPECT_DETAIL.Scheme_Item,TSPL_PROSPECT_DETAIL.Item_Tax,TSPL_PROSPECT_DETAIL.Total_MRP_Amt, " & _
            "TSPL_PROSPECT_DETAIL.Total_Basic_Amt,TSPL_PROSPECT_DETAIL.Total_Disc_Amt,TSPL_PROSPECT_DETAIL.Cust_Discount, " & _
            "TSPL_PROSPECT_DETAIL.Total_Cust_Discount,TSPL_PROSPECT_DETAIL.ActualRate,TSPL_PROSPECT_DETAIL.Cust_DiscountQty, " & _
            "TSPL_PROSPECT_DETAIL.Price_code,TSPL_PROSPECT_DETAIL.Abatement_Per,TSPL_PROSPECT_DETAIL.Abatement_Amt, " & _
            "TSPL_PROSPECT_DETAIL.FOC_Item,TSPL_PROSPECT_DETAIL.Item_Weight,TSPL_PROSPECT_DETAIL.Price_Date,TSPL_PROSPECT_DETAIL.Batch_No,TSPL_PROSPECT_DETAIL.MFG_Date,TSPL_PROSPECT_DETAIL.Expiry_Date, " & _
            "TSPL_PROSPECT_DETAIL.TotalItem_Weight,TSPL_PROSPECT_DETAIL.Conv_Factor,TSPL_PROSPECT_DETAIL.Purchase_Cost,TSPL_PROSPECT_DETAIL.OrgRate, " & _
            "TSPL_PROSPECT_DETAIL.Bin_No,TSPL_PROSPECT_DETAIL.vendor_code,TSPL_PROSPECT_DETAIL.vendor_desc,TSPL_PROSPECT_DETAIL.PrincipleCode,TSPL_PROSPECT_DETAIL.PrincipleDesc,TSPL_PROSPECT_DETAIL.Markup_On,TSPL_PROSPECT_DETAIL.Markup_Percent,TSPL_PROSPECT_DETAIL.Landing_Cost,TSPL_PROSPECT_DETAIL.HeadDiscAmt,TSPL_PROSPECT_DETAIL.CustDiscPer,TSPL_PROSPECT_DETAIL.CasdDiscScheme_Code " & _
            "FROM TSPL_PROSPECT_DETAIL left outer join TSPL_LOCATION_MASTER on " & _
            "TSPL_LOCATION_MASTER.Location_Code=TSPL_PROSPECT_DETAIL.Location left outer join TSPL_ITEM_MASTER on " & _
            "TSPL_ITEM_MASTER.Item_Code=TSPL_PROSPECT_DETAIL.Item_Code where " & _
            "TSPL_PROSPECT_DETAIL.Document_Code='" + obj.Document_Code + "' ORDER BY TSPL_PROSPECT_DETAIL.Line_No asc"

            dt = New DataTable()
            dt = clsDBFuncationality.GetDataTable(qry, trans)
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                obj.Arr = New List(Of clsProspectDetail)
                Dim objTr As clsProspectDetail
                For Each dr As DataRow In dt.Rows
                    objTr = New clsProspectDetail
        ...
        End If
        '' new code
        obj.ArrComm = clsProspectCommunication.GetProspectCommunication(obj.Document_Code, trans)
        obj.ArrLoc = clsProspectLocation.GetProspectLocation(obj.Document_Code, trans)
        Return obj
    End Function
    ...
    Public Shared Function PostData(ByVal FormId As String, ByVal strDocNo As String) As Boolean
        ...
        Try
            ...
            Dim obj As clsProspectHead = clsProspectHead.GetData(strDocNo, NavigatorType.Current, trans)

            If (obj Is Nothing OrElse clsCommon.myLen(obj.Document_Code) <= 0) Then
                Throw New Exception("No Data found to Post")
            End If
            'checkApprovalNotification(obj.Document_Date, obj.Customer_Code, trans)
            checkApprovalNotification(obj.Document_Date, obj.Customer_Name, trans)
            ...
            Dim isResult As Boolean = clsApprovalScreen.CheckApprovalLevel(FormId, "TSPL_PROSPECT_HEAD", "Document_Code", obj.Document_Code, trans)
            If isResult = False Then
                trans.Commit()
                Return False
            End If

            Dim qry As String = "Update TSPL_PROSPECT_HEAD set Status=1, Posting_Date='" + strPostDate + "', Modify_Date='" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy") + "' where Document_Code='" + strDocNo + "' "
    ...
    Public Shared Function DeleteData(ByVal strCode As String) As Boolean
        ...
        End If
        Dim obj As clsProspectHead = clsProspectHead.GetData(strCode, NavigatorType.Current)
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
            Try
                ...
                End If
                Dim qry As String = "delete from TSPL_PROSPECT_DETAIL where Document_Code='" + strCode + "'"
                isSaved = clsDBFuncationality.ExecuteNonQuery(qry, trans)

                qry = "delete from TSPL_PROSPECT_COMMUNICATION where Document_Code='" + strCode + "'"
                isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)

                qry = "delete from TSPL_PROSPECT_Location where Document_Code='" + strCode + "'"
                isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)

                qry = "delete from TSPL_PROSPECT_HEAD where Document_Code='" + strCode + "'"
                isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Function IsValidCustomer(ByVal ArrPONo As List(Of String), ByVal strVendorCode As String) As Boolean
        If ArrPONo IsNot Nothing AndAlso ArrPONo.Count > 0 Then
            Dim qry As String = "select TSPL_PROSPECT_HEAD.Document_Code,TSPL_PROSPECT_HEAD.Customer_Name from TSPL_PROSPECT_HEAD  where Document_Code  in (" + clsCommon.GetMulcallString(ArrPONo) + ") and Customer_Code not in ('" + strVendorCode + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim msg As String = "Error. "
                For Each dr As DataRow In dt.Rows
                    msg += Environment.NewLine + "Order No:" + clsCommon.myCstr(dr("Document_Code")) + " Customer Name:" + clsCommon.myCstr(dr("Customer_Name"))
                Next
    ...
    Public Shared Function IsValidTaxGroupForPO(ByVal ArrPONo As List(Of String), ByVal strTaxGroupCode As String) As Boolean
        If ArrPONo IsNot Nothing AndAlso ArrPONo.Count > 0 Then
            Dim qry As String = "select Document_Code,Tax_Group from TSPL_PROSPECT_HEAD where Document_Code  in (" + clsCommon.GetMulcallString(ArrPONo) + ") and Tax_Group not in ('" + strTaxGroupCode + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim msg As String = "Error. "
                For Each dr As DataRow In dt.Rows
    ...
    Public Shared Function IsValidProjectForSO(ByVal strPONo As String, ByVal strProject As String) As String
        Dim strProj As String = clsDBFuncationality.getSingleValue("select PROJECT_ID from TSPL_PROSPECT_HEAD where Document_Code ='" + strPONo + "'")
        Return strProj
    End Function
End Class

Public Class clsProspectDetail
#Region "Variables"
    Public Document_Code As String = Nothing
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal Arr As List(Of clsProspectDetail), ByVal trans As SqlTransaction) As Boolean

        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsProspectDetail In Arr
                Dim coll As New Hashtable()
               ...
               "TSPL_ITEM_PRICE_MASTER.Item_Basic_Net=" & obj.MRP & " and TSPL_ITEM_PRICE_MASTER.UOM='" & obj.Unit_code & "' "
                ...
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_PROSPECT_DETAIL", OMInsertOrUpdate.Insert, "", trans)
            Next
        End If
        Return True
    End Function
    ...
    Public Shared Function GetBalancePOQty(ByVal strPOCode As String, ByVal strICode As String, ByVal strCurrGRNNo As String, ByVal strUOM As String, ByVal dblMRP As Double, ByVal dblAssessable As Double) As Double
        Dim qry As String = "select SUM(qty * RI) as Balance from(  " & _
            " select TSPL_PROSPECT_DETAIL.Item_Code as ICode,TSPL_PROSPECT_DETAIL.Qty as Qty,1 as RI from TSPL_PROSPECT_DETAIL left outer join TSPL_PROSPECT_HEAD on TSPL_PROSPECT_HEAD.Document_Code=TSPL_PROSPECT_DETAIL.Document_Code where TSPL_PROSPECT_DETAIL.Status=0 and TSPL_PROSPECT_HEAD.Status=1 and TSPL_PROSPECT_DETAIL.Document_Code ='" + strPOCode + "' and TSPL_PROSPECT_DETAIL.Item_Code='" + strICode + "' and  TSPL_PROSPECT_DETAIL.Unit_code='" + strUOM + "' and isnull(TSPL_PROSPECT_DETAIL.MRP,0)='" + clsCommon.myCstr(dblMRP) + "' and isnull(TSPL_PROSPECT_DETAIL.Assessable,0)='" + clsCommon.myCstr(dblAssessable) + "'" & _
            " union all " & _
    ...
    Public Shared Function CompletePO(ByVal strReqCode As String, ByVal strICode As String, ByVal LineNo As Integer) As Boolean
        Dim qry As String = "update TSPL_PROSPECT_DETAIL set Status ='1' where Document_Code='" + strReqCode + "' and Line_No='" + clsCommon.myCstr(LineNo) + "' and Item_Code='" + strICode + "'"
        Return clsDBFuncationality.ExecuteNonQuery(qry)
    End Function
End Class
Public Class clsProspectCommunication
    Public Document_Code As String = Nothing
    Public Line_No As Integer = 0
    Public Comm_Date As Date = Nothing
    Public Comm_Remarks As String = Nothing
    Public Comm_Type As String = Nothing
    Public Comm_Type_Seq As Integer = 1

    Public Shared Function SaveData(ByVal strDocNo As String, ByVal Arr As List(Of clsProspectCommunication), ByVal trans As SqlTransaction) As Boolean

        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsProspectCommunication In Arr
                Dim coll As New Hashtable()
                clsCommon.AddColumnsForChange(coll, "Document_Code", strDocNo)
                clsCommon.AddColumnsForChange(coll, "Line_No", obj.Line_No)
                clsCommon.AddColumnsForChange(coll, "Comm_Date", clsCommon.GetPrintDate(obj.Comm_Date, "dd/MMM/yyyy"))
                clsCommon.AddColumnsForChange(coll, "Comm_Remarks", obj.Comm_Remarks)
                clsCommon.AddColumnsForChange(coll, "Comm_Type", obj.Comm_Type)
                clsCommon.AddColumnsForChange(coll, "Comm_Type_Seq", obj.Comm_Type_Seq)

                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_PROSPECT_COMMUNICATION", OMInsertOrUpdate.Insert, "", trans)
            Next
        End If
        Return True
    End Function
    Public Shared Function GetProspectCommunication(ByVal strDocNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsProspectCommunication)
        Dim dt As New DataTable
        Dim qry As String = ""
        qry = "SELECT * FROM TSPL_PROSPECT_COMMUNICATION " & _
              " where TSPL_PROSPECT_COMMUNICATION.Document_Code='" & strDocNo & "'"
        dt = clsDBFuncationality.GetDataTable(qry, trans)
        Dim objtr As clsProspectCommunication
        Dim ObjList As New List(Of clsProspectCommunication)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            For Each dr As DataRow In dt.Rows
                objtr = New clsProspectCommunication()

                objtr.Document_Code = clsCommon.myCstr(dr("Document_Code"))
                objtr.Line_No = clsCommon.myCdbl(dr("Line_No"))
                objtr.Comm_Date = clsCommon.myCDate(dr("Comm_Date"))
                objtr.Comm_Remarks = clsCommon.myCstr(dr("Comm_Remarks"))
                objtr.Comm_Type = clsCommon.myCstr(dr("Comm_Type"))
                objtr.Comm_Type_Seq = clsCommon.myCdbl(dr("Comm_Type_Seq"))
    ...
    End Function
End Class
Public Class clsProspectLocation
    Public Doc_Code As String = Nothing
    Public State As String = Nothing
    Public Zone As String = Nothing
    Public Location As String = Nothing
    Public StateDesc As String = Nothing
    Public ZoneDesc As String = Nothing
    Public LocationDesc As String = Nothing
    Public Address As String = Nothing

    Public Shared Function SaveData(ByVal strDocNo As String, ByVal isNewEntry As Boolean, ByVal Arr As List(Of clsProspectLocation), ByVal trans As SqlTransaction) As Boolean
        Try

            If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
                For Each obj As clsProspectLocation In Arr
                    Dim coll As New Hashtable()
                    clsCommon.AddColumnsForChange(coll, "Document_Code", strDocNo)
                    clsCommon.AddColumnsForChange(coll, "State_Code", obj.State)
                    clsCommon.AddColumnsForChange(coll, "Zone_Code", obj.Zone)
                    clsCommon.AddColumnsForChange(coll, "Location", obj.Location)
                    clsCommon.AddColumnsForChange(coll, "Address", obj.Address)
                    ...
                    If isNewEntry Then
                        ...
                        clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Prospect_Location", OMInsertOrUpdate.Insert, "", trans)
                    Else
                        clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Prospect_Location", OMInsertOrUpdate.Update, "TSPL_Prospect_Location.Document_Code='" + strDocNo + "'", trans)
                    End If
        ...
        End Try
    End Function
    Public Shared Function GetProspectLocation(ByVal strDocNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsProspectLocation)
        Dim dt As New DataTable
        Dim qry As String = ""
        qry = "SELECT e1.Location,e2.Location_Desc,e3.STATE_NAME,e1.State_Code,e1.Zone_Code,e4.Zone_Code as Zone,e1.Address FROM TSPL_Prospect_Location as e1 " & _
            "inner join TSPL_LOCATION_MASTER as e2 on e1.Location=e2.Location_Code " & _
            "inner join TSPL_STATE_MASTER AS E3 ON e1.State_Code=e3.STATE_CODE " & _
            "inner join TSPL_ZONE_MASTER as e4 on e1.Zone_Code=e4.Zone_Code " & _
              " where e1.Document_Code='" & strDocNo & "'"
        dt = clsDBFuncationality.GetDataTable(qry, trans)
        Dim objtr As clsProspectLocation
        Dim ObjList As New List(Of clsProspectLocation)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            For Each dr As DataRow In dt.Rows
                objtr = New clsProspectLocation()

                ' objtr.Doc_Code = clsCommon.myCstr(dr("Document_Code"))
                objtr.State = clsCommon.myCstr(dr("State_Code"))
                objtr.Zone = clsCommon.myCstr(dr("Zone_code"))
                objtr.Location = clsCommon.myCstr(dr("Location"))
                objtr.Address = clsCommon.myCstr(dr("Address"))
                objtr.StateDesc = clsCommon.myCstr(dr("STATE_NAME"))
                objtr.ZoneDesc = clsCommon.myCstr(dr("Zone"))
                objtr.LocationDesc = clsCommon.myCstr(dr("Location_Desc"))
