...
#Region "Variables"
    ...
    Public Shared ObjList As List(Of clsCSABookingDetail)
#End Region

    Public Shared Function GetData(ByVal strCode As String, ByVal arrloc As String, ByVal NavType As NavigatorType, ByVal strTransType As String) As clsCSABooking
    ...
    Public Shared Function GetBookingDescrptn(ByVal strTransType As String) As String
        Dim str As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select top 1 description from tspl_csa_booking_head where trans_type='" + strTransType + "' and isnull(description,'')<>'' order by booking_date desc"))
    ...
    Public Shared Function DeleteData(ByVal strCode As String, ByVal Trans_Type As String, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            Dim dt As DataTable = clsDBFuncationality.GetDataTable("select BOOKING_DATE,Location_Code from TSPL_CSA_BOOKING_HEAD where BOOKING_NO='" + strCode + "'", trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleCSASale, clsUserMgtCode.frmCSABooking, clsCommon.myCstr(dt.Rows(0)("Location_Code")), clsCommon.myCDate(dt.Rows(0)("BOOKING_DATE")), trans)

            End If
            Dim qry As String
            qry = "delete from TSPL_CSA_BOOKING_DETAIL where BOOKING_NO ='" + strCode + "'"
            isSaved = clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_CSA_BOOKING_HEAD where BOOKING_NO ='" + strCode + "' and trans_type='" + Trans_Type + "'"
        ...
        Return isSaved
    End Function
    Public Shared Function GetData(ByVal strCode As String, ByVal arrloc As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction, ByVal strTransType As String) As clsCSABooking
        Dim obj As New clsCSABooking()
        Dim objtr As New clsCSABookingDetail()

        ObjList = New List(Of clsCSABookingDetail)

        Dim qry As String = " select TSPL_CSA_BOOKING_HEAD.Booking_Type,TSPL_CSA_BOOKING_HEAD.BOOKING_NO,TSPL_CSA_BOOKING_HEAD.DESCRIPTION,TSPL_CSA_BOOKING_HEAD.BOOKING_DATE, " & _
        " TSPL_CSA_BOOKING_HEAD.CSA_CODE,TSPL_CUSTOMER_MASTER.Customer_Name as CSA_NAME,TSPL_CSA_BOOKING_HEAD.Posted,TSPL_CSA_BOOKING_HEAD.Request_Mode,TSPL_CSA_BOOKING_HEAD.Location_Code," & _
        " TSPL_CSA_BOOKING_HEAD.POSTING_DATE,TSPL_CSA_BOOKING_HEAD.Created_By,TSPL_CSA_BOOKING_HEAD.Modified_By " & _
        " from TSPL_CSA_BOOKING_HEAD left join TSPL_CUSTOMER_MASTER on TSPL_CSA_BOOKING_HEAD.CSA_CODE=TSPL_CUSTOMER_MASTER.Cust_Code WHERE TSPL_CSA_BOOKING_HEAD.COMP_CODE='" & objCommonVar.CurrentCompanyCode & "'"

        Dim whrcond As String = ""

        qry += " and TSPL_CSA_BOOKING_HEAD.Trans_Type='" + strTransType + "' "
        If clsCommon.CompairString(clsCommon.myCstr(strTransType), "Request") = CompairStringResult.Equal AndAlso clsCommon.myLen(arrloc) > 0 Then
            qry += " and TSPL_CSA_BOOKING_HEAD.Location_Code in (" + arrloc + ") "
            whrcond = " and Location_Code in (" + arrloc + ") "
        End If

        Select Case NavType
            Case NavigatorType.First
                qry += " AND BOOKING_NO = (select MIN(BOOKING_NO) from TSPL_CSA_BOOKING_HEAD where trans_type='" + strTransType + "' " + whrcond + ") "
            Case NavigatorType.Last
                qry += " AND BOOKING_NO = (select Max(BOOKING_NO) from TSPL_CSA_BOOKING_HEAD where trans_type='" + strTransType + "' " + whrcond + ")"
            Case NavigatorType.Next
                qry += " AND BOOKING_NO = (select Min(BOOKING_NO) from TSPL_CSA_BOOKING_HEAD where BOOKING_NO>'" + strCode + "' and trans_type='" + strTransType + "' " + whrcond + ") "
            Case NavigatorType.Previous
                qry += " AND BOOKING_NO = (select Max(BOOKING_NO) from TSPL_CSA_BOOKING_HEAD where BOOKING_NO<'" + strCode + "' and trans_type='" + strTransType + "' " + whrcond + ") "
        ...
        clsCSABooking.ObjList = clsCSABookingDetail.GetCSADetailDetail(strCode, trans)
        Return obj
    End Function

    Public Function SaveData(ByVal obj As clsCSABooking, ByVal objList As List(Of clsCSABookingDetail), ByVal isNewEntry As Boolean, Optional ByVal strCode As String = "") As Boolean
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
    ...
    Public Function SaveData(ByVal obj As clsCSABooking, ByVal objList As List(Of clsCSABookingDetail), ByVal isNewEntry As Boolean, ByVal trans As SqlTransaction, Optional ByVal strCode As String = "") As Boolean
        ...
        Try
            If isNewEntry Then
                If strCode = "" Then
                    If clsCommon.CompairString(clsCommon.myCstr(obj.Trans_Type), "Booking") = CompairStringResult.Equal Then
                        obj.BOOKING_NO = clsERPFuncationality.GetNextCode(trans, obj.BOOKING_DATE, clsDocType.CSABooking, "", "")
                    ElseIf clsCommon.CompairString(clsCommon.myCstr(obj.Trans_Type), "Request") = CompairStringResult.Equal Then
                        obj.BOOKING_NO = clsERPFuncationality.GetNextCode(trans, obj.BOOKING_DATE, clsDocType.CSARequest, "", obj.Location_Code)
                    End If
                Else
            ...
            End If
            clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleCSASale, clsUserMgtCode.frmCSABooking, obj.Location_Code, obj.BOOKING_DATE, trans)
            ...
            If isNewEntry Then
                ...
                Dim Strqry As String = "SELECT Count(*) FROM TSPL_CSA_BOOKING_HEAD where BOOKING_NO = '" & obj.BOOKING_NO & "'"
                Dim check As Integer = clsDBFuncationality.getSingleValue(Strqry, trans)
                If check = 0 Then
                    isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_CSA_BOOKING_HEAD", OMInsertOrUpdate.Insert, "", trans)
                Else
                    Throw New Exception("This Code:" + obj.BOOKING_NO + " Is Already Exist")
                End If
            Else
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_CSA_BOOKING_HEAD", OMInsertOrUpdate.Update, "TSPL_CSA_BOOKING_HEAD.BOOKING_NO='" + obj.BOOKING_NO + "'", trans)
            End If

            isSaved = isSaved AndAlso clsCSABookingDetail.SaveDetailData(obj.BOOKING_NO, obj, objList, trans)
    ...
    Public Shared Function PostData(ByVal strDocNo As String, ByVal arrloc As String, ByVal isCheckForPosted As Boolean, ByVal trans As SqlTransaction, ByVal strTranstype As String) As Boolean
        ...
        Try
            ...
            Dim obj As clsCSABooking = clsCSABooking.GetData(strDocNo, arrloc, NavigatorType.Current, trans, strTranstype)
            clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleCSASale, clsUserMgtCode.frmCSABooking, obj.Location_Code, obj.BOOKING_DATE, trans)
            ...
            End If
            Dim isSaved As Boolean = True
            Dim qry As String = "Update TSPL_CSA_BOOKING_HEAD set POSTED=1, Posting_Date='" + strPostDate + "',Modified_By='" + objCommonVar.CurrentUserCode + "' where BOOKING_NO ='" + strDocNo + "'"
    ...
    Public Shared Function CheckValidCode(ByVal Doc_No As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim qry As String = "select count(*) from TSPL_CSA_BOOKING_HEAD where BOOKING_NO='" & Doc_No & "' and comp_code='" + objCommonVar.CurrentCompanyCode + "' "
    ...
    Public Shared Function GetFinder(ByVal whrCls As String, ByVal currCode As String, ByVal isButtonClicked As Boolean) As String
        Dim qry As String = " select TSPL_CSA_BOOKING_HEAD.BOOKING_NO as Code,TSPL_CSA_BOOKING_HEAD.DESCRIPTION,TSPL_CSA_BOOKING_HEAD.BOOKING_DATE, " & _
        " TSPL_CSA_BOOKING_HEAD.CSA_CODE,TSPL_CUSTOMER_MASTER.Customer_Name as CSA_NAME,ISNULL(TSPL_CUSTOMER_MASTER.Alies_Name,'') As [Alies Name],TSPL_CSA_BOOKING_HEAD.Posted," & _
        "  POSTING_DATE=(convert(varchar(10),TSPL_CSA_BOOKING_HEAD.POSTING_DATE,103)),TSPL_CSA_BOOKING_HEAD.Created_By,TSPL_CSA_BOOKING_HEAD.Modified_By " & _
        " from TSPL_CSA_BOOKING_HEAD left join TSPL_CUSTOMER_MASTER on TSPL_CSA_BOOKING_HEAD.CSA_CODE=TSPL_CUSTOMER_MASTER.Cust_Code "
        Dim str As String = ""
        If clsCommon.myLen(whrCls) > 0 Then
            whrCls = whrCls + " and TSPL_CSA_BOOKING_HEAD.comp_code='" + objCommonVar.CurrentCompanyCode + "'"
        Else
...
Public Class clsCSABookingDetail
#Region "Variables"
...
#End Region
    ...
    Public Shared Function SaveDetailData(ByVal strDocNo As String, ByVal objRec As clsCSABooking, ByVal Arr As List(Of clsCSABookingDetail), ByVal trans As SqlTransaction) As Boolean

        Try
            If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
                Dim qry As String = "DELETE FROM TSPL_CSA_BOOKING_DETAIL WHERE BOOKING_NO='" + strDocNo + "'"
                clsDBFuncationality.ExecuteNonQuery(qry, trans)

                For Each obj As clsCSABookingDetail In Arr
                    ...
                    clsCommonFunctionality.UpdateDataTable(coll, "TSPL_CSA_BOOKING_DETAIL", OMInsertOrUpdate.Insert, "TSPL_CSA_BOOKING_DETAIL.BOOKING_NO='" + strDocNo + "' ", trans)
        ...
        Return True
    End Function
    Public Shared Function GetCSADetailDetail(ByVal strCode As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsCSABookingDetail)
        Dim qry As String
        qry = "select TSPL_CSA_BOOKING_DETAIL.Line_No,TSPL_CSA_BOOKING_DETAIL.BOOKING_NO,TSPL_CSA_BOOKING_DETAIL.CSA_ITEM_TYPE,TSPL_CSA_BOOKING_DETAIL.BOOK_QTY,TSPL_CSA_BOOKING_DETAIL.BOOK_QTY_UOM,TSPL_CSA_BOOKING_DETAIL.TAX_PAID,TSPL_CSA_BOOKING_DETAIL.BOOK_Rate,TSPL_CSA_BOOKING_DETAIL.BOOK_RATE_UOM,TSPL_CSA_BOOKING_DETAIL.TOTAL_QTY,TSPL_CSA_BOOKING_DETAIL.item_code " & _
              " from TSPL_CSA_BOOKING_DETAIL WHERE 2=2 " & _
              " AND TSPL_CSA_BOOKING_DETAIL.BOOKING_NO = '" + strCode + "' ORDER BY Line_No"
