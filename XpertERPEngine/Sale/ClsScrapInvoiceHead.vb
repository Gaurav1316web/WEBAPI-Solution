...
left outer join TSPL_TERMS_MASTER on TSPL_TERMS_MASTER.Terms_Code=tspl_scrapsale_head.Terms_Code where TSPL_SCRAPINVOICE_HEAD.shipment_No='" + strPONO + "'"
    ...
    Public Shared Function PostData(ByVal strDocNo As String) As Boolean
        ...
        Return True
    End Function
    Public Shared Function PrintEWayBill(ByVal strInvoiceNO As String, ByVal strCust As String) As String

        If clsCommon.myLen(strInvoiceNO) <= 0 Then
            Throw New Exception("Document not found !")
        End If
        Dim Qry As String = " select cast(TSPL_SCRAPINVOICE_HEAD.EWayBill_QR_Code as image) as EWayBill_QR_Code,
TSPL_SCRAPINVOICE_HEAD.EWayBillNo,TSPL_SCRAPINVOICE_HEAD.ewayBillDate,TSPL_COMPANY_MASTER.GSTReg_No +' ' + TSPL_COMPANY_MASTER.Comp_Name as Generated_By,TSPL_SCRAPINVOICE_HEAD.EWayBillValidDate,
TSPL_SCRAPINVOICE_HEAD.EWayBillRemarks,TSPL_SCRAPINVOICE_HEAD.Freight_Distance,TSPL_SCRAPINVOICE_HEAD.IRN_No,TSPL_SCRAPINVOICE_HEAD.Ack_No,TSPL_SCRAPINVOICE_HEAD.Ack_Date,TSPL_CUSTOMER_MASTER.GSTNO+' '+ TSPL_CUSTOMER_MASTER.Customer_Name+' '+TSPL_CUSTOMER_MASTER.City_Code as GSTIN_Of_Recipient,TSPL_COMPANY_MASTER.City_Code+' - '+TSPL_COMPANY_MASTER.State+' - '+TSPL_COMPANY_MASTER.Pincode as Place_Of_Dispatch, TSPL_CUSTOMER_MASTER.City_Code+' '+TSPL_CUSTOMER_MASTER.State +' '+CONVERT(varchar(10),isNull(TSPL_CUSTOMER_MASTER.PIN_Code,'')) as Place_Of_delivery,
TSPL_SCRAPINVOICE_HEAD.invoice_No,TSPL_SCRAPINVOICE_HEAD.shipment_Date,'Regular' as Transaction_Type,TSPL_SCRAPINVOICE_HEAD.Doc_Amt,TSPL_ITEM_MASTER.HSN_Code +' '+TSPL_ITEM_MASTER.Short_Description  as HsnCode,
--convert(VARCHAR(10), case when(select COUNT(*) from TSPL_SCRAPINVOICE_DETAIL where Document_Code=TSPL_SCRAPINVOICE_HEAD.Document_Code)-1>0 then (select COUNT(*) from TSPL_SCRAPINVOICE_DETAIL where Document_Code=TSPL_SCRAPINVOICE_HEAD.Document_Code)-1  else '' end) as countofItem,
'Outward - Supply' as ResonofTransport,'' as Trans_Type,TSPL_SCRAPINVOICE_HEAD.Transporter_Name_Manual,'Road' As Mode_of_Trans,TSPL_VEHICLE_MASTER.Vehicle_No,TSPL_SCRAPINVOICE_HEAD.shipment_Date,TSPL_SCRAPINVOICE_HEAD.Loc_Code
...
TSPL_Customer_master.Alies_name as toTrdName,
    ...
    Case when ISNULL(TSPL_SCRAPINVOICE_HEAD.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_HEAD.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX3_Amt  when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX4_Amt else 0 end) else (case when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX1_Amt when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX2_Amt else 0 end) end cgstValue,
    Case when ISNULL(TSPL_SCRAPINVOICE_HEAD.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_HEAD.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX3_Amt when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX4_Amt else 0 end) else (case when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX1_Amt when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX2_Amt else 0 end) end sgstValue,
    Case when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'IGST' then TSPL_SCRAPINVOICE_HEAD.TAX1_Amt else (case when ISNULL(TSPL_SCRAPINVOICE_HEAD.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_HEAD.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'IGST' then TSPL_SCRAPINVOICE_HEAD.TAX3_Amt else 0 end) else 0 end )end igstValue, 
    ...
    left Outer Join TSPL_SHIP_TO_LOCATION as Ship_To_Location on Ship_To_Location.Ship_To_Code = TSPL_SCRAPINVOICE_HEAD.Loc_Code 
    ...
    left outer join TSPL_ADDITIONAL_CHARGES on TSPL_ADDITIONAL_CHARGES.CODE = TSPL_SCRAPINVOICE_DETAIL.Item_code 
    ...
    left outer join tspl_city_master on tspl_city_master.city_code = TSPL_Customer_master.City_Code left outer join tspl_tax_master as TCS1 on TCS1.Tax_Code = TSPL_SCRAPINVOICE_HEAD.Tax2 
    left outer join tspl_tax_master as TCS2 on TCS2.Tax_Code = TSPL_SCRAPINVOICE_HEAD.Tax3 Left Outer Join tspl_vendor_master on tspl_vendor_master.vendor_code = TSPL_SCRAPINVOICE_HEAD.Transport_Code 
    left outer join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_Id=TSPL_SCRAPINVOICE_HEAD.Vehicle_Code
        where TSPL_SCRAPINVOICE_HEAD.invoice_No = '" & strDocNo & "'"
            ...
            If objResult IsNot Nothing Then
                ...
                Try
                    ...
                    clsDBFuncationality.ExecuteNonQuery("update TSPL_SCRAPINVOICE_HEAD set  EWayBillNo ='" & EWayBillNo & "',EWayBillDate='" & clsCommon.GetPrintDate(clsCommon.myCDate(EWayBillDate), "dd/MMM/yyyy hh:mm tt") & "',EWayBillValidDate='" & clsCommon.GetPrintDate(clsCommon.myCDate(EWayBillValidDate), "dd/MMM/yyyy hh:mm tt") & "',EWayBillRemarks='" & EWayBillRemarks & "' where TSPL_SCRAPINVOICE_HEAD.invoice_No ='" & strDocNo & "'", trans)
...
Left Outer Join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_Id  =TSPL_SCRAPINVOICE_HEAD.Vehicle_Id
 where TSPL_SCRAPINVOICE_HEAD.invoice_No ='" & strDocNo & "'"
                Else
                    '                    strQry = " select TSPL_Customer_master .Cust_Code ,TSPL_SCRAPINVOICE_HEAD.invoice_No as DocNo,convert(date,TSPL_SCRAPINVOICE_HEAD.shipment_Date,103) as DocDate,case when TSPL_Customer_Invoice_Head.Document_Type='D' then 'DBN' when TSPL_Customer_Invoice_Head.Document_Type ='C' then 'CRN' else 'INV' end as DocType ,'B2B' as SupTyp, 'N'  as IgstOnIntra,Bill_To_Location.GSTNO as SellerGSTINNo ,Bill_To_Location.location_desc as SellerLglNm,TSPL_COMPANY_MASTER.Comp_Name as SellerTrdNm,Bill_To_Location.Add1 as SellerAdd1,Bill_To_Location.Add2 as SellerAdd2 ,Bill_To_Location.city_code as SellerLoc,Bill_To_Location.Pin_Code  as SellerPincode,BillToLocation_State_Master.GST_STATE_Code as SellerStcd,Bill_To_Location.Phone1 as SellerPhone,Bill_To_Location.Email as SellerEmail,TSPL_Customer_master.GSTNo as BuyerGSTINNo ,TSPL_Customer_master.Customer_Name as BuyerLglNm,TSPL_Customer_master.alies_name as BuyerTrdNm,case when isnull(TSPL_SCRAPINVOICE_HEAD.ToLoc_Code,'')='' then Customer_State_Master.GST_STATE_Code else Ship_To_Location_State_Master.GST_STATE_Code end as BuyerPOS,TSPL_Customer_master.Add1 as BuyerAdd1,TSPL_Customer_master.Add2 as BuyerAdd2 ,tspl_city_master.City_Name as BuyerLoc,cast(TSPL_Customer_master.PIN_NO as int) as BuyerPincode,Customer_State_Master.GST_STATE_Code as BuyerStcd,TSPL_Customer_master.Phone1 as BuyerPhone,TSPL_Customer_master.Email as BuyerEmail,TSPL_SCRAPINVOICE_DETAIL.Line_No as ItemSlNo, 'N' as ItemIsServc,TSPL_ITEM_MASTER.Item_Desc AS ItemPrdDesc,TSPL_ITEM_MASTER.HSN_Code AS ItemHsnCd,TSPL_SCRAPINVOICE_DETAIL.invoice_Qty as ItemQty,TSPL_SCRAPINVOICE_DETAIL.Unit_code as ItemUnit,TSPL_SCRAPINVOICE_DETAIL.price as ItemUnitPrice,TSPL_SCRAPINVOICE_DETAIL.ItemAmt as ItemTotAmt,
                    'TSPL_SCRAPINVOICE_DETAIL.DiscountAmt as ItemDiscount,TSPL_SCRAPINVOICE_DETAIL.ItemNetAmt as ItemAssAmt,
                    'Case when ISNULL( TSPL_SCRAPINVOICE_DETAIL.tax1, '' ) = 'IGST' THEN TSPL_SCRAPINVOICE_DETAIL.TAX1_Rate 
                    ' when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_DETAIL.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_DETAIL.TAX3_Rate + TSPL_SCRAPINVOICE_DETAIL.TAX4_Rate else TSPL_SCRAPINVOICE_DETAIL.TAX3_Rate  end)
                    'when ISNULL( TSPL_SCRAPINVOICE_DETAIL.tax1, '' ) = 'CGST' AND ISNULL( TSPL_SCRAPINVOICE_DETAIL.tax2, '' ) = 'SGST' THEN TSPL_SCRAPINVOICE_DETAIL.TAX1_Rate + TSPL_SCRAPINVOICE_DETAIL.TAX2_Rate ELSE 0 end  as ItemGstRt,
                    'Case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_DETAIL.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX4 = 'SGST' then ((TSPL_SCRAPINVOICE_DETAIL.TAX3_Base_Amt )* (TSPL_SCRAPINVOICE_DETAIL.TAX3_Rate ) )/100 when TSPL_SCRAPINVOICE_DETAIL.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX4 = 'SGST' then ((TSPL_SCRAPINVOICE_DETAIL.TAX4_Base_Amt)* (TSPL_SCRAPINVOICE_DETAIL.TAX4_Rate ) )/100 else 0 end) else (case when TSPL_SCRAPINVOICE_DETAIL.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_DETAIL.TAX1_Amt when TSPL_SCRAPINVOICE_DETAIL.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_DETAIL.TAX2_Amt else 0 end) end ItemSgstAmt,
                    'Case when TSPL_SCRAPINVOICE_DETAIL.TAX1 = 'IGST' then TSPL_SCRAPINVOICE_DETAIL.TAX1_Amt else (case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')= 'KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')= 'KKF' then(case when TSPL_SCRAPINVOICE_DETAIL.TAX3 = 'IGST' then ( (TSPL_SCRAPINVOICE_DETAIL.TAX3_Base_Amt)* (TSPL_SCRAPINVOICE_DETAIL.TAX3_Rate))/ 100 else 0 end) else 0 end)  end ItemIgstAmt,
                    'case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_DETAIL.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX4 = 'SGST' then ((TSPL_SCRAPINVOICE_DETAIL.TAX3_Base_Amt )* (TSPL_SCRAPINVOICE_DETAIL.TAX3_Rate ) )/100 when TSPL_SCRAPINVOICE_DETAIL.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX4 = 'SGST' then ((TSPL_SCRAPINVOICE_DETAIL.TAX4_Base_Amt )* (TSPL_SCRAPINVOICE_DETAIL.TAX4_Rate) )/100 else 0 end) else (case when TSPL_SCRAPINVOICE_DETAIL.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_DETAIL.TAX1_Amt when TSPL_SCRAPINVOICE_DETAIL.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_DETAIL.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_DETAIL.TAX2_Amt else 0 end) end ItemCgstAmt,
                    '0 as ItemOthChrg,
                    'TSPL_SCRAPINVOICE_DETAIL.TotalAmt-(Case when TSPL_SCRAPINVOICE_DETAIL.TAX1 = 'IGST' then isnull(TSPL_SCRAPINVOICE_DETAIL.TAX2_Amt,0) else(
                    'Case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_DETAIL.TAX3='IGST' then TSPL_SCRAPINVOICE_DETAIL.TAX4_Amt else (case when    
                    ...
                    'Case when ISNULL(TSPL_SCRAPINVOICE_HEAD.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_HEAD.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX3_Amt when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX4 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX4_Amt else 0 end) else (case when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX1_Amt when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'CGST' AND TSPL_SCRAPINVOICE_HEAD.TAX2 = 'SGST' then TSPL_SCRAPINVOICE_HEAD.TAX2_Amt else 0 end) end ValDtlsSgstVal,
                    'Case when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'IGST' then TSPL_SCRAPINVOICE_HEAD.TAX1_Amt else (case when ISNULL(TSPL_SCRAPINVOICE_HEAD.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_HEAD.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_HEAD.TAX3 = 'IGST' then TSPL_SCRAPINVOICE_HEAD.TAX3_Amt else 0 end) else 0 end )end ValDtlsIgstVal, 
                    'TSPL_SCRAPINVOICE_HEAD.Discount_Amt as ValDtlsDiscount,
                    'Case when TSPL_SCRAPINVOICE_HEAD.TAX1 = 'IGST' then isnull(TSPL_SCRAPINVOICE_HEAD.TAX2_Amt,0) else(
                    'Case when ISNULL(TSPL_SCRAPINVOICE_HEAD.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_HEAD.tax2,'')='KKF' then (case when TSPL_SCRAPINVOICE_HEAD.TAX3='IGST' then TSPL_SCRAPINVOICE_HEAD.TAX4_Amt else (case when    
                    'TSPL_SCRAPINVOICE_HEAD.TAX5='TCS' then TSPL_SCRAPINVOICE_HEAD.TAX5_Amt else 0 end) end) else (case when TSPL_SCRAPINVOICE_HEAD.tax3='TCS' then TSPL_SCRAPINVOICE_HEAD.TAX3_Amt else 0 end) end) end as ValDtlsOthChrg,
                    'TSPL_SCRAPINVOICE_HEAD.Doc_Amt  as ValDtlsTotInvVal,
                    'TSPL_SCRAPINVOICE_HEAD.RoundOffAmount  as ValDtlsRndOffAmt
                    ',ISNULL(tspl_vendor_master.GSTFinalNo,'') AS EwbTransId,ISNULL(tspl_vendor_master.Vendor_Name,'') AS EwbTransName,TSPL_SCRAPINVOICE_HEAD.Freight_Distance as EwbDistance,isnull(TSPL_VEHICLE_MASTER.Number,'') as EwbVehNo
                    'from TSPL_SCRAPINVOICE_HEAD
                    'Left Outer Join TSPL_Customer_Invoice_Head on TSPL_Customer_Invoice_Head.Against_Sale_No =TSPL_SCRAPINVOICE_HEAD.invoice_No
                    'Left Outer Join TSPL_COMPANY_MASTER  on TSPL_COMPANY_MASTER.Comp_Code  ='" & objCommonVar.CurrentCompanyCode & "'
                    ...
                    'Left Outer Join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_Id  =TSPL_SCRAPINVOICE_HEAD.Vehicle_Id
                    ' where TSPL_SCRAPINVOICE_HEAD.invoice_No ='" & strDocNo & "'"
                    strQry = " select TSPL_Customer_master .Cust_Code ,TSPL_SCRAPINVOICE_HEAD.invoice_No as DocNo,convert(date,TSPL_SCRAPINVOICE_HEAD.shipment_Date,103) as DocDate,case when TSPL_Customer_Invoice_Head.Document_Type='D' then 'DBN' when TSPL_Customer_Invoice_Head.Document_Type ='C' then 'CRN' else 'INV' end as DocType ,'B2B' as SupTyp, 'N'  as IgstOnIntra,Bill_To_Location.GSTNO as SellerGSTINNo ,Bill_To_Location.location_desc as SellerLglNm,TSPL_COMPANY_MASTER.Comp_Name as SellerTrdNm,Bill_To_Location.Add1 as SellerAdd1,Bill_To_Location.Add2 as SellerAdd2 ,Bill_To_Location.city_code as SellerLoc,Bill_To_Location.Pin_Code  as SellerPincode,BillToLocation_State_Master.GST_STATE_Code as SellerStcd,Bill_To_Location.Phone1 as SellerPhone,Bill_To_Location.Email as SellerEmail,TSPL_Customer_master.GSTNo as BuyerGSTINNo ,TSPL_Customer_master.Customer_Name as BuyerLglNm,TSPL_Customer_master.alies_name as BuyerTrdNm,case when isnull(TSPL_SCRAPINVOICE_HEAD.ToLoc_Code,'')='' then Customer_State_Master.GST_STATE_Code else Ship_To_Location_State_Master.GST_STATE_Code end as BuyerPOS,TSPL_Customer_master.Add1 as BuyerAdd1,TSPL_Customer_master.Add2 as BuyerAdd2 ,tspl_city_master.City_Name as BuyerLoc,cast(TSPL_Customer_master.PIN_NO as int) as BuyerPincode,Customer_State_Master.GST_STATE_Code as BuyerStcd,TSPL_Customer_master.Phone1 as BuyerPhone,TSPL_Customer_master.Email as BuyerEmail,TSPL_SCRAPINVOICE_DETAIL.Line_No as ItemSlNo, 'N' as ItemIsServc,TSPL_ITEM_MASTER.Item_Desc AS ItemPrdDesc,TSPL_ITEM_MASTER.HSN_Code AS ItemHsnCd,TSPL_SCRAPINVOICE_DETAIL.invoice_Qty as ItemQty,TSPL_SCRAPINVOICE_DETAIL.Unit_code as ItemUnit,
 
 --TSPL_SCRAPINVOICE_DETAIL.price as ItemUnitPrice,
 case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (TSPL_SCRAPINVOICE_DETAIL.TAX3_Base_Amt / TSPL_SCRAPINVOICE_DETAIL.invoice_Qty) else TSPL_SCRAPINVOICE_DETAIL.price end as ItemUnitPrice,
  case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (TSPL_SCRAPINVOICE_DETAIL.TAX3_Base_Amt ) else TSPL_SCRAPINVOICE_DETAIL.ItemAmt end as ItemTotAmt,
TSPL_SCRAPINVOICE_DETAIL.DiscountAmt as ItemDiscount,
  case when ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax1,'')='KKF' or ISNULL(TSPL_SCRAPINVOICE_DETAIL.tax2,'')='KKF' then (TSPL_SCRAPINVOICE_DETAIL.TAX3_Base_Amt ) else TSPL_SCRAPINVOICE_DETAIL.ItemNetAmt end as ItemAssAmt,
--TSPL_SCRAPINVOICE_DETAIL.ItemNetAmt as ItemAssAmt,
