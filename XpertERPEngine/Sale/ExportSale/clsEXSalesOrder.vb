...
#Region "Variables"
    ...
    Public Commission_Payee_name As String = Nothing
    ...
    Public Arr_Notify As List(Of clsEXSalesOrderNotifyParty) = Nothing
    ...
    Public Cust_PO_Type As String = Nothing
    ...
    Public Arr As List(Of clsEXSalesOrderDetail) = Nothing
...
#End Region
    ...
    ''Note Very Important If any change made in SO Head or SO Detail table allso update it's History table.

    Public Shared Function IsVirtualLocation(ByVal LocCode As String) As Boolean
        Dim isTrue As Boolean = True
        Dim qry As Integer = CInt(clsDBFuncationality.getSingleValue("select count(*) from tspl_location_master where location_type='Virtual'"))
        If qry > 0 Then
            isTrue = True
        Else
            isTrue = False
    ...
    Public Function SaveData(ByVal obj As clsEXSalesOrder, ByVal isNewEntry As Boolean, ByVal isMakeAbandomentNo As Boolean) As Boolean
    ...
    Public Function SaveData(ByVal obj As clsEXSalesOrder, ByVal isNewEntry As Boolean, ByVal isMakeAbandomentNo As Boolean, ByVal trans As SqlTransaction) As Boolean
        Dim isSaved As Boolean = True
        Try
            ...
            qry = "delete from TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL where Document_Code='" + obj.Document_Code + "'"
            ...
            If isNewEntry Then
                If clsCommon.CompairString(obj.SalesOrder_Type, "EX") = CompairStringResult.Equal Then
                    If clsCommon.CompairString(obj.Item_Type, "F") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.EXSALESORDER, clsDocTransactionType.SNQuotationFinishedGoods, obj.Bill_To_Location)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "S") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.EXSALESORDER, clsDocTransactionType.POSemiFinishedGoods, obj.Bill_To_Location)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "R") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.EXSALESORDER, clsDocTransactionType.PORawMaterial, obj.Bill_To_Location)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "O") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.EXSALESORDER, clsDocTransactionType.SNQuotationOther, obj.Bill_To_Location)
                    Else
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.EXSALESORDER, clsDocTransactionType.SNOrderExport, obj.Bill_To_Location)
                    End If
                Else
                    If clsCommon.CompairString(obj.Item_Type, "F") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.MTSALESORDER, clsDocTransactionType.SNQuotationFinishedGoods, obj.Bill_To_Location)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "S") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.MTSALESORDER, clsDocTransactionType.POSemiFinishedGoods, obj.Bill_To_Location)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "R") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.MTSALESORDER, clsDocTransactionType.PORawMaterial, obj.Bill_To_Location)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "O") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.MTSALESORDER, clsDocTransactionType.SNQuotationOther, obj.Bill_To_Location)
                    Else
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, clsCommon.myCDate(obj.Document_Date), clsDocType.MTSALESORDER, clsDocTransactionType.SNOrderExport, obj.Bill_To_Location)
                    End If
                End If
            ...
            clsCommon.AddColumnsForChange(coll, "Cust_PO_Type", obj.Cust_PO_Type)
            ''richa agarwal 13/03/2015 
            If obj.Cust_Po_Date.HasValue Then
                clsCommon.AddColumnsForChange(coll, "Cust_PODate", clsCommon.GetPrintDate(obj.Cust_Po_Date, "dd/MMM/yyyy"), True)
            Else
            ...
            Else
                ...
                End If
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_SD_SALES_ORDER_HEAD", OMInsertOrUpdate.Update, "TSPL_SD_SALES_ORDER_HEAD.Document_Code='" + obj.Document_Code + "' and TSPL_SD_SALES_ORDER_HEAD.trans_type='EXP'", trans)
            End If

            isSaved = isSaved AndAlso clsEXSalesOrderDetail.SaveData(obj.Document_Code, Arr, trans)
            isSaved = isSaved AndAlso clsCustomFieldValues.SaveData(obj.Form_ID, obj.Document_Code, obj.arrCustomFields, trans)
            isSaved = isSaved AndAlso clsEXSalesOrderNotifyParty.SaveData(obj.Document_Code, obj.Arr_Notify, trans)
    ...
    Public Shared Function CancelData(ByVal Form_Id As String, ByVal Doc_No As String, ByVal arrLoc As String, ByVal NavType As NavigatorType, ByVal Export_Merchant As String) As Boolean
        ...
        Try
            '' table list 
            ''1. TSPL_SD_SALES_ORDER_HEAD
            ''2. TSPL_SD_SALES_ORDER_DETAIL
            ''3. TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL
            ''4.TSPL_CUSTOM_FIELD_VALUES

            '' steps for checking the items stock and batch wise stock

            Dim obj As clsEXSalesOrder = clsEXSalesOrder.GetData(Doc_No, NavType, arrLoc, Export_Merchant, trans)
            ...
            clsCommonFunctionality.SaveCancelData(objCommonVar.CurrentUserCode, Doc_No, "TSPL_SD_SALES_ORDER_HEAD", "Document_Code", "TSPL_SD_SALES_ORDER_DETAIL", "Document_Code", trans)
            clsCommonFunctionality.SaveCancelData(objCommonVar.CurrentUserCode, Doc_No, "TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL", "Document_Code", trans)
            ...
            qry = "delete from TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL where Document_Code='" & Doc_No & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_SD_SALES_ORDER_DETAIL where Document_Code='" & Doc_No & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_SD_SALES_ORDER_HEAD where Document_Code='" & Doc_No & "' and SalesOrder_Type='" & Export_Merchant & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
        ...
        Return True
    End Function
    Public Shared Function checkSaveNotification(ByVal obj As clsEXSalesOrder, ByVal trans As SqlTransaction)
        Try
    ...
    Public Shared Function GetTermsAndConditions() As String
        Dim str As String = clsDBFuncationality.getSingleValue("select top 1 Comments from TSPL_SD_SALES_ORDER_HEAD where Trans_Type='EXP' and ISNULL(comments,'')<>'' order by Document_Date desc")
    ...
    Public Shared Function GetData(ByVal strDocumentNo As String, ByVal NavType As NavigatorType, ByVal arrLoc As String, ByVal Export_Merchant As String) As clsEXSalesOrder
        Try
    ...
    Public Shared Function GetData(ByVal strPONo As String, ByVal NavType As NavigatorType, ByVal arrLoc As String, ByVal Export_Merchant As String, ByVal trans As SqlTransaction) As clsEXSalesOrder
        Dim obj As clsEXSalesOrder = Nothing
        Try
            Dim qry As String = "SELECT TSPL_SD_SALES_ORDER_HEAD.Is_Taxable,TSPL_SD_SALES_ORDER_HEAD.Auto_SaleOrder,TSPL_SD_SALES_ORDER_HEAD.Cust_PODate,tspl_vendor_master.vendor_name,TSPL_SD_SALES_ORDER_HEAD.Payment_Terms,TSPL_SD_SALES_ORDER_HEAD.Advance_Percentage,TSPL_SD_SALES_ORDER_HEAD.Commission_Paid,TSPL_SD_SALES_ORDER_HEAD.Commission_Amount,TSPL_SD_SALES_ORDER_HEAD.Comm_Amt_Type,TSPL_SD_SALES_ORDER_HEAD.Commission_Payee_Code,TSPL_SD_SALES_ORDER_HEAD.Commission_Instruction,TSPL_SD_SALES_ORDER_HEAD.EX_Term_Code,TSPL_SD_SALES_ORDER_HEAD.close_yn,TSPL_SD_SALES_ORDER_HEAD.cust_po_type,TSPL_SD_SALES_ORDER_HEAD.Cust_PO_No,TSPL_SD_SALES_ORDER_HEAD.price_group_code, TSPL_SD_SALES_ORDER_HEAD.Route_No,TSPL_SD_SALES_ORDER_HEAD.Route_Desc,TSPL_SD_SALES_ORDER_HEAD.Price_Code,TSPL_SD_SALES_ORDER_HEAD.Document_Code,TSPL_SD_SALES_ORDER_HEAD.SalesOrder_Type ,TSPL_SD_SALES_ORDER_HEAD.Document_Date, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Customer_Code,TSPL_CUSTOMER_MASTER.Customer_Name ,TSPL_SD_SALES_ORDER_HEAD.Status, " & _
        " TSPL_SD_SALES_ORDER_HEAD.On_Hold,TSPL_SD_SALES_ORDER_HEAD.Ref_No,TSPL_SD_SALES_ORDER_HEAD.Description,TSPL_SD_SALES_ORDER_HEAD.Remarks, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Tax_Group,TSPL_SD_SALES_ORDER_HEAD.Bill_To_Location,TSPL_SD_SALES_ORDER_HEAD.Ship_To_Location, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX1,TSPL_SD_SALES_ORDER_HEAD.TAX1_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX1_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX1_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX2,TSPL_SD_SALES_ORDER_HEAD.TAX2_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX2_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX2_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX3,TSPL_SD_SALES_ORDER_HEAD.TAX3_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX3_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX3_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX4,TSPL_SD_SALES_ORDER_HEAD.TAX4_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX4_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX4_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX5,TSPL_SD_SALES_ORDER_HEAD.TAX5_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX5_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX5_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX6,TSPL_SD_SALES_ORDER_HEAD.TAX6_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX6_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX6_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX7,TSPL_SD_SALES_ORDER_HEAD.TAX7_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX7_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX7_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX8,TSPL_SD_SALES_ORDER_HEAD.TAX8_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX8_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX8_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX9,TSPL_SD_SALES_ORDER_HEAD.TAX9_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX9_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX9_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.TAX10,TSPL_SD_SALES_ORDER_HEAD.TAX10_Rate,TSPL_SD_SALES_ORDER_HEAD.TAX10_Amt,TSPL_SD_SALES_ORDER_HEAD.TAX10_Base_Amt, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Discount_Base,TSPL_SD_SALES_ORDER_HEAD.Discount_Amt,TSPL_SD_SALES_ORDER_HEAD.Amount_Less_Discount, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Total_Tax_Amt,TSPL_SD_SALES_ORDER_HEAD.Total_Amt,TSPL_SD_SALES_ORDER_HEAD.Mode_Of_Transport," & _
        " TSPL_SD_SALES_ORDER_HEAD.Comments,TSPL_SD_SALES_ORDER_HEAD.Comp_Code,TSPL_SD_SALES_ORDER_HEAD.Terms_Code,TSPL_SD_SALES_ORDER_HEAD.Due_Date , " & _
        " TSPL_LOCATION_MASTER.Location_Desc as BillToLocationName,TSPL_SHIP_TO_LOCATION.Ship_To_Desc as ShipToLocationName, " & _
        " TSPL_TAX_GROUP_MASTER.Tax_Group_Desc as TaxGroupName,TSPL_TERMS_MASTER.Terms_Desc as TermsName,TSPL_SD_SALES_ORDER_HEAD.Posting_Date, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Delivery_date,TSPL_SD_SALES_ORDER_HEAD.Dept,TSPL_SD_SALES_ORDER_HEAD.Dept_Desc,TSPL_SD_SALES_ORDER_HEAD.Item_Type, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Modify_By,TSPL_SD_SALES_ORDER_HEAD.Modify_Date,TSPL_SD_SALES_ORDER_HEAD.Created_By," & _
        " TSPL_SD_SALES_ORDER_HEAD.Created_Date,TSPL_SD_SALES_ORDER_HEAD.Abandonment_No,TSPL_SD_SALES_ORDER_HEAD.Against_Quotation_No, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code1,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name1,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt1, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code2,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name2,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt2, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code3,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name3,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt3, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code4,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name4,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt4, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code5,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name5,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt5, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code6,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name6,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt6, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code7,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name7,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt7, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code8,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name8,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt8, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code9 ,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name9,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt9 ," & _
        " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Code10 ,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name10,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt10, " & _
        " TSPL_SD_SALES_ORDER_HEAD.Total_Add_Charge,TSPL_SD_SALES_ORDER_HEAD.Salesman_Code,TSPL_SD_SALES_ORDER_HEAD.Salesman_Name, " & _
        " TSPL_SD_SALES_ORDER_HEAD.CURRENCY_CODE,TSPL_SD_SALES_ORDER_HEAD.CONVRATE,TSPL_SD_SALES_ORDER_HEAD.APPLICABLEFROM ,TSPL_SD_SALES_ORDER_HEAD.PROJECT_ID,TSPL_SD_SALES_ORDER_HEAD.Approvel_Required,TSPL_SD_SALES_ORDER_HEAD.Is_Approved" & _
        " ,TSPL_SD_SALES_ORDER_HEAD.HeadDisc_Per,TSPL_SD_SALES_ORDER_HEAD.HeadDisc_PerAmt,TSPL_SD_SALES_ORDER_HEAD.HeadDisc_Amt,TSPL_SD_SALES_ORDER_HEAD.TotCashDiscAmt,TSPL_SD_SALES_ORDER_HEAD.CloseRemarks FROM TSPL_SD_SALES_ORDER_HEAD left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=TSPL_SD_SALES_ORDER_HEAD.Bill_To_Location " & _
        " left outer join TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code=TSPL_SD_SALES_ORDER_HEAD.Ship_To_Location left outer join  " & _
        " TSPL_TAX_GROUP_MASTER on TSPL_TAX_GROUP_MASTER.Tax_Group_Code= TSPL_SD_SALES_ORDER_HEAD.Tax_Group left " & _
        " outer join TSPL_TERMS_MASTER on TSPL_TERMS_MASTER.Terms_Code=TSPL_SD_SALES_ORDER_HEAD.Terms_Code left " & _
        " outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALES_ORDER_HEAD.Customer_Code left outer join tspl_vendor_master on tspl_vendor_master.vendor_code=TSPL_SD_SALES_ORDER_HEAD.Commission_Payee_Code  where 2=2 and TSPL_SD_SALES_ORDER_HEAD.trans_type='EXP' and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + Export_Merchant + "' "
            ...
            Select Case NavType
                Case NavigatorType.First
                    qry += " and TSPL_SD_SALES_ORDER_HEAD.Document_Code = (select MIN(Document_Code) from TSPL_SD_SALES_ORDER_HEAD where 1=1 and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + Export_Merchant + "'  and trans_type='EXP' " + whrClas + ")"
                Case NavigatorType.Last
                    qry += " and TSPL_SD_SALES_ORDER_HEAD.Document_Code = (select Max(Document_Code) from TSPL_SD_SALES_ORDER_HEAD where 1=1 and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + Export_Merchant + "' and trans_type='EXP' " + whrClas + ")"
                Case NavigatorType.Next
                    qry += " and TSPL_SD_SALES_ORDER_HEAD.Document_Code = (select Min(Document_Code) from TSPL_SD_SALES_ORDER_HEAD where Document_Code>'" + strPONo + "' and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + Export_Merchant + "' and trans_type='EXP' " + whrClas + ")"
                Case NavigatorType.Previous
                    qry += " and TSPL_SD_SALES_ORDER_HEAD.Document_Code = (select Max(Document_Code) from TSPL_SD_SALES_ORDER_HEAD where Document_Code<'" + strPONo + "' and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + Export_Merchant + "' and trans_type='EXP' " + whrClas + ")"
            ...
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                ...
                obj.Auto_SaleOrder = clsCommon.myCdbl(dt.Rows(0)("Auto_SaleOrder"))
                ...
                obj.Cust_PO_Type = clsCommon.myCstr(dt.Rows(0)("cust_po_type"))
                If dt.Rows(0)("Cust_PODate") Is DBNull.Value Then
                    obj.Cust_Po_Date = clsCommon.GETSERVERDATE(trans)
                Else
                    obj.Cust_Po_Date = clsCommon.myCDate(dt.Rows(0)("Cust_PODate"))
                End If
                ...
                obj.Commission_Payee_name = clsCommon.myCstr(dt.Rows(0)("vendor_name"))
                ...
                End If
                '' END CURRENCYCONVERSION 
                qry = "SELECT  TSPL_SD_SALES_ORDER_DETAIL.cust_po_no,TSPL_SD_SALES_ORDER_DETAIL.Document_Code,TSPL_SD_SALES_ORDER_DETAIL.Line_No,TSPL_SD_SALES_ORDER_DETAIL.Status, " & _
                ...
                "FROM TSPL_SD_SALES_ORDER_DETAIL left outer join TSPL_LOCATION_MASTER on " & _
                ...
                If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                    ...
                    Dim objTr As clsEXSalesOrderDetail
                    For Each dr As DataRow In dt.Rows
                        ...
                        objTr.PurchaseOrder_No = clsCommon.myCstr(dr("cust_po_no"))
                ...
                qry = "select TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL.*,tspl_customer_master.customer_name,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add1,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add2,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add3,TSPL_NOTIFY_PARTY_SHIP_DETAIL.country_code,TSPL_NOTIFY_PARTY_SHIP_DETAIL.city_code,TSPL_NOTIFY_PARTY_SHIP_DETAIL.state_code from TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL left outer join TSPL_NOTIFY_PARTY_SHIP_DETAIL on TSPL_NOTIFY_PARTY_SHIP_DETAIL.cust_code=TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL.cust_code left outer join tspl_customer_master on tspl_customer_master.cust_code=TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL.cust_code where document_code='" + obj.Document_Code + "'"
    ...
    Public Shared Function PostData(ByVal FormId As String, ByVal strDocNo As String, ByVal arrLoc As String, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            Dim obj As clsEXSalesOrder = clsEXSalesOrder.GetData(strDocNo, NavigatorType.Current, arrLoc, IIf(clsCommon.CompairString(FormId, clsUserMgtCode.frmEXSalesOrder) = CompairStringResult.Equal, "EX", "MT"), trans)
            ...
            qry = "Update TSPL_SD_SALES_ORDER_HEAD set Status=1, Posting_Date='" + strPostDate + "',Modify_By='" + objCommonVar.CurrentUserCode + "', Modify_Date='" + strPostDate + "' where Document_Code='" + strDocNo + "' and trans_type='EXP'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Function DeleteData(ByVal Formid As String, ByVal strCode As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            qry = "delete from TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL where Document_Code='" + strCode + "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_SD_SALES_ORDER_HEAD where Document_Code='" + strCode + "' and trans_type='EXP'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Function IsValidCustomer(ByVal ArrPONo As List(Of String), ByVal strVendorCode As String) As Boolean
        If ArrPONo IsNot Nothing AndAlso ArrPONo.Count > 0 Then
            Dim qry As String = "select TSPL_SD_SALES_ORDER_HEAD.Document_Code,TSPL_SD_SALES_ORDER_HEAD.Customer_Code,TSPL_CUSTOMER_MASTER.Customer_Name from TSPL_SD_SALES_ORDER_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALES_ORDER_HEAD.Customer_Code where trans_type='EXP' and Document_Code  in (" + clsCommon.GetMulcallString(ArrPONo) + ") and Customer_Code not in ('" + strVendorCode + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim msg As String = "Error. "
                For Each dr As DataRow In dt.Rows
                    msg += Environment.NewLine + "Sales Order No:" + clsCommon.myCstr(dr("Document_Code")) + " Is For Buyer Code: " + clsCommon.myCstr(dr("Customer_Code")) + " Buyer Name:" + clsCommon.myCstr(dr("Customer_Name"))
                Next
    ...
    Public Shared Function IsValidDocumentType(ByVal ArrPONo As List(Of String), ByVal strDoctype As String) As Boolean
        If ArrPONo IsNot Nothing AndAlso ArrPONo.Count > 0 Then
            Dim qry As String = "select TSPL_SD_SALES_ORDER_HEAD.Document_Code,(case when salesorder_type='MT' then 'Merchant Trade' else 'Export Sale' end) as Customer_Code from TSPL_SD_SALES_ORDER_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALES_ORDER_HEAD.Customer_Code where trans_type='EXP' and Document_Code  in (" + clsCommon.GetMulcallString(ArrPONo) + ") and salesorder_type not in ('" + strDoctype + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim msg As String = "Error. "
                For Each dr As DataRow In dt.Rows
                    msg += Environment.NewLine + "Sales Order No:" + clsCommon.myCstr(dr("Document_Code")) + " Is For Document Type: " + clsCommon.myCstr(dr("Customer_Code")) + ""
                Next
                Throw New Exception(msg)
            End If
        End If
    ...
    Public Shared Function IsValidTaxGroupForPO(ByVal ArrPONo As List(Of String), ByVal strTaxGroupCode As String) As Boolean
        If ArrPONo IsNot Nothing AndAlso ArrPONo.Count > 0 Then
            Dim qry As String = "select Document_Code,Tax_Group from TSPL_SD_SALES_ORDER_HEAD where trans_type='EXP' and Document_Code  in (" + clsCommon.GetMulcallString(ArrPONo) + ") and Tax_Group not in ('" + strTaxGroupCode + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim msg As String = "Error. "
                For Each dr As DataRow In dt.Rows
    ...
    Public Shared Function GetBaseCurrencyCode() As String
        Dim strCurr As String = ""
        strCurr = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select basecurrencycode from tspl_company_master where comp_code='" + objCommonVar.CurrentCompanyCode + "'"))

        Return strCurr
    End Function

    Public Shared Function GetCurrencyRate(ByVal strCurr_Code As String) As Double
        Dim Curr_Rate As Double = 0
        Curr_Rate = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select rate from TSPL_CURRENCY_CONVERSION_RATE where to_currency='" + strCurr_Code + "'"))

        Return Curr_Rate
    End Function

    Public Shared Function GetCurrencyApplydate(ByVal strCurr_Code As String, ByVal strDocDate As Date) As String
        Dim Curr_Rate As String = ""
        Curr_Rate = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select convert(varchar,from_date,103) as frmdate from TSPL_CURRENCY_CONVERSION_RATE where to_currency='" + strCurr_Code + "'  and ('" + clsCommon.GetPrintDate(strDocDate, "dd/MMM/yyyy") + "' between from_date and to_date)"))

        Return Curr_Rate
    End Function

    Public Shared Function IsValidProjectForSO(ByVal strPONo As String, ByVal strProject As String) As String
        Dim strProj As String = clsDBFuncationality.getSingleValue("select PROJECT_ID from TSPL_SD_SALES_ORDER_HEAD where trans_type='EXP' and Document_Code ='" + strPONo + "'")
        Return strProj
    End Function

    Public Shared Function ClosedData(ByVal obj As clsEXSalesOrder, ByVal strCode As String, ByVal strCloseRemarks As String) As Boolean
        Try
            Dim qry As String = "update TSPL_SD_SALES_ORDER_HEAD set close_yn='" + obj.CloseSO + "',CloseRemarks='" + strCloseRemarks + "' where document_code='" + strCode + "' and trans_type='EXP'"
...
Public Class clsEXSalesOrderDetail
#Region "Variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal Arr As List(Of clsEXSalesOrderDetail), ByVal trans As SqlTransaction) As Boolean

        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsEXSalesOrderDetail In Arr
                Dim coll As New Hashtable()
                ...
                clsCommon.AddColumnsForChange(coll, "Cust_PO_NO", obj.PurchaseOrder_No)
        ...
        Return True
    End Function
    Public Shared Function DemoSaveData(ByVal strDocNo As String, ByVal Arr As List(Of clsEXSalesOrderDetail), ByVal trans As SqlTransaction, ByVal strDatabase As String) As Boolean
...
Public Class clsEXSalesOrderHist
#Region "Variables"
#End Region
    ...
    Public Shared Function SaveData(ByVal strCode As String, ByVal intAmbandentNo As Integer, ByVal trans As SqlTransaction) As Boolean
        Dim isSaved As Boolean = True
        Try
            Dim obj As clsEXSalesOrder = clsEXSalesOrder.GetData(strCode, NavigatorType.Current, Nothing, "EX", trans)
            Dim coll As New Hashtable()
...
Public Class clsEXSalesOrderNotifyParty
#Region "variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDoc As String, ByVal Arr As List(Of clsEXSalesOrderNotifyParty), ByVal trans As SqlTransaction) As Boolean
        Try
            Dim isSaved As Boolean = True

            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery("delete from TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL where document_code='" + strDoc + "'", trans)

            If Arr IsNot Nothing AndAlso Arr.Count > 0 Then
                For Each objtr As clsEXSalesOrderNotifyParty In Arr
                    Dim coll As New Hashtable()
                    ...
                    isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_SD_SALES_ORDER_NOTIFY_PARTY_DETAIL", OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
