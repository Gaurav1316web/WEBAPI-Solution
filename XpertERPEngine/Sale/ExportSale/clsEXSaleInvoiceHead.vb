...
Public Class clsEXSaleInvoiceHead
...
#Region "Variables"
    ...
    Public Arr As List(Of clsEXSaleInvoiceDetail) = Nothing
    ...
    Public ExportInvoiceType As String = Nothing
    Public BRANCH_CODE As String = Nothing
    Public BANK_CODE As String = Nothing
    Public Arr_Notify As List(Of clsEXSaleInvoiceNotifyDetail) = Nothing
    Public LUT_No As String = Nothing
#End Region
    Public Shared Function GetProformaDescrptn() As String
        Dim str As String = ""
        str = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select top 1 comments from TSPL_SD_SALE_INVOICE_HEAD where isnull(comments,'')<>'' and trans_type='EXP' order by document_date desc"))
    ...
    Public Shared Function GetProformaIncentiveDescrptn() As String
        Dim str As String = ""
        str = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select top 1 Incentive_Declaration from TSPL_SD_SALE_INVOICE_HEAD where isnull(Incentive_Declaration,'')<>'' and trans_type='EXP' order by document_date desc"))

        Return str
    End Function

    Public Function SaveData(ByVal obj As clsEXSaleInvoiceHead, ByVal isNewEntry As Boolean) As Boolean
    ...
    Public Function SaveData(ByVal obj As clsEXSaleInvoiceHead, ByVal isNewEntry As Boolean, ByVal trans As SqlTransaction) As Boolean
        Dim isSaved As Boolean = True
        ...
        Try
            ...
            qry = "delete from TSPL_EX_sale_INVOICE_NOTIFY_PARTY_DETAIL where Document_Code='" + obj.Document_Code + "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
            ...
            If isNewEntry Then
                If clsCommon.CompairString(obj.Document_Type, "EX") = CompairStringResult.Equal Then
                    If clsCommon.CompairString(obj.Item_Type, "F") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.EXSALESINVOICE, clsDocTransactionType.SNQuotationFinishedGoods, obj.Bill_To_Location, False)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "S") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.EXSALESINVOICE, clsDocTransactionType.POSemiFinishedGoods, obj.Bill_To_Location, False)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "R") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.EXSALESINVOICE, clsDocTransactionType.PORawMaterial, obj.Bill_To_Location, False)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "O") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.EXSALESINVOICE, clsDocTransactionType.SNQuotationOther, obj.Bill_To_Location, False)
                    Else
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.EXSALESINVOICE, clsDocTransactionType.SNOrderExport, obj.Bill_To_Location, False)
                    End If
                ElseIf clsCommon.CompairString(obj.Document_Type, "MT") = CompairStringResult.Equal Then
                    If clsCommon.CompairString(obj.Item_Type, "F") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.MTSALESINVOICE, clsDocTransactionType.SNQuotationFinishedGoods, obj.Bill_To_Location, False)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "S") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.MTSALESINVOICE, clsDocTransactionType.POSemiFinishedGoods, obj.Bill_To_Location, False)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "R") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.MTSALESINVOICE, clsDocTransactionType.PORawMaterial, obj.Bill_To_Location, False)
                    ElseIf clsCommon.CompairString(obj.Item_Type, "O") = CompairStringResult.Equal Then
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.MTSALESINVOICE, clsDocTransactionType.SNQuotationOther, obj.Bill_To_Location, False)
                    Else
                        obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.MTSALESINVOICE, clsDocTransactionType.SNOrderExport, obj.Bill_To_Location, False)
                    End If
                End If
            End If
            ...
            End If
            clsCommon.AddColumnsForChange(coll, "ExportInvoiceType", obj.ExportInvoiceType)
            ...
            clsCommon.AddColumnsForChange(coll, "Import_Export_No", obj.Import_Export_No)
            ...
            clsCommon.AddColumnsForChange(coll, "Against_Com_Inv_No", obj.Against_Com_Inv_No, True)
            ...
            End If
            clsCommon.AddColumnsForChange(coll, "WayBillNo", obj.WayBillNo, True)
            ...
            End If
            clsCommon.AddColumnsForChange(coll, "LUT_No", obj.LUT_No)
            If isNewEntry Then
            ...
            Else
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_SD_SALE_INVOICE_HEAD", OMInsertOrUpdate.Update, " TSPL_SD_SALE_INVOICE_HEAD.Document_Code='" + obj.Document_Code + "' and TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP'", trans)
            End If
            isSaved = isSaved AndAlso clsEXSaleInvoiceDetail.SaveData(obj.Document_Code, Arr, trans, obj)
            isSaved = isSaved AndAlso clsCustomFieldValues.SaveData(obj.Form_ID, obj.Document_Code, obj.arrCustomFields, trans)
            isSaved = isSaved AndAlso clsEXSaleInvoiceNotifyDetail.SaveData(obj.Document_Code, obj.Arr_Notify, trans)
    ...
    Public Shared Function CancelData(ByVal Form_Id As String, ByVal Doc_No As String, ByVal arrLoc As String, ByVal NavType As NavigatorType, ByVal Export_Merchant As String) As Boolean
        '' created by Richa Agarwal against ticket No- TEC/23/05/18-000252 on date 05-06-2018
        Dim qry As String = ""
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
            '' table list 
            ...
            Dim obj As clsEXSaleInvoiceHead = clsEXSaleInvoiceHead.GetData(Doc_No, NavType, "'T','R'", arrLoc, Export_Merchant, trans)
            'Dim obj As clsEXSaleInvoiceHead = clsEXSaleInvoiceHead.GetData(Doc_No, "'T','R'", arrLoc, NavType, Export_Merchant, trans)
            ...
            ''richa agarwal 11 Jan,2021
            Dim dtirn As DataTable = clsDBFuncationality.GetDataTable("select Einvoice_type,IRN_No,Is_Taxable,Bill_To_Location from TSPL_SD_SALE_INVOICE_HEAD where document_code='" & Doc_No & "'", trans)
            ...
            clsCommonFunctionality.SaveCancelData(objCommonVar.CurrentUserCode, Doc_No, "TSPL_SD_SALE_INVOICE_HEAD", "Document_Code", "TSPL_SD_SALE_INVOICE_DETAIL", "Document_Code", trans)
            clsCommonFunctionality.SaveCancelData(objCommonVar.CurrentUserCode, Doc_No, "TSPL_EX_SALE_INVOICE_NOTIFY_PARTY_DETAIL", "Document_Code", trans)

            '' cancel customer invoice data
            qry = "select Document_No from TSPL_Customer_Invoice_Head  where against_Sale_no='" & Doc_No & "'"
            ...
            '' cancel journal master data
            qry = "select Voucher_No from TSPL_JOURNAL_MASTER  where Source_Doc_No in (select Document_No from TSPL_Customer_Invoice_Head  where against_Sale_no='" & Doc_No & "')"
            ...
            qry = "delete from TSPL_JOURNAL_DETAILS where Voucher_No in (select Voucher_No from TSPL_JOURNAL_MASTER where Source_Doc_No in (select Document_No from TSPL_Customer_Invoice_Head  where against_Sale_no='" & Doc_No & "'))"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JOURNAL_MASTER where Source_Doc_No in (select Document_No from TSPL_Customer_Invoice_Head  where against_Sale_no='" & Doc_No & "')"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_Customer_Invoice_Detail where Document_No in (Select Document_No from TSPL_Customer_Invoice_Head  where against_Sale_no='" & Doc_No & "')"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_Customer_Invoice_Head where against_Sale_no='" & Doc_No & "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" & Doc_No & "' and Trans_Type='" & Form_Id & "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_BATCH_ITEM where  Document_Code='" & Doc_No & "' and Document_Type='" & strInvTransType & "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_CUSTOM_FIELD_VALUES where Transaction_Code='" & Doc_No & "' and Program_Code='" & Form_Id & "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_EX_SALE_INVOICE_NOTIFY_PARTY_DETAIL where Document_Code='" & Doc_No & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_SD_SALE_INVOICE_DETAIL where Document_Code='" & Doc_No & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_SD_SALE_INVOICE_HEAD where Document_Code='" & Doc_No & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Function GetData(ByVal strPONo As String, ByVal NavType As NavigatorType, ByVal strInvoiceType As String, ByVal arrLoc As String, ByVal Export_Merchant As String, ByVal trans As SqlTransaction) As clsEXSaleInvoiceHead
        Try
            ...
            qry += "TSPL_SD_SALE_INVOICE_HEAD.Exporter_Ref_No,TSPL_SD_SALE_INVOICE_HEAD.Pre_Carriage_By,TSPL_SD_SALE_INVOICE_HEAD.Discharge_Port,TSPL_SD_SALE_INVOICE_HEAD.Final_Destination,TSPL_SD_SALE_INVOICE_HEAD.Origin_Country,TSPL_SD_SALE_INVOICE_HEAD.Final_Destination_Country,"
            qry += "TSPL_SD_SALE_INVOICE_HEAD.Loading_Port,TSPL_SD_SALE_INVOICE_HEAD.Vessel_Flight_No,TSPL_SD_SALE_INVOICE_HEAD.CHA_Code,TSPL_SD_SALE_INVOICE_HEAD.CHA_Charge_Amount,TSPL_SD_SALE_INVOICE_HEAD.CHA_FOB_Amount,TSPL_SD_SALE_INVOICE_HEAD.CHA_Frieght_Kg_Amount,TSPL_SD_SALE_INVOICE_HEAD.Final_Gross_Wt,TSPL_SD_SALE_INVOICE_HEAD.Gross_Wt,TSPL_SD_SALE_INVOICE_HEAD.CHA_Basic_Freight_Amount,TSPL_SD_SALE_INVOICE_HEAD.BANK_CODE,TSPL_SD_SALE_INVOICE_HEAD.BRANCH_CODE,TSPL_SD_SALE_INVOICE_HEAD.LUT_No"
            ...
            qry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALE_INVOICE_HEAD.Customer_Code where 2=2 and TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP' and TSPL_SD_SALE_INVOICE_HEAD.document_type='" + Export_Merchant + "' "
            ...
            If clsCommon.myLen(arrLoc) > 0 And clsCommon.myLen(strwherecls) > 0 Then
                whrCls = " AND Bill_To_Location in (" + arrLoc + ") and TSPL_SD_SALE_INVOICE_HEAD.Customer_Code in (" + strwherecls + ") and TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP' "
            ElseIf clsCommon.myLen(arrLoc) > 0 Then
                whrCls = " AND Bill_To_Location in (" + arrLoc + ") and TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP'"
            ElseIf clsCommon.myLen(strwherecls) > 0 Then
                whrCls = " AND TSPL_SD_SALE_INVOICE_HEAD.Customer_Code in (" + strwherecls + ") and TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP' "
            End If
            '-----------------------------------------------------
            Select Case NavType
                Case NavigatorType.First
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.Document_Code = (select MIN(Document_Code) from TSPL_SD_SALE_INVOICE_HEAD WHERE TSPL_SD_SALE_INVOICE_HEAD.document_type='" + Export_Merchant + "' " + whrCls + ")"
                Case NavigatorType.Last
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.Document_Code = (select Max(Document_Code) from TSPL_SD_SALE_INVOICE_HEAD WHERE TSPL_SD_SALE_INVOICE_HEAD.document_type='" + Export_Merchant + "' " + whrCls + ")"
                Case NavigatorType.Current
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.Document_Code = '" + strPONo + "'"
                Case NavigatorType.Next
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.Document_Code = (select Min(Document_Code) from TSPL_SD_SALE_INVOICE_HEAD where Document_Code>'" + strPONo + "' and TSPL_SD_SALE_INVOICE_HEAD.document_type='" + Export_Merchant + "' " + whrCls + ")"
                Case NavigatorType.Previous
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.Document_Code = (select Max(Document_Code) from TSPL_SD_SALE_INVOICE_HEAD where Document_Code<'" + strPONo + "' and TSPL_SD_SALE_INVOICE_HEAD.document_type='" + Export_Merchant + "' " + whrCls + ")"
            End Select
            ...
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                ...
                If clsCommon.myLen((dt.Rows(0)("MT_Against_PO_Date"))) > 0 AndAlso Not (dt.Rows(0)("MT_Against_PO_Date")) Is DBNull.Value Then
                    obj.MT_Against_PO_Date = clsCommon.myCDate(dt.Rows(0)("MT_Against_PO_Date"))
                End If
                obj.ExportInvoiceType = clsCommon.myCstr(dt.Rows(0)("ExportInvoiceType"))
                ...
                obj.Import_Export_No = clsCommon.myCstr(dt.Rows(0)("Import_Export_No"))
                ...
                obj.Against_Com_Inv_No = clsCommon.myCstr(dt.Rows(0)("Against_Com_Inv_No"))
                ...
                obj.LUT_No = clsCommon.myCstr(dt.Rows(0)("LUT_No"))

                qry = "SELECT TSPL_SD_SALE_INVOICE_detail.No_Kind_Package,TSPL_SD_SALE_INVOICE_detail.Container_No,TSPL_SD_SALE_INVOICE_detail.Shipping_Mark,TSPL_SD_SALE_INVOICE_detail.Is_Mannual_Amt,TSPL_SD_SALE_INVOICE_detail.Document_Code,TSPL_SD_SALE_INVOICE_detail.Line_No, " & _
                ...
                "TSPL_SD_SALE_INVOICE_detail.Free_Qty,TSPL_SD_SALE_INVOICE_detail.Commercial_Inv_No as PI_CODE, " & _
                ...
                If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                    ...
                    Dim objTr As clsEXSaleInvoiceDetail
                    For Each dr As DataRow In dt.Rows
                        objTr = New clsEXSaleInvoiceDetail
                        ...
                        Else
                            strInvTransType = "EX_SALE_IN"
                        End If
                        objTr.arrBatchItem = clsBatchInventory.GetData(strInvTransType, objTr.Document_Code, objTr.Item_Code, objTr.Line_No, trans)
                        obj.Arr.Add(objTr)
                    Next
                End If

                qry = "select TSPL_EX_Sale_INVOICE_NOTIFY_PARTY_DETAIL.*,tspl_customer_master.customer_name,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add1,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add2,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add3,TSPL_NOTIFY_PARTY_SHIP_DETAIL.country_code,TSPL_NOTIFY_PARTY_SHIP_DETAIL.city_code,TSPL_NOTIFY_PARTY_SHIP_DETAIL.state_code from TSPL_EX_Sale_INVOICE_NOTIFY_PARTY_DETAIL left outer join TSPL_NOTIFY_PARTY_SHIP_DETAIL on TSPL_NOTIFY_PARTY_SHIP_DETAIL.cust_code=TSPL_EX_Sale_INVOICE_NOTIFY_PARTY_DETAIL.cust_code left outer join tspl_customer_master on tspl_customer_master.cust_code=TSPL_EX_Sale_INVOICE_NOTIFY_PARTY_DETAIL.cust_code where document_code='" + obj.Document_Code + "'"
    ...
    Public Shared Function EInvoice_Implementation(ByVal strDocNo As String, ByVal strLocation As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            If clsCommon.myLen(strtoken) > 0 Then
                Dim strQry As String = "select TSPL_Customer_master .Cust_Code ,TSPL_SD_SALE_INVOICE_HEAD.Document_Code as DocNo,convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) as DocDate,case when TSPL_Customer_Invoice_Head.Document_Type='D' then 'DBN' when TSPL_Customer_Invoice_Head.Document_Type ='C' then 'CRN' else 'INV' end as DocType ,case when TSPL_SD_SALE_INVOICE_HEAD.Total_Amt> 0 then 'EXPWP'  else 'EXPWOP' end as SupTyp, 'N'  as IgstOnIntra,Bill_To_Location.GSTNO as SellerGSTINNo ,Bill_To_Location.location_desc as SellerLglNm,TSPL_COMPANY_MASTER.Comp_Name as SellerTrdNm,Bill_To_Location.Add1 as SellerAdd1,Bill_To_Location.Add2 as SellerAdd2 ,Bill_To_Location.City_Code  as SellerLoc,Bill_To_Location.Pin_Code  as SellerPincode,BillToLocation_State_Master.GST_STATE_Code as SellerStcd,Bill_To_Location.Phone1 as SellerPhone,Bill_To_Location.Email as SellerEmail,'URP' as BuyerGSTINNo ,TSPL_Customer_master.Customer_Name as BuyerLglNm,TSPL_Customer_master.Alies_name as BuyerTrdNm,96 as BuyerPOS,TSPL_Customer_master.Add1 as BuyerAdd1,TSPL_Customer_master.Add2 as BuyerAdd2 ,tspl_city_master.City_Name as BuyerLoc,999999 as BuyerPincode,96 as BuyerStcd,TSPL_Customer_master.Phone1 as BuyerPhone,TSPL_Customer_master.Email as BuyerEmail,TSPL_SD_SALE_INVOICE_DETAIL.Line_No as ItemSlNo,case when isnull(TSPL_SD_SALE_INVOICE_DETAIL.Row_Type,'') ='Item' then 'N' else 'Y' end as ItemIsServc,COAlESCE(TSPL_ITEM_MASTER.Item_Desc,TSPL_Additional_Charges.DESCRIPTION ) AS ItemPrdDesc,COAlESCE(TSPL_ITEM_MASTER.HSN_Code,TSPL_Additional_Charges.SAC_CODE) AS ItemHsnCd,TSPL_SD_SALE_INVOICE_DETAIL.Qty as ItemQty, TSPL_SD_SALE_INVOICE_DETAIL.Unit_code as ItemUnit,TSPL_SD_SALE_INVOICE_DETAIL.Item_Cost as ItemUnitPrice,TSPL_SD_SALE_INVOICE_DETAIL.Amount as ItemTotAmt,TSPL_SD_SALE_INVOICE_DETAIL.total_disc_amt as ItemDiscount,TSPL_SD_SALE_INVOICE_DETAIL.Amt_Less_Discount as ItemAssAmt,case when ISNULL(TSPL_SD_SALE_INVOICE_DETAIL .tax1,'') ='IGST' THEN TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Rate when ISNULL(TSPL_SD_SALE_INVOICE_DETAIL .tax1,'') ='CGST' AND   ISNULL(TSPL_SD_SALE_INVOICE_DETAIL .tax2,'') ='SGST'  THEN TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Rate+TSPL_SD_SALE_INVOICE_DETAIL.TAX2_Rate  ELSE 0 end as ItemGstRt, case when TSPL_SD_SALE_INVOICE_DETAIL .TAX1 ='SGST' AND TSPL_SD_SALE_INVOICE_DETAIL .TAX2  ='CGST' then TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Amt when TSPL_SD_SALE_INVOICE_DETAIL .TAX1 ='CGST' AND TSPL_SD_SALE_INVOICE_DETAIL .TAX2  ='SGST' then TSPL_SD_SALE_INVOICE_DETAIL.TAX2_Amt else 0 end ItemSgstAmt,case when TSPL_SD_SALE_INVOICE_DETAIL .TAX1 ='IGST' then TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Amt else 0 end ItemIgstAmt,
case when TSPL_SD_SALE_INVOICE_DETAIL .TAX1 ='CGST' AND TSPL_SD_SALE_INVOICE_DETAIL .TAX2  ='SGST' then TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Amt when TSPL_SD_SALE_INVOICE_DETAIL .TAX1 ='SGST' AND TSPL_SD_SALE_INVOICE_DETAIL .TAX2  ='CGST' then TSPL_SD_SALE_INVOICE_DETAIL.TAX2_Amt else 0 end ItemCgstAmt,0 as ItemOthChrg,TSPL_SD_SALE_INVOICE_DETAIL.Item_Net_Amt-case when isnull(TCS1.is_tcs,'')='Y' THEN  TSPL_SD_SALE_INVOICE_DETAIL.TAX2_AMT when isnull(TCS2.is_tcs,'')='Y' THEN  TSPL_SD_SALE_INVOICE_DETAIL.TAX3_AMT ELSE 0 END as ItemTotItemVal,TSPL_SD_SALE_INVOICE_HEAD .Amount_Less_Discount as ValDtlsAssVal,case when TSPL_SD_SALE_INVOICE_HEAD .TAX1 ='CGST' AND TSPL_SD_SALE_INVOICE_HEAD .TAX2  ='SGST' then TSPL_SD_SALE_INVOICE_HEAD.TAX1_Amt when TSPL_SD_SALE_INVOICE_HEAD .TAX1 ='SGST' AND TSPL_SD_SALE_INVOICE_HEAD .TAX2  ='CGST' then TSPL_SD_SALE_INVOICE_HEAD.TAX2_Amt else 0 end ValDtlsCgstVal, case when TSPL_SD_SALE_INVOICE_HEAD .TAX1 ='SGST' AND TSPL_SD_SALE_INVOICE_HEAD .TAX2  ='CGST' then TSPL_SD_SALE_INVOICE_HEAD.TAX1_Amt when TSPL_SD_SALE_INVOICE_HEAD .TAX1 ='CGST' AND TSPL_SD_SALE_INVOICE_HEAD .TAX2  ='SGST' then TSPL_SD_SALE_INVOICE_HEAD.TAX2_Amt else 0 end ValDtlsSgstVal,case when TSPL_SD_SALE_INVOICE_HEAD .TAX1 ='IGST' then TSPL_SD_SALE_INVOICE_HEAD.TAX1_Amt else 0 end ValDtlsIgstVal,TSPL_SD_SALE_INVOICE_HEAD.Discount_Amt as ValDtlsDiscount,case when isnull(TCS1.is_tcs,'')='Y' THEN  TSPL_SD_SALE_INVOICE_HEAD.TAX2_AMT when isnull(TCS2.is_tcs,'')='Y' THEN  TSPL_SD_SALE_INVOICE_HEAD.TAX3_AMT ELSE 0 END as ValDtlsOthChrg,TSPL_SD_SALE_INVOICE_HEAD.Total_Amt as ValDtlsTotInvVal,TSPL_SD_SALE_INVOICE_HEAD.RoundOffAmount  as ValDtlsRndOffAmt
...
left outer join tspl_tax_master as TCS1 on TCS1.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD.Tax2
left outer join tspl_tax_master as TCS2 on TCS2.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD.Tax3
where TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP' and TSPL_SD_SALE_INVOICE_HEAD.document_type='EX' 
and TSPL_SD_SALE_INVOICE_head.Document_Code ='" & strDocNo & "'"
                ...
                If objResult IsNot Nothing Then
                    ...
                    Dim strExCommercialInvoice As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Document_Code from TSPL_EX_COMMERCIAL_INVOICE_HEAD where Document_Code =(select Against_Com_Inv_No  from TSPL_SD_SALE_INVOICE_head where Document_Code='" & strDocNo & "' and TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP' and TSPL_SD_SALE_INVOICE_HEAD.document_type='EX' )", trans))
                    If clsCommon.myLen(strExCommercialInvoice) > 0 Then
                        clsDBFuncationality.ExecuteNonQuery("update TSPL_EX_COMMERCIAL_INVOICE_HEAD set  IRN_No ='" & Irn & "',qr_code='" & SignedQRCode & "',ack_no='" & AckNo & "',ack_date='" & clsCommon.GetPrintDate(AckDt, "dd/MMM/yyyy hh:mm tt") & "' where Document_Code ='" & strExCommercialInvoice & "' ", trans)

                        clsDBFuncationality.UpdateImage("BarCode_Img", TempByte, "TSPL_EX_COMMERCIAL_INVOICE_HEAD", "TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code='" & strExCommercialInvoice & "'", trans)
                    End If
    ...
    Public Shared Function PostData(ByVal FormId As String, ByVal strDocNo As String, ByVal arrLoc As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            If (clsCommon.myLen(strDocNo) <= 0) Then
                Throw New Exception("Code not found to Post")
            End If
            Dim obj As clsEXSaleInvoiceHead = clsEXSaleInvoiceHead.GetData(strDocNo, NavigatorType.Current, "", arrLoc, IIf(clsCommon.CompairString(FormId, clsUserMgtCode.frmEXSalesInvoice) = CompairStringResult.Equal, "EX", "MT"), trans)
            ...
            SendToInventoryMovement(obj, trans)
    ...
    Public Shared Function SendToInventoryMovement(ByVal obj As clsEXSaleInvoiceHead, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim isSaved As Boolean = True
        Try
            ...
            For Each objTr As clsEXSaleInvoiceDetail In obj.Arr
                ...
                If clsCommon.CompairString(objTr.Row_Type, clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    '=========================check stoch bal-----------------
                    Dim stck_qty As Double = clsCommon.myCdbl(clsItemLocationDetails.getBalance(objTr.Item_Code, obj.Bill_To_Location, obj.Document_Code, obj.Document_Date, trans, objTr.Unit_code, objTr.MRP))

                    If objTr.Qty > stck_qty Then
                        Throw New Exception("Available stock qty is " + clsCommon.myCstr(stck_qty) + " and required qty is " + clsCommon.myCstr(objTr.Qty) + ".")
                    End If
                    '=================================================================================
                ...
                End If
            Next
            ''richa agarwal change transaction type according to document type BM00000007137
            Dim strInvTransType As String = String.Empty
            If clsCommon.CompairString(obj.Document_Type, "MT") = CompairStringResult.Equal Then
                strInvTransType = "MT_SALE_IN"
            Else
    ...
    Private Shared Function createARInvoice(ByVal obj As clsEXSaleInvoiceHead, ByVal trans As SqlTransaction) As Boolean
        ...
        ''richa agarwal change transaction type according to document type BM00000007137
        If clsCommon.CompairString(obj.Document_Type, "MT") = CompairStringResult.Equal Then
            objCustInv.RefDocType = "MT_SALE_IN"
            objCustInv.Trans_Type = "MT"
        Else
            objCustInv.RefDocType = "EX_SALE_IN"
    ...
    Private Shared Function GetFirstItemCode(ByVal Arr As List(Of clsEXSaleInvoiceDetail)) As String
        For Each objtr As clsEXSaleInvoiceDetail In Arr
            If clsCommon.CompairString(objtr.Row_Type, clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
    ...
    Public Shared Function DeleteData(ByVal strCode As String, ByVal FormId As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            qry = "delete from TSPL_EX_SALE_INVOICE_NOTIFY_PARTY_DETAIL where Document_Code='" + strCode + "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Function IsValidCustomer(ByVal Arr As List(Of String), ByVal strVendorCode As String) As Boolean
        If Arr IsNot Nothing AndAlso Arr.Count > 0 Then
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                For Each dr As DataRow In dt.Rows
                    msg += Environment.NewLine + "Invoice No:" + clsCommon.myCstr(dr("Document_Code")) + " Is For Buyer Code: " + clsCommon.myCstr(dr("Customer_Code")) + " Buyer Name:" + clsCommon.myCstr(dr("Customer_Name"))
                Next
    ...
    Public Shared Function IsValidDocumentType(ByVal Arr As List(Of String), ByVal strDocType As String) As Boolean
        If Arr IsNot Nothing AndAlso Arr.Count > 0 Then
            Dim qry As String = "select TSPL_SD_SALE_INVOICE_HEAD.Document_Code,(case when document_type='MT' then 'Merchant Trade' else 'Export Sale' end) as Customer_Code from TSPL_SD_SALE_INVOICE_HEAD where Document_Code in (" + clsCommon.GetMulcallString(Arr) + ") and document_type not in ('" + strDocType + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim msg As String = "Error. "
                For Each dr As DataRow In dt.Rows
                    msg += Environment.NewLine + "Invoice No:" + clsCommon.myCstr(dr("Document_Code")) + " Is For Document Type: " + clsCommon.myCstr(dr("Customer_Code")) + ""
                Next
                Throw New Exception(msg)
            End If
        End If
...
Public Class clsEXSaleInvoiceDetail
#Region "Variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal Arr As List(Of clsEXSaleInvoiceDetail), ByVal trans As SqlTransaction, ByVal objHead As clsEXSaleInvoiceHead) As Boolean

        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            Dim counter As Integer = 1
            For Each obj As clsEXSaleInvoiceDetail In Arr
                Dim coll As New Hashtable()
                ...
                clsCommon.AddColumnsForChange(coll, "Commercial_Inv_No", obj.PI_CODE, True)
            ...
            "TSPL_ITEM_PRICE_MASTER.Item_Basic_Net=" & obj.MRP & " and TSPL_ITEM_PRICE_MASTER.UOM='" & obj.Unit_code & "' "
                ...
                Else
                    strInvTransType = "EX_SALE_IN"
                End If
                clsBatchInventory.SaveData(strInvTransType, strDocNo, objHead.Document_Date, "O", obj.Item_Code, objHead.Bill_To_Location, counter, obj.MRP, obj.Unit_code, obj.arrBatchItem, trans) ' Change by Prabhakar
                counter = counter + 1
            Next
        End If
        Return True
...
Public Class clsEXSaleInvoiceNotifyDetail
#Region "variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDoc As String, ByVal Arr As List(Of clsEXSaleInvoiceNotifyDetail), ByVal trans As SqlTransaction) As Boolean
        Try
            Dim isSaved As Boolean = True

            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery("delete from TSPL_EX_SALE_INVOICE_NOTIFY_PARTY_DETAIL where document_code='" + strDoc + "'", trans)

            If Arr IsNot Nothing AndAlso Arr.Count > 0 Then
                For Each objtr As clsEXSaleInvoiceNotifyDetail In Arr
                    Dim coll As New Hashtable()
                    ...
                    isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_EX_Sale_INVOICE_NOTIFY_PARTY_DETAIL", OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
