...
Public Class clsPSSalesReturnHead
#Region "Variables"
    ...
    Public Arr As List(Of clsPSSalesReturnDetail) = Nothing
...
#End Region
    ...
    Public Function SaveData(ByVal obj As clsPSSalesReturnHead, ByVal isNewEntry As Boolean) As Boolean

        Dim isSaved As Boolean = True
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
            clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleProductSale, clsUserMgtCode.frmSaleReturnProductSale, obj.Bill_To_Location, obj.Document_Date, trans)
            ...
            clsBatchInventory.DeleteData("PS-SR", obj.Document_Code, trans)
            ...
            If isNewEntry Then
                If obj.Is_Cancelled = 1 Then
                    obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.frmSaleReturnProductSale, clsDocTransactionType.SaleReturnCancel, obj.Bill_To_Location)
                Else
                    obj.Document_Code = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.frmSaleReturnProductSale, clsDocTransactionType.NA, obj.Bill_To_Location)
                End If
            ...
            End If
            isSaved = isSaved AndAlso clsPSSalesReturnDetail.SaveData(obj.Document_Code, obj.Document_Date, Arr, trans)
        ...
        Return isSaved
    End Function
    Public Shared Function getGateReturnFinder(ByVal whrcls As String, ByVal curGE_CODE As String, ByVal isButtonClicked As Boolean) As String
        Dim str As String = ""
        Dim qry As String = " select DOC_CODE as Code,Transfer_Date as Date,Cust_Code as Customer,From_Location_Code as Location from TSPL_GATE_ENTRY_RETURN_PS "
        str = clsCommon.ShowSelectForm("GateEntryRefRetFnd", qry, "Code", whrcls, curGE_CODE, "Code", isButtonClicked)
        Return str
    End Function
    Public Shared Function GetData(ByVal strDocumentNo As String, ByVal NavType As NavigatorType) As clsPSSalesReturnHead
        Return GetData(strDocumentNo, NavType, Nothing)
    End Function

    Public Shared Function GetData(ByVal strPONo As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction) As clsPSSalesReturnHead
        Dim obj As clsPSSalesReturnHead = Nothing
        Dim qry As String = "SELECT  TSPL_SD_SALE_RETURN_HEAD.Gate_Entry_No, TSPL_SD_SALE_RETURN_HEAD.RoundOffAmount,TSPL_SD_SALE_RETURN_HEAD.Is_Taxable,TSPL_SD_SALE_RETURN_HEAD.Gate_ReturnNo,TSPL_SD_SALE_RETURN_HEAD.CrateMan,TSPL_SD_SALE_RETURN_HEAD.Jaali,TSPL_SD_SALE_RETURN_HEAD.Box , TSPL_SD_SALE_RETURN_HEAD.Trans_Type,TSPL_SD_SALE_RETURN_HEAD.Return_Type,TSPL_SD_SALE_RETURN_HEAD.Damage_Type,TSPL_SD_SALE_RETURN_HEAD.HeadDisc_PerAmt,TSPL_SD_SALE_RETURN_HEAD.Cust_PO_No,TSPL_SD_SALE_RETURN_HEAD.VehicleNo, TSPL_SD_SALE_RETURN_HEAD.Vehicle_Code,TSPL_SD_SALE_RETURN_HEAD.price_group_code , " & _
        ...
        qry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALE_RETURN_HEAD.Customer_Code where TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' "
        ...
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            ...
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                obj.Arr = New List(Of clsPSSalesReturnDetail)
                Dim objTr As clsPSSalesReturnDetail
                For Each dr As DataRow In dt.Rows
                    objTr = New clsPSSalesReturnDetail
                    ...
                    objTr.arrBatchItem = clsBatchInventory.GetData("PS-SR", objTr.Document_Code, objTr.Item_Code, objTr.Line_No, trans)
                    obj.Arr.Add(objTr)
                Next
            End If
    ...
    Public Shared Function PostData(ByVal FormId As String, ByVal strDocNo As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            End If
            Dim obj As clsPSSalesReturnHead = clsPSSalesReturnHead.GetData(strDocNo, NavigatorType.Current, trans)
            ...
            For Each objTr As clsPSSalesReturnDetail In obj.Arr
                ...
                If clsCommon.CompairString(objTr.Row_Type, clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    If Not objInv Is Nothing Then
                        For Each objInvDetail As clsSNInvoiceDetail In objInv.Arr
                            If objInvDetail.Item_Code = objTr.Item_Code AndAlso objInvDetail.Unit_code = objTr.Unit_code Then
            ...
            isSaved = isSaved AndAlso clsInventoryMovement.SaveData("PS-SR", obj.Document_Code, obj.Document_Date, clsCommon.GetPrintDate(obj.Document_Date, "dd/MM/yyyy"), ArrInventoryMovement, trans)
            ...
            If clsCommon.CompairString(ECustomerType, "BB") = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCstr(obj.Is_Taxable), "1") = CompairStringResult.Equal AndAlso clsERPFuncationality.GetEInvoiceStatus(obj.Document_Date, trans) = True Then
                If clsPSSalesReturnHead.EInvoice_Implementation(obj.Document_Code, obj.Bill_To_Location, trans) = True Then
                Else
                    Throw New Exception("Invalid JSON Value")
                End If
            End If

            qry = "Update TSPL_SD_SALE_RETURN_HEAD set Status=1, Posting_Date='" + clsCommon.GetPrintDate(obj.Document_Date, "dd/MMM/yyyy") + "',Modify_By='" + objCommonVar.CurrentUserCode + "',EInvoice_Type='" + ECustomerType + "',EInvoice_Posting_Date='" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy hh:mm tt") + "'"
            qry += " where Document_Code='" + strDocNo + "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)

            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, strDocNo, "TSPL_SD_SALE_RETURN_HEAD", "Document_Code", trans)
        Catch ex As Exception
    ...
    Public Shared Function CancelData(ByVal Form_Id As String, ByVal Doc_No As String, ByVal NavType As NavigatorType) As Boolean
        ...
        Try
            ...
            Dim obj As clsPSSalesReturnHead = clsPSSalesReturnHead.GetData(Doc_No, NavType, trans)
            ...
            qry = "delete from TSPL_BATCH_ITEM where  Document_Code='" & Doc_No & "' and Document_Type='PS-SR'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" & Doc_No & "' and Trans_Type='PS-SR'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
...
left outer join tspl_tax_master as TCS2 on TCS2.Tax_Code =TSPL_SD_SALE_RETURN_HEAD.Tax3
    ...
    Public Shared Function ReverseAndUnpost(ByVal strCode As String) As Boolean
        ...
        Try
            ...
            If clsCommon.myLen(strARInvoiceNo) > 0 Then
                Qry = "select distinct Receipt_No from TSPL_RECEIPT_DETAIL where Document_No='" + strARInvoiceNo + "'"
                Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry, trans)
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    Qry = "Current Sale Return - " + strCode + ".AR No - " + strARInvoiceNo + " is used in following Receipt Entry -"
                    For Each dr As DataRow In dt.Rows
                ...
                Dim VoucherNo As String = clsDBFuncationality.getSingleValue("select Voucher_No from TSPL_JOURNAL_MASTER where Source_Code='AR-CR' and Source_Doc_No='" + strARInvoiceNo + "'", trans)
                If clsCommon.myLen(VoucherNo) > 0 Then
                    ...
                    clsDBFuncationality.ExecuteNonQuery(Qry, trans)
                End If
                clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, strARInvoiceNo, "TSPL_Customer_Invoice_HEAD", "Document_No", "TSPL_Customer_Invoice_Detail", "Document_No", "TSPL_REMITTANCE", "Document_No", trans)
            ...
            Qry = "update tspl_batch_item set against_inv_movement_trans_id=NULL where document_type='PS-SR' and document_code='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, strCode, "tspl_batch_item", "document_code", trans)

            Qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" & strCode & "' and Trans_Type= 'PS-SR'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TSPL_INVENTORY_MOVEMENT_New where Source_Doc_No='" & strCode & "' and Trans_Type= 'PS-SR'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "Update TSPL_SD_SALE_RETURN_HEAD set Status = 0 where Document_Code='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "update TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL set is_reverse=1 where document_code='" + strCode + "' and trans_code='" + clsCommon.myCstr(clsUserMgtCode.frmSaleReturnProductSale) + "' and is_reverse=0"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, strCode, "TSPL_SD_SALE_RETURN_HEAD", "Document_Code", trans)
            trans.Commit()
        Catch ex As Exception
            trans.Rollback()
            Throw New Exception(ex.Message)
        ...
        Return True
    End Function
    Public Shared Function createARInvoice(ByVal obj As clsPSSalesReturnHead, ByVal trans As SqlTransaction, Optional ByVal strARNoForRecreate As String = Nothing, Optional ByVal strVoucherForRecreate As String = Nothing) As Boolean
        ...
        objCustInv.Trans_Type = obj.Trans_type
        ...
        objCustInv.RefDocNo = obj.Against_Invoice_No
        ...
        ''richa BHA/01/03/19-000830 send form id 05 March,2019
    ...
    Private Shared Function GetFirstItemCode(ByVal Arr As List(Of clsPSSalesReturnDetail)) As String
        For Each objtr As clsPSSalesReturnDetail In Arr
            If clsCommon.CompairString(objtr.Row_Type, clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
    ...
    Public Shared Function DeleteData(ByVal strCode As String) As Boolean
        ...
        End If
        Dim obj As clsPSSalesReturnHead = clsPSSalesReturnHead.GetData(strCode, NavigatorType.Current)
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
            Try
                ...
                clsBatchInventory.DeleteData("PS-SR", strCode, trans)
    ...
    Public Shared Function IsValidCustomer(ByVal Arr As List(Of String), ByVal strVendorCode As String) As Boolean
        If Arr IsNot Nothing AndAlso Arr.Count > 0 Then
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                For Each dr As DataRow In dt.Rows
                    msg += Environment.NewLine + "Invoice No:" + clsCommon.myCstr(dr("Document_Code")) + " Is For Customer Code: " + clsCommon.myCstr(dr("Customer_Code")) + " Customer Name:" + clsCommon.myCstr(dr("Customer_Name"))
                Next
...
Public Class clsPSSalesReturnDetail
#Region "Variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal dtDocDate As DateTime, ByVal Arr As List(Of clsPSSalesReturnDetail), ByVal trans As SqlTransaction) As Boolean

        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsPSSalesReturnDetail In Arr
                Dim coll As New Hashtable()
                ...
                clsBatchInventory.SaveData("PS-SR", strDocNo, dtDocDate, "I", obj.Item_Code, obj.Location, obj.Line_No, obj.MRP, obj.Unit_code, obj.arrBatchItem, trans)
