...
#Region "Variables"
    ...
    Public Against_Demand_No As String = Nothing
...
#End Region
    ...
    Public Shared Function SaveData(ByVal obj As clsPSShipmentHead, ByVal isNewEntry As Boolean, ByVal trans As SqlTransaction, Optional ByVal IsDairyModule As Boolean = False) As Boolean
        ...
        Try
            ...
            clsCommon.AddColumnsForChange(coll, "BoothSecurity_TotalAmt", obj.BoothSecurity_TotalAmt)
    ...
    Public Shared Function GetData(ByVal strPONo As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction, ByVal IsProductDispacth As Boolean, ByVal IsAPS As Boolean, Optional ByVal IsDairyModule As Boolean = False) As clsPSShipmentHead
        ...
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            ...
            qry += " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_SD_SHIPMENT_DETAIL.Item_Code " &
                  " left join TSPL_BOOKING_MATSER on TSPL_SD_SHIPMENT_HEAD.Against_Booking_No=TSPL_BOOKING_MATSER.Document_No " &
                   " left join TSPL_USER_MASTER on TSPL_BOOKING_MATSER.Created_By=TSPL_USER_MASTER.User_Code " &
                   " left join TSPL_SECONDARY_CUSTOMER_MASTER SecCust on SecCust.Cust_Code=TSPL_USER_MASTER.Distributor_Retailer_Code" +
            ...
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                ...
                For Each dr As DataRow In dt.Rows
                    ...
                    objTr.Booth_Security_Amt = clsCommon.myCdbl(dr("Booth_Security_Amt"))
        ...
        Try
            ...
            If Not isSkipCogsGL Then    '' Done By Pankaj Jha For Skipping Cogs GL
                ...
                If strVoucherNoForRecreateOnly IsNot Nothing AndAlso clsCommon.myLen(strVoucherNoForRecreateOnly) > 0 Then
                    clsJournalMaster.FunGrnlEntryWithTrans(obj.Bill_To_Location, False, strVoucherNoForRecreateOnly, trans, obj.Document_Date, obj.Remarks, "SD-SH", "Shipment", obj.Document_Code, "", "C", obj.Customer_Code, obj.Customer_Name, objCommonVar.CurrentUserCode, objCommonVar.CurrentCompanyCode, ArryLstGLAC, , obj.Description, obj.Remarks)
                Else
    ...
    Public Shared Function DeleteData(ByVal strCode As String, ByVal strInvoiceNo As String, ByVal trans As SqlTransaction) As Boolean
        ...
        If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
            Try
                ...
                clsCommonFunctionality.SaveDeletedData(objCommonVar.CurrentUserCode, strInvoiceNo, "TSPL_SD_SALE_INVOICE_HEAD", "Document_Code", "TSPL_SD_SALE_INVOICE_DETAIL", "Document_Code", trans)
