...
#End Region
    ...
    Public Shared Function SaveData(ByVal obj As clsPSShipmentHead, ByVal isNewEntry As Boolean, ByVal trans As SqlTransaction, Optional ByVal IsDairyModule As Boolean = False) As Boolean
        ...
        'isDispatchfromDelivery = IIf(clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Description from TSPL_FIXED_PARAMETER where Code='" & clsFixedParameterCode.DairyDispatchFromDeliveryNote & "'")) = 0, False, True)

        Try

            ' clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, "Product Sale", "Product Dispatch", obj.Bill_To_Location, obj.Document_Date, trans)
            ...
            If isNewEntry Then
                ...
                If IsDairyModule = False Then
                    ...
                    ' clsCommon.AddColumnsForChange(coll, "Screen_Type", "")
                Else
                    clsCommon.AddColumnsForChange(coll, "Trans_Type", obj.Trans_Type)
                    'clsCommon.AddColumnsForChange(coll, "Screen_Type", "DS")
                End If
            ...
            End If
            isSaved = isSaved AndAlso clsPSShipmentHeadDetail.SaveData(obj.Document_Code, obj.Arr, trans, obj.Document_Date, IsDairyModule, obj.Trans_Type)
    ...
    Public Shared Function GetData(ByVal strPONo As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction, Optional ByVal IsDairyModule As Boolean = False) As clsPSShipmentHead
        ...
        ",TSPL_SD_SHIPMENT_HEAD.Itemwise,TSPL_SD_SHIPMENT_HEAD.Delivery_Code_PS,TSPL_SD_SHIPMENT_HEAD.Advance_Percentage,TSPL_SD_SHIPMENT_HEAD.GR_Date,TSPL_SD_SHIPMENT_HEAD.RoadPermit_Date,TSPL_SD_SHIPMENT_HEAD.Sale_Invoice_Date,TSPL_SD_SHIPMENT_HEAD.Removal_Date,TSPL_SD_SHIPMENT_HEAD.Cash_Customer,TSPL_SD_SHIPMENT_HEAD.Insurance,TSPL_SD_SHIPMENT_HEAD.ManualVehicle,TSPL_SD_SHIPMENT_HEAD.Freight_Distance "
    ...
    Public Shared Function PrintDataBatchWiseInvoice(ByVal StrDocNo As String) As Boolean
        Try
            ...
            qry = "  Select * from ( " & _
                  ...
                  "  TSPL_BATCH_ITEM.Batch_No as Batch_No, " & _
                  ...
                  "  TSPL_COMPANY_MASTER.Bank_Name, TSPL_COMPANY_MASTER.BankAccountNo, TSPL_COMPANY_MASTER.BankIFSCCode, TSPL_COMPANY_MASTER.BankBranchAddress,TSPL_Company_Master.Logo_Img as Logo_Img2GSTReg_NO,TSPL_Company_Master.GSTReg_NO as CompGSTReg_NO  " & _
                  " ,TSPL_SD_SHIPMENT_HEAD. Customer_Code  as cust_Code ,TSPL_CUSTOMER_MASTER.Customer_Name ,TSPL_CUSTOMER_MASTER.Add1 as cust_add1 ,TSPL_CUSTOMER_MASTER. Add2 as cust_add2 ,TSPL_CUSTOMER_MASTER.Add3 cust_add3,isnull(TSPL_CUSTOMER_MASTER.pin_Code,'') as Cust_Pin,(select top 1 tspl_employee_master.emp_name from tspl_salesman_detail left join tspl_employee_master on tspl_employee_master.emp_code=tspl_salesman_detail.salesman_code where route_code=tspl_route_master.route_no) as  Sales_Name,isnull(TSPL_CUSTOMER_MASTER.FSSAI_NO,'') as Cust_FSSAI_LIC_NO ,case when ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')='(+__)__________' then ''  else TSPL_CUSTOMER_MASTER.Phone1 end +  Case When ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'(+__)__________' Then ', '+  TSPL_CUSTOMER_MASTER.Phone2 Else'' End as CustPhone ,TSPL_CUSTOMER_MASTER.Fax as cust_fax ,TSPL_CUSTOMER_MASTER.Email as  cust_Email,TSPL_CUSTOMER_MASTER.WebSite as cust_website,TSPL_CUSTOMER_MASTER.pan as Customer_Pan,TSPL_SD_SHIPMENT_DETAIL.item_cost, TSPL_SD_SHIPMENT_DETAIL.amount , TSPL_SD_SHIPMENT_HEAD.Discount_amt,TSPL_SD_SHIPMENT_HEAD.Total_Tax_Amt,TSPL_SD_SHIPMENT_HEAD.CrateQty,TSPL_SD_SHIPMENT_HEAD.Crate , TSPL_SD_SHIPMENT_HEAD.Is_Taxable  from  TSPL_BATCH_ITEM " & _
                  "  Left Outer Join  TSPL_SD_SHIPMENT_DETAIL on  TSPL_SD_SHIPMENT_DETAIL.Document_Code   =TSPL_BATCH_ITEM.Document_Code AND TSPL_BATCH_ITEM.Parent_Line_No=TSPL_SD_SHIPMENT_DETAIL.Line_No AND TSPL_BATCH_ITEM.Item_Code=TSPL_SD_SHIPMENT_DETAIL.Item_Code AND TSPL_BATCH_ITEM.UOM =TSPL_SD_SHIPMENT_DETAIL.Unit_code " & _
