...
#Region "Variable"
    ...
    Public Supply_Date As DateTime?
    ...
    Public PO_Indent_No As String = ""
    Public PO_Indent_Date As DateTime?
    ...
    Public NoCrateIssue As Decimal = 0
        ...
        Try
            ...
            ElseIf isNewEntry = True Then
                If IsDemandUploader Then
                    obj.Document_No = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.frmDairySaleBookingUploader, "", obj.location_code, False, True, False, False, False)
                Else
                    obj.Document_No = clsERPFuncationality.GetNextCode(trans, obj.Document_Date, clsDocType.frmDairySaleBooking, "", obj.location_code, False, True, False, False, False)
                End If
            End If
            ...
            If DateTime = "1" Then
                ...
                clsCommon.AddColumnsForChange(coll, "Supply_Date", clsCommon.GetPrintDate(obj.Supply_Date, "dd/MMM/yyyy hh:mm tt"))
            Else
            ...
            End If
            clsCommon.AddColumnsForChange(coll, "PO_Indent_No", obj.PO_Indent_No, True)

            If obj.PO_Indent_Date IsNot Nothing Then
                clsCommon.AddColumnsForChange(coll, "PO_Indent_Date", clsCommon.GetPrintDate(obj.PO_Indent_Date, "dd/MMM/yyyy hh:mm tt"), True)
            Else
                clsCommon.AddColumnsForChange(coll, "PO_Indent_Date", Nothing, True)
            End If
            ...
            End If
            clsCommon.AddColumnsForChange(coll, "Exclude_KKF_And_Mandi", obj.Exclude_KKF_And_Mandi)
            ...
            clsCommon.AddColumnsForChange(coll, "Is_APS", obj.Is_APS, True)
...
isnull(TSPL_DELIVERY_NOTE_MASTER_FRESHSALE.Short_Close,'N')='N' "
    ...
    Public Shared Function GetData(ByVal strDocumentNo As String, ByVal NavType As NavigatorType, ByVal Trans As SqlTransaction, Optional ByVal FormId As String = "") As clsBookingEntryDairySale
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            obj.MobileNO = clsCommon.myCstr(dt.Rows(0)("MobileNO"))
            obj.AdharNo = clsCommon.myCstr(dt.Rows(0)("AdharNo"))
            ...
            obj.Is_APS = clsCommon.myCdbl(dt.Rows(0)("Is_APS"))
            ...
            If dt.Rows(0)("PO_Indent_Date") IsNot DBNull.Value Then
                obj.PO_Indent_No = clsCommon.myCstr(dt.Rows(0)("PO_Indent_No"))
                obj.PO_Indent_Date = clsCommon.myCDate(dt.Rows(0)("PO_Indent_Date"))
            End If
            ...
            Try
               ...
               " ))) Final where Document_No<>'" + strCode + "' "
                ...
                If dtDoc IsNot Nothing AndAlso dtDoc.Rows.Count > 0 Then
                    ...
                    Next
                End If
                clsCommonFunctionality.SaveDeletedData(objCommonVar.CurrentUserCode, strCode, "TSPL_BOOKING_MATSER", "Document_No", "TSPL_BOOKING_DETAIL", "Document_No", "TSPL_BOOKING_PAYMENT_MODE_DETAIL", "Document_No", trans)
                ...
                clsCommonFunctionality.SaveCancelData(objCommonVar.CurrentUserCode, strCode, "TSPL_BOOKING_MATSER", "Document_No", "TSPL_BOOKING_DETAIL", "Document_No", "TSPL_BOOKING_PAYMENT_MODE_DETAIL", "Document_No", trans)
    ...
    Public Shared Function CancelData(ByVal Form_Id As String, ByVal Doc_No As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            If clsCommon.myLen(DispatchDocNo) > 0 Then
                Dim obj As clsPSShipmentHead = clsPSShipmentHead.GetData(DispatchDocNo, NavigatorType.Current, trans, False, False)
...
where TSPL_VENDOR_INVOICE_HEAD.RefDocType='BOK-CRD' and TSPL_VENDOR_INVOICE_HEAD.RefDocNo='" + strCode + "'", trans)
            ...
            If clsCommon.myLen(ShipmentDoc) > 0 Then
                Dim obj As clsPSShipmentHead = clsPSShipmentHead.GetData(ShipmentDoc, NavigatorType.Current, trans, False, False, False)
                FlagDocumentIsTaxable = obj.Is_Taxable
                EInvoiceType = clsERPFuncationality.GetCustomerEInvoiceTypeFromTransationTable("TSPL_SD_SALE_INVOICE_HEAD", "Document_Code", obj.Invoice_No, trans)
                Dim strSaleReturnNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select Document_Code from TSPL_SD_SALE_RETURN_HEAD where Against_Invoice_No='" & obj.Invoice_No & "' ", trans))
                If clsCommon.myLen(strSaleReturnNo) > 0 Then
