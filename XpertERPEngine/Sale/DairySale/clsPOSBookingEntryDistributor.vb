...
Public Class clsPOSBookingEntryDistributor
...
#Region "Variable"
    ...
    Public Arr As List(Of clsPOSBookingEntryDistributorDetail) = Nothing
...
#End Region
    Public Function SaveData(ByVal obj As clsPOSBookingEntryDistributor, ByVal isNewEntry As Boolean) As Boolean
        ...
        Return True
    End Function
    Public Function SaveData(ByVal obj As clsPOSBookingEntryDistributor, ByVal isNewEntry As Boolean, ByVal trans As SqlTransaction) As Boolean
        Dim qry As String = String.Empty
        Try
            If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
                For Each obj1 As clsPOSBookingEntryDistributorDetail In Arr
            ...
            qry = "delete from TSPL_POS_BOOKING_DETAIL where Main_Booking_No='" + obj.Main_Booking_No + "'  and Document_No='" + obj.Document_No + "' "
            ...
            If isNewEntry Then
                ...
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_POS_BOOKING_MATSER", OMInsertOrUpdate.Insert, "", trans)
            Else
                isSaved = isSaved AndAlso clsCommonFunctionality.UpdateDataTable(coll, "TSPL_POS_BOOKING_MATSER", OMInsertOrUpdate.Update, "TSPL_POS_BOOKING_MATSER.Main_Booking_No='" + obj.Main_Booking_No + "'", trans)
            End If

            isSaved = isSaved AndAlso clsPOSBookingEntryDistributorDetail.SaveData(obj.Document_No, Arr, trans, isNewEntry, obj.Document_Date)
            Return isSaved
        Catch ex As Exception
            Throw New Exception(ex.Message)
        Finally
    ...
    Public Shared Function GetData(ByVal strDocumentNo As String, ByVal NavType As NavigatorType) As clsPOSBookingEntryDistributor
        Return GetData(strDocumentNo, NavType, Nothing)
    End Function

    Public Shared Function GetData(ByVal strDocumentNo As String, ByVal NavType As NavigatorType, ByVal Trans As SqlTransaction) As clsPOSBookingEntryDistributor
        Dim obj As clsPOSBookingEntryDistributor = Nothing
        Dim qry As String = "select distinct TSPL_POS_BOOKING_MATSER.Main_Booking_No,TSPL_POS_BOOKING_MATSER.Document_Date,TSPL_POS_BOOKING_MATSER.Posted,CreateDO_Automatic,TSPL_POS_BOOKING_MATSER.location_code,Cust_Group_Code,TSPL_POS_BOOKING_DETAIL.Cust_Code from TSPL_POS_BOOKING_MATSER inner join TSPL_POS_BOOKING_DETAIL on TSPL_POS_BOOKING_MATSER.Main_Booking_No=TSPL_POS_BOOKING_DETAIL.Main_Booking_No where TSPL_POS_BOOKING_MATSER.comp_code='" + objCommonVar.CurrentCompanyCode + "' and "

        Select Case NavType
            Case NavigatorType.Current
                qry += "  TSPL_POS_BOOKING_MATSER.Main_Booking_No='" + strDocumentNo + "'"
            Case NavigatorType.Next
                qry += " TSPL_POS_BOOKING_MATSER.Main_Booking_No in (select min(t.Main_Booking_No) from TSPL_POS_BOOKING_MATSER  as t where t.Main_Booking_No  >'" + strDocumentNo + "')"
            Case NavigatorType.First
                qry += " TSPL_POS_BOOKING_MATSER.Main_Booking_No in (select min(t.Main_Booking_No ) from TSPL_POS_BOOKING_MATSER  as t )"
            Case NavigatorType.Last
                qry += " TSPL_POS_BOOKING_MATSER.Main_Booking_No in (select max(t.Main_Booking_No ) from TSPL_POS_BOOKING_MATSER  as t )"
            Case NavigatorType.Previous
                qry += " TSPL_POS_BOOKING_MATSER.Main_Booking_No in (select max(t.Main_Booking_No ) from TSPL_POS_BOOKING_MATSER  as t where t.Main_Booking_No  <'" + strDocumentNo + "')"
        End Select
    ...
    Public Shared Function DeleteData(ByVal strCode As String) As Boolean
        ...
        End If
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Dim obj As clsPOSBookingEntryDistributor = clsPOSBookingEntryDistributor.GetData(strCode, NavigatorType.Current, trans)

        If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Main_Booking_No) > 0) Then
            Try
                qry = "delete from TSPL_POS_BOOKING_DETAIL where Main_Booking_No='" + strCode + "'"
                isSaved = clsDBFuncationality.ExecuteNonQuery(qry, trans)

                qry = "delete from TSPL_POS_BOOKING_MATSER where Main_Booking_No='" + strCode + "'"
                isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
                If (isSaved) Then
                    trans.Commit()
                Else
...
End Class
Public Class clsPOSBookingEntryDistributorDetail
#Region "Variable"
    Public Document_No As String = Nothing
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal Arr As List(Of clsPOSBookingEntryDistributorDetail), ByVal trans As SqlTransaction, ByVal isNewEntry As Boolean, ByVal Docdate As String) As Boolean
        ...
        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsPOSBookingEntryDistributorDetail In Arr
                ...
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_POS_BOOKING_DETAIL", OMInsertOrUpdate.Insert, "", trans)
                If clsCommon.myLen(clsCommon.myCstr(obj.SchemeType)) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(obj.SchemeType), "Quantitive") = CompairStringResult.Equal Then
                    ...
                    clsCommonFunctionality.UpdateDataTable(colll, "TSPL_POS_BOOKING_DETAIL", OMInsertOrUpdate.Insert, "", trans)
