    ...
    Private Sub FrmSerializeItemIn_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If arr IsNot Nothing AndAlso arr.Count > 0 Then
            For Each obj As clsBatchInventory In arr
                ...
                gv1.Rows(gv1.Rows.Count - 1).Cells(colMfgDate).Value = obj.Manufacture_Date
                gv1.Rows(gv1.Rows.Count - 1).Cells(colExpDate).Value = obj.Expiry_Date
                gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = obj.Qty
                gv1.Rows(gv1.Rows.Count - 1).Cells(colManualBatch).Value = obj.Manual_BatchNo
            Next
        End If
        gv1.Rows.AddNew()
        RefeshSNO()
        ...
        End If
        gv1.Focus()
        intShelfLife = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select isnull(tech_shelf_life,0) from TSPL_ITEM_MASTER where ITEM_Code ='" + strItemCode + "'"))
        ...
        For ii As Integer = 0 To gv1.RowCount - 1
            If clsCommon.myLen(gv1.Rows(ii).Cells(colBatchNo).Value) > 0 Then
                If clsCommon.myLen(gv1.Rows(ii).Cells(colMfgDate).Value) <= 0 Then
                    Throw New Exception("Please enter Manufacturer Date at Row No" + clsCommon.myCstr(ii + 1))
                End If
                If clsCommon.myLen(gv1.Rows(ii).Cells(colExpDate).Value) <= 0 Then
                    Throw New Exception("Please enter Expiry Date at Row No" + clsCommon.myCstr(ii + 1))
                End If
                If IsmanualBatchNomandatory = 1 Then
                    If clsCommon.myLen(gv1.Rows(ii).Cells(colManualBatch).Value) <= 0 Then
                        Throw New Exception("Please enter Manual Batch No at Row No" + clsCommon.myCstr(ii + 1))
                    End If
                End If
