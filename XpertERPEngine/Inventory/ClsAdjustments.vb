...
#Region "Variables"
    ...
    Public Arr As List(Of ClsAdjustmentsDetails) = Nothing
    Public Against_Transfer_In_Return_Doc_No As String = Nothing
    Public Against_PurchaseReturn_No As String = Nothing
#End Region
    Public Shared Function funAdjStorePrint(ByVal Form_ID As String, ByVal isCancel As Boolean, ByVal strDate As DateTime, ByVal strAdjustmentNo As String, ByVal IsPreprinted As Boolean, ByVal IsEmpty As Boolean) As Boolean

        Dim TSPL_ADJUSTMENT_DETAIL As String = Nothing
        Dim TSPL_ADJUSTMENT_HEADER As String = Nothing
        If isCancel Then
            TSPL_ADJUSTMENT_DETAIL = "TSPL_ADJUSTMENT_DETAIL_cancel_data"
            TSPL_ADJUSTMENT_HEADER = "TSPL_ADJUSTMENT_HEADER_cancel_data"
        Else
            TSPL_ADJUSTMENT_DETAIL = "TSPL_ADJUSTMENT_DETAIL"
            TSPL_ADJUSTMENT_HEADER = "TSPL_ADJUSTMENT_HEADER"
        End If
        ...
        Else
            If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Adjustment_Type")), "BD") = CompairStringResult.Equal And IsPreprinted = True Then
                ...
                If IsPreprinted Then
                    ...
                    dt = clsDBFuncationality.GetDataTable(qry)
                    frmCRV.funreport(Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.PaperSize10x6, "crptAdjustmentCustomIssue", "Adjustment Detail")
                Else
                    ...
                    dt = clsDBFuncationality.GetDataTable(qry)
                    frmCRV.funreport(Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.NA, "rptEmptyOutward", "Empty Issue Challan")
                End If
...
"from ( " &
                ...
                If IsEmpty Then
                    If IsPreprinted Then
                        frmCRV.funreport(Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.PaperSize10x12, "crptAdjustmentCustomReceiptGun", "Adjustment Detail")
                    Else

                        If (clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "guntur") = CompairStringResult.Equal) Then
                            frmCRV.funreport(Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.NA, "crptAdjustmentCustomReceiptGuntur", "Adjustment Detail")
                        Else
                            frmCRV.funreport(Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.NA, "crptAdjustmentCustomReceiptVizag", "Adjustment Detail")

                        End If
                    End If
                Else
                    frmCRV.funreport(Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.PaperSize10x6, "crptAdjustmentCustomReceipt", "Adjustment Detail")
                End If
            End If
        End If
        frmCRV = Nothing
        Return True
    End Function
    Public Function SaveData(ByVal obj As ClsAdjustments, ByVal isNewEntry As Boolean) As Boolean
    ...
    Public Function SaveData(ByVal obj As ClsAdjustments, ByVal isNewEntry As Boolean, ByVal strAdjustmentNoTemp As String, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            qry = "delete from TSPL_ADJUSTMENT_DETAIL_Delete_Data where Adjustment_No='" + obj.Adjustment_No + "'"
            ...
            Else
                If isNewEntry Then
                    ...
                    If clsCommon.CompairString(obj.Adjustment_Type, "PRE") = CompairStringResult.Equal Then
                        strDoc = clsDocType.StoreAdjustmentProductionEntry
                        strDocTrans = clsDocTransactionType.StoreAdjustmentAdjustmentProductionEntry
                    Else
            ...
            If clsCommon.CompairString("Y", clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Posted from TSPL_ADJUSTMENT_HEADER where Adjustment_No='" + obj.Adjustment_No + "'", trans))) = CompairStringResult.Equal Then
        ...
        Try
            ...
            Else
                    ...
                    ElseIf clsCommon.CompairString(productype, "MI") = CompairStringResult.Equal Then
                        ...
                        Else
                            ...
                            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                                ...
                                clsDBFuncationality.ExecuteNonQuery("update TSPL_ADJUSTMENT_HEADER set Loc_Code='" & objInventoryMovemntnew.Location_Code & "',MainLocationCode='" & objInventoryMovemntnew.main_location & "',loc_desc='" & clsLocation.GetName(objInventoryMovemntnew.Location_Code, trans) & "' where Adjustment_No='" & obj.Adjustment_No & "'", trans)
                            End If
                        End If
                ...
                If ArrInventoryMovement IsNot Nothing AndAlso ArrInventoryMovement.Count > 0 Then
                    clsInventoryMovement.SaveData("IC-AD", obj.Adjustment_No, obj.Adjustment_Date, clsCommon.GetPrintDate(obj.Adjustment_Date, "dd/MM/yyyy"), ArrInventoryMovement, trans)
                End If

                If ArrInventoryMovementNew IsNot Nothing AndAlso ArrInventoryMovementNew.Count > 0 Then
                    clsInventoryMovementNew.SaveData("IC-AD", obj.Adjustment_No, obj.Adjustment_Date, clsCommon.GetPrintDate(obj.Adjustment_Date, "dd/MM/yyyy"), ArrInventoryMovementNew, trans)
                End If
                '--------------------------------------------------------------------------------------------------------------------------------------
                ''--- GL End
            ...
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, obj.Adjustment_No, "TSPL_ADJUSTMENT_HEADER", "adjustment_no", trans)
        Catch ex As Exception
    ...
    Public Shared Function CreateMilkTransferAdjustmentDoc(objDisp As clsMccDispatch, obj As clsMilkTransferIn, trans As SqlTransaction) As Boolean
        Try
            ...
            If objAdjIn.Arr.Count > 0 Then
                ...
                ClsAdjustments.PostData(objAdjIn.Adjustment_No, objAdjIn.Trans_Type, trans, True)
            End If
            If objAdjOut.Arr.Count > 0 Then
                objAdjOut.SaveData(objAdjOut, True, "", trans)
                ClsAdjustments.PostData(objAdjOut.Adjustment_No, objAdjOut.Trans_Type, trans, True)
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    ...
    Public Shared Function CreateJE(ByVal obj As ClsAdjustments, ByVal strType As String, ByVal trans As SqlTransaction, Optional ByVal strVourcherNoForRecreateOnly As String = Nothing) As Boolean
        ...
        Dim strSourceCode As String = "IC-AD"
        Dim strSourceCodeName As String = "I/C Adjustments"
        ...
        For Each objtr As ClsAdjustmentsDetails In obj.Arr
            ...
            ElseIf clsCommon.CompairString(objtr.Adjustment_Type, "QD") = CompairStringResult.Equal Then
                'No need to make Journal Entry
            ElseIf clsCommon.CompairString(objtr.Adjustment_Type, "CI") = CompairStringResult.Equal OrElse clsCommon.CompairString(objtr.Adjustment_Type, "CD") = CompairStringResult.Equal OrElse + _
                clsCommon.CompairString(objtr.Adjustment_Type, "FI") = CompairStringResult.Equal OrElse clsCommon.CompairString(objtr.Adjustment_Type, "FD") = CompairStringResult.Equal Then
                If clsCommon.myLen(obj.Customer_CODE) <= 0 Then
                    ...
                    Else
                        ...
                        If clsCommon.CompairString(objtr.Adjustment_Type, "CD") = CompairStringResult.Equal OrElse clsCommon.CompairString(objtr.Adjustment_Type, "BD") = CompairStringResult.Equal OrElse clsCommon.CompairString(objtr.Adjustment_Type, "FD") = CompairStringResult.Equal Then
                            clsInventoryMovement.UpdateInvControlAccount(obj.Adjustment_No, "IC-AD", objtr.Item_Code, "", strInvAcc, "", trans)
                        Else
                            clsInventoryMovement.UpdateInvControlAccount(obj.Adjustment_No, "IC-AD", objtr.Item_Code, strInvAcc, "", "", trans)
                        End If
                        '------------------
                ...
                ElseIf clsCommon.myLen(obj.Against_Physical_Stock_No) > 0 Then
                    ...
                    If dtTemp IsNot Nothing AndAlso dtTemp.Rows.Count > 0 Then
                        ...
                        Else
                            ...
                            clsInventoryMovement.UpdateInvControlAccount(obj.Adjustment_No, "IC-AD", objtr.Item_Code, strInvAcc, "", "I", trans)
                            'Ticket No- TEC/12/03/19-000442 sanjay
                        End If
                        strAdjAcc = clsERPFuncationality.ChangeGLAccountLocationSegment(strAdjAcc, strsegment, True, trans)
                        Dim Acc2() As String = {strAdjAcc, -1 * objtr.Item_Cost}
    ...
    Public Shared Function CreateJETransferWithBranchAc(ByVal obj As ClsAdjustments, ByVal strType As String, ByVal trans As SqlTransaction, Optional ByVal strVourcherNoForRecreateOnly As String = Nothing) As Boolean
        If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.AllowPurchaseAccounting, clsFixedParameterCode.AllowPurchaseAccounting, trans)) = 0 Then
            Dim strAdjAcc As String = String.Empty
            Dim strEmpty As String = String.Empty
            Dim qry As String = String.Empty
    ...
    Public Shared Function GetIsMilkType(ByVal strAdjNo As String, ByVal trans As SqlTransaction) As Integer
        Dim qry As String = "select IsMilkType from TSPL_ADJUSTMENT_HEADER where Adjustment_No='" & strAdjNo & "'"
        Return clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry, trans))
    End Function
    ...
    Public Shared Function CreateEmptyReceiptOfSalesInvoiceOfTransfer(ByVal strSaleInvoiceNo As String, ByVal trans As SqlTransaction) As String
        Dim obj As ClsAdjustments = New ClsAdjustments()
    ...
    Public Function GetData(ByVal strDocNo As String, ByVal AdjustmentType As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction, Optional ByVal isJobWorkConsuption As Boolean = False, Optional ByVal isProductionEntry As Boolean = False) As ClsAdjustments
        ...
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            ...
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                obj.Arr = New List(Of ClsAdjustmentsDetails)
                Dim objTr As ClsAdjustmentsDetails
                For Each dr As DataRow In dt.Rows
                    ...
                    objTr.arrSrItem = clsSerializeInvenotry.GetData("IC-AD", objTr.Adjustment_No, objTr.Item_Code, objTr.Adjustment_Line_No, trans)
                    objTr.arrBatchItem = clsBatchInventory.GetData("IC-AD", objTr.Adjustment_No, objTr.Item_Code, objTr.Adjustment_Line_No, trans)
                    objTr.arrBatchItemNew = clsBatchInventoryNew.GetData("IC-AD", objTr.Adjustment_No, objTr.Item_Code, objTr.Adjustment_Line_No, trans)
                    obj.Arr.Add(objTr)
                Next
            End If
        End If
    ...
    Public Shared Function DeleteData(ByVal strCode As String, ByVal AdjustmentType As String, ByVal trans As SqlTransaction) As Boolean
        ...
        If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Adjustment_No) > 0) Then
            Try
                ...
                clsCommonFunctionality.SaveCancelData(objCommonVar.CurrentUserCode, strCode, "TSPL_ADJUSTMENT_HEADER", "adjustment_no", "TSPL_ADJUSTMENT_DETAIL", "Adjustment_No", trans)
                ...
                qry = "update TSPL_ADJUSTMENT_DETAIL_Delete_Data set Delete_By = '" + objCommonVar.CurrentUserCode + "' where Adjustment_No='" + strCode + "'"
                clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Sub ReverseAndUnpost(ByVal strCode As String)
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
            ClsAdjustments.ReverseAndUnpost(strCode, trans)
            trans.Commit()
        Catch ex As Exception
            trans.Rollback()
            Throw New Exception(ex.Message)
    ...
    Public Shared Function ReverseAndUnpost(ByVal strCode As String, ByVal trans As SqlTransaction, ByVal isSkipBalanceCheck As Boolean) As Boolean
        ...
        Try
                ...
                For Each objtr As ClsAdjustmentsDetails In obj.Arr
                    ...
                    If dblBalance < objtr.Item_Quantity Then
                        If Not SettDoNotStopOnItemBalanceExceptionStoreAdjustment Then
                            If Math.Abs(dblBalance - objtr.Item_Quantity) > 0.01 Then
            ...
            Qry = "select Against_Physical_Stock_No from TSPL_ADJUSTMENT_HEADER where Adjustment_No ='" + strCode + "'  and isnull(Against_Physical_Stock_No,'')<>'' "
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            If clsCommon.myLen(VoucherNo) > 0 Then
                ...
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            End If
            VoucherNo = clsDBFuncationality.getSingleValue("select Voucher_No from TSPL_JOURNAL_MASTER_OP where Source_Code in('IC-AD', 'GL-JE') and Source_Doc_No='" + strCode + "'", trans)
            If clsCommon.myLen(VoucherNo) > 0 Then
            ...
            Qry = "update TSPL_BATCH_ITEM set Against_Inv_Movement_Trans_Id=null where Against_Inv_Movement_Trans_Id in (select Trans_Id from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" + strCode + "' and Trans_Type='IC-AD')"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            Qry = " update TSPL_BATCH_ITEM_New set Against_Inv_Movement_New_Trans_Id=null where Against_Inv_Movement_New_Trans_Id in (select Trans_Id from TSPL_INVENTORY_MOVEMENT_NEW where Source_Doc_No='" + strCode + "' and Trans_Type='IC-AD') "
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            ...
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            Qry = "delete from TSPL_INVENTORY_MOVEMENT_New where Source_Doc_No='" + strCode + "' and Trans_Type='IC-AD'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            ...
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, strCode, "TSPL_ADJUSTMENT_HEADER", "adjustment_no", trans)
        Catch ex As Exception
            Throw New Exception("Error in Adjustment no [" + strCode + "]" + Environment.NewLine + ex.Message)
        End Try
        Return True
    End Function
...
Public Class ClsAdjustmentsDetails
#Region "Variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal strLocationCode As String, ByVal Arr As List(Of ClsAdjustmentsDetails), ByVal trans As SqlTransaction, ByVal InoutType As String, ByVal DocDate As DateTime, ByVal HeaderAdjustmentType As String) As Boolean
        Dim ItemCostZeroOnStoreAdjForTypeFlushing = IIf(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.ItemCostZeroOnStoreAdjForTypeFlushing, clsFixedParameterCode.ItemCostZeroOnStoreAdjForTypeFlushing, trans)) = 0, False, True)
        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            Dim counter As Integer = 1
            For Each objtr As ClsAdjustmentsDetails In Arr
                Dim coll As New Hashtable()
                ...
                If (clsCommon.CompairString(HeaderAdjustmentType, "FLG") = CompairStringResult.Equal AndAlso ItemCostZeroOnStoreAdjForTypeFlushing = True) Then
                    clsCommon.AddColumnsForChange(coll, "Unit_Cost", 0)
                    clsCommon.AddColumnsForChange(coll, "Item_Cost", 0)
                    clsCommon.AddColumnsForChange(coll, "FAT_KG", 0)
                    clsCommon.AddColumnsForChange(coll, "FAT_Pers", 0)
                    clsCommon.AddColumnsForChange(coll, "SNF_Pers", 0)
                    clsCommon.AddColumnsForChange(coll, "SNF_KG", 0)
                ...
                clsSerializeInvenotry.SaveData("IC-AD", strDocNo, DocDate, IIf(clsCommon.CompairString(InoutType, "In") = CompairStringResult.Equal, "I", "O"), objtr.Item_Code, strLocationCode, counter, objtr.arrSrItem, trans)
                clsBatchInventory.SaveData("IC-AD", strDocNo, DocDate, IIf(clsCommon.CompairString(InoutType, "In") = CompairStringResult.Equal, "I", "O"), objtr.Item_Code, strLocationCode, counter, objtr.mrp, objtr.Unit_Code, objtr.arrBatchItem, trans)
                clsBatchInventoryNew.SaveData("IC-AD", strDocNo, DocDate, IIf(clsCommon.CompairString(InoutType, "In") = CompairStringResult.Equal, "I", "O"), objtr.Item_Code, strLocationCode, counter, objtr.mrp, objtr.Unit_Code, objtr.arrBatchItemNew, trans)

                ' SaveTagNo()
                Dim objAd As New ClsAdjustmentsDetails
