...
Public Class clsMilkQualityCheck_JOW
    ...
    Public arrQcParam As List(Of clsMilkQcParam_JOW) = Nothing
    ...
    Public Shared Function ReverseAndUnpost(ByVal strDocNo As String) As Boolean
        ...
        Try
            ...
            End If
            Dim Qry As String = "select isPosted from TSPL_JWO_QUALITY_CHECK where Qc_no='" + strDocNo + "'"
            If Not clsCommon.myCdbl(clsDBFuncationality.getSingleValue(Qry, trans)) = 1 Then
                Throw New Exception("Transaction status should be posted for reverse and unpost")
            End If
            Dim isUsed As Integer = clsDBFuncationality.getSingleValue(" select count(*) from TSPL_JWO_UNLOADING where Qc_no='" & strDocNo & "'", trans)
            If isUsed > 0 Then
                Throw New Exception("QC No is in use")
            End If
            Qry = "Update TSPL_JWO_QUALITY_CHECK set isPosted = 0,Posting_Date=null where QC_No='" + strDocNo + "'"
    ...
    Public Shared Function postData(ByVal strQCNo As String, ByVal docType As String, ByVal formId As String, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            Dim obj As clsMilkQualityCheck_JOW = clsMilkQualityCheck_JOW.getData(strQCNo, docType, NavigatorType.Current, trans)
            ...
            '--------------------
            'Dim isResult As Boolean = clsApprovalScreen.CheckApprovalLevel(formId, "TSPL_JWO_QUALITY_CHECK", "QC_no", obj.QC_No, trans)
            ...
            Dim strQry As String = " update TSPL_JWO_QUALITY_CHECK set isPosted='1', Posting_Date='" & clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy") & "' where qc_no='" & strQCNo & "'"
            isPosted = isPosted AndAlso clsDBFuncationality.ExecuteNonQuery(strQry, trans)
            'isPosted = clsMilkQualityCheck_JOW.SaveAndPostUnloadingGateOutMilkTransferIn(obj.Gate_Entry_No, trans)
            'strQry = " update TSPL_JWO_Weighment set status=2 where gate_entry_no='" & obj.Gate_Entry_No & "'"
            'isPosted = isPosted AndAlso clsDBFuncationality.ExecuteNonQuery(strQry, trans)
    ...
    Public Shared Function deleteData(ByVal strDocNo As String, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            Dim qry2 As String = "delete from TSPL_JWO_QUALITY_CHECK where qc_no='" & strDocNo & "'"
            Dim qry1 As String = "delete from TSPL_JWO_QC_Parameter_Detail where qc_no='" & strDocNo & "'"
    ...
    Public Shared Function getFinder(ByVal whrcls As String, ByVal curcode As String, ByVal isButtonClicked As Boolean) As String
        Dim str As String = ""
        Try
            Dim qry As String = " select TSPL_JWO_QUALITY_CHECK.QC_No as Code ,TSPL_JWO_QUALITY_CHECK.QC_In_Date_Time as [Qc In Date Time] ,TSPL_JWO_QUALITY_CHECK.QC_Out_Date_Time as [Qc Out Date Time] ,TSPL_JWO_QUALITY_CHECK.Gate_Entry_No as [Gate Entry No] ,TSPL_JWO_QUALITY_CHECK.Doc_Type as [Doc Type] ,TSPL_JWO_QUALITY_CHECK.Gate_Entry_Date_And_Time as [Gate Entry Date And Time] ,TSPL_JWO_QUALITY_CHECK.Challan_No as [Challan No] ,TSPL_JWO_QUALITY_CHECK.Challan_Date as [Challan Date] ,TSPL_JWO_QUALITY_CHECK.Tanker_No as [Tanker No] ,case when isnull( TSPL_JWO_QUALITY_CHECK.isPosted,0)=0 then 'No' else 'Yes' end as [Is Posted] ,TSPL_JWO_QUALITY_CHECK.Posting_Date as [Posting Date] ,TSPL_JWO_QUALITY_CHECK.Dispatched_From_Mcc_Code as [Dispatched From Mcc Code] ,TSPL_JWO_QUALITY_CHECK.Dispatched_From_Mcc_Desc as [Dispatched From Mcc Desc] ,TSPL_JWO_QUALITY_CHECK.location_Code as [Location Code] ,TSPL_JWO_QUALITY_CHECK.Location_Desc as [Location Desc] ,TSPL_JWO_QUALITY_CHECK.Vendor_Code as [Vendor Code] ,TSPL_JWO_QUALITY_CHECK.Vendor_Desc as [Vendor Desc] ,TSPL_JWO_QUALITY_CHECK.Item_Code as [Item Code] ,TSPL_JWO_QUALITY_CHECK.Item_Desc as [Item Desc] ,TSPL_JWO_QUALITY_CHECK.Qty_In_Kg as [Qty In Kg] ,TSPL_JWO_QUALITY_CHECK.snf_Per as [SNF %] ,TSPL_JWO_QUALITY_CHECK.fat_per as [FAT %] ,TSPL_JWO_QUALITY_CHECK.Dip_Value as [Dip Value] ,TSPL_JWO_QUALITY_CHECK.Created_By as [Created By] ,TSPL_JWO_QUALITY_CHECK.Created_Date as [Created Date] ,TSPL_JWO_QUALITY_CHECK.Modify_By as [Modify By] ,TSPL_JWO_QUALITY_CHECK.Modify_Date as [Modify Date] ,TSPL_JWO_QUALITY_CHECK.comp_code as [Company Code] ,TSPL_JWO_QUALITY_CHECK.Weighment_No as [Weighment No] ,TSPL_JWO_QUALITY_CHECK.Weighment_Date as [Weighment Date],ISNULL(TSPL_JWO_UNLOADING.unloading_no,'') as [Unloading No] , case when ISNULL(TSPL_JWO_UNLOADING.isPosted ,0)=0 then 'Not Done' else 'Done' end as [Unloading Posting Status],case when  ISNULL(TSPL_JWO_QUALITY_CHECK.is_Param_Accepted ,0)=0 and  ISNULL(TSPL_JWO_QUALITY_CHECK.isPosted,0)=0 and  ISNULL(TSPL_JWO_QUALITY_CHECK.QC_No ,'')='' then 'QC Status Not Found' when  ISNULL(TSPL_JWO_QUALITY_CHECK.is_Param_Accepted ,0)=1 and  ISNULL(TSPL_JWO_QUALITY_CHECK.isPosted,0)=1 then 'QC Accepted' when  ISNULL(TSPL_JWO_QUALITY_CHECK.is_Param_Accepted ,0)=0 and  ISNULL(TSPL_JWO_QUALITY_CHECK.isPosted,0)=1 then 'QC Rejected' when  ISNULL(TSPL_JWO_QUALITY_CHECK.is_Param_Accepted ,0)=2 and  ISNULL(TSPL_JWO_QUALITY_CHECK.isPosted,0)=1 then 'Accepted With Special Approval' else 'QC Pending' end as [QC Status]  From TSPL_JWO_QUALITY_CHECK left outer join TSPL_JWO_UNLOADING on TSPL_JWO_UNLOADING.Gate_Entry_No=TSPL_JWO_QUALITY_CHECK.Gate_Entry_No "
    ...
    Public Shared Function saveData(ByVal obj As clsMilkQualityCheck_JOW, ByVal trans As SqlTransaction, Optional ByVal isHistory As Boolean = False) As Boolean
        Try
            ...
            If obj.isNewEntry OrElse isHistory Then
                ...
                issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, IIf(isHistory, "TSPL_JWO_QUALITY_CHECK_Hist_Data", "TSPL_JWO_QUALITY_CHECK"), OMInsertOrUpdate.Insert, "", trans)
            Else
                issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_QUALITY_CHECK", OMInsertOrUpdate.Update, "TSPL_JWO_QUALITY_CHECK.QC_No='" + obj.QC_No + "'", trans)
            End If
            issaved = issaved AndAlso clsMilkQcParam_JOW.saveData(obj.arrQcParam, obj.QC_No, trans, isHistory)
            Return issaved
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    ...
    '    qry = "select count(*) from TSPL_JWO_QUALITY_CHECK where gate_entry_no='" & strGateEntryNo & "' and isposted=1"
    ...
    Public Shared Function getData(ByVal strCode As String, ByVal docType As String, ByVal navtype As NavigatorType, Optional ByVal trans As SqlTransaction = Nothing) As clsMilkQualityCheck_JOW
        Dim obj As clsMilkQualityCheck_JOW = Nothing
        Try
            Dim whrCls As String = String.Empty
            Dim qst As String = " select *   From TSPL_JWO_QUALITY_CHECK   where 1=1 and doc_type='" & docType & "' "
            If Not clsMccMaster.isCurrentUserHO(trans) Then
                If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                    whrCls = "and TSPL_JWO_QUALITY_CHECK.location_code in (" & objCommonVar.strCurrUserLocations & ")"
                End If
            End If
            qst = qst & whrCls
            Select Case navtype
                Case NavigatorType.Current
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in ('" + strCode + "') "
                Case NavigatorType.Next
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select min(QC_No ) from TSPL_JWO_QUALITY_CHECK where QC_No  >'" + strCode + "' and doc_type='" & docType & "'  " & whrCls & ")"
                Case NavigatorType.First
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select MIN(QC_No ) from TSPL_JWO_QUALITY_CHECK where doc_type='" & docType & "'  " & whrCls & ")"
                Case NavigatorType.Last
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select Max(QC_No ) from TSPL_JWO_QUALITY_CHECK where doc_type='" & docType & "'  " & whrCls & ")"
                Case NavigatorType.Previous
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select Max(QC_No ) from TSPL_JWO_QUALITY_CHECK where QC_No  <'" + strCode + "' and doc_type='" & docType & "'  " & whrCls & ")"
            End Select
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qst, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                obj = New clsMilkQualityCheck_JOW()
                ...
                obj.Weighment_No = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select weighment_no from TSPL_JWO_Weighment where gate_entry_no='" & obj.Gate_Entry_No & "'", trans))
                If clsCommon.myLen(obj.Weighment_No) > 0 Then
                    obj.Weighment_Date = clsCommon.myCDate(clsDBFuncationality.getSingleValue("select weighment_date from TSPL_JWO_Weighment where gate_entry_no='" & obj.Gate_Entry_No & "'", trans))
                End If
                ...
                ''-----------------------------
                obj.arrQcParam = clsMilkQcParam_JOW.getData(obj.QC_No, trans)
        ...
        Return obj
    End Function
    Public Shared Function getData(ByVal strCode As String, ByVal navtype As NavigatorType, Optional ByVal trans As SqlTransaction = Nothing) As clsMilkQualityCheck_JOW
        Dim obj As New clsMilkQualityCheck_JOW
        Try
            Dim whrCls As String = String.Empty
            Dim qst As String = " select *   From TSPL_JWO_QUALITY_CHECK   where 1=1 "
            ...
            Select Case navtype
                Case NavigatorType.Current
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in ('" + strCode + "') "
                Case NavigatorType.Next
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select min(QC_No ) from TSPL_JWO_QUALITY_CHECK where QC_No  >'" + strCode & "'" & whrCls & ")"
                Case NavigatorType.First
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select MIN(QC_No ) from TSPL_JWO_QUALITY_CHECK where 1=1 " & whrCls & ")"
                Case NavigatorType.Last
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select Max(QC_No ) from TSPL_JWO_QUALITY_CHECK where 1=1  " & whrCls & ")"
                Case NavigatorType.Previous
                    qst += " and TSPL_JWO_QUALITY_CHECK.QC_No in (select Max(QC_No ) from TSPL_JWO_QUALITY_CHECK where QC_No  <'" + strCode + "'   " & whrCls & ")"
            End Select
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qst, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                obj.QC_No = clsCommon.myCstr(dt.Rows(0)("QC_No"))
    ...
    Public Shared Function isControlSample(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        If clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select case when isnull(Control_Sample,'')='NO' then 0 else 1 end as ControlSample  from TSPL_MCC_Dispatch_Challan left outer join TSPL_JWO_GATE_ENTRY on TSPL_JWO_GATE_ENTRY.Challan_No=TSPL_MCC_Dispatch_Challan.Chalan_NO where TSPL_JWO_GATE_ENTRY.Gate_Entry_No='" & GateEntryNo & "'  ", trans)) = 1 Then
            rValue = True
        Else
            rValue = False
        End If
    ...
    Public Shared Function isMccInDoc(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim rValue As Boolean = False
        If clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select case when ISNULL(Doc_Type,'')='Sku_Receipt' then 1 else 0 end as DocType   from TSPL_JWO_GATE_ENTRY where TSPL_JWO_GATE_ENTRY.Gate_Entry_No='" & GateEntryNo & "'  ", trans)) = 1 Then
            rValue = True
        Else
            rValue = False
        End If
    ...
    Public Shared Function isWeighmentDone(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim rValue As Boolean = False
        If clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select case when isnull(Weighment_No,'')='' then 0 else 1 end as WeighmentNo   from TSPL_JWO_Weighment  where Gate_Entry_No='" & GateEntryNo & "' and isPosted=1", trans)) = 1 Then
            rValue = True
        Else
            rValue = False
        End If
    ...
    Public Shared Function getChallanNo(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As String
        Dim rValue As String = ""
        rValue = clsCommon.myCstr(clsDBFuncationality.getSingleValue(" select isnull(Challan_No,'') as ChallanNo  from TSPL_JWO_GATE_ENTRY where Gate_Entry_No ='" & GateEntryNo & "' ", trans))
        Return rValue
    End Function
    Public Shared Function getWeighmentNo(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As String
        Dim rValue As String = ""
        rValue = clsCommon.myCstr(clsDBFuncationality.getSingleValue(" select  isnull(Weighment_No,'') as WeighmentNo   from TSPL_JWO_Weighment  where Gate_Entry_No='" & GateEntryNo & "' and isPosted=1", trans))
        Return rValue
    End Function
    Public Shared Function isQCDone(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim rValue As Boolean = False
        If clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select case when isnull(QC_No ,'')='' then 0 else 1 end as QCNO   from TSPL_JWO_QUALITY_CHECK  where Gate_Entry_No='" & GateEntryNo & "' and isPosted=1", trans)) = 1 Then
            rValue = True
        Else
            rValue = False
        End If
    ...
    Public Shared Function getQCNo(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As String
        Dim rValue As String = ""
        rValue = clsCommon.myCstr(clsDBFuncationality.getSingleValue(" select  isnull(QC_No ,'') as QCNO   from TSPL_JWO_QUALITY_CHECK where Gate_Entry_No='" & GateEntryNo & "' and isPosted=1 ", trans))
        Return rValue
    End Function
    Public Shared Function ControlSampleFAT(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Double
        ...
        rValue = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select  isnull(control_sample_fat ,0) ControlSample  from TSPL_MCC_Dispatch_Challan left outer join TSPL_JWO_GATE_ENTRY on TSPL_JWO_GATE_ENTRY.Challan_No=TSPL_MCC_Dispatch_Challan.Chalan_NO where TSPL_JWO_GATE_ENTRY.Gate_Entry_No='" & GateEntryNo & "'  ", trans))
        Return rValue
    End Function
    ...
    Public Shared Function SaveAndPostUnloadingData(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            obj.Weighment_No = clsMilkQualityCheck_JOW.getWeighmentNo(GateEntryNo, trans)
            obj.QC_No = clsMilkQualityCheck_JOW.getQCNo(GateEntryNo, trans)
            obj.location_Code = objGt.location_Code
            obj.Sub_location_Code = clsMilkQualityCheck_JOW.getVirtualSilo(objGt.location_Code, trans)
    ...
    Public Shared Function SaveAndPostUnloadingGateOutMilkTransferIn(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim isSaved As Boolean = True
        Try
            If clsMilkQualityCheck_JOW.isMccInDoc(GateEntryNo, trans) Then
                If clsMilkQualityCheck_JOW.isIntermittentDoc(clsMilkQualityCheck_JOW.getChallanNo(GateEntryNo, trans), trans) Then
                    If clsMilkQualityCheck_JOW.isWeighmentDone(GateEntryNo, trans) And clsMilkQualityCheck_JOW.isQCDone(GateEntryNo, trans) Then
                        Dim objGt As clsGateEntry = clsGateEntry.getData(GateEntryNo, NavigatorType.Current, trans)
                        If clsMilkQualityCheck_JOW.isVirtualSiloFound(objGt.location_Code, trans) Then
                            isSaved = clsMilkQualityCheck_JOW.SaveAndPostUnloadingData(GateEntryNo, trans)
                            isSaved = clsMilkQualityCheck_JOW.SaveGateOutData(GateEntryNo, trans)
                            isSaved = clsMilkQualityCheck_JOW.SaveAndPostTransferInData(GateEntryNo, trans)
                        Else
    ...
    Public Shared Function SaveAndPostTransferInData(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            obj.Dispatch_Challan_No = clsCommon.myCstr(clsMilkQualityCheck_JOW.getChallanNo(GateEntryNo, trans))
            obj.Weighment_No = clsMilkQualityCheck_JOW.getWeighmentNo(GateEntryNo, trans)
            obj.Qc_No = clsMilkQualityCheck_JOW.getQCNo(GateEntryNo, trans)
    ...
    Public Shared Function SaveGateOutData(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            Dim objGt As clsGateEntry = clsGateEntry.getData(GateEntryNo, NavigatorType.Current, trans)
            obj.isNewEntry = True
            Dim strLoc As String = clsDBFuncationality.getSingleValue("select location_code from TSPL_JWO_GATE_ENTRY  where gate_entry_no='" & GateEntryNo & "'", trans)
            Dim dt As Date = clsCommon.GETSERVERDATE(trans, "dd/MMM/yyyy hh:mm:ss tt")
    ...
    End Function
End Class
Public Class clsMilkQcParam_JOW
    Public QC_No As String = String.Empty
    ...
    Public Shared Function saveData(ByVal arrObj As List(Of clsMilkQcParam_JOW), ByVal strQCNo As String, ByVal trans As SqlTransaction, Optional ByVal isHistory As Boolean = False) As Boolean
        Dim issaved As Boolean = True
        Try
            ...
            If arrObj IsNot Nothing Then
                If Not isHistory Then
                    Dim qry As String = "delete from TSPL_JWO_QC_Parameter_Detail where QC_No='" & strQCNo & "'"
                    issaved = issaved AndAlso clsDBFuncationality.ExecuteNonQuery(qry, trans)
                End If
                For Each obj As clsMilkQcParam_JOW In arrObj
                    coll = New Hashtable()
                    ...
                    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, IIf(isHistory, "TSPL_JWO_QC_Parameter_Detail_History", "TSPL_JWO_QC_Parameter_Detail"), OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
        ...
        Return issaved
    End Function
    Public Shared Function getData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsMilkQcParam_JOW)
        Dim arrObj As List(Of clsMilkQcParam_JOW) = Nothing
        Try
            Dim obj As clsMilkQcParam_JOW = Nothing
            Dim qry As String = "select * from TSPL_JWO_QC_Parameter_Detail where QC_No='" & strQCNo & "'"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsMilkQcParam_JOW)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsMilkQcParam_JOW()
    ...
    End Function
End Class
Public Class clsMilkQCManualSealDetail_JOW
    Public Chalan_No As String = String.Empty
    Public Seal_No As String = String.Empty
    Public Status As String = String.Empty
    Public Shared Function SaveData(ByVal arr As List(Of clsMilkQCManualSealDetail_JOW), ByVal tran As SqlTransaction) As Boolean
        'Try
        '    Dim i As Integer = 0
        '    Dim issaved As Boolean = True
        '    If arr.Count > 0 Then
    ...
    End Function
    Public Shared Function SaveData(ByVal arr As List(Of clsMilkQCManualSealDetail_JOW), ByVal tran As SqlTransaction, ByVal isReversed As Boolean) As Boolean
        Dim issaved As Boolean = True
        Try
            Dim i As Integer = 0
            If arr.Count > 0 Then
                If Not isReversed Then
                    Dim qry As String = "delete from TSPL_JWO_QC_Manual_Seal_Details where  chalan_No='" & arr.Item(0).Chalan_No & "'"
                    clsDBFuncationality.ExecuteNonQuery(qry, tran)
                End If
                For i = 0 To arr.Count - 1
                    Dim coll As New Hashtable()
                    ...
                    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, IIf(isReversed, "TSPL_JWO_QC_Manual_Seal_Details_History", "TSPL_JWO_QC_Manual_Seal_Details"), OMInsertOrUpdate.Insert, "", tran)
                Next
            End If
    ...
    Public Shared Function DeleteData(ByVal challan_no As String, ByVal tran As SqlTransaction) As Boolean
        ...
        Dim qry As String = "delete from TSPL_JWO_QC_Manual_Seal_Details where  chalan_No='" & challan_no & "'"
        isdeleted = clsDBFuncationality.ExecuteNonQuery(qry, tran)
        Return isdeleted
    End Function
    Public Shared Function getData(ByVal chalanNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsMilkQCManualSealDetail_JOW)
        Dim arr As New List(Of clsMilkQCManualSealDetail_JOW)
        Try
            Dim obj As New clsMilkQCManualSealDetail_JOW
            Dim q As String = "select * from TSPL_JWO_QC_Manual_Seal_Details where chalan_no='" & chalanNo & "'"
            Dim dtbl As DataTable = clsDBFuncationality.GetDataTable(q, trans)
            If dtbl IsNot Nothing AndAlso dtbl.Rows.Count > 0 Then
                For i As Integer = 0 To dtbl.Rows.Count - 1
                    obj = New clsMilkQCManualSealDetail_JOW()
...
Public Class clsMilkQCPaperSealDetail_JOW
    Public Chalan_No As String = String.Empty
    Public Seal_No As String = String.Empty
    Public Status As String = String.Empty
    Public Shared Function SaveData(ByVal arr As List(Of clsMilkQCPaperSealDetail_JOW), ByVal tran As SqlTransaction) As Boolean
        'Try
        '    Dim i As Integer = 0
        '    Dim issaved As Boolean = True
        '    If arr.Count > 0 Then
    ...
    Public Shared Function DeleteData(ByVal challan_no As String, ByVal tran As SqlTransaction) As Boolean
        Dim isdeleted As Boolean = False
        Dim qry As String = "delete from TSPL_JWO_QC_Paper_Seal_Details where  chalan_No='" & challan_no & "'"
        isdeleted = clsDBFuncationality.ExecuteNonQuery(qry, tran)
        Return isdeleted
    End Function
    Public Shared Function SaveData(ByVal arr As List(Of clsMilkQCPaperSealDetail_JOW), ByVal tran As SqlTransaction, ByVal isReversed As Boolean) As Boolean
        Dim issaved As Boolean = True
        Try
            Dim i As Integer = 0
            ...
            If arr.Count > 0 Then
                If Not isReversed Then
                    Dim qry As String = "delete from TSPL_JWO_QC_Paper_Seal_Details where  chalan_No='" & arr.Item(0).Chalan_No & "'"
                    clsDBFuncationality.ExecuteNonQuery(qry, tran)
                End If
                ...
                For i = 0 To arr.Count - 1
                    ...
                    clsCommon.AddColumnsForChange(coll, "Seal_No", arr.Item(i).Seal_No)
                    clsCommon.AddColumnsForChange(coll, "Status", arr.Item(i).Status)
                    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, IIf(isReversed, "TSPL_JWO_QC_Paper_Seal_Details_History", "TSPL_JWO_QC_Paper_Seal_Details"), OMInsertOrUpdate.Insert, "", tran)
                Next
            End If
    ...
    Public Shared Function getData(ByVal chalanNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsMilkQCPaperSealDetail_JOW)
        Dim arr As New List(Of clsMilkQCPaperSealDetail_JOW)
        Try
            Dim obj As New clsMilkQCPaperSealDetail_JOW
            Dim q As String = "select * from TSPL_JWO_QC_Paper_Seal_Details where chalan_no='" & chalanNo & "'"
            Dim dtbl As DataTable = clsDBFuncationality.GetDataTable(q, trans)
            If dtbl IsNot Nothing AndAlso dtbl.Rows.Count > 0 Then
                For i As Integer = 0 To dtbl.Rows.Count - 1
                    obj = New clsMilkQCPaperSealDetail_JOW()
                    obj.Chalan_No = clsCommon.myCstr(dtbl.Rows(i)("Chalan_No"))
                    obj.Seal_No = clsCommon.myCstr(dtbl.Rows(i)("Seal_No"))
                    obj.Status = clsCommon.myCstr(dtbl.Rows(i)("Status"))
                    arr.Add(obj)
