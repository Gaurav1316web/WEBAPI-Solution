...
#Region "Variables"
    ...
    Public ArrRawItem As List(Of clsJWOEstimateRawItem) = Nothing
#End Region

    Public Function saveData(ByVal obj As clsJWOEstimate, ByVal isNewEntry As Boolean) As Boolean
        Dim trans As SqlTransaction = Nothing
        Try
            trans = clsDBFuncationality.GetTransactin()
            Dim qry As String = "delete from TSPL_JWO_ESTIMATION_RAW_ITEM where Document_NO='" + obj.Document_NO + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_SNF_PRODUCTION_QC_PARAMETER where Document_NO='" + obj.Document_NO + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_SNF_PRODUCTION where Document_NO='" + obj.Document_NO + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_FAT_PRODUCTION_QC_PARAMETER where Document_NO='" + obj.Document_NO + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_FAT_PRODUCTION where Document_NO='" + obj.Document_NO + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_TRANSFER where Document_NO='" + obj.Document_NO + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
            ...
            If isNewEntry Then
                obj.Document_NO = clsERPFuncationality.GetNextCode(trans, clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MM/yyyy"), clsDocType.JWOEstimate, "", obj.Location_Code)
                ...
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_HEAD", OMInsertOrUpdate.Insert, "", trans)
            Else
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_HEAD", OMInsertOrUpdate.Update, "TSPL_JWO_ESTIMATION_HEAD.Document_NO='" + obj.Document_NO + "'", trans)
            End If
            clsJWOEstimateTransfer.SaveData(obj.Document_NO, obj.Document_Date, ArrWeighment, trans)
            clsJWOEstimateFATProduction.SaveData(obj.Document_NO, obj.Document_Date, ArrFATProduction, trans)
            clsJWOEstimateSNFProduction.SaveData(obj.Document_NO, obj.Document_Date, ArrSNFProducion, trans)
            clsJWOEstimateRawItem.SaveData(obj.Document_NO, obj.Document_Date, ArrRawItem, trans)
    ...
    Public Shared Function getFinder(ByVal whrcls As String, ByVal curcode As String, ByVal isButtonClicked As Boolean) As String
        Dim str As String = ""
        Dim qry As String = "  select TSPL_JWO_ESTIMATION_HEAD.Document_NO as  Code , convert (varchar,TSPL_JWO_ESTIMATION_HEAD.Document_Date,103) as Date,case when  TSPL_JWO_ESTIMATION_HEAD.Status = 1 then 'Posted' else 'Pending' end Status ,TSPL_JWO_ESTIMATION_HEAD.Location_Code as Location , TSPL_LOCATION_MASTER.Location_Desc as [Location Desc] ,TSPL_JWO_ESTIMATION_HEAD.Vendor_Code as [Vendor Code],tspl_vendor_master.Vendor_Name as [Vendor Name],TSPL_JWO_ESTIMATION_HEAD.Item_Structure_FAT as [FAT Structure Code],TSPL_STRUCTURE_MASTERFATFAT.Structure_Descq as [FAT Structure Desc],TSPL_JWO_ESTIMATION_HEAD.Formula_code_FAT as [FAT Formula Code],TSPL_JWO_ESTIMATION_HEAD.Formula_Date_FAT as [FAT Formula Date],TSPL_JWO_ESTIMATION_HEAD.Formula_FAT as [FAT Formula],TSPL_JWO_ESTIMATION_HEAD.Item_Structure_SNF as [SNF Structure Code],TSPL_STRUCTURE_MASTERSNFSNF.Structure_Descq as [SNF Structure Desc],TSPL_JWO_ESTIMATION_HEAD.Formula_code_SNF as [SNF Formula Code],TSPL_JWO_ESTIMATION_HEAD.Formula_Date_SNF as [SNF Formula Date],TSPL_JWO_ESTIMATION_HEAD.Formula_SNF as [SNF Formula]" + Environment.NewLine +
        "From TSPL_JWO_ESTIMATION_HEAD" + Environment.NewLine +
        "left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code = TSPL_JWO_ESTIMATION_HEAD.Location_Code   " + Environment.NewLine +
        "left outer join tspl_vendor_master on tspl_vendor_master.Vendor_Code = TSPL_JWO_ESTIMATION_HEAD.Vendor_Code  " + Environment.NewLine +
        "left outer join TSPL_STRUCTURE_MASTER as TSPL_STRUCTURE_MASTERFATFAT on TSPL_STRUCTURE_MASTERFATFAT.Structure_Code = TSPL_JWO_ESTIMATION_HEAD.Item_Structure_FAT " + Environment.NewLine +
        "left outer join TSPL_STRUCTURE_MASTER as TSPL_STRUCTURE_MASTERSNFSNF on TSPL_STRUCTURE_MASTERSNFSNF.Structure_Code = TSPL_JWO_ESTIMATION_HEAD.Item_Structure_SNF "
        str = clsCommon.ShowSelectForm("JWESTIMATE@FND", qry, "Code", whrcls, curcode, "TSPL_JWO_ESTIMATION_HEAD.Document_NO", isButtonClicked)
        Return str
    End Function

    Public Shared Function GetData(ByVal strCode As String, ByVal NavType As NavigatorType, ByVal trans As SqlTransaction) As clsJWOEstimate
        Dim obj As clsJWOEstimate = Nothing
        Dim qry As String = "SELECT  TSPL_JWO_ESTIMATION_HEAD.* FROM TSPL_JWO_ESTIMATION_HEAD where 2=2"
        Select Case NavType
            Case NavigatorType.First
                qry += " and TSPL_JWO_ESTIMATION_HEAD.Document_NO = (select MIN(Document_NO) from TSPL_JWO_ESTIMATION_HEAD where 1=1 )"
            Case NavigatorType.Last
                qry += " and TSPL_JWO_ESTIMATION_HEAD.Document_NO = (select Max(Document_NO) from TSPL_JWO_ESTIMATION_HEAD where 1=1 )"
            Case NavigatorType.Next
                qry += " and TSPL_JWO_ESTIMATION_HEAD.Document_NO = (select Min(Document_NO) from TSPL_JWO_ESTIMATION_HEAD where Document_NO>'" + strCode + "' )"
            Case NavigatorType.Previous
                qry += " and TSPL_JWO_ESTIMATION_HEAD.Document_NO = (select Max(Document_NO) from TSPL_JWO_ESTIMATION_HEAD where Document_NO<'" + strCode + "')"
            Case NavigatorType.Current
                qry += " and TSPL_JWO_ESTIMATION_HEAD.Document_NO = '" + strCode + "'"
        End Select
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)

        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            obj = New clsJWOEstimate()
            ...
            obj.ArrSNFProducion = clsJWOEstimateSNFProduction.GetData(obj.Document_NO, trans)
            obj.ArrRawItem = clsJWOEstimateRawItem.GetData(obj.Document_NO, trans)
        End If
        Return obj
    End Function
    ...
    Public Shared Function DeleteData(ByVal strCode As String) As Boolean
        ...
        Try
            ...
            End If
            Dim qry As String = "select status from TSPL_JWO_ESTIMATION_HEAD where Document_NO='" + strCode + "'"
            ...
            End If
            qry = "delete from TSPL_JWO_ESTIMATION_RAW_ITEM where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_SNF_PRODUCTION_QC_PARAMETER where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_SNF_PRODUCTION where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_FAT_PRODUCTION_QC_PARAMETER where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_FAT_PRODUCTION where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_TRANSFER where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)

            qry = "delete from TSPL_JWO_ESTIMATION_HEAD where Document_NO='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Shared Function PostData(ByVal strCode As String, ByVal trans As SqlTransaction) As Boolean
        ...
        If obj.SNF_KG_Raw_Item > 0 Then
            If obj.ArrSNFProducion Is Nothing OrElse obj.ArrSNFProducion.Count <= 0 Then
                Throw New Exception("Please Fill FAT Items")
            End If
            For Each objSNF As clsJWOEstimateSNFProduction In obj.ArrSNFProducion
        ...
        'SaveInventoryMovement(obj, trans)
    ...
    Public Shared Function CreateJournalEntry(ByVal trans As SqlTransaction, ByVal Document_date As Date, ByVal Doc_Code As String, ByVal Location_code As String, Optional ByVal strVourcherNoForRecreateOnly As String = "") As Boolean
        Try
            If clsCommon.myLen(Doc_Code) > 0 Then
                ...
                Dim qry As String = " select TSPL_INVENTORY_MOVEMENT.Item_Code,TSPL_PURCHASE_ACCOUNTS.Purchase_Class_Code,TSPL_PURCHASE_ACCOUNTS.Inv_Control_Account,TSPL_PURCHASE_ACCOUNTS.RM_Consumption AS GL_account ,TSPL_INVENTORY_MOVEMENT.Avg_Cost,TSPL_JWO_ESTIMATION_HEAD.Location_Code ,TSPL_JWO_ESTIMATION_HEAD.Document_NO ,TSPL_JWO_ESTIMATION_HEAD.Document_Date   from TSPL_INVENTORY_MOVEMENT  " & Environment.NewLine &
" Left OUTER JOIN TSPL_JWO_ESTIMATION_HEAD On TSPL_JWO_ESTIMATION_HEAD.Document_NO =TSPL_INVENTORY_MOVEMENT.Source_Doc_No  " & Environment.NewLine &
...
" where TSPL_INVENTORY_MOVEMENT.Source_Doc_No ='" & clsCommon.myCstr(Doc_Code) & "' and isnull(tspl_item_master.Product_Type,'') <>'MI' AND TSPL_INVENTORY_MOVEMENT.InOut ='O' AND TSPL_INVENTORY_MOVEMENT.Location_Code <>'" & clsCommon.myCstr(Location_code) & "' "
        ...
        Return True
    End Function
    Public Shared Function SaveInventoryMovement(ByVal obj As clsJWOEstimate, ByVal trans As SqlTransaction) As Boolean
        ...
        If obj.ArrWeighment IsNot Nothing AndAlso obj.ArrWeighment.Count > 0 Then
            For Each objtr As clsJWOEstimateTransfer In obj.ArrWeighment
        ...
        If obj.ArrRawItem IsNot Nothing AndAlso obj.ArrRawItem.Count > 0 Then
            Dim CreateJVofPackingMaterialofJWInwardinJWEstimate As Boolean = IIf(clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.CreateJVofPackingMaterialofJWInwardinJWEstimate, clsFixedParameterCode.CreateJVofPackingMaterialofJWInwardinJWEstimate, trans)) = 1, True, False)
            For Each objtr As clsJWOEstimateRawItem In obj.ArrRawItem
                Dim objInventoryMovemntNew As New clsInventoryMovementNew
                Dim strProductType As String
        ...
        If obj.ArrFATProduction IsNot Nothing AndAlso obj.ArrFATProduction.Count > 0 Then
            For Each objtr As clsJWOEstimateFATProduction In obj.ArrFATProduction
                Dim objInventoryMovemnt As New clsInventoryMovement()
                Dim objInventoryMovemntNew As New clsInventoryMovementNew
                Dim strProductType As String
                strProductType = clsItemMaster.GetItemProductType(objtr.Item_Code, trans)
        ...
        If obj.ArrSNFProducion IsNot Nothing AndAlso obj.ArrSNFProducion.Count > 0 Then
            For Each objtr As clsJWOEstimateSNFProduction In obj.ArrSNFProducion
                Dim objInventoryMovemnt As New clsInventoryMovement()
                Dim objInventoryMovemntNew As New clsInventoryMovementNew
                Dim strProductType As String
                strProductType = clsItemMaster.GetItemProductType(objtr.Item_Code, trans)
    ...
    Public Shared Function ReverseAndUnpostData(ByVal strCode As String) As Boolean
        ...
        Try
            ...
            Dim VoucherNo As String = clsDBFuncationality.getSingleValue("select Document_No from tspl_jwo_srn_head where Against_Estimate='" + strCode + "'", trans)
            If clsCommon.myLen(VoucherNo) > 0 Then
                Throw New Exception("Estimate No [" + strCode + "] is used in JWO SRN [" + VoucherNo + "]")
            End If
...
Public Class clsJWOEstimateTransfer
#Region "Variables"
    Public TR_Code As String = Nothing
    Public Document_NO As String = Nothing
    Public SNo As Integer
    Public Parent_SNo As Integer
    Public Transfer_Code As String
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal dtDocDate As DateTime, ByVal Arr As List(Of clsJWOEstimateTransfer), ByVal trans As SqlTransaction) As Boolean

        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsJWOEstimateTransfer In Arr
                Dim coll As New Hashtable()
                ...
                clsCommon.AddColumnsForChange(coll, "Transfer_Code", obj.Transfer_Code)
                clsCommon.AddColumnsForChange(coll, "Transfer_Date", clsCommon.GetPrintDate(obj.Transfer_Date, "dd/MMM/yyyy hh:mm:ss tt"))
                ...
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_TRANSFER", OMInsertOrUpdate.Insert, "", trans)
            Next
        End If
        Return True
    End Function

    Public Shared Function GetData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsJWOEstimateTransfer)
        ...
        Try
            ...
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsJWOEstimateTransfer)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsJWOEstimateTransfer()
...
Public Class clsJWOEstimateFATProduction
#Region "Variables"
    Public TR_Code As String = Nothing
    ...
    Public Estimated_FAT_KG As Decimal = 0
    Public ArrQC As List(Of clsJWOEstimateFATProductionQCParam) = Nothing
#End Region

    Public Shared Function SaveData(ByVal strDocNo As String, ByVal dtDocDate As DateTime, ByVal Arr As List(Of clsJWOEstimateFATProduction), ByVal trans As SqlTransaction) As Boolean
        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsJWOEstimateFATProduction In Arr
                Dim coll As New Hashtable()
                obj.TR_Code = clsERPFuncationality.GetNextCode(trans, dtDocDate, clsDocType.Detail, clsDocTransactionType.Detail, "")
                ...
                clsCommon.AddColumnsForChange(coll, "Estimated_FAT_KG", obj.Estimated_FAT_KG)
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_FAT_PRODUCTION", OMInsertOrUpdate.Insert, "", trans)
                clsJWOEstimateFATProductionQCParam.saveData(strDocNo, obj.TR_Code, obj.ArrQC, trans)
            Next
        End If
        Return True
    End Function

    Public Shared Function GetData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsJWOEstimateFATProduction)
        Dim arrObj As List(Of clsJWOEstimateFATProduction) = Nothing
        Try
            Dim obj As clsJWOEstimateFATProduction = Nothing
            Dim qry As String = "select TSPL_JWO_ESTIMATION_FAT_PRODUCTION.*,TSPL_ITEM_MASTER.Item_Desc as Item_Name from TSPL_JWO_ESTIMATION_FAT_PRODUCTION left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_JWO_ESTIMATION_FAT_PRODUCTION.Item_Code where TSPL_JWO_ESTIMATION_FAT_PRODUCTION.Document_NO='" & strQCNo & "'"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsJWOEstimateFATProduction)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsJWOEstimateFATProduction()
                    obj.TR_Code = clsCommon.myCstr(dt.Rows(i)("TR_Code"))
                    ...
                    obj.Estimated_FAT_KG = clsCommon.myCdbl(dt.Rows(i)("Estimated_FAT_KG"))
                    obj.ArrQC = clsJWOEstimateFATProductionQCParam.GetData(obj.TR_Code, trans)
                    arrObj.Add(obj)
                Next
            End If
        Catch ex As Exception
...
Public Class clsJWOEstimateFATProductionQCParam
#Region "Variables"
...
#End Region
    ...
    Public Shared Function saveData(ByVal strQCNo As String, ByVal strTRNo As String, ByVal arrObj As List(Of clsJWOEstimateFATProductionQCParam)) As Boolean
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
    ...
    Public Shared Function saveData(ByVal strQCNo As String, ByVal strTRNo As String, ByVal arrObj As List(Of clsJWOEstimateFATProductionQCParam), ByVal trans As SqlTransaction) As Boolean
        Try
            Dim coll As Hashtable
            If arrObj IsNot Nothing Then
                For Each obj As clsJWOEstimateFATProductionQCParam In arrObj
                    coll = New Hashtable()
                    ...
                    clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_FAT_PRODUCTION_QC_PARAMETER", OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
    ...
    Public Shared Function GetData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsJWOEstimateFATProductionQCParam)
        Dim arrObj As List(Of clsJWOEstimateFATProductionQCParam) = Nothing
        Try
            Dim obj As clsJWOEstimateFATProductionQCParam = Nothing
            Dim qry As String = "select * from TSPL_JWO_ESTIMATION_FAT_PRODUCTION_QC_PARAMETER where Parent_TR_Code='" & strQCNo & "'"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsJWOEstimateFATProductionQCParam)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsJWOEstimateFATProductionQCParam()
                    obj.Document_NO = clsCommon.myCstr(dt.Rows(i)("Document_NO"))
...
Public Class clsJWOEstimateSNFProduction
#Region "Variables"
    Public TR_Code As String = Nothing
    Public TR_Date As DateTime
    Public Document_NO As String = Nothing
    ...
    Public Estimated_Qty As Decimal = 0
    Public Estimated_SNF_KG As Decimal = 0
    Public ArrQC As List(Of clsJWOEstimateSNFProductionQCParam) = Nothing
#End Region

    Public Shared Function SaveData(ByVal strDocNo As String, ByVal dtDocDate As DateTime, ByVal Arr As List(Of clsJWOEstimateSNFProduction), ByVal trans As SqlTransaction) As Boolean
        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsJWOEstimateSNFProduction In Arr
                Dim coll As New Hashtable()
                obj.TR_Code = clsERPFuncationality.GetNextCode(trans, dtDocDate, clsDocType.Detail, clsDocTransactionType.Detail, "")
                ...
                clsCommon.AddColumnsForChange(coll, "Estimated_Qty", obj.Estimated_Qty)
                clsCommon.AddColumnsForChange(coll, "Estimated_SNF_KG", obj.Estimated_SNF_KG)
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_SNF_PRODUCTION", OMInsertOrUpdate.Insert, "", trans)
                clsJWOEstimateSNFProductionQCParam.saveData(strDocNo, obj.TR_Code, obj.ArrQC, trans)
            Next
        End If
        Return True
    End Function

    Public Shared Function GetData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsJWOEstimateSNFProduction)
        Dim arrObj As List(Of clsJWOEstimateSNFProduction) = Nothing
        Try
            Dim obj As clsJWOEstimateSNFProduction = Nothing
            Dim qry As String = "select TSPL_JWO_ESTIMATION_SNF_PRODUCTION.*,TSPL_ITEM_MASTER.Item_Desc as Item_Name from TSPL_JWO_ESTIMATION_SNF_PRODUCTION left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_JWO_ESTIMATION_SNF_PRODUCTION.item_code where TSPL_JWO_ESTIMATION_SNF_PRODUCTION.Document_NO='" & strQCNo & "'"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsJWOEstimateSNFProduction)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsJWOEstimateSNFProduction()
                    obj.TR_Code = clsCommon.myCstr(dt.Rows(i)("TR_Code"))
                    obj.TR_Date = clsCommon.myCDate(dt.Rows(i)("TR_Date"))
                    obj.Document_NO = clsCommon.myCstr(dt.Rows(i)("Document_NO"))
                    obj.SNo = clsCommon.myCdbl(dt.Rows(i)("SNo"))
                    ...
                    obj.Estimated_Qty = clsCommon.myCdbl(dt.Rows(i)("Estimated_Qty"))
                    obj.Estimated_SNF_KG = clsCommon.myCdbl(dt.Rows(i)("Estimated_SNF_KG"))
                    obj.ArrQC = clsJWOEstimateSNFProductionQCParam.GetData(obj.TR_Code, trans)
                    arrObj.Add(obj)
                Next
            End If
        Catch ex As Exception
...
Public Class clsJWOEstimateSNFProductionQCParam
#Region "Variables"
    Public Document_NO As String = String.Empty
    Public Parent_TR_Code As String = String.Empty
    Public Param_Field_Code As String = String.Empty
...
#End Region
    ...
    Public Shared Function saveData(ByVal strQCNo As String, ByVal strTRNo As String, ByVal arrObj As List(Of clsJWOEstimateSNFProductionQCParam)) As Boolean
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
            saveData(strQCNo, strTRNo, arrObj, trans)
            trans.Commit()
    ...
    Public Shared Function saveData(ByVal strQCNo As String, ByVal strTRNo As String, ByVal arrObj As List(Of clsJWOEstimateSNFProductionQCParam), ByVal trans As SqlTransaction) As Boolean
        Try
            Dim coll As Hashtable
            If arrObj IsNot Nothing Then
                For Each obj As clsJWOEstimateSNFProductionQCParam In arrObj
                    coll = New Hashtable()
                    clsCommon.AddColumnsForChange(coll, "Document_NO", strQCNo)
                    clsCommon.AddColumnsForChange(coll, "Parent_TR_Code", strTRNo)
                    clsCommon.AddColumnsForChange(coll, "Param_Field_Code", obj.Param_Field_Code)
                    ...
                    clsCommon.AddColumnsForChange(coll, "Param_Type", obj.Param_Type)
                    clsCommon.AddColumnsForChange(coll, "LINE_NO", obj.LINE_NO)
                    clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_SNF_PRODUCTION_QC_PARAMETER", OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
    ...
    Public Shared Function GetData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsJWOEstimateSNFProductionQCParam)
        Dim arrObj As List(Of clsJWOEstimateSNFProductionQCParam) = Nothing
        Try
            Dim obj As clsJWOEstimateSNFProductionQCParam = Nothing
            Dim qry As String = "select * from TSPL_JWO_ESTIMATION_SNF_PRODUCTION_QC_PARAMETER where Parent_TR_Code='" & strQCNo & "' order by LINE_NO"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsJWOEstimateSNFProductionQCParam)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsJWOEstimateSNFProductionQCParam()
                    obj.Document_NO = clsCommon.myCstr(dt.Rows(i)("Document_NO"))
                    obj.Parent_TR_Code = clsCommon.myCstr(dt.Rows(i)("Parent_TR_Code"))
                    obj.Param_Field_Code = clsCommon.myCstr(dt.Rows(i)("Param_Field_Code"))
                    obj.Param_Field_Desc = clsCommon.myCstr(dt.Rows(i)("Param_Field_Desc"))
...
Public Class clsJWOEstimateRawItem
#Region "Variables"
    Public TR_Code As String = Nothing
    Public TR_Date As DateTime
    Public Document_NO As String = Nothing
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal dtDocDate As DateTime, ByVal Arr As List(Of clsJWOEstimateRawItem), ByVal trans As SqlTransaction) As Boolean
        If (Arr IsNot Nothing AndAlso Arr.Count > 0) Then
            For Each obj As clsJWOEstimateRawItem In Arr
                Dim coll As New Hashtable()
                obj.TR_Code = clsERPFuncationality.GetNextCode(trans, dtDocDate, clsDocType.Detail, clsDocTransactionType.Detail, "")
                clsCommon.AddColumnsForChange(coll, "TR_Code", obj.TR_Code)
                clsCommon.AddColumnsForChange(coll, "TR_Date", clsCommon.GetPrintDate(dtDocDate, "dd/MMM/yyyy hh:mm:ss tt"))
                ...
                clsCommonFunctionality.UpdateDataTable(coll, "TSPL_JWO_ESTIMATION_RAW_ITEM", OMInsertOrUpdate.Insert, "", trans)
            Next
        End If
        Return True
    End Function

    Public Shared Function GetData(ByVal strQCNo As String, Optional ByVal trans As SqlTransaction = Nothing) As List(Of clsJWOEstimateRawItem)
        Dim arrObj As List(Of clsJWOEstimateRawItem) = Nothing
        Try
            Dim obj As clsJWOEstimateRawItem = Nothing
            Dim qry As String = "select TSPL_JWO_ESTIMATION_RAW_ITEM.*,TSPL_ITEM_MASTERMain.Item_Desc as Main_Item_Name,TSPL_ITEM_MASTERRaw.Item_Desc as Raw_Item_Name  from TSPL_JWO_ESTIMATION_RAW_ITEM left outer join TSPL_ITEM_MASTER as TSPL_ITEM_MASTERMain on  TSPL_ITEM_MASTERMain.Item_Code=TSPL_JWO_ESTIMATION_RAW_ITEM.Main_Item_Code left outer join TSPL_ITEM_MASTER as TSPL_ITEM_MASTERRaw on  TSPL_ITEM_MASTERRaw.Item_Code=TSPL_JWO_ESTIMATION_RAW_ITEM.Raw_Item_Code where Document_NO='" & strQCNo & "' order by SNo"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arrObj = New List(Of clsJWOEstimateRawItem)
                For i As Integer = 0 To dt.Rows.Count - 1
                    obj = New clsJWOEstimateRawItem()
                    obj.Document_NO = clsCommon.myCstr(dt.Rows(i)("Document_NO"))
                    obj.TR_Code = clsCommon.myCstr(dt.Rows(i)("TR_Code"))
                    obj.TR_Date = clsCommon.myCDate(dt.Rows(i)("TR_Date"))
                    obj.SNo = clsCommon.myCdbl(dt.Rows(i)("SNo"))
