...
Public Class FrmPrintFreshInvoice
    ...
    Public Function PrintInvoiceForAll(ByVal strinvoiceNo As String, ByVal docDate As DateTime, ByVal CustCode As String) As String
        ...
        If clsCommon.myLen(strinvoiceNo) > 0 Then
            ...
            End If
            Qry = "   select Final.*  from ( select Main_Final.*,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.GSTReg_No As SellerGST,TSPL_COMPANY_MASTER.Pan_No,Convert(decimal(18,2),(valueInRs/((Qty_Default*ConversionFactor)/CF))) As RateLtr "
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "GNG") = CompairStringResult.Equal Then
		  ...
		  where Sale_Invoice_No = TSPL_SD_sale_invoice_DETAIL.DOCUMENT_CODE and item_code=TSPL_SD_sale_invoice_DETAIL.Item_Code)) as batchNO, Case when TSPL_CUSTOMER_MASTER.Credit_Customer='Y' THEN 'CREDIT' else '' end as Credit_Customer " &
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JPR") = CompairStringResult.Equal Then
                ...
                Qry += " ) as ITEMDETAIL1 on ITEMDETAIL1.Item_code=TSPL_SD_sale_invoice_DETAIL.Item_Code  " &
                    ...
                    " left outer join TSPL_ITEM_PRICE_MASTER on TSPL_ITEM_PRICE_MASTER.Price_Code=TSPL_SD_sale_invoice_DETAIL.Price_code " &
                    " and TSPL_ITEM_PRICE_MASTER.Location_Code=TSPL_SD_sale_invoice_DETAIL.Location " &
                    " and TSPL_ITEM_PRICE_MASTER.Item_Code=TSPL_SD_sale_invoice_DETAIL.Item_Code and  len(isnull( TSPL_SD_sale_invoice_DETAIL.Price_code  ,''))>0  left outer join TSPL_DELIVERY_NOTE_master_FRESHSALE on TSPL_DELIVERY_NOTE_master_FRESHSALE.Document_No=TSPL_SD_SHIPMENT_HEAD.Against_Delivery_Code " &
                    " where 2=2 and " &
  ...
  left join TSPL_RECEIPT_HEADER on TSPL_BOOKING_MATSER.Against_Receipt_No=TSPL_RECEIPT_HEADER.Receipt_No "
    ...
    Public Function PrintInvoiceForTruckSheetReport(ByVal FromDate, ByVal ToDate, ByVal whrcls) As String
        ...
        Qry = " Select '" + FromDate + "' As 'From_Date',Convert(Varchar(10),'" + ToDate + "') As 'To_Date',*, TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.GSTReg_No As SellerGST,TSPL_COMPANY_MASTER.Pan_No,(Convert(decimal(18,2),(Item_Net_Amt/((QtyCrates*ConversionFactor)/CF)))) As RateLtr  from( select Max(Zone_Code)Zone_Code,Max(Structure_Code)Structure_Code,Max(Main_Final.EInvoice_Type)EInvoice_Type,Max(Main_Final.CF)CF,	Max(Main_Final.ConversionFactor)ConversionFactor,Max(Main_Final.Location_Desc)Location_Desc,	Max(Main_Final.Loc_Short_Name)Loc_Short_Name,	Max(Main_Final.Loc_Pin)Loc_Pin,	
                    ...
                    Max(Main_Final.FreeSchemeInLitres)FreeSchemeInLitres,	Max(Main_Final.RatePerPcs)RatePerPcs,Sum(Main_Final.valueInRs)valueInRs,Sum(Main_Final.Item_Net_Amt)Item_Net_Amt,Sum(Main_Final.Cash_Scheme_Amount)Cash_Scheme_Amount,	Sum(Main_Final.schemeInCrates)schemeInCrates,
                    ...
                    from   ( select DISTINCT final.*,tbl_Brand.Brand,tbl_Brand.BRANDDESC 
