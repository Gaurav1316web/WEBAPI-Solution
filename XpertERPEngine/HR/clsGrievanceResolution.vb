...
Public Class clsGrievanceResolution
...
#Region "Variables"
    ...
    Public ResolutionRemark As String
    Public Allocated_to As String
    Public Resolution_Date As Date
    ...
    Public Shared ObjList As List(Of clsGrievanceResolution)

#End Region

    Public Shared Function GetData(ByVal strCode As String, ByVal strRoute_Code As String, ByVal NavType As NavigatorType) As List(Of clsGrievanceResolution)
    ...
    Public Shared Function GetData(ByVal trans As SqlTransaction) As List(Of clsGrievanceResolution)

        Dim obj As New clsGrievanceResolution()
        Dim objtr As New clsGrievanceResolution

        ObjList = New List(Of clsGrievanceResolution)
        ...
        qry = "select tspl_Grievance_Logging_Detail.*,tspl_grievance_type_master.name as Grv_Name,tspl_employee_Master.emp_Name as Applied_name, frm_dpt.department_name " _
          & " as [frm_dpt_name],to_dpt.department_name as [for_dpt_name],Allocated_to,Alloated_Date as Allocated_date,all_t.emp_name as [Allocated_To_Name],Tspl_Grievance_Logging_Allocation.status,Tspl_Grievance_Logging_Allocation.priority ,Tspl_Grievance_Logging_Allocation.resolution_date,Tspl_Grievance_Logging_Allocation.resolution_Remark,tspl_Grievance_Logging_Detail.doc_date as Logging_Date from tspl_Grievance_Logging_Detail left join  tspl_grievance_type_master on " _
          ...
          & " Tspl_Grievance_Logging_Allocation on Tspl_Grievance_Logging_Allocation.grievance_logging_code=tspl_Grievance_Logging_Detail.code Left join tspl_employee_master all_t on all_t.emp_code=Allocated_to where 2=2   and   Allocated_to in (select EMP_CODE from TSPL_EMPLOYEE_MASTER where user_code='" & objCommonVar.CurrentUserCode & "')"
        Dt = clsDBFuncationality.GetDataTable(qry, trans)
        If (Dt IsNot Nothing AndAlso Dt.Rows.Count > 0) Then
            For Each row As DataRow In Dt.Rows
                obj = New clsGrievanceResolution()

                obj.Greivance_Logging_Code = clsCommon.myCstr(row.Item("Code"))
                obj.Description = clsCommon.myCstr(row.Item("Description"))
                If clsCommon.myLen(row.Item("Logging_Date")) > 0 Then
                    obj.Logging_Date = clsCommon.myCDate(row.Item("Logging_Date"))
                Else
                ...
                obj.Status = clsCommon.myCstr(row.Item("Status"))
                obj.Priority = clsCommon.myCstr(row.Item("Priority"))
                obj.ResolutionRemark = clsCommon.myCstr(row.Item("Resolution_Remark"))
                If clsCommon.myLen(clsCommon.myCstr(row.Item("Resolution_Date"))) > 0 Then
                    obj.Resolution_Date = clsCommon.myCDate(row.Item("Resolution_Date"))
                End If
    ...
    Public Shared Function SaveData(ByVal objList As List(Of clsGrievanceResolution), ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            For Each obj As clsGrievanceResolution In objList
                ...
                clsCommon.AddColumnsForChange(coll, "Resolution_Remark", obj.ResolutionRemark)
                If Not IsNothing(obj.Resolution_Date) Then
                    clsCommon.AddColumnsForChange(coll, "Resolution_Date", clsCommon.GetPrintDate(obj.Resolution_Date, "dd-MMM-yyyy hh:mm:ss"))
                End If
    ...
    Public Shared Function PostData(ByVal strDocNo As String, ByVal strRouteNo As String, ByVal isCheckForPosted As Boolean) As Boolean
        ...
        Try
            ...
            Dim obj As List(Of clsGrievanceResolution) = clsGrievanceResolution.GetData(Nothing)
            For Each objl As clsGrievanceResolution In obj
