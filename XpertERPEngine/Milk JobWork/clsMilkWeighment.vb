...
Public Class clsMilkWeighment
    ...
    Public Shared Function ReverseAndUnpost(ByVal strDocNo As String) As Boolean
        ...
        Try
            ...
            End If
            Dim Qry As String = "select isPosted from tspl_Milk_weighment_detail where Weighment_no='" + strDocNo + "'"
            ...
            End If
            Qry = "Update tspl_Milk_weighment_detail set isPosted = 0,Posting_Date=null where weighment_no='" + strDocNo + "'"
    ...
    Public Shared Function saveData(ByVal obj As clsMilkWeighment, ByVal trans As SqlTransaction, Optional ByVal isHistory As Boolean = False) As Boolean
        Try
            ...
            If obj.isNewEntry Or isHistory Then
                ...
                issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, IIf(isHistory, "tspl_Milk_weighment_detail_history", "tspl_Milk_weighment_detail"), OMInsertOrUpdate.Insert, "", trans)
            Else
                issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "tspl_Milk_weighment_detail", OMInsertOrUpdate.Update, "tspl_Milk_weighment_detail.Weighment_No='" + obj.Weighment_No + "'", trans)
            End If
            If Not isHistory Then
                If clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*) from tspl_Milk_quality_check where isposted=1 and gate_entry_no='" & obj.Gate_Entry_No & "'  and weighment_no=''", trans)) > 0 Then
                    issaved = issaved AndAlso clsDBFuncationality.ExecuteNonQuery("update tspl_Milk_quality_check set weighment_no='" & obj.Weighment_No & "',weighment_date='" & clsCommon.GetPrintDate(obj.Weighment_Date, "dd/MMM/yyyy") & "' where gate_entry_no='" & obj.Gate_Entry_No & "'", trans)
                End If
            End If
            Return issaved
        Catch ex As Exception
    ...
    End Function
    Public Shared Function getData(ByVal strCode As String, ByVal docType As String, ByVal navtype As NavigatorType, Optional ByVal trans As SqlTransaction = Nothing) As clsMilkWeighment
        Return getData(strCode, docType, navtype, False, trans)
    End Function
    Public Shared Function getData(ByVal strCode As String, ByVal docType As String, ByVal navtype As NavigatorType, ByVal isPendingOnly As Boolean, Optional ByVal trans As SqlTransaction = Nothing) As clsMilkWeighment
        Try
            Dim obj As New clsMilkWeighment
            ...
            End If
            Dim qst As String = " select *   From tspl_Milk_weighment_detail   where 1=1 "
            qst = qst & whrCls
            Select Case navtype
                Case NavigatorType.Current
                    qst += " and tspl_Milk_weighment_detail.Weighment_No in ('" + strCode + "') "
                Case NavigatorType.Next
                    qst += " and tspl_Milk_weighment_detail.Weighment_No in (select min(Weighment_No ) from tspl_Milk_weighment_detail where Weighment_No  >'" + strCode + "'" & whrCls & ")"
                Case NavigatorType.First
                    qst += " and tspl_Milk_weighment_detail.Weighment_No in (select MIN(Weighment_No ) from tspl_Milk_weighment_detail where 1=1 " & whrCls & ")"
                Case NavigatorType.Last
                    qst += " and tspl_Milk_weighment_detail.Weighment_No in (select Max(Weighment_No ) from tspl_Milk_weighment_detail where 1=1" & whrCls & ")"
                Case NavigatorType.Previous
                    qst += " and tspl_Milk_weighment_detail.Weighment_No in (select Max(Weighment_No ) from tspl_Milk_weighment_detail where Weighment_No  <'" + strCode + "'" & whrCls & ")"
            End Select
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                If clsDBFuncationality.getSingleValue("select count(*) from tspl_Milk_quality_check where isPosted=1 and gate_entry_no='" & obj.Gate_Entry_No & "'", trans) = 0 Then
                    obj.status = 0
                Else
                    obj.status = 1
                End If
    ...
    Public Shared Function getData(ByVal strCode As String, ByVal navtype As NavigatorType, ByVal isPendingOnly As Boolean, Optional ByVal trans As SqlTransaction = Nothing) As clsMilkWeighment
        Try
            Dim obj As New clsMilkWeighment
            Dim whrCls As String = String.Empty
            Dim whrcls2 As String = String.Empty
    ...
    Public Shared Function isWeighmentDone(ByVal strGateEntryNo As String, ByVal trans As SqlTransaction) As Boolean
        ...
        qry = "select count(*) from tspl_Milk_weighment_detail where gate_entry_no='" & strGateEntryNo & "' and isposted=1"
    ...
    Public Shared Function deleteData(ByVal strWeighmentNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Try
            Dim isUsed As Integer = clsDBFuncationality.getSingleValue("select count(*) from TSPL_JOB_MILK_UNLOADING where weighment_no='" & strWeighmentNo & "'", trans)
            If isUsed > 0 Then
                Throw New Exception("Record is in use.")
            End If
            Dim strQry As String = "delete from tspl_Milk_weighment_detail where Weighment_No='" & strWeighmentNo & "'"
    ...
    Public Shared Function getFinder(ByVal whrcls As String, ByVal curcode As String, ByVal isButtonClicked As Boolean) As String
        Dim str As String = ""
        Try
            Dim qry As String = " select tspl_Milk_weighment_detail.Weighment_No as [Code],tspl_Milk_weighment_detail.Weighment_Slip_no as [Weighment Slip No] ,tspl_Milk_weighment_detail.Weighment_date as [Weighment Date], tspl_Milk_weighment_detail.Gate_Entry_No as [Gate Entry No] ,tspl_Milk_weighment_detail.Doc_Type as [Doc Type] ,tspl_Milk_weighment_detail.Date_And_Time as [Gate Entry Date And Time] ,tspl_Milk_weighment_detail.Challan_No as [Challan No] ,tspl_Milk_weighment_detail.Challan_Date as [Challan Date] ,tspl_Milk_weighment_detail.Tanker_No as [Tanker No] ,case when isnull(tspl_Milk_weighment_detail.isPosted,0)=0 then 'No' else 'Yes' end  as [Is Posted] ,tspl_Milk_weighment_detail.Posting_Date as [Posting Date] ,tspl_Milk_weighment_detail.Dispatched_From_Mcc as [Dispatched From Mcc] ,tspl_Milk_weighment_detail.location_Code as [Location Code] ,tspl_Milk_weighment_detail.Location_Desc as [Location Desc] ,tspl_Milk_weighment_detail.Vendor_Code as [Vendor Code] ,tspl_Milk_weighment_detail.Vendor_Desc as [Vendor Desc] ,tspl_Milk_weighment_detail.Item_Code as [Item Code] ,tspl_Milk_weighment_detail.Item_Desc as [Item Desc] ,tspl_Milk_weighment_detail.Qty_In_Kg as [Qty In Kg] ,tspl_Milk_weighment_detail.snf_Per as [SNF Per] ,tspl_Milk_weighment_detail.fat_per as [FAT Per] ,tspl_Milk_weighment_detail.Created_By as [Created By] ,tspl_Milk_weighment_detail.Created_Date as [Created Date] ,tspl_Milk_weighment_detail.Modify_By as [Modify By] ,tspl_Milk_weighment_detail.Modify_Date as [Modify Date] ,tspl_Milk_weighment_detail.comp_code as [Company Code] ,tspl_Milk_weighment_detail.Gross_Weight as [Gross Weight] ,tspl_Milk_weighment_detail.Dip_Value as [Dip Value] ,tspl_Milk_weighment_detail.Tare_Weight as [Tare Weight] ,tspl_Milk_weighment_detail.Net_Weight as [Net Weight] ,case when isnull (tspl_Milk_weighment_detail.status,0)=0 then 'Not Sent For QC' when tspl_Milk_weighment_detail.status=1 then 'Sent For QC' else 'QC Done' end as [Status] ,tspl_Milk_weighment_detail.Sent_to_QC_By as [Sent To Qc By] ,tspl_Milk_weighment_detail.Sent_To_QC_Date as [Sent To Qc Date] ,tspl_Milk_weighment_detail.QC_Done_By as [Qc Done By] ,tspl_Milk_weighment_detail.QC_Done_Date as [Qc Done Date]   From tspl_Milk_weighment_detail"
    ...
    Public Shared Function postData(ByVal strWeighmentNo As String, ByVal docType As String, ByVal formId As String, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            Dim obj As clsMilkWeighment = clsMilkWeighment.getData(strWeighmentNo, docType, NavigatorType.Current, trans)
            ...
            '--------------------
            'Dim isResult As Boolean = clsApprovalScreen.CheckApprovalLevel(formId, "tspl_Milk_weighment_detail", "Weighment_No", obj.Weighment_No, trans)
            'If isResult = False Then
            '    trans.Commit()
            '    Return False
            'End If
            Dim strQry As String = " update tspl_Milk_weighment_detail set isPosted='1', Posting_Date='" & clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy") & "' where Weighment_No='" & strWeighmentNo & "'"
