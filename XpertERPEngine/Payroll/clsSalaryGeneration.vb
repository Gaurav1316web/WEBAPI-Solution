...
Group By EMP.EMP_CODE)xxx Left Outer Join TSPL_COMPANY_MASTER On TSPL_COMPANY_MASTER.Comp_Code1='" + objCommonVar.CurrComp_Code1 + "'"
    ...
    Public Shared Function Generate_SalaryOLD(ByVal Pay_Period_Code As String, ByVal EmpList As ArrayList, ByVal trans As SqlTransaction, Optional ByVal Is_Arrear As Boolean = False) As Boolean
        ...
        strq = "UPDATE " & Calc_Table & " SET RATE_AMOUNT=RATE_AMOUNT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS,ACTUAL_AMOUNT=ACTUAL_AMOUNT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS,ACTUL_AMT=ACTUL_AMT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS, " _
        ...
        strq = "UPDATE " & Calc_Table & "" _
              & " SET Actual_Amount =0,ACTUL_AMT =0  " _
        ...
        strq = "UPDATE  " & Calc_Table & " SET ACTUAL_AMOUNT=0,ACTUL_AMT =0 WHERE SUB_HEAD_TYPE='LOAN' and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        ...
        strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=(T1.EMI+COALESCE(T2.ADJUSTMENT_PLUS,0)-COALESCE(T2.ADJUSTMENT_MINUS,0)),ACTUL_AMT=(T1.EMI+COALESCE(T2.ADJUSTMENT_PLUS,0)-COALESCE(T2.ADJUSTMENT_MINUS,0)) from ( " _
        ...
        strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT =0,ACTUL_AMT =0 WHERE SUB_HEAD_TYPE='BONUS' and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        ...
        strq = "UPDATE  " & Calc_Table & " SET ACTUAL_AMOUNT=T1.BONUS_AMOUNT,ACTUL_AMT=T1.BONUS_AMOUNT,BONUS_CODE=T1.BONUS_CODE," _
        ...
        strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=T1.REIMBURSEMENT_AMOUNT,ACTUL_AMT=T1.REIMBURSEMENT_AMOUNT FROM " _
...
") * RATE_AMOUNT / 100 " _
  ...
  & " AND " & Calc_Table & ".SUB_HEAD_TYPE = 'COEPS' and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
       ...
       & " WHERE SUB_HEAD_TYPE='EMPESI' AND ACTUAL_AMOUNT>0 and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        ...
        strq = " UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=mediclim.Total_Amount,ACTUL_AMT=mediclim.Total_Amount " &
        ...
        strq = " UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=GRATUITY.Total_Amount,ACTUL_AMT=GRATUITY.Total_Amount " &
        ...
        strq = " UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=LTA.Total_Amount,ACTUL_AMT=LTA.Total_Amount " &
        ...
        strq = " UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=(CASE WHEN MAX_AMOUNT>0 AND  Conv.Total_Amount>MAX_AMOUNT THEN  MAX_AMOUNT ELSE Conv.Total_Amount END),ACTUL_AMT=(CASE WHEN MAX_AMOUNT>0 AND  Conv.Total_Amount>MAX_AMOUNT THEN  MAX_AMOUNT ELSE Conv.Total_Amount END)  " &
       ...
       & " WHERE SUB_HEAD_TYPE='EPF' AND ACTUAL_AMOUNT>0 and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        ...
        ''adjustment IN PRINCIPAL AMOUNT details
        clsCommon.ProgressBarUpdate("updating adjustment in principal amount of pay heads...")
        strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=ACTUAL_AMOUNT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS,ACTUL_AMT=ACTUL_AMT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS, " _
             & " ADJUSTMENT_PLUS=T1.ADJUSTMENT_PLUS,ADJUSTMENT_MINUS=T1.ADJUSTMENT_MINUS FROM ( " _
             & " SELECT T2.EMP_CODE,T2.PAY_HEAD_CODE,SUM(T2.ADJUSTMENT_PLUS) AS ADJUSTMENT_PLUS,SUM(T2.ADJUSTMENT_MINUS) AS ADJUSTMENT_MINUS " _
             & " FROM TSPL_ADJUSTMENT_VOUCHER T1  INNER JOIN TSPL_EMPADJUSTMENT_DETAIL T2 ON T1.ADJUSTMENT_CODE=T2.ADJUSTMENT_CODE inner join TSPL_PAYHEAD_MASTER TPH ON TPH.PAY_HEAD_CODE=T2.PAY_HEAD_CODE " _
        ...
        strq = "UPDATE TSPL_SALARY_CALCULATION SET ACTUAL_AMOUNT=round(ACTUAL_AMOUNT,2,2) ,ACTUL_AMT=round(ACTUL_AMT,2,2) "
        ...
        & " WHEN ROUND_OFF_TYPE='L' THEN  FLOOR(ACTUAL_AMOUNT) WHEN ROUND_OFF_TYPE='U' THEN CEILING(ACTUAL_AMOUNT) END),ACTUL_AMT=(CASE WHEN ROUND_OFF_TYPE='R' THEN ROUND(ACTUL_AMT,0) WHEN ROUND_OFF_TYPE='L' THEN  FLOOR(ACTUL_AMT) WHEN ROUND_OFF_TYPE='U' THEN CEILING(ACTUL_AMT) END)," _
        & " PRINCIPAL_ROUND_OFF=(COALESCE(PRINCIPAL_ROUND_OFF,0)+(CASE WHEN ROUND_OFF_TYPE='R' THEN ROUND(ACTUAL_AMOUNT,0) " _
        & " WHEN ROUND_OFF_TYPE='L' THEN  FLOOR(ACTUAL_AMOUNT) WHEN ROUND_OFF_TYPE='U' THEN CEILING(ACTUAL_AMOUNT) END)-ACTUAL_AMOUNT) where  " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        ...
        If clsCommon.myLen(strTDS) > 0 Then
            ...
            If clsCommon.myLen(strFiscalCode) > 0 Then
                strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=xx.Total_TDS_Amt ,ACTUL_AMT=xx.Total_TDS_Amtfrom (" + Environment.NewLine +
        ...
        STRQ1 = "update " & Calc_Table & " set ACTUAL_AMOUNT=0,ACTUL_AMT=0 where Payable_Days=0 and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        If clsDBFuncationality.ExecuteNonQuery(STRQ1, trans) = True Then
        ...
        strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT=0,ACTUL_AMT=0 where SUB_HEAD_TYPE='OT' and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        If clsDBFuncationality.ExecuteNonQuery(strq, trans) = True Then
        Else
        ...
        strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT = 0,ACTUL_AMT=0,OT_HOURS=T11.OT_HOURS,OT_RATE=(CASE WHEN IS_ASPER_ACTUAL_CALC=0 THEN T11.OT_RATE ELSE COALESCE(HEAD_VALUE,0)/PAYPERIOD_DAYS END) " _
        & " FROM ( " _
        ...
        Else
            strq = "UPDATE " & Calc_Table & " SET ACTUAL_AMOUNT = 0,ACTUL_AMT=0,OT_HOURS=(" & Calc_Table & ".OT_HOURS-EMP_LEAVE.EXCESS_LEAVE * " & WorkingHours & ")  " _
        ...
        For Each dr As DataRow In dtOT.Rows
            strq = " update  " & Calc_Table & " SET ACTUAL_AMOUNT=" & CalculateOT(dr.Item("OT_CODE"), dr.Item("OT_HOURS"), clsCommon.myCdbl(dr.Item("HEAD_VALUE")) / (dr.Item("PAYPERIOD_DAYS") * WorkingHours), clsCommon.myCdbl(dr.Item("HEAD_VALUE")), trans, clsCommon.myCdbl(dr.Item("GrossSalary")) / (dr.Item("PAYPERIOD_DAYS") * WorkingHours)) & "
                      ACTUL_AMT=" & CalculateOT(dr.Item("OT_CODE"), dr.Item("OT_HOURS"), clsCommon.myCdbl(dr.Item("HEAD_VALUE")) / (dr.Item("PAYPERIOD_DAYS") * WorkingHours), clsCommon.myCdbl(dr.Item("HEAD_VALUE")), trans, clsCommon.myCdbl(dr.Item("GrossSalary")) / (dr.Item("PAYPERIOD_DAYS") * WorkingHours)) & "
                        WHERE SALARY_CALCULATION_CODE=" & dr.Item("SALARY_CALCULATION_CODE") & ""
            If clsDBFuncationality.ExecuteNonQuery(strq, trans) = True Then
    ...
    Public Shared Function CalculateArrear(ByVal Pay_Period_Code As String, ByVal trans As SqlTransaction) As Boolean
        ...
        qry = "UPDATE TSPL_SALARY_CALCULATION SET ACTUAL_AMOUNT=ACTUAL_AMOUNT+COALESCE(ARREAR_AMT,0), ACTUL_AMT=ACTUL_AMT+COALESCE(ARREAR_AMT,0)  "
...
INSERT INTO " & strTableName & " ( EMP_CODE, EMP_SAL_CODE,SALARY_STRUCTURE_CODE,ISHIDDENCOMPONENT,PAY_HEAD_CODE, LINE_NO,  HEAD_TYPE,SUB_HEAD_TYPE,CALC_BASIS,PAYHEAD_FORMULA,RATE_AMOUNT,STD_AMOUNT,PAYPERIOD_DAYS,  PRESENT_DAYS,ABSENT_DAYS,LEAVE_DAYS,HOLIDAY_DAYS,PAYABLE_DAYS,LOP_DAYS,IS_PF_APPL,IS_PF_ATTN_ENABLE,  PF_MAX_LIM,EPS_TO_EPF,EPS_MAX,COEPF_ROUNDOFF_YPE,EMPEPF_ROUNDOFF_YPE,IS_ESI_APPL,FORMULA_HEAD, IS_OT_APPL,OT_CODE,OT_HOURS,OT_RATE,HOUR_MULTIPLIER,IS_ASPER_ACTUAL_CALC,IS_BONUS_APPL,BONUS_CODE,ACTUAL_AMOUNT,ACTUL_AMT,PAYABLE_AMOUNT,ROUND_OFF_TYPE,MAX_AMOUNT,EMP_STATUS_CODE,ESI_MAX_LIM,EPF_RATE,ESI_RATE,PAY_PERIOD_CODE, PF_Calculation_Type,PF_Rule_Max_Lim,Custom_PF_Max_Lim,PF_No,ESI_Calculation_Type,ESI_Rule_Max_Lim,Custom_ESI_Max_Lim,ESI_No,OD_Applicable,Is_Earning_Payhead,Is_Professional_Tax_Applicable,ISESI  )
select * from CTE "
        ...
        ''adjustment IN PRINCIPAL AMOUNT details
        clsCommon.ProgressBarUpdate("updating adjustment in principal amount of pay heads...")
        strq = "UPDATE " & strTableName & " SET RATE_AMOUNT=RATE_AMOUNT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS,ACTUAL_AMOUNT=ACTUAL_AMOUNT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS,ACTUL_AMT=ACTUL_AMT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS, " _
             & " ADJUSTMENT_PLUS=T1.ADJUSTMENT_PLUS,ADJUSTMENT_MINUS=T1.ADJUSTMENT_MINUS FROM ( " _
             & " SELECT T2.EMP_CODE,T2.PAY_HEAD_CODE,SUM(T2.ADJUSTMENT_PLUS) AS ADJUSTMENT_PLUS,SUM(T2.ADJUSTMENT_MINUS) AS ADJUSTMENT_MINUS " _
             & " FROM TSPL_ADJUSTMENT_VOUCHER T1  INNER JOIN TSPL_EMPADJUSTMENT_DETAIL T2 ON T1.ADJUSTMENT_CODE=T2.ADJUSTMENT_CODE inner join TSPL_PAYHEAD_MASTER TPH ON TPH.PAY_HEAD_CODE=T2.PAY_HEAD_CODE " _
             & " WHERE T1.PAY_PERIOD_CODE='" & Pay_Period_Code & "' and ADJUSTMENT_TYPE='PA' and TPH.HEAD_TYPE NOT IN ('F') GROUP BY T2.EMP_CODE,T2.PAY_HEAD_CODE) AS T1  " _
        ...
        clsCommon.ProgressBarUpdate("updating loan/advance pay heads...")
        strq = "UPDATE  " & strTableName & " SET ACTUAL_AMOUNT=0,ACTUL_AMT =0 WHERE SUB_HEAD_TYPE='LOAN' and " & strTableName & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"

        If Not clsDBFuncationality.ExecuteNonQuery(strq, trans) Then
            Throw New Exception("Error in Updating Loan !")
        End If

        strq = "UPDATE " & strTableName & " SET ACTUAL_AMOUNT=(T1.EMI+COALESCE(T2.ADJUSTMENT_PLUS,0)-COALESCE(T2.ADJUSTMENT_MINUS,0)),ACTUL_AMT=(T1.EMI+COALESCE(T2.ADJUSTMENT_PLUS,0)-COALESCE(T2.ADJUSTMENT_MINUS,0)) from ( " _
             & " select T2.EMP_CODE,T2.EMI_AMOUNT AS EMI FROM TSPL_LOAN_GENERATION T1  " _
             & " INNER JOIN TSPL_LOANGENERATION_DETAIL T2 ON T1.LOAN_GENERATION_CODE=T2.LOAN_GENERATION_CODE " _
             & " WHERE  T2.PAY_PERIOD_CODE='" & Pay_Period_Code & "' ) AS T1 " _
             & " LEFT JOIN (SELECT EMP_CODE,SUM(ADJUSTMENT_PLUS) AS ADJUSTMENT_PLUS," _
        ...
        '' bonus calculation/generation
        clsCommon.ProgressBarUpdate("updating Bonus pay heads...")
        strq = "UPDATE " & strTableName & " SET ACTUAL_AMOUNT =0,ACTUL_AMT =0 WHERE SUB_HEAD_TYPE='BONUS' and " & strTableName & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
        If Not clsDBFuncationality.ExecuteNonQuery(strq, trans) Then
            Throw New Exception("Error in Updating Bonus !")
        End If

        strq = "UPDATE  " & strTableName & " SET ACTUAL_AMOUNT=T1.BONUS_AMOUNT,ACTUL_AMT=T1.BONUS_AMOUNT,BONUS_CODE=T1.BONUS_CODE," _
             & " BONUS_FROM_PAY_PERIOD_CODE=T1.FROM_PAY_PERIOD_CODE,BONUS_TO_PAY_PERIOD_CODE=T1.TO_PAY_PERIOD_CODE  " _
             & " FROM (SELECT T1.FROM_PAY_PERIOD_CODE,T1.TO_PAY_PERIOD_CODE,T2.EMP_CODE,T2.BONUS_AMOUNT," _
             & " T2.BONUS_CODE FROM TSPL_EMPLOYEE_BONUS T1  " _
             & " INNER JOIN TSPL_EMPBONUS_DETAIL T2 ON T1.EMP_BONUS_CODE=T2.EMP_BONUS_CODE" _
        ...
        '' REIMBURSEMENT AMOUNT CALCULATION 
        clsCommon.ProgressBarUpdate("updating reimbursement pay heads...")
        strq = "UPDATE " & strTableName & " SET ACTUAL_AMOUNT=T1.REIMBURSEMENT_AMOUNT,ACTUL_AMT=T1.REIMBURSEMENT_AMOUNT FROM " _
        & " (select T1.EMP_CODE,T2.PAY_HEAD_CODE,SUM(COALESCE(T2.REIMBURSEMENT_AMOUNT,0)) AS REIMBURSEMENT_AMOUNT  " _
        & " from TSPL_EMP_REIMBURSEMENT T1 " _
        & " INNER JOIN  TSPL_EMPREIMBURSEMENT_DETAIL T2 " _
        & " ON T1.REIMBURSEMENT_CODE=T2.REIMBURSEMENT_CODE " _
...
") * RATE_AMOUNT / 100 " _
       ...
       & "     ) " _
       & "     End " _
       & "     ),ACTUL_AMT =(  CASE WHEN SUB_HEAD_TYPE IN ('COPF', 'COEPS') THEN  (  (  CASE  WHEN IS_PF_APPL = 1 THEN  (  CASE  WHEN SUB_HEAD_TYPE = 'COPF' THEN ( (CASE WHEN PF_MAX_LIM >0 THEN (CASE WHEN IS_PF_ATTN_ENABLE = 0 THEN (CASE WHEN (case when PAYABLE_DAYS=0 then 0 else   HEAD_VALUE end  ) > PF_MAX_LIM THEN 
                ((Isnull(HEAD_VALUE,0)+IsNull(ACTUL_AMT,0))  )
                ELSE (case when PAYABLE_DAYS=0 then 0 else HEAD_VALUE end) End )ELSE (CASE WHEN HEAD_VALUE > PF_MAX_LIM THEN ((PF_MAX_LIM  )) ELSE ( (HEAD_VALUE))End)End)ELSE(CASE WHEN IS_PF_ATTN_ENABLE = 0 THEN FORMULA_VALUE ELSE ((CASE WHEN HEAD_VALUE > PF_MAX_LIM THEN ((HEAD_VALUE))ELSE((HEAD_VALUE))End))End)End)) * RATE_AMOUNT / 100  WHEN SUB_HEAD_TYPE IN ('COPF', 'COEPS') THEN  (  CASE  WHEN EPS_TO_EPF = 1 THEN  	(   	CASE  	WHEN SUB_HEAD_TYPE = 'COPF' THEN  	(  	(  		CASE  		WHEN PF_MAX_LIM>0 THEN  		(  		CASE  	    WHEN FORMULA_VALUE > PF_MAX_LIM THEN       PF_MAX_LIM  		ELSE       FORMULA_VALUE       End  		)  	ELSE  (FORMULA_VALUE)   End  			)  			) * RATE_AMOUNT / 100  	ELSE       0       End  							)  		ELSE  		(  	    CASE  		WHEN SUB_HEAD_TYPE = 'COPF' THEN  	    (  		CASE  	    WHEN (  		(  		(  		(  		CASE  	    WHEN PF_MAX_LIM>0  THEN  		(  		CASE  		WHEN FORMULA_VALUE > PF_MAX_LIM THEN       PF_MAX_LIM  		ELSE       FORMULA_VALUE       End  		)  		ELSE  		(FORMULA_VALUE)       End  		)  		)  		) * TSPL_EMPLOYEE_MASTER.EMPEPF_PER / 100  		) > (PF_MAX_LIM * TSPL_EMPLOYEE_MASTER.EMPEPF_PER / 100) THEN  	    ((PF_MAX_LIM * TSPL_EMPLOYEE_MASTER.EMPEPF_PER) / 100)  		ELSE  		(  	    (  	    (  		(  		CASE  	    WHEN PF_MAX_LIM>0 THEN  		(  		CASE  		WHEN FORMULA_VALUE > PF_MAX_LIM THEN       PF_MAX_LIM  		ELSE       FORMULA_VALUE       End  	 	)  		ELSE  	    (FORMULA_VALUE)       End  	    )  		)  	    ) * TSPL_EMPLOYEE_MASTER.EMPEPF_PER / 100  	    )       End  		)  		ELSE       0       End  		)       End  	    )  			ELSE  			(  			(  	 		CASE  			WHEN PF_MAX_LIM>0 THEN  			(  			CASE  		    WHEN FORMULA_VALUE > PF_MAX_LIM THEN           PF_MAX_LIM  			ELSE          FORMULA_VALUE          End  		   )  		  ELSE  		(FORMULA_VALUE)       End  	    )    	) * RATE_AMOUNT / 100       End  		)  	   ELSE      0      End  	   )  	   )      ELSE  	   (  	   COALESCE (  	   FORMULA_VALUE,      '0'  	   )      )      End      )  " _
  ...
  & " ACTUAL_AMOUNT = ROUND(T1.EPS, 2),ACTUL_AMT = ROUND(T1.EPS, 2)  " _
  & " FROM " _
  & " ( " _
  & " SELECT " _
  & " T1.EMP_CODE, " _
  ...
  & " AND " & strTableName & ".SUB_HEAD_TYPE = 'COEPS' and " & strTableName & ".PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' and Is_Earning_Payhead=" + isEarning + ""
        ...
        strq = "UPDATE " & strTableName & "" _
              ...
              & " ACTUAL_AMOUNT " _
              & " End " _
              & " ),ACTUL_AMT =(  CASE  WHEN IS_ESI_APPL = 1  AND HEAD_VALUE > ESI_MAX_LIM and 0=0 AND 5 IN (4,10) THEN  0  WHEN IS_ESI_APPL = 1  AND HEAD_VALUE > ESI_MAX_LIM AND COALESCE(PREV_ESI,0)<=0 and 0=0 THEN  0  WHEN IS_ESI_APPL = 0 THEN  0  ELSE  ACTUL_AMT  End  ) " _
              & " , Payable_Amount = ( " _
              & " CASE " _
              & " WHEN IS_ESI_APPL = 1 " _
              & " AND HEAD_VALUE > ESI_MAX_LIM and " & Arrear & "=0 AND " & PP_END_DATE.Month & " IN (" & ESIFrom & "," & ESITo & ") THEN " _
       ...
       & " WHERE SUB_HEAD_TYPE='EMPESI' AND ACTUAL_AMOUNT>0 and " & strTableName & ".PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' and Is_Earning_Payhead=" + isEarning + ""
        ...
        '' Mediclaim CALCULATION
        clsCommon.ProgressBarUpdate("updating Mediclaim...")
        strq = " UPDATE " & strTableName & " SET ACTUAL_AMOUNT=mediclim.Total_Amount,ACTUL_AMT=mediclim.Total_Amount " &
               " from (select EMP_CODE,SUM(Total_Amount) AS Total_Amount from TSPL_MEDICLAIM_HEAD " &
               " where PAY_PERIOD_CODE is null or PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' GROUP BY EMP_CODE) as mediclim " &
        ...
        '' Gratuity CALCULATION
        clsCommon.ProgressBarUpdate("updating Gratuity...")
        strq = " UPDATE " & strTableName & " SET ACTUAL_AMOUNT=GRATUITY.Total_Amount,ACTUL_AMT=GRATUITY.Total_Amount " &
               " from (select EMP_CODE,SUM(GRATUITYAMT) AS Total_Amount from TSPL_GRATUITY  " &
               " where PAY_PERIOD_CODE is null or PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' GROUP BY EMP_CODE) as GRATUITY " &
        ...
        '' LTA CALCULATION
        clsCommon.ProgressBarUpdate("updating LTA...")
        strq = " UPDATE " & strTableName & " SET ACTUAL_AMOUNT=LTA.Total_Amount,ACTUL_AMT=LTA.Total_Amount " &
               " from (select EMP_CODE,SUM(Claim_Amount) AS Total_Amount from TSPL_LTA_Claim_Head   " &
               " where PAY_PERIOD_CODE is null or PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' GROUP BY EMP_CODE) as LTA " &
        ...
        '' Conveyance
        clsCommon.ProgressBarUpdate("updating Conveyance...")
        strq = " UPDATE " & strTableName & " SET ACTUAL_AMOUNT=(CASE WHEN MAX_AMOUNT>0 AND  Conv.Total_Amount>MAX_AMOUNT THEN  MAX_AMOUNT ELSE Conv.Total_Amount END),ACTUL_AMT=(CASE WHEN MAX_AMOUNT>0 AND  Conv.Total_Amount>MAX_AMOUNT THEN  MAX_AMOUNT ELSE Conv.Total_Amount END)  " &
               " from (select EMP_CODE,SUM(Claim_Amount) AS Total_Amount from TSPL_CONVEYANCE_CLAIM   " &
               " where PAY_PERIOD_CODE is null or PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' GROUP BY EMP_CODE) as Conv " &
       ...
       & " WHERE SUB_HEAD_TYPE='EPF' AND ACTUAL_AMOUNT>0 and " & strTableName & ".PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' and Is_Earning_Payhead=" + isEarning + ""
        ...
        ''adjustment IN PRINCIPAL AMOUNT details
        clsCommon.ProgressBarUpdate("updating adjustment in principal amount of pay heads...")
        strq = "UPDATE " & strTableName & " SET ACTUAL_AMOUNT=ACTUAL_AMOUNT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS,ACTUL_AMT=ACTUL_AMT+T1.ADJUSTMENT_PLUS-T1.ADJUSTMENT_MINUS, " _
             & " ADJUSTMENT_PLUS=T1.ADJUSTMENT_PLUS,ADJUSTMENT_MINUS=T1.ADJUSTMENT_MINUS FROM ( " _
             & " SELECT T2.EMP_CODE,T2.PAY_HEAD_CODE,SUM(T2.ADJUSTMENT_PLUS) AS ADJUSTMENT_PLUS,SUM(T2.ADJUSTMENT_MINUS) AS ADJUSTMENT_MINUS " _
             & " FROM TSPL_ADJUSTMENT_VOUCHER T1  INNER JOIN TSPL_EMPADJUSTMENT_DETAIL T2 ON T1.ADJUSTMENT_CODE=T2.ADJUSTMENT_CODE inner join TSPL_PAYHEAD_MASTER TPH ON TPH.PAY_HEAD_CODE=T2.PAY_HEAD_CODE " _
             & " WHERE T1.PAY_PERIOD_CODE='" & PAY_PERIOD_CODE & "' and ADJUSTMENT_TYPE='PA' and TPH.HEAD_TYPE IN ('F') GROUP BY T2.EMP_CODE,T2.PAY_HEAD_CODE) AS T1  " _
