  ...
  & " AND " & Calc_Table & ".SUB_HEAD_TYPE = 'COEPS' and " & Calc_Table & ".PAY_PERIOD_CODE='" & Pay_Period_Code & "'"
    ...
    Public Shared Function Generate_Salary(ByVal Pay_Period_Code As String, ByVal EmpList As ArrayList, ByVal trans As SqlTransaction, Optional ByVal Is_Arrear As Boolean = False) As Boolean
        ...
        strq = "INSERT INTO " & strTableName & " ( EMP_CODE, EMP_SAL_CODE,SALARY_STRUCTURE_CODE,ISHIDDENCOMPONENT,PAY_HEAD_CODE, LINE_NO, " _
           ...
           & " (CASE WHEN T4.HEAD_TYPE IN ('ATTN', 'FIXED','F') and (t4.SUB_HEAD_TYPE not in('COPF')) THEN T2.RATE_AMOUNT WHEN (t4.SUB_HEAD_TYPE in('COPF') and t4.ISEARNING=0) THEN T7.EMPEPF_PER WHEN T4.HEAD_TYPE = 'UD' THEN COALESCE(T8.ALLOWANCE_AMOUNT,0) when ((t4.SUB_HEAD_TYPE)='DAARREAR' and t4.ISEARNING=1) then isnull(daarrear.da_arrear,0) ELSE 0 End) AS RATE , " _
...
where TSPL_DA_ARREAR_HEADER.Pay_Period='APR' )DAArrear on DAArrear.Emp_Code=T1.EMP_CODE  " _
