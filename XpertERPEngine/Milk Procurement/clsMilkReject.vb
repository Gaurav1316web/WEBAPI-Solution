...
#End Region
    ...
    Public Shared Function GetPost(ByVal DocDate As Date, ByVal MCC_Code As String, ByVal Shift As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            Return True
        End If
        qry = "SELECT 1 FROM TSPL_MILK_SRN_HEAD WHERE MCC_CODE='" & MCC_Code & "' AND  convert(date, DOC_DATE,103)='" & clsCommon.GetPrintDate(DocDate, "dd/MMM/yyyy") & "' AND SHIFT='" & Shift & "'"
        dt = clsDBFuncationality.GetDataTable(qry, trans)
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            Return True
        End If
    ...
    Public Shared Function GetMCCRegisterQuery(ByVal FromDate As DateTime, ByVal ToDate As DateTime, ByVal FromShift As String, ByVal ToShift As String, ByVal SRNAmounType As String, ByVal StrPermission As String, ByVal arrPlant As ArrayList, ByVal arrMCC As ArrayList, ByVal arrRoute As ArrayList, ByVal arrVLC As ArrayList, ByVal ReceivingUOM As String, ByVal MilkType As String, ByVal arrVSP As ArrayList, ByVal IsMilkBillReport As Boolean, ByVal Area As String) As String
        ...
        Dim PartialSRNAmount As Decimal = 0
        PartialSRNAmount = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.PartialSRNAmount, clsFixedParameterCode.PartialSRNAmount, Nothing))

        Dim qry As String = "Select  final.Registered_PDCS_CLUSTER AS [Dcs Type],final.[Milk Receipt Code] ,final.MCC as [MCC Code] ,final.[MCC Name],final.Short_Description_MCC,final.[MCC Type] ,final.[Chilling Center],final.[Plant Code],final.[Plant Name] ,final.Date ,final.[Doc Date] ,final.Shift ,"
        If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "CHU") = CompairStringResult.Equal Then
            qry += " Convert(int,final.[Route Code])[Route Code],"
        Else
            qry += " final.[Route Code],"
        End If
        qry += " final.[Route Name],final.[Zone Code],final.Short_Description_Route ,final.[Vehicle Code] ,final.[VSP Code],final.[VSP Name],final.[Vendor Group Code] ,final.[Vendor Group Desc],final.[Vlc Uploader Code] ,final.[Vlc Code] ,final.[VLC Name],final.[Alias Name],final.[isOwnBMC(Y/N)],final.Short_Description_VLC ," &
        ...
        If PartialSRNAmount > 0 Then
            qry += " cast(CASE WHEN final.[SRN Amount] IS NOT NULL THEN final.[SRN Amount] * " & PartialSRNAmount & " / 100  ELSE 0 END as decimal(18,2)) AS [Partial Amount], "
        End If
        qry += " final.[SRN Qty],final.[SRN Rate],final.[Shift Status] ,Invoice_no ,Invoice_Date ,IS_MANUAL, MACHINE_NO,(CASE WHEN [Sample Status]=0 THEN 'N' ELSE 'Y' END) AS IS_MILK_SAMPLE_MANUAL,[Transporter Code],[Transporter Name],EMP_Amount,TIP_Amount,NET_AMOUNT,Round_Off,Handling_Charges_Amount,final.Planning_Code,final.Planning_Posted_Date,final.Planning_Posted_Time ,final.Declared_Rate,final.[Price Code],final.Purchase_Order_No,final.Head_Load_Amount,final.SNF_Ded_Value,final.SNF_Ded_Rate,final.SNF_Ded_Amount,final.VSP_Commission_Amount,final.VSP_Deduction_Amount,final.VSP_Day_Wise_Incentive,final.IncetiveAmt,final.SubStandard,final.Vehicle,Mcc_Code_VLC_Uploader as [Mcc_Uploader_Code], final.[SubStandardQty],final.[Doc Date Time],final.QAT_Rate,final.QAT_Amt " &
                " From (Select
CASE 
    WHEN TSPL_VLC_MASTER_HEAD.REGISTERED_PDCS_CLUSTER = 'PDCS' THEN 'PDCS'
    WHEN TSPL_VLC_MASTER_HEAD.REGISTERED_PDCS_CLUSTER = 'Registered' THEN 'DCS'
END AS REGISTERED_PDCS_CLUSTER
,
case when isnull(TSPL_MILK_SRN_HEAD.Capping_Apply,0)=1 then TSPL_MILK_SRN_DETAIL.Capping_FAT else null end as Capping_FAT,case when isnull(TSPL_MILK_SRN_HEAD.Capping_Apply,0)=1 then TSPL_MILK_SRN_DETAIL.Capping_SNF else null end as Capping_SNF,TSPL_MCC_MASTER.MCC_Type as [MCC Type],TSPL_MCC_MASTER.Short_Description as Short_Description_MCC,TSPL_MCC_ROUTE_MASTER.Short_Description as Short_Description_Route,TSPL_VENDOR_MASTER.Zone_Code as [Zone Code],TSPL_VLC_MASTER_HEAD.Short_Description as Short_Description_VLC,case when TSPL_MCC_MASTER.is_Mcc=1 then 'MCC' else 'BMCC' end [Chilling Center] ,TSPL_MILK_SRN_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc "
        If objCommonVar.PricePlan = 6 OrElse objCommonVar.PricePlan = 7 Then
            qry += " , Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'C' Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Cow FAT(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'C' Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Cow SNF(%)]," &
    " Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'B' Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Buffalo FAT(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'B' Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Buffalo SNF(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'C' Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Cow Milk Qty (KG)],TSPL_MCC_MASTER.Mcc_Code_VLC_Uploader,Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type= 'C' Then TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR Else 0 End [Cow Milk Qty (Ltr)], " &
    " Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'B' Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Buffalo Milk Qty (KG)],Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'B' Then TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR Else 0 End [Buffalo Milk Qty (Ltr)] "
            qry += ", TPCP.Dock_Collection_Milk_Type As [Milk Type]"
        ElseIf objCommonVar.DisplayTypeInMilkReceipt Then
            qry += " , Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'C' Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Cow FAT(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'C' Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Cow SNF(%)]," &
                " Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'B' Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Buffalo FAT(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'B' Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Buffalo SNF(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'C' Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Cow Milk Qty (KG)],TSPL_MCC_MASTER.Mcc_Code_VLC_Uploader,Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'C' Then TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR Else 0 End [Cow Milk Qty (Ltr)], " &
                " Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'B' Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Buffalo Milk Qty (KG)],Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type = 'B' Then TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR Else 0 End [Buffalo Milk Qty (Ltr)] "
            qry += ", TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type  As [Milk Type]"
        Else
            qry += " ,Case When TSPL_MILK_SRN_DETAIL.FAT_PER <= 5 Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Cow FAT(%)], Case When TSPL_MILK_SRN_DETAIL.FAT_PER <= 5 Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Cow SNF(%)]," &
                " Case When TSPL_MILK_SRN_DETAIL.FAT_PER > 5 Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Buffalo FAT(%)], Case When TSPL_MILK_SRN_DETAIL.FAT_PER > 5 Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Buffalo SNF(%)], Case When TSPL_MILK_SRN_DETAIL.FAT_PER <= 5 Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Cow Milk Qty (KG)],TSPL_MCC_MASTER.Mcc_Code_VLC_Uploader, Case When TSPL_MILK_SRN_DETAIL.FAT_PER <= 5 Then TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR Else 0 End [Cow Milk Qty (Ltr)]," &
                " Case When TSPL_MILK_SRN_DETAIL.FAT_PER > 5 Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Buffalo Milk Qty (KG)] ,Case When TSPL_MILK_SRN_DETAIL.FAT_PER > 5 Then TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR Else 0 End [Buffalo Milk Qty (Ltr)] "
            qry += " , Case When Coalesce(TSPL_MILK_SRN_DETAIL.FAT_PER, 0) <= 0 Then '' When Coalesce(TSPL_MILK_SRN_DETAIL.FAT_PER, 0) <= " + clsCommon.myCstr(SetCowFatPer) + "  Then 'C' Else 'M' End As [Milk Type]"
        End If
        ...
        qry += " Convert(decimal(18," + clsCommon.myCstr(objCommonVar.MilkSRNFATSNFDecimalPlaces) + "), ROUND(TSPL_MILK_SRN_DETAIL.FAT_PER * TSPL_MILK_SRN_DETAIL.ACC_QTY_LTR / 100," + clsCommon.myCstr(objCommonVar.MilkSRNFATSNFDecimalPlaces) + ",1)) As [FAT(LTR)]," &
                ...
                " Case When TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.Manual_Sample = 0 Then 0 Else 1 End As [Sample Status]," &
                ...
                " left outer join TSPL_BULK_ROUTE_MASTER On TSPL_BULK_ROUTE_MASTER.ROUTE_NO=TSPL_MILK_SRN_HEAD.ROUTE_CODE " + Environment.NewLine +
        ...
        End If
        qry += " where 2 = 2  and TSPL_MILK_SHIFT_UPLOADER_DETAIL.Reject_Type is null and TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.Reject_Type is null "
        ...
        Else
            qry += " and Cast(TSPL_MILK_SRN_HEAD.DOC_DATE as Date) >='" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(FromDate), "dd/MMM/yyyy") + "' and Cast(TSPL_MILK_SRN_HEAD.DOC_DATE as Date) <='" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(ToDate), "dd/MMM/yyyy") + "'"
            If clsCommon.CompairString(FromShift, "E") = CompairStringResult.Equal Then
                qry += " and 2=( case when Cast(TSPL_MILK_SRN_HEAD.DOC_DATE as Date) >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(FromDate), "dd/MMM/yyyy") + "' and Cast(TSPL_MILK_SRN_HEAD.DOC_DATE as Date) <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(FromDate), "dd/MMM/yyyy") + "' and TSPL_MILK_SRN_HEAD.SHIFT='M' then 3 else 2 end  )"
            End If
            If clsCommon.CompairString(ToShift, "M") = CompairStringResult.Equal Then
                qry += " and 2=( case when Cast(TSPL_MILK_SRN_HEAD.DOC_DATE as Date) >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(ToDate), "dd/MMM/yyyy") + "' and Cast(TSPL_MILK_SRN_HEAD.DOC_DATE as Date) <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(ToDate), "dd/MMM/yyyy") + "' and TSPL_MILK_SRN_HEAD.SHIFT='E' then 3 else 2 end  )"
            End If
        End If
        ...
        If arrMCC IsNot Nothing AndAlso arrMCC.Count > 0 Then
            qry += "and TSPL_MILK_SRN_HEAD.MCC_Code  IN (" + clsCommon.GetMulcallString(arrMCC) + ") "
        Else
            If clsCommon.myLen(StrPermission) > 0 Then
                qry += "And TSPL_MILK_SRN_HEAD.mcc_Code in (" & StrPermission & ") "
            End If
        End If
        ...
        If arrRoute IsNot Nothing AndAlso arrRoute.Count > 0 Then
            qry += " and TSPL_MILK_SRN_HEAD.Route_Code in (" + clsCommon.GetMulcallString(arrRoute) + ")  "
        End If
        If arrVLC IsNot Nothing AndAlso arrVLC.Count > 0 Then
            qry += " and TSPL_MILK_SRN_HEAD.VLC_CODE in (" + clsCommon.GetMulcallString(arrVLC) + ")  "
        End If
        ...
        If clsCommon.myLen(ReceivingUOM) > 0 Then
            qry += " and TSPL_MILK_SRN_DETAIL.UOM_Code='" + ReceivingUOM + "'"
        End If
        If clsCommon.myLen(MilkType) > 0 Then
            qry += " and TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type='" + MilkType + "'"
        End If
    ...
    Public Shared Function GetMCCRegisterWithRejectionColumnQuery(ByVal FromDate As DateTime, ByVal ToDate As DateTime, ByVal FromShift As String, ByVal ToShift As String, ByVal SRNAmounType As String, ByVal StrPermission As String, ByVal arrMCC As ArrayList, ByVal arrRoute As ArrayList, ByVal arrVLC As ArrayList, ByVal ReceivingUOM As String, ByVal strRejection As String, ByVal ShowVLCUploaderData As Boolean, ByVal SetCowFatPer As Decimal, ByVal Area As String) As String
        ...
        strSRNQuery = "Select  TSPL_MILK_SRN_DETAIL.QAT_Amt,TSPL_MILK_SRN_DETAIL.QAT_Rate,TSPL_MCC_MASTER.MCC_Type as [MCC Type],case when TSPL_MCC_MASTER.is_Mcc=1 then 'MCC' else 'BMCC' end [Chilling Center] ,TSPL_MILK_SRN_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc, TSPL_MILK_SRN_DETAIL.EMP_Amount,TSPL_MILK_SRN_DETAIL.TIP_Amount,TSPL_MILK_SRN_DETAIL.Service_Charge_Amount"
        If objCommonVar.DisplayTypeInMilkReceipt Then
            strSRNQuery += ",Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'C' Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Cow FAT(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type  = 'C' Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Cow SNF(%)]," &
        " Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'B' Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Buffalo FAT(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'B' Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Buffalo SNF(%)], Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type  = 'C' Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Cow Milk Qty (KG)]," &
        " Case When TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type   = 'B' Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Buffalo Milk Qty (KG)]" + Environment.NewLine
            strSRNQuery += ",TSPL_MILK_SRN_HEAD.Dock_Collection_Milk_Type  As [Milk Type] "
        Else
            strSRNQuery += ",Case When TSPL_MILK_SRN_DETAIL.FAT_PER <= 5 Then TSPL_MILK_SRN_DETAIL.FAT_PER Else 0 End [Cow FAT(%)], Case When TSPL_MILK_SRN_DETAIL.FAT_PER <= 5 Then TSPL_MILK_SRN_DETAIL.SNF_PER Else 0 End [Cow SNF(%)]," &
        ...
        " Case When TSPL_MILK_SRN_DETAIL.FAT_PER > 5 Then TSPL_MILK_SRN_DETAIL.ACC_QTY Else 0 End [Buffalo Milk Qty (KG)]" + Environment.NewLine
            strSRNQuery += ", Case When Coalesce(TSPL_MILK_SRN_DETAIL.FAT_PER, 0) <= 0 Then 'M' When Coalesce(TSPL_MILK_SRN_DETAIL.FAT_PER, 0) <= " + clsCommon.myCstr(SetCowFatPer) + "  Then 'C' Else 'M' End As [Milk Type]"
        End If
        ...
        " TSPL_MILK_SRN_DETAIL.FAT_kg As [FAT(KG)], TSPL_MILK_SRN_DETAIL.SNF_kg As [SNF(KG)], Case When TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.MANUAL_SAMPLE = 0 Then 0 Else 1 End As [Sample Status]," &
        " TSPL_MILK_SRN_HEAD.DOC_CODE As [SRN No], Convert(decimal(18,2),TSPL_MILK_SRN_DETAIL.AMOUNT) As [SRN Amount], TSPL_MILK_SRN_DETAIL.RATE As [SRN Rate], TSPL_MILK_SRN_DETAIL.Qty As [SRN Qty], Case When TSPL_MILK_SRN_HEAD.DOC_CODE Is Null Then 'Open' Else 'Close' End [Shift Status],TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_CODE as Invoice_no," &
        ...
        If Area IsNot Nothing AndAlso clsCommon.myLen(Area) > 0 Then
            strSRNQuery += " left join tspl_location_master on tspl_location_master.location_code=TSPL_MCC_MASTER.Area_Location_Code   "
        Else
            strSRNQuery += " left join tspl_location_master on tspl_location_master.location_code=TSPL_MCC_MASTER.Plant_Code "
        End If
        ...
        If Not ShowVLCUploaderData Then
            strSRNQuery += " and TSPL_MILK_SRN_HEAD.Against_Uploader_TR_No is null "
        End If
        If clsCommon.myLen(ReceivingUOM) > 0 Then
            strSRNQuery += " and TSPL_MILK_SRN_DETAIL.UOM_Code='" + ReceivingUOM + "'"
        End If

        If arrMCC IsNot Nothing AndAlso arrMCC.Count > 0 Then
            strSRNQuery += "and TSPL_MILK_SRN_HEAD.MCC_Code  IN (" + clsCommon.GetMulcallString(arrMCC) + ") "
        Else
            strSRNQuery += "And TSPL_MILK_SRN_HEAD.mcc_Code in (" & StrPermission & ")"
        End If
        If arrRoute IsNot Nothing AndAlso arrRoute.Count > 0 Then
            strSRNQuery += " and TSPL_MILK_SRN_HEAD.Route_Code in (" + clsCommon.GetMulcallString(arrRoute) + ")  "
        End If
        If arrVLC IsNot Nothing AndAlso arrVLC.Count > 0 Then
            strSRNQuery += " and TSPL_MILK_SRN_HEAD.VLC_CODE in (" + clsCommon.GetMulcallString(arrVLC) + ")  "
        End If
        If Area IsNot Nothing AndAlso clsCommon.myLen(Area) > 0 Then
    ...
    Public Shared Function GetMCCRegisterRejectionQuery(ByVal FromDate As DateTime, ByVal ToDate As DateTime, ByVal FromShift As String, ByVal ToShift As String, ByVal StrPermission As String, ByVal arrMCC As ArrayList, ByVal arrRoute As ArrayList, ByVal arrVLC As ArrayList, ByVal ReceivingUOM As String, ByVal SetCowFatPer As Decimal, ByVal Area As String) As String
        ...
        strRejectionQuery += " TSPL_MILK_REJECT_HEAD.DOC_CODE As [Milk Receipt Code], TSPL_MILK_REJECT_HEAD.MCC_CODE As MCC, TSPL_MCC_MASTER.MCC_NAME As [MCC Name],TSPL_LOCATION_MASTER.Location_Desc as[Area],
isnull(TSPL_MCC_MASTER.plant_code,'') As [Plant Code], isnull(tspl_location_master.location_desc,'') As [Plant Name], " &
        " Convert(date,TSPL_MILK_REJECT_HEAD.DOC_DATE,103) As Date,  Convert(varchar,TSPL_MILK_REJECT_HEAD.DOC_DATE,103) As [Doc Date], Case When TSPL_MILK_REJECT_HEAD.SHIFT = 'M' Then 'Morning' Else 'Evening' End As Shift,  TSPL_MILK_REJECT_DETAIL.ROUTE_CODE As [Route Code],tspl_mcc_route_master.Supervisor_Name as [SuperVisor Code], TSPL_MCC_ROUTE_MASTER.Route_Name As [Route Name], TSPL_MILK_REJECT_DETAIL.VEHICLE_CODE As [Vehicle Code], TSPL_MILK_REJECT_DETAIL.VSP_CODE As [VSP Code], TSPL_VENDOR_MASTER.Vendor_Name As [VSP Name],TSPL_VENDOR_MASTER.Vendor_Group_Code As [Vendor Group Code],TSPL_VENDOR_GROUP.Group_Desc as [Vendor Group Desc] ,TSPL_VLC_MASTER_HEAD.VLC_Code As [Vlc Code], TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader As [Vlc Uploader Code], (case when TSPL_VLC_MASTER_HEAD.isOwnBMC =1 then 'Y' else 'N' end) as [isOwnBMC(Y/N)],TSPL_VLC_MASTER_HEAD.VLC_Name As [VLC Name], TSPL_MILK_REJECT_DETAIL.SAMPLE_NO As [Sample No],  TSPL_MILK_REJECT_DETAIL.NO_OF_CANS As [No Of Cans], TSPL_MILK_REJECT_DETAIL.MILK_WEIGHT As [Milk Weight],TSPL_MILK_REJECT_DETAIL.UOM_Code, TSPL_MILK_REJECT_DETAIL.ACC_WEIGHT_KG As [Milk Weight(KG)], TSPL_MILK_REJECT_DETAIL.ACC_WEIGHT_LTR As [Milk Weight(LTR)], TSPL_MILK_REJECT_DETAIL.FAT As [FAT(%)], TSPL_MILK_REJECT_DETAIL.SNF As [SNF(%)],0 as CLR, Convert(decimal(18,3), TSPL_MILK_REJECT_DETAIL.FAT * TSPL_MILK_REJECT_DETAIL.ACC_WEIGHT_KG / 100) As [FAT(KG)], " &
        " Convert(decimal(18,3),TSPL_MILK_REJECT_DETAIL.SNF * TSPL_MILK_REJECT_DETAIL.ACC_WEIGHT_KG / 100) As [SNF(KG)], Case When TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.Manual_Sample = 0 Then 0 Else 1 End As [Sample Status], " &
        ...
        " Case WHEN TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.Manual_Sample=0 THEN 'N' ELSE 'Y' END AS IS_MANUAL ,'' as MACHINE_NO ,(CASE WHEN (Case When TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.Manual_Sample = 0 Then 0 Else TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.Manual_Sample End)=1 THEN 'N' ELSE 'Y' END) as IS_MILK_SAMPLE_MANUAL,TSPL_MILK_SRN_HEAD.Purchase_Order_No,TSPL_MILK_SRN_DETAIL.Head_Load_Amount,TSPL_MILK_REJECT_TYPE.Code as RejectType, " &
