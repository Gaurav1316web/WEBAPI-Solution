...
isnull (convert(decimal(18,2), ( sum( [Good SNFKG]) * 100/ nullif((sum([Good Qty])),0)    )),0) as [Good SNF %],sum([Good SNFKG]) as [Good SNFKG] " + DocRejectType + "  from ( " + qry + " ) XXXFinal group by PageNo"
    ...
    Public Shared Function MilkProcurementUploader(ByVal obj As clsMilkShiftUploaderHead, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            qry = "select TSPL_MILK_SHIFT_UPLOADER_HEAD.Shift_Date,TSPL_MILK_SHIFT_UPLOADER_HEAD.Shift,"
            If objCommonVar.DisplayTypeInMilkReceipt Then
                qry += "'M' as Dock_Collection_Milk_Type"
            Else
                qry += "TSPL_MILK_SHIFT_UPLOADER_DETAIL.Dock_Collection_Milk_Type "
            End If
            qry += " from TSPL_MILK_SHIFT_UPLOADER_DETAIL LEFT OUTER JOIN TSPL_MILK_SHIFT_UPLOADER_HEAD on TSPL_MILK_SHIFT_UPLOADER_HEAD.Document_No= TSPL_MILK_SHIFT_UPLOADER_DETAIL.Document_No where TSPL_MILK_SHIFT_UPLOADER_HEAD.Document_No='" + obj.Document_No + "' and len(isnull( TSPL_MILK_SHIFT_UPLOADER_DETAIL.Reject_Type,''))<=0 group by Shift_Date,Shift"
                ...
                For Each drDateShift As DataRow In dtDateShift.Rows
                    ...
                    If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                        If CreateNewDocumentOnUploader Then
                            ...
                            objShiftEnd.CLR = 0
                            objShiftEnd.Mix_Milk = obj.Mix_Milk
                    ...
                    Dim arr As List(Of clsMilkShiftUploaderDetail) = clsMilkShiftUploaderDetail.GetData(obj.Document_No, "  len(isnull( Reject_Type,''))<=0  " + IIf(objCommonVar.DisplayTypeInMilkReceipt, "", " and Dock_Collection_Milk_Type='" + strDockCollectionMilkType + "'"), trans)
                    If arr IsNot Nothing AndAlso arr.Count > 0 Then
                        For ii As Integer = 0 To arr.Count - 1
                            SampleNo += 1
                            Dim objtr As clsMilkShiftUploaderDetail = arr(ii)
                            ...
                            objMilkReceiptDetail.SHIFT = obj.Shift
                            ...
                            objMilkReceiptDetail.Against_Shift_Uploader_TR_No = objtr.TR_No
                            ...
                            Dim ArrParamDetailMilkProcuremnt As List(Of clsMilkShiftUploaderQCParameterDetail) = clsMilkShiftUploaderQCParameterDetail.getData(obj.Document_No, trans, objtr.SNo)
                            ...
                            ElseIf clsCommon.CompairString(clsCommon.myCstr(objHeadLoad.Head_Load_Basis), "L") = CompairStringResult.Equal Then
                                ...
                                '        Dim dblFATRate As Decimal = objMilkSRNDetail.Head_Load_Rate * clsCommon.myCdbl(dtTemp.Rows(0)("Ratio")) / clsCommon.myCdbl(dtTemp.Rows(0)("FAT_Pers"))
                                '        Dim dblSNFRate As Decimal = objMilkSRNDetail.Head_Load_Rate * clsCommon.myCdbl(dtTemp.Rows(0)("SNF_Ratio")) / clsCommon.myCdbl(dtTemp.Rows(0)("SNF_Pers"))
...
select Against_Uploader_TR_No,Against_Shift_Uploader_TR_No from TSPL_MILK_RECEIPT_DETAIL where DOC_CODE in ( select DOC_CODE from TSPL_MILK_RECEIPT_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + ")
union all
select null as Against_Uploader_TR_No, TSPL_MILK_SHIFT_UPLOADER_DETAIL.TR_No as Against_Shift_Uploader_TR_No  from TSPL_MILK_SHIFT_UPLOADER_DETAIL 
left outer join TSPL_MILK_SHIFT_UPLOADER_HEAD on TSPL_MILK_SHIFT_UPLOADER_HEAD.Document_No=TSPL_MILK_SHIFT_UPLOADER_DETAIL.Document_No
...
select Document_No from TEMP_TSPL_MILK_COLLECTION_DCS_DETAIL))X"
        ...
        clsDBFuncationality.ExecuteNonQuery(qry, tran)
        '----Milk Sample
        qry = "delete from TSPL_INVENTORY_MOVEMENT_new where Source_Doc_No in ( select DOC_CODE from TSPL_MILK_SRN_HEAD where MILK_SAMPLE_CODE in (select DOC_CODE from TSPL_MILK_SAMPLE_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + "))"
        clsDBFuncationality.ExecuteNonQuery(qry, tran)
        qry = "delete from TSPL_JOURNAL_DETAILS  where Voucher_No in ( select Voucher_No from TSPL_JOURNAL_MASTER  where Source_Doc_No in ( select DOC_CODE from TSPL_MILK_SRN_HEAD where MILK_SAMPLE_CODE in (select DOC_CODE from TSPL_MILK_SAMPLE_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + ")))"
        clsDBFuncationality.ExecuteNonQuery(qry, tran)
        qry = "delete from TSPL_JOURNAL_MASTER  where Source_Doc_No in ( select DOC_CODE from TSPL_MILK_SRN_HEAD where MILK_SAMPLE_CODE in (select DOC_CODE from TSPL_MILK_SAMPLE_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + "))"
        clsDBFuncationality.ExecuteNonQuery(qry, tran)
        qry = "delete from TSPL_MILK_SRN_DETAIL where DOC_CODE in ( select DOC_CODE from TSPL_MILK_SRN_HEAD where MILK_SAMPLE_CODE in (select DOC_CODE from TSPL_MILK_SAMPLE_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + "))"
        clsDBFuncationality.ExecuteNonQuery(qry, tran)
        'qry = "delete from TSPL_MILK_SRN_HEAD where MILK_SAMPLE_CODE in (select DOC_CODE from TSPL_MILK_SAMPLE_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + ")"
        qry = "delete from TSPL_MILK_SRN_HEAD where DOC_CODE in ( select DOC_CODE  from TSPL_MILK_SRN_HEAD where MILK_SAMPLE_CODE in (select DOC_CODE from TSPL_MILK_SAMPLE_HEAD where convert(date, DOC_DATE,103) " + transDate + "" + strMCCcode + "" + strShiftCon + "))"
        clsDBFuncationality.ExecuteNonQuery(qry, tran)
