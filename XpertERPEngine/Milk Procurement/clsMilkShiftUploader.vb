...
isnull (convert(decimal(18,2), ( sum( [Good SNFKG]) * 100/ nullif((sum([Good Qty])),0)    )),0) as [Good SNF %],sum([Good SNFKG]) as [Good SNFKG] " + DocRejectType + "  from ( " + qry + " ) XXXFinal group by PageNo"
    ...
    Public Shared Function PostDataForBatch(ByVal strCode As String) As Boolean
        ...
        Try
            ...
            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, obj.Document_No, "TSPL_MILK_SHIFT_UPLOADER_HEAD", "Document_No", trans)
    ...
    Public Shared Function MilkProcurementUploader(ByVal obj As clsMilkShiftUploaderHead, ByVal trans As SqlTransaction) As Boolean
        ...
        Try
            ...
            If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                For Each objtr As clsMilkShiftUploaderDetail In obj.Arr
                    Dim strDockCollectionMilkType As String = "M"
                    If objCommonVar.DisplayTypeInMilkReceipt Then
                        strDockCollectionMilkType = objtr.Dock_Collection_Milk_Type
                    ElseIf (objCommonVar.SepratePriceChartForCamel AndAlso clsCommon.CompairString(objtr.Dock_Collection_Milk_Type, "L") = CompairStringResult.Equal) Then
                        strDockCollectionMilkType = objtr.Dock_Collection_Milk_Type
                    End If
...
and TSPL_MILK_PURCHASE_INVOICE_HEAD.VSP_CODE='" + clsCommon.myCstr(dtVLC.Rows(0)("VSP_Code")) + "'"
    ...
    Public Shared Sub DeleteCollectionBulk(FromDate As Date, ToDate As Date, strShiftCon As String, ArrMCC As ArrayList, DeleteBMCCollection As Boolean, tran As SqlTransaction)
        ...
        End If
        qry = clsERPFuncationality.GetConstraintWorking("TSPL_MILK_PURCHASE_INVOICE_INCENTIVEDETAIL", "MILK_SRN_Code", tran)
        If clsCommon.myLen(qry) > 0 Then
            qry = "alter table TSPL_MILK_PURCHASE_INVOICE_INCENTIVEDETAIL drop  " + qry
            clsDBFuncationality.ExecuteNonQuery(qry, tran)
        End If
        qry = clsERPFuncationality.GetConstraintWorking("TSPL_MILK_PURCHASE_INVOICE_PROVISION_DETAIL", "SRN_CODE", tran)
        If clsCommon.myLen(qry) > 0 Then
            qry = "alter table TSPL_MILK_PURCHASE_INVOICE_PROVISION_DETAIL drop  " + qry
            clsDBFuncationality.ExecuteNonQuery(qry, tran)
        End If
        qry = clsERPFuncationality.GetConstraintWorking("TSPL_MILK_PURCHASE_INVOICE_PROVISON_INCENTIVEDETAIL", "MILK_SRN_Code", tran)
        If clsCommon.myLen(qry) > 0 Then
            qry = "alter table TSPL_MILK_PURCHASE_INVOICE_PROVISON_INCENTIVEDETAIL drop  " + qry
            clsDBFuncationality.ExecuteNonQuery(qry, tran)
        End If
        qry = clsERPFuncationality.GetConstraintWorking("TSPL_VENDOR_INVOICE_DETAIL", "Against_Milk_SRN_Commission_No", tran)
        If clsCommon.myLen(qry) > 0 Then
            qry = "alter table TSPL_VENDOR_INVOICE_DETAIL drop  " + qry
            clsDBFuncationality.ExecuteNonQuery(qry, tran)
        End If
        ...
        End If
        qry = "select * into TEMP_TSPL_MILK_RECEIPT_DETAIL from (
