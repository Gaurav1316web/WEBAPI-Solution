...
Public Class clsEkoPro
    ...
    Public Shared Function Convert50To52(value As Decimal) As Decimal
        Dim intPart As Integer = Math.Floor(value)
        Dim decimalPart As Decimal = value - intPart
            ...
            If Not isDripSaver Then
                ...
                Else
                    ...
                    If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JDH") = CompairStringResult.Equal Then
                        qry += " left outer join TSPL_FAT_SNF_UPLOADER_MCC on TSPL_FAT_SNF_UPLOADER_MCC.Code = TSPL_FAT_SNF_UPLOADER_MASTER.Code "
                    End If
...
where  TSPL_FAT_SNF_UPLOADER_MASTER.Posted='1' "
 ...
 and (TSPL_FAT_SNF_UPLOADER_MASTER.Date< '" & clsCommon.GetPrintDate(Doc_Date, "dd/MMM/yyyy") & "' or (TSPL_FAT_SNF_UPLOADER_MASTER.Date= '" & clsCommon.GetPrintDate(Doc_Date, "dd/MMM/yyyy") & "' and Price_code_shift>='" & Shift & "')) and ( TSPL_FAT_SNF_UPLOADER_MASTER.In_Active_From is null or TSPL_FAT_SNF_UPLOADER_MASTER.In_Active_From > '" & clsCommon.GetPrintDate(Doc_Date, "dd/MMM/yyyy") & "' ) " + Environment.NewLine +
    ...
    Public Shared Function GetRateMccSale(ByVal mccCode As String, ByVal Itemcode As String, ByVal Unitcode As String, ByVal Effctv_date As Date, ByVal trans As SqlTransaction)
        Return GetRateMccSale(mccCode, Itemcode, Unitcode, Effctv_date, Nothing, "")
    End Function
    Public Shared Function GetRateMccSale(ByVal mccCode As String, ByVal Itemcode As String, ByVal Unitcode As String, ByVal Effctv_date As Date, ByVal trans As SqlTransaction, ByVal PriceCode As String)
        ...
        Dim qry As String = "select top 1 Price from TSPL_MCC_RATE_UPLOADER_master inner join TSPL_MCC_RATE_UPLOADER_MCC on " _
              ...
              & " and TSPL_MCC_RATE_UPLOADER_Detail.RATE_UOM='" & Unitcode & "' and convert(date,TSPL_MCC_RATE_UPLOADER_Master.Date,103) <=convert(date,'" & tranDate & "',103) and convert(date,TSPL_MCC_RATE_UPLOADER_Master.Effective_date,103) >=convert(date,'" & tranDate & "',103) "
        If clsCommon.myLen(PriceCode) > 0 Then
            qry += " and TSPL_MCC_RATE_UPLOADER_Master.Price_Code = '" & PriceCode & "'"
        End If
        qry += " order by date desc ,TSPL_MCC_RATE_UPLOADER_master.code desc "
        ...
        If Rate <= 0 Then
            qry = "select top 1 TSPL_ITEM_UOM_DETAIL.Item_Code,Price,TSPL_ITEM_UOM_DETAIL.Conversion_Factor from TSPL_MCC_RATE_UPLOADER_master inner join TSPL_MCC_RATE_UPLOADER_MCC on " _
             ...
             & " and convert(date,TSPL_MCC_RATE_UPLOADER_Master.Date,103) <=convert(date,'" & tranDate & "',103) and convert(date,TSPL_MCC_RATE_UPLOADER_Master.Effective_date,103) >=convert(date,'" & tranDate & "',103) "
            If clsCommon.myLen(PriceCode) > 0 Then
                qry += " and TSPL_MCC_RATE_UPLOADER_Master.Price_Code = '" & PriceCode & "'"
            End If
            qry += " order by Date desc , TSPL_MCC_RATE_UPLOADER_master.code desc "
