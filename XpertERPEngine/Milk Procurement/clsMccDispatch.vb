...
'============ TIkcet No: BM00000009880 (Parteek)'
Imports common
Imports System.Data.SqlClient
Public Class clsMccDispatch
#Region "Variables"
    ...
    Public No_Of_Cans As Integer = 0
    Public Against_Gate_Out As String = Nothing
    Public Rejected_Only As Boolean = False
    Public arr As List(Of clsMCCDispatchDetail) = Nothing
    Public arrStock As List(Of clsMCCDispatchStockDetail) = Nothing
    Public ArrSiloWise As List(Of clsMCCDispatchSiloWise) = Nothing
    Public Collection_Of_Milk As String = Nothing
    Public Delivery_Challan_No As String = Nothing
#End Region
    ...
    Public Shared Function getFATPer(ByVal challanNo As String, ByVal trans As SqlTransaction) As Double
        Dim fatPer As Double = 0
        Dim qry As String = "select Param_Field_Value  from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type='FAT' and Chalan_No='" & challanNo & "'"
        fatPer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry, trans))
        Return fatPer

    End Function

    Public Shared Function getSNFPer(ByVal challanNo As String, ByVal trans As SqlTransaction) As Double
        Dim snfPer As Double = 0
        Dim qry As String = "select Param_Field_Value  from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type='SNF' and Chalan_No='" & challanNo & "'"
        snfPer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry, trans))
        Return snfPer

    End Function

    Public Shared Function getCLRPer(ByVal challanNo As String, ByVal trans As SqlTransaction) As Double
        Dim clrPer As Double = 0
        Dim qry As String = "select Param_Field_Value  from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type='CLR' and Chalan_No='" & challanNo & "'"
        clrPer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry, trans))
        Return clrPer

    End Function
    'done by priti KDI/15/05/18-000316
    Public Shared Function getFinder(ByVal whrcls As String, ByVal curcode As String, ByVal isButtonClicked As Boolean) As String
        Dim str As String = ""
        Try
            Dim qry As String = " select tspl_mcc_dispatch_challan.Tanker_No as [TankerNo], tspl_mcc_dispatch_challan.Chalan_NO as [ChallanNo],tspl_mcc_dispatch_challan.MCC_Code as [Mcc Code] ,tspl_mcc_dispatch_challan.MCC_Name as [Mcc Name] ,CONVERT(varchar(10), tspl_mcc_dispatch_challan.Dispatch_Date,103)+' '+ CONVERT(varchar(5), tspl_mcc_dispatch_challan.Dispatch_Date,114)   as [Dispatch Date]  ,tspl_mcc_dispatch_challan.Tanker_Dispatch_To as [Tanker Dispatch To] ,tspl_mcc_dispatch_challan.Mcc_Or_Plant_Code as [Mcc Or Plant Code],TSPL_LOCATION_MASTER.Location_Desc as [Mcc Or Plant Name] , case when TSPL_MCC_Dispatch_Challan.isPosted=0 then 'NO' else 'YES' end as [Posting Status] ,isnull(tspl_mcc_dispatch_challan.Tanker_KM_Reading,0) as [Tanker Km Reading] ,tspl_mcc_dispatch_challan.Drip_Marking as [Drip Marking] ,tspl_mcc_dispatch_challan.Tanker_Full as [Tanker Full] ,tspl_mcc_dispatch_challan.Control_Sample as [Control Sample] ,tspl_mcc_dispatch_challan.Name_Of_Custodian as [Name Of Custodian],tspl_mcc_dispatch_challan.item_Code as [Item Code], tspl_mcc_dispatch_challan.Item_Desc as [Item Description],tspl_mcc_dispatch_challan.Tare_Weight as [Tare Weight] ,tspl_mcc_dispatch_challan.Gross_Weight as [Gross Weight] ,tspl_mcc_dispatch_challan.Net_Qty as [Net Qty] ,tspl_mcc_dispatch_challan.Transfer_Price as [Transfer Price] ,  tspl_mcc_dispatch_challan.Comp_Code as [Company Code] ,tspl_mcc_dispatch_challan.Created_By as [Created By] ,tspl_mcc_dispatch_challan.Created_Date as [Created Date] ,tspl_mcc_dispatch_challan.Modified_By as [Modified By] ,tspl_mcc_dispatch_challan.Modified_Date as [Modified Date],tspl_mcc_dispatch_challan.RefDocType as [Ref Doc Type],tspl_mcc_dispatch_challan.RefDocNo as [Ref Doc No]  From tspl_mcc_dispatch_challan left join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code= tspl_mcc_dispatch_challan.Mcc_Or_Plant_Code"
            str = clsCommon.ShowSelectForm("MCCDISPATCH", qry, "ChallanNo", whrcls, curcode, "tspl_mcc_dispatch_challan.Dispatch_Date desc", isButtonClicked)
    ...
    Public Shared Function SaveData(ByVal obj As clsMccDispatch) As Boolean
    ...
    Public Shared Function SaveData(ByVal obj As clsMccDispatch, ByVal tran As SqlTransaction, Optional PostingStatus As Integer = 0, Optional chkAlreadyPosted As Boolean = True, Optional RefBulkDispatchUploader As String = "") As Boolean
        ...
        Try
            ...
            If chkAlreadyPosted Then
                If Not obj.isNewEntry Then
                    If clsERPFuncationality.IsDocumentAlreadyPosted("TSPL_MCC_DISPATCH_CHALLAN", "Chalan_no", obj.Chalan_NO, "isposted=1", tran) Then
            ...
            Dim qry As String = ""
            Dim dt As DataTable
            If clsCommon.myLen(obj.Against_Gate_Out) > 0 Then
                qry = "select 1 from (" + clsMCCTankerGateOut.GetPendingGateOutQry(obj.Chalan_NO, tran) + ")xxxx where xxxx.GATE_OUT_NO='" + obj.Against_Gate_Out + "' "
                dt = clsDBFuncationality.GetDataTable(qry, tran)
                If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                    Throw New Exception("Gate oute No  -" + obj.Against_Gate_Out + " is in use.")
                End If
                dt = Nothing
            End If

            If Not obj.isNewEntry Then
                HistoryUpdate(obj.Chalan_NO, tran)
            End If
            clsDBFuncationality.ExecuteNonQuery("Delete from TSPL_MCC_DISPATCH_CHALLAN_DETAIL where Chalan_NO='" + obj.Chalan_NO + "'", tran)
            clsDBFuncationality.ExecuteNonQuery("Delete from TSPL_MCC_Dispatch_Challan_Stock_Detail where Chalan_NO='" + obj.Chalan_NO + "'", tran)
            clsDBFuncationality.ExecuteNonQuery("Delete from TSPL_MCC_Dispatch_Silo_Wise where Challan_No='" + obj.Chalan_NO + "'", tran)
            ...
            clsCommon.AddColumnsForChange(coll, "Collection_Of_Milk", obj.Collection_Of_Milk)
            clsCommon.AddColumnsForChange(coll, "Delivery_Challan_No", obj.Delivery_Challan_No)
            ...
            clsCommon.AddColumnsForChange(coll, "Rejected_Only", IIf(obj.Rejected_Only, 1, 0))
            ...
            clsCommon.AddColumnsForChange(coll, "IsAgainstJobWork", clsCommon.myCdbl(obj.IsAgainstJobWork))
            clsCommon.AddColumnsForChange(coll, "Sublocation_Code", clsCommon.myCstr(obj.Sublocation_Code))
            ...
            Try
                Dim objMCT As MIlkComponentType = clsInventoryMovementNew.GetAvgCost("MI", obj.Item_Code, obj.MCC_Code, 1, obj.UOM_Code, 1, 1, obj.Dispatch_Date, obj.Dispatch_Date, False, tran, obj.Chalan_NO)
                obj.Avg_FAT_Rate = Math.Round(objMCT.FAT_Cost, 3, MidpointRounding.ToEven)
                obj.Avg_SNF_Rate = Math.Round(objMCT.SNF_Cost, 3, MidpointRounding.ToEven)
                obj.Avg_FAT_Amount = Math.Round(obj.Avg_FAT_Rate * obj.FAT_KG, 2, MidpointRounding.ToEven)
                obj.Avg_SNF_Amount = Math.Round(obj.Avg_SNF_Rate * obj.SNF_KG, 2, MidpointRounding.ToEven)
                obj.Avg_Amount = Math.Round(obj.Avg_FAT_Amount + obj.Avg_SNF_Amount, 2, MidpointRounding.ToEven)
            Catch ex As Exception
            End Try

            clsCommon.AddColumnsForChange(coll, "Avg_FAT_Rate", obj.Avg_FAT_Rate)
            clsCommon.AddColumnsForChange(coll, "Avg_SNF_Rate", obj.Avg_SNF_Rate)
            clsCommon.AddColumnsForChange(coll, "Avg_FAT_Amount", obj.Avg_FAT_Amount)
            clsCommon.AddColumnsForChange(coll, "Avg_SNF_Amount", obj.Avg_SNF_Amount)
            clsCommon.AddColumnsForChange(coll, "Avg_Amount", obj.Avg_Amount)

            '======================Added by preti Gupta[13/04/2018]=============
            clsCommon.AddColumnsForChange(coll, "openingKM", obj.openingKM)
            clsCommon.AddColumnsForChange(coll, "closingKM", obj.closingKM)
            '======================================================================

            clsCommon.AddColumnsForChange(coll, "RefBulkDispatchUploader", RefBulkDispatchUploader)
            clsCommon.AddColumnsForChange(coll, "No_Of_Cans", obj.No_Of_Cans)
            clsCommon.AddColumnsForChange(coll, "Against_Gate_Out", obj.Against_Gate_Out, True) '' By balwinder KDI/10/07/18-000394 last parameter true was not passed.
            If obj.isNewEntry Then
                If clsERPFuncationality.GetGSTStatus(obj.Dispatch_Date) Then
                    If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.CreateCommonSeriesLocationwiseForAllSale, clsFixedParameterCode.CreateCommonSeriesLocationwiseForAllSale, tran)) = 0 Then
                        ' if common series setting is OFF
                        If clsLocation.GetGSTLocationInterState(obj.MCC_Code, obj.Mcc_Or_Plant_Code, "T", tran) Then
                            obj.GSTStatus = "I"
                            obj.Chalan_NO = clsERPFuncationality.GetNextCode(tran, obj.Dispatch_Date, clsDocType.MccDispatchChallan, clsDocTransactionType.GSTInterstate, obj.MCC_Code)
                        Else
                            obj.Chalan_NO = clsERPFuncationality.GetNextCode(tran, obj.Dispatch_Date, clsDocType.MccDispatchChallan, clsDocTransactionType.GSTLocal, obj.MCC_Code)
                            obj.GSTStatus = "L"
                        End If
                    Else
                        ...
                        If clsLocation.GetGSTLocationInterState(obj.MCC_Code, obj.Mcc_Or_Plant_Code, "T", tran) Then
                            obj.GSTStatus = "I"
                            obj.Chalan_NO = clsERPFuncationality.GetNextCode(tran, obj.Dispatch_Date, clsDocType.CommonSaleSeries, clsDocTransactionType.GSTBillofSupply, obj.MCC_Code)
                        Else
                            obj.Chalan_NO = clsERPFuncationality.GetNextCode(tran, obj.Dispatch_Date, clsDocType.CommonSaleSeries, clsDocTransactionType.GSTDeliveryChallan, obj.MCC_Code)
                            obj.GSTStatus = "L"
                        End If
                    End If
                Else
                    obj.GSTStatus = "L"
                    obj.Chalan_NO = clsERPFuncationality.GetNextCode(tran, obj.Dispatch_Date, clsDocType.MccDispatchChallan, clsDocTransactionType.GSTNA, obj.MCC_Code)
                End If

                If clsCommon.myLen(obj.Chalan_NO) <= 0 Then
                    Throw New Exception("Error In Document code Generation")
                End If
                clsCommon.AddColumnsForChange(coll, "GSTStatus", obj.GSTStatus)
                ...
                clsCommonFunctionality.UpdateDataTable(coll, "tspl_mcc_dispatch_challan", OMInsertOrUpdate.Insert, "", tran)
            Else
                PrevTanker = clsMccDispatch.GetTankerNO(obj.Chalan_NO, tran)
                clsCommonFunctionality.UpdateDataTable(coll, "tspl_mcc_dispatch_challan", OMInsertOrUpdate.Update, "tspl_mcc_dispatch_challan.chalan_no='" + obj.Chalan_NO + "'", tran)
                'If obj.isReversed = 1 Then
                '    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "tspl_mcc_dispatch_challan_History", OMInsertOrUpdate.Insert, "", tran)
                'End If
            End If
            Mcc_Dispatch_Chalan_Parameter.SaveData(obj.Chalan_NO, obj.arrParmValue, tran, obj.isReversed)
            If obj.arrPaperSeal IsNot Nothing AndAlso obj.arrPaperSeal.Count >= 0 Then
                clsMccDispatchPaperSealDetail.SaveData(obj.Chalan_NO, obj.arrPaperSeal, tran, obj.isReversed)
            End If
            ...
            If Not obj.isBulkSaleData Then
                ...
                clsCommon.AddColumnsForChange(coll, "Ref_Doc_No", obj.Chalan_NO, True)
                clsCommonFunctionality.UpdateDataTable(coll, "tspl_tanker_master", OMInsertOrUpdate.Update, "tspl_tanker_master.tanker_no='" + obj.Tanker_No + "'", tran)
                If clsCommon.CompairString(obj.Tanker_No, PrevTanker) <> CompairStringResult.Equal Then
                    ...
                    clsCommonFunctionality.UpdateDataTable(coll, "tspl_tanker_master", OMInsertOrUpdate.Update, "tspl_tanker_master.tanker_no='" + PrevTanker + "'", tran)
                End If
            ...
            If clsCommon.myLen(RefBulkDispatchUploader) = 0 Then
                ...
                End If
            End If
            CreateTankerDispatchStockDetail(obj, tran)
            clsMCCDispatchSiloWise.SaveData(obj.Chalan_NO, obj.ArrSiloWise, tran)
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
        Return True
    ...
    Public Shared Function getData(ByVal strCode As String, ByVal navtype As NavigatorType) As clsMccDispatch
        Return getData(strCode, navtype, Nothing)
    End Function

    Public Shared Function getData(ByVal strCode As String, ByVal navtype As NavigatorType, ByVal trans As SqlTransaction) As clsMccDispatch
        ...
        Try
            ...
            Dim qst As String = " select *   From tspl_mcc_dispatch_challan   where 1=1 " & whrCls
            Select Case navtype
                Case NavigatorType.Current
                    qst += " and tspl_mcc_dispatch_challan.chalan_no in ('" + strCode + "')"
                Case NavigatorType.Next
                    qst += " and tspl_mcc_dispatch_challan.chalan_no in (select min(chalan_no ) from tspl_mcc_dispatch_challan where chalan_no  >'" + strCode + "' " & whrCls & ") "
                Case NavigatorType.First
                    qst += " and tspl_mcc_dispatch_challan.chalan_no in (select MIN(chalan_no ) from tspl_mcc_dispatch_challan where 1=1 " & whrCls & ") "
                Case NavigatorType.Last
                    qst += " and tspl_mcc_dispatch_challan.chalan_no in (select Max(chalan_no ) from tspl_mcc_dispatch_challan where 1=1 " & whrCls & ") "
                Case NavigatorType.Previous
                    qst += " and tspl_mcc_dispatch_challan.chalan_no in (select Max(chalan_no ) from tspl_mcc_dispatch_challan where chalan_no  <'" + strCode + "' " & whrCls & ") "
            End Select
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                obj.Collection_Of_Milk = clsCommon.myCstr(dt.Rows(0)("Collection_Of_Milk"))
                obj.Delivery_Challan_No = clsCommon.myCstr(dt.Rows(0)("Delivery_Challan_No"))
                ...
                End If
                obj.Rejected_Only = (clsCommon.myCdbl(dt.Rows(0)("Rejected_Only")) = 1)
                ...
                obj.Avg_FAT_Rate = clsCommon.myCdbl(dt.Rows(0)("Avg_FAT_Rate"))
                obj.Avg_FAT_Amount = clsCommon.myCdbl(dt.Rows(0)("Avg_FAT_Amount"))
                obj.Avg_SNF_Rate = clsCommon.myCdbl(dt.Rows(0)("Avg_SNF_Rate"))
                obj.Avg_SNF_Amount = clsCommon.myCdbl(dt.Rows(0)("Avg_SNF_Amount"))
                obj.Avg_Amount = clsCommon.myCdbl(dt.Rows(0)("Avg_Amount"))
                '=================Added by preeti Gupta [13/04/2018]============
                ...
                obj.No_Of_Cans = clsCommon.myCdbl(dt.Rows(0)("No_Of_Cans"))
                obj.Against_Gate_Out = clsCommon.myCstr(dt.Rows(0)("Against_Gate_Out"))

                obj.arrParmValue = Mcc_Dispatch_Chalan_Parameter.getData(obj.Chalan_NO, trans)
                obj.arrPaperSeal = clsMccDispatchPaperSealDetail.getData(obj.Chalan_NO, trans)
                obj.arr = clsMCCDispatchDetail.GetData(obj.Chalan_NO, trans)
            End If
            obj.ArrSiloWise = clsMCCDispatchSiloWise.GetData(obj.Chalan_NO, trans)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(ex.Message)
        End Try
        Return obj
    ...
    Public Shared Function ReverseAndUnpost(ByVal strCode As String, Optional trans As SqlTransaction = Nothing) As Boolean
        ...
        Try
            Dim Qry As String = "select isPosted from TSPL_MCC_Dispatch_Challan where chalan_no='" + strCode + "'"
            ...
            Dim VoucherNo As String = clsDBFuncationality.getSingleValue("select Voucher_No from TSPL_JOURNAL_MASTER where Source_Code='DI-CH' and Source_Doc_No='" + strCode + "'", trans)
            ...
            End If
            'Dim dt As DataTable
            'Qry = "select InOut,Trans_Type,Item_Code,Item_Desc,TSPL_INVENTORY_MOVEMENT_new.Location_Code,case when InOut='I' then -1 else 1 end *Qty_ as Qty ,UOM,MRP,ItemType,case when InOut='I' then -1 else 1 end* Basic_Cost as Basic_Cost from TSPL_INVENTORY_MOVEMENT " _
            '& " Left join  (select Item_Code as it_code,case when InOut='I' then -1 else 1 end *sum(Qty) as Qty_  from TSPL_INVENTORY_MOVEMENT  where Source_Doc_No='" + strCode + "' " _
            ...
            '& " =TSPL_INVENTORY_MOVEMENT.Location_Code and Rejected_Type='N' where Source_Doc_No='" + strCode + "' and Trans_Type='SRN'"
            'dt = clsDBFuncationality.GetDataTable(Qry, trans)
            ...
            Qry = "delete from TSPL_INVENTORY_MOVEMENT_new where Source_Doc_No='" + strCode + "' and Trans_Type='DispChallan'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            ...
            Qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" + strCode + "' and Trans_Type='DispChallan'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "Update TSPL_MCC_Dispatch_Challan set isPosted = 0,isreversed='1' where chalan_no='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "Update TSPL_MCC_Dispatch_Challan_Stock_Detail set isPosted = 0 where chalan_no='" + strCode + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
    ...
    Public Shared Function PostData(ByVal FormId As String, ByVal strDocNo As String, Optional trans As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            If (clsCommon.myLen(strDocNo) <= 0) Then
                Throw New Exception("Challan No not found to Post")
            End If
            If trans Is Nothing Then
                trans = clsDBFuncationality.GetTransactin()
                isChildTrans = True
            ...
            If (obj Is Nothing OrElse clsCommon.myLen(obj.Chalan_NO) <= 0) Then
                Throw New Exception("No Data found to Post")
            End If
            clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleMCCMilkProcurement, clsUserMgtCode.frmMCCDispatch, obj.MCC_Code, obj.Dispatch_Date, trans)
            ...
            '--------------------
            Dim isResult As Boolean = clsApprovalScreen.CheckApprovalLevel(FormId, "tspl_mcc_dispatch_challan", "chalan_no", obj.Chalan_NO, trans)
            If isResult = False Then
                ...
                Return False
            End If
            If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.TankerDispatchFinancialImpactInTransferIn, clsFixedParameterCode.TankerDispatchFinancialImpactInTransferIn, trans)) = 0 Then
                CreateJEAndInvenotryMovement(obj, trans, "", "")
            End If

            '' richa agarwal 11 April,2018
            Dim strQry1 As String = " update TSPL_MCC_Dispatch_Challan_Stock_Detail set isPosted='1',Posting_Date='" & clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy") & "' where chalan_no='" & obj.Chalan_NO & "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(strQry1, trans)
            ''---------

            Dim strQry As String = " update tspl_mcc_dispatch_challan set isPosted='1', isReversed='0',Posting_Date='" & clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy") & "' where chalan_no='" & obj.Chalan_NO & "'"
            isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(strQry, trans)

            If obj.ReachedAtFinal = 1 Then
                strQry = "  update tspl_mcc_dispatch_challan set ReachedAtFinal='1' where tanker_no='" & obj.Tanker_No & "' "
                If clsCommon.myLen(obj.Against_Gate_Out) > 0 Then
                    strQry += " and TSPL_MCC_Dispatch_Challan.Against_Gate_Out='" & obj.Against_Gate_Out & "' "
                End If
                ...
                If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ShowTankerWithoutCheckingAnyValidation, clsFixedParameterCode.ShowTankerWithoutCheckingAnyValidation, trans)) = 1 Then
                    strQry += " and Level1ChallanNo='" & obj.Level1ChallanNo & "'"
                End If
                isSaved = isSaved AndAlso clsDBFuncationality.ExecuteNonQuery(strQry, trans)
            End If
            If Not obj.isBulkSaleData Then
                If isSaved AndAlso clsCommon.myCdbl(obj.Payment_Amount) > 0 AndAlso clsCommon.myLen(obj.Payment_Type) > 0 Then
                    If obj.isIntermittent = 1 Then
                    Else
                        If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.CreateProvisionOfTankerDispatchWithClosingKM, clsFixedParameterCode.CreateProvisionOfTankerDispatchWithClosingKM, trans)) = 0 Then
                            If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ShowTankerWithoutCheckingAnyValidation, clsFixedParameterCode.ShowTankerWithoutCheckingAnyValidation, trans)) = 0 Then
                                ...
                                ''richa agarwal 27 May,2019 add toll_amt into Amount ERO/27/05/19-000623
                                ...
                                objProv.Amount = obj.Payment_Amount + dbltollAmt
            ...
            If obj.isIntermittent = 1 Then
                If settTankerDispatchIntermittentSingleGateIn Then
                    If Not clsCommon.CompairString(obj.FinalLoc, obj.Mcc_Or_Plant_Code) = CompairStringResult.Equal Then
                        Dim coll As New Hashtable()
                        coll = New Hashtable()
                        ...
                        clsCommon.AddColumnsForChange(coll, "isGateOut", 1)
                        clsCommon.AddColumnsForChange(coll, "Ref_Doc_No", "", True)
                        clsCommonFunctionality.UpdateDataTable(coll, "tspl_tanker_master", OMInsertOrUpdate.Update, "tspl_tanker_master.tanker_no='" + obj.Tanker_No + "'", trans)
                    End If
                Else
                    SaveAndPostFromGateEntryToMilkTransferIn(obj.Chalan_NO, trans)
                End If
            End If

            clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, clsCommon.myCstr(obj.Chalan_NO), "TSPL_MCC_DISPATCH_CHALLAN", "chalan_No", "TSPL_MCC_Dispatch_Challan_Stock_Detail", "chalan_No", trans)
            If isSaved Then
                If isChildTrans Then
                    trans.Commit()
                End If
    ...
    Public Shared Function CreateProvison(ByVal strDocNo As String, ByVal FormId As String, ByVal trans As SqlTransaction) As Boolean
        ...
        objProv.Loc_Desc = obj.MCC_Name
        objProv.Toll_Amt = dbltollAmt
        objProv.GL_Location_Seg = clsFixedParameter.GetData(clsFixedParameterType.TankerDispatchProvisionLocationSegment, clsFixedParameterCode.TankerDispatchProvisionLocationSegment, trans)
        'objProv.
    ...
    Public Shared Function HistoryUpdate(ByVal strCode As String, ByVal trans As SqlTransaction) As Boolean
        clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, clsCommon.myCstr(strCode), "TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details", "chalan_No", "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail", "chalan_No", "TSPL_MCC_Dispatch_Challan_Stock_Detail", "chalan_No", trans)
        clsCommonFunctionality.SaveHistoryData(objCommonVar.CurrentUserCode, clsCommon.myCstr(strCode), "TSPL_MCC_DISPATCH_CHALLAN", "chalan_No", "TSPL_MCC_DISPATCH_CHALLAN_DETAIL", "chalan_No", "TSPL_MCC_DISPATCH_SILO_WISE", "Challan_No", trans)
        Return True
    End Function

    Public Shared Function deleteData(ByVal challano As String, ByVal tran As SqlTransaction) As Boolean
        Try
            Dim dt As DataTable = clsDBFuncationality.GetDataTable("select tspl_mcc_dispatch_challan.MCC_Code,tspl_mcc_dispatch_challan.Dispatch_Date  from tspl_mcc_dispatch_challan where Chalan_NO='" + challano + "'", tran)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleMCCMilkProcurement, clsUserMgtCode.frmMCCDispatch, clsCommon.myCstr(dt.Rows(0)("MCC_Code")), clsCommon.myCDate(dt.Rows(0)("Dispatch_Date")), tran)
            ...
            HistoryUpdate(challano, tran)
            Dim qry As String = "delete from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where  chalan_No='" & challano & "'"
            ...
            isDeleted = isDeleted AndAlso clsDBFuncationality.ExecuteNonQuery(qry, tran)
            qry = "delete from TSPL_MCC_DISPATCH_CHALLAN_DETAIL where  chalan_No='" & challano & "'"
            isDeleted = isDeleted AndAlso clsDBFuncationality.ExecuteNonQuery(qry, tran)
            qry = "delete from TSPL_MCC_Dispatch_Challan_Stock_Detail where  chalan_No='" & challano & "'"
            isDeleted = isDeleted AndAlso clsDBFuncationality.ExecuteNonQuery(qry, tran)
            qry = "delete from TSPL_MCC_DISPATCH_SILO_WISE where Challan_No='" & challano & "'" ''Added by balwinder on 01/04/2020
            isDeleted = isDeleted AndAlso clsDBFuncationality.ExecuteNonQuery(qry, tran)
            qry = "delete from TSPL_MCC_DISPATCH_CHALLAN where  chalan_No='" & challano & "'"
            isDeleted = isDeleted AndAlso clsDBFuncationality.ExecuteNonQuery(qry, tran)
    ...
    Public Shared Function getTransferAmountChamberwiseByChamberDescription(ByVal challanNo As String, ByVal trans As SqlTransaction, ByVal strChamerDesc As String) As Double
        Dim snfRate As Double = 0
        Dim qry As String = "select Amount from TSPL_MCC_DISPATCH_CHALLAN_DETAIL where Chalan_No='" & challanNo & "' and TSPL_MCC_DISPATCH_CHALLAN_DETAIL.Chamber_Description='" & strChamerDesc & "'"
        snfRate = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry, trans))
        Return snfRate
    End Function
    ...
    Public Shared Function SaveAndPostFromGateEntryToMilkTransferIn(ByVal TankerDispatchNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Dim isSaved As Boolean = True
        Try
            Dim isReachedAtFinal As Boolean = clsCommon.myCBool(IIf(clsDBFuncationality.getSingleValue("select ReachedAtFinal  from TSPL_MCC_Dispatch_Challan where chalan_no='" & TankerDispatchNo & "'", trans) = 0, False, True))
            If isReachedAtFinal = False Then
                isSaved = clsMccDispatch.SaveAndPostGateEntryData(TankerDispatchNo, trans)
                Dim strGateEntryNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Gate_Entry_No from Tspl_Gate_Entry_Details where challan_No='" & TankerDispatchNo & "'", trans))
                isSaved = clsMccDispatch.SaveAndPostWeighmentData(strGateEntryNo, trans)
                Dim strWeighmentEntryNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Weighment_No from tspl_weighment_detail where challan_No='" & TankerDispatchNo & "'", trans))
                isSaved = clsMccDispatch.SaveAndPostQualityCheckData(strWeighmentEntryNo, trans)
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    ...
    Public Shared Function SaveAndPostGateEntryData(ByVal TankerDispatchNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        Try
            ...
            Dim obj As clsMccDispatch = clsMccDispatch.getData(TankerDispatchNo, NavigatorType.Current, trans)

            ''to save data into gate entry header table
            objGt.Doc_Type = "MccProc"
            objGt.IsAgainstJobWork = 0
            objGt.Date_And_Time = clsCommon.GetPrintDate(obj.Dispatch_Date, "dd/MMM/yyyy hh:mm:ss tt")
            ...
            End If
            objGt.Challan_No = clsCommon.myCstr(obj.Chalan_NO)
            ...
            If obj.arr IsNot Nothing AndAlso obj.arr.Count > 0 Then
                For Each objTrDD As clsMCCDispatchDetail In obj.arr
                    ...
                    objTr.snf_Per = clsCommon.myCdbl(clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Param_Field_Value from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where param_type='SNF' and Chalan_No='" & obj.Chalan_NO & "' and SNO='" & objTrDD.SNo & "'", trans)))
                    objTr.fat_per = clsCommon.myCdbl(clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Param_Field_Value from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where param_type='FAT' and Chalan_No='" & obj.Chalan_NO & "' and SNO='" & objTrDD.SNo & "'", trans)))
    ...
    Public Shared Function SaveAndPostQualityCheckData(ByVal GateEntryNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            ''for detail table of Quality Check chamber detail
            Dim dt As DataTable = clsDBFuncationality.GetDataTable("select * from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type in ('FAT','SNF','CF') and Chalan_No ='" & objWE.Challan_No & "' order by SNO ", trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                For Each dr As DataRow In dt.Rows
                ...
                Dim dtCount As DataTable = clsDBFuncationality.GetDataTable("select distinct SNO from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type in ('FAT','SNF','CF') and Chalan_No ='" & objWE.Challan_No & "' order by SNO ", trans)
                If dtCount IsNot Nothing AndAlso dtCount.Rows.Count > 0 Then
                    For Each dr As DataRow In dtCount.Rows
                        ...
                        Dim cf As Double = 0
                        Dim CLR As Double = 0
                        fat = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Param_Field_Value from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type='FAT' and Chalan_No ='" & objWE.Challan_No & "' and SNO ='" & clsCommon.myCstr(dr("SNO")) & "'", trans))
                        snf = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Param_Field_Value from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where Param_Type='SNF' and Chalan_No ='" & objWE.Challan_No & "' and SNO ='" & clsCommon.myCstr(dr("SNO")) & "'", trans))
...
Public Class Mcc_Dispatch_Chalan_Parameter
#Region "Variables"
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strChallanNo As String, ByVal arr As List(Of Mcc_Dispatch_Chalan_Parameter), ByVal tran As SqlTransaction) As Boolean
        'Try
        '    Dim i As Integer = 0
        '    Dim issaved As Boolean = True
        '    If arr.Count > 0 Then
        '        Dim qry As String = "delete from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where  chalan_No='" & arr.Item(0).Chalan_No & "'"
        ...
        '            issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail", OMInsertOrUpdate.Insert, "", tran)
        '            issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail_History", OMInsertOrUpdate.Insert, "", tran)
        '        Next
        '    End If
        '    Return issaved
        'Catch ex As Exception
        '    clsCommon.MyMessageBoxShow(ex.Message)
        'End Try
        Return SaveData(strChallanNo, arr, tran, False)
    End Function

    Public Shared Function SaveData(ByVal strChallanNo As String, ByVal arr As List(Of Mcc_Dispatch_Chalan_Parameter), ByVal tran As SqlTransaction, ByVal isReversed As Boolean) As Boolean
        Dim issaved As Boolean = True
        Try
            Dim i As Integer = 0
            If arr.Count > 0 Then
                Dim qry As String = "delete from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where  chalan_No='" & strChallanNo & "'"
                clsDBFuncationality.ExecuteNonQuery(qry, tran)
                For i = 0 To arr.Count - 1
                    Dim coll As New Hashtable()
                    ...
                    clsCommon.AddColumnsForChange(coll, "Param_Field_Desc", arr.Item(i).Param_Field_Desc)
                    clsCommon.AddColumnsForChange(coll, "Param_Field_Value", arr.Item(i).Param_Field_Value)
                    clsCommon.AddColumnsForChange(coll, "Chalan_No", strChallanNo)
                    clsCommon.AddColumnsForChange(coll, "Param_Type", arr.Item(i).Param_Type)
                    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail", OMInsertOrUpdate.Insert, "", tran)
                    If isReversed Then
                        issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail_History", OMInsertOrUpdate.Insert, "", tran)
    ...
    Public Shared Function getData(ByVal chalanNo As String) As List(Of Mcc_Dispatch_Chalan_Parameter)
        Return getData(chalanNo, Nothing)
    End Function

    Public Shared Function getData(ByVal chalanNo As String, ByVal trans As SqlTransaction) As List(Of Mcc_Dispatch_Chalan_Parameter)
        Dim arr As New List(Of Mcc_Dispatch_Chalan_Parameter)
        Try
            Dim obj As New Mcc_Dispatch_Chalan_Parameter
            Dim q As String = "select * from TSPL_Mcc_Dispatch_Chalan_Parameter_Detail where chalan_no='" & chalanNo & "'"
...
Public Class clsMccDispatchPaperSealDetail
#Region "Varibales"
    Public Chalan_No As String = String.Empty
    Public Seal_No As String = String.Empty
#End Region

    Public Shared Function SaveData(ByVal strChalanNo As String, ByVal arr As List(Of clsMccDispatchPaperSealDetail), ByVal tran As SqlTransaction) As Boolean
        'Try
        '    Dim i As Integer = 0
        '    Dim issaved As Boolean = True
        '    If arr.Count > 0 Then
        ...
        '    clsCommon.MyMessageBoxShow(ex.Message)
        'End Try
        Return SaveData(strChalanNo, arr, tran, False)
    End Function

    Public Shared Function SaveData(ByVal strChalanNo As String, ByVal arr As List(Of clsMccDispatchPaperSealDetail), ByVal tran As SqlTransaction, ByVal isReversed As Boolean) As Boolean
        Dim issaved As Boolean = True
        Try
            Dim i As Integer = 0
            If arr.Count > 0 Then
                Dim qry As String = "delete from TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details where  chalan_No='" & strChalanNo & "'"
                clsDBFuncationality.ExecuteNonQuery(qry, tran)
                For i = 0 To arr.Count - 1
                    Dim coll As New Hashtable()
                    clsCommon.AddColumnsForChange(coll, "Chalan_No", strChalanNo)
                    clsCommon.AddColumnsForChange(coll, "Seal_No", arr.Item(i).Seal_No)
                    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details", OMInsertOrUpdate.Insert, "", tran)
                    If isReversed Then
                        issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details_History", OMInsertOrUpdate.Insert, "", tran)
    ...
    Public Shared Function getData(ByVal chalanNo As String) As List(Of clsMccDispatchPaperSealDetail)
        Return getData(chalanNo, Nothing)
    End Function

    Public Shared Function getData(ByVal chalanNo As String, ByVal trans As SqlTransaction) As List(Of clsMccDispatchPaperSealDetail)
        Dim arr As New List(Of clsMccDispatchPaperSealDetail)
        Try
            Dim obj As New clsMccDispatchPaperSealDetail
            Dim q As String = "select * from TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details where chalan_no='" & chalanNo & "'"
            Dim dtbl As DataTable = clsDBFuncationality.GetDataTable(q, trans)
            If dtbl IsNot Nothing AndAlso dtbl.Rows.Count > 0 Then
                For i As Integer = 0 To dtbl.Rows.Count - 1
                    obj = New clsMccDispatchPaperSealDetail
                    obj.Chalan_No = clsCommon.myCstr(dtbl.Rows(i)("Chalan_No"))
...
Public Class clsMCCDispatchDetail
#Region "Variables"
    Public SNo As Integer
    Public isCanType As Integer = 0
    Public Chalan_No As String
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal arr As List(Of clsMCCDispatchDetail), ByVal trans As SqlTransaction) As Boolean
        Try
            If arr IsNot Nothing AndAlso arr.Count > 0 Then
                For Each obj As clsMCCDispatchDetail In arr
                    Dim coll As New Hashtable()
                    clsCommon.AddColumnsForChange(coll, "SNo", obj.SNo)
                    clsCommon.AddColumnsForChange(coll, "Chalan_No", strDocNo)
                    ...
                    clsCommon.AddColumnsForChange(coll, "Intermittent_Dispatch_No", obj.Intermittent_Dispatch_No, True)
                    clsCommon.AddColumnsForChange(coll, "isCanType", obj.isCanType)
                    clsCommonFunctionality.UpdateDataTable(coll, "TSPL_MCC_DISPATCH_CHALLAN_DETAIL", OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
    ...
    Public Shared Function GetData(ByVal strChalanNo As String, ByVal trans As SqlTransaction) As List(Of clsMCCDispatchDetail)
        Dim arr As List(Of clsMCCDispatchDetail) = Nothing
        Try
            Dim qry As String = "select TSPL_MCC_DISPATCH_CHALLAN_DETAIL.*,TSPL_ITEM_MASTER.Item_Desc FROM  TSPL_MCC_DISPATCH_CHALLAN_DETAIL left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.item_Code=TSPL_MCC_DISPATCH_CHALLAN_DETAIL.item_Code where Chalan_No='" + strChalanNo + "' order by TSPL_MCC_DISPATCH_CHALLAN_DETAIL.sno " ''Added Order by by balwinder on 04/07/2022 becuase it pick rate at the time of milk transer in.
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                arr = New List(Of clsMCCDispatchDetail)
                For Each dr As DataRow In dt.Rows
                    Dim obj As New clsMCCDispatchDetail
                    obj.SNo = clsCommon.myCdbl(dr("SNo"))
                    obj.Chalan_No = clsCommon.myCstr(dr("Chalan_No"))
                    ...
                    obj.Intermittent_Dispatch_No = clsCommon.myCstr(dr("Intermittent_Dispatch_No"))
...
Public Class clsMCCDispatchStockDetail
#Region "Variables"
    ...
    Public Chalan_No As String = Nothing
    Public Chalan_Date As DateTime? = Nothing
...
#End Region
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal arr As List(Of clsMCCDispatchStockDetail), ByVal trans As SqlTransaction) As Boolean
        Try
            Dim LineNo As Integer = 1
            If (arr IsNot Nothing AndAlso arr.Count > 0) Then
                For Each obj As clsMCCDispatchStockDetail In arr
                    Dim coll As New Hashtable()
                    ...
                    clsCommon.AddColumnsForChange(coll, "Chalan_No", strDocNo)
                    clsCommon.AddColumnsForChange(coll, "Chalan_Date", clsCommon.GetPrintDate(obj.Chalan_Date, "dd/MM/yyyy"))
                    ...
                    objCost = clsInventoryMovementNew.GetAvgCost("MI", obj.Item_Code, obj.Location_Code, obj.Qty, obj.UOM, obj.FAT_KG, obj.SNF_KG, clsCommon.GetPrintDate(obj.Chalan_Date, "dd/MMM/yyyy"), clsCommon.GetPrintDate(obj.Chalan_Date, "dd/MMM/yyyy"), True, trans)
                    ...
                    clsCommonFunctionality.UpdateDataTable(coll, "TSPL_MCC_Dispatch_Challan_Stock_Detail", OMInsertOrUpdate.Insert, "", trans)
                    LineNo += 1
                Next
...
Public Class clsMCCDispatchSiloWise
#Region "Variables"
    Public Challan_No As String = Nothing
    Public Chamber_No As Integer = Nothing
    Public Item_Code As String = Nothing
    Public Location_Code As String = Nothing
    Public Qty As Double = 0
    Public UOM As String = Nothing
...
#End Region
    ...
    Public Shared Function InventoryMovementAvgFATSNF(ByVal DocNo As String, ByVal DocDate As DateTime, ByVal arr As List(Of clsMCCDispatchDetail), ByVal strMCC_Code As String, ByVal strMcc_Or_Plant_Code As String, ByVal IsRejectOnly As Boolean, ByVal trans As SqlTransaction, ByVal FromDateForAvg As Date?, ByRef dtSummary As DataTable, ByVal isConsiderSameDayDispatch As Boolean) As List(Of clsMCCDispatchSiloWise)
        Dim qry As String = ""
        Dim arrReturn As New List(Of clsMCCDispatchSiloWise)
        dtSummary = Nothing
        Try
            If clsCommon.myLen(DocNo) <= 0 Then
                DocNo = clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "yyyyMMddhhmmssmstt")
            End If
            Dim extraWhr As String = ""
            If FromDateForAvg IsNot Nothing Then
                extraWhr = " and Punching_Date >='" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(FromDateForAvg), "dd/MMM/yyyy hh:mm tt") + "'"
            End If
            If isConsiderSameDayDispatch Then
                qry = "select TSPL_MCC_Dispatch_Challan.Chalan_NO from TSPL_MCC_Dispatch_Challan inner join " + Environment.NewLine +
            "(select MCC_CODE,Chalan_NO,Dispatch_Date from TSPL_MCC_Dispatch_Challan  where Chalan_NO='" + DocNo + "')FilTable on FilTable.MCC_CODE=TSPL_MCC_Dispatch_Challan.MCC_CODE and  convert(date,FilTable.Dispatch_Date,103)= convert(date,TSPL_MCC_Dispatch_Challan.Dispatch_Date,103)" + Environment.NewLine +
            "where  TSPL_MCC_Dispatch_Challan.isPosted=1"
                dtSummary = clsDBFuncationality.GetDataTable(qry, trans)
                If dtSummary IsNot Nothing AndAlso dtSummary.Rows.Count > 0 Then
                    extraWhr += " and 2= (case when  Trans_Type='DispChallan' and Source_Doc_No in ("
                    For ii As Integer = 0 To dtSummary.Rows.Count - 1
                        extraWhr += "'" + clsCommon.myCstr(dtSummary.Rows(ii)("Chalan_NO")) + "'"
                        If ii <> dtSummary.Rows.Count - 1 Then
                            extraWhr += ","
                        End If
                    Next
                    extraWhr += ") then 3 else 2 end) "
                End If
            End If

            qry = "Delete from tspl_inventory_movement_new where Trans_Type='TempDispChallan' and Source_Doc_No='" + DocNo + "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
            For Each objtr As clsMCCDispatchDetail In arr
                Dim strItemType As String = clsItemMaster.GetItemType(objtr.Item_Code, trans)
                Dim strItemTypeToSave As String = ""
                If clsCommon.CompairString(strItemType, "R") = CompairStringResult.Equal Then
                    strItemTypeToSave = "RM"
                ...
                Dim strtransactionLocation As String = strMCC_Code
                Dim IsSubLocation As Boolean = False
                balqtyfortotallocation = objtr.Qty_KG
                dtSummary = clsDBFuncationality.GetDataTable(strqry, trans)
                If dtSummary IsNot Nothing AndAlso dtSummary.Rows.Count > 0 Then
                    For i As Integer = 0 To dtSummary.Rows.Count - 1
                        strsublocation = clsCommon.myCstr(dtSummary.Rows(i)("Code"))
                        If clsCommon.myLen(strsublocation) > 0 Then
                            balqtyofvl = clsCommon.myCdbl(ClsLoadingTanker.getBalance(objtr.Item_Code, strMCC_Code, strsublocation, DocNo, DocDate, trans, "KG"))
                        End If
                        If balqtyfortotallocation > 0 Then
                            ...
                            objInventoryMovemnt1.Item_Desc = clsItemMaster.GetItemName(objtr.Item_Code, trans)
                            objInventoryMovemnt1.Qty = balqtyofvl
                            objInventoryMovemnt1.UOM = objtr.Item_UOM
                            objInventoryMovemnt1.MRP = 0
                            objInventoryMovemnt1.Add_Cost = 0
                            objInventoryMovemnt1.itemstatus = objtr.Chamber_No

                            Dim objMCT As MIlkComponentType = clsInventoryMovementNew.GetAvgCost(True, False, IsRejectOnly, False, extraWhr, "MI", objtr.Item_Code, strsublocation, 1, objtr.Item_UOM, 1, 1, DocDate, DocDate, False, trans, DocNo)
                            objInventoryMovemnt1.FAT_Per = Math.Round(objMCT.FAT_Per, 10, MidpointRounding.ToEven)
                            objInventoryMovemnt1.SNF_Per = Math.Round(objMCT.SNF_Per, 10, MidpointRounding.ToEven)
                            ...
                            totalamt += objInventoryMovemnt1.Fat_Amt + objInventoryMovemnt1.SNF_Amt
                            objInventoryMovemnt1.ItemType = strItemTypeToSave
                            ...
                            ArrInventoryMovement1.Add(objInventoryMovemnt1)
                            If balqtyofvl > 0 Or CurBalOFVL_FAT > 0 Or CurBalOFVL_SNF > 0 Then
                                clsInventoryMovementNew.SaveData("TempDispChallan", DocNo, DocDate, clsCommon.GetPrintDate(DocDate, "dd/MM/yyyy"), ArrInventoryMovement1, trans)
                            End If
                            balqtyfortotallocation = balqtyfortotallocation - balqtyofvl
                        End If
                    Next
                ...
                Else
                    If balqtyfortotallocation > 0 Then
                        ...
                        Dim ArrInventoryMovement1 As List(Of clsInventoryMovementNew) = New List(Of clsInventoryMovementNew)
                        objInventoryMovemnt1.InOut = "O"
                        balqtyofvl = clsCommon.myCdbl(ClsLoadingTanker.getBalance(objtr.Item_Code, strMCC_Code, "", DocNo, DocDate, trans, "KG"))
                        If balqtyofvl >= balqtyfortotallocation Then
                            balqtyofvl = balqtyfortotallocation
                        End If
                        objInventoryMovemnt1.main_location = strMCC_Code
                        ...
                        objInventoryMovemnt1.Add_Cost = 0
                        objInventoryMovemnt1.itemstatus = objtr.Chamber_No
                        Dim objMCT As MIlkComponentType = clsInventoryMovementNew.GetAvgCost(True, False, IsRejectOnly, False, extraWhr, "MI", objtr.Item_Code, strMCC_Code, 1, objtr.Item_UOM, 1, 1, DocDate, DocDate, False, trans, DocNo)
                        objInventoryMovemnt1.FAT_Per = Math.Round(objMCT.FAT_Per, 10, MidpointRounding.ToEven)
                        objInventoryMovemnt1.SNF_Per = Math.Round(objMCT.SNF_Per, 10, MidpointRounding.ToEven)
                        objInventoryMovemnt1.FAT_KG = Math.Round(objMCT.FAT_Per * balqtyofvl / 100, 3, MidpointRounding.ToEven)
                        objInventoryMovemnt1.SNF_KG = Math.Round(objMCT.SNF_Per * balqtyofvl / 100, 3, MidpointRounding.ToEven)
            ...
            qry = "select Location_Code,item_status as Chamber_No,Item_Code,Qty,UOM,Stock_Qty,Stock_UOM,Fat_Per,Fat_Rate,Fat_KG,Fat_Amt,SNF_Per,SNF_Rate,SNF_KG,SNF_Amt,(FAT_amt+ SNF_Amt) as Amount from tspl_inventory_movement_new where Trans_Type='TempDispChallan' and Source_Doc_No='" + DocNo + "' "
            dtSummary = clsDBFuncationality.GetDataTable(qry, trans)
            For i As Integer = 0 To dtSummary.Rows.Count - 1
                Dim obj As New clsMCCDispatchSiloWise
                'obj.Challan_No = clsCommon.myCstr(dtSummary.Rows(i)("Challan_No"))
                obj.Chamber_No = clsCommon.myCstr(dtSummary.Rows(i)("Chamber_No"))
                obj.Item_Code = clsCommon.myCstr(dtSummary.Rows(i)("Item_Code"))
                obj.Location_Code = clsCommon.myCstr(dtSummary.Rows(i)("Location_Code"))
                obj.Qty = clsCommon.myCdbl(dtSummary.Rows(i)("Qty"))
                obj.UOM = clsCommon.myCstr(dtSummary.Rows(i)("UOM"))
                obj.Stock_Qty = clsCommon.myCdbl(dtSummary.Rows(i)("Stock_Qty"))
                obj.Stock_UOM = clsCommon.myCstr(dtSummary.Rows(i)("Stock_UOM"))
                obj.FAT_Per = clsCommon.myCdbl(dtSummary.Rows(i)("FAT_Per"))
                obj.Fat_Rate = clsCommon.myCdbl(dtSummary.Rows(i)("Fat_Rate"))
                obj.FAT_KG = clsCommon.myCdbl(dtSummary.Rows(i)("FAT_KG"))
                obj.Fat_Amt = clsCommon.myCdbl(dtSummary.Rows(i)("Fat_Amt"))
                obj.SNF_Per = clsCommon.myCdbl(dtSummary.Rows(i)("SNF_Per"))
                obj.SNF_Rate = clsCommon.myCdbl(dtSummary.Rows(i)("SNF_Rate"))
                obj.SNF_KG = clsCommon.myCdbl(dtSummary.Rows(i)("SNF_KG"))
                obj.SNF_Amt = clsCommon.myCdbl(dtSummary.Rows(i)("SNF_Amt"))
                obj.Amount = clsCommon.myCdbl(dtSummary.Rows(i)("Amount"))
                arrReturn.Add(obj)
            Next
            qry = "select Chamber_No,sum(Qty) as Qty,sum(Stock_Qty) as Stock_Qty,(case when sum(Qty)=0 then 0 else sum(Fat_KG)*100/sum(Qty) end) as Fat_Per,case when sum(Fat_KG)=0 then 0 else sum(Fat_Amt)/sum(Fat_KG) end as Fat_Rate,sum(Fat_KG) as Fat_KG,sum(Fat_Amt) as Fat_Amt,(case when sum(Qty)=0 then 0 else sum(SNF_KG)*100/sum(Qty) end) as SNF_Per,  case when sum(SNF_KG)=0 then 0 else sum(SNF_Amt)/sum(SNF_KG) end as SNF_Rate,sum(SNF_KG) as SNF_KG,sum(SNF_Amt) as SNF_Amt,sum(Amount) as Amount from (" + qry + ")xx group by Chamber_No"
            dtSummary = clsDBFuncationality.GetDataTable(qry, trans)
        Catch ex As Exception
            Throw New Exception(ex.Message)
        Finally
    ...
    Public Shared Function SaveData(ByVal strDocNo As String, ByVal arr As List(Of clsMCCDispatchSiloWise), ByVal trans As SqlTransaction) As Boolean
        Try
            Dim LineNo As Integer = 1
            If (arr IsNot Nothing AndAlso arr.Count > 0) Then
                For Each obj As clsMCCDispatchSiloWise In arr
                    Dim coll As New Hashtable()

                    clsCommon.AddColumnsForChange(coll, "Challan_No", strDocNo)
                    clsCommon.AddColumnsForChange(coll, "Chamber_No", obj.Chamber_No)
                    clsCommon.AddColumnsForChange(coll, "Item_Code", obj.Item_Code)
                    clsCommon.AddColumnsForChange(coll, "Location_Code", obj.Location_Code)
                    clsCommon.AddColumnsForChange(coll, "Qty", obj.Qty)
                    ...
                    clsCommon.AddColumnsForChange(coll, "SNF_Amt", obj.SNF_Amt)
                    clsCommon.AddColumnsForChange(coll, "Amount", obj.Amount)
                    clsCommonFunctionality.UpdateDataTable(coll, "TSPL_MCC_Dispatch_Silo_Wise", OMInsertOrUpdate.Insert, "", trans)
                Next
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
    ...
    Public Shared Function GetData(ByVal strDocNo As String, ByVal trans As SqlTransaction) As List(Of clsMCCDispatchSiloWise)
        Dim arr As New List(Of clsMCCDispatchSiloWise)
        Try
            Dim obj As New clsMCCDispatchSiloWise
            Dim q As String = "select * from TSPL_MCC_Dispatch_Silo_Wise where Challan_No='" & strDocNo & "'"
            Dim dtbl As DataTable = clsDBFuncationality.GetDataTable(q, trans)
            If dtbl IsNot Nothing AndAlso dtbl.Rows.Count > 0 Then
                For i As Integer = 0 To dtbl.Rows.Count - 1
                    obj = New clsMCCDispatchSiloWise
                    obj.Challan_No = clsCommon.myCstr(dtbl.Rows(i)("Challan_No"))
                    obj.Chamber_No = clsCommon.myCstr(dtbl.Rows(i)("Chamber_No"))
                    obj.Item_Code = clsCommon.myCstr(dtbl.Rows(i)("Item_Code"))
                    obj.Location_Code = clsCommon.myCstr(dtbl.Rows(i)("Location_Code"))
                    obj.Qty = clsCommon.myCdbl(dtbl.Rows(i)("Qty"))
                    obj.UOM = clsCommon.myCstr(dtbl.Rows(i)("UOM"))
                    obj.Stock_Qty = clsCommon.myCdbl(dtbl.Rows(i)("Stock_Qty"))
                    obj.Stock_UOM = clsCommon.myCstr(dtbl.Rows(i)("Stock_UOM"))
                    obj.FAT_Per = clsCommon.myCdbl(dtbl.Rows(i)("FAT_Per"))
                    obj.Fat_Rate = clsCommon.myCdbl(dtbl.Rows(i)("Fat_Rate"))
                    obj.FAT_KG = clsCommon.myCdbl(dtbl.Rows(i)("FAT_KG"))
                    obj.Fat_Amt = clsCommon.myCdbl(dtbl.Rows(i)("Fat_Amt"))
                    obj.SNF_Per = clsCommon.myCdbl(dtbl.Rows(i)("SNF_Per"))
                    obj.SNF_Rate = clsCommon.myCdbl(dtbl.Rows(i)("SNF_Rate"))
                    obj.SNF_KG = clsCommon.myCdbl(dtbl.Rows(i)("SNF_KG"))
                    obj.SNF_Amt = clsCommon.myCdbl(dtbl.Rows(i)("SNF_Amt"))
