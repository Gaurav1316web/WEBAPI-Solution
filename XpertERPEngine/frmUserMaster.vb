...
#Region "Methods"
    ...
    Private Sub funUpdate()
        Try
            If IsCorrectUSer(Nothing) Then
                ...
                clsDBFuncationality.SaveAStorePorcedure("sp_tspl_user_master_update", New SqlParameter("@Usercode", fndUserCode.Value), New SqlParameter("@UserName", txtUserName.Text), New SqlParameter("@EmployeeCode", fndEmployeeCode.Value), New SqlParameter("@EmployeeName", txtEmployeeName.Text), New SqlParameter("@Password", clsCommon.EncryptString(txtPassword.Text)), New SqlParameter("@UserType", ddlUserType.Text), New SqlParameter("@Level1", fndLabel1.Value), New SqlParameter("@Level2", fndLabel2.Value), New SqlParameter("@Level3", fndLabel3.Value), New SqlParameter("@Level4", fndLabel4.Value), New SqlParameter("@Createdby", userCode), New SqlParameter("@Createddate", connectSql.serverDate()), New SqlParameter("@Modifiedby", userCode), New SqlParameter("@Modifieddate", connectSql.serverDate()), New SqlParameter("@CompCode", companyCode), New SqlParameter("@ApprovalLevel", cmbLevel.SelectedValue))
    ...
    Private Sub funDelete()
        ...
        clsDBFuncationality.SaveAStorePorcedure("sp_TSPL_USER_MASTER_delete", New SqlParameter("@UserCode", fndUserCode.Value))
        myMessages.delete()
        btnSave.Text = "Save"
        btnDelete.Enabled = False
    End Sub
...
#Region "Button Click Event"
    ...
    Function saveCancelLog(ByVal Reason As String, ByVal Activity_Type As String, Optional ByVal trans As System.Data.SqlClient.SqlTransaction = Nothing) As Boolean
        ...
        obj.DOCUMENT_NO = clsCommon.myCstr(Me.fndUserCode.Value)
        obj.REASON = Reason
    ...
    Private Sub SaveUpdateData()
        Try
            If clsCommon.CompairString(btnSave.Text, "Update") = CompairStringResult.Equal Then
                Dim dt As DataTable = clsDBFuncationality.GetDataTable("Select Cust_Code,Customer_Name,Status from TSPL_Customer_Master Where Cust_Code='" + fndCustCode.Value + "'")
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Status")), "Y") = CompairStringResult.Equal Then
                        Throw New Exception("Customer :'" + clsCommon.myCstr(dt.Rows(0)("Cust_Code")) + " " + clsCommon.myCstr(dt.Rows(0)("Customer_Name")) + "'  Inactive in customer master")
                    End If
                End If
            End If
