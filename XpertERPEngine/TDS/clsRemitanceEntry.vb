...
#End Region
    ...
    ''        transportSql.FunGrnlEntryWithTrans(trans, paymentdate, clsCommon.myCstr(dt.Rows(0)("Description")), srctype, strsrcdesc, STRPAYmentno, strdocdesc, strsrctype, strsrctypecode, strsrctypedesc, objCommonVar.CurrentUserCode, objCommonVar.CurrentCompanyCode, arrmis)
    ''        trans.Commit()
    ...
    Public Shared Function PostData(ByVal strDocNo As String) As Boolean
        ...
        Try
            ...
            'transportSql.FunGrnlEntryWithTrans(trans, obj.AP_Payment_Date, clsCommon.myCstr(obj.Description), srctype, strsrcdesc, STRPAYmentno, strdocdesc, strsrctype, strsrctypecode, strsrctypedesc, objCommonVar.CurrentUserCode, objCommonVar.CurrentCompanyCode, arrmis)
            'trans.Commit()
            'Dim journalno As String = connectSql.RunScalar("select MAX(Voucher_No ) from TSPL_JOURNAL_MASTER")
            'connectSql.RunSql("update TSPL_JOURNAL_MASTER set Source_Desc = '" + strsrcdesc + " ', Remarks = '" + reference + "', Comments = '" + narration + "' where Voucher_No = '" + journalno + "'")
            '------end Commit
