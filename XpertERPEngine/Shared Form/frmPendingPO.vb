...
#Region "Variables"
    ...
    Const colItemStatus As String = "colItemStatus"
...
, tspl_item_master.tolerence as tolerencePer ,(case when isnull(TSPL_ITEM_SLAB_TOLERANCE.Qty,0)>0 and isnull(TSPL_ITEM_SLAB_TOLERANCE.Qty,0)<((TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty * ISNULL(TSPL_ITEM_MASTER.tolerence, 0))/ 100 ) then isnull(TSPL_ITEM_SLAB_TOLERANCE.Qty,0) else ((TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty * ISNULL(TSPL_ITEM_MASTER.tolerence, 0))/ 100) end) as ItemTolerenceQty
,TSPL_PURCHASE_ORDER_DETAIL.Row_Type as IType"
                If AutoClosePOBasedOnSRNQtyWithTolerance Then
                    qry += ",(TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty+(case when isnull(TSPL_ITEM_SLAB_TOLERANCE.Qty,0)>0 and isnull(TSPL_ITEM_SLAB_TOLERANCE.Qty,0)<((TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty * ISNULL(TSPL_ITEM_MASTER.tolerence, 0))/ 100 ) then isnull(TSPL_ITEM_SLAB_TOLERANCE.Qty,0) else ((TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty * ISNULL(TSPL_ITEM_MASTER.tolerence, 0))/ 100) end))"
                Else
                    qry += ",TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty "
                End If

                qry += " as Qty,0 as Unapproved,TSPL_PURCHASE_ORDER_DETAIL.Unit_Code as Unit,TSPL_PURCHASE_ORDER_DETAIL.Location as Location,1 as RI,TSPL_PURCHASE_ORDER_DETAIL.Item_Cost as Rate,1 as Chk,TSPL_PURCHASE_ORDER_HEAD.Tax_Group,TSPL_PURCHASE_ORDER_DETAIL.TAX1_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX2_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX3_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX4_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX5_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX6_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX7_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX8_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX9_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX10_Rate,TSPL_PURCHASE_ORDER_DETAIL.TAX1_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX2_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX3_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX4_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX5_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX6_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX7_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX8_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX9_Amt,TSPL_PURCHASE_ORDER_DETAIL.TAX10_Amt ,TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_Date as TransDate,ISNULL(TSPL_PURCHASE_ORDER_DETAIL.Assessable,0) AS Assessable,ISNULL(TSPL_PURCHASE_ORDER_DETAIL.MRP,0) as MRP,0 as DamageQty,TSPL_PURCHASE_ORDER_DETAIL.AbatementRate,TSPL_PURCHASE_ORDER_DETAIL.Item_Insurance_Apply_On,TSPL_PURCHASE_ORDER_DETAIL.Item_Insurance_Rate,TSPL_PURCHASE_ORDER_DETAIL.Item_Insurance_Amt,TSPL_PURCHASE_ORDER_HEAD.RefTendorNo,Case When IsNull(TSPL_PURCHASE_ORDER_DETAIL.Item_Close_YN,'')='' Then 'Open' When IsNull(TSPL_PURCHASE_ORDER_DETAIL.Item_Close_YN,'')='N' Then 'Open'	Else 'Close' End As [Item Status]  
...
where TSPL_PURCHASE_ORDER_DETAIL.Status=0 and TSPL_PURCHASE_ORDER_HEAD.Status=1 and TSPL_PURCHASE_ORDER_HEAD.close_yn='N' and  TSPL_PURCHASE_ORDER_HEAD.IsCancel=0 and TSPL_PURCHASE_ORDER_HEAD.item_type<>'N' " + strPOType + " " + strCondition + " "
...
END AS Qty,0 as Unapproved,TSPL_SRN_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,
                ...
                '' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt, null as RefTendorNo,'' As [Item Status]
...
END AS Qty,0 as Unapproved,TSPL_GRN_DETAIL.Unit_code as Unit,'' as Location,
                ...
                0 as AbatementRate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt, null as RefTendorNo,'' As [Item Status]  
	  ...
	  0 as ItemTolerenceQty,'' as IType,TSPL_RGP_JOB_WORK_DETAIL.rgp_qty as Qty,0 as Unapproved,TSPL_RGP_JOB_WORK_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate ,0 as  TAX1_Amt,0 as  TAX2_Amt,0 as  TAX3_Amt,0 as  TAX4_Amt,0 as  TAX5_Amt,0 as  TAX6_Amt,0 as  TAX7_Amt,0 as  TAX8_Amt,0 as  TAX9_Amt,0 as  TAX10_Amt,null as TransDate,0 as Assessable,0 as MRP,0 as DamageQty,0 as AbatementRate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt, null as RefTendorNo,'' As [Item Status]  from TSPL_RGP_JOB_WORK_DETAIL left outer join tspl_rgp_head on tspl_rgp_head.rgp_no=TSPL_RGP_JOB_WORK_DETAIL.rgp_no left outer join tspl_item_master on tspl_item_master.item_code=TSPL_RGP_JOB_WORK_DETAIL.item_code where tspl_rgp_head.status=1 and len(isnull(TSPL_RGP_JOB_WORK_DETAIL.PO_id,''))>0 and len(isnull(TSPL_RGP_JOB_WORK_DETAIL.against_schedule_code,''))<=0  " + Environment.NewLine &
                " union all  " + Environment.NewLine &
	  ...
	  0 as ItemTolerenceQty,'' as IType,0  as Qty,TSPL_RGP_JOB_WORK_DETAIL.rgp_qty as Unapproved,TSPL_RGP_JOB_WORK_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate ,0 as  TAX1_Amt,0 as  TAX2_Amt,0 as  TAX3_Amt,0 as  TAX4_Amt,0 as  TAX5_Amt,0 as  TAX6_Amt,0 as  TAX7_Amt,0 as  TAX8_Amt,0 as  TAX9_Amt,0 as  TAX10_Amt,null as TransDate,0 as Assessable,0 as MRP,0 as DamageQty, 0 as AbatementRate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt, null as RefTendorNo,'' As [Item Status]  from TSPL_RGP_JOB_WORK_DETAIL left outer join tspl_rgp_head on tspl_rgp_head.rgp_no=TSPL_RGP_JOB_WORK_DETAIL.rgp_no left outer join tspl_item_master on tspl_item_master.item_code=TSPL_RGP_JOB_WORK_DETAIL.item_code where tspl_rgp_head.status=0 and TSPL_RGP_JOB_WORK_DETAIL.rgp_no not in ('" + strCurrCode + "') and len(isnull(TSPL_RGP_JOB_WORK_DETAIL.PO_id,''))>0 and len(isnull(TSPL_RGP_JOB_WORK_DETAIL.against_schedule_code,''))<=0 " + Environment.NewLine
                Else
	  ...
	  0 as ItemTolerenceQty,'' as IType,TSPL_RGP_DETAIL.rgp_qty as Qty,0 as Unapproved,TSPL_RGP_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate ,0 as  TAX1_Amt,0 as  TAX2_Amt,0 as  TAX3_Amt,0 as  TAX4_Amt,0 as  TAX5_Amt,0 as  TAX6_Amt,0 as  TAX7_Amt,0 as  TAX8_Amt,0 as  TAX9_Amt,0 as  TAX10_Amt,null as TransDate,0 as Assessable,0 as MRP,0 as DamageQty,0 as AbatementRate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt, null as RefTendorNo,'' As [Item Status]  from TSPL_RGP_DETAIL left outer join tspl_rgp_head on tspl_rgp_head.rgp_no=TSPL_RGP_DETAIL.rgp_no left outer join tspl_item_master on tspl_item_master.item_code=TSPL_RGP_DETAIL.item_code where tspl_rgp_head.status=1 and isnull(tspl_rgp_head.Against_JobWork,0)=0 and len(isnull(TSPL_RGP_DETAIL.PO_id,''))>0 and len(isnull(TSPL_RGP_DETAIL.against_schedule_code,''))<=0  " + Environment.NewLine &
                " union all  " + Environment.NewLine &
	  ...
	  0 as ItemTolerenceQty,'' as IType,0  as Qty,TSPL_RGP_DETAIL.rgp_qty as Unapproved,TSPL_RGP_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate ,0 as  TAX1_Amt,0 as  TAX2_Amt,0 as  TAX3_Amt,0 as  TAX4_Amt,0 as  TAX5_Amt,0 as  TAX6_Amt,0 as  TAX7_Amt,0 as  TAX8_Amt,0 as  TAX9_Amt,0 as  TAX10_Amt,null as TransDate,0 as Assessable,0 as MRP,0 as DamageQty, 0 as AbatementRate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt, null as RefTendorNo,'' As [Item Status]  from TSPL_RGP_DETAIL left outer join tspl_rgp_head on tspl_rgp_head.rgp_no=TSPL_RGP_DETAIL.rgp_no left outer join tspl_item_master on tspl_item_master.item_code=TSPL_RGP_DETAIL.item_code where tspl_rgp_head.status=0 and isnull(tspl_rgp_head.Against_JobWork,0)=0 and TSPL_RGP_DETAIL.rgp_no not in ('" + strCurrCode + "') and len(isnull(TSPL_RGP_DETAIL.PO_id,''))>0 and len(isnull(TSPL_RGP_DETAIL.against_schedule_code,''))<=0 " + Environment.NewLine
                End If
    ...
    Sub LoadBlankGridDetail()
        ...
        Dim repoItemStatus As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoItemStatus = New GridViewTextBoxColumn()
        repoItemStatus.FormatString = ""
        repoItemStatus.HeaderText = "Item Status"
        repoItemStatus.Name = colItemStatus
        repoItemStatus.ReadOnly = True
        repoItemStatus.Width = 80
        repoItemStatus.WrapText = True
        repoItemStatus.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gv1.MasterTemplate.Columns.Add(repoItemStatus)
            ...
            For Each dr As DataRow In dtAllData.Rows
                ...
                Else
                    If clsCommon.CompairString(strCode, clsCommon.myCstr(dr("Code"))) = CompairStringResult.Equal Then
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDUnApprovedQty).Value = clsCommon.myCDecimal(dr("UnapprovedQty"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDPendingQty).Value = clsCommon.myCDecimal(dr("PedningQty"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colItemStatus).Value = clsCommon.myCstr(dr("Item Status"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDTaxGroup).Value = clsCommon.myCstr(dr("Tax_Group"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDBinNo).Value = clsCommon.myCstr(dr("Bin_No"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDTaxGroupName).Value = clsCommon.myCstr(dr("TaxGroupName"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDTaxRate1).Value = clsCommon.myCDecimal(dr("TAX1_Rate"))
                        ...
                        If IsItemInsuranceColumn Then
                            ...
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colItemInsuranceAmt).Value = clsCommon.myCDecimal(dr("Item_Insurance_Amt"))
                        End If
                        If clsCommon.CompairString(gv1.Rows(gv1.Rows.Count - 1).Cells(colItemStatus).Value, "Close") = CompairStringResult.Equal Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colDSelect).ReadOnly = True
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colDSelect).Value = False
                        Else
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colDSelect).ReadOnly = False
                        End If
                    End If
                End If
