        ...
        Try
            ...
            For ii As Integer = 0 To gvDetail.Rows.Count - 1
                If clsCommon.myLen(gvDetail.Rows(ii).Cells("Doc_Code").Value) > 0 Then
                    clsMilkSRNMCC.Correction(clsCommon.myCstr(gvDetail.Rows(ii).Cells("Doc_Code").Value), False, True, False, 0, "", clsCommon.myCdbl(gvDetail.Rows(ii).Cells("NewFAT").Value), clsCommon.myCdbl(gvDetail.Rows(ii).Cells("NewSNF").Value), "", "")
                End If
                flag = True
            Next
            If flag Then
