            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    ElseIf e.Column Is gv1.Columns(colEveningSNFPerNoDecimal) Then
                        gv1.CurrentRow.Cells(colEveningSNFPer).Value = Xtra.MyNoDecimalToOneDecimal(gv1.CurrentRow.Cells(colEveningSNFPerNoDecimal).Value)
                    ...
                    ElseIf e.Column Is gv1.Columns(colMorningFATPerNoDecimal) Then
                        gv1.CurrentRow.Cells(colMorningFATPer).Value = Xtra.MyNoDecimalToOneDecimal(gv1.CurrentRow.Cells(colMorningFATPerNoDecimal).Value)
                        Dim CalSettMaxFATPerLimit As Decimal = GetCalSettMaxFATPerLimit()
                        If CalSettMaxFATPerLimit > 0 Then
                    ...
                    ElseIf e.Column Is gv1.Columns(colMorningSNFPerNoDecimal) Then
                        gv1.CurrentRow.Cells(colMorningSNFPer).Value = Xtra.MyNoDecimalToOneDecimal(gv1.CurrentRow.Cells(colMorningSNFPerNoDecimal).Value)
                        Dim CalsettMaxSNFPerLimit As Decimal = GetCalSettMaxSNFPerLimit()
                        If CalsettMaxSNFPerLimit > 0 AndAlso Not isPickCLRInsteadOfSNF Then
...
left outer join TSPL_MCC_MASTER on TSPL_MCC_MASTER.MCC_Code=TSPL_MILK_COLLECTION_MCC_DETAIL.MCC_Code
    ...
    Private Sub txtMCC__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtMCC._MYValidating
        Try
            Dim Baseqry As String = "select TSPL_MCC_MASTER.Mcc_Code_VLC_Uploader as UploaderNo,TSPL_MCC_MASTER.MCC_NAME, TSPL_MILK_COLLECTION_MCC_DETAIL.MCC_Code,TSPL_MILK_COLLECTION_MCC.Tanker_No,TSPL_MILK_COLLECTION_MCC_DETAIL.Milk_Type, TSPL_MILK_COLLECTION_MCC.Route_Code ,TSPL_BULK_ROUTE_MASTER.ROUTE_NAME
                                ...
                                For j As Integer = 0 To dtItem.Rows.Count - 1
                                    ...
                                    If SettMilkCollectionFATSNFType = 0 Then
                                        If SettFATSNFNoDecimalDCS = True Then
                                            objTr.FAT = Xtra.MyNoDecimalToOneDecimal(dtItem.Rows(j)("FAT"))
                                            objTr.SNF = Xtra.MyNoDecimalToOneDecimal(dtItem.Rows(j)(colSNForCLR))
                                            objTr.FATKG = Math.Round(clsCommon.myCDecimal(objTr.Qty) * Xtra.MyNoDecimalToOneDecimal(dtItem.Rows(j)("FAT")) / 100, 3, MidpointRounding.ToEven)
                                            objTr.SNFKG = Math.Round(clsCommon.myCDecimal(objTr.Qty) * Xtra.MyNoDecimalToOneDecimal(dtItem.Rows(j)(colSNForCLR)) / 100, 3, MidpointRounding.ToEven)
                                        Else
...
Left outer join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code = XXXMain.Comp_Code order by XXXMain.Document_No,TSPL_VLC_MASTER_HEAD.isOwnBMC"
    ...
    Public Function Print(ByVal arr As ArrayList, ByVal TranDate As DateTime)
        Try
            ...
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count >= 0 Then
                Dim arrMPKID As New List(Of Integer)
                Dim arrEPKID As New List(Of Integer)
                For ii As Integer = 0 To dt.Rows.Count - 1
                    If arrMPKID.Contains(clsCommon.myCDecimal(dt.Rows(ii)("MorningPK_Id"))) Then
                        dt.Rows(ii)("Milk_Type") = DBNull.Value
                        dt.Rows(ii)("Dock_Collection_Milk_Type") = DBNull.Value
                        dt.Rows(ii)("Morning_Qty") = DBNull.Value
                        dt.Rows(ii)("Morning_FAT") = DBNull.Value
                        dt.Rows(ii)("Morning_SNF") = DBNull.Value
                        dt.Rows(ii)("Morning_FATKG") = DBNull.Value
                        dt.Rows(ii)("Morning_SNFKG") = DBNull.Value
                    Else
                        arrMPKID.Add(clsCommon.myCDecimal(dt.Rows(ii)("MorningPK_Id")))
                    End If

                    If arrEPKID.Contains(clsCommon.myCDecimal(dt.Rows(ii)("Evening_PK_Id"))) Then
                        dt.Rows(ii)("Evening_Milk_Type") = DBNull.Value
                        dt.Rows(ii)("Evening_Dock_Collection_Milk_Type") = DBNull.Value
                        dt.Rows(ii)("Evening_Qty") = DBNull.Value
                        dt.Rows(ii)("Evening_FAT") = DBNull.Value
                        dt.Rows(ii)("Evening_SNF") = DBNull.Value
                        dt.Rows(ii)("Evening_FATKG") = DBNull.Value
                        dt.Rows(ii)("Evening_SNFKG") = DBNull.Value
                    Else
                        arrEPKID.Add(clsCommon.myCDecimal(dt.Rows(ii)("Evening_PK_Id")))
                    End If
                Next
                dt.AcceptChanges()
                arrMPKID = Nothing
...
where convert(date, TSPL_MILK_COLLECTION_DCS.Document_Date,103)='" + clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") + "' and TSPL_MILK_COLLECTION_MCC_DETAIL.MCC_Code='" + clsCommon.myCstr(txtMCC.Tag) + "'"
    ...
    Private Sub RadButton2_Click_1(sender As Object, e As EventArgs) Handles RadButton2.Click
        Try
            ...
            End If
            Dim qry As String = "select TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader as UploaderNo,TSPL_VLC_MASTER_HEAD.VLC_Code as Code,TSPL_VLC_MASTER_HEAD.VLC_Name as Name,TSPL_VLC_MASTER_HEAD.VSP_Code as DCSName,TSPL_VLC_MASTER_HEAD.MCC as BMC,TSPL_MCC_MASTER.MCC_NAME as BMCName,case when TSPL_VLC_MASTER_HEAD.isOwnBMC=1 then 'Y' else 'N' end as [Own BMC]
