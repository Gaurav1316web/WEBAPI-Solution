  ...
  " and TSPL_GL_ACCOUNTS.Account_Code in (select TSPL_CONTROL_ACC_MAPPING.Account_Code  from TSPL_CONTROL_ACC_MAPPING where IsForAP =1) and  TSPL_GL_ACCOUNTS.Account_Seg_Code7='" + txtlocation.Value + "' "
    ...
    Sub LoadData(ByVal strDocumentNo As String)
        Try
            ...
            Dim obj As New clsTransferToSaving()
            obj = clsTransferToSaving.GetData(strDocumentNo, Nothing)
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            Dim qry As String = "select Document_No from TSPL_TRANSFER_TO_SAVING where Document_No="
            Select Case NavType
                Case NavigatorType.First
                    qry += "(select MIN(TSPL_TRANSFER_TO_SAVING.Document_No) from TSPL_TRANSFER_TO_SAVING left outer join TSPL_MULTIPLE_DEDUCTION_DETAIL on TSPL_MULTIPLE_DEDUCTION_DETAIL.Document_No=TSPL_TRANSFER_TO_SAVING.Document_No and TSPL_MULTIPLE_DEDUCTION_DETAIL.Line_No=1 where 2=2  )"
                Case NavigatorType.Last
                    qry += "(select Max(TSPL_TRANSFER_TO_SAVING.Document_No) from TSPL_TRANSFER_TO_SAVING left outer join TSPL_MULTIPLE_DEDUCTION_DETAIL on TSPL_MULTIPLE_DEDUCTION_DETAIL.Document_No=TSPL_TRANSFER_TO_SAVING.Document_No and TSPL_MULTIPLE_DEDUCTION_DETAIL.Line_No=1 where 2=2 )"
                Case NavigatorType.Next
                    qry += "(select Min(TSPL_TRANSFER_TO_SAVING.Document_No) from TSPL_TRANSFER_TO_SAVING left outer join TSPL_MULTIPLE_DEDUCTION_DETAIL on TSPL_MULTIPLE_DEDUCTION_DETAIL.Document_No=TSPL_TRANSFER_TO_SAVING.Document_No and TSPL_MULTIPLE_DEDUCTION_DETAIL.Line_No=1 where TSPL_TRANSFER_TO_SAVING.Document_No>'" + txtDocNo.Value + "')"
                Case NavigatorType.Previous
                    qry += "(select Max(TSPL_TRANSFER_TO_SAVING.Document_No) from TSPL_TRANSFER_TO_SAVING left outer join TSPL_MULTIPLE_DEDUCTION_DETAIL on TSPL_MULTIPLE_DEDUCTION_DETAIL.Document_No=TSPL_TRANSFER_TO_SAVING.Document_No and TSPL_MULTIPLE_DEDUCTION_DETAIL.Line_No=1 where TSPL_TRANSFER_TO_SAVING.Document_No<'" + txtDocNo.Value + "' )"
            End Select
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        Dim qry As String = "select TSPL_TRANSFER_TO_SAVING.Document_No ,convert(varchar,TSPL_TRANSFER_TO_SAVING.Document_date,103) as Date,TSPL_TRANSFER_TO_SAVING.Loc_Code as [Location Code],TSPL_TRANSFER_TO_SAVING.MCC_Code as [MCC Code],TSPL_TRANSFER_TO_SAVING.MCC_Name as [MCC Name],case when TSPL_TRANSFER_TO_SAVING.IsPosted =1  then 'Approved' else 'Pending' end as Status   from TSPL_TRANSFER_TO_SAVING "
        LoadData(clsCommon.ShowSelectForm("PROCDeduction1", qry, "Document_No", "", txtDocNo.Value, "Document_No", isButtonClicked, ""))
