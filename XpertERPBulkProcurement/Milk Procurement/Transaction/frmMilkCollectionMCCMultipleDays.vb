...
Public Class frmMilkCollectionMCCMultipleDays
...
Left outer join TSPL_GAZE_READING on TSPL_GAZE_READING.Code=tspl_Silo_Detail.Gaze_Reading_Code "
    ...
    Public Function SaveData() As Boolean
        Try
            If (AllowToSave()) Then
                Dim obj As New clsMilkCollectionMCCMulipleDays()
    ...
    Function GetTRData(ByVal isMissingOnly As Boolean) As List(Of clsMilkCollectionMCCMulipleDaysDetail)
        Dim Arr As New List(Of clsMilkCollectionMCCMulipleDaysDetail)
        For ii As Integer = 0 To gv1.RowCount - 1
            If clsCommon.myLen(gv1.Rows(ii).Cells(colMCCCode).Value) > 0 Then
                If clsCommon.myCDecimal(gv1.Rows(ii).Cells(colQty).Value) > 0 Then
                    ...
                    End If
                    If flag Then
                        Dim objTr As New clsMilkCollectionMCCMulipleDaysDetail()
                        objTr.SNo = ii + 1
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            LoadBlankGrid()
            Dim obj As New clsMilkCollectionMCCMulipleDays()
            obj = clsMilkCollectionMCCMulipleDays.GetData(strCode, NavTyep, Nothing)
        ...
        Try
            ...
            Else
                If UsLock1.Status = ERPTransactionStatus.Approved Then
                    clsMilkCollectionMCCMulipleDaysDetail.DeleteData(clsCommon.myCdbl(gv1.CurrentRow.Cells(ColPKID).Value))
                End If
            End If
        Catch ex As Exception
            common.clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
...
case when TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Status=1 then 'Posted' else 'Pending' end as Status  from TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS left outer join TSPL_BULK_ROUTE_MASTER on TSPL_BULK_ROUTE_MASTER.ROUTE_NO= TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Route_Code"
    ...
    Sub DeleteData()
        Try
            ...
            If clsCommon.MyMessageBoxShow(Me, "Delete the current document" + Environment.NewLine + "Are you sure ? ", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                clsMilkCollectionMCCMulipleDays.DeleteData(txtDocNo.Value)
    ...
    Sub PostData()
        Try
            ...
            If clsCommon.MyMessageBoxShow(Me, "Post the Current Document [" + txtDocNo.Value + "]" + Environment.NewLine + "Are You Sure.", Me.Text, MessageBoxButtons.YesNo, WinControls.RadMessageIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                clsMilkCollectionMCCMulipleDays.PostData(txtDocNo.Value)
            ...
            Try
                For i As Integer = 0 To dtCust.Rows.Count - 1
                    ...
                    If dr IsNot Nothing AndAlso dr.Length > 0 Then
                        ...
                        obj.Description = ""
                        obj.Arr = New List(Of clsMilkCollectionMCCMulipleDaysDetail)
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Try
            ...
            End If
            Dim qry = " select TSPL_COMPANY_MASTER.Comp_Code , TSPL_COMPANY_MASTER.Comp_Name , TSPL_COMPANY_MASTER.Add1 , TSPL_COMPANY_MASTER.Add2 , TSPL_COMPANY_MASTER.Add3 ,TSPL_COMPANY_MASTER.City_Code, TSPL_COMPANY_MASTER.State ,TSPL_COMPANY_MASTER.Pan_No ,TSPL_COMPANY_MASTER.GSTReg_No,TSPL_COMPANY_MASTER.GSTINNo, TSPL_COMPANY_MASTER.CINNo ,TSPL_COMPANY_MASTER.Phone1 , TSPL_COMPANY_MASTER.Phone2,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,TSPL_COMPANY_MASTER.Pan_No  ,TSPL_COMPANY_MASTER.Email ,TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Document_No,convert (varchar, TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Document_Date,103) as Document_Date, TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Route_Code,TSPL_BULK_ROUTE_MASTER.ROUTE_NAME , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Tanker_No, TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Vehicle_No , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Entered_Qty , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Entered_FATKg, TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Entered_SNFKg , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Status , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.SNo , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.MCC_Code , TSPL_MCC_MASTER.MCC_NAME,TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.Milk_Type, TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.Qty , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.FAT , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.SNF , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.FATKG , TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.SNFKG
                        ...
                        left outer join TSPL_USER_MASTER on TSPL_USER_MASTER.User_Code = TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Created_By
                        ...
                        left outer join TSPL_MCC_MASTER on TSPL_MCC_MASTER.MCC_Code = TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL.MCC_Code
                         where TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS.Document_No = '" + txtDocNo.Value + "'
                        "
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            ...
            End If
            clsERPFuncationalityOLD.ShowTransHistoryData(txtDocNo.Value, "Document_No", "TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS", "TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS_DETAIL")
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    End Sub
    ...
    Private Sub LoadTransactionData()
        Try
            If clsCommon.myLen(txtRoute.Value) > 0 Or clsCommon.myLen(txtTankerNo.Value) > 0 Then
                Dim strQry1 As String = ""
                If clsCommon.myLen(txtTankerNo.Value) > 0 Then
                    strQry1 += " And Tanker_No='" + txtTankerNo.Value + "'"
                End If
                Dim strDocNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Document_No from TSPL_MILK_COLLECTION_MCC_MULTIPLE_DAYS where convert(date, Document_Date,103)='" + clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") + "' and Route_Code='" + txtRoute.Value + "'" + strQry1))
    ...
    Private Sub btnAddMissing_Click(sender As Object, e As EventArgs) Handles btnAddMissing.Click
        Try
            Dim Arr As List(Of clsMilkCollectionMCCMulipleDaysDetail) = GetTRData(True)
            ...
            Else
                If clsCommon.MyMessageBoxShow(Me, "This Feature will add only Missing Simples" + Environment.NewLine + "Add [" + clsCommon.myCstr(Arr.Count) + "] Missing Samples" + Environment.NewLine + "Are You sure", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.Yes Then
                    clsMilkCollectionMCCMulipleDaysDetail.AddMissing(txtDocNo.Value, Arr)
...
where TSPL_BULK_ROUTE_MASTER_MCC.ROUTE_NO not in ('" + txtRoute.Value + "')"
    ...
    Private Sub btnReverse_Click(sender As Object, e As EventArgs) Handles btnReverse.Click
        Try
            ...
            'If clsMilkCollectionMCCMulipleDays.ReverseAndUnpost(False) Then
            ...
            If clsCommon.MyMessageBoxShow("Do you want to Reverse and unpost the current Document" + Environment.NewLine + "Are you sure?", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                ...
                clsMilkCollectionMCCMulipleDays.ReverseAndUnpost(txtDocNo.Value)
