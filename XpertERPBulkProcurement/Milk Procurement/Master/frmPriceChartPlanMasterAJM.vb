...
Public Class frmPriceChartPlanMasterAJM
...
#End Region
    ...
    Private Sub FrmPriceChartMaster_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        UcAttachment1.MandatoryPDFFile = (clsCommon.myCDecimal(clsFixedParameter.GetData(clsFixedParameterType.MandatoryPDFFileMilkPricePlan, clsFixedParameterCode.MandatoryPDFFileMilkPricePlan, Nothing)) = 1)
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            For ii As Integer = 0 To gvSNF.RowCount - 1
                If clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colRatePer).Value) > 0 AndAlso clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colFixedRate).Value) > 0 Then
                    If clsCommon.MyMessageBoxShow(Me, "FAT rate / fixed rate is zero at row no [ " + clsCommon.myCstr(ii + 1) + " ] " + Environment.NewLine + "Do you want to continue", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.No Then
                        Return False
                    End If
                End If
            Next
        ...
        Try
            If AllowToSave() Then
                ...
                For ii As Integer = 0 To gvSNF.RowCount - 1
                    ...
                    objTSDDCF.SNF_From = clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colFrom).Value)
                    objTSDDCF.SNF_To = clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colTo).Value)
                    objTSDDCF.Rate_Per = clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colRatePer).Value)
                    objTSDDCF.Fixed_Rate = clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colFixedRate).Value)
                    objTSDDCF.arrSNF = TryCast(gvSNF.Rows(ii).Cells(colSNo).Tag, Dictionary(Of Decimal, Decimal))
                    If objTSDDCF.SNF_From > 0 AndAlso objTSDDCF.SNF_To > 0 Then
                ...
                If obj.arrTSDDCS IsNot Nothing AndAlso obj.arrTSDDCS.Count > 0 Then
                    For Each objTSDDCS As clsPriceChartPlanningTSDDCF In obj.arrTSDDCS
                        If objTSDDCS.SNF_From > 0 AndAlso objTSDDCS.SNF_To > 0 Then
                            ...
                            gvSNF.Rows(gvSNF.Rows.Count - 1).Cells(colFixedRate).Value = objTSDDCS.Fixed_Rate
    ...
    Private Function CalculateRate(ByVal dblFATPer As Decimal, ByVal dblSNFPer As Decimal) As Double
        ...
        Else
            For ii As Integer = 0 To gvSNF.Rows.Count - 1
                If clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colRatePer).Value) > 0 Then
                    If dblSNFPer >= clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colFrom).Value) AndAlso dblSNFPer <= clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colTo).Value) Then
                        If clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colFixedRate).Value) > 0 Then
                            dclRate = clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colFixedRate).Value)
                        Else
                            dclRate = (dblFATPer * clsCommon.myCDecimal(gvSNF.Rows(ii).Cells(colRatePer).Value)) / 100
                        End If
                        Dim arrSNFDed As Dictionary(Of Decimal, Decimal) = TryCast(gvSNF.Rows(ii).Cells(colSNo).Tag, Dictionary(Of Decimal, Decimal))
                        If arrSNFDed IsNot Nothing AndAlso arrSNFDed.Count > 0 Then
                            If arrSNFDed.ContainsKey(dblSNFPer) Then
                                dclRate += arrSNFDed.Item(dblSNFPer)
                            End If
                        End If
                        Exit For
                    End If
    ...
    Private Sub gvFATDed_CommandCellClick(sender As Object, e As EventArgs) Handles gvSNF.CommandCellClick
        Try
            If gvSNF.CurrentColumn Is gvSNF.Columns(colSNFAddDed) Then
                Dim frm As New frmPriceChartPlanMasterTSDDCFDeduction()
                frm.ArrDed = gvSNF.CurrentRow.Cells(colSNo).Tag
                frm.SNFFrom = gvSNF.CurrentRow.Cells(colFrom).Value
                frm.SNFTo = gvSNF.CurrentRow.Cells(colTo).Value
                frm.ShowDialog()
                If frm.isOK Then
                    gvSNF.CurrentRow.Cells(colSNo).Tag = frm.ArrDed
                End If
            End If
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
    ...
    Private Sub gvSNF_CurrentColumnChanged(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.CurrentColumnChangedEventArgs) Handles gvSNF.CurrentColumnChanged
