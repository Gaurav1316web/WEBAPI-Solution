...
where TSPL_PAYMENT_PROCESS_MCC_SALE.Doc_No In (" & Document & ")
            ...
            If dtDesc.Rows.Count > 0 Then
                ...
                Next
            End If
            Dim sQuery As String = " Select VSP_CODE,max(DCSCode)DCSCode,max(VSP_NAME)VSP_NAME,max(Registered_PDCS_CLUSTER)Registered_PDCS_CLUSTER,max(Gender)Gender,sum(Milk_Qty)Milk_Qty,
                                    sum(Milk_Amount)Milk_Amount,sum(Head_Load_Amount)Head_Load_Amount,sum(Deduction_Amount)Deduction_Amount,
                                    sum(Credit_Note_Amount)Credit_Note_Amount," & DescName3 & " ,Sum(SweetQty)SweetQty,Sum(SourQty)SourQty,sum(CurdQty)CurdQty,sum(Payable_Amount)Payable_Amount from 
                                   (Select *,0 as SweetQty,0 as SourQty,0 as CurdQty 
                                   from (Select max(Registered_PDCS_CLUSTER)Registered_PDCS_CLUSTER,max(Gender)Gender,VSP_CODE,max(DCSCode)DCSCode,max(VSP_NAME)VSP_NAME,sum(Milk_Qty)Milk_Qty,sum(Milk_Amount)Milk_Amount,
                                   ...
                                   from (Select max(yy.Registered_PDCS_CLUSTER)Registered_PDCS_CLUSTER,max(yy.Gender)Gender,yy.VSP_CODE,max(yy.VLC_CODE_Uploader)DCSCode,max(yy.VSP_NAME)VSP_NAME,SUM(yy.Milk_Qty)Milk_Qty,sum(yy.Milk_Amount)Milk_Amount,
                                   ...
                                   from (Select max(Registered_PDCS_CLUSTER)Registered_PDCS_CLUSTER,max(Gender)Gender,max(VLC_CODE_Uploader)VLC_CODE_Uploader,VSP_CODE,max(VSP_NAME)VSP_NAME,sum(Milk_Qty)Milk_Qty,sum(Milk_Amount)Milk_Amount,
                                   sum(Head_Load_Amount)Head_Load_Amount,sum(Payable_Amount)Payable_Amount,sum(Deduction_Amount)Deduction_Amount,sum(Credit_Note_Amount)Credit_Note_Amount,
                                   'A' as DCS_Addition_Deduction,'A' as DCSDescription,0 as Amount 
                                   from (Select TSPL_VLC_MASTER_HEAD.Registered_PDCS_CLUSTER,TSPL_VENDOR_MASTER.Gender,VLC_CODE_Uploader,TSPL_PAYMENT_PROCESS_DETAIL.VSP_CODE,VSP_NAME,Milk_Qty,Milk_Amount,Head_Load_Amount,Payable_Amount,Deduction_Amount,
                                   Credit_Note_Amount from TSPL_PAYMENT_PROCESS_DETAIL
                                   ...
                                   where 2=2 and TSPL_PAYMENT_PROCESS_HEAD.Doc_No IN(" & Document & ") "
            If txtDCS.arrValueMember IsNot Nothing Then
                sQuery += " and TSPL_PAYMENT_PROCESS_DETAIL.VSP_CODE In (" + clsCommon.GetMulcallString(txtDCS.arrValueMember) + ") "
            End If
            sQuery += "
                        ) xx group by xx.VSP_CODE
                        union all
                        Select '' as Registered_PDCS_CLUSTER,'' as Gender,max(VLC_Code_VLC_Uploader) as VLC_CODE_Uploader,Vendor_CODE as VSP_CODE,max(Vendor_Name)VSP_NAME,0 as Milk_Qty,0 as Milk_Amount,0 as Head_Load_Amount,0 as Payable_Amount ,
                        0 as Deduction_Amount,0 as Credit_Note_Amount,DCS_Addition_Deduction,max(DCSDescription)DCSDescription,sum(Amount)Amount 
                        from (
				                    select TSPL_PAYMENT_PROCESS_CREDIT_NOTE.Doc_No,TSPL_PAYMENT_PROCESS_CREDIT_NOTE.Vendor_CODE,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader,TSPL_VENDOR_INVOICE_DETAIL.DCS_Addition_Deduction,TSPL_DCS_ADDITION_DEDUCTION.Description as DCSDescription,TSPL_PAYMENT_PROCESS_CREDIT_NOTE.Amount,TSPL_VENDOR_INVOICE_DETAIL.Amount as VendorAmt,TSPL_VENDOR_INVOICE_HEAD.Document_No as InvoiceNo,TSPL_VENDOR_INVOICE_HEAD.Main_VSP_Milk_AP_Invoice_No,TSPL_VENDOR_INVOICE_HEAD.Vendor_Name 
									from TSPL_PAYMENT_PROCESS_CREDIT_NOTE
									...
									left outer join TSPL_VLC_MASTER_HEAD on TSPL_VLC_MASTER_HEAD.VSP_Code=TSPL_VENDOR_INVOICE_HEAD.Vendor_Code
									 where 2=2 and 
									 TSPL_PAYMENT_PROCESS_HEAD.Doc_No IN(" & Document & ")"
            If txtDCS.arrValueMember IsNot Nothing Then
                sQuery += " and TSPL_PAYMENT_PROCESS_CREDIT_NOTE.Vendor_CODE In (" + clsCommon.GetMulcallString(txtDCS.arrValueMember) + ") "
            End If
            sQuery += "   Union all								   
									SELECT TSPL_PAYMENT_PROCESS_DEDUCTION.Doc_No,TSPL_PAYMENT_PROCESS_DEDUCTION.Vendor_CODE,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader,TSPL_VENDOR_INVOICE_DETAIL.DCS_Addition_Deduction,TSPL_DCS_ADDITION_DEDUCTION.Description as DCSDescription,TSPL_PAYMENT_PROCESS_DEDUCTION.Amount,TSPL_VENDOR_INVOICE_DETAIL.Amount as VendorAmt,TSPL_VENDOR_INVOICE_HEAD.Document_No as InvoiceNo,TSPL_VENDOR_INVOICE_HEAD.Main_VSP_Milk_AP_Invoice_No ,TSPL_VENDOR_INVOICE_HEAD.Vendor_Name
									FROM TSPL_PAYMENT_PROCESS_DEDUCTION
									left outer join TSPL_PAYMENT_PROCESS_HEAD on TSPL_PAYMENT_PROCESS_HEAD.Doc_No = TSPL_PAYMENT_PROCESS_DEDUCTION.Doc_No
									...
									where 2=2 and TSPL_PAYMENT_PROCESS_HEAD.Doc_No IN(" & Document & ") "
            If txtDCS.arrValueMember IsNot Nothing Then
                sQuery += " and TSPL_PAYMENT_PROCESS_DEDUCTION.Vendor_CODE In (" + clsCommon.GetMulcallString(txtDCS.arrValueMember) + ") "
            End If

            sQuery += "	union all
									SELECT TSPL_PAYMENT_PROCESS_SAVING.Doc_No,TSPL_VENDOR_INVOICE_HEAD.Vendor_CODE,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader,TSPL_VENDOR_INVOICE_DETAIL.DCS_Addition_Deduction,TSPL_DCS_ADDITION_DEDUCTION.Description as DCSDescription,TSPL_VENDOR_INVOICE_DETAIL.Amount as Amount,0 AS VendorAmt,TSPL_VENDOR_INVOICE_HEAD.Document_No as InvoiceNo,TSPL_VENDOR_INVOICE_HEAD.Main_VSP_Milk_AP_Invoice_No ,TSPL_VENDOR_INVOICE_HEAD.Vendor_Name
									FROM TSPL_PAYMENT_PROCESS_SAVING
									...
									where 2=2 and TSPL_PAYMENT_PROCESS_HEAD.Doc_No IN(" & Document & ") "
            If txtDCS.arrValueMember IsNot Nothing Then
                sQuery += " and TSPL_VENDOR_INVOICE_HEAD.Vendor_CODE In (" + clsCommon.GetMulcallString(txtDCS.arrValueMember) + ") "
            End If

            sQuery += " union all
									SELECT TSPL_PAYMENT_PROCESS_COMPULSORY.Doc_No,TSPL_VENDOR_INVOICE_HEAD.Vendor_CODE,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader,TSPL_VENDOR_INVOICE_DETAIL.DCS_Addition_Deduction,TSPL_DCS_ADDITION_DEDUCTION.Description as DCSDescription,TSPL_VENDOR_INVOICE_DETAIL.Amount as Amount,0 AS VendorAmt,TSPL_VENDOR_INVOICE_HEAD.Document_No as InvoiceNo,TSPL_VENDOR_INVOICE_HEAD.Main_VSP_Milk_AP_Invoice_No ,TSPL_VENDOR_INVOICE_HEAD.Vendor_Name
									FROM TSPL_PAYMENT_PROCESS_COMPULSORY
            ...
            sQuery += " union all
									SELECT TSPL_PAYMENT_PROCESS_MCC_SALE.Doc_No,TSPL_VENDOR_INVOICE_HEAD.Vendor_CODE,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader,TSPL_VENDOR_INVOICE_DETAIL.DCS_Addition_Deduction,TSPL_DCS_ADDITION_DEDUCTION.Description as DCSDescription,TSPL_VENDOR_INVOICE_DETAIL.Amount as Amount,0 AS VendorAmt,TSPL_VENDOR_INVOICE_HEAD.Document_No as InvoiceNo,TSPL_VENDOR_INVOICE_HEAD.Main_VSP_Milk_AP_Invoice_No ,TSPL_VENDOR_INVOICE_HEAD.Vendor_Name
									FROM TSPL_PAYMENT_PROCESS_MCC_SALE
            ...
            sQuery += " )xx group by xx.Doc_No,xx.Vendor_CODE,xx.DCS_Addition_Deduction )YY group by VSP_CODE,DCS_Addition_Deduction)
									Tab1
									PIVOT(SUM(Amount) FOR DCS_Addition_Deduction IN (" & Description & ")) AS Tab2 )tmp
									group by VSP_CODE )YY
