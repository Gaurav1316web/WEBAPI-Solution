...
Public Class rptCollectionDataChangeReport
    ...
    Sub Print(ByVal IsPrint As Exporter)
        Try
            ...
            '    ", Case When Coalesce(TSPL_MILK_SAMPLE_DETAIL.FAT, 0) <= 0 Then '' When Coalesce(TSPL_MILK_SAMPLE_DETAIL.FAT, 0) <= 5 Then 'C' Else 'B' End As [Milk Type], TSPL_MILK_SRN_HEAD.DOC_CODE As [Milk Receipt Code], TSPL_MILK_SRN_HEAD.MCC_CODE As MCC " & _
            '    " , TSPL_MCC_MASTER.MCC_NAME As [MCC Name] " & _
            '    " ,  Convert(varchar,TSPL_MILK_SRN_HEAD.DOC_DATE,103) As [Doc Date], Case When TSPL_MILK_RECEIPT_DETAIL.SHIFT = 'M' Then 'Morning' Else 'Evening' End As Shift " & _
            ...
            '    " Left Outer Join TSPL_MILK_RECEIPT_HEAD On TSPL_MILK_SRN_HEAD.DOC_CODE = TSPL_MILK_RECEIPT_DETAIL.DOC_CODE  " & _
            '    " Left Outer Join TSPL_MILK_SAMPLE_HEAD On TSPL_MILK_SAMPLE_HEAD.MILK_RECEIPT_CODE = TSPL_MILK_SRN_HEAD.DOC_CODE " & _
            ...
            '    " Left Outer Join TSPL_MILK_Shift_End_HEAD On TSPL_MILK_Shift_End_HEAD.MCC_CODE = TSPL_MILK_SRN_HEAD.MCC_CODE  " & _
            '    " And convert(date,TSPL_MILK_Shift_End_HEAD.DOC_DATE,103) = convert(date,TSPL_MILK_SRN_HEAD.DOC_DATE,103)  " & _
            ...
            '    ", TSPL_MILK_SRN_HEAD.DOC_CODE As [Milk Receipt Code], TSPL_MILK_SRN_HEAD.MCC_CODE As MCC, TSPL_MCC_MASTER.MCC_NAME As [MCC Name] " & _
            '     ",  Convert(varchar,TSPL_MILK_SRN_HEAD.DOC_DATE,103) As [Doc Date], Case When TSPL_MILK_RECEIPT_DETAIL.SHIFT = 'M' Then 'Morning' Else 'Evening' End As Shift " & _
            '    ",TSPL_VLC_MASTER_HEAD.VLC_Code As [Vlc Code], TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader As [Vlc Uploader Code], TSPL_VLC_MASTER_HEAD.VLC_Name As [VLC Name], TSPL_MILK_RECEIPT_DETAIL.SAMPLE_NO As [Sample No],  TSPL_MILK_RECEIPT_DETAIL.NO_OF_CANS As [No Of Cans], TSPL_MILK_RECEIPT_DETAIL.MILK_WEIGHT As [Milk Weight], TSPL_MILK_SAMPLE_DETAIL.FAT As [FAT(%)], TSPL_MILK_SAMPLE_DETAIL.SNF As [SNF(%)], TSPL_MILK_SAMPLE_DETAIL.CLR,  Convert(decimal(18,3), TSPL_MILK_SAMPLE_DETAIL.FAT * TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT / 100) As [FAT(KG)], Convert(decimal(18,3),TSPL_MILK_SAMPLE_DETAIL.SNF * TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT / 100) As [SNF(KG)] " & _
            ...
            Dim qry As String = "select HistData.[Milk Receipt Code],HistData.[MCC Code] ,HistData.[MCC Name] ,HistData.[Doc Date] ,HistData.Shift" &
               ...
               ", Case When Coalesce(TSPL_MILK_SRN_DETAIL_hist_data.FAT_PER, 0) <= 0 Then '' When Coalesce(TSPL_MILK_SRN_DETAIL_hist_data.FAT_PER, 0) <= 5 Then 'C' Else 'B' End As [Milk Type], TSPL_MILK_SRN_HEAD.DOC_CODE As [Milk Receipt Code], TSPL_MILK_SRN_HEAD.MCC_CODE As MCC " &
               ...
               ",TSPL_VLC_MASTER_HEAD.VLC_Code As [Vlc Code], TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader As [Vlc Uploader Code], TSPL_VLC_MASTER_HEAD.VLC_Name As [VLC Name], TSPL_MILK_SRN_HEAD.SAMPLE_NO As [Sample No],  TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL.NO_OF_CANS As [No Of Cans], TSPL_MILK_SRN_DETAIL_hist_data.ACC_Qty As [Milk Weight], TSPL_MILK_SRN_DETAIL_hist_data.FAT_PER As [FAT(%)], TSPL_MILK_SRN_DETAIL_hist_data.SNF_PER As [SNF(%)], TSPL_MILK_SRN_DETAIL_hist_data.CLR as CLR,  Convert(decimal(18,3), TSPL_MILK_SRN_DETAIL_hist_data.FAT_PER * TSPL_MILK_SRN_DETAIL_hist_data.ACC_Qty / 100) As [FAT(KG)], Convert(decimal(18,3),TSPL_MILK_SRN_DETAIL_hist_data.SNF_PER * TSPL_MILK_SRN_DETAIL_hist_data.ACC_Qty / 100) As [SNF(KG)] " &
               ...
               " left outer join (select code,max(Price_code) as Price_code from  TSPL_FAT_SNF_UPLOADER_MASTER group by code) as TabTSPL_FAT_SNF_UPLOADER_MASTER on TabTSPL_FAT_SNF_UPLOADER_MASTER.code=TSPL_MILK_SRN_DETAIL_hist_data.Price_Code  " &
            ...
            qry += " ) As final where 2=2  " &
                ...
                ", TSPL_MILK_SRN_HEAD.DOC_CODE As [Milk Receipt Code], TSPL_MILK_SRN_HEAD.MCC_CODE As MCC, TSPL_MCC_MASTER.MCC_NAME As [MCC Name] " &
                 ",  Convert(varchar,TSPL_MILK_SRN_HEAD.DOC_DATE,103) As [Doc Date], Case When TSPL_MILK_SRN_HEAD.SHIFT = 'M' Then 'Morning' Else 'Evening' End As Shift " &
            ...
            If rbtnMilkProcUpl.Checked Then
                ...
                Else
                   ...
                   convert(date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_On,103) AS Org_Weighment_Date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.FAT AS Org_FAT,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.SNF AS Org_SNF, case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,0)=0 then 1 else  TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Manual_Sample end  AS Org_Manual_Sample ,
                   TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_By AS Org_Sample_User,convert(date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_On,103) AS Org_Sample_Date,  CONVERT(VARCHAR, TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Document_Date, 103) AS Org_Document_Date FROM FirstMilkWeight 
                   ...
                   TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Milk_Weight AS First_Milk_Weight,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_By AS Org_Sample_User, convert(date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_On,103) AS Org_Sample_Date, case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,0)=0 then 1 else  TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Manual_Sample end  AS Org_Manual_Sample FROM FirstNonNullFatSnf left JOIN TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data 
                   ON FirstNonNullFatSnf.Document_No = TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Document_No AND FirstNonNullFatSnf.SNo = TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.SNo  AND FirstNonNullFatSnf.FirstNonNullFatSnfVersion = TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_Version LEFT  JOIN TSPL_MILK_PROCUREMENT_UPLOADER_HEAD ON TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Document_No = TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Document_No ), MaxVersionData AS (
                   SELECT MaxVersion.Document_No,TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.MCC_Code, MaxVersionData.Shift,MaxVersionData.VLC_Code ,MaxVersionData.Hist_By as Upd_VLC_User,MaxVersion.SNo,MaxVersionData.Bulk_Route_Code AS Upd_Route_No,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader AS Upd_VLC_Code_VLC_Uploader, TSPL_VLC_MASTER_HEAD.VLC_Name AS Upd_VLC_Name,MaxVersionData.Milk_Weight AS Upd_Milk_Weight,MaxVersionData.FAT AS Upd_FAT
                   ,MaxVersionData.SNF AS Upd_SNF,   case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,0)=0 then 1 else MaxVersionData.Manual_Weight end AS Upd_Manual_Weight,MaxVersionData.Hist_By AS Upd_Weighment_User,MaxVersionData.Hist_On AS Upd_Weighment_Date,case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,0)=0 then 1 else MaxVersionData.Manual_Sample end AS Upd_Manual_Sample,MaxVersionData.Hist_By AS Upd_Sample_User,MaxVersionData.Hist_On AS Upd_Sample_Date,TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,
                  ...
                  LEFT JOIN TSPL_MILK_PROCUREMENT_UPLOADER_HEAD ON TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Document_No = MaxVersionData.Document_No      LEFT JOIN TSPL_VLC_MASTER_HEAD  ON TSPL_VLC_MASTER_HEAD.VLC_Code = MaxVersionData.VLC_Code )  SELECT FirstMilkWeightData.Org_Route_No,FirstMilkWeightData.Org_VLC_Code_VLC_Uploader,FirstMilkWeightData.Org_VLC_Name ,FirstMilkWeightData.Org_VLC_User, FirstMilkWeightData.Org_Document_Date,
                  FirstMilkWeightData.Org_Document_No,FirstMilkWeightData.SNo as Org_SNo,ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) AS Org_Milk_Weight, FirstMilkWeightData.Org_Manual_Weight,FirstMilkWeightData.Org_Weighment_User,convert(date,FirstMilkWeightData.Org_Weighment_Date,103) Org_Weighment_Date,ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) AS Org_FAT,
                  ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) AS Org_SNF,FirstNonNullFatSnfData.Org_Manual_Sample as Org_Manual_Sample,FirstNonNullFatSnfData.Org_Sample_User,    convert(date,FirstNonNullFatSnfData.Org_Sample_Date,103)Org_Sample_Date,CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader
                  or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight
                  or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample 	THEN MaxVersionData.Upd_Route_No ELSE NULL END AS Upd_Route_No,
                  CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    ISNULL(FirstNonNullFatSnfData.First_Milk_Weight
                  ...
                  or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample THEN MaxVersionData.Upd_VLC_Code_VLC_Uploader ELSE null END AS Upd_VLC_Code_VLC_Uploader, CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or  
                  ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample	 THEN MaxVersionData.Upd_VLC_Name ELSE NULL END AS Upd_VLC_Name, 
                   CASE when ( FirstMilkWeightData.Org_VLC_Code_VLC_Uploader = MaxVersionData.Upd_VLC_Code_VLC_Uploader ) and ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <>  MaxVersionData.Upd_Milk_Weight or
				  FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No or    FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT
                  or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample  THEN FirstMilkWeightData.Org_VLC_User ELSE (case when FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <>  MaxVersionData.Upd_VLC_Code_VLC_Uploader then MaxVersionData.Upd_VLC_User else null end ) 
				  END AS Upd_VLC_User,CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or  
                  ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or	ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample	 THEN MaxVersionData.Upd_Document_Date ELSE NULL END AS Upd_Document_Date, CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or   FirstMilkWeightData.Org_Document_Date <> MaxVersionData.Upd_Document_Date 
                  or FirstMilkWeightData.Org_Document_No <> MaxVersionData.Upd_Document_No  or ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample	 THEN MaxVersionData.Upd_Document_No ELSE NULL END AS Upd_Document_No,CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or   FirstMilkWeightData.Org_Document_Date <> MaxVersionData.Upd_Document_Date 
                  or FirstMilkWeightData.Org_Document_No <> MaxVersionData.Upd_Document_No  or ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample	 THEN MaxVersionData.SNo else null end 
				  as Upd_SNo,CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or  
                  ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample	 THEN MaxVersionData.Upd_Milk_Weight ELSE NULL END AS Upd_Milk_Weight,CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader
                  or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample	 THEN MaxVersionData.Upd_Manual_Weight ELSE NULL END AS Upd_Manual_Weight, CASE when (ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) = MaxVersionData.Upd_Milk_Weight) and 
				  FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT
                  or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample  THEN FirstMilkWeightData.Org_Weighment_User ELSE (case when ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight then MaxVersionData.Upd_Weighment_User else null end ) 
				  END AS Upd_Weighment_User,CASE when (ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) = MaxVersionData.Upd_Milk_Weight) and  FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT
                  or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample THEN convert(date,FirstMilkWeightData.Org_Weighment_Date,103) ELSE (case when ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight then MaxVersionData.Upd_Weighment_Date else null end )  END AS Upd_Weighment_Date, CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or   ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or
                  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample THEN MaxVersionData.Upd_FAT ELSE NULL END AS Upd_FAT, CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight
                  or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample THEN MaxVersionData.Upd_SNF ELSE NULL END AS Upd_SNF, CASE WHEN FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or   ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight
                  or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample THEN MaxVersionData.Upd_Manual_Sample ELSE NULL END AS Upd_Manual_Sample, CASE when ( ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) = MaxVersionData.Upd_FAT and ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) = MaxVersionData.Upd_SNF ) and ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <> MaxVersionData.Upd_Milk_Weight or  FirstMilkWeightData.Org_Route_No <> MaxVersionData.Upd_Route_No Or FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader or FirstMilkWeightData.Org_VLC_Name <> MaxVersionData.Upd_VLC_Name or    FirstMilkWeightData.Org_Manual_Weight <> MaxVersionData.Upd_Manual_Weight or 
 FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample  THEN FirstNonNullFatSnfData.Org_Sample_User ELSE (case when ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF then MaxVersionData.Upd_Sample_User else null end ) END AS Upd_Sample_User, CASE when (ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) = MaxVersionData.Upd_FAT and ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) = MaxVersionData.Upd_SNF)  and ISNULL(FirstNonNullFatSnfData.First_Milk_Weight, FirstMilkWeightData.Org_Milk_Weight) <>  MaxVersionData.Upd_Milk_Weight or
    ...
    Sub SetGridFormationOFGV1()
        ...
        If rbtnMilkProcUpl.Checked Then
            ...
            gv1.Columns("Org_VLC_User").HeaderText = "DCS User"
            gv1.Columns("Upd_VLC_User").HeaderText = "DCS User"
            ...
            gv1.Columns("Org_Weighment_Date").FormatString = "{0:d}"
            gv1.Columns("Org_Weighment_Date").HeaderText = "Weighment Date"
            gv1.Columns("Upd_Weighment_Date").FormatString = "{0:d}"
            ...
            gv1.Columns("Org_Sample_Date").FormatString = "{0:d}"
            gv1.Columns("Org_Sample_Date").HeaderText = "Sample Date"

            gv1.Columns("Upd_Sample_Date").FormatString = "{0:d}"
    ...
    Sub View()
        If gv1.Rows.Count > 0 Then
            ...
            If rbtnMilkProcUpl.Checked Then
                ...
                For ii As Integer = 16 To gv1.Columns.Count - 1
            ...
            If rbtnMilkProcUpl.Checked Then
                If gv1.Rows.Count > 0 Then
                    ...
                    For i As Integer = 0 To gv1.Rows.Count - 1
                        Dim upd As Integer = 16
                        For org As Integer = 0 To 15
    ...
    Function LoadDataFromRetestingTable() As String
        ...
        FORMAT(Document_Date, 'dd/MM/yyyy hh:mm:ss tt') AS Upd_Document_Date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_RETESTING.Document_No as Upd_Document_No,SNo as Upd_SNo,Milk_Weight AS Upd_Milk_Weight,case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,0)=0 then 1 else Manual_Weight end AS Upd_Manual_Weight,Hist_By AS Upd_Weighment_User,FORMAT(Hist_On, 'dd/MM/yyyy hh:mm:ss tt')  AS Upd_Weighment_Date,FAT AS Upd_FAT,SNF AS Upd_SNF,
		case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,0)=0 then 1 else Manual_Sample end AS Upd_Manual_Sample,Hist_By AS Upd_Sample_User,FORMAT(Hist_On, 'dd/MM/yyyy hh:mm:ss tt') AS Upd_Sample_Date,ROW_NUMBER() OVER (PARTITION BY TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_RETESTING.Document_No, SNo ORDER BY Hist_On DESC ) AS RowNum FROM TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_RETESTING Left join TSPL_MILK_PROCUREMENT_UPLOADER_HEAD on TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Document_No= TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_RETESTING.Document_No
    ...
    Private Sub txtVLC__My_Click(sender As Object, e As EventArgs) Handles txtVLC._My_Click
        Try
            ...
            txtVLC.arrValueMember = clsCommon.ShowMultipleSelectForm("PCUVLC", qry, "Dcs Code", "Dcs Name", txtVLC.arrValueMember, txtVLC.arrDispalyMember)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    End Sub
