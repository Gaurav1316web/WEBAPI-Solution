        ...
        Try
            ...
            If rbtnMilkProcUpl.Checked Then
                ...
                qry = " WITH FirstMilkWeight AS ( SELECT  Document_No as Org_Document_No, SNo, MIN(Hist_Version) AS FirstMilkHistVersion FROM  TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data   WHERE Milk_Weight IS NOT NULL GROUP BY  Document_No, SNo ),
                   ...
                   vh.VLC_Code_VLC_Uploader AS Org_VLC_Code_VLC_Uploader,vh.VLC_Name AS Org_VLC_Name, TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_By as Org_VLC_User,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Milk_Weight AS Org_Milk_Weight, case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,'')=0 then 1 else TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Manual_Weight end AS Org_Manual_Weight,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_By AS Org_Weighment_User,
                   convert(date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Hist_On,103) AS Org_Weighment_Date,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.FAT AS Org_FAT,TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.SNF AS Org_SNF, case when isnull(TSPL_MILK_PROCUREMENT_UPLOADER_HEAD.Source_API,'')=0 then 1 else  TSPL_MILK_PROCUREMENT_UPLOADER_DETAIL_Hist_Data.Manual_Sample end  AS Org_Manual_Sample ,
                  ...
                  CASE WHEN  FirstMilkWeightData.Org_VLC_Code_VLC_Uploader <> MaxVersionData.Upd_VLC_Code_VLC_Uploader THEN MaxVersionData.Upd_VLC_User ELSE (case when FirstMilkWeightData.Org_VLC_Code_VLC_Uploader = MaxVersionData.Upd_VLC_Code_VLC_Uploader   then FirstMilkWeightData.Org_VLC_User else null end )  END AS Upd_VLC_User,
                  ...
                  or ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF or FirstNonNullFatSnfData.Org_Manual_Sample <> MaxVersionData.Upd_Manual_Sample THEN MaxVersionData.Upd_Manual_Sample ELSE NULL END AS Upd_Manual_Sample,  CASE when (ISNULL(FirstNonNullFatSnfData.First_FAT, 0) <> MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, 0) <> MaxVersionData.Upd_SNF)  THEN MaxVersionData.Upd_Sample_User ELSE (case when ISNULL(FirstNonNullFatSnfData.First_FAT, 0) <> MaxVersionData.Upd_FAT and ISNULL(FirstNonNullFatSnfData.First_SNF, 0) = MaxVersionData.Upd_SNF  then FirstNonNullFatSnfData.Org_Sample_User else null end )  END AS Upd_Sample_User, CASE when (ISNULL(FirstNonNullFatSnfData.First_FAT, FirstMilkWeightData.Org_FAT) = MaxVersionData.Upd_FAT or ISNULL(FirstNonNullFatSnfData.First_SNF, FirstMilkWeightData.Org_SNF) <> MaxVersionData.Upd_SNF)  and 
