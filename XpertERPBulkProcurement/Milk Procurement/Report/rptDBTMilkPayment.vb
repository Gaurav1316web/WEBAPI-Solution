...
#End Region
    ...
    Private Function GetAttachQry() As String
        ...
        Else
            Qry += " final.MCC_NAME as AreaName  "
        End If


        Qry += "   From ( Select  TSPL_MP_INCENTIVE1.Incetive_Rate,TSPL_ITEM_UOM_DETAIL.Conversion_Factor,TSPL_MILK_SRN_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_MILK_SRN_HEAD.DOC_CODE As [Milk Receipt Code], "
        If AreaWiseBilling = True Then
        ...
        End If
        Qry += " Left outer join (Select  VLC_Code, max(Incetive_Rate) as Incetive_Rate from TSPL_MP_INCENTIVE_ENTRY_HEAD
                         Left outer join  TSPL_MP_INCENTIVE_ENTRY_DETAIL on TSPL_MP_INCENTIVE_ENTRY_DETAIL.Document_Code= TSPL_MP_INCENTIVE_ENTRY_HEAD.Document_Code group by VLC_Code) as TSPL_MP_INCENTIVE1 on TSPL_MP_INCENTIVE1.VLC_Code=TSPL_MILK_SRN_HEAD.VLC_CODE 
                         where 2 = 2 And Cast(TSPL_MILK_SRN_HEAD.DOC_DATE As Date) >='" + clsCommon.GetPrintDate(txtFromDate.Text, "dd/MMM/yyyy") + "'  
                         And Cast(TSPL_MILK_SRN_HEAD.DOC_DATE As Date) <='" + clsCommon.GetPrintDate(txtToDate.Text, "dd/MMM/yyyy") + "'   
                         " + whrclsRecpt + " " + whre + "  
                          Union All 
                          Select TSPL_MP_INCENTIVE1.Incetive_Rate,TSPL_ITEM_UOM_DETAIL.Conversion_Factor,TSPL_MILK_SRN_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_MILK_REJECT_HEAD.DOC_CODE As [Milk Receipt Code], "
        If AreaWiseBilling = True Then
            Qry += " tspl_location_master.Location_Desc As [MCC Name],"
        Else
             Qry += "  TSPL_MCC_MASTER.MCC_NAME As [MCC Name],"
        ...
        Qry += " left join TSPL_MILK_REJECT_TYPE on TSPL_MILK_REJECT_TYPE.code=TSPL_MILK_REJECT_DETAIL.Reject_Type 
    ...
    Sub FormatGrid()
        ...
        gv.Columns("SRN QtyLtr").FormatString = "{0n2}"

        gv.Columns("DBT Amount").Width = 100
        gv.Columns("DBT Amount").IsVisible = True
        gv.Columns("DBT Amount").HeaderText = "DBT Amount"
        gv.Columns("DBT Amount").FormatString = "{0n2}"

        Dim summaryRowItem As New GridViewSummaryRowItem()

        Dim item1 As New GridViewSummaryItem("SRN QtyLtr", "{0F2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item1)

        Dim item2 As New GridViewSummaryItem("DBT Amount", "{0F2}", GridAggregateFunction.Sum)
