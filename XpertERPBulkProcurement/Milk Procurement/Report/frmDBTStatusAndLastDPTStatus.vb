...
Public Class frmDBTStatusAndLastDPTStatus
    ...
    Public Sub fillGridReport(ByVal isPrint As Boolean)
        Try
            ...
            End If
            TemplateGridview = MyRadGridView2

            Dim query As String
            MyRadGridView2.DataSource = Nothing
            If objCommonVar.RCDFCFP = True Then
    ...
    left join [" + clsCommon.myCstr(dt.Rows(ii).Item("DataBase_Name")) + "].[dbo].TSPL_DBT_NEFT ON [" + clsCommon.myCstr(dt.Rows(ii).Item("DataBase_Name")) + "].[dbo].TSPL_DBT_NEFT_DETAIL.Document_Code= [" + clsCommon.myCstr(dt.Rows(ii).Item("DataBase_Name")) + "].[dbo].TSPL_DBT_NEFT.Document_Code where [" + clsCommon.myCstr(dt.Rows(ii).Item("DataBase_Name")) + "].[dbo].TSPL_DBT_NEFT.Document_Code= '" & docNo & "' group by [" + clsCommon.myCstr(dt.Rows(ii).Item("DataBase_Name")) + "].[dbo].TSPL_DBT_NEFT.From_Date, [" + clsCommon.myCstr(dt.Rows(ii).Item("DataBase_Name")) + "].[dbo].TSPL_DBT_NEFT.To_Date"
            ...
            If (dt2 IsNot Nothing AndAlso dt2.Rows.Count > 0) Then
                MyRadGridView2.Visible = True
                MyRadGridView2.DataSource = dt2
                MyRadGridView2.ReadOnly = True
                SetGridFormat(MyRadGridView2)
                ReStoreGridLayout()
                ...
                If isPrint Then
                    ...
                    If objCommonVar.RCDFCFP Then
                        frmCRV.funreport(CrystalReportFolder.SalesReport, dt2, "rptERPStatusTrackingReport", Label2.Text)
                    Else
    ...
    Private Sub ReStoreGridLayout()
        Try
            If clsCommon.myLen(PageSetupReport_ID) > 0 Then
                ...
                If Not obj Is Nothing AndAlso obj.GridColumns >= MyRadGridView2.ColumnCount Then
                    Dim ii As Integer
                    For ii = 0 To gv1.Columns.Count - 1 Step ii + 1
                        MyRadGridView2.Columns(ii).IsVisible = False
                        MyRadGridView2.Columns(ii).VisibleInColumnChooser = True
                    Next
                    MyRadGridView2.LoadLayout(obj.GridLayout)
    ...
    Private Sub print(ByVal exporter As EnumExportTo)
        Try
            ...
            If exporter = EnumExportTo.Excel Then
                transportSql.applyExportTemplate(MyRadGridView2, PageSetupReport_ID)
            ...
            Else
                ...
                doc.AssociatedObject = MyRadGridView2
    ...
    Private Sub rdbDBTStatus_CheckedChanged_1(sender As Object, e As EventArgs) Handles rdbDBTStatus.CheckedChanged
    ...
    Private Sub rdbLastDBTStatus_CheckedChanged_1(sender As Object, e As EventArgs) Handles rdbLastDBTStatus.CheckedChanged
        If rdbLastDBTStatus.Checked Then
            Label2.Text = "Last DBT Status Report"
            rdbLastDBTStatus.Location = New Point(174, 26)
        End If
    End Sub
