    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        ...
        Try
            ...
            qry = "  select  (xx.MCC),max(MCCName)MCCName " + AreaName + ",sum(xx.SrnQty)SrnQty,sum(xx.SrnAmt)SrnAmt,sum(xx.FATKG)FATKG,sum(xx.SNFKG)SNFKG,
                            ...
                            , Case When Coalesce(TSPL_MILK_SAMPLE_DETAIL.FAT, 0) <= 0 Then 'M' When Coalesce(TSPL_MILK_SAMPLE_DETAIL.FAT, 0) <= 5.5  Then 'C' Else 'M' End As [Milk Type], TSPL_MILK_RECEIPT_HEAD.DOC_CODE As [Milk Receipt Code],TSPL_MILK_RECEIPT_HEAD.MCC_CODE As MCC, "
            ...
            qry += " where 2 = 2 And convert(date,TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103) >=convert(date,'" + clsCommon.GetPrintDate(Slot1FD) + "',103) and convert(date,TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103) <=convert(date,'" + clsCommon.GetPrintDate(Slot3TD) + "',103) " + whrRec + whre + "
            ...
            qry += "Left Join TSPL_MILK_REJECT_TYPE on TSPL_MILK_REJECT_TYPE.code=TSPL_MILK_REJECT_DETAIL.Reject_Type  where 2=2 
