...
Public Class rptBMCPerodicalReport
    ...
    Public Sub Griddata(ByVal print As Boolean)
        Try
            ...
            FinalQuery = "select From_Date = '" & fromDate.Value & "',To_Date= '" & ToDate.Value & "',aa.[MCC Code] ,aa.Mcc_Uploader_Code,aa.[MCC Name],aa.[Vlc Uploader Code],[VLC Name] ,aa.[Milk Weight(KG)] ,aa.[FAT(%)] ,aa.[SNF(%)] ,aa.[FAT(KG)] ,aa.[SNF(KG)] ,
...
from  " & Environment.NewLine & ""
            ...
            FinalQuery += "( select pp.[MCC Code]  as [MCC Code],max(pp.[MCC Name] )  as [MCC Name],sum([Milk Weight(KG)] ) as [Milk Weight(KG)], case when sum([Milk Weight(KG)] )=0 then 0 else (sum([FAT(KG)] )/sum([Milk Weight(KG)] ))*100 end as [FAT(%)], case when sum([Milk Weight(KG)] )=0 then 0 else (sum([SNF(KG)] )/sum([Milk Weight(KG)] ))*100 end as [SNF(%)] ,sum([FAT(KG)] ) as [FAT(KG)] ,sum([SNF(KG)] ) as [SNF(KG)], sum([FAT(LTR)] ) as [FAT(LTR)] ,sum([SNF(LTR)] ) as [SNF(LTR)] ,sum(NET_AMOUNT) as NET_AMOUNT,max(Mcc_Uploader_Code) as Mcc_Uploader_Code,[Vlc Code],max([Vlc Uploader Code])[Vlc Uploader Code],max([VLC Name])[VLC Name] ,QBD,SUM(SweetQty) SweetQty,sum(SweetFatKG) SweetFatKG,sum(SweetSNFKG) SweetSNFKG,sum(SourQty) SourQty ,SUM(SourFATKG) SourFATKG, sum(SourSNFKG) SourSNFKG, sum(CurdQty) CurdQty , sum(Qty)qty from (  " & Environment.NewLine & ""

            FinalQuery += "Select  final.MCC as [MCC Code] ,final.[MCC Name],final.Date ,final.[Doc Date] ,final.[Vlc Uploader Code] ,final.[Vlc Code] ,final.[VLC Name],final.UOM_Code as [UOM],final.[Milk Weight(KG)],final.[DBT Amount],final.[FAT(%)]  ,final.[SNF(%)] ,final.[FAT(KG)],final.[SNF(KG)],final.[FAT(LTR)],final.[SNF(LTR)] ,NET_AMOUNT,Mcc_Code_VLC_Uploader as [Mcc_Uploader_Code],final.[Doc Date Time] , final.QBD , final.Qty, Case when QBD = 'SWEET' then Qty else 0 end as SweetQty,  Case when  QBD = 'SWEET' then [FAT(KG)] else 0  end  as SweetFatKG,  Case when  QBD = 'SWEET' then [SNF(KG)] else 0  end  as SweetSNFKG, Case when  QBD = 'SOUR' then Qty else 0 end as SourQty, Case when  QBD = 'SOUR' then [FAT(KG)] else 0 end as SourFATKG,Case when  QBD = 'SOUR' then [SNF(KG)] else 0 end as SourSNFKG,Case when QBD = 'CURD' then Qty else 0 end as CurdQty    From ( " & Environment.NewLine & " " & BaseQuery & " ) As final where 2=2 
              ) as  pp group by pp.[MCC Code] , pp.[Vlc Code] , QBD 
              ) as aa order by [MCC Code]"
