...
Public Class rptMachineSurveyRegister
    ...
    Function ReturnFinalQry(ByVal isPrint As Boolean) As String
        ...
        Else
            ...
            End If
            Qry &= " [Union],Code,IsNull(MachineType,'') As [Machine Type],IsNull(MachineName,'') As [Machine Name],SUM(MachineCount) As [No of Machine] from (
Select [Union],AMCU_Code As Code,BrandName As MachineName,Case When IsAMCU=1 Then 'Analyzer' Else Null End As MachineType,Case When IsAMCU=1 Then 1 Else 0 End As MachineCount from BaseQry where IsAMCU=1 And BrandName is not null
Union All
Select [Union],Weighing_Code As Code,Weighing_BrandName As MachineName,Case When IsWeighing=1 Then 'Weighing Scale' Else Null End As MachineType,Case When IsWeighing=1 Then 1 Else 0 End As MachineCount from BaseQry where IsWeighing=1 And Weighing_BrandName is not null
)finalQry
Group By [Union],MachineType,Code,MachineName"
            If isPrint AndAlso rbtnUnionWise.Checked Then
...
Left Outer Join TSPL_STATE_MASTER On TSPL_STATE_MASTER.STATE_CODE=TSPL_COMPANY_MASTER.State Order By [Union],[Machine Type],[Machine Name]"
            Else
                Qry &= " order by [Union],MachineType,MachineName"
            End If
        End If
    ...
    Function ReturnBaseQry() As String
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            For Each strUnion In dt.Rows
                ...
                End If
                Qry &= "Select Row_Number() Over (Order By (Select 1)) As [S.No.],'" & clsCommon.myCstr(strUnion("Location_Name")) & "' As [Union],TSPL_MCC_MASTER.MCC_Code,TSPL_MCC_MASTER.MCC_NAME ,TSPL_ZONE_MASTER.Zone_Code ,TSPL_ZONE_MASTER.Description,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader,TSPL_VENDOR_MASTER.Vendor_Name,TSPL_VENDOR_MASTER.IsAMCU,TSPL_AMCU_MASTER.Code As AMCU_Code,TSPL_AMCU_MASTER.Name As BrandName,TSPL_VENDOR_MASTER.IsWeighing,TSPL_WEIGHING_MASTER.Code As Weighing_Code,TSPL_WEIGHING_MASTER.Name As Weighing_BrandName
