...
Public Class FrmMilkTransferIn
        ...
        Try
            ...
            If ((objW IsNot Nothing AndAlso objQ IsNot Nothing AndAlso objD IsNot Nothing) OrElse AllowBulkProcMCCwithoutTankerDispatch = 1 AndAlso objW IsNot Nothing AndAlso objQ IsNot Nothing) Then
                ...
                If AllowBulkProcMCCwithoutTankerDispatch = 1 Then
                    ...
                    fndReferenceNo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Reference_No from tspl_gate_entry_details where Gate_Entry_No = '" + objW.Gate_Entry_No + "' "))
                Else
                    ...
                    fndReferenceNo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Reference_No from tspl_gate_entry_details where Gate_Entry_No = '" + strGateEntryNo + "' "))
                End If
                ...
                If MCCChamberwise = 1 Then
                    Dim isCreateBulkProcPriceChartItemWise As Integer = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.isCreateBulkProcPriceChartItemWise, clsFixedParameterCode.isCreateBulkProcPriceChartItemWise, Nothing))
                    If objW.Arr IsNot Nothing AndAlso objW.Arr.Count > 0 Then
                        ...
                        For Each objTr As clsWeighmentChemberNoDetails In objW.Arr
                            ...
                            ' done by priti BHA/16/07/18-000172 for bharat to show silo location chamber wise
                            If isCreateBulkProcPriceChartItemWise = 1 Then
                                subLocCode = clsDBFuncationality.getSingleValue("select TSPL_Milk_unloading_Chember_Details.Sublocation_Code   from TSPL_MILK_UNLOADING left join " & _
                                            ...
                                            "where TSPL_Milk_unloading_Chember_Details.line_no='" & objTr.Line_No & "' and Gate_Entry_No='" & objW.Gate_Entry_No & "'", Nothing)
                            Else
                                subLocCode = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Sub_location_Code from TSPL_MILK_UNLOADING where weighment_no='" & objW.Weighment_No & "'", Nothing))
                            End If
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells("colSilo").Value = clsCommon.myCstr(subLocCode)
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells("colSiloDesc").Value = clsCommon.myCstr(clsLocation.GetName(subLocCode, Nothing))
                            ...
                            If AllowBulkProcMCCwithoutTankerDispatch = 0 Then
                                ''richa agarwal 7 Feb,2019 pick qty as chamber wise ERO/07/02/19-000488
                                Dim dblQtyKGCgamberwise As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select isnull(Qty_KG,0) from tspl_MCC_dispatch_challan_detail where Chalan_No ='" & clsCommon.myCstr(strDispChallanNo) & "' and Chamber_Description ='" & clsCommon.myCstr(objTr.Chamber_Desc) & "' and Item_code ='" & clsCommon.myCstr(objTr.Item_Code) & "'", Nothing))
                                gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colNetWeightDisp).Value = dblQtyKGCgamberwise
    ...
    Function SaveData(ByVal isPost As Boolean, ByVal AutoEntry As Boolean) As Boolean
        Dim AutoMilkTransferInDateSameasWeighmentDate As Integer = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.AutoMilkTransferInDateSameasWeighmentDate, clsFixedParameterCode.AutoMilkTransferInDateSameasWeighmentDate, Nothing))
        ...
        Try
            ...
            If allowToSave(AutoEntry) Then
                ...
                If AutoEntry = True AndAlso AutoMilkTransferInDateSameasWeighmentDate = 1 Then
                    obj.Receipt_Challan_Date = clsDBFuncationality.getSingleValue("select Weighment_date from TSPL_Weighment_Detail where Weighment_No='" & clsCommon.myCstr(txtWeighmentNo.Text) & "'", trans)
                End If
                ...
                strQry = "Update TSPL_Weighment_Detail set  " &
                                        "FAT_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATRate).Value) & "', " &
                                        "SNF_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFRate).Value) & "', " &
                                        "FAT_Value='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATValue).Value) & "', " &
                                        "SNF_Value='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFValue).Value) & "', " &
                                        "FAT_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATKG).Value) & "', " &
                                        "SNF_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFKG).Value) & "', " &
                ...
                If MCCChamberwise = 1 Then
                    For ii As Integer = 0 To gvWeighment.Rows.Count - 1
                        strQry = "Update TSPL_WEIGHMENT_CHEMBER_DETAILS set  " &
                     "CH_FAT_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colFATRate).Value) & "', " &
                     "CH_SNF_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colSNFRate).Value) & "', " &
                     "CH_FAT_Value='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colFATValue).Value) & "', " &
                     "CH_SNF_Value='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colSNFValue).Value) & "', " &
                     "CH_FAT_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colFATKG).Value) & "', " &
                     "CH_SNF_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colSNFKG).Value) & "', " &
                     "CH_Amount='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colRcptAmt).Value) & "' " &
                     "where Weighment_No='" & txtWeighmentNo.Text & "' and LINE_NO='" & gvWeighment.Rows(ii).Cells(colSLNo).Value & "'"
                        'and Chamber_Desc='" & gvWeighment.Rows(ii).Cells(colChamberDesc).Value & "'
                        clsDBFuncationality.ExecuteNonQuery(strQry, trans)
                    Next
                End If
        ...
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                If SaveData(True, AutoEntry) Then
                    ...
                    If (clsMilkTransferIn.postData(fndRcptChalanNo.Value)) Then
                        If AutoEntry = False Then
        ...
        If obj IsNot Nothing Then
            ...
            If objW IsNot Nothing AndAlso objQ IsNot Nothing AndAlso objD IsNot Nothing Then
                ...
                If AllowBulkProcMCCwithoutTankerDispatch = 0 Then
                    ...
                    txtDispControlSampleFAT.Text = objD.control_sample_fat
                    txtDispControlSampleSNF.Text = objD.control_sample_snf
                    fndReferenceNo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Reference_No from tspl_gate_entry_details where Gate_Entry_No = '" + obj.Gate_Entry_no + "' "))
                ...
                gvWeighment.Rows(0).Cells("colSiloDesc").Value = clsCommon.myCstr(clsLocation.GetName(gvWeighment.Rows(0).Cells("colSilo").Value, Nothing))
                gvWeighment.Rows(0).Cells(colFATPer).Value = objW.fat_per
                'gvWeighment.Rows(0).Cells(colFATKG).Value = clsCommon.myCdbl(objW.wt_Weight) * clsCommon.myCdbl(objW.fat_per) / 100
                gvWeighment.Rows(0).Cells(colSNFPer).Value = objW.snf_Per
                'gvWeighment.Rows(0).Cells(colSNFKG).Value = clsCommon.myCdbl(objW.Net_Weight) * clsCommon.myCdbl(objW.snf_Per) / 100
                ...
                If MCCChamberwise = 1 Then
                    If objW.Arr IsNot Nothing AndAlso objW.Arr.Count > 0 Then
                        ...
                        For Each objTr As clsWeighmentChemberNoDetails In objW.Arr
                            ...
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colNetWeightRcpt).Value = objTr.Net_Weight
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colTareWeightRcpt).Value = objTr.Tare_Weight
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colFATKG).Value = objTr.CH_FAT_Kg
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colSNFKG).Value = objTr.CH_SNF_Kg
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colFATRate).Value = objTr.CH_FAT_Rate
                            ...
                            '                "where TSPL_Milk_unloading_Chember_Details.line_no='" & objTr.Line_No & "' and Gate_Entry_No='" & obj.Gate_Entry_no & "'", Nothing)
                            'Else
                            '    subLocCode = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Sub_location_Code from TSPL_MILK_UNLOADING where weighment_no='" & objW.Weighment_No & "'", Nothing))
                            'End If
                            ...
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colGrossWeightDisp).Value = 0
                            If AllowBulkProcMCCwithoutTankerDispatch = 0 Then
                                ''richa agarwal 6 Feb,2019 pick qty as chamber wise ERO/07/02/19-000488
                                Dim dblQtyKGCgamberwise As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select isnull(Qty_KG,0) from tspl_MCC_dispatch_challan_detail where Chalan_No ='" & clsCommon.myCstr(fndDispatchChallanNo.Value) & "' and Chamber_Description ='" & clsCommon.myCstr(objTr.Chamber_Desc) & "' and Item_code ='" & clsCommon.myCstr(objTr.Item_Code) & "'", Nothing))
                                gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colNetWeightDisp).Value = dblQtyKGCgamberwise
                                'gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colNetWeightDisp).Value = objD.arr(intCount).Qty_KG
                            End If
                            gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colTareWeightDisp).Value = 0
    ...
    Private Sub FrmMilkTransferIn_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'coll.Add("Receipt_Challan_No", "Varchar(30)   Primary Key")
        'coll.Add("Receipt_Challan_Date", "Datetime not null")
        'coll.Add("Dispatch_Challan_No", "Varchar(30)   not null")
        'coll.Add("Weighment_No", "Varchar(30)  not null ")
        'coll.Add("Qc_No", "Varchar(30)   not null")
        'coll.Add("Gate_Entry_no", "Varchar(30)   not null")
        'coll.Add("location_code", "Varchar(12)   not null References TSPL_LOCATION_MASTER(Location_Code) ")
        'coll.Add("km_reading_receipt", "Varchar(20)   not null")
        'coll.Add("New_Seal_No1", "varchar(30) null")
        'coll.Add("New_Seal_No2", "varchar(30) null")
        'coll.Add("New_Seal_No3", "varchar(30) null")
        'coll.Add("New_Seal_No4", "varchar(30) null")
        'coll.Add("New_Seal_No5", "varchar(30) null")
        'coll.Add("New_Seal_No6", "varchar(30) null")
        'coll.Add("New_Seal_No7", "varchar(30) null")
        'coll.Add("New_Seal_No8", "varchar(30) null")
        'coll.Add("New_Seal_No9", "varchar(30) null")
        'coll.Add("New_Seal_No10", "varchar(30) null")
        'coll.Add("isNewSealNo", "integer not null default 0")
        ...
        'coll.Add("Modified_Date", "varchar(30) NOT NULL")
    ...
    Sub printData()
        Try
            clsMilkTransferIn.funMilkTrnasferPrint(MyBase.Form_ID, False, dtpRcptDateAndTime.Value, fndRcptChalanNo.Value)
    ...
    Function SaveMILKRGPData()
        Try
            Dim strSiloNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Sub_location_Code  from TSPL_MILK_UNLOADING where Gate_Entry_No='" & txtgateEntryNo.Text & "'"))
...
a:
    ...
    Private Sub btnJE_Click(sender As Object, e As EventArgs) Handles btnJE.Click
        ShowJE(MyBase.Form_ID, fndRcptChalanNo.Value)
    End Sub
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(fndRcptChalanNo.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowHistoryData(fndRcptChalanNo.Value, "Receipt_Challan_No", "tspl_milk_transfer_in")
        Catch ex As Exception
