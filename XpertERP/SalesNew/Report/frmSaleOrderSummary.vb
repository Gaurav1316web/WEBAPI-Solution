...
'-Updation By--[Pankaj Kumar Chaudhary]-against Ticket No-[BM00000001661]
'' Anubhooti(2-July-2014) Added Export Permission Against BM00000003016 ''''''''
'Puran Singh Negi (13/Aug/2014) Ticket No- BM00000003400
...
Imports common
Public Class FrmSaleOrderSummary
    ...
    Sub LoadData(ByVal IsPrint As Exporter)
        ...
        ElseIf chkclosed.Checked = False Then
        ...
        arrHeader.Add("Sale Order Summary : " + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(), "dd/MMM/yyyy hh:mm tt"))
        ...
        If IsPrint = Exporter.Print OrElse IsPrint = Exporter.Refresh Then
            ...
            End If
            If rbtnlocslct.IsChecked AndAlso cbgLocation.CheckedValue.Count > 0 Then
                strLocation += " and  TSPL_SD_SALES_ORDER_HEAD.Bill_To_Location in (" + clsCommon.GetMulcallString(cbgLocation.CheckedValue) + ") "
            Else
                strLocation = ""
            End If
            Dim str As String = "select '" & fromDate.Value & "' as Fdate,'" & ToDate.Value & "' as Tdate,'" & strStatus & "' as status, " & _
            "Document_Code,Document_Date,max(Parent_Customer_No)as ParentCode,Customer_Code,Customer_Name,Salesman_Name ,Bill_To_Location,Location_Desc,Delivery_date, " & _
            ...
            " convert(date,TSPL_SD_SALES_ORDER_HEAD.Document_Date,103) <= '" & clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") & "'  " & strCustomer & "  " & strLocation & " " & strclosed & "" & _
            " union all " & _
            ...
            " TSPL_ITEM_MASTER.Item_Desc,0 as OrderQty,  TSPL_SD_SHIPMENT_DETAIL.Qty  as ShippedQty,TSPL_SD_SHIPMENT_DETAIL.Unit_code, " & _
            ...
            " where convert(date,TSPL_SD_SALES_ORDER_HEAD.Document_Date,103) >= '" & clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") & "' and " & _
            " convert(date,TSPL_SD_SALES_ORDER_HEAD.Document_Date,103) <= '" & clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") & "'  " & strCustomer & "  " & strLocation & " " & strclosed & ") " & _
            " xx group by Item_Code,Document_Code,Document_Date,Customer_Name,Customer_Code,Salesman_Name,Bill_To_Location,Location_Desc,Delivery_date, " & _
            " Item_Code,Item_Desc,Unit_code "

            'Dim catCode As String = ""
            'Dim dtt As DataTable = clsDBFuncationality.GetDataTable("select distinct Item_Category_Code  from TSPL_ITEM_MASTER_CATEGORY ")
            'If dtt IsNot Nothing Then
            '    For j As Integer = 0 To dtt.Rows.Count - 1
            '        catCode = catCode + "[" + dtt.Rows(j)("Item_Category_Code").ToString + "]"
            '        If j <> (dtt.Rows.Count - 1) Then
            '            catCode = catCode + ","
            '        End If
            '    Next
            'End If
            ...
            str = "select TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code as [MainGroupCode],  TSPL_ITEM_CATEGORY_LEVEL.description as [Main Group],TSPL_ITEM_MASTER_CATEGORY.item_cagetory_values as [GroupCode], TSPL_ITEM_CATEGORY_LEVEL_VALUES.description as [Group Name],a.* from (" + str + ") a left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.item_code=a.item_code LEFT OUTER JOIN TSPL_ITEM_MASTER_CATEGORY ON TSPL_ITEM_MASTER_CATEGORY.item_code=  TSPL_ITEM_MASTER.item_code LEFT OUTER JOIN TSPL_ITEM_CATEGORY_LEVEL ON TSPL_ITEM_CATEGORY_LEVEL.item_category_code= TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code LEFT OUTER JOIN TSPL_ITEM_CATEGORY_LEVEL_VALUES ON TSPL_ITEM_CATEGORY_LEVEL_VALUES.item_category_code= TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code and TSPL_ITEM_CATEGORY_LEVEL_VALUES.code= TSPL_ITEM_MASTER_CATEGORY.item_cagetory_values"
            str = "select ass.* from (" + str + ")ass "

            'str = " select " + catCode + " , a.*  from ( " + str + " ) a "

            Dim whrcate As String = ""

            If rbtnCategorySelect.IsChecked Then
                Dim isFirstTime As Boolean = True
                str += " where exists (select 1  from TSPL_ITEM_MASTER_CATEGORY where Item_code in (select distinct item_code from (" + query + ")a1) and ( " + Environment.NewLine
                For Each Ctr As RadTreeNode In tvCategory.CheckedNodes
                    If (Ctr.Checked) And Ctr.Parent IsNot Nothing Then
                        If Not isFirstTime Then
                            ...
                            whrcate += " or "
                        End If
                        str += " ( maingroupcode='" + clsCommon.myCstr(Ctr.Parent.Value) + "' and groupcode='" + clsCommon.myCstr(Ctr.Value) + "' )" + Environment.NewLine
            ...
            qry = "select max(Fdate) as Fdate,max(Tdate) as Tdate,max(aa.status) as status,Document_Code,Document_Date, " & _
            "max(aa.ParentCode) as ParentCustomerCode,MAX(Parent_Master.Customer_Name) as ParentName,Customer_Code,aa.Customer_Name,Salesman_Name,Bill_To_Location,Location_Desc,Delivery_date,SUM(TotalOrderAmt) as TotalOrderAmt,SUM(totalShipAmt) as totalShipAmt, " & _
            "SUM(Outstanding) as Outstanding,Category from ( " & str & " ) aa left outer join TSPL_CUSTOMER_MASTER as Parent_Master on Parent_Master.Cust_Code=aa.ParentCode group by Document_Code,Document_Date, " & _
            "Customer_Code,aa.Customer_Name,Salesman_Name,Bill_To_Location,Location_Desc,Delivery_date,Category"
            ...
            ElseIf rdbpending.IsChecked Then
                qry = qry & " having SUM(OrderQty) = SUM(Outstanding) order by Document_Code"
            ElseIf rdbPartial.IsChecked Then
                qry = qry & " having SUM(OrderQty) > SUM(Outstanding) and SUM(Outstanding) <> 0 order by Document_Code"
            ElseIf rdbComplete.IsChecked Then
        ...
        ElseIf IsPrint = Exporter.Excel Then
            clsCommon.MyExportToExcel("Sale Order Summary", gv, arrHeader, Me.Text)
        Else
    ...
    Sub SetGridFormation()
        ...
        gv.Columns("ParentCustomerCode").IsVisible = True
        gv.Columns("ParentCustomerCode").Width = 100
        gv.Columns("ParentCustomerCode").HeaderText = "Parent Code"
        ...
        gv.Columns("totalShipAmt").HeaderText = "Total Shiped Amt"
