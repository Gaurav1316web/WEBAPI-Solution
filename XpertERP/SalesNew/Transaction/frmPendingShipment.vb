Imports common
Public Class frmPendingShipment
#Region "Variables"
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        qry += " where TSPL_SD_SALE_INVOICE_HEAD.Status=1 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALE_INVOICE_DETAIL.Shipment_Code,''))>0   " + Environment.NewLine
        qry += " union all   " + Environment.NewLine
        ...
        qry += "  ,Scheme_Code from TSPL_SD_SALE_INVOICE_DETAIL " + Environment.NewLine
        qry += " left outer join TSPL_SD_SALE_INVOICE_HEAD on TSPL_SD_SALE_INVOICE_HEAD.Document_Code=TSPL_SD_SALE_INVOICE_DETAIL.Document_Code" + Environment.NewLine
        qry += " where TSPL_SD_SALE_INVOICE_HEAD.Status=0 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALE_INVOICE_DETAIL.Shipment_Code,''))>0 and TSPL_SD_SALE_INVOICE_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
    ...
    Sub btnOKPressed()
        ArrReturn = New List(Of clsSNShipmentDetail)
        Dim obj As clsSNShipmentDetail = Nothing
        For ii As Integer = 0 To gv1.RowCount - 1
            If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                obj = New clsSNShipmentDetail()
