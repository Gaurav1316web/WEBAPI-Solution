...
#Region "Variables"
    ...
    Const colCommitedQty As String = "colCommitedQty"
    Const colISCommited As String = "colISCommited"
...
#End Region
    Public Sub SetUserMgmtNew()
        ...
        'MyBase.SetUserMgmt(clsUserMgtCode.frmSNSalesOrder)
    ...
    Sub LoadBlankGrid()
        ...
        Dim repoCommitedQty As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoCommitedQty = New GridViewDecimalColumn()
        repoCommitedQty.FormatString = ""
        repoCommitedQty.HeaderText = "Commited Quantity"
        repoCommitedQty.Name = colCommitedQty
        repoCommitedQty.Width = 80
        repoCommitedQty.Minimum = 0
        repoCommitedQty.ShowUpDownButtons = False
        repoCommitedQty.Step = 0
        repoCommitedQty.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gv1.MasterTemplate.Columns.Add(repoCommitedQty)

        Dim repoIsCommited As GridViewComboBoxColumn = New GridViewComboBoxColumn()
        repoIsCommited = New GridViewComboBoxColumn()
        repoIsCommited.FormatString = ""
        repoIsCommited.HeaderText = "Is Commited"
        repoIsCommited.Name = colISCommited
        repoIsCommited.Width = 80
        repoIsCommited.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        repoIsCommited.DataSource = GetISCommited()
        repoIsCommited.ValueMember = "Code"
        repoIsCommited.DisplayMember = "Name"

        gv1.MasterTemplate.Columns.Add(repoIsCommited)
        ...
        ReStoreGridLayout()
    End Sub
    Public Shared Function GetISCommited() As DataTable
    ...
    Sub ChkCommitedQty(ByVal rowIndx As Integer)
        Try
            Dim ShowCommitedQty As Boolean = clsCommon.myCBool(IIf(clsFixedParameter.GetData(clsFixedParameterType.CommitedDefaultQty, clsFixedParameterCode.CommitedDefaultQty, Nothing) = "1", True, False))

           
            Dim Qty As Decimal = clsCommon.myCdbl(gv1.Rows(rowIndx).Cells(colCommitedQty).Value)
            Dim dblShipmentqty As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select sum(Qty)  as Qty from TSPL_SD_SHIPMENT_DETAIL  left join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.Document_Code =TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE where TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order ='" + txtDocNo.Value + "' and TSPL_SD_SHIPMENT_DETAIL.Item_Code ='" + clsCommon.myCstr(gv1.Rows(rowIndx).Cells(colICode).Value) + "' group by TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order,TSPL_SD_SHIPMENT_DETAIL.Item_Code "))
            Dim dblSalesOrderQQty As Decimal = clsCommon.myCdbl(gv1.Rows(rowIndx).Cells(colQty).Value)
            'Dim dblSalesOrderQQty As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select TSPL_SD_SALES_ORDER_DETAIL.qty from TSPL_SD_SALES_ORDER_DETAIL  where TSPL_SD_SALES_ORDER_DETAIL.Document_Code ='" + txtDocNo.Value + "' and TSPL_SD_SALES_ORDER_DETAIL.Item_Code ='" + clsCommon.myCstr(gv1.Rows(rowIndx).Cells(colICode).Value) + "'"))

           


            If dblShipmentqty = dblSalesOrderQQty Then
                'gv1.Rows(rowIndx).Cells(colCommitedQty).ReadOnly = True
                Dim dblcommitedqtyforslaesorder As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select isnull(CommitedQty ,0) from TSPL_SD_SALES_ORDER_DETAIL where Document_Code ='" & clsCommon.myCstr(txtDocNo.Value) & "' and Item_Code ='" & clsCommon.myCstr(gv1.Rows(rowIndx).Cells(colICode).Value) & "'"))
                If Qty <> dblcommitedqtyforslaesorder Then
                    Throw New Exception("Committed Qty cannot be changed. it should be  '" + clsCommon.myCstr(dblcommitedqtyforslaesorder) + "' at line no : '" + clsCommon.myCstr(rowIndx + 1) + "'")
                End If
                gv1.Rows(rowIndx).Cells(colCommitedQty).Value = gv1.Rows(rowIndx).Cells(colQty).Value
            Else
                'gv1.Rows(rowIndx).Cells(colCommitedQty).Value = dblSalesOrderQQty - dblShipmentqty
                If Qty > (dblSalesOrderQQty - dblShipmentqty) AndAlso Not ShowCommitedQty Then
                    Throw New Exception("Committed Qty is not greater than '" + clsCommon.myCstr(dblSalesOrderQQty - dblShipmentqty) + "' at line no : '" + clsCommon.myCstr(rowIndx + 1) + "'")
                End If
            End If
           

            '   Dim ChkBalance As Decimal = clsCommon.myCdbl(gv1.Rows(rowIndx).Cells(colQty).Value) - Qty




            'Dim ChkBalance As Decimal = clsCommon.myCdbl(gv1.Rows(rowIndx).Cells(colQty).Value) - clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select sum(Qty)  as Qty from TSPL_SD_SHIPMENT_DETAIL  left join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.Document_Code =TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE where TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order ='" + txtDocNo.Value + "' and TSPL_SD_SHIPMENT_DETAIL.Item_Code ='" + clsCommon.myCstr(gv1.Rows(rowIndx).Cells(colICode).Value) + "' group by TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order,TSPL_SD_SHIPMENT_DETAIL.Item_Code "))
            ''If ChkBalance > 0 Then
            'If Qty > ChkBalance AndAlso Not ShowCommitedQty Then
            '    Throw New Exception("Committed Qty is not greater than '" + clsCommon.myCstr(ChkBalance) + "'")
            'End If
            ''End If
            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colQty) Then
                        gv1.CurrentRow.Cells(colCommitedQty).Value = clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value)
                        gv1.CurrentRow.Cells(colISCommited).Value = "Yes"
                    End If

                    Dim ShowCommitedQty As Boolean = False
                    If e.Column Is gv1.Columns(colCommitedQty) Or e.Column Is gv1.Columns(colISCommited) Then
                        ShowCommitedQty = clsCommon.myCBool(IIf(clsFixedParameter.GetData(clsFixedParameterType.CommitedDefaultQty, clsFixedParameterCode.CommitedDefaultQty, Nothing) = "1", True, False))

                        If ShowCommitedQty = True Then
                            gv1.CurrentRow.Cells(colCommitedQty).ReadOnly = True
                            gv1.CurrentRow.Cells(colISCommited).ReadOnly = True
                            gv1.CurrentRow.Cells(colCommitedQty).Value = clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value)
                            gv1.CurrentRow.Cells(colISCommited).Value = "Yes"
                        ElseIf ShowCommitedQty = False AndAlso (clsCommon.myLen(gv1.CurrentRow.Cells(colISCommited).Value) <= 0 OrElse clsCommon.myCstr(gv1.CurrentRow.Cells(colISCommited).Value) = "Yes") Then
                            gv1.CurrentRow.Cells(colCommitedQty).ReadOnly = False
                            gv1.CurrentRow.Cells(colISCommited).ReadOnly = False
                            'gv1.CurrentRow.Cells(colCommitedQty).Value = clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value)
                            gv1.CurrentRow.Cells(colISCommited).Value = "Yes"
                        ElseIf ShowCommitedQty = False AndAlso clsCommon.myCstr(gv1.CurrentRow.Cells(colISCommited).Value) = "No" Then
                            gv1.CurrentRow.Cells(colCommitedQty).ReadOnly = True
                            gv1.CurrentRow.Cells(colISCommited).ReadOnly = False
                            gv1.CurrentRow.Cells(colCommitedQty).Value = 0
                            'gv1.CurrentRow.Cells(colISCommited).Value = "Yes"
                        End If
                    End If
                    If e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colCommitedQty) OrElse e.Column Is gv1.Columns(colISCommited) Then
                        ChkCommitedQty(gv1.CurrentRow.Index)
                    End If
                    ''-----------------------
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) <= 0 Then
             ...
             "where TSPL_ITEM_MASTER.Active=1 " & strTax & " )xxx Where  Tax_group='" & txtTaxGroup.Value & "'   " & strPriceCondition & " " & _
             "" & strTaxRate & " " & _
            ...
            If clsCommon.myLen(pivotheader) > 0 Then
             ...
             "TSPL_ITEM_PRICE_MASTER  where Start_Date<='" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "' AND (End_Date  >='" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "' OR End_Date IS NULL) and TSPL_ITEM_PRICE_MASTER.Location_Code='" & txtBillToLocation.Value & "'  GROUP BY Item_Code, UOM,Price_Code, Tax_group   " &
             ")  AS groupedP  ON TSPL_ITEM_PRICE_MASTER.Item_Code = groupedP.Item_Code AND  TSPL_ITEM_PRICE_MASTER.Start_Date = groupedP.MaxDateTime AND " &
            ...
            If Not dr Is Nothing Then
                If clsCommon.myLen(clsCommon.myCstr(dr("Item"))) > 0 Then
                    ...
                    'End If
                    If PurchaseOneItemOneVendor = True Then  '' condition commented by panch raj for dcc (Anand)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) <= 0 Then
                ...
                Else
                    qry = "select TSPL_CUSTOMER_ITEM_DETAIL.item_no as Item,TSPL_CUSTOMER_ITEM_DETAIL.item_desc as [ItemDesc],TSPL_CUSTOMER_ITEM_DETAIL.uom as Unit , " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.item_rate as BasicRate,TSPL_CUSTOMER_ITEM_DETAIL.Item_MRP as MRP, TSPL_ITEM_MASTER.ITF_CODE as [ITF Code], " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.Start_Date as [Start Date],'' as [Vendor code],'' as [Vendor Desc],'' as PrincipleCode,'' as PrincipleDesc, " & _
                    "Weight_Value as [Weight Value] from TSPL_CUSTOMER_ITEM_DETAIL left outer join TSPL_ITEM_MASTER on " & _
            ...
            If clsCommon.myLen(pivotheader) > 0 Then
                ...
                Else
                    ''BM00000008218 add vendor code,vendor name ,principlecode and principlename column as blank because these fields are in above query and used in grid columns
                    qry = "select * from (select a.DESCRIPTION,a.cat_value, TSPL_CUSTOMER_ITEM_DETAIL.item_no as Item,TSPL_CUSTOMER_ITEM_DETAIL.item_desc as [ItemDesc],TSPL_CUSTOMER_ITEM_DETAIL.uom as Unit , " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.item_rate as BasicRate,TSPL_CUSTOMER_ITEM_DETAIL.Item_MRP as MRP, TSPL_ITEM_MASTER.ITF_CODE as [ITF Code], " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.Start_Date as [Start Date],'' as [Vendor code],'' as [Vendor Desc],'' as PrincipleCode,'' as PrincipleDesc, " & _
                    "Weight_Value as [Weight Value] from TSPL_CUSTOMER_ITEM_DETAIL left outer join TSPL_ITEM_MASTER on " & _
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("SNSOPI@32", qry)
            If Not dr Is Nothing Then
                If clsCommon.myLen(clsCommon.myCstr(dr("Item"))) > 0 Then
                    SetitemWiseTaxSetting(True, True)
                    gv1.CurrentRow.Cells(colRowType).Value = RowTypeItem
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        ...
        Dim dblConvBasicRate As Double = dblRate 'dblOrgBasicRate '* dblConvF
    ...
    Sub AddNew()
        ...
        Dim ShowCommitedQty As Boolean = clsCommon.myCBool(IIf(clsFixedParameter.GetData(clsFixedParameterType.CommitedDefaultQty, clsFixedParameterCode.CommitedDefaultQty, Nothing) = "1", True, False))
        If ShowCommitedQty = True Then
            gv1.Rows(gv1.Rows.Count - 1).Cells(colCommitedQty).ReadOnly = True
            gv1.Rows(gv1.Rows.Count - 1).Cells(colISCommited).ReadOnly = True
        ElseIf ShowCommitedQty = False Then
            gv1.Rows(gv1.Rows.Count - 1).Cells(colCommitedQty).ReadOnly = False
            gv1.Rows(gv1.Rows.Count - 1).Cells(colISCommited).ReadOnly = False
        End If
        ...
        ''End of For Custom Fields
        UcAttachment1.BlankAllControls()
        btnUpdateCommitedQty.Visible = False
    End Sub
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If Not isNewEntry AndAlso clsCommon.myLen(txtDocNo.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow("Pucahse Order code Not found to save")
                txtDocNo.Focus()
                Return False
            End If
            ...
            If chkclose.Checked = True AndAlso IsRemarksMandatory = True AndAlso clsCommon.myLen(txtCloseRemarks.Value) = 0 Then
                ...
                Return False
            End If
            'BM00000008026 Zero/Negative Qty DisAllow
            ' Dim arrZItem As New List(Of String)
            Dim ZeroItems As String = String.Empty
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                End If
                '' BM00000008026
                If dblQty <= 0 And clsCommon.myLen(strICode) > 0 Then
                    'arrZItem.Add(strICode)
                    ZeroItems += " (" + strICode + ")"
                End If
            ...
            Next
            ''
            If clsCommon.myLen(ZeroItems) > 0 Then
                If common.clsCommon.MyMessageBoxShow("Do you want to refill quantities of these items " + ZeroItems, Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                    Return False
                End If
            End If
            ''
    ...
    Function CFormFunction()
        ...
        Return True
    End Function
    Private Function SavingData(ByVal ChekBtnPost As Boolean)
    ...
    Private Function SaveData(ByVal isDoAbandomentNo As Boolean) As Boolean
        Try
            ...
            End If
            '=====================end here===================
            Dim ChkBalance As String = Nothing
            If (AllowToSave()) Then
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsSNSalesOrderDetail()
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colICode).Value)) > 0 AndAlso clsCommon.myCdbl(grow.Cells(colQty).Value) > 0 Then ' BM00000008026 
                        ...
                        objTr.CommitedQty = clsCommon.myCdbl(grow.Cells(colCommitedQty).Value)
                        objTr.IsCommited = clsCommon.myCstr(grow.Cells(colISCommited).Value)
                ...
                If clsSNSalesOrderHead.checkSaveNotification(obj, Nothing) Then
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsSNSalesOrderHead()
            obj = clsSNSalesOrderHead.GetData(strCode, NavTyep)
            ...
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsSNSalesOrderDetail In obj.Arr
                        gv1.Rows.AddNew()
                        ...
                        If ShowCommitedQty = True Then
                            ...
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colCommitedQty).Value = objTr.Qty
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colISCommited).Value = "Yes"
                        ElseIf ShowCommitedQty = False Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colCommitedQty).ReadOnly = False
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colISCommited).ReadOnly = False

                            gv1.Rows(gv1.Rows.Count - 1).Cells(colCommitedQty).Value = objTr.CommitedQty
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colISCommited).Value = objTr.IsCommited
                        ...
                        If clsCommon.myLen(objTr.Quotation_Code) > 0 Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsSNSalesQuotationDetail.GetBalanceRequitionQty(objTr.Quotation_Code, objTr.Item_Code, obj.Document_Code)
                        End If
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                If SavingData(True) = False Then ' BM00000008026 
                    Exit Sub
                End If
                ...
                ''=========================================
                If (clsSNSalesOrderHead.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                ...
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsSNSalesOrderHead.DeleteData(txtDocNo.Value)) Then
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        qry = "select Document_Code as Code,convert(varchar(10),Document_Date,103)  as Date,Customer_Code as [Customer Code], Customer_Name as Customer,Total_Amt as Amount,case when TSPL_SD_SALES_ORDER_HEAD.status='0' then 'Pending' else 'Approved' end as [Status],Bill_To_Location as [Location],(select Location_Desc  from TSPL_LOCATION_MASTER where Location_Code =Bill_To_Location ) as [Location Name] ,TSPL_USER_MASTER.User_Name as [User Name] from TSPL_SD_SALES_ORDER_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALES_ORDER_HEAD.Customer_Code left join TSPL_USER_MASTER on TSPL_USER_MASTER.User_Code =TSPL_SD_SALES_ORDER_HEAD.Created_By"
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and TSPL_SD_SALES_ORDER_HEAD.Customer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ")"
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " TSPL_SD_SALES_ORDER_HEAD.Customer_Code in (" + strwherecls + ")"
        End If
        '-----------------------------------------------------
        LoadData(clsCommon.ShowSelectForm("SNSOOrderNoFndd", qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked), NavigatorType.Current)
        btnCopy.Enabled = False
    End Sub
    ...
    Private Sub FrmAPInvoiceEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F1 Then
            Dim ShowCommitedQty As Boolean = clsCommon.myCBool(IIf(clsFixedParameter.GetData(clsFixedParameterType.CommitedDefaultQty, clsFixedParameterCode.CommitedDefaultQty, Nothing) = "1", True, False))
            If ShowCommitedQty = False Then
                Dim frm As New FrmPWD(Nothing)
                frm.strType = "PWD"
                frm.strCode = "UserPWD"
                frm.ShowDialog()
                If frm.isPasswordCorrect Then
                    btnUpdateCommitedQty.Visible = True
                End If
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
            ButtonToolTip.SetToolTip(btnSave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine + _
                                       "TSPL_SD_SALES_ORDER_HEAD (In case of Auto Sale Invoice) " + Environment.NewLine + _
                                       "TSPL_SD_SALES_ORDER_DETAIL " + Environment.NewLine + _
                                       "TSPL_CUSTOM_FIELD_VALUES (For Custom Field)  ")
    ...
    Private Sub txtVendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtVendorNo._MYValidating
        ...
        '-------richa 30/07/2014 Ticket No. BM00000003242---------
        If clsCommon.myLen(strwherecls) = 0 Then
            txtVendorNo.Value = clsCommon.ShowSelectForm("SNSOVendorFndr", qry, "Code", " TSPL_CUSTOMER_MASTER.Status='N'", txtVendorNo.Value, "Code", isButtonClicked)
        Else
            txtVendorNo.Value = clsCommon.ShowSelectForm("SNSOVendorFndr", qry, "Code", " TSPL_CUSTOMER_MASTER.Status='N' and TSPL_CUSTOMER_MASTER.Cust_Code in (" + strwherecls + ")", txtVendorNo.Value, "Code", isButtonClicked)
    ...
    Private Sub funPrint(Optional ByVal IsPdf As Boolean = False)
        ...
        Try
            ...
            Qry = " select" & _
           " tax1.type as TaxType1,tax2.type as TaxType2,tax3.type as TaxType3,tax4.type as TaxType4,tax5.type as TaxType5,tax6.type as TaxType6, " & _
                " tax7.type as TaxType7,tax8.type as TaxType8,tax9.type as TaxType9,tax10.type as TaxType10,TSPL_SD_SALES_ORDER_DETAIL.TAX1_Amt as dTAX1_Amt , " & _
                " TSPL_SD_SALES_ORDER_DETAIL.TAX2_Amt as dTAX2_Amt  ,TSPL_SD_SALES_ORDER_DETAIL.TAX3_Amt as dTAX3_Amt ,TSPL_SD_SALES_ORDER_DETAIL.TAX4_Amt as dTAX4_Amt  , " & _
               " TSPL_SD_SALES_ORDER_DETAIL.TAX5_Amt as dTAX5_Amt  ,TSPL_SD_SALES_ORDER_DETAIL.TAX6_Amt as dTAX6_Amt ,TSPL_SD_SALES_ORDER_DETAIL.TAX7_Amt as dTAX7_Amt ," & _
               " TSPL_SD_SALES_ORDER_DETAIL.TAX8_Amt as dTAX8_Amt ,TSPL_SD_SALES_ORDER_DETAIL.TAX9_Amt as dTAX9_Amt ,TSPL_SD_SALES_ORDER_DETAIL.TAX10_Amt as dTAX10_Amt ," & _
                " TSPL_SD_SALES_ORDER_DETAIL.TAX1_Rate as dTAX1_Rate ,TSPL_SD_SALES_ORDER_DETAIL.TAX2_Rate as dTAX2_Rate ,TSPL_SD_SALES_ORDER_DETAIL.TAX3_Rate as dTAX3_Rate ," & _
               " TSPL_SD_SALES_ORDER_DETAIL.TAX4_Rate as dTAX4_Rate ,TSPL_SD_SALES_ORDER_DETAIL.TAX5_Rate as dTAX5_Rate ,TSPL_SD_SALES_ORDER_DETAIL.TAX6_Rate as dTAX6_Rate , " & _
               " TSPL_SD_SALES_ORDER_DETAIL.TAX7_Rate as dTAX7_Rate ,TSPL_SD_SALES_ORDER_DETAIL.TAX8_Rate as dTAX8_Rate ,TSPL_CUSTOMER_MASTER.State as Cust_Sate_Code ," & _
                "TSPL_LOCATION_MASTER.State as Loc_Sate_Code,Cust_State.STATE_NAME as Cust_State_Name,Cust_State.GST_STATE_Code as Cust_GST_State_Code," & _
               " Loc_State.STATE_NAME as Loc_State_Name, Loc_State.GST_STATE_Code as Loc_GST_State_Code,TSPL_SD_SALES_ORDER_HEAD.Is_Taxable ," & _
               " Cust_State.Is_GST_UT as Cust_Is_UT,Loc_State.Is_GST_UT as Loc_Is_UT, TSPL_LOCATION_MASTER.Location_Code as Loc_Code,TSPL_LOCATION_MASTER.Location_Desc ," & _
                " TSPL_CUSTOMER_MASTER.Cust_Code, TSPL_CUSTOMER_MASTER.Add1 as Cust_Add1,TSPL_CUSTOMER_MASTER.Add2 as Cust_Add2,TSPL_CUSTOMER_MASTER.Add3 as Cust_Add3," & _
                " TSPL_CUSTOMER_MASTER.PAN as Cust_Pan_no,convert(varchar,TSPL_SD_SALES_ORDER_HEAD.Document_Date,103) as Document_Date, Cust_City.City_Name ," & _
                " TSPL_ITEM_MASTER .HSN_Code  ,TSPL_SD_SALES_ORDER_HEAD.Price_code , '' as Cust_PO_Date,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt1 ," & _
                " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt2, TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt3,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt4,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt5, " & _
                "  TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt6,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt7,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt8, TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt9," & _
                " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Amt10,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name1 , TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name2 ," & _
                " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name3 ,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name4 , TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name5 ," & _
                " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name6 ,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name7 , TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name8 ," & _
                 " TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name9 ,TSPL_SD_SALES_ORDER_HEAD.Add_Charge_Name10  ,'1' as CopyType ,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add1  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add1 end as P_Add1, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add2  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add2 end as P_Add2, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add3  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add3 end as P_Add3," & _
                "case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .PIN_Code   when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Pin_No  end as P_PinNo, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .CST    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_CST_No   end as P_CstNo," & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Tin_No     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .p_Tin_No   end as P_TinNo," & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .State     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .p_State   end as p_State, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Email    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Email  end as P_Email," & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Fax     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Fax   end as P_Fax, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER.Lst_No when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_LST_No    end as P_LstNo, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER.GSTNO when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_GST_No    end as P_GST_No, " & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Cust_Code      when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_code   end as P_CustCode," & _
                " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Customer_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_name    end as P_Cust_Name, " & _
                ...
                " case when coalesce(p_cust.P_cust_code,'')='' then     case when ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')='(+__)__________' then '' else TSPL_CUSTOMER_MASTER.Phone1 end +  Case When   ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_CUSTOMER_MASTER.Phone2 Else'' End   when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Phn    end as P_Cust_Phn," & _
                " case when coalesce(p_cust.P_Pan,'')='' then TSPL_CUSTOMER_MASTER  .PAN      when coalesce(p_cust.P_Pan,'')<>'' then p_cust.P_Pan   end as P_Pan,TSPL_LOCATION_MASTER .GSTNO as Loc_GSTIN," & _
                " TSPL_CUSTOMER_MASTER.GSTNO as Cust_GSTIN, TSPL_SD_SALES_ORDER_HEAD.Total_Amt,TSPL_TAX_GROUP_MASTER.Is_Tax_Exempted ," & _
                "  TSPL_SD_SALES_ORDER_DETAIL.line_no,'" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_SD_SALES_ORDER_HEAD.Cust_PO_No,'" + FooterText + "' as FooterText , TSPL_SD_SALES_ORDER_HEAD.Abandonment_No, TSPL_SD_SALES_ORDER_HEAD.Dept_Desc ,TSPL_SD_SALES_ORDER_HEAD.Delivery_date ,TSPL_SD_SALES_ORDER_HEAD.Remarks ,(case when TSPL_SD_SALES_ORDER_HEAD .status =1 then TSPL_SD_SALES_ORDER_HEAD.Modify_By else '' end) as Modify_By ,TSPL_SD_SALES_ORDER_DETAIL.Landing_Cost,TSPL_SD_SALES_ORDER_HEAD.Created_By , "
            ...
            Qry += " TSPL_SD_SALES_ORDER_HEAD.Document_Code as DocNo ,TSPL_SHIP_TO_LOCATION.add1 +case when len(TSPL_SHIP_TO_LOCATION.add2)>0 then ', '+TSPL_SHIP_TO_LOCATION.add2 else '' end +case when LEN(isnull(TSPL_SHIP_TO_LOCATION.Add3,''))>0 then ', '+isnull(TSPL_SHIP_TO_LOCATION.Add3,'') else ' ' end  + case when len(TSPL_SHIP_TO_LOCATION.State )>0 then TSPL_SHIP_TO_LOCATION.State else '' end  as Ship_address, TSPL_SHIP_TO_LOCATION.Tin_No as Ship_ToLocation_Tin_No, convert(varchar(10),TSPL_SD_SALES_ORDER_HEAD .Document_Date,103)  as po_date,  convert (varchar,Document_Date ,103)  as po_dateTime ,case TSPL_SD_SALES_ORDER_HEAD .SalesOrder_Type when 'L'then 'Local' when 'I' then 'Import' when 'J' then 'Job Work' when 'O' then 'Open' when 'S' then 'Specific'else 'Null' end as po_type , "
            ...
            Qry += " LEFT OUTER JOIN TSPL_ITEM_CATEGORY_LEVEL_VALUES ON TSPL_ITEM_CATEGORY_LEVEL_VALUES.CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values " & _
            ...
            Dim DOCDATE As Date = clsCommon.myCDate(clsDBFuncationality.getSingleValue("select convert (date,Document_Date ,103) as Document_Date from TSPL_SD_SALES_ORDER_HEAD where Document_Code ='" & txtDocNo.Value & "'"))
            '========================================
            If GSTStatus Then
                If dt1.Rows.Count > 0 Then
                    If clsCommon.myCdbl(dt1.Rows(0)("Is_Tax_Exempted")) = 1 Then
                        strPdfAttachmentPath = frmCRV.funsubreportWithdt(MyBase.Form_ID, IsPdf, CrystalReportFolder.KwalitySalesReport, dt1, clsERPFuncationality.CompanyAddresShowinFooter(), "rptSaleOrder_Exempted", "Sale Order", clsCommon.myCDate(dt1.Rows(0)("Document_Date")), "rptCompanyAddress.rpt")
                    Else
                        If clsCommon.CompairString(clsCommon.myCstr(dt1.Rows(0)("Cust_Sate_Code")), clsCommon.myCstr(dt1.Rows(0)("Loc_Sate_Code"))) = CompairStringResult.Equal Then
                            If clsCommon.myCdbl(dt1.Rows(0)("Cust_Is_UT")) = 1 OrElse clsCommon.myCdbl(dt1.Rows(0)("Loc_Is_UT")) = 1 Then
                                strPdfAttachmentPath = frmCRV.funsubreportWithdt(MyBase.Form_ID, IsPdf, CrystalReportFolder.KwalitySalesReport, dt1, clsERPFuncationality.CompanyAddresShowinFooter(), "rptSaleOrder_IntrastateUGST", "Sale Order", clsCommon.myCDate(dt1.Rows(0)("Document_Date")), "rptCompanyAddress.rpt")
                            Else
                                strPdfAttachmentPath = frmCRV.funsubreportWithdt(MyBase.Form_ID, IsPdf, CrystalReportFolder.KwalitySalesReport, dt1, clsERPFuncationality.CompanyAddresShowinFooter(), "rptSaleOrder_Intrastate", "Sale Order", clsCommon.myCDate(dt1.Rows(0)("Document_Date")), "rptCompanyAddress.rpt")
                            End If

                        Else
                            strPdfAttachmentPath = frmCRV.funsubreportWithdt(MyBase.Form_ID, IsPdf, CrystalReportFolder.KwalitySalesReport, dt1, clsERPFuncationality.CompanyAddresShowinFooter(), "rptSaleOrder_Interstate", "Sale Order", clsCommon.myCDate(dt1.Rows(0)("Document_Date")), "rptCompanyAddress.rpt")
                        End If
                    End If
    ...
    Private Sub gv1_CellFormatting(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.CellFormattingEventArgs) Handles gv1.CellFormatting
        Try
            ...
            ElseIf e.Column Is gv1.Columns(colAmt) Then
                ...
                End If
                'ElseIf e.Column Is gv1.Columns(colCommitedQty) Or e.Column Is gv1.Columns(colISCommited) Then
                '    Dim ShowCommitedQty As Boolean = clsCommon.myCBool(IIf(clsFixedParameter.GetData(clsFixedParameterType.CommitedDefaultQty, clsFixedParameterCode.CommitedDefaultQty, Nothing) = "1", True, False))
    ...
    Private Sub btnAmendment_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAmendment.Click
        ...
        '    IsSavedData = IsSavedData AndAlso clsSNSalesOrderHead.PostData(txtDocNo.Value)
    ...
    Private Sub btnCopy_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCopy.Click
        ...
        frm.ShowDialog()
        If frm.ArrReturn IsNot Nothing AndAlso frm.ArrReturn.Count > 0 Then
            Dim objReq As clsSNSalesOrderHead = clsSNSalesOrderHead.GetData(frm.strFirstSO, NavigatorType.Current)
            ...
            If gv1.Rows.Count > 0 AndAlso clsCommon.myLen(gv1.Rows(gv1.Rows.Count - 1).Cells(colICode).Value) <= 0 Then
                gv1.Rows.RemoveAt(gv1.Rows.Count - 1)
            End If
            For Each obj As clsSNSalesOrderDetail In frm.ArrReturn
                gv1.Rows.AddNew()
...
#Region "New Mail System"
    ...
    Private Sub SendSMSandEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        Try
            ...
            'sanjay
            Dim strContactperson As String = ""
            Dim dtContent As DataTable = clsDBFuncationality.GetDataTable("SELECT SMS_Text,Email_Text,Email_subject from TSPL_ES_Content where Form_ID='" + clsUserMgtCode.frmSNSalesOrder + "'", Nothing)
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                objEmailH.Email_Text = objEmailH.Email_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSNSalesOrder)
                ...
                objEmailH.SaveData(clsUserMgtCode.frmSNSalesOrder, objEmailH, Nothing)
                objEmailH = Nothing
                clsCommon.MyMessageBoxShow("E-Mail Send Successfully", Me.Text)
                If Not clsSMSAtPost_Sales.SMSATPOST_SALE() Then
                    SMSSENDONLY(False)
        ...
        Try
            ...
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                If clsCommon.myLen(dtContent.Rows(0)("SMS_Text")) > 0 Then
                    ...
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSNSalesOrder)
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.ContactPerson, strContactPerson)

                    objSMSH.SaveData(clsUserMgtCode.frmSNSalesOrder, objSMSH, Nothing)
                    objSMSH = Nothing
                    If Not isPost Then
                        clsCommon.MyMessageBoxShow("SMS Send Successfully", Me.Text)
                    End If
...
#End Region
    ...
    Private Sub chkclose_ToggleStateChanged(ByVal sender As System.Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles chkclose.ToggleStateChanged
        ...
        If clsSNSalesOrderHead.ClosedData(obj, txtDocNo.Value, txtCloseRemarks.Value) Then
            If chkclose.Checked Then
    ...
    Private Sub btnUpdateCommitedQty_Click(sender As Object, e As EventArgs) Handles btnUpdateCommitedQty.Click
        Try
            Dim chkstatus As String = Nothing
            Dim ChkDocNo As String = Nothing
            For Each grow As GridViewRowInfo In gv1.Rows

                If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colICode).Value)) > 0 AndAlso clsCommon.myCdbl(grow.Cells(colQty).Value) > 0 Then ' BM00000008026 

                    chkstatus = clsDBFuncationality.getSingleValue("select case when TSPL_SD_SHIPMENT_HEAD.Status=1 then 'Y' else 'N' end as Status  from TSPL_SD_SHIPMENT_DETAIL  left join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.Document_Code =TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE left join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.Document_Code =TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order  where TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order ='" + txtDocNo.Value + "' and TSPL_SD_SHIPMENT_DETAIL.Item_Code ='" + clsCommon.myCstr(grow.Cells(colICode).Value) + "'")

                    ChkDocNo = clsDBFuncationality.getSingleValue("select TSPL_SD_SHIPMENT_HEAD.Document_Code  from TSPL_SD_SHIPMENT_DETAIL  left join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.Document_Code =TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE left join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.Document_Code =TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order  where TSPL_SD_SHIPMENT_HEAD.Against_Sales_Order ='" + txtDocNo.Value + "' and TSPL_SD_SHIPMENT_DETAIL.Item_Code ='" + clsCommon.myCstr(grow.Cells(colICode).Value) + "'")
                    If clsCommon.CompairString(chkstatus, "Y") = CompairStringResult.Equal Then

                        ChkCommitedQty(grow.Index)
                        Dim qryCommitedUpdate As String = "update TSPL_SD_SALES_ORDER_DETAIL set CommitedQty ='" + clsCommon.myCstr(grow.Cells(colCommitedQty).Value) + "' , Iscommited ='" + clsCommon.myCstr(grow.Cells(colISCommited).Value) + "' where Document_Code ='" + txtDocNo.Value + "' and Item_Code ='" + clsCommon.myCstr(grow.Cells(colICode).Value) + "'"
                        clsDBFuncationality.ExecuteNonQuery(qryCommitedUpdate)

                        'End If
                    Else
                        Throw New Exception("Commited qty can't be change for item this '" + clsCommon.myCstr(grow.Cells(colIName).Value) + "'  before posting shipment('" + ChkDocNo + "') ") 'at line No : '" + clsCommon.myCstr(grow.Cells(colLineNo).Value) + "'
                    End If
            ...
            LoadData(txtDocNo.Value, NavigatorType.Current)
            clsCommon.MyMessageBoxShow("Update Successfuly")
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
