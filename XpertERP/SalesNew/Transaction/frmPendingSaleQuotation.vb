'-changes By--[Pankaj Kumar Chaudhary]--Against Ticket No -[BM00000002083]g
Imports common
Public Class frmPendingSaleQuotation
#Region "Variables"
    ...
    Public strDocumentType As String = Nothing
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        '" select TSPL_SD_SALES_Quotation_DETAIL.Document_Code as Code,TSPL_SD_SALES_Quotation_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_Quotation_DETAIL.Item_Code as ICode,TSPL_SD_SALES_Quotation_DETAIL.Item_Desc as IName,TSPL_SD_SALES_Quotation_DETAIL.Row_Type as IType,(TSPL_SD_SALES_Quotation_DETAIL.MRN_Qty +isnull(TSPL_SD_SALES_Quotation_DETAIL.Leak_Qty,0)+isnull(TSPL_SD_SALES_Quotation_DETAIL.Burst_Qty,0)+isnull(TSPL_SD_SALES_Quotation_DETAIL.Short_Qty,0)) as Qty,0 as Unapproved,TSPL_SD_SALES_Quotation_DETAIL.Unit_Code as Unit,TSPL_SD_SALES_Quotation_DETAIL.Location as Location,1 as RI,TSPL_SD_SALES_Quotation_DETAIL.Item_Cost as Rate,1 as Chk,TSPL_SD_SALES_Quotation_HEAD.Tax_Group,TSPL_SD_SALES_Quotation_DETAIL.TAX1_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX2_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX3_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX4_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX5_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX6_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX7_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX8_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX9_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX10_Rate,TSPL_SD_SALES_Quotation_DETAIL.MRP,TSPL_SD_SALES_Quotation_DETAIL.Batch_No,TSPL_SD_SALES_Quotation_DETAIL.MFG_Date,TSPL_SD_SALES_Quotation_DETAIL.Expiry_Date,TSPL_SD_SALES_Quotation_DETAIL.Disc_Per,MRN_Date as TransDate,TSPL_SD_SALES_Quotation_DETAIL.Assessable,TSPL_SD_SALES_Quotation_DETAIL.Leak_Qty,TSPL_SD_SALES_Quotation_DETAIL.Burst_Qty,TSPL_SD_SALES_Quotation_DETAIL.Short_Qty  from TSPL_SD_SALES_Quotation_DETAIL left outer join TSPL_SD_SALES_Quotation_HEAD on TSPL_SD_SALES_Quotation_HEAD.Document_Code=TSPL_SD_SALES_Quotation_DETAIL.Document_Code where TSPL_SD_SALES_Quotation_DETAIL.Status=0 and TSPL_SD_SALES_Quotation_HEAD.Status=1 "
        'If clsCommon.myLen(VendorCode) > 0 Then
        '    qry += " and TSPL_SD_SALES_Quotation_HEAD.Customer_Code='" + VendorCode + "'"
        'End If
        'qry += " union all" & _
        '" select TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code as Code,TSPL_SD_SALES_ORDER_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_ORDER_DETAIL.Item_Code as ICode,TSPL_SD_SALES_ORDER_DETAIL.Item_Desc as IName,'' as IType,(isnull(TSPL_SD_SALES_ORDER_DETAIL.SRN_Qty,0) +isnull(TSPL_SD_SALES_ORDER_DETAIL.Leak_Qty,0)+isnull(TSPL_SD_SALES_ORDER_DETAIL.Burst_Qty,0)+isnull(TSPL_SD_SALES_ORDER_DETAIL.Short_Qty,0)+isnull(TSPL_SD_SALES_ORDER_DETAIL.Rejected_Qty,0) ) as Qty,0 as Unapproved,TSPL_SD_SALES_ORDER_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,isnull(TSPL_SD_SALES_ORDER_DETAIL.MRP,0) as MRP,'' as Batch_No,null as MFG_Date,null as Expiry_Date,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALES_ORDER_DETAIL.Assessable,0)as Assessable,0 as Leak_Qty,0 as Burst_Qty,0 as Short_Qty from TSPL_SD_SALES_ORDER_DETAIL left outer join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.SRN_No=TSPL_SD_SALES_ORDER_DETAIL.SRN_No where TSPL_SD_SALES_ORDER_HEAD.Status=1 and len(isnull(TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code,''))>0  " & _
        '" union all  " & _
        '" select TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code as Code,TSPL_SD_SALES_ORDER_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_ORDER_DETAIL.Item_Code as ICode,TSPL_SD_SALES_ORDER_DETAIL.Item_Desc as IName,'' as IType,0  as Qty,(isnull(TSPL_SD_SALES_ORDER_DETAIL.SRN_Qty,0) +isnull(TSPL_SD_SALES_ORDER_DETAIL.Leak_Qty,0)+isnull(TSPL_SD_SALES_ORDER_DETAIL.Burst_Qty,0)+isnull(TSPL_SD_SALES_ORDER_DETAIL.Short_Qty,0)+isnull(TSPL_SD_SALES_ORDER_DETAIL.Rejected_Qty,0)) as Unapproved,TSPL_SD_SALES_ORDER_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,isnull(TSPL_SD_SALES_ORDER_DETAIL.MRP,0) as MRP,'' as Batch_No,null as MFG_Date,null as Expiry_Date,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALES_ORDER_DETAIL.Assessable,0) as Assessable,0 as Leak_Qty,0 as Burst_Qty,0 as Short_Qty from TSPL_SD_SALES_ORDER_DETAIL left outer join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.SRN_No=TSPL_SD_SALES_ORDER_DETAIL.SRN_No  where TSPL_SD_SALES_ORDER_HEAD.Status=0 and len(isnull(TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code,''))>0 and TSPL_SD_SALES_ORDER_DETAIL.SRN_No not in ('" + strCurrCode + "')   " & _
        ...
        Else
            StrCondition = ""
        End If
        '---------------------add FormType variables for export sale 17/11/2014 (Monika)----------------
        Dim qry As String = ""
        ...
        If strFormType IsNot Nothing AndAlso clsCommon.myLen(strFormType) > 0 AndAlso clsCommon.CompairString(strFormType, "EXPORT SO") = CompairStringResult.Equal Then
            ...
            qry += " select TSPL_SD_SALES_Quotation_DETAIL.Line_No,TSPL_SD_SALES_Quotation_DETAIL.Document_Code as Code,TSPL_SD_SALES_Quotation_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_Quotation_DETAIL.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName,TSPL_SD_SALES_Quotation_DETAIL.Row_Type as IType, TSPL_SD_SALES_Quotation_DETAIL.Qty  as Qty,0 as Unapproved,TSPL_SD_SALES_Quotation_DETAIL.Unit_Code as Unit,TSPL_SD_SALES_Quotation_DETAIL.Location as Location,1 as RI,TSPL_SD_SALES_Quotation_DETAIL.Item_Cost as Rate,1 as Chk,TSPL_SD_SALES_Quotation_HEAD.Tax_Group,TSPL_SD_SALES_Quotation_DETAIL.TAX1_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX2_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX3_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX4_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX5_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX6_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX7_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX8_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX9_Rate,TSPL_SD_SALES_Quotation_DETAIL.TAX10_Rate,TSPL_SD_SALES_Quotation_DETAIL.MRP ,TSPL_SD_SALES_Quotation_DETAIL.Disc_Per,Document_Date as TransDate,TSPL_SD_SALES_Quotation_DETAIL.Assessable " + Environment.NewLine
            qry += " ,Scheme_Code from TSPL_SD_SALES_Quotation_DETAIL " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALES_Quotation_HEAD on TSPL_SD_SALES_Quotation_HEAD.Document_Code=TSPL_SD_SALES_Quotation_DETAIL.Document_Code " + Environment.NewLine
            qry += " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_SD_SALES_Quotation_DETAIL.Item_Code" + Environment.NewLine
            qry += " where TSPL_SD_SALES_Quotation_DETAIL.Status=0 and Scheme_Item='N'  and TSPL_SD_SALES_Quotation_HEAD.Status=1 and TSPL_SD_SALES_Quotation_HEAD.Trans_Type='EXP'  " + Environment.NewLine
            If clsCommon.myLen(VendorCode) > 0 Then
                qry += " and TSPL_SD_SALES_Quotation_HEAD.Customer_Code='" + VendorCode + "'"
            End If
            If clsCommon.myLen(strDocumentType) > 0 Then
                qry += " and TSPL_SD_SALES_Quotation_HEAD.salesorder_type='" + strDocumentType + "' "
            End If
            qry += " union all " + Environment.NewLine
            qry += " select TSPL_SD_SALES_ORDER_DETAIL.Line_No,TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code as Code,TSPL_SD_SALES_ORDER_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_ORDER_DETAIL.Item_Code as ICode,'' as IName,'' as IType,isnull(TSPL_SD_SALES_ORDER_DETAIL.Qty,0) as Qty,0 as Unapproved,TSPL_SD_SALES_ORDER_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALES_ORDER_DETAIL.Assessable,0)as Assessable" + Environment.NewLine
            ...
            qry += " where TSPL_SD_SALES_ORDER_HEAD.Status=1 and TSPL_SD_SALES_ORDER_HEAD.trans_type='EXP' and Scheme_Item='N'  and len(isnull(TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code,''))>0   " + Environment.NewLine
            If clsCommon.myLen(strDocumentType) > 0 Then
                qry += " and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + strDocumentType + "' "
            End If
            qry += " union all   " + Environment.NewLine
            qry += " select TSPL_SD_SALES_ORDER_DETAIL.Line_No,TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code as Code,TSPL_SD_SALES_ORDER_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_ORDER_DETAIL.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,isnull(TSPL_SD_SALES_ORDER_DETAIL.Qty,0) as Unapproved,TSPL_SD_SALES_ORDER_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALES_ORDER_DETAIL.Assessable,0)as Assessable" + Environment.NewLine
            qry += " ,Scheme_Code from TSPL_SD_SALES_ORDER_DETAIL " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.Document_Code=TSPL_SD_SALES_ORDER_DETAIL.Document_Code" + Environment.NewLine
            qry += " where TSPL_SD_SALES_ORDER_HEAD.Status=0 and TSPL_SD_SALES_ORDER_HEAD.trans_type='EXP' and Scheme_Item='N'  and len(isnull(TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code,''))>0 and TSPL_SD_SALES_ORDER_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
            If clsCommon.myLen(strDocumentType) > 0 Then
                qry += " and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='" + strDocumentType + "' "
            End If
        ...
        Else
            ...
            qry += " left outer join TSPL_SD_SALES_Quotation_HEAD on TSPL_SD_SALES_Quotation_HEAD.Document_Code=TSPL_SD_SALES_Quotation_DETAIL.Document_Code " + Environment.NewLine
            qry += " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_SD_SALES_Quotation_DETAIL.Item_Code" + Environment.NewLine
            qry += " where TSPL_SD_SALES_Quotation_DETAIL.Status=0 and Scheme_Item='N'  and TSPL_SD_SALES_Quotation_HEAD.Status=1  " + Environment.NewLine
            If clsCommon.myLen(VendorCode) > 0 Then
                qry += " and TSPL_SD_SALES_Quotation_HEAD.Customer_Code='" + VendorCode + "'"
            End If
            qry += " union all " + Environment.NewLine
            ...
            qry += " ,Scheme_Code from TSPL_SD_SALES_ORDER_DETAIL " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.Document_Code=TSPL_SD_SALES_ORDER_DETAIL.Document_Code" + Environment.NewLine
            qry += " where TSPL_SD_SALES_ORDER_HEAD.Status=1 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code,''))>0   " + Environment.NewLine
            qry += " union all   " + Environment.NewLine
            qry += " select TSPL_SD_SALES_ORDER_DETAIL.Line_No,TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code as Code,TSPL_SD_SALES_ORDER_HEAD.Customer_Code as Vendor,TSPL_SD_SALES_ORDER_DETAIL.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,isnull(TSPL_SD_SALES_ORDER_DETAIL.Qty,0) as Unapproved,TSPL_SD_SALES_ORDER_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALES_ORDER_DETAIL.Assessable,0)as Assessable" + Environment.NewLine
            qry += " ,Scheme_Code from TSPL_SD_SALES_ORDER_DETAIL " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.Document_Code=TSPL_SD_SALES_ORDER_DETAIL.Document_Code" + Environment.NewLine
            qry += " where TSPL_SD_SALES_ORDER_HEAD.Status=0 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALES_ORDER_DETAIL.Quotation_Code,''))>0 and TSPL_SD_SALES_ORDER_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
            qry += " )Final " + Environment.NewLine
            qry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=final.Vendor " + Environment.NewLine
            qry += " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=Final.Location " + Environment.NewLine
            qry += " left outer join TSPL_TAX_GROUP_MASTER on TSPL_TAX_GROUP_MASTER.Tax_Group_Code=Final.Tax_Group and TSPL_TAX_GROUP_MASTER.Tax_Group_Type='S' " + StrCondition + " " + Environment.NewLine
