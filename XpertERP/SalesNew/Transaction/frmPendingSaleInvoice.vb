Imports common
Public Class frmPendingSaleInvoice
#Region "Variables"
    ...
    Public ArrReturn_EX As List(Of clsEXSaleInvoiceDetail) = Nothing
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            'qry += " where TSPL_SD_SALE_INVOICE_DETAIL.Status=0 and Scheme_Item='N'  and TSPL_SD_SALE_INVOICE_HEAD.Status=1 and Invoice_Type in ('T','R')  " + Environment.NewLine
            ...
            '    If clsCommon.myLen(strExport_Merchant) > 0 Then
            '        qry += "  and TSPL_SD_SALE_RETURN_HEAD.Document_Type='" + strExport_Merchant + "' "
            '    End If
            'End If
            'qry += " union all   " + Environment.NewLine
            ...
            'qry += " WHERE  CONVERT(DATE,Document_Date ,103) >= '" + strFromDate + "' and   CONVERT(DATE,Document_Date,103) <= '" + strToDate + "'  "
            ...
            '' 30-Nov-2015 (One Month Filtered Data)
            txtFromDate.Text = clsCommon.GETSERVERDATE().AddMonths(-1)
            txtToDate.Value = clsCommon.GETSERVERDATE()
            Dim strFromDate As String = clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy")
            Dim strToDate As String = clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy")
    ...
    Sub LoadData(ByVal strFromDate As String, ByVal strToDate As String, ByVal IsDetailON As String)
        Try
            ...
            'End If
            If clsCommon.myLen(strwherecls) > 0 Then
                StrCondition += " and TSPL_CUSTOMER_MASTER.Cust_Code IN (" + strwherecls + ")"
            End If
            If objCommonVar.ApplyLocationFilterBasedOnPermission = True AndAlso clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                StrCondition += " and TSPL_LOCATION_MASTER.Location_Code IN (" + objCommonVar.strCurrUserLocations + ")"
            End If
...
Left Outer Join (Select Item_Code, Conversion_Factor As 'ConvInQtl' from TSPL_ITEM_UOM_DETAIL Where Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code And UOM_Code='Qtl')CInQtl ON CInQtl.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code "
    ...
    Sub btnOKPressed()
        Try
            If clsCommon.CompairString(strDocType, "Export Sale Invoice") = CompairStringResult.Equal Then
                ArrReturn_EX = New List(Of clsEXSaleInvoiceDetail)
                Dim obj As clsEXSaleInvoiceDetail = Nothing
                For ii As Integer = 0 To gv1.RowCount - 1
                    If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                        obj = New clsEXSaleInvoiceDetail()
