...
#Region "Variables"
    ...
    Dim isInsideLoadForm As Boolean = False
    Dim strPdfAttachmentPath As String = ""
#End Region
    Public Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.frmSNSaleReturn)
                ...
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colDamageQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colUnit) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) Then
                        ...
                        ElseIf e.Column Is gv1.Columns(colICode) Then
                            ...
                            End If
                            If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal AndAlso clsCommon.myLen(txtReqNo.Value) = 0 Then
                                gv1.CurrentRow.Cells(colIsMannualAmt).Value = 1
                            End If
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) <= 0 Then
                If PurchaseOneItemOneVendor = True Then
                    ...
                    "where Customer_Code='" & txtVendorNo.Value & "' and TSPL_ITEM_MASTER.Active=1  "
                Else

                    qry = "select TSPL_CUSTOMER_ITEM_DETAIL.item_no as Item,TSPL_CUSTOMER_ITEM_DETAIL.item_desc as [ItemDesc],TSPL_CUSTOMER_ITEM_DETAIL.uom as Unit , " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.item_rate as BasicRate,TSPL_CUSTOMER_ITEM_DETAIL.Item_MRP as MRP, " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.Start_Date as [Start Date], " & _
                    "'' as [Vendor code],'' as [Vendor Desc],'' as PrincipleCode,'' as PrincipleDesc, " & _
                    "Weight_Value as [Weight Value] from TSPL_CUSTOMER_ITEM_DETAIL left outer join TSPL_ITEM_MASTER on " & _
            ...
            If clsCommon.myLen(pivotheader) > 0 Then
                If PurchaseOneItemOneVendor = True Then
                    ...
                    "where Customer_Code='" & txtVendorNo.Value & "' and TSPL_ITEM_MASTER.Active=1) as s pivot(max(cat_value) for description in (" + pivotheader + "))t  "
                Else
                    qry = "select * from (select a.DESCRIPTION,a.cat_value,TSPL_CUSTOMER_ITEM_DETAIL.item_no as Item,TSPL_CUSTOMER_ITEM_DETAIL.item_desc as [ItemDesc],TSPL_CUSTOMER_ITEM_DETAIL.uom as Unit , " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.item_rate as BasicRate,TSPL_CUSTOMER_ITEM_DETAIL.Item_MRP as MRP, " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.Start_Date as [Start Date], " & _
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("SNRSRG@34", qry)
        ...
        Try
            ...
            If (AllowToSave()) Then
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsSNSalesReturnDetail()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsSNSalesReturnHead()
            obj = clsSNSalesReturnHead.GetData(strCode, NavTyep)
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                End If
                ''
                If (clsSNSalesReturnHead.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsSNSalesReturnHead.DeleteData(txtDocNo.Value)) Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ...
            '-----------------------------------------------------
            Dim qst As String = "select count(*) from TSPL_SD_SALE_RETURN_HEAD where Document_Code='" + txtDocNo.Value + "' " + strcondition + ""
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        Dim qry As String = "select Document_Code as Code,CONVERT(varchar(10), Document_Date,103)+' '+ CONVERT(varchar(5), Document_Date,114) as Date,Customer_Code as [Customer Code], Customer_Name as Customer,Total_Amt as Amount,case when TSPL_SD_SALE_RETURN_HEAD.Status=0 then 'Pending' else 'Approved' end as [Status] ,TSPL_USER_MASTER.User_Name  as [User Name] from TSPL_SD_SALE_RETURN_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALE_RETURN_HEAD.Customer_Code left join TSPL_USER_MASTER on TSPL_USER_MASTER.User_Code =TSPL_SD_SALE_RETURN_HEAD.Created_By "
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") "
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ") "
        End If
        '-----------------------------------------------------
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating
        ...
        txtShipToLocation.Value = clsCommon.ShowSelectForm("ShipmentShindrlter", qry, "Code", "", txtShipToLocation.Value, "Code", isButtonClicked)
    ...
    Sub SelectMRNItems()
        ...
        If frm.ArrReturn IsNot Nothing AndAlso frm.ArrReturn.Count > 0 Then
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsSNInvoiceDetail In objOrderHead.Arr
    ...
    Function IsValidItem(ByVal obj As clsSNInvoiceDetail)
        If clsCommon.myLen(txtTaxGroup.Value) <= 0 Then
            ...
            ElseIf gv1.CurrentColumn Is gv1.Columns(colComplete) AndAlso UsLock1.Status = ERPTransactionStatus.Approved Then
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 AndAlso clsCommon.myLen(strICode) > 0 AndAlso intSNo > 0 AndAlso clsCommon.CompairString(strStatus, "No") = CompairStringResult.Equal Then
                    If common.clsCommon.MyMessageBoxShow("Do you want to complete the item " + clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value), Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                        If clsSNSalesReturnDetail.CompleteSRN(txtDocNo.Value, strICode, intSNo) Then
    ...
    Private Sub funPrint(Optional ByVal IsPdf As Boolean = False)
        ...
        Try
            ...
            Dim Qry As String = "  select TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_No ,convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103)  as Sale_Invoice_Date,TSPL_SD_SALE_RETURN_DETAIL.Item_Net_Amt as Total_Item_Net_amt,TSPL_SD_SALE_RETURN_DETAIL.disc_Amt," & _
                                ...
                                " tax7.type as TaxType7,tax8.type as TaxType8,tax9.type as TaxType9,tax10.type as TaxType10,TSPL_SD_SALE_RETURN_DETAIL.TAX1_Amt as dTAX1_Amt ,  " & _
                                  "  TSPL_SD_SALE_RETURN_DETAIL.TAX2_Amt as dTAX2_Amt  ,TSPL_SD_SALE_RETURN_DETAIL.TAX3_Amt as dTAX3_Amt ,TSPL_SD_SALE_RETURN_DETAIL.TAX4_Amt as dTAX4_Amt  ,  " & _
                                 " TSPL_SD_SALE_RETURN_DETAIL.TAX5_Amt as dTAX5_Amt  ,TSPL_SD_SALE_RETURN_DETAIL.TAX6_Amt as dTAX6_Amt ,TSPL_SD_SALE_RETURN_DETAIL.TAX7_Amt as dTAX7_Amt , " & _
                                 " TSPL_SD_SALE_RETURN_DETAIL.TAX8_Amt as dTAX8_Amt ,TSPL_SD_SALE_RETURN_DETAIL.TAX9_Amt as dTAX9_Amt ,TSPL_SD_SALE_RETURN_DETAIL.TAX10_Amt as dTAX10_Amt , " & _
                                 " TSPL_SD_SALE_RETURN_DETAIL.TAX1_Rate as dTAX1_Rate ,TSPL_SD_SALE_RETURN_DETAIL.TAX2_Rate as dTAX2_Rate ,TSPL_SD_SALE_RETURN_DETAIL.TAX3_Rate as dTAX3_Rate , " & _
                                 " TSPL_SD_SALE_RETURN_DETAIL.TAX4_Rate as dTAX4_Rate ,TSPL_SD_SALE_RETURN_DETAIL.TAX5_Rate as dTAX5_Rate ,TSPL_SD_SALE_RETURN_DETAIL.TAX6_Rate as dTAX6_Rate , " & _
                                 " TSPL_SD_SALE_RETURN_DETAIL.TAX7_Rate as dTAX7_Rate ,TSPL_SD_SALE_RETURN_DETAIL.TAX8_Rate as dTAX8_Rate ,TSPL_CUSTOMER_MASTER.State as Cust_Sate_Code , " & _
                                 ...
                                 " Loc_State.STATE_NAME as Loc_State_Name, Loc_State.GST_STATE_Code as Loc_GST_State_Code,TSPL_SD_SALE_RETURN_HEAD.Is_Taxable , Cust_State.Is_GST_UT as Cust_Is_UT, " & _
                                 " Loc_State.Is_GST_UT as Loc_Is_UT, TSPL_LOCATION_MASTER.Location_Code as Loc_Code,TSPL_LOCATION_MASTER.Location_Desc , TSPL_CUSTOMER_MASTER.Cust_Code, " & _
                                 " TSPL_CUSTOMER_MASTER.Add1 as Cust_Add1,TSPL_CUSTOMER_MASTER.Add2 as Cust_Add2,TSPL_CUSTOMER_MASTER.Add3 as Cust_Add3, TSPL_CUSTOMER_MASTER.PAN as Cust_Pan_no," & _
                                " convert(varchar,TSPL_SD_SALE_RETURN_HEAD.Document_Date,103) as Document_Date, Cust_City.City_Name , TSPL_ITEM_MASTER .HSN_Code  , " & _
                                " TSPL_SD_SALE_RETURN_HEAD.Price_code ,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt1 , TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt2," & _
                                " TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt3,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt4,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt5,  TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt6," & _
                                " TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt7,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt8, TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt9, " & _
                                 " TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Amt10,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name1 , TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name2 , " & _
                                " TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name3 ,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name4 ,  TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name5 , " & _
                            " TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name6 ,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name7 , TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name8 ," & _
                             " TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name9 ,TSPL_SD_SALE_RETURN_HEAD.Add_Charge_Name10  ,'1' as CopyType ," & _
                            " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add1  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add1 end as P_Add1, " & _
            ...
            " case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Customer_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_name    " & _
            " end as P_Cust_Name,  case when coalesce(p_cust.P_cust_code,'')='' then Cust_City    .City_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_City_Name   " & _
            " end as P_City_Name,  case when coalesce(p_cust.P_cust_code,'')='' then Cust_State .state_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_state_Name  " & _
             " end as P_State_Name, case when coalesce(p_cust.P_cust_code,'')='' then     case when ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')='(+__)__________' then '' " & _
            " else TSPL_CUSTOMER_MASTER.Phone1 end +  Case When   ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_CUSTOMER_MASTER.Phone2 Else'' End " & _
             " when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Phn    end as P_Cust_Phn, case when coalesce(p_cust.P_Pan,'')='' then TSPL_CUSTOMER_MASTER  .PAN      " & _
            " when coalesce(p_cust.P_Pan,'')<>'' then p_cust.P_Pan   end as P_Pan, TSPL_LOCATION_MASTER .GSTNO as Loc_GSTIN, TSPL_CUSTOMER_MASTER.GSTNO as Cust_GSTIN, " & _
            " TSPL_SD_SALE_RETURN_HEAD.Total_Amt,TSPL_TAX_GROUP_MASTER.Is_Tax_Exempted , TSPL_SD_SALE_RETURN_DETAIL.line_no, TSPL_SD_SALE_INVOICE_HEAD.ewaybillno, " & _
            "  convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.ewaybillDate,103) as ewaybillDate, TSPL_SD_SALE_INVOICE_HEAD.electronic_ref_no,  " & _
            "   '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate, TSPL_SD_SALE_RETURN_HEAD.Inv_No, TSPL_SD_SALE_RETURN_HEAD.Dept_Desc ,   TSPL_SD_SALE_RETURN_HEAD.Terms_Code,TSPL_SD_SALE_RETURN_HEAD.VehicleNo , "
            ...
            Qry += " TSPL_SD_SALE_RETURN_HEAD.Against_Invoice_No, TSPL_SD_SALE_INVOICE_HEAD.Total_Amt as SaleInvoiceAmt , TSPL_SD_SALE_RETURN_HEAD.Item_Type ,  TSPL_SD_SALE_RETURN_HEAD.Customer_Code, "
            Qry += " TSPL_SD_SALE_RETURN_HEAD .Terms_Code as termscode ,TSPL_SD_SALE_RETURN_HEAD .Ref_No as ref_no ,"
            ...
            Qry += " TSPL_SD_SALE_RETURN_DETAIL.item_code as item_code, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_SD_SALE_RETURN_DETAIL.Row_Type,TSPL_SD_SALE_RETURN_DETAIL.Qty as qty,TSPL_SD_SALE_RETURN_DETAIL.unit_code as uom,TSPL_SD_SALE_RETURN_DETAIL.item_cost as itemcost,TSPL_SD_SALE_RETURN_HEAD.TAX1,TSPL_SD_SALE_RETURN_HEAD.TAX2,TSPL_SD_SALE_RETURN_HEAD.TAX3,TSPL_SD_SALE_RETURN_HEAD.TAX4,TSPL_SD_SALE_RETURN_HEAD.TAX5,TSPL_SD_SALE_RETURN_HEAD.Total_Add_Charge, "
            ...
            Qry += " left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_SD_SALE_RETURN_DETAIL.tax2  "
            Qry += " left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_SD_SALE_RETURN_DETAIL .TAX3  "
            Qry += " left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_SD_SALE_RETURN_DETAIL .tax4  "
            Qry += " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_SD_SALE_RETURN_DETAIL .tax5  "
            Qry += " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL .TAX6  "
            Qry += " left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL .TAX7  "
            Qry += " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL .TAX8  "
            Qry += " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL .TAX9 "
            Qry += " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL .TAX10     "
        ...
        Try
            If e.Column.Index >= 0 Then
                ...
                ElseIf e.Column Is gv1.Columns(colRate) Then
                    If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                        If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                            Me.gv1.Columns(colRate).ReadOnly = False
                        Else
                    ...
                    '    Me.gv1.Columns(colRate).ReadOnly = False
                    'Else
                    '    Me.gv1.Columns(colRate).ReadOnly = True
                    'End If
        ...
        Try
            ...
            If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                If dblConvF = 1 And dblOrgBasicRate Then
                    dblConvBasicRate = dblRate
                Else
                    dblConvBasicRate = dblRate * dblConvF
                End If
            Else
                If dblConvF = 1 And dblOrgBasicRate Then
            ...
            Dim dblBasicAmt As Double = 0 'dblQty * dblConvBasicRate
            'dblBasicRate = dblConvBasicRate
            'dblRate = dblBasicRate

            Dim dblReturnAmt As Double = 0 ' (dblQty * dblRate)
            Dim dblDamageAmt As Double = 0 '(dblDamageQty * dblRate)
            Dim dblAmt As Double = dblReturnAmt + dblDamageAmt ''+ dblFAmt
            If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                dblBasicAmt = dblConvBasicRate
                dblBasicRate = dblConvBasicRate
                dblRate = dblConvBasicRate
                dblReturnAmt = dblConvBasicRate
                dblDamageAmt = 0
                dblAmt = dblBasicAmt
            Else
                dblBasicAmt = dblQty * dblConvBasicRate
                dblBasicRate = dblConvBasicRate
                dblRate = dblBasicRate
                dblReturnAmt = (dblQty * dblRate)
                dblDamageAmt = (dblDamageQty * dblRate)
                dblAmt = dblReturnAmt + dblDamageAmt
            End If
            If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                gv1.Rows(IntRowNo).Cells(colAmt).Value = dblAmt
                gv1.Rows(IntRowNo).Cells(colActualCost).Value = dblAmt
                gv1.Rows(IntRowNo).Cells(colReturnAmt).Value = Math.Round(dblAmt, 2)
            Else
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("SRSaleman", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
...
#Region "Email SMS Setting"
    Private Sub RadMenuItem5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem5.Click
        ...
        frm.FormId = clsUserMgtCode.frmSNSaleReturn
        frm.ShowDialog()
    End Sub
    ...
    Private Sub SendSMSandEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        Try
            ...
            'sanjay
            Dim strContactperson As String = ""
            Dim dtContent As DataTable = clsDBFuncationality.GetDataTable("SELECT SMS_Text,Email_Text,Email_subject from TSPL_ES_Content where Form_ID='" + clsUserMgtCode.frmSNSaleReturn + "'", Nothing)
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                objEmailH.Email_Text = objEmailH.Email_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSNSaleReturn)
                ...
                objEmailH.SaveData(clsUserMgtCode.frmSNSaleReturn, objEmailH, Nothing)
                objEmailH = Nothing
                clsCommon.MyMessageBoxShow("E-Mail Send Successfully", Me.Text)
                If Not clsSMSAtPost_Sales.SMSATPOST_SALE() Then
                    SMSSENDONLY(False)
        ...
        Try
            ...
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                If clsCommon.myLen(dtContent.Rows(0)("SMS_Text")) > 0 Then
                    ...
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSNSaleReturn)
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.ContactPerson, strContactPerson)

                    objSMSH.SaveData(clsUserMgtCode.frmSNSaleReturn, objSMSH, Nothing)
                    objSMSH = Nothing
                    If Not isPost Then
                        clsCommon.MyMessageBoxShow("SMS Send Successfully", Me.Text)
                    End If
...
#End Region
    ...
    '    If isInsideLoadForm = False Then
    '        '    If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
    '        '        Me.gv1.Columns(colRate).ReadOnly = False                
    '        '    Else
    '        '        Me.gv1.Columns(colRate).ReadOnly = True
