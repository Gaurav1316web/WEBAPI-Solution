...
#End Region
    ...
    Sub LoadBlankGridPurQCParameter()
        ...
        txtBox.HeaderImage = Global.ERP.My.Resources.Resources.search4
        ...
        Try
            If AllowToSave() Then
                ...
                For ii As Integer = 0 To gvCategory.Rows.Count - 1
                    ...
                    'If clsCommon.CompairString(btnSave.Text, "Save") = CompairStringResult.Equal AndAlso txtCode.MyReadOnly = True Then
                    '    ''richa agarwal 13/10/2014 code generated by full category value
                    '    'If clsCommon.myLen(objItemCatg.Item_Cagetory_Values) >= 2 Then
                    '    '    txtCode.Value = txtCode.Value + "" + auto_icode_seperator + "" + clsCommon.myCstr(objItemCatg.Item_Cagetory_Values.Substring(0, 2))
                    '    'Else
                    '    txtCode.Value = txtCode.Value + "" + auto_icode_seperator + "" + clsCommon.myCstr(objItemCatg.Item_Cagetory_Values)
                    '    'End If
                    '    ''==========================
                    '    txtDesc.Text = txtDesc.Text + "" + auto_iname_seperator + "" + clsCommon.myCstr(gvCategory.Rows(ii).Cells(CatcolValueDesc).Value)
                    'End If
                ...
                If obj.SaveDataRMOther(obj, GetDatabase(), isNewEntry) Then
                    common.clsCommon.MyMessageBoxShow("Data Saved Successfully", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Info)
    ...
    Private Function AllowToSave() As Boolean
        Try
            ...
            '    RadPageView1.SelectedPage = RadPageViewPage1
            '    common.clsCommon.MyMessageBoxShow("Please Enter item Code", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
            '    txtCode.Focus()
            '    Return False
            'Else
            If clsCommon.myLen(txtDesc.Text) <= 0 AndAlso txtDesc.ReadOnly = False Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please Enter Item Description", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtDesc.Focus()
                Return False
            ElseIf chkFresh.Checked = True AndAlso chkAmbient.Checked = True AndAlso Not (chkFresh.Enabled = False AndAlso chkAmbient.Enabled = False) Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("You cannot select Fresh item/Ambient at a time, Please select either fresh Item or Ambient.", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                Return False
                ...
                'RadPageView1.SelectedPage = RadPageViewPage1
                'common.clsCommon.MyMessageBoxShow("Please select either fresh Item or Ambient.", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                'Return False

                '' Anubhooti 10-Sep-2014 BM00000003847
            ElseIf chkFresh.Checked = True AndAlso clsCommon.myLen(txtShortDescription.Text) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please fill short description", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtShortDescription.Focus()
                Return False
            ElseIf ShortDesp > 0 AndAlso AllowDuplicateItemShortDescriptionInItemMaster = False Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please check ! short description should not be duplicate", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtShortDescription.Focus()
                Return False
            ElseIf clsCommon.myLen(txtStructurer.Value) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please select Structure Code", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtStructurer.Focus()
                Return False
            ElseIf clsCommon.myLen(txtPurchaseACSet.Value) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please select Purchase Account set", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtPurchaseACSet.Focus()
                Return False
            ElseIf clsCommon.myLen(txtSaleAcSet.Value) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please select Sale Account set", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtSaleAcSet.Focus()
                Return False
                'ElseIf clsCommon.myLen(txtCategory.Value) <= 0 Then
                '    common.clsCommon.MyMessageBoxShow("Please select Category", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                '    txtCategory.Focus()
                '    Return False
                'ElseIf clsCommon.myLen(txtSubCategory.Value) <= 0 Then
                '    common.clsCommon.MyMessageBoxShow("Please select Sub category", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                '    txtSubCategory.Focus()
                '    Return False

            ElseIf clsCommon.myLen(cboItemType.SelectedValue) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please select Item Type", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                cboItemType.Focus()
                Return False
            ElseIf clsCommon.myLen(cboItemType.SelectedValue) > 0 AndAlso clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.AutoItemNLevel, clsFixedParameterCode.AutoItemNLevel, Nothing)) = 1 Then
                ...
                If isPrefixExit = False Then
                    RadPageView1.SelectedPage = RadPageViewPage1
                    common.clsCommon.MyMessageBoxShow("Please Enter Prefix of Item Type (" + cboItemType.Text + ") in Inventory Setting screen.", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                    cboItemType.Focus()
                    Return False
                End If
            ElseIf clsCommon.CompairString(cboItemType.SelectedValue, "F") = CompairStringResult.Equal AndAlso clsCommon.myLen(txtWeightUOM.Value) = 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please select Weight UOM", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtWeightUOM.Focus()
                Return False
            ElseIf clsCommon.CompairString(cboItemType.SelectedValue, "F") = CompairStringResult.Equal AndAlso clsCommon.myCdbl(txtWeightValue.Value) = 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow("Please select Weight Value", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtWeightValue.Focus()
                Return False
                'ElseIf clsCommon.myLen(cmbUsedAs.SelectedValue) <= 0 Then
                '    RadPageView1.SelectedPage = RadPageViewPage1
                '    common.clsCommon.MyMessageBoxShow("Please select Used as", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                '    cmbUsedAs.Focus()
                '    Return False
                'ElseIf clsCommon.myLen(cboType.SelectedValue) <= 0 Then
                '    common.clsCommon.MyMessageBoxShow("Please select Type", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                '    cboType.Focus()
                '    Return False
                'ElseIf CreateGLAccToItem = True AndAlso clsCommon.myLen(fndGLAcc.Value) <= 0 Then
                '    RadPageView1.SelectedPage = RadPageViewPage1   
                '    common.clsCommon.MyMessageBoxShow("Please select GL account", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                '    fndGLAcc.Focus()
                '    Return False
            End If
            ...
            If clsCommon.CompairString(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.INDUSTRYTYPE, clsFixedParameterCode.INDUSTRYTYPE, Nothing)), "A") = CompairStringResult.Equal Then
                If clsCommon.myLen(txtWarranty.Value) > 0 AndAlso clsCommon.myLen(CmbWarrApp.SelectedValue) <= 0 Then
                    ...
                    common.clsCommon.MyMessageBoxShow("Please select warranty applied from", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                    CmbWarrApp.Focus()
                    Return False
                End If
            End If
            ...
            If ShelfLifeManadatoryOnFG Then
                If clsCommon.myLen(txt_shelflife.Text) <= 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(cboItemType.SelectedValue), "F") = CompairStringResult.Equal Then
                    common.clsCommon.MyMessageBoxShow("Please enter Shelf life", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                    txt_shelflife.Focus()
                    Return False
                End If
            End If
            ...
            If ToleranceMandatoryFor_RM_Other_Trade AndAlso clsCommon.myCdbl(txt_tolerance.Text) = 0 AndAlso (clsCommon.CompairString(cboItemType.SelectedValue, "R") = CompairStringResult.Equal OrElse clsCommon.CompairString(cboItemType.SelectedValue, "O") = CompairStringResult.Equal OrElse clsCommon.CompairString(cboItemType.SelectedValue, "T") = CompairStringResult.Equal) Then
                ...
                clsCommon.MyMessageBoxShow("Please fill tolerance%.")
                Return False
            End If
            ''=========================================================================================================
            If TxtProdTolerance.Value < 0 Then
                ...
                clsCommon.MyMessageBoxShow("Production Tolerance should be non negative.")
                Return False
            End If

            If chkIsSerailzedInventory.Checked = True Then
                If clsCommon.myLen(txtNextAutoSerialCounter.Text) < 4 Then
                    clsCommon.MyMessageBoxShow("Please check Next Auto Serial No.Its Length should be minimum 4.")
                    Return False
                ElseIf clsCommon.CompairString(clsCommon.myCstr(cmbUsedAs.SelectedValue), "S") = CompairStringResult.Equal And chkIsSerailzedInventory.Checked = True Then
                    clsCommon.MyMessageBoxShow("You can either make Item serialized or Mcc Sale(used as).")
                    Return False
                End If
            End If
            ...
            If chkScrapItem.Checked = True Then
                If clsCommon.myLen(fndScrapItem.Value) <= 0 Then
                    clsCommon.MyMessageBoxShow("Please fill Scrap Item.")
                    fndScrapItem.Focus()
                    Return False
                End If
                If clsCommon.myLen(fndScrapItem.Value) > 0 Then
                    ...
                    If chkStructCode <= 0 Then
                        clsCommon.MyMessageBoxShow("Structure Code of Item Code and Scrap Item Should be Same.")
                        fndScrapItem.Value = ""
                        fndScrapItem.Focus()
                        Return False
                    End If
            ...
            If True Then
                ...
                If chkIsSerailzedInventory.Checked Then
                    If clsCommon.myLen(txtNextAutoSerialCounter.Text) <= 0 Then
                        RadPageView1.SelectedPage = pageviewSerializedIinvenoty
                        common.clsCommon.MyMessageBoxShow("Please Enter Next Serialzed counter", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                    ...
                    If clsCommon.CompairString(txtNextAutoSerialCounter.Text, clsCommon.incval(txtNextAutoSerialCounter.Text)) = CompairStringResult.Equal Then
                        RadPageView1.SelectedPage = pageviewSerializedIinvenoty
                        common.clsCommon.MyMessageBoxShow("Not a valid Serialzed counter", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                        txtNextAutoSerialCounter.Focus()
                        Return False
                    End If
                    If clsCommon.myLen(txtWarranty.Value) <= 0 Then
                        RadPageView1.SelectedPage = pageviewSerializedIinvenoty
                        common.clsCommon.MyMessageBoxShow("Please select warranty", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                        txtWarranty.Focus()
                        Return False
                    End If
                    ...
                    If isMultipleUOM Then
                        RadPageView1.SelectedPage = pageviewSerializedIinvenoty
                        common.clsCommon.MyMessageBoxShow("Multiple UOM is not applicable for serial item", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                ...
                If clsCommon.myLen(txtCode.Value) > 0 Then
                    ...
                    If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                        ...
                        If Not isSerialInventoryOLD = chkIsSerailzedInventory.Checked Then
                            RadPageView1.SelectedPage = pageviewSerializedIinvenoty
                            common.clsCommon.MyMessageBoxShow("Can't change the serialization because item is in use.", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                            chkIsSerailzedInventory.Focus()
                            Return False
                        End If
                    End If
            ...
            If clsCommon.myLen(txtUOM.Value) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage2
                common.clsCommon.MyMessageBoxShow("Please select UOM", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                txtUOM.Focus()
                Return False
            End If
            ...
            If clsCommon.myLen(txtCategoryStructureCode.Value) > 0 Then
                ...
                For ii = 0 To gvCategory.Rows.Count - 1
                    ...
                    If ii = 0 AndAlso clsCommon.myLen(CatcolCode) <= 0 Then
                        RadPageView1.SelectedPage = RadPageViewPage4
                        clsCommon.MyMessageBoxShow("Please fill category code in Category Structure at row no. " + clsCommon.myCstr(ij) + "", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                        Return False
                    End If
                    If clsCommon.myLen(categcode) > 0 AndAlso clsCommon.myLen(valuecatcode) <= 0 Then
                        RadPageView1.SelectedPage = RadPageViewPage4
                        clsCommon.MyMessageBoxShow("Please fill category values in Category Structure at row no. " + clsCommon.myCstr(ij) + "", Me.Text, MessageBoxButtons.OK, RadMessageIcon.Error)
                        Return False
                    End If
                Next
            ...
            If clsCommon.CompairString(fndProductType.SelectedValue, Product_Type) <> CompairStringResult.Equal Then
                If clsCommon.CompairString(Product_Type, "MI") = CompairStringResult.Equal Then
                    ...
                    If totalCount > 0 Then
                        clsCommon.MyMessageBoxShow("Product Type of this item can not be changed because some transactions are already done for this item in Product Type Milk.")
                        Return False
                    End If
                ElseIf clsCommon.CompairString(fndProductType.SelectedValue, "MI") = CompairStringResult.Equal Then
                    ...
                    Dim totalCount As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry))
                    If totalCount > 0 Then
                        clsCommon.MyMessageBoxShow("Product Type of this item can not be changed because some transactions are already done for this item.")
                        Return False
                    End If
                End If
            End If
                ...
                If clsCommon.myLen(gvUOM.Rows(i).Cells(UOMColUnit).Value) > 0 Then
                    ...
                    For j As Integer = i + 1 To gvUOM.Rows.Count - 1
                        ...
                        If UOM = SecondUOM Then
                            clsCommon.MyMessageBoxShow("Please check ! duplicate UOM in grid")
                            Return False
                        End If
                    Next
                End If
            ...
            If clsCommon.CompairString(clsCommon.myCstr(fndProductType.SelectedValue), "PS") = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(fndProductType.SelectedValue), "MS") = CompairStringResult.Equal Then
                If Not chkIsSerailzedInventory.Checked Then
                    clsCommon.MyMessageBoxShow("When Item Type is Paper Seal Or Manual Seal, It Must be serialized")
                    Return False
                End If
            End If
            ...
            If clsCommon.CompairString(clsCommon.myCstr(fndProductType.SelectedValue), "PS") = CompairStringResult.Equal Then
                ...
                If recCount > 0 Then
                    clsCommon.MyMessageBoxShow("Paper Seal Type of item can not be created twice")
                    Return False
                End If
            End If
            If clsCommon.CompairString(clsCommon.myCstr(fndProductType.SelectedValue), "MS") = CompairStringResult.Equal Then
                ...
                If recCount > 0 Then
                    clsCommon.MyMessageBoxShow("Manual Seal Type of item can not be created twice")
                    Return False
                End If
            End If
            ...
            If clsCommon.myLen(txtSeqNo.Text) > 0 And clsCommon.myCdbl(txtSeqNo.Text) > 0 Then
                ...
                If recCount > 0 Then
                    clsCommon.MyMessageBoxShow("Duplicate Sequence No")
                    Return False
                End If
            End If
            ...
            If clsCommon.myLen(txtMarSeqNo.Text) > 0 And clsCommon.myCdbl(txtMarSeqNo.Text) > 0 Then
                ...
                If recCount > 0 Then
                    clsCommon.MyMessageBoxShow("Duplicate Marketing Sequence No")
                    Return False
                End If
            End If
        ...
        If clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry)) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowItemWiseCSAAccountingON_CSASale, clsFixedParameterCode.AllowItemWiseCSAAccountingON_CSASale, Nothing)), "1") = CompairStringResult.Equal Then
            If clsCommon.myLen(fndCSA_AC_Code.Value) <= 0 Then
                ...
                clsCommon.MyMessageBoxShow("Please select CSA Account Set.")
                Return False
            End If
        End If
        '==========================================================================
        If AllowGSTApplicable = True Then
            ...
            If chkTaxable.Checked = True Then
                If clsCommon.myLen(FndHSNCode.Value) <= 0 Then
                    clsCommon.MyMessageBoxShow("Please Fill HSN Code.")
                    FndHSNCode.Focus()
    ...
    Public Function deletedata() As Boolean
        If txtCode.Value <> "" Then
            ...
            End If
        Else
            common.clsCommon.MyMessageBoxShow("You Can Not delete The Record")
        End If
    End Function
    ...
    Private Sub gvUOM_UserDeletingRow(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.GridViewRowCancelEventArgs) Handles gvUOM.UserDeletingRow
        If clsCommon.myCdbl(gvUOM.CurrentRow.Cells(UOMColStockUnitChangable).Value) = 1 Then
            If Not chkDoNotCheckOnSave.Checked Then
                clsCommon.MyMessageBoxShow("The Item '" + txtCode.Value + "' with UOM '" + clsCommon.myCstr(gvUOM.CurrentRow.Cells(UOMColUnit).Value) + "' is in use.")
                e.Cancel = True
            End If
        End If
    End Sub
        ...
        Try
            ...
            If txtCategory.Enabled = False Then
                ...
                If clsCommon.myLen(code) <= 0 Then
                    If Not clsCommon.MyMessageBoxShow("Are You Sure,Want To Export Item Detail Without Any Category Filter?", "Attention", MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
                        Return
                    End If
                Else
    ...
    Private Sub rmiExportCategoryStructure_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rmiExportCategoryStructure.Click
        Try
            qry = "select  TSPL_ITEM_MASTER.Item_Code as [Item Code], TSPL_ITEM_CATEGORY_STRUCT_DETAIL.ITEM_CATEGORY_STRUCT_CODE as [Category Structure],TSPL_ITEM_CATEGORY_STRUCT_DETAIL.line_no, TSPL_ITEM_CATEGORY_STRUCT_DETAIL.ITEM_CATEGORY_CODE as [Category Code], TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values as [Category Value] from TSPL_ITEM_MASTER " & _
            " LEFT OUTER JOIN TSPL_ITEM_CATEGORY_STRUCTURE ON TSPL_ITEM_CATEGORY_STRUCTURE.ITEM_CATEGORY_STRUCT_CODE=TSPL_ITEM_MASTER.Item_Category_Struct_Code and isnull(TSPL_ITEM_CATEGORY_STRUCTURE.form_type,'item')='item'" & _
            ...
            transportSql.ExporttoExcel(qry, "", orderByCls, Me)
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Exclamation, "UOM")
        End Try
    End Sub
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    ...
                    qry = " update TSPL_ITEM_MASTER set STD_FatPer=QC.Fat_Per,STD_SNFPer=QC.SNF_Per from (select Item_Code,MAX(Fat_Per) as Fat_Per,MAX(SNF_Per) as SNF_Per from ( select Item_QCP.Item_Code,Item_QCP.Code as Parameter_Code,(case when QCP.Type='FAT' then Item_QCP.Actual_Range else 0 end) as Fat_Per, " & _
                     " (case when QCP.Type='SNF' then Item_QCP.Actual_Range else 0  end) as SNF_Per from TSPL_ITEM_QC_PARAMETER_MASTER Item_QCP  left join TSPL_PARAMETER_MASTER QCP  on Item_QCP.Code=QCP.Code where Item_QCP.Item_Code='" & Item_Code & "') as QC  group by Item_Code) QC " & _
    ...
    Sub LoadItemCSAType()
        ...
        qry = "select distinct code,Name  from( " & _
                " select 'None' as Code,'None' as Name "
        If AllowDoNotShowDairyTypeItems = False Then
            qry += " union all select 'CPD-DESI GHEE' as Code,'CPD-DESI GHEE' as Name " & _
               " union all select 'BULK -DESI GHEE' as Code,'BULK-DESI GHEE' as Name" & _
               " union all select 'CPD-OTHER' as Code,'CPD-OTHER' as Name " & _
               " union all select 'BULK-OTHER' as Code,'BULK-OTHER' as Name" & _
...
ExitLOOP:
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtCode.Value) <= 0 Then
                clsCommon.MyMessageBoxShow("Select Item Code")
                Exit Sub
            End If
    ...
    Sub OpenPurchaseQCParameters(ByVal isButtonClicked As Boolean)
        Dim qry As String = "select Code,Description from TSPL_QC_LOG_SHEET_MASTER where nature='A'"
    ...
    Private Sub txtItemCopy__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtItemCopy._MYValidating
        Try
            Dim str As String = "select count(*) from tspl_item_master where item_code ='" + txtCode.Value + "' "
            Dim no As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(str))
            If no = 0 Then
            ...
            If txtCode.MyReadOnly OrElse isButtonClicked Then
                txtCode.Value = clsItemMaster.getFinderForActiveAndIncative("", txtCode.Value, isButtonClicked) 'Item_Type in ('R','O','A','F')
                LoadData(txtCode.Value, NavigatorType.Current)
                txtItemCopy.Value = txtCode.Value
                txtCode.Value = ""
                txtDesc.Text = ""
                txtShortDescription.Text = ""
                txtAliesName.Text = ""
