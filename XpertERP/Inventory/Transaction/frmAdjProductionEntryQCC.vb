...
Imports common
Public Class frmAdjProductionEntryQCC
...
#End Region
    ...
    Sub LoadQCStatus()
        ...
        cmbQCStatus.DataSource = dt
        cmbQCStatus.ValueMember = "Code"
        cmbQCStatus.DisplayMember = "Name"
    End Sub
    ...
    Private Function AllowToSave() As Boolean
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            ...
            End If
            'Batch Qty Check
            If clsCommon.myLen(strQCStatus) <= 0 Then
                Throw New Exception("For Item : " + strICode + " Please Select QC Status.At Line No" + clsCommon.myCstr(ii + 1))
            End If
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Adjustment_No) > 0) Then
                ...
                txtProductionEntry.Value = obj.Production_Entry
    ...
    Sub PostData()
        ...
        Try
            ...
            If clsCommon.myLen(txtAdjustmentNo.Value) > 0 Then
                Doc_Code = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select COUNT(*) AS adjustment_no from TSPL_ADJUSTMENT_HEADER_QC  where TSPL_ADJUSTMENT_HEADER_QC.adjustment_no='" + txtAdjustmentNo.Value + "'"))
                If Doc_Code > 0 Then
                    If (myMessages.postConfirm()) Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtAdjustmentNo._MYNavigator
        Try
            Dim qst As String = "select count(*) from TSPL_ADJUSTMENT_HEADER_QC where Adjustment_No='" + txtAdjustmentNo.Value + "'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtAdjustmentNo._MYValidating
        ...
        Dim qry As String = "SELECT Adjustment_No AS [AdjustmentNumber],CONVERT(varchar(10), TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Date,103)+' '+ CONVERT(varchar(5), TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Date,114) as [Date],case when Posted='Y' then 'Yes' else 'No' end as Posted" &
            " ,TSPL_ADJUSTMENT_HEADER_QC.Production_Entry as [Against Production Entry] " &
            " FROM  TSPL_ADJUSTMENT_HEADER_QC  "
        ...
        txtAdjustmentNo.Value = clsCommon.ShowSelectForm("AdjustmentStoreDoc1", qry, "AdjustmentNumber", whrClas, txtAdjustmentNo.Value, "TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Date desc", isButtonClicked)
        LoadData(txtAdjustmentNo.Value, NavigatorType.Current)
    End Sub
    ...
    Private Sub RadButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton1.Click
        ...
        Try
            If clsCommon.myLen(txtAdjustmentNo.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow(Me, " QC  No Not Available for Print .", Me.Text)
                Return
            End If
            'Dim qry As String = "  select TSPL_ADJUSTMENT_Detail_QC.REMARKS,TSPL_ADJUSTMENT_HEADER.Description as Production_Entry_Desc , TSPL_LOCATION_MASTER_From_loc.Location_Desc,TSPL_COMPANY_MASTER.Logo_Img,TSPL_LOCATION_MASTER_From_loc.Location_Desc as FromLoaction,TSPL_LOCATION_MASTER_From_loc.GSTNO as From_location_GSTIN,TSPL_STATE_MASTER_From_loc.GST_STATE_Code as From_Loc_GST_STATE_Code,TSPL_COMPANY_MASTER.Comp_Name as Comp_Name, '' as Company_Address ,TSPL_ADJUSTMENT_Detail_QC.Adjustment_No as Transfer_No,convert(datetime,TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Date,103) as  Date_And_Time,TSPL_ADJUSTMENT_Detail_QC.Adjustment_Line_No as SNO,TSPL_ADJUSTMENT_Detail_QC.Item_Code,TSPL_ITEM_MASTER.Item_Desc as itemdesc,TSPL_ITEM_MASTER.HSN_Code, " &
            '                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.UOM ELSE TSPL_ADJUSTMENT_Detail_QC.Unit_Code END AS UOM, " &
            '                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 then BI.QTY else TSPL_ADJUSTMENT_Detail_QC.Item_Quantity end as Qty, " &
            '                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 then convert (varchar, BI.Manufacture_Date,103) else '' end as Manufacture_Date, " &
            '                    " TSPL_ADJUSTMENT_Detail_QC.Item_Cost as Rate, TSPL_ADJUSTMENT_HEADER_QC.Description  as QC_Desc , " &
            '                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.BATCH_NO ELSE '' END AS Item_BatchNo,TSPL_ITEM_MASTER.Is_Batch_Item  " &
            '                    "  ,TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Type as Document_Type,TSPL_ADJUSTMENT_HEADER_QC.Loc_Code, " &
            '                    " TSPL_ADJUSTMENT_HEADER_QC.Posted , TSPL_ADJUSTMENT_HEADER_QC.Posting_Date as Posted_Date,TSPL_ADJUSTMENT_HEADER_QC.Created_By,TSPL_ADJUSTMENT_HEADER_QC.Modify_By as Modified_By, case when TSPL_ADJUSTMENT_HEADER_QC.Posted='Y' then tspl_user_master_Modified_By.User_Name else '' end as PostedBy,tspl_user_master_Created_By.User_Name as CreatedBy, TSPL_LOCATION_MASTER_From_loc.Add1 + case when len(TSPL_LOCATION_MASTER_From_loc. Add2) > 0 then +','+ TSPL_LOCATION_MASTER_From_loc. Add2 else ' '     end + Case when len( TSPL_LOCATION_MASTER_From_loc. Add3) >0 then ','+TSPL_LOCATION_MASTER_From_loc. Add3 else '' end + case when len     (TSPL_LOCATION_MASTER_From_loc.Add4) >0 then ','+TSPL_LOCATION_MASTER_From_loc.Add4 else '' end + case when len     (TSPL_LOCATION_MASTER_From_loc.City_Code)>0 then ','+ TSPL_LOCATION_MASTER_From_loc.City_Code else '' end + case when len    (TSPL_LOCATION_MASTER_From_loc.state) > 0 then ','+ TSPL_STATE_MASTER_From_loc.STATE_NAME else ''  end + Case when len     (TSPL_LOCATION_MASTER_From_loc.Pin_Code ) >0 then ' - '+ convert(varchar, TSPL_LOCATION_MASTER_From_loc.Pin_Code,103) else '' end as From_Location_Address  from TSPL_ADJUSTMENT_Detail_QC  " &
            '                    " left outer join TSPL_ADJUSTMENT_HEADER_QC on TSPL_ADJUSTMENT_Detail_QC.Adjustment_No =TSPL_ADJUSTMENT_HEADER_QC.Adjustment_No  " &
            '                    " left outer join TSPL_ITEM_MASTER on TSPL_ADJUSTMENT_Detail_QC.item_code= TSPL_ITEM_MASTER.Item_Code  " &
            '                    " Left outer join tspl_user_master as tspl_user_master_Modified_By on tspl_user_master_Modified_By.User_Code =TSPL_ADJUSTMENT_HEADER_QC.posted_By  " &
            '                    " left outer join tspl_user_master as tspl_user_master_Created_By on tspl_user_master_Created_By.User_Code =TSPL_ADJUSTMENT_HEADER_QC.Created_By  " &
            '                    " Left outer join TSPL_LOCATION_MASTER as TSPL_LOCATION_MASTER_From_loc on TSPL_LOCATION_MASTER_From_loc.Location_Code =TSPL_ADJUSTMENT_HEADER_QC.loc_Code  " &
            ...
            '                    " Left outer join TSPL_CITY_MASTER as TSPL_CITY_MASTER_From_location on TSPL_CITY_MASTER_From_location.City_Code = TSPL_LOCATION_MASTER_From_loc.City_Code                              Left Join TSPL_BATCH_ITEM AS BI ON TSPL_ADJUSTMENT_Detail_QC.Adjustment_No=BI.Document_Code And TSPL_ADJUSTMENT_Detail_QC.Item_Code=BI.Item_Code  " &
            '                    " Left Outer Join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code =TSPL_ADJUSTMENT_HEADER_QC.Comp_Code  " &
            '                    " Left Outer Join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_HEADER.Adjustment_No =TSPL_ADJUSTMENT_HEADER_QC.Production_Entry " &
            '                    " where  TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Type = 'PRE' and TSPL_ADJUSTMENT_HEADER_QC.Adjustment_No= '" + txtAdjustmentNo.Value + "'  "

            Dim qry As String = "  select TSPL_ADJUSTMENT_DETAIL.Qty as Production_Qty,TSPL_ADJUSTMENT_Detail_QC.REMARKS,TSPL_ADJUSTMENT_HEADER.Description as Production_Entry_Desc , TSPL_LOCATION_MASTER_From_loc.Location_Desc,TSPL_COMPANY_MASTER.Logo_Img,TSPL_LOCATION_MASTER_From_loc.Location_Desc as FromLoaction,TSPL_LOCATION_MASTER_From_loc.GSTNO as From_location_GSTIN,TSPL_STATE_MASTER_From_loc.GST_STATE_Code as From_Loc_GST_STATE_Code,TSPL_COMPANY_MASTER.Comp_Name as Comp_Name, '' as Company_Address ,TSPL_ADJUSTMENT_Detail_QC.Adjustment_No as Transfer_No,convert(datetime,TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Date,103) as  Date_And_Time,TSPL_ADJUSTMENT_Detail_QC.Adjustment_Line_No as SNO,TSPL_ADJUSTMENT_Detail_QC.Item_Code,TSPL_ITEM_MASTER.Item_Desc as itemdesc,TSPL_ITEM_MASTER.HSN_Code, " &
                                " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.UOM ELSE TSPL_ADJUSTMENT_Detail_QC.Unit_Code END AS UOM, " &
                                " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 then BI.QTY else TSPL_ADJUSTMENT_Detail_QC.Item_Quantity end as Qty, " &
                                " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 then convert (varchar, BI.Manufacture_Date,103) else '' end as Manufacture_Date, " &
                                " TSPL_ADJUSTMENT_Detail_QC.Item_Cost as Rate, TSPL_ADJUSTMENT_HEADER_QC.Description  as QC_Desc , " &
                                ...
                                " Left Join TSPL_BATCH_ITEM AS BI ON TSPL_ADJUSTMENT_Detail_QC.Adjustment_No=BI.Document_Code And TSPL_ADJUSTMENT_Detail_QC.Item_Code=BI.Item_Code and TSPL_ADJUSTMENT_Detail_QC.Unit_Code=BI.UOM  " &
                                " Left Outer Join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code =TSPL_ADJUSTMENT_HEADER_QC.Comp_Code  " &
                                " Left Outer Join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_HEADER.Adjustment_No =TSPL_ADJUSTMENT_HEADER_QC.Production_Entry " &
                                ...
                                " Left Outer Join (select TSPL_ADJUSTMENT_HEADER.Adjustment_No, " &
                                " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.batch_no ELSE '' END AS Item_BatchNo, " &
                                " Case when TSPL_ITEM_MASTER.Is_Batch_Item=1 then BI.QTY else TSPL_ADJUSTMENT_DETAIL.Item_Quantity end as Qty, " &
                                " TSPL_ADJUSTMENT_DETAIL.Unit_Code,TSPL_ADJUSTMENT_DETAIL.Item_Code from TSPL_ADJUSTMENT_HEADER  " &
                                " Left Outer Join TSPL_ADJUSTMENT_DETAIL on TSPL_ADJUSTMENT_HEADER.Adjustment_No =TSPL_ADJUSTMENT_DETAIL.Adjustment_No  " &
                                " left outer join TSPL_ITEM_MASTER on TSPL_ADJUSTMENT_DETAIL.item_code= TSPL_ITEM_MASTER.Item_Code " &
                                " Left Join TSPL_BATCH_ITEM AS BI ON TSPL_ADJUSTMENT_DETAIL.Adjustment_No=BI.Document_Code And TSPL_ADJUSTMENT_DETAIL.Item_Code=BI.Item_Code and TSPL_ADJUSTMENT_Detail.Unit_Code=BI.UOM" &
                                " where TSPL_ADJUSTMENT_HEADER.Adjustment_No='" + txtProductionEntry.Value + "')TSPL_ADJUSTMENT_DETAIL on TSPL_ADJUSTMENT_HEADER.Adjustment_No =TSPL_ADJUSTMENT_DETAIL.Adjustment_No " &
                                " and TSPL_ADJUSTMENT_DETAIL.Item_Code=TSPL_ADJUSTMENT_Detail_QC.Item_Code and TSPL_ADJUSTMENT_DETAIL.Unit_Code=TSPL_ADJUSTMENT_Detail_QC.Unit_Code and TSPL_ADJUSTMENT_DETAIL.Item_BatchNo=(case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.BATCH_NO ELSE '' END)" &
                                " where TSPL_ADJUSTMENT_Detail_QC.QC_Status='OK' and TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Type = 'PRE' and TSPL_ADJUSTMENT_HEADER_QC.Adjustment_No= '" + txtAdjustmentNo.Value + "'  "
    ...
    Private Sub btnReverse_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverse.Click
        ...
        Try
            If common.clsCommon.MyMessageBoxShow(Me, "Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                End If
                trans = clsDBFuncationality.GetTransactin()
                If ClsAdjustmentsQCC.ReverseAndUnpost(txtAdjustmentNo.Value, trans) Then
...
#End Region
    ...
    Private Sub txtProductionEntry__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtProductionEntry._MYValidating
        ...
        ''whrClas += " AND ItemType IN ('RM', 'OT')"
        txtAdjustmentNo.Value = ""
        Dim qry As String = "select xx.Code As Code,CONVERT(varchar, TSPL_ADJUSTMENT_HEADER.Adjustment_Date,103) as Date from ( select distinct TT.Code from " &
            "(SELECT TSPL_ADJUSTMENT_DETAIL.Adjustment_No AS Code,Item_Code as ICode,TSPL_ADJUSTMENT_DETAIL.Unit_Code,Item_Quantity as Qty,1 as Chk,1 as RI" &
            " FROM TSPL_ADJUSTMENT_DETAIL left join TSPL_ADJUSTMENT_HEADER ON TSPL_ADJUSTMENT_DETAIL.Adjustment_No= TSPL_ADJUSTMENT_HEADER.Adjustment_No " &
            " where isnull(AdjustType,'') <> 'Consume' and TSPL_ADJUSTMENT_HEADER.Adjustment_Type = 'PRE' and Posted = 'Y'"
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            qry += " AND (Loc_Code in (" + objCommonVar.strCurrUserLocations + ") or  mainlocationcode in (" + objCommonVar.strCurrUserLocations + "))"
        End If
        qry += " union all " &
         " SELECT TSPL_ADJUSTMENT_HEADER_QC.Production_Entry AS Code,Item_Code as ICode,TSPL_ADJUSTMENT_DETAIL_QC.Unit_Code,Item_Quantity as Qty,0 as Chk,-1 as RI " &
         " FROM  TSPL_ADJUSTMENT_DETAIL_QC left join TSPL_ADJUSTMENT_HEADER_QC on TSPL_ADJUSTMENT_DETAIL_QC.Adjustment_No=TSPL_ADJUSTMENT_HEADER_QC.Adjustment_No " &
         " where isnull(AdjustType,'') <> 'Consume' and TSPL_ADJUSTMENT_HEADER_QC.Adjustment_Type = 'PRE' " &
        ")TT group by Code,ICode,Unit_Code having SUM(Chk)>0 and SUM(Qty *RI)>0 )xx " &
        " left outer join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_HEADER.Adjustment_No = xx.Code "
        'and TSPL_ADJUSTMENT_HEADER_QC.Production_Entry not in ('" + txtAdjustmentNo.Value + "')
        txtProductionEntry.Value = clsCommon.ShowSelectForm("AdjStoreDoc", qry, "Code", "", txtProductionEntry.Value, "Code", isButtonClicked)
        LoadDataForProductionEntry(txtProductionEntry.Value, NavigatorType.Current)
        txtAdjustmentNo.Value = ""
    End Sub

    Sub LoadDataForProductionEntry(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            btnSave.Enabled = True
            btnPost.Enabled = True
            btnDelete.Enabled = True
            ...
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Adjustment_No) > 0) Then
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As ClsAdjustmentsDetails In obj.Arr
                        ...
                        Dim strqry As String = "SELECT isnull(TSPL_ADJUSTMENT_DETAIL.item_quantity,0)-isnull(qc.item_quantity,0) AS Pending_Qty  FROM " &
                        " TSPL_ADJUSTMENT_DETAIL left join TSPL_ADJUSTMENT_HEADER On TSPL_ADJUSTMENT_DETAIL.Adjustment_No= TSPL_ADJUSTMENT_HEADER.Adjustment_No " &
                        " left join (select TSPL_ADJUSTMENT_HEADER_QC.Production_Entry,TSPL_ADJUSTMENT_DETAIL_QC.item_code,TSPL_ADJUSTMENT_DETAIL_QC.Unit_Code,sum(TSPL_ADJUSTMENT_DETAIL_QC.item_quantity) as item_quantity " &
                        " From TSPL_ADJUSTMENT_DETAIL_QC  " &
                        " LEFT OUTER JOIN TSPL_ADJUSTMENT_HEADER_QC ON TSPL_ADJUSTMENT_DETAIL_QC.Adjustment_No = TSPL_ADJUSTMENT_HEADER_QC.Adjustment_No  " &
                         " Where 1 = 1 And TSPL_ADJUSTMENT_HEADER_QC.Production_Entry ='" & objTr.Adjustment_No & "' and TSPL_ADJUSTMENT_DETAIL_QC.item_code='" & objTr.Item_Code & "'" &
                         " Group by TSPL_ADJUSTMENT_HEADER_QC.Production_Entry,TSPL_ADJUSTMENT_DETAIL_QC.item_code,TSPL_ADJUSTMENT_DETAIL_QC.Unit_Code " &
                         "  ) qc on qc.item_code=TSPL_ADJUSTMENT_DETAIL.item_code and qc.Unit_Code=TSPL_ADJUSTMENT_DETAIL.Unit_Code and qc.Production_Entry =TSPL_ADJUSTMENT_DETAIL.Adjustment_No " &
                        " WHERE  TSPL_ADJUSTMENT_DETAIL.Adjustment_No ='" & objTr.Adjustment_No & "' and TSPL_ADJUSTMENT_DETAIL.item_code='" & objTr.Item_Code & "' and TSPL_ADJUSTMENT_DETAIL.Unit_Code='" & objTr.Unit_Code & "'"
                        ...
                        Else
                            ...
                            If objTr.arrBatchItem IsNot Nothing AndAlso objTr.arrBatchItem.Count > 0 Then
                                ...
                                For Each objBatch As clsBatchInventory In objTr.arrBatchItem
                                    TempdblBalance = ClsAdjustmentsQCC.GetBatchBalanceAdjustmentsQCC(objBatch.Item_Code, objBatch.Location_Code, objBatch.Batch_No, objBatch.MRP, objBatch.UOM, objBatch.Document_Code, objBatch.Document_Type, Nothing)
