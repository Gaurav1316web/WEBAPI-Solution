...
Public Class frmPhysicalStockMultipleLocation
...
#Region "Variables"
    ...
    Const colSLCode As String = "colSLCode"
    Const colSLName As String = "colSLName"
...
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        repoICode.HeaderText = "Location Code"
        repoICode.Name = colLCode
        ...
        repoIName.HeaderText = "Location"
        repoIName.Name = colLName
        repoIName.Width = 150
        ...
        repoICode.FormatString = ""
        repoICode.HeaderText = "Sub Location Code"
        repoICode.Name = colSLCode
        repoICode.ReadOnly = True
        repoICode.Width = 100
        gv1.MasterTemplate.Columns.Add(repoICode)
        ...
        repoIName.FormatString = ""
        repoIName.HeaderText = "Sub Location"
        repoIName.Name = colSLName
        repoIName.Width = 150
        repoIName.ReadOnly = True
        gv1.MasterTemplate.Columns.Add(repoIName)
    ...
    Sub SetGridGroup()
        If gv1.Columns.Count > 0 Then
            ...
            view.ColumnGroups(0).Rows(0).ColumnNames.Add(gv1.Columns(colLCode).Name)
            view.ColumnGroups(0).Rows(0).ColumnNames.Add(gv1.Columns(colLName).Name)
            view.ColumnGroups(0).Rows(0).ColumnNames.Add(gv1.Columns(colSLCode).Name)
            view.ColumnGroups(0).Rows(0).ColumnNames.Add(gv1.Columns(colSLName).Name)
            ...
            If (AllowToSave(isposted)) Then
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    If clsCommon.myCdbl(grow.Cells(colPhyQty).Value) <> 0 OrElse clsCommon.myCBool(grow.Cells(colPhyNillBalance).Value) Then
                        ...
                        obj.Multiple_Location = True
                        obj.Main_Location = clsCommon.myCstr(grow.Cells(colLCode).Value)
                        obj.Location_Code = clsCommon.myCstr(grow.Cells(colSLCode).Value)
                ...
                ElseIf diffQty > 0 Then
                    If RunBatchFifowise Then
                        If clsCommon.myLen(gv1.CurrentRow.Cells(colICode).Value) > 0 Then
                            ...
                            frm.strItemCode = clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value)
                            frm.strItemName = clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value)
                            frm.strLocationCode = clsCommon.myCstr(gv1.CurrentRow.Cells(colLCode).Value)
    ...
    Private Sub OpenGLAccount(ByVal isButtonClick As Boolean)
        ...
        Dim logSeg As ArrayList = clsLocation.GetSegmentCode(txtLocation.arrValueMember, Nothing)
        ...
        End If
        whrcls += "   and TSPL_GL_ACCOUNTS.Account_Seg_Code7 in ( " + clsCommon.GetMulcallString(logSeg) + ")  and TSPL_GL_ACCOUNTS.ControlAccount='N'  "

        gv1.CurrentRow.Cells(colGLAccount).Value = clsCommon.ShowSelectForm("PhystLocSeg@PHSM", qry, "Account_Code", whrcls, clsCommon.myCstr(gv1.CurrentRow.Cells(colGLAccount).Value), "Account_Code", isButtonClick)
    ...
    Private Sub LoadData(ByVal strcode As String, ByVal strLocation As String, ByVal SubLocCode As String, ByVal NavType As NavigatorType)
        Try
            ...
            arr = clsPhysicalstock.GetData(txtCode.Value, "", "", chkMilk.Checked, NavType, dtpdate.Value.ToString())
            isformLoad = True
            ...
            If arr IsNot Nothing AndAlso arr.Count > 0 Then
                For Each obj As clsPhysicalstock In arr
                    ...
                    If obj.Is_Milk Then
                        If clsCommon.myLen(obj.Main_Location) > 0 Then
                            If Not arrLocation.Contains(obj.Main_Location) Then
                                arrLocation.Add(obj.Main_Location)
                            End If
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colLCode).Value = obj.Main_Location
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colLName).Value = clsLocation.GetName(obj.Main_Location, Nothing)
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colSLCode).Value = obj.Location_Code
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colSLName).Value = clsLocation.GetName(obj.Location_Code, Nothing)
                        Else
                            If Not arrLocation.Contains(obj.Location_Code) Then
                                arrLocation.Add(obj.Location_Code)
                            End If
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colLCode).Value = obj.Location_Code
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colLName).Value = clsLocation.GetName(obj.Location_Code, Nothing)
                        End If
                    Else
                        If Not arrLocation.Contains(obj.Location_Code) Then
                            arrLocation.Add(obj.Location_Code)
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colLName).Value = clsLocation.GetName(obj.Location_Code, Nothing)
                    End If
                    If Not arrICode.Contains(obj.Item_Code) Then
                        arrICode.Add(obj.Item_Code)
                    End If
                ...
                txtItem.arrValueMember = arrICode
    ...
    Private Sub LoadDataForFirstTime(ByVal strcode As String, ByVal NavType As NavigatorType)
        Try
            LoadBlankGrid()
            isNewEntry = True
            ...
            End If
            For Each strLocation As String In txtLocation.arrValueMember
                fill(strLocation, "", NavType, strInventoryAccount)
                Dim qry As String = "select Location_Code as [Code],Location_Desc as [Description]from TSPL_Location_MASTER  left join TSPL_GL_SEGMENT_CODE as Seg on TSPL_Location_MASTER.Loc_Segment_Code=Seg.Segment_Code   " &
            " where (isnull(is_sub_location,'N')='Y' or isnull(Is_Section,'N')='Y') and Loc_Segment_Code='" + clsLocation.GetSegmentCode(strLocation, Nothing) + "'"
                ...
                If dtLocation IsNot Nothing AndAlso dtLocation.Rows.Count > 0 Then
                    For Each dr As DataRow In dtLocation.Rows
                        fill(strLocation, clsCommon.myCstr(dr("Code")), NavType, strInventoryAccount)
                    Next
                End If
    ...
    Sub fill(ByVal strLocation As String, ByVal strSubLocation As String, ByVal NavType As NavigatorType, ByVal strInventoryAccount As String)
        Dim dt As DataTable = clsPhysicalstock.GetDataForFirstTime(txtItem.arrValueMember, txtCode.Value, strLocation, strSubLocation, chkMilk.Checked, NavType, Nothing, dtpdate.Value.ToString(), "", strInventoryAccount)
        isformLoad = True
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            For Each dr As DataRow In dt.Rows
                ...
                gv1.Rows(gv1.Rows.Count - 1).Cells(colLCode).Value = strLocation
                gv1.Rows(gv1.Rows.Count - 1).Cells(colLName).Value = clsLocation.GetName(strLocation, Nothing)

                gv1.Rows(gv1.Rows.Count - 1).Cells(colSLCode).Value = strSubLocation
                gv1.Rows(gv1.Rows.Count - 1).Cells(colSLName).Value = clsLocation.GetName(strSubLocation, Nothing)
    ...
    Private Sub txtCode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtCode._MYValidating
        Dim qry As String = "select count(*) from TSPL_PHYSICAL_STOCK where physical_no='" + txtCode.Value + "' and isnull(Multiple_Location,0)=1 "
        ...
        If txtCode.MyReadOnly OrElse isButtonClicked Then
            txtCode.Value = clsPhysicalstock.GetFinder(" isnull(Multiple_Location,0)=1 ", txtCode.Value, isButtonClicked)
            LoadData(txtCode.Value, "", "", NavigatorType.Current)
        End If
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        ...
        LoadDataForFirstTime(txtCode.Value, NavigatorType.Current)
        isGoClick = False
    End Sub
        ...
        Try
            ...
            If gv1.Rows.Count > 0 Then
                For ii As Integer = 0 To gv1.Rows.Count - 1
                    ...
                    End If
                    If chkMilk.Checked Then
                        str += "select '" + clsCommon.myCstr(gv1.Rows(ii).Cells(colLCode).Value) + "' as Location,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colLName).Value) + "' as LocationName,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colSLCode).Value) + "' as SubLocation,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colSLName).Value) + "' as SubLocationName,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colICode).Value) + "' as Item,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colIName).Value).Replace("'", "''") + "' as ItemName,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colGLAccountInvControl).Value).Replace("'", "''") + "' as InventoryAccount,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colGLAccount).Value) + "' as GLAccount,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrQty).Value) + "' as CurrentQty,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrFATPers).Value) + "' as CurrentFATPer,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrFATKG).Value) + "' as CurrentFATKg,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrFATAmount).Value) + "' as CurrentFATAmt,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrSNFPers).Value) + "' as CurrentSNFPer,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrSNFKG).Value) + "' as CurrentSNFKg,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrSNFAmt).Value) + "' as CurrentSNFAmt,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrAmt).Value) + "' as CurrentAmt ,'" + IIf(clsCommon.myCBool(gv1.Rows(ii).Cells(colPhyNillBalance).Value), "Y", "N") + "' as [NillBalance(Y/N)],0 as Qty,0 as FAT,0 as FATAmount,0 as SNF,0 as SNFAmount"
                    Else
                        str += "select '" + clsCommon.myCstr(gv1.Rows(ii).Cells(colLCode).Value) + "' as Location,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colLName).Value) + "' as LocationName,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colICode).Value) + "' as Item,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colIName).Value).Replace("'", "''") + "' as ItemName,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colGLAccountInvControl).Value).Replace("'", "''") + "' as InventoryAccount,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colGLAccount).Value) + "' as GLAccount,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrQty).Value) + "' as CurrentQty,'" + clsCommon.myCstr(gv1.Rows(ii).Cells(colCurrAmt).Value) + "' as CurrentAmt,'" + IIf(clsCommon.myCBool(gv1.Rows(ii).Cells(colPhyNillBalance).Value), "Y", "N") + "' as [NillBalance(Y/N)],0 as Qty,0 as Amount "
                    End If
                Next
            Else
                If chkMilk.Checked Then
                    str = "select  '' as Location,'' as LocationName,'' as SubLocation,'' as SubLocationName,'' as Item,'' as ItemName,'' as InventoryAccount,'' as GLAccount,0 as CurrentQty,0 as CurrentFATPer,0 as as CurrentFATKg,0 as CurrentFATAmt,0 as CurrentSNFPer,0 as CurrentSNFKg,0 as CurrentSNFAmt,0 as CurrentAmt ,'N' as [NillBalance(Y/N)],0 as Qty,0 as FAT,0 as FATAmount,0 as SNF,0 as SNFAmount"
                Else
                    str = "select '' as Location,'' as LocationName,'' as Item,'' as ItemName,'' as InventoryAccount,'' as GLAccount,0 as CurrentQty,0 as CurrentAmt,'N' as [NillBalance(Y/N)],0 as Qty,0 as Amount "
                End If
            End If
    ...
    Private Sub btnimport_Click(sender As Object, e As EventArgs) Handles btnimport.Click
        ...
        Try
            If chkMilk.Checked Then
                transportSql.importExcel(gvImport, "Location", "LocationName", "SubLocation", "SubLocationName", "Item", "ItemName", "GLAccount", "CurrentQty", "CurrentFATPer", "CurrentFATKg", "CurrentFATAmt", "CurrentSNFPer", "CurrentSNFKg", "CurrentSNFAmt", "CurrentAmt", "NillBalance(Y/N)", "Qty", "FAT", "FATAmount", "SNF", "SNFAmount")
            Else
                transportSql.importExcel(gvImport, "Location", "LocationName", "Item", "ItemName", "GLAccount", "CurrentQty", "CurrentAmt", "NillBalance(Y/N)", "Qty", "Amount")
            End If
            For ii As Integer = 0 To gvImport.RowCount - 1
                Dim strICode As String = clsCommon.myCstr(gvImport.Rows(ii).Cells("Item").Value)
                Dim strLCode As String = clsCommon.myCstr(gvImport.Rows(ii).Cells("Location").Value)
                Dim strSLCode As String = ""
                If chkMilk.Checked Then
                    strSLCode = clsCommon.myCstr(gvImport.Rows(ii).Cells("SubLocation").Value)
                End If

                For jj As Integer = 0 To gv1.RowCount - 1
                    Dim strICodeGrid As String = clsCommon.myCstr(gv1.Rows(jj).Cells(colICode).Value)
                    Dim strLCodeGrid As String = clsCommon.myCstr(gv1.Rows(jj).Cells(colLCode).Value)
                    Dim strSLCodeGrid As String = ""
                    If chkMilk.Checked Then
                        strSLCodeGrid = clsCommon.myCstr(gv1.Rows(jj).Cells(colSLCode).Value)
                    End If

                    If clsCommon.CompairString(strICode + strLCode + strSLCode, strICodeGrid + strLCodeGrid + strSLCodeGrid) = CompairStringResult.Equal Then
    ...
    Private Sub txtInventoryAccount__My_Click(sender As Object, e As EventArgs) Handles txtInventoryAccount._My_Click
        Dim qry As String = " select distinct TSPL_PURCHASE_ACCOUNTS.Inv_Control_Account as Code, TSPL_GL_ACCOUNTS.Description as Name from TSPL_PURCHASE_ACCOUNTS " &
    ...
    'Private Sub txtsubLoc__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean)
    ...
    Private Sub txtLocation__My_Click(sender As Object, e As EventArgs) Handles txtLocation._My_Click
        ...
        End If
        txtLocation.arrValueMember = clsCommon.ShowMultipleSelectForm("location@PhsicalStockM", qry, "Code", "", txtLocation.arrValueMember, Nothing)
    End Sub

    Private Sub txtItemType__My_Click(sender As Object, e As EventArgs) Handles txtItem._My_Click
        Dim whrclas As String = ""
        Dim qry As String = "select Item_Code as Code,Item_Desc as Name from TSPL_ITEM_MASTER where Product_Type " + IIf(chkMilk.Checked, "=", "<>") + " 'MI'"
        txtItem.arrValueMember = clsCommon.ShowMultipleSelectForm("Item@PhsicalStockM", qry, "Code", "", txtItem.arrValueMember, Nothing)
