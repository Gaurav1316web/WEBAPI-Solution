...
'-16/09/2013-Version-2.0.2.35-Updation By Shipra Jain [BM00000000488,BM00000000489]
...
#Region "Custom Variable"
    ...
    Public arrTransferNo As List(Of String) = Nothing
#End Region
    ...
    Private Sub AddNew()
        ...
        txtTime.Value = txtTransferDate.Value
        lblSalesman.Text = ""
        txtSalesman.Value = ""
        ...
        If txtTransferDate.ShowCheckBox Then
            txtTransferDate.Checked = False
        End If
    End Sub
    ...
    Private Sub frmTransfer_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.AskForDate, clsFixedParameterCode.AskForDate, Nothing)) = 1 Then
            txtTransferDate.ShowCheckBox = True
        End If
        ...
        ButtonToolTip.SetToolTip(btnadjustment, "Press Alt+A View Adjustment Details")
    ...
    Private Sub frmTransfer_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.Alt AndAlso e.KeyCode = Keys.R AndAlso rdbtnprint.Enabled Then
            printdata(True)
        ElseIf e.Alt AndAlso e.KeyCode = Keys.A AndAlso btnadjustment.Enabled Then
            show_adjustment()
    ...
    Private Sub FunAddShellItem()
        ...
        If myreader.Rows.Count > 0 Then
            For Each row As DataRow In myreader.Rows
                ...
                gv1.Rows(gv1.Rows.Count - 1).Cells(colItemCode).Value = "PC300BSH"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colItemName).Value = "PEPSI COLA 300 ML BOTTLE SHELL"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colPriceDate).Value = CDate(row("Start_Date")).ToString("dd-MM-yyyy")
                gv1.Rows(gv1.Rows.Count - 1).Cells(colUOM).Value = Convert.ToString(row("UOM"))
                gv1.Rows(gv1.Rows.Count - 1).Cells(colAssessableAmt).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colAmount).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colTax).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colTotal).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colTPT).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colMRP).Value = Convert.ToString(row("Item_Basic_Net"))
                gv1.Rows(gv1.Rows.Count - 1).Cells(colBatchNo).Value = ""
                gv1.Rows(gv1.Rows.Count - 1).Cells(colBasicPrice).Value = Convert.ToString(row("Item_Basic_Price"))
                gv1.Rows(gv1.Rows.Count - 1).Cells(colLeak).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colLeak).ReadOnly = True
                gv1.Rows(gv1.Rows.Count - 1).Cells(colShortage).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colShortage).ReadOnly = True
                gv1.Rows(gv1.Rows.Count - 1).Cells(colItemCost).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colConversion).Value = "1"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingBalance).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colTransferQty).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colLoadInQty).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colBreakage).Value = "0.00"
                gv1.Rows(gv1.Rows.Count - 1).Cells(colItemCost).Value = Convert.ToString(row("Item_Basic_Net"))
                gv1.Rows(gv1.Rows.Count - 1).Cells(colBasicPriceWithTax).Value = Convert.ToString(row("Item_Basic_Price"))
    ...
    Public Sub RouteNo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If lblRouteNo.Text <> "" Then
            ...
            If myreader.Rows.Count > 0 Then
                For Each row As DataRow In myreader.Rows
                    txtSalesman.Value = Convert.ToString(row("employee_code"))
    ...
    Private Sub fndloadno_Load(ByVal sender As System.Object, ByVal e As System.EventArgs)
        '' fndloadno.Query = "select transfer_no as 'Load Out Number',description as 'Description' from TSPL_TRANSFER_HEAD where (From_Location in (Select location_code from TSPL_LOCATION_MASTER where (Excisable ='F'))) and Post ='Y' order by 'Load Out Number'desc "
        'fndloadno.Query = "select distinct TSPL_TRANSFER_HEAD .transfer_no as 'Load Out Number',description as 'Description',CONVERT(varchar(15), Transfer_Date , 103) as [Transfer Date], From_Location as [From Location], To_Location as [To Location] from TSPL_TRANSFER_HEAD inner join TSPL_TRANSFER_DETAIL on TSPL_TRANSFER_HEAD.Transfer_No=TSPL_TRANSFER_DETAIL.Transfer_No  where From_Location in (Select location_code from TSPL_LOCATION_MASTER) and Post ='Y' and Transfer_Type ='LO' and Pending_Qty > 0 AND  TSPL_TRANSFER_HEAD.Post ='Y' and TSPL_TRANSFER_HEAD .transfer_no not in(select Load_Out_No from TSPL_TRANSFER_HEAD  ) "
    ...
    Public Sub loadOutNo_Text_changed(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'Dim LoadOut As String = connectSql.RunScalar("Select distinct TSPL_TRANSFER_HEAD.Transfer_No from tspl_transfer_head inner join TSPL_TRANSFER_DETAIL on TSPL_TRANSFER_HEAD.Transfer_No=TSPL_TRANSFER_DETAIL.Transfer_No  where TSPL_TRANSFER_HEAD.Transfer_No='" + fndloadno.Value + "' and  Pending_Qty > 0 AND TSPL_TRANSFER_HEAD.Post ='Y' and TSPL_TRANSFER_HEAD .transfer_no not in(select Load_Out_No from TSPL_TRANSFER_HEAD  ) ")
    ...
    Public Sub show_adjustment()
        Dim value As String = String.Empty
        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal And clsCommon.myLen(txtLoadoutNo.Value) > 0 Then
            Dim AdjustmentNoForEmpty As String = clsDBFuncationality.getSingleValue("select Adjustment_No from TSPL_ADJUSTMENT_HEADER where Document_No ='" + txtLoadoutNo.Value + "'")
            If clsCommon.myLen(AdjustmentNoForEmpty) > 0 Then
                'common.clsCommon.MyMessageBoxShow("Empty Adjustment:'" + AdjustmentNoForEmpty + "' Is Already Exist For Transfer:'" + txtLoadoutNo.Value + "' .", "Transfer", MessageBoxButtons.OK, RadMessageIcon.Info)
                Dim frm1 As New frmAdjustmentEmpty()
                frm1.strDocumentNo = AdjustmentNoForEmpty
                frm1.Show()
                Return
            End If
            value = "Load Out" + "," + txtLoadoutNo.Value
            'Dim F As New FrmAdjustments1(value, txtLoadoutNo.Value, objCommonVar.CurrentUserCode, objCommonVar.CurrentCompanyCode, txtFromLoaction.Value, lblFromLocation.Text, txtToLocation.Value, lblToLocation.Text, txtVehicleCode.Value, lblVehicleDesc.Text, txtTransferDate.Value, True, "Empty Transactions")
            Dim frm As New frmAdjustmentEmpty()
            frm.strLoadoutNo = txtLoadoutNo.Value
            frm.strSalesman = txtFromLoaction.Value
            frm.strLocation = txtToLocation.Value
            frm.dtTransDate = txtTransferDate.Value
            frm.strVehicleCode = txtVehicleCode.Value
            frm.Show()
        Else
            Dim adj As New frmAdjustmentEmpty()
            adj.Show()
        End If
    ...
    Private Function UpdateBalanceQtyOfTransferLO(ByVal strTransferNo As String, ByVal trans As SqlTransaction) As Boolean
        If clsCommon.myLen(strTransferNo) > 0 Then
            ...
            qry += " where  TSPL_TRANSFER_HEAD.Transfer_Type='LO' and TSPL_TRANSFER_HEAD.Post='Y' and  TSPL_TRANSFER_HEAD.Transfer_No='" + strTransferNo + "'"
            qry += " union all "
    ...
    Private Sub LoadPendingBalanceAgainstTransfer()
        If clsCommon.myLen(txtLoadoutNo.Value) > 0 Then
            Dim qry As String = "select Transfer_No,Item_Code,Price_Date,sum(Item_Qty * case when RI =1 then 1 else case when RI in (2,3,4) then -1 else 0 end end) as BalanceQty from (" + Environment.NewLine
            qry += " select Transfer_No,Item_Code,Price_Date,Item_Qty,1 as RI from TSPL_TRANSFER_DETAIL where Transfer_No='" + txtLoadoutNo.Value + "'" + Environment.NewLine
            ...
            qry += "where TSPL_SHIPMENT_MASTER.Is_Post='Y' and  TSPL_SHIPMENT_MASTER.Transfer_No='" + txtLoadoutNo.Value + "' " + Environment.NewLine
            qry += "union all " + Environment.NewLine
            ...
            qry += "left outer join TSPL_SHIPMENT_MASTER on TSPL_SHIPMENT_MASTER.Shipment_No=TSPL_SHIPMENT_DETAILS.Shipment_No" + Environment.NewLine
            qry += "left outer join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_SHIPMENT_DETAILS.Item_Code and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_SHIPMENT_DETAILS.Unit_code" + Environment.NewLine
            qry += "where TSPL_SHIPMENT_MASTER.Is_Post='N' and  TSPL_SHIPMENT_MASTER.Transfer_No='" + txtLoadoutNo.Value + "'  and TSPL_SHIPMENT_MASTER.Shipment_No not in('" + txtTransferNo.Value + "') " + Environment.NewLine
            qry += " union all " + Environment.NewLine
            ...
            qry += "where TSPL_TRANSFER_HEAD.Load_Out_No='" + txtLoadoutNo.Value + "' and Transfer_Type='LI'  "
            qry += ") xxx group by Transfer_No,Item_Code,Price_Date"
    ...
    Private Sub FunDelete(ByVal Reason As String)
        Dim TotalLoadInQty, BalanceQty, PendingQty, MrpPrice, pendinginbottle, applyinbottle As Decimal

        Try
            Dim qry As String = "select Quick_SettleMent_Id from tspl_QuickSettleMent where Transfer_Number in (select Load_Out_No from TSPL_TRANSFER_HEAD where Transfer_Type='LI' and Transfer_No='" + txtTransferNo.Value + "')"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                common.clsCommon.MyMessageBoxShow("Please First Delete the Quick settlement No - " + clsCommon.myCstr(dt.Rows(0)("Quick_SettleMent_Id")), Me.Text)
                Exit Sub
            End If
            ...
            Dim strInvColumns As String = clsERPFuncationality.GetTableColumnNameForQry("TSPL_TRANSFER_HEAD", trans)
            qry = "INSERT INTO TSPL_TRANSFER_HEAD_CANCEL(" + strInvColumns + ",Cancel_By,Cancel_Date,Cancel_Remarks) SELECT " + strInvColumns + ",'" + objCommonVar.CurrentUserCode + "','" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(trans), "dd/MMM/yyyy hh:mm tt") + "','" + frm.strRmks + "' FROM TSPL_TRANSFER_HEAD WHERE Transfer_No='" + txtTransferNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
            qry = "INSERT INTO TSPL_TRANSFER_DETAIL_CANCEL SELECT * FROM TSPL_TRANSFER_DETAIL WHERE Transfer_No='" + txtTransferNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
            '------------End of Saving Data in cancel tables


            Dim obj As clsTransferMaster = clsTransferMaster.GetData(txtTransferNo.Value, trans)
            If clsCommon.myLen(obj.Route_No) > 0 AndAlso clsCommon.CompairString(obj.Transfer_Type, "LO") = CompairStringResult.Equal Then
                If clsCommon.CompairString(obj.Trans_Type, "Excise") = CompairStringResult.Equal OrElse clsCommon.CompairString(obj.Trans_Type, "Depot") = CompairStringResult.Equal Then
                    ''delete Loadin
                    qry = " select Transfer_No from  TSPL_TRANSFER_HEAD where Load_Out_No ='" + txtTransferNo.Value + "'"
                    Dim strLoadinNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry, trans))
                    connectSql.RunSpTransaction(trans, "sp_transferdetails_delete", New SqlParameter("@transfer_no", strLoadinNo))
                    connectSql.RunSpTransaction(trans, "sp_tspl_route_transferhead_delete", New SqlParameter("@transfer_no", strLoadinNo))
                    ''End of Loadin no
                End If
            End If

            connectSql.RunSpTransaction(trans, "sp_transferdetails_delete", New SqlParameter("@transfer_no", txtTransferNo.Value))
            connectSql.RunSpTransaction(trans, "sp_tspl_route_transferhead_delete", New SqlParameter("@transfer_no", txtTransferNo.Value))
            connectSql.RunSqlTransaction(trans, "delete from  TEMP_PROVISIONAL_SALES where Transfer_No='" & txtTransferNo.Value & "'")

            For Each grow As GridViewRowInfo In gv1.Rows
                If Not clsCommon.myCdbl(grow.Cells(colLoadInQty).Value) = 0 Or Not clsCommon.myCdbl(grow.Cells(colBreakage).Value) = 0 Or Not clsCommon.myCdbl(grow.Cells(colLeak).Value) = 0 Or Not clsCommon.myCdbl(grow.Cells(colShortage).Value) = 0 Then
                    If cmbitemtype.SelectedIndex = 1 Then
                        If CStr(grow.Cells(colUOM).Value) <> "SH" Then
                            MrpPrice = grow.Cells(colMRP).Value * grow.Cells(colConversion).Value
                        Else
                            MrpPrice = grow.Cells(colMRP).Value * grow.Cells(colConversion).Value
                        End If
                    Else
                        If clsCommon.myCdbl(grow.Cells(colConversion).Value) = 1 Then
                            MrpPrice = grow.Cells(colMRP).Value * grow.Cells(colConversion).Value
                        Else
                            MrpPrice = grow.Cells(colMRP).Value * grow.Cells(colConversion).Value + 100
                        End If
                    End If

                    pendinginbottle = clsCommon.myCdbl(connectSql.RunScalar(trans, "select ISNULL(Pending_Balance_In_Bottle,0) from TSPL_TRANSFER_DETAIL where Transfer_No ='" + txtLoadoutNo.Value + "'and Item_Code ='" + grow.Cells(colItemCode).Value + "'and MRP='" + Convert.ToString(MrpPrice) + "' and price_date = '" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "'"))
                    PendingQty = clsCommon.myCdbl(connectSql.RunScalar(trans, "select ISNULL(Pending_qty,0) from TSPL_TRANSFER_DETAIL where Transfer_No ='" + txtLoadoutNo.Value + "'and Item_Code ='" + grow.Cells(colItemCode).Value + "'and MRP='" + Convert.ToString(MrpPrice) + "' and price_date = '" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "'"))
                    TotalLoadInQty = Math.Round(grow.Cells(colLoadInQty).Value / grow.Cells(colConversion).Value + grow.Cells(colBreakage).Value / grow.Cells(colConversion).Value + grow.Cells(colShortage).Value / grow.Cells(colConversion).Value, 2)
                    BalanceQty = PendingQty + TotalLoadInQty + Math.Round(grow.Cells(colLeak).Value / grow.Cells(colConversion).Value, 2)
                    If clsCommon.myCdbl(grow.Cells(colConversion).Value) = 1 Then
                        Dim convfactor As Decimal = clsCommon.myCdbl(connectSql.RunScalar(trans, "select UD.Conversion_Factor   from TSPL_ITEM_UOM_DETAIL UD JOIN TSPL_UNIT_MASTER UM ON UD.UOM_Code = UM.Unit_Code  where Item_Code= '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and UOM_Code <> '" + Convert.ToString(grow.Cells(colUOM).Value) + "' AND UM.Create_Price = 'Y'"))
                        applyinbottle = clsCommon.myCdbl(grow.Cells(colLoadInQty).Value) + clsCommon.myCdbl(grow.Cells(colBreakage).Value) + clsCommon.myCdbl(grow.Cells(colShortage).Value) + clsCommon.myCdbl(grow.Cells(colLeak).Value)
                        pendinginbottle = pendinginbottle + applyinbottle * convfactor
                    Else
                        applyinbottle = clsCommon.myCdbl(grow.Cells(colLoadInQty).Value) + clsCommon.myCdbl(grow.Cells(colBreakage).Value) + clsCommon.myCdbl(grow.Cells(colShortage).Value) + clsCommon.myCdbl(grow.Cells(colLeak).Value)
                        pendinginbottle = pendinginbottle + applyinbottle
                    End If
                    Dim QryChk As String = "update TSPL_TRANSFER_DETAIL set pending_qty='" + Convert.ToString(BalanceQty) + "', Pending_Balance_In_Bottle = '" + Convert.ToString(pendinginbottle) + "' where Transfer_No='" + txtLoadoutNo.Value + "' and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date = '" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP='" + Convert.ToString(MrpPrice) + "'"
                    connectSql.RunSqlTransaction(trans, "update TSPL_TRANSFER_DETAIL set pending_qty='" + Convert.ToString(BalanceQty) + "', Pending_Balance_In_Bottle = '" + Convert.ToString(pendinginbottle) + "' where Transfer_No='" + txtLoadoutNo.Value + "' and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date = '" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP='" + Convert.ToString(MrpPrice) + "'")
                    If cmbitemtype.Text = "Empty" Then
                        connectSql.RunSqlTransaction(trans, "update TSPL_TRANSFER_DETAIL set Pending_Qty =Item_Qty , Pending_Balance_In_Bottle = Item_Qty*(select Conversion_Factor  from TSPL_ITEM_UOM_DETAIL where TSPL_ITEM_UOM_DETAIL.Item_Code =TSPL_TRANSFER_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.UOM_Code='EB' ) where TSPL_TRANSFER_DETAIL.Transfer_No ='" + txtLoadoutNo.Value + "'")
                    Else
                        connectSql.RunSqlTransaction(trans, "update TSPL_TRANSFER_DETAIL set Pending_Qty =Item_Qty , Pending_Balance_In_Bottle = Item_Qty*(select Conversion_Factor  from TSPL_ITEM_UOM_DETAIL where TSPL_ITEM_UOM_DETAIL.Item_Code =TSPL_TRANSFER_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.UOM_Code='FB' ) where TSPL_TRANSFER_DETAIL.Transfer_No ='" + txtLoadoutNo.Value + "'")
                    End If
                    connectSql.RunSqlTransaction(trans, "update TEMP_PROVISIONAL_SALES set  LoadIn_EmptyValue=0, Shortage=0, Amount=0, LoadInQty = 0, Breakage =0, Leak = 0 where Transfer_No = '" + txtLoadoutNo.Value + "' ")

                End If
            Next grow

            If clsCommon.myLen(obj.Reference_Doc_No) > 0 Then
                'Dim obj3rdDoc As clsTransferMaster = clsTransferMaster.GetData(obj.Reference_Doc_No, trans)
                'If clsCommon.myLen(obj3rdDoc.Reference_Doc_No) > 0 AndAlso clsCommon.myLen(obj3rdDoc.Reference_Doc_No_DPL) > 0 Then
                ...
                '    End If
                '    Dim obj1stDoc As clsTransferMaster
                '    If clsCommon.CompairString(obj.Transfer_No, obj3rdDoc.Reference_Doc_No) = CompairStringResult.Equal Then
                '        obj1stDoc = clsTransferMaster.GetData(obj3rdDoc.Reference_Doc_No_DPL, trans)

                '    Else
                '        obj1stDoc = clsTransferMaster.GetData(obj3rdDoc.Reference_Doc_No, trans)

                '    End If
                '    'Dim objSMLO As clsTransferMaster = clsTransferMaster.createLoadout(obj1stDoc, strSalesmanLocation, False, trans)
                'Else
                ''delete 3 rd document 
                connectSql.RunSpTransaction(trans, "sp_transferdetails_delete", New SqlParameter("@transfer_no", obj.Reference_Doc_No))
                connectSql.RunSpTransaction(trans, "sp_tspl_route_transferhead_delete", New SqlParameter("@transfer_no", obj.Reference_Doc_No))

                qry = "update TSPL_TRANSFER_HEAD set Reference_Doc_No='' where Transfer_No in (" + obj.Reference_Doc_No_DPL + ")"
                clsDBFuncationality.ExecuteNonQuery(qry, trans)
                ''End of delete 3 rd document 
                'End If
            End If
    ...
    Public Sub PostData()
        ...
        If myMessages.postConfirm() Then
            Try
                If chkIsAutoCreate.Checked Then
                    If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal Then
                        Throw New Exception("Auto Generated Transaction can't be Directly Post")
                        'Else
                        'Dim arrLONO As New List(Of String)
                        'If clsCommon.myLen(lblReferenceDocumentNo.Text) > 0 Then
                        '    arrLONO.Add(lblReferenceDocumentNo.Text)
                        'End If
                        'If clsCommon.myLen(lblReferenceDocumentNo2.Text) > 0 Then
                        '    arrLONO.Add(lblReferenceDocumentNo2.Text)
                        'End If
                        'If arrLONO.Count > 0 Then
                        '    Sql = "select Transfer_No from TSPL_TRANSFER_HEAD where  Transfer_No in (" + clsCommon.GetMulcallString(arrLONO) + ") and Post='N'"
                        '    Dim dt As DataTable = clsDBFuncationality.GetDataTable(Sql, trans)
                        '    If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                        '        Sql = ""
                        '        For Each dr As DataRow In dt.Rows
                        '            Sql += Environment.NewLine + clsCommon.myCstr(dr("Transfer_No"))
                        '        Next
                        '        Throw New Exception("Please first post following Transafer " + Sql)
                        '    End If
                        'End If
                    End If
                End If

                If clsTransferMaster.postTransfer(txtTransferNo.Value) Then
                    myMessages.post()
                    rdbtnPost.Enabled = False
    ...
    Public Sub FillItemDetails(ByVal TransferNo As String)
        LoadBlankGrid()
        Dim sql As String = "SELECT convert(varchar(10),Price_Date,103) as Date , Item_Qty,Assessable_Amt, Amount, Net_Amount, Pending_Qty,LoadIn_Qty,Total_Tax,Total_Item_Amt,uom,breakage,MRP, TSPL_TRANSFER_DETAIL.Item_Code, TSPL_TRANSFER_DETAIL.Item_Desc, Basic_Price,Item_Price,TSPL_TRANSFER_DETAIL.batch_no,BasicPrice_WithTax,Empty_Value,TPT_Value, Burst, Leak , Shortage, (select Conversion_Factor  from TSPL_ITEM_UOM_DETAIL where Item_Code = TSPL_TRANSFER_DETAIL.Item_Code and UOM_Code = TSPL_TRANSFER_DETAIL.Uom ) as [ConvFactor],mrp_in_bottle ,MRP*(select Conversion_Factor  from TSPL_ITEM_UOM_DETAIL where Item_Code = TSPL_TRANSFER_DETAIL.Item_Code and UOM_Code = TSPL_TRANSFER_DETAIL.Uom ) AS [ORDERBY],case when UOM ='SH' then ( select max(Sku_Seq)+1  from TSPL_ITEM_MASTER) else Sku_Seq end SKuSeq FROM tspl_transfer_detail left outer join TSPL_ITEM_MASTER on TSPL_TRANSFER_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code  where transfer_no='" + TransferNo + "' ORDER BY  SKuSeq ,ORDERBY,Uom desc  "
        ...
        For Each grow As GridViewRowInfo In gv1.Rows
            If cmbitemtype.SelectedIndex = 2 Then
                If clsCommon.myCdbl(grow.Cells(colConversion).Value) = 1 Then
                    ...
                    PendingQty = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Pending_Qty,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(MrpPrice) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
                    grow.Cells(colPendingQty).Value = PendingQty
                    PendingQtyInBottle = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Pending_Balance_In_Bottle,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(MrpPrice) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
                    grow.Cells(colPensingBalanceInBottle).Value = PendingQtyInBottle
                    grow.Cells(colTransferQUANTITY).Value = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Item_Qty,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(MrpPrice) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
                    PendingQty = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Pending_Qty,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
                Else
                    MrpPrice = clsCommon.myCdbl(grow.Cells(colMRP).Value) * clsCommon.myCdbl(grow.Cells(colConversion).Value) + 100
                    PendingQty = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Pending_Qty,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(grow.Cells(colMRP).Value) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
                    grow.Cells(colPendingQty).Value = PendingQty
                    PendingQtyInBottle = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Pending_Balance_In_Bottle,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(MrpPrice) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
                    grow.Cells(colPensingBalanceInBottle).Value = PendingQtyInBottle
                    grow.Cells(colTransferQUANTITY).Value = clsCommon.myCdbl(connectSql.RunScalar("select ISNULL(Item_Qty,0) from TSPL_TRANSFER_DETAIL where Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date ='" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP = '" + Convert.ToString(MrpPrice) + "' and transfer_no = '" + Convert.ToString(txtLoadoutNo.Value) + "'"))
    ...
    Private Sub funfillofloadinagainstloadout()
        ...
        Dim sql As String = "SELECT convert(varchar(10),Price_Date,103) as Date , Item_Qty,Assessable_Amt, Amount, Net_Amount, Pending_Qty,LoadIn_Qty,Total_Tax,Total_Item_Amt,uom,breakage,MRP, Item_Code, Item_Desc, Basic_Price,Item_Price,batch_no,BasicPrice_WithTax,Empty_Value,TPT_Value, Burst, Leak , Shortage FROM tspl_transfer_detail where transfer_no='" + txtTransferNo.Value + "'"
    ...
    Private Sub FunEmpName()
        lblSalesman.Text = connectSql.RunScalar("select Emp_Name  from TSPL_EMPLOYEE_MASTER  where EMP_CODE = '" + Convert.ToString(txtSalesman.Value) + "'")
    End Sub
        ...
        If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal Then
            ...
            If pdDR.Rows.Count > 0 Then
                For Each row As DataRow In pdDR.Rows
                    ...
                    Dim query As String = "select transfer_no from TSPL_TRANSFER_DETAIL where transfer_no='" + txtTransferNo.Value + "'and Item_Code='" + datarowinfo.Cells(colItemCode).Value.ToString() + "'"
               ...
               " AND location_code='" + loc + "' AND MRP='" + datarowinfo.Cells(colMRP).Value.ToString() + "' "
                        ...
                        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal Then
                            ...
                            Dim sql3 As String = "Select to_location from tspl_transfer_head where transfer_no='" + txtLoadoutNo.Value + "' "
                    ...
                    Else
                    End If
                    Dim query1 As String = "select item_price from tspl_transfer_detail where transfer_no ='" + txtTransferNo.Value + "'and item_code='" + datarowinfo.Cells(colItemCode).Value.ToString() + "'"
...
" AND location_code='" + Convert.ToString(txtToLocation.Value) + "' AND MRP='" + grow.Cells(colMRP).Value.ToString() + "' "
            ...
            If FunItemLocationUpdateLoadIn(trans) Then
              ...
              " TSPL_PURCHASE_ACCOUNTS AS PA ON IM.Purchase_Class_Code = PA.Purchase_Class_Code INNER JOIN " & _
               " TSPL_GL_ACCOUNTS AS GLA ON PA.Inv_Control_Account = GLA.Account_Code WHERE IM.Item_Code='" + gv1.Rows(0).Cells(colItemCode).Value + "'"
                ...
                '                  " where H.Transfer_No  ='" + fndloadno.Value + "'"
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim Qry As String = "select DISTINCT total_item_cost from TSPL_TRANSFER_DETAIL L inner join TSPL_TRANSFER_HEAD H on L.Transfer_No=H.Transfer_No" & _
                                " where H.Transfer_No  ='" + txtLoadoutNo.Value + "'  and L.Item_Code ='" + grow.Cells(colItemCode).Value + "' "
                ...
                If Not (strToLType = "Logical" OrElse strFromLType = "Logical") Then
                    If cmbitemtype.Text = "Full" Then
                        Dim strFrmFilledAcc As String = Nothing
                        Dim Loc1 As String = "select From_Location  from TSPL_TRANSFER_HEAD where Transfer_No ='" + txtLoadoutNo.Value + "'"
                ...
                If cmbitemtype.SelectedIndex <> 2 Then
                    ...
                    If Not (strToLType = "Logical" OrElse strFromLType = "Logical") Then
                        ...
                        Next
                        Dim Loc2 As String = "select From_Location  from TSPL_TRANSFER_HEAD where Transfer_No ='" + txtLoadoutNo.Value + "'"
    ...
    Private Function FunItemLocationUpdateLoadIn(ByVal trans As SqlTransaction) As Boolean
        ...
        " AND location_code='" + Convert.ToString(txtFromLoaction.Value) + "' AND MRP='" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "' and batch_no = '" + grow.Cells(colBatchNo).Value + "' "
                            LoadOutItmCost = connectSql.RunScalar(trans, " select  Total_Item_Cost from TSPL_TRANSFER_DETAIL where Transfer_No ='" + txtLoadoutNo.Value + "' and Item_Code ='" + grow.Cells(colItemCode).Value.ToString() + "'" & _
                                 " AND MRP='" + Convert.ToString(mrpprice1) + "' and batch_no = '" + grow.Cells(colBatchNo).Value + "' ")
                            pndqty = connectSql.RunScalar(trans, "select Pending_qty from TSPL_TRANSFER_DETAIL where Transfer_No ='" + txtLoadoutNo.Value + "'and Item_Code ='" + grow.Cells(colItemCode).Value + "'and MRP='" + Convert.ToString(mrpprice1) + "' and price_date = '" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "'")
                        Else
                            If clsCommon.myCdbl(grow.Cells(colConversion).Value) <> 1 Then
                        ...
                        'connectSql.RunSqlTransaction(trans, "update TSPL_TRANSFER_DETAIL set pending_qty='" + Convert.ToString(BalanceQty) + "' where Transfer_No='" + fndloadno.Value + "' and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and price_date = '" + CDate(grow.Cells(colPriceDate).Value).ToString("yyyy-MM-dd") + "' and MRP='" + Convert.ToString(mrpprice1) + "'")
                        Dim loadintem As Decimal = clsCommon.myCdbl(connectSql.RunScalar(trans, "select loadinqty  from TEMP_PROVISIONAL_SALES where Transfer_No='" + txtLoadoutNo.Value + "'  and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and mrp = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "'"))
                        Dim breakagetem As Decimal = clsCommon.myCdbl(connectSql.RunScalar(trans, "select  breakage  from TEMP_PROVISIONAL_SALES where Transfer_No='" + txtLoadoutNo.Value + "'  and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and mrp = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "'"))
                        Dim leaktem As Decimal = clsCommon.myCdbl(connectSql.RunScalar(trans, "select leak from TEMP_PROVISIONAL_SALES where Transfer_No='" + txtLoadoutNo.Value + "'  and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and mrp = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "'"))
                        Dim Amount As Decimal = clsCommon.myCdbl(connectSql.RunScalar(trans, "select isnull(Amount,0) from TEMP_PROVISIONAL_SALES where Transfer_No='" + txtLoadoutNo.Value + "'  and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and mrp = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "'"))
                        Dim Shortage As Decimal = clsCommon.myCdbl(connectSql.RunScalar(trans, "select Shortage from TEMP_PROVISIONAL_SALES where Transfer_No='" + txtLoadoutNo.Value + "'  and item_code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and mrp = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "'"))
                        ...
                        connectSql.RunSqlTransaction(trans, "update TEMP_PROVISIONAL_SALES set Shortage=" + Shortage.ToString() + ", Amount=" + Amount.ToString() + ", LoadInQty = '" + Convert.ToString(loadintem) + "', Breakage ='" + Convert.ToString(breakagetem) + "', Leak = '" + Convert.ToString(leaktem) + "' where Transfer_No = '" + txtLoadoutNo.Value + "' and Item_Code = '" + Convert.ToString(grow.Cells(colItemCode).Value) + "' and MRP = '" + Convert.ToString(grow.Cells(colMRP).Value * grow.Cells(colConversion).Value) + "'")
    ...
    Public Sub FunFillItemDetails_LoadOut_No(Optional ByVal LoadOutNo As String = Nothing)
        ...
        If dr1.Rows.Count > 0 Then
            For Each row As DataRow In dr1.Rows
                ...
                Dim cost As Decimal = clsCommon.myCdbl(connectSql.RunScalar("select item_price from tspl_transfer_detail where transfer_no ='" + txtTransferNo.Value + "'and item_code='" + grow.Cells(colItemCode).Value.ToString() + "'"))
                If cost <> 0 Then
    ...
    Private Sub funloadinagainstloadout(Optional ByVal itemtype As String = "ALL")
        ...
        If dr1.Rows.Count > 0 Then
            ...
            For Each row As DataRow In dr1.Rows
                ...
                End If
                'dr12.Close()
                Dim query1 As String = "select item_price from tspl_transfer_detail where transfer_no ='" + txtTransferNo.Value + "'and item_code='" + grow.Cells(colItemCode).Value.ToString() + "'"
                Dim cost As Decimal = clsCommon.myCdbl(connectSql.RunScalar(query1))
                If cost <> 0 Then
                    grow.Cells(colItemCost).Value = cost
                Else
    ...
    Public Sub itemfunfill_loadoutno()
        Try
            Dim myreader As DataTable = clsDBFuncationality.GetDataTable("SELECT CONVERT(varchar(10),Transfer_Date,103)as transfer_date ,CONVERT(varchar(10),Posting_Date,103) as posting_date, Convert(varchar(10),Load_out_date,103) as Load_Out_Date,To_Location,From_Location,Price_Code,convert(varchar(10),Price_Date,103)as price_date, Reference, description, Route_No,Salesmancode, Vehicle_Code, Vehicle_No, Mode_Of_Transport, Km_Reading, Item_Amount,Total_Item_Amount,post,trip_no,item_type,Trans_Type FROM tspl_transfer_head where transfer_no='" + txtLoadoutNo.Value + "'")
            If myreader.Rows.Count > 0 Then
                For Each row As DataRow In myreader.Rows
                    txtTransferDate.Value = CDate(row("transfer_date")).ToString("dd/MM/yyyy")
                    txtTransferDate.Text = CDate(row("posting_date").ToString()).ToString("dd/MM/yyyy")
                    txtLoadoutDate.Value = CDate(row("transfer_date")).ToString("dd/MM/yyyy")
                    ...
                    txtRouteNo.Enabled = False
                    txtSalesman.Value = Convert.ToString(row("Salesmancode"))
                    txtVehicleCode.Value = Convert.ToString(row("Vehicle_Code"))
                    'txtVehicle.Text = txtVehicleCode.Value
    ...
    Public Sub PrintDataNew(ByVal isPrePrint As Boolean)
        PrintDataNew(isPrePrint, False)
    End Sub
    Public Sub PrintDataNew(ByVal isPrePrint As Boolean, ByVal isPrePrintTAx As Boolean)
        Try
            If clsCommon.myLen(txtTransferNo.Value) > 0 Then
                If clsCommon.myCdbl(txtLoadInOutAmt.Text) > 0 Then
                    Dim rp As ReportTransfer = New ReportTransfer
                    rp.PrintData(txtTransferNo.Value, isPrePrint, isPrePrintTAx)
                Else
                    Dim qry As String = "select Post from TSPL_TRANSFER_HEAD where Transfer_No='" + txtTransferNo.Value + "'"
                    If clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry)), "N") = CompairStringResult.Equal Then
                        ...
                        Else
                            isCorrect = "Y"
                        End If
                        qry = "update TSPL_TRANSFER_HEAD set printed='" + isCorrect + "' where transfer_no='" + txtTransferNo.Value + "'"
    ...
    Public Sub printdata(ByVal isPrePrint As Boolean)
        Try
            If clsCommon.myLen(txtTransferNo.Value) > 0 Then
                Dim qry As String = "select Transfer_Type,Tax_Group from TSPL_TRANSFER_HEAD where Transfer_No='" + txtTransferNo.Value + "'"
                Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
        ...
        If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal Then
            ...
            If pdDR.Rows.Count > 0 Then
                ...
                For Each row As DataRow In pdDR.Rows
                    ...
                    datarowinfo.Cells(colBatchNo).Value = connectSql.RunScalar("SELECT top 1 Batch_No  FROM TSPL_ITEM_LOCATION_DETAILS WHERE Item_Code = '" + Convert.ToString(datarowinfo.Cells(colItemCode).Value) + "' AND Location_Code = '" + Convert.ToString(txtFromLoaction.Value) + "' AND MRP = '" + Convert.ToString(datarowinfo.Cells(colMRP).Value) + "' AND Batch_No <> '' and Item_Qty>0 order by Expiry_Date")
                    ...
                    Dim cost As Decimal = clsCommon.myCdbl(connectSql.RunScalar("select item_price from tspl_transfer_detail where transfer_no ='" + txtTransferNo.Value + "'and item_code='" + datarowinfo.Cells(colItemCode).Value.ToString() + "'"))
                    If cost <> 0 Then
                        datarowinfo.Cells(colItemCost).Value = cost
                    Else
                    ...
                    If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Excise") = CompairStringResult.Equal Then
                        datarowinfo.Cells(colTaxRate4).Value = 0
                        datarowinfo.Cells(colTaxRate5).Value = 0
                        datarowinfo.Cells(colTaxRate6).Value = 0
                        datarowinfo.Cells(colTaxRate7).Value = 0
                        datarowinfo.Cells(colTaxRate8).Value = 0
                        datarowinfo.Cells(colTaxRate9).Value = 0
                        datarowinfo.Cells(colTaxRate10).Value = 0
                    End If
    ...
    Private Sub fndfromlocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtFromLoaction._MYValidating
        ...
        txtFromLoaction.Value = clsCommon.ShowSelectForm("From Location", qry, "Code", whrclas, txtFromLoaction.Value, "Code", isButtonClicked)
    ...
    Private Sub fndtolocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtToLocation._MYValidating
        ...
        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal Then
            ...
            End If
            txtToLocation.Value = clsCommon.ShowSelectForm("To Location", qry, "Code", whrclas, txtToLocation.Value, "Code", isButtonClicked)
        ElseIf clsLocation.isLocatinExcisable(txtFromLoaction.Value) AndAlso clsCommon.myLen(txtRouteNo.Value) > 0 Then
            ...
            txtToLocation.Value = clsCommon.ShowSelectForm("To Location", qry, "GITLocation", whrclas, txtToLocation.Value, "GITLocation", isButtonClicked)
        Else
            If clsCommon.CompairString(cmbitemtype.Text, "Full") = CompairStringResult.Equal Then
                whrclas += " and LM.Excisable<>'T'"
            End If
            If clsCommon.myLen(txtRouteNo.Value) > 0 Then
                whrclas += " and LM.Excisable ='F' AND LM.Location_Type ='Logical'"
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrclas += " and LM.Location_Code in (" + objCommonVar.strCurrUserLocations + ")"
        End If
        lblToLocation.Text = clsLocation.GetName(txtToLocation.Value, Nothing)
        If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Route") = CompairStringResult.Equal Then
            txtSalesman.Value = txtToLocation.Value
            lblSalesman.Text = lblToLocation.Text
        End If
        ...
        If IsRouteFillAgain = False Then
            ...
            If clsCommon.myLen(txtRouteNo.Value) > 0 Then
                ...
                If myreader IsNot Nothing AndAlso myreader.Rows.Count > 0 Then
                    txtSalesman.Value = Convert.ToString(myreader.Rows(0)("employee_code"))
                    lblSalesman.Text = clsLocation.GetName(txtSalesman.Value, Nothing)
                    txtToLocation.Value = Convert.ToString(myreader.Rows(0)("employee_code"))

                    If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Excise") = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Depot") = CompairStringResult.Equal Then
    ...
    Private Sub fndPriceCode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtPriceCode._MYValidating
        ...
        txtPriceCode.Value = clsCommon.ShowSelectForm("Price Code", qry, "Price_Code", "", txtPriceCode.Value, "Price_Code", isButtonClicked)
        lblPriceCode.Text = connectSql.RunScalar("select Price_Code_Desc  from TSPL_PRICE_COMPONENT_MAPPING where Price_Code =  '" + Convert.ToString(txtPriceCode.Value) + "'")
    ...
    Private Sub fndloadno__MYValidating_1(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtLoadoutNo._MYValidating
        ...
        Dim qry As String = ""
        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal Then
            ' qry = "select distinct TSPL_TRANSFER_HEAD .transfer_no ,description as 'Description',CONVERT(varchar(15), Transfer_Date , 103) as [Transfer Date], From_Location as [From Location], To_Location as [To Location], Reference   from TSPL_TRANSFER_HEAD inner join TSPL_TRANSFER_DETAIL on TSPL_TRANSFER_HEAD.Transfer_No=TSPL_TRANSFER_DETAIL.Transfer_No "
            qry = "select distinct Transfer_No ,description ,[Transfer Date] ,[From Location] ,[To Location],Reference_Doc_No as [Reference Doc No] from (select distinct TSPL_TRANSFER_HEAD .transfer_no ,description as 'Description',CONVERT(varchar(15), Transfer_Date , 103) as [Transfer Date], From_Location as [From Location], To_Location as [To Location], Reference,case when len(Route_No)>0 then From_Location else To_Location end as [Location]   ,Post,Transfer_Type ,Pending_Qty,Reference_Doc_No  from TSPL_TRANSFER_HEAD inner join TSPL_TRANSFER_DETAIL on TSPL_TRANSFER_HEAD.Transfer_No=TSPL_TRANSFER_DETAIL.Transfer_No ) as Query"
            If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                WhrCls += " and  Query.Location  in(" + objCommonVar.strCurrUserLocations + " ) "
            End If
            txtLoadoutNo.Value = clsCommon.ShowSelectForm("Load Out NO", qry, "transfer_no", "Query.[From Location]  in (Select location_code from TSPL_LOCATION_MASTER) and Post ='Y' and Transfer_Type ='LO' and Pending_Qty > 0 AND Query.Post ='Y' and Query .transfer_no not in(select Load_Out_No from TSPL_TRANSFER_HEAD  )" + WhrCls + "", txtLoadoutNo.Value, "transfer_no", isButtonClicked)
            If clsCommon.myLen(txtLoadoutNo.Value) <= 0 Then
                Exit Sub
            End If
        Else
            ...
            End If
            txtLoadoutNo.Value = clsCommon.ShowSelectForm("Load Out NO", qry, "transfer_no", "From_Location in (Select location_code from TSPL_LOCATION_MASTER) and Post ='Y' and Transfer_Type ='LO' and Pending_Qty > 0 AND  TSPL_TRANSFER_HEAD.Post ='Y' and TSPL_TRANSFER_HEAD .transfer_no not in(select Load_Out_No from TSPL_TRANSFER_HEAD  ) " + WhrCls + "", txtLoadoutNo.Value, "transfer_no", isButtonClicked)
        End If
        Dim LoadOut As String = connectSql.RunScalar("Select distinct TSPL_TRANSFER_HEAD.Transfer_No from tspl_transfer_head inner join TSPL_TRANSFER_DETAIL on TSPL_TRANSFER_HEAD.Transfer_No=TSPL_TRANSFER_DETAIL.Transfer_No  where TSPL_TRANSFER_HEAD.Transfer_No='" + txtLoadoutNo.Value + "' and  Pending_Qty > 0 AND TSPL_TRANSFER_HEAD.Post ='Y' and TSPL_TRANSFER_HEAD .transfer_no not in(select Load_Out_No from TSPL_TRANSFER_HEAD  ) ")
        If LoadOut IsNot Nothing Then
            If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal Then
                itemfunfill_loadoutno()
            ElseIf clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal Then
    ...
    Private Function GetReportID() As String
        If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal Then
            Return "LoadoutMainGrid"
        Else
            Return "LoadinMainGrid"
        End If
    End Function
    ...
    Private Sub txtVehicle_Leave(ByVal sender As Object, ByVal e As System.EventArgs)
    ...
    Private Sub txtVehicle_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            'Dim Qry As String = "select  Load_Out_No,Transfer_No from TSPL_TRANSFER_HEAD where len(Route_No)>0 and Transfer_Type ='LI' and Load_Out_No<>''  and Transfer_Date ='2012-06-01'"
            'Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry)
            'clsTransferMaster.FunInsertTEMP_PROVISIONAL(dt)
            Dim Qry As String = "select Voucher_No from TSPL_JOURNAL_MASTER where Source_Doc_No  ='" + txtTransferNo.Value + "'"
            Dim VoucherNo As String = clsDBFuncationality.getSingleValue(Qry, trans)
            ...
            Qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No ='" + txtTransferNo.Value + "' and Trans_Type ='Transfer'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TEMP_PROVISIONAL_SALES where Transfer_No ='" + txtLoadoutNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Dim obj As clsTransferMaster = clsTransferMaster.GetData(txtTransferNo.Value, trans)
            clsTransferMaster.FunItemLocationUpdateLoadInRrvs(trans, obj)

            Dim EmptyAdj As String = clsDBFuncationality.getSingleValue(" select Adjustment_No  from TSPL_ADJUSTMENT_HEADER where Document_No ='" + txtLoadoutNo.Value + "' and Reference_Document ='Load Out/Transfer'")

            If clsCommon.myLen(EmptyAdj) > 0 Then
                Qry = "delete from TSPL_ADJUSTMENT_DETAIL  where Adjustment_No='" + EmptyAdj + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)

                Qry = "delete from TSPL_ADJUSTMENT_HEADER  where Adjustment_No='" + EmptyAdj + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            ...
            Qry = " update TSPL_TRANSFER_HEAD set quick_settlement='N'  where Transfer_No='" + txtLoadoutNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TSPL_TRANSFER_DETAIL  where Transfer_No='" + txtTransferNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TSPL_TRANSFER_HEAD  where Transfer_No='" + txtTransferNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
    ...
    Private Sub funRollbackLoadIn()
        Try
            ...
            Dim QuickSettlementNo As String = clsDBFuncationality.getSingleValue(" select distinct Quick_SettleMent_Id  from tspl_QuickSettleMent  where Transfer_Number='" + txtLoadoutNo.Value + "'", trans)
            If clsCommon.myLen(QuickSettlementNo) > 0 Then
                Qry = " delete from tspl_QuickSettleMent where Quick_SettleMent_Id='" + QuickSettlementNo + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            End If

            Dim EmptyAdj As String = clsDBFuncationality.getSingleValue(" select Adjustment_No  from TSPL_ADJUSTMENT_HEADER where Document_No ='" + txtLoadoutNo.Value + "' and Reference_Document ='Load Out/Transfer'", trans)
    ...
    Private Sub btnRollback_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRollback.Click
        Dim frm As New FrmPWD(Nothing)
        frm.strCode = "LoadInRollback"
        frm.strType = "TF"
        frm.ShowDialog()
        If frm.isPasswordCorrect Then
            If clsCommon.myLen(txtTransferNo.Value) > 0 And clsCommon.myLen(txtLoadoutNo.Value) > 0 Then
                funRollbackLoadIn()
            End If
        End If
    ...
    Private Function AllowToSave() As Boolean
        ...
        End If
        If txtTransferDate.ShowCheckBox AndAlso Not txtTransferDate.Checked Then
            txtTransferDate.Focus()
        ...
        Dim IsPOst As Decimal = clsCommon.myCdbl(connectSql.RunScalar("select 1 from TSPL_TRANSFER_HEAD where Transfer_No ='" + txtTransferNo.Value + "' and Post ='Y'"))
        If IsPOst = 1 Then
            Throw New Exception("Document Already Posted.")
        End If
        If clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.TransferTransTypeRouteHide, clsFixedParameterCode.TransferTransTypeRouteHide, Nothing)) = 1 Then
            If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Route") = CompairStringResult.Equal Then
                Throw New Exception("Can not create direcly Route Type Transaction ")
            End If
        End If
        Dim qry As String = ""
        ''Salesman check
        ''If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Route") = CompairStringResult.Equal Then
        ''    qry = "select Transfer_No from TSPL_TRANSFER_HEAD where To_Location='" + txtToLocation.Value + "' and Transfer_Type='LO' and  Transfer_No not in (select Load_Out_No from TSPL_TRANSFER_HEAD) and Transfer_No not in ('" + txtTransferNo.Value + "')"
        ''    Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)

        ''    If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
        ''        Dim strMsg As String = "Please First Make Loadin of Loadout no" + Environment.NewLine
        ''        For Each dr As DataRow In dt.Rows
        ''            strMsg += clsCommon.myCstr(dr("Transfer_No")) + Environment.NewLine
        ''        Next
        ''        Throw New Exception(strMsg)
        ''    End If
        ''End If
        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal Then
            qry = "select EntryDateTime  from TSPL_TRANSFER_HEAD where Transfer_No='" + txtLoadoutNo.Value + "'"
        ...
        If (clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal And clsCommon.myLen(txtRouteNo.Value) = 0) Then
            Dim LoadOutDate As Date = clsCommon.myCDate(connectSql.RunScalar("Select distinct TSPL_TRANSFER_HEAD.Transfer_Date from tspl_transfer_head   where TSPL_TRANSFER_HEAD.Transfer_No='" + txtLoadoutNo.Value + "'  AND TSPL_TRANSFER_HEAD.Post ='Y'  "))
        ...
        For count As Integer = 0 To gv1.Rows.Count - 1
            ...
            If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal Then
                If dblQty > 0 AndAlso (clsCommon.CompairString(strUOM, "FB") = CompairStringResult.Equal OrElse clsCommon.CompairString(strUOM, "FC") = CompairStringResult.Equal) Then
                    Dim dblBalQty As Double = Math.Round(clsItemLocationDetails.getBalanceWithUnapprove(strICode, txtFromLoaction.Value, strMRP, strUOM, txtTransferNo.Value, txtTransferDate.Value), 2, MidpointRounding.ToEven)
            ...
            If clsCommon.myLen(txtIndentNo.Value) > 0 Then
                Dim dblBalQty As Double = clsIndentDetail.GetBalanceQty(txtIndentNo.Value, strICode, clsCommon.myCdbl(strMRP), txtTransferNo.Value, True)
                If dblQty > dblBalQty Then
                    Throw New Exception("Item - " + strICode + " , MRP - " + strMRP + Environment.NewLine + "Entered Quantity - " + clsCommon.myCstr(dblQty) + " and Actual Balance Quantity of Indent With Tolerance- " + clsCommon.myCstr(dblBalQty) + " at Row No " + clsCommon.myCstr(count + 1))
                End If
            End If
        ...
        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal And LOcTypeChk <> "Logical" Then
            ...
            ElseIf cmbitemtype.SelectedIndex = 1 Then
                CUom = "FB"
            End If
            Dim LOudOutBottleTot As Decimal = clsCommon.myCdbl(connectSql.RunScalar(" select isnull(SUM(tot),0) from ( select Item_Qty * (select Conversion_Factor  from TSPL_ITEM_UOM_DETAIL where TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_TRANSFER_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.UOM_Code ='" + CUom + "' ) as tot from TSPL_TRANSFER_DETAIL  where Transfer_No ='" + txtLoadoutNo.Value + "')as xxx"))
        ...
        If clsCommon.CompairString(cboTransferType.Text, "Load In") = CompairStringResult.Equal And Not (txtLoadoutNo.Value = String.Empty) Then
            Dim LoadInNO As String = clsDBFuncationality.getSingleValue("select top 1 Transfer_No from TSPL_TRANSFER_HEAD where Load_Out_No ='" + txtLoadoutNo.Value + "' and Transfer_No not in ('" + txtTransferNo.Value + "')")
            If clsCommon.myLen(LoadInNO) > 0 Then
                common.clsCommon.MyMessageBoxShow("LoadOut No. already has been used by LoadIn No:'" + LoadInNO + "'  ", "Transfer", MessageBoxButtons.OK, RadMessageIcon.Info)
                txtLoadoutNo.Value = ""
                Return False
            End If
        End If
        ...
        '    Dim sqlRouteCheck As String = "select 1  from TSPL_TRANSFER_HEAD where Route_No ='" + txtRouteNo.Value + "' and convert(date,Transfer_Date,103) = convert(date,'" + clsCommon.myCDate(txtTransferDate.Value, "dd/MM/yyyy") + "',103)and Transfer_Type ='LO' and LEN(Route_No )>0"
    ...
    Private Function SaveData() As Boolean
        Try
            Dim obj As clsTransferMaster = New clsTransferMaster()
            obj.Transfer_No = txtTransferNo.Value
            obj.Reference_Doc_No = lblReferenceDocumentNo.Text
            obj.Reference_Doc_No_DPL = lblReferenceDocumentNo2.Text
            txtTransferDate.Value = New DateTime(txtTransferDate.Value.Year, txtTransferDate.Value.Month, txtTransferDate.Value.Day, txtTime.Value.Hour, txtTime.Value.Minute, txtTime.Value.Second)
            ...
            If isNewEntry AndAlso clsCommon.myLen(txtMannaulInvoiceNo.Text) > 0 Then
                obj.ManualTransferNo = txtMannaulInvoiceNo.Text
            End If
            ...
            'obj.Printed
            'obj.Is_Shipped()
            obj.Against_Indent_No = txtIndentNo.Value
            obj.Arr = New List(Of clsTransferDetails)
            Dim counter As Integer = 1
            For Each grow As GridViewRowInfo In gv1.Rows

                Dim objTr As New clsTransferDetails()
    ...
    Private Sub fndTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndTaxGroup._MYValidating
        ...
        fndTaxGroup.Value = clsCommon.ShowSelectForm("transferTaxgroup", qry, "Code", whrclas, fndTaxGroup.Value, "Code", isButtonClicked)
        funtaxgroupfill()
    End Sub
    ...
    Public Sub funtaxgroupfill()
      ...
      " FROM TSPL_TAX_GROUP_DETAILS AS G INNER JOIN TSPL_TAX_RATES AS R ON G.Tax_Code = R.Tax_Code " & _
        ...
        End If
        If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Excise") = CompairStringResult.Equal Then
            For ii As Integer = 3 To gv2.RowCount - 1
                gv2.Rows(ii).Cells(2).Value = 0
            Next
        End If
    End Sub
    ...
    Private Sub fndtrnasferno__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTransferNo._MYValidating
        ...
        Dim qry As String = "Select Transfer_no ,Description, (case when Transfer_Type = 'LO' THEN 'Load Out' else 'Load In' end) as [Transfer Type],(case when Transfer_Type = 'LI' THEN Load_Out_No ELSE 'NULL' END) AS [Load Out No],CONVERT(varchar(15),Transfer_Date , 103) as [Transfer Date],  From_Location as [From Location], To_Location as [To Location], Vehicle_Code as [Vehicle Code],Vehicle_Desc,  Mode_Of_Transport as [Mode Of Transport] ,Route_No as [Route No], Salesmancode as [SalesMan Code] , Reference ,(case when Post='Y' then 'Posted' else 'Pending' end) as status,Item_Type as [Item Type],Reference_Doc_No as [Reference Document No],Reference_Doc_No_DPL as [Reference Document No 2] from tspl_transfer_head"
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            WhrCls += " and case when Transfer_Type = 'LO' THEN From_Location else To_Location end in (" + objCommonVar.strCurrUserLocations + ")"
        End If
        txtTransferNo.Value = clsCommon.ShowSelectForm("Transfer_no", qry, "Transfer_no", WhrCls, txtTransferNo.Value, "transfer_no", isButtonClicked)
        'FunFill()
    ...
    Private Sub fndtrnasferno__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtTransferNo._MYNavigator
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            WhrCls += " and case when Transfer_Type = 'LO' THEN From_Location else To_Location end in (" + objCommonVar.strCurrUserLocations + ")"
        End If
        Dim qry As String = "select Transfer_No  from TSPL_TRANSFER_HEAD  Where 2=2 " + WhrCls + " "
        Select Case NavType
            Case NavigatorType.First
                qry += " and TSPL_TRANSFER_HEAD.Transfer_No=(select MIN(Transfer_No) from TSPL_TRANSFER_HEAD)"
            Case NavigatorType.Last
                qry += " and TSPL_TRANSFER_HEAD.Transfer_No=(select MAX(Transfer_No) from TSPL_TRANSFER_HEAD)"
            Case NavigatorType.Next
                qry += " and TSPL_TRANSFER_HEAD.Transfer_No=(select Min(Transfer_No) from TSPL_TRANSFER_HEAD where Transfer_No > '" + txtTransferNo.Value + "')"
            Case NavigatorType.Previous
                qry += " and TSPL_TRANSFER_HEAD.Transfer_No=(select Max(Transfer_No) from TSPL_TRANSFER_HEAD where Transfer_No < '" + txtTransferNo.Value + "')"
            Case NavigatorType.Current
                qry += " and TSPL_TRANSFER_HEAD.Transfer_No='" + txtTransferNo.Value + "'"
        End Select

        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            txtTransferNo.Value = clsCommon.myCstr(dt.Rows(0)("Transfer_No"))
            'FunFill()
            'cboTransferType.Enabled = False
        End If
        LoadData(txtTransferNo.Value)
    ...
    Sub LoadData(ByVal strLONo As String)
        isInsideLoadData = True
        LoadBlankGrid()
        Dim obj As clsTransferMaster = clsTransferMaster.GetData(strLONo, Nothing)
        If obj IsNot Nothing Then
            ...
            txtMannaulInvoiceNo.Text = ""
            isNewEntry = False
            'Dim myreader As DataTable = clsDBFuncationality.GetDataTable("SELECT CONVERT(varchar(10),Transfer_Date,103)as transfer_date ,CONVERT(varchar(10),Posting_Date,103) as posting_date, Transfer_Type,  Load_Out_No,From_Location,To_Location,Tax_Group,Price_Code,convert(varchar(10),Price_Date,103)as price_date, Reference, description, Route_No,Salesmancode, Vehicle_Code, Vehicle_No, Mode_Of_Transport, Km_Reading, Item_Amount, Total_Tax_Amount , Total_Item_Amount  ,post,trip_no , item_type, date_time_removal,fromloc_desc,toloc_desc,route_desc,price_desc,vehicle_desc,Is_AgainstFormF FROM tspl_transfer_head where transfer_no='" + txtTransferNo.Value + "'")
            txtTime.Value = obj.EntryDateTime
            ...
            lblReferenceDocumentNo2.Text = obj.Reference_Doc_No_DPL
            ...
            lblSalesman.Text = clsLocation.GetName(txtSalesman.Value, Nothing) '' connectSql.RunScalar("select  Emp_Name as Name from TSPL_EMPLOYEE_MASTER where EMP_CODE= '" + txtSalesman.Value + "'")
            ...
            If clsCommon.myLen(txtRouteNo.Value) > 0 And clsCommon.CompairString(obj.Transfer_Type, "LI") = CompairStringResult.Equal Then
                txtToLocation.Enabled = False
            End If
            UsLock1.Status = IIf(clsCommon.CompairString("Y", obj.Post) = CompairStringResult.Equal, ERPTransactionStatus.Approved, ERPTransactionStatus.Pending)
            ...
            txtIndentNo.Value = obj.Against_Indent_No
            ...
            Next
            If clsCommon.CompairString("Y", obj.Post) = CompairStringResult.Equal Then
                rdbtnsave.Enabled = False
                rdbtndelete.Enabled = False
                rdbtnPost.Enabled = False
            Else
            ...
            For count As Integer = 1 To 10
                Sql = "Select (case When Tax" + count.ToString() + " is NULL THEN '' else Tax" + count.ToString() + " end),Tax" + count.ToString() + "_Rate,Tax" + count.ToString() + "_Assessable_Amt,Tax" + count.ToString() + "_Amt from tspl_transfer_head WHERE Transfer_No='" + txtTransferNo.Value + "'"
                ds = connectSql.RunSQLReturnDS(Sql)
            ...
            Next count
            txtRouteNo.Enabled = False
            If Not clsCommon.CompairString("Y", obj.Post) = CompairStringResult.Equal Then
        ...
        End If
        FunFbFcTotal()
        If clsCommon.myLen(txtRouteNo.Value) > 0 AndAlso (clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Excise") = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Depot") = CompairStringResult.Equal) Then
            txtSalesman.Enabled = True
            txtToLocation.Enabled = False
        Else
    ...
    Private Sub txtIndentNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtIndentNo._MYValidating
        Dim qry As String = "select Indent_No as code,CONVERT(varchar, Indent_Date,103) as Date,From_Location as [From Location Code], FromLoc_Desc as [From Location],To_Location as [To Location Code],ToLoc_Desc as [To Location],TSPL_INDENT_HEAD.*  from TSPL_INDENT_HEAD "
        txtIndentNo.Value = clsCommon.ShowSelectForm("TransferIndentFinder", qry, "Code", "Post=1", txtIndentNo.Value, "", isButtonClicked)
        LoadIndentData(txtIndentNo.Value)
    End Sub

    Sub LoadIndentData(ByVal strIndentNo As String)
        isInsideLoadData = True
        LoadBlankGrid()
        Dim obj As clsIndentHead = clsIndentHead.GetData(strIndentNo, Nothing)
        ...
        If obj IsNot Nothing Then
            ...
            For Each objtr As clsIndentDetail In obj.Arr
                ...
                grow.Cells(colTaxRate10).Value = objtr.TAX10_Rate
                If clsCommon.CompairString(cboTransferType.Text, "Load Out") = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Excise") = CompairStringResult.Equal Then
                    grow.Cells(colTaxRate4).Value = 0
                    grow.Cells(colTaxRate5).Value = 0
                    grow.Cells(colTaxRate6).Value = 0
                    grow.Cells(colTaxRate7).Value = 0
                    grow.Cells(colTaxRate8).Value = 0
                    grow.Cells(colTaxRate9).Value = 0
                    grow.Cells(colTaxRate10).Value = 0
                End If
                grow.Cells(colBasicAmt).Value = objtr.Basic_Amt
            ...
            gv2.Rows.Clear()
            For count As Integer = 1 To 10
                Sql = "Select (case When Tax" + count.ToString() + " is NULL THEN '' else Tax" + count.ToString() + " end),Tax" + count.ToString() + "_Rate,Tax" + count.ToString() + "_Assessable_Amt,Tax" + count.ToString() + "_Amt from TSPL_INDENT_HEAD WHERE Indent_No='" + txtIndentNo.Value + "'"
                ds = connectSql.RunSQLReturnDS(Sql)
                If ds.Tables(0) IsNot Nothing AndAlso ds.Tables(0).Rows.Count > 0 Then
                    Dim taxCode As String = Convert.ToString(ds.Tables(0).Rows(0)(0))
                    Dim taxRate As Decimal = clsCommon.myCdbl(ds.Tables(0).Rows(0)(1))
        ...
        For count As Integer = 0 To gv1.Rows.Count - 1
            Dim strICode As String = clsCommon.myCstr(gv1.Rows(count).Cells(colItemCode).Value)
            Dim strUOM As String = clsCommon.myCstr(gv1.Rows(count).Cells(colUOM).Value)
            Dim dblMRP As Double = clsCommon.myCdbl(gv1.Rows(count).Cells(colMRP).Value)
            Dim dblBalQty As Double = clsIndentDetail.GetBalanceQty(txtIndentNo.Value, strICode, dblMRP, txtTransferNo.Value, False)
            gv1.Rows(count).Cells(colTransferQty).Value = dblBalQty
        Next
    ...
    Private Sub RadButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton1.Click
        Try
            Dim qry As String = "select DocNo from xxx_Transfer_LO_Tax where not exists(select 1 from xxx_Transfer_LO_Tax_Complete where xxx_Transfer_LO_Tax_Complete.DocNo=xxx_Transfer_LO_Tax.docNo)"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                If common.clsCommon.MyMessageBoxShow("Pending Task " + clsCommon.myCstr(dt.Rows.Count) + Environment.NewLine + "Do you want to Continue", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                    For Each dr As DataRow In dt.Rows
                        Try
                            Dim TranasferNo As String = clsCommon.myCstr(dr("DocNo"))
                            Dim Post As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Post from TSPL_TRANSFER_HEAD where Transfer_No='" + TranasferNo + "'"))
                            If clsCommon.CompairString(Post, "Y") = CompairStringResult.Equal Then
                                clsDBFuncationality.ExecuteNonQuery("update TSPL_TRANSFER_HEAD set Post='N' where Transfer_No='" + TranasferNo + "'")
                            End If
                            Try
                                LoadData(TranasferNo)
                                totalAmounts()
                                FunFillTotalAmount()
                                FunFbFcTotal()
                                FunLoadinValue()
                            ...
                            If clsCommon.CompairString(Post, "Y") = CompairStringResult.Equal Then
                                Dim tra As SqlTransaction = clsDBFuncationality.GetTransactin()
                                Try
                                    qry = "delete from TSPL_JOURNAL_DETAILS where TSPL_JOURNAL_DETAILS.Voucher_No in (select Voucher_No from TSPL_JOURNAL_MASTER where TSPL_JOURNAL_MASTER.Source_Code='MM-TF' and TSPL_JOURNAL_MASTER.Source_Doc_No='" + TranasferNo + "')"
                                    clsDBFuncationality.ExecuteNonQuery(qry, tra)

                                    qry = "delete from TSPL_JOURNAL_MASTER where TSPL_JOURNAL_MASTER.Source_Code='MM-TF' and TSPL_JOURNAL_MASTER.Source_Doc_No='" + TranasferNo + "'"
                                    clsDBFuncationality.ExecuteNonQuery(qry, tra)

                                    Dim obj As clsTransferMaster = clsTransferMaster.GetData(TranasferNo, tra)
                                    If (clsTransferMaster.postTransferNew(tra, obj, True)) Then
                                        tra.Commit()
                                    Else
                                        tra.Rollback()
                                    End If
                                Catch ex As Exception
                                    ...
                                    Throw New Exception(ex.Message)
                                Finally
                                    clsDBFuncationality.ExecuteNonQuery("update TSPL_TRANSFER_HEAD set Post='" + Post + "' where Transfer_No='" + TranasferNo + "'")
                                End Try
                            Else
                                'Post = "Y"
                                clsDBFuncationality.ExecuteNonQuery("update TSPL_TRANSFER_HEAD set Post='" + Post + "' where Transfer_No='" + TranasferNo + "'")
    ...
    Private Sub RadButton2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton2.Click
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
        Try
            Dim Qry As String = "select Post, Transfer_Type,Transfer_No,case when  TSPL_TRANSFER_HEAD.Transfer_Type='LO' then (select inn.Transfer_No from TSPL_TRANSFER_HEAD as inn where inn.Load_Out_No=TSPL_TRANSFER_HEAD.Transfer_No) else '' end as LoadInNo from TSPL_TRANSFER_HEAD where Transfer_No='" + txtTransferNo.Value + "' "
            ...
            If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Transfer_Type")), "LI") = CompairStringResult.Equal Then
                RevLoadIn(clsCommon.myCstr(dt.Rows(0)("Transfer_No")), trans)
            Else
                RevLoadIn(clsCommon.myCstr(dt.Rows(0)("LoadInNo")), trans)

                Dim strLoadoutNo As String = clsCommon.myCstr(dt.Rows(0)("Transfer_No"))

                Qry = "select Voucher_No from TSPL_JOURNAL_MASTER where Source_Doc_No  ='" + strLoadoutNo + "'"
                Dim VoucherNo As String = clsDBFuncationality.getSingleValue(Qry, trans)
                ...
                Qry = "select InOut,Trans_Type,Item_Code,Item_Desc,Location_Code,case when InOut='I' then -1 else 1 end *Qty as Qty ,UOM,MRP,ItemType,case when InOut='I' then -1 else 1 end* Basic_Cost as Basic_Cost from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" + strLoadoutNo + "' and Trans_Type='Transfer'"
                ...
                Qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No='" + strLoadoutNo + "' and Trans_Type='Transfer'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)

                Qry = "delete from TEMP_PROVISIONAL_SALES where Transfer_No ='" + strLoadoutNo + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)

                Dim QuickSettlementNo As String = clsDBFuncationality.getSingleValue(" select distinct Quick_SettleMent_Id  from tspl_QuickSettleMent  where Transfer_Number='" + strLoadoutNo + "'", trans)
                If clsCommon.myLen(QuickSettlementNo) > 0 Then
                    Qry = " delete from tspl_QuickSettleMent where Quick_SettleMent_Id='" + QuickSettlementNo + "'"
                    clsDBFuncationality.ExecuteNonQuery(Qry, trans)
                End If

                Dim EmptyAdj As String = clsDBFuncationality.getSingleValue(" select Adjustment_No  from TSPL_ADJUSTMENT_HEADER where Document_No ='" + strLoadoutNo + "' and Reference_Document ='Load Out/Transfer'", trans)
                ...
                Qry = "delete from TSPL_TRANSFER_DETAIL  where Transfer_No='" + strLoadoutNo + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)

                Qry = "delete from TSPL_TRANSFER_HEAD  where Transfer_No='" + strLoadoutNo + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)
    ...
    Private Sub RevLoadIn(ByVal strLoadinNo As String, ByVal trans As SqlTransaction)
        Dim Qry As String = ""
        If clsCommon.myLen(strLoadinNo) > 0 Then
            Qry = "select Voucher_No from TSPL_JOURNAL_MASTER where Source_Doc_No  ='" + strLoadinNo + "'"
            Dim VoucherNo As String = clsDBFuncationality.getSingleValue(Qry, trans)
            ...
            Qry = "delete from TSPL_INVENTORY_MOVEMENT where Source_Doc_No ='" + strLoadinNo + "' and Trans_Type ='Transfer'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TEMP_PROVISIONAL_SALES where Transfer_No ='" + strLoadinNo + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Dim obj As clsTransferMaster = clsTransferMaster.GetData(strLoadinNo, trans)
            clsTransferMaster.FunItemLocationUpdateLoadInRrvs(trans, obj)



            Dim QuickSettlementNo As String = clsDBFuncationality.getSingleValue(" select distinct Quick_SettleMent_Id  from tspl_QuickSettleMent  where Transfer_Number='" + obj.Load_Out_No + "'", trans)
            If clsCommon.myLen(QuickSettlementNo) > 0 Then
                Qry = " delete from tspl_QuickSettleMent where Quick_SettleMent_Id='" + QuickSettlementNo + "'"
                clsDBFuncationality.ExecuteNonQuery(Qry, trans)
            End If

            Dim EmptyAdj As String = clsDBFuncationality.getSingleValue(" select Adjustment_No  from TSPL_ADJUSTMENT_HEADER where Document_No ='" + obj.Load_Out_No + "' and Reference_Document ='Load Out/Transfer'", trans)
            ...
            Qry = "update TSPL_TRANSFER_DETAIL set Pending_Qty =Item_Qty , Pending_Balance_In_Bottle = Item_Qty*(select Conversion_Factor  from TSPL_ITEM_UOM_DETAIL where TSPL_ITEM_UOM_DETAIL.Item_Code =TSPL_TRANSFER_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.UOM_Code='FB' ) where TSPL_TRANSFER_DETAIL.Transfer_No ='" + obj.Load_Out_No + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = " update TSPL_TRANSFER_HEAD set quick_settlement='N'  where Transfer_No='" + obj.Load_Out_No + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TSPL_TRANSFER_DETAIL  where Transfer_No='" + strLoadinNo + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)

            Qry = "delete from TSPL_TRANSFER_HEAD  where Transfer_No='" + strLoadinNo + "'"
            clsDBFuncationality.ExecuteNonQuery(Qry, trans)
