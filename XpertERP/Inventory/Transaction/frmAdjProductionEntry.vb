...
Imports common
Public Class frmAdjProductionEntry
...
#End Region
                ...
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colAdjustmentType) OrElse e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colComment) Then
                        ...
                        ElseIf e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colAdjustmentType) Or e.Column Is gv1.Columns(colItemCost) Then
                            '==========Added by preeti Gupta against ticket no[BHA/23/08/18-000477]=======
                    ...
                    ElseIf clsCommon.CompairString(cboTransType.SelectedValue, "Out") = CompairStringResult.Equal And ChkMilkType.Checked = True Then
                        ...
                        End If
                        ''Comment by balwinder on 29/10/2018 becuase it override cost pick from item master
                        'Try
                        ...
                        '    ''richa 10 Sep BHA/07/08/18-000394 ,BHA/18/09/18-000556,richa BHA/07/09/18-000526
                        '    If AllowpurchaseAccounting = False Then
                        '        If clsCommon.CompairString(gv1.Rows(e.RowIndex).Cells(colAdjustmentType).Value, "Cost") <> CompairStringResult.Equal Then
                        '            If clsCommon.myCdbl(objCost.FAT_Cost) > 0 Then
                        ...
                        '            End If
                        '            If clsCommon.myCdbl(objCost.SNF_Cost) > 0 Then
                        '                gv1.Rows(e.RowIndex).Cells(colsnf_Rate).Value = objCost.SNF_Cost / IIf(gv1.Rows(e.RowIndex).Cells(colSNFKG).Value <= 0, 1, gv1.Rows(e.RowIndex).Cells(colSNFKG).Value)
                    ...
                    If ChkMilkType.Checked = True Then
                        If AllowpurchaseAccounting = False Then
                            ...
                            If clsCommon.CompairString(gv1.Rows(e.RowIndex).Cells(colAdjustmentType).Value, "Both") = CompairStringResult.Equal Then
                                ''Comment by balwinder on 12/10/2018 becuase unit cost is amount/qty.
                                'gv1.Rows(e.RowIndex).Cells(colItemCost).Value = clsBOM.GetItemCostOfMilk_AfterConversion(clsCommon.myCstr(gv1.Rows(e.RowIndex).Cells(colICode).Value), clsCommon.myCstr(gv1.Rows(e.RowIndex).Cells(colUnit).Value), clsCommon.myCdbl(gv1.Rows(e.RowIndex).Cells(colQty).Value), clsCommon.myCdbl(gv1.Rows(e.RowIndex).Cells(colCost).Value), Nothing)
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        Else
            obj = clsItemMaster.FinderForItem(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), "", isButtonClick, " Item_Type in ('F','S')")
        End If
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtAdjustmentNo._MYValidating
        ...
        txtAdjustmentNo.Value = clsCommon.ShowSelectForm("AdjustmentStoreDoc", qry, "AdjustmentNumber", whrClas, txtAdjustmentNo.Value, "TSPL_ADJUSTMENT_HEADER.Adjustment_Date desc", isButtonClicked)
        LoadData(txtAdjustmentNo.Value, NavigatorType.Current)
    End Sub
    ...
    Private Sub rdbtnprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If clsCommon.myLen(txtAdjustmentNo.Value) <= 0 Then
            myMessages.blankValue(Me, "Adjustment No not found to Print", Me.Text)
        Else
            funPrint()
        End If
    End Sub
        ...
        Try
            ...
            Else
                If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Adjustment_Type")), "BD") = CompairStringResult.Equal Then
                    ...
                    " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_ADJUSTMENT_HEADER.Customer_CODE" &
                ...
                Else
                    ...
                    qry += " where TSPL_ADJUSTMENT_HEADER.Adjustment_No='" + txtAdjustmentNo.Value + "'  " &
    ...
    Private Sub RadButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton1.Click
        'PrintData()
        printData(txtAdjustmentNo.Value)
    End Sub
    ...
    '                        "TSPL_ADJUSTMENT_HEADER.Adjustment_No, TSPL_ADJUSTMENT_HEADER.Adjustment_Date, TSPL_ADJUSTMENT_HEADER.Customer_NAME," &
    '                        "TSPL_ADJUSTMENT_HEADER.Description, TSPL_ADJUSTMENT_DETAIL.Item_Code, TSPL_ADJUSTMENT_DETAIL.Item_Description, " &
    '                        "TSPL_ADJUSTMENT_DETAIL.Unit_Code, CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'FC' THEN Item_Quantity END AS FCS, " &
    '                        "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'FB' THEN Item_Quantity END AS FBS, " &
    '                        "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'SH' THEN Item_Quantity END AS FSH,  " &
    '                        "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'EC' THEN Item_Quantity END AS ECS, " &
    '                        "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'EB' THEN Item_Quantity END AS EBS, 0 AS Leak_Qty, TSPL_ADJUSTMENT_DETAIL.Breakage, " &
    '                        "0 AS Short_Qty, CASE WHEN TSPL_CUSTOMER_MASTER.Cust_Type_Code NOT IN ('F', 'S') THEN " &
    '                        "(ISNULL(TSPL_ADJUSTMENT_DETAIL.Item_Cost, 0)  + ISNULL(TSPL_ADJUSTMENT_DETAIL.Breakage_Cost, 0)) ELSE " &
    '                        "ISNULL(TSPL_ADJUSTMENT_DETAIL.Item_Cost, 0) END AS Amount,TSPL_ADJUSTMENT_HEADER.EMP_NAME AS SalesManName, " &
    '                        "TSPL_ADJUSTMENT_HEADER.Challan_No, " &
    '                        "TSPL_ADJUSTMENT_HEADER.Challan_date, TSPL_ADJUSTMENT_HEADER.Vehicle_No, TSPL_CUSTOMER_MASTER.Add1, " &
    ...
    '                        "Sale_Invoice_Date else Transfer_Date end as Docdate,TSPL_ADJUSTMENT_HEADER.created_by as [Created by],TSPL_ADJUSTMENT_HEADER.modify_by as [Modified by] FROM TSPL_TRANSFER_HEAD RIGHT OUTER JOIN " &
    '                        "TSPL_ADJUSTMENT_HEADER ON TSPL_TRANSFER_HEAD.Transfer_No = TSPL_ADJUSTMENT_HEADER.Document_No LEFT OUTER JOIN " &
    '                        "TSPL_SALE_INVOICE_HEAD ON TSPL_ADJUSTMENT_HEADER.Document_No = TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No RIGHT OUTER JOIN " &
    '                        "TSPL_ADJUSTMENT_DETAIL ON TSPL_ADJUSTMENT_HEADER.Adjustment_No = TSPL_ADJUSTMENT_DETAIL.Adjustment_No LEFT OUTER JOIN " &
    '                        "TSPL_CUSTOMER_MASTER   ON TSPL_ADJUSTMENT_HEADER.Customer_CODE = TSPL_CUSTOMER_MASTER.Cust_Code LEFT OUTER JOIN " &
    '                        "TSPL_CITY_MASTER ON TSPL_CITY_MASTER.City_Code = TSPL_CUSTOMER_MASTER.City_Code LEFT OUTER JOIN " &
    '                        "TSPL_TDS_STATE_MASTER ON TSPL_TDS_STATE_MASTER.State_Code = TSPL_CUSTOMER_MASTER.State  LEFT OUTER JOIN TSPL_COMPANY_MASTER ON TSPL_COMPANY_MASTER.Comp_Code=TSPL_ADJUSTMENT_HEADER.Comp_Code  left outer join TSPL_ITEM_UOM_DETAIL on TSPL_ADJUSTMENT_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code  and TSPL_ADJUSTMENT_DETAIL.Unit_Code=TSPL_ITEM_UOM_DETAIL.UOM_Code " &
    ...
    '                            " TSPL_GATEPASS_DETAIL.GPCode as GPCode,Transporter_Name,0 as RGB,0 as Pet,case when TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'SH' THEN Breakage else  0 END AS FSHBreakage , " &
    '                            "TSPL_COMPANY_MASTER.Tin_No, TSPL_COMPANY_MASTER.CST_LST, TSPL_COMPANY_MASTER.Ecc_No, TSPL_COMPANY_MASTER.Comp_Name,  (Case When ISNULL(TSPL_COMPANY_MASTER.Add1,'')='' Then '' Else TSPL_COMPANY_MASTER.Add1 + case When ISNULL(TSPL_COMPANY_MASTER.Add1,'')='' Then '' Else ', '+ TSPL_COMPANY_MASTER.Add2 + Case When ISNULL(TSPL_COMPANY_MASTER.Add3,'')='' Then '' Else TSPL_COMPANY_MASTER.Add3 End End End) AS CompAddress, " &
    '                            "TSPL_ADJUSTMENT_HEADER.Adjustment_No, TSPL_ADJUSTMENT_HEADER.Adjustment_Date, TSPL_ADJUSTMENT_HEADER.Customer_NAME," &
    '                            "TSPL_ADJUSTMENT_HEADER.Description, TSPL_ADJUSTMENT_DETAIL.Item_Code, TSPL_ADJUSTMENT_DETAIL.Item_Description, " &
    '                            "TSPL_ADJUSTMENT_DETAIL.Unit_Code, CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'FC' THEN Item_Quantity END AS FCS, " &
    ...
    '                            "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'SH' THEN Item_Quantity END AS FSH,  " &
    '                            "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'EC' THEN Item_Quantity END AS ECS, " &
    '                            "CASE WHEN TSPL_ADJUSTMENT_DETAIL.Unit_Code = 'EB' THEN Item_Quantity END AS EBS, 0 AS Leak_Qty, " &
    '                            "case when TSPL_ADJUSTMENT_DETAIL.Unit_Code='EC' then Breakage  else Breakage end as Breakage, " &
    '                            "0 AS Short_Qty, CASE WHEN TSPL_CUSTOMER_MASTER.Cust_Type_Code NOT IN ('F', 'S') THEN " &
    '                            "(ISNULL(TSPL_ADJUSTMENT_DETAIL.Item_Cost, 0)  + ISNULL(TSPL_ADJUSTMENT_DETAIL.Breakage_Cost, 0)) ELSE " &
    '                            "ISNULL(TSPL_ADJUSTMENT_DETAIL.Item_Cost, 0) END AS Amount,TSPL_ADJUSTMENT_HEADER.EMP_NAME AS SalesManName, " &
    '                            "TSPL_ADJUSTMENT_HEADER.Challan_No, " &
    ...
    '                            "TSPL_COMPANY_MASTER ON TSPL_COMPANY_MASTER.Comp_Code=TSPL_ADJUSTMENT_HEADER.Comp_Code  " &
    ...
    '                            " TSPL_ITEM_UOM_DETAIL on TSPL_ADJUSTMENT_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code   " &
    '                            " left outer join TSPL_GATEPASS_DETAIL on TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No=TSPL_GATEPASS_DETAIL.DocNo " &
    '                            " left outer join TSPL_LOCATION_MASTER on TSPL_ADJUSTMENT_HEADER.Loc_Code=TSPL_LOCATION_MASTER.Location_Code  " &
    ...
    '                       "TSPL_ADJUSTMENT_HEADER.Adjustment_No, TSPL_ADJUSTMENT_HEADER.Adjustment_Date, " &
    '                       "TSPL_ADJUSTMENT_HEADER.Customer_NAME,TSPL_ADJUSTMENT_HEADER.Description, " &
    ...
    '                       "TSPL_ADJUSTMENT_HEADER.EMP_NAME AS SalesManName, TSPL_ADJUSTMENT_HEADER.Challan_No, " &
    '                       "TSPL_ADJUSTMENT_HEADER.Challan_date, TSPL_ADJUSTMENT_HEADER.Vehicle_No, " &
    '                       "'' as Add1, '' as Add2, '' as Add3, '' as City_Name, '' as State_Name, " &
    '                       "TSPL_ADJUSTMENT_HEADER.Document_No,case when Reference_Document='Sale Invoice' then Sale_Invoice_Date else null end as Docdate , '' as  locPin, '' as locTinNo, '' as locCSTNo, '' as locName " &
    '                       ",TSPL_ADJUSTMENT_HEADER.created_by as [Created by],TSPL_ADJUSTMENT_HEADER.modify_by as [Modified by] from TSPL_ADJUSTMENT_HEADER inner join TSPL_SALE_INVOICE_HEAD on " &
    '                       "TSPL_ADJUSTMENT_HEADER.Document_No=TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No left outer join " &
        ...
        Try
            If common.clsCommon.MyMessageBoxShow(Me, "Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    Qry = "Production Entry is used in following Production Entry QC"
    ...
    Private Sub ExporttoExcelWithSerial_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ExporttoExcelWithSerial.Click
        Try
            ...
            qryExport = " Select TSPL_ADJUSTMENT_detail.Location_Code as [Location], TSPL_ADJUSTMENT_HEADER.Adjustment_Date , TSPL_ADJUSTMENT_HEADER.Adjustment_No, TSPL_ADJUSTMENT_detail.Item_Code as [Item Code],TSPL_ADJUSTMENT_detail.unit_code as [unit code], Item_Quantity as [Quantity], Item_Cost as [Cost Adjustment],'N' as [Third Party Location], TSPL_SERIAL_ITEM.Auto_Sr_No as [Serial No],'' as [Tag No],'' as Type, case when TSPL_ADJUSTMENT_HEADER.adjustment_type='ADJ' then 'Adjustment'" &
                        " when TSPL_ADJUSTMENT_HEADER.adjustment_type='FLG' then 'Flushing'" &
                        " when TSPL_ADJUSTMENT_HEADER.adjustment_type='OPG' then 'Opening'" &
                         " when TSPL_ADJUSTMENT_HEADER.adjustment_type='CLG' then 'Closing'" &
                         " when TSPL_ADJUSTMENT_HEADER.adjustment_type='AAD' then 'Auto Adjustment'" &
                         "  when TSPL_ADJUSTMENT_HEADER.adjustment_type='OTH' then 'Other'" &
...
#End Region
    ...
    Private Sub RadMenuItem6_Click(sender As Object, e As EventArgs) Handles RadMenuItem6.Click
        Try
            ...
            qryExport = " select distinct '31-Mar-2017' as AdjDate,TSPL_INV_MOVE_DL.Location_Code as [Main Location Code],TSPL_INV_MOVE_DL.Location_Code as [Location Code],'In' as Inout,'Both'as [Adjustment Type], TSPL_INV_MOVE_DL.Item_Code as [Item Code],Stock_UOM as UOM,0 as Qty,0 as Rate,0 as Amount,0 as [Fat%],0 as [Fat KG],0 as [SNF%],0 as [SNF KG],'' as [Transaction Type] " &
                        ...
                        " ) TSPL_INV_MOVE_DL left join ( select distinct TSPL_ADJUSTMENT_DETAIL.Item_Code,TSPL_ADJUSTMENT_DETAIL.Location_Code from TSPL_ADJUSTMENT_DETAIL " &
                        " inner join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_DETAIL.Adjustment_No=TSPL_ADJUSTMENT_HEADER.Adjustment_No  where TSPL_ADJUSTMENT_HEADER.Adjustment_Type='AAD') as Adj on " &
    ...
    Private Sub btnShowInventory_Click(sender As Object, e As EventArgs)   ' Ticket : TEC/29/10/18-000353 By Sanjay
    ...
    Sub printData(ByVal DocNo As String)
        ...
        If clsCommon.myLen(DocNo) > 0 Then
            ...
            Dim strQuery As String = "  select TSPL_ADJUSTMENT_DETAIL.REMARKS,TSPL_COMPANY_MASTER.Logo_Img,TSPL_LOCATION_MASTER_From_loc.Location_Desc as FromLoaction,TSPL_LOCATION_MASTER_From_loc.GSTNO as From_location_GSTIN,TSPL_STATE_MASTER_From_loc.GST_STATE_Code as From_Loc_GST_STATE_Code,'" + strCompany_Name + "' as Comp_Name, '" + strCompanyLocation + "' as Company_Address ,TSPL_ADJUSTMENT_Detail.Adjustment_No as Document_No,convert(varchar,TSPL_ADJUSTMENT_HEADER.Adjustment_Date,103) as  Document_Date,TSPL_ADJUSTMENT_HEADER.Description as Document_Desc,TSPL_ADJUSTMENT_Detail.Adjustment_Line_No as SNO,TSPL_ADJUSTMENT_Detail.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_ITEM_MASTER.HSN_Code,	" &
                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.UOM ELSE TSPL_ADJUSTMENT_Detail.Unit_Code END AS UOM, " &
                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 then BI.QTY else TSPL_ADJUSTMENT_Detail.Item_Quantity end as Qty, " &
                    " TSPL_ADJUSTMENT_Detail.Item_Cost as Rate, " &
                    " case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN BI.BATCH_NO ELSE '' END AS Item_BatchNo,case when TSPL_ITEM_MASTER.Is_Batch_Item=1 THEN convert (varchar, BI.Manufacture_Date,103) ELSE '' END AS  Manufacture_Date,TSPL_ITEM_MASTER.Is_Batch_Item  " &
                    "  ,TSPL_ADJUSTMENT_HEADER.Adjustment_Type as Document_Type,TSPL_ADJUSTMENT_HEADER.Loc_Code, " &
                    " TSPL_ADJUSTMENT_HEADER.Posted , TSPL_ADJUSTMENT_HEADER.Posting_Date as Posted_Date,TSPL_ADJUSTMENT_HEADER.Created_By,TSPL_ADJUSTMENT_HEADER.Modify_By as Modified_By, case when TSPL_ADJUSTMENT_HEADER.Posted='Y' then tspl_user_master_Modified_By.User_Name else '' end as PostedBy,tspl_user_master_Created_By.User_Name as Created_Name, TSPL_LOCATION_MASTER_From_loc.Add1 + case when len(TSPL_LOCATION_MASTER_From_loc. Add2) > 0 then +','+ TSPL_LOCATION_MASTER_From_loc. Add2 else ' '     end + Case when len( TSPL_LOCATION_MASTER_From_loc. Add3) >0 then ','+TSPL_LOCATION_MASTER_From_loc. Add3 else '' end + case when len     (TSPL_LOCATION_MASTER_From_loc.Add4) >0 then ','+TSPL_LOCATION_MASTER_From_loc.Add4 else '' end + case when len     (TSPL_LOCATION_MASTER_From_loc.City_Code)>0 then ','+ TSPL_LOCATION_MASTER_From_loc.City_Code else '' end + case when len    (TSPL_LOCATION_MASTER_From_loc.state) > 0 then ','+ TSPL_STATE_MASTER_From_loc.STATE_NAME else ''  end + Case when len     (TSPL_LOCATION_MASTER_From_loc.Pin_Code ) >0 then ' - '+ convert(varchar, TSPL_LOCATION_MASTER_From_loc.Pin_Code,103) else '' end as From_Location_Address  from TSPL_ADJUSTMENT_Detail " &
                    ...
                    " Left outer join tspl_user_master as tspl_user_master_Modified_By on tspl_user_master_Modified_By.User_Code =TSPL_ADJUSTMENT_HEADER.Modify_By  " &
                    ...
                    " Left outer join TSPL_LOCATION_MASTER as TSPL_LOCATION_MASTER_From_loc on TSPL_LOCATION_MASTER_From_loc.Location_Code =TSPL_ADJUSTMENT_HEADER.loc_Code  " &
                    ...
                    " Left outer join TSPL_CITY_MASTER as TSPL_CITY_MASTER_From_location on TSPL_CITY_MASTER_From_location.City_Code = TSPL_LOCATION_MASTER_From_loc.City_Code                              Left Join TSPL_BATCH_ITEM AS BI ON TSPL_ADJUSTMENT_Detail.Adjustment_No=BI.Document_Code And TSPL_ADJUSTMENT_Detail.Item_Code=BI.Item_Code " &
                    " Left Outer Join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code =TSPL_ADJUSTMENT_HEADER.Comp_Code  " &
                    " where  TSPL_ADJUSTMENT_HEADER.Adjustment_Type = 'PRE' and TSPL_ADJUSTMENT_HEADER.Adjustment_No= '" + DocNo + "' "
            ...
            If dt.Rows.Count > 0 Then
                ...
                Dim frmCRV As New frmCrystalReportViewer()
                frmCRV.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, "rptADJProductionEntry", "ProductionEntry", dtDocdate)
                frmCRV = Nothing
            End If
        Else
            clsCommon.MyMessageBoxShow(Me, "Please select an Production Entry No to print", Me.Text)
        End If
    End Sub
