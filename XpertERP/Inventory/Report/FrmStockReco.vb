    ...
    Private Sub LoadData(ByVal isPrintCrystalReport As Integer)
        Try
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboUOMType.SelectedValue), "ReportUOM") = CompairStringResult.Equal Then
                qry += " TSPL_ITEM_UOM_DETAIL.UOM_Code as Stock_UOM,cast((InventroyMovement.Stock_Qty /  (case when InventroyMovement.Custom_UOM=TSPL_ITEM_UOM_DETAIL.UOM_Code and InventroyMovement.Custom_Coversion_Factor>0 then  InventroyMovement.Custom_Coversion_Factor else TSPL_ITEM_UOM_DETAIL.Conversion_Factor end)) as decimal(18,10)) as Stock_Qty,"
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboUOMType.SelectedValue), "UOM") = CompairStringResult.Equal Then
                ...
                qry += " isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end) * (TSPL_ITEM_UOM_DETAIL.Conversion_Factor)) from TSPL_ITEM_QC_PARAMETER_MASTER left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) as SNFPer,"
            Else
                Throw New Exception("Invalid UOM Type")
            End If
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date and Item Wise Stock") = CompairStringResult.Equal Then
                strFinalQry = "select *,(case when (CLRate=0 or IssRate=0) then 0 else (IssRate-CLRate) end) as DiffRate,(case when (CLRate=0 or IssRate=0) then 0 else (IssFATPER-CLFATPER) end) as DiffFATPer,(case when (CLRate=0 or IssRate=0) then 0 else (IssSNFPER-CLSNFPER) end) as DiffSNFPer from (select xxxxxxx.Location_Code,[Loc Desp],convert(varchar, Punching_Date,103) as Punching_Date,Structure_Descq,Item_Type_Name,RACK_NO    ,Item_Code ,Item_Desc,Stock_UOM, case when ( abs(cast((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))as  decimal(18,2)))<=0.11 or (abs(cast((ISNULL(CLCost,0) - isnull(RecCost,0)+isnull(IssCost,0))as  decimal(18,2)))<0.11 and tspl_location_master.Is_jobWork=0) ) then 0 else  Convert(decimal(18,3),(ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))) end as OPQty," + Environment.NewLine +
...
",0 as SNFAmount" + Environment.NewLine +
    ...
    Sub SetGridFormationOFGV1()
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date, Item And Document Wise Detail") = CompairStringResult.Equal Then
            ...
            gv1.Columns("Structure_Descq").IsVisible = False
            gv1.Columns("Structure_Descq").Width = 100
            gv1.Columns("Structure_Descq").HeaderText = "Structure"
            ...
            gv1.Columns("Rack_No").IsVisible = False
            gv1.Columns("Rack_No").Width = 100
            gv1.Columns("Rack_No").HeaderText = "Rack No"
