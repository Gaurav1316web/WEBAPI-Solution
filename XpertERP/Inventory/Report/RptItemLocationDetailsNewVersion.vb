    ...
    Sub printdata()
        Try
            ...
            If chkbatchselect.IsChecked = True AndAlso cbgbatch.CheckedValue.Count <= 0 Then
                common.clsCommon.MyMessageBoxShow("Please select at least one Batch or select ALL")
                Exit Sub
            End If
            If chkmrpselect.IsChecked = True AndAlso cbgmrp.CheckedValue.Count <= 0 Then
                common.clsCommon.MyMessageBoxShow("Please select at least one MRP or select ALL")
                Exit Sub
            End If
