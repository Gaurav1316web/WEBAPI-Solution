...
Public Class FrmStockDetail
    ...
    Private Sub gv1_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles gv1.DoubleClick
        ...
        If clsCommon.myLen(strType) > 0 AndAlso clsCommon.myLen(strDoc) > 0 Then
            Select Case strType
                Case "IC-AD"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.ICAdj, strDoc)
                Case "ISSTRAN"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.ICAdj, strDoc)
                Case "SRN"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.SRN, strDoc)
                Case "SD-IN"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.SaleInvoice, strDoc)
                Case "Sale Return"
                    If objCommonVar.IsDemoERP Then
                        clsOpenTransactionForm.OpenTransacionForm(EnumTransType.SDSaleReturn, strDoc)
                    Else
                        clsOpenTransactionForm.OpenTransacionForm(EnumTransType.SaleReturn, strDoc)
                    End If
                Case "Purchase Return"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.PurchaseReturn, strDoc)
                Case "Purchase Return"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.PurchaseReturn, strDoc)
                Case "Transfer"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.MMTrans, strDoc)
                Case "Adjustment"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.ICAdj, strDoc)
                Case "Load Out"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.SaleInvoice, strDoc)
                Case "DISPATCH-DS"
                    clsOpenTransactionForm.OpenTransacionForm(EnumTransType.DispatchDairy, strDoc)
            End Select
        End If
