...
Public Class rptCustomerWiseStockReco
...
#End Region
    ...
    Private Sub FrmKPIReport_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'If clsCommon.myLen(MIS_Item_Group) <= 0 Then ''TEC/18/06/19-000542 by balwinder on 25/06/2019
    ...
    Private Sub LoadDataNew(ByVal isPrintCrystalReport As Integer)
        ...
        Try
            ...
            ' objFilter.arrItem = txtItem.Value
...
#End Region
    ...
    Private Sub LoadData(ByVal isPrintCrystalReport As Integer)
        Try
            ...
            OuterOpClo = " [OPBal],(case when OPBal=0 or abs(cast((ISNULL(OPBalCost,0))as decimal(18,2)))<" + RateTunning + " then 0 else OPBalCost/OPBal end) as OPBalrate,case when convert(decimal(18,2),[OPQTYKG])=0 or abs(Convert(decimal(18,2),(isnull([OPFAT],0))))<" + RateTunning + "  then 0 else convert(decimal(18,2),([OPFAT]*100/[OPQTYKG])) end as [OPFATPER],convert(decimal(18,2),[OPFAT]) as [OPFAT],case when convert(decimal(18,2),[OPQTYKG])=0 or abs(Convert(decimal(18,2),(isnull([OPSNF],0))))<" + RateTunning + " then 0 else convert(decimal(18,2),([OPSNF]*100/[OPQTYKG])) end as [OPSNFPER],convert(decimal(18,2), [OPSNF]) as [OPSNF],OPBalCost,Received_Qty,(case when Received_Qty=0 then 0 else RecdCost/Received_Qty end) as RecdRate, RecdCost ,case when convert(decimal(18,2),Received_QtyKG)=0 then 0 else convert(decimal(18,2),(Received_FAT*100/Received_QtyKG)) end as Received_FATPER,convert(decimal(18,2), Received_FAT) as Received_FAT,case when convert(decimal(18,2),Received_QTYKG)=0 then 0 else convert(decimal(18,2),(Received_SNF*100/Received_QTYKG)) end as Received_SNFPER,convert(decimal(18,2), Received_SNF) as Received_SNF,convert(decimal(18,2),Issued_Qty) as Issued_Qty, (case when Issued_Qty=0 then 0 else IssueCost/Issued_Qty end) as IssueRate,IssueCost,case when convert(decimal(18,2),Issued_QTYKG)=0 then 0 else convert(decimal(18,2),(Issued_FAT*100/Issued_QTYKG)) end as Issued_FATPER,convert(decimal(18,2), Issued_FAT) as Issued_FAT,case when convert(decimal(18,2),Issued_QTYKG)=0 then 0 else convert(decimal(18,2),(Issued_SNF*100/Issued_QTYKG)) end as Issued_SNFPER,convert(decimal(18,2) ,Issued_SNF) as Issued_SNF  ,[Balance_Qty],convert(decimal(28,3), case when Balance_Qty=0 or abs(cast((ISNULL(Cost,0))as decimal(18,2)))<" + RateTunning + "  then 0 else Cost/Balance_Qty end) as Rate,Cost,case when convert(decimal(18,2),[Balance_QTYKG])=0 or abs(Convert(decimal(18,2),(isnull(Balance_FAT,0))))<" + RateTunning + " then 0 else convert(decimal(18,2),([Balance_FAT]*100/[Balance_QTYKG])) end as [Balance_FATPER],convert(decimal(18,2), [Balance_FAT]) as  [Balance_FAT],case when convert(decimal(18,2),[Balance_QTYKG])=0 or abs(Convert(decimal(18,2),(isnull([Balance_SNF],0))))<" + RateTunning + " then 0 else convert(decimal(18,2),([Balance_SNF]*100/[Balance_QTYKG])) end as [Balance_SNFPER],convert(decimal(18,2), [Balance_SNF]) as [Balance_SNF] "
            InnerOpClo = "  SUM(STOCK_QTY * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPBal]  ," &
                            " SUM(Cost  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPBalCost]  , " &
                            " SUM(QtyKG * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS OPQTYKG ," &
                            " SUM((case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end)  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPFAT], " &
                            " SUM((case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end)  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPSNF], " &
                            " SUM(STOCK_QTY * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end))  AS Received_Qty , " &
                            " SUM(Cost * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end))  AS RecdCost , " &
                            " SUM(QtyKG * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end))  AS Received_QtyKG , " &
                            " SUM((case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end))  AS Received_FAT , " &
                            " SUM((case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end))  AS Received_SNF , " &
                            " SUM(STOCK_QTY * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end))  AS Issued_Qty," &
                            " SUM(Cost * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end))  AS IssueCost," &
                            " SUM(QtyKG * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end))  AS Issued_QtyKG," &
                            " SUM((case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end))  AS Issued_FAT," &
                            " SUM((case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end ) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end))  AS Issued_SNF," &
                            " SUM(STOCK_QTY * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Balance_Qty]," &
                            " SUM(cost * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Cost], " &
                            " SUM(QtyKG * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS Balance_QtyKG," &
                            " SUM((case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end ) * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Balance_FAT]," &
                            " SUM((case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end ) * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Balance_SNF]"
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Category Wise Summary") = CompairStringResult.Equal Then
                ...
                strFinalQry += OuterOpClo
                strFinalQry += " from (" + Environment.NewLine
                strFinalQry += " select  Item_Type,max(Item_Type_Name) as Item_Type_Name,Item_Group,max(Group_Description) as Group_Description, " + strCodeColumn + "," + strCodeDescColumnMax + ",Item_Category_Struct_Code,SUM(Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty," + strStockUOM + ","
                strFinalQry += InnerOpClo
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Wise Summary") = CompairStringResult.Equal Then
                ...
                End If
                strFinalQry += " from (" + Environment.NewLine
                strFinalQry += " select  Item_Type,max(Item_Type_Name) as Item_Type_Name,Item_Group,max(Group_Description) as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",max(Item_Category_Struct_Code) as Item_Category_Struct_Code,Item_Code,MAX(Item_Desc) as Item_Desc,max(itf_code)as itf_code, SUM(Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty," + strStockUOM + ","
                strFinalQry += InnerOpClo
                If ChkMRPWise.Checked = True Then
                    strFinalQry += ",MRP "
                End If
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item And Location Wise Summary") = CompairStringResult.Equal Then
                ...
                End If
                strFinalQry += "  from (" + Environment.NewLine
                strFinalQry += " select  Item_Type,max(Item_Type_Name) as Item_Type_Name,Item_Group,max(Group_Description) as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",max(Item_Category_Struct_Code) as Item_Category_Struct_Code,Main_Location_Code,max(MainLocationDesc) as MainLocationDesc, Location_Code,max([Loc Desp]) as [Loc Desp],Item_Code,MAX(Item_Desc) as Item_Desc,max(itf_code)as itf_code, SUM(Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty," + strStockUOM + ", max(Inventory_Control_Acc) as Inventory_Control_Acc,max(Inventory_Control_Acc_desc) as Inventory_Control_Acc_desc, "
                strFinalQry += InnerOpClo
                If ChkMRPWise.Checked = True Then
                    strFinalQry += ",MRP "
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal Then
                strFinalQry = " Select CompName,Trans_Type, InOutView, XXXXFinal.FromDate,XXXXFinal.ToDate, XXXXFinal.Trans_Id, XXXXFinal.Trans_Type_Name, XXXXFinal.Source_Doc_No,XXXXFinal.Punching_Date,XXXXFinal.Location_Code, XXXXFinal.[Loc Desp] , XXXXFinal.PrimaryLocation,XXXXFinal.SourceCode, XXXXFinal.SourceName , Item_Code, Item_Desc , Stock_UOM , Stock_Qty ,Rate,Cost from ( "
                strFinalQry += "select case when '" & LocationFirstTime & "'='1' then '" & LocationAddress & "' else '" + objCommonVar.CurrentCompanyName + " ' end as CompName,'" + clsCommon.myCDate(txtFromDate.Value, "dd/MMM/yyyy") + "' as FromDate,'" + clsCommon.myCDate(txtToDate.Value, "dd/MMM/yyyy") + "' as ToDate , Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,convert(varchar,Punching_Date,103) as Punching_Date,InOutView, InOut,Location_Code,[Loc Desp],PrimaryLocation,SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description," + strCodeColumn + "," + strCodeDescColumn + ",Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,Stock_Qty,case when (Source_Doc_No='Opening Balance' and ABS(isnull(cast(Stock_Qty as decimal(18,2)),0))=0 or ABS(isnull(cast(Cost as decimal(18,2)),0))<" + RateTunning + " )then 0 else Rate end as Rate,Cost,isnull((CASE when (Balance_QTYKG=0 or (Source_Doc_No='Opening Balance' and [Balance_FAT]<" + RateTunning + " ) ) then 0 else cast(([Balance_FAT]*100/[Balance_QTYKG]) as decimal(18,2)) end),0) as [Balance_FATPER] ,cast(Balance_FAT as decimal(18,2)) as Balance_FAT,isnull((CASE when (Balance_QTYKG=0 or (Source_Doc_No='Opening Balance' and [Balance_SNF]<" + RateTunning + " ) ) then 0 else cast(([Balance_SNF]*100/[Balance_QTYKG]) as decimal(18,2)) end),0) as [Balance_SNFPER] , cast(Balance_SNF as decimal(18,2)) as Balance_SNF , cast(FATRATE as decimal(18,2)) as FATRATE, cast(SNFRATE as decimal(18,2)) as SNFRATE "
                If ChkMRPWise.Checked Then
                    strFinalQry += ",MRP "
                End If
                strFinalQry += " from ("
                strFinalQry += "select (case when sum((case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end)=0 then 0 else sum((case when IsFromMilk=1 then MilkFATKG*Fat_Rate else (Stock_Qty*FatPer*Fat_Rate) end) * case when InOut='I' then 1 else -1 end)/sum((case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) end) AS FATRATE,(case when sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end))=0 then 0 else sum(( (case when IsFromMilk=1 then MilkSNFKG*SNF_Rate else (Stock_Qty*SNFPer*SNF_Rate) end ) * case when InOut='I' then 1 else -1 end))/sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) end ) AS SNFRATE, 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as Location_Code,'' as [Loc Desp],'' as [LocAddress],'' as PrimaryLocation,'Opening' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",max(Item_Code)  as Item_Code ,max(Item_Desc)  as Item_Desc,'' as Item_Category_Struct_Code,'' as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,case when sum( Stock_Qty * case when InOut='I' then 1 else -1 end) =0 then 0 else convert(decimal(28,3), sum(Cost * case when InOut='I' then 1 else -1 end)/sum( Stock_Qty * case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF "
                If ChkMRPWise.Checked Then
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date and Item Wise Stock") = CompairStringResult.Equal Then
                strFinalQry = "select *,(IssRate-CLRate) as DiffRate,(IssFATPER-CLFATPER) as DiffFATPer,(IssSNFPER-CLSNFPER) as DiffSNFPer from (select Location_Code,[Loc Desp],convert(varchar, Punching_Date,103) as Punching_Date  ,Item_Code ,Item_Desc,Stock_UOM, Convert(decimal(18,2),(ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))) as OPQty," + Environment.NewLine +
                  " case when ( abs(cast((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))as  decimal(18,2)))=0 or abs(cast((ISNULL(CLCost,0) - isnull(RecCost,0)+isnull(IssCost,0))as  decimal(18,2)))<" + RateTunning + ") then 0 else  Convert(decimal(18,2),((isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0))/((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))))) end as OPRate," + Environment.NewLine +
                  " Convert(decimal(18,2),(isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0))) as OPFAT" + Environment.NewLine +
                  ",Convert(decimal(18,2),(case when Convert(decimal(18,2),(ISNULL(CLBalance_QTYKG,0) - isnull(Balance_QTYKG,0)))=0 or abs(Convert(decimal(18,2),(isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0))))<" + RateTunning + " then 0 else (Convert(decimal(18,2),(isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0))*100)/Convert(decimal(18,2),(ISNULL(CLBalance_QTYKG,0) - isnull(Balance_QTYKG,0)))) end)) as OPFATPER," + Environment.NewLine +
                  " Convert(decimal(18,2),(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0))) as OPSNF" + Environment.NewLine +
                  ",Convert(decimal(18,2),(case when Convert(decimal(18,2),(ISNULL(CLBalance_QTYKG,0) - isnull(Balance_QTYKG,0)))=0 or abs(Convert(decimal(18,2),(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0))) )<" + RateTunning + "  then 0 else (Convert(decimal(18,2),(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0))*100)/Convert(decimal(18,2),(ISNULL(CLBalance_QTYKG,0) - isnull(Balance_QTYKG,0)))) end)) as OPSNFPER ," + Environment.NewLine +
                  "Convert(decimal(18,2),(isnull(CLCost,0)-isnull(RecCost,0)+ isnull(IssCost,0))) as OPCost, RecPurQty,RecPurRate,cast(RecPurFAT as decimal(18,2)) as RecPurFAT,cast(RecPurFATPER as decimal(18,2)) as RecPurFATPER,cast(RecPurSNF as decimal(18,2)) as RecPurSNF,cast(RecPurSNFPER as decimal(18,2)) as RecPurSNFPER,RecPurCost ,RecAdjProQty,RecAdjProRate ,cast(RecAdjProFAT as decimal(18,2)) as RecAdjProFAT,cast(RecAdjProFATPER as decimal(18,2)) as RecAdjProFATPER,cast(RecAdjProSNF as decimal(18,2)) as RecAdjProSNF,cast(RecAdjProSNFPER  as decimal(18,2)) as RecAdjProSNFPER,RecAdjProCost ,RecOthQty,RecOthRate ,cast(RecOthFAT as decimal(18,2)) as RecOthFAT,cast(RecOthFATPER as decimal(18,2)) as RecOthFATPER,cast(RecOthSNF as decimal(18,2)) as RecOthSNF,cast(RecOthSNFPER as decimal(18,2)) as RecOthSNFPER,RecOthCost,RecQty,RecRate,cast(RecFAT as decimal(18,2)) as RecFAT,cast(RecFATPER as decimal(18,2)) as RecFATPER,cast(RecSNF as decimal(18,2)) as RecSNF,cast(RecSNFPER as decimal(18,2)) as RecSNFPER,RecCost  ,IssSaleQty ,IssSaleRate ,cast(IssSaleFAT as decimal(18,2)) as IssSaleFAT,cast(IssSaleFATPER as decimal(18,2)) as IssSaleFATPER,cast(IssSaleSNF as decimal(18,2)) as IssSaleSNF,cast(IssSaleSNFPER as decimal(18,2)) as IssSaleSNFPER ,IssSaleCost , IssIssAdjQty , IssIssAdjRate , cast(IssIssAdjFAT as decimal(18,2)) as IssIssAdjFAT, cast(IssIssAdjFATPER as decimal(18,2)) as IssIssAdjFATPER,cast(IssIssAdjSNF as decimal(18,2)) as IssIssAdjSNF, cast(IssIssAdjSNFPER as decimal(18,2)) as IssIssAdjSNFPER,IssIssAdjCost , IssOthQty , IssOthRate , cast(IssOthFAT as decimal(18,2)) as IssOthFAT,cast(IssOthFATPER as decimal(18,2)) as IssOthFATPER, cast(IssOthSNF as decimal(18,2)) as IssOthSNF,cast(IssOthSNFPER as decimal(18,2)) as IssOthSNFPER,IssOthCost ,IssQty,IssRate,cast(IssFAT as decimal(18,2)) as IssFAT,cast(IssFATPER as decimal(18,2)) as IssFATPER,cast(IssSNF as decimal(18,2)) as IssSNF,cast(IssSNFPER as decimal(18,2)) as IssSNFPER,IssCost ,CLQty," + Environment.NewLine +
                  "case when (ABS(isnull(cast(CLQty as decimal(18,2)),0))=0 or ABS(isnull(cast(CLCost as decimal(18,2)),0))<" + RateTunning + " ) then 0 else CLCost/CLQty end as CLRate," + Environment.NewLine +
                  " cast(CLBalance_FAT as decimal(18,2)) as CLFAT, isnull((CASE when CLBalance_QTYKG=0 or CLBalance_FAT<" + RateTunning + " then 0 else cast((CLBalance_FAT*100/CLBalance_QTYKG) as decimal(18,2)) end),0) as CLFATPER, cast(CLBalance_SNF as decimal(18,2)) as CLSNF, isnull((CASE when CLBalance_QTYKG=0 or CLBalance_SNF<" + RateTunning + " then 0 else cast((CLBalance_SNF*100/CLBalance_QTYKG) as decimal(18,2)) end),0) as CLSNFPER, CLCost from ( "
                strFinalQry += "select  Location_Code,max([Loc Desp]) as [Loc Desp],Punching_Date as Punching_Date, Item_Code ,max(Item_Desc) as Item_Desc, " + strStockUOM + ", sum(Balance_FAT) as Balance_FAT,sum(Balance_SNF) as Balance_SNF  ,sum(isnull(Balance_QTYKG,0)) as Balance_QTYKG ,sum(case when InOut='I' and In_Category in ('PU') then Stock_Qty else 0 end) as RecPurQty ,case when sum(case when InOut='I' and In_Category in ('PU') then Stock_Qty else 0 end)=0 then 0 else (sum(case when InOut='I' and In_Category in ('PU') then Cost else 0 end)/sum(case when InOut='I' and In_Category in ('PU') then Stock_Qty else 0 end)) end as RecPurRate  ,sum(case when InOut='I' and In_Category in ('PU') then Balance_FAT else 0 end) as RecPurFAT  ,(case when sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('PU') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end) end)  as RecPurFATPER  ,sum(case when InOut='I' and In_Category in ('PU') then Balance_SNF else 0 end) as RecPurSNF  ,(case when sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('PU') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end) end)  as RecPurSNFPER  ,sum(case when InOut='I' and In_Category in ('PU') then Cost else 0 end) as RecPurCost  ,sum(case when InOut='I' and In_Category in ('AD') then Stock_Qty else 0 end) as RecAdjProQty  ,case when sum(case when InOut='I' and In_Category in ('AD') then Stock_Qty else 0 end)=0 then 0 else (sum(case when InOut='I' and In_Category in ('AD') then Cost else 0 end)/sum(case when InOut='I' and In_Category in ('AD') then Stock_Qty else 0 end)) end as RecAdjProRate  ,sum(case when InOut='I' and In_Category in ('AD') then Balance_FAT else 0 end) as RecAdjProFAT  ,(case when sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('AD') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end) end)  as RecAdjProFATPER  ,sum(case when InOut='I' and In_Category in ('AD') then Balance_SNF else 0 end) as RecAdjProSNF  ,(case when sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('AD') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end) end)  as RecAdjProSNFPER  ,sum(case when InOut='I' and In_Category in ('AD') then Cost else 0 end) as RecAdjProCost ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Stock_Qty else 0 end) as RecOthQty  ,case when sum(case when InOut='I' and In_Category not in ('AD','PU') then Stock_Qty else 0 end)=0 then 0 else (sum(case when InOut='I' and In_Category not in ('AD','PU') then Cost else 0 end)/sum(case when InOut='I' and In_Category not in ('AD','PU') then Stock_Qty else 0 end)) end as RecOthRate  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Balance_FAT else 0 end) as RecOthFAT  ,(case when sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end) end)  as RecOthFATPER  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Balance_SNF else 0 end) as RecOthSNF ,(case when sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end) end)  as RecOthSNFPER  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Cost else 0 end) as RecOthCost ,sum(case when InOut='I' then Stock_Qty else 0 end) as RecQty  ,case when cast(sum(case when InOut='I' then Stock_Qty else 0 end)as  decimal(18,2))=0 then 0 else (sum(case when InOut='I' then Cost else 0 end)/sum(case when InOut='I' then Stock_Qty else 0 end)) end as RecRate  ,sum(case when InOut='I' then Balance_FAT else 0 end) as RecFAT,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_FAT else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecFATPER  ,sum(case when InOut='I' then Balance_SNF else 0 end) as RecSNF  ,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_SNF else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecSNFPER  ,sum(case when InOut='I' then Cost else 0 end) as RecCost,sum(case when InOut='O' and Out_Category in ('SA') then -1*Stock_Qty else 0 end) as IssSaleQty  ,case when sum(case when InOut='O' and Out_Category in ('SA') then -1*Stock_Qty else 0 end)=0 then 0 else (sum(case when InOut='O' and Out_Category in ('SA') then -1*Cost else 0 end)/sum(case when InOut='O' and Out_Category in ('SA') then -1*Stock_Qty else 0 end)) end as IssSaleRate  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Balance_FAT else 0 end) as IssSaleFAT  ,(case when sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('SA') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end) end)  as IssSaleFATPER  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Balance_SNF else 0 end) as IssSaleSNF  ,(case when sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('SA') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end) end)  as IssSaleSNFPER  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Cost else 0 end) as IssSaleCost  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Stock_Qty else 0 end) as IssIssAdjQty  ,case when sum(case when InOut='O' and Out_Category in ('IS') then -1*Stock_Qty else 0 end)=0 then 0 else (sum(case when InOut='O' and Out_Category in ('IS') then -1*Cost else 0 end)/sum(case when InOut='O' and Out_Category in ('IS') then -1*Stock_Qty else 0 end)) end as IssIssAdjRate  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Balance_FAT else 0 end) as IssIssAdjFAT  ,(case when sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('IS') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end) end)  as IssIssAdjFATPER  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Balance_SNF else 0 end) as IssIssAdjSNF  ,(case when sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('IS') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end) end)  as IssIssAdjSNFPER  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Cost else 0 end) as IssIssAdjCost ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Stock_Qty else 0 end) as IssOthQty  ,case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Stock_Qty else 0 end)=0 then 0 else (sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Cost else 0 end)/sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Stock_Qty else 0 end)) end as IssOthRate  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Balance_FAT else 0 end) as IssOthFAT  ,(case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end) end)  as IssOthFATPER  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Balance_SNF else 0 end) as IssOthSNF  ,(case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end) end)  as IssOthSNFPER  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Cost else 0 end) as IssOthCost ,sum(case when InOut='O' then -1*Stock_Qty else 0 end) as IssQty  ,case when cast(sum(case when InOut='O' then -1*Stock_Qty else 0 end)as  decimal(18,2))=0 then 0 else (sum(case when InOut='O' then -1*Cost else 0 end)/sum(case when InOut='O' then -1*Stock_Qty else 0 end)) end as IssRate  ,sum(case when InOut='O' then -1*Balance_FAT else 0 end) as IssFAT  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_FAT else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssFATPER ,sum(case when InOut='O' then -1*Balance_SNF else 0 end) as IssSNF  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_SNF else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssSNFPER  ,sum(case when InOut='O' then -1*Cost else 0 end) as IssCost ,SUM(sum(Stock_Qty)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code,Punching_Date) as CLQty   ,SUM(sum(Cost)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code, Item_Code,Punching_Date) as CLCost ,SUM(sum(isnull(Balance_QTYKG,0))) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code, Punching_Date) as CLBalance_QTYKG  ,SUM(sum(Balance_FAT)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code, Punching_Date) as CLBalance_FAT ,SUM(sum(Balance_SNF)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code,Punching_Date) as CLBalance_SNF   from ("
                ...
                strFinalQry += " select Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,cast(Punching_Date as date) as Punching_Date,InOutView, InOut,Location_Code,[Loc Desp], [LocAddress],SourceCode,SourceName,SourceType ,Item_Type," &
                    ...
                    " ( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end) as Balance_SNF ,In_Category,Out_Category  "
                If ChkMRPWise.Checked Then
                    strFinalQry += ",MRP "
                End If
                strFinalQry += " from (" + qry + ") xxx " + Environment.NewLine
               ...
               " Items.Stock_UOM ,itf_code ,Stock_Qty,Balance_QTYKG,Rate,Cost,Balance_FAT, Balance_SNF ,In_Category,Out_Category "
                If ChkMRPWise.Checked Then
                    strFinalQry += ",MRP "
                End If
                ...
                Else
                    ...
                    End If
                End If
                strFinalQry += " )xxxxxx Group by  Item_Code,Location_Code,Punching_Date )xxxxxxx where Punching_Date is not null )x Order by  Punching_Date,Location_Code"
            ...
            If isPrintCrystalReport = 4 Then
                transportSql.BulkExport("Customer Stock Reco", strFinalQry, "order by Item_Code,Punching_Date", "csv")
                Exit Sub
            ElseIf isPrintCrystalReport = 5 Then
                transportSql.BulkExport("Customer Stock Reco", strFinalQry, "order by Item_Code,Punching_Date", "xls")
                Exit Sub
            End If
            ...
            If isPrintCrystalReport = 1 Then
                If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal Then
                    strFinalQry = " ;with stockCTE as   ( " + strFinalQry + " ) " &
                                  " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from ( " &
                                  " Select  1 As SNo,stockCTE.CompName,stockCTE.FromDate,stockCTE.ToDate ,stockCTE.Item_Code As FItem_code,stockCTE.Item_Desc As FItem_Desc, 'Opening' as Type, stockCTE.Item_Code , 'Opening' as Item_Desc , stockCTE.Stock_Qty from stockCTE where SourceCode = 'Opening' " &
                                    " union All " &
                                    " Select  1 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Opening' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc " &
                                    " union all " &
                                    " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from ( " &
                                    " Select  2 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferIn' as Type, stockCTE.Item_Code , 'Transfer In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('Transfer') and InOutView in( 'I','In' ) group by stockCTE.Item_Code " &
                                    " union All " &
                                    " Select  2 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer In' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc " &
                                    " union all " &
                                    " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from ( " &
                                    " Select  3 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,   'SaleReurnIn' as type, stockCTE.Item_Code , 'Sale Reurn In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('PS-SR','FS-SR') and InOutView in( 'I','In' ) group by stockCTE.Item_Code " &
                                    " union All " &
                                    " Select  3 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Sale Reurn In' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc " &
                                    " union all " &
                                    " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from ( " &
                                    " Select  4 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherIn' as type, stockCTE.Item_Code , 'Other In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'I','In' ) group by stockCTE.Item_Code " &
                                    " union All " &
                                    " Select  4 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other In' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc " &
                                    " union all " &
                                    " Select  5 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate,max(stockCTE.ToDate) As ToDate ,max(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'CustomerSale' as type, stockCTE.SourceCode as Item_Code , max(stockCTE.SourceName) as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.SourceCode " &
                                    " union all " &
                                    " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from ( " &
                                    " Select  6 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'TotalSale' as type, stockCTE.Item_Code as Item_Code , 'Total Sale' as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code " &
                                    " union All " &
                                    " Select  6 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Total Sale' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc " &
                                    " union All " &
                                    " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As  Stock_Qty from ( " &
                                    " Select  7 As Sno, max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferOut' as Type, stockCTE.Item_Code , 'Transfer Out' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('Transfer') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code " &
                                    " union All " &
                                    " Select  7 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer Out' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc " &
                                    " union all " &
                                    " Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from ( " &
                                    " Select  8 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,Max(stockCTE.ToDate) As ToDate ,(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherOut' as type, stockCTE.Item_Code , 'Other Out' as Item_Desc , sum (isnull(stockCTE.Stock_Qty,0)) as Stock_Qty  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code " &
                                    " union All " &
                                    " Select  8 As Sno,'' as CompName,'' as FromDate ,'' as ToDate " &
                                    " ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other Out' as Item_Desc , 0 as Stock_Qty " &
                                    " ) PPP group by PPP.Item_Desc "
    ...
    Sub SetGridFormationOFGV1()
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal Then
            ...
            gv1.Columns("InOutView").IsVisible = False
            ...
            ' kunal > ticket : BM00000009881 > Date : 01 - 0ct - 2016
            gv1.Columns("SourceCode").IsVisible = True
            gv1.Columns("SourceCode").Width = 100
            gv1.Columns("SourceCode").HeaderText = "Customer Code"

            gv1.Columns("SourceName").IsVisible = True
            gv1.Columns("SourceName").Width = 100
            gv1.Columns("SourceName").HeaderText = "Customer Name"
            ...
            'gv1.Columns("Group_Description").IsVisible = False
            'gv1.Columns("Group_Description").Width = 100
            'gv1.Columns("Group_Description").HeaderText = "Item Group"
            ...
            'summaryRowItem.Add(item2)
            'item2 = New GridViewSummaryItem("Prod_SNF_KG", "{0:F2}", GridAggregateFunction.Sum)
            'summaryRowItem.Add(item2)
            'item2 = New GridViewSummaryItem("Prod_Fat_KG", "{0:F2}", GridAggregateFunction.Sum)
            'summaryRowItem.Add(item2)

            'If (clsCommon.CompairString(cboDisplayMethod.SelectedValue, "None") = CompairStringResult.Equal) Then
            '    item2 = New GridViewSummaryItem("FATRATE", "{0:F2}", GridAggregateFunction.Avg)
            '    summaryRowItem.Add(item2)
            '    item2 = New GridViewSummaryItem("SNFRATE", "{0:F2}", GridAggregateFunction.Avg)
            '    summaryRowItem.Add(item2)
            'End If
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date and Item Wise Stock") = CompairStringResult.Equal Then
            ...
            gv1.Columns("RecAdjProFAT").IsVisible = (clsCommon.CompairString(clsCommon.myCstr(cboFATSNF.SelectedValue), "FS") = CompairStringResult.Equal)
            ...
            gv1.Columns("RecAdjProFATPER").IsVisible = (clsCommon.CompairString(clsCommon.myCstr(cboFATSNF.SelectedValue), "FS") = CompairStringResult.Equal)
            ...
            gv1.Columns("RecAdjProSNF").IsVisible = (clsCommon.CompairString(clsCommon.myCstr(cboFATSNF.SelectedValue), "FS") = CompairStringResult.Equal)
            ...
            gv1.Columns("RecAdjProSNFPER").IsVisible = (clsCommon.CompairString(clsCommon.myCstr(cboFATSNF.SelectedValue), "FS") = CompairStringResult.Equal)
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Transaction Wise") = CompairStringResult.Equal Then
            ...
            If TransResult IsNot Nothing AndAlso TransResult.Rows.Count > 0 Then
                For ii As Integer = 0 To TransResult.Rows.Count - 1
                    Dim item2 As New GridViewSummaryItem(TransResult.Rows(ii)("Name"), "{0:F2}", GridAggregateFunction.Sum)
                    ...
                    End If
                    summaryRowItem.Add(item2)
                    item2 = New GridViewSummaryItem(TransResult.Rows(ii)("Name") + " Fat_%", "{0:F2}", GridAggregateFunction.Sum)
                    summaryRowItem.Add(item2)
                    item2 = New GridViewSummaryItem(TransResult.Rows(ii)("Name") + "_Fat_KG", "{0:F2}", GridAggregateFunction.Sum)
                    summaryRowItem.Add(item2)
                    item2 = New GridViewSummaryItem(TransResult.Rows(ii)("Name") + " SNF_%", "{0:F2}", GridAggregateFunction.Sum)
                    summaryRowItem.Add(item2)
                    item2 = New GridViewSummaryItem(TransResult.Rows(ii)("Name") + "_SNF_KG", "{0:F2}", GridAggregateFunction.Sum)
                    summaryRowItem.Add(item2)
                    ...
                    summaryRowItem.Add(item2)
                    If gv1.Columns.Contains(clsCommon.myCstr("QC " + TransResult.Rows(ii)("Name") + " Fat_%")) Then
                        item2 = New GridViewSummaryItem("QC " + TransResult.Rows(ii)("Name") + " Fat_%", "{0:F2}", GridAggregateFunction.Sum)
                        summaryRowItem.Add(item2)
                    End If
                    If gv1.Columns.Contains(clsCommon.myCstr("QC " + TransResult.Rows(ii)("Name") + "_Fat_KG")) Then
                        item2 = New GridViewSummaryItem("QC " + TransResult.Rows(ii)("Name") + "_Fat_KG", "{0:F2}", GridAggregateFunction.Sum)
                        summaryRowItem.Add(item2)
                    End If
                    If gv1.Columns.Contains(clsCommon.myCstr("QC " + TransResult.Rows(ii)("Name") + " SNF_%")) Then
                        item2 = New GridViewSummaryItem("QC " + TransResult.Rows(ii)("Name") + " SNF_%", "{0:F2}", GridAggregateFunction.Sum)
                        summaryRowItem.Add(item2)
                    End If
                    If gv1.Columns.Contains(clsCommon.myCstr("QC " + TransResult.Rows(ii)("Name") + "_SNF_KG")) Then
                        item2 = New GridViewSummaryItem("QC " + TransResult.Rows(ii)("Name") + "_SNF_KG", "{0:F2}", GridAggregateFunction.Sum)
                        summaryRowItem.Add(item2)
                    End If
    ...
    Sub print(ByVal exporter As EnumExportTo)
        Try
            ...
            If clsCommon.myLen(txtItem.Value) > 0 Then
                arrHeader.Add("Item : " + txtItem.Value)
            End If
            If clsCommon.myLen(cmbUnit.SelectedValue) > 0 Then
    ...
    'Private Sub TxtMultiSelectFinder1__My_Click(sender As Object, e As EventArgs)
    '    Dim qry As String
    ...
    Private Sub btnQuickExport_Click(sender As Object, e As EventArgs)
        Try
            ...
            arrHeader.Add("Date Range: " + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") + " To " + clsCommon.GetPrintDate(txtToDate.Value, "dd/MM/yyyy"))
            arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.rptCustomerWiseStockReco & "'"))
            If clsCommon.myLen(txtItem.Value) > 0 Then
                arrHeader.Add("Item : " + txtItem.Value)
            End If
            If clsCommon.myLen(cmbUnit.SelectedValue) > 0 Then
    ...
    Private Sub PDFGrid_Click(sender As Object, e As EventArgs) Handles PDFGrid.Click
        Try
            ...
            pdfExporter.PageTitle = "Customer Stock Reco"
    ...
    Private Sub txtItem__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtItem._MYValidating
        ...
        txtItem.Value = clsCommon.ShowSelectForm("Item_Cutomer_Stock_Reco@Finder", qry, "Code", "Item_Type in ('F')", txtItem.Value, "Code", isButtonClicked)
    ...
    Sub PrintData()
        Try
            ...
            Query = " ;with stockCTE as   (  Select CompName,Trans_Type, InOutView, XXXXFinal.FromDate,XXXXFinal.ToDate, XXXXFinal.Trans_Id, XXXXFinal.Trans_Type_Name, XXXXFinal.Source_Doc_No,XXXXFinal.Punching_Date,XXXXFinal.Location_Code, XXXXFinal.[Loc Desp] , XXXXFinal.PrimaryLocation,XXXXFinal.SourceCode, XXXXFinal.SourceName , Item_Code, Item_Desc , Stock_UOM , Stock_Qty ,Rate,Cost,UOM  from ( select case when '0'='1' then '' else 'Alwar Zila Dugdh Utpadak Sahakari Sangh Ltd. ' end as CompName,'01/01/2025' as FromDate,'28/01/2025' as ToDate , Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,convert(varchar,Punching_Date,103) as Punching_Date,InOutView, InOut,Location_Code,[Loc Desp],PrimaryLocation,SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description,[GENERAL],[GENERALDESC],Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,Stock_Qty,case when (Source_Doc_No='Opening Balance' and ABS(isnull(cast(Stock_Qty as decimal(18,2)),0))=0 or ABS(isnull(cast(Cost as decimal(18,2)),0))<0.11 )then 0 else Rate end as Rate,Cost,isnull((CASE when (Balance_QTYKG=0 or (Source_Doc_No='Opening Balance' and [Balance_FAT]<0.11 ) ) then 0 else cast(([Balance_FAT]*100/[Balance_QTYKG]) as decimal(18,2)) end),0) as [Balance_FATPER] ,cast(Balance_FAT as decimal(18,2)) as Balance_FAT,isnull((CASE when (Balance_QTYKG=0 or (Source_Doc_No='Opening Balance' and [Balance_SNF]<0.11 ) ) then 0 else cast(([Balance_SNF]*100/[Balance_QTYKG]) as decimal(18,2)) end),0) as [Balance_SNFPER] , cast(Balance_SNF as decimal(18,2)) as Balance_SNF , cast(FATRATE as decimal(18,2)) as FATRATE, cast(SNFRATE as decimal(18,2)) as SNFRATE ,UOM 
 
 from (select (case when sum((case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end)=0 then 0 else sum((case when IsFromMilk=1 then MilkFATKG*Fat_Rate else (Stock_Qty*FatPer*Fat_Rate) end) * case when InOut='I' then 1 else -1 end)/sum((case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) end) AS FATRATE,(case when sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end))=0 then 0 else sum(( (case when IsFromMilk=1 then MilkSNFKG*SNF_Rate else (Stock_Qty*SNFPer*SNF_Rate) end ) * case when InOut='I' then 1 else -1 end))/sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) end ) AS SNFRATE, 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as Location_Code,'' as [Loc Desp],'' as [LocAddress],'' as PrimaryLocation,'Opening' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description,max([GENERAL]) as [GENERAL],max([GENERALDESC]) as [GENERALDESC],max(Item_Code)  as Item_Code ,max(Item_Desc)  as Item_Desc,'' as Item_Category_Struct_Code,'' as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,case when sum( Stock_Qty * case when InOut='I' then 1 else -1 end) =0 then 0 else convert(decimal(28,3), sum(Cost * case when InOut='I' then 1 else -1 end)/sum( Stock_Qty * case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF,max(UOM)UOM  
 
 from (select * from ( select CASE WHEN ITEMDETAIL1.Report_UOM=1 THEN ((InventroyMovement.Stock_Qty *isnull((TSPL_ITEM_UOM_DETAIL.Conversion_Factor),1))/ITEMDETAIL1.Conversion_Factor) end as Stock_Qty,
case when ITEMDETAIL1.Report_UOM=1 then (ITEMDETAIL1.UOM_Code) end as [UOM], 
 gl1.Account_code as Inventory_Control_Acc,gl1.Description as Inventory_Control_Acc_desc ,InventroyMovement.Fat_Rate,InventroyMovement.SNF_Rate ,InventroyMovement.Trans_Id,InventroyMovement.Trans_Type, (CASE WHEN (InventroyMovement.Trans_Type='IC-AD' AND TSPL_ADJUSTMENT_HEADER.Reference_Document='JWO-SRN-JLO') THEN 'Jobwork Consumption' ELSE  TSPL_INVENTORY_SOURCE_CODE.Name END )as Trans_Type_Name,InventroyMovement.Source_Doc_No,InventroyMovement.Punching_Date, InventroyMovement.InOut,case when InventroyMovement.InOut='I' then 'In' else case when InventroyMovement.InOut='O' then 'Out' else '' end end as 'InOutView', case when TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' then TSPL_LOCATION_MASTER.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end as Main_Location_Code,MainLocationTable.Location_Desc as MainLocationDesc, InventroyMovement.Location_Code,TSPL_LOCATION_MASTER.Location_Desc AS [Loc Desp],TSPL_LOCATION_MASTER.Add1+Case When ISNULL(TSPL_LOCATION_MASTER.Add2,'')='' Then ''  else ', '+TSPL_LOCATION_MASTER.Add2+ Case When ISNULL(TSPL_LOCATION_MASTER.Add3,'')='' Then '' Else ', '+TSPL_LOCATION_MASTER.Add3+ Case When ISNULL(TSPL_LOCATION_MASTER.Pin_Code ,'')='' Then '' else '-'+CONVERT(varchar, TSPL_LOCATION_MASTER.Pin_Code) End End End  as [LocAddress],SourceCode,SourceName,SourceType  ,Item_Group.Item_Group,Item_Group.Group_Description, InventroyMovement.Item_Code, InventroyMovement.MRP ,TSPL_ITEM_MASTER.Item_Desc,tspl_item_master.itf_code,TSPL_ITEM_MASTER.Structure_Code,TSPL_STRUCTURE_MASTER.Structure_Descq, IsFromMilk,MilkFATKG,MilkSNFKG,case when IsFromMilk=1 then MilkFatPer else isnull((select TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range  from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) end as MilkFatPer,case when IsFromMilk=1 then MilkSNFPer else isnull((select TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range  from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) end as MilkSNFPer,TSPL_LOCATION_MASTER.Is_Section,TSPL_LOCATION_MASTER.Is_Sub_Location, isnull((InventroyMovement.Stock_Qty * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end))  ,0) as QtyKG, InventroyMovement.Stock_UOM, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) as FatPer, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) as SNFPer, (case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=3 then InventroyMovement.FIFO_Cost else case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=2 then InventroyMovement.LIFO_Cost else InventroyMovement.Avg_Cost end end ) as Cost,TSPL_ITEM_MASTER.Item_Category_Struct_Code 
,VirtualCategoryTabel.[GENERAL],VirtualCategoryTabel.[GENERALDESC] ,TSPL_ITEM_MASTER.Item_Type,VirtualTableItemType.Name as Item_Type_Name,TSPL_INVENTORY_SOURCE_CODE.In_Category,TSPL_INVENTORY_SOURCE_CODE.Out_Category,TSPL_INVENTORY_SOURCE_CODE.Code,(case when ISNULL(InventroyMovement.Location_Code,'')='' then InventroyMovement.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) as PrimaryLocation  from (  select 0 AS Fat_Rate,0 AS SNF_Rate ,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Qty as Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_INVENTORY_MOVEMENT 
 union all 
 select ISNULL(Fat_Rate,0) AS Fat_Rate,ISNULL(SNF_Rate,0) AS SNF_Rate,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Qty as Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,1 as IsFromMilk,Fat_Per as MilkFatPer ,SNF_Per as MilkSNFPer,Fat_KG as MilkFATKG,SNF_KG as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,isnull(Custom_UOM,'') as Custom_UOM,isnull(Custom_Coversion_Factor,0) as Custom_Coversion_Factor from TSPL_INVENTORY_MOVEMENT_NEW
) InventroyMovement 
 left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=InventroyMovement.Item_Code
 Left outer join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=InventroyMovement.Item_Code And  TSPL_ITEM_UOM_DETAIL .UOM_Code=InventroyMovement.UOM
...
and TSPL_item_uom_detail.Report_UOM=1)and TSPL_item_uom_detail.Report_UOM=1   ) as ITEMDETAIL1 on ITEMDETAIL1.Item_code=InventroyMovement.Item_code
 ...
 Pivot 
 ( max(Item_Cagetory_Values) for Item_Category_Code   in ( [GENERAL])
 ) Pivt
 Pivot 
 (
 max(Category_Value_Desc) for Item_Category_CodeDesc in ([GENERALDESC])
 ) Pivt1 
            ...
            If clsCommon.myLen(txtItem.Value) > 0 Then
                Query += " and Item_Code in ('" + txtItem.Value + "') " + Environment.NewLine
            Else
                clsCommon.MyMessageBoxShow(Me, "Please select Item First", Me.Text)
                Exit Sub
            End If
...
group by Item_Code  
 ...
 select Fat_Rate AS FATRATE ,SNF_Rate AS SNFRATE, Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,Punching_Date,InOutView, InOut,Location_Code,[Loc Desp], [LocAddress],PrimaryLocation,SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description,[GENERAL],[GENERALDESC],Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,(QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,convert(decimal(28,3),case when Stock_Qty=0 then 0 else Cost/Stock_Qty end) as Rate,(Cost * case when InOut='I' then 1 else -1 end) as Cost,( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT, ( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end) as Balance_SNF ,UOM 
 
 from (select * from
 
 ( select CASE WHEN ITEMDETAIL1.Report_UOM=1 THEN ((InventroyMovement.Stock_Qty *isnull((TSPL_ITEM_UOM_DETAIL.Conversion_Factor),1))/ITEMDETAIL1.Conversion_Factor) end as Stock_Qty,
 Case when ITEMDETAIL1.Report_UOM=1 then (ITEMDETAIL1.UOM_Code) end as [UOM], 
 gl1.Account_code as Inventory_Control_Acc,gl1.Description as Inventory_Control_Acc_desc ,InventroyMovement.Fat_Rate,InventroyMovement.SNF_Rate ,InventroyMovement.Trans_Id,InventroyMovement.Trans_Type, (CASE WHEN (InventroyMovement.Trans_Type='IC-AD' AND TSPL_ADJUSTMENT_HEADER.Reference_Document='JWO-SRN-JLO') THEN 'Jobwork Consumption' ELSE  TSPL_INVENTORY_SOURCE_CODE.Name END )as Trans_Type_Name,InventroyMovement.Source_Doc_No,InventroyMovement.Punching_Date, InventroyMovement.InOut,case when InventroyMovement.InOut='I' then 'In' else case when InventroyMovement.InOut='O' then 'Out' else '' end end as 'InOutView', case when TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' then TSPL_LOCATION_MASTER.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end as Main_Location_Code,MainLocationTable.Location_Desc as MainLocationDesc, InventroyMovement.Location_Code,TSPL_LOCATION_MASTER.Location_Desc AS [Loc Desp],TSPL_LOCATION_MASTER.Add1+Case When ISNULL(TSPL_LOCATION_MASTER.Add2,'')='' Then ''  else ', '+TSPL_LOCATION_MASTER.Add2+ Case When ISNULL(TSPL_LOCATION_MASTER.Add3,'')='' Then '' Else ', '+TSPL_LOCATION_MASTER.Add3+ Case When ISNULL(TSPL_LOCATION_MASTER.Pin_Code ,'')='' Then '' else '-'+CONVERT(varchar, TSPL_LOCATION_MASTER.Pin_Code) End End End  as [LocAddress],SourceCode,SourceName,SourceType  ,Item_Group.Item_Group,Item_Group.Group_Description, InventroyMovement.Item_Code, InventroyMovement.MRP ,TSPL_ITEM_MASTER.Item_Desc,tspl_item_master.itf_code,TSPL_ITEM_MASTER.Structure_Code,TSPL_STRUCTURE_MASTER.Structure_Descq, IsFromMilk,MilkFATKG,MilkSNFKG,case when IsFromMilk=1 then MilkFatPer else isnull((select TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range  from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) end as MilkFatPer,case when IsFromMilk=1 then MilkSNFPer else isnull((select TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range  from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) end as MilkSNFPer,TSPL_LOCATION_MASTER.Is_Section,TSPL_LOCATION_MASTER.Is_Sub_Location, isnull((InventroyMovement.Stock_Qty * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end))  ,0) as QtyKG, InventroyMovement.Stock_UOM, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) as FatPer, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) as SNFPer, (case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=3 then InventroyMovement.FIFO_Cost else case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=2 then InventroyMovement.LIFO_Cost else InventroyMovement.Avg_Cost end end ) as Cost,TSPL_ITEM_MASTER.Item_Category_Struct_Code 
,VirtualCategoryTabel.[GENERAL],VirtualCategoryTabel.[GENERALDESC] ,TSPL_ITEM_MASTER.Item_Type,VirtualTableItemType.Name as Item_Type_Name,TSPL_INVENTORY_SOURCE_CODE.In_Category,TSPL_INVENTORY_SOURCE_CODE.Out_Category,TSPL_INVENTORY_SOURCE_CODE.Code,(case when ISNULL(InventroyMovement.Location_Code,'')='' then InventroyMovement.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) as PrimaryLocation  from (  select 0 AS Fat_Rate,0 AS SNF_Rate ,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Qty as Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_INVENTORY_MOVEMENT 
 union all 
...
Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from (  Select  2 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferIn' as Type, stockCTE.Item_Code , 'Transfer In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('ITransfer') and InOutView in( 'I','In' ) group by stockCTE.Item_Code  union All  Select  2 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer In' as Item_Desc , 0 as Stock_Qty  ) PPP group by PPP.Item_Desc  union all  Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from (  Select  3 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,   'SaleReurnIn' as type, stockCTE.Item_Code , 'Sale Reurn In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('PS-SR','FS-SR') and InOutView in( 'I','In' ) group by stockCTE.Item_Code  union All  Select  3 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Sale Reurn In' as Item_Desc , 0 as Stock_Qty  ) PPP group by PPP.Item_Desc  union all  Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from (  Select  4 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherIn' as type, stockCTE.Item_Code , 'Other In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'I','In' ) group by stockCTE.Item_Code  union All  Select  4 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other In' as Item_Desc , 0 as Stock_Qty  ) PPP group by PPP.Item_Desc  union all  Select  5 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate,max(stockCTE.ToDate) As ToDate ,max(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'CustomerSale' as type, stockCTE.SourceCode as Item_Code , max(stockCTE.SourceName) as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.SourceCode  union all  Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from (  Select  6 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'TotalSale' as type, stockCTE.Item_Code as Item_Code , 'Total Sale' as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code  union All  Select  6 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Total Sale' as Item_Desc , 0 as Stock_Qty  ) PPP group by PPP.Item_Desc  union All  Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As  Stock_Qty from (  Select  7 As Sno, max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferOut' as Type, stockCTE.Item_Code , 'Transfer Out' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty  from stockCTE where Trans_Type in ('ITransfer') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code  union All  Select  7 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer Out' as Item_Desc , 0 as Stock_Qty  ) PPP group by PPP.Item_Desc  union all  Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty from (  Select  8 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,Max(stockCTE.ToDate) As ToDate ,(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherOut' as type, stockCTE.Item_Code , 'Other Out' as Item_Desc , sum (isnull(stockCTE.Stock_Qty,0)) as Stock_Qty  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code  union All  Select  8 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other Out' as Item_Desc , 0 as Stock_Qty  ) PPP group by PPP.Item_Desc "
            '            Query = " ;with stockCTE as   (  Select CompName,Trans_Type, InOutView, XXXXFinal.FromDate,XXXXFinal.ToDate, XXXXFinal.Trans_Id, XXXXFinal.Trans_Type_Name, XXXXFinal.Source_Doc_No,XXXXFinal.Punching_Date,XXXXFinal.Location_Code, XXXXFinal.[Loc Desp] , XXXXFinal.PrimaryLocation,XXXXFinal.SourceCode, XXXXFinal.SourceName , Item_Code, Item_Desc , Stock_UOM , Stock_Qty ,Rate,Cost from 
            '( 
            '  select case when case when '" & LocationFirstTime & "'='1' then '" & LocationAddress & "' else '" + objCommonVar.CurrentCompanyName + " ' end as CompName,'" + clsCommon.myCDate(txtFromDate.Value, "dd/MMM/yyyy") + "' as FromDate,'" + clsCommon.myCDate(txtToDate.Value, "dd/MMM/yyyy") + "' as ToDate,Fat_Rate AS FATRATE ,SNF_Rate AS SNFRATE, Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,Punching_Date,InOutView, InOut,Location_Code,[Loc Desp], [LocAddress],PrimaryLocation,SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description,[GENERAL],[GENERALDESC],Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,
            '( Quantity * case when InOut='I' then 1 else -1 end) as Stock_Qty,(QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,convert(decimal(28,3),case when Stock_Qty=0 then 0 else Cost/Stock_Qty end) as Rate,(Cost * case when InOut='I' then 1 else -1 end) as Cost,( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT, ( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end) as Balance_SNF,Quantity,UOM from (select * from ( select CASE WHEN ITEMDETAIL1.Report_UOM=1 THEN ((Stock_Qty *isnull((TSPL_ITEM_UOM_DETAIL.Conversion_Factor),1))/ITEMDETAIL1.Conversion_Factor) end as [Quantity],case when ITEMDETAIL1.Report_UOM=1 then (ITEMDETAIL1.UOM_Code) end as [UOM], gl1.Account_code as Inventory_Control_Acc,gl1.Description as Inventory_Control_Acc_desc ,InventroyMovement.Fat_Rate,InventroyMovement.SNF_Rate ,InventroyMovement.Trans_Id,InventroyMovement.Trans_Type, (CASE WHEN (InventroyMovement.Trans_Type='IC-AD' AND TSPL_ADJUSTMENT_HEADER.Reference_Document='JWO-SRN-JLO') THEN 'Jobwork Consumption' ELSE  TSPL_INVENTORY_SOURCE_CODE.Name END )as Trans_Type_Name,InventroyMovement.Source_Doc_No,InventroyMovement.Punching_Date, InventroyMovement.InOut,case when InventroyMovement.InOut='I' then 'In' else case when InventroyMovement.InOut='O' then 'Out' else '' end end as 'InOutView', case when TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' then TSPL_LOCATION_MASTER.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end as Main_Location_Code,MainLocationTable.Location_Desc as MainLocationDesc, InventroyMovement.Location_Code,TSPL_LOCATION_MASTER.Location_Desc AS [Loc Desp],TSPL_LOCATION_MASTER.Add1+Case When ISNULL(TSPL_LOCATION_MASTER.Add2,'')='' Then ''  else ', '+TSPL_LOCATION_MASTER.Add2+ Case When ISNULL(TSPL_LOCATION_MASTER.Add3,'')='' Then '' Else ', '+TSPL_LOCATION_MASTER.Add3+ Case When ISNULL(TSPL_LOCATION_MASTER.Pin_Code ,'')='' Then '' else '-'+CONVERT(varchar, TSPL_LOCATION_MASTER.Pin_Code) End End End  as [LocAddress],SourceCode,SourceName,SourceType  ,Item_Group.Item_Group,Item_Group.Group_Description, InventroyMovement.Item_Code, InventroyMovement.MRP ,TSPL_ITEM_MASTER.Item_Desc,tspl_item_master.itf_code,TSPL_ITEM_MASTER.Structure_Code,TSPL_STRUCTURE_MASTER.Structure_Descq, IsFromMilk,MilkFATKG,MilkSNFKG,case when IsFromMilk=1 then MilkFatPer else isnull((select TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range  from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) end as MilkFatPer,case when IsFromMilk=1 then MilkSNFPer else isnull((select TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range  from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) end as MilkSNFPer,TSPL_LOCATION_MASTER.Is_Section,TSPL_LOCATION_MASTER.Is_Sub_Location, isnull((InventroyMovement.Stock_Qty * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end))  ,0) as QtyKG, InventroyMovement.Stock_UOM,InventroyMovement.Stock_Qty, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER  left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='FAT'),0) as FatPer, isnull((select ((TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range/100) * (case when FATSNFConvertedUnit.Conversion_Factor=0 then 0 else 1/FATSNFConvertedUnit.Conversion_Factor end)) from TSPL_ITEM_QC_PARAMETER_MASTER left outer join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where item_code=InventroyMovement.Item_Code and Type='SNF'),0) as SNFPer, (case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=3 then InventroyMovement.FIFO_Cost else case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=2 then InventroyMovement.LIFO_Cost else InventroyMovement.Avg_Cost end end ) as Cost,TSPL_ITEM_MASTER.Item_Category_Struct_Code 
            ',VirtualCategoryTabel.[GENERAL],VirtualCategoryTabel.[GENERALDESC] ,TSPL_ITEM_MASTER.Item_Type,VirtualTableItemType.Name as Item_Type_Name,TSPL_INVENTORY_SOURCE_CODE.In_Category,TSPL_INVENTORY_SOURCE_CODE.Out_Category,TSPL_INVENTORY_SOURCE_CODE.Code,(case when ISNULL(InventroyMovement.Location_Code,'')='' then InventroyMovement.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) as PrimaryLocation  from (  select 0 AS Fat_Rate,0 AS SNF_Rate ,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Qty as Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,'' as Custom_UOM,0 as Custom_Coversion_Factor  from TSPL_INVENTORY_MOVEMENT union all  select ISNULL(Fat_Rate,0) AS Fat_Rate,ISNULL(SNF_Rate,0) AS SNF_Rate,Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Qty as Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,1 as IsFromMilk,Fat_Per as MilkFatPer ,SNF_Per as MilkSNFPer,Fat_KG as MilkFATKG,SNF_KG as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType,isnull(Custom_UOM,'') as Custom_UOM,isnull(Custom_Coversion_Factor,0) as Custom_Coversion_Factor from TSPL_INVENTORY_MOVEMENT_NEW ) InventroyMovement  left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=InventroyMovement.Item_Code  Left outer join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=InventroyMovement.Item_Code And  TSPL_ITEM_UOM_DETAIL .UOM_Code=InventroyMovement.UOM  left join (select Conversion_factor,TSPL_ITEM_UOM_DETAIL.Item_code,Report_UOM,UOM_Code from TSPL_ITEM_UOM_DETAIL where UOM_code in (select UOM_Code  from TSPL_item_uom_detail where Item_Code = TSPL_ITEM_UOM_DETAIL.Item_code and TSPL_item_uom_detail.Report_UOM=1)and TSPL_item_uom_detail.Report_UOM=1   ) as ITEMDETAIL1 on ITEMDETAIL1.Item_code=InventroyMovement.Item_code  left outer join TSPL_STRUCTURE_MASTER on TSPL_STRUCTURE_MASTER.Structure_Code=TSPL_ITEM_MASTER.Structure_Code  left outer join TSPL_PURCHASE_ACCOUNTS on TSPL_PURCHASE_ACCOUNTS.Purchase_Class_Code=TSPL_ITEM_MASTER.Purchase_Class_Code  left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code = InventroyMovement.Location_Code left outer join TSPL_LOCATION_MASTER as MainLocationTable on MainLocationTable.Location_Code =(case when TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' then TSPL_LOCATION_MASTER.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) left outer join TSPL_ITEM_UOM_DETAIL as FATSNFConvertedUnit on FATSNFConvertedUnit.Item_Code=InventroyMovement.Item_Code and FATSNFConvertedUnit.UOM_Code='KG' left outer join TSPL_INVENTORY_SOURCE_CODE on TSPL_INVENTORY_SOURCE_CODE.code=InventroyMovement.Trans_Type  left outer join TSPL_ADJUSTMENT_HEADER ON TSPL_ADJUSTMENT_HEADER.Adjustment_No=InventroyMovement.Source_Doc_No   left outer join (select Item_Code,max([GENERAL]) as [GENERAL],max([GENERALDESC]) as [GENERALDESC]  from ( select * from (  select TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code  ,TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code+'DESC' as Item_Category_CodeDesc ,TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION as Category_Value_Desc  from  TSPL_ITEM_MASTER left outer join TSPL_ITEM_MASTER_CATEGORY on  TSPL_ITEM_MASTER_CATEGORY.Item_code = TSPL_ITEM_MASTER.Item_code  left outer join TSPL_ITEM_CATEGORY_LEVEL_VALUES on TSPL_ITEM_CATEGORY_LEVEL_VALUES.ITEM_CATEGORY_CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code and TSPL_ITEM_CATEGORY_LEVEL_VALUES.CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values
            ' where 2=2 
            ' )xx
            ' Pivot 
            ' ( max(Item_Cagetory_Values) for Item_Category_Code   in ( [GENERAL])
            ...
            '            Query += "
            ') xxx 
            ' where Punching_Date>='" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' and Punching_Date<='" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' --and SourceType = 'C' 
            ')XXXXFinal  )
            'Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,MAX(UOM)UOM from 
            '(  Select  1 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferIn' as Type, stockCTE.Item_Code , 'Transfer In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(UOM) AS UOM  from stockCTE where Trans_Type in ('ITransfer') and InOutView in( 'I','In' ) group by stockCTE.Item_Code  
            'union All  
            'Select  1 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer In' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc  
            'union all  
            'Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,MAX(UOM)UOM from (  Select  2 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherIn' as type, stockCTE.Item_Code , 'Other In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(UOM)UOM  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'I','In' ) group by stockCTE.Item_Code 
            'union All  
            'Select  2 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other In' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc  
            'union all 
            'Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,MAX(UOM)UOM from (  Select  3 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'TotalSale' as type, stockCTE.Item_Code as Item_Code , 'Total Sale' as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(UOM)UOM from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code 
            'union All 
            'Select  3 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Total Sale' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc  
            'union All  
            'Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As  Stock_Qty,MAX(UOM)UOM from (  Select  4 As Sno, max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferOut' as Type, stockCTE.Item_Code , 'Transfer Out' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(UOM)UOM  from stockCTE where Trans_Type in ('ITransfer') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code  
            'union All  
            'Select  4 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer Out' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc  union all  Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,MAX(UOM)UOM from (  Select  5 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,Max(stockCTE.ToDate) As ToDate ,(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherOut' as type, stockCTE.Item_Code , 'Other Out' as Item_Desc , sum (isnull(stockCTE.Stock_Qty,0)) as Stock_Qty,max(stockCTE.UOM) As UOM  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code 
            'union All  
            'Select  5 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other Out' as Item_Desc , 0 as Stock_Qty,'' AS UOM 
            ') PPP group by PPP.Item_Desc
            '"
