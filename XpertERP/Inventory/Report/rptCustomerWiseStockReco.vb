...
Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,max(UOM)UOM 

from (  Select  2 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferIn' as Type, stockCTE.Item_Code , 'Transfer InWard' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty ,max(stockCTE.UOM)UOM from stockCTE where Trans_Type in ('ITransfer') and InOutView in( 'I','In' ) and Location_Code='FGS' AND SourceCode='ALWAR' group by stockCTE.Item_Code  
union All 
Select  2 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer InWard' as Item_Desc , 0 as Stock_Qty ,'' as UOM ) PPP group by PPP.Item_Desc 

union all 
Select max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty ,max(UOM)UOM

from (  Select  3 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,   'SaleReurnIn' as type, stockCTE.Item_Code , 'Sale Reurn In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(stockCTE.UOM)UOM  from stockCTE where Trans_Type in ('PS-SR','FS-SR') and InOutView in( 'I','In' ) group by stockCTE.Item_Code  union All  Select  3 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Sale Reurn In' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc 

union all  

Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,Max(UOM)UOM
from (  Select  4 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherIn' as type, stockCTE.Item_Code , 'Other In' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(stockCTE.UOM)UOM  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'I','In' ) group by stockCTE.Item_Code 
union All  Select  4 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other In' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc 

union all 
Select  5 As Sno,(stockCTE.CompName) As CompName,(stockCTE.FromDate) As FromDate,(stockCTE.ToDate) As ToDate ,(stockCTE.Item_Code) As FItem_code,(stockCTE.Item_Desc) As FItem_Desc, '' as type, '' as Item_Code , 'Movement OutWard' as Item_Desc , 0 as Stock_Qty,'' AS UOM  from stockCTE 
where Trans_Type not in ('PS-SH','FS-SH') and InOutView not in( 'O','Out','I','In' ) 
union all
Select  6 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate,max(stockCTE.ToDate) As ToDate ,max(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'CustomerSale' as type, stockCTE.SourceCode as Item_Code , max(stockCTE.SourceName) as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(stockCTE.UOM)UOM  from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.SourceCode 

union all 
Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty ,MAX(UOM)UOM
from (  Select  7 As Sno,max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'TotalSale' as type, stockCTE.Item_Code as Item_Code , 'Total Sale' as Item_Desc , -1* sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(stockCTE.UOM)UOM from stockCTE where Trans_Type in ('PS-SH','FS-SH') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code 
union All  Select  7 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Total Sale' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc 

union All 
Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As  Stock_Qty ,MAX(UOM)UOM
from (  Select  8 As Sno, max (stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,max(stockCTE.ToDate) As ToDate ,stockCTE.Item_Code As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc,'TransferOut' as Type, stockCTE.Item_Code , 'Transfer OutWard' as Item_Desc , sum (stockCTE.Stock_Qty) as Stock_Qty,MAX(stockCTE.UOM)UOM  from stockCTE where Trans_Type in ('ITransfer') and InOutView in( 'O','Out' ) and Location_Code='FGS' AND SourceCode='ALWAR' group by stockCTE.Item_Code  union All  Select  8 As Sno,'' as CompName,'' as FromDate ,'' as ToDate   ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Transfer OutWard' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc 
union all 
Select  max(PPP.Sno) As Sno , max (ppp.CompName) As CompName, max (FromDate) As FromDate  , max(ToDate) As ToDate,max(FItem_code) As FItem_code  ,max( FItem_Desc) As FItem_Desc , max(type) As type , max(Item_Code) As Item_Code , Item_Desc, sum ( isnull (Stock_Qty,0)) As Stock_Qty,MAX(UOM)UOM from (  Select  9 As Sno,max(stockCTE.CompName) As CompName,max(stockCTE.FromDate) As FromDate ,Max(stockCTE.ToDate) As ToDate ,(stockCTE.Item_Code) As FItem_code,max(stockCTE.Item_Desc) As FItem_Desc, 'OtherOut' as type, stockCTE.Item_Code , 'Other Out' as Item_Desc , sum (isnull(stockCTE.Stock_Qty,0)) as Stock_Qty,MAX(stockCTE.UOM)UOM  from stockCTE where Trans_Type in ('IC-AD','SRN') and InOutView in( 'O','Out' ) group by stockCTE.Item_Code  union All  Select  9 As Sno,'' as CompName,'' as FromDate ,'' as ToDate  ,'' as FItem_code,'' as FItem_Desc, '' as type, '' as Item_Code , 'Other Out' as Item_Desc , 0 as Stock_Qty,'' AS UOM  ) PPP group by PPP.Item_Desc
"
