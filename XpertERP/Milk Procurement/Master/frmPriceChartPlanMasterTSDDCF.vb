...
Public Class frmPriceChartPlanMasterTSDDCF
...
#End Region
    ...
    Sub SaveData()
        Try
            If AllowToSave() Then
                ...
                obj.TSDDCS_Rate = txtTSDDCFRate.Value
                If rbtnTSDDCFFatKg.IsChecked Then
                    obj.TSDDCS_Calcualtion_Method = 1
                ElseIf rbtnTSDDCFTotalSolid.IsChecked Then
                    obj.TSDDCS_Calcualtion_Method = 2
                End If
                ...
                For ii As Integer = 0 To gvFATDed.RowCount - 1
                    ...
                    objTSDDCF.FAT_From = clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colFATFrom).Value)
                    objTSDDCF.FAT_To = clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colFATTo).Value)
                    objTSDDCF.Apply_FAT = clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplyFAT).Value)
                    objTSDDCF.SNF_From = clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colSNFFrom).Value)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavType As NavigatorType)
        Try
            ...
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Planning_Code) > 0 Then
                ...
                txtTSDDCFRate.Value = obj.TSDDCS_Rate
                If obj.TSDDCS_Calcualtion_Method = 1 Then
                    rbtnTSDDCFFatKg.IsChecked = True
                ElseIf obj.TSDDCS_Calcualtion_Method = 2 Then
                    rbtnTSDDCFTotalSolid.IsChecked = True
                End If
    ...
    Private Function CalculateRate(ByVal dblFATPer As Decimal, ByVal dblSNFPer As Decimal) As Double
        ...
        If dblFATPer = 5 AndAlso dblSNFPer = 7.5 Then
            Dim x As Integer = 0
        End If
        ...
        For ii As Integer = 0 To gvFATDed.Rows.Count - 1
            If clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colRatePer).Value) > 0 Then
                If dblFATPer >= clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colFATFrom).Value) AndAlso dblFATPer <= clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colFATTo).Value) Then
                    If dblSNFPer >= clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colSNFFrom).Value) AndAlso dblSNFPer <= clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colSNFTo).Value) Then
                        If clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplyFAT).Value) > 0 Then
                            If dblFATPer > clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplyFAT).Value) Then
                                dblFATPer = clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplyFAT).Value)
                            End If
                        End If
                        If clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplySNF).Value) > 0 Then
                            If dblSNFPer > clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplySNF).Value) Then
                                dblSNFPer = clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colApplySNF).Value)
                            End If
                        End If
                        dclRate = (txtTSDDCFRate.Value * clsCommon.myCdbl(gvFATDed.Rows(ii).Cells(colRatePer).Value)) / 100
                        If rbtnTSDDCFFatKg.IsChecked Then
                            dclReturnMilkValue = (dblFATPer * dclRate) / 100
                        ElseIf rbtnTSDDCFTotalSolid.IsChecked Then
                            dclReturnMilkValue = ((dblFATPer + txtSnfPer.Value) * dclRate) / 100
                        End If
            ...
            End If
        Next
        dclReturnMilkValue = Math.Round(dclReturnMilkValue, 3)
