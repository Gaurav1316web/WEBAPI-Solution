Imports common
Public Class frmPendingSaleInvoiceMCC
#Region "Variables"
...
#End Region
    ...
    Sub LoadData()
        Try
            ...
            qry += " where TSPL_SD_SALE_INVOICE_DETAIL.Status=0 and Scheme_Item='N' and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='MCC' and TSPL_SD_SALE_INVOICE_HEAD.Status=1 and Invoice_Type in ('N','T','R')  and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) " + Environment.NewLine
            If clsCommon.myLen(VendorCode) > 0 Then
            ...
            qry += " where  TSPL_SD_SALE_RETURN_HEAD.Status=1 and Scheme_Item='N' and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='MCC' and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0   " + Environment.NewLine
            qry += " union all   " + Environment.NewLine
            ...
            qry += " ,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALE_RETURN_HEAD on TSPL_SD_SALE_RETURN_HEAD.Document_Code=TSPL_SD_SALE_RETURN_DETAIL.Document_Code" + Environment.NewLine
            qry += " where  TSPL_SD_SALE_RETURN_HEAD.Status=0 and Scheme_Item='N' and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='MCC' and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0 and TSPL_SD_SALE_RETURN_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
