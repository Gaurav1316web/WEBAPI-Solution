...
'created by --> Monika
'createddate --> 24/05/2014
'==============BM00000003315==========================
'' updation by richa agarwal against ticket no BM00000006003 27/03/2015
'-------update by Preeti Gupta Against Ticket No[BM00000008210]
Public Class frmTankerTransporterMaster
...
#Region "Page Load"
    ...
    Private Sub frmTankerTransporterMaster_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        LoadBlankGvTankerDetails()
        RadPageViewPage8.Item.Visibility = ElementVisibility.Hidden
    End Sub
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.frmTankerTransporterMaster)
...
#Region "Function"
    ...
    Public Sub funfillfndGroupCode()
        Try
            ...
            If dr.Rows IsNot Nothing AndAlso dr.Rows.Count > 0 Then
                ...
                'txtbankcodedes.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select description from tspl_bank_master where bank_code='" + fndbankcode.Value + "'"))
    ...
    Public Sub funfill()
        Try
            ...
            strCmd = " Select Vendor_Name, Vendor_Group_Code,  Vendor_Group_Code_Desc,  Status ,OnHold  ,Convert(Date,Closing_Date,103) as Closing_Date  ,Add1 ,	Add2 ,Add3 ," & _
                     "City_Code ,City_Code_Desc ,State ,Country ,	Phone1 ,Phone2 ,Fax,Email ,WebSite ,Contact_Person_Name ,Contact_Person_Phone ," & _
                     "Contact_Person_Fax ,Contact_Person_Website ,Contact_Person_Email ,Terms_Code ,Terms_Code_Desc ,Vendor_Account ,Vendor_Account_Desc ," & _
                     "Payment_Code,Payment_Code_Desc ,Ven_Type_Code ,Ven_Type_Desc ,Bank_Code ,Bank_Code_Desc ,Service_Tax_No ,Lst_No ,Tin_No ,	Credit_Limit ," & _
                     "Tax_Group ,Tax_Group_Desc ,TAX1 ,TAX1_Rate ,TAX2,TAX2_Rate ,TAX3 ,TAX3_Rate ,TAX4 ,TAX4_Rate ,TAX5 ,TAX5_Rate ,TAX6 ,TAX6_Rate ," & _
                     "TAX7 ,TAX7_Rate ,TAX8 ,TAX8_Rate ,TAX9 ,TAX9_Rate ,TAX10 ,TAX10_Rate ,Remarks1 ,Remarks2 ,Additional1 ,Additional2 ,Additional3,transporter,CST,ECC,Range,Collectorate,PAN,is_Gross_Receipt,Inter_branch,currency_code,franchise_yn,state_code,country_code,vsp_payee_name,zila,tehsil,branch_name,ifci_code,account_no,industry_type,industry_person,agreement,Security_Cheque,IsPermanent,IsTemporary,branch_code,Account_Type,Start_period,Expired_Period,IsBlankCheque,Cheque_In_Favour_Of,MinimumQtyRequired_pertrip,GSTRegistered,GSTEntity,GSTLastEntity,GSTFinalNo, isBulkProcurement from tspl_vendor_master where vendor_code='" + fndvendorNo.Value + "' and form_type='TTM'	"
            ...
            For Each myDr In myDs.Tables(0).Rows
                ...
                End If
                Dim strIsBulkPro As String = myDr("isBulkProcurement").ToString()
                If strIsBulkPro = "1" Then
                    chkBulkProcurement.Checked = True
                    RadPageViewPage8.Item.Visibility = ElementVisibility.Visible
                Else
                    chkBulkProcurement.Checked = False
                    RadPageViewPage8.Item.Visibility = ElementVisibility.Hidden
                End If
                ...
                If (myDr("Closing_Date").ToString()) = Nothing Then
                    Me.dtClosing.Value = System.DateTime.Now.Date
                Else
                    Me.dtClosing.Value = CDate(myDr("Closing_Date").ToString())
                End If
                ...
                '====
                cmbsecurity.SelectedValue = clsCommon.myCstr(myDr("Security_Cheque"))
                TxtMinimumQtyRequired.Value = clsCommon.myCdbl(myDr("MinimumQtyRequired_pertrip"))
                rbtnpermanent.IsChecked = False
                rbtntemp.IsChecked = False
                If clsCommon.myCstr(myDr("IsPermanent")) = "1" Then
                    rbtnpermanent.IsChecked = True
                    rbtntemp.IsChecked = False
                    dtpStartDate.Value = clsCommon.GETSERVERDATE()
                    dtpEndDate.Value = clsCommon.GETSERVERDATE()
                    dtpStartDate.Enabled = False
                    dtpEndDate.Enabled = False
                ElseIf clsCommon.myCstr(myDr("IsTemporary")) = "1" Then
                    rbtnpermanent.IsChecked = False
                    rbtntemp.IsChecked = True
                    Try
                        dtpStartDate.Value = clsCommon.myCDate(myDr("Start_Period"))
                    Catch ex As Exception

                    End Try

                    Try
                        dtpEndDate.Value = clsCommon.myCDate(myDr("Expired_Period"))
                    Catch ex As Exception
            ...
            If chkBulkProcurement.Checked = True Then
                Dim objTTD As List(Of clsBulkProcurementTankerDetails) = clsBulkProcurementTankerDetails.LoadData(fndvendorNo.Value)
                If objTTD IsNot Nothing AndAlso objTTD.Count > 0 Then
                    For i As Integer = 0 To objTTD.Count - 1
                        gvTankerDetails.Rows.AddNew()
                        gvTankerDetails.Rows(i).Cells("COL_Tanker_No").Value = objTTD.Item(i).Tanker_No
                        gvTankerDetails.Rows(i).Cells("COL_Tanker_Details").Value = objTTD.Item(i).Tanker_Details
                        ' gvTankerDetails.Rows(i).Cells(COL_Tanker_No)..Add(objTTD.Item(i).Tanker_No)
                        'gvTankerDetails.Rows.Add(objTTD.Item(i).Tanker_Details)
                    Next
                End If
            End If

            'objTTD.Tanker_No = clsCommon.myCstr(gvTankerDetails.Rows(i).Cells("COL_Tanker_No").Value)
            ' objTTD.Tanker_Details = clsCommon.myCstr(gvTankerDetails.Rows(i).Cells("COL_Tanker_Details").Value)
            '********************************End*********************************************
    ...
    Sub GeneratedVendorNo(ByVal trans As SqlTransaction)
        ...
        Try
            If clsCommon.CompairString(btnsave.Text, "Save") = CompairStringResult.Equal Then
                fndvendorNo.Value = clsERPFuncationality.GetNextCode(trans, clsCommon.GETSERVERDATE(trans), clsDocType.TTMMASTER, "", "")
            End If
            'trans.Commit()
        Catch ex As Exception
            'trans.Rollback()
    ...
    Public Sub funinsert()
        ...
        Try
            ...
            'Ticket No  ERO/30/08/19-001011 ,Sanjay
            If clsCommon.myLen(fndvendorNo.Value) > 12 Then
                trans.Rollback()
                common.clsCommon.MyMessageBoxShow(Me, "Tanker Transporter No length Can't be greater than 12 character, Please reduce prefix length.", Me.Text)
                fndvendorNo.Value = ""
                Exit Sub
            End If
            ...
            security = clsCommon.myCstr(cmbsecurity.SelectedValue)
            ...
            Dim strCmd11 As String = "Update TSPL_VENDOR_MASTER set  MinimumQtyRequired_pertrip=" & TxtMinimumQtyRequired.Value & ",franchise_yn='" + strTagAsFranchise + "',vsp_payee_name='" + clsCommon.myCstr(txtpayeename.Text.Replace("'", "`")) + "',zila='" + clsCommon.myCstr(txtzila.Text.Replace("'", "`")) + "',tehsil='" + clsCommon.myCstr(txttehsil.Text.Replace("'", "`")) + "',branch_code='" + clsCommon.myCstr(txtbranchcode.Value) + "',branch_name='" + clsCommon.myCstr(txtbranchname.Text.Replace("'", "`")) + "',ifci_code='" + clsCommon.myCstr(txtbranchcode.Value.Replace("'", "`")) + "',account_no='" + clsCommon.myCstr(txtaccno.Text.Replace("'", "`")) + "',Industry_Type='" + indutrytype + "',Industry_Person='" + indutryperson + "',Agreement='" + agrremnt + "',Security_Cheque='" + security + "',IsPermanent='" + clsCommon.myCstr(IIf(rbtnpermanent.IsChecked, 1, 0)) + "',IFSC_Code='" & clsCommon.myCstr(txtbranchcode.Value) & "',IsTemporary='" + clsCommon.myCstr(IIf(rbtntemp.IsChecked, 1, 0)) + "',Account_type='" & clsCommon.myCstr(cmbAccount_Type.Text) & "',isBlankCheque=" & IIf(clsCommon.CompairString(cmbBlankCheque.Text, "YES") = CompairStringResult.Equal, 1, 0) & " where Vendor_Code='" + fndvendorNo.Value + "'"
            connectSql.RunSqlTransaction(trans, strCmd11)

            If rbtntemp.IsChecked Then
            Else
                Dim strCmd111 As String = "Update TSPL_VENDOR_MASTER set  start_Period='" & clsCommon.GetPrintDate(dtpStartDate.Value, "dd/MMM/yyyy") & "',Expired_Period='" & clsCommon.GetPrintDate(dtpEndDate.Value, "dd/MMM/yyyy") & "' where Vendor_Code='" & fndvendorNo.Value & "'"
            ...
            Next
            '=====update by preeti gupta against ticket no[ERO/17/05/19-000607]
            Dim streq1 As String = "Update TSPL_VENDOR_MASTER set GSTRegistered='" & Registered & "',GSTEntity='" & txtEntity.Text & "',GSTLastEntity='" & MyTextBox2.Text & "',GSTFinalNo='" & txtGSTIN_No_final.Text & "',GSTMiddle='" & txtFixxed.Text & "',transporter='Y' where Vendor_Code='" + fndvendorNo.Value + "'"
            ...
            End If
            '******************************** Tanker Details Ticket No : BHA/21/06/18-000073 ******************************
            Dim objTTD As clsBulkProcurementTankerDetails = Nothing
            Dim arrObjTTD As List(Of clsBulkProcurementTankerDetails) = Nothing
            If chkBulkProcurement.Checked = True Then

                If gvTankerDetails.Rows.Count > 0 AndAlso clsCommon.myLen(gvTankerDetails.Rows(0).Cells("COL_Tanker_No").Value) <= 0 Then
                    Throw New Exception("Please Fill Atleast One Tanker No")
                End If
                arrObjTTD = New List(Of clsBulkProcurementTankerDetails)
                For i As Integer = 0 To gvTankerDetails.Rows.Count - 1
                    Dim strTankerNo As String = clsCommon.myCstr(gvTankerDetails.Rows(i).Cells("COL_Tanker_No").Value)
                    Dim strTankerDetails As String = clsCommon.myCstr(gvTankerDetails.Rows(i).Cells("COL_Tanker_Details").Value)
                    If gvTankerDetails.Rows.Count > 0 AndAlso clsCommon.myLen(strTankerNo) > 0 AndAlso clsCommon.myLen(strTankerDetails) > 0 Then
                        objTTD = New clsBulkProcurementTankerDetails()

                        objTTD.Tanker_Transpoter_No = fndvendorNo.Value
                        objTTD.Tanker_No = clsCommon.myCstr(gvTankerDetails.Rows(i).Cells("COL_Tanker_No").Value)
                        objTTD.Tanker_Details = clsCommon.myCstr(gvTankerDetails.Rows(i).Cells("COL_Tanker_Details").Value)
                        arrObjTTD.Add(objTTD)
                    End If
                Next
            End If

            If chkBulkProcurement.Checked = True AndAlso (arrObjTTD Is Nothing Or arrObjTTD.Count <= 0) Then
                Throw New Exception("Please Fill Atleast One Tanker No")
            End If

            If objTTD IsNot Nothing AndAlso arrObjTTD.Count > 0 Then
                clsBulkProcurementTankerDetails.SaveData(arrObjTTD, trans)
            End If

            Dim qryIsBulk As String = ""
            If chkBulkProcurement.Checked = True Then
                qryIsBulk = "update TSPL_VENDOR_MASTER set isBulkProcurement = 1 where Vendor_Code = '" + fndvendorNo.Value + "'"
            Else
                qryIsBulk = "update TSPL_VENDOR_MASTER set isBulkProcurement = 0 where Vendor_Code = '" + fndvendorNo.Value + "'"
            End If

            clsDBFuncationality.ExecuteNonQuery(qryIsBulk, trans)
            '**********************************End*****************************************
    ...
    Public Sub funupdate()
        ...
        Try
            ...
            strCmd11 = "Update TSPL_VENDOR_MASTER set MinimumQtyRequired_pertrip=" & TxtMinimumQtyRequired.Value & ", franchise_yn='" & strTagAsFranchise & "',Form_Type='" + txtvndrtype.Text + "',state_code='" + txtstatecode.Value + "',country_code='" + txtcountrycode.Value + "',vsp_payee_name='" + clsCommon.myCstr(txtpayeename.Text.Replace("'", "`")) + "',zila='" + clsCommon.myCstr(txtzila.Text.Replace("'", "`")) + "',tehsil='" + clsCommon.myCstr(txttehsil.Text.Replace("'", "`")) + "',branch_code='" + clsCommon.myCstr(txtbranchcode.Value) + "',branch_name='" + clsCommon.myCstr(txtbranchname.Text.Replace("'", "`")) + "',ifci_code='" + clsCommon.myCstr(txtbranchcode.Value.Replace("'", "`")) + "',account_no='" + clsCommon.myCstr(txtaccno.Text.Replace("'", "`")) + "',Industry_Type='" + indutrytype + "',Industry_Person='" + indutryperson + "',Agreement='" + agrremnt + "',Security_Cheque='" + security + "',IsPermanent='" + clsCommon.myCstr(IIf(rbtnpermanent.IsChecked, 1, 0)) + "',IsTemporary='" + clsCommon.myCstr(IIf(rbtntemp.IsChecked, 1, 0)) + "', Account_type='" & clsCommon.myCstr(cmbAccount_Type.Text) & "',IFSC_Code='" & clsCommon.myCstr(txtbranchcode.Value) & "',isBlankCheque=" & IIf(clsCommon.CompairString(cmbBlankCheque.Text, "YES") = CompairStringResult.Equal, 1, 0) & " where Vendor_Code='" + fndvendorNo.Value + "'"
            ...
            Dim streq As String = "Update TSPL_VENDOR_MASTER set GSTRegistered='" & Registered & "',GSTEntity='" & txtEntity.Text & "',GSTLastEntity='" & MyTextBox2.Text & "',GSTFinalNo='" & txtGSTIN_No_final.Text & "',GSTMiddle='" & txtFixxed.Text & "',transporter='Y' where Vendor_Code='" + fndvendorNo.Value + "'"
            clsDBFuncationality.ExecuteNonQuery(streq, trans)
    ...
    Public Sub fundelete()
        ...
        Try
            ...
            qst = "select count(*) from tspl_tanker_master where Tanker_Transporter_Code='" + fndvendorNo.Value + "'"
            dpt = clsDBFuncationality.getSingleValue(qst, trans)
            If CInt(dpt) > 0 AndAlso dpt IsNot Nothing Then
                Throw New Exception("This Transporter Cannot be deleted." + Environment.NewLine + "This is already in Process")
                'Return
            End If
    ...
    Public Sub funExport()
        Try
            ...
            Dim strCmd As String = "select count(*) from tspl_vendor_master where form_type='TTM'"
            Dim check As Integer = clsDBFuncationality.getSingleValue(strCmd)
            Dim whrCls As String = ""


            If check > 0 Then
                strCmd = "select Vendor_Code as [Tanker Transporter No],Vendor_Name as[Tanker Transporter Name],Add1 as [Address1],Add2 as [Address2]" &
                          ",Add3 as [Address3],Vendor_Group_Code as [Group Code]" &
                          ",Vendor_Group_Code_Desc as [Tanker Transporter Group Description],City_Code as [City]" &
                          ...
                          ",Terms_Code as [Terms Code],Terms_Code_Desc as [Terms Description],Vendor_Account as [Tanker Transporter Account]" &
                           ",Vendor_Account_Desc as [Tanker Transporter Account Description],Payment_Code as [Payment Code]" &
                           ",Payment_Code_Desc as [Paymnet Code Description],Bank_Code as [Bank Code]" &
                           ",Ven_Type_Code as [Tanker Transporter Type],Ven_Type_Desc as [Tanker Transporter Type Description]" &
                           ...
                           ",Collectorate as [Collectorate],PAN as [PAN],State_Code,Country_Code,Zila,Tehsil,VSP_payee_name as [Payee Name],Account_No,Industry_Type,(case when industry_type='Prop.' then industry_person else '' end) as [Prop Name],(case when industry_type='Partnership' then industry_person else '' end) as [Partner Name],(case when industry_type in ('Public','Pvt') then industry_person else '' end) as [Director Name],IsPermanent,IsTemporary,Agreement,Security_Cheque,form_type,IFSC_Code,Branch_Name,CHEQUE_IN_FAVOUR_OF AS [Cheque in Favour of],MinimumQtyRequired_pertrip AS [Minimum Qty Required pertrip], GSTRegistered as [GST Register], GSTFinalNo as [GSTIN No],TSPL_VENDOR_MASTER.CURRENCY_CODE as [Currency Code],TSPL_VENDOR_MASTER.IsBulkProcurement  "
                For j As Integer = 1 To 20
                    strCmd += " , (select Tanker_Id from (Select ROW_NUMBER () over (order by transpoter_Id,Tanker_Id ) As SNo,transpoter_Id,Tanker_Id,Transpoter_Name  From tspl_transport__Tanker_Details where transpoter_Id=tspl_vendor_master.Vendor_code)xxx where xxx.SNo=" & j & ") as [Tanker Id" & j & "] "
                    strCmd += " , (select Transpoter_Name from (Select ROW_NUMBER () over (order by transpoter_Id,Tanker_Id ) As SNo,transpoter_Id,Tanker_Id ,Transpoter_Name From tspl_transport__Tanker_Details where transpoter_Id=tspl_vendor_master.Vendor_code)xxx where xxx.SNo=" & j & ") as [Tanker Desc" & j & "]"
                Next
                strCmd += " from TSPL_VENDOR_MASTER  "

                whrCls = " and form_type='TTM'"
            Else
                whrCls = ""

                strCmd = "select '' as [Tanker Transporter No],'' as[Tanker Transporter Name],'' as [Address1],'' as [Address2]" &
                          ",'' as [Address3],'' as [Group Code]" &
                          ",'' as [Tanker Transporter Group Description],'' as [City]" &
                          ",'' as [State],'' as [Country],'' as [Phone Num1]" &
                          ",'' as [Phone Num2],'' as [Fax],'' as [Email Id],'' as [Website]" &
                          ",'' as [Terms Code],'' as [Terms Description],'' as [Tanker Transporter Account]" &
                           ",'' as [Tanker Transporter Account Description],'' as [Payment Code]" &
                           ",'' as [Paymnet Code Description],'' as [Bank Code]" &
                           ",'' as [Tanker Transporter Type],'' as [Tanker Transporter Type Description]" &
                           ...
                           ",'' as [Collectorate],'' as [PAN],'' as State_Code,'' as Country_Code,'' as Zila,'' as Tehsil,'' as [Payee Name],'' as Account_No,'' as Industry_Type,'' as [Prop Name],'' as [Partner Name],'' as [Director Name],'0' as IsPermanent,'0' as IsTemporary,'' as Agreement,'' as Security_Cheque,'TTM' as form_type,'' as IFSC_Code,'' as Branch_Name,'' as [Cheque in Favour of],0 AS [Minimum Qty Required pertrip], '' as [GST Register], '' as [GSTIN No], '' as  [Currency Code],'' as IsBulkProcurement,'' as [Tanker ID],'' as [Tanker Desc]"
            End If
            ListImpExpColumnsMandatory = New List(Of String)({"Tanker Transporter No", "Tanker Transporter Name", "Group Code", "country_code", "state_code", "City", "Terms Code", "Tanker Transporter Account", "Bank Code", "Tax Group", "Industry_Type", "Prop Name", "Partner Name", "Director Name", "GST Register", "Pan"})
            ListImpExpColumnsSuperMandatory = New List(Of String)({"Tanker Transporter No"})
        ...
        If transportSql.importExcel(gv, "Tanker Transporter No", "Tanker Transporter Name", "Address1", "Address2", "Address3", "Group Code", "Tanker Transporter Group Description", "City", "State", "Country", "Phone Num1", "Phone Num2", "Fax", "Email Id", "Website", "Terms Code", "Terms Description", "Tanker Transporter Account", "Tanker Transporter Account Description", "Payment Code", "Paymnet Code Description", "Bank Code", "Tanker Transporter Type", "Tanker Transporter Type Description", "Tax Group", "Tax Group Description", "Tax1", "Tax1 Rate", "Tax2", "Tax2 Rate", "Tax3", "Tax3 Rate", "Tax4", "Tax4 Rate", "Tax5", "Tax5 Rate", "Tax6", "Tax6 Rate", "Tax7", "Tax7 Rate", "Tax8", "Tax8 Rate", "Tax9", "Tax9 Rate", "Tax10", "Tax10 Rate", "Transporter", "Created By", "Created Date", "Modify By", "Modify Date", "Company Code", "Collectorate", "PAN", "State_Code", "Country_Code", "Zila", "Tehsil", "Payee Name", "Account_No", "Industry_Type", "Prop Name", "Partner Name", "Director Name", "IsPermanent", "IsTemporary", "Agreement", "Security_Cheque", "form_type", "IFSC_Code", "Branch_Name", "Cheque in Favour of", "Minimum Qty Required pertrip", "GST Register", "GSTIN No", "Currency Code", "IsBulkProcurement", "Tanker ID1", "Tanker Desc1", "Tanker ID2", "Tanker Desc2", "Tanker ID3", "Tanker Desc3", "Tanker ID4", "Tanker Desc4", "Tanker ID5", "Tanker Desc5", "Tanker ID6", "Tanker Desc6", "Tanker ID7", "Tanker Desc7", "Tanker ID8", "Tanker Desc8", "Tanker ID9", "Tanker Desc9", "Tanker ID10", "Tanker Desc10", "Tanker ID11", "Tanker Desc11", "Tanker ID12", "Tanker Desc12", "Tanker ID13", "Tanker Desc13", "Tanker ID14", "Tanker Desc14", "Tanker ID15", "Tanker Desc15", "Tanker ID16", "Tanker Desc16", "Tanker ID17", "Tanker Desc17", "Tanker ID18", "Tanker Desc18", "Tanker ID19", "Tanker Desc19", "Tanker ID20", "Tanker Desc20") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    '    Throw New Exception("Branch code not exist for selected bank,please fill bank branch also.")
                    'End If
                    ...
                    ''richa agarwal 27 sep BM00000009745
                    ...
                    ' ''======================Added by preeti Gupta Against Ticket No[BHA/21/06/18-000073]===========
                    Dim strIsBulkProcurement As String = clsCommon.myCstr(grow.Cells("IsBulkProcurement").Value)
                    ...
                    For j As Integer = 1 To 20
                        Dim strTankerNo As String = Nothing
                        Dim strTankerDesc As String = Nothing

                        strTankerNo = clsCommon.myCstr(grow.Cells("Tanker Id" & clsCommon.myCstr(j) & "").Value)
                        strTankerDesc = clsCommon.myCstr(grow.Cells("Tanker Desc" & clsCommon.myCstr(j) & "").Value)

                        Dim sqlTanker As String = "select count(*) from tspl_transport__Tanker_Details  where  Transpoter_Id='" + strvendorNo + "' and Tanker_Id='" + strTankerNo + "' "
                        Dim CountTanker As Integer = CInt(connectSql.RunScalar(trans, sqlTanker))
                        If clsCommon.myCdbl(strIsBulkProcurement) = 1 Then
                            If clsCommon.myLen(strTankerNo) > 0 Then
                                If (CountTanker = 0) Then
                                    Dim strcmd1 As String = "Insert into tspl_transport__Tanker_Details (Transpoter_Id,Tanker_Id,Transpoter_Name) values('" + strvendorNo + "','" + strTankerNo + "','" + strTankerDesc + "')"
                                    connectSql.RunSqlTransaction(trans, strcmd1)
                                Else
                                    Dim strcmd1 As String = "Update  tspl_transport__Tanker_Details set Transpoter_Id='" + strvendorNo + "' ,Tanker_Id='" + strTankerNo + "',Transpoter_Name='" + strTankerDesc + "' where  Transpoter_Id='" + strvendorNo + "' and Tanker_Id='" + strTankerNo + "' "
                                    connectSql.RunSqlTransaction(trans, strcmd1)
                                End If
                            End If
                        Else
                            Dim strcmd1 As String = "delete tspl_transport__Tanker_Details where Transpoter_Id='" + strvendorNo + "' "
                            connectSql.RunSqlTransaction(trans, strcmd1)
                        End If
    ...
    Sub SaveData()
        ...
        Try
            If txtvendorname.Text = "" Then
                ...
                Errorcontrol.SetError(txtvendorname, "Please Fill Tanker Transporter Name")
                Return
            Else
            ...
            If fndAccntSet.Value = "" Then
                ...
                fndAccntSet.Focus()
                fndAccntSet.Select()
                Errorcontrol.SetError(fndAccntSet, "Please Fill Account Set")
                Return
            Else
            ...
            If fndbankcode.Value = "" Then
                ...
                'fndbankcode.Focus()
                'fndbankcode.Select()
                Errorcontrol.SetError(fndbankcode, "Please Fill Bank Code")
                Return
            Else
            ...
            If fndTxGrp.Value = "" Then
                ...
                fndTxGrp.Focus()
                fndTxGrp.Select()
                Errorcontrol.SetError(fndTxGrp, "Please Fill Tax Group")
                Return
            Else
            ...
            If clsCommon.myLen(txtpan.Text) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Please Fill PAN No.", Me.Text)
                pageCus.SelectedPage = RadPageViewPage1
                txtpan.Focus()
                txtpan.Select()
            ...
            If clsCommon.myLen(txtpan) > 0 Then
                Dim qry As String = "select count(*) from tspl_vendor_master where PAN='" + clsCommon.myCstr(txtpan.Text) + "' and Form_Type='TTM'"
            ...
            If clsCommon.myLen(txtAdd1.Text) <= 0 AndAlso clsCommon.myLen(txtAdd2.Text) <= 0 AndAlso clsCommon.myLen(txtAdd3.Text) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Please Fill Address Of Primary Transporter Master", Me.Text)
                pageCus.SelectedPage = RadPageViewPage1
                txtAdd1.Focus()
                txtAdd1.Select()
            ...
            '    clsCommon.MyMessageBoxShow("Please Select City")
            '    pageCus.SelectedPage = RadPageViewPage1
            '        fndCity.Focus()
            '    fndCity.Select()
            ...
            If Not rbtntemp.IsChecked AndAlso Not rbtnpermanent.IsChecked Then
                ...
                Panel1.Focus()
                Panel1.Select()
                Errorcontrol.SetError(Panel1, "Please Select Tanker Transporter Is Temporary Or Permanent")
                Return
            Else
            ...
            If rbtnpermanent.IsChecked AndAlso cmbagreemnt.SelectedValue = "" AndAlso cmbsecurity.SelectedValue = "" Then
                ...
                If cmbagreemnt.SelectedValue = "" Then
                    cmbagreemnt.Select()
                    Errorcontrol.SetError(cmbagreemnt, "Please Select Agreement Value")
                ElseIf cmbsecurity.SelectedValue = "" Then
                    cmbsecurity.Select()
                    Errorcontrol.SetError(cmbsecurity, "Please Select Security Cheque Value")
                End If
                Return
            Else
                Errorcontrol.ResetError(cmbagreemnt)
                Errorcontrol.ResetError(cmbsecurity)
            End If
            ...
            If MyBase.isModifyonPasswordFlag Then
                If clsPasswordCheckForMasters.CheckMasterPwd(clsUserMgtCode.frmTankerTransporterMaster, clsCommon.myCstr(objCommonVar.CurrentCompanyCode)) Then
                Else
                    Return
                End If
            End If
...
#End Region
    ...
    Private Sub frmTankerTransporterMaster_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub fndvendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndvendorNo._MYValidating
        If isButtonClicked Then
            ...
            fndvendorNo.Value = clsVendorMaster.getFinder(" form_type='TTM'", fndvendorNo.Value, isButtonClicked)
    ...
    '        txtstatecode.Focus()
    '        txtstatecode.Select()
    '        Errorcontrol.SetError(txtState, "Please Select State Code First")
    '        Return
    '    Else
    '        Errorcontrol.ResetError(txtState)
    '    End If
    ...
    Private Sub fndvendorNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavigatorType As common.NavigatorType) Handles fndvendorNo._MYNavigator
        Dim qst As String = "select Vendor_Code as [Vendor Code],Vendor_Name as [Vendor Name],Vendor_Group_Code as [Vendor Group],(select case when Status='N' then 'Active' else 'In.Active' end ) as [Status] from TSPL_VENDOR_MASTER    where  2=2 and form_type='TTM'"
    ...
    Private Sub fndbankcode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndbankcode._MYValidating
        ...
        'If clsCommon.myLen(fndbankcode.Value) > 0 Then
        '    txtbankcodedes.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select Description from TSPL_BANK_MASTER where BANK_CODE='" + fndbankcode.Value + "'"))
        '    txtbankcountry.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select country from TSPL_BANK_MASTER where BANK_CODE='" + fndbankcode.Value + "'"))
        '    txtbankstate.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select state from TSPL_BANK_MASTER where BANK_CODE='" + fndbankcode.Value + "'"))
        '    txtbankcity.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select city from TSPL_BANK_MASTER where BANK_CODE='" + fndbankcode.Value + "'"))
    ...
    Private Sub txtstatecode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtstatecode._MYValidating
        ...
        '    txtcountrycode.Focus()
        '    txtcountrycode.Select()
        '    Errorcontrol.SetError(txtCountry, "Please Select Country First")
        '    Return
        'Else
        '    Errorcontrol.ResetError(txtCountry)
        'End If
        ...
        Try
            ...
            txtstatecode.Value = clsCommon.ShowSelectForm("STAFND", qry, "Code", "", txtcountrycode.Value, "Code", isButtonClicked)
    ...
    Private Sub txtWeb_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtWeb.Leave
        Try
            If clsCommon.myLen(txtWeb) > 0 Then
                If Not txtWeb.Text.ToUpper().Contains("WWW.") Then
                    clsCommon.MyMessageBoxShow(Me, "Please Fill Valid Web Site Address", Me.Text)
                    txtWeb.Focus()
                    txtWeb.Select()
                    txtWeb.Text = ""
                    Errorcontrol.SetError(txtWeb, "Please Fill Valid Web Site Address")
                Else
    ...
    Private Sub rbtnpermanent_ToggleStateChanged(ByVal sender As System.Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles rbtnpermanent.ToggleStateChanged, rbtntemp.ToggleStateChanged
        If rbtnpermanent.IsChecked Then
            cmbagreemnt.Enabled = True
            cmbsecurity.Enabled = True
            cmbagreemnt.MendatroryField = True
            cmbsecurity.MendatroryField = False
            dtpStartDate.Enabled = False
            dtpEndDate.Enabled = False
        ElseIf rbtntemp.IsChecked Then
            cmbagreemnt.Enabled = False
            cmbsecurity.Enabled = False
            cmbagreemnt.MendatroryField = False
            cmbsecurity.MendatroryField = False
            cmbagreemnt.SelectedValue = "NO"
            cmbsecurity.SelectedValue = "NO"
            dtpStartDate.Enabled = True
            dtpEndDate.Enabled = True
        End If
    End Sub
    ...
    '    If clsCommon.myLen(fndbankcode.Value) <= 0 Then
    '        clsCommon.MyMessageBoxShow("Please select bank first", Me.Text)
    '        fndbankcode.Focus()
    '        fndbankcode.Select()
    '        Errorcontrol.SetError(fndbankcode, "Please select bank first")
    '    Else
    '        Errorcontrol.ResetError(fndbankcode)
    '    End If

    '    Dim qry As String = "select TSPL_BANK_BRANCH_MASTER.BRANCH_CODE as Code,TSPL_BANK_BRANCH_MASTER.BRANCH_NAME as Name,TSPL_BANK_BRANCH_MASTER.IFSC_Code as [IFSC Code] from TSPL_BANK_BRANCH_MASTER where bank_code='" + fndbankcode.Value + "'"
    '    Dim dr As DataRow = clsCommon.ShowSelectFormForRow("BNCFND", qry, "Code", txtbranchcode.Value)

    '    If dr IsNot Nothing Then
    '        txtbranchcode.Value = clsCommon.myCstr(dr("code"))
    '        txtbranchname.Text = clsCommon.myCstr(dr("name"))
    '        txtifcicode.Text = clsCommon.myCstr(dr("ifsc code"))
    '    Else
    '        txtbranchcode.Value = ""
    '        txtbranchname.Text = ""
    '        txtifcicode.Text = ""
    ...
    Private Sub frmTankerTransporterMaster_MaximumSizeChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.MaximumSizeChanged
    ...
    Private Sub fndgroupcode__MYOpenMasterForm(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndgroupcode._MYOpenMasterForm
    ...
    Private Sub txtbranchcode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtbranchcode._MYValidating
        If clsCommon.myLen(fndbankcode.Value) > 0 Then
    ...
    Sub LoadBlankGvTankerDetails()
        Try
            gvTankerDetails.Rows.Clear()
            gvTankerDetails.Columns.Clear()
            gvTankerDetails.Columns.Add("COL_Tanker_No", "Tanker No")
            gvTankerDetails.Columns.Add("COL_Tanker_Details", "Tanker Details")
            ...
            'repoCode.Width = 150
            'repoCode.CustomFormat = "dd/MM/yyyy"
            'gvTankerDetails.MasterTemplate.Columns.Add(repoCode)

            gvTankerDetails.Columns("COL_Tanker_No").Width = 100
            gvTankerDetails.Columns("COL_Tanker_Details").Width = 150


            gvTankerDetails.AllowAddNewRow = True
            gvTankerDetails.MasterTemplate.AddNewRowPosition = SystemRowPosition.Bottom
            gvTankerDetails.AllowEditRow = True
            gvTankerDetails.AllowDeleteRow = True
            gvTankerDetails.AllowRowResize = False
            gvTankerDetails.AllowRowReorder = False
            gvTankerDetails.AllowColumnResize = True
            gvTankerDetails.AllowColumnChooser = False
            gvTankerDetails.AllowAutoSizeColumns = False
            gvTankerDetails.ShowGroupPanel = False
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    End Sub
...
Public Class clsBulkProcurementTankerDetails
    Public Tanker_Transpoter_No As String = Nothing
    Public Tanker_No As String = Nothing
    Public Tanker_Details As String = Nothing

    Public Shared Function LoadData(ByVal strTranspoterId As String) As List(Of clsBulkProcurementTankerDetails)
        Dim arr As New List(Of clsBulkProcurementTankerDetails)
        Try
            Dim obj As New clsBulkProcurementTankerDetails
            Dim q As String = "select Tanker_Id ,Transpoter_Name  from tspl_transport__Tanker_Details where Transpoter_Id='" & strTranspoterId & "' "
            Dim dtbl As DataTable = clsDBFuncationality.GetDataTable(q)
            If dtbl IsNot Nothing AndAlso dtbl.Rows.Count > 0 Then
                For i As Integer = 0 To dtbl.Rows.Count - 1
                    obj = New clsBulkProcurementTankerDetails
                    obj.Tanker_No = clsCommon.myCstr(dtbl.Rows(i)("Tanker_Id"))
                    obj.Tanker_Details = clsCommon.myCstr(dtbl.Rows(i)("Transpoter_Name"))
                    arr.Add(obj)
                Next
            End If
        Catch ex As Exception
        ...
        Return arr
    End Function
    Public Shared Function SaveData(ByVal arr As List(Of clsBulkProcurementTankerDetails), ByVal Trans As SqlTransaction) As Boolean
        Dim issaved As Boolean = True
        Try
            Dim i As Integer = 0

            If arr.Count > 0 Then
                deleteData(arr.Item(0).Tanker_Transpoter_No, Trans)
                For i = 0 To arr.Count - 1
                    Dim coll As New Hashtable()
                    clsCommon.AddColumnsForChange(coll, "Transpoter_Id", arr.Item(0).Tanker_Transpoter_No)
                    clsCommon.AddColumnsForChange(coll, "Tanker_Id", arr.Item(i).Tanker_No)
                    clsCommon.AddColumnsForChange(coll, "Transpoter_Name", arr.Item(i).Tanker_Details)

                    issaved = issaved And clsCommonFunctionality.UpdateDataTable(coll, "tspl_transport__Tanker_Details", OMInsertOrUpdate.Insert, "", Trans)
                Next
            End If
        Catch ex As Exception
            Throw New Exception(ex.Message)
        ...
        Return issaved
    End Function
    Public Shared Function deleteData(ByVal strTranspoterId As String, ByVal trans As SqlTransaction) As Boolean
        Try
            Dim qry As String = "delete from tspl_transport__Tanker_Details where Transpoter_Id='" & strTranspoterId & "' "
