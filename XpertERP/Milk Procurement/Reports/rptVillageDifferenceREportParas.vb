...
Imports common
Imports System.IO
Public Class RptVillageDifferenceREportParas
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.RptVillageDiffReportParas)
        ...
        Try
            If gv.Rows.Count > 0 Then
                ...
                If exporter = EnumExportTo.Excel Then
                    clsCommon.MyExportToExcelGrid("Village Difference Report", gv, arrHeader, Me.Text)
                Else
                    clsCommon.MyExportToPDF("Village Difference Report", gv, arrHeader, Me.Text, PageSetupReport_ID, objCommonVar.CurrentUserCode)
                End If
            Else
    ...
    Private Sub RptVillageDifferenceREportParas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Public Sub Load_Report()
        Try
            ...
            Dim QryUploader As String = "select  '" & clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.MCCDefaultMilkItem, clsFixedParameterCode.MilkSetting, Nothing)) & "'  as item_code,convert(varchar,File_Date,103) as DOC_DATE ,shift ,case when SHIFT='M' then 'M'  else 'E' end as  Shift_type , " & _
                " TSPL_VLC_DATA_UPLOADER.Route_No  ,TSPL_VLC_DATA_UPLOADER.MCC_Code  ,isnull(TSPL_VLC_DATA_UPLOADER.qty,0) as VLC_QTY,TSPL_VLC_DATA_UPLOADER.fat as VLC_Fat ,TSPL_VLC_DATA_UPLOADER.snf as VLC_SNF  ," & _
                " isnull(TSPL_VLC_DATA_UPLOADER.Amount,0) as VLC_Amount  ,UOM_Code,VLC_Code_VLC_Uploader,TSPL_VLC_MASTER_HEAD.VLC_CODE,0 as MCC_Qty,0 as MCC_Fat,0 as MCC_SNF," & _
                " 0 AS MCC_Amount  from TSPL_VLC_DATA_UPLOADER  " & _
                " left join TSPL_VLC_MASTER_HEAD on TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader=TSPL_VLC_DATA_UPLOADER.VLC_CODE " & _
                " and TSPL_VLC_MASTER_HEAD.MCC=TSPL_VLC_DATA_UPLOADER.MCC_Code left join TSPL_MP_MASTER on TSPL_MP_MASTER.MP_Code_VLC_Uploader =TSPL_VLC_DATA_UPLOADER.MP_CODE " & _
                " and TSPL_MP_MASTER. VLC_Code =TSPL_VLC_MASTER_HEAD.VLC_CODE " & _
            ...
            Dim QryMCC As String = "select  tspl_milk_srn_detail.item_code,convert(varchar,DOC_DATE,103) as DOC_DATE ,SHIFT ,case when SHIFT='M' then 'M'  else 'E' end as  Shift_type , " & _
                ...
                " UOM_Code,VLC_Code_VLC_Uploader,tspl_milk_srn_Head.VLC_CODE,isnull(tspl_milk_srn_detail.Qty,0) as MCC_Qty,tspl_milk_srn_detail.FAT_PER as MCC_FAT,tspl_milk_srn_detail.SNF_PER as MCC_SNF, " & _
            ...
            sQuery = " (select " & PeriodDate & " as PeriodDate,'" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") & "'  as fromDate ,'" & clsCommon.GetPrintDate(txtToDate.Value, "dd/MM/yyyy") & "'  as Todate , companyADD, '" & objCommonVar.CurrentCompanyName & "'  as CompName,'" & objCommonVar.CurrentCompanyCode & "'  as CompCode," & Environment.NewLine & _
            "   MCC_Code , 'MCCId :'+ MCC_Code +' MCCName: '+ MCC_NAME  as MCC_NAME  , Route_No , Route_name as Route_name,VLC_Code_VLC_Uploader ,VLC_Code  ,VLC_Name ,MCC_Qty as MCC_Qty, MCC_TFAT as MCC_TFAT, MCC_TSNF as MCC_TSNF , MCC_Amount as MCC_Amount,VLC_QTY as  VLC_QTY, VLC_TFAT as VLC_TFAT, VLC_TSNF as VLC_TSNF,VLC_Amount as VLC_Amount ,FromUOM,TOUOM,(MCC_Qty-VLC_QTY) as Diff_Qty, (MCC_TFAT-VLC_TFAT) as Diff_TFAT, (MCC_TSNF-VLC_TSNF) as Diff_TSNF,(MCC_Amount-VLC_Amount) as Diff_Amt from " _
            & " (select ff.MCC_Code,max(MCC_NAME) as MCC_NAME ,ff.Route_No,max(Route_Name) as Route_Name ,sum(VLC_QTY) as VLC_QTY,sum(VLC_TFAT) as VLC_TFAT,sum(VLC_TSNF) as VLC_TSNF,sum(VLC_Amount) as VLC_Amount,ff.VLC_Code_VLC_Uploader,ff.VLC_CODE,max(VLC_Name) as VLC_Name ,sum( MCC_Qty) as MCC_Qty,sum( MCC_TFAT) as MCC_TFAT,sum(MCC_TSNF) as MCC_TSNF ,sum(MCC_Amount) as MCC_Amount, max(UOM_Code) as  UOM_Code,max(FromUOM) as FromUOM,max(TOUOM) as TOUOM, max(TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_COMPANY_MASTER.City_Code)>0 then ', '+TSPL_COMPANY_MASTER.City_Code else ' ' end + ','+case when len(TSPL_STATE_MASTER.STATE_NAME  )>0 then TSPL_STATE_MASTER.STATE_NAME else '' end)  as companyADD   from" _
            & " (select ttt.* from (select  DOC_DATE ,shift , Shift_type ,Route_No  ,MCC_Code  , (cf * VLC_QTY) as  VLC_QTY, (cf * VLC_TFAT) as VLC_TFAT , (cf *VLC_TSNF) as VLC_TSNF    ,(cf * VLC_Amount) as  VLC_Amount,UOM_Code,VLC_Code_VLC_Uploader,VLC_CODE, (cf * MCC_Qty) as MCC_Qty, (cf * MCC_TFAT) as MCC_TFAT, (cf * MCC_TSNF) as MCC_TSNF , (cf * MCC_Amount) as MCC_Amount,FromUOM ,TOUOM  from " _
            ...
            & " " & QryManual & "" _
            & " union all " _
            & " " & QryMCC & " )pp ) xx" & Environment.NewLine & _
            ...
            sQuery += " group by ff.MCC_Code ,Route_No,ff.VLC_CODE,ff.VLC_Code_VLC_Uploader ) MM )"
