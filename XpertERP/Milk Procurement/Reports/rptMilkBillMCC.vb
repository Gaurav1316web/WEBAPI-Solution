...
Public Class RptMilkBillMCC
    ...
    Public Sub Load_Report()
        Try
            ...
            sQuery += "select  '" & fromDate & "'  as fromDate ,'" & Todate & "'  as Todate ,'" & objCommonVar.CurrentUser & "' as User_Name ,'" & companyADD & "'  as companyADD, '" & CompName & "'  as CompName,'" & CompCode & "'  as CompCode,''  as compLogo1 ,'' as compLogo2,*,convert(decimal(18,2),(FAT_Per *NewQty)/100 ) as T_FAT,convert(decimal(18,2),(SNF_Per *NewQty )/100) as T_SNF from(select convert(varchar,DOC_DATE,103) as DOC_DATE ,SHIFT,convert(decimal(18,2),SUM(NewQty )) as NewQty,convert(decimal(18,1),(SUM(FATQTY )/isnull(SUM(NewQty ),0))*100) as FAT_Per,convert(decimal(18,1),(SUM(SNFQTY )/isnull(SUM(NewQty ),0))*100) as SNF_Per,convert(decimal(18,2),SUM(Amount )/isnull(SUM(NewQty ),0)) as Rate,convert(decimal(18,2),SUM(Amount)) as Amount,MCC_CODE ,max(MCC_NAME) as MCC_NAME from " & Environment.NewLine &
            " (select DOC_DATE,UOM_Code,FATQTY*CF as FATQTY,SNFQTY*CF as SNFQTY,Qty*CF as NewQty, Qty,FromUOM,TOUOM,CF,xx.RATE ,cf*Net_AMOUNT as Amount ,MCC_CODE+' -'+MCC_NAME+'  QtyMode :'+TOUOM   as MCC_CODE, MCC_NAME ,VSP_CODE ,SHIFT,ROUTE_CODE ,Vendor_Name   from " & Environment.NewLine &
            " (select TSPL_MILK_PURCHASE_INVOICE_DETAIL.Item_Code , TSPL_MILK_RECEIPT_DETAIL.UOM_Code,TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty  ,FAT_PER ,(TSPL_MILK_PURCHASE_INVOICE_DETAIL.FAT_PER*TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty/100) as FATQTY,SNF_PER,(TSPL_MILK_PURCHASE_INVOICE_DETAIL.SNF_PER *TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty/100) as SNFQTY  ,TSPL_MILK_PURCHASE_INVOICE_DETAIL.RATE as RATE,TSPL_MILK_PURCHASE_INVOICE_DETAIL.Net_AMOUNT, TSPL_MILK_PURCHASE_INVOICE_HEAD.MCC_CODE , TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_DATE,TSPL_MILK_PURCHASE_INVOICE_HEAD.VSP_CODE ,TSPL_MILK_SAMPLE_HEAD.SHIFT, TSPL_MILK_PURCHASE_INVOICE_HEAD.ROUTE_CODE ,TSPL_VENDOR_MASTER.Vendor_Name,TSPL_MCC_MASTER .MCC_NAME  from TSPL_MILK_PURCHASE_INVOICE_DETAIL " & Environment.NewLine &
            " Inner Join TSPL_MILK_PURCHASE_INVOICE_HEAD On TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_CODE =TSPL_MILK_PURCHASE_INVOICE_DETAIL.DOC_CODE " & Environment.NewLine &
            ...
            " left outer join TSPL_MILK_RECEIPT_HEAD on TSPL_MILK_RECEIPT_HEAD.DOC_CODE =TSPL_MILK_SAMPLE_HEAD.MILK_RECEIPT_CODE  " & Environment.NewLine &
            " left outer join TSPL_MILK_RECEIPT_DETAIL on TSPL_MILK_RECEIPT_DETAIL.DOC_CODE =TSPL_MILK_RECEIPT_HEAD.DOC_CODE and " & Environment.NewLine &
            "  TSPL_MILK_SRN_HEAD.vlc_doc_Code = TSPL_MILK_RECEIPT_DETAIL.VLC_DOC_CODE" & Environment.NewLine &
            " Left Outer Join TSPL_VENDOR_MASTER On TSPL_MILK_PURCHASE_INVOICE_HEAD.VSP_CODE =TSPL_VENDOR_MASTER.Vendor_Code And TSPL_VENDOR_MASTER.Form_Type = 'VSP' " & Environment.NewLine &
            " Left Outer Join TSPL_MCC_MASTER On TSPL_MILK_RECEIPT_HEAD .MCC_CODE = TSPL_MCC_MASTER.MCC_Code" & Environment.NewLine &
            " left outer join TSPL_vlc_master_head on TSPL_vlc_master_head.vlc_code=TSPL_MILK_PURCHASE_INVOICE_detail.VLC_no" & Environment.NewLine &
            " " & whrcls & " and convert(date,TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_DATE,103)>=convert(date,('" + txtFromDate.Value + "'),103) and convert(date,TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_DATE,103) <=convert(date,('" + txtToDate.Value + "'),103) " & Environment.NewLine &
            ...
            If ItemStructureMandatoryOnWeightConversion = False Then
                sQuery = "select  '" & fromDate & "'  as fromDate ,'" & Todate & "'  as Todate ,'" & objCommonVar.CurrentUser & "' as User_Name,'" & companyADD & "'  as companyADD, '" & CompName & "'  as CompName,'" & CompCode & "'  as CompCode,''  as compLogo1 ,'' as compLogo2,*,[MCC Code]+' -'+[MCC Name]+'  QtyMode :'" + "'" + cboUnit.Text + "'" + " as MCC_CODE,convert(decimal(18,2),(FAT_Per *NewQty)/100 ) as T_FAT,convert(decimal(18,2),(SNF_Per *NewQty )/100) as T_SNF  from(select convert(varchar,INVOICE_DATE,103) as DOC_DATE,Case When SHIFT = 'Morning' Then 'M' Else 'E' End As [SHIFT]"
