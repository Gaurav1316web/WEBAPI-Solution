...
Public Class FrmMCCShiftReportRouteWise
    ...
    Public Sub Load_Report()
        Try
            ...
            sQuery = " (select '" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") & "'  as fromDate ,'" & clsCommon.GetPrintDate(txtToDate.Value, "dd/MM/yyyy") & "'  as Todate , (TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_COMPANY_MASTER.City_Code)>0 then ', '+TSPL_COMPANY_MASTER.City_Code else ' ' end + case when len(TSPL_COMPANY_MASTER.State )>0 then TSPL_COMPANY_MASTER.State else '' end)  as companyADD, '" & objCommonVar.CurrentCompanyName & "'  as CompName,'" & objCommonVar.CurrentCompanyCode & "'  as CompCode," & Environment.NewLine & _
