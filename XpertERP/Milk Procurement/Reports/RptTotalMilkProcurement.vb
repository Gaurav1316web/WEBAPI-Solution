...
Public Class rpttotalMilkProcurement
    ...
    " where 2=2 and [DocType] in ('Bulk In','Bulk Ret') ) BulkMilkData group By BulkMilkData.[Vendor Code],BulkMilkData.[Gate Entry Date]"
            ...
            SQuery += " select convert(varchar,aa.[Doc Date],103) ,'MCC Procurement' as [DocType],aa.[MCC Code] ,aa.[MCC Name],aa.[Milk Weight] ,aa.[Milk Weight(KG)]	,aa.[Milk Weight(LTR)] ,Convert(decimal(18,4),aa.[FAT(%)]) as [FAT Per],Convert(decimal(18,4),aa.[SNF(%)]) as [SNF Per],aa.[FAT(KG)] as [FAT KG],aa.[SNF(KG)] AS [SNF KG],aa.[SRN Amount] " & _
            " from (  select xxx.*  from ( select xx.* from ( select PP.[Doc Date] ,pp.[MCC Code]  as [MCC Code],max(pp.[MCC Name] )  as [MCC Name],sum([Milk Weight] ) as [Milk Weight],sum([Milk Weight(KG)] ) as [Milk Weight(KG)],sum([Milk Weight(LTR)] ) as [Milk Weight(LTR)], case when sum([Milk Weight(KG)] )=0 then 0 else (sum([FAT(KG)] )/sum([Milk Weight(KG)] ))*100 end as [FAT(%)], case when sum([Milk Weight(KG)] )=0 then 0 else (sum([SNF(KG)] )/sum([Milk Weight(KG)] ))*100 end as [SNF(%)] ,sum([FAT(KG)] ) as [FAT(KG)] ,sum([SNF(KG)] ) as [SNF(KG)],  sum([SRN Qty]) as [SRN Qty] ,sum([SRN Amount]) as [SRN Amount],avg(CLR) as CLR  from ( " & Environment.NewLine & _
            " Select final.[Milk Receipt Code] ,final.MCC as [MCC Code] ,final.[MCC Name],final.Date ,final.[Doc Date] ,final.Shift ,final.[Route Code],final.[Route Name] ,final.[Vehicle Code] ,final.[VSP Code],final.[VSP Name],final.[Vendor Group Code] ,final.[Vlc Uploader Code] ,final.[Vlc Code] ,final.[VLC Name] ,final.Item_Code,final.Item_Desc,final.[Milk Weight],final.[Milk Weight(KG)], final.[Milk Weight(LTR)]  as [Milk Weight(LTR)], final.[FAT(%)]  ,final.CLR,final.[SNF(%)] ,final.[FAT(KG)],final.[SNF(KG)] ,final.[Milk Type],final.[SRN No],final.[SRN Amount], final.[SRN Qty],final.[SRN Rate],final.[Shift Status] ,Invoice_no ,Invoice_Date ,MACHINE_NO,(CASE WHEN [Sample Status]='Auto' THEN 'N' ELSE 'Y' END) AS IS_MILK_SAMPLE_MANUAL   From " & Environment.NewLine & _
            " (Select TSPL_MILK_SRN_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc,Case When TSPL_MILK_SAMPLE_DETAIL.FAT <= 5 Then TSPL_MILK_SAMPLE_DETAIL.FAT Else 0 End [Cow FAT(%)], Case When TSPL_MILK_SAMPLE_DETAIL.FAT <= 5 Then TSPL_MILK_SAMPLE_DETAIL.SNF Else 0 End [Cow SNF(%)], Case When TSPL_MILK_SAMPLE_DETAIL.FAT > 5 Then TSPL_MILK_SAMPLE_DETAIL.FAT Else 0 End [Buffalo FAT(%)], Case When TSPL_MILK_SAMPLE_DETAIL.FAT > 5 Then TSPL_MILK_SAMPLE_DETAIL.SNF Else 0 End [Buffalo SNF(%)], Case When TSPL_MILK_SAMPLE_DETAIL.FAT <= 5 Then TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT Else 0 End [Cow Milk Qty (KG)], Case When TSPL_MILK_SAMPLE_DETAIL.FAT > 5 Then TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT Else 0 End [Buffalo Milk Qty (KG)] " & Environment.NewLine & _
            " , Case When Coalesce(TSPL_MILK_SAMPLE_DETAIL.FAT, 0) <= 0 Then '' When Coalesce(TSPL_MILK_SAMPLE_DETAIL.FAT, 0) <= 5 Then 'C' Else 'B' End As [Milk Type], TSPL_MILK_RECEIPT_HEAD.DOC_CODE As [Milk Receipt Code], TSPL_MILK_RECEIPT_HEAD.MCC_CODE As MCC, TSPL_MCC_MASTER.MCC_NAME As [MCC Name],isnull(TSPL_MCC_MASTER.plant_code,'') As [Plant Code], isnull(tspl_location_master.location_desc,'') As [Plant Name], Convert(date,TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103) As Date,  Convert(varchar,TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103) As [Doc Date], Case When TSPL_MILK_RECEIPT_DETAIL.SHIFT = 'M' Then 'Morning' Else 'Evening' End As Shift,  TSPL_MILK_RECEIPT_DETAIL.ROUTE_CODE As [Route Code], TSPL_MCC_ROUTE_MASTER.Route_Name As [Route Name], TSPL_MILK_RECEIPT_DETAIL.VEHICLE_CODE As [Vehicle Code], TSPL_MILK_SRN_HEAD.VSP_CODE As [VSP Code], TSPL_VENDOR_MASTER.Vendor_Name As [VSP Name], TSPL_VENDOR_MASTER.Vendor_Group_Code As [Vendor Group Code],TSPL_VLC_MASTER_HEAD.VLC_Code As [Vlc Code], TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader As [Vlc Uploader Code], TSPL_VLC_MASTER_HEAD.VLC_Name As [VLC Name], TSPL_MILK_RECEIPT_DETAIL.SAMPLE_NO As [Sample No],  TSPL_MILK_RECEIPT_DETAIL.NO_OF_CANS As [No Of Cans], TSPL_MILK_RECEIPT_DETAIL.MILK_WEIGHT As [Milk Weight], TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT As [Milk Weight(KG)], TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT_LTR As [Milk Weight(LTR)], TSPL_MILK_SAMPLE_DETAIL.FAT As [FAT(%)], TSPL_MILK_SAMPLE_DETAIL.SNF As [SNF(%)], TSPL_MILK_SAMPLE_DETAIL.CLR,  Convert(decimal(18,2), TSPL_MILK_SAMPLE_DETAIL.FAT * TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT / 100) As [FAT(KG)], Convert(decimal(18,2),TSPL_MILK_SAMPLE_DETAIL.SNF * TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT / 100) As [SNF(KG)], Case When TSPL_MILK_SAMPLE_DETAIL.IS_MANUAL = '' Then 'Auto' Else TSPL_MILK_SAMPLE_DETAIL.IS_MANUAL End As [Sample Status], TSPL_MILK_SRN_HEAD.DOC_CODE As [SRN No], Convert(decimal(18,2),TSPL_MILK_SRN_DETAIL.AMOUNT) As [SRN Amount], TSPL_MILK_SRN_DETAIL.RATE As [SRN Rate], TSPL_MILK_SRN_DETAIL.Qty As [SRN Qty], Case When TSPL_MILK_Shift_End_HEAD.DOC_CODE Is Null Then 'Open' Else 'Close' End [Shift Status],TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_CODE as Invoice_no, convert(varchar,TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_DATE,103) as Invoice_Date , tspl_milk_receipt_detail.IS_MANUAL , tspl_milk_receipt_detail.MACHINE_NO,[Transporter Code],[Transporter Name],isnull( TSPL_MILK_SRN_DETAIL.EMP_Amount,0) as EMP_Amount,TSPL_MILK_SRN_DETAIL.TIP_Amount,isnull(TSPL_MILK_SRN_DETAIL.NET_AMOUNT,0) as NET_AMOUNT,isnull(TSPL_MILK_SRN_DETAIL.Round_Off,0) as Round_Off,isnull(TSPL_MILK_PURCHASE_INVOICE_DETAIL.Handling_Charges_Amount,0) as Handling_Charges_Amount,TSPL_MILK_SRN_DETAIL.Price_Code as [Price Code],TSPL_MILK_SRN_HEAD.Purchase_Order_No,TSPL_MILK_SRN_DETAIL.Head_Load_Amount  " & Environment.NewLine & _
            " ,TSPL_MILK_PRICE_SNF_DEDUCTION.Amount as SNF_Ded_Value,cast((TSPL_MILK_PRICE_SNF_DEDUCTION.Amount+TSPL_MILK_SRN_DETAIL.RATE) as decimal(18,2)) as SNF_Ded_Rate,cast((TSPL_MILK_PRICE_SNF_DEDUCTION.Amount+TSPL_MILK_SRN_DETAIL.RATE)*TSPL_MILK_SRN_DETAIL.ACC_Qty as decimal(18,2)) as SNF_Ded_Amount  " & Environment.NewLine & _
            ...
            " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.item_code=TSPL_MILK_SRN_DETAIL.item_code  " & Environment.NewLine & _
            ...
            " Left Outer Join TSPL_MILK_Shift_End_HEAD On TSPL_MILK_Shift_End_HEAD.MCC_CODE = TSPL_MILK_RECEIPT_HEAD.MCC_CODE  " & Environment.NewLine & _
            " And convert(date,TSPL_MILK_Shift_End_HEAD.DOC_DATE,103) = convert(date,TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103)  " & Environment.NewLine & _
            " And TSPL_MILK_Shift_End_HEAD.SHIFT = TSPL_MILK_RECEIPT_HEAD.SHIFT  Left Outer Join TSPL_MILK_Shift_End_Route_DETAIL On TSPL_MILK_Shift_End_Route_DETAIL.DOC_CODE = TSPL_MILK_Shift_End_HEAD.DOC_CODE  And TSPL_MILK_Shift_End_Route_DETAIL.Route_CODE = TSPL_MCC_ROUTE_MASTER.Route_Code  left outer join (select code,max(Price_code) as Price_code from  TSPL_FAT_SNF_UPLOADER_MASTER group by code) as TabTSPL_FAT_SNF_UPLOADER_MASTER on TabTSPL_FAT_SNF_UPLOADER_MASTER.code=TSPL_MILK_SRN_DETAIL.Price_Code  " & Environment.NewLine & _
            " left outer join TSPL_MILK_PRICE_SNF_DEDUCTION on TSPL_MILK_PRICE_SNF_DEDUCTION.Price_code=TabTSPL_FAT_SNF_UPLOADER_MASTER.Price_code and cast(TSPL_MILK_SRN_DETAIL.SNF_PER as decimal(18,1))=TSPL_MILK_PRICE_SNF_DEDUCTION.Per  " & Environment.NewLine & _
            " left join tspl_location_master on tspl_location_master.location_code=TSPL_MCC_MASTER.Plant_Code  where 2 = 2 " & _
            " and month(Cast(TSPL_MILK_RECEIPT_HEAD.DOC_DATE as Date)) ='" & clsCommon.myCstr(Month(txtFromDate.Value)) & "' and year(Cast(TSPL_MILK_RECEIPT_HEAD.DOC_DATE as Date)) ='" & clsCommon.myCstr(Year(txtFromDate.Value)) & "' ) As final where 2=2  " & Environment.NewLine & _
    ...
    Public Sub Load_Report()
        Try
            ...
            SQuery += " select MAX(BulkMilkData.[DocType]) AS [DocType],BulkMilkData.[Vendor Code] ,max(BulkMilkData.[Vendor Name]) as [Vendor Name],sum(BulkMilkData.[Challan Qty]) as [Qty] " & Environment.NewLine & _
    " ,sum(BulkMilkData.[Challan Qty]) as [Qty_In_Kg],Convert(decimal(18,3),sum(BulkMilkData.[Challan Qty Ltr] )) as [Qty_In_Ltr] " & Environment.NewLine & _
    " ,Convert(decimal(18,4),((sum(BulkMilkData.FATKG)*100) /sum(BulkMilkData.[Challan Qty] ))) as [FAT_Per],Convert(decimal(18,4),((sum(BulkMilkData.SNFKG)*100) /sum(BulkMilkData.[Challan Qty] ))) as [SNF_Per], " & Environment.NewLine & _
    " sum(BulkMilkData.FATKG ) as [FAT_KG],sum(BulkMilkData.SNFKG ) as [SNF_KG],sum(BulkMilkData.[Total Amount]  ) as [Total_Amount] " & _
    " ,(case when (sum(BulkMilkData.FATKG)=0 and sum(BulkMilkData.SNFKG)=0) then 0 else Convert(decimal(18,2),(sum(BulkMilkData.[Total Amount]  )/(sum(BulkMilkData.FATKG)+(sum(BulkMilkData.SNFKG)*2/3)))*2) end) as [EFU Price]" & _
    " ,Convert(decimal(18,2),sum(BulkMilkData.[Total Amount])/ sum(BulkMilkData.[Challan Qty])) as [Price Per KG]" & _
    " ,(case when sum(BulkMilkData.FATKG)=0 then 0 else Convert(decimal(18,2),sum(BulkMilkData.[Total Amount])/sum(BulkMilkData.FATKG)) end) as [FAT Rate Per KG]" & _
    " from (select wholeData.[DocType],wholeData.[Vendor Code] ," & Environment.NewLine & _
    " wholeData.[Vendor Name] ,wholeData.[Challan Qty],wholeData.[Challan Qty Ltr],wholeData.[FAT %] ,wholeData.CLR,wholeData.[SNF %],wholeData.FATKG,wholeData .SNFKG,wholeData.[Total Amount]  " & Environment.NewLine & _
    "  from (Select  DISTINCT Line_No as Line_No_a, xx.[To MCC or Plant Code], xx.DocType,xx.[Vendor Code],xx.[Vendor Name],xx.[Gate Entry No],xx.[Gate Entry Date],xx.[Challan Qty]," & Environment.NewLine & _
    "  xx.[Item Code],xx.[Item Desc],xx.UOM,xx.[QC No], XX.STATUS,xx.[MCC Name], xx.[Net Rate], xx.[FAT %] ,xx.[SNF %] ,xx.CLR , Convert(decimal(18,3),(xx.[Net Weight] * xx.[FAT %] /    100)) As FATKG," & Environment.NewLine & _
    "  Convert(decimal(18,3),(xx.[Net Weight] * xx.[SNF %] /    100)) As SNFKG, case when SRN_Return_NO is not null then [Total Amount temp]*-1 else [Total Amount temp] end [Total Amount] " & _
    ...
    " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAIL_KG on TSPL_GATE_ENTRY_CHEMBER_DETAILS.Item_Code=TSPL_ITEM_UOM_DETAIL_KG.Item_Code and TSPL_ITEM_UOM_DETAIL_KG.UOM_Code=Case When IsNull(TSPL_GATE_ENTRY_CHEMBER_DETAILS.UOM, '') = '' Then TSPL_ITEM_UOM_DETAIL.UOM_Code Else TSPL_GATE_ENTRY_CHEMBER_DETAILS.UOM End" & _
    " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAIL_LTR on TSPL_GATE_ENTRY_CHEMBER_DETAILS.Item_Code=TSPL_ITEM_UOM_DETAIL_LTR.Item_Code and TSPL_ITEM_UOM_DETAIL_LTR.UOM_Code='LTR' " & _
    " where 2 = 2 and convert(date,Tspl_Gate_Entry_Details.Date_And_Time,103)>=convert(date,'" + txtFromDate.Value + "',103) and convert(date,Tspl_Gate_Entry_Details.Date_And_Time,103) <=convert(date,'" + txtToDate.Value + "' ,103)" & Environment.NewLine & _
    " Union ALL" & Environment.NewLine & _
    " Select  (TSPL_GATE_ENTRY_CHEMBER_DETAILS.Chamber_Qty*TSPL_ITEM_UOM_DETAIL_KG.Conversion_Factor)/TSPL_ITEM_UOM_DETAIL_LTR.Conversion_Factor AS [Challan Qty LTR],'' as Batch_No,TSPL_QUALITY_CHEMBER_DETAILS.MIKL_TYPE_CODE,TSPL_QUALITY_CHEMBER_DETAILS.Milk_Grade_code,GRADE_TYPE,TSPL_GATE_ENTRY_CHEMBER_DETAILS.Line_No," & Environment.NewLine & _
    " TSPL_Bulk_Milk_SRN_Return.SRN_Return_NO, Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then 'Bulk Ret' Else 'MCC Ret' End As DocType," & Environment.NewLine & _
    " Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then 'Not Req' Else IsNull(TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_No, '') End As [Milk Receipt Challan No]," & Environment.NewLine & _
    ...
    " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAIL_KG on TSPL_GATE_ENTRY_CHEMBER_DETAILS.Item_Code=TSPL_ITEM_UOM_DETAIL_KG.Item_Code and TSPL_ITEM_UOM_DETAIL_KG.UOM_Code=Case When IsNull(TSPL_GATE_ENTRY_CHEMBER_DETAILS.UOM, '') = '' Then TSPL_ITEM_UOM_DETAIL.UOM_Code Else TSPL_GATE_ENTRY_CHEMBER_DETAILS.UOM End" & _
    " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAIL_LTR on TSPL_GATE_ENTRY_CHEMBER_DETAILS.Item_Code=TSPL_ITEM_UOM_DETAIL_LTR.Item_Code and TSPL_ITEM_UOM_DETAIL_LTR.UOM_Code='LTR' " & _
    " where 2 = 2 and convert(date,Tspl_Gate_Entry_Details.Date_And_Time,103)>=convert(date,'" + txtFromDate.Value + "',103) and convert(date,Tspl_Gate_Entry_Details.Date_And_Time,103) <=convert(date,'" + txtToDate.Value + "' ,103) ) As xx " & Environment.NewLine & _
    " left join TSPL_SHIFT_MASTER on xx.Shift_Code=TSPL_SHIFT_MASTER.SHIFT_CODE " & _
    " ) wholeData " & Environment.NewLine & _
    " where 2=2 and [DocType] in ('Bulk In','Bulk Ret') ) BulkMilkData group By BulkMilkData.[Vendor Code]"
            ...
            SQuery += " select 'MCC Procurement' as [DocType],aa.[MCC Code] ,aa.[MCC Name],aa.[Milk Weight] ,aa.[Milk Weight(KG)]	,aa.[Milk Weight(LTR)] ,Convert(decimal(18,4),aa.[FAT(%)]) as [FAT Per],Convert(decimal(18,4),aa.[SNF(%)]) as [SNF Per],aa.[FAT(KG)] as [FAT KG],aa.[SNF(KG)] AS [SNF KG],aa.[SRN Amount] " & _
            "  ,(case when (aa.[FAT(KG)]=0 and aa.[SNF(KG)]=0) then 0 else Convert(decimal(18,2),(aa.[SRN Amount]/(aa.[FAT(KG)]+(aa.[SNF(KG)]*2/3)))*2) end) as [EFU Price]" & _
            " ,Convert(decimal(18,2),aa.[SRN Amount]/ aa.[Milk Weight(KG)]) as [Price Per KG]" & _
            " ,(case when aa.[FAT(KG)]=0 then 0 else Convert(decimal(18,2),aa.[SRN Amount]/aa.[FAT(KG)]) end) as [FAT Rate Per KG]" & _
            ...
            " And TSPL_MILK_Shift_End_HEAD.SHIFT = TSPL_MILK_RECEIPT_HEAD.SHIFT  Left Outer Join TSPL_MILK_Shift_End_Route_DETAIL On TSPL_MILK_Shift_End_Route_DETAIL.DOC_CODE = TSPL_MILK_Shift_End_HEAD.DOC_CODE  And TSPL_MILK_Shift_End_Route_DETAIL.Route_CODE = TSPL_MCC_ROUTE_MASTER.Route_Code  left outer join (select code,max(Price_code) as Price_code from  TSPL_FAT_SNF_UPLOADER_MASTER group by code) as TabTSPL_FAT_SNF_UPLOADER_MASTER on TabTSPL_FAT_SNF_UPLOADER_MASTER.code=TSPL_MILK_SRN_DETAIL.Price_Code  " & Environment.NewLine & _
            " left outer join TSPL_MILK_PRICE_SNF_DEDUCTION on TSPL_MILK_PRICE_SNF_DEDUCTION.Price_code=TabTSPL_FAT_SNF_UPLOADER_MASTER.Price_code and cast(TSPL_MILK_SRN_DETAIL.SNF_PER as decimal(18,1))=TSPL_MILK_PRICE_SNF_DEDUCTION.Per  " & Environment.NewLine & _
            " left join tspl_location_master on tspl_location_master.location_code=TSPL_MCC_MASTER.Plant_Code  where 2 = 2  and Cast(TSPL_MILK_RECEIPT_HEAD.DOC_DATE as Date) >=convert(date,'" + txtFromDate.Value + "',103) and Cast(TSPL_MILK_RECEIPT_HEAD.DOC_DATE as Date) <=convert(date,'" + txtToDate.Value + "',103) ) As final where 2=2  " & Environment.NewLine & _
