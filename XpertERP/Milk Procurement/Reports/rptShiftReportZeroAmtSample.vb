...
Public Class RptShiftReportZeroAmtSample
    ...
    Public Sub Load_Report()
        ...
        whrcls += "  and convert(date,TSPL_MILK_SAMPLE_HEAD.DOC_DATE,103)>=convert(date,('" + txtFromDate.Value + "'),103) and convert(date,TSPL_MILK_SAMPLE_HEAD.DOC_DATE,103) <=convert(date,('" + txtToDate.Value + "'),103) "
        If clsCommon.CompairString(txtFromShift.Text, "E") = CompairStringResult.Equal Then
            whrcls += " and 2=( case when TSPL_MILK_SAMPLE_HEAD.DOC_DATE >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "' and TSPL_MILK_SAMPLE_HEAD.DOC_DATE <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "' and TSPL_MILK_SAMPLE_HEAD.SHIFT='M' then 3 else 2 end  )"
        End If
        If clsCommon.CompairString(txtToShift.Text, "M") = CompairStringResult.Equal Then
            whrcls += " and 2=( case when TSPL_MILK_SAMPLE_HEAD.DOC_DATE >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtToDate.Value), "dd/MMM/yyyy hh:mm tt") + "' and TSPL_MILK_SAMPLE_HEAD.DOC_DATE <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm tt") + "' and TSPL_MILK_SAMPLE_HEAD.SHIFT='E' then 3 else 2 end  )"
        End If
        sQuery = ""
        ...
        " from(select TSPL_MILK_SRN_DETAIL.Item_Code ,TSPL_MILK_SRN_DETAIL.UOM_Code ,TSPL_MILK_SRN_DETAIL.FAT_PER,TSPL_MILK_SRN_DETAIL.SNF_PER ,TSPL_MCC_MASTER.MCC_Code ,TSPL_MCC_MASTER.MCC_NAME ," & Environment.NewLine & _
        " TSPL_MCC_ROUTE_MASTER.Route_Code ,TSPL_MCC_ROUTE_MASTER.Route_Name ,TSPL_VLC_MASTER_HEAD.VLC_Code,TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader ,TSPL_VLC_MASTER_HEAD.VLC_Name ,convert(varchar,TSPL_MILK_SRN_HEAD.DOC_DATE ,103) as DOC_DATE ,TSPL_MILK_SAMPLE_HEAD.SHIFT ,TSPL_MILK_SAMPLE_DETAIL.TYPE ,TSPL_MILK_SAMPLE_DETAIL.SAMPLE_NO ,TSPL_MILK_SRN_DETAIL.Qty,TSPL_MILK_SRN_DETAIL.FAT_KG  ,TSPL_MILK_SRN_DETAIL.SNF_KG  ,TSPL_MILK_SRN_DETAIL.RATE ,TSPL_MILK_SRN_DETAIL.AMOUNT  from TSPL_MILK_SRN_HEAD" & Environment.NewLine & _
        " left outer join TSPL_MILK_SRN_DETAIL on TSPL_MILK_SRN_DETAIL.DOC_CODE =TSPL_MILK_SRN_HEAD.DOC_CODE " & Environment.NewLine & _
        " left outer join TSPL_VLC_MASTER_HEAD  on TSPL_VLC_MASTER_HEAD.VLC_Code =TSPL_MILK_SRN_HEAD.VLC_CODE " & Environment.NewLine & _
        " left outer join TSPL_VENDOR_MASTER on TSPL_VENDOR_MASTER.Vendor_Code =TSPL_MILK_SRN_HEAD.VSP_CODE And TSPL_VENDOR_MASTER.Form_Type = 'VSP'" & Environment.NewLine & _
        ...
        " left outer join TSPL_MILK_SAMPLE_DETAIL on TSPL_MILK_SAMPLE_DETAIL.DOC_CODE =TSPL_MILK_SAMPLE_HEAD.DOC_CODE " & Environment.NewLine & _
        " and TSPL_MILK_SRN_HEAD.VLC_DOC_CODE=TSPL_MILK_SAMPLE_DETAIL.VLC_DOC_CODE " & whrcls & " and TSPL_MILK_SRN_DETAIL.RATE ='0'" & Environment.NewLine & _
