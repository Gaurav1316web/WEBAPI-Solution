' Created By preeti gupta Ticket no[BM00000004284]
...
Public Class RptMilkBillRouteWise
    ...
    Sub print(ByVal exporter As EnumExportTo)
        Try
            ...
            If exporter = EnumExportTo.Excel Then
                clsCommon.MyExportToExcelGrid("MCC Milk Bill Details(Route Wise)", gv, arrHeader, Me.Text)
            Else
                clsCommon.MyExportToPDF("MCC Milk Bill Details(Route Wise)", gv, arrHeader, Me.Text, PageSetupReport_ID, objCommonVar.CurrentUserCode)
            End If
        Catch ex As Exception
    ...
    Private Sub RptMilkBillRouteWise_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub Load_Report()
        Try
            ...
            sQuery += " select '" & fromDate & "'  as fromDate ,'" & Todate & "'  as Todate ,'" & companyADD & "'  as companyADD, '" & CompName & "'  as CompName,'" & CompCode & "'  as CompCode,''  as compLogo1 ,'' as compLogo2,*,convert(decimal(18,2),(FAT_Per *NewQty)/100 ) as T_FAT,convert(decimal(18,2),(SNF_Per *NewQty )/100) as T_SNF from(select DOC_DATE ,SHIFT,convert(decimal(18,2),SUM(NewQty )) as NewQty,convert(decimal(18,1),(SUM(FATQTY )/nullif(SUM(NewQty ),0))*100) as FAT_Per,convert(decimal(18,1),(SUM(SNFQTY )/nullif(SUM(NewQty ),0))*100) as SNF_Per,convert(decimal(18,2),SUM(Amount )/nullif(SUM(NewQty ),0)) as Rate,convert(decimal(18,2),SUM(Amount )) as Amount,MCC_CODE,ROUTE_CODE  from (select convert(varchar,DOC_DATE,103) as DOC_DATE,UOM_Code,FATQTY*CF as FATQTY,SNFQTY*CF as SNFQTY,Qty*CF as NewQty, Qty,FromUOM,TOUOM,CF,xx.RATE ,cf*Net_AMOUNT as Amount ,MCC_CODE+' -'+MCC_NAME+'  QtyMode :'+TOUOM   as MCC_CODE ,VSP_CODE ,SHIFT,ROUTE_CODE+' -'+Route_Name as ROUTE_CODE  ,Vendor_Name ,MCC_NAME " & Environment.NewLine & _
                " from (select TSPL_MILK_PURCHASE_INVOICE_DETAIL.Item_Code , TSPL_MILK_RECEIPT_DETAIL.UOM_Code,TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty  ,FAT_PER ,(TSPL_MILK_PURCHASE_INVOICE_DETAIL.FAT_PER*TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty/100) as FATQTY,SNF_PER,(TSPL_MILK_PURCHASE_INVOICE_DETAIL.SNF_PER *TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty/100) as SNFQTY  ,TSPL_MILK_PURCHASE_INVOICE_DETAIL.RATE as RATE,TSPL_MILK_PURCHASE_INVOICE_DETAIL.Net_AMOUNT, TSPL_MILK_PURCHASE_INVOICE_HEAD.MCC_CODE , TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_DATE,TSPL_MILK_PURCHASE_INVOICE_HEAD.VSP_CODE ,TSPL_MILK_SAMPLE_HEAD.SHIFT, TSPL_MILK_PURCHASE_INVOICE_HEAD.ROUTE_CODE ,TSPL_VENDOR_MASTER.Vendor_Name,TSPL_MCC_ROUTE_MASTER .Route_Name  ,TSPL_MCC_MASTER .MCC_NAME   from TSPL_MILK_PURCHASE_INVOICE_DETAIL  Inner Join TSPL_MILK_PURCHASE_INVOICE_HEAD On TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_CODE =TSPL_MILK_PURCHASE_INVOICE_DETAIL.DOC_CODE  left outer join TSPL_MILK_SRN_HEAD  on TSPL_MILK_SRN_HEAD .DOC_CODE  =TSPL_MILK_PURCHASE_INVOICE_DETAIL.SRN_CODE  left outer join TSPL_MILK_SAMPLE_HEAD on TSPL_MILK_SAMPLE_HEAD.DOC_CODE =TSPL_MILK_SRN_HEAD.MILK_SAMPLE_CODE  left outer join TSPL_MILK_RECEIPT_HEAD on TSPL_MILK_RECEIPT_HEAD.DOC_CODE =TSPL_MILK_SAMPLE_HEAD.MILK_RECEIPT_CODE   left outer join TSPL_MILK_RECEIPT_DETAIL on TSPL_MILK_RECEIPT_DETAIL.DOC_CODE =TSPL_MILK_RECEIPT_HEAD.DOC_CODE and   TSPL_MILK_SRN_HEAD.vlc_doc_Code = TSPL_MILK_RECEIPT_DETAIL.VLC_DOC_CODE Left Outer Join TSPL_VENDOR_MASTER On TSPL_MILK_PURCHASE_INVOICE_HEAD.VSP_CODE =TSPL_VENDOR_MASTER.Vendor_Code And TSPL_VENDOR_MASTER.Form_Type = 'VSP'  " & Environment.NewLine & _
            " Left Outer Join TSPL_MCC_MASTER On TSPL_MILK_RECEIPT_HEAD .MCC_CODE = TSPL_MCC_MASTER.MCC_Code" & Environment.NewLine & _
            " Left Outer Join TSPL_MCC_ROUTE_MASTER On TSPL_MILK_RECEIPT_DETAIL.ROUTE_CODE =TSPL_MCC_ROUTE_MASTER.Route_Code" & Environment.NewLine & _
            ...
            sQuery += " ttt group by DOC_DATE,MCC_CODE,SHIFT,ROUTE_CODE  ) FFF left outer join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_name='" & CompName & "'" & Environment.NewLine & _
             "  order by convert(date,DOC_DATE,103 )"

            If ItemStructureMandatoryOnWeightConversion = False Then
                sQuery = "select  '" & fromDate & "'  as fromDate ,'" & Todate & "'  as Todate ,'" & companyADD & "'  as companyADD, '" & CompName & "'  as CompName,'" & CompCode & "'  as CompCode,''  as compLogo1 ,'' as compLogo2,*,[MCC Code]+' -'+[MCC Name]+'  QtyMode :'" + "'" + cboUnit.Text + "'" + " as MCC_CODE,[Route Code]+' -'+[Route Name] as ROUTE_CODE,convert(decimal(18,2),(FAT_Per *NewQty)/100 ) as T_FAT,convert(decimal(18,2),(SNF_Per *NewQty )/100) as T_SNF  from(select convert(varchar,INVOICE_DATE,103) as DOC_DATE,Case When SHIFT = 'Morning' Then 'M' Else 'E' End As [SHIFT]"
                ...
                End If
                sQuery += ",SUM(Net_Amount) as Amount,[MCC Code],max([MCC Name]) as [MCC Name],[Route Code],max([Route Name]) as [Route Name],max(UOM) AS [UOM] from("
                ...
                sQuery += " )xx group by convert(varchar,INVOICE_DATE,103),[MCC Code],[SHIFT],[Route Code]
    ...
    End Sub
    Private Sub RptMilkBillRouteWise_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
