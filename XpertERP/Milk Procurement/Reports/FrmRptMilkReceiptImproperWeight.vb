...
Public Class FrmRptMilkReceiptImproperWeight
...
#End Region
        ...
        Try
            ...
            ElseIf rbtnVLCWise.Checked Then
                FinalQuery = "select MCC_CODE,max(MCC_NAME) as MCC_NAME,ROUTE_CODE,MAX(Route_Name) as Route_Name,VLC_CODE,max(VLC_Name) as VLC_Name,max(VLC_Code_VLC_Uploader) as VLC_Code_VLC_Uploader,VSP_CODE,max(VSP_Name) as VSP_Name,sum(Min_Weight_Value) as Min_Weight_Value,sum(MILK_WEIGHT) as MILK_WEIGHT from (" + Environment.NewLine + _
                    ...
                    " )xx group by MCC_CODE,ROUTE_CODE,VLC_CODE,VSP_CODE order by MCC_CODE,ROUTE_CODE,VLC_Code_VLC_Uploader"
            ElseIf chkRoutewise.Checked Then
                FinalQuery = "select MCC_CODE,max(MCC_NAME) as MCC_NAME,ROUTE_CODE,MAX(Route_Name) as Route_Name,sum(Min_Weight_Value) as Min_Weight_Value,sum(MILK_WEIGHT) as MILK_WEIGHT from ( " + Environment.NewLine + _
                    qry + Environment.NewLine + _
                    ")xx group by MCC_CODE,ROUTE_CODE order by MCC_CODE,ROUTE_CODE"

            ElseIf ChkMCCWise.Checked Then
                FinalQuery = "select MCC_CODE,max(MCC_NAME) as MCC_NAME,sum(Min_Weight_Value) as Min_Weight_Value,sum(MILK_WEIGHT) as MILK_WEIGHT from (" + Environment.NewLine + _
                    qry + Environment.NewLine + _
    ...
    Sub FormatGrid()
        ...
        If ChkDetailWise.Checked Then
            ...
            gv.Columns("VLC_Name").HeaderText = "VSP Name"
            ...
            gv.Columns("VSP_Name").HeaderText = " VSP Code"

            gv.Columns("DOC_CODE").IsVisible = True
            gv.Columns("DOC_CODE").Width = 100
            gv.Columns("DOC_CODE").HeaderText = "Milk Receipt No"

            gv.Columns("DOC_DATE").IsVisible = True
            gv.Columns("DOC_DATE").Width = 100
            gv.Columns("DOC_DATE").HeaderText = "Milk Receipt Date"
            ...
            gv.Columns("Created_By").HeaderText = "User Code"

            gv.Columns("User_Name").IsVisible = True
            gv.Columns("User_Name").Width = 100
            gv.Columns("User_Name").HeaderText = "User"

            gv.Columns("Sample_No").IsVisible = True
            gv.Columns("Sample_No").Width = 100
            gv.Columns("Sample_No").HeaderText = "Sample No"

            gv.Columns("Min_Weight_Value").IsVisible = True
            gv.Columns("Min_Weight_Value").Width = 100
            gv.Columns("Min_Weight_Value").HeaderText = "Min Reading"

            gv.Columns("MILK_WEIGHT").IsVisible = True
            gv.Columns("MILK_WEIGHT").Width = 100
            gv.Columns("MILK_WEIGHT").HeaderText = "Milk Weight"
