...
Public Class RptMilkRouteVehicleReport
    ...
    Public Sub Load_Report()
        Try
            ...
            End If
            sQuery = " Select xxx.*,  Case When xxx.[Vehicle Capicity] = 0 Then 0    Else Convert(decimal(18,2),(xxx.[Total Receipt Capicity ] /    xxx.[Vehicle Capicity]) * 100) End As [Per Vechicle]From (Select final.MCC_Code As [MCC Code],        final.MCC_NAME As [MCC Name],   convert(varchar,final.DOC_DATE,103) as DOC_DATE, final.DOC_DATE as DATE,      final.SHIFT As Shift,      final.Route_CODE As [Route No.],    final.Route_Name As [Route Name],    final.Vehicle_Code As [Vehicle No.],    final.[Vehicle Name],    final.[Max Storage Capacity] As [Vehicle Capicity],    final.Milk_Weight As [Total Receipt Capicity ],   cast ( final.Milk_Weight as numeric(15,2)) - cast ( final.[Max Storage Capacity] as numeric(15,2))  As Difference  From (Select xx.*,      TSPL_MCC_ROUTE_MASTER.Route_Name,       TSPL_MILK_SRN_dETAIL.Qty AS Milk_Weight,      TSPL_Primary_Vehicle_Master.Storage_Capacity As [Max Storage Capacity]    From (Select Max(TSPL_MCC_MASTER.MCC_Code) As MCC_Code,        Max(TSPL_MCC_MASTER.MCC_NAME) As MCC_NAME,        Max(TSPL_MILK_SRN_HEAD.DOC_DATE) As DOC_DATE,        Max(TSPL_MILK_SRN_HEAD.DOC_CODE) As DOC_CODE,        TSPL_MILK_SRN_HEAD.SHIFT,        (TSPL_Primary_Vehicle_Master.Vehicle_Code) As Vehicle_Code,        TSPL_MILK_SRN_HEAD.Route_CODE,        Max(TSPL_Primary_Vehicle_Master.Description) As [Vehicle Name]      From TSPL_MILK_SRN_DETAIL        Left Outer Join TSPL_MILK_SRN_HEAD          On TSPL_MILK_SRN_HEAD.DOC_CODE =          TSPL_MILK_SRN_DETAIL.DOC_CODE        Left Outer Join TSPL_MCC_MASTER On TSPL_MCC_MASTER.MCC_Code =          TSPL_MILK_SRN_HEAD.MCC_CODE        Left Outer Join TSPL_Primary_Vehicle_Master          On TSPL_Primary_Vehicle_Master.Vehicle_Code =          TSPL_MILK_SRN_HEAD.VEHICLE_CODE        Group By  TSPL_MILK_SRN_HEAD.Route_CODE, TSPL_Primary_Vehicle_Master.Vehicle_Code,        TSPL_MILK_SRN_HEAD.DOC_DATE,TSPL_MILK_SRN_HEAD.SHIFT) xx   Left Outer Join TSPL_MILK_SRN_DETAIL On TSPL_MILK_SRN_DETAIL.DOC_CODE = xx.DOC_CODE Left Outer Join TSPL_MILK_SRN_HEAD On TSPL_MILK_SRN_HEAD.DOC_CODE=TSPL_MILK_SRN_DETAIL.DOC_CODE AND  TSPL_MILK_SRN_HEAD.Route_CODE = xx.Route_CODE  Left Outer Join TSPL_MCC_ROUTE_MASTER On TSPL_MCC_ROUTE_MASTER.Route_Code        = xx.Route_CODE      Left Outer Join TSPL_Primary_Vehicle_Master        On TSPL_Primary_Vehicle_Master.Vehicle_Code =        TSPL_MCC_ROUTE_MASTER.Vehicle_Code) final) xxx"
