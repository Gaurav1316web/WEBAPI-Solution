...
#End Region
    ...
    Private Sub FrmMccDispatch_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        ButtonToolTip.SetToolTip(btnSave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine + _
                                     ...
                                     "TSPL_MCC_DISPATCH_CHALLAN_DETAIL" + Environment.NewLine + _
                                     "TSPL_MCC_Dispatch_Challan_Stock_Detail" + Environment.NewLine + _
                                     "TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details" + Environment.NewLine + _
                                     "TSPL_Mcc_Dispatch_Challan_Paper_Seal_Details_History" + Environment.NewLine + _
                                     "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail" + Environment.NewLine + _
                                     "TSPL_Mcc_Dispatch_Chalan_Parameter_Detail_History" + Environment.NewLine + _
                                     "tspl_provision_entry  (For provision entry when post record)" + Environment.NewLine + _
                                     "tspl_tanker_master (for Bulk Sale Data)" + Environment.NewLine + _
                                     ...
                                     "AllowJobWorkonGateEntryBulkProc  (For Allow Job Work ON Gate Enty BulkProc)" + Environment.NewLine + _
                                     "AllowFractionInMCCTankerDispatchGrossQty (Allow Fraction In MCC Tanker Dispatch Gross Qty)" + Environment.NewLine + _
                                     "IsItemEditableOnMCCDispatch (When 1, Then Item Finder Editable, When 0,  Item Finder Non-Editable)" + Environment.NewLine + _
                                     "DipMarkingMendatory" + Environment.NewLine + _
                                     "IsUOMSelectableOnMCCDispatch (When 1, Then UOM Finder Editable, When 0,  UOM Finder Non-Editable)" + Environment.NewLine + _
                                     "AllowtoNegativeStockInventoryAtTankerDispatch" + Environment.NewLine + _
                                     "UpdateInventorySummaryTable" + Environment.NewLine + _
                                     "AllowStockToleranceNegative (Allow Stock Tolerance Negative)" + Environment.NewLine + _
                                     "DisAllowIntermittentTankerForPlantDispatch" + Environment.NewLine + _
                                     "ItemDescForTankerDispatchPrint" + Environment.NewLine + _
                                     "TankerDispatchFinancialImpactInTransferIn" + Environment.NewLine + _
                                     "CreateTankerDispatchGL" + Environment.NewLine + _
                                     "SkipCogsEntry" + Environment.NewLine + _
                                     "TransferEntryOnInvCtrlAccount (For Transfer Entry On Inventory Control Account)" + Environment.NewLine + _
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(fndChalanNo.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowTransHistoryData(fndChalanNo.Value, "Document_No", "TSPL_TRANSFER_TO_SAVING", "TSPL_TRANSFER_TO_SAVING_DETAIL")
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    End Sub
