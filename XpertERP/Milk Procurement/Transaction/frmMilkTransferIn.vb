...
'============================mpdify by preeti gupta against ticket no[BM00000008137]
Public Class FrmMilkTransferIn
    ...
    Public obj As clsMilkTransferIn = Nothing
    Sub loadDispatchData(ByVal strDispChallanNo As String)
        Try

            Dim RcptChlnNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select receipt_challan_no from tspl_milk_transfer_in  where   tspl_milk_transfer_in.In_Return=0 and dispatch_challan_no='" & strDispChallanNo & "'"))
            If clsCommon.myLen(RcptChlnNo) > 0 Then
                LoadData(RcptChlnNo, NavigatorType.Current)
                Exit Sub
            End If
            Dim strWeighmentNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Weighment_No  from TSPL_Weighment_Detail where Challan_No='" & strDispChallanNo & "' and In_Return=0 "))
            Dim strGateEntryNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Gate_Entry_No  from TSPL_Weighment_Detail where Weighment_No='" & strWeighmentNo & "' and In_Return=0 "))

            Dim objW As clsWeighment = clsWeighment.getData(strWeighmentNo, "MccProc", NavigatorType.Current)
            Dim strQcNo As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select QC_No  from TSPL_QUALITY_CHECK where Gate_Entry_No='" & strGateEntryNo & "' "))
            Dim objQ As clsQualityCheck = clsQualityCheck.getData(strQcNo, "MccProc", NavigatorType.Current)
            Dim objD As clsMccDispatch = clsMccDispatch.getData(strDispChallanNo, NavigatorType.Current)
    ...
    Private Sub UpdateTotal()
        ...
        For ii As Integer = 0 To gvWeighment.Rows.Count - 1
            dblTotAmt = dblTotAmt + clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colRcptAmt).Value)
        Next
    ...
    Function SaveData(ByVal isPost As Boolean, ByVal AutoEntry As Boolean) As Boolean
            ...
            If allowToSave(AutoEntry) Then
                ...
                If obj.isNewEntry Then
                    ...
                    If chkJobWork.Checked Then
                        obj.Receipt_Challan_No = clsERPFuncationality.GetNextCode(trans, dtpRcptDateAndTime.Value, clsDocType.MilkTransferIn, clsDocTransactionType.MCCProcJobWorkOutward, clsCommon.myCstr(txtSubLocation.Value))
                    Else
                        obj.Receipt_Challan_No = clsERPFuncationality.GetNextCode(trans, dtpRcptDateAndTime.Value, clsDocType.MilkTransferIn, clsDocTransactionType.NA, clsCommon.myCstr(txtMccPlantCode.Value))
                    End If
                ...
                '    Throw New Exception("Document cannot be updated ,Already Post on :" + obj.Posting_Date)
                'End If
                If Not obj.isNewEntry Then
                    Dim Status As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select isPosted from tspl_milk_transfer_in Where Receipt_Challan_No='" + obj.Receipt_Challan_No + "'", trans))
                    If Status = 1 Then
                ...
                fndRcptChalanNo.Value = obj.Receipt_Challan_No
                ...
                strQry = "Update TSPL_Weighment_Detail set  " & _
                                        "FAT_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATRate).Value) & "', " & _
                                        "SNF_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFRate).Value) & "', " & _
                                        "FAT_Value='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATValue).Value) & "', " & _
                                        "SNF_Value='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFValue).Value) & "', " & _
                                        "FAT_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATKG).Value) & "', " & _
                                        "SNF_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFKG).Value) & "', " & _
                                        "Amount='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colRcptAmt).Value) & "' where Weighment_No='" & txtWeighmentNo.Text & "'"
                clsDBFuncationality.ExecuteNonQuery(strQry, trans)

                If AllowBulkProcMCCwithoutTankerDispatch = 1 Then
                    If MCCChamberwise = 1 Then
                        For ii As Integer = 0 To gvWeighment.Rows.Count - 1
                            strQry = "Update TSPL_WEIGHMENT_CHEMBER_DETAILS set  " & _
                         "CH_FAT_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colFATRate).Value) & "', " & _
                         "CH_SNF_Rate='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colSNFRate).Value) & "', " & _
                         "CH_FAT_Value='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colFATValue).Value) & "', " & _
                         "CH_SNF_Value='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colSNFValue).Value) & "', " & _
                         "CH_FAT_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colFATKG).Value) & "', " & _
                         "CH_SNF_Kg='" & clsCommon.myCdbl(gvWeighment.Rows(0).Cells(colSNFKG).Value) & "', " & _
                         "CH_Amount='" & clsCommon.myCdbl(gvWeighment.Rows(ii).Cells(colRcptAmt).Value) & "' " & _
                         "where Weighment_No='" & txtWeighmentNo.Text & "' and Chamber_Desc='" & gvWeighment.Rows(ii).Cells(colChamberDesc).Value & "'"
                            clsDBFuncationality.ExecuteNonQuery(strQry, trans)
                        Next
                    End If

                End If
                If clsMilkTransferIn.saveData(obj, trans) Then
                    trans.Commit()
                    If Not isPost Then
                        ...
                        End If
                    End If
                    LoadData(obj.Receipt_Challan_No, NavigatorType.Current)
                    btnSave.Text = "Update"
    ...
    Sub postData(ByVal AutoEntry As Boolean)
        Try
            ...
            If (myMessages.postConfirm()) Then
                If Not allowToSave(AutoEntry) Then
                    Exit Sub
                End If
                If SaveData(True, AutoEntry) Then
                    ...
                    If (clsMilkTransferIn.postData(fndRcptChalanNo.Value, Me.Form_ID, Nothing)) Then
                        If AutoEntry = False Then
        ...
        End Try
    End Sub
    Sub LoadData(ByVal strRcptChallanNo As String, ByVal nav As NavigatorType)
        ...
        obj = clsMilkTransferIn.getData(strRcptChallanNo, nav)
    ...
    Sub loadBlankGvNewSeal()
        ...
        gvNewSeal.Columns(colSealName).HeaderImage = Global.ERP.My.Resources.Resources.search4
    ...
    Sub loadBlankGvNewSealPaper()
        ...
        gvNewSealPaper.Columns(colSealName).HeaderImage = Global.ERP.My.Resources.Resources.search4
    ...
    Private Sub FrmMilkTransferIn_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        coll.Add("PriceCode", "varchar(30) null")
        coll.Add("FAT_W", "float  null default 0")
        coll.Add("SNF_W", "float  null default 0")
        coll.Add("FAT_R", "float  null default 0")
        coll.Add("SNF_R", "float  null default 0")
        coll.Add("Transfer_Price", "float  null default 0")
        coll.Add("In_Return", "integer NULL default 0")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_MILK_TRANSFER_IN", coll)

        coll = New Dictionary(Of String, String)
        coll.Add("In_Return", "integer NULL default 0")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_gate_entry_details", coll)

        coll = New Dictionary(Of String, String)
        coll.Add("FAT_Rate", "float  null default 0")
        coll.Add("SNF_Rate", "float  null default 0")
        coll.Add("FAT_Value", "float  null default 0")
        coll.Add("SNF_Value", "float  null default 0")
        coll.Add("FAT_Kg", "float  null default 0")
        coll.Add("SNF_Kg", "float  null default 0")
        coll.Add("Amount", "float  null default 0")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_Weighment_Detail", coll)
        ...
        coll.Add("SNF_Kg", "float  null default 0")
        coll.Add("Amount", "float  null default 0")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_Weighment_Detail_History", coll)

        coll = New Dictionary(Of String, String)
        coll.Add("CH_FAT_Rate", "float  null default 0")
        coll.Add("CH_SNF_Rate", "float  null default 0")
        coll.Add("CH_FAT_Value", "float  null default 0")
        coll.Add("CH_SNF_Value", "float  null default 0")
        coll.Add("CH_FAT_Kg", "float  null default 0")
        coll.Add("CH_SNF_Kg", "float  null default 0")
        coll.Add("CH_Amount", "float  null default 0")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_WEIGHMENT_CHEMBER_DETAILS", coll)
    ...
    Function allowToSave(ByVal AutoEntry As Boolean) As Boolean
        Try
            ...
            End If
            'If dtpRcptDateAndTime.Value < dtpDispatchDateAndTime.Value Then
            '    Throw New Exception("Receipt date can't be smaller than dispatch date ")
            'End If

            'If dtpRcptDateAndTime.Value < dtpQcInDate.Value Then
            '    Throw New Exception("Receipt date can't be smaller than QC in Date ")
            'End If

            'If dtpRcptDateAndTime.Value < dtpQCOutDate.Value Then
            ...
            Dim chk As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*) from tspl_milk_transfer_in where gate_entry_no='" & txtgateEntryNo.Text & "' and receipt_challan_no <>'" & fndRcptChalanNo.Value & "' "))
            If chk > 0 Then
    ...
    Sub printData()
        ...
        If clsCommon.myLen(fndRcptChalanNo.Value) > 0 Then
            ...
            frmCrystalReportViewer.funreport(CrystalReportFolder.MilkProcurement, dt, "rptMilkTransferIn", "Milk Transfer In", clsCommon.myCDate(dt.Rows(0)("Receipt_Challan_Date")))
        Else
    ...
    Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        Try
            ...
            Dim Status As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select isPosted from tspl_milk_transfer_in Where Receipt_Challan_No='" + obj.Receipt_Challan_No + "'"))
            If Status = 1 Then
                lblPending.Status = ERPTransactionStatus.Approved
                btnSave.Enabled = False
                btnDelete.Enabled = False
                ...
                Throw New Exception("Document cannot be updated ,Already Post ")
            End If
            ...
            Else
                ...
                If myMessages.deleteConfirm() Then
                    If clsMilkTransferIn.deleteData(fndRcptChalanNo.Value, Nothing) Then
                        reset()
    ...
    Private Sub fndDispatchChallanNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndDispatchChallanNo._MYValidating
        Try
            If AllowBulkProcMCCwithoutTankerDispatch = 0 Then
                Dim whrCls As String = " TSPL_MCC_Dispatch_Challan.Chalan_NO in (Select Chalan_NO  from TSPL_Weighment_Detail left outer join TSPL_MCC_Dispatch_Challan on TSPL_MCC_Dispatch_Challan.Chalan_NO= TSPL_Weighment_Detail.Challan_No  WHERE TSPL_Weighment_Detail.isPosted=1 AND " & _
                    " TSPL_Weighment_Detail.Gate_Entry_No NOT IN (Select Gate_Entry_no from tspl_milk_transfer_in where Gate_Entry_no<>'') and TSPL_MCC_Dispatch_Challan.Mcc_Or_Plant_Code='" & txtMccPlantCode.Value & "')"
                ' TSPL_Weighment_Detail.Challan_No NOT IN (Select Dispatch_Challan_No from tspl_milk_transfer_in where Dispatch_Challan_No<>'" & fndDispatchChallanNo.Value & "'  and tspl_milk_transfer_in.In_Return=0  ) 
    ...
    Private Sub fndRcptChalanNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndRcptChalanNo._MYValidating
        ...
        If Not clsMccMaster.isCurrentUserHO() Then
            If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                whrCls = " tspl_milk_transfer_in.location_code in ( " & objCommonVar.strCurrUserLocations & ")"
            End If
        End If
        fndRcptChalanNo.Value = clsMilkTransferIn.getFinder(whrCls, fndRcptChalanNo.Value, isButtonClicked)
        If clsCommon.myLen(fndRcptChalanNo.Value) > 0 Then
            LoadData(fndRcptChalanNo.Value, NavigatorType.Current)
        Else
            reset()
