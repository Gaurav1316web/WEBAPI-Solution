 ...
 where  (isnull(is_sub_location,'N')='Y' or isnull(Is_Section,'N')='Y') and Main_Location_Code='" & strCellValue & "' and Location_Code = '" & strSubLocation & "'")
    ...
    Private Sub CreateAutoInvoiceAgainstMultipleDispatch(ByVal trans As SqlTransaction)
        ...
        Try
            ...
            dtout = clsDBFuncationality.GetDataTable("Select MCCSaleDate,Location,Customer,MAX(VLCCode) As VLCCode,Item_code,SUM(Qty) As Qty, UOM,max(rate) As rate,sum(Amount) As Amount,InvoiceType,max(taxGroup) As taxGroup,max(Tax1) As Tax1,max(TAx1rate) As TAx1rate,sum(Tax1Amt) As Tax1Amt,max(Tax2) As Tax2,max(TAx2rate) As TAx2rate,sum(Tax2Amt) As Tax2Amt,Sub_Location_code from Temp_table_MCC_Material_Sale_uploader GROUP BY Location,Sub_Location_code,Customer,InvoiceType,MCCSaleDate,Item_code,UOM", trans)
            ...
            If ValidatedCount > 0 Then
                For Each dr As DataRow In dtout.Rows
                    ...
                    If clsCommon.CompairString(clsCommon.myCDate(strdocdate), clsCommon.myCDate(dr("MCCSaleDate"))) = CompairStringResult.Equal And clsCommon.CompairString(SalePriceCode, clsCommon.myCstr(dr("InvoiceType"))) = CompairStringResult.Equal And clsCommon.CompairString(CustomerCode, clsCommon.myCstr(dr("Customer"))) = CompairStringResult.Equal And clsCommon.CompairString(LocationCode, clsCommon.myCstr(dr("Location"))) = CompairStringResult.Equal Then
