...
#End Region
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    If clsCommon.myLen(strReqNo) > 0 Then
                        ...
                        If (dblQty + dblDamageQty) > dblPendingQty Then
                            common.clsCommon.MyMessageBoxShow("Item " + strICode + "( " + strIName.Trim() + " ) Entered Quantity(" + clsCommon.myCstr(dblQty + dblDamageQty) + ") Can't be more Pending Quantity(" + clsCommon.myCstr(dblPendingQty) + ").At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                            Return False
                        End If
                        '============================
                    End If
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                Dim isEnableDisable As Boolean = CheckInvoiceWithGateEntry(obj.Against_Invoice_No, obj.Gate_Entry_No)  'clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select count(*) from TSPL_Sale_Return_Gate_Entry_Invoice_Wise where Gate_Entry_No ='" + obj.Gate_Entry_No + "'  and Invoice_No = '" + obj.Against_Invoice_No + "'"))
    ...
    Private Sub funPrint(Optional ByVal IsPDF As Boolean = False)
        ...
        Try
            ...
            Dim Qry As String = " select Customer_State_Master.STATE_CODE,TSPL_SD_SALE_RETURN_HEAD.Is_Taxable,TSPL_SD_SHIPMENT_HEAD.Electronic_Ref_No,TSPL_SD_SHIPMENT_HEAD.EWayBillNo," & _
                                " convert(varchar,TSPL_SD_SHIPMENT_HEAD.EWayBillDate,103) as EWayBillDate,TSPL_STATE_MASTER.GST_STATE_Code as Loc_GST_State,TSPL_LOCATION_MASTER.GSTNO as Loc_GST_INNo," & _
                                 "   Customer_State_Master.GST_STATE_Code as Cust_GST_State,TSPL_CUSTOMER_MASTER .GSTNO as Cust_GST_No,TSPL_ITEM_MASTER .HSN_Code ," & _
                                 " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX1_Amt ,0)as DTax1_Amt,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX2_Amt ,0)as DTax2_Amt, ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX3_Amt ,0)as DTax3_Amt," & _
                               "   ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX4_Amt ,0)as DTax4_Amt,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX5_Amt ,0)as DTax5_Amt,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX6_Amt ,0)as DTax6_Amt," & _
                                " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX7_Amt ,0)as DTax7_Amt,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX8_Amt ,0)as DTax8_Amt,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX9_Amt ,0)as DTax9_Amt,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX10_Amt ,0)as DTax10_Amt," & _
                                " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX1_Rate,0)as DTax1_Rate,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX2_Rate,0)as DTax2_Rate," & _
                                    " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX3_Rate,0)as DTax3_Rate,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX4_Rate,0)as DTax4_Rate," & _
                              " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX5_Rate,0)as DTax5_Rate,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX6_Rate,0)as DTax6_Rate," & _
                             " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX7_Rate,0)as DTax7_Rate,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX8_Rate,0)as DTax8_Rate," & _
                            " ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX9_Rate,0)as DTax9_Rate,ISNULL(TSPL_SD_SALE_RETURN_DETAIL.TAX10_Rate,0)as DTax10_Rate," & _
                              "  dtax1.Type as tax1Type,dtax2.Type as tax2Type,dtax3.Type as tax3Type,dtax4.Type as tax4Type,dtax5.Type as tax5Type,dtax6.Type as tax6Type,dtax7.Type as tax7Type," & _
            ...
            Qry += " LEFT OUTER JOIN TSPL_SD_SALE_INVOICE_DETAIL on TSPL_SD_SALE_INVOICE_DETAIL.DOCUMENT_CODE = TSPL_SD_SALE_INVOICE_HEAD.Document_Code and TSPL_SD_SALE_INVOICE_DETAIL.LINE_NO = TSPL_SD_SALE_RETURN_DETAIL.LINE_NO  and TSPL_SD_SALE_INVOICE_DETAIL.Item_Code= TSPL_SD_SALE_RETURN_DETAIL.Item_Code " & _
                   ...
                   " TSPL_ITEM_BARCODE.Item_MRP = TSPL_SD_SALE_RETURN_DETAIL.MRP " & _
                   " left outer join TSPL_TAX_MASTER as dtax1 on dtax1.tax_code =TSPL_SD_SALE_RETURN_DETAIL.tax1 " & _
                    "  left outer join tspl_tax_master as dtax2 on dtax2.tax_code = TSPL_SD_SALE_RETURN_DETAIL.tax2 " & _
                    "   left outer join tspl_tax_master as dtax3 on dtax3.Tax_Code=TSPL_SD_SALE_RETURN_DETAIL.TAX3  " & _
                    " left outer join TSPL_TAX_MASTER as dtax4 on tax4.Tax_Code= TSPL_SD_SALE_RETURN_DETAIL.tax4   " & _
                    " left outer join TSPL_TAX_MASTER as dtax5 on dtax5.Tax_Code=TSPL_SD_SALE_RETURN_DETAIL.tax5  " & _
                    " left outer join TSPL_TAX_MASTER as dtax6 on dtax6.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL.TAX6 " & _
                    "  left outer join TSPL_TAX_MASTER as dtax7 on dtax7.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL.TAX7  " & _
                    "  left outer join TSPL_TAX_MASTER as dtax8 on dtax8.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL.TAX8  " & _
                    "  left outer join TSPL_TAX_MASTER as dtax9 on dtax9.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL.TAX9 " & _
                    " left outer join TSPL_TAX_MASTER as dtax10 on dtax10.Tax_Code =TSPL_SD_SALE_RETURN_DETAIL.TAX10 " & _
