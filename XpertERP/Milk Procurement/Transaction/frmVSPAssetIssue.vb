...
'BM00000009499 by balwwinder on 06/09/2016
''BM00000009750
Public Class frmVSPAssetIssue
...
#End Region
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.frmVSPAssetIssue)
        ...
        Try
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    End If
                    If e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colReqQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colCCCode) OrElse e.Column Is gv1.Columns(colRetQty) Then
                        If gv1.CurrentColumn Is gv1.Columns(colQty) Then
                            ...
                            If clsCommon.myLen(txtFromLocation.Value) <> 0 And clsCommon.myLen(clsCommon.myCdbl(gv1.CurrentRow.Cells(colICode).Value)) <> 0 Then
                                ...
                                If clsCommon.CompairString(cboDocType.Text, "Issue") = CompairStringResult.Equal And clsCommon.CompairString(btnSave.Text, "Update") = CompairStringResult.Equal Then
                                    str = "SELECT Requisition_Qty - (select isnull(SUM(Issued_Qty),0) from TSPL_VSPAsset_HEAD inner join " &
                                    "TSPL_VSPAsset_DETAIL on TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No and " &
                                    "TSPL_VSPAsset_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Issue' and " &
                                    "TSPL_VSPAsset_HEAD.Doc_No <> '" & txtDocNo.Value & "' ) +   (select isnull(SUM(Issued_Qty),0) from TSPL_VSPAsset_HEAD inner join " &
                                    "TSPL_VSPAsset_DETAIL on TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No and " &
                                    "TSPL_VSPAsset_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Return') FROM " &
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        Else
            ...
            Dim obj As clsItemMaster = clsItemMaster.FinderForItem(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), "A", True, isButtonClick, "", "")
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Item_Code) > 0 Then
                ...
                If PickAvgCostonAssetissue = True AndAlso clsCommon.CompairString(clsCommon.myCstr(cboDocType.Text), "Issue") = CompairStringResult.Equal Then
                    ...
                    Dim dblCostMethod As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Costing_Method as Costing_Method from TSPL_ITEM_MASTER left outer join TSPL_PURCHASE_ACCOUNTS on TSPL_PURCHASE_ACCOUNTS.Purchase_Class_Code=TSPL_ITEM_MASTER.Purchase_Class_Code where Item_Code='" & obj.Item_Code & "' "))
                    If dblCostMethod <> 0 Then
                        gv1.CurrentRow.Cells(colRate).Value = clsInventoryMovement.GetCost(dblCostMethod, obj.Item_Code, txtFromLocation.Value, 1, txtDate.Value, txtDate.Value, False, Nothing, "TSPL_INVENTORY_MOVEMENT", obj.Unit_Code)
    ...
    Sub SaveData(ByVal ChekPostBTn As Boolean)
        Try
            If ChekPostBTn = False Then
                If FrmMainTranScreen.ValidateTransactionAccToFinYear("VSP Asset Issue", txtDate.Value) = False Then
                    Exit Sub
                End If
            End If
            ''
            If (AllowToSave()) Then
                Dim obj As New clsMCCIssueReturnHead()
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsMCCIssueReturnDetail()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsMCCIssueReturnHead()
            obj = clsMCCIssueReturnHead.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Doc_No) > 0) Then
                ...
                If clsCommon.myLen(txtIssueTo.Value) > 0 Then
                    lblIssueTo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT ISNULL(Vendor_Name,'') AS Vendor_Name FROM TSPL_VENDOR_MASTER WHERE Vendor_Code='" & clsCommon.myCstr(txtIssueTo.Value) & "'"))
                Else
                    lblIssueTo.Text = ""
                End If
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            Dim qst As String = "select count(*) from TSPL_VSPAsset_HEAD where Doc_No='" + txtDocNo.Value + "'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        & " ,vendor_name as [VSP Name],Doc_Type as Type,case when TSPL_VSPAsset_HEAD.Status='0' then 'Pending' else 'Approved' end as [Status] " _
        & " from TSPL_VSPAsset_HEAD left join tspl_mcc_master on tspl_mcc_master.mcc_code=from_location left join tspl_vendor_master on vendor_code=issue_to"
    ...
    Private Sub FrmVSPAssetIssue_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Public Sub Print()
        Try
            '            Dim qry As String = "SELECT     TSPL_VSPAsset_HEAD.Doc_No, TSPL_VSPAsset_HEAD.Doc_Date,TSPL_VSPAsset_HEAD.Doc_Type, TSPL_VSPAsset_HEAD.Remarks, TSPL_VSPAsset_HEAD.Comment, " & _
            '                     " case when  TSPL_VSPAsset_HEAD.Status=0 then 'Pending' else 'Approved' end as Status, TSPL_VSPAsset_HEAD.Posting_Date, TSPL_VSPAsset_DETAIL.Item_Code,  " & _
            '                      " TSPL_VSPAsset_DETAIL.Item_Desc, TSPL_VSPAsset_DETAIL.Required_Qty, TSPL_VSPAsset_DETAIL.Issued_Qty,  " & _
            '                      " TSPL_VSPAsset_DETAIL.Unit_code, TSPL_COMPANY_MASTER.Comp_Name, TSPL_COMPANY_MASTER.Add1, TSPL_COMPANY_MASTER.Add2,  " & _
            ...
            '    " FROM  TSPL_VSPAsset_HEAD INNER JOIN TSPL_VSPAsset_DETAIL ON TSPL_VSPAsset_HEAD.Doc_No = TSPL_VSPAsset_DETAIL.Doc_No  " & _
            '" LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp1 ON TSPL_VSPAsset_HEAD.Issue_To = emp1.EMP_CODE  " & _
            '"  LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp2 ON TSPL_VSPAsset_HEAD.Request_By = emp2.EMP_CODE  " & _
            '"   LEFT OUTER JOIN  TSPL_LOCATION_MASTER as loc1 ON TSPL_VSPAsset_HEAD.From_Location = loc1.Location_Code  " & _
            ' "   LEFT OUTER JOIN  TSPL_LOCATION_MASTER  as loc2 ON TSPL_VSPAsset_HEAD.To_Location = loc2.Location_Code  " & _
            ' "   LEFT OUTER JOIN TSPL_COMPANY_MASTER ON TSPL_VSPAsset_HEAD.comp_code = TSPL_COMPANY_MASTER.Comp_Code  " & _
            '"  where TSPL_VSPAsset_HEAD.Doc_No='" + txtDocNo.Value + "'"
            ''------------------------------ Changes by shipra on 24/06/13--------------------------------------------''

            ''Dim qry As String = " SELECT     TSPL_VSPAsset_HEAD.Doc_No, TSPL_VSPAsset_HEAD.Doc_Date,TSPL_VSPAsset_HEAD.Doc_Type, TSPL_VSPAsset_HEAD.Remarks, TSPL_VSPAsset_HEAD.Comment,  case when  TSPL_VSPAsset_HEAD.Status=0 then 'Pending' else 'Approved' end as Status, TSPL_VSPAsset_HEAD.Posting_Date, TSPL_VSPAsset_DETAIL.Item_Code,   TSPL_VSPAsset_DETAIL.Item_Desc, TSPL_VSPAsset_DETAIL.Required_Qty, TSPL_VSPAsset_DETAIL.Issued_Qty as returnqty,   TSPL_VSPAsset_DETAIL.Unit_code, TSPL_COMPANY_MASTER.Comp_Name, TSPL_COMPANY_MASTER.Add1, TSPL_COMPANY_MASTER.Add2,   TSPL_COMPANY_MASTER.Add3, TSPL_COMPANY_MASTER.State, TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2, loc1.Location_Desc as Fromloc,loc2.Location_Desc as Toloc, emp1.Emp_Name as IssuesTo,emp2.Emp_Name as RequestBy,(select xxxx.Issued_Qty  from TSPL_VSPAsset_DETAIL  xxxx where xxxx.Doc_No=TSPL_VSPAsset_HEAD.Req_IssueNo and xxxx.Item_Code=TSPL_VSPAsset_DETAIL .Item_Code  )as [Issued_Qty]     FROM  TSPL_VSPAsset_HEAD "
            ''qry += " INNER JOIN TSPL_VSPAsset_DETAIL ON TSPL_VSPAsset_HEAD.Doc_No = TSPL_VSPAsset_DETAIL.Doc_No"
            ''qry += " LEFT OUTER JOIN TSPL_COMPANY_MASTER ON TSPL_VSPAsset_HEAD.comp_code = TSPL_COMPANY_MASTER.Comp_Code"
            ''qry += " LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp1 ON TSPL_VSPAsset_HEAD .Issue_To = emp1.EMP_CODE  "
            ''qry += " LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp2 ON TSPL_VSPAsset_HEAD.Request_By = emp2.EMP_CODE    "
            ''qry += " LEFT OUTER JOIN  TSPL_LOCATION_MASTER as loc1 ON TSPL_VSPAsset_HEAD.From_Location = loc1.Location_Code"
            ''qry += " LEFT OUTER JOIN  TSPL_LOCATION_MASTER  as loc2 ON TSPL_VSPAsset_HEAD.To_Location = loc2.Location_Code    "
            ''qry += " where TSPL_VSPAsset_HEAD.Doc_No='" + txtDocNo.Value + "'"

            ''Dim qry As String = "     SELECT  TSPL_CostCenter_MASTER.cost_name,  TSPL_VSPAsset_HEAD.Created_By ,TSPL_VSPAsset_HEAD.Modify_By ,   TSPL_VSPAsset_HEAD.Doc_No, TSPL_VSPAsset_HEAD.Doc_Date,TSPL_VSPAsset_HEAD.Doc_Type, TSPL_VSPAsset_HEAD.Remarks, TSPL_VSPAsset_HEAD.Comment,  case when  TSPL_VSPAsset_HEAD.Status=0 then 'Pending' else 'Approved' end as Status, TSPL_VSPAsset_HEAD.Posting_Date, TSPL_VSPAsset_DETAIL.Item_Code,   TSPL_VSPAsset_DETAIL.Item_Desc, TSPL_VSPAsset_DETAIL.Required_Qty, TSPL_VSPAsset_DETAIL.Issued_Qty_AgainstRet as returnqty,   TSPL_VSPAsset_DETAIL.Unit_code, TSPL_COMPANY_MASTER.Comp_Name, TSPL_COMPANY_MASTER.Add1, TSPL_COMPANY_MASTER.Add2,   TSPL_COMPANY_MASTER.Add3, TSPL_COMPANY_MASTER.State, TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2, loc1.Location_Desc as Fromloc,loc2.Location_Desc as Toloc, emp1.Emp_Name as IssuesTo,emp2.Emp_Name as RequestBy,"
            ' ''qry += " --(select xxxx.Issued_Qty  from TSPL_VSPAsset_DETAIL  xxxx where xxxx.Doc_No=TSPL_VSPAsset_HEAD.Req_IssueNo and xxxx.Item_Code=TSPL_VSPAsset_DETAIL .Item_Code  )"
            ''qry += " TSPL_VSPAsset_DETAIL.Issued_Qty as [Issued_Qty]     FROM  TSPL_VSPAsset_HEAD  INNER JOIN TSPL_VSPAsset_DETAIL ON TSPL_VSPAsset_HEAD.Doc_No = TSPL_VSPAsset_DETAIL.Doc_No LEFT OUTER JOIN TSPL_COMPANY_MASTER ON TSPL_VSPAsset_HEAD.comp_code = TSPL_COMPANY_MASTER.Comp_Code LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp1 ON TSPL_VSPAsset_HEAD .Issue_To = emp1.EMP_CODE   LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp2 ON TSPL_VSPAsset_HEAD.Request_By = emp2.EMP_CODE     LEFT OUTER JOIN  TSPL_LOCATION_MASTER as loc1 ON TSPL_VSPAsset_HEAD.From_Location = loc1.Location_Code LEFT OUTER JOIN  TSPL_LOCATION_MASTER  as loc2 ON TSPL_VSPAsset_HEAD.To_Location = loc2.Location_Code              LEFT OUTER JOIN  TSPL_CostCenter_MASTER   ON TSPL_VSPAsset_DETAIL.Cost_Code  = TSPL_CostCenter_MASTER.Cost_Code  "
            ''qry += " where TSPL_VSPAsset_HEAD.Doc_No='" + txtDocNo.Value + "'"
            ...
            '            Dim type As String = "select Doc_type from TSPL_VSPAsset_HEAD where Doc_No='" + txtDocNo.Value + "'"
            ...
            '                Dim QryTrnsfr As String = "select TSPL_VSPAsset_HEAD.Created_By,TSPL_VSPAsset_HEAD.Modify_By, '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE) + "' as RunDate, TSPL_COMPANY_MASTER.logo_img, TSPL_COMPANY_MASTER.logo_img2, TSPL_COMPANY_MASTER.Comp_Name  as CompanyName, " & _
            ...
            '    " TSPL_VSPAsset_HEAD.Doc_No, TSPL_VSPAsset_HEAD.Doc_Date, TSPL_VSPAsset_HEAD.Doc_Type, " & _
            ...
            '    " (select Location_Desc from TSPL_LOCATION_MASTER where location_Code=TSPL_VSPAsset_HEAD.To_Location) as ToLocDesc, " & _
            '    " (select Loc_Segment_Code from TSPL_LOCATION_MASTER where location_Code=TSPL_VSPAsset_HEAD.To_Location) as ToLocSegCode, " & _
            '    " (select TIN_No from TSPL_LOCATION_MASTER where location_Code=TSPL_VSPAsset_HEAD.To_Location) as TinNo, " & _
            '    " (select TCAN_No from TSPL_LOCATION_MASTER where location_Code=TSPL_VSPAsset_HEAD.To_Location) as CstNo, " & _
            '    " (select TIN_No from TSPL_LOCATION_MASTER where location_Code=TSPL_VSPAsset_HEAD.From_Location) as CompanyTin, " & _
            '    " '' as NRG_No, TSPL_VSPAsset_DETAIL.Item_Code AS ItemCode, TSPL_VSPAsset_DETAIL.Item_Desc AS Desciption, " & _
            '    " TSPL_VSPAsset_DETAIL.Issued_Qty AS Quantity, TSPL_VSPAsset_DETAIL.Unit_code AS Uom, TSPL_VSPAsset_DETAIL.Unit_Cost AS Rate, " & _
            '    " TSPL_VSPAsset_DETAIL.Amount AS Amount, TSPL_VSPAsset_HEAD.TAX1 AS TaxRateDesc1, TSPL_VSPAsset_HEAD.TAX1_Amt as TaxRate1, " & _
            '    " TSPL_VSPAsset_HEAD.TAX2 as TaxRateDesc2, TSPL_VSPAsset_HEAD.TAX2_Amt as TaxRate2, TSPL_VSPAsset_HEAD.TAX3 as TaxRateDesc3, " & _
            '    " TSPL_VSPAsset_HEAD.TAX3_Amt as TaxRate3, TSPL_VSPAsset_HEAD.TAX4 as TaxRateDesc4, TSPL_VSPAsset_HEAD.TAX4_Amt as TaxRate4, " & _
            '    " TSPL_VSPAsset_HEAD.TAX5 as TaxRateDesc5, TSPL_VSPAsset_HEAD.TAX5_Amt as TaxRate5, TSPL_VSPAsset_HEAD.TAX6 as TaxRateDesc6, " & _
            '    " TSPL_VSPAsset_HEAD.TAX6_Amt as TaxRate6, TSPL_VSPAsset_HEAD.TAX7 as TaxRateDesc7, TSPL_VSPAsset_HEAD.TAX7_Amt as TaxRate7, " & _
            '    " TSPL_VSPAsset_HEAD.TAX8 as TaxRateDesc8, TSPL_VSPAsset_HEAD.TAX8_Amt as TaxRate8, TSPL_VSPAsset_HEAD.TAX9 as TaxRateDesc9, " & _
            '    " TSPL_VSPAsset_HEAD.TAX9_Amt as TaxRate9, TSPL_VSPAsset_DETAIL.TAX10 as TaxRateDesc10, TSPL_VSPAsset_DETAIL.TAX10_Amt as  TaxRate10 " & _
            '    " FROM TSPL_VSPAsset_HEAD " & _
            '    " INNER JOIN TSPL_VSPAsset_DETAIL ON TSPL_VSPAsset_HEAD.Doc_No = TSPL_VSPAsset_DETAIL.Doc_No " & _
            '    " LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp1 ON TSPL_VSPAsset_HEAD.Issue_To = emp1.EMP_CODE " & _
            '    " LEFT OUTER JOIN  TSPL_EMPLOYEE_MASTER as emp2 ON TSPL_VSPAsset_HEAD.Request_By = emp2.EMP_CODE " & _
            '    " LEFT OUTER JOIN  TSPL_LOCATION_MASTER as loc1 ON TSPL_VSPAsset_HEAD.From_Location = loc1.Location_Code " & _
            '    " LEFT OUTER JOIN  TSPL_LOCATION_MASTER  as loc2 ON TSPL_VSPAsset_HEAD.To_Location = loc2.Location_Code " & _
            '    " LEFT OUTER JOIN TSPL_COMPANY_MASTER ON TSPL_VSPAsset_HEAD.comp_code = TSPL_COMPANY_MASTER.Comp_Code " & _
            '    " where TSPL_VSPAsset_HEAD.Doc_No='" + txtDocNo.Value + "' and TSPL_VSPAsset_HEAD.Doc_Type='" + val + "' "
            '                ''''--------------------------------------------------Code Ends Here--------------------------------------------------
            ...
            Dim Qry As String = "Select tspl_serial_item.auto_sr_no,  tspl_location_master.add1 +case when len(tspl_location_master.add2)>0 then ', '+tspl_location_master.add2 else '' end +case when LEN(isnull(tspl_location_master.Add3,''))>0  then ', '+isnull(tspl_location_master.Add3,'') else ' ' end  + case when len(state_forlocation.State_name )>0 then state_forlocation.state_name else '' end  as Loc_address,TSPL_VlC_master_head.VLC_Code,TSPL_VlC_master_head.vlc_name,TSPL_VlC_master_head.vlc_code_vlc_uploader, TSPL_VSPAsset_HEAD.Doc_Type,TSPL_VSPAsset_HEAD.Issue_No  ,TSPL_VSPAsset_HEAD.From_Location,TSPL_LOCATION_MASTER.Location_Desc , TSPL_VSPAsset_HEAD.Created_By ,TSPL_VSPAsset_HEAD.Modify_By ,TSPL_COMPANY_MASTER.Comp_Code ,TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Email AS Comp_Email, case when ISNULL(TSPL_COMPANY_MASTER .Phone1,'')='(+__)__________' then '' else TSPL_COMPANY_MASTER.Phone1 end +  Case When   ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as  Comp_Phn, TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 "
            Qry += " then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end  + case when len(TSPL_COMPANY_MASTER.State )>0 then TSPL_COMPANY_MASTER.State else '' end  as Comp_address,TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2  ,TSPL_COMPANY_MASTER.Tin_No as Comp_Tin_No,TSPL_COMPANY_MASTER.CST_LST as Comp_CSt_LST, TSPL_VENDOR_MASTER .add1 +case when len(TSPL_VENDOR_MASTER.add2)>0 then ', '+TSPL_VENDOR_MASTER.add2 else '' end +case when LEN(isnull(TSPL_VENDOR_MASTER.Add3,''))>0 then ', '+isnull(TSPL_VENDOR_MASTER.Add3,'') else ' ' end  + case when len(TSPL_VENDOR_MASTER.State )>0 then TSPL_VENDOR_MASTER.State else '' end  as Ven_address,TSPL_VENDOR_MASTER.Vendor_Code  ,TSPL_VENDOR_MASTER.Vendor_Name  ,TSPL_VENDOR_MASTER.CST as Ven_CST_No,TSPL_VENDOR_MASTER.TIN_No as Ven_Tin_no,"
            Qry += " case when ISNULL(TSPL_VENDOR_MASTER .Phone1,'')='(+__)__________' then '' else TSPL_VENDOR_MASTER.Phone1 end +  Case When   ISNULL(TSPL_VENDOR_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_VENDOR_MASTER.Phone2 Else'' End as  Ven_Phn, TSPL_VENDOR_MASTER.Email as Ven_Email,TSPL_VSPAsset_HEAD.Doc_No ,convert(varchar,TSPL_VSPAsset_HEAD.Doc_Date,103) as Doc_Date ,TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_VSPAsset_DETAIL.Unit_code ,TSPL_VSPAsset_DETAIL.Amount ,case when TSPL_VSPAsset_DETAIL.Issued_Qty_againstret > 0  then TSPL_VSPAsset_DETAIL.Issued_Qty_againstret  else Issued_Qty end  as  Issued_Qty,TSPL_VSPAsset_DETAIL.Unit_Cost as Rate ,  TSPL_VSPAsset_DETAIL.Total_Tax_Amt ,TSPL_VSPAsset_HEAD.Doc_Amt , TAX1 .Tax_Code_Desc as tax1name,isnull (TSPL_VSPAsset_HEAD.tax1_amt,0) as txt1amt,"
            Qry += "  tax2.Tax_Code_Desc as tax2name,isnull (TSPL_VSPAsset_HEAD.tax2_amt,0) as txt2amt,  tax3.Tax_Code_Desc as tax3name,isnull (TSPL_VSPAsset_HEAD.tax3_amt,0) as txt3amt,  tax4.Tax_Code_Desc as tax4name,isnull (TSPL_VSPAsset_HEAD.tax4_amt,0) as txt4amt,  tax5.Tax_Code_Desc as tax5name,isnull (TSPL_VSPAsset_HEAD.tax5_amt,0) as txt5amt, tax6.Tax_Code_Desc as tax6name,isnull (TSPL_VSPAsset_HEAD.tax6_amt,0) as txt6amt,   tax7.Tax_Code_Desc as tax7name,isnull (TSPL_VSPAsset_HEAD.tax7_amt,0) as txt7amt,  tax8.Tax_Code_Desc as tax8name,isnull (TSPL_VSPAsset_HEAD.tax8_amt,0) as txt8amt,   tax9.Tax_Code_Desc as tax9name,isnull (TSPL_VSPAsset_HEAD.tax9_amt,0) as txt9amt,  tax10.Tax_Code_Desc as tax10name,isnull (TSPL_VSPAsset_HEAD.tax10_amt,0) as txt10amt  from TSPL_VSPAsset_DETAIL left outer join TSPL_VSPAsset_HEAD on TSPL_VSPAsset_HEAD.Doc_No =TSPL_VSPAsset_DETAIL.Doc_No "
            Qry += " left outer join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER .Comp_Code =TSPL_VSPAsset_HEAD.comp_code  left outer join TSPL_VENDOR_MASTER  on TSPL_VENDOR_MASTER.Vendor_Code  =TSPL_VSPAsset_HEAD.Issue_To  and Form_Type in ('VSP','PTM') left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code =TSPL_VSPAsset_DETAIL.Item_Code  left outer join tspl_tax_master as tax1 on tax1.tax_code = TSPL_VSPAsset_HEAD.tax1  left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_VSPAsset_HEAD.tax2   left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_VSPAsset_HEAD .TAX3  left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_VSPAsset_HEAD .tax4  left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_VSPAsset_HEAD .tax5  left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_VSPAsset_HEAD .TAX6   left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_VSPAsset_HEAD .TAX7  left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_VSPAsset_HEAD .TAX8  left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_VSPAsset_HEAD .TAX9  left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_VSPAsset_HEAD .TAX10  left outer join TSPL_LOCATION_MASTER  on TSPL_LOCATION_MASTER .Location_Code =TSPL_VSPAsset_HEAD.From_Location left join TSPL_STATE_MASTER as state_forlocation on state_forlocation.state_code=TSPL_LOCATION_MASTER.state"
            Qry += " left join TSPL_VlC_master_head on TSPL_VlC_master_head.vsp_code=TSPL_VENDOR_MASTER.vendor_code left join tspl_serial_item on tspl_serial_item.document_code=TSPL_VSPAsset_HEAD.Doc_No and tspl_serial_item.item_code=TSPL_VSPAsset_DETAIL.Item_Code "
    ...
    Private Sub txtIssueTo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtIssueTo._MYValidating
        Dim qry As String = ""
        qry = "select Vendor_Code AS [Code],Vendor_Name As [Vendor Name],vlc_code_vlc_uploader as [Vlc Code],Vlc_Name as [Vlc Name],Add1 + ' ' + Add2 + ' ' + Add3 AS [Vendor Address],Closing_Date As [Closing Date] " &
                " ,Vendor_Group_Code As [Vendor Group Code],Vendor_Group_Code_Desc  AS [Vendor Group Code Desc] ,City_Code As [City Code],City_Code_Desc AS [City Code Desc], " &
                " State,Country ,Phone1 ,Phone2 ,Fax,Email ,WebSite,Contact_Person_Name As [Contact Person Name],Contact_Person_Email As [Contact Person Email] " &
                " ,Contact_Person_Phone As [Contact Person Phone],Contact_Person_Fax As [Contact Person Fax],Contact_Person_Website AS [Contact Person Website] " &
                " ,Terms_Code AS [Terms Code],Terms_Code_Desc As [Terms Code Desc],Vendor_Account As [Vendor Account],Vendor_Account_Desc As [Vendor Account Desc],Payment_Code As [Payment Code],Bank_Code As [Bank Code],Tax_Group As [Tax Group],Ven_Type_Code As [Ven Type Code],Status ,OnHold ,CURRENCY_CODE As [Currency Code],Form_Type As [Form Type],State_Code As [State Code],Country_Code AS [Country Code],IFSC_Code As [IFSC Code],Account_Type As [Account Type] from TSPL_VENDOR_MASTER left join tspl_vlc_master_Head on tspl_vlc_master_Head.vsp_code=vendor_code and mcc='" & clsCommon.myCstr(txtFromLocation.Value) & "'"
        ''richa agarwal against ticket no.BM00000004462 (add condition in finder where own asset is not true)
        txtIssueTo.Value = clsCommon.ShowSelectForm("VSPISSUE@1", qry, "Code", "  TSPL_VENDOR_MASTER.Form_Type in ('VSP','PTM') and isnull(Is_Own_Asset,'')<>'T' and TSPL_VENDOR_MASTER.Is_Inactive_In_Milk_Procurement=0", txtIssueTo.Value, "Code", isButtonClicked)

        If clsCommon.myLen(txtIssueTo.Value) > 0 Then
            lblIssueTo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT Vendor_Name as [Vendor Name] FROM TSPL_VENDOR_MASTER Where Vendor_Code='" + clsCommon.myCstr(txtIssueTo.Value) + "'"))
        Else
            lblIssueTo.Text = ""
        End If
    End Sub
    ...
    Private Sub fndReqNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean)
        ...
        '    Dim qry As String = "select Doc_No as Code,Doc_date as Date from TSPL_VSPAsset_HEAD "
    ...
    '        End If
    '    Else
    '        qry = "SELECT TSPL_VSPAsset_HEAD.Doc_Date,TSPL_VSPAsset_HEAD.From_Location,FLocation.Location_Desc as FromLocationName, " & _
    '        "TSPL_VSPAsset_HEAD.To_Location,TLocation.Location_Desc as ToLocationName,TSPL_VSPAsset_HEAD.Issue_To, " & _
    '        "IssueEmp.Emp_Name as IssueToName ,TSPL_VSPAsset_HEAD.Request_By,RequestEmp.Emp_Name as RequestByName,TSPL_VSPAsset_HEAD.vehicle_Id,Req_IssueNo " & _
    '        "FROM TSPL_VSPAsset_HEAD left outer join TSPL_LOCATION_MASTER as FLocation on  " & _
    '        "FLocation.Location_Code=TSPL_VSPAsset_HEAD.From_Location left outer join TSPL_LOCATION_MASTER as TLocation on " & _
    '        "TLocation.Location_Code=TSPL_VSPAsset_HEAD.To_Location left outer join TSPL_EMPLOYEE_MASTER as IssueEmp on " & _
    '        "IssueEmp.EMP_CODE= TSPL_VSPAsset_HEAD.issue_To left outer join  " & _
    '        "TSPL_EMPLOYEE_MASTER as RequestEmp on RequestEmp.EMP_CODE= TSPL_VSPAsset_HEAD.Request_By where TSPL_VSPAsset_HEAD.doc_no='" & fndReqNo.Value & "'"
    '        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    '        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    ...
    '    gv1.Rows.Clear()
    '    If clsCommon.CompairString(cboDocType.Text, "Issue") = CompairStringResult.Equal Then
    '        Dim Qry As String = "select Item_Code,Item_Desc,Unit_Code,Requisition_Qty - (select isnull(SUM(Issued_Qty),0) from TSPL_VSPAsset_HEAD inner join " & _
    '        "TSPL_VSPAsset_DETAIL on TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No and " & _
    '        "TSPL_VSPAsset_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Issue') + " & _
    '        "(select isnull(SUM(Issued_Qty),0) from TSPL_VSPAsset_HEAD inner join TSPL_VSPAsset_DETAIL on " & _
    '        "TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No and " & _
    '        "TSPL_VSPAsset_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Return') as Requisition_Qty  " & _
    ...
    '        End If
    '    Else
    '        Dim Qry As String = "select Item_Code,Item_Desc,Unit_code,Required_Qty,Issued_Qty from TSPL_VSPAsset_DETAIL where Doc_No='" & fndReqNo.Value & "'"
    '        Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry)
    '        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    '            For Each dr As DataRow In dt.Rows
    '                gv1.Rows.AddNew()
    ...
    Private Sub chkWithoutRefNo_ToggleStateChanged(ByVal sender As System.Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs)
        'chkWithoutRefNoChanged()
    End Sub
    ...
    Private Sub txtIssueNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtIssueNo._MYValidating
        ...
        qry = " Select Doc_No As [Code] ,  ISNULL(Issue_To,'' ) As [Issue To],vendor_name as [Issue To Name],Doc_Date As [Doc Date],From_Location As [From Location],Mcc_Name as [Mcc Name],Remarks ,Comment ," &
              " Request_By As [Request By],TSPL_VSPAsset_HEAD.Status ,On_Hold AS [On Hold] from TSPL_VSPAsset_HEAD left join tspl_mcc_master on tspl_mcc_master.mcc_code=from_location left join tspl_vendor_master on vendor_code=issue_to "

        txtIssueNo.Value = clsCommon.ShowSelectForm("VSPISSUENo", qry, "Code", " TSPL_VSPAsset_HEAD.Status =1 And Doc_Type <> 'Return'  and convert(date,doc_date,103)<=convert(date,'" & txtDate.Value & "',103) ", txtIssueNo.Value, "Code", isButtonClicked)

        If clsCommon.myLen(txtIssueNo.Value) > 0 Then
            txtIssueTo.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  ISNULL(Issue_To,'' ) As [Issue To] FROM TSPL_VSPAsset_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            If clsCommon.myLen(txtIssueTo.Value) > 0 Then
                lblIssueTo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select ISNULL( Vendor_Name,'') As Vendor_Name  from TSPL_VENDOR_MASTER where Vendor_Code ='" + clsCommon.myCstr(txtIssueTo.Value) + "'"))
            End If
            txtFromLocation.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  ISNULL(From_Location,'' ) As [From Location] FROM TSPL_VSPAsset_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            If clsCommon.myLen(txtFromLocation.Value) > 0 Then
                lblFromLocation.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  ISNULL(Location_Desc,'') As [Location Desc] FROM TSPL_LOCATION_MASTER Where Location_Code ='" + clsCommon.myCstr(txtFromLocation.Value) + "'"))
            End If
            txtRemarks.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  isnull(Remarks,'' ) As Remarks  FROM TSPL_VSPAsset_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            txtComment.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT   isnull(Comment ,'' ) As Comment  FROM TSPL_VSPAsset_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            txtRequestBy.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT   isnull(Request_By ,'' ) As Request_By  FROM TSPL_VSPAsset_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            If clsCommon.myLen(txtRequestBy.Value) > 0 Then
                lblRequestBy.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select isnull(Emp_Name,'' ) As Name from TSPL_EMPLOYEE_MASTER where EMP_CODE='" + clsCommon.myCstr(txtRequestBy.Value) + "'"))
            End If
    ...
    Private Function ReturnBalQty(ByVal strDocNo As String, ByVal strItemCode As String, ByVal strIssueNo As String) As Double
        ...
        BalQty = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select SUM(aa.Qty * RI) AS BalQty from (" & _
                " select  Item_Code, Issued_Qty As Qty,1 as RI From TSPL_VSPAsset_DETAIL where TSPL_VSPAsset_DETAIL.Doc_No='" & strIssueNo & "' " & _
                "  Union All " & _
                " select  Item_Code, Issued_Qty_againstret As Qty,-1 as RI From TSPL_VSPAsset_DETAIL " & _
                " LEFT OUTER JOIN TSPL_VSPAsset_HEAD ON TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No " & _
                " where TSPL_VSPAsset_HEAD.Issue_No='" & strIssueNo & "' and TSPL_VSPAsset_HEAD.Doc_No not IN('" & strDocNo & "') " & _
                " )  aa " & _
    ...
    Private Sub LoadGridForReturn(ByVal strIssueNo As String, Optional ByVal strDocNo As String = "")
        ...
        Dim Qry As String = "select * from  TSPL_VSPAsset_DETAIL  where Doc_No='" & strIssueNo & "'"
        'Dim Qry As String = "Select * from (select Issued_Qty - ISNULL((Select SUM(Issued_Qty_againstret) From TSPL_VSPAsset_DETAIL dtl LEFT OUTER JOIN TSPL_VSPAsset_HEAD ON TSPL_VSPAsset_HEAD.Doc_No=dtl.Doc_No WHERE TSPL_VSPAsset_HEAD.Issue_No='" & strDocNo & "' AND dtl.Item_Code=TSPL_VSPAsset_DETAIL.Item_Code ),0) AS BalQty, *  from TSPL_VSPAsset_DETAIL Where Doc_No ='" & strDocNo & "' ) XXX WHERE BalQty>0"
        'Dim DocNoDB As Double = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select Count(*) As Row from TSPL_VSPAsset_HEAD where Doc_No ='" & clsCommon.myCstr(txtDocNo.Value) & "'"))
        'If DocNoDB > 0 Then
        '    IssueNo = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select ISNULL(Issue_No,'') As Issue_No From TSPL_VSPAsset_DETAIL LEFT OUTER JOIN TSPL_VSPAsset_HEAD ON TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No Where  TSPL_VSPAsset_HEAD.Doc_No='" & strDocNo & "'"))
        'Else
        '    IssueNo = ""
        'End If
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            For Each dr As DataRow In dt.Rows
                ...
                Dim BalQty As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select SUM(aa.Qty * RI) AS BalQty from (" & _
                " select  Item_Code, Issued_Qty As Qty,1 as RI From TSPL_VSPAsset_DETAIL where TSPL_VSPAsset_DETAIL.Doc_No='" & strIssueNo & "' " & _
                "  Union All " & _
                " select  Item_Code, Issued_Qty_againstret As Qty,-1 as RI From TSPL_VSPAsset_DETAIL " & _
                " LEFT OUTER JOIN TSPL_VSPAsset_HEAD ON TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No " & _
            ...
            Next
            'Dim Arr_serial As New ArrayList
            'Qry = "select * from TSPL_SERIAL_ITEM where Document_Code='" & txtIssueNo.Value & "'"
            'dt = clsDBFuncationality.GetDataTable(Qry)
            'For Each grow As GridViewRowInfo In gv1.Rows
            '    Arr_serial.Clear()
            '    For Each row As DataRow In dt.Select("Item_Code='" & grow.Cells(colICode).Value & "'")
            '        'Arr_serial.Add(row.Item("Auto_Sr_No"))
            '        Arr_serial.Add(row)
            '    Next
            '    grow.Tag = Arr_serial
            'Next
    ...
    Private Sub BtnDeleteLayout_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnDeleteLayout.Click
        clsGridLayout.DeleteData("MilkASTGrid", objCommonVar.CurrentUserCode)
    End Sub
    ...
    Private Sub ReStoreGridLayout()
        Try
            ...
            obj = CType(obj.GetData("MilkASTGrid", "", objCommonVar.CurrentUserCode), clsGridLayout)
    ...
    Private Sub Btn_export_Click(sender As Object, e As EventArgs) Handles Btn_export.Click
        Dim Str As String = "select tspl_vspasset_Head.Doc_no as [Document Code],Doc_date as [Document Date],Issue_No as [Issue No],From_Location as [From Location]," _
                            ...
                            & " [Requested By Name],Comment,Remarks,Item_code as [Item Code],Item_Desc as [Item Desc],tspl_vspasset_detail.Cost_code as [Cost Center Code] " _
                            & " ,cost_name as  [Cost Center Name],Unit_code as [UOM],Issued_Qty as [Issue Qty]  from tspl_vspasset_Head inner join tspl_vspasset_detail on " _
                            & " tspl_vspasset_detail.doc_no=tspl_vspasset_Head.Doc_No left join TSPL_CostCenter_MASTER on TSPL_CostCenter_MASTER.cost_code=" _
                            & " tspl_vspasset_detail.cost_code left join tspl_location_master on Location_Code=From_Location left join tspl_vendor_master on" _
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.myLen(Issue_No) > 0 Then
                        qry = "select count(*) from tspl_VspAsset_Head where Doc_No='" + Issue_No + "'"
                    ...
                    gv1.Rows.AddNew()
                    gv1.Rows(0).Cells(colICode).Value = Item_Code
                    gv1.Rows(0).Cells(colIName).Value = Item_Desc
...
a:              Next
        ...
        If transportSql.importExcel(gv, "Doc Type", "Document Code", "Document Date", "Issue No", "From Location", "From Locattion Name", "VLC Uploader Code", "Issued To", "Issued To Name", "Requested By", "Requested By Name", "Comment", "Remarks", "IsLost", "Item Code", "Item Desc", "Cost Center Code", "Cost Center Name", "UOM", "Unit Cost", "Issue Qty", "Return Qty", "Serial No") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.CompairString(Doc_Type, "Return") = CompairStringResult.Equal Then
                        If clsCommon.myLen(Issue_No) > 0 Then
                            ...
                            If check <= 0 Then
                                Throw New Exception("Issue No Does Not Exist At Line No. " + clsCommon.myCstr(counter) + ",First Do Entry Of Issue No")
                            End If
                            qry = "select count(*) from tspl_VspAsset_Head where tspl_vspasset_Head.Status = 1 and Doc_No='" + Issue_No + "'"
                            check = CInt(clsDBFuncationality.getSingleValue(qry))
                            If check <= 0 Then
                    ...
                    If clsCommon.myLen(Frm_Location) > 0 Then
                        ...
                        End If
                        If clsCommon.CompairString(Doc_Type, "Return") = CompairStringResult.Equal Then
                            Dim isValidFromLocation As Boolean = clsCommon.myCBool(clsDBFuncationality.getSingleValue("select  count (*) from tspl_vspasset_Head where Doc_no ='" + Issue_No + "' and From_Location = '" + Frm_Location + "'"))
                    ...
                    If clsCommon.myLen(Issued_To) > 0 Then
                        ...
                        End If
                        If clsCommon.CompairString(Doc_Type, "Return") = CompairStringResult.Equal Then
                            Dim isValidIssueTo As Boolean = clsCommon.myCBool(clsDBFuncationality.getSingleValue("select  count (*) from tspl_vspasset_Head where Doc_no ='" + Issue_No + "' and Issue_To = '" + Issued_To + "'"))
                    ...
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells("Document Date").Value)) > 0 Then
                        ...
                        End If
                        If clsCommon.CompairString(Doc_Type, "Return") = CompairStringResult.Equal Then
                            Dim qry555 As String = "select  count (*) from tspl_vspasset_Head where Doc_no ='" + Issue_No + "'  and convert (date,Doc_Date,103) <= convert (date, '" + clsCommon.myCstr(clsCommon.myCDate(grow.Cells("Document Date").Value)) + "',103) "
                            Dim isValidReturnDate As Boolean = clsCommon.myCBool(clsDBFuncationality.getSingleValue("select  count (*) from tspl_vspasset_Head where Doc_no ='" + Issue_No + "'  and convert (date,Doc_Date,103) <= convert (date, '" + clsCommon.myCstr(clsCommon.myCDate(grow.Cells("Document Date").Value)) + "',103) "))
                    ...
                    If clsCommon.myLen(Item_Code) > 0 Then
                        ...
                        End If
                        If clsCommon.CompairString(Doc_Type, "Return") = CompairStringResult.Equal Then
                            Dim chckItemExistInIssue As Boolean = clsCommon.myCBool(clsDBFuncationality.getSingleValue("select count(*) from TSPL_VSPAsset_DETAIL where Item_Code = '" + Item_Code + "' and Doc_No = '" + Issue_No + "'"))
                    ...
                    If clsCommon.CompairString(Doc_Type, "Return") = CompairStringResult.Equal Then
                        Dim chckItemExistInIssue As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Issued_Qty from TSPL_VSPAsset_DETAIL where Item_Code = '" + Item_Code + "' and Doc_No = '" + Issue_No + "'"))
            ...
            Try
                ...
                For i As Integer = 0 To dtCust.Rows.Count - 1
                    ...
                    If dr IsNot Nothing AndAlso dr.Length > 0 Then
                        ...
                        For k As Integer = 0 To dtItem.Rows.Count - 1
                            ...
                            If drItem IsNot Nothing AndAlso drItem.Length > 0 Then
                                ...
                                '''''''''''''''''''''''''''''''''''
                                If clsCommon.CompairString(obj.Doc_Type, "Return") = CompairStringResult.Equal Then
                                    Dim dblIssueCost As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Unit_Cost  from  tspl_vspasset_detail  left outer join tspl_vspasset_Head on tspl_vspasset_Head.Doc_No = tspl_vspasset_detail.Doc_No where tspl_vspasset_Head.Doc_No = '" + obj.Issue_No + "' and tspl_vspasset_detail.Item_Code = '" + objTr.Item_Code + "' and tspl_vspasset_Head.Doc_Type = 'Issue'  ", trans))
                   ...
                   " select  Item_Code, Issued_Qty As Qty,1 as RI From TSPL_VSPAsset_DETAIL where TSPL_VSPAsset_DETAIL.Doc_No='" & obj.Issue_No & "' " &
                   "  Union All " &
                   " select  Item_Code, Issued_Qty_againstret As Qty,-1 as RI From TSPL_VSPAsset_DETAIL " &
                   " LEFT OUTER JOIN TSPL_VSPAsset_HEAD ON TSPL_VSPAsset_HEAD.Doc_No=TSPL_VSPAsset_DETAIL.Doc_No " &
                   " where TSPL_VSPAsset_HEAD.Issue_No='" & obj.Issue_No & "'  " &
                   " )  aa " &
    ...
    Private Sub rmUploderBlankSheet_Click(sender As Object, e As EventArgs) Handles rmUploderBlankSheet.Click
        Try
            Dim Str As String = " select tspl_vspasset_Head.Doc_Type as [Doc Type], tspl_vspasset_Head.Doc_no as [Document Code],Doc_date as [Document Date],Issue_No as [Issue No],From_Location as [From Location],
                            ...
                            [Requested By Name],Comment,Remarks,tspl_vspasset_detail.Item_code as [Item Code],Item_Desc as [Item Desc],tspl_vspasset_detail.Cost_code as [Cost Center Code] 
                            ,cost_name as  [Cost Center Name],Unit_code as [UOM],TSPL_VSPAsset_DETAIL.Unit_Cost as [Unit Cost] ,Issued_Qty as [Issue Qty], Issued_Qty_againstret as [Return Qty],tspl_vspasset_Head.IS_LOST as IsLost,TSPL_SERIAL_ITEM.Auto_Sr_No as [Serial No] from tspl_vspasset_Head inner join tspl_vspasset_detail on 
                             tspl_vspasset_detail.doc_no=tspl_vspasset_Head.Doc_No left join TSPL_CostCenter_MASTER on TSPL_CostCenter_MASTER.cost_code=
                             tspl_vspasset_detail.cost_code left join tspl_location_master on Location_Code=From_Location left join tspl_vendor_master on
                             tspl_vendor_master.vendor_code=Issue_To left join TSPL_EMPLOYEE_MASTER on TSPL_EMPLOYEE_MASTER.EMP_CODE=Request_By  
                             left join TSPL_SERIAL_ITEM on TSPL_SERIAL_ITEM.Document_code=tspl_vspasset_Head.Doc_no and TSPL_SERIAL_ITEM.Item_code =tspl_vspasset_detail.Item_code 
                             left join  TSPL_VLC_MASTER_HEAD on TSPL_VLC_MASTER_HEAD.VSP_Code = tspl_vspasset_Head.Issue_to and  TSPL_VLC_MASTER_HEAD.MCC =  tspl_vspasset_Head.From_Location       "
            transportSql.ExporttoExcel(Str, Me)
            Str = Nothing
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtDocNo.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowTransHistoryData(txtDocNo.Value, "Doc_No", "TSPL_VSPAsset_HEAD", "TSPL_VSPAsset_DETAIL")
        Catch ex As Exception
