'' =============Created by Rohit gupta on Nov 10,2014,BM00000007906
Imports common
...
#End Region
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.frmVSPItemIssue)
    ...
    Private Sub frmVSPItemIssue_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
    ...
    Sub LoadBlankGrid()
        ...
        repoRate.HeaderText = "Approximate Cost"
        ...
        Try
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    End If
                    If e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRetQty) OrElse e.Column Is gv1.Columns(colReqQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colCCCode) Then
                        If gv1.CurrentColumn Is gv1.Columns(colQty) Then
                            ...
                            If clsCommon.myLen(txtFromLocation.Value) <> 0 And clsCommon.myLen(clsCommon.myCdbl(gv1.CurrentRow.Cells(colICode).Value)) <> 0 Then
                                ...
                                If clsCommon.CompairString(cboDocType.Text, "Issue") = CompairStringResult.Equal And clsCommon.CompairString(btnSave.Text, "Update") = CompairStringResult.Equal Then
                                    str = "SELECT Requisition_Qty - (select isnull(SUM(Issued_Qty),0) from TSPL_VSPItem_HEAD inner join " & _
                                    "TSPL_VSPItem_DETAIL on TSPL_VSPItem_HEAD.Doc_No=TSPL_VSPItem_DETAIL.Doc_No and " & _
                                    "TSPL_VSPItem_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Issue' and " & _
                                    "TSPL_VSPItem_HEAD.Doc_No <> '" & txtDocNo.Value & "' ) +   (select isnull(SUM(Issued_Qty),0) from TSPL_VSPItem_HEAD inner join " & _
                                    "TSPL_VSPItem_DETAIL on TSPL_VSPItem_HEAD.Doc_No=TSPL_VSPItem_DETAIL.Doc_No and " & _
                                    "TSPL_VSPItem_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Return') FROM " & _
    ...
    Sub OpenBatchItem()
        If clsCommon.myCBool(clsDBFuncationality.getSingleValue("select TSPL_ITEM_MASTER.Is_Batch_Item  from TSPL_ITEM_MASTER where TSPL_ITEM_MASTER.Item_Code ='" + clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value) + "'", Nothing)) Then
            If clsCommon.CompairString("Return", clsCommon.myCstr(cboDocType.SelectedValue)) = CompairStringResult.Equal Then
    ...
    Sub OpenUOMList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            ...
            gv1.CurrentRow.Cells(colRate).Value = clsEkoPro.GetRateMccSale(txtFromLocation.Value, clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), txtDate.Value)
            UpdateCurrentRow(gv1.CurrentRow.Index)
            UpdateAllTotals()
        End If
    End Sub
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        Else
            ...
            Dim obj As clsItemMaster = clsItemMaster.FinderForItem(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), "", False, isButtonClick, "", "", "Product_Type not in ('MI') and Item_Type not in ('A') and coalesce(Item_used_as,'')='I' ")
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Item_Code) > 0 Then
                ...
                'End If
                gv1.CurrentRow.Cells(colRate).Value = clsEkoPro.GetRateMccSale(txtFromLocation.Value, clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), txtDate.Value) 'cost
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If btnSave.Text = "Update" Then
                Dim strchk As String = "select Status from TSPL_VSPItem_HEAD where Doc_No='" + txtDocNo.Value + "'"
            ...
            If clsCommon.myLen(txtRequestBy.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow(Me, "Please select Requested By", Me.Text)
                txtRequestBy.Focus()
                Return False
            End If
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If clsCommon.myLen(strICode) > 0 Then
                    ...
                    If dblEnteredQty > dblBalQty And clsCommon.CompairString(cboDocType.SelectedValue, "Return") <> CompairStringResult.Equal Then
                    ...
                    If clsCommon.myCdbl(gv1.Rows(ii).Cells(colRate).Value) <= 0 Then
                        'BHA/29/10/18-000644 Correct message
                        common.clsCommon.MyMessageBoxShow("Please Enter Approximate Cost in Mcc Item Price Chart for item : " + strICode + " . At Line No" + clsCommon.myCstr(ii + 1))
                        Return False
                    End If
    ...
    Sub SaveData(ByVal ChekPostBTn As Boolean)
        Try
            ...
            'Done by stuti on 30/11/2016 against approval work==========
            ...
            If ChekPostBTn = False Then
                If FrmMainTranScreen.ValidateTransactionAccToFinYear("VSP Item Issue", txtDate.Value) = False Then
                    Exit Sub
                End If
            End If
            ''
            If (AllowToSave()) Then
                Dim obj As New clsVSPItemIssue()
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim Rate_Mcc_Item As Double = clsEkoPro.GetRateMccSale(txtFromLocation.Value, clsCommon.myCstr(grow.Cells(colICode).Value), clsCommon.myCstr(grow.Cells(colUnit).Value), txtDate.Value)
                    Dim objTr As New clsVSPItemIssueDetail()

                    If Rate_Mcc_Item <> clsCommon.myCdbl(grow.Cells(colRate).Value) Then
                        ''======================for approval oonly========================Stuti 01/12/2016=======
                        objApproval.Item_Rate = Rate_Mcc_Item
                        objApproval.Comparision_Rate = clsCommon.myCdbl(grow.Cells(colRate).Value)
                        ''======================for approval oonly===============================
                        obj.Rate_Status = 2
                    End If
                    ...
                    ElseIf clsCommon.CompairString(obj.Doc_Type, "Return") = CompairStringResult.Equal Then
                        If clsCommon.myLen(objTr.Item_Code) > 0 AndAlso clsCommon.myCdbl(objTr.Issued_Qty_AgainstRet) > 0 Then
                            obj.Arr.Add(objTr)
                            totalqty += clsCommon.myCdbl(grow.Cells(colRetQty).Value)
                        End If
                    End If
                ...
                If (obj.SaveData(obj, isNewEntry)) Then
                    ...
                    clsApply_Approval.CheckApprovalRequired(MyBase.Form_ID, clsCommon.myCstr(obj.Doc_No), txtDate.Text, "", clsCommon.myCstr(txtRemarks.Text), clsCommon.myCdbl(lblTotRAmt.Text), clsCommon.myCdbl(totalqty), "", objApproval)
                    '================================================================
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim objApproval As New clsApply_Approval()
            Dim obj As New clsVSPItemIssue()
            obj = clsVSPItemIssue.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Doc_No) > 0) Then
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsVSPItemIssueDetail In obj.Arr
                        ...
                        Dim Rate_Mcc_Item As Double = clsEkoPro.GetRateMccSale(clsCommon.myCstr(obj.From_Location), clsCommon.myCstr(objTr.Item_Code), clsCommon.myCstr(objTr.Unit_code), obj.Doc_Date)
                        If Rate_Mcc_Item <> clsCommon.myCdbl(objTr.Unit_Cost) Then
                            objApproval.Item_Rate = Rate_Mcc_Item
                            objApproval.Comparision_Rate = clsCommon.myCdbl(objTr.Unit_Cost)
                        End If
            ...
            btnPost.Visible = MyBase.isPostFlag
            If Not clsApply_Approval.Visibility_PostButtonForApproval(MyBase.Form_ID, clsCommon.myCstr(txtDocNo.Value), clsCommon.myCdbl(lblTotRAmt.Text), 0, "", objApproval) Then
    ...
    Sub PostData()
        Try
            If (myMessages.postConfirm()) Then
                ...
                End If
                ''
                If (clsVSPItemIssue.PostData(txtDocNo.Value)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                ...
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsVSPItemIssue.DeleteData(txtDocNo.Value)) Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            Dim qst As String = "select count(*) from TSPL_VSPItem_HEAD where Doc_No='" + txtDocNo.Value + "'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        Dim qry As String = "select Doc_No as Code,Doc_Date as Date,from_location as [Mcc Code],Mcc_Name as [Mcc Name],issue_to as [Vsp Code],vendor_name " _
        & " as [VSP Name],Doc_Type as Type,case when TSPL_VSPItem_HEAD.Status='0' then 'Pending' else 'Approved' end as [Status] from TSPL_VSPItem_HEAD" _
        & " left join tspl_mcc_master on tspl_mcc_master.mcc_code=from_location left join tspl_vendor_master on vendor_code=issue_to"
    ...
    Private Sub frmVSPItemIssue_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub gv1_UserDeletedRow(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.GridViewRowEventArgs) Handles gv1.UserDeletedRow
        ...
        Next
    End Sub
    '=================Preeti gupta ticket no[BM00000004870]
    ...
    Public Sub Print()
        Try
            Dim qry As String = " Select TSPL_VSPItem_HEAD.Doc_Type,TSPL_VSPItem_HEAD.Issue_No  ,TSPL_VSPItem_HEAD.From_Location,TSPL_LOCATION_MASTER.Location_Desc , TSPL_VSPItem_HEAD.Created_By ,TSPL_VSPItem_HEAD.Modify_By ,TSPL_COMPANY_MASTER.Comp_Code ,TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Email AS Comp_Email,"

            qry += " case when ISNULL(TSPL_COMPANY_MASTER .Phone1,'')='(+__)__________' then '' else TSPL_COMPANY_MASTER.Phone1 end +  Case When   ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as  Comp_Phn,"

            qry += " TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end  + case when len(TSPL_COMPANY_MASTER.State )>0 then TSPL_COMPANY_MASTER.State else '' end  as Comp_address,TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2  ,TSPL_COMPANY_MASTER.Tin_No as Comp_Tin_No,TSPL_COMPANY_MASTER.CST_LST as Comp_CSt_LST,"

            qry += " TSPL_VENDOR_MASTER .add1 +case when len(TSPL_VENDOR_MASTER.add2)>0 then ', '+TSPL_VENDOR_MASTER.add2 else '' end +case when LEN(isnull(TSPL_VENDOR_MASTER.Add3,''))>0 then ', '+isnull(TSPL_VENDOR_MASTER.Add3,'') else ' ' end  + case when len(TSPL_VENDOR_MASTER.State )>0 then TSPL_VENDOR_MASTER.State else '' end  as Ven_address,TSPL_VENDOR_MASTER.Vendor_Code  ,TSPL_VENDOR_MASTER.Vendor_Name  ,TSPL_VENDOR_MASTER.CST as Ven_CST_No,TSPL_VENDOR_MASTER.TIN_No as Ven_Tin_no,"

            qry += " case when ISNULL(TSPL_VENDOR_MASTER .Phone1,'')='(+__)__________' then '' else TSPL_VENDOR_MASTER.Phone1 end +  Case When   ISNULL(TSPL_VENDOR_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_VENDOR_MASTER.Phone2 Else'' End as  Ven_Phn,"
            qry += " TSPL_VENDOR_MASTER.Email as Ven_Email,"

            qry += "TSPL_VSPItem_HEAD.Doc_No ,convert(varchar,TSPL_VSPItem_HEAD.Doc_Date,103) as Doc_Date ,TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_VSPItem_DETAIL.Unit_code ,TSPL_VSPItem_DETAIL.Amount ,case when TSPL_VSPItem_DETAIL.Issued_Qty_againstret > 0  then TSPL_VSPItem_DETAIL.Issued_Qty_againstret  else Issued_Qty end  as  Issued_Qty,TSPL_VSPItem_DETAIL.Unit_Cost as Rate ,  TSPL_VSPItem_DETAIL.Total_Tax_Amt ,TSPL_VSPItem_HEAD.Doc_Amt ,"
            qry += " TAX1 .Tax_Code_Desc as tax1name,isnull (TSPL_VSPItem_HEAD.tax1_amt,0) as txt1amt,       "
            qry += "   tax2.Tax_Code_Desc as tax2name,isnull (TSPL_VSPItem_HEAD.tax2_amt,0) as txt2amt, "
            qry += " tax3.Tax_Code_Desc as tax3name,isnull (TSPL_VSPItem_HEAD.tax3_amt,0) as txt3amt, "
            qry += " tax4.Tax_Code_Desc as tax4name,isnull (TSPL_VSPItem_HEAD.tax4_amt,0) as txt4amt, "
            qry += " tax5.Tax_Code_Desc as tax5name,isnull (TSPL_VSPItem_HEAD.tax5_amt,0) as txt5amt, "
            qry += "tax6.Tax_Code_Desc as tax6name,isnull (TSPL_VSPItem_HEAD.tax6_amt,0) as txt6amt, "
            qry += "  tax7.Tax_Code_Desc as tax7name,isnull (TSPL_VSPItem_HEAD.tax7_amt,0) as txt7amt, "
            qry += " tax8.Tax_Code_Desc as tax8name,isnull (TSPL_VSPItem_HEAD.tax8_amt,0) as txt8amt,  "
            qry += " tax9.Tax_Code_Desc as tax9name,isnull (TSPL_VSPItem_HEAD.tax9_amt,0) as txt9amt, "
            qry += " tax10.Tax_Code_Desc as tax10name,isnull (TSPL_VSPItem_HEAD.tax10_amt,0) as txt10amt "

            qry += " from TSPL_VSPItem_DETAIL"
            qry += " left outer join TSPL_VSPItem_HEAD on TSPL_VSPItem_HEAD.Doc_No =TSPL_VSPItem_DETAIL.Doc_No "
            qry += " left outer join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER .Comp_Code =TSPL_VSPItem_HEAD.comp_code "

            qry += " left outer join TSPL_VENDOR_MASTER  on TSPL_VENDOR_MASTER.Vendor_Code  =TSPL_VSPItem_HEAD.Issue_To  and Form_Type='VSP'"
            qry += " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code =TSPL_VSPItem_DETAIL.Item_Code "
            qry += " left outer join tspl_tax_master as tax1 on tax1.tax_code = TSPL_VSPItem_HEAD.tax1 "
            qry += " left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_VSPItem_HEAD.tax2 "
            qry += "  left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_VSPItem_HEAD .TAX3 "
            qry += " left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_VSPItem_HEAD .tax4 "
            qry += " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_VSPItem_HEAD .tax5 "
            qry += " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_VSPItem_HEAD .TAX6 "
            qry += "  left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_VSPItem_HEAD .TAX7 "
            qry += " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_VSPItem_HEAD .TAX8 "
            qry += " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_VSPItem_HEAD .TAX9 "
            qry += " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_VSPItem_HEAD .TAX10 "
            qry += " left outer join TSPL_LOCATION_MASTER  on TSPL_LOCATION_MASTER .Location_Code =TSPL_VSPItem_HEAD.From_Location "
            qry += " where 2=2 and TSPL_VSPItem_HEAD.Doc_No='" + txtDocNo.Value + "' "
    ...
    Private Sub txtIssueTo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtIssueTo._MYValidating
        Dim qry As String = ""
        qry = "select Vendor_Code AS [Code],Vendor_Name As [Vendor Name],vlc_code_vlc_uploader as [Vlc Code],Vlc_Name as [Vlc Name],Add1 + ' ' + Add2 + ' ' + Add3 AS [Vendor Address],Closing_Date As [Closing Date] " & _
    ...
    Private Sub fndReqNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean)
        ...
        '    Dim qry As String = "select Doc_No as Code,Doc_date as Date from TSPL_VSPItem_HEAD "
    ...
    '        End If
    '    Else
    '        qry = "SELECT TSPL_VSPItem_HEAD.Doc_Date,TSPL_VSPItem_HEAD.From_Location,FLocation.Location_Desc as FromLocationName, " & _
    '        "TSPL_VSPItem_HEAD.To_Location,TLocation.Location_Desc as ToLocationName,TSPL_VSPItem_HEAD.Issue_To, " & _
    '        "IssueEmp.Emp_Name as IssueToName ,TSPL_VSPItem_HEAD.Request_By,RequestEmp.Emp_Name as RequestByName,TSPL_VSPItem_HEAD.vehicle_Id,Req_IssueNo " & _
    '        "FROM TSPL_VSPItem_HEAD left outer join TSPL_LOCATION_MASTER as FLocation on  " & _
    '        "FLocation.Location_Code=TSPL_VSPItem_HEAD.From_Location left outer join TSPL_LOCATION_MASTER as TLocation on " & _
    '        "TLocation.Location_Code=TSPL_VSPItem_HEAD.To_Location left outer join TSPL_EMPLOYEE_MASTER as IssueEmp on " & _
    '        "IssueEmp.EMP_CODE= TSPL_VSPItem_HEAD.issue_To left outer join  " & _
    '        "TSPL_EMPLOYEE_MASTER as RequestEmp on RequestEmp.EMP_CODE= TSPL_VSPItem_HEAD.Request_By where TSPL_VSPItem_HEAD.doc_no='" & fndReqNo.Value & "'"
    '        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    '        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    ...
    '    gv1.Rows.Clear()
    '    If clsCommon.CompairString(cboDocType.Text, "Issue") = CompairStringResult.Equal Then
    '        Dim Qry As String = "select Item_Code,Item_Desc,Unit_Code,Requisition_Qty - (select isnull(SUM(Issued_Qty),0) from TSPL_VSPItem_HEAD inner join " & _
    '        "TSPL_VSPItem_DETAIL on TSPL_VSPItem_HEAD.Doc_No=TSPL_VSPItem_DETAIL.Doc_No and " & _
    '        "TSPL_VSPItem_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Issue') + " & _
    '        "(select isnull(SUM(Issued_Qty),0) from TSPL_VSPItem_HEAD inner join TSPL_VSPItem_DETAIL on " & _
    '        "TSPL_VSPItem_HEAD.Doc_No=TSPL_VSPItem_DETAIL.Doc_No and " & _
    '        "TSPL_VSPItem_HEAD.RequisitionNo=TSPL_REQUISITION_DETAIL.Requisition_Id where Doc_Type='Return') as Requisition_Qty  " & _
    ...
    '        End If
    '    Else
    '        Dim Qry As String = "select Item_Code,Item_Desc,Unit_code,Required_Qty,Issued_Qty from TSPL_VSPItem_DETAIL where Doc_No='" & fndReqNo.Value & "'"
    '        Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry)
    '        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    '            For Each dr As DataRow In dt.Rows
    '                gv1.Rows.AddNew()
    ...
    Private Sub btnReverse_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverse.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                If clsVSPItemIssue.ReverseAndUnpost(txtDocNo.Value) Then
    ...
    Private Sub txtIssueNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtIssueNo._MYValidating
        Dim qry As String = ""
        qry = " Select Doc_No As [Code] ,  ISNULL(Issue_To,'' ) As [Issue To],vendor_name as [Issue To Name],Doc_Date As [Doc Date],From_Location As [From Location],Mcc_Name as [Mcc Name],Remarks ,Comment ," & _
              " Request_By As [Request By],TSPL_VSPItem_HEAD.Status ,On_Hold AS [On Hold] from TSPL_VSPItem_HEAD left join tspl_mcc_master on tspl_mcc_master.mcc_code=from_location left join tspl_vendor_master on vendor_code=issue_to"

        txtIssueNo.Value = clsCommon.ShowSelectForm("VSPISSUENo", qry, "Code", " TSPL_VSPItem_HEAD.Status =1 And Doc_Type <> 'Return' and convert(date,doc_date,103)<=convert(date,'" & txtDate.Value & "',103)", txtIssueNo.Value, "Code", isButtonClicked)

        If clsCommon.myLen(txtIssueNo.Value) > 0 Then
            txtIssueTo.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  ISNULL(Issue_To,'' ) As [Issue To] FROM TSPL_VSPItem_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            If clsCommon.myLen(txtIssueTo.Value) > 0 Then
                lblIssueTo.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select ISNULL( Vendor_Name,'') As Vendor_Name  from TSPL_VENDOR_MASTER where Vendor_Code ='" + clsCommon.myCstr(txtIssueTo.Value) + "'"))
            End If
            txtFromLocation.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  ISNULL(From_Location,'' ) As [From Location] FROM TSPL_VSPItem_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            If clsCommon.myLen(txtFromLocation.Value) > 0 Then
            ...
            End If
            txtRemarks.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT  isnull(Remarks,'' ) As Remarks  FROM TSPL_VSPItem_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            txtComment.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT   isnull(Comment ,'' ) As Comment  FROM TSPL_VSPItem_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            txtRequestBy.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("SELECT   isnull(Request_By ,'' ) As Request_By  FROM TSPL_VSPItem_HEAD Where Doc_No='" + clsCommon.myCstr(txtIssueNo.Value) + "'"))
            If clsCommon.myLen(txtRequestBy.Value) > 0 Then
    ...
    Private Function ReturnBalQty(ByVal strDocNo As String, ByVal strItemCode As String, ByVal strIssueNo As String) As Double
        ...
        BalQty = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select SUM(aa.Qty * RI) AS BalQty from (" & _
                " select  Item_Code, Issued_Qty As Qty,1 as RI From TSPL_VSPItem_DETAIL where TSPL_VSPItem_DETAIL.Doc_No='" & strIssueNo & "' " & _
                "  Union All " & _
                " select  Item_Code, Issued_Qty_againstret As Qty,-1 as RI From TSPL_VSPItem_DETAIL " & _
                " LEFT OUTER JOIN TSPL_VSPItem_HEAD ON TSPL_VSPItem_HEAD.Doc_No=TSPL_VSPItem_DETAIL.Doc_No " & _
                " where TSPL_VSPItem_HEAD.Issue_No='" & strIssueNo & "' and TSPL_VSPItem_HEAD.Doc_No not IN('" & strDocNo & "') " & _
                " )  aa " & _
    ...
    Private Sub LoadGridForReturn(ByVal strIssueNo As String, Optional ByVal strDocNo As String = "")
        ...
        Dim Qry As String = "select * from  TSPL_VSPItem_DETAIL  where Doc_No='" & strIssueNo & "'"
        'Dim Qry As String = "Select * from (select Issued_Qty - ISNULL((Select SUM(Issued_Qty_againstret) From TSPL_VSPItem_DETAIL dtl LEFT OUTER JOIN TSPL_VSPItem_HEAD ON TSPL_VSPItem_HEAD.Doc_No=dtl.Doc_No WHERE TSPL_VSPItem_HEAD.Issue_No='" & strDocNo & "' AND dtl.Item_Code=TSPL_VSPItem_DETAIL.Item_Code ),0) AS BalQty, *  from TSPL_VSPItem_DETAIL Where Doc_No ='" & strDocNo & "' ) XXX WHERE BalQty>0"
        'Dim DocNoDB As Double = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select Count(*) As Row from TSPL_VSPItem_HEAD where Doc_No ='" & clsCommon.myCstr(txtDocNo.Value) & "'"))
        'If DocNoDB > 0 Then
        '    IssueNo = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select ISNULL(Issue_No,'') As Issue_No From TSPL_VSPItem_DETAIL LEFT OUTER JOIN TSPL_VSPItem_HEAD ON TSPL_VSPItem_HEAD.Doc_No=TSPL_VSPItem_DETAIL.Doc_No Where  TSPL_VSPItem_HEAD.Doc_No='" & strDocNo & "'"))
        'Else
        '    IssueNo = ""
        'End If
