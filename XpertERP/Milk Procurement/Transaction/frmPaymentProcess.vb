...
#Region "Variables"
    ...
    Public Const colFileInfo As String = "colFileInfo"
    Public Const colZone As String = "colZone"
...
#End Region
    ...
    Sub LoadBlankGridInvoice()
        ...
        colTextBox7.FormatString = ""
        colTextBox7.HeaderText = "Zone Code"
        colTextBox7.Name = colZone
        colTextBox7.Width = 200
        colTextBox7.ReadOnly = True
        gvInvoice.MasterTemplate.Columns.Add(colTextBox7)
        ...
        colTextBox = New GridViewTextBoxColumn()
        colTextBox.FormatString = ""
        colTextBox.HeaderText = "File Info"
        colTextBox.Name = colFileInfo
        colTextBox.ReadOnly = True
        colTextBox.IsVisible = False
        gvInvoice.MasterTemplate.Columns.Add(colTextBox)
    ...
    Sub LoadBlankGridGV()
        ...
        colTextBox7.Width = 200
        colTextBox7.ReadOnly = True
        gv.MasterTemplate.Columns.Add(colTextBox7)
...
where  1=1 "
            ...
            If clsCommon.myLen(fndLoc.Value) > 0 AndAlso MultipleFinderFillAuto = False Then
                qry += " And xxx.MCC_CODE in  (  select location_code from TSPL_LOCATION_MASTER where Loc_Segment_Code='" & fndLoc.Value & "')"
            End If
            If MultipleFinderFillAuto Then
                qry += " And xxx.MCC_CODE in (" + clsCommon.GetMulcallString(mfndMcc.arrValueMember) + ") "
            End If
            If clsCommon.myLen(fndLoc.Value) <= 0 AndAlso MultipleFinderFillAuto = False Then
                fndLoc.Focus()
...
group by Against_MillkPurchaseInvoice_No) as Extra on Extra.Against_MillkPurchaseInvoice_No=x.[Milk Purchase Invoice Doc No]"
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                gvInvoice.Columns(colTDSAmt).FieldName = "TDSAmount"
                ...
                gvInvoice.Columns(colFileInfo).FieldName = "FILE_INFO"
                gvInvoice.Columns(colZone).FieldName = "Zone_Code"
            End If
        Catch ex As Exception
...
where TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' and  TSPL_VENDOR_INVOICE_HEAD.Balance_Amt>0 and coalesce(refDocType,'') not in ('Milk_HE','Milk_OW','V_I_Issue_Return','COM-INC')  "
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "ALW") = CompairStringResult.Equal Then
                qry = qry & whrCls & " and isnull(TSPL_VENDOR_INVOICE_HEAD.Saving,0)=3  and " & IIf(chkSkipPrevCreditNote.Checked, " convert(date,TSPL_VENDOR_INVOICE_HEAD.Invoice_Entry_Date,103) between '" & clsCommon.GetPrintDate(dtpFromDate.Value, "dd/MMM/yyyy") & "' and '" & clsCommon.GetPrintDate(dtpToDate.Value, "dd/MMM/yyyy") & "'", " convert(date,TSPL_VENDOR_INVOICE_HEAD.Invoice_Entry_Date,103) <= '" & clsCommon.GetPrintDate(dtpToDate.Value, "dd/MMM/yyyy") & "' ")
            Else
        ...
        Try
            ...
            If obj IsNot Nothing Then
                ...
                If obj.dtPPDetail IsNot Nothing AndAlso obj.dtPPDetail.Rows.Count > 0 Then
                    ...
                    gv.Columns(colZone).FieldName = "Zone_Code"
            ...
            Try
                ...
                If Rownum <> -1 Then
                    ...
                    If rownummain <> -1 Then
                        ...
                        gv.Rows(rownummain).Cells(colPaybleAmt).Value = (((gv.Rows(rownummain).Cells(colInvAndEMPAmtAndIncenAmtAndIncenEmpAmt).Value - gv.Rows(rownummain).Cells(colTDSAmt).Value + gv.Rows(rownummain).Cells(colTotalCreditNoteAmount).Value + gv.Rows(rownummain).Cells(colVSPOwnSystemAmt).Value + gv.Rows(rownummain).Cells(colHeadLoadAmt).Value) - gv.Rows(rownummain).Cells(colInvDeduc).Value) + getTotalMccSaleReturnSum(gv.Rows(rownummain).Cells(colVendorCode).Value) - (getTotalDeductionSum(gv.Rows(rownummain).Cells(colVendorCode).Value) + getTotalItemIssueSum(gv.Rows(rownummain).Cells(colVendorCode).Value) + getTotalMccSaleSum(gv.Rows(rownummain).Cells(colVendorCode).Value))) + clsCommon.myCDecimal(gv.Rows(rownummain).Cells(colReduceDeduc).Value) + getTotalItemIssueReturnSum(gv.Rows(rownummain).Cells(colVendorCode).Value)
        ...
        Try
            ...
            ElseIf clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "RCDF") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "UDP") = CompairStringResult.Equal Then
                ...
                If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "ALW") = CompairStringResult.Equal Then
                    ...
                    clsPaymentProcessHead.PaymentProcessDrCrPrint("'" + fndDocNo.Value + "'", dtpFromDate.Text, dtpToDate.Text, "'" + fndLoc.Value + "'", "", "", "", txtMCC.Text, "", False)
                Else
                    clsPaymentProcessHead.Load_Report_Paymnet_RCDF("'" + fndDocNo.Value + "'", dtpFromDate.Text, dtpToDate.Text, "", clsCommon.GetMulcallString(txtVSP.arrValueMember), "", "", "", False)
                End If
            Else
...
where TSPL_PAYMENT_PROCESS_DETAIL.Doc_No='" + fndDocNo.Value + "' and TSPL_MILK_PURCHASE_INVOICE_HEAD.FILE_INFO is null"
    ...
    Private Sub gvInvoice_CellDoubleClick(sender As Object, e As GridViewCellEventArgs) Handles gvInvoice.CellDoubleClick
        Try
            Dim Code As String = clsCommon.myCstr(gvInvoice.CurrentRow.Cells(colFileInfo).Value)
            If clsCommon.myLen(Code) > 0 Then
                Dim filePath As String = "C:\XpertServices\XpertFileUpload\Upload\" + clsCommon.myCstr(gvInvoice.CurrentRow.Cells(colFileInfo).Value) + ".pdf" ' Replace with your file path
