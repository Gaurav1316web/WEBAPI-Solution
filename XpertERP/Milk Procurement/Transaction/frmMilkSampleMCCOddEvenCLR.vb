...
Public Class frmMilkSampleMCCOddEvenCLR
...
#Region "Variables"
    ...
    Const colPrevCLR As String = "colPrevCLR"
    ...
    Dim IsRoundOffPaiseAmount As Boolean ''SHR/14/06/18-000025 by balwinder
...
#End Region
    ...
    Private Sub frmMilkReceiptMCC_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If IsShowTwoGrid Then
            ...
            CboMachine2.SelectedValue = clsMccMaster.DefaultSampleMachine2(fndMCCCode.Tag, clsCommon.myCstr(cboDockCollectionMilkType.SelectedValue), Nothing)
            CboComport2.SelectedValue = clsMccMaster.DefaultSampleComport2(fndMCCCode.Tag, clsCommon.myCstr(cboDockCollectionMilkType.SelectedValue), Nothing)
    ...
    Function SaveData(ByVal GGGrid As RadGridView, ByVal IsPostData As Boolean) As Boolean
        ...
        Try
            ...
            For Each grow As GridViewRowInfo In GGGrid.Rows
                counter += 1
                If grow.Cells(colFAT).Value <= 0 Or grow.Cells(colCLR).Value <= 0 Then
                    Throw New Exception("Please Fill Fat And CLR in all Rows. sample no [" & grow.Index + 1 & "]")
                End If
                isCellValueChanged = False
                grow.Cells(colFAT).Value = Math.Truncate(clsCommon.myCdbl(grow.Cells(colFAT).Value) * 10) / 10
                grow.Cells(colCLR).Value = Math.Truncate(clsCommon.myCdbl(grow.Cells(colCLR).Value) * 10) / 10
                isCellValueChanged = True
                ''SHR/04/06/18-000024 by balwinder on 04/06/2018
                grow.Cells(colSNF).Value = Math.Round(clsEkoPro.getSnfOnCalculation(clsCommon.myCdbl(grow.Cells(colFAT).Value), clsCommon.myCdbl(grow.Cells(colCLR).Value), clsCommon.myCdbl(grow.Cells(colCOR).Value)), 2, MidpointRounding.ToEven)
                Dim PriceCode As String = ""
                grow.Cells(ColRATE).Value = clsEkoPro.getRateFromUploaderShiftWiseCLR(clsCommon.myCdbl(grow.Cells(colFAT).Value), clsCommon.myCdbl(grow.Cells(colCLR).Value), clsCommon.myCstr(fndMCCCode.Tag), clsCommon.myCstr(grow.Cells(ColVlcCode).Value), IIf(cboShift.SelectedValue.ToString.Contains("M"), "M", "E"), clsCommon.myCDate(dtpDocDate.Value), Nothing, clsCommon.myCstr(cboDockCollectionMilkType.SelectedValue), PriceCode)
                grow.Cells(ColPriceCode).Value = PriceCode
            ...
            If (AllowToSave(GGGrid, IsPostData)) Then
                ...
                For Each grow As GridViewRowInfo In GGGrid.Rows
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colSRNo).Value)) > 0 And clsCommon.myLen(clsCommon.myCstr(grow.Cells(ColVlcDocCode).Value)) > 0 Then
                        ...
                        If clsCommon.myCdbl(grow.Cells(colPrevFAT).Value) > 0 And ((clsCommon.myCdbl(grow.Cells(colFAT).Value) <> clsCommon.myCdbl(grow.Cells(colPrevFAT).Value)) Or (clsCommon.myCdbl(grow.Cells(colCLR).Value) <> clsCommon.myCdbl(grow.Cells(colPrevCLR).Value))) Then
                            ...
                            objH.New_CLR = clsCommon.myCdbl(grow.Cells(colCLR).Value)
                            objH.Prev_FAT = clsCommon.myCdbl(grow.Cells(colPrevFAT).Value)
                            objH.Prev_CLR = clsCommon.myCdbl(grow.Cells(colPrevCLR).Value)

                            objListHistory.Add(objH)

                            grow.Cells(colPrevCLR).Value = clsCommon.myCdbl(grow.Cells(colCLR).Value)
    ...
    Private Function AllowToSave(ByVal GGGrid As RadGridView, ByVal IsPostData As Boolean) As Boolean
        Try
            ...
            For Each grow As GridViewRowInfo In GGGrid.Rows
                totqty += clsCommon.myCdbl(grow.Cells(ColQty).Value)
                totsnf += clsCommon.myCdbl(grow.Cells(colCLR).Value) * clsCommon.myCdbl(grow.Cells(ColQty).Value) / 100
                totfat += clsCommon.myCdbl(grow.Cells(colFAT).Value) * clsCommon.myCdbl(grow.Cells(ColQty).Value) / 100
                If IsPostData Then
    ...
    Private Sub frmMilkReceiptMCC_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F2 Then
            If gv1.Rows.Count > 0 Then
                If irowIndex_Global_gv1 < 0 And (clsCommon.myCdbl(gv1.Rows(0).Cells(colCLR).Value) > 0 Or clsCommon.myCdbl(gv1.Rows(0).Cells(colCLR).Value) > 0) Then
                    For Each row As GridViewRowInfo In gv1.Rows
                        If clsCommon.myCdbl(row.Cells(colCLR).Value) > 0 Or clsCommon.myCdbl(row.Cells(colCLR).Value) > 0 Then
                            irowIndex_Global_gv1 += 1
                        Else
                            Exit For
                        End If
                ...
                If gv1.Rows.Count > irowIndex_Global_gv1 Then
                    If irowIndex_Global_gv1 >= 0 Then
                        If clsCommon.myCdbl(LblNFAT.Text) = clsCommon.myCdbl(gv1.Rows(irowIndex_Global_gv1).Cells(colFAT).Value) And clsCommon.myCdbl(LblCLR.Text) = clsCommon.myCdbl(gv1.Rows(irowIndex_Global_gv1).Cells(colCLR).Value) Then
                        ...
                        End If
                    End If
                    gv1.Rows(irowIndex_Global_gv1 + 1).Cells(colFAT).Value = clsCommon.myCdbl(LblNFAT.Text)
                    gv1.Rows(irowIndex_Global_gv1 + 1).Cells(colCLR).Value = clsCommon.myCdbl(LblCLR.Text)
                    ...
                    If irowIndex_Global_gv1 <> clsCommon.myCdbl(gv1.Rows.Count - 1) Then
                        If clsCommon.myCdbl(gv1.Rows(irowIndex_Global_gv1 + 1).Cells(colFAT).Value) > 0 And clsCommon.myCdbl(gv1.Rows(irowIndex_Global_gv1 + 1).Cells(colCLR).Value) > 0 Then
                            SaveData_Record(gv1)
                        End If
                    End If
                End If
        ...
        ElseIf e.KeyCode = Keys.F9 Then
            If gv2.Rows.Count > 0 Then
                If gv2.Rows.Count > 0 Then
                    If irowIndex_Global_gv2 < 0 And (clsCommon.myCdbl(gv2.Rows(0).Cells(colCLR).Value) > 0 Or clsCommon.myCdbl(gv2.Rows(0).Cells(colCLR).Value) > 0) Then
                    ...
                    If gv2.Rows.Count > irowIndex_Global_gv2 Then
                        If irowIndex_Global_gv2 >= 0 Then
                            If clsCommon.myCdbl(LblFatpanel2.Text) = clsCommon.myCdbl(gv2.Rows(irowIndex_Global_gv2).Cells(colFAT).Value) And clsCommon.myCdbl(LblSNFPanel2.Text) = clsCommon.myCdbl(gv2.Rows(irowIndex_Global_gv2).Cells(colCLR).Value) Then
                                If isStopForRepeatedFATSNF Then
                                    Exit Sub
                                ElseIf clsCommon.MyMessageBoxShow(Me, "Previous Row have the same reading . Are you sure to Update this in this row ?", "Message", MessageBoxButtons.YesNo, RadMessageIcon.Question, MessageBoxDefaultButton.Button2) = System.Windows.Forms.DialogResult.No Then
                                    Exit Sub
                        ...
                        gv2.Rows(irowIndex_Global_gv2 + 1).Cells(colCLR).Value = clsCommon.myCdbl(LblSNFPanel2.Text)
                        ...
                        If irowIndex_Global_gv2 <> clsCommon.myCdbl(gv2.Rows.Count - 1) Then
                            If clsCommon.myCdbl(gv2.Rows(irowIndex_Global_gv2 + 1).Cells(colFAT).Value) > 0 And clsCommon.myCdbl(gv2.Rows(irowIndex_Global_gv2 + 1).Cells(colCLR).Value) > 0 Then
                                SaveData_Record(gv2)
                            End If
                        End If
                    End If
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
            ButtonToolTip.SetToolTip(btnsave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine +
                          ...
                          "MilkSamplShowOddEvenTwoGrid" + Environment.NewLine +
    ...
    Function SaveData_Record(ByVal GGGrid As RadGridView)
        ...
        Try
            ...
            Dim totfat As Double = 0
            For Each grow As GridViewRowInfo In GGGrid.Rows
                If clsCommon.myCdbl(grow.Cells(colFAT).Value) > 0 And clsCommon.myCdbl(grow.Cells(colCLR).Value) > 0 Then
                    Dim strPriceCode As String = ""
                    grow.Cells(ColRATE).Value = clsEkoPro.getRateFromUploaderShiftWiseCLR(clsCommon.myCdbl(grow.Cells(colFAT).Value), clsCommon.myCdbl(grow.Cells(colCLR).Value), clsCommon.myCstr(fndMCCCode.Tag), clsCommon.myCstr(grow.Cells(ColVlcCode).Value), IIf(cboShift.SelectedValue.ToString.Contains("M"), "M", "E"), clsCommon.myCDate(dtpDocDate.Value), Nothing, clsCommon.myCstr(cboDockCollectionMilkType.SelectedValue), strPriceCode)
            ...
            For Each grow As GridViewRowInfo In GGGrid.Rows
                If clsCommon.myCdbl(grow.Cells(colFAT).Value) > 0 And clsCommon.myCdbl(grow.Cells(colCLR).Value) > 0 And clsCommon.myCstr(grow.Cells(colCLR).Value) <> "T" Then 'And irowIndex_Global_GGGrid = grow.Index Then
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colSRNo).Value)) > 0 And clsCommon.myLen(clsCommon.myCstr(grow.Cells(ColVlcDocCode).Value)) > 0 Then
                        Dim dr() As DataRow = DtMilkReceipt.Select("DOC_CODE='" & fndMCCCode.Value & "' and sample_nO='" & clsCommon.myCstr(grow.Cells(colSRNo).Value) & "'")
                        obj1 = New clsMilkSampleMCCDetail()
                        obj1.DOC_CODE = txtCode.Value
                ...
                If (clsMilkSampleMCC.ObjList IsNot Nothing AndAlso clsMilkSampleMCC.ObjList.Count > 0) Then
                    ...
                    For Each obj1 As clsMilkSampleMCCDetail In clsMilkSampleMCC.ObjList
                        If IsShowTwoGrid AndAlso (obj1.SAMPLE_NO Mod 2) = 0 Then
                            ...
                            gv2.Rows(gv2.Rows.Count - 1).Cells(colPrevCLR).Value = obj1.CLR
                            If clsCommon.myCdbl(obj1.FAT) > 0 And clsCommon.myCdbl(obj1.CLR) > 0 Then
                                gv2.Rows(gv2.Rows.Count - 1).Cells(colFAT).ReadOnly = True
                                gv2.Rows(gv2.Rows.Count - 1).Cells(colCLR).ReadOnly = True
                            End If
                            '========================================
            ...
            Else
                ...
                If (clsMilkReceiptMCC.ObjList IsNot Nothing AndAlso clsMilkReceiptMCC.ObjList.Count > 0) Then
                    For Each obj1 As clsMilkReceiptMCCDetail In clsMilkReceiptMCC.ObjList
                        ...
                        DtMilkReceiptEcoProWise.Rows(DtMilkReceiptEcoProWise.Rows.Count - 1).Item(colCLR) = obj1.CLR
    ...
    Sub LoadColumnReceiptDT()
        ...
        DtMilkReceiptEcoProWise.Columns.Add(colCLR)
            ...
            If DtMilkReceiptEcoProWise.Rows.Count > 0 Then
                ...
                Else
                    ...
                    For Each row As DataRow In DtMilkReceiptEcoProWise.Select("" & colECOPro & " ='' and " & colSRNo & ">=" & clsCommon.myCdbl(txtRangeFrom.Text) & " and " & colSRNo & "<=" & clsCommon.myCdbl(TxtRangeTo.Text) & "", "SRNo")
                        ...
                        If clsCommon.myCdbl(gv.Rows(gv.Rows.Count - 1).Cells(colFAT).Value) > 0 And clsCommon.myCdbl(gv.Rows(gv.Rows.Count - 1).Cells(colCLR).Value) > 0 Then
                            gv.Rows(gv.Rows.Count - 1).Cells(colFAT).ReadOnly = True
                            gv.Rows(gv.Rows.Count - 1).Cells(colCLR).ReadOnly = True
    ...
    Sub LoadBlankGrid(ByVal GGGrid As RadGridView)
        ...
        repoPrevSNF.HeaderText = "Prev. CLR"
        repoPrevSNF.Name = colPrevCLR
    ...
    Sub CellFormatNew(ByVal GGGrid As RadGridView, ByVal e As Telerik.WinControls.UI.CellFormattingEventArgs)
        Try
            ...
            If Not isInsideImportData Then
                If e.Column Is GGGrid.Columns(colFAT) Or e.Column Is GGGrid.Columns(colCLR) Then
    ...
    Sub CellValidatingNew(ByVal GGGrid As RadGridView, ByVal e As Telerik.WinControls.UI.CellValidatingEventArgs)
        Try
            If Not isInsideImportData Then
                oldValue = 0.0
                IsCanceled = False
                If (GGGrid.CurrentColumn Is GGGrid.Columns(colFAT) Or GGGrid.CurrentColumn Is GGGrid.Columns(colCLR)) And Not isInsideLoadData Then
                    If e.Value <> e.OldValue And e.OldValue > 0 Then
                        isInsideLoadData = True
                        If clsCommon.myCdbl(GGGrid.Rows.Count - 1) > GGGrid.CurrentRow.Index Then
                            If clsCommon.myCdbl(GGGrid.Rows(GGGrid.CurrentRow.Index + 1).Cells(colFAT).Value) <= 0 And clsCommon.myCdbl(GGGrid.Rows(GGGrid.CurrentRow.Index + 1).Cells(colCLR).Value) <= 0 Then
                                Exit Sub
                            End If
                        End If
                        oldValue = e.OldValue
            ...
            If Not isInsideImportData Then
                If Not isInsideLoadData Then
                    ...
                    If isCellValueChanged And e.Column Is GGGrid.Columns(colFAT) Then
                        ...
                        isCellValueChanged = True
                    End If
                    If isCellValueChanged And e.Column Is GGGrid.Columns(colCLR) Then
                        isCellValueChanged = False
                        GGGrid.Rows(irowIndex).Cells(colCLR).Value = Math.Truncate(clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colCLR).Value) * 10) / 10
                        isCellValueChanged = True
                    End If

                    isCellValueChanged = True
                    If isCellValueChanged And irowIndex <> 0 And (clsCommon.myCdbl(GGGrid.CurrentRow.Cells(colCLR).Value) <> 0 Or clsCommon.myCdbl(GGGrid.CurrentRow.Cells(colFAT).Value) <> 0) Then
                        If clsCommon.myCdbl(GGGrid.Rows(irowIndex - 1).Cells(colFAT).Value) = 0 Or clsCommon.myCdbl(GGGrid.Rows(irowIndex - 1).Cells(colCLR).Value) = 0 Then
                            ...
                            GGGrid.Rows(irowIndex).Cells(colCLR).Value = 0
                            isCellValueChanged = True
                            isInsideLoadData = False
                            Exit Sub
                        End If
                    ...
                    If Not isCellValueChangedOpen And IIf(clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.AddValidationofMilkTypeinsample, clsFixedParameterCode.AddValidationofMilkTypeinsample, Nothing)) = 0, False, True) = True Then
                        ...
                        If dr.Length > 0 Then
                            If e.Column Is GGGrid.Columns(colCLR) Then
                                If clsCommon.myCdbl(dr(0)("SNF(Min)")) > clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colCLR).Value) Or clsCommon.myCdbl(dr(0)("SNF(Max)")) < clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colCLR).Value) Then
                    ...
                    If Not isCellValueChangedOpen Then
                        isCellValueChangedOpen = True
                        If clsCommon.myCdbl(GGGrid.CurrentRow.Cells(colCLR).Value) <> 0 And clsCommon.myCdbl(GGGrid.CurrentRow.Cells(colFAT).Value) <> 0 Then
                            isInsideLoadData = True
                    ...
                    If isCellValueChanged And irowIndex <> 0 And clsCommon.CompairString(clsCommon.myCstr(e.Row.Cells(COL_IS_MANUAL).Value), clsCommon.myCstr("Auto")) = CompairStringResult.Equal Then
                        If (clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colCLR).Value) <> 0 And clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colFAT).Value) <> 0) Then
                            If clsCommon.myCdbl(GGGrid.Rows.Count - 1) > GGGrid.CurrentRow.Index Then
                                SaveData_Record(GGGrid)
                                isInsideImportData = False
                                Issaved = True
                                GGGrid.Rows(irowIndex).Cells(colFAT).ReadOnly = True
                                GGGrid.Rows(irowIndex).Cells(colCLR).ReadOnly = True
                    ...
                    ElseIf isCellValueChanged And irowIndex <> 0 And clsCommon.CompairString(clsCommon.myCstr(e.Row.Cells(COL_IS_MANUAL).Value), clsCommon.myCstr("Manual")) = CompairStringResult.Equal Then
                        If isCellValueChanged And irowIndex <> 0 And (clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colCLR).Value) <> 0 And clsCommon.myCdbl(GGGrid.Rows(irowIndex).Cells(colFAT).Value) <> 0) Then
                            If clsCommon.myCdbl(GGGrid.Rows.Count - 1) > GGGrid.CurrentRow.Index Then
                                SaveData_Record(GGGrid)
                                isInsideImportData = False
                                Issaved = True
    ...
    Private Sub BtnRead_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
            If gv1.Rows.Count > 0 Then
                ...
                LblNFAT.Text = clsEkoPro.FAT
                LblCLR.Text = clsEkoPro.SNF
            End If
        Catch ex As Exception
    ...
    Private Sub Timer1_Start()
        Try
            ...
            objSr._LblFat = LblNFAT
            objSr._LblSNF = LblCLR
    ...
    Private Sub rmExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rmExport.Click
        Dim str As String
        str = "select '1' as [Sample No],'5.5' as Fat,'8.5' as CLR,'MCUP00001' as [MCC Code],'01-Nov-2014' as Date,'Morning' as Shift"
    ...
    Private Sub rmImport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rmImport.Click
        ...
        Dim totfat As Double = 0
        Me.Controls.Add(gv)
        If transportSql.importExcel(gv, "Sample No", "Fat", "CLR", "MCC Code", "Date", "Shift") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    Dim strCLR As Double = clsCommon.myCstr(grow.Cells("CLR").Value)
                    ...
                    End If
                    If clsCommon.myLen(strCLR) <= 0 Then
                        ...
                        If gv1.Rows.Count > 0 Then
                            For Each row As GridViewRowInfo In gv1.Rows
                                If row.Cells(colSRNo).Value = strCode Then
                                    ...
                                    row.Cells(colCLR).Value = Math.Truncate(strCLR * 10) / 10
                                    row.Cells(colSNF).Value = Math.Round(clsEkoPro.getSnfOnCalculation(clsCommon.myCdbl(row.Cells(colFAT).Value), clsCommon.myCdbl(row.Cells(colCLR).Value), clsCommon.myCdbl(row.Cells(colCOR).Value)), 2, MidpointRounding.ToEven)
                                    Dim strpriceCode As String = ""
                                    row.Cells(ColRATE).Value = clsEkoPro.getRateFromUploaderShiftWiseCLR(clsCommon.myCdbl(row.Cells(colFAT).Value), clsCommon.myCdbl(row.Cells(colCLR).Value), clsCommon.myCstr(fndMCCCode.Tag), clsCommon.myCstr(row.Cells(ColVlcCode).Value), IIf(cboShift.SelectedValue.ToString.Contains("M"), "M", "E"), clsCommon.myCDate(dtpDocDate.Value), Nothing, clsCommon.myCstr(cboDockCollectionMilkType.SelectedValue), strpriceCode)
