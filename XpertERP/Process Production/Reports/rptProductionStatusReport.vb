...
Public Class rptProductionStatusReport
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        ...
        Try
            ...
            qry = " Select  Plan_Date, Plan_Code ,Item_Code,Item_Desc, case when RowNumberByPlanCode = 1 then   Plan_Qty else 0 end  as Plan_Qty, Unit_Code , Plan_Status," & _
                  " Batch_Date, Batch_Code ,  Batch_Order_Item_Code, case when RowNumberByBatchCode = 1 then  Batch_Order_Qty else 0 end Batch_Order_Qty  , Batch_Order_Unit_Code, Batch_Order_Status, " & _
                  " Issue_Date,Issue_Code, Issue_Item_Code,  case when RowNumberByIssueCode = 1 then  Issue_Qty else 0 end as Issue_Qty,  Issue_Unit_Code, Issue_Status,  " & _
                  " Standardization_Date,Standardization_Code,  Standardization_Item_Code, case when RowNumberByStandardizationCode = 1 then Standardization_Qty else 0 end as Standardization_Qty, Standardization_Unit_Code,Standardization_Status," & _
                  " QC_Date, QC_Code,QC_Item_Code, case when RowNumberByQCCode = 1 then QC_Qty else 0 end as QC_Qty , QC_Unit_Code,QC_Status, " & _
                  " STAGE_Date,STAGE_PROCESS_CODE,STAGE_Item_Code ,case when RowNumberByStageProcessCode =1 then  STAGE_Qty else 0 end as STAGE_Qty ,STAGE_Unit_Code,STAGE_Status, " & _
                  " PROD_Date,PROD_ENTRY_CODE,PROD_Item_Code,case when RowNumberByProdEntry =1 then PROD_Qty else 0 end PROD_Qty,PROD_Unit_Code,PROD_Status  " & _
                  "  from ( " & _
                  " select row_number() over (partition by TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Code order by TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Code) as RowNumberByPlanCode," & _
                  " row_number() over (partition by  TSPL_PP_BATCH_ORDER_HEAD.Batch_Code order by TSPL_PP_BATCH_ORDER_HEAD.Batch_Code) as RowNumberByBatchCode, " & _
                  " row_number() over (partition by TSPL_PP_ISSUE_HEAD.Issue_Code order by TSPL_PP_ISSUE_HEAD.Issue_Code) as RowNumberByIssueCode, " & _
                  " row_number() over (partition by TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code order by TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code) as RowNumberByStandardizationCode, " & _
                  " row_number() over (partition by TSPL_PP_STD_FINALQC_HEAD.QC_Code order by TSPL_PP_STD_FINALQC_HEAD.QC_Code) as RowNumberByQCCode, " & _
                  " row_number() over (partition by TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_CODE order by TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_CODE) as RowNumberByStageProcessCode, " & _
                  " row_number() over (partition by  TSPL_PP_PRODUCTION_ENTRY.PROD_ENTRY_CODE  order by  TSPL_PP_PRODUCTION_ENTRY.PROD_ENTRY_CODE ) as RowNumberByProdEntry, " & _
                  " convert (varchar, TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Date,103) as Plan_Date ,TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Code ,TSPL_PP_PRODUCTION_PLAN_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc,TSPL_PP_PRODUCTION_PLAN_DETAIL.Plan_Qty , TSPL_PP_PRODUCTION_PLAN_DETAIL.Unit_Code, case when  TSPL_PP_PRODUCTION_PLAN_HEAD.Is_Post  = 1 then 'APPROVED' else 'UN-APPROVED' end  as Plan_Status, " & _
                  " case when len (TSPL_PP_BATCH_ORDER_HEAD.Batch_Code) > 0 then convert (varchar, TSPL_PP_BATCH_ORDER_HEAD.Batch_Date,103) else '' end  as  Batch_Date , TSPL_PP_BATCH_ORDER_HEAD.Batch_Code,TSPL_PP_BATCH_ORDER_BOM_DETAIL.Item_Code as Batch_Order_Item_Code ,TSPL_PP_BATCH_ORDER_BOM_DETAIL.Quantity as Batch_Order_Qty, TSPL_PP_BATCH_ORDER_BOM_DETAIL.Unit_Code as Batch_Order_Unit_Code, case when len (TSPL_PP_BATCH_ORDER_HEAD.Batch_Code) > 0 then  case when  TSPL_PP_BATCH_ORDER_HEAD.Is_Post  = 1 then 'APPROVED' else 'UN-APPROVED' end  else '' end as Batch_Order_Status  " & _
                  ...
                  " , case when len (TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code)> 0 then Convert (varchar, TSPL_PP_STANDARDIZATION_HEAD.Standardization_Date,103) else '' end as Standardization_Date , TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code  , TSPL_PP_BATCH_ITEM_PRODUCTION_DETAIL.Item_Code as Standardization_Item_Code , TSPL_PP_BATCH_ITEM_PRODUCTION_DETAIL.Produced_Qty as Standardization_Qty, TSPL_PP_BATCH_ITEM_PRODUCTION_DETAIL.Unit_Code as Standardization_Unit_Code, case when len (TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code)> 0 then  case when  TSPL_PP_STANDARDIZATION_HEAD.Posted  = 1 then 'APPROVED' else 'UN-APPROVED' end else '' end  as Standardization_Status  " & _
                  ...
                  " , Case when len(TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_CODE) > 0 then Convert (varchar, TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_DATE,103) else '' end  STAGE_Date, TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_CODE,TSPL_PP_SP_BATCH_ITEM_PRODUCTION_DETAIL.Item_Code as STAGE_Item_Code  ,TSPL_PP_SP_BATCH_ITEM_PRODUCTION_DETAIL.FINAL_PROD_Qty as STAGE_Qty , TSPL_PP_SP_BATCH_ITEM_PRODUCTION_DETAIL.Unit_Code as STAGE_Unit_Code , Case when len(TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_CODE) > 0 then  case when  TSPL_PP_STAGE_PROCESS_HEAD.Posted  = 1 then 'APPROVED' else 'UN-APPROVED' end else '' end  as STAGE_Status " & _
                  " , Case when len(TSPL_PP_PRODUCTION_ENTRY.PROD_ENTRY_CODE) > 0 then Convert (varchar, TSPL_PP_PRODUCTION_ENTRY.PROD_DATE,103) else '' end  PROD_Date, TSPL_PP_PRODUCTION_ENTRY.PROD_ENTRY_CODE,TSPL_PP_PRODUCTION_ENTRY_DETAIL.Item_Code as PROD_Item_Code  ,TSPL_PP_PRODUCTION_ENTRY_DETAIL.Final_Production_Qty as PROD_Qty , TSPL_PP_PRODUCTION_ENTRY_DETAIL.Unit_Code as PROD_Unit_Code , Case when len(TSPL_PP_PRODUCTION_ENTRY.PROD_ENTRY_CODE) > 0 then  case when  TSPL_PP_PRODUCTION_ENTRY.Posted  = 1 then 'APPROVED' else 'UN-APPROVED' end  else '' end as PROD_Status " & _
                  " from TSPL_PP_PRODUCTION_PLAN_DETAIL " & _
                  " inner join TSPL_PP_PRODUCTION_PLAN_HEAD on TSPL_PP_PRODUCTION_PLAN_DETAIL.Plan_Code = TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Code " & _
                  " left outer Join TSPL_PP_BATCH_ORDER_HEAD on TSPL_PP_BATCH_ORDER_HEAD.Plan_Code = TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Code " & _
                  " left outer Join TSPL_PP_BATCH_ORDER_BOM_DETAIL on TSPL_PP_BATCH_ORDER_BOM_DETAIL.Batch_Code = TSPL_PP_BATCH_ORDER_HEAD.Batch_Code " & _
                  " left outer join TSPL_PP_ISSUE_HEAD on TSPL_PP_ISSUE_HEAD.Batch_Code = TSPL_PP_BATCH_ORDER_HEAD.Batch_Code " & _
                  " left outer join TSPL_PP_STANDARDIZATION_HEAD on TSPL_PP_STANDARDIZATION_HEAD.Child_Batch_Code = TSPL_PP_BATCH_ORDER_HEAD.Batch_Code " & _
                  " left Outer Join TSPL_PP_BATCH_ITEM_PRODUCTION_DETAIL on TSPL_PP_BATCH_ITEM_PRODUCTION_DETAIL.Standardization_Code = TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code  " & _
                  " left outer join TSPL_PP_STD_FINALQC_HEAD on TSPL_PP_STD_FINALQC_HEAD.Child_Batch_Code = TSPL_PP_BATCH_ORDER_HEAD.Batch_Code And TSPL_PP_STD_FINALQC_HEAD.Against_STD_Code = TSPL_PP_STANDARDIZATION_HEAD.Standardization_Code  " & _
                  " left Outer Join TSPL_PP_STD_FINALQC_BATCH_ITEM_DETAIL on TSPL_PP_STD_FINALQC_BATCH_ITEM_DETAIL.QC_Code = TSPL_PP_STD_FINALQC_HEAD.QC_Code " & _
                  " left Outer Join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code = TSPL_PP_PRODUCTION_PLAN_DETAIL.Item_Code " & _
                  " left outer join TSPL_PP_STAGE_PROCESS_HEAD on TSPL_PP_STAGE_PROCESS_HEAD.Main_Batch_Code = TSPL_PP_BATCH_ORDER_HEAD.Batch_Code " & _
                  " left outer join TSPL_PP_SP_BATCH_ITEM_PRODUCTION_DETAIL on TSPL_PP_SP_BATCH_ITEM_PRODUCTION_DETAIL.STAGE_PROCESS_CODE = TSPL_PP_STAGE_PROCESS_HEAD.STAGE_PROCESS_CODE " & _
                  " left outer join TSPL_PP_PRODUCTION_ENTRY on TSPL_PP_PRODUCTION_ENTRY.Batch_Code = TSPL_PP_BATCH_ORDER_HEAD.Batch_Code " & _
                  ...
                  " where convert(date,TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Date ,103)>=convert(date,'" + fromDate.Value + "',103) and convert(date,TSPL_PP_PRODUCTION_PLAN_HEAD.Plan_Date ,103) <=convert(date,'" + ToDate.Value + "' ,103) " & _
    ...
    Sub View()
        If Gv1.Rows.Count > 0 Then
            ...
            'Plan_Date, Plan_Code , Plan_Qty, Item_Code, Unit_Code , Plan_Status
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Plan_Date"))
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Plan_Code"))
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Item_Code"))
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Item_Desc"))
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Plan_Qty"))
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Unit_Code"))
            view.ColumnGroups(0).Rows(0).Columns.Add(Gv1.Columns("Plan_Status"))

            view.ColumnGroups.Add(New GridViewColumnGroup("Batch Order"))
            view.ColumnGroups(1).Rows.Add(New GridViewColumnGroupRow())
            ' Batch_Date, Batch_Code , Batch_Order_Qty , Batch_Order_Item_Code , Batch_Order_Unit_Code, Batch_Order_Status
            view.ColumnGroups(1).Rows(0).Columns.Add(Gv1.Columns("Batch_Date"))
            view.ColumnGroups(1).Rows(0).Columns.Add(Gv1.Columns("Batch_Code"))
            view.ColumnGroups(1).Rows(0).Columns.Add(Gv1.Columns("Batch_Order_Item_Code"))
            view.ColumnGroups(1).Rows(0).Columns.Add(Gv1.Columns("Batch_Order_Qty"))
            view.ColumnGroups(1).Rows(0).Columns.Add(Gv1.Columns("Batch_Order_Unit_Code"))
            view.ColumnGroups(1).Rows(0).Columns.Add(Gv1.Columns("Batch_Order_Status"))

            view.ColumnGroups.Add(New GridViewColumnGroup("Production Issue Entry"))
            view.ColumnGroups(2).Rows.Add(New GridViewColumnGroupRow())
            ' Issue_Date, Issue_Code, Issue_Qty, Issue_Item_Code, Issue_Unit_Code, Issue_Status
            view.ColumnGroups(2).Rows(0).Columns.Add(Gv1.Columns("Issue_Date"))
            view.ColumnGroups(2).Rows(0).Columns.Add(Gv1.Columns("Issue_Code"))
            view.ColumnGroups(2).Rows(0).Columns.Add(Gv1.Columns("Issue_Item_Code"))
            view.ColumnGroups(2).Rows(0).Columns.Add(Gv1.Columns("Issue_Qty"))
            view.ColumnGroups(2).Rows(0).Columns.Add(Gv1.Columns("Issue_Unit_Code"))
            view.ColumnGroups(2).Rows(0).Columns.Add(Gv1.Columns("Issue_Status"))
            ...
            If rdbSFGItem.Checked = True Then
                ...
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("Standardization_Date"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("Standardization_Code"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("Standardization_Item_Code"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("Standardization_Qty"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("Standardization_Unit_Code"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("Standardization_Status"))

                ' QC_Date, QC_Code,QC_Qty , QC_Item_Code, QC_Unit_Code,QC_Status
                view.ColumnGroups.Add(New GridViewColumnGroup("Production Standardization Final QC"))
                view.ColumnGroups(4).Rows.Add(New GridViewColumnGroupRow())
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("QC_Date"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("QC_Code"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("QC_Item_Code"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("QC_Qty"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("QC_Unit_Code"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("QC_Status"))
            Else
                ' STAGE_Date,STAGE_PROCESS_CODE ,STAGE_Qty, STAGE_Item_Code,STAGE_Unit_Code,STAGE_Status
                view.ColumnGroups.Add(New GridViewColumnGroup("Stage Process"))
                view.ColumnGroups(3).Rows.Add(New GridViewColumnGroupRow())
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("STAGE_Date"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("STAGE_PROCESS_CODE"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("STAGE_Item_Code"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("STAGE_Qty"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("STAGE_Unit_Code"))
                view.ColumnGroups(3).Rows(0).Columns.Add(Gv1.Columns("STAGE_Status"))

                ' ' PROD_Date,PROD_ENTRY_CODE,PROD_Qty,PROD_Item_Code,PROD_Unit_Code,PROD_Status
                view.ColumnGroups.Add(New GridViewColumnGroup("Production Entry"))
                view.ColumnGroups(4).Rows.Add(New GridViewColumnGroupRow())
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("PROD_Date"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("PROD_ENTRY_CODE"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("PROD_Item_Code"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("PROD_Qty"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("PROD_Unit_Code"))
                view.ColumnGroups(4).Rows(0).Columns.Add(Gv1.Columns("PROD_Status"))
            End If
