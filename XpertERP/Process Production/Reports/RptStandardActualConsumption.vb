...
Public Class rptStandardActualConsumption
    ...
    Sub Print(ByVal IsPrint As Exporter)
        ...
        Dim Query As String = " select [Batch No],[Main Item],[Main Item Desc],UOM,case when RowNumber=1 then [Batch Main Qty] else 0 end [Batch Main Qty],case when RowNumber=1 then Main_item_cost else 0 end [Main item cost],[BOM No],case when RowNumber =1 then [BOM Qty] else 0 end [BOM Qty],[BOM UOM], [BOM Item],tspl_item_master.item_desc as [BOM Item Desc],[Standard Qty],[Item UOM], [Required Qty],[Issue Qty],[Total Cost], [Consumed Qty],[Consumed Value],AddRemoveQty,AddRemoveValue,Location_Code,convert(decimal(18,3),[Issue Difference Qty]) as [Issue Difference Qty],convert(decimal(18,3),[Difference Qty]) as [Difference Qty] from (" + Environment.NewLine +
         ...
         " ,isnull([Required Qty],0) as [Required Qty],isnull(Issue_Item.Issue_Qty,0) as [Issue Qty],isnull(Issue_Item.[Total Cost],0) as [Total Cost]" + Environment.NewLine +
         ...
         " left outer join (select Item_Code as RequiredItem,Quantity as [Required Qty],TSPL_PP_BATCH_ORDER_HEAD.Batch_Code as [BatchCode],TSPL_PP_BATCH_ORDER_HEAD.Location_Code from TSPL_PP_BATCH_ORDER_HEAD" + Environment.NewLine +
...
"where TSPL_PP_STD_ADD_REMOVE_ITEM_DETAIL.ADD_REMOVE_TYPE='Remove' )x group by Batch_Code,CONSM_ITEM_CODE)TSPL_PP_PRODUCTION_ENTRY on TSPL_PP_PRODUCTION_ENTRY.Batch_Code=TSPL_PP_BATCH_ORDER_BOM_DETAIL.Batch_Code" + Environment.NewLine +
        ...
        " left join (   select Issue_Code,sum(Issue_Qty) as Issue_Qty,Batch_Code,Item_Code,sum([Total Cost]) as [Total Cost] from (" + Environment.NewLine +
 ...
 ")x group by Issue_Code,Batch_Code,item_code" + Environment.NewLine +
    ...
    Sub SetGridFormationOFGV1()
        ...
        Dim item8 As New GridViewSummaryItem("Total Cost", "{0:F2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item8)

        Dim item9 As New GridViewSummaryItem("Main item cost", "{0:F2}", GridAggregateFunction.Sum)
