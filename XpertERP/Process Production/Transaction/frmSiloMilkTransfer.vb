'---Richa Agarwal---Ticket No.-BHA/24/07/18-000190--24/07/2018,BHA/15/08/18-000427 (multiple silo selects on grid)
'' against Ticket no.BHA/10/08/18-000412 t be done.
Imports System
Imports System.Data
Imports System.Data.SqlClient
Imports common
Public Class frmSiloMilkTransfer
...
#End Region
        ...
        Try
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colSiloCode) OrElse e.Column Is gv1.Columns(colFATPers) OrElse e.Column Is gv1.Columns(colSNFPers) Then
    ...
    " select Location_Code as [Code],Location_Desc as [Description],TSPL_Location_MASTER.Loc_Short_Name as [Short Name],Add1,Add2,Add3,Add4,City_Code as [City Code],State,Pin_Code as [Pin Code],Country,Hoadd1 ,Hoadd2,Telphone,Email,Location_Type as [Location Type],Loc_Status as [Location Status],Status_Date as [Status Date],Excisable,Loc_Segment_Code as [Location Segment Code],Seg.Description as [Location Segment Description],Type,Purchase_Tax_Group as [Purchase Tax Group],Sales_Tax_Group as [Sales Tax Group],Ecc_Number as [ECC Number],Registration_Number as [Registration Number],Commissionerate as [Commission Rate],Range_Code as [Range Code],Range_Name as [Range Name],Range_Address as [Range Address],Division_Code as [Division Code],Division_Name as [Division Name],Division_Address as [Division Address],tspl_location_master.Created_By as [Created By],tspl_location_master.Created_Date as [Created Date],tspl_location_master.Modify_By as [Modify By],tspl_location_master.Modify_Date as [Modify Date],tspl_location_master.Comp_code as [Company Code],TIN_No as [TIN No],TAN_No as [TAN No],TCAN_No as [TCAN No],Service_Tax_Reg_No as [Service Tax Registration No],DutyPaid as [Duty Paid],Purchase_Tax_GroupIS as [Purchase Tax Group Inter State],Sales_Tax_GroupIS as [Sales Tax Group Inter State],Stock_Transfer_Filled_Ac as [Stock Transfer Filled Account],Stock_Transfer_Empty_Ac as [Stock Transfer Empty Account],GIT_Location as [GIT Location],GIT_Type as [GIT Type],Rejected_Type as [Rejected Type],Rejected_Location as [Rejected Location],CSA_Type as [CSA Type],Cust_Code as [Cust Code],CST_No as [CST No],Phone1,Phone2,stock_transfer_ac as [Stock Tranfer A/C],Loss_Ac as [Loss A/C] ,Is_Consumption_Location as [Is Consumption Location],Is_Section as [Is Section],Section_Code as [Section Code],Is_Sub_Location as [Is Sub Location],Main_Location_Code as [Main Location Code] from TSPL_Location_MASTER  left join TSPL_GL_SEGMENT_CODE as Seg on TSPL_Location_MASTER.Loc_Segment_Code=Seg.Segment_Code   where  (Location_Type='Physical' and Is_Sub_Location='Y' and Is_Section ='N' and (isnull(Is_Jobwork,0)=1)) or (Location_Type='Physical' and Is_Sub_Location='N' and Is_Section ='N' and (isnull(Is_Jobwork,0)=0)) and Location_Code ='" + FndMainLocation.Value + "' )z"
            ...
            Else
                If clsCommon.myLen(FndMainLocation.Value) > 0 Then
                    gv1.CurrentRow.Cells(colSiloCode).Value = clsLocation.getFinder(" (isnull(is_sub_location,'N')='Y' or isnull(Is_Section,'N')='Y') and Main_Location_Code='" + FndMainLocation.Value + "'", gv1.CurrentRow.Cells(colSiloCode).Value, isButtonClick)
                    If clsCommon.myLen(gv1.CurrentRow.Cells(colSiloCode).Value) > 0 Then
                        gv1.CurrentRow.Cells(colSiloName).Value = clsLocation.GetName(gv1.CurrentRow.Cells(colSiloCode).Value, Nothing)
                    Else
                        gv1.CurrentRow.Cells(colSiloName).Value = ""
        ...
        Try
            If clsCommon.myLen(FndMainLocation.Value) > 0 Then
                ...
                If gv1.CurrentRow.Cells(colFATPers).Value > 0 Then
                    gv1.CurrentRow.Cells(colFATKG).Value = Math.Round(dblQty * gv1.CurrentRow.Cells(colFATPers).Value / 100, 3)
                End If
                If (gv1.CurrentRow.Cells(colSNFPers).Value) > 0 Then
                    gv1.CurrentRow.Cells(colSNFKG).Value = Math.Round(dblQty * gv1.CurrentRow.Cells(colSNFPers).Value / 100, 3)
                End If
            End If
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(ex.Message)
    ...
    Sub UpdateFatSnfPer(ByVal isButtonClick As Boolean)
        Try
            ''richa BHA/11/09/18-000537 11 Sep,2018
            ...
            If clsCommon.myLen(FndMainLocation.Value) > 0 And ((clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value) = dblBalQty) OrElse (gv1.CurrentRow.Cells(colFATPers).Value = 0 AndAlso gv1.CurrentRow.Cells(colSNFPers).Value = 0)) Then
                'Dim obj As ClsSiloMilkTransfer

                ''richa GKD/28/02/19-000177 
                If AllowSiloMilkTransfertoMainLocation = True Then
                    ...
                    Else
                        'Balwinder on 21/06/2020
                        'dt = clsInventoryMovementNew.getBalance_FatAndSnfKG(gv1.CurrentRow.Cells(colICode).Value, txtSilo.Value, gv1.CurrentRow.Cells(colSiloCode).Value, txtAdjustmentNo.Value, txtDate.Value, Nothing, gv1.CurrentRow.Cells(colUnit).Value)
                ...
                Else
                    dt = clsInventoryMovementNew.getBalance_FatAndSnfKG(gv1.CurrentRow.Cells(colICode).Value, FndMainLocation.Value, gv1.CurrentRow.Cells(colSiloCode).Value, txtAdjustmentNo.Value, txtDate.Value, Nothing, gv1.CurrentRow.Cells(colUnit).Value)
                End If
                ''richa agarwal BHA/10/10/19-000931
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    If clsCommon.myCdbl(dt.Rows(0)("FAT_KG")) < 0 Then
                        gv1.CurrentRow.Cells(colFATPers).Value = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select iSNULL(ACTUAL_RANGE,0) from TSPL_ITEM_QC_PARAMETER_MASTER LEFT OUTER JOIN TSPL_PARAMETER_MASTER ON TSPL_PARAMETER_MASTER.Code =TSPL_ITEM_QC_PARAMETER_MASTER.CODE where TSPL_ITEM_QC_PARAMETER_MASTER.item_code='" & gv1.CurrentRow.Cells(colICode).Value & "' AND TSPL_PARAMETER_MASTER.Type='FAT' "))
                        If clsCommon.myCdbl(gv1.CurrentRow.Cells(colFATPers).Value) = 0 Then
                            Throw New Exception("Plz fill fat% in QC Detail tab on Item Master.")
                        End If
                    Else
                        gv1.CurrentRow.Cells(colFATKG).Value = Math.Round(clsCommon.myCdbl(dt.Rows(0)("FAT_KG")), 3)
                    End If
                    If clsCommon.myCdbl(dt.Rows(0)("SNF_KG")) < 0 Then
                        gv1.CurrentRow.Cells(colSNFPers).Value = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select iSNULL(ACTUAL_RANGE,0) from TSPL_ITEM_QC_PARAMETER_MASTER LEFT OUTER JOIN TSPL_PARAMETER_MASTER ON TSPL_PARAMETER_MASTER.Code =TSPL_ITEM_QC_PARAMETER_MASTER.CODE where TSPL_ITEM_QC_PARAMETER_MASTER.item_code='" & gv1.CurrentRow.Cells(colICode).Value & "' AND TSPL_PARAMETER_MASTER.Type='SNF' "))
                        If clsCommon.myCdbl(gv1.CurrentRow.Cells(colSNFPers).Value) = 0 Then
                            Throw New Exception("Plz fill SNF% in QC Detail tab on Item Master.")
                        End If
                    Else
                        gv1.CurrentRow.Cells(colSNFKG).Value = Math.Round(clsCommon.myCdbl(dt.Rows(0)("SNF_KG")), 3)
                    End If

                    Dim dblQty As Double = clsItemMaster.GetKGConvQty(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value))
                    If dblQty > 0 Then
                        If clsCommon.myCdbl(dt.Rows(0)("FAT_KG")) < 0 Then
                            gv1.CurrentRow.Cells(colFATKG).Value = Math.Round(gv1.CurrentRow.Cells(colFATPers).Value * dblQty / 100, 3)
                        Else
                            gv1.CurrentRow.Cells(colFATPers).Value = Math.Round(clsCommon.myCdbl(dt.Rows(0)("FAT_KG")) * 100 / dblQty, 3)
                        End If

                        If clsCommon.myCdbl(dt.Rows(0)("SNF_KG")) < 0 Then
                            gv1.CurrentRow.Cells(colSNFKG).Value = Math.Round(gv1.CurrentRow.Cells(colSNFPers).Value * dblQty / 100, 3)
                        Else
                            gv1.CurrentRow.Cells(colSNFPers).Value = Math.Round(clsCommon.myCdbl(dt.Rows(0)("SNF_KG")) * 100 / dblQty, 3)
                        End If
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Item_Code) > 0 Then
            ...
            gv1.CurrentRow.Cells(colUnit).Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select top 1 isnull(UOM_Code,'') from TSPL_ITEM_UOM_DETAIL where Item_Code ='" & obj.Item_Code & "' and Stocking_Unit='Y'", Nothing))
        Else
        ...
        Try
            ...
            If clsCommon.myLen(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value)) > 0 AndAlso clsCommon.myLen(clsCommon.myCstr(gv1.CurrentRow.Cells(colSiloCode).Value)) > 0 Then
                ...
                If AllowSiloMilkTransfertoMainLocation = True Then
                    ...
                    Else
                        'Balwinder on 21/06/2020
                        'BalQty = clsCommon.myCdbl(ClsLoadingTanker.getBalance(gv1.CurrentRow.Cells(colICode).Value, txtSilo.Value, gv1.CurrentRow.Cells(colSiloCode).Value, txtAdjustmentNo.Value, txtDate.Value, Nothing, gv1.CurrentRow.Cells(colUnit).Value))
    ...
    Private Sub txtLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSilo._MYValidating
        Try
            If AllowSiloMilkTransfertoMainLocation = True Then
                ''richa agarwal 29 Aug,2019 show only those segments whose segment no is 7 i.e only for locations
                Dim qryforsilo As String = "select * from (select Location_Code as [Code],Location_Desc as [Description],TSPL_Location_MASTER.Loc_Short_Name as [Short Name],Add1,Add2,Add3,Add4,City_Code as [City Code],State,Pin_Code as [Pin Code],Country,Hoadd1 ,Hoadd2,Telphone,Email,Location_Type as [Location Type],Loc_Status as [Location Status],Status_Date as [Status Date],Excisable,Loc_Segment_Code as [Location Segment Code],Seg.Description as [Location Segment Description],Type,Purchase_Tax_Group as [Purchase Tax Group],Sales_Tax_Group as [Sales Tax Group],Ecc_Number as [ECC Number],Registration_Number as [Registration Number],Commissionerate as [Commission Rate],Range_Code as [Range Code],Range_Name as [Range Name],Range_Address as [Range Address],Division_Code as [Division Code],Division_Name as [Division Name],Division_Address as [Division Address],tspl_location_master.Created_By as [Created By],tspl_location_master.Created_Date as [Created Date],tspl_location_master.Modify_By as [Modify By],tspl_location_master.Modify_Date as [Modify Date],tspl_location_master.Comp_code as [Company Code],TIN_No as [TIN No],TAN_No as [TAN No],TCAN_No as [TCAN No],Service_Tax_Reg_No as [Service Tax Registration No],DutyPaid as [Duty Paid],Purchase_Tax_GroupIS as [Purchase Tax Group Inter State],Sales_Tax_GroupIS as [Sales Tax Group Inter State],Stock_Transfer_Filled_Ac as [Stock Transfer Filled Account],Stock_Transfer_Empty_Ac as [Stock Transfer Empty Account],GIT_Location as [GIT Location],GIT_Type as [GIT Type],Rejected_Type as [Rejected Type],Rejected_Location as [Rejected Location],CSA_Type as [CSA Type],Cust_Code as [Cust Code],CST_No as [CST No],Phone1,Phone2,stock_transfer_ac as [Stock Tranfer A/C],Loss_Ac as [Loss A/C] ,Is_Consumption_Location as [Is Consumption Location],Is_Section as [Is Section],Section_Code as [Section Code],Is_Sub_Location as [Is Sub Location],Main_Location_Code as [Main Location Code] from TSPL_Location_MASTER  left join TSPL_GL_SEGMENT_CODE as Seg on TSPL_Location_MASTER.Loc_Segment_Code=Seg.Segment_Code and Seg.Seg_No='7'  where  (isnull(is_sub_location,'N')='Y' or isnull(Is_Section,'N')='Y') and Main_Location_Code='" + FndMainLocation.Value + "' " & Environment.NewLine &
    "                union all " & Environment.NewLine &
    " select Location_Code as [Code],Location_Desc as [Description],TSPL_Location_MASTER.Loc_Short_Name as [Short Name],Add1,Add2,Add3,Add4,City_Code as [City Code],State,Pin_Code as [Pin Code],Country,Hoadd1 ,Hoadd2,Telphone,Email,Location_Type as [Location Type],Loc_Status as [Location Status],Status_Date as [Status Date],Excisable,Loc_Segment_Code as [Location Segment Code],Seg.Description as [Location Segment Description],Type,Purchase_Tax_Group as [Purchase Tax Group],Sales_Tax_Group as [Sales Tax Group],Ecc_Number as [ECC Number],Registration_Number as [Registration Number],Commissionerate as [Commission Rate],Range_Code as [Range Code],Range_Name as [Range Name],Range_Address as [Range Address],Division_Code as [Division Code],Division_Name as [Division Name],Division_Address as [Division Address],tspl_location_master.Created_By as [Created By],tspl_location_master.Created_Date as [Created Date],tspl_location_master.Modify_By as [Modify By],tspl_location_master.Modify_Date as [Modify Date],tspl_location_master.Comp_code as [Company Code],TIN_No as [TIN No],TAN_No as [TAN No],TCAN_No as [TCAN No],Service_Tax_Reg_No as [Service Tax Registration No],DutyPaid as [Duty Paid],Purchase_Tax_GroupIS as [Purchase Tax Group Inter State],Sales_Tax_GroupIS as [Sales Tax Group Inter State],Stock_Transfer_Filled_Ac as [Stock Transfer Filled Account],Stock_Transfer_Empty_Ac as [Stock Transfer Empty Account],GIT_Location as [GIT Location],GIT_Type as [GIT Type],Rejected_Type as [Rejected Type],Rejected_Location as [Rejected Location],CSA_Type as [CSA Type],Cust_Code as [Cust Code],CST_No as [CST No],Phone1,Phone2,stock_transfer_ac as [Stock Tranfer A/C],Loss_Ac as [Loss A/C] ,Is_Consumption_Location as [Is Consumption Location],Is_Section as [Is Section],Section_Code as [Section Code],Is_Sub_Location as [Is Sub Location],Main_Location_Code as [Main Location Code] from TSPL_Location_MASTER  left join TSPL_GL_SEGMENT_CODE as Seg on TSPL_Location_MASTER.Loc_Segment_Code=Seg.Segment_Code and Seg.Seg_No='7'  where  (Location_Type='Physical' and Is_Sub_Location='Y' and Is_Section ='N' and (isnull(Is_Jobwork,0)=1)) or (Location_Type='Physical' and Is_Sub_Location='N' and Is_Section ='N' and (isnull(Is_Jobwork,0)=0)) and Location_Code ='" + FndMainLocation.Value + "')z"
    ...
    Private Function AllowToSave(ByVal blnPost As Boolean) As Boolean
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            ...
            If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.myLen(strSiloCode) <= 0 Then
                Throw New Exception("Please enter Silo at Row No " + clsCommon.myCstr(ii + 1))
            End If
            ...
            If clsCommon.myLen(strSiloCode) > 0 Then
                ...
                For jj As Integer = 0 To gv1.Rows.Count - 1
                    If jj = ii Then
                        Continue For
                    End If
                    Dim strInnerSiloCode As String = clsCommon.myCstr(gv1.Rows(jj).Cells(colSiloCode).Value)
                ...
                If blnPost = True Then
                    ...
                    If clsCommon.myCdbl(gv1.Rows(ii).Cells(colFATPers).Value) > dblCurrentFatPerMax OrElse clsCommon.myCdbl(gv1.Rows(ii).Cells(colFATPers).Value) < dblCurrentFatPerMin Then
                        gv1.Rows(ii).Cells(colFATPers).Value = objPer.FAT_Per
                    End If
                    If clsCommon.myCdbl(gv1.Rows(ii).Cells(colSNFPers).Value) > dblCurrentSNFPerMax OrElse clsCommon.myCdbl(gv1.Rows(ii).Cells(colSNFPers).Value) < dblCurrentSNFPerMin Then
                        gv1.Rows(ii).Cells(colSNFPers).Value = objPer.SNF_Per
                    End If
                End If
                ...
                If clsCommon.CompairString(clsFixedParameter.GetData(clsFixedParameterType.AllowtoNegativeStockInventoryAtTankerDispatch, clsFixedParameterCode.AllowtoNegativeStockInventoryAtTankerDispatch, Nothing), "0") = CompairStringResult.Equal Then
                    ...
                    If clsCommon.CompairString(FndMainLocation.Value, strSiloCode) = CompairStringResult.Equal Then
                        BalQty = clsCommon.myCdbl(ClsLoadingTanker.getBalance(strICode, strSiloCode, "", txtAdjustmentNo.Value, txtDate.Value, Nothing, strUOM))
                    Else
                        BalQty = clsCommon.myCdbl(ClsLoadingTanker.getBalance(strICode, FndMainLocation.Value, strSiloCode, txtAdjustmentNo.Value, txtDate.Value, Nothing, strUOM))
                    End If
                    ...
                    Else
                        Dim dt As DataTable = Nothing
                        If AllowSiloMilkTransfertoMainLocation = True Then
                            Dim strSubLocation As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue(" Select isnull(is_sub_location,'N') from TSPL_LOCATION_MASTER where Location_Code ='" & clsCommon.myCstr(gv1.Rows(ii).Cells(colSiloCode).Value) & "'"))
                            If clsCommon.CompairString(strSubLocation, "N") = CompairStringResult.Equal Then        ''------------means location is main location
                                dt = clsInventoryMovementNew.getBalance_FatAndSnfKG(gv1.Rows(ii).Cells(colICode).Value, gv1.Rows(ii).Cells(colSiloCode).Value, "", txtAdjustmentNo.Value, txtDate.Value, Nothing, gv1.Rows(ii).Cells(colUnit).Value)
                            Else
                                dt = clsInventoryMovementNew.getBalance_FatAndSnfKG(gv1.Rows(ii).Cells(colICode).Value, FndMainLocation.Value, gv1.Rows(ii).Cells(colSiloCode).Value, txtAdjustmentNo.Value, txtDate.Value, Nothing, gv1.Rows(ii).Cells(colUnit).Value)
                            End If
                        Else
                            dt = clsInventoryMovementNew.getBalance_FatAndSnfKG(gv1.Rows(ii).Cells(colICode).Value, FndMainLocation.Value, gv1.Rows(ii).Cells(colSiloCode).Value, txtAdjustmentNo.Value, txtDate.Value, Nothing, gv1.Rows(ii).Cells(colUnit).Value)
                        End If
                        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                            CurBal_SNF = Math.Round(clsCommon.myCdbl(dt.Rows(0)("SNF_KG")), 3, MidpointRounding.ToEven)
                            CurBal_FAT = Math.Round(clsCommon.myCdbl(dt.Rows(0)("FAT_KG")), 3, MidpointRounding.ToEven)
                        End If
                    End If
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtAdjustmentNo._MYNavigator
        Try
            Dim qst As String = "select count(*) from TSPL_SILO_MILK_TRANSFER_HEAD where Document_Code='" + txtAdjustmentNo.Value + "'  and IsJobWork=0"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtAdjustmentNo._MYValidating
        ...
        Dim whrClas As String = " TSPL_SILO_MILK_TRANSFER_HEAD.IsJobWork=0 "
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrClas += " AND TSPL_SILO_MILK_TRANSFER_HEAD.MainLocation_Code in (" + objCommonVar.strCurrUserLocations + ") "
        End If
        txtAdjustmentNo.Value = clsCommon.ShowSelectForm("SiloMilkTransfer", qry, "Code", whrClas, txtAdjustmentNo.Value, "Code", isButtonClicked)
        LoadData(txtAdjustmentNo.Value, NavigatorType.Current)
    End Sub
    ...
    Private Sub FrmAPInvoiceEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
            ButtonToolTip.SetToolTip(btnSave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine +
                                              "TSPL_SILO_MILK_TRANSFER_HEAD " + Environment.NewLine +
                                              "TSPL_SILO_MILK_TRANSFER_DETAIL " + Environment.NewLine +
    ...
    Private Sub FndMainLocation__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles FndMainLocation._MYValidating
        FndMainLocation.Value = clsLocation.getFinder("  Seg.Seg_No='7' and ((Location_Type='Physical' and Is_Sub_Location='Y' and Is_Section ='N' and (isnull(Is_Jobwork,0)=1)) or (Location_Type='Physical' and Is_Sub_Location='N' and Is_Section ='N' and (isnull(Is_Jobwork,0)=0))) ", FndMainLocation.Value, isButtonClicked)
        If clsCommon.myLen(FndMainLocation.Value) > 0 Then
    ...
    Private Sub FndItemCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles FndItemCode._MYValidating
        Try
            ...
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Item_Code) > 0 Then
                FndItemCode.Value = obj.Item_Code
                LblItemName.Text = obj.Item_Desc
                fnditemuom.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select top 1 isnull(UOM_Code,'') from TSPL_ITEM_UOM_DETAIL where Item_Code ='" & obj.Item_Code & "' and Stocking_Unit='Y'", Nothing))
            Else
                FndItemCode.Value = ""
                LblItemName.Text = ""
                fnditemuom.Value = ""
