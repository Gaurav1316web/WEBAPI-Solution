...
Public Class frmProcessProductionStandardizationFinalQC
...
#End Region
    ...
    Private Sub frmProcessProductionStandardizationFinalQC_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'BHA/09/08/18-000409 by balwinder on 13/08/2018
        'Dim coll As Dictionary(Of String, String)
        'Try
        '    clsDBFuncationality.ExecuteNonQuery("alter table TSPL_PP_STD_FINALQC_ISSUE_ITEM_DETAIL alter column Avail_Qty decimal(18,3)")
        '    clsDBFuncationality.ExecuteNonQuery("alter table TSPL_PP_STD_FINALQC_ISSUE_ITEM_DETAIL alter column Avail_FAT_Per decimal(18,3)")
        '    clsDBFuncationality.ExecuteNonQuery("alter table TSPL_PP_STD_FINALQC_ISSUE_ITEM_DETAIL alter column Avail_SNF_Per decimal(18,3)")
        '    clsDBFuncationality.ExecuteNonQuery("alter table TSPL_PP_STD_FINALQC_ISSUE_ITEM_DETAIL alter column Avail_FAT_KG decimal(18,3)")
        '    clsDBFuncationality.ExecuteNonQuery("alter table TSPL_PP_STD_FINALQC_ISSUE_ITEM_DETAIL alter column Avail_SNF_KG decimal(18,3)")
        ...
        AutoCalcQtyAddRem = False ''If(clsCommon.CompairString(clsFixedParameter.GetData(clsFixedParameterType.AllowAutoCalculateADDREMOVEQty, clsFixedParameterCode.AllowAutoCalculateADDREMOVEQty, Nothing), "1") = CompairStringResult.Equal, True, False)
    ...
    Sub DisableAllTabPages()
        ...
        RadPageViewPage1.Item.Visibility = Telerik.WinControls.ElementVisibility.Visible
    End Sub
    ...
    Private Sub LoadSPBlankGrid()
        ...
        Status.DataSource = clsPPStdFinalQCHead.GetStageQCStatus()
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.frmProcessProductionStandardizationFinalQC)
    ...
    Private Sub frmProcessProductionStandardizationFinalQC_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
            ButtonToolTip.SetToolTip(btnsave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine + _
                                     "TSPL_PP_STD_FINALQC_HEAD " + Environment.NewLine + _
                                     "TSPL_PP_STD_FINALQC_BATCH_ITEM_DETAIL " + Environment.NewLine + _
                                     "TSPL_PP_STD_FINALQC_ISSUE_ITEM_DETAIL " + Environment.NewLine + _
                                     "TSPL_PP_STD_FINALQC_ADD_REMOVE_ITEM_DETAIL " + Environment.NewLine + _
                                     "TSPL_PP_STD_QC_DETAIL " + Environment.NewLine + _
                                     "TSPL_PP_STD_FINALQC_STAGE_DETAIL " + Environment.NewLine + _
                                     "TSPL_PP_STD_FINALQC_QC_LOG_SHEET " + Environment.NewLine + _
        ...
        If e.KeyData = Keys.F2 AndAlso gvARDetail.CurrentColumn IsNot Nothing AndAlso gvARDetail.CurrentColumn Is gvARDetail.Columns(colLoaction_Code) Then
            isCellValueChanged = True
    ...
    Private Sub LoadARBlankGrid()
        ...
        repoARQty.ReadOnly = If(AutoCalcQtyAddRem = True, True, False)
        ...
        repoLoaction_Code.Name = colLoaction_Code
        ...
        repoLoaction_Desc.Name = colLoaction_Desc
    ...
    Private Sub LoadQCBlankGrid()
        ...
        'repoActual_Value.DataSource = clsPPStdFinalQCHead.OpenParameterValueList("")
    ...
    Private Function AllowToSave(Optional ByVal IsPost As Boolean = False) As Boolean
        Try
            ...
            If clsCommon.myLen(fndChildBatchNo.Value) <= 0 Then
                ...
                Throw New Exception("Please select child Batch")
            End If
            If clsCommon.myLen(txtAgainstStdCode.Value) <= 0 Then
                txtAgainstStdCode.Focus()
                Throw New Exception("Please select Standaradization Code")
            End If
            ...
            Dim strBatchORderExistIntPIE As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select QC_code from TSPL_PP_STD_FINALQC_HEAD where TSPL_PP_STD_FINALQC_HEAD.Main_Batch_Code='" + fndMainBatchNo.Value + "' and TSPL_PP_STD_FINALQC_HEAD.QC_code not in ('" + txtCode.Value + "')"))
            If clsCommon.myLen(clsCommon.myCstr(strBatchORderExistIntPIE)) > 0 Then
                Throw New Exception("Please select different Batch Order, Same Batch exists with Production Standardization Final QC " & strBatchORderExistIntPIE & "  ")
            End If
            ...
            If IsPost = True Then
                ...
                For ii As Integer = 0 To gv_qc.Rows.Count - 1
                    ...
                    If clsCommon.myLen(paramcode) > 0 AndAlso clsCommon.CompairString(nature, "B") = CompairStringResult.Equal AndAlso (clsCommon.myLen(Actual_Status) <= 0) Then
                        RadPageView1.SelectedPage = pageParameterDetail
                        'gv_qc.Rows(ii).Cells(colQCrange1).Value = Nothing
                        'gv_qc.Rows(ii).Cells(colQCrange2).Value = Nothing
                        'gv_qc.Rows(ii).Cells(colQCvalue1).Value = Nothing
                        'gv_qc.Rows(ii).Cells(colQCvalue2).Value = Nothing
                    ...
                    If clsCommon.myLen(paramcode) > 0 AndAlso clsCommon.CompairString(nature, "R") = CompairStringResult.Equal AndAlso (clsCommon.myLen(Actual_Range) <= 0) Then
                        RadPageView1.SelectedPage = pageParameterDetail
                        'gv_qc.Rows(ii).Cells(colQCstatus).Value = ""
                        'gv_qc.Rows(ii).Cells(colQCvalue1).Value = Nothing
                        'gv_qc.Rows(ii).Cells(colQCvalue2).Value = Nothing
            ...
            For Each dr As GridViewRowInfo In gvARDetail.Rows
                If clsCommon.myLen(dr.Cells(colARItemCode).Value) <= 0 Then
                    Continue For
                End If
                If clsCommon.myLen(dr.Cells(colLoaction_Code).Value) <= 0 Then
                ...
                availQty = clsCommon.myCdbl(dr.Cells(colARAvailQty).Value) 'clsItemLocationDetails.getBalanceWithUnapproveForRMOther(dr.Cells(colARItemCode).Value, dr.Cells(colLoaction_Code).Value, Me.txtCode.Value, dtpDate.Value, Nothing, dr.Cells(colARUom).Value)
                reqQty = dr.Cells(colARQty).Value
                ...
                If clsCommon.CompairString(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.ConsiderSiloCapicityForStockIn, clsFixedParameterCode.ConsiderSiloCapicityForStockIn, Nothing)), "1") = CompairStringResult.Equal Then
                    If clsCommon.CompairString(clsCommon.myCstr(dr.Cells(colARType).Value), "Remove") = CompairStringResult.Equal AndAlso (clsCommon.CompairString(clsCommon.myCstr(dr.Cells(colARItemProductType).Value), "MI") = CompairStringResult.Equal Or clsCommon.CompairString(clsCommon.myCstr(dr.Cells(colARItemProductType).Value), "MP") = CompairStringResult.Equal) Then
                        Dim strsilo As String = clsCommon.myCstr(dr.Cells(colLoaction_Code).Value)
    ...
    Function ValidateFatSNFQuantityControl() As Boolean
        ...
        For Each grow As GridViewRowInfo In gvIssue.Rows
            If clsCommon.CompairString(grow.Cells(colIssueItemProductType).Tag, "MI") = CompairStringResult.Equal Or clsCommon.CompairString(grow.Cells(colIssueItemProductType).Tag, "MP") = CompairStringResult.Equal Then
                ...
                TotIssueQty = TotIssueQty + clsPPStdFinalQCHead.GetKG_AfterConversion(grow.Cells(colIssueItemCode).Value, grow.Cells(colIssueUom).Value, clsCommon.myCdbl(grow.Cells(colIssued_Qty).Value), Nothing) 'clsCommon.myCdbl(grow.Cells(colIssued_Qty).Value)
            End If
        Next
        For Each grow As GridViewRowInfo In gvARDetail.Rows
            If clsCommon.CompairString(grow.Cells(colARItemProductType).Value, "MI") = CompairStringResult.Equal Or clsCommon.CompairString(grow.Cells(colARItemProductType).Value, "MP") = CompairStringResult.Equal Then
                If clsCommon.CompairString(grow.Cells(colARType).Value, "Add") = CompairStringResult.Equal Then
                    ...
                    TotIssueQty = TotIssueQty + clsPPStdFinalQCHead.GetKG_AfterConversion(grow.Cells(colARItemCode).Value, grow.Cells(colARItemCode).Value, clsCommon.myCdbl(grow.Cells(colARQty).Value), Nothing) 'clsCommon.myCdbl(grow.Cells(colARQty).Value)
                End If
            End If
        Next
        ...
        For Each grow As GridViewRowInfo In gv.Rows
            If clsCommon.CompairString(grow.Cells(colitemproducttype).Value, "MI") = CompairStringResult.Equal Or clsCommon.CompairString(grow.Cells(colitemproducttype).Value, "MP") = CompairStringResult.Equal Then
                ...
                TotProdQty = TotProdQty + clsPPStdFinalQCHead.GetKG_AfterConversion(grow.Cells(colitemcode).Value, grow.Cells(coluom).Value, clsCommon.myCdbl(grow.Cells(colProduced_Qty).Value), Nothing) 'clsCommon.myCdbl(grow.Cells(colProduced_Qty).Value)
            End If
        Next
        For Each grow As GridViewRowInfo In gvARDetail.Rows
            If clsCommon.CompairString(grow.Cells(colARItemProductType).Value, "MI") = CompairStringResult.Equal Or clsCommon.CompairString(grow.Cells(colARItemProductType).Value, "MP") = CompairStringResult.Equal Then
                If clsCommon.CompairString(grow.Cells(colARType).Value, "Remove") = CompairStringResult.Equal Then
                    ...
                    TotProdQty = TotProdQty + clsPPStdFinalQCHead.GetKG_AfterConversion(grow.Cells(colARItemCode).Value, grow.Cells(colARItemCode).Value, clsCommon.myCdbl(grow.Cells(colARQty).Value), Nothing) 'clsCommon.myCdbl(grow.Cells(colARQty).Value)
                End If
            End If
        Next
    ...
    Private Function SaveData(ByVal isPost As Boolean) As Boolean
        Try
            ...
            obj.QC_Code = clsCommon.myCstr(txtCode.Value)
            obj.QC_Date = clsCommon.myCDate(dtpDate.Text)
            obj.Against_STD_Code = txtAgainstStdCode.Value
            ...
            obj.JW_Estimated_FAT_KG = clsCommon.myCdbl(lblJWEFATKg.Text)
            ...
            obj.ArrBatchItem = New List(Of clsPPStdFinalQCBatchItemDetail)
            obj.ArrIssueItem = New List(Of clsPPStdFinalQCIssueItemDetail)
            obj.ArrARItem = New List(Of clsPPStdFinalQCAddRemoveItemDetail)
            obj.ArrQC = New List(Of clsPPStdFinalQCQCDetail)
            obj.ArrStageQC = New List(Of clsPPStdFinalQCStageDetail)
            ...
            For Each grow As GridViewRowInfo In gv.Rows
                ...
                objtr.QC_Code = clsCommon.myCstr(txtCode.Value)
            ...
            For Each grow As GridViewRowInfo In gvARDetail.Rows
                ...
                objtr.Loaction_Code = clsCommon.myCstr(grow.Cells(colLoaction_Code).Value)
            ...
            obj.ArrDetail = New List(Of clsPPStdFinalQCDetail)
            For Each grow As GridViewRowInfo In gvParam.Rows
                Dim objTr As New clsPPStdFinalQCDetail()
            ...
            If clsPPStdFinalQCHead.SaveData(isNewEntry, obj) Then
    ...
    Private Function GetParamCollection() As List(Of clsPPStdFinalQCQCParam)
        Dim ArrParamDetail = New List(Of clsPPStdFinalQCQCParam)
        Dim objParam As clsPPStdFinalQCQCParam = Nothing
        For i As Integer = 0 To gvParam.Columns.Count - 1
            If clsCommon.CompairString(gvParam.Columns(i).Name, colParamSNO) = CompairStringResult.Equal OrElse clsCommon.CompairString(gvParam.Columns(i).Name, colParamItemCode) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamFrom) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamParentID) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamItemName) = CompairStringResult.Equal Then
            Else
                For jj As Integer = 0 To gvParam.Rows.Count - 1
                    objParam = New clsPPStdFinalQCQCParam
    ...
    Private Sub DeleteData()
        ...
        Try
            ...
            qry = "select count(*) from TSPL_PP_STD_FINALQC_HEAD where  QC_Code='" + txtCode.Value + "'"
            ...
            If clsPPStdFinalQCHead.DeleteData(txtCode.Value, trans) Then
                trans.Commit()
    ...
    Private Sub PostData()
        Try
            ...
            If check <= 0 Then
                ...
                Throw New Exception("Code not found.")
            End If
            If clsCommon.MyMessageBoxShow("Are you sure,want to post entry no. " + txtCode.Value + "?", "Attention", MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.Yes Then
                ''BHA/02/11/18-000660 by balwinder on 15/11/2018
                If clsPPStdFinalQCHead.PostData(Me.Form_ID, txtCode.Value, arrLoc) Then
    ...
    Private Sub txtCode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtCode._MYValidating
        ...
        If txtCode.MyReadOnly Or isButtonClicked Then
            txtCode.Value = clsCommon.myCstr(clsPPStdFinalQCHead.GetFinder(" TSPL_PP_BATCH_ORDER_HEAD.Location_Code in (" + arrLoc + ")", txtCode.Value, isButtonClicked))
            LoadData(txtCode.Value, NavigatorType.Current)
        Else
            FunReset()
        End If
    ...
    Private Sub LoadData(ByVal strCode As String, ByVal NavType As NavigatorType)
        Try
            Dim obj As New clsPPStdFinalQCHead()
            isNewEntry = True
            obj = clsPPStdFinalQCHead.GetData(strCode, arrLoc, NavType, Nothing)
            isInsideLoadData = True

            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.QC_Code) > 0 Then
                isNewEntry = False

                txtCode.Value = obj.QC_Code
                dtpDate.Text = obj.QC_Date
                ...
                Else
                    ...
                    btnCancel.Enabled = False
                End If
                txtAgainstStdCode.Value = obj.Against_STD_Code
                ...
                If obj.ArrBatchItem IsNot Nothing AndAlso obj.ArrBatchItem.Count > 0 Then
                    For Each objtr As clsPPStdFinalQCBatchItemDetail In obj.ArrBatchItem
                        gv.Rows.AddNew()
                ...
                If obj.ArrIssueItem IsNot Nothing AndAlso obj.ArrIssueItem.Count > 0 Then
                    For Each objtr As clsPPStdFinalQCIssueItemDetail In obj.ArrIssueItem
                        gvIssue.Rows.AddNew()
                ...
                If obj.ArrARItem IsNot Nothing AndAlso obj.ArrARItem.Count > 0 Then
                    For Each objtr As clsPPStdFinalQCAddRemoveItemDetail In obj.ArrARItem
                        ...
                        gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colLoaction_Code).Value = objtr.Loaction_Code
                        gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colLoaction_Desc).Value = objtr.Location_Desc
                        gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARRemarks).Value = objtr.Remarks
                        'gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARAvailQty).Value = clsItemLocationDetails.getBalanceWithUnapproveForRMOther(gvARDetail.CurrentRow.Cells(colARItemCode).Value, gvARDetail.CurrentRow.Cells(colLoaction_Code).Value, Me.txtCode.Value, dtpDate.Value, Nothing, gvARDetail.CurrentRow.Cells(colARUom).Value)
                        'If clsCommon.CompairString(gvARDetail.CurrentRow.Cells(colARItemProductType).Value, "MI") = CompairStringResult.Equal Then
                        '    gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARAvailQty).Value = ClsLoadingTanker.getBalance(gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARItemCode).Value, Me.txtlocation.Text, gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colLoaction_Code).Value, Me.txtCode.Value, dtpDate.Value, Nothing, gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARUom).Value)
                        'Else
                        '    gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARAvailQty).Value = clsItemLocationDetails.getBalanceWithUnapproveForRMOther(gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARItemCode).Value, gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colLoaction_Code).Value, Me.txtCode.Value, dtpDate.Value, Nothing, gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARUom).Value)
                        'End If
                        Dim loc_type As Integer = 0
                        qry = "select case when (isnull(is_section,'N')='N' and isnull(is_sub_location,'N')='N') then 'MAIN' when (isnull(is_section,'N')='Y' and isnull(is_sub_location,'N')='N') then 'SEC' when (isnull(is_section,'N')='N' and isnull(is_sub_location,'Y')='Y') then 'SUB' else'MAIN' end as [type] from tspl_location_master where location_code='" + clsCommon.myCstr(gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colLoaction_Code).Value) + "'"
                        ...
                        Dim dt As DataTable = clsProcessProductionPlanning.GetMilkAndALLItemStockBalance_With_FATSNFKG(clsCommon.myCstr(gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARItemCode).Value), txtlocation.Text, clsCommon.myCstr(gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colLoaction_Code).Value), IIf(CheckStockServerDate = True, clsCommon.GETSERVERDATE(Nothing), dtpDate.Value), Nothing, clsCommon.myCstr(gvARDetail.Rows(gvARDetail.Rows.Count - 1).Cells(colARUom).Value), loc_type, IIf(clsCommon.CompairString(clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colARType).Value), "Remove") = CompairStringResult.Equal, True, False))
                ...
                If obj.ArrQC IsNot Nothing AndAlso obj.ArrQC.Count > 0 Then
                    ...
                    For Each objtr As clsPPStdFinalQCQCDetail In obj.ArrQC
                        gv_qc.Rows.AddNew()
                ...
                If obj.ArrStageQC IsNot Nothing AndAlso obj.ArrStageQC.Count > 0 Then
                    For Each objtr As clsPPStdFinalQCStageDetail In obj.ArrStageQC
                        gvStage.Rows.AddNew()
                ...
                Next
                For Each objTr As clsPPStdFinalQCQCParam In obj.ArrParamDetail
                    Try
    ...
    Private Sub OpenGVLocation(ByVal isButtonClicked As Boolean)
        If OpenAvailorEmptyStckLocationOn_Standardization Then
            ...
            End If
            Dim qry As String = clsPPStdFinalQCHead.GetMilkAndALLItemStockBalance_With_FATSNFKG(clsCommon.myCstr(gv.CurrentRow.Cells(colitemcode).Value), txtlocation.Text, clsCommon.myCstr(gv.CurrentRow.Cells(colSTD_Loaction_Code).Value), IIf(CheckStockServerDate = True, clsCommon.GETSERVERDATE(Nothing), dtpDate.Value), Nothing, clsCommon.myCstr(gv.CurrentRow.Cells(coluom).Value), True)

            gv.CurrentRow.Cells(colSTD_Loaction_Code).Value = clsCommon.ShowSelectForm("LOCSUBFND", qry, "Code", " [Type] in ('Section','Sub Location') and [Stock Qty]>=0 ", clsCommon.myCstr(gv.CurrentRow.Cells(colSTD_Loaction_Code).Value), "Code", isButtonClicked) 'clsLocation.getFinder("((Is_Section='Y' or Is_Sub_Location='Y') and Main_Location_Code='" & txtlocation.Text & "')", gv.CurrentRow.Cells(colSTD_Loaction_Code).Value, False) '
        Else
            gv.CurrentRow.Cells(colSTD_Loaction_Code).Value = clsLocation.getFinder("((Is_Section='Y' or Is_Sub_Location='Y') and Main_Location_Code='" & txtlocation.Text & "')", gv.CurrentRow.Cells(colSTD_Loaction_Code).Value, False)
        End If
        ...
        If Not isInsideLoadData Then
            If Not isCellValueChanged Then
                If e.Column Is gv_qc.Columns(colActual_Value) Then
                    ...
                    gv_qc.CurrentRow.Cells(colActual_Value).Value = clsPPStdFinalQCHead.GetFinderParameterValueList("Value in (" & Values & ")", gv_qc.CurrentRow.Cells(colActual_Value).Value, False) 'clsItemMaster.getFinder("", gvARDetail.CurrentRow.Cells(colARItemCode).Value, False)
                    isCellValueChanged = False
    ...
    Sub FillStageDetail()
        ...
        obj = clsPPStdFinalQCHead.FillStageDetail(Me.fndChildBatchNo.Value)
    ...
    Sub FillBatchOrder()
        ...
        If Not objBO Is Nothing Then
            For Each dr As clsProcessBatchOrderMainDetail In objBO.ArrMainItem
                ...
                Dim dtQC As DataTable = clsPPStdFinalQCHead.GetItemQCParameter(dr.icode)
    ...
    Sub FillIssueAgainstBatchOrder()
        ...
        Dim dt As DataTable = clsPPStdFinalQCHead.GetIssueAgainstBatch(Me.fndChildBatchNo.Value, Me.txtCode.Value)
    ...
    Sub FillQCGrid(ByVal ParentLineNo As Integer, Optional ByVal IsAddRemoveItem As Boolean = False, Optional ByVal Item_Code As String = "")
        ...
        If IsAddRemoveItem = False Then
            dt = clsPPStdFinalQCHead.GetQCParameters(Me.fndChildBatchNo.Value)
            gv_qc.Rows.Clear()
        Else
            dt = clsPPStdFinalQCHead.GetQCParametersForItem(Item_Code)
        End If
        ...
        If Not isInsideLoadData Then
            ...
            If Not isCellValueChanged Then
                ...
                If e.Column Is gvARDetail.Columns(colARUom) Then
                    ...
                    isCellValueChanged = False
                End If
                If e.Column Is gvARDetail.Columns(colLoaction_Code) Then
                    isCellValueChanged = True
                ...
                ElseIf e.Column Is gvARDetail.Columns(colAR_FAT_KG) AndAlso AutoCalcQtyAddRem = True Then
                    isCellValueChanged = True
                    gvARDetail.CurrentRow.Cells(colARQty).Value = gvARDetail.CurrentRow.Cells(colAR_FAT_KG).Value * 100 / gvARDetail.CurrentRow.Cells(colAR_FAT_Per).Value
                    gvARDetail.CurrentRow.Cells(colAR_SNF_KG).Value = clsBOM.GetFatSNFKG_AfterConversion(gvARDetail.CurrentRow.Cells(colARItemCode).Value, gvARDetail.CurrentRow.Cells(colARUom).Value, gvARDetail.CurrentRow.Cells(colARQty).Value, gvARDetail.CurrentRow.Cells(colAR_SNF_Per).Value, Nothing)
                    calculateALL()
                    isCellValueChanged = False
                ElseIf e.Column Is gvARDetail.Columns(colAR_SNF_KG) AndAlso AutoCalcQtyAddRem = True Then
                    isCellValueChanged = True
                    gvARDetail.CurrentRow.Cells(colARQty).Value = gvARDetail.CurrentRow.Cells(colAR_SNF_KG).Value * 100 / gvARDetail.CurrentRow.Cells(colAR_SNF_Per).Value
                    gvARDetail.CurrentRow.Cells(colAR_FAT_KG).Value = clsBOM.GetFatSNFKG_AfterConversion(gvARDetail.CurrentRow.Cells(colARItemCode).Value, gvARDetail.CurrentRow.Cells(colARUom).Value, gvARDetail.CurrentRow.Cells(colARQty).Value, gvARDetail.CurrentRow.Cells(colAR_FAT_Per).Value, Nothing)
                    calculateALL()
                    isCellValueChanged = False
                End If
    ...
    Private Sub OpenARLocationCode(ByVal isButtonClicked As Boolean)
        ...
        If OpenAvailorEmptyStckLocationOn_Standardization Then
            ...
            End If
            Dim qry As String = clsPPStdFinalQCHead.GetMilkAndALLItemStockBalance_With_FATSNFKG(clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colARItemCode).Value), txtlocation.Text, clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colLoaction_Code).Value), IIf(CheckStockServerDate = True, clsCommon.GETSERVERDATE(Nothing), dtpDate.Value), Nothing, clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colARUom).Value), IIf(clsCommon.CompairString(clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colARType).Value), "Remove") = CompairStringResult.Equal, True, False))
            gvARDetail.CurrentRow.Cells(colLoaction_Code).Value = clsCommon.ShowSelectForm("LOCSUBLOCSTCKFND", qry, "Code", " [Type] in ('Section','Sub Location') and [Stock Qty]>=0 " & strItemLoc, clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colLoaction_Code).Value), "Code", isButtonClicked) '
        Else
            If ShowLocationItemLocationwise = 1 AndAlso clsCommon.CompairString(gvARDetail.CurrentRow.Cells(colARType).Value, "Add") = CompairStringResult.Equal Then
            ...
            End If
            gvARDetail.CurrentRow.Cells(colLoaction_Code).Value = clsLocation.getFinder("(((Is_Section='Y' or Is_Sub_Location='Y') and Main_Location_Code='" & txtlocation.Text & "') or Location_Code='" & txtlocation.Text & "')" & strItemLoc, gvARDetail.CurrentRow.Cells(colLoaction_Code).Value, False)
        End If

        gvARDetail.CurrentRow.Cells(colLoaction_Desc).Value = clsLocation.GetName(clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colLoaction_Code).Value), Nothing)
        Dim loc_type As Integer = 0
        qry = "select case when (isnull(is_section,'N')='N' and isnull(is_sub_location,'N')='N') then 'MAIN' when (isnull(is_section,'N')='Y' and isnull(is_sub_location,'N')='N') then 'SEC' when (isnull(is_section,'N')='N' and isnull(is_sub_location,'Y')='Y') then 'SUB' else'MAIN' end as [type] from tspl_location_master where location_code='" + clsCommon.myCstr(gvARDetail.CurrentRow.Cells(colLoaction_Code).Value) + "'"
        If clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry)), "MAIN") = CompairStringResult.Equal Then
            loc_type = 2
        ElseIf clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry)), "SUB") = CompairStringResult.Equal Then
            loc_type = 1
        ...
        If Not isInsideLoadData Then
            ...
            If Not isCellValueChanged Then
                If e.Column Is gvStage.Columns(colStatus) Then
                    ...
                    '                Exit Sub
                    '            End If
                    '            For Each obj As clsPPStdFinalQCLogSheetDetail In grow.Tag
        ...
        Try
            If common.clsCommon.MyMessageBoxShow("Amend and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Else
                    Reason = frm.strRmks
                End If
                If clsPPStdFinalQCHead.UnpostData(txtCode.Value, Me.Form_ID, True) Then
                ...
                If clsCommon.CompairString(grow.Cells(colARItemCode).Value, Item_Code) = CompairStringResult.Equal Then
                    If clsCommon.CompairString(Type, "FAT") = CompairStringResult.Equal Then
                        ...
                        If Value > 0 AndAlso AutoCalcQtyAddRem = True Then
                            grow.Cells(colARQty).Value = grow.Cells(colAR_FAT_KG).Value * 100 / Value
                        Else
                    ...
                    ElseIf clsCommon.CompairString(Type, "SNF") = CompairStringResult.Equal Then
                        ...
                        If Value > 0 AndAlso AutoCalcQtyAddRem = True Then
                            grow.Cells(colARQty).Value = grow.Cells(colAR_SNF_KG).Value * 100 / Value
                        Else
    ...
    Function CancelData() As Boolean
        Try
            ...
            If clsCommon.MyMessageBoxShow("Are you sure to Cancel the Record?", "", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.No Then
                Exit Function
            End If
            clsPPStdFinalQCHead.CancelData(Me.Form_ID, txtCode.Value)
    ...
    Sub calculateALL()
        ...
        For ii As Integer = 0 To gvARDetail.Rows.Count - 1
            ...
            Else
                RI = -1
            End If
            dblQty += clsCommon.myCdbl(gvARDetail.Rows(ii).Cells(colARQty).Value) * RI
            dblFATKG += clsCommon.myCdbl(gvARDetail.Rows(ii).Cells(colAR_FAT_KG).Value) * RI
            dblSNFKG += clsCommon.myCdbl(gvARDetail.Rows(ii).Cells(colAR_SNF_KG).Value) * RI
        Next
        ...
        For jj As Integer = 0 To gvParam.Rows.Count - 1
            If clsCommon.CompairString(clsCommon.myCstr(gvParam.Rows(jj).Cells(colParamFrom).Value), "Batch Order") = CompairStringResult.Equal Then
                For i As Integer = 0 To gvParam.Columns.Count - 1
                    If clsCommon.CompairString(gvParam.Columns(i).Name, colParamSNO) = CompairStringResult.Equal OrElse clsCommon.CompairString(gvParam.Columns(i).Name, colParamItemCode) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamFrom) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamParentID) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamItemName) = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(gvParam.Columns(i).Tag), "FAT") = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(gvParam.Columns(i).Tag), "SNF") = CompairStringResult.Equal Then
                    Else
                        arrQC.Add(gvParam.Columns(i).Name, clsCommon.myCstr(gvParam.Rows(jj).Cells(i).Value))
                    End If
                Next
            End If
        Next
        ...
        Dim objJobEst As clsJWFormulaSoln = clsJWFormulaSoln.CalculateEstimate(dtpDate.Value, strStructureCode, txtlocation.Text, clsCommon.myCdbl(IIf(SettUseProductFATSNFKgForEstimationCost, lblTotProduceFATKG.Text, lblTotIssueFATKG.Text)), clsCommon.myCdbl(IIf(SettUseProductFATSNFKgForEstimationCost, lblTotProduceSNFKG.Text, lblTotIssueSNFKG.Text)), arrQC)
    ...
    Sub loadBlankParameterGrid()
        ...
        repoDecimalColumn = New GridViewDecimalColumn()
        repoDecimalColumn.Name = colParamParentID
        repoDecimalColumn.FormatString = "{0:n0}"
        repoDecimalColumn.DecimalPlaces = 0
        repoDecimalColumn.HeaderText = "Parent ID"
        repoDecimalColumn.Tag = colParamParentID
    ...
    Sub fillDataFromStdQC()
        ...
        For intCount = 0 To gv_qc.Rows.Count - 1
            Dim strTemp As String = clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCType).Value) + clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCItemCode).Value) + clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCParentLineNo).Value)
            If Not arrAdded.Contains(strTemp) Then
                ...
                gvParam.Rows(intCount).Cells(colParamFrom).Value = clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCType).Value)
                gvParam.Rows(intCount).Cells(colParamItemCode).Value = clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCItemCode).Value)
                gvParam.Rows(intCount).Cells(colParamItemName).Value = clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCItemName).Value)
                gvParam.Rows(intCount).Cells(colParamParentID).Value = clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCParentLineNo).Value)
            End If
            If clsCommon.CompairString(clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCType).Value), "Add/Remove") = CompairStringResult.Equal Then
                If clsCommon.CompairString(clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCparam_type).Value), "FAT") = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCparam_type).Value), "SNF") = CompairStringResult.Equal Then
                    For kk As Integer = 0 To gvParam.Rows.Count - 1
                        If clsCommon.CompairString(clsCommon.myCstr(gvParam.Rows(kk).Cells(colParamFrom).Value), "Add/Remove") = CompairStringResult.Equal Then
                            If clsCommon.CompairString(clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCItemCode).Value), clsCommon.myCstr(gvParam.Rows(kk).Cells(colParamItemCode).Value)) = CompairStringResult.Equal Then
                                If clsCommon.CompairString(clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCparam_type).Value), "FAT") = CompairStringResult.Equal Then
                                    gvParam.Rows(kk).Cells(FATColName).Value = clsCommon.myCdbl(gv_qc.Rows(intCount).Cells(colActual_Range).Value)
                                ElseIf clsCommon.CompairString(clsCommon.myCstr(gv_qc.Rows(intCount).Cells(colQCparam_type).Value), "SNF") = CompairStringResult.Equal Then
                                    gvParam.Rows(kk).Cells(SNFColName).Value = clsCommon.myCdbl(gv_qc.Rows(intCount).Cells(colActual_Range).Value)
                                End If
                            End If
                        End If
                    Next
    ...
    Private Function getFinderBaseQty(ByVal strChildBatch As String) As String
        Dim qry As String = "select Standardization_Code,max(Standardization_Date) as Standardization_Date,max(Child_Batch_Code) as Child_Batch_Code,max(Main_Batch_Code) as Main_Batch_Code,max(Loaction_Code) as Loaction_Code,max(ManualBatchNo) as ManualBatchNo from (" + Environment.NewLine + _
        "select Standardization_Code,Standardization_Date,Child_Batch_Code,Main_Batch_Code,Loaction_Code,1 as RI,1 as Chk,ManualBatchNo  from TSPL_PP_STANDARDIZATION_HEAD" + Environment.NewLine + _
        "where 2=2 and Posted=1 "
        If clsCommon.myLen(strChildBatch) > 0 Then
            qry += " and Child_Batch_Code='" + fndChildBatchNo.Value + "'"
        End If
        qry += "union all" + Environment.NewLine + _
        "select Against_STD_Code as Standardization_Code,null as Standardization_Date,null as Child_Batch_Code,null as Main_Batch_Code,null as Loaction_Code,-1 as RI,0 as chk,ManualBatchNo   from TSPL_PP_STD_FINALQC_HEAD where Status<>'P' and QC_Code not in ('" + txtCode.Value + "') " + Environment.NewLine + _
        ")xx group by Standardization_Code  having sum(ri)>0 and sum(chk)>0 "
        Return qry
    End Function
    ...
    Private Sub txtfrmsub__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndChildBatchNo._MYValidating
        Dim qry As String = "select * from ( select Child_Batch_Code,max(Main_Batch_Code) as Main_Batch_Code,max(Loaction_Code) as Loaction_Code,max(ManualBatchNo) as ManualBatchNo from (" + getFinderBaseQty("") + ")xxx group by Child_Batch_Code)xxxx"
        fndChildBatchNo.Value = clsCommon.ShowSelectForm("ChBPPFinalQC", qry, "Child_Batch_Code", "", fndChildBatchNo.Value, "", isButtonClicked)
        If clsCommon.myLen(fndChildBatchNo.Value) > 0 AndAlso UseProductionPlaningDateForWholeProductionCycle = True Then
    ...
    Private Sub txtAgainstStdCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtAgainstStdCode._MYValidating
        Try
            If clsCommon.myLen(fndChildBatchNo.Value) <= 0 Then
                Throw New Exception("Please first select child batch")
            End If
            Dim qry As String = "select * from (" + getFinderBaseQty(fndChildBatchNo.Value) + ")xxxx"
            txtAgainstStdCode.Value = clsCommon.ShowSelectForm("ChBSdFinalQC", qry, "Standardization_Code", "", txtAgainstStdCode.Value, "", isButtonClicked)
            If clsCommon.myLen(txtAgainstStdCode.Value) > 0 Then
                FillSTDData(txtAgainstStdCode.Value)
            End If
        Catch ex As Exception
    ...
    Private Sub FillSTDData(ByVal strCode As String)
        Try
            Dim obj As New clsProcessProductionStandardization()
            obj = clsProcessProductionStandardization.GetData(strCode, arrLoc, NavigatorType.Current, Nothing)
            isInsideLoadData = True
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Standardization_Code) > 0 Then
                txtAgainstStdCode.Value = obj.Standardization_Code
                fndChildBatchNo.Value = obj.Child_Batch_Code
                txtlocation.Text = obj.Loaction_Code
                txtlocationname.Text = obj.Loaction_Desc
                fndMainBatchNo.Value = obj.Main_Batch_Code
                ...
                chkJobWorkInward.Checked = clsProcessBatchOrder.IsJobWorkBatchOrder(obj.Child_Batch_Code, Nothing)
            ...
            If Not isInsideLoadData Then
                If Not isCellValueChangedParam Then
                    ...
                    If e.Column Is gvParam.Columns(FATColName) Then
                        For ii As Integer = 0 To gv_qc.Rows.Count - 1
                            If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamItemCode).Value), clsCommon.myCstr(gv_qc.Rows(ii).Cells(colQCItemCode).Value)) = CompairStringResult.Equal Then
                                If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamFrom).Value), clsCommon.myCstr(gv_qc.Rows(ii).Cells(colQCType).Value)) = CompairStringResult.Equal Then
                                    If clsCommon.myCdbl(gvParam.CurrentRow.Cells(colParamParentID).Value) = clsCommon.myCdbl(gv_qc.Rows(ii).Cells(colQCParentLineNo).Value) Then
                                        If clsCommon.CompairString("FAT", clsCommon.myCstr(gv_qc.Rows(ii).Cells(colQCparam_type).Value)) = CompairStringResult.Equal Then
                                            gv_qc.CurrentRow = gv_qc.Rows(ii)
                                            gv_qc.CurrentColumn = gv_qc.Columns(colActual_Range)
                                            gv_qc.Rows(ii).Cells(colActual_Range).Value = clsCommon.myCdbl(gvParam.CurrentRow.Cells(FATColName).Value)
                    ...
                    ElseIf e.Column Is gvParam.Columns(SNFColName) Then
                        For ii As Integer = 0 To gv_qc.Rows.Count - 1
                            If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamItemCode).Value), clsCommon.myCstr(gv_qc.Rows(ii).Cells(colQCItemCode).Value)) = CompairStringResult.Equal Then
                                If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamFrom).Value), clsCommon.myCstr(gv_qc.Rows(ii).Cells(colQCType).Value)) = CompairStringResult.Equal Then
                                    If clsCommon.myCdbl(gvParam.CurrentRow.Cells(colParamParentID).Value) = clsCommon.myCdbl(gv_qc.Rows(ii).Cells(colQCParentLineNo).Value) Then
                                        If clsCommon.CompairString("SNF", clsCommon.myCstr(gv_qc.Rows(ii).Cells(colQCparam_type).Value)) = CompairStringResult.Equal Then
                                            gv_qc.CurrentRow = gv_qc.Rows(ii)
                                            gv_qc.CurrentColumn = gv_qc.Columns(colActual_Range)
                                            gv_qc.Rows(ii).Cells(colActual_Range).Value = clsCommon.myCdbl(gvParam.CurrentRow.Cells(SNFColName).Value)
                                            'UpdateBatchFatSNF(clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCItemCode).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colActual_Range).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCparam_type).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCType).Value))
                                        End If
                                    End If
                                End If
    ...
    Private Sub btnUpdateParameters_Click(sender As Object, e As EventArgs) Handles btnUpdateParameters.Click
        Try
            clsPPStdFinalQCQCParam.saveData(txtCode.Value, GetParamCollection())
    ...
    Private Sub gvParam_CellFormatting(sender As Object, e As CellFormattingEventArgs) Handles gvParam.CellFormatting
        Try
            If e.Column Is gvParam.Columns(FATColName) Then
                gvParam.Columns(FATColName).ReadOnly = False
                If settTankerDispatchAvgFATSNFPer Then
                    If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamFrom).Value), "Add/Remove") = CompairStringResult.Equal Then
                        gvParam.Columns(FATColName).ReadOnly = True
                    End If
                End If
            ElseIf e.Column Is gvParam.Columns(SNFColName) Then
                gvParam.Columns(SNFColName).ReadOnly = False
                If settTankerDispatchAvgFATSNFPer Then
                    If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamFrom).Value), "Add/Remove") = CompairStringResult.Equal Then
                        gvParam.Columns(SNFColName).ReadOnly = True
                    End If
