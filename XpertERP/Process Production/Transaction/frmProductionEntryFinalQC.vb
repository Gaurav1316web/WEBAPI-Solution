...
Public Class frmProductionEntryFinalQC
...
#Region "Variables"
    ...
    Dim isCellValueChangedParam As Boolean = False
    '' issue tab columns
...
#End Region
    ...
    Sub DisableAllTabPages()
        For Each page As RadPageViewPage In RadPageView1.Pages
            page.Item.Visibility = IIf(page.Item.Visibility = Telerik.WinControls.ElementVisibility.Visible, Telerik.WinControls.ElementVisibility.Collapsed, Telerik.WinControls.ElementVisibility.Visible)
        Next
        RadPageViewPage2.Item.Visibility = Telerik.WinControls.ElementVisibility.Visible
    End Sub

    Sub LoadStatus()
        cboStaus.DataSource = GetStatus()
        cboStaus.ValueMember = "Code"
        cboStaus.DisplayMember = "Name"
    End Sub
    ...
    Sub BlankAllControls()
        ...
        txtProductionEntryNo.Value = ""
        txtCode.MyReadOnly = False
        cboStaus.SelectedValue = "A"
        txtProductionEntryNo.Value = ""
        gvIssue.Rows.Clear()
        gvStage.Rows.Clear()
        gvBatch.Rows.Clear()
    ...
    Function AllowToSave(Optional ByVal isPost As Boolean = False) As Boolean
        ...
        'If clsProductionEntryFinalQCHead.checkStock(IIf(MOActive = True, "MO", "BO"), Me.txtBatchNo.Value, ProdItemsQty, clsCommon.GetPrintDate(Me.dtpDate.Value, "dd/MMM/yyyy"), Me.txtLocation.Value, Me.txtCode.Value) = False Then
        ...
        If isPost = True Then
            ...
            'Dim dt As DataTable = clsProductionEntryFinalQCConsumption.GetRM(txtCode.Value)
            ...
            For Each grow As GridViewRowInfo In gvWreckage.Rows
                ...
                '    End If
                'Next
                ''   Consm_Item_Code,Consm_Unit_Code,SUM(CONSM_QTY) as CONSM_QTY,SUM(FAT_KG) as FAT_KG,SUM(SNF_KG) as SNF_KG,(SUM(FAT_KG)/SUM(CONSM_QTY))*100 as FAT_Pers," & _
                ''" (SUM(SNF_KG)/SUM(CONSM_QTY))*100 as SNF_Pers
                'If found = False Then
                ...
                'End If
            Next
            ' '' consolidated validation for stock on section location
            'Dim Stockmsg As String = String.Empty
            'For Each dr As DataRow In dt.Rows
            '    Dim found As Boolean = False
            '    For Each grow As GridViewRowInfo In gvSectionStock.Rows
            '        Dim Requir_Qty As Decimal = Math.Round(clsCommon.myCdbl(dr.Item("Consm_Qty")) * clsItemMaster.GetConvertionFactor(clsCommon.myCstr(dr.Item("Consm_Item_Code")), clsCommon.myCstr(dr.Item("Consm_Unit_Code")), Nothing), 2)
            '        If clsCommon.CompairString(grow.Cells("Item Code").Value, dr.Item("Consm_Item_Code")) = CompairStringResult.Equal Then
            '            found = True
            '            If Requir_Qty > clsCommon.myCdbl(grow.Cells("Balance").Value) Then
            '                Stockmsg = Stockmsg & " " & Environment.NewLine & " Item Code-" & clsCommon.myCstr(dr.Item("Consm_Item_Code")) & " Required Qty:" & Requir_Qty & " Avail Qty : " & clsCommon.myCdbl(grow.Cells("Balance").Value) & " " & clsCommon.myCstr(grow.Cells("Stock_UOM").Value)
            '            End If
            ...
            '    Next
            '    If found = False Then
            '        Stockmsg = Stockmsg & " " & Environment.NewLine & " Item Code-" & clsCommon.myCstr(dr.Item("Consm_Item_Code")) & " Required Qty:" & clsCommon.myCdbl(dr.Item("Consm_Qty")) & " Avail Qty : 0 " & clsCommon.myCstr(dr.Item("Consm_Unit_Code"))
            '    End If
            'Next
            'If clsCommon.myLen(Stockmsg) > 0 Then
            '    Stockmsg = "Insufficient Stock " & Environment.NewLine & Stockmsg
            '    clsCommon.MyMessageBoxShow(Stockmsg)
            'End If
            ...
            For ii As Integer = 0 To gvParameter.Rows.Count - 1
                ...
                If clsCommon.myLen(paramcode) > 0 AndAlso clsCommon.CompairString(nature, "B") = CompairStringResult.Equal AndAlso (clsCommon.myLen(Actual_Status) <= 0) Then
                    RadPageView1.SelectedPage = pageParameterDetail
                    'gvParameter.Rows(ii).Cells(colQCrange1).Value = Nothing
                    'gvParameter.Rows(ii).Cells(colQCrange2).Value = Nothing
                    'gvParameter.Rows(ii).Cells(colQCvalue1).Value = Nothing
                    'gvParameter.Rows(ii).Cells(colQCvalue2).Value = Nothing
                ...
                If clsCommon.myLen(paramcode) > 0 AndAlso clsCommon.CompairString(nature, "R") = CompairStringResult.Equal AndAlso (clsCommon.myLen(Actual_Range) <= 0) Then
                    RadPageView1.SelectedPage = pageParameterDetail
                    'gvParameter.Rows(ii).Cells(colQCstatus).Value = ""
                    'gvParameter.Rows(ii).Cells(colQCvalue1).Value = Nothing
                    'gvParameter.Rows(ii).Cells(colQCvalue2).Value = Nothing
        ...
        For jj As Integer = 0 To gvParam.Rows.Count - 1
            For i As Integer = 0 To gvParam.Columns.Count - 1
                isManadatory = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select IsProductionMandatory from TSPL_PARAMETER_MASTER where Code='" & gvParam.Columns(i).Name & "'"))
    ...
    Function SaveData(ByVal ChekBtnPost As Boolean) As Boolean
        Try
            ...
            If AllowToSave() Then
                Dim obj As New clsProductionEntryFinalQCHead
                obj.QC_Code = Me.txtCode.Value
                obj.DESCRIPTION = Me.txtDesc.Text
                obj.Is_Job_Work_Inward = chkJobWorkInward.Checked
                obj.Status = clsCommon.myCstr(cboStaus.SelectedValue)
                obj.Against_PE_Code = txtProductionEntryNo.Value
                obj.Status = (cboStaus.SelectedValue)
                obj.QC_Date = Me.dtpDate.Value
                ...
                ''----------------
                Dim obj1 As clsProductionEntryFinalQCDetail
                'objList = New List(Of clsProductionEntryFinalQCDetail)

                obj.ArrBatchItem = New List(Of clsProductionEntryFinalQCDetail)
                obj.ArrIssueItem = New List(Of clsProductionEntryFinalQCIssueItems)
                obj.ArrQC = New List(Of clsProductionEntryFinalQCQCDetail)
                obj.ArrStage = New List(Of clsProductionEntryFinalQCStageDetail)
                obj.ArrWF = New List(Of clsProductionEntryFinalQCWreckageFlushingItem)
                obj.ArrScrap = New List(Of clsProductionEntryFinalQCScrapItems)


                For Each grow As GridViewRowInfo In gvBatch.Rows
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colItemCode).Value)) > 0 Then
                        obj1 = New clsProductionEntryFinalQCDetail()
                        obj1.QC_Code = txtCode.Value
                ...
                For Each grow As GridViewRowInfo In GvScrap.Rows
                    Dim objScrap As New clsProductionEntryFinalQCScrapItems
                ...
                obj.ArrDetail = New List(Of clsProductionEntryFinalQCParameterDetail)
                For Each grow As GridViewRowInfo In gvParam.Rows
                    Dim objTr As New clsProductionEntryFinalQCParameterDetail()
                    objTr.Line_No = clsCommon.myCdbl(grow.Cells(colParamSNO).Value)
                    objTr.Item_Code = clsCommon.myCstr(grow.Cells(colParamItemCode).Value)
                    objTr.FAT_Per = clsCommon.myCdbl(grow.Cells(FATColName).Value)
                    objTr.SNF_Per = clsCommon.myCdbl(grow.Cells(SNFColName).Value)
                    objTr.QC_From = clsCommon.myCstr(grow.Cells(colParamFrom).Value)
                    'objTr.Parent_ID = clsCommon.myCdbl(grow.Cells(colParamParentID).Value)
                    If (clsCommon.myLen(objTr.Item_Code) > 0) Then
                        obj.ArrDetail.Add(objTr)
                    End If
                Next
                obj.ArrParamDetail = GetParamCollection()
    ...
    Private Function GetParamCollection() As List(Of clsProductionEntryFinalQCQCParam)
        Dim ArrParamDetail = New List(Of clsProductionEntryFinalQCQCParam)
        Dim objParam As clsProductionEntryFinalQCQCParam = Nothing
        For i As Integer = 0 To gvParam.Columns.Count - 1
            If clsCommon.CompairString(gvParam.Columns(i).Name, colParamSNO) = CompairStringResult.Equal OrElse clsCommon.CompairString(gvParam.Columns(i).Name, colParamItemCode) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamFrom) = CompairStringResult.Equal Or clsCommon.CompairString(gvParam.Columns(i).Name, colParamItemName) = CompairStringResult.Equal Then
            Else
                For jj As Integer = 0 To gvParam.Rows.Count - 1
                    objParam = New clsProductionEntryFinalQCQCParam
                    ...
                    objParam.Param_Field_Value = clsCommon.myCstr(gvParam.Rows(jj).Cells(i).Value)
                    ...
                    If clsCommon.CompairString(objParam.Param_Type, "FAT") = CompairStringResult.Equal OrElse clsCommon.CompairString(objParam.Param_Type, "SNF") = CompairStringResult.Equal Then
                        objParam.Param_Field_Value = clsCommon.myFormat(clsCommon.myCstr(gvParam.Rows(jj).Cells(i).Value))
                    End If
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        obj = clsProductionEntryFinalQCHead.GetData(strCode, arrLoc, NavTyep)
        If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Batch_Code) > 0 Then
            ...
            Dim ii As Int16 = 0
            LoadBlankGrid()
            txtCode.Value = obj.QC_Code
            txtProductionEntryNo.Value = obj.Against_PE_Code
            cboStaus.SelectedValue = obj.Status
            ...
            If (obj.ArrBatchItem IsNot Nothing AndAlso obj.ArrBatchItem.Count > 0) Then
                For Each objTr As clsProductionEntryFinalQCDetail In obj.ArrBatchItem
                    gvBatch.Rows.AddNew()
                    ...
                    '' new code 
                    Me.gvBatch.Rows(gvBatch.Rows.Count - 1).Cells(colPrevProdQty).Value = clsProductionEntryFinalQCHead.GetPrevProductionQty(txtBatchNo.Value, txtCode.Value, objTr.ITEM_CODE, Nothing)
            ...
            If obj.ArrIssueItem IsNot Nothing AndAlso obj.ArrIssueItem.Count > 0 Then
                For Each objtr As clsProductionEntryFinalQCIssueItems In obj.ArrIssueItem
                    gvIssue.Rows.AddNew()
            ...
            If obj.ArrStage IsNot Nothing AndAlso obj.ArrStage.Count > 0 Then
                For Each objtr As clsProductionEntryFinalQCStageDetail In obj.ArrStage
                    gvStage.Rows.AddNew()
            ...
            If obj.ArrQC IsNot Nothing AndAlso obj.ArrQC.Count > 0 Then
                ...
                For Each objtr As clsProductionEntryFinalQCQCDetail In obj.ArrQC
                    gvParameter.Rows.AddNew()
                    ...
                    End If
                Next
                UcAttachment1.LoadData(obj.QC_Code)
            End If
            If ShowoverheadCost = "1" Then
            ...
            loadBlankParameterGrid()
            For Each objTr As clsProductionEntryFinalQCParameterDetail In obj.ArrDetail
                gvParam.Rows.AddNew()
            ...
            Next
            For Each objTr As clsProductionEntryFinalQCQCParam In obj.ArrParamDetail
                Try
                    gvParam.Rows(objTr.LINE_NO).Cells(objTr.Param_Field_Code).Value = objTr.Param_Field_Value
                Catch ex As Exception
                End Try
            Next
        End If
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtCode._MYValidating
        ...
        check = clsProductionEntryFinalQCHead.CheckValidCode(Me.txtCode.Value)
        ...
        If txtCode.MyReadOnly Or isButtonClicked Then
            txtCode.Value = clsCommon.myCstr(clsProductionEntryFinalQCHead.GetFinder(" TSPL_PE_FINALQC_HEAD.location_code in (" + arrLoc + ")", txtCode.Value, isButtonClicked))
            LoadData(txtCode.Value, NavigatorType.Current)
        Else
            funReset()
        End If
    ...
    Private Sub frmProductionEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
            ButtonToolTip.SetToolTip(btnSave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_HEAD " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_DETAIL " + Environment.NewLine + _
                                                "TSPL_BATCH_ITEM " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_ISSUE_ITEM " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_STAGE_DETAIL " + Environment.NewLine + _
                                                "TSPL_PP_PE_STAGE_QC_LOG_SHEET " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_QC_DETAIL " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_WRECKAGE_FLUSHING_ITEM " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_SCRAP_ITEM " + Environment.NewLine + _
                                                "TSPL_PP_COST_WITHOUT_BATCH " + Environment.NewLine + _
                                                "Press Alt+P for Post Trasnaction " + Environment.NewLine + _
                                                "TSPL_PE_FINALQC_CONSUMPTION " + Environment.NewLine + _
    ...
    Private Sub txtBatchNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtBatchNo._MYValidating
        Try
            ...
            Me.txtBatchNo.Value = clsProductionEntryFinalQCHead.GetBatchFinder("Posted='1' and Quantity>coalesce([Production Quantity],0) and [Manual Closed]=0 and convert (date, Date,103) <= convert (date,'" + dtpDate.Value + "',103)  and [Location Code] in (" + arrLoc + ")", Me.txtBatchNo.Value, isButtonClicked)
            If clsCommon.myLen(Me.txtBatchNo.Value) > 0 Then
                ...
                If Not objBatch Is Nothing Then
                    ...
                    ''----------------
                    lblConsmSectionLocCode.Text = clsProductionEntryFinalQCHead.GetBatchConsumptionSection(txtLocation.Value, txtBatchNo.Value)
                    If clsCommon.myLen(lblConsmSectionLocCode.Text) <= 0 Then
                        lblConsmSectionCode.Text = ""
    ...
    Sub ShowBatchItems(ByVal objBatch As clsProcessBatchOrder)
        ...
        For Each obj As clsProcessBatchOrderMainDetail In objBatch.ArrMainItem
            ...
            Me.gvBatch.Rows(gvBatch.Rows.Count - 1).Cells(colPrevProdQty).Value = clsProductionEntryFinalQCHead.GetPrevProductionQty(objBatch.Batchcode, txtCode.Value, obj.icode, Nothing)
            ...
            ' Me.gvBatch.Rows(gvBatch.Rows.Count - 1).Cells(colFAT_Per).Value = clsProductionEntryFinalQCHead.GetLastStageParameterValue(Me.txtBatchNo.Value, "FAT")
            'Me.gvBatch.Rows(gvBatch.Rows.Count - 1).Cells(colSNF_Per).Value = clsProductionEntryFinalQCHead.GetLastStageParameterValue(Me.txtBatchNo.Value, "SNF")
    ...
    Sub FillIssueAgainstBatchOrder()
        ...
        Dim dt As DataTable = clsProductionEntryFinalQCHead.GetIssueAgainstBatch(Me.txtBatchNo.Value, Me.txtCode.Value)
    ...
    Private Sub gvWreckage_CellValueChanged(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.GridViewCellEventArgs) Handles gvWreckage.CellValueChanged
        If Not isInsideLoadData Then
            ...
            If Not isCellValueChanged Then
                ...
                '    isCellValueChanged = True
                '    gvWreckage.CurrentRow.Cells(colWFItem_Code).Value = clsProductionEntryFinalQCHead.getSectionStockItemFinder("", lblConsmSectionLocCode.Text, gvWreckage.CurrentRow.Cells(colWFItem_Code).Value, False)
                ...
                If e.Column Is gvWreckage.Columns(colWFItem_Code) Then
                    ...
                    arr1 = clsProductionEntryFinalQCHead.getSectionStockItemMultipleFinder("", lblConsmSectionLocCode.Text, arrList)
        ...
        If Not isInsideLoadData Then
            ...
            If Not isCellValueChanged Then
                ...
                If e.Column Is GvScrap.Columns(colScrapItem_Code) Then
                    ...
                    'arr1 = clsProductionEntryFinalQCHead.getSectionStockItemMultipleFinder("", lblConsmSectionLocCode.Text, arrList, "Scrap")
                    arr1 = clsProductionEntryFinalQCHead.getItemFinder("", "", arrList, "Scrap")
                    If arr1 IsNot Nothing AndAlso arr1.Count > 0 Then
                        For ii As Integer = 0 To arr1.Count - 1
                            If ii = 0 Then
    ...
    Private Sub btnunpost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnunpost.Click
        Try
            ...
            End If
            Dim qry As String = "select count(*) from TSPL_PE_FINALQC_HEAD where Posted='0' and QC_Code='" + txtCode.Value + "'"
            ...
            If common.clsCommon.MyMessageBoxShow("Amend and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                If clsProductionEntryFinalQCHead.UnpostData(txtCode.Value, Me.Form_ID) Then
                    '------------------
                    Dim obj As New clsCancelLog
    ...
    Sub FillSection()
        gvSectionStock.DataSource = clsProductionEntryFinalQCHead.GetSectionStock(lblConsmSectionLocCode.Text)
        gvSectionStock.AutoSizeColumnsMode = GridViewAutoSizeColumnsMode.Fill
    End Sub

    Sub FillSectionHistory(ByVal Item_Code As String)
        gvSectionStockHistory.DataSource = clsProductionEntryFinalQCHead.GetSectionStockHistory(lblConsmSectionLocCode.Text, Item_Code)
    ...
    Sub OpenScrapUnitCode(ByVal isButtonClick As Boolean)
        ...
        GvScrap.CurrentRow.Cells(colScrapUnit_Code).Value = clsCommon.ShowSelectForm("TSPLPEFQC", qry, "Code", WhrCls, GvScrap.CurrentRow.Cells(colScrapUnit_Code).Value, "", False)
    ...
    Sub OpenWFUnitCode(ByVal isButtonClick As Boolean)
        ...
        Dim qry As String = "select TSPL_ITEM_UOM_DETAIL.UOM_Code as Code,TSPL_ITEM_UOM_DETAIL.UOM_Description as [UOM Description] from TSPL_ITEM_UOM_DETAIL left outer join TSPL_ITEM_MASTER on tspl_item_master.item_code=tspl_item_uom_detail.item_code  "
        gvWreckage.CurrentRow.Cells(colWFUnit_Code).Value = clsCommon.ShowSelectForm("TSPLHEAPEFQCD", qry, "Code", WhrCls, gvWreckage.CurrentRow.Cells(colWFUnit_Code).Value, "", False)
    ...
    Sub LoadBlankGridCost()
        ...
        gvProductionCost.ReadOnly = True
    End Sub
    ...
    Sub loadBlankParameterGrid()
        ...
        repoTextColumn.Name = colParamFrom
        repoTextColumn.Width = 100
        repoTextColumn.HeaderText = "QC From"
        repoTextColumn.Tag = colParamFrom
        repoTextColumn.ReadOnly = True
        gvParam.MasterTemplate.Columns.Add(repoTextColumn)


        repoTextColumn = New GridViewTextBoxColumn()
        repoTextColumn.Name = colParamItemCode
        repoTextColumn.Width = 100
        repoTextColumn.HeaderText = "Item Code"
        repoTextColumn.Tag = colParamItemCode
        repoTextColumn.ReadOnly = True
        gvParam.MasterTemplate.Columns.Add(repoTextColumn)


        repoTextColumn = New GridViewTextBoxColumn()
        repoTextColumn.Name = colParamItemName
        repoTextColumn.Width = 100
        repoTextColumn.HeaderText = "Item Description"
        repoTextColumn.Tag = colParamItemName
        repoTextColumn.ReadOnly = True
        gvParam.MasterTemplate.Columns.Add(repoTextColumn)
    ...
    Sub fillDataFromStdQC()
        Dim intCount As Integer = 0
        Dim arrAdded As New List(Of String)
        For intCount = 0 To gvParameter.Rows.Count - 1
            Dim strTemp As String = clsCommon.myCstr(gvParameter.Rows(intCount).Cells(colQCType).Value) + clsCommon.myCstr(gvParameter.Rows(intCount).Cells(colQCItemCode).Value)
            If Not arrAdded.Contains(strTemp) Then
                gvParam.Rows.AddNew()
                arrAdded.Add(strTemp)
                gvParam.Rows(intCount).Cells(colParamSNO).Value = gvParam.Rows.Count
                gvParam.Rows(intCount).Cells(colParamFrom).Value = clsCommon.myCstr(gvParameter.Rows(intCount).Cells(colQCType).Value)
                gvParam.Rows(intCount).Cells(colParamItemCode).Value = clsCommon.myCstr(gvParameter.Rows(intCount).Cells(colQCItemCode).Value)
                gvParam.Rows(intCount).Cells(colParamItemName).Value = clsCommon.myCstr(gvParameter.Rows(intCount).Cells(colQCItemName).Value)
                'gvParam.Rows(intCount).Cells(colParamParentID).Value = clsCommon.myCstr(gvParameter.Rows(intCount).Cells(colQCParentLineNo).Value)
            End If
        Next
    End Sub

    Private Sub txtProductionEntryNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtProductionEntryNo._MYValidating
        Try
            Dim qry As String = "select * from (" + getFinderBaseQty() + ")xxxx"
            txtProductionEntryNo.Value = clsCommon.ShowSelectForm("ChBPEFinalQC", qry, "PROD_ENTRY_CODE", "", txtProductionEntryNo.Value, "", isButtonClicked)
            If clsCommon.myLen(txtProductionEntryNo.Value) > 0 Then
                FillProductionEntryData(txtProductionEntryNo.Value)
            End If
        Catch ex As Exception
    ...
    Private Function getFinderBaseQty() As String
        Dim qry As String = " select PROD_ENTRY_CODE,max(PROD_DATE) as PROD_DATE,max(Batch_Code) as Batch_Code,max(LOCATION_CODE) as Loaction_Code,max(ManualBatchNo) as ManualBatchNo from (" + Environment.NewLine + _
        "select PROD_ENTRY_CODE,PROD_DATE,Batch_Code,LOCATION_CODE,1 as RI,1 as Chk,ManualBatchNo  from TSPL_PP_PRODUCTION_ENTRY" + Environment.NewLine + _
        "where 2=2 and Posted=1 " + Environment.NewLine + _
        "union all" + Environment.NewLine + _
        "select Against_PE_Code as PROD_ENTRY_CODE,null as PROD_DATE,null as Batch_Code,null as LOCATION_CODE,-1 as RI,0 as chk,null as ManualBatchNo from TSPL_PE_FINALQC_HEAD where Status<>'P' and QC_Code not in (' " + txtCode.Value + "') " + Environment.NewLine + _
        ")xx group by PROD_ENTRY_CODE  having sum(ri)>0 and sum(chk)>0 "
        Return qry
    End Function

    Sub FillProductionEntryData(ByVal strCode As String)
        BlankAllControls()
        Dim obj As clsProductionEntry = clsProductionEntry.GetData(strCode, arrLoc, NavigatorType.Current)
        If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Batch_Code) > 0 Then

            Dim ii As Int16 = 0
            LoadBlankGrid()
            txtProductionEntryNo.Value = obj.PROD_ENTRY_CODE
            Me.txtDesc.Text = clsCommon.myCstr(obj.DESCRIPTION)
            Me.txtComment.Text = clsCommon.myCstr(obj.COMMENTS)
            TxtManualBatchNo.Text = obj.ManualBatchNo
            lblLineNo.Text = obj.LINE_NO
    ...
    Private Sub gvParam_CellValueChanged(sender As Object, e As GridViewCellEventArgs) Handles gvParam.CellValueChanged
        Try
            If Not isInsideLoadData Then
                If Not isCellValueChangedParam Then
                    isCellValueChangedParam = True
                    If e.Column Is gvParam.Columns(FATColName) Then
                        For ii As Integer = 0 To gvParameter.Rows.Count - 1
                            If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamItemCode).Value), clsCommon.myCstr(gvParameter.Rows(ii).Cells(colQCItemCode).Value)) = CompairStringResult.Equal Then
                                If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamFrom).Value), clsCommon.myCstr(gvParameter.Rows(ii).Cells(colQCType).Value)) = CompairStringResult.Equal Then
                                    'If clsCommon.myCdbl(gvParam.CurrentRow.Cells(colParamParentID).Value) = clsCommon.myCdbl(gvParameter.Rows(ii).Cells(colQCParentLineNo).Value) Then
                                    If clsCommon.CompairString("FAT", clsCommon.myCstr(gvParameter.Rows(ii).Cells(colQCparam_type).Value)) = CompairStringResult.Equal Then
                                        gvParameter.CurrentRow = gvParameter.Rows(ii)
                                        gvParameter.CurrentColumn = gvParameter.Columns(colActual_Range)
                                        gvParameter.Rows(ii).Cells(colActual_Range).Value = clsCommon.myCdbl(gvParam.CurrentRow.Cells(FATColName).Value)
                                        'UpdateBatchFatSNF(clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCItemCode).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colActual_Range).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCparam_type).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCType).Value))
                                    End If
                                    'End If
                                End If
                            End If
                        Next
                    ElseIf e.Column Is gvParam.Columns(SNFColName) Then
                        For ii As Integer = 0 To gvParameter.Rows.Count - 1
                            If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamItemCode).Value), clsCommon.myCstr(gvParameter.Rows(ii).Cells(colQCItemCode).Value)) = CompairStringResult.Equal Then
                                If clsCommon.CompairString(clsCommon.myCstr(gvParam.CurrentRow.Cells(colParamFrom).Value), clsCommon.myCstr(gvParameter.Rows(ii).Cells(colQCType).Value)) = CompairStringResult.Equal Then
                                    'If clsCommon.myCdbl(gvParam.CurrentRow.Cells(colParamParentID).Value) = clsCommon.myCdbl(gvParameter.Rows(ii).Cells(colQCParentLineNo).Value) Then
                                    If clsCommon.CompairString("SNF", clsCommon.myCstr(gvParameter.Rows(ii).Cells(colQCparam_type).Value)) = CompairStringResult.Equal Then
                                        gvParameter.CurrentRow = gvParameter.Rows(ii)
                                        gvParameter.CurrentColumn = gvParameter.Columns(colActual_Range)
                                        gvParameter.Rows(ii).Cells(colActual_Range).Value = clsCommon.myCdbl(gvParam.CurrentRow.Cells(SNFColName).Value)
                                        'UpdateBatchFatSNF(clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCItemCode).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colActual_Range).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCparam_type).Value), clsCommon.myCstr(gvParam.CurrentRow.Cells(colQCType).Value))
                                    End If
                                    'End If
                                End If
    ...
    Sub SetReadOnlyParameters()
        Try
            Dim isManadatory As Integer = 0
            For i As Integer = 0 To gvParam.Columns.Count - 1
                gvParam.Columns(i).ReadOnly = Not (clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select IsProductionMandatory from TSPL_PARAMETER_MASTER where Code='" & gvParam.Columns(i).Name & "'")) = 0)
            Next
            gvParam.Columns(FATColName).ReadOnly = True
            gvParam.Columns(SNFColName).ReadOnly = True
            gvParam.Columns(colParamSNO).ReadOnly = True
            gvParam.Columns(colParamItemCode).ReadOnly = True
            gvParam.Columns(colParamItemName).ReadOnly = True
            gvParam.Columns(colParamFrom).ReadOnly = True
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtCode.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationality.ShowTransHistoryData(txtCode.Value, "QC_Code", "TSPL_PE_FINALQC_HEAD", "TSPL_PE_FINALQC_DETAIL")
        Catch ex As Exception
