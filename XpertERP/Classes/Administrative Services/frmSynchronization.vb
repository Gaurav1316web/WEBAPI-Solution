...
#End Region
    ...
    Sub LoadTargetTables()
        gvTargetTables.Rows.Clear()
        Dim qry As String = "select SEQ_NO,TABLE_NAME,PRIMARY_KEY,Is_Detail  from TSPL_SYNC_TABLES order by TABLE_NAME"
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        For Each dr As DataRow In dt.Rows
            gvTargetTables.Rows.AddNew()
    ...
    Private Sub btnSyncClient_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSyncClient.Click
        Sync()
    End Sub
    Function Sync(Optional ByVal _Type As String = "All") As Boolean
