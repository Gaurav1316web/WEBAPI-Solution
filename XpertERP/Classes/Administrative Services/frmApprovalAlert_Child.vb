...
#End Region
    ...
    Private Sub LoadData(ByVal ScreenCode As String, ByVal Document_Code As String)
        ...
        Try
            Dim qry As String = "select max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Created_By) as created_by,max(No_Of_Level) as max_app_lvl,max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Document_Code) as doc_code,max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Document_Date) as doc_date,case when isnull(max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Status),'')='' then 'Pending' else max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Status) end as Doc_Status,max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Created_Date) as created_date,max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Amount) as amt,max(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Qty) as qty " & _
                                ",max(isnull(is_posted,0)) as posted from TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL where TRANS_Code='" + ScreenCode + "' and Document_Code='" + Document_Code + "' and is_reverse=0 "
            dt = New DataTable()
    ...
    Private Sub LoadGridData()
        Dim dt As New DataTable()
        Try
            Dim qry As String = "select TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.no_of_level as [Level No],tspl_user_master.user_name as [Authorizer],case when isnull(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Status,'')='' then '' else cast(TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.modified_date as varchar) end as [Authorized Date],TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.Status,TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.approval_remark as [Remarks] " & _
                                " from TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL left outer join tspl_user_master on tspl_user_master.user_code=TSPL_APPROVAL_LEVEL_TRANSACTION_DETAIL.user_code " & _
    ...
    Private Sub SaveData()
        ...
        Try
            ...
            If ((clsCommon.CompairString(ScreenCode, "PO-REQ") = CompairStringResult.Equal) Or (clsCommon.CompairString(ScreenCode, "PO-ODR") = CompairStringResult.Equal)) AndAlso clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "UDL") = CompairStringResult.Equal AndAlso clsCommon.myCstr(cboApproval.SelectedValue) <> "Rejected" Then
            Else
