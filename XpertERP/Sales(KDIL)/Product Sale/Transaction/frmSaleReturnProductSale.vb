...
Public Class frmSaleReturnProductSale
    Inherits FrmMainTranScreen
    '=================preeti Gupta Against Ticket No [KDI/03/04/18-000178][this ticket fixed by Priti Mam]
#Region "Variables"
    ...
    Const colInvoiceCashSchemeCode As String = "colInvoiceCashSchemeCode"
    ...
    Dim CancelDoc As Boolean = False
    Dim SaleInvoiceDate As DateTime
    Dim EInvoiceType As String = ""
#End Region
    Public Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.frmSaleReturnProductSale)
    ...
    Private Sub FrmAPInvoiceEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        LoadBlankGrid()
        CancelDoc = IIf(clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.DocumentCancelReturn, clsFixedParameterCode.DocumentCancelReturn, Nothing)) = 1, True, False)
        LoadBlankGridTax()
        LoadItemType()
        LoadBlankGridAC()
        AddNew()
    ...
    End Sub
    Public Function GetReturnType() As DataTable
        ...
        If clsCommon.myCBool(CancelDoc) = True Then
            dr = dt.NewRow()
            dr("Code") = "C"
            dr("Name") = "Cancel Type"
            dt.Rows.Add(dr)
    ...
    Sub LoadBlankGrid()
        ...
        Dim repoInvoiceCashSchemeCode As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoInvoiceCashSchemeCode.HeaderText = "Invoice Cash Scheme Code"
        repoInvoiceCashSchemeCode.Name = colInvoiceCashSchemeCode
        repoInvoiceCashSchemeCode.Width = 80
        repoInvoiceCashSchemeCode.ReadOnly = True
        repoInvoiceCashSchemeCode.IsVisible = False
        gv1.MasterTemplate.Columns.Add(repoInvoiceCashSchemeCode)
            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colTotTaxAmt) Then
                        ...
                        UpdateAllTotals()
                    End If
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colDamageQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colUnit) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) OrElse e.Column Is gv1.Columns(colReturnAmt) Then
                        If (e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colDamageQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colDisPer) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) OrElse e.Column Is gv1.Columns(colReturnAmt)) Then
    ...
    Sub OpenICodeListCurrentCalaculation(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("SRPDWQ@1i", qry)
    ...
    Sub OpenItemList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(txtReqNo.Value) = 0 Then
            ...
            whrCls = " inout='I' and Is_FreshItem=0 and Product_Type not in ('MI') and Item_Type='F' and Is_Serial_Item=0 " & strTax
    ...
    Function AllowToSave() As Boolean
        Try
           ...
           "where TSPL_RECEIPT_HEADER.Posted ='N' and tspl_sd_sale_invoice_head.Trans_Type='PS' and tspl_sd_sale_invoice_head.Screen_Type='' and TSPL_SD_SALE_INVOICE_HEAD.Document_Code='" & txtReqNo.Value & "' for xml path('')),'')  as DocNo ")
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    If clsCommon.myLen(strReqNo) > 0 Then
                        ...
                        End If
                    End If
                    ''added by richa agarwal ERO/13/12/18-000440
                    ...
                    If clsCommon.CompairString(clsCommon.myCstr(cboItemType.SelectedValue), "F") = CompairStringResult.Equal AndAlso IsBatchDetailMandatory(gv1.Rows(ii).Cells(colUnit).Value) Then
                        ...
                        If clsCommon.myLen(gv1.Rows(ii).Cells(colBatchNo).Value) <= 0 Then
                            ...
                            Return False
                        End If
                        If clsCommon.myLen(gv1.Rows(ii).Cells(colExpiry).Value) <= 0 Then '============Added by Preeti Gupta================
                            If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                                If clsCommon.myLen(gv1.Rows(ii).Cells(colReturnAmt).Value) <= 0 Then
                                    common.clsCommon.MyMessageBoxShow("Please enter Return Amount for " + strIName + ". At Line No" + clsCommon.myCstr(ii + 1))
                                    Return False
                                End If
                            End If
                            '=================================================
                    ...
                    If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                        If clsCommon.myCdbl(gv1.Rows(ii).Cells(colReturnAmt).Value) <= 0 Then
                            common.clsCommon.MyMessageBoxShow("Please enter Return Amount for " + strIName + ". At Line No" + clsCommon.myCstr(ii + 1))
                            Return False
                        End If
                    End If
    ...
    Sub SaveData(ByVal ChekPostBtn As Boolean)
        Try
            ...
            If (AllowToSave()) Then
                ...
                Dim obj As New clsPSSalesReturnHead()
                ...
                If clsCommon.CompairString(obj.Return_Type, "D") = CompairStringResult.Equal Then
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsPSSalesReturnDetail()
                    ...
                    objTr.InvoiceCashScheme_Code = clsCommon.myCstr(grow.Cells(colInvoiceCashSchemeCode).Value)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsPSSalesReturnHead()
            obj = clsPSSalesReturnHead.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                'Dim isEnableDisable As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select count(*) from TSPL_Sale_Return_Gate_Entry_Invoice_Wise where Gate_Entry_No ='" + obj.Gate_Entry_No + "'  and Invoice_No = '" + obj.Against_Invoice_No + "'"))
                ...
                End If
                If clsCommon.myLen(txtReqNo.Value) > 0 Then
                    SaleInvoiceDate = clsCommon.myCDate(clsDBFuncationality.getSingleValue("select document_date from TSPL_SD_SALE_INVOICE_HEAD where TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='PS' and Document_Code='" + txtReqNo.Value + "'"))
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsPSSalesReturnDetail In obj.Arr
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colInvoiceCashSchemeCode).Value = objTr.InvoiceCashScheme_Code
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                End If
                ''
                If (clsPSSalesReturnHead.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ...
            If clsCommon.myLen(strwherecls) > 0 Then
                strcondition = "and TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ") and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS'"
            End If

            '-----------------------------------------------------
            Dim qst As String = "select count(*) from TSPL_SD_SALE_RETURN_HEAD where Document_Code='" + txtDocNo.Value + "' " + strcondition + " and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type=''"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' and TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' "
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ") and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' "
        Else
            whrClas = " TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' "
        End If
        '-----------------------------------------------------

        LoadData(clsCommon.ShowSelectForm("PSSaleReturnDocfnd", qry, "Code", whrClas, txtDocNo.Value, "TSPL_SD_SALE_RETURN_HEAD.Document_Date desc", isButtonClicked, "TSPL_SD_SALE_RETURN_HEAD.Document_Date"), NavigatorType.Current)
    End Sub
    ...
    Sub SelectMRNItems()
        isInsideLoadData = True
        Dim frm As New frmPendingSaleInvoicePS()
        frm.VendorCode = txtVendorNo.Value
        frm.strCurrCode = txtDocNo.Value
        frm.ReturnType = ddlReturnType.SelectedValue
        frm.CancelDesc = CancelDoc
        frm.CurrentInvoiceNo = txtReqNo.Value
                    ...
                    For Each obj As clsPSInvoiceHeadDetail In objOrderHead.Arr
                        ...
                        If (obj.Item_Code = frm.ArrReturn(ii).Item_Code AndAlso obj.Unit_code = frm.ArrReturn(ii).Unit_code AndAlso obj.Scheme_Item = "N") Then ''OrElse (obj.Scheme_Code = frm.ArrReturn(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            ...
                            If IsValidItem(obj) Then
                                ...
                                If clsCommon.CompairString(ddlReturnType.Text, "Price Only") = CompairStringResult.Equal Then
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = 0
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = 0
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colBalanceQty).Value = 0
                                Else
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colInvoiceCashSchemeCode).Value = obj.Cash_Scheme_Code
    ...
    Public Sub funPrint(ByVal StrCode As String, Optional ByVal isPDF As Boolean = False)
        ...
        Try
            ...
            Dim StrSql = "Select Document_Date,Customer_Code,Bill_To_Location,is_taxable,Tax_Group from TSPL_SD_SALE_RETURN_HEAD where Document_Code in ('" & StrCode & "')"
            ...
            Dim Qry As String = "  select cast(TSPL_SD_SALE_RETURN_HEAD.BarCode_Img as image) As BarCode_Img,isnull (TSPL_SD_SALE_RETURN_HEAD.IRN_No,'') as IRN_No,isnull (TSPL_SD_SALE_RETURN_HEAD.Ack_No,'') as Ack_No,case when len(isnull (TSPL_SD_SALE_RETURN_HEAD.Ack_No,'')) > 0 then convert (varchar, TSPL_SD_SALE_RETURN_HEAD.Ack_Date,103) else ''  end as Ack_Date , case when TSPL_SD_SALE_RETURN_HEAD.Is_Taxable=1 and isnull(TSPL_SD_SALE_RETURN_HEAD.EInvoice_Type,'')='BB' AND convert(date ,TSPL_SD_SALE_RETURN_HEAD.Document_Date,103)>=convert(date ,'" + clsCommon.myCstr(DateOfEInvoiceImplementation) + "',103) then 1 else 0 end as  IsEInvoiceApply, " &
                " TSPL_SD_SALE_RETURN_HEAD.RoundOffAmount,isnull(TSPL_SD_SALE_RETURN_HEAD.HeadDisc_Amt,0)+isnull(TSPL_SD_SALE_RETURN_HEAD.HeadDisc_peramt,0) as HeadDisc_Amt,isnull(TSPL_ITEM_MASTER.Is_Batch_Item,0) as Is_Batch_Item," &
...
"isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX9_Rate,0) as DTax9_Rate,  isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX10_Rate,0) as DTax10_Rate,TSPL_SD_SALE_RETURN_DETAIL.Total_Tax_Amt as TaxableAmount" &
        ...
        Try
            If e.Column.Index >= 0 Then
                    ...
                    Else
                        If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                            ...
                            ElseIf clsCommon.myLen(txtReqNo.Value) > 0 AndAlso clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                                ...
                                gv1.CurrentRow.Cells(colDamageQty).ReadOnly = True
                                gv1.CurrentRow.Cells(colFreeQty).ReadOnly = True
                                gv1.CurrentRow.Cells(colReturnAmt).ReadOnly = False
                            Else
                                gv1.CurrentRow.Cells(colQty).ReadOnly = False
                                gv1.CurrentRow.Cells(colFreeQty).ReadOnly = False
                                gv1.CurrentRow.Cells(colDamageQty).ReadOnly = False
                                gv1.CurrentRow.Cells(colReturnAmt).ReadOnly = True
                            End If
                        Else
                            gv1.CurrentRow.Cells(colQty).ReadOnly = True
                            gv1.CurrentRow.Cells(colFreeQty).ReadOnly = True
                ...
                ElseIf e.Column Is gv1.Columns(colUnit) Then
                    ...
                    End If
                    '=================Added by preeti============
                    'ElseIf e.Column Is gv1.Columns(colReturnAmt) Then
                    '    If clsCommon.CompairString(ddlReturnType.SelectedValue, "P") = CompairStringResult.Equal Then
                    '        gv1.CurrentRow.Cells(colReturnAmt).ReadOnly = False
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        Try
            ...
            Dim strSchemeCode As String = clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colInvoiceCashSchemeCode).Value)
            ...
            If clsCommon.CompairString(ddlReturnType.Text, "Price only") = CompairStringResult.Equal Then
                dblReturnAmt = gv1.Rows(IntRowNo).Cells(colReturnAmt).Value
            Else
    ...
    Sub OpenBatchItem()
        ...
        If RunBatchFifowise = 0 Then
            If clsCommon.myCBool(gv1.CurrentRow.Cells(colIsBatchItem).Value) Then
                ...
                frm.dblqty = clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value)
                frm.strSplTransaction = "PSSaleReturn"
    ...
    Private Sub gv1_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles gv1.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F5 Then
            '======update by ERO/13/12/18-000440 richa agarwal 17 MAy,2019
            If RunBatchFifowise = 0 Then
                OpenBatchItem()
            Else
        ...
        End If
    End Sub
    '============ ERO/13/12/18-000440 richa agarwal 17 MAy,2019===============
...
#Region "Email SMS Setting"
    ...
    Private Sub SendSMSandEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        Try
            ...
            Dim strContactperson As String = ""
            Dim dtContent As DataTable = clsDBFuncationality.GetDataTable("SELECT SMS_Text,Email_Text,Email_subject from TSPL_ES_Content where Form_ID='" + clsUserMgtCode.frmSaleReturnProductSale + "'", Nothing)
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                objEmailH.Email_Text = objEmailH.Email_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSaleReturnProductSale)
                ...
                objEmailH.SaveData(clsUserMgtCode.frmSaleReturnProductSale, objEmailH, Nothing)
                objEmailH = Nothing
                clsCommon.MyMessageBoxShow("E-Mail Send Successfully", Me.Text)
                If Not clsSMSAtPost_Sales.SMSATPOST_SALE() Then
                    SMSSENDONLY(False)
        ...
        Try
            ...
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                If clsCommon.myLen(dtContent.Rows(0)("SMS_Text")) > 0 Then
                    ...
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSaleReturnProductSale)
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.ContactPerson, strContactPerson)

                    objSMSH.SaveData(clsUserMgtCode.frmSaleReturnProductSale, objSMSH, Nothing)
                    objSMSH = Nothing
                    If Not isPost Then
                        clsCommon.MyMessageBoxShow("SMS Send Successfully", Me.Text)
                    End If
...
#End Region
    ...
    Private Sub fndGateReturnNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndGateReturnNo._MYValidating
        fndGateReturnNo.Value = clsGateEntryReturnPS.getFinder("Posted=1", fndGateReturnNo.Value, isButtonClicked)
        If clsCommon.myLen(fndGateReturnNo.Value) Then
            txtReqNo.Value = clsDBFuncationality.getSingleValue("Select REF_DOC_No from TSPL_GATE_ENTRY_RETURN_PS where GE_CODE='" & fndGateReturnNo.Value & "'")
            If clsCommon.myLen(txtReqNo.Value) > 0 Then
                SelectMRNItems()
            End If
        End If
    ...
    Private Sub ddlReturnType_SelectedValueChanged(sender As Object, e As EventArgs) Handles ddlReturnType.SelectedValueChanged
        If ddlReturnType.Items.Count > 0 AndAlso ddlReturnType.SelectedIndex >= 0 AndAlso loading = 0 Then
            If clsCommon.CompairString(ddlReturnType.SelectedValue, "D") = CompairStringResult.Equal Then
                pnl_damage.Visible = False 'done by stuti 18/04/2017 for adinath
            Else
    ...
    Private Sub btnReverseAndUnpost_Click(sender As Object, e As EventArgs) Handles btnReverseAndUnpost.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsPSSalesReturnHead.ReverseAndUnpost(txtDocNo.Value) Then
    ...
    Private Sub fndGateEntryNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndGateEntryNo._MYValidating
        Try
            ...
            WhrCls = " Doc_Type = 'PS' and POSTED = 1  and Gate_Entry_No not in ( select case when TSPL_SD_SALE_RETURN_HEAD.Gate_Entry_No is Null then '' else TSPL_SD_SALE_RETURN_HEAD.Gate_Entry_No end  from TSPL_SD_SALE_RETURN_HEAD ) "  'and POSTED = 1
            If clsCommon.myLen(txtVendorNo.Value) > 0 Then
    ...
    Private Sub btnShowInventory_Click(sender As Object, e As EventArgs) Handles btnShowInventory.Click
        ...
        'Try
        '    If clsCommon.myLen(txtDocNo.Value) > 0 Then
        '        clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmSaleReturnProductSale, txtDocNo.Value)
        '    Else
        '        clsCommon.MyMessageBoxShow("No document found", Me.Text)
        '    End If
    ...
    Function CancelData() As Boolean
        Try
            ...
            clsPSSalesReturnHead.CancelData(Me.Form_ID, txtDocNo.Value, NavigatorType.Current)
