...
Imports System.IO
'============Shivani Tyagi
Public Class FrmShortCloseDOPS
    ...
    Sub LoadData()
        ...
        Dim Qry As String = "select CAST(1 as bit) as Sel,Document_No,Document_Date,max(Location) as Location,MAX(TSPL_LOCATION_MASTER.Location_Desc) as LocationName , " & _
            "MAX(Vendor) as Vendor,MAX(TSPL_CUSTOMER_MASTER.Customer_Name) as VendorName, MAX(Total_Amt) as Document_Amount,SUM(Qty) as POQty from ( " & _
            "select TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Posted,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Date as Document_Date , " & _
            ...
            "TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Total_Amt,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Qty,1 as RI from  TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE left outer join " & _
            "TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE on TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Code=TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Document_Code " & _
            "where isnull(TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Short_Close,'')='N'    and Posted=1  " & _
            "Union all " & _
            "select TSPL_SD_SHIPMENT_HEAD.Status,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Date as Document_Date , TSPL_SD_SHIPMENT_DETAIL.Delivery_Code_PS as Document_No, " & _
            "TSPL_SD_SHIPMENT_HEAD.Customer_Code as Vendor, TSPL_SD_SHIPMENT_HEAD.Bill_To_Location as Location,TSPL_SD_SHIPMENT_HEAD.Document_Date as TransDate, " & _
            "TSPL_SD_SHIPMENT_HEAD.Total_Amt,-1 * TSPL_SD_SHIPMENT_DETAIL.Qty,-1 as RI from  TSPL_SD_SHIPMENT_HEAD left outer join " & _
            "TSPL_SD_SHIPMENT_DETAIL on TSPL_SD_SHIPMENT_HEAD.Document_Code=TSPL_SD_SHIPMENT_DETAIL.Document_Code " & _
            ...
            "group by Document_No,Document_Date having convert(date ,Document_Date,103)='" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "' " & _
            "and  SUM(Qty) > 0  order by Document_No"
