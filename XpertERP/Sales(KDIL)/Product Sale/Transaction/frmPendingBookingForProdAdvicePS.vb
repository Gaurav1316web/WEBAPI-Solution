...
Imports common
Public Class frmPendingBookingForProdAdvicePS
#Region "Variables"
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'End If
        'qry += " union all" & _
        '" select TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code as Code,TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Customer_Code as Vendor,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Item_Code as ICode,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Item_Desc as IName,'' as IType,(isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.SRN_Qty,0) +isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Leak_Qty,0)+isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Burst_Qty,0)+isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Short_Qty,0)+isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Rejected_Qty,0) ) as Qty,0 as Unapproved,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.MRP,0) as MRP,'' as Batch_No,null as MFG_Date,null as Expiry_Date,0 as Disc_Per,null as TransDate,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Assessable,0)as Assessable,0 as Leak_Qty,0 as Burst_Qty,0 as Short_Qty from TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE left outer join TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE on TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.SRN_No=TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.SRN_No where TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Status=1 and len(isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code,''))>0  " & _
        '" union all  " & _
        '" select TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code as Code,TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Customer_Code as Vendor,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Item_Code as ICode,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Item_Desc as IName,'' as IType,0  as Qty,(isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.SRN_Qty,0) +isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Leak_Qty,0)+isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Burst_Qty,0)+isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Short_Qty,0)+isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Rejected_Qty,0)) as Unapproved,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.MRP,0) as MRP,'' as Batch_No,null as MFG_Date,null as Expiry_Date,0 as Disc_Per,null as TransDate,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Assessable,0) as Assessable,0 as Leak_Qty,0 as Burst_Qty,0 as Short_Qty from TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE left outer join TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE on TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.SRN_No=TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.SRN_No  where TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Status=0 and len(isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code,''))>0 and TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.SRN_No not in ('" + strCurrCode + "')   " & _
        ...
        qry += " where TSPL_BOOKING_DETAIL_PRODUCTSALE.Status=0 and TSPL_BOOKING_MASTER_PRODUCTSALE.Itemwise=1 and Scheme_Item='N'  and TSPL_BOOKING_MASTER_PRODUCTSALE.Status=1 and TSPL_BOOKING_MASTER_PRODUCTSALE.close_yn='N'  and TSPL_BOOKING_MASTER_PRODUCTSALE.Document_Code not in (Select Booking_Code from TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE) " + Environment.NewLine
        If clsCommon.myLen(VendorCode) > 0 Then
            qry += " and TSPL_BOOKING_MASTER_PRODUCTSALE.Customer_Code='" + VendorCode + "'"
        End If

        qry += " union all " + Environment.NewLine
        qry += " select TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Line_No,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code as Code,TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Customer_Code as Vendor,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Item_Code as ICode,'' as IName,'' as IType,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Qty,0) as Qty,0 as Unapproved,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Assessable,0)as Assessable" + Environment.NewLine
        qry += " ,Scheme_Code from TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE " + Environment.NewLine
        qry += " left outer join TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE on TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Document_Code=TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Document_Code" + Environment.NewLine
        qry += " where TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Status=1  and TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Itemwise=1 and Scheme_Item='N'  and len(isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code,''))>0   " + Environment.NewLine
        qry += " union all   " + Environment.NewLine
        qry += " select TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Line_No,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code as Code,TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Customer_Code as Vendor,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Qty,0) as Unapproved,TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Assessable,0)as Assessable" + Environment.NewLine
        qry += " ,Scheme_Code from TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE " + Environment.NewLine
        qry += " left outer join TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE on TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Document_Code=TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Document_Code" + Environment.NewLine
        qry += " where TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Status=0 and TSPL_DISPATCH_ADVICE_HEAD_PRODUCTSALE.Itemwise=1  and Scheme_Item='N'  and len(isnull(TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.Booking_Code,''))>0 and TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
        ...
        If IsAllowSingleSI4SingleSO = True Then
            qry += " WHERE Code Not in (Select Distinct Booking_Code from TSPL_DISPATCH_ADVICE_DETAIL_PRODUCTSALE WHERE ISNULL(Booking_Code,'')<>'') " + StrCondition + " "
        Else
            qry += "" + StrCondition1 + "  "
        End If
