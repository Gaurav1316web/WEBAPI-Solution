...
Public Class frmPendingSaleInvoicePS
#Region "Variables"
    Public FromDate As Date?
    Public Todate As Date?
    Public CurrentInvoiceNo As String = Nothing
    Dim IsInsideLoadData As Boolean = False
    Public VendorCode As String = Nothing
    Public VendorName As String = Nothing
    Public ReturnType As String = Nothing
    Public strCurrCode As String = Nothing
    Public CancelDesc As Boolean = Nothing
...
#End Region
    ...
    Sub LoadData()
        Try
            ...
            qry += " where TSPL_SD_SALE_INVOICE_DETAIL.Status=0 and Scheme_Item='N' and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='PS' AND TSPL_SD_SALE_INVOICE_HEAD.Screen_Type=''  and TSPL_SD_SALE_INVOICE_HEAD.Status=1 " + Environment.NewLine
            qry += " and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) " + Environment.NewLine

            If clsCommon.myLen(VendorCode) > 0 Then
                qry += " and TSPL_SD_SALE_INVOICE_HEAD.Customer_Code='" + VendorCode + "'"
            End If
            If clsCommon.myLen(CurrentInvoiceNo) > 0 Then
                qry += " and TSPL_SD_SALE_INVOICE_HEAD.Document_Code='" + CurrentInvoiceNo + "'"
            End If
            If clsCommon.myCBool(CancelDesc) = True Then
                If clsCommon.CompairString(ReturnType, "C") = CompairStringResult.Equal Then
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.CancelFlag is not null "
                Else
                    qry += " and TSPL_SD_SALE_INVOICE_HEAD.CancelFlag is null "
                End If

            End If
            qry += " union all " + Environment.NewLine
            qry += " select TSPL_SD_SALE_RETURN_DETAIL.Line_No,TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code as Code,TSPL_SD_SALE_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_RETURN_DETAIL.Item_Code as ICode,'' as IName,'' as IType,isnull(TSPL_SD_SALE_RETURN_DETAIL.Qty,0) + isnull(TSPL_SD_SALE_RETURN_DETAIL.DamageQty,0) as Qty,TSPL_SD_SALE_RETURN_DETAIL.Amount,0 as Unapproved,TSPL_SD_SALE_RETURN_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date" + Environment.NewLine
            ...
            qry += " where  TSPL_SD_SALE_RETURN_HEAD.Status=1 and Scheme_Item='N' and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' AND TSPL_SD_SALE_RETURN_HEAD.Screen_Type=''  and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0   " + Environment.NewLine
            qry += " and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) " + Environment.NewLine

            If clsCommon.myLen(CurrentInvoiceNo) > 0 Then
                qry += " and TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code='" + CurrentInvoiceNo + "'"
            End If
            qry += " union all   " + Environment.NewLine
            qry += " select TSPL_SD_SALE_RETURN_DETAIL.Line_No,TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code as Code,TSPL_SD_SALE_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_RETURN_DETAIL.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,0 as Amount,isnull(TSPL_SD_SALE_RETURN_DETAIL.Qty,0)  + isnull(TSPL_SD_SALE_RETURN_DETAIL.DamageQty,0)  as Unapproved,TSPL_SD_SALE_RETURN_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date" + Environment.NewLine
            qry += " ,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALE_RETURN_HEAD on TSPL_SD_SALE_RETURN_HEAD.Document_Code=TSPL_SD_SALE_RETURN_DETAIL.Document_Code" + Environment.NewLine
            qry += " left outer join TSPL_SD_SALE_INVOICE_HEAD on TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code=TSPL_SD_SALE_INVOICE_HEAD.Document_Code " + Environment.NewLine
            qry += " where  TSPL_SD_SALE_RETURN_HEAD.Status=0 and Scheme_Item='N' and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS'  AND TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0 and TSPL_SD_SALE_RETURN_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
            qry += " and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) " + Environment.NewLine
            ...
            qry += " group by Code,ICode,Unit,MRP having SUM(Chk)>0 and (SUM(Qty *RI) <>0 and SUM(Amount  *RI) <>0) order by Code,max(Line_No)"
