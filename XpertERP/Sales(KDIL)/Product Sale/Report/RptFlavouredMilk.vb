...
'' Updation by Richa Agarwal Against Ticket No BM00000003705,BM00000006866
'update by preeti gupta Against ticket No[BM00000008203,ERO/23/04/19-000569]
'BM00000008317,BM00000008318,BM00000008319,BM00000008187,BM00000008525,BM00000008505
'' Work to be done agaist ticket no. BHA/22/11/18-000698
Imports common
...
#End Region
    ...
    Sub LoadType()
        ...
        dt.Columns.Add("Code", GetType(String))
        Dim dr As DataRow = dt.NewRow()
        If clsCommon.CompairString(FORMTYPE, "STO-REC-RPT") = CompairStringResult.Equal Or clsCommon.CompairString(FORMTYPE, "MIS-SRec-RPT") = CompairStringResult.Equal Then
    ...
    Private Sub SetUserMgmtNew()
        ...
        btnExport_new.Visible = MyBase.isExport
    ...
    Private Sub RadButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton1.Click
        Try
            PageSetupReport_ID = MyBase.Form_ID 'GetReportID()
    ...
    Private Sub LoadData(ByVal isPrintCrystalReport As Integer)
        Try
            ...
            End If
            'If ChkMRPWise.Checked = True AndAlso (clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Wise Summary") = CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal) Then
            '    qry += ",MRP "
            'End If
            ...
            ' ''==Added Parteek
            '' Commented by Panch Raj on 10-02-2017 because invalid join of production table  applied with inventorymovement table. Joining with inventorymovement with production table is resulting in duplicate records because no proper relation exists.
            If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Transaction Wise") = CompairStringResult.Equal Then
                ' qry += " ,case TSPL_PP_PRODUCTION_ENTRY_DETAIL.FAT_KG when TSPL_PP_PRODUCTION_ENTRY_DETAIL.FAT_KG then TSPL_PP_PRODUCTION_ENTRY_DETAIL.FAT_KG else -1 end as Prod_Fat_KG"
                ...
                '  qry += " ,case TSPL_PP_PRODUCTION_ENTRY_DETAIL.SNF_KG when TSPL_PP_PRODUCTION_ENTRY_DETAIL.SNF_KG then TSPL_PP_PRODUCTION_ENTRY_DETAIL.SNF_KG else -1 end as Prod_SNF_KG"
            ...
            ''==End
            qry += " from ( "
            qry += " select Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType  from TSPL_INVENTORY_MOVEMENT " + Environment.NewLine
            qry += " union all " + Environment.NewLine
            qry += " select Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,1 as IsFromMilk,Fat_Per as MilkFatPer ,SNF_Per as MilkSNFPer,Fat_KG as MilkFATKG,SNF_KG as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType from TSPL_INVENTORY_MOVEMENT_NEW" + Environment.NewLine
            ...
            'InnerOpClo = "  SUM(STOCK_QTY * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPBal]  ," & _
            '                " SUM(Cost  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPBalCost]  , " & _
            '                " SUM(QtyKG * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS OPQTYKG ," & _
            '                " SUM((case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end)  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPFAT], " & _
            '                " SUM((case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end)  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [OPSNF], " & _
            ...
            '                " SUM(STOCK_QTY * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Balance_Qty]," & _
            '                " SUM(cost * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Cost], " & _
            '                " SUM(QtyKG * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS Balance_QtyKG," & _
            '                " SUM((case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end ) * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end))  AS [Balance_FAT]," & _
            ...
            InnerOpClo = "  STOCK_QTY * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [OPBal]  ," & _
                            " Cost  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [OPBalCost]  , " & _
                            " QtyKG * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as OPQTYKG ," & _
                            " (case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end)  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [OPFAT], " & _
                            " (case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end)  * (CASE WHEN PUNCHING_DAte < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [OPSNF], " & _
                            " STOCK_QTY * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end) as Received_Qty , " & _
                            " Cost * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end) as RecdCost , " & _
                            " QtyKG * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end) as Received_QtyKG , " & _
                            " (case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end) as Received_FAT , " & _
                            " (case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else 0 end) as Received_SNF , " & _
                            " STOCK_QTY * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end) as Issued_Qty," & _
                            " Cost * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end) as IssueCost," & _
                            " QtyKG * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end) as Issued_QtyKG," & _
                            " (case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end) as Issued_FAT," & _
                            " (case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end ) * (CASE WHEN PUNCHING_DAte >= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' AND PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' THEN 1 ELSE 0 end) * (case when InOut='I' then 0 else 1 end) as Issued_SNF," & _
                            " STOCK_QTY * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [Balance_Qty]," & _
                            " cost * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [Cost], " & _
                            " QtyKG * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as Balance_QtyKG," & _
                            " (case when IsFromMilk=1 then MilkFATKG else (STOCK_QTY*FatPer) end ) * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [Balance_FAT]," & _
                            " (case when IsFromMilk=1 then MilkSNFKG else (STOCK_QTY*SNFPer) end ) * (CASE WHEN PUNCHING_DAte <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'  THEN 1 ELSE 0 end) * (case when InOut='I' then 1 else -1 end) as [Balance_SNF]"


            Dim strFinalQry As String = ""
            strFinalQry = "select " + strCodeDescColumnMax + ",sum(Stock_Qty) as [OP.ST.],sum([OPBal]) as [OP/BAL] ,sum(Received_Qty) as [CUM.RPT.],sum(Issued_Qty) as [CUM.ISSUE],SUM(Balance_Qty) AS [BALANCE] from ("
            ...
            strFinalQry += " select  Item_Type,Item_Type_Name as Item_Type_Name,Item_Group,Group_Description as Group_Description, " + strCodeColumn + "," + strCodeDescColumn + ",Item_Category_Struct_Code,Stock_Qty * (case when InOut='I' then 1 else -1 end) as Stock_Qty,Stock_UOM, "
            strFinalQry += InnerOpClo
            strFinalQry += "  from (" + qry + ")xxx)xxxx ) Main_Query Group by " + strCodeColumn + " " + Environment.NewLine
            ...
            If dt.Rows.Count > 0 Then
                ...
                Dim OPST As New GridViewSummaryItem("OP.ST.", "{0:F2}", GridAggregateFunction.Sum)
                summaryRowItem.Add(OPST)
                Dim OPBal As New GridViewSummaryItem("OP/BAL", "{0:F2}", GridAggregateFunction.Sum)
                summaryRowItem.Add(OPBal)
                Dim CUMRPT As New GridViewSummaryItem("CUM.RPT.", "{0:F2}", GridAggregateFunction.Sum)
                summaryRowItem.Add(CUMRPT)
                Dim CUMISSUE As New GridViewSummaryItem("CUM.ISSUE", "{0:F2}", GridAggregateFunction.Sum)
                summaryRowItem.Add(CUMISSUE)
            ...
            '    strFinalQry = "select Item_Type,Item_Type_Name,Item_Group,Group_Description," + strCodeColumn + "," + strCodeDescColumn + ", Item_Category_Struct_Code  as Item_Category_Struct_Code,Main_Location_Code,MainLocationDesc,Location_Code,[Loc Desp],Item_Code,Item_Desc,itf_code,Stock_Qty,Stock_UOM,"
            '    strFinalQry += OuterOpClo
            '    If ChkMRPWise.Checked = True Then
            '        strFinalQry += ",MRP "
            '    End If
            ...
            '    'strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as Location_Code,'' as [Loc Desp],'' as [LocAddress],'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",'' as Item_Code ,'' as Item_Desc,'' as Item_Category_Struct_Code,'' as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,sum(convert(decimal(18,3),case when Stock_Qty=0 then 0 else Cost/Stock_Qty end)) as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF "
            '    strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as Location_Code,'' as [Loc Desp],'' as [LocAddress],'' as PrimaryLocation,'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",max(Item_Code)  as Item_Code ,max(Item_Desc)  as Item_Desc,'' as Item_Category_Struct_Code,'' as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,case when sum( Stock_Qty * case when InOut='I' then 1 else -1 end) =0 then 0 else convert(decimal(18,3), sum(Cost * case when InOut='I' then 1 else -1 end)/sum( Stock_Qty * case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF "
            '    ' ''===Added Parteek 18/11/2016
            '    ''strFinalQry += " ,isnull(convert(decimal(18,2),sum(Prod_Fat_KG)),0) as Prod_Fat_KG,isnull(convert(decimal(18,2),sum(Prod_Fat_Per)),0) as Prod_Fat_Per,isnull(convert(decimal(18,2),sum(Prod_SNF_KG)),0) as Prod_SNF_KG,isnull(convert(decimal(18,2),sum(Prod_SNF_Per)),0) as Prod_SNF_Per"

            '    'strFinalQry += " ,isnull(convert(decimal(18,2),sum(( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end)-Prod_Fat_KG)),0) as Diff_Fat"
            '    'strFinalQry += " ,isnull(convert(decimal(18,2),sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end) * case when InOut='I' then 1 else -1 end)-Prod_SNF_KG)),0) as Diff_SNF"
            '    ' ''==End
            '    ''---------------
            '    'If ChkMRPWise.Checked Then
            '    '    strFinalQry += ",MRP "
            ...
            '    ''richa agarwal 04 Aug,2016
            '    'strFinalQry += " where Punching_Date < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' " + Environment.NewLine
            ...
            '    ' ''===Added PArteek 18/11/2016
            '    'strFinalQry += "  ,convert(decimal(18,2),(Prod_Fat_KG)) as Prod_Fat_KG,convert(decimal(18,2),(Prod_Fat_Per)) as Prod_Fat_Per,convert(decimal(18,2),(Prod_SNF_KG)) as Prod_SNF_KG,convert(decimal(18,2),(Prod_SNF_Per)) as Prod_SNF_Per  ,(( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end-Prod_Fat_KG)) as Diff_Fat"
            '    'strFinalQry += "   ,(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)-Prod_SNF_KG) Diff_SNF "
            '    ' ''==End
            '    If ChkMRPWise.Checked Then
            '        strFinalQry += ",MRP "
            '    End If
            ...
            '    ' strFinalQry += ")xxxxxx Order by Item_Code"
            '    ''---------------
            ...
            '    strFinalQry = "select Location_Code,[Loc Desp],convert(varchar, Punching_Date,103) as Punching_Date  ,Item_Code ,Item_Desc,Stock_UOM, Convert(decimal(18,2),(ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))) as OPQty, case when (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))=0 then 0 else  Convert(decimal(18,2),((isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0))/((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))))) end as OPRate , Convert(decimal(18,2),(isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0))) as OPFAT , Convert(decimal(18,2),isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_FAT+Balance_FAT)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0)) as OPFATPER, Convert(decimal(18,2),(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0))) as OPSNF , Convert(decimal(18,2),isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_SNF+Balance_SNF)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0)) as OPSNFPER  ,Convert(decimal(18,2),(isnull(CLCost,0)-isnull(RecCost,0)+ isnull(IssCost,0))) as OPCost, RecPurQty,RecPurRate,RecPurFAT,RecPurFATPER,RecPurSNF,RecPurSNFPER,RecPurCost ,RecAdjProQty,RecAdjProRate ,RecAdjProFAT ,RecAdjProFATPER,RecAdjProSNF ,RecAdjProSNFPER ,RecAdjProCost ,RecOthQty,RecOthRate ,RecOthFAT ,RecOthFATPER ,RecOthSNF,RecOthSNFPER ,RecOthCost,RecQty,RecRate,RecFAT,RecFATPER,RecSNF,RecSNFPER,RecCost  ,IssSaleQty ,IssSaleRate ,IssSaleFAT ,IssSaleFATPER,IssSaleSNF,IssSaleSNFPER ,IssSaleCost , IssIssAdjQty , IssIssAdjRate , IssIssAdjFAT , IssIssAdjFATPER ,IssIssAdjSNF , IssIssAdjSNFPER ,IssIssAdjCost , IssOthQty , IssOthRate , IssOthFAT ,IssOthFATPER , IssOthSNF ,IssOthSNFPER ,IssOthCost ,IssQty,IssRate,IssFAT,IssFATPER,IssSNF,IssSNFPER,IssCost ,CLQty ,case when isnull(CLQty,0)=0 then 0 else CLCost/CLQty end as CLRate, CLBalance_FAT as CLFAT, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_FAT*100/CLBalance_QTYKG) end),0) as CLFATPER, CLBalance_SNF as CLSNF, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_SNF*100/CLBalance_QTYKG) end),0) as CLSNFPER, CLCost from ( "
            ...
            '    '    Dim IsApplicable As Boolean = False
            '    '    For ii As Integer = 0 To gvLocation.RowCount - 1
            '    '        strWhrCatg += " and ((case when Is_Section='N' and Is_Sub_Location='N' then Location_Code else Main_Location_Code end) = '" + clsCommon.myCstr(gvLocation.Rows(ii).Cells("CODE").Value) + "') "
            '    '        IsApplicable = True
            '    '        Dim arr As Dictionary(Of String, Object) = gvLocation.Rows(ii).Tag
            '    '        If arr IsNot Nothing AndAlso arr.Count > 0 Then
            '    '            strWhrCatg += " and Location_Code in ("
            ...
            '    strFinalQry += "  and TSPL_ITEM_UOM_DETAIL.Stocking_Unit='Y' and TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code"
            ...
            '    'strFinalQry += "select  Location_Code,max([Loc Desp]) as [Loc Desp],convert(date,Punching_Date,103) as Punching_Date, Item_Code ,max(Item_Desc) as Item_Desc, max(Stock_UOM) as Stock_UOM, sum(Balance_FAT) as Balance_FAT,sum(Balance_SNF) as Balance_SNF  ,sum(isnull(Balance_QTYKG,0)) as Balance_QTYKG ,sum(case when InOut='I' and In_Category in ('PU') then Stock_Qty else 0 end) as RecPurQty ,sum(case when InOut='I' and In_Category in ('PU') then Rate else 0 end) as RecPurRate  ,sum(case when InOut='I' and In_Category in ('PU') then Balance_FAT else 0 end) as RecPurFAT  ,(case when sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('PU') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end) end)  as RecPurFATPER  ,sum(case when InOut='I' and In_Category in ('PU') then Balance_SNF else 0 end) as RecPurSNF  ,(case when sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('PU') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end) end)  as RecPurSNFPER  ,sum(case when InOut='I' and In_Category in ('PU') then Cost else 0 end) as RecPurCost  ,sum(case when InOut='I' and In_Category in ('AD') then Stock_Qty else 0 end) as RecAdjProQty  ,sum(case when InOut='I' and In_Category in ('AD') then Rate else 0 end) as RecAdjProRate  ,sum(case when InOut='I' and In_Category in ('AD') then Balance_FAT else 0 end) as RecAdjProFAT  ,(case when sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('AD') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end) end)  as RecAdjProFATPER  ,sum(case when InOut='I' and In_Category in ('AD') then Balance_SNF else 0 end) as RecAdjProSNF  ,(case when sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('AD') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end) end)  as RecAdjProSNFPER  ,sum(case when InOut='I' and In_Category in ('AD') then Cost else 0 end) as RecAdjProCost ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Stock_Qty else 0 end) as RecOthQty  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Rate else 0 end) as RecOthRate  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Balance_FAT else 0 end) as RecOthFAT  ,(case when sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end) end)  as RecOthFATPER  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Balance_SNF else 0 end) as RecOthSNF ,(case when sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end) end)  as RecOthSNFPER  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Cost else 0 end) as RecOthCost ,sum(case when InOut='I' then Stock_Qty else 0 end) as RecQty  ,sum(case when InOut='I' then Rate else 0 end) as RecRate  ,sum(case when InOut='I' then Balance_FAT else 0 end) as RecFAT,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_FAT else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecFATPER  ,sum(case when InOut='I' then Balance_SNF else 0 end) as RecSNF  ,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_SNF else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecSNFPER  ,sum(case when InOut='I' then Cost else 0 end) as RecCost,sum(case when InOut='O' and Out_Category in ('SA') then -1*Stock_Qty else 0 end) as IssSaleQty  ,sum(case when InOut='O' and Out_Category in ('SA') then Rate else 0 end) as IssSaleRate  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Balance_FAT else 0 end) as IssSaleFAT  ,(case when sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('SA') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end) end)  as IssSaleFATPER  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Balance_SNF else 0 end) as IssSaleSNF  ,(case when sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('SA') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end) end)  as IssSaleSNFPER  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Cost else 0 end) as IssSaleCost  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Stock_Qty else 0 end) as IssIssAdjQty  ,sum(case when InOut='O' and Out_Category in ('IS') then Rate else 0 end) as IssIssAdjRate  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Balance_FAT else 0 end) as IssIssAdjFAT  ,(case when sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('IS') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end) end)  as IssIssAdjFATPER  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Balance_SNF else 0 end) as IssIssAdjSNF  ,(case when sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('IS') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end) end)  as IssIssAdjSNFPER  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Cost else 0 end) as IssIssAdjCost ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Stock_Qty else 0 end) as IssOthQty  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then Rate else 0 end) as IssOthRate  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Balance_FAT else 0 end) as IssOthFAT  ,(case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end) end)  as IssOthFATPER  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Balance_SNF else 0 end) as IssOthSNF  ,(case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end) end)  as IssOthSNFPER  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Cost else 0 end) as IssOthCost ,sum(case when InOut='O' then -1*Stock_Qty else 0 end) as IssQty  ,sum(case when InOut='O' then Rate else 0 end) as IssRate  ,sum(case when InOut='O' then -1*Balance_FAT else 0 end) as IssFAT  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_FAT else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssFATPER ,sum(case when InOut='O' then -1*Balance_SNF else 0 end) as IssSNF  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_SNF else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssSNFPER  ,sum(case when InOut='O' then -1*Cost else 0 end) as IssCost ,SUM(sum(Stock_Qty)) OVER (Partition BY Item_Code ORDER BY Item_Code,Location_Code,convert(date,Punching_Date,103)) as CLQty   ,SUM(sum(Cost)) OVER (Partition BY Item_Code ORDER BY Item_Code,Location_Code, Item_Code,convert(date,Punching_Date,103)) as CLCost ,SUM(sum(isnull(Balance_QTYKG,0))) OVER (Partition BY Item_Code ORDER BY Item_Code,Location_Code, convert(date,Punching_Date,103)) as CLBalance_QTYKG  ,SUM(sum(Balance_FAT)) OVER (Partition BY Item_Code ORDER BY Item_Code,Location_Code, convert(date,Punching_Date,103)) as CLBalance_FAT ,SUM(sum(Balance_SNF)) OVER (Partition BY Item_Code ORDER BY Item_Code,Location_Code,convert(date,Punching_Date,103)) as CLBalance_SNF   from ("
            '    'strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,xxx.Location_Code ,'' as [Loc Desp],'' as [LocAddress],'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",xxx.Item_Code as Item_Code ,max(xxx.Item_Desc) as Item_Desc,'' as Item_Category_Struct_Code,max(xxx.Stock_UOM) as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG, case when sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end))=0 then 0 else sum(convert(decimal(18,3),Cost* case when InOut='I' then 1 else -1 end))/sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF,'' as In_Category,'' as Out_Category "
            '    'If ChkMRPWise.Checked Then
            '    '    strFinalQry += ",Max(MRP) as MRP "
            '    'End If
            ...
            '    'strFinalQry += " )xxxxxx Group by  Item_Code,Location_Code,convert(date,Punching_Date,103) )xxxxxxx where Punching_Date is not null  Order by  convert(date,Punching_Date,103),Location_Code"



            '    'strFinalQry = "select Location_Code,[Loc Desp],convert(varchar, Punching_Date,103) as Punching_Date  ,Item_Code ,Item_Desc,Stock_UOM, (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)) as OPQty, case when (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))=0 then 0 else  ((isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0))/((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)))) end as OPRate , (isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0)) as OPFAT,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_FAT+Balance_FAT)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPFATPER,(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0)) as OPSNF,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_SNF+Balance_SNF)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPSNFPER ,(isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0)) as OPCost, RecPurQty,RecPurRate,RecPurFAT,RecPurFATPER,RecPurSNF,RecPurSNFPER,RecPurCost ,RecAdjProQty,RecAdjProRate ,RecAdjProFAT ,RecAdjProFATPER,RecAdjProSNF ,RecAdjProSNFPER ,RecAdjProCost ,RecOthQty,RecOthRate ,RecOthFAT ,RecOthFATPER ,RecOthSNF,RecOthSNFPER ,RecOthCost,RecQty,RecRate,RecFAT,RecFATPER,RecSNF,RecSNFPER,RecCost  ,IssSaleQty ,IssSaleRate ,IssSaleFAT ,IssSaleFATPER,IssSaleSNF,IssSaleSNFPER ,IssSaleCost , IssIssAdjQty , IssIssAdjRate , IssIssAdjFAT , IssIssAdjFATPER ,IssIssAdjSNF , IssIssAdjSNFPER ,IssIssAdjCost , IssOthQty , IssOthRate , IssOthFAT ,IssOthFATPER , IssOthSNF ,IssOthSNFPER ,IssOthCost ,IssQty,IssRate,IssFAT,IssFATPER,IssSNF,IssSNFPER,IssCost ,CLQty ,case when isnull(CLQty,0)=0 then 0 else CLCost/CLQty end as CLRate, CLBalance_FAT as CLFAT, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_FAT*100/CLBalance_QTYKG) end),0) as CLFATPER, CLBalance_SNF as CLSNF, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_SNF*100/CLBalance_QTYKG) end),0) as CLSNFPER, CLCost from ( "
            '    'strFinalQry += "select  Location_Code,max([Loc Desp]) as [Loc Desp],convert(date,Punching_Date,103) as Punching_Date, Item_Code ,max(Item_Desc) as Item_Desc, max(Stock_UOM) as Stock_UOM, sum(Balance_FAT) as Balance_FAT,sum(Balance_SNF) as Balance_SNF  ,sum(isnull(Balance_QTYKG,0)) as Balance_QTYKG ,sum(case when InOut='I' and In_Category in ('PU') then Stock_Qty else 0 end) as RecPurQty ,sum(case when InOut='I' and In_Category in ('PU') then Rate else 0 end) as RecPurRate  ,sum(case when InOut='I' and In_Category in ('PU') then Balance_FAT else 0 end) as RecPurFAT  ,(case when sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('PU') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end) end)  as RecPurFATPER  ,sum(case when InOut='I' and In_Category in ('PU') then Balance_SNF else 0 end) as RecPurSNF  ,(case when sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('PU') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category in ('PU') then  Balance_QTYKG else 0 end) end)  as RecPurSNFPER  ,sum(case when InOut='I' and In_Category in ('PU') then Cost else 0 end) as RecPurCost  ,sum(case when InOut='I' and In_Category in ('AD') then Stock_Qty else 0 end) as RecAdjProQty  ,sum(case when InOut='I' and In_Category in ('AD') then Rate else 0 end) as RecAdjProRate  ,sum(case when InOut='I' and In_Category in ('AD') then Balance_FAT else 0 end) as RecAdjProFAT  ,(case when sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('AD') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end) end)  as RecAdjProFATPER  ,sum(case when InOut='I' and In_Category in ('AD') then Balance_SNF else 0 end) as RecAdjProSNF  ,(case when sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category in ('AD') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category in ('AD') then  Balance_QTYKG else 0 end) end)  as RecAdjProSNFPER  ,sum(case when InOut='I' and In_Category in ('AD') then Cost else 0 end) as RecAdjProCost ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Stock_Qty else 0 end) as RecOthQty  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Rate else 0 end) as RecOthRate  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Balance_FAT else 0 end) as RecOthFAT  ,(case when sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_FAT else 0 end)*100/sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end) end)  as RecOthFATPER  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Balance_SNF else 0 end) as RecOthSNF ,(case when sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_SNF else 0 end)*100/sum(case when InOut='I' and In_Category not in ('AD','PU') then  Balance_QTYKG else 0 end) end)  as RecOthSNFPER  ,sum(case when InOut='I' and In_Category not in ('AD','PU') then Cost else 0 end) as RecOthCost ,sum(case when InOut='I' then Stock_Qty else 0 end) as RecQty  ,sum(case when InOut='I' then Rate else 0 end) as RecRate  ,sum(case when InOut='I' then Balance_FAT else 0 end) as RecFAT,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_FAT else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecFATPER  ,sum(case when InOut='I' then Balance_SNF else 0 end) as RecSNF  ,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_SNF else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecSNFPER  ,sum(case when InOut='I' then Cost else 0 end) as RecCost,sum(case when InOut='O' and Out_Category in ('SA') then -1*Stock_Qty else 0 end) as IssSaleQty  ,sum(case when InOut='O' and Out_Category in ('SA') then Rate else 0 end) as IssSaleRate  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Balance_FAT else 0 end) as IssSaleFAT  ,(case when sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('SA') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end) end)  as IssSaleFATPER  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Balance_SNF else 0 end) as IssSaleSNF  ,(case when sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('SA') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category in ('SA') then  Balance_QTYKG else 0 end) end)  as IssSaleSNFPER  ,sum(case when InOut='O' and Out_Category in ('SA') then -1*Cost else 0 end) as IssSaleCost  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Stock_Qty else 0 end) as IssIssAdjQty  ,sum(case when InOut='O' and Out_Category in ('IS') then Rate else 0 end) as IssIssAdjRate  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Balance_FAT else 0 end) as IssIssAdjFAT  ,(case when sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('IS') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end) end)  as IssIssAdjFATPER  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Balance_SNF else 0 end) as IssIssAdjSNF  ,(case when sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category in ('IS') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category in ('IS') then  Balance_QTYKG else 0 end) end)  as IssIssAdjSNFPER  ,sum(case when InOut='O' and Out_Category in ('IS') then -1*Cost else 0 end) as IssIssAdjCost ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Stock_Qty else 0 end) as IssOthQty  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then Rate else 0 end) as IssOthRate  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Balance_FAT else 0 end) as IssOthFAT  ,(case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_FAT else 0 end)*100/sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end) end)  as IssOthFATPER  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Balance_SNF else 0 end) as IssOthSNF  ,(case when sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_SNF else 0 end)*100/sum(case when InOut='O' and Out_Category not in ('SA','IS') then  Balance_QTYKG else 0 end) end)  as IssOthSNFPER  ,sum(case when InOut='O' and Out_Category not in ('SA','IS') then -1*Cost else 0 end) as IssOthCost ,sum(case when InOut='O' then -1*Stock_Qty else 0 end) as IssQty  ,sum(case when InOut='O' then Rate else 0 end) as IssRate  ,sum(case when InOut='O' then -1*Balance_FAT else 0 end) as IssFAT  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_FAT else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssFATPER ,sum(case when InOut='O' then -1*Balance_SNF else 0 end) as IssSNF  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_SNF else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssSNFPER  ,sum(case when InOut='O' then -1*Cost else 0 end) as IssCost ,SUM(sum(Stock_Qty)) OVER (Partition BY Item_Code ORDER BY Item_Code,convert(date,Punching_Date,103),Location_Code) as CLQty   ,SUM(sum(Cost)) OVER (Partition BY Item_Code ORDER BY Item_Code, Item_Code,convert(date,Punching_Date,103),Location_Code) as CLCost ,SUM(sum(isnull(Balance_QTYKG,0))) OVER (Partition BY Item_Code ORDER BY Item_Code, convert(date,Punching_Date,103),Location_Code) as CLBalance_QTYKG  ,SUM(sum(Balance_FAT)) OVER (Partition BY Item_Code ORDER BY Item_Code, convert(date,Punching_Date,103),Location_Code) as CLBalance_FAT ,SUM(sum(Balance_SNF)) OVER (Partition BY Item_Code ORDER BY Item_Code, convert(date,Punching_Date,103),Location_Code) as CLBalance_SNF   from ("
            '    'strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as Location_Code,'' as [Loc Desp],'' as [LocAddress],'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",xxx.Item_Code as Item_Code ,max(xxx.Item_Desc) as Item_Desc,'' as Item_Category_Struct_Code,max(xxx.Stock_UOM) as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG, case when sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end))=0 then 0 else sum(convert(decimal(18,3),Cost* case when InOut='I' then 1 else -1 end))/sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF,'' as In_Category,'' as Out_Category "
            '    'If ChkMRPWise.Checked Then
            '    '    strFinalQry += ",Max(MRP) as MRP "
            '    'End If
            ...
            '    'strFinalQry += " from (" + qry + ") xxx " + Environment.NewLine
            '    'strFinalQry += " where Punching_Date>='" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' and Punching_Date<='" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' " + Environment.NewLine
            '    'strFinalQry += ")xxxxxx Group by Item_Type,Item_Group,Item_Code,Location_Code,convert(date,Punching_Date,103) )xxxxxxx where Punching_Date is not null  Order by  convert(date,Punching_Date,103),Location_Code"
            ...
            '            'strTransCatg += " sum(case when InOut='I' and code in ('" + TransResult.Rows(ii)("Code") + "') then Stock_Qty else case when InOut='O' and code in ('" + TransResult.Rows(ii)("Code") + "') then Stock_Qty else 0 end end) as '" + TransResult.Rows(ii)("Name") + "'"
            ...
            '            '  strTransCatg += " ,MAX(isnull(convert(decimal(18,2),(CASE when case when InOut='" + TransResult.Rows(ii)("InOutType") + "' and code in ('" + TransResult.Rows(ii)("Code") + "') then ([Balance_FAT]*100/[Balance_QTYKG]) else 0 end))),0)) as '" + TransResult.Rows(ii)("Name") + " Fat_%', sum(convert(decimal(18,2),Balance_FAT))  as [" + TransResult.Rows(ii)("Name") + "_Fat_KG]"
            '            ' strTransCatg += " ,max(isnull(convert(decimal(18,2),(CASE when case when InOut='" + TransResult.Rows(ii)("InOutType") + "' and code in ('" + TransResult.Rows(ii)("Code") + "') then ([Balance_SNF]*100/[Balance_QTYKG]) else 0 end))),0)) as '" + TransResult.Rows(ii)("Name") + " SNF_%', sum(convert(decimal(18,2),Balance_SNF)) as [" + TransResult.Rows(ii)("Name") + "_SNF_KG]"
            ...
            '        End If
            '    Next
            '    strFinalQry = "select convert(varchar, Punching_Date,103) as Punching_Date  ,Item_Code as [Item Code] ,Item_Desc as [Item Description],Stock_UOM as UOM, (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)) as OPQty, case when (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))=0 then 0 else  ((isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0))/((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)))) end as OPRate , (isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0)) as OPFAT,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_FAT+Balance_FAT)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPFATPER,(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0)) as OPSNF,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_SNF+Balance_SNF)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPSNFPER ,(isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0)) as OPCost," + strTransName + " ,CLQty ,case when isnull(CLQty,0)=0 then 0 else CLCost/CLQty end as CLRate, CLBalance_FAT as CLFAT, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_FAT*100/CLBalance_QTYKG) end),0) as CLFATPER, CLBalance_SNF as CLSNF, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_SNF*100/CLBalance_QTYKG) end),0) as CLSNFPER, CLCost from ( "
            '    strFinalQry += "select  cast(Punching_Date as date) as Punching_Date, Item_Code ,max(Item_Desc) as Item_Desc, max(Stock_UOM) as Stock_UOM, sum(Balance_FAT) as Balance_FAT,sum(Balance_SNF) as Balance_SNF ,sum(case when InOut='I' then Stock_Qty else 0 end) as RecQty ,SUM(sum(Stock_Qty)) OVER (Partition BY Item_Code ORDER BY Item_Code,Punching_Date) as CLQty,sum(case when InOut='O' then -1*Stock_Qty else 0 end) as IssQty,SUM(sum(Cost)) OVER (Partition BY Item_Code ORDER BY Item_Code, Item_Code,Punching_Date) as CLCost,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_SNF else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecSNFPER  ,sum(case when InOut='I' then Cost else 0 end) as RecCost,sum(case when InOut='O' then -1*Cost else 0 end) as IssCost,SUM(sum(Balance_FAT)) OVER (Partition BY Item_Code ORDER BY Item_Code, Punching_Date) as CLBalance_FAT ,SUM(sum(Balance_SNF)) OVER (Partition BY Item_Code ORDER BY Item_Code,Punching_Date) as CLBalance_SNF ,sum(case when InOut='O' then -1*Balance_FAT else 0 end) as IssFAT  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_FAT else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssFATPER ,sum(case when InOut='O' then -1*Balance_SNF else 0 end) as IssSNF  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_SNF else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssSNFPER ,sum(case when InOut='I' then Balance_FAT else 0 end) as RecFAT ,sum(case when InOut='I' then Balance_SNF else 0 end) as RecSNF ,SUM(sum(isnull(Balance_QTYKG,0))) OVER (Partition BY Item_Code ORDER BY Item_Code, Punching_Date) as CLBalance_QTYKG , " + strTransCatg + "  ,sum(isnull(Balance_QTYKG,0)) as Balance_QTYKG  from ("
            '    strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",xxx.Item_Code as Item_Code ,max(xxx.Item_Desc) as Item_Desc,'' as Item_Category_Struct_Code,max(xxx.Stock_UOM) as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG, case when sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end))=0 then 0 else sum(convert(decimal(18,3),Cost* case when InOut='I' then 1 else -1 end))/sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF,'' as In_Category,'' as Out_Category,'' as Code "
            '    If ChkMRPWise.Checked Then
            '        strFinalQry += ",Max(MRP) as MRP "
            '    End If
            '    strFinalQry += " from (" + qry + ") xxx " + Environment.NewLine
            ...
            '    strFinalQry += " union  all "
            '    strFinalQry += "SELECT 0 as Trans_Id,null as Trans_Type,null as Trans_Type_Name, null as Source_Doc_No, thedate as Punching_Date,'In' as InOutView,'I' as InOut,null as SourceCode,null as SourceName,null as SourceType ,TSPL_ITEM_MASTER.Item_Type,null as Item_Type_Name,null as Item_Group,null as Group_Description," + strCodeColumnNull + "," + strCodeDescColumnNull + ",TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER.Item_Desc,null as Item_Category_Struct_Code,TSPL_ITEM_UOM_DETAIL.UOM_Code as Stock_UOM ,null as  itf_code ,0 as Stock_Qty,0 as Balance_QTYKG,0 as Rate,0 as Cost,0 as Balance_FAT, 0 as Balance_SNF ,null as In_Category,null as Out_Category,null as Code  "
            '    If ChkMRPWise.Checked Then
            '        strFinalQry += ",0 as MRP "
            '    End If
            '    strFinalQry += " FROM ExplodeDates('" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "','" + clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") + "') as d,TSPL_ITEM_MASTER,TSPL_LOCATION_MASTER,TSPL_ITEM_UOM_DETAIL  where 2=2 "
            ...
            '    strFinalQry += " and ((TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' and (TSPL_LOCATION_MASTER.Location_Type='Physical' or TSPL_LOCATION_MASTER.Location_Type='Logical') ) or (TSPL_LOCATION_MASTER.CSA_Type='Y') )  and TSPL_ITEM_UOM_DETAIL.Stocking_Unit='Y' and TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code"
            '    strFinalQry += " )xxxxxx Group by  Item_Code,Punching_Date )xxxxxxx where Punching_Date is not null  Order by item_code"

            '    ''==========Location Wise Item in transaction

            '    'strFinalQry = "select Location_Code,[Loc_Desp],convert(varchar, Punching_Date,103) as Punching_Date  ,Item_Code as [Item Code] ,Item_Desc as [Item Description],Stock_UOM as UOM, (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)) as OPQty, case when (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))=0 then 0 else  ((isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0))/((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)))) end as OPRate , (isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0)) as OPFAT,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_FAT+Balance_FAT)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPFATPER,(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0)) as OPSNF,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_SNF+Balance_SNF)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPSNFPER ,(isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0)) as OPCost," + strTransName + " ,CLQty ,case when isnull(CLQty,0)=0 then 0 else CLCost/CLQty end as CLRate, CLBalance_FAT as CLFAT, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_FAT*100/CLBalance_QTYKG) end),0) as CLFATPER, CLBalance_SNF as CLSNF, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_SNF*100/CLBalance_QTYKG) end),0) as CLSNFPER, CLCost from ( "
            '    'strFinalQry += "select  Location_Code,max([Loc Desp]) as [Loc_Desp],Punching_Date as Punching_Date, Item_Code ,max(Item_Desc) as Item_Desc, max(Stock_UOM) as Stock_UOM, sum(Balance_FAT) as Balance_FAT,sum(Balance_SNF) as Balance_SNF ,sum(case when InOut='I' then Stock_Qty else 0 end) as RecQty ,SUM(sum(Stock_Qty)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code,Punching_Date) as CLQty,sum(case when InOut='O' then -1*Stock_Qty else 0 end) as IssQty,SUM(sum(Cost)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code, Item_Code,Punching_Date) as CLCost,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_SNF else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecSNFPER  ,sum(case when InOut='I' then Cost else 0 end) as RecCost,sum(case when InOut='O' then -1*Cost else 0 end) as IssCost,SUM(sum(Balance_FAT)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code, Punching_Date) as CLBalance_FAT ,SUM(sum(Balance_SNF)) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code,Punching_Date) as CLBalance_SNF ,sum(case when InOut='O' then -1*Balance_FAT else 0 end) as IssFAT  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_FAT else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssFATPER ,sum(case when InOut='O' then -1*Balance_SNF else 0 end) as IssSNF  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_SNF else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssSNFPER ,sum(case when InOut='I' then Balance_FAT else 0 end) as RecFAT ,sum(case when InOut='I' then Balance_SNF else 0 end) as RecSNF ,SUM(sum(isnull(Balance_QTYKG,0))) OVER (Partition BY Item_Code,Location_Code ORDER BY Item_Code,Location_Code, Punching_Date) as CLBalance_QTYKG ,sum(isnull(Balance_QTYKG,0)) as Balance_QTYKG, " + strTransCatg + "    from ("
            '    'strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,xxx.Location_Code ,'' as [Loc Desp],'' as [LocAddress],'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",xxx.Item_Code as Item_Code ,max(xxx.Item_Desc) as Item_Desc,'' as Item_Category_Struct_Code,max(xxx.Stock_UOM) as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG, case when sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end))=0 then 0 else sum(convert(decimal(18,3),Cost* case when InOut='I' then 1 else -1 end))/sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF,'' as In_Category,'' as Out_Category,'' as Code "
            '    'If ChkMRPWise.Checked Then
            '    '    strFinalQry += ",Max(MRP) as MRP "
            '    'End If
            ...
            '    'strFinalQry += " and ((TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' and (TSPL_LOCATION_MASTER.Location_Type='Physical' or TSPL_LOCATION_MASTER.Location_Type='Logical') ) or (TSPL_LOCATION_MASTER.CSA_Type='Y') )  and TSPL_ITEM_UOM_DETAIL.Stocking_Unit='Y' and TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code"
            '    'strFinalQry += " )xxxxxx Group by  Item_Code,Location_Code,Punching_Date )xxxxxxx where Punching_Date is not null  Order by item_code"
            ...
            ''KUNAL > KDIL > 21 DEC 2016
            'Dim itemWiseQry As String = Nothing
            'If chkFATAndSNF.Checked Then
            '    If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date and Item Wise Stock") = CompairStringResult.Equal Then
            '        'If strFinalQry.Contains("Order by  Punching_Date,Location_Code") Then
            '        '    strFinalQry = strFinalQry.Replace("Order by  Punching_Date,Location_Code", "")
            '        '    itemWiseQry = " SELECT  location_code , max([loc desp]) [Loc Desp] , punching_date ,item_code, max(item_desc) AS item_desc ,max(stock_uom) AS stock_uom ,max(opqty) AS opqty ,max(oprate) AS oprate ,max(opfat) AS opfat ,max(opfatper) AS opfatper ,max(opsnf) AS opsnf ,max(opsnfper) AS opsnfper ,max(opcost) AS opcost ,max(convert(decimal(18,2),recpurqty)) AS recpurqty ,max(convert(decimal(18,2),recpurrate)) AS recpurrate ,max(convert(decimal(18,2),recpurfat)) AS recpurfat ,max(convert(decimal(18,2),recpurfatper)) AS recpurfatper ,max(convert(decimal(18,2),recpursnf)) AS recpursnf ,max(recpursnfper) AS recpursnfper ,max(recpurcost) AS recpurcost ,max(convert(decimal(18,2),recadjproqty)) AS recadjproqty ,max(convert(decimal(18,2),recadjprorate)) AS recadjprorate ,max(convert(decimal(18,2),recadjprofat)) AS recadjprofat ,max(convert(decimal(18,2),recadjprofatper)) AS recadjprofatper  ,max(convert(decimal(18,2),recadjprosnf)) AS  recadjprosnf,max(convert(decimal(18,2),recadjprosnfper)) AS recadjprosnfper,max(convert(decimal(18,2),recadjprocost)) AS recadjprocost ,max(convert(decimal(18,2),recothqty)) AS recothqty ,max(convert(decimal(18,2),recothrate)) AS recothrate ,max(convert(decimal(18,2),recothfat)) AS recothfat ,max(convert(decimal(18,2),recothfatper)) AS recothfatper ,max(convert(decimal(18,2),recothsnf)) AS recothsnf ,max(convert(decimal(18,2),recothsnfper)) AS recothsnfper ,max(convert(decimal(18,2),recothcost)) AS recothcost ,max(convert(decimal(18,2),recqty)) AS recqty ,max(convert(decimal(18,2),recrate)) AS recrate ,max(convert(decimal(18,2),recfat)) AS recfat ,max(convert(decimal(18,2),recfatper)) AS recfatper ,max(convert(decimal(18,2),recsnf)) AS  recsnf ,max(convert(decimal(18,2),recsnfper)) AS recsnfper ,max(convert(decimal(18,2),reccost)) AS reccost ,max(convert(decimal(18,2),isssaleqty)) AS isssaleqty ,max(convert(decimal(18,2),isssalerate)) AS isssalerate,max(convert(decimal(18,2),isssalefat)) AS isssalefat ,max(convert(decimal(18,2),isssalefatper)) AS isssalefatper ,max(convert(decimal(18,2),isssalesnf)) AS isssalesnf ,max(convert(decimal(18,2),isssalesnfper)) AS isssalesnfper ,max(convert(decimal(18,2),isssalecost)) AS isssalecost ,max(convert(decimal(18,2),ississadjqty)) AS ississadjqty ,max(convert(decimal(18,2),ississadjrate)) AS ississadjrate ,max(convert(decimal(18,2),ississadjfat)) AS ississadjfat ,max(convert(decimal(18,2),ississadjfatper)) AS ississadjfatper ,max(convert(decimal(18,2),ississadjsnf)) AS ississadjsnf ,max(convert(decimal(18,2),ississadjsnfper)) AS ississadjsnfper ,max(convert(decimal(18,2),ississadjcost)) AS ississadjcost ,max(convert(decimal(18,2),issothqty)) AS issothqty ,max(convert(decimal(18,2),issothrate)) AS issothrate ,max(convert(decimal(18,2),issothfat)) AS  issothfat ,max(convert(decimal(18,2),issothfatper)) AS issothfatper ,max(convert(decimal(18,2),issothsnf)) AS issothsnf ,max(convert(decimal(18,2),issothsnfper)) AS issothsnfper ,max(convert(decimal(18,2),issothcost)) AS issothcost ,max(convert(decimal(18,2),issqty)) AS issqty ,max(convert(decimal(18,2),issrate)) AS issrate ,max(convert(decimal(18,2),issfat)) AS issfat ,max(convert(decimal(18,2),issfatper)) AS issfatper ,max(convert(decimal(18,2),isssnf)) AS isssnf ,max(convert(decimal(18,2),isssnfper)) AS isssnfper ,max(convert(decimal(18,2),isscost)) AS isscost ,max(convert(decimal(18,2),clqty)) AS clqty  , Max(CONVERT(DECIMAL(18,2),CLRate)) as  CLRate, max(convert(decimal(18,2),clfat)) AS clfat ,max(convert(decimal(18,2),clfatper)) AS clfatper ,max(convert(decimal(18,2),clsnf)) AS clsnf ,max(convert(decimal(18,2),clsnfper)) AS clsnfper ,max(convert(decimal(18,2),clcost)) AS clcost FROM ( " + strFinalQry + " )  ad WHERE 1=1 AND  ad.opfatper <> 0 OR       ad.opsnfper <> 0 AND      ad.opqty <> 0 OR  ( ad.recpurqty <> 0 AND      ad.recadjproqty <> 0  AND      ad.recothqty <> 0 AND      ad.isssaleqty <> 0) GROUP BY ad.item_code,  ad.location_code, punching_date ORDER BY punching_date,  location_code "
            '        '    strFinalQry = itemWiseQry
            '        'End If
            '    End If
            'Else
            '    If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date and Item Wise Stock") = CompairStringResult.Equal Then
            '        'If strFinalQry.Contains("Order by  Punching_Date,Location_Code") Then
            '        '    strFinalQry = strFinalQry.Replace("Order by  Punching_Date,Location_Code", "")
            '        '    itemWiseQry = " SELECT  location_code , max([loc desp]) [Loc Desp] ,  CONVERT(DATE, punching_date ,103) as punching_date   ,item_code, max(item_desc) AS item_desc ,max(stock_uom) AS stock_uom ,max(opqty) AS opqty ,max(oprate) AS oprate ,max(opfat) AS opfat ,max(opfatper) AS opfatper ,max(opsnf) AS opsnf ,max(opsnfper) AS opsnfper ,max(opcost) AS opcost ,max(convert(decimal(18,2),recpurqty)) AS recpurqty ,max(convert(decimal(18,2),recpurrate)) AS recpurrate ,max(convert(decimal(18,2),recpurfat)) AS recpurfat ,max(convert(decimal(18,2),recpurfatper)) AS recpurfatper ,max(convert(decimal(18,2),recpursnf)) AS recpursnf ,max(recpursnfper) AS recpursnfper ,max(recpurcost) AS recpurcost ,max(convert(decimal(18,2),recadjproqty)) AS recadjproqty ,max(convert(decimal(18,2),recadjprorate)) AS recadjprorate ,max(convert(decimal(18,2),recadjprofat)) AS recadjprofat ,max(convert(decimal(18,2),recadjprofatper)) AS recadjprofatper  ,max(convert(decimal(18,2),recadjprosnf)) AS  recadjprosnf,max(convert(decimal(18,2),recadjprosnfper)) AS recadjprosnfper,max(convert(decimal(18,2),recadjprocost)) AS recadjprocost ,max(convert(decimal(18,2),recothqty)) AS recothqty ,max(convert(decimal(18,2),recothrate)) AS recothrate ,max(convert(decimal(18,2),recothfat)) AS recothfat ,max(convert(decimal(18,2),recothfatper)) AS recothfatper ,max(convert(decimal(18,2),recothsnf)) AS recothsnf ,max(convert(decimal(18,2),recothsnfper)) AS recothsnfper ,max(convert(decimal(18,2),recothcost)) AS recothcost ,max(convert(decimal(18,2),recqty)) AS recqty ,max(convert(decimal(18,2),recrate)) AS recrate ,max(convert(decimal(18,2),recfat)) AS recfat ,max(convert(decimal(18,2),recfatper)) AS recfatper ,max(convert(decimal(18,2),recsnf)) AS  recsnf ,max(convert(decimal(18,2),recsnfper)) AS recsnfper ,max(convert(decimal(18,2),reccost)) AS reccost ,max(convert(decimal(18,2),isssaleqty)) AS isssaleqty ,max(convert(decimal(18,2),isssalerate)) AS isssalerate,max(convert(decimal(18,2),isssalefat)) AS isssalefat ,max(convert(decimal(18,2),isssalefatper)) AS isssalefatper ,max(convert(decimal(18,2),isssalesnf)) AS isssalesnf ,max(convert(decimal(18,2),isssalesnfper)) AS isssalesnfper ,max(convert(decimal(18,2),isssalecost)) AS isssalecost ,max(convert(decimal(18,2),ississadjqty)) AS ississadjqty ,max(convert(decimal(18,2),ississadjrate)) AS ississadjrate ,max(convert(decimal(18,2),ississadjfat)) AS ississadjfat ,max(convert(decimal(18,2),ississadjfatper)) AS ississadjfatper ,max(convert(decimal(18,2),ississadjsnf)) AS ississadjsnf ,max(convert(decimal(18,2),ississadjsnfper)) AS ississadjsnfper ,max(convert(decimal(18,2),ississadjcost)) AS ississadjcost ,max(convert(decimal(18,2),issothqty)) AS issothqty ,max(convert(decimal(18,2),issothrate)) AS issothrate ,max(convert(decimal(18,2),issothfat)) AS  issothfat ,max(convert(decimal(18,2),issothfatper)) AS issothfatper ,max(convert(decimal(18,2),issothsnf)) AS issothsnf ,max(convert(decimal(18,2),issothsnfper)) AS issothsnfper ,max(convert(decimal(18,2),issothcost)) AS issothcost ,max(convert(decimal(18,2),issqty)) AS issqty ,max(convert(decimal(18,2),issrate)) AS issrate ,max(convert(decimal(18,2),issfat)) AS issfat ,max(convert(decimal(18,2),issfatper)) AS issfatper ,max(convert(decimal(18,2),isssnf)) AS isssnf ,max(convert(decimal(18,2),isssnfper)) AS isssnfper ,max(convert(decimal(18,2),isscost)) AS isscost ,max(convert(decimal(18,2),clqty)) AS clqty  , Max(CONVERT(DECIMAL(18,2),CLRate)) as  CLRate, max(convert(decimal(18,2),clfat)) AS clfat ,max(convert(decimal(18,2),clfatper)) AS clfatper ,max(convert(decimal(18,2),clsnf)) AS clsnf ,max(convert(decimal(18,2),clsnfper)) AS clsnfper ,max(convert(decimal(18,2),clcost)) AS clcost FROM ( " + strFinalQry + " )  ad WHERE  1=1 AND    ad.opfatper = 0 OR  ad.opsnfper = 0  GROUP BY ad.item_code, ad.location_code, CONVERT(DATE, ad.punching_date,103)  ORDER BY CONVERT(DATE, ad.punching_date,103) , location_code "
            '        '    strFinalQry = itemWiseQry
            '        'End If
            '    End If
            'End If
            ...
            '            Throw New Exception("No Data Found to Display")
            '        End If
            '        frmCrystalReportViewer.funreport(CrystalReportFolder.PurchaseOrder, dt1, "rptStockLedgerReport", "Stock Reco Report")
            '    End If
            'ElseIf isPrintCrystalReport = 2 Then
            '    'If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date, Item And Document Wise Detail") = CompairStringResult.Equal Then
            '    'End If
            '    'strFinalQry = " select top 10300 * from TSPL_INVENTORY_MOVEMENT "
            ...
            'SetGridFormationOFGV1()
            'RadPageView1.SelectedPage = RadPageViewPage2
            'If isPrintCrystalReport Then
            '    If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail Ledger") = CompairStringResult.Equal Then
            '        frmCrystalReportViewer.funreport(CrystalReportFolder.PurchaseOrder, dt, "rptStockLedgerReport", "Stock Reco Report")
            '    End If
            'End If
            'dt = Nothing
    ...
    '        ''==End
    '        qry += " from ( "
    '        qry += " select Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,0 as IsFromMilk,0 as MilkFatPer,0 as MilkSNFPer,0 as MilkFATKG,0 as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType  from TSPL_INVENTORY_MOVEMENT " + Environment.NewLine
    '        qry += " union all " + Environment.NewLine
    '        qry += " select Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,1 as IsFromMilk,Fat_Per as MilkFatPer ,SNF_Per as MilkSNFPer,Fat_KG as MilkFATKG,SNF_KG as MilkSNFKG,case when cust_code is not null and len(cust_code)>0 then cust_code else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Code else Other_Location_Code end end as SourceCode,case when cust_code is not null and len(cust_code)>0 then Cust_Name else case when Vendor_Code is not null and len(Vendor_Code)>0 then Vendor_Name else Other_Location_Desc end end as SourceName, case when cust_code is not null and len(cust_code)>0 then 'C' else case when Vendor_Code is not null and len(Vendor_Code)>0 then 'V' else case when Other_Location_Code is not null and len(Other_Location_Code)>0 then 'L' else '' end end end as SourceType from TSPL_INVENTORY_MOVEMENT_NEW" + Environment.NewLine
    '        qry += ") InventroyMovement " + Environment.NewLine
    '        qry += " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=InventroyMovement.Item_Code" + Environment.NewLine
    '        qry += " left outer join TSPL_PURCHASE_ACCOUNTS on TSPL_PURCHASE_ACCOUNTS.Purchase_Class_Code=TSPL_ITEM_MASTER.Purchase_Class_Code" + Environment.NewLine
    '        qry += " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code = InventroyMovement.Location_Code " + Environment.NewLine
    '        ''==PArteek Added
    '        qry += " left outer join TSPL_PP_PRODUCTION_ENTRY_DETAIL on TSPL_PP_PRODUCTION_ENTRY_DETAIL.PROD_ENTRY_CODE=InventroyMovement.Source_Doc_No " + Environment.NewLine
    '        ''==End
    '        qry += " left outer join TSPL_LOCATION_MASTER as MainLocationTable on MainLocationTable.Location_Code =(case when TSPL_LOCATION_MASTER.Is_Section='N' and TSPL_LOCATION_MASTER.Is_Sub_Location='N' then TSPL_LOCATION_MASTER.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end)"
    '        qry += " left outer join TSPL_ITEM_UOM_DETAIL as FATSNFConvertedUnit on FATSNFConvertedUnit.Item_Code=InventroyMovement.Item_Code and FATSNFConvertedUnit.UOM_Code='KG'"
    '        qry += " left outer join TSPL_INVENTORY_SOURCE_CODE on TSPL_INVENTORY_SOURCE_CODE.code=InventroyMovement.Trans_Type"
    ...
    '        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal Then
    '            strFinalQry = "select  case when '" & LocationFirstTime & "'='1' then '" & LocationAddress & "' else '" + objCommonVar.CurrentCompanyName + " ' end as CompName,'" + clsCommon.myCDate(txtFromDate.Value, "dd/MMM/yyyy") + "' as FromDate,'" + clsCommon.myCDate(txtToDate.Value, "dd/MMM/yyyy") + "' as ToDate , Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,convert(varchar,Punching_Date,103) as Punching_Date,InOutView, InOut,Location_Code,[Loc Desp],SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description," + strCodeColumn + "," + strCodeDescColumn + ",Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,Stock_Qty,Rate,Cost,isnull((CASE when Balance_QTYKG=0 then 0 else (Balance_FAT*100/Balance_QTYKG) end),0) as Balance_FATPER ,Balance_FAT,isnull((CASE when Balance_QTYKG=0 then 0 else ([Balance_SNF]*100/[Balance_QTYKG]) end),0) as [Balance_SNFPER] , Balance_SNF ,Prod_Fat_KG ,Prod_Fat_Per,	Prod_SNF_KG,Prod_SNF_Per,Diff_Fat,Diff_SNF "
    '            If ChkMRPWise.Checked Then
    '                strFinalQry += ",MRP "
    '            End If
    '            strFinalQry += " from ("
    ...
    '            strFinalQry += " from (" + qry + ") xxx " + Environment.NewLine
    '            ''richa agarwal 04 Aug,2016
    '            'strFinalQry += " where Punching_Date < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "' " + Environment.NewLine
    '            strFinalQry += " where Punching_Date < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "' group by Item_Code  " + Environment.NewLine
    '            ''-------------------
    '            strFinalQry += " union all "
    '            strFinalQry += " select Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,Punching_Date,InOutView, InOut,Location_Code,[Loc Desp], [LocAddress],SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description," + strCodeColumn + "," + strCodeDescColumn + ",Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,(QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,convert(decimal(18,3),case when Stock_Qty=0 then 0 else Cost/Stock_Qty end) as Rate,(Cost * case when InOut='I' then 1 else -1 end) as Cost,( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT, ( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end) as Balance_SNF  "
    '            ''===Added PArteek 18/11/2016
    ...
    '            End If
    '            strFinalQry += " from (" + qry + ") xxx " + Environment.NewLine
    '            strFinalQry += " where Punching_Date < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "' group by xxx.Item_Code,xxx.Location_Code " + Environment.NewLine
    '            strFinalQry += " union all "
    '            strFinalQry += " select Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,Punching_Date,InOutView, InOut,Location_Code,[Loc Desp], [LocAddress],SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description," + strCodeColumn + "," + strCodeDescColumn + ",Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,(QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,convert(decimal(18,3),case when Stock_Qty=0 then 0 else Cost/Stock_Qty end) as Rate,(Cost * case when InOut='I' then 1 else -1 end) as Cost,( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT, ( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end) as Balance_SNF ,In_Category,Out_Category  "
    '            If ChkMRPWise.Checked Then
    '                strFinalQry += ",MRP "
    ...
    '            Dim strTransCatg As String = ""
    '            Dim strTransName As String = ""
    '            Dim TransQry As String = "select code,Name,InOutType,In_Category,Out_Category,isnull(Sequence,500) as Sequence,Type as Typed from TSPL_INVENTORY_SOURCE_CODE order by Sequence asc "
    '            Dim TransResult As DataTable = clsDBFuncationality.GetDataTable(TransQry)
    '            If TransResult IsNot Nothing AndAlso TransResult.Rows.Count > 0 Then
    '                For ii As Integer = 0 To TransResult.Rows.Count - 1
    '                    If ii <> 0 Then
    ...
    '            Next
    '            strFinalQry = "select convert(varchar, Punching_Date,103) as Punching_Date  ,Item_Code as [Item Code] ,Item_Desc as [Item Description],Stock_UOM as UOM, (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)) as OPQty, case when (ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0))=0 then 0 else  ((isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0))/((ISNULL(CLQty,0) - isnull(RecQty,0)+isnull(IssQty,0)))) end as OPRate , (isnull(CLBalance_FAT,0)-isnull(RecFAT,0)+isnull(IssFAT,0)) as OPFAT,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_FAT+Balance_FAT)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPFATPER,(isnull(CLBalance_SNF,0)-isnull(RecSNF,0)+isnull(IssSNF,0)) as OPSNF,isnull((CASE when (CLBalance_QTYKG+Balance_QTYKG) =0 then 0 else ((CLBalance_SNF+Balance_SNF)*100/(CLBalance_QTYKG+Balance_QTYKG)) end),0) as OPSNFPER ,(isnull(CLCost,0)-isnull(RecCost,0)+isnull(IssCost,0)) as OPCost," + strTransName + " ,CLQty ,case when isnull(CLQty,0)=0 then 0 else CLCost/CLQty end as CLRate, CLBalance_FAT as CLFAT, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_FAT*100/CLBalance_QTYKG) end),0) as CLFATPER, CLBalance_SNF as CLSNF, isnull((CASE when CLBalance_QTYKG=0 then 0 else (CLBalance_SNF*100/CLBalance_QTYKG) end),0) as CLSNFPER, CLCost from ( "
    '            strFinalQry += "select  Punching_Date as Punching_Date, Item_Code ,max(Item_Desc) as Item_Desc, max(Stock_UOM) as Stock_UOM, sum(Balance_FAT) as Balance_FAT,sum(Balance_SNF) as Balance_SNF ,sum(case when InOut='I' then Stock_Qty else 0 end) as RecQty ,SUM(sum(Stock_Qty)) OVER (Partition BY Item_Code ORDER BY Item_Code,Punching_Date) as CLQty,sum(case when InOut='O' then -1*Stock_Qty else 0 end) as IssQty,SUM(sum(Cost)) OVER (Partition BY Item_Code ORDER BY Item_Code, Item_Code,Punching_Date) as CLCost,(case when sum(case when InOut='I' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='I' then  Balance_SNF else 0 end)*100/sum(case when InOut='I' then  Balance_QTYKG else 0 end) end)  as RecSNFPER  ,sum(case when InOut='I' then Cost else 0 end) as RecCost,sum(case when InOut='O' then -1*Cost else 0 end) as IssCost,SUM(sum(Balance_FAT)) OVER (Partition BY Item_Code ORDER BY Item_Code, Punching_Date) as CLBalance_FAT ,SUM(sum(Balance_SNF)) OVER (Partition BY Item_Code ORDER BY Item_Code,Punching_Date) as CLBalance_SNF ,sum(case when InOut='O' then -1*Balance_FAT else 0 end) as IssFAT  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_FAT else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssFATPER ,sum(case when InOut='O' then -1*Balance_SNF else 0 end) as IssSNF  ,(case when sum(case when InOut='O' then  Balance_QTYKG else 0 end)=0 then 0  else  sum(case when InOut='O' then  Balance_SNF else 0 end)*100/sum(case when InOut='O' then  Balance_QTYKG else 0 end) end)  as IssSNFPER ,sum(case when InOut='I' then Balance_FAT else 0 end) as RecFAT ,sum(case when InOut='I' then Balance_SNF else 0 end) as RecSNF ,SUM(sum(isnull(Balance_QTYKG,0))) OVER (Partition BY Item_Code ORDER BY Item_Code, Punching_Date) as CLBalance_QTYKG , " + strTransCatg + "  ,sum(isnull(Balance_QTYKG,0)) as Balance_QTYKG  from ("
    '            strFinalQry += "select 0 as Trans_Id,'' as Trans_Type,'' as Trans_Type_Name,'Opening Balance' as Source_Doc_No,null as Punching_Date,'' as InOutView, '' as InOut,'' as SourceCode,'' as SourceName,'' as SourceType ,'' as Item_Type,'' as Item_Type_Name,'' as Item_Group,'' as Group_Description," + strCodeColumnMax + "," + strCodeDescColumnMax + ",xxx.Item_Code as Item_Code ,max(xxx.Item_Desc) as Item_Desc,'' as Item_Category_Struct_Code,max(xxx.Stock_UOM) as Stock_UOM,'' as itf_code ,sum( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,sum( QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG, case when sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end))=0 then 0 else sum(convert(decimal(18,3),Cost* case when InOut='I' then 1 else -1 end))/sum(convert(decimal(18,3),Stock_Qty* case when InOut='I' then 1 else -1 end)) end as Rate,sum(Cost * case when InOut='I' then 1 else -1 end) as Cost,sum( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT,sum(( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end)) as Balance_SNF,'' as In_Category,'' as Out_Category,'' as Code "
    '            If ChkMRPWise.Checked Then
    '                strFinalQry += ",Max(MRP) as MRP "
    '            End If
    ...
    '            strFinalQry += " where Punching_Date < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm tt") + "' group by xxx.Item_Code " + Environment.NewLine
    '            strFinalQry += " union all "
    '            strFinalQry += " select Trans_Id,Trans_Type,Trans_Type_Name,Source_Doc_No,Punching_Date,InOutView, InOut,SourceCode,SourceName,SourceType ,Item_Type,Item_Type_Name,Item_Group,Group_Description," + strCodeColumn + "," + strCodeDescColumn + ",Item_Code ,Item_Desc,Item_Category_Struct_Code,Stock_UOM,itf_code ,( Stock_Qty * case when InOut='I' then 1 else -1 end) as Stock_Qty,(QtyKG * case when InOut='I' then 1 else -1 end) as Balance_QTYKG,convert(decimal(18,3),case when Stock_Qty=0 then 0 else Cost/Stock_Qty end) as Rate,(Cost * case when InOut='I' then 1 else -1 end) as Cost,( (case when IsFromMilk=1 then MilkFATKG else (Stock_Qty*FatPer) end) * case when InOut='I' then 1 else -1 end) as Balance_FAT, ( (case when IsFromMilk=1 then MilkSNFKG else (Stock_Qty*SNFPer) end ) * case when InOut='I' then 1 else -1 end) as Balance_SNF ,In_Category,Out_Category,code  "
    '            If ChkMRPWise.Checked Then
    '                strFinalQry += ",MRP "
    '            End If
    '            strFinalQry += " from (" + qry + ") xxx " + Environment.NewLine
    ...
    '        'Dim ResultsData As DataTable = New DataTable()
    ...
    '        '    Next
    '        'End If
    '        'gv1.DataSource = ResultsData
    '        'Dim intReaderCounter As Integer = 0
    '        'RadPageView1.SelectedPage = RadPageViewPage2
    '        'While reader.Read()
    '        '    intReaderCounter += 1
    '        '    clsCommon.ProgressBarUpdate("Loading data in Grid " + clsCommon.myCstr(intReaderCounter))
    '        '    gv1.Rows.AddNew()
    '        '    gv1.Rows(gv1.Rows.Count - 1) = DirectCast(reader, GridViewRowInfo)
    '        '    For i As Integer = 0 To reader.FieldCount - 1
    '        '        gv1.Rows(intReaderCounter - 1).Cells(i).Value = reader(i)
    '        '    Next
    '        '    gv1.Refresh()
    '        'End While
    ...
    Function GetCollectionForVisibleColumns() As Dictionary(Of String, String)
        ...
        If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Type Wise Summary") = CompairStringResult.Equal Then
            ...
            If chkFATAndSNF.Checked Then
                ...
                arrVisibleColumAndCaption.Add("Diff_SNF", "Diference SNF")
    ...
    Sub SetGridFormationOFGV1()
        ...
        If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Type Wise Summary") = CompairStringResult.Equal Then
            ...
            ''== Parteek Added 18/11/2016 ''below code are commented on 05/12/2016 as discussed with Parteek
            'gv1.Columns("Prod_Fat_KG").IsVisible = chkFATAndSNF.Checked
            'gv1.Columns("Prod_Fat_KG").Width = 100
            'gv1.Columns("Prod_Fat_KG").HeaderText = "Production Fat (KG)"
            'gv1.Columns("Prod_Fat_KG").FormatString = "{0:n2}"

            'gv1.Columns("Prod_Fat_Per").IsVisible = chkFATAndSNF.Checked
            'gv1.Columns("Prod_Fat_Per").Width = 100
            'gv1.Columns("Prod_Fat_Per").HeaderText = "Production Fat (%)"
            'gv1.Columns("Prod_Fat_Per").FormatString = "{0:n2}"

            'gv1.Columns("Prod_SNF_KG").IsVisible = chkFATAndSNF.Checked
            'gv1.Columns("Prod_SNF_KG").Width = 100
            'gv1.Columns("Prod_SNF_KG").HeaderText = "Production SNF (KG)"
            'gv1.Columns("Prod_SNF_KG").FormatString = "{0:n2}"

            'gv1.Columns("Prod_SNF_Per").IsVisible = chkFATAndSNF.Checked
            'gv1.Columns("Prod_SNF_Per").Width = 100
            'gv1.Columns("Prod_SNF_Per").HeaderText = "Production SNF (%)"
            'gv1.Columns("Prod_SNF_Per").FormatString = "{0:n2}"

            'gv1.Columns("Diff_Fat").IsVisible = chkFATAndSNF.Checked
            'gv1.Columns("Diff_Fat").Width = 100
            'gv1.Columns("Diff_Fat").HeaderText = "Difference Fat"
            'gv1.Columns("Diff_Fat").FormatString = "{0:n2}"

            'gv1.Columns("Diff_SNF").IsVisible = chkFATAndSNF.Checked
            'gv1.Columns("Diff_SNF").Width = 100
            'gv1.Columns("Diff_SNF").HeaderText = "Difference SNF"
            ...
            ''below code are commented on 05/12/2016 as discussed with Parteek
            'Smitem = New GridViewSummaryItem("Prod_SNF_KG", "{0:F2}", GridAggregateFunction.Sum)
            'summaryRowItem.Add(Smitem)
            'Smitem = New GridViewSummaryItem("Prod_Fat_KG", "{0:F2}", GridAggregateFunction.Sum)
            'summaryRowItem.Add(Smitem)
    ...
    Private Sub ReStoreGridLayout()
        Try
            Dim ReportID As String = MyBase.Form_ID 'GetReportID()
    ...
    '    Try
    '        If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Group Wise Summary") = CompairStringResult.Equal Then
    '            Dim frm As New FrmStockReco
    '            frm.isDataLoad = True
    '            frm.dtFrom = txtFromDate.Value
    '            frm.dtTo = txtToDate.Value
    '            frm.Unit_Code = cmbUnit.SelectedValue
    ...
    '            frm.arrItemGroup.Add(clsCommon.myCstr(gv1.CurrentRow.Cells("Item_Group").Value))
    '            frm.InOutType = cboInOutType.SelectedValue
    '            If rbtnCategorySelect.IsChecked Then
    '                frm.arrCat = New Dictionary(Of String, Object)
    '                For ii As Integer = 0 To gvCategory.RowCount - 1
    '                    If clsCommon.myCBool(gvCategory.Rows(ii).Cells("SEL").Value) Then
    '                        frm.arrCat.Add(clsCommon.myCstr(gvCategory.Rows(ii).Cells("CODE").Value), gvCategory.Rows(ii).Tag)
    '                    End If
    '                Next
    '            End If
    '            If rbtnLocationSelect.IsChecked Then
    ...
    '                For ii As Integer = 0 To gvLocation.RowCount - 1
    '                    If clsCommon.myCBool(gvLocation.Rows(ii).Cells("SEL").Value) Then
    '                        frm.arrLoc.Add(clsCommon.myCstr(gvLocation.Rows(ii).Cells("CODE").Value), gvLocation.Rows(ii).Tag)
    '                    End If
    '                Next
    '            End If
    '            frm.strType = "Category Wise Summary"
    ...
    '                    frm.arrCat.Add(clsCommon.myCstr(dr("CodeColumn")), arrIn)
    '                End If
    '            Next
    '            If rbtnLocationSelect.IsChecked Then
    '                frm.arrLoc = New Dictionary(Of String, Object)
    ...
    '            frm.arrItem.Add(clsCommon.myCstr(gv1.CurrentRow.Cells("Item_Code").Value))
    ...
    '            frm.arrLoc.Add(clsCommon.myCstr(gv1.CurrentRow.Cells("Main_Location_Code").Value), IIf(clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells("Location_Code").Value), clsCommon.myCstr(gv1.CurrentRow.Cells("Main_Location_Code").Value)) = CompairStringResult.Equal, Nothing, arrInn))
    '            If rbtnCategorySelect.IsChecked Then
    '                frm.arrCat = New Dictionary(Of String, Object)
    '                For ii As Integer = 0 To gvCategory.RowCount - 1
    '                    If clsCommon.myCBool(gvCategory.Rows(ii).Cells("SEL").Value) Then
    ...
    '                Next
    '            End If
    '            frm.strType = "Document Wise Detail"
    '            frm.WindowState = FormWindowState.Maximized
    '            frm.Show()
    '        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal Then
    ...
    Sub print(ByVal exporter As EnumExportTo)
        Try
            ...
            If exporter = EnumExportTo.Excel Then
                clsCommon.MyExportToExcelGrid("Flavoured Milk", gv1, arrHeader, Me.Text)
            Else
    ...
    Private Function GetReportID() As String
        ...
        If cboType.Enabled = False Then
            If clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Type Wise Summary") = CompairStringResult.Equal Then
                ReportID = "stReNity" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Category Wise Summary") = CompairStringResult.Equal Then
                ReportID = "stReNCatg" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Wise Summary") = CompairStringResult.Equal Then
                ReportID = "stReNItem" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item And Location Wise Summary") = CompairStringResult.Equal Then
                ReportID = "stReNLOCItem" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail") = CompairStringResult.Equal Then
                ReportID = "stReNDoc" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")

            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Item Group Wise Summary") = CompairStringResult.Equal Then
                ReportID = "stReIGWS" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date, Item And Document Wise Detail") = CompairStringResult.Equal Then
                ReportID = "stReNDIDW" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Date and Item Wise Stock") = CompairStringResult.Equal Then
                ReportID = "stReNDIWS" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboType.SelectedValue), "Document Wise Detail Ledger") = CompairStringResult.Equal Then
                ReportID = "stReNDWDL" + IIf(chkFATAndSNF.Checked, "F", "") + IIf(ChkMRPWise.Checked, "M", "")
            End If
        End If
        Return ReportID
    End Function
    ...
    Private Sub btn_Excel_Click(sender As Object, e As EventArgs) Handles btn_Excel.Click
        Try
            ...
            arrHeader.Add("Date Range: " + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") + " To " + clsCommon.GetPrintDate(txtToDate.Value, "dd/MM/yyyy"))
            arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptFlavouredMilk & "'"))
