...
'--------Created By Richa 13/12/2017 Bulk sale trade dispatch return

Imports System.Data.SqlClient
Imports common
Imports System.IO

Public Class FrmDispatchBulkSaleTradereturn
...
#End Region
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                SaveData()
                If (ClsDispatchBulkSaleTradeReturn.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
    ...
    Private Function AllowToSave() As Boolean
        ...
        For i As Integer = 0 To gv1.Rows.Count - 1
            ...
            'If clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select count(Document_No ) from TSPL_Dispatch_BulkSale_Trade_Return where Document_No ='" & txtDocNo.Value & "' and ISNULL(Against_SRN_No,'' )<>''")), "1") = CompairStringResult.Equal Then
    ...
    Sub SaveData()
        ...
        Dim objTr As New clsDispatchDetailTradeBulkSaleReturn
        Try
            If AllowToSave() Then
                BulkMilkSRNNo = clsDBFuncationality.getSingleValue("SELECT SRN_NO  FROM TSPL_Bulk_MILK_SRN WHERE Challan_No='" & FndDispatchCode.Value & "' AND isPosted=1 and  TSPL_Bulk_MILK_SRN.FormType='Bulk Milk SRN Trade'")
                obj.Against_SRN_No = BulkMilkSRNNo
                ...
                obj.DispatchTrade_No = FndDispatchCode.Value
                'If chkCreateAoutoInvoice.Checked = True Then
                '    obj.Is_Create_Auto_Invoice = 1
                'Else
                '    obj.Is_Create_Auto_Invoice = 0
                'End If
                obj.arrDispatchDetailTradeBulkSaleReturn = New List(Of clsDispatchDetailTradeBulkSaleReturn)




                For Each grow As GridViewRowInfo In gv1.Rows
                    objTr = New clsDispatchDetailTradeBulkSaleReturn()
                    ...
                    obj.arrDispatchDetailTradeBulkSaleReturn.Add(objTr)
                Next

                If (ClsDispatchBulkSaleTradeReturn.SaveData(obj, isNewEntry)) Then
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Dim dt As DataTable = Nothing
        Try
            obj = ClsDispatchBulkSaleTradeReturn.GetData(strCode, NavTyep)
            ...
            If obj IsNot Nothing Then
                ...
                'End If
                FndDispatchCode.Value = obj.DispatchTrade_No
                TxtEWayBillNo.Text = obj.EWayBillNo
                If obj.EWayBillDate IsNot Nothing Then
                    txtewaybilldate.Value = obj.EWayBillDate
                    txtewaybilldate.Checked = True
                Else
                    ...
                    txtewaybilldate.Checked = False
                End If
                If obj.arrDispatchDetailTradeBulkSaleReturn IsNot Nothing AndAlso obj.arrDispatchDetailTradeBulkSaleReturn.Count > 0 Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        ...
        Try
            qry = "select count(*) from TSPL_Dispatch_BulkSale_Trade_Return where Document_No='" + txtDocNo.Value + "' and Comp_Code='" + objCommonVar.CurrentCompanyCode + "'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtDocNo._MYValidating
        Dim qry As String = "Select TSPL_Dispatch_BulkSale_Trade_Return.Document_No as Code,Convert(varchar,TSPL_Dispatch_BulkSale_Trade_Return.Document_Date,103) as [Date],TSPL_Dispatch_BulkSale_Trade_Return.Customer_Code as [Customer Code],TSPL_CUSTOMER_MASTER.Customer_Name as [Customer Name],ISNULL(TSPL_CUSTOMER_MASTER.Alies_Name,'') As [Alies Name],TSPL_Dispatch_BulkSale_Trade_Return.Location_Code as [Dispatch Location Code],TSPL_LOCATION_MASTER.Location_Desc as [Dispatch Location Name],TSPL_Dispatch_BulkSale_Trade_Return.Price_Code as [Price Chart Code],TSPL_Dispatch_BulkSale_Trade_Return.Tanker_No as [Tanker No],TSPL_Dispatch_BulkSale_Trade_Return.Against_SRN_No_Return as [Against SRN No],case when TSPL_Dispatch_BulkSale_Trade_Return.Posted=0 then 'Pending' else 'Approved' end as Status from TSPL_Dispatch_BulkSale_Trade_Return Left Outer Join TSPL_LOCATION_MASTER on TSPL_Dispatch_BulkSale_Trade_Return.Location_Code =TSPL_LOCATION_MASTER.Location_Code Left Outer Join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_Dispatch_BulkSale_Trade_Return.Customer_Code"
        Dim whrcls As String = ""
        If clsCommon.myLen(arrLoc) > 0 Then
            whrcls = " TSPL_Dispatch_BulkSale_Trade_Return.Location_Code in (" & arrLoc & ")"
        End If
        txtDocNo.Value = clsCommon.ShowSelectForm("DispatchTradeBulkSaleReturn", qry, "Code", whrcls, txtDocNo.Value, "", isButtonClicked)
        LoadData(txtDocNo.Value, NavigatorType.Current)
        qry = Nothing
    End Sub
    ...
    Private Sub FndDispatchCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles FndDispatchCode._MYValidating
        Qry = "Select * from tspl_dispatch_bulksale_trade "
        Dim whrCls As String = "  Document_No not in (" & _
" Select distinct TSPL_INVOICE_DETAIL_BULKSALE.Dispatch_Code from TSPL_INVOICE_DETAIL_BULKSALE left outer join TSPL_INVOICE_MASTER_BULKSALE on TSPL_INVOICE_DETAIL_BULKSALE.Document_No =TSPL_INVOICE_MASTER_BULKSALE.Document_No where TSPL_INVOICE_MASTER_BULKSALE.InvoiceAgainst ='Against Dispatch Trade'  " & _
" ) and Against_SRN_No not in (Select distinct tspl_Bulk_milk_purchase_Invoice_Detail.SRN_NO from tspl_Bulk_milk_purchase_Invoice_Detail left outer join  tspl_Bulk_milk_purchase_Invoice_head on tspl_Bulk_milk_purchase_Invoice_Detail.DOC_NO =tspl_Bulk_milk_purchase_Invoice_head.DOC_NO where tspl_Bulk_milk_purchase_Invoice_head.isSRNTradeInvoice =1) "
        whrCls += " and tspl_dispatch_bulksale_trade.Location_Code in (" + arrLoc + ") and tspl_dispatch_bulksale_trade.Posted=1 and tspl_dispatch_bulksale_trade.Document_No not in (Select DispatchTrade_No from tspl_dispatch_bulksale_trade_return where Document_No<>'" + txtDocNo.Value + "' and tspl_dispatch_bulksale_trade_return.Location_Code in (" + arrLoc + ") ) "
        FndDispatchCode.Value = clsCommon.ShowSelectForm("DispatchTradeFinder", Qry, "Document_No", whrCls, clsCommon.myCstr(FndDispatchCode.Value), "Document_No", isButtonClicked)
        If clsCommon.myLen(FndDispatchCode.Value) > 0 Then
            LoadDispatchData(FndDispatchCode.Value, NavigatorType.Current)
        Else
            Reset()
        End If
    End Sub
