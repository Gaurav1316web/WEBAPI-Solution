...
#End Region
    ...
    Sub OpenCustomerList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            ...
            gv1.CurrentRow.Cells(colCustCode).Value = clsCustomerMaster.getFinder(" isnull(Manual_Customer,'')='Y'", clsCommon.myCstr(gv1.CurrentRow.Cells(colCustCode).Value), isButtonClick)
        End If
        strICode = Nothing
    End Sub
            ...
            If clsCommon.myCdbl(StandardRate) > 0 Then
                ...
                For IntRowNo As Integer = 0 To gv1.Rows.Count - 1
                    If clsCommon.myLen(clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colCustCode).Value)) > 0 Or clsCommon.myLen(clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colItemCode).Value)) > 0 Then
                        ...
                        'TotalAmount = Qty * StandardRate

                        TotalAmount = Qty * clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colMilkRate).Value)

                        ' ''SNFRate =Total Amount/(((Fat Weightage/Fat%)*( SNF %/SNF Weightage))* Fat kg)+ SNF Kg)					
                        'SNFRate = TotalAmount / ((((FatWeightage / FatPer) * (SNFPer / SNFWeightage)) * FatKG) + SNFKG)
                        ''Fat Rate=	(Total Amount-(SNF Rate * SNF KG))/Fat Kg			
