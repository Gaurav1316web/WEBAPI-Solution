' '' '' ''Created By richa for ticket BHA/09/05/18-000021
Imports common
...
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        repoCrateQty.HeaderText = "Can Qty"
        ...
        repoCrateQtyRecd.HeaderText = "Can Qty Recd."
    ...
    Private Sub fndCustomerNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndCustomerNo._MYValidating
        Dim strWhr As String = " and isnull(Manual_Customer,'')='Y' "
    ...
    Sub FillGrid()
        ...
        Qry = "SELECT Customer_Code,MAX(Customer_Name) as  Customer_Name,SUM(Can) AS Can FROM " & _
        " ( select 0 as posted, TSPL_CANSALE_DISPATCH_HEAD.Location_Code ,TSPL_CANSALE_DISPATCH_HEAD.Customer_Code,TSPL_CUSTOMER_MASTER.Customer_Name," & _
        " TSPL_CANSALE_DISPATCH_HEAD.TotalNoofCans as Can from TSPL_CANSALE_DISPATCH_HEAD " & _
        " LEFT OUTER JOIN TSPL_CUSTOMER_MASTER ON TSPL_CANSALE_DISPATCH_HEAD.Customer_Code=TSPL_CUSTOMER_MASTER.Cust_Code" & _
        " where isnull(TSPL_CANSALE_DISPATCH_HEAD.CanInventoryType,0) =1 and TSPL_CANSALE_DISPATCH_HEAD.TotalNoofCans  > 0 and TSPL_CANSALE_DISPATCH_HEAD.Location_Code ='" & fndLocation.Value & "' " & _
        " and convert(date,TSPL_CANSALE_DISPATCH_HEAD.Document_Date,103)='" & clsCommon.GetPrintDate(txtInvoiceDate.Value, "dd/MMM/yyyy") & "' " & _
        "UNION ALL " & _
        "SELECT  case when posted=1 then 0 else -1 end as posted , Location_Code,Customer_Code,Customer_Name,-1 * CrateQtyRecd FROM " & _
        "TSPL_CRATE_RECEIVED_HEAD_FRESHSALE LEFT OUTER JOIN TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE ON " & _
        "TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Document_No=TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE.Document_No  LEFT OUTER JOIN " & _
        "TSPL_CUSTOMER_MASTER ON TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE.Customer_Code=TSPL_CUSTOMER_MASTER.Cust_Code " & _
        " where TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Trans_Type='Can' " & _
        " and Location_Code='" & fndLocation.Value & "' and convert(date,TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.invoice_date,103)= '" & clsCommon.GetPrintDate(txtInvoiceDate.Value, "dd/MMM/yyyy") & "'  " & _
        ") FINAL  GROUP BY Customer_Code  having sum(posted)=0 and sum(Can ) > 0 "
        ...
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            For Each dr As DataRow In dt.Rows
                ...
                gv1.Rows(gv1.Rows.Count - 1).Cells(colCreateQty).Value = clsCommon.myCdbl(dr("Can"))
            Next
            SetIDs()
        Else
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ''richa BHA/05/11/18-000666 13 Nov,2015
            Qry = "select count(*) from TSPL_CRATE_RECEIVED_HEAD_FRESHSALE where Document_No='" + txtDocNo.Value + "' and TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Trans_Type='Can'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtDocNo._MYValidating
        ...
        Dim whrClas As String = " TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Trans_Type='Can' "
        ''richa BHA/05/11/18-000666 13 Nov,2015
