...
Public Class FrmCSATransferGateOut
    Inherits FrmMainTranScreen
    Public isLoadData As Boolean = False
    Dim obj As clsCSATransferGateOut = Nothing
    Private Sub btnClose_Click(sender As Object, e As EventArgs) Handles btnClose.Click
        Me.Close()
    End Sub
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.FrmCSATransferGateOut)
    ...
    Function allowToSave() As Boolean
        Try
            ...
            End If
            '===============================================================
            If clsCommon.myLen(fndCSATransferNo.Value) <= 0 Then
    ...
    Sub SaveData()
        Try
            ...
            obj.CSATransfer_No = clsCommon.myCstr(fndCSATransferNo.Value)
            obj.CSATransfer_Date = clsCommon.GetPrintDate(txtCSATransferDate.Text, "dd/MMM/yyyy")
            ...
            obj.Cust_Code = clsCommon.myCstr(txtCustomerCode.Text)
    ...
    Sub loadGateoutData(ByVal str As String, ByVal navtype As NavigatorType)
        obj = clsCSATransferGateOut.getGateOutData(str, navtype)
        If obj IsNot Nothing Then
            Reset()
            isLoadData = True
            fndDocNo.Value = obj.Document_No
            txtGateOutDate.Text = obj.Gate_Out_Date
            fndCSATransferNo.Value = obj.CSATransfer_No
            txtCSATransferDate.Text = obj.CSATransfer_Date
            ...
            txtCustomerCode.Text = obj.Cust_Code
    ...
    Private Sub FrmCSATransferGateOut_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Reset()
    End Sub

    Private Sub fndCSATransferNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndCSATransferNo._MYValidating
        fndCSATransferNo.Value = clsCSATransferGateOut.getCSATransferFinder("", fndCSATransferNo.Value, isButtonClicked)
        loadCSATransferData(fndCSATransferNo.Value, NavigatorType.Current)
    End Sub
    Sub loadCSATransferData(ByVal str As String, ByVal navtype As NavigatorType)
        obj = clsCSATransferGateOut.getCSATransferData(str, navtype)
        If obj IsNot Nothing Then
            Reset()
            isLoadData = True
            fndCSATransferNo.Value = obj.CSATransfer_No
    ...
    Private Sub fndDocNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndDocNo._MYValidating
        fndDocNo.Value = clsCSATransferGateOut.getGateOutFinder("", fndDocNo.Value, isButtonClicked)
    ...
    Sub deleteData()
        ...
        Try
            If clsCommon.myLen(fndDocNo.Value) > 0 Then
                ...
                If clsCSATransferGateOut.deleteData(fndDocNo.Value, trans) Then
                    myMessages.delete()
                    trans.Commit()
                    Reset()
                Else
