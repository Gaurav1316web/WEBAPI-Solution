...
Public Class frmGateEntryReturnCSA
    Inherits FrmMainTranScreen
    Private MainFormId As String = clsUserMgtCode.frmGateEntryReturnCS
    ...
    Sub loadData(ByVal str As String, ByVal navtype As NavigatorType)
        obj = clsGateEntryReturnCSA.GetData(str, navtype)
        If obj IsNot Nothing Then
            ...
            If clsCommon.myLen(fndRefDocNo.Value) > 0 Then
                txtRefDocDate.Text = clsGateEntryReturnCSA.GetRefDocDate(fndRefDocNo.Value, Nothing)
            End If
            txtVehicleNo.Text = obj.Manual_Vehicle
            txtRemarks.Text = obj.Remarks
            txtCustomerCode.Text = obj.Customer
            txtCustomerName.Text = clsGateEntryReturnCSA.GetCustomerName(txtCustomerCode.Text, Nothing)
    ...
    Sub deleteData()
        Try
            If clsCommon.myLen(fndDocNo.Value) > 0 Then
                ...
                If clsGateEntryReturnCSA.DeleteData(fndDocNo.Value) Then
                    myMessages.delete()
                    Reset()
                Else
    ...
    Private Sub fndDocNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndDocNo._MYValidating
        fndDocNo.Value = clsGateEntryReturnCSA.getFinder("", fndDocNo.Value, isButtonClicked)
    ...
    Private Sub fndDispatchNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndRefDocNo._MYValidating
        fndRefDocNo.Value = clsGateEntryReturnCSA.getRefDocFinder("Status=1 and trans_type='CSA'", fndRefDocNo.Value, isButtonClicked)
        If clsCommon.myLen(fndRefDocNo.Value) > 0 Then
            txtRefDocDate.Text = clsGateEntryReturnCSA.GetRefDocDate(fndRefDocNo.Value, Nothing)
            txtCustomerCode.Text = clsGateEntryReturnCSA.GetCustomerCode(fndRefDocNo.Value, Nothing)
            txtCustomerName.Text = clsGateEntryReturnCSA.GetCustomerName(txtCustomerCode.Text, Nothing)
        End If
    End Sub
    ...
    Sub PostData()
        Try
            If clsCommon.myLen(fndDocNo.Value) > 0 Then
                If clsGateEntryReturnCSA.PostData(fndDocNo.Value) Then
                    myMessages.post()
                    loadData(fndDocNo.Value, NavigatorType.Current)
                Else
