...
'---preeti Gupta---Ticket No.-BM00000003015,BM00000003793,BM00000005174--
'-Updation By [Pankaj Kumar Chaudhary]--Against Ticket No--[BM00000003165 14/07/2014]
'' Work done in print agaist ticket no. SWA/04/07/18-000030 
Imports common
...
Public Class frmInvoiceFreshSale
...
#Region "Variables"
    ...
    Const ReportID As String = "InvoiceFreshSaleItemGrid"
...
#End Region
    Public Sub SetUserMgmtNew()
        ...
        'MyBase.SetUserMgmt(clsUserMgtCode.frmInvoiceFreshSale)
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            clsDispatchNoteFreshSale.IsValidCustomer(arrReqNo, txtVendorNo.Value)
        ...
        Try
            If (AllowToSave()) Then
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsSaleInvoiceDetailFreshSale()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsSaleInvoiceFreshSale()
            obj = clsSaleInvoiceFreshSale.GetData(strCode, "'T','R','N'", NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsSaleInvoiceDetailFreshSale In obj.Arr
                        ...
                        '    gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsDispatchNoteFreshSaleDetail.GetBalanceSRNQty(objTr.Shipment_Code, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable)
                        'End If
        ...
        Try
            ...
            If clsCommon.myLen(dtContent.Rows(0)("Email_Text")) > 0 Then
                ...
                If dt1.Rows.Count > 0 Then
                    ...
                    strRptPath = frmCRV.funreport(True, CrystalReportFolder.NewSalesReports, dt1, "crptShipment", "Shippment Detail")
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                SaveData(True)
                If (clsSaleInvoiceFreshSale.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsSaleInvoiceFreshSale.DeleteData(txtDocNo.Value, txtReqNo.Value)) Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ...
            If clsCommon.myLen(strwherecls) = 0 Then
                qst = "select count(*) from TSPL_SD_SALE_INVOICE_HEAD where Document_Code='" + txtDocNo.Value + "'   and Invoice_Type in ('T','R','N') and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='FS' "
            Else
                qst = "select count(*) from TSPL_SD_SALE_INVOICE_HEAD where Document_Code='" + txtDocNo.Value + "'   and Invoice_Type in ('T','R','N') and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='FS' and TSPL_SD_SALE_INVOICE_HEAD.Customer_Code in (" + strwherecls + ")"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        LoadData(clsCommon.ShowSelectForm("ShipmentCofnd", qry, "Code", whrClas, txtDocNo.Value, "TSPL_SD_SALE_INVOICE_HEAD.Document_Date desc", isButtonClicked, "TSPL_SD_SALE_INVOICE_HEAD.Document_Date"), NavigatorType.Current)
    End Sub
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating, TxtFinder5._MYValidating, TxtFinder1._MYValidating
        If clsCommon.myLen(txtBillToLocation.Value) = 0 Then
            clsCommon.MyMessageBoxShow(Me, "Please select Location first", Me.Text)
            txtBillToLocation.Focus()
            Exit Sub
    ...
    Sub SelectMRNItems()
        isInsideLoadData = True
        Dim frm As New frmPendingDispatchNoteFreshSale()
        frm.VendorCode = txtVendorNo.Value
        frm.strCurrCode = txtDocNo.Value
        frm.ShowDialog()
        LoadBlankGrid()
        Dim objOrderHead As clsDispatchNoteFreshSale = Nothing
        If frm.ArrReturn IsNot Nothing AndAlso frm.ArrReturn.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn(0).Document_Code) > 0 Then
                objOrderHead = clsDispatchNoteFreshSale.GetData(frm.ArrReturn(0).Document_Code, NavigatorType.Current)
                If objOrderHead IsNot Nothing AndAlso clsCommon.myLen(objOrderHead.Document_Code) > 0 Then
                    ...
                    If (clsCommon.myLen(txtVehicleCapacity.Text) <= 0) Then
                        ...
                        txtCrateQty.Value = objOrderHead.CrateQty
                    End If
            ...
            For ii As Integer = 0 To frm.ArrReturn.Count - 1
                If clsCommon.myLen(frm.ArrReturn(ii).Document_Code) > 0 Then
                    ...
                    '    arr.Add(strCode)
                    objOrderHead = clsDispatchNoteFreshSale.GetData(frm.ArrReturn(ii).Document_Code, NavigatorType.Current)
                    For Each obj As clsDispatchNoteFreshSaleDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn(ii).Item_Code AndAlso obj.Scheme_Item = "N") OrElse (obj.Scheme_Code = frm.ArrReturn(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            ...
                            If IsValidItem(obj) Then
                                ...
                                'Dim dt As DataTable = clsSaleInvoiceFreshSale.GetOriginalQty(obj.Document_Code, obj.Item_Code, obj.Unit_code, obj.Assessable, obj.MRP, Nothing)
                                'If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsDispatchNoteFreshSaleDetail In objOrderHead.Arr
    ...
    Function IsValidItem(ByVal obj As clsDispatchNoteFreshSaleDetail)
        If clsCommon.myLen(txtTaxGroup.Value) <= 0 Then
            ...
            ElseIf gv1.CurrentColumn Is gv1.Columns(colComplete) AndAlso UsLock1.Status = ERPTransactionStatus.Approved Then
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 AndAlso clsCommon.myLen(strICode) > 0 AndAlso intSNo > 0 AndAlso clsCommon.CompairString(strStatus, "No") = CompairStringResult.Equal Then
                    If common.clsCommon.MyMessageBoxShow("Do you want to complete the item " + clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value), Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                        If clsSaleInvoiceDetailFreshSale.CompleteSRN(txtDocNo.Value, strICode, intSNo) Then
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Else
            ...
            'objMultPrintInvoice.LoadPrintQuery(txtDocNo.Value)
    ...
    Public Sub funPrint(ByVal strDocNo As String)
        ...
        Try
            ...
            'Qry += "   Scheme_Item, TSPL_LOCATION_MASTER.Add1 as Loc_Add1,TSPL_LOCATION_MASTER.Add2  as Loc_Add2,TSPL_LOCATION_MASTER.Add3  as Loc_Add3,TSPL_LOCATION_MASTER.Pin_Code  as Loc_PinNo,TSPL_LOCATION_MASTER.Email  as Loc_Email ,TSPL_LOCATION_MASTER.TIN_No as Loc_Tin_No,TSPL_COMPANY_MASTER.Comp_Code ,TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2 , TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER_For_Comp.City_Name )>0 then ', '+TSPL_CITY_MASTER_For_Comp.City_Name else ' ' end + case when len(TSPL_STATE_MASTER.STATE_NAME  )>0 then TSPL_STATE_MASTER.STATE_NAME  else '' end +   "
            'Qry += " case when LEN(TSPL_COMPANY_MASTER.Tin_No  )>0 then ', '+TSPL_COMPANY_MASTER.Tin_No else ' ' end "

            'Qry += " as Loc_Add  , TSPL_CUSTOMER_MASTER.City_Code ,TSPL_CITY_MASTER.City_Name as Cust_City_Name ,TSPL_SD_SALE_INVOICE_HEAD.VehicleNo, TSPL_SD_SALE_INVOICE_HEAD.Document_Code as InvoiceNo,convert(varchar ,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) as InvoiceDate "
            'Qry += " ,TSPL_SD_SALE_INVOICE_HEAD.Against_Shipment_No as ShipmentNo,TSPL_SD_SHIPMENT_HEAD.Document_Date as ShipmentDate,"
            ...
            'Qry += " Case when ISNULL(TSPL_COMPANY_MASTER.Phone1,'')='(+__)__________' then '' else TSPL_COMPANY_MASTER.Phone1 end +  Case When   ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'(+__)__________' Then ',  '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as CompPhone,"
            ...
            'Qry += "  TSPL_CUSTOMER_MASTER.Customer_Name as CustName,TSPL_CUSTOMER_MASTER.Add1 as Customer_Add1,TSPL_CUSTOMER_MASTER.add2 as"
            'Qry += "   customer_Add2,TSPL_CUSTOMER_MASTER.Add3 as customer_Add3 "
            'Qry += ",TSPL_CUSTOMER_MASTER.State as customer_State ,TSPL_CUSTOMER_MASTER.PIN_Code as Customer_Pin_Code,TSPL_CUSTOMER_MASTER.Tin_No as CustTinNo,"
            ...
            'Qry += " ,TSPL_SD_SALE_INVOICE_DETAIL.mrp,TSPL_SD_SALE_INVOICE_DETAIL.amount as amount,TSPL_SD_SALE_INVOICE_DETAIL.unit_code as uom"
            ...
            'Qry += "  isnull (TSPL_SD_SALE_INVOICE_HEAD.tax10_amt,0) as txt10amt,TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Rate,   isnull(TSPL_SD_SALE_INVOICE_HEAD .Total_Tax_Amt,0) as total_tax_amt, TSPL_SD_SALE_INVOICE_HEAD.Total_Amt as DocAmt "
            ...
            '=============shivani(Adding Sampling)
            Dim Qry As String = "  select  TSPL_SD_SALE_INVOICE_HEAD.Comments ,TSPL_SD_SALE_INVOICE_HEAD.Description ,tspl_company_master.cst_lst as Comp_CSt_LST, TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER_fOR_Comp.City_Name)>0 then ', '+TSPL_CITY_MASTER_fOR_Comp.City_Name else ' ' end + case when len(TSPL_STATE_MASTER_For_Comp.STATE_NAME  )>0 then ', '+ TSPL_STATE_MASTER_For_Comp.STATE_NAME else ' ' end  + case when len(TSPL_COMPANY_MASTER.Pincode    )>0 then ', Pin Code - '+ cast(TSPL_COMPANY_MASTER.Pincode as varchar)  else ' ' end  + case when len(TSPL_COMPANY_MASTER.Tin_No     )>0 then ', Tin No - '+ cast(TSPL_COMPANY_MASTER.Tin_No as varchar)  else ' ' end  + case when len(TSPL_COMPANY_MASTER.CINNo      )>0 then ', CIN No - '+ cast(TSPL_COMPANY_MASTER.CINNo as varchar)  else ' ' end  + case when len(TSPL_COMPANY_MASTER.Fax     )>0 then ',Fax '+ TSPL_COMPANY_MASTER.Fax else '' end   + Case when len(ISNULL(TSPL_COMPANY_MASTER.Phone1,''))>0 and TSPL_COMPANY_MASTER.Phone1='(+__)__________' then '' else ' ,Phone'+TSPL_COMPANY_MASTER.Phone1 end   +  Case When   ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'(+__)__________' Then '  '+ TSPL_COMPANY_MASTER.Phone2 Else'' End   + case when len(TSPL_COMPANY_MASTER.Email    )>0 then ',Email - '+ TSPL_COMPANY_MASTER.Email else '' end  as Company_Address,isnull(Sampling,0) as Sampling ,   Scheme_Item, TSPL_LOCATION_MASTER.Add1 as Loc_Add1,TSPL_LOCATION_MASTER.Add2  as Loc_Add2,TSPL_LOCATION_MASTER.Add3 "
            Qry += " as Loc_Add3,TSPL_LOCATION_MASTER.Pin_Code  as Loc_PinNo,TSPL_LOCATION_MASTER.Email  as Loc_Email ,TSPL_LOCATION_MASTER.TIN_No as Loc_Tin_No,TSPL_COMPANY_MASTER.Comp_Code ,TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2 , TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER_For_Comp.City_Name )>0 then ', '+TSPL_CITY_MASTER_For_Comp.City_Name else ' ' end + case when len(TSPL_STATE_MASTER.STATE_NAME  )>0 then TSPL_STATE_MASTER.STATE_NAME  else '' end +    case when LEN(TSPL_COMPANY_MASTER.Tin_No  )>0 then ', '+TSPL_COMPANY_MASTER.Tin_No else ' ' end  as Loc_Add  , TSPL_CUSTOMER_MASTER.City_Code ,TSPL_CITY_MASTER.City_Name as Cust_City_Name ,TSPL_SD_SALE_INVOICE_HEAD.VehicleNo, TSPL_SD_SALE_INVOICE_HEAD.Document_Code as InvoiceNo,convert(varchar ,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) as InvoiceDate  ,TSPL_SD_SALE_INVOICE_HEAD.Against_Shipment_No as ShipmentNo,TSPL_SD_SHIPMENT_HEAD.Document_Date as ShipmentDate, TSPL_SD_SHIPMENT_HEAD.Against_Delivery_Code as DeliveryOrderNo,TSPL_SD_SALE_INVOICE_HEAD.Terms_Code as TermCondition, TSPL_LOCATION_MASTER.Location_Desc , TSPL_COMPANY_MASTER.Comp_Name as CompName,"
            Qry += " ISNULL(TSPL_COMPANY_MASTER.Phone1,'')+  Case when ISNULL(TSPL_COMPANY_MASTER.Phone1,'')='(+__)__________' then '' else TSPL_COMPANY_MASTER.Phone1 end +  Case When   ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'(+__)__________' Then ',  '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as CompPhone, TSPL_COMPANY_MASTER.Fax as CompFax,TSPL_COMPANY_MASTER.Email as ComMail,TSPL_COMPANY_MASTER.CINNo as CompCinNo,TSPL_COMPANY_MASTER.Pan_No as ComPanNo,  TSPL_COMPANY_MASTER.CST_LST as CompCSTLST,TSPL_COMPANY_MASTER.Pincode as ComPINCode, TSPL_COMPANY_MASTER .Tin_No as ComTinNO,ISNULL(tspl_company_Master.ADD1,'') as Compaddress1,ISNULL(tspl_company_Master.ADD2,'')   as Compaddress2,ISNULL(tspl_company_Master.ADD3,'') as Compaddress3,  TSPL_CUSTOMER_MASTER.Customer_Name as CustName,TSPL_CUSTOMER_MASTER.Add1 as Customer_Add1,TSPL_CUSTOMER_MASTER.add2 as   customer_Add2,TSPL_CUSTOMER_MASTER.Add3 as customer_Add3 ,TSPL_CUSTOMER_MASTER.State as customer_State"
            Qry += " ,TSPL_CUSTOMER_MASTER.PIN_Code as Customer_Pin_Code,TSPL_CUSTOMER_MASTER.Tin_No as CustTinNo,TSPL_SD_SALE_INVOICE_DETAIL.item_code+ case when  Scheme_Item='Y' then ' (Free Scheme)' when Scheme_Item='N' then '' end +  case when isnull(TSPL_SD_SALE_INVOICE_DETAIL.Sampling,0)='1' then ' (Sampling)' when isnull(TSPL_SD_SALE_INVOICE_DETAIL.Sampling,0)='0' then ''  end as item_code, TSPL_ITEM_MASTER.Item_Desc + case when Scheme_Item='Y' then ' (Free Scheme)' when Scheme_Item='N' then ''  end +  case when isnull(TSPL_SD_SALE_INVOICE_DETAIL.Sampling,0)='1' then ' (Sampling)' when isnull(TSPL_SD_SALE_INVOICE_DETAIL.Sampling,0)='0' then ''  end as itemdesc    ,TSPL_SD_SALE_INVOICE_DETAIL.Qty as qty ,TSPL_SD_SALE_INVOICE_DETAIL.mrp,TSPL_SD_SALE_INVOICE_DETAIL.amount as amount,TSPL_SD_SALE_INVOICE_DETAIL.unit_code as uom ,TSPL_SD_SALE_INVOICE_DETAIL.item_cost as itemcost, tax1.Tax_Code_Desc as tax1name,isnull (TSPL_SD_SALE_INVOICE_HEAD.tax1_amt,0) as txt1amt,   tax2.Tax_Code_Desc as tax2name,isnull (TSPL_SD_SALE_INVOICE_HEAD.tax2_amt,0) as txt2amt,   tax3.Tax_Code_Desc as tax3name,   isnull (TSPL_SD_SALE_INVOICE_HEAD.tax3_amt,0) as txt3amt,   tax4.Tax_Code_Desc as tax4name,   isnull (TSPL_SD_SALE_INVOICE_HEAD.tax4_amt,0) as txt4amt,   tax5.Tax_Code_Desc as tax5name,  isnull (TSPL_SD_SALE_INVOICE_HEAD.tax5_amt,0) as txt5amt,   tax6.Tax_Code_Desc as tax6name,  isnull (TSPL_SD_SALE_INVOICE_HEAD.tax6_amt,0) as txt6amt,   tax7.Tax_Code_Desc as tax7name, isnull (TSPL_SD_SALE_INVOICE_HEAD.tax7_amt,0) as txt7amt,   tax8.Tax_Code_Desc as tax8name,"
            Qry += " isnull (TSPL_SD_SALE_INVOICE_HEAD.tax8_amt,0) as txt8amt, tax9.Tax_Code_Desc as tax9name,  isnull (TSPL_SD_SALE_INVOICE_HEAD.tax9_amt,0) as txt9amt,   tax10.Tax_Code_Desc as tax10name,  isnull (TSPL_SD_SALE_INVOICE_HEAD.tax10_amt,0) as txt10amt,TSPL_SD_SALE_INVOICE_DETAIL.TAX1_Rate,   isnull(TSPL_SD_SALE_INVOICE_HEAD .Total_Tax_Amt,0) as total_tax_amt, TSPL_SD_SALE_INVOICE_HEAD.Total_Amt as DocAmt      from TSPL_SD_SALE_INVOICE_HEAD left outer join TSPL_SD_SALE_INVOICE_DETAIL on TSPL_SD_SALE_INVOICE_HEAD.Document_Code =TSPL_SD_SALE_INVOICE_DETAIL.DOCUMENT_CODE  left outer join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.Document_Code  = TSPL_SD_SALE_INVOICE_HEAD.Against_Shipment_No left outer join TSPL_LOCATION_MASTER on TSPL_SD_SALE_INVOICE_HEAD.Bill_To_Location =TSPL_LOCATION_MASTER.Location_Code   left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_SD_SALE_INVOICE_HEAD.comp_code  left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_SD_SALE_INVOICE_HEAD.Customer_Code  Left Outer Join TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_SD_SALE_INVOICE_DETAIL.Item_Code  left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_SD_SALE_INVOICE_HEAD.tax1 left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_SD_SALE_INVOICE_HEAD.tax2    left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_SD_SALE_INVOICE_HEAD .TAX3   left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_SD_SALE_INVOICE_HEAD .tax4   left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_SD_SALE_INVOICE_HEAD .tax5   left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD .TAX6    left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD .TAX7    left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD .TAX8  left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD .TAX9    left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_SD_SALE_INVOICE_HEAD .TAX10   left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code =TSPL_CUSTOMER_MASTER.City_Code  left outer join TSPL_STATE_MASTER on TSPL_STATE_MASTER.STATE_CODE =TSPL_COMPANY_MASTER.State  LEFT OUTER JOIN TSPL_CITY_MASTER  AS TSPL_CITY_MASTER_fOR_Comp ON TSPL_CITY_MASTER_fOR_Comp.City_Code =TSPL_COMPANY_MASTER.City_Code  LEFT OUTER JOIN TSPL_STATE_MASTER AS TSPL_STATE_MASTER_For_Comp  ON TSPL_STATE_MASTER_For_Comp.STATE_CODE  =TSPL_COMPANY_MASTER.State "
            Qry += " where TSPL_SD_SALE_INVOICE_HEAD.Document_Code ='" + strDocNo + "' order by sku_seq "
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry)

            If dt.Rows.Count > 0 Then
                'SetItemWiseTax(dt, strDocNo)
                ''KwalitySalesReportViewer.funreport(dt, "rptFreshSaleInvoice", "Retail Invoice")
                Dim frmCRV As New frmCrystalReportViewer()
                frmCRV.funsubreportWithdt(CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "rptFreshSaleInvoice", "Retail Invoice", txtDate.Value, "rptCompanyAddress.rpt")
                frmCRV = Nothing
            End If
    ...
    Private Sub txtDept__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDept._MYValidating, TxtFinder6._MYValidating, TxtFinder2._MYValidating
        Try
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating, TxtFinder7._MYValidating, TxtFinder3._MYValidating
    ...
    Private Sub txtCurrencyCode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtCurrencyCode._MYValidating
        ...
        ShowCurrencyDetail()
    End Sub
    Private Sub fndRouteNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtRouteNo._MYValidating, TxtFinder8._MYValidating, TxtFinder4._MYValidating
    ...
    Private Sub txtDept_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtDept.Load, TxtFinder6.Load, TxtFinder2.Load
    ...
    Private Sub btnReverseAndUnpost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverseAndUnpost.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsSaleInvoiceFreshSale.ReverseAndUnpost(txtDocNo.Value) Then
                    common.clsCommon.MyMessageBoxShow(e, "Successfully Reversed and Recreated", Me.Text)
                    LoadData(txtDocNo.Value, NavigatorType.Current)
                End If
            End If
        Catch ex As Exception
...
#Region "SMS Email Setting"
    ...
    Private Sub RadMenuItem5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem5.Click
        ...
        frm.FormId = clsUserMgtCode.frmInvoiceFreshSale
        frm.ShowDialog()
    End Sub
...
#End Region
    ...
    Private Sub txtVehicleNo_Click(sender As Object, e As EventArgs) Handles txtVehicleNo.Click

    End Sub
    Private Sub txtVehcileCode_Click(sender As Object, e As EventArgs) Handles txtVehcileCode.Click
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        Try
            If clsCommon.myLen(txtDocNo.Value) > 0 Then
                ...
                End If
                If clsSaleInvoiceFreshSale.UpdateAfterPosting(obj, txtReqNo.Value, Nothing) Then
