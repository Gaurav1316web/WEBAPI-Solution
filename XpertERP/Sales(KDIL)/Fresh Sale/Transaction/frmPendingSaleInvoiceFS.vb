Imports common
Public Class frmPendingSaleInvoiceFS
...
#Region "Variables"
    ...
    Public ArrReturn As List(Of clsSaleInvoiceDetailFreshSale) = Nothing
    ...
    Const colDSampling As String = "colDSampling"
    ...
    Const colDDeliveryNo As String = "colDDeliveryNo"
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        tp_FromDate.Value = clsCommon.GETSERVERDATE() 'tp_ToDate.Value.AddMonths(-1)
        LoadBlankHeadGrid()
        LoadBlankGridDetail()
        LoadData()
    End Sub
    ...
    Sub LoadBlankGridDetail()
        ...
        repoDeliveryCode.Name = colDDeliveryNo
        repoDeliveryCode.Width = 180
        repoDeliveryCode.IsVisible = False
        ...
        Dim reposampling As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        reposampling.FormatString = ""
        reposampling.HeaderText = "Sampling"
        reposampling.Name = colDSampling
        reposampling.Width = 180
        reposampling.ReadOnly = True
        gv1.MasterTemplate.Columns.Add(reposampling)
    ...
    Sub btnOKPressed()
        ArrReturn = New List(Of clsSaleInvoiceDetailFreshSale)
        Dim obj As clsSaleInvoiceDetailFreshSale = Nothing
        For ii As Integer = 0 To gv1.RowCount - 1
            If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                obj = New clsSaleInvoiceDetailFreshSale()
                obj.Document_Code = clsCommon.myCstr(gv1.Rows(ii).Cells(colDCode).Value)
                obj.Delivery_Code = clsCommon.myCstr(gv1.Rows(ii).Cells(colDDeliveryNo).Value)
                ...
                obj.Sampling = clsCommon.myCdbl(gv1.Rows(ii).Cells(colDSampling).Value)
        ...
        If NewVal Then
            For Each dr As DataRow In dtAllData.Rows
                If clsCommon.CompairString(strCode, clsCommon.myCstr(dr("Code"))) = CompairStringResult.Equal Then
                    ...
                    gv1.Rows(gv1.Rows.Count - 1).Cells(colDDeliveryNo).Value = clsCommon.myCstr(dr("Delivery_Code"))
                    ...
                    gv1.Rows(gv1.Rows.Count - 1).Cells(colDSampling).Value = clsCommon.myCdbl(dr("Sampling"))
    ...
    Sub LoadData()
        ...
        Try
            ...
            qry += " where TSPL_SD_SALE_INVOICE_DETAIL.Status=0 and Scheme_Item='N'  and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='FS' AND TSPL_SD_SALE_INVOICE_HEAD.Screen_Type='' and  TSPL_SD_SALE_INVOICE_HEAD.Status=1 and Invoice_Type in ('T','R','N')   and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) " + Environment.NewLine
            If clsCommon.myLen(VendorCode) > 0 Then
                qry += " and TSPL_SD_SALE_INVOICE_HEAD.Customer_Code='" + VendorCode + "'"
            End If
            qry += " union all " + Environment.NewLine
            qry += " select TSPL_SD_SALE_RETURN_DETAIL.Delivery_Code,TSPL_SD_SALE_RETURN_DETAIL.Sampling,TSPL_SD_SALE_RETURN_DETAIL.Line_No,  " + Environment.NewLine
            qry += " TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code as Code,TSPL_SD_SALE_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_RETURN_DETAIL.Item_Code as ICode,'' as IName,  " + Environment.NewLine
            qry += " '' as IType,case when OrgUnit_code <> Unit_code then convert(decimal(18,2),(TSPL_SD_SALE_RETURN_DETAIL.Qty  / ReturnUnit.Conversion_Factor) * StockingUOM.Conversion_Factor * OrgUnit.Conversion_Factor) else Qty end as Qty,  " + Environment.NewLine
            qry += " 0 as Unapproved,TSPL_SD_SALE_RETURN_DETAIL.OrgUnit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,  " + Environment.NewLine
            qry += " mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL   " + Environment.NewLine
            ...
            qry += " left outer join tspl_item_uom_detail as ReturnUnit on TSPL_SD_SALE_RETURN_DETAIL.Item_Code=ReturnUnit.Item_Code and TSPL_SD_SALE_RETURN_DETAIL.OrgUnit_code=ReturnUnit.UOM_Code  " + Environment.NewLine
            qry += " left outer join tspl_item_uom_detail as OrgUnit on TSPL_SD_SALE_RETURN_DETAIL.Item_Code=OrgUnit.Item_Code and TSPL_SD_SALE_RETURN_DETAIL.Unit_code=OrgUnit.UOM_Code  " + Environment.NewLine
            qry += " left outer join tspl_item_uom_detail as StockingUOM on TSPL_SD_SALE_RETURN_DETAIL.Item_Code=StockingUOM.Item_Code  and StockingUOM.Stocking_Unit='Y' " + Environment.NewLine
            qry += " where  TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' AND TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' and TSPL_SD_SALE_RETURN_HEAD.Status=1 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0   " + Environment.NewLine
            qry += " union all   " + Environment.NewLine
            qry += "  select TSPL_SD_SALE_RETURN_DETAIL.Delivery_Code,TSPL_SD_SALE_RETURN_DETAIL.Sampling,TSPL_SD_SALE_RETURN_DETAIL.Line_No, "
            qry += " TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code as Code,TSPL_SD_SALE_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_RETURN_DETAIL.Item_Code as ICode,'' as IName,  " + Environment.NewLine
            qry += " '' as IType,0 as Qty,case when OrgUnit_code <> Unit_code then convert(decimal(18,2),(TSPL_SD_SALE_RETURN_DETAIL.Qty  / ReturnUnit.Conversion_Factor) * StockingUOM.Conversion_Factor * OrgUnit.Conversion_Factor) else Qty end as Unapproved,  " + Environment.NewLine
            qry += " TSPL_SD_SALE_RETURN_DETAIL.OrgUnit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,  " + Environment.NewLine
            qry += " mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL  " + Environment.NewLine
            qry += " left outer join TSPL_SD_SALE_RETURN_HEAD on TSPL_SD_SALE_RETURN_HEAD.Document_Code=TSPL_SD_SALE_RETURN_DETAIL.Document_Code  " + Environment.NewLine
            qry += " left outer join tspl_item_uom_detail as ReturnUnit on TSPL_SD_SALE_RETURN_DETAIL.Item_Code=ReturnUnit.Item_Code and TSPL_SD_SALE_RETURN_DETAIL.OrgUnit_code=ReturnUnit.UOM_Code  " + Environment.NewLine
            qry += " left outer join tspl_item_uom_detail as OrgUnit on TSPL_SD_SALE_RETURN_DETAIL.Item_Code=OrgUnit.Item_Code and TSPL_SD_SALE_RETURN_DETAIL.Unit_code=OrgUnit.UOM_Code  " + Environment.NewLine
            qry += " left outer join tspl_item_uom_detail as StockingUOM on TSPL_SD_SALE_RETURN_DETAIL.Item_Code=StockingUOM.Item_Code  and StockingUOM.Stocking_Unit='Y'  " + Environment.NewLine
            qry += " where  TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' AND TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' and TSPL_SD_SALE_RETURN_HEAD.Status=0 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0 and TSPL_SD_SALE_RETURN_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
            'qry += " select TSPL_SD_SALE_RETURN_DETAIL.Delivery_Code,TSPL_SD_SALE_RETURN_DETAIL.Sampling,TSPL_SD_SALE_RETURN_DETAIL.Line_No,TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code as Code,TSPL_SD_SALE_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_RETURN_DETAIL.Item_Code as ICode,'' as IName,'' as IType,isnull(TSPL_SD_SALE_RETURN_DETAIL.Qty,0) as Qty,0 as Unapproved,TSPL_SD_SALE_RETURN_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date" + Environment.NewLine
            'qry += " ,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL " + Environment.NewLine
            'qry += " left outer join TSPL_SD_SALE_RETURN_HEAD on TSPL_SD_SALE_RETURN_HEAD.Document_Code=TSPL_SD_SALE_RETURN_DETAIL.Document_Code" + Environment.NewLine
            'qry += " where  TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Status=1 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0   " + Environment.NewLine
            'qry += " union all   " + Environment.NewLine
            ...
            'qry += " ,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL " + Environment.NewLine
            'qry += " left outer join TSPL_SD_SALE_RETURN_HEAD on TSPL_SD_SALE_RETURN_HEAD.Document_Code=TSPL_SD_SALE_RETURN_DETAIL.Document_Code" + Environment.NewLine
            'qry += " where  TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Status=0 and Scheme_Item='N'  and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0 and TSPL_SD_SALE_RETURN_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
            ...
            qry += " group by Code,ICode,Unit,Sampling,Delivery_Code having SUM(Chk)>0 and SUM(Qty *RI) <>0 order by Code,max(Line_No)"
