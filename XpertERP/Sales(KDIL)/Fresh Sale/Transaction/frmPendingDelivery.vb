...
Imports common
Public Class frmPendingDelivery
#Region "Variables"
...
#End Region
    ...
    Private Sub frmPendingDelivery_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'End If
        '-------------------------------------
        Dim qry As String = "select CAST(0 as bit) as Sel,code,ICode,max(IName) as IName,Unit,max(Location) as Location,MAX(TSPL_LOCATION_MASTER.Location_Desc) as LocationName,max(TransDate) as TransDate , " & _
        ...
        qry += " select TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Line_No,TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Document_No as Code,TSPL_DELIVERY_NOTE_MASTER_FRESHSALE.Customer_Code as Vendor, " & _
        "TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName, TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Qty  as Qty, " & _
        "0 as Unapproved,TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Unit_Code as Unit,TSPL_DELIVERY_NOTE_MASTER_FRESHSALE.Location_Code as Location, " & _
        "1 as RI,TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Rate as Rate,1 as Chk,Document_Date as TransDate, " & _
        "TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Price_code,TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Price_Date,TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.Conv_Factor,TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE.MRP from TSPL_DELIVERY_NOTE_DETAIL_FRESHSALE " & _
        ...
        "where TSPL_DELIVERY_NOTE_master_FRESHSALE.Posted=1  and TSPL_DELIVERY_NOTE_master_FRESHSALE.Short_Close='N'  and TSPL_DELIVERY_NOTE_master_FRESHSALE.OnHold='N'  " + Environment.NewLine
        ...
        qry += " where TSPL_SD_SHIPMENT_Head.Status=1 and Scheme_Item='N' and TSPL_SD_SHIPMENT_Head.Trans_Type='FS' and len(isnull(TSPL_SD_SHIPMENT_DETAIL.Delivery_Code,''))>0   " + Environment.NewLine
        qry += " union all   " + Environment.NewLine
        ...
        qry += " from TSPL_SD_SHIPMENT_DETAIL " + Environment.NewLine
        qry += " left outer join TSPL_SD_SHIPMENT_Head on TSPL_SD_SHIPMENT_Head.Document_Code=TSPL_SD_SHIPMENT_DETAIL.Document_Code" + Environment.NewLine
        qry += " where TSPL_SD_SHIPMENT_Head.Status=0 and Scheme_Item='N' and TSPL_SD_SHIPMENT_Head.Trans_Type='FS'  and len(isnull(TSPL_SD_SHIPMENT_DETAIL.Delivery_Code,''))>0 and TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
