...
Public Class frmSaleReturnFreshSale
...
#Region "Variables"
    ...
    Const ReportID As String = "FSSaleRetuenItemGrid"
    ...
    Const colDeliveryNo As String = "colDeliveryNo"
...
#End Region
    Public Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.frmSaleReturnFreshSale)
    ...
    Sub LoadBlankGrid()
        ...
        Dim repoSampling As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoSampling = New GridViewDecimalColumn()
        repoSampling.FormatString = ""
        repoSampling.HeaderText = "Sampling"
        repoSampling.Name = colSampling
        repoSampling.IsVisible = False
        repoSampling.Minimum = 0
        repoSampling.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        ...
        repoFreeQty.VisibleInColumnChooser = False
        ...
        repoHDisPer.VisibleInColumnChooser = True
        ...
        Dim repoDelivery As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoDelivery.FormatString = ""
        repoDelivery.HeaderText = "Delivery No"
        repoDelivery.Name = colDeliveryNo
        repoDelivery.ReadOnly = True
        repoDelivery.Width = 100
        gv1.MasterTemplate.Columns.Add(repoDelivery)
                    ...
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colDamageQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colUnit) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) Then
                        If (e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colDamageQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colDisPer) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal)) Then
                            If ((e.Column Is gv1.Columns(colQty))) Then
                                ...
                                OpenSerialItem()
                            End If
                            If clsCommon.myLen(gv1.CurrentRow.Cells(colUnit).Value) > 0 Then 'If clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value) > 0 And clsCommon.myCdbl(gv1.CurrentRow.Cells(ColFOC).Value) = 0 Then
    ...
    Sub OpenUOMList(ByVal isButtonClick As Boolean)
        Dim strICode As String = clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value)
        Dim strschemeItem As String = clsCommon.myCstr(gv1.CurrentRow.Cells(colSchemeItem).Value)
        If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(strschemeItem.ToUpper, "NO") = CompairStringResult.Equal Then
            ...
            'gv1.CurrentRow.Cells(colRate).Value = clsDBFuncationality.getSingleValue(" Select RowNo, Item_Price_ID, XXXE.Item_Code, UOM, Start_Date, Item_Basic_Price,Item_Basic_Net,Price_Code from ( " & _
            ...
            ' "TSPL_ITEM_PRICE_MASTER.Price_Code='" & lblPriceCode.Text & "' and UOM='" & gv1.CurrentRow.Cells(colOrgUnit).Value & "' and TSPL_ITEM_PRICE_MASTER.item_code='" & gv1.CurrentRow.Cells(colICode).Value & "' AND Location_Code='" & txtBillToLocation.Value & "'  " & _
            ' ") XXXE WHERE RowNo=1  ")

            Dim dt As DataTable = clsDBFuncationality.GetDataTable(" Select  Item_Basic_Price,Item_Basic_Net from ( " & _
           "Select ROW_NUMBER() OVER (Partition By TSPL_ITEM_PRICE_MASTER.Item_Code ORDER BY TSPL_ITEM_PRICE_MASTER.Item_Code,  " & _
           "Start_Date Desc) as RowNo, Item_Price_ID, TSPL_ITEM_PRICE_MASTER.Item_Code, UOM, Start_Date,  " & _
           "Item_Basic_Price,Item_Basic_Net,Price_Code from TSPL_ITEM_PRICE_MASTER  left  outer join  " & _
           "TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_PRICE_MASTER.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and  " & _
           "TSPL_ITEM_PRICE_MASTER.UOM=TSPL_ITEM_UOM_DETAIL.UOM_Code   where  Start_Date<='" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  and  " & _
           "TSPL_ITEM_PRICE_MASTER.Price_Code='" & lblPriceCode.Text & "' and UOM='" & gv1.CurrentRow.Cells(colUnit).Value & "' and TSPL_ITEM_PRICE_MASTER.item_code='" & gv1.CurrentRow.Cells(colICode).Value & "' AND Location_Code='" & txtBillToLocation.Value & "'  " & _
           ") XXXE WHERE RowNo=1  ")
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                gv1.CurrentRow.Cells(colRate).Value = clsCommon.myCdbl(dt.Rows(0)("Item_Basic_Price"))
                gv1.CurrentRow.Cells(colMRP).Value = clsCommon.myCdbl(dt.Rows(0)("Item_Basic_Net"))
            End If
          
            If clsCommon.myCdbl(gv1.CurrentRow.Cells(colRate).Value) <= 0 Then
                clsCommon.MyMessageBoxShow("Rate not found for UOM '" & gv1.CurrentRow.Cells(colUnit).Value & "' in item '" & gv1.CurrentRow.Cells(colICode).Value & "' in Price Code '" & lblPriceCode.Text & "' ")
            End If

            UpdateCurrentRow(gv1.CurrentRow.Index, True) ''-1 is for current row
            UpdateAllTotals()
        Else
            gv1.CurrentRow.Cells(colUnit).Value = gv1.CurrentRow.Cells(colOrgUnit).Value
        End If
        gv1.CurrentRow.Cells(colPendingQty).Value = clsSaleInvoiceDetailFreshSale.GetBalanceSRNQty(clsCommon.myCstr(gv1.CurrentRow.Cells(colOrderNo).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colDeliveryNo).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), clsCommon.myCstr(txtDocNo.Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colOrgUnit).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), clsCommon.myCdbl(gv1.CurrentRow.Cells(colMRP).Value))
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) > 0 Then
                If PurchaseOneItemOneVendor = True Then
                    ...
                    "where Customer_Code='" & txtVendorNo.Value & "' and TSPL_ITEM_MASTER.Active=1  and is_freshitem=1) as s pivot(max(cat_value) for description in (" + pivotheader + "))t  "
                Else
                    qry = "select * from (select a.DESCRIPTION,a.cat_value,TSPL_CUSTOMER_ITEM_DETAIL.item_no as Item,TSPL_CUSTOMER_ITEM_DETAIL.item_desc as [ItemDesc],TSPL_CUSTOMER_ITEM_DETAIL.uom as Unit , " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.item_rate as BasicRate,TSPL_CUSTOMER_ITEM_DETAIL.Item_MRP as MRP, " & _
                    "TSPL_CUSTOMER_ITEM_DETAIL.Start_Date as [Start Date], " & _
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("SRFS@It", qry)
    ...
    Sub OpenItemList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(txtReqNo.Value) = 0 Then
            ...
            whrCls = " inout='I' and Is_FreshItem=1  and Is_Serial_Item=0"
            gv1.CurrentRow.Cells(colICode).Value = clsCommon.ShowSelectForm("FSSaleReturnItemfnd", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), "Code", isButtonClick)
        End If
        'SetBlankOfItemColumns()
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) <= 0 Then
             ...
             "where TSPL_ITEM_MASTER.Active=1  and is_freshitem=1 )xxx Where  Tax_group='" & txtTaxGroup.Value & "' " & strPriceCondition & " " & _
             "" & strTaxRate & " " & _
    ...
    Private Sub UpdateAllTotals()
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            If (clsCommon.myLen(gv1.Rows(ii).Cells(colICode).Value) > 0) AndAlso (clsCommon.myCdbl(gv1.Rows(ii).Cells(ColFOC).Value) = 1 OrElse clsCommon.myCdbl(gv1.Rows(ii).Cells(colSampling).Value) = 1) Then '' clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colSampling).Value), "Yes") = CompairStringResult.Equal) Then  And clsCommon.myCdbl(gv1.Rows(ii).Cells(ColFOC).Value) = 0
    ...
    End Sub
    Private Function GetConvQty(ByVal IntRowNo As Integer, ByVal dblQty As Double) As Double
        Dim strItem As String = clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colICode).Value)
        Dim strOrgUnit As String = clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colOrgUnit).Value)
        Dim strCurrentUnit As String = clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colUnit).Value)
        ...
        If clsCommon.myLen(strOrgUnit) > 0 Then
            ...
            If dblCurrentConvF > 0 Then
                dblConvQty = Math.Round((dblQty / dblOrgConvF) * dblStockingUnitConvF * dblCurrentConvF, 2)
            End If
        End If
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            "where TSPL_RECEIPT_HEADER.Posted ='N' and tspl_sd_sale_invoice_head.Trans_Type='FS' and tspl_sd_sale_invoice_head.Screen_Type='' and TSPL_SD_SALE_INVOICE_HEAD.Document_Code='" & txtReqNo.Value & "' for xml path('')),'')  as DocNo ")
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    If clsCommon.myLen(strReqNo) > 0 Then
                        ...
                        If clsCommon.myCdbl(gv1.Rows(ii).Cells(ColFOC).Value) = 0 Then
                            'dblQty = GetConvQty(ii, dblQty)
                            'dblDamageQty = GetConvQty(ii, dblDamageQty)
            ...
            clsSaleInvoiceFreshSale.IsValidCustomer(arrReqNo, txtVendorNo.Value)
    ...
    Sub SaveData(ByVal ChekPostBtn As Boolean)
        Try
            ...
            If (AllowToSave()) Then
                ...
                Dim obj As New clsSalesReturnFreshSale()
                ...
                obj.Arr = New List(Of clsSalesReturnFreshSaleDetail)
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsSalesReturnFreshSaleDetail()
                    objTr.Crate = clsCommon.myCdbl(grow.Cells(colCrate).Value)
                    objTr.Sampling = clsCommon.myCdbl(grow.Cells(colSampling).Value)
                    ...
                    objTr.Delivery_Code = clsCommon.myCstr(grow.Cells(colDeliveryNo).Value)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsSalesReturnFreshSale()
            obj = clsSalesReturnFreshSale.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                fndGateEntryNo.Value = obj.Gate_Entry_No
                Dim isEnableDisable As Boolean = CheckInvoiceWithGateEntry(obj.Against_Invoice_No, obj.Gate_Entry_No)
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsSalesReturnFreshSaleDetail In obj.Arr
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colSampling).Value = objTr.Sampling
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colDeliveryNo).Value = objTr.Delivery_Code
                        ...
                        If clsCommon.myLen(objTr.Invoice_Code) > 0 Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsSaleInvoiceDetailFreshSale.GetBalanceSRNQty(objTr.Invoice_Code, objTr.Delivery_Code, objTr.Item_Code, obj.Document_Code, objTr.OrgUnit_code, objTr.Unit_code, objTr.MRP)
                        End If
    ...
    Private Sub gv2_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gv2.DoubleClick
        Try
            If rbtnTaxCalAutomatic.IsChecked Then
                ...
                Dim dblNewRate As Double = clsCommon.myCdbl(clsCommon.ShowSelectForm("FSSaleRetfnddxRate", qry, "Rate", "Tax_Code='" + clsCommon.myCstr(gv2.CurrentRow.Cells(colTTaxAutCode).Value) + "' and Tax_Type='S'", "", "", True))
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                End If
                ''
                If (clsSalesReturnFreshSale.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsSalesReturnFreshSale.DeleteData(txtDocNo.Value)) Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ...
            '-----------------------------------------------------
            Dim qst As String = "select count(*) from TSPL_SD_SALE_RETURN_HEAD where Document_Code='" + txtDocNo.Value + "' " + strcondition + " and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' "
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type='' and TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type=''"
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " TSPL_SD_SALE_RETURN_HEAD.Customer_Code in (" + strwherecls + ")  and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type=''"
        Else
            whrClas = " TSPL_SD_SALE_RETURN_HEAD.Trans_Type='FS' and TSPL_SD_SALE_RETURN_HEAD.Screen_Type=''"
        End If
        '-----------------------------------------------------

        LoadData(clsCommon.ShowSelectForm("FSSaleRetDocqry", qry, "Code", whrClas, txtDocNo.Value, "TSPL_SD_SALE_RETURN_HEAD.Document_Date desc", isButtonClicked, "TSPL_SD_SALE_RETURN_HEAD.Document_Date"), NavigatorType.Current)
    End Sub
    ...
    Private Sub txtTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTaxGroup._MYValidating
        ...
        txtTaxGroup.Value = clsCommon.ShowSelectForm("FSSaleretTaxGrp", qry, "Code", "Tax_Group_Type='S'", txtTaxGroup.Value, "Code", isButtonClicked)
        SetTaxDetails()
    ...
    Private Sub txtVendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtVendorNo._MYValidating
        ...
        End If
        '-----------------------------------------------------'
        txtVendorNo.Value = clsCommon.ShowSelectForm("FSSaleRetCustomer", qry, "Code", strwhrcondition, txtVendorNo.Value, "Code", isButtonClicked)
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBillToLocation._MYValidating
        ...
        txtBillToLocation.Value = clsCommon.ShowSelectForm("FSSaleRetBillLoc", qry, "Code", WhrCls, txtBillToLocation.Value, "Code", isButtonClicked)
        lblBillToLocation.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Location_Desc from TSPL_LOCATION_MASTER where Location_Code='" + txtBillToLocation.Value + "'"))
    ...
    Private Sub fndRouteNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtRouteNo._MYValidating
        ...
        txtRouteNo.Value = clsCommon.ShowSelectForm("FSSaleRetRoute", qry, "Code", "", txtRouteNo.Value, "", isButtonClicked)
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating
        ...
        txtShipToLocation.Value = clsCommon.ShowSelectForm("FSSaleRetShipLoc", qry, "Code", "", txtShipToLocation.Value, "Code", isButtonClicked)
    ...
    Sub SelectMRNItems()
        isInsideLoadData = True
        Dim frm As New frmPendingSaleInvoiceFS()
        frm.VendorCode = txtVendorNo.Value
        frm.strCurrCode = txtDocNo.Value
        frm.ShowDialog()
        LoadBlankGrid()
        Dim objOrderHead As clsSaleInvoiceFreshSale = Nothing
        If frm.ArrReturn IsNot Nothing AndAlso frm.ArrReturn.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn(0).Document_Code) > 0 Then
                objOrderHead = clsSaleInvoiceFreshSale.GetData(frm.ArrReturn(0).Document_Code, "", NavigatorType.Current)
                If objOrderHead IsNot Nothing AndAlso clsCommon.myLen(objOrderHead.Document_Code) > 0 Then
                    ...
                    End If
                    If (clsCommon.myLen(txtPriceCode.Text)) <= 0 Then
                        txtPriceCode.Text = clsDBFuncationality.getSingleValue("select price_CodeNon from TSPL_CUSTOMER_MASTER where Cust_Code='" + txtVendorNo.Value + "'")
                    End If
            ...
            For ii As Integer = 0 To frm.ArrReturn.Count - 1
                If clsCommon.myLen(frm.ArrReturn(ii).Document_Code) > 0 Then
                    ...
                    '    arr.Add(strCode)
                    objOrderHead = clsSaleInvoiceFreshSale.GetData(frm.ArrReturn(ii).Document_Code, "", NavigatorType.Current)
                    For Each obj As clsSaleInvoiceDetailFreshSale In objOrderHead.Arr
                        ...
                        If (obj.Item_Code = frm.ArrReturn(ii).Item_Code AndAlso obj.Delivery_Code = frm.ArrReturn(ii).Delivery_Code AndAlso obj.Unit_code = frm.ArrReturn(ii).Unit_code AndAlso obj.Scheme_Item = "N" AndAlso obj.Sampling = frm.ArrReturn(ii).Sampling) Then ''OrElse (obj.Scheme_Code = frm.ArrReturn(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            If IsValidItem(obj) Then
                                gv1.Rows.AddNew()
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colLineNo).Value = gv1.Rows.Count
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colRowType).Value = RowTypeItem
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colSampling).Value = obj.Sampling
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colCrate).Value = obj.Crate
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colOrderNo).Value = obj.Document_Code
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colDeliveryNo).Value = obj.Delivery_Code
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colCashSchemeCode).Value = obj.Cash_Scheme_Code
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colCashSchemeType).Value = obj.Cash_Scheme_Type
                                ...
                                lblPriceCode.Text = obj.Price_code
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsSaleInvoiceDetailFreshSale In objOrderHead.Arr
    ...
    Function IsValidItem(ByVal obj As clsSaleInvoiceDetailFreshSale)
        If clsCommon.myLen(txtTaxGroup.Value) <= 0 Then
            ...
            ElseIf gv1.CurrentColumn Is gv1.Columns(colComplete) AndAlso UsLock1.Status = ERPTransactionStatus.Approved Then
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 AndAlso clsCommon.myLen(strICode) > 0 AndAlso intSNo > 0 AndAlso clsCommon.CompairString(strStatus, "No") = CompairStringResult.Equal Then
                    If common.clsCommon.MyMessageBoxShow("Do you want to complete the item " + clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value), Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                        If clsSalesReturnFreshSaleDetail.CompleteSRN(txtDocNo.Value, strICode, intSNo) Then
    ...
    Public Sub funPrint(ByVal StrCode As String, Optional ByVal IsPDF As Boolean = False)
        ...
        Try
            ...
            Dim Qry As String = "  select TSPL_SD_SALE_RETURN_head.Gate_Entry_No,TSPL_SD_SALE_RETURN_DETAIL.Cash_Scheme_Amount  as Cash_Scheme_Amount,TSPL_SD_SALE_RETURN_DETAIL.Scheme_Item,isnull(TSPL_SD_SALE_RETURN_DETAIL.Sampling,0) as Sampling, TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER_fOR_Comp.City_Name)>0 then ', '+TSPL_CITY_MASTER_fOR_Comp.City_Name else ' ' end + case when len(TSPL_STATE_MASTER_For_Comp.STATE_NAME  )>0 then ', '+ TSPL_STATE_MASTER_For_Comp.STATE_NAME else ' ' end"
            ...
            Qry += " as Company_Address,  TSPL_LOCATION_MASTER.add1 +case when len(TSPL_LOCATION_MASTER.add2)>0 then ', '+TSPL_LOCATION_MASTER.add2 else '' end +case when LEN(isnull(TSPL_LOCATION_MASTER.Add3,''))>0 then ', '+isnull(TSPL_LOCATION_MASTER.Add3,'') else ' ' end "
            ...
            Qry += " as lOC_Address,TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER_fOR_Comp.City_Name)>0 then ', '+TSPL_CITY_MASTER_fOR_Comp.City_Name else ' ' end + case when len(TSPL_STATE_MASTER_For_Comp.STATE_NAME  )>0 then ', '+ TSPL_STATE_MASTER_For_Comp.STATE_NAME else ' ' end"
            Qry += "  + case when len(TSPL_COMPANY_MASTER.Pincode    )>0 then ', Pin Code - '+ cast(TSPL_COMPANY_MASTER.Pincode as varchar)  else ' ' end"
            Qry += "  + case when len(TSPL_COMPANY_MASTER.Tin_No     )>0 then ', Tin No - '+ cast(TSPL_COMPANY_MASTER.Tin_No as varchar)  else ' ' end"
            Qry += "  + case when len(TSPL_COMPANY_MASTER.CINNo      )>0 then ', CIN No - '+ cast(TSPL_COMPANY_MASTER.CINNo as varchar)  else ' ' end"
            Qry += "  + case when len(TSPL_COMPANY_MASTER.Fax     )>0 then ',Fax '+ TSPL_COMPANY_MASTER.Fax else '' end "
            ...
            Qry += "   '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate, TSPL_SD_SALE_RETURN_HEAD.Inv_No, TSPL_SD_SALE_RETURN_HEAD.Dept_Desc , TSPL_SD_SALE_RETURN_HEAD.Remarks ,  TSPL_SD_SALE_RETURN_HEAD.Terms_Code,TSPL_SD_SALE_RETURN_HEAD.VehicleNo , "
            ...
            Qry += " TSPL_SD_SALE_RETURN_DETAIL.item_code as item_code, TSPL_ITEM_MASTER.Item_Desc + case when TSPL_SD_SALE_RETURN_DETAIL.Scheme_Item='Y' then ' (Free Scheme)' when TSPL_SD_SALE_RETURN_DETAIL.Scheme_Item='N' then ''  end  +   case when isnull(TSPL_SD_SALE_RETURN_DETAIL.Sampling,0)='1' then ' (Sampling)' when isnull(TSPL_SD_SALE_RETURN_DETAIL.Sampling,0)='0' then '' end  as itemdesc, TSPL_SD_SALE_RETURN_DETAIL.Row_Type,TSPL_SD_SALE_RETURN_DETAIL.Qty as qty,TSPL_SD_SALE_RETURN_DETAIL.unit_code as uom,TSPL_SD_SALE_RETURN_DETAIL.item_cost as itemcost,TSPL_SD_SALE_RETURN_DETAIL.amount as amount,TSPL_SD_SALE_RETURN_HEAD.TAX1,TSPL_SD_SALE_RETURN_HEAD.TAX2,TSPL_SD_SALE_RETURN_HEAD.TAX3,TSPL_SD_SALE_RETURN_HEAD.TAX4,TSPL_SD_SALE_RETURN_HEAD.TAX5,TSPL_SD_SALE_RETURN_HEAD.Total_Add_Charge, "
          ...
          " where 2=2  and  TSPL_SD_SALE_RETURN_HEAD.Document_Code = '" + StrCode + "'"
            ...
            If e.Column.Index >= 0 Then
                ...
                ElseIf e.Column Is gv1.Columns(colQty) Then
                    ...
                    Else
                        ...
                        End If
                    End If
                ElseIf e.Column Is gv1.Columns(colDamageQty) Then
                    If clsCommon.CompairString(gv1.CurrentRow.Cells(colSchemeItem).Value, "Yes") = CompairStringResult.Equal Then
                        gv1.CurrentRow.Cells(colQty).ReadOnly = True
                    Else
                        gv1.CurrentRow.Cells(colQty).ReadOnly = False
    ...
    Private Sub findQtyandPromoSchemeCode(ByVal isButtonClick As Boolean, ByVal strSchemeCode As String)
        If blnSave = False Then
            ...
            Try
                ...
                If clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(Index).Cells(colSchmCodeType).Value), "None") <> CompairStringResult.Equal OrElse clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(Index).Cells(colSchmCodeType).Value), "") <> CompairStringResult.Equal Then
                    ...
                    If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colSchemeApplicable).Value), "Yes") = CompairStringResult.Equal Then
                        For schemeRow As Integer = gv1.Rows.Count - 1 To 0 Step -1
                            ...
                            End If
                        Next
                        Dim objD As clsSchemeApplyOnDairy = clsSchemeApplyOnDairy.GetSchemeData(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colQty).Value), txtVendorNo.Value, clsCommon.myCstr(gv1.CurrentRow.Cells(colSchmCodeType).Value), strSchemeCode)
                        If objD IsNot Nothing AndAlso objD.Arr.Count > 0 Then
                            For Each objtr As clsSchemeApplyOnDairy In objD.Arr
                                ...
                                Dim MainDeliveryCode As String = gv1.Rows(Index).Cells(colDeliveryNo).Value
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colOrgUnit).Value = objtr.Schm_Item_Uom
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colDeliveryNo).Value = MainDeliveryCode
                ...
                "TSPL_ITEM_PRICE_MASTER.Price_Code='" & txtPriceCode.Text & "' and UOM='" & objtr.Schm_Item_Uom & "' and  " & _
                "TSPL_ITEM_PRICE_MASTER.item_code='" & objtr.Schm_Icode & "' AND Location_Code='" & clsCommon.myCstr(txtBillToLocation.Value) & "'" & _
                ") XXXE WHERE RowNo=1  "
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer, ByVal CheckConFactor As Boolean)
        Try
            ...
            If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(ColFOC).Value) = 0 Then
                Dim strSchemeCode As String = clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colCashSchemeCode).Value)
                Dim obj_Cash As clsSchemeApplyOnDairy = clsSchemeApplyOnDairy.GetPriceSchemeData(clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colICode).Value), clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colUnit).Value), clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colQty).Value), txtVendorNo.Value, strSchemeCode)
            ...
            If clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(ColFOC).Value) = 0 AndAlso clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colSampling).Value) = 0 Then
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("FSSaleRetSalesman", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
...
#Region "Email SMS Setting"
    Private Sub RadMenuItem5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem5.Click
        ...
        frm.FormId = clsUserMgtCode.frmSaleReturnFreshSale
        frm.ShowDialog()
    End Sub
    ...
    Private Sub SendSMSandEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        Try
            ...
            Dim strContactPerson As String = ""
            Dim dtContent As DataTable = clsDBFuncationality.GetDataTable("SELECT SMS_Text,Email_Text,Email_subject from TSPL_ES_Content where Form_ID='" + clsUserMgtCode.frmSaleReturnFreshSale + "'", Nothing)
            ...
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                If clsCommon.myLen(dtContent.Rows(0)("Email_Text")) > 0 Then
                    ...
                    objEmailH.Email_Text = objEmailH.Email_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSaleReturnFreshSale)
                    ...
                    objEmailH.SaveData(clsUserMgtCode.frmSaleReturnFreshSale, objEmailH, Nothing)
                    objEmailH = Nothing
        ...
        Try
            ...
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                If clsCommon.myLen(dtContent.Rows(0)("SMS_Text")) > 0 Then
                    ...
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.Form_Code, clsUserMgtCode.frmSaleReturnFreshSale)


                    objSMSH.SaveData(clsUserMgtCode.frmSaleReturnFreshSale, objSMSH, Nothing)
                    objSMSH = Nothing
                    If Not isPost Then
...
#End Region
    ...
    Private Sub btnReverseAndUnpost_Click(sender As Object, e As EventArgs) Handles btnReverseAndUnpost.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsSalesReturnFreshSale.ReverseAndUnpost(txtDocNo.Value) Then
    ...
    Private Sub fndGateEntryNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndGateEntryNo._MYValidating
        ...
        WhrCls = " Doc_Type = 'FS' and POSTED = 1  and Gate_Entry_No not in ( select case when TSPL_SD_SALE_RETURN_HEAD.Gate_Entry_No is Null then '' else TSPL_SD_SALE_RETURN_HEAD.Gate_Entry_No end  from TSPL_SD_SALE_RETURN_HEAD ) "  'and POSTED = 1
        If clsCommon.myLen(txtVendorNo.Value) > 0 Then
