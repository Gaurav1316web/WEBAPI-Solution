...
'========shivani Tyagi=========='
Public Class RptFreshSaleRegister1
    Inherits FrmMainTranScreen
    Dim isInsideLoadData As Boolean = False
    Dim atchqry As String = ""
#Region "Functions"
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.RptFreshSaleRegister1)
...
#End Region
    ...
    Private Sub RptFreshSaleRegister1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        Reset()
    ...
    Sub Print(ByVal IsPrint As Exporter)
        Try
            ...
            strSaleInvoice = "SELECT TSPL_VEHICLE_MASTER.Number as Vehicle_Name ,TSPL_SD_SALE_INVOICE_HEAD.VehicleNo,TSPL_ZONE_MASTER.Zone_Code,TSPL_ZONE_MASTER.Description as Zone_Name, TSPL_SD_SALE_INVOICE_DETAIL.Conv_Factor as OldConvF,OrgRate,'" & fromDate.Value & "' as Fdate,'" & ToDate.Value & "' as Tdate, case when TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='FS' then 'Sale Invoice' end as Type, TSPL_SD_SALE_INVOICE_HEAD.Document_Code, " & _
                        ...
                        "(case when TSPL_SD_SALE_INVOICE_HEAD.ConvRate=0 then 1 else TSPL_SD_SALE_INVOICE_HEAD.ConvRate end)*TSPL_SD_SALE_INVOICE_DETAIL.Item_Net_Amt as Total_BaseCurr,FOC_Item,TSPL_SD_SALE_INVOICE_DETAIL.Sampling ,TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code,Cust_Group_Desc,sku_seq,Delivery_Code,Shipment_Code ,TSPL_DELIVERY_NOTE_MASTER_FRESHSALE.Booking_No FROM TSPL_CUSTOMER_MASTER RIGHT OUTER JOIN " & _
                        ...
                        "left outer join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_Id =TSPL_SD_SALE_INVOICE_HEAD.Vehicle_Code " & _
                        ...
                        " where TSPL_SD_SALE_INVOICE_HEAD.Trans_Type ='FS' and " & strInvoicePosted & " TSPL_SD_SALE_INVOICE_HEAD.Document_Date  >= '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(fromDate.Value), "dd/MMM/yyyy hh:mm tt") & "' and " & _
            ...
            strSaleReturn = "SELECT   '' as Vehicle_Name,'' as VehicleNo,'' as Zone_Code,'' as Zone_Name,  TSPL_SD_SALE_RETURN_DETAIL.Conv_Factor as OldConvF,TSPL_SD_SALE_RETURN_DETAIL.OrgRate,'" & fromDate.Value & "' as Fdate,'" & ToDate.Value & "' as Tdate, 'Sale Return' as Type, TSPL_SD_SALE_RETURN_HEAD.Document_Code, " & _
                            ...
                            "TSPL_SD_SALE_RETURN_DETAIL.Item_Cost as rate,(-1 * TSPL_SD_SALE_RETURN_DETAIL.Amount) as Amount, (-1 * TSPL_SD_SALE_RETURN_DETAIL.Disc_Per) as Disc_Per,TSPL_SD_SALE_RETURN_DETAIL.Disc_Amt, (-1* TSPL_SD_SALE_RETURN_DETAIL.Amt_Less_Discount*(case when TSPL_SD_SALE_RETURN_DETAIL.Total_Tax_Amt=0 then 0 else 1 end)) as Amt_Less_Discount,  " & _
                            ...
                            " (case when TSPL_SD_SALE_RETURN_HEAD.ConvRate=0 then 1 else TSPL_SD_SALE_RETURN_HEAD.ConvRate end)*TSPL_SD_SALE_RETURN_DETAIL.Item_Net_Amt  * -1 as Total_BaseCurr,TSPL_SD_SALE_RETURN_DETAIL.FOC_Item,TSPL_SD_SALE_RETURN_DETAIL.Sampling,TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code ,Cust_Group_Desc,sku_seq,TSPL_SD_SALE_RETURN_DETAIL.Delivery_Code,Against_Shipment_No as Shipment_Code,TSPL_DELIVERY_NOTE_MASTER_FRESHSALE.Booking_No FROM  " & _
                            ...
                            "  where TSPL_SD_SALE_RETURN_HEAD.Trans_Type ='FS' and " & strReturnPosted & "  TSPL_SD_SALE_RETURN_HEAD.Document_Date  >= '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(fromDate.Value), "dd/MMM/yyyy hh:mm tt") & "' and  " & _
            ...
            If clsCommon.myLen(txtUOM.Value) > 0 Then
                str = "select Fdate, Tdate,Type, Document_Code, Document_Date, Vehicle_Name, VehicleNo, Zone_Code, Zone_Name, Customer_Code, Customer_Name,FOC_Item,Sampling,Parent_Customer_No,address,city_name,state_name,telephone, Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc ,xxxx.Item_Code, Item_Desc,itf_code,CONVERT(decimal(18,2), InvoiceQty) as InvoiceQty, ShippedQty, '" + txtUOM.Value + "' as Unit_code, rate,Amount,Disc_Per,Disc_Amt, Amt_Less_Discount,Total_Tax_Amt,TotalAmt,currency_code,convrate,total_basecurr,Cust_Group_Code,Cust_Group_Desc,sku_seq,Delivery_Code,Shipment_Code ,Booking_No  from ( select  Fdate, Tdate,Type, Document_Code, Document_Date, Customer_Code, Customer_Name,Parent_Customer_No,address,city_name,state_name,telephone, Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc ,xxx.Item_Code, Item_Desc,itf_code, (InvoiceQty*OldConvF/TSPL_ITEM_UOM_DETAIL.Conversion_Factor) as InvoiceQty, ShippedQty,TSPL_ITEM_UOM_DETAIL.UOM_code as Unit_code,InvoiceQty*OrgRate as rate,Amount,Disc_Per,Disc_Amt, Amt_Less_Discount,Total_Tax_Amt,TotalAmt,sku_seq,Delivery_Code,Shipment_Code ,Booking_No ,currency_code,convrate,total_basecurr,FOC_Item,Sampling,Cust_Group_Code,Cust_Group_Desc,TSPL_ITEM_UOM_DETAILForDivide.Conversion_Factor as DivideConverstionFactor,Vehicle_Name,VehicleNo,Zone_Code, Zone_Name  from(" + str + " ) xxx  Inner join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=xxx.Item_Code  and TSPL_ITEM_UOM_DETAIL.UOM_Code='" + txtUOM.Value + "'  left outer join TSPL_ITEM_UOM_DETAIL as TSPL_ITEM_UOM_DETAILForDivide on TSPL_ITEM_UOM_DETAILForDivide.Item_Code=xxx.Item_Code and TSPL_ITEM_UOM_DETAILForDivide.UOM_Code='" + txtUOM.Value + "'  )xxxx where xxxx.unit_code='" + txtUOM.Value + "' and DivideConverstionFactor is not null"
            End If
            Dim qry As String

            qry = "select Vehicle_Name,VehicleNo,Zone_Code,Zone_Name, fdate,tdate,type,document_code,document_date,Parent_Customer_No,customer_code,customer_name,address,city_name,state_name,telephone,salesman_code,salesman_name,bill_to_location,location_desc,axa.item_code,axa.item_desc,axa.itf_code,invoiceqty,shippedqty,axa.unit_code,axa.rate,amount,disc_per,disc_amt,amt_less_discount,total_tax_amt,totalamt,currency_code,convrate,total_basecurr,FOC_Item,Sampling,case when FOC_Item='1' then  '0'   when Sampling='1' then '0' else TotalAmt end as TotalAmtNew,axa.Cust_Group_Code,axa.Cust_Group_Desc,axa.sku_seq,Delivery_Code,Shipment_Code ,Booking_No  from (" + str + ")axa left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.item_code=axa.item_code) as ada"

            If rdbSummary.IsChecked Then
                qry = "select  Type, Document_Code,convert(varchar,Document_Date,103)as Document_Date ,max(ada1 .Vehicle_Name) as Vehicle_Name," & _
                      " max(ada1.VehicleNo) as VehicleNo,max(ada1.Zone_Code) as Zone_Code,max(ada1.Zone_Name) as Zone_Name ,MAX(ada1.Parent_Customer_No) as Parent_Customer_No, MAX(Parent_Master.Customer_Name) as ParentName,Customer_Code,ada1.Customer_Name,ada1.Cust_Group_Code,ada1.Cust_Group_Desc,address,city_name,state_name,telephone,ada1.Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc,sum(Amount) as Amount, " & _
                "sum(Disc_Amt) as Disc_Amt,sum(Amt_Less_Discount) as Amt_Less_Discount,sum(Total_Tax_Amt) as Total_Tax_Amt,sum(TotalAmt) as TotalAmt,convert(decimal(18,2),SUM(Total_BaseCurr)) as Total_BaseCurr,sum(TotalAmtNew)as TotalAmtNew,max(sku_seq)as sku_seq,Delivery_Code,Shipment_Code ,Booking_No  " & _
                "from (select ada.* from( " & qry & ") ada1 LEFT OUTER JOIN TSPL_CUSTOMER_MASTER as Parent_Master ON Parent_Master.Cust_Code=ada1.Parent_Customer_No group by Type, Document_Code,Document_Date,Customer_Code,ada1.Customer_Name,ada1.Cust_Group_Code,ada1.Cust_Group_Desc,address,city_name,state_name,telephone,ada1.Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc ,Delivery_Code ,Booking_No ,Shipment_Code "
            ElseIf rdbDetail.IsChecked = True Then
                qry = "select   Type, Fdate,Tdate,Document_Code,convert(varchar,Document_Date,103)as Document_Date ,ada1.Vehicle_Name,ada1.VehicleNo,ada1.Zone_Code,ada1.Zone_Name,(ada1.Parent_Customer_No) as Parent_Customer_No, (Parent_Master.Customer_Name) as ParentName,Customer_Code,ada1.Customer_Name,ada1.Cust_Group_Code,ada1.Cust_Group_Desc,address,city_name,state_name,telephone,ada1.Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc,Item_Code,Item_Desc,cast('ITF code-'+ (itf_code) as varchar)as Itf_code,(InvoiceQty) as InvoiceQty,(ShippedQty) as ShippedQty,Unit_Code,Rate,(Amount) as Amount, Disc_Per,(Disc_Amt) as Disc_Amt,(Amt_Less_Discount) as Amt_Less_Discount,(Total_Tax_Amt) as Total_Tax_Amt,(TotalAmt) as TotalAmt,ada1.Currency_Code,ConvRate,convert(decimal(18,2),(Total_BaseCurr) )as Total_BaseCurr ,FOC_Item,Sampling,TotalAmtNew ,sku_seq,Delivery_Code,Shipment_Code ,Booking_No  " & _
            ...
            If chkSerializeInv.Checked Then
                qry = "select final.Type, final.Fdate, final.Tdate, final.Document_Code, final.Document_Date,final.Customer_Code,final.Customer_Name,final.Cust_Group_Code,final.Cust_Group_Desc,final.Parent_Customer_No,final.ParentName,final.Address, final.city_name, final.STATE_NAME,final.telephone, final.Salesman_Code, final.Salesman_Name,final.Bill_To_Location,final.Location_Desc,TSPL_SERIAL_ITEM.Auto_Sr_No [Item Serial No],final.Item_Code, final.Item_Desc, final.Unit_code,final.TotalAmt,Final.FOC_Item,Final.Sampling ,final.TotalAmtNew,final.sku_seq,final.Delivery_Code,final.Shipment_Code ,final.Booking_No  from ( " + qry + ") as final left outer join TSPL_SD_SALE_INVOICE_HEAD on TSPL_SD_SALE_INVOICE_HEAD.Document_Code = final.Document_Code left outer join TSPL_SERIAL_ITEM on TSPL_SERIAL_ITEM.Item_Code= final.Item_Code and TSPL_SERIAL_ITEM.Document_Code= TSPL_SD_SALE_INVOICE_HEAD.Against_Shipment_No "

            End If
            qry += " order by convert(date,Document_Date,103), Document_Code,sku_seq "
    ...
    Sub SetGridFormationOFGV1()
        ...
        If rdbDetail.IsChecked = True Then
            ...
            Gv1.Columns("Delivery_Code").IsVisible = True
            Gv1.Columns("Delivery_Code").Width = 100
            Gv1.Columns("Delivery_Code").HeaderText = "Delivery Code"

            Gv1.Columns("Shipment_Code").IsVisible = True
            Gv1.Columns("Shipment_Code").Width = 100
            Gv1.Columns("Shipment_Code").HeaderText = "Shippment Code"

            Gv1.Columns("Booking_No").IsVisible = True
            Gv1.Columns("Booking_No").Width = 100
            Gv1.Columns("Booking_No").HeaderText = "Booking No"
            ...
            Gv1.Columns("Vehicle_Name").HeaderText = "Vehicle Name"

            Gv1.Columns("VehicleNo").IsVisible = False
            Gv1.Columns("VehicleNo").Width = 100
            Gv1.Columns("VehicleNo").HeaderText = "Vehicle No"

            Gv1.Columns("Zone_Name").IsVisible = True
            Gv1.Columns("Zone_Name").Width = 100
            Gv1.Columns("Zone_Name").HeaderText = "Zone Name"
            Gv1.Columns("Zone_Code").IsVisible = False
            Gv1.Columns("Zone_Code").Width = 100
            ...
            Gv1.Columns("Sampling").Width = 80
            Gv1.Columns("Sampling").HeaderText = "Sample Item"
        ...
        If rdbSummary.IsChecked Then
            If Gv1.Rows.Count > 0 Then
                ...
                If strTransType = "Sale Invoice" Then
                    strTransType = "SD-IN"
                ElseIf clsCommon.CompairString(strTransType, "Fresh Sale") = CompairStringResult.Equal Then
                    strTransType = "Fresh-Sale"
                Else
                    strTransType = "Sale Return"
                End If
                Select Case strTransType
                    Case "SD-IN"
                        clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmInvoiceFreshSale, strDoc)
                    Case "Fresh_Sale"
                        clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmInvoiceFreshSale, strDoc)
                    Case "Sale Return"
    ...
    Private Sub RadMenuItem1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnSaveLayout.Click
        If clsCommon.myLen(MyBase.Form_ID) > 0 Then
    ...
    Private Sub btnQuickExport_Click(sender As Object, e As EventArgs) Handles btnQuickExport.Click
        Try
            ...
            arrHeader.Add("Company : " + objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptFreshSaleRegister1 & "'"))
            If rbtnLocationSelect.IsChecked Then
                Dim strLocationName As String = ""
                For Each StrName As String In cbgLocation.CheckedDisplayMember
                    If clsCommon.myLen(strLocationName) > 0 Then
