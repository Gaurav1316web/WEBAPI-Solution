...
'-Updation By [Pankaj Kumar Chaudhary]--Against Ticket No--[BM00000003165 14/07/2014]
''updation by richa agarwal against ticket no BM00000003745(add condition in customer finder),BM00000006347,BM00000006364,BM00000006386,BM00000006648,BM00000006655,BM00000006589,BM00000006733,BM00000006873,BM00000007399
'-BM00000003441
'=============BM00000003471-
Imports common
...
Imports System.Net
Imports System.IO
Public Class frmEXCommercialInvoice
...
#Region "Variables"
    ...
    Const ReportID As String = "EXCOMMINVOICE"
    ...
    '============added by shivani==============================>
    Const colTotGrossItemWt As String = "colTotGrossItemWt"
    Const colWeighmentUnit As String = "colWeighmentUnit"
    '================================================================
...
#End Region
    ...
    Private Sub LOCATIONRIGTHS()
        ...
        Try
            ...
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Default_LocCode) > 0 Then
                ...
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXCommercialInvoice) = CompairStringResult.Equal Then
                    WhrCls = " and Location_Type='Physical'  "
                ElseIf clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
    ...
    Private Sub LoadTerms()
        Dim qry As String = "select '' as Code,'None' as Name union all select 'CIF' as Code,'Cost, Insurance & Freight' as Name union all select 'CFR' as Code,'Cost & Freight' as Name union all select 'FOB' as Code,'Free on Board' as Name union all select 'C&F' as Code,'Cost & Freight' as Name union all select 'EPH' as Code,'EX-Works, Palwal-Haryana' as Name"
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    ...
    Private Sub FrmAPInvoiceEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                RadGroupBox1.Visible = True
                TxtMTPONo.Visible = True
                TxtMTPOdate.Visible = True
                MyLabel39.Visible = True
                MyLabel40.Visible = True
            Else
                RadGroupBox1.Visible = False
                TxtMTPONo.Visible = False
                TxtMTPOdate.Visible = False
                MyLabel39.Visible = False
                MyLabel40.Visible = False
    ...
    Sub SetMultiCurrencyVisibility()
        Try
            ...
            Else
                ...
                Throw New Exception("Do multicurrency on for Export Commercial Invoice.")
            End If
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    ...
    Sub BlankAllControls()
        ...
        txtIncentiveDecalration.Text = ""
        txtIncentiveDecalration.Text = clsEXCommercialInvoiceHead.GetProformaIncentiveDeclaration()
        txtBankCode.Value = ""
        txtbankName.Text = ""
        fndBankState.Value = ""
        txtbankState.Text = ""
        ...
        txtDesc.Text = ""
        txtInvNo.Text = ""
        txtComment.Text = clsEXCommercialInvoiceHead.GetProformaDescrptn()
        txtTaxGroup.Value = ""
        lblTaxGrpName.Text = ""
        txtTermCode.Value = ""
        lblTermName.Text = ""
        ...
        txtVehicleNo.Text = ""
        txtVehcileCode.Text = ""
        txtIm_Ex_No.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select isnull(IECode,'') from TSPL_COMPANY_MASTER where Comp_Code ='" & objCommonVar.CurrentCompanyCode & "'"))
        txtIm_Ex_No.Enabled = False
        txtGENo.Text = ""
        ...
        TxtFinalGrossWeight.Value = 0
        TxtMTPONo.Text = ""
        TxtMTPONo.Enabled = True
        TxtMTPOdate.Enabled = False
        TxtMTPOdate.Checked = False
        TxtMTPOdate.Value = clsCommon.GETSERVERDATE()
        rdbAgainstWithLC.IsChecked = True
        lbldocumentAcceptance.Text = ""
        RadPageView1.SelectedPage = RadPageViewPage1
        txtDesc.Focus()
        txtDesc.Select()
    ...
    Sub LoadBlankGrid()
        ...
        Dim repoTotGrossItemWt As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoTotGrossItemWt = New GridViewDecimalColumn()
        repoTotGrossItemWt.FormatString = ""
        repoTotGrossItemWt.HeaderText = "Tot Gross Item Weight"
        repoTotGrossItemWt.Name = colTotGrossItemWt
        repoTotGrossItemWt.Width = 80
        repoTotGrossItemWt.Minimum = 0
        repoTotGrossItemWt.ReadOnly = False
        repoTotGrossItemWt.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gv1.MasterTemplate.Columns.Add(repoTotGrossItemWt)

        Dim repoWeighmentUnit As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoWeighmentUnit.FormatString = ""
        repoWeighmentUnit.HeaderText = "Weighment UOM"
        repoWeighmentUnit.Name = colWeighmentUnit
        repoWeighmentUnit.Width = 80
        repoWeighmentUnit.ReadOnly = False
        repoWeighmentUnit.HeaderImage = Global.ERP.My.Resources.Resources.search4
        repoWeighmentUnit.TextImageRelation = TextImageRelation.TextBeforeImage
        gv1.MasterTemplate.Columns.Add(repoWeighmentUnit)
        ...
        'repoRequition.HeaderText = "Export PI No"
        ...
        Dim repoPO As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoPO.FormatString = ""
        'repoRequition.HeaderText = "Export PI No"
        repoPO.HeaderText = "PO No"
        repoPO.Name = colPONo
        repoPO.ReadOnly = True
        repoPO.Width = 100
        repoPO.IsVisible = False
        gv1.MasterTemplate.Columns.Add(repoPO)
            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colTotTaxAmt) Then
                        ...
                        UpdateAllTotals()
                    End If
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colSchemeApplicable) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colTotGrossItemWt) OrElse e.Column Is gv1.Columns(colWeighmentUnit) OrElse e.Column Is gv1.Columns(colUnit) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) Then
                        ...
                        ElseIf e.Column Is gv1.Columns(colUnit) Then
                            ...
                            UpdateAllTotals()
                            '===============shivani===============
                        ElseIf e.Column Is gv1.Columns(colWeighmentUnit) Then
                            OpenWeighmentUOMList(False)
                            '============================
                        ElseIf e.Column Is gv1.Columns(colMRP) Then
                            ...
                            'End If
                        ElseIf e.Column Is gv1.Columns(colTotGrossItemWt) Then
                            UpdateCurrentRow(gv1.CurrentRow.Index)
                            UpdateAllTotals()
    ...
    Sub OpenUOMList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            ...
            gv1.CurrentRow.Cells(colUnit).Value = clsCommon.ShowSelectForm("EXCOMUNTFND", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), "Code", isButtonClick)
    ...
    End Sub
    '===============added by shivani
    Sub OpenWeighmentUOMList(ByVal isButtonClick As Boolean)
        Dim strICode As String = clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value)
        If clsCommon.myLen(strICode) > 0 Then
            Dim qry As String = "select UOM_Code as Code,UOM_Description as [Description] from TSPL_ITEM_UOM_DETAIL"
            Dim whrCls As String = "Item_Code='" + strICode + "'"
            gv1.CurrentRow.Cells(colWeighmentUnit).Value = clsCommon.ShowSelectForm("EXCOMUNTFND", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colWeighmentUnit).Value), "Code", isButtonClick)
        End If
    End Sub
        ...
        Try
            If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colSchemeApplicable).Value), "No") = CompairStringResult.Equal Then
                For schemeRow As Integer = gv1.Rows.Count - 1 To 0 Step -1
                    ...
                    End If
                Next
                'gv1.CurrentRow.Cells(schemeCodeCol).Value = String.Empty
             ...
             "TSPL_SCHEME_MASTER_NEW.MRP='" & clsCommon.myCdbl(gv1.CurrentRow.Cells(colMRP).Value) & "' AND TSPL_SCHEME_BENEFICIARY.Cust_Code  = '" & txtVendorNo.Value & "' order by Start_Date desc"
                ...
                If dr1 IsNot Nothing AndAlso dr1.Rows.Count > 0 Then
                    ...
                    If discountRatio > 0 Then
                        ...
                        '"(TSPL_SCHEME_MASTER_NEW.Scheme_Code = '" & schemeCodeCol & "')"
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(icode) > 0 Then
                ...
                gv1.CurrentRow.Cells(colRowType).Value = RowTypeItem
                gv1.CurrentRow.Cells(colICode).Value = icode
                ...
                '============shivani===========
                gv1.CurrentRow.Cells(colWeighmentUnit).Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select unit_code from tspl_item_master where item_code='" + icode + "'")) 'clsCommon.myCstr(dr("Unit"))
                '=============================================================
                gv1.CurrentRow.Cells(ColActualBalQty).Value = clsItemLocationDetails.getBalance(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), txtBillToLocation.Value, txtDocNo.Value, txtDate.Value, Nothing, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), 0) 'clsCommon.myCstr(dr("MRP"))
    ...
    Private Sub UpdateAllTotals()
        ...
        Dim dblTotalGrossWeight As Double = 0
        '==================shivani
        Dim dblTotalGrossItemWeight As Double = 0
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            If (clsCommon.myLen(gv1.Rows(ii).Cells(colICode).Value) > 0) AndAlso clsCommon.myCdbl(gv1.Rows(ii).Cells(ColFOC).Value) = 0 Then
                ...
                dblTotalGrossWeight = dblTotalGrossWeight + clsCommon.myCdbl(gv1.Rows(ii).Cells(colTotItemWt).Value)
                dblTotalGrossItemWeight = dblTotalGrossItemWeight + clsCommon.myCdbl(gv1.Rows(ii).Cells(colTotGrossItemWt).Value)
        ...
        txtGrossWeight.Value = dblTotalGrossWeight
        ' TxtFinalGrossWeight.Value = dblTotalGrossItemWeight
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If clsCommon.CompairString(cmbDocType.SelectedValue, "") = CompairStringResult.Equal Then
                common.clsCommon.MyMessageBoxShow(Me, "Select document type.", Me.Text)
                RadPageView1.SelectedPage = RadPageViewPage1
                cmbDocType.Focus()
                cmbDocType.Select()
            ...
            If clsCommon.CompairString(cboItemType.SelectedValue, "") = CompairStringResult.Equal Then
                common.clsCommon.MyMessageBoxShow(Me, "Select item type.", Me.Text)
                RadPageView1.SelectedPage = RadPageViewPage1
                cboItemType.Focus()
                cboItemType.Select()
            ...
            If Not isNewEntry AndAlso clsCommon.myLen(txtDocNo.Value) <= 0 Then
                ...
                txtDocNo.Focus()
                txtDocNo.Select()
                Errorcontrol.SetError(txtDocNo, "Invoice No. not found to save.")
                Return False
            Else
            ...
            '        RadPageView1.SelectedPage = RadPageViewPage1
            '        cboItemType.Focus()
            '        Errorcontrol.SetError(cboItemType, "Select invoice type for creating invoice.")
            '        Return False
            '    Else
            '        Errorcontrol.ResetError(cboItemType)
            '    End If
            ...
            ''richa agarwal 26/04/2015
            Dim arrPurchaseNo As New List(Of String)
            ''----------------------
            For ii As Integer = 0 To gv1.Rows.Count - 1
                RadPageView1.SelectedPage = RadPageViewPage1
                gv1.Focus()
                ...
                ''richa agarwal 26/04/2015
                Dim strPurchaseNo As String = clsCommon.myCstr(gv1.Rows(ii).Cells(colPONo).Value)
                ''----------------------
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    If clsCommon.CompairString(cboItemType.SelectedValue, strItemType) <> CompairStringResult.Equal Then
                        clsCommon.MyMessageBoxShow(Me, "" + strIName + " item is not of selected type i.e. " + cboItemType.SelectedValue + " at row no. " + clsCommon.myCstr(ii + 1) + "")
                    ...
                    End If
                    ''richa agarwal 26/04/2015
                    If clsCommon.myLen(strPurchaseNo) > 0 Then
                        If Not (arrPurchaseNo.Contains(strPurchaseNo)) Then
                            arrPurchaseNo.Add(strPurchaseNo)
                        End If
                        If dblQty > dblPendingQty Then
                            RadPageView1.SelectedPage = RadPageViewPage1
                            common.clsCommon.MyMessageBoxShow(Me, "Item " + strICode + "( " + strIName.Trim() + " ) Entered Quantity(" + clsCommon.myCstr(dblQty) + ") Cannot be more than Pending Quantity(" + clsCommon.myCstr(dblPendingQty) + ").At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                    ...
                    If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(strSchemeItem, "No") = CompairStringResult.Equal Then
                        ...
                        For jj As Integer = ii + 1 To gv1.Rows.Count - 1
                            ...
                            If clsCommon.CompairString(strICode, strInICode) = CompairStringResult.Equal Then
                                ...
                                gv1.CurrentColumn = gv1.Columns(colICode)
                                common.clsCommon.MyMessageBoxShow(Me, "Item Code " + strICode + " is repeted at Row No" + clsCommon.myCstr(ii + 1) + " and " + clsCommon.myCstr(jj + 1), Me.Text)
                                Return False
                            End If
                        Next
                    End If
            ...
            If clsCommon.myLen(txtBankCode.Value) > 0 AndAlso clsCommon.myLen(txtBankBranchCode.Value) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage5
            ...
            clsEXPorformaInvoiceHead.IsValidCustomer(arrReqNo, txtVendorNo.Value)
            clsEXPorformaInvoiceHead.IsValidDocumentType(arrReqNo, cmbDocType.SelectedValue)
    ...
    Sub SaveData(ByVal ChekPostBtn As Boolean)
        Dim obj As New clsEXCommercialInvoiceHead()
        Try
            If (AllowToSave()) Then

                obj = New clsEXCommercialInvoiceHead()
                obj.Document_Type = clsCommon.myCstr(cmbDocType.SelectedValue)
                obj.Incentive_Declaration = clsCommon.myCstr(txtIncentiveDecalration.Text).Replace("'", "`")
                ...
                ''richa agarwal 21/04/2015
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                    If clsCommon.myLen(TxtMTPONo.Value) > 0 Then
                        obj.MT_Against_PO = clsCommon.myCstr(TxtMTPONo.Value)
                    End If

                    If TxtMTPOdate.Checked Then
                        obj.MT_Against_PO_Date = clsCommon.myCDate(TxtMTPOdate.Value)
                    End If
                    If rdbAgainstWithLC.IsChecked Then
                        obj.MT_Against_LC = 1
                    End If
                    If clsCommon.myLen(lbldocumentAcceptance.Text) > 0 Then
                        obj.DocumentAcceptanceNo = clsCommon.myCstr(lbldocumentAcceptance.Text)
                    End If
                End If
                obj.Gross_Wt = clsCommon.myCdbl(txtGrossWeight.Value)
                obj.Final_Gross_Wt = clsCommon.myCdbl(TxtFinalGrossWeight.Text)
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsEXCommercialInvoiceDetail()
                    ...
                    'CInt(IIf(clsCommon.myCBool(grow.Cells(colShippingMark).Value) = True, 1, 0))
                    ...
                    ''richa agarwal 26/04/2015
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colOrderNo).Value)) > 0 Then
                        objTr.Shipment_Code = clsCommon.myCstr(grow.Cells(colOrderNo).Value)
                    Else
                        objTr.MT_Against_PO = clsCommon.myCstr(grow.Cells(colPONo).Value)
                    End If
                    ...
                    objTr.Weight_UOM = clsCommon.myCstr(grow.Cells(colWeighmentUnit).Value)
                    objTr.Gross_Item_Weight = clsCommon.myCdbl(grow.Cells(colTotGrossItemWt).Value)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Try
            ...
            obj = clsEXCommercialInvoiceHead.GetData(strCode, "'T','R'", NavTyep, arrLoc, IIf(clsCommon.CompairString(clsUserMgtCode.frmCommercialInvoiceMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                ''richa agarwal 30/06/2015 BM00000007233
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                    txtDate.Enabled = True
                Else
                ...
                txtIncentiveDecalration.Text = obj.Incentive_Declaration
                ...
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                    If clsCommon.myLen(obj.MT_Against_PO) > 0 Then
                        TxtMTPONo.Value = clsCommon.myCstr(obj.MT_Against_PO)
                        If obj.MT_Against_PO_Date.HasValue Then
                            TxtMTPOdate.Value = clsCommon.myCDate(obj.MT_Against_PO_Date)
                            TxtMTPOdate.Checked = True
                        End If
                    End If
                    If clsCommon.CompairString(obj.MT_Against_LC, "1") = CompairStringResult.Equal Then
                        rdbAgainstWithLC.IsChecked = True
                    Else
                        rdbAgainstWithoutLC.IsChecked = True
                    End If
                    lbldocumentAcceptance.Text = obj.DocumentAcceptanceNo
                End If
                txtGrossWeight.Value = obj.Gross_Wt
                TxtFinalGrossWeight.Value = obj.Final_Gross_Wt
                ''-------------------------
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsEXCommercialInvoiceDetail In obj.Arr
                        gv1.Rows.AddNew()
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colShippingMark).Value = clsCommon.myCstr(objTr.Shipping_Mark) 'clsCommon.myCBool(IIf(objTr.Shipping_Mark = 1, True, False))
                        ...
                        ''richa agarwal 26/04/2015
                        If clsCommon.myLen(clsCommon.myCstr(objTr.Shipment_Code)) > 0 Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colOrderNo).Value = objTr.Shipment_Code
                        Else
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colPONo).Value = objTr.MT_Against_PO
                        End If
                        ''---------------------------
                        ...
                        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXCommercialInvoice) = CompairStringResult.Equal Then
                            If clsCommon.myLen(objTr.Shipment_Code) > 0 Then
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXPorformaInvoiceDetail.GetBalanceSRNQty(objTr.Shipment_Code, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable))
                            End If
                        End If
                        ''richa agarwal 27/04/2015
                        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then

                            If clsCommon.myLen(objTr.MT_Against_PO) > 0 AndAlso rdbAgainstWithoutLC.IsChecked Then
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXCommercialInvoiceDetail.GetBalancePOQtyWithoutLC(objTr.MT_Against_PO, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable))
                            ElseIf clsCommon.myLen(objTr.MT_Against_PO) > 0 AndAlso rdbAgainstWithLC.IsChecked Then
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXCommercialInvoiceDetail.GetBalancePOQtyWithLC(objTr.MT_Against_PO, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable, lbldocumentAcceptance.Text))
                            End If
                            If clsCommon.myLen(objTr.Shipment_Code) > 0 AndAlso rdbAgainstWithoutLC.IsChecked Then
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXCommercialInvoiceDetail.GetBalancePIQtyWithoutLC(objTr.Shipment_Code, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable))
                            ElseIf clsCommon.myLen(objTr.Shipment_Code) > 0 AndAlso rdbAgainstWithLC.IsChecked Then
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXCommercialInvoiceDetail.GetBalancePIQtyWithLC(objTr.Shipment_Code, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable, lbldocumentAcceptance.Text))
                            End If
                        End If
                        ''-------------------
                        ...
                        '===========shivani
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colWeighmentUnit).Value = objTr.Weight_UOM
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTotGrossItemWt).Value = objTr.Gross_Item_Weight
                ...
                ''richa agarwal 26/04/2015
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                    gv1.Columns(colPONo).IsVisible = True
                End If
                ''-------------------------------
                '================notify party===================
                gv_Notify_Party.Rows.Clear()
                ...
                If obj.Arr_Notify IsNot Nothing AndAlso obj.Arr_Notify.Count > 0 Then
                    For Each objtr As clsEXCommercialInvoiceNotifyDetail In obj.Arr_Notify
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                If (clsEXCommercialInvoiceHead.PostData(MyBase.Form_ID, txtDocNo.Value, arrLoc)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsEXCommercialInvoiceHead.DeleteData(txtDocNo.Value, MyBase.Form_ID)) Then
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        '-----------------------------------------------------
        Dim qry As String = "select Document_Code as Code,CONVERT(varchar(10), Document_Date,103)+' '+ CONVERT(varchar(5), Document_Date,114) as Date,(case when document_type='MT' then 'Merchant Trade' else 'Export Sale' end) as [Document Type],Customer_Code as [Customer Code], Customer_Name as Customer,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Comments,Total_Amt as Amount,case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.Status=0 then 'Pending' else 'Approved' end as [Status],Against_PI_No as [Export PI No] from TSPL_EX_COMMERCIAL_INVOICE_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_EX_COMMERCIAL_INVOICE_HEAD.Customer_Code "

        Dim whrClas As String = ""
        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
            whrClas = " TSPL_EX_COMMERCIAL_INVOICE_HEAD.document_type='MT' "
        Else
            whrClas = " TSPL_EX_COMMERCIAL_INVOICE_HEAD.document_type='EX' "
        End If

        If clsCommon.myLen(arrLoc) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas += " and Bill_To_Location in (" + arrLoc + ") and TSPL_EX_COMMERCIAL_INVOICE_HEAD.Customer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(arrLoc) > 0 Then
            whrClas += " and Bill_To_Location in (" + arrLoc + ") "
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas += " and TSPL_EX_PI_HEAD.Customer_Code in (" + strwherecls + ") "
        End If

        '-----------------------------------------------------


        LoadData(clsCommon.ShowSelectForm("COMFND", qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked), NavigatorType.Current)
    End Sub
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating
        ...
        If clsCommon.myLen(txtVendorNo.Value) = 0 Then
            ...
            txtVendorNo.Focus()
            txtVendorNo.Select()
            Errorcontrol.SetError(lblVendorName, "Please select buyer first")
            Exit Sub
        Else
            Errorcontrol.ResetError(lblVendorName)
        End If
        ...
        txtShipToLocation.Value = clsCommon.ShowSelectForm("EXCOMMSHP1FND", qry, "Code", "Ship_To_Type_Code='" & clsCommon.myCstr(txtVendorNo.Value) & "' and loc_code='" & clsCommon.myCstr(txtBillToLocation.Value) & "'", txtShipToLocation.Value, "Code", isButtonClicked)
    ...
    Sub SelectMRNItems()
        ...
        frm.strDocType = "Export COMMERCIAL"
        frm.Text = "Pending Proforma Invoice"
        frm.strExport_Merchant = "EX"
        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
            frm.strExport_Merchant = "MT"
        End If
        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal AndAlso rdbAgainstWithoutLC.IsChecked Then
            frm.strDocType = "Merchant PI Without LC"
        End If
        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal AndAlso rdbAgainstWithLC.IsChecked Then
            frm.strDocType = "Merchant PI With LC"
        End If
        frm.ShowDialog()



        Dim objOrderHead As clsEXPorformaInvoiceHead = Nothing
        If frm.ArrReturn_EX_PI IsNot Nothing AndAlso frm.ArrReturn_EX_PI.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn_EX_PI(0).Document_Code) > 0 Then
                objOrderHead = clsEXPorformaInvoiceHead.GetData(frm.ArrReturn_EX_PI(0).Document_Code, "'T','R'", NavigatorType.Current, arrLoc, frm.strExport_Merchant)
                If objOrderHead IsNot Nothing AndAlso clsCommon.myLen(objOrderHead.Document_Code) > 0 Then
                    ...
                    If clsCommon.myLen(txtPONo.Text) <= 0 Then
                        ...
                        If clsCommon.myLen(objOrderHead.podate) > 0 Then
                            txtpodate.Value = objOrderHead.podate
                        End If
                    End If
                    ...
                    If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                        If clsCommon.myLen(objOrderHead.MT_Against_PO) > 0 Then
                            TxtMTPONo.Value = objOrderHead.MT_Against_PO
                            TxtMTPOdate.Value = clsCommon.myCDate(objOrderHead.MT_Against_PO_Date)
                            TxtMTPOdate.Checked = True
                        End If
                        lbldocumentAcceptance.Text = frm.strDocumentAcceptanceNo
                    End If
                    ''-------------------------
            ...
            If objOrderHead.Arr_Notify IsNot Nothing AndAlso objOrderHead.Arr_Notify.Count > 0 Then
                For Each objtr As clsEXPorformaInvoiceNotifyDetail In objOrderHead.Arr_Notify
            ...
            For ii As Integer = 0 To frm.ArrReturn_EX_PI.Count - 1
                If clsCommon.myLen(frm.ArrReturn_EX_PI(ii).Document_Code) > 0 Then
                    Dim strCode As String = frm.ArrReturn_EX_PI(ii).Document_Code
                    'If Not arr.Contains(strCode) Then
                    '    arr.Add(strCode)
                    objOrderHead = clsEXPorformaInvoiceHead.GetData(frm.ArrReturn_EX_PI(ii).Document_Code, "'T','R'", NavigatorType.Current, arrLoc, frm.strExport_Merchant)
                    For Each obj As clsEXPorformaInvoiceDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn_EX_PI(ii).Item_Code AndAlso obj.Scheme_Item = "N") OrElse (obj.Scheme_Code = frm.ArrReturn_EX_PI(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            ...
                            If IsValidItem(obj) Then
                                ...
                                '==============
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colWeighmentUnit).Value = obj.Unit_code
                                '==============
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colICode).ReadOnly = True
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colUnit).ReadOnly = True

                                gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = frm.ArrReturn_EX_PI(ii).Balance_Qty 'obj.Balance_Qty


                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = frm.ArrReturn_EX_PI(ii).Balance_Qty
                                ...
                                'Dim dt As DataTable = clsEXCommercialInvoiceHead.GetOriginalQty(obj.Document_Code, obj.Item_Code, obj.Unit_code, obj.Assessable, obj.MRP, Nothing)
                                'If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPackingInstruction).Value = obj.Packing_Inst
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsEXPorformaInvoiceDetail In objOrderHead.Arr
    ...
    Sub SelectPurchaseItems()
        ...
        cboItemType.Enabled = True
        ''richa agarwal 21/04/2015
        'gv1.Columns(colOrderNo).IsVisible = False
        gv1.Columns(colPONo).IsVisible = True
        ''------------------------
        Dim frm As New frmPendingSaleOrder
        frm.VendorCode = txtVendorNo.Value
        ...
        frm.Text = "Pending Proforma Invoice"
        frm.strExport_Merchant = "MT"
        If rdbAgainstWithoutLC.IsChecked Then
            frm.strDocType = "Merchant PO Without LC"
        Else
            frm.strDocType = "Merchant PO With LC"
        End If
        frm.ShowDialog()



        Dim objOrderHead As clsPurchaseOrderHead = Nothing
        If frm.arrReturn_MT_Purc IsNot Nothing AndAlso frm.arrReturn_MT_Purc.Count > 0 Then
            If clsCommon.myLen(frm.arrReturn_MT_Purc(0).PurchaseOrder_No) > 0 Then
                objOrderHead = clsPurchaseOrderHead.GetData(frm.arrReturn_MT_Purc(0).PurchaseOrder_No, NavigatorType.Current)
                If objOrderHead IsNot Nothing AndAlso clsCommon.myLen(objOrderHead.PurchaseOrder_No) > 0 Then
                    '' currency details
                    '  txtVendorNo.Enabled = False
                    txtBillToLocation.Enabled = False
                    cmbDocType.Enabled = False
                    ...
                    cmbDocType.SelectedValue = "MT"
                    cboItemType.SelectedValue = objOrderHead.Item_Type
                    TxtMTPONo.Value = objOrderHead.PurchaseOrder_No
                    TxtMTPOdate.Value = objOrderHead.PurchaseOrder_Date
                    TxtMTPOdate.Checked = True
                    ...
                    ''richa agarwal 20/04/2015
                    If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal Then
                        If clsCommon.myLen(objOrderHead.MT_PI_No) > 0 Then
                            txtReqNo.Value = objOrderHead.MT_PI_No
                        End If
                        lbldocumentAcceptance.Text = frm.strDocumentAcceptanceNo
                    End If
                    ''-------------------------
                    txtPre_Carriage_By.Text = objOrderHead.MT_Pre_Carriage_By
                    txtPort_Discharge.Text = objOrderHead.MT_Discharge_Port
                    txtFinal_Destination.Text = objOrderHead.MT_Final_Destination
                    fndCountry_Origin.Value = objOrderHead.MT_Origin_Country
                    fndCountry_Final_Destination.Value = objOrderHead.MT_Final_Destination_Country
                    txtTermCode.Value = objOrderHead.Terms_Code
                    lblTermName.Text = objOrderHead.TermsName
                    If clsCommon.myLen(txtReqNo.Value) > 0 Then
                        Dim qry2 As String = "Select TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_EX_PI_HEAD.BANK_CODE,TSPL_EX_PI_HEAD.BRANCH_CODE  ,TSPL_EX_PI_HEAD.Customer_Code ,TSPL_EX_PI_HEAD.Import_Export_No ,TSPL_EX_PI_HEAD.Exporter_Ref_No from TSPL_EX_PI_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_EX_PI_HEAD .Customer_Code =TSPL_CUSTOMER_MASTER.Cust_Code  where TSPL_EX_PI_HEAD.Document_Code='" & txtReqNo.Value & "'"
                        Dim dt3 As DataTable = clsDBFuncationality.GetDataTable(qry2)

                        If dt3 IsNot Nothing AndAlso dt3.Rows.Count > 0 Then
                            For Each dr As DataRow In dt3.Rows
                                txtVendorNo.Value = clsCommon.myCstr(dr("Customer_Code"))
                                lblVendorName.Text = clsCommon.myCstr(dr("Customer_Name"))
                                txtIm_Ex_No.Text = clsCommon.myCstr(dr("Import_Export_No"))
                                txtExporter_Ref_No.Text = clsCommon.myCstr(dr("Exporter_Ref_No"))
                                txtBankCode.Value = clsCommon.myCstr(dr("BANK_CODE"))
                                FillBankDetail(txtBankCode.Value)
                                txtBankBranchCode.Value = clsCommon.myCstr(dr("BRANCH_CODE"))
                                FillBranchDetail(txtBankBranchCode.Value, txtBankCode.Value)
                            Next
                    ...
                    'txtComm_Pay_name.Text = clsVendorMaster.GetName(fndComm_Pay_Code.Value, Nothing)
                    'txtOthr_Instructn.Text = objOrderHead.Commission_Instruction
                    cmbTerms.SelectedValue = objOrderHead.MT_EX_Term_Code
                    cmbTerms_Payment.SelectedValue = objOrderHead.MT_Payment_Terms
                    'chkAdvance.Checked = False
                    chkAdvance.Visible = False
                    txtAdvance_Pers.Enabled = False
                    txtAdvance_Pers.MendatroryField = False
                    If objOrderHead.MT_PT_Advance_Amount > 0 Then
                        chkAdvance.Checked = True
                        txtAdvance_Pers.Enabled = True
                        txtAdvance_Pers.MendatroryField = True
                    End If
                    txtAdvance_Pers.Text = objOrderHead.MT_PT_Advance_Amount

                    'txtIm_Ex_No.Text = objOrderHead.GRNo
                    'txtExporter_Ref_No.Text = objOrderHead.Exporter_Ref_No
                    txtCarrier.Text = objOrderHead.MT_Carrier
            ...
            gv_Notify_Party.Rows.AddNew()
            If clsCommon.myLen(txtReqNo.Value) > 0 Then
                Dim qry As String = "select TSPL_EX_NOTIFY_PARTY_DETAIL.*,tspl_customer_master.customer_name,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add1,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add2,TSPL_NOTIFY_PARTY_SHIP_DETAIL.add3,TSPL_NOTIFY_PARTY_SHIP_DETAIL.country_code,TSPL_NOTIFY_PARTY_SHIP_DETAIL.city_code,TSPL_NOTIFY_PARTY_SHIP_DETAIL.state_code from TSPL_EX_NOTIFY_PARTY_DETAIL left outer join TSPL_NOTIFY_PARTY_SHIP_DETAIL on TSPL_NOTIFY_PARTY_SHIP_DETAIL.cust_code=TSPL_EX_NOTIFY_PARTY_DETAIL.cust_code left outer join tspl_customer_master on tspl_customer_master.cust_code=TSPL_EX_NOTIFY_PARTY_DETAIL.cust_code where document_code='" + txtReqNo.Value + "'"
                Dim dt1 As DataTable = clsDBFuncationality.GetDataTable(qry)

                If dt1 IsNot Nothing AndAlso dt1.Rows.Count > 0 Then
                    For Each dr As DataRow In dt1.Rows
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Add1).Value = clsCommon.myCstr(dr("add1"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Add2).Value = clsCommon.myCstr(dr("add2"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Add3).Value = clsCommon.myCstr(dr("add3"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_City).Value = clsCommon.myCstr(dr("city_code"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Country).Value = clsCommon.myCstr(dr("country_code"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Cust_Code).Value = clsCommon.myCstr(dr("cust_code"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Cust_Name).Value = clsCommon.myCstr(dr("customer_name"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Lineno).Value = clsCommon.myCstr(dr("line_no"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_Location_Code).Value = clsCommon.myCstr(dr("location_code"))
                        gv_Notify_Party.Rows(gv_Notify_Party.Rows.Count - 1).Cells(colNT_State).Value = clsCommon.myCstr(dr("state_code"))
                        gv_Notify_Party.Rows.AddNew()
                    Next
                End If
                qry = Nothing
            ...
            Dim mrnno As String = String.Empty
            Dim arr As New List(Of String)
            For ii As Integer = 0 To frm.arrReturn_MT_Purc.Count - 1
                If clsCommon.myLen(frm.arrReturn_MT_Purc(ii).PurchaseOrder_No) > 0 Then
                    Dim strCode As String = frm.arrReturn_MT_Purc(ii).PurchaseOrder_No
                    'If Not arr.Contains(strCode) Then
                    '    arr.Add(strCode)
                    objOrderHead = clsPurchaseOrderHead.GetData(frm.arrReturn_MT_Purc(ii).PurchaseOrder_No, NavigatorType.Current)
                    For Each obj As clsPurchaseOrderDetail In objOrderHead.Arr
                        'If (obj.Item_Code = frm.ArrReturn_EX_PI(ii).Item_Code AndAlso obj.Scheme_Item = "N") OrElse (obj.Scheme_Code = frm.ArrReturn_EX_PI(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = frm.arrReturn_MT_Purc(ii).Balance_Qty 'obj.Balance_Qty


                        gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = frm.arrReturn_MT_Purc(ii).Balance_Qty
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRate1).Value = obj.TAX1_Rate
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRate2).Value = obj.TAX2_Rate
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRate3).Value = obj.TAX3_Rate
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRate4).Value = obj.TAX4_Rate
                        ...
                        'End If
                        If clsCommon.myLen(txtReqNo.Value) > 0 Then
                            Dim qry1 As String = "Select TSPL_EX_PI_DETAIL.Packing_Inst ,TSPL_EX_PI_DETAIL.Shipping_Mark ,TSPL_EX_PI_DETAIL.Bin_No ,TSPL_EX_PI_DETAIL.Batch_No ,TSPL_EX_PI_DETAIL.Qty ,TSPL_EX_PI_DETAIL.Conv_Factor,TSPL_EX_PI_DETAIL.Item_Weight from TSPL_EX_PI_HEAD Left outer Join TSPL_EX_PI_DETAIL on TSPL_EX_PI_HEAD.Document_Code =TSPL_EX_PI_DETAIL.DOCUMENT_CODE where TSPL_EX_PI_HEAD.Document_Code='" & txtReqNo.Value & "' and TSPL_EX_PI_DETAIL.Item_Code ='" & obj.Item_Code & "'"
                            Dim dt2 As DataTable = clsDBFuncationality.GetDataTable(qry1)

                            If dt2 IsNot Nothing AndAlso dt2.Rows.Count > 0 Then
                                For Each dr As DataRow In dt2.Rows
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colShippingMark).Value = clsCommon.myCstr(dr("Shipping_Mark"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colPackingInstruction).Value = clsCommon.myCstr(dr("Packing_Inst"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colBatchNo).Value = clsCommon.myCstr(dr("Batch_No"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colBinNo).Value = clsCommon.myCstr(dr("Bin_No"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colItemWeight).Value = clsCommon.myCdbl(dr("Item_Weight"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colConvF).Value = clsCommon.myCdbl(dr("Conv_Factor"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colTotItemWt).Value = clsCommon.myCdbl(frm.arrReturn_MT_Purc(ii).Balance_Qty) * clsCommon.myCdbl(dr("Item_Weight"))
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colNo_Kind_Package).Value = clsCommon.myCdbl(frm.arrReturn_MT_Purc(ii).Balance_Qty) * clsCommon.myCdbl(dr("Conv_Factor"))
                                Next
                            End If
                            qry1 = Nothing
                            dt2 = Nothing
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsPurchaseOrderDetail In objOrderHead.Arr
                    If objTr.Row_Type = "Misc" Then
                        gv1.Rows.AddNew()
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colLineNo).Value = gv1.Rows.Count
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colOrderNo).Value = mrnno
    ...
    End Sub
    ''-------------------------------------
    Function IsValidItem(ByVal obj As clsEXPorformaInvoiceDetail)
        If clsCommon.myLen(txtTaxGroup.Value) <= 0 Then
    ...
    Public Function funPrint(ByVal strDocNo As String) As DataTable
        ...
        Try
            ...
            If clsCommon.CompairString(clsCommon.myCstr(ShowStatusForSale), "1") = CompairStringResult.Equal Then
                QryShowStatus = " ,(case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.status =1 then 'AUTHORIZED' else 'NOT AUTHORIZED' end) as InvStatus "
            Else
                QryShowStatus = ""
            End If
            ...
            qry1 += " from TSPL_EX_COMMERCIAL_INVOICE_DETAIL "
            qry1 += "left outer join TSPL_EX_PI_DETAIL on TSPL_EX_PI_DETAIL.DOCUMENT_CODE=TSPL_EX_COMMERCIAL_INVOICE_DETAIL.PI_Code and TSPL_EX_PI_DETAIL.Item_Code=TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Item_Code "
            qry1 += " where TSPL_EX_COMMERCIAL_INVOICE_DETAIL.DOCUMENT_CODE='" + strDocNo + "'"
            ...
            '' end 
            ' Dim strval As String = "(Select Value from Tspl_CUstom_field_Values where Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Bill of Lading Number'))"
            'clsFixedParameter()
            'changes by shivani against(BM00000007742,BM00000007741),ALF/09/04/19-000098 show batch item details.
            'Ticket no-ALF/09/04/19-000099, Sanjay, Add HSN Code
            ...
            Dim Qry As String = "select cast(TSPL_SD_SALE_INVOICE_HEAD.BarCode_Img as image) As BarCode_Img,isnull (TSPL_SD_SALE_INVOICE_HEAD.IRN_No,'') as IRN_No,isnull (TSPL_SD_SALE_INVOICE_HEAD.Ack_No,'') as Ack_No,case when len(isnull (TSPL_SD_SALE_INVOICE_HEAD.Ack_No,'')) > 0 then convert (varchar, TSPL_SD_SALE_INVOICE_HEAD.Ack_Date,103) else ''  end as Ack_Date, case when  isnull(TSPL_SD_SALE_INVOICE_HEAD.EInvoice_Type,'')='BB' AND convert(date ,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103)>=convert(date ,'" + clsCommon.myCstr(DateOfEInvoiceImplementation) + "',103) then 1 else 0 end as  IsEInvoiceApply," &
                                "  case when  isnull(tspl_batch_item.qty,0) > 0 then  " &
                                " case when TSPL_EX_COMMERCIAL_INVOICE_Detail.Unit_Code <> 'MT' and TSPL_ITEM_UOM_DETAIL_MT.Conversion_Factor is null then convert (decimal(18,3), ( (isnull(tspl_batch_item.qty,0) * TBL_ITEM_STOCKING_Unit_Conv.Conversion_Factor)/TSPL_ITEM_UOM_DETAIL_KG.Conversion_Factor)/1000)  when   TSPL_EX_COMMERCIAL_INVOICE_Detail.Unit_Code <> 'MT' and TSPL_ITEM_UOM_DETAIL_MT.Conversion_Factor is not null then convert (decimal(18,3), ( (isnull(tspl_batch_item.qty,0) * TBL_ITEM_STOCKING_Unit_Conv.Conversion_Factor)/TSPL_ITEM_UOM_DETAIL_KG.Conversion_Factor))  else  isnull(tspl_batch_item.qty,0) end " &
                                " else  case when TSPL_EX_COMMERCIAL_INVOICE_Detail.Unit_Code <> 'MT' and TSPL_ITEM_UOM_DETAIL_MT.Conversion_Factor is null then convert (decimal(18,3), ( (TSPL_EX_COMMERCIAL_INVOICE_Detail.Qty * TBL_ITEM_STOCKING_Unit_Conv.Conversion_Factor)/TSPL_ITEM_UOM_DETAIL_KG.Conversion_Factor)/1000)  when   TSPL_EX_COMMERCIAL_INVOICE_Detail.Unit_Code <> 'MT' and TSPL_ITEM_UOM_DETAIL_MT.Conversion_Factor is not null then convert (decimal(18,3), ( (TSPL_EX_COMMERCIAL_INVOICE_Detail.Qty * TBL_ITEM_STOCKING_Unit_Conv.Conversion_Factor)/TSPL_ITEM_UOM_DETAIL_KG.Conversion_Factor))  else  TSPL_EX_COMMERCIAL_INVOICE_Detail.Qty end end " &
                                "  as Qty_In_MT , TSPL_EX_COMMERCIAL_INVOICE_HEAD.Branch_code,TSPL_EX_COMMERCIAL_INVOICE_HEAD. Advance_Percentage,isnull(TSPL_ITEM_MASTER.HSN_Code,'') as HSN_Code,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Gross_Wt as Net_weight,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Weight_UOM ,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Gross_Item_Weight,convert(int,left(TSPL_EX_COMMERCIAL_INVOICE_HEAD.Total_Amt,len(TSPL_EX_COMMERCIAL_INVOICE_HEAD.Total_Amt)-3)) as docamt1,(Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Bill of Lading Date')) as BillofLadingDate,(Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Bill Of Lading No.')) as BillofLading,(Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Shipping Bill No.')) as Shippment,(Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Shipping Bill Date')) as Shippment_Date,(Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Merchant Policy No')) as MerchantPolicyNo, (Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Bill of Lading Number')) as Value,(Select Value from Tspl_CUstom_field_Values where Tspl_CUstom_field_Values.Transaction_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Shipment Port'))+',' as ShipmentPort ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Against_PI_No ,(Select convert(varchar,Document_Date,103)  from TSPL_EX_PI_HEAD where TSPL_EX_PI_HEAD.Document_Code = TSPL_EX_COMMERCIAL_INVOICE_HEAD.Against_PI_No ) as PI_DATE,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.TotalItem_Weight,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Final_Gross_Wt as Gross_Wt,TSPL_CURRENCY_MASTER.CURRENCY_SIGN, Loc_for_city.City_Name as Loc_City_Name,TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code as EX_Term_Name,Loc_for_state .STATE_NAME as Loc_state_Name, TSPL_COMPANY_MASTER .City_Code as Comp_City_Name,Comp_for_State.STATE_NAME as Comp_State_name, TSPL_COMPANY_MASTER.Pincode as Comp_Pin_No,TSPL_COMPANY_MASTER.IECode,TSPL_COMPANY_MASTER.Access_Officer,TSPL_COMPANY_MASTER.GSTReg_No,TSPL_Company_Master.Regn_No as Comp_Regn_No,TSPL_Company_Master.Ecc_No as Comp_Ecc_No,tspl_bank_master.Description as bank_name,tspl_bank_master.add1 as Bank_add1,tspl_bank_master.add2 as Bank_add2,tspl_bank_master.add3 as Bank_add3,tspl_bank_master.ADD4 as Bank_Add4,tspl_bank_master.STATE  as Bank_state_Name,tspl_bank_master.CITY  as Bank_City_Name,tspl_bank_master.Postal as Bank_Postal,tspl_bank_master.CONTACT as Bank_Contact,tspl_bank_master.fax as Bank_Fax ,tspl_bank_master.BankAccNumber,tspl_bank_branch_master.IFSC_Code as bank_IFSC_Code ,TSPL_BANK_MASTER.Swift_Code, tspl_purchase_order_head.MT_Beneficiary_code,TSPL_COMPANY_MASTER.Pan_No as Comp_Pan_No,  Case when len(ISNULL(TSPL_SHIP_TO_LOCATION.telphone,''))>0 and TSPL_SHIP_TO_LOCATION.telphone='(+__)__________' then '' else ' '+TSPL_SHIP_TO_LOCATION.telphone end " &
            "	 as Ship_Location_Phone, TSPL_EX_COMMERCIAL_INVOICE_HEAD.Incentive_Declaration,TSPL_EX_COMMERCIAL_INVOICE_detail.packing_instruction,TSPL_EX_COMMERCIAL_INVOICE_detail.Shipping_mark,TSPL_EX_COMMERCIAL_INVOICE_detail.specification ,tspl_item_master.itf_code as Item_iecode,TSPL_ITEM_MASTER.HSN_Code,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code1 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt1 ," &
            "  (Select add1 from TSPL_LOCATION_MASTER where Location_Code ='002') as softaadd1,(Select add2 from TSPL_LOCATION_MASTER where Location_Code ='002') as softaadd2,(Select Add3 from TSPL_LOCATION_MASTER where Location_Code ='002') as softaadd3 , (Select STATE_NAME   from TSPL_LOCATION_MASTER left join tspl_state_master on tspl_state_master.STATE_CODE =TSPL_LOCATION_MASTER.State   where Location_Code ='002')as SoftStateName," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code2 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt2," &
            "TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code3 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt3," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code4 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt4," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code5 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt5," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code6 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt6," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code7 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt7," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code8 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt8," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code9 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt9," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code10 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Amt10 ,TSPL_LOCATION_MASTER.Location_Desc,TSPL_EX_COMMERCIAL_INVOICE_HEAD. Total_Add_Charge,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Total_Disc_Amt,'" + strSoNo + "' as SONo, '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_LOCATION_MASTER.Add1 as Location_Add1,TSPL_LOCATION_MASTER.Add1 as [Location Address],TSPL_LOCATION_MASTER.Add2 as Location_Add2,TSPL_LOCATION_MASTER.Add3 as Location_Add3,TSPL_LOCATION_MASTER.Add4 as Location_Add4,TSPL_LOCATION_MASTER.TIN_No ,Case when len(ISNULL(TSPL_LOCATION_MASTER.Phone1,''))>0 and TSPL_LOCATION_MASTER.Phone1='(+__)__________' then '' else ' '+TSPL_LOCATION_MASTER.Phone1 end " &
            "   +  Case When ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'(+__)__________' Then '  '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as Location_Phone, TSPL_SHIP_TO_LOCATION.Ship_To_Desc as shipName, TSPL_SHIP_TO_LOCATION.Ship_To_Desc as shipName, TSPL_SHIP_TO_LOCATION.add1 as ship_Add1, TSPL_SHIP_TO_LOCATION.Add2 as ship_add2 ,TSPL_SHIP_TO_LOCATION.Add3 as ship_add3  ,TSPL_SHIP_TO_LOCATION.Pin_Code , Convert (varchar,TSPL_SHIP_TO_LOCATION.Pin_Code,103) as Ship_Pin_Code  ,TSPL_CITY_MASTER.STATE_CODE  ,TSPL_CITY_MASTER.City_Name," &
            "TSPL_EMPLOYEE_MASTER.Emp_Name as SalesManName,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Inv_No, TSPL_EX_COMMERCIAL_INVOICE_HEAD.Dept_Desc , TSPL_EX_COMMERCIAL_INVOICE_HEAD.Remarks ,  TSPL_EX_COMMERCIAL_INVOICE_HEAD.Terms_Code,TSPL_TERMS_MASTER.Terms_Desc as Term_Desc,TSPL_EX_COMMERCIAL_INVOICE_HEAD.VehicleNo,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Vehicle_Code,TSPL_VEHICLE_MASTER.Description as Vehicle_Desc , " &
            " Notify.customer_name as Notify_Party,Notify.add1 as Notify_add1,Notify.add2 as Notify_add2,Notify.add3 as Notify_add3,Case when len(ISNULL(Notify.Phone1,''))>0 and Notify.Phone1='(+__)__________' then '' else ' '+Notify.Phone1 end  as Notify_Phone,Notify.pin_code as Notify_pin_Code,Notify.city_name as Notify_City,Notify.state_name as Notify_state, " &
            " TSPL_EX_COMMERCIAL_INVOICE_DETAIL .Specification as  specification,   TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code as DocNo , TSPL_EX_COMMERCIAL_INVOICE_HEAD.Description,TSPL_EX_COMMERCIAL_INVOICE_HEAD.CUST_PO_NO,convert(varchar,TSPL_EX_COMMERCIAL_INVOICE_HEAD.CUST_PO_DATE,103) as CUST_PO_DATE, " &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Loading_Port,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Vessel_Flight_No,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Import_Export_No,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Container_No,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.No_Kind_Package,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Exporter_Ref_No,(case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.Payment_Terms='LC' then 'Letter of Credit' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.Payment_Terms='DA' then 'Document against Acceptance' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.Payment_Terms='DP' then 'Document against Payment' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.Payment_Terms='AD' then 'Advance' else '' end end end end) as Payment_Terms,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Pre_Carriage_By,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Carrier as place_of_pre_carriage,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Discharge_Port,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Final_Destination,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Origin_Country,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Final_Destination_Country,(case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code='CIF' then 'Cost, Insurance & Freight' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code='CFR' then 'Cost & Freight' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code='FOB' then 'Free on Board' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code='EPH' then 'EX-Works,Palwal-Haryana' else case when TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code='C&F' then 'Cost & Freight' else TSPL_EX_COMMERCIAL_INVOICE_HEAD.EX_Term_Code end end end end end) as EX_Term_Code,(case when TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Shipping_Mark='1' then 'Marked' else '' end) as Shipping_Mark, " &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Description,convert(varchar, TSPL_EX_COMMERCIAL_INVOICE_HEAD .Document_Date,103) as Document_Date , TSPL_EX_COMMERCIAL_INVOICE_HEAD.Against_PI_No as Against_Shipment_No, TSPL_EX_COMMERCIAL_INVOICE_HEAD.Item_Type ,  TSPL_EX_COMMERCIAL_INVOICE_HEAD.Customer_Code, " &
            " TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Add1 as Customer_Add1,TSPL_CUSTOMER_MASTER.add2 as customer_Add2,TSPL_CUSTOMER_MASTER.Add3 as customer_Add3 ,TSPL_CUSTOMER_MASTER.State as customer_city_State ,TSPL_CUSTOMER_MASTER.PIN_Code as Customer_Pin_Code,COALESCE(TSPL_SHIP_TO_LOCATION.CST_No,TSPL_CUSTOMER_MASTER.CST) as Cust_CST_No,COALESCE(TSPL_SHIP_TO_LOCATION.Tin_No,TSPL_CUSTOMER_MASTER.Tin_No)as Cust_Tin_No,TSPL_CUSTOMER_MASTER.Contact_Person_Name as Cust_Contact_Name,TSPL_CUSTOMER_MASTER.Contact_Person_Phone as Cust_Contact_Number , TSPL_EX_COMMERCIAL_INVOICE_HEAD .Terms_Code as termscode ,TSPL_EX_COMMERCIAL_INVOICE_HEAD .Ref_No as ref_no ," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD .Comments as comments ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Status ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.On_Hold ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Comp_Code ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Due_Date ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Posting_Date ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Carrier ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.GENo as GRNo ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.GENo ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.GEDate ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Code1 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Add_Charge_Name1 ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Challan_No ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Challan_Date ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Inv_Date ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.CURRENCY_CODE ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.ConvRate ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.ApplicableFrom ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Against_C_Form ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.CFormApplied ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.CFormRecd ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.PROJECT_ID ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Price_code ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Route_No ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Route_Desc ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.HeadDisc_Per ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.HeadDisc_Amt ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.TotCashDiscAmt ,  TSPL_EX_COMMERCIAL_INVOICE_HEAD .Discount_Amt as dis_amt,(case when Scheme_Item='Y' then (TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Disc_Amt+TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Amount) else (TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Disc_Amt) end) as dis_amt1," &
            " TSPL_EX_COMMERCIAL_INVOICE_HEAD.Amount_Less_Discount  as aftrdiscount ,TSPL_EX_COMMERCIAL_INVOICE_HEAD .Total_Amt as Total_amount," &
            ...
            " tax1.Tax_Code_Desc as tax1name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax1_amt,0) as txt1amt,  " &
            " tax2.Tax_Code_Desc as tax2name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax2_amt,0) as txt2amt,  " &
            " tax3.Tax_Code_Desc as tax3name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax3_amt,0) as txt3amt,  " &
            " tax4.Tax_Code_Desc as tax4name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax4_amt,0) as txt4amt,  " &
            " tax5.Tax_Code_Desc as tax5name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax5_amt,0) as txt5amt,  " &
            " tax6.Tax_Code_Desc as tax6name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax6_amt,0) as txt6amt,  " &
            " tax7.Tax_Code_Desc as tax7name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax7_amt,0) as txt7amt,  " &
            " tax8.Tax_Code_Desc as tax8name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax8_amt,0) as txt8amt,   " &
            " tax9.Tax_Code_Desc as tax9name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax9_amt,0) as txt9amt,  " &
            " tax10.Tax_Code_Desc as tax10name,isnull (TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax10_amt,0) as txt10amt,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.TAX1_Rate,  " &
            " isnull(TSPL_EX_COMMERCIAL_INVOICE_HEAD .Total_Tax_Amt,0) as total_tax_amt, TSPL_EX_COMMERCIAL_INVOICE_HEAD.Total_Amt as DocAmt,  TSPL_COMPANY_MASTER.Comp_Name as compname, ISNULL(TSPL_COMPANY_MASTER.Phone1,'')+ Case When ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as Phone,TSPL_COMPANY_MASTER.Fax,TSPL_COMPANY_MASTER.Email ,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,ISNULL(tspl_company_Master.ADD1,'') as address1,ISNULL(tspl_company_Master.ADD2,'') as address2,ISNULL(tspl_company_Master.ADD3,'') as address3," &
            " TSPL_EX_COMMERCIAL_INVOICE_DETAIL.item_code as item_code,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION as PrincipleDesc,TSPL_CUSTOMER_ITEM_MAPPING.CUSTOMER_ITEM_NO, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Row_Type,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Qty as qty,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.unit_code as uom,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.item_cost as itemcost,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.amount as amount,TSPL_EX_COMMERCIAL_INVOICE_HEAD.TAX1,TSPL_EX_COMMERCIAL_INVOICE_HEAD.TAX2,TSPL_EX_COMMERCIAL_INVOICE_HEAD.TAX3,TSPL_EX_COMMERCIAL_INVOICE_HEAD.TAX4,TSPL_EX_COMMERCIAL_INVOICE_HEAD.TAX5,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Total_Add_Charge,cast(datepart(MONTH,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.MFG_Date) as varchar(2)) + '/' + " &
            " SUBSTRING(cast(datepart(YYYY,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.MFG_Date) as varchar(4)),3,2) as MFG_Date," &
            " cast(datepart(MONTH,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Expiry_Date) as varchar(2)) + '/' + " &
            " SUBSTRING(cast(datepart(YYYY,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Expiry_Date) as varchar(4)),3,2) as Exp_Date,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.mrp,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Scheme_Item,TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Item_Weight,(case when COALESCE(TSPL_EX_COMMERCIAL_INVOICE_HEAD.Invoice_Type,'R')='T' then 'Tax Invoice' else 'Retail Invoice' end) as Invoice_Type " & colsTaxRateType & " " &
            " " & QryShowStatus & " " &
            " " & SerialNoColumn & " " &
            " ,TSPL_ITEM_MASTER.ITF_Code,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Gross_Wt ,TSPL_EX_COMMERCIAL_INVOICE_HEAD.Final_Gross_Wt,tspl_batch_item.Batch_No,convert(varchar,tspl_batch_item.Manufacture_date,103)as MFG_Date1 ,convert(varchar,tspl_batch_item.Expiry_Date,103)as Expiry_Date,isnull(tspl_batch_item.qty,0) as BatchQty " &
            " from TSPL_EX_COMMERCIAL_INVOICE_DETAIL  " &
            " " & SerialNo & " " &
            ...
            "  left outer join tspl_purchase_order_head on tspl_purchase_order_head.purchaseorder_no=TSPL_EX_COMMERCIAL_INVOICE_HEAD.CUST_PO_NO" &
            "	 left outer join TSPL_BANK_MASTER on TSPL_BANK_MASTER .BANK_CODE =TSPL_EX_COMMERCIAL_INVOICE_HEAD.BANK_CODE " &
            " left outer join TSPL_BANK_BRANCH_MASTER on TSPL_BANK_BRANCH_MASTER.Bank_CODE =TSPL_BANK_MASTER.BANK_CODE" &
            " left outer join  TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD .Ship_To_Location " &
            " left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code =TSPL_SHIP_TO_LOCATION.City_Code " &
            " left outer join TSPL_EMPLOYEE_MASTER on TSPL_EMPLOYEE_MASTER.EMP_CODE =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Salesman_Code " &
            " left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax1  " &
            " left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_EX_COMMERCIAL_INVOICE_HEAD.tax2  " &
            " left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_EX_COMMERCIAL_INVOICE_HEAD .TAX3  " &
            " left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_EX_COMMERCIAL_INVOICE_HEAD .tax4  " &
            " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_EX_COMMERCIAL_INVOICE_HEAD .tax5  " &
            " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD .TAX6  " &
            " left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD .TAX7  " &
            " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD .TAX8  " &
            " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD .TAX9 " &
            " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD .TAX10 " &
            " left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_EX_COMMERCIAL_INVOICE_HEAD.comp_code  " &
            "  left outer join TSPL_STATE_MASTER as Comp_for_State on Comp_for_State.STATE_CODE =TSPL_COMPANY_MASTER.State " &
            " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_EX_COMMERCIAL_INVOICE_HEAD.Customer_Code   " &
            " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER .Location_Code=  TSPL_EX_COMMERCIAL_INVOICE_HEAD.Bill_To_Location " &
            " left outer join TSPL_CITY_MASTER as Loc_for_city on Loc_for_city.City_Code =TSPL_LOCATION_MASTER.City_Code " &
            " left outer join TSPL_STATE_MASTER  as Loc_for_state on Loc_for_state.STATE_CODE =TSPL_LOCATION_MASTER.State " &
            " left join TSPL_TERMS_MASTER on TSPL_EX_COMMERCIAL_INVOICE_HEAD.Terms_Code=TSPL_TERMS_MASTER.Terms_Code " &
            " left join TSPL_VEHICLE_MASTER on TSPL_EX_COMMERCIAL_INVOICE_HEAD.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id" &
            " left join TSPL_CUSTOMER_ITEM_MAPPING on TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Item_Code=TSPL_CUSTOMER_ITEM_MAPPING.item_no " &
            " and TSPL_EX_COMMERCIAL_INVOICE_HEAD.Customer_Code=TSPL_CUSTOMER_ITEM_MAPPING.Customer_Code " &
            " Left Outer Join TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Item_Code " &
            " lEFT OUTER JOIN TSPL_ITEM_MASTER_CATEGORY ON TSPL_ITEM_MASTER.Item_Code=TSPL_ITEM_MASTER_CATEGORY.Item_code AND TSPL_ITEM_MASTER_CATEGORY.SNO=1 " &
            " LEFT OUTER JOIN TSPL_ITEM_CATEGORY_LEVEL_VALUES ON TSPL_ITEM_CATEGORY_LEVEL_VALUES.CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values " &
            " left outer join (select  top 1 TSPL_EX_COMMERCIAL_INVOICE_NOTIFY_PARTY_DETAIL.Document_code,TSPL_EX_COMMERCIAL_INVOICE_NOTIFY_PARTY_DETAIL.Cust_Code,TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Add1,TSPL_CUSTOMER_MASTER.Add2,TSPL_CUSTOMER_MASTER.Add3,TSPL_CUSTOMER_MASTER.Phone1,TSPL_CUSTOMER_MASTER.PIN_Code,TSPL_CITY_MASTER.City_Name,TSPL_STATE_MASTER.STATE_NAME,TSPL_EX_COMMERCIAL_INVOICE_NOTIFY_PARTY_DETAIL.Location_Code from TSPL_EX_COMMERCIAL_INVOICE_NOTIFY_PARTY_DETAIL left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_EX_COMMERCIAL_INVOICE_NOTIFY_PARTY_DETAIL.Cust_Code left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code=TSPL_CUSTOMER_MASTER.City_Code left outer join TSPL_STATE_MASTER on TSPL_STATE_MASTER.STATE_CODE=TSPL_CUSTOMER_MASTER.State where TSPL_EX_COMMERCIAL_INVOICE_NOTIFY_PARTY_DETAIL.Document_code='" + strDocNo + "') Notify " &
            " on Notify.document_code=TSPL_EX_COMMERCIAL_INVOICE_HEAD.document_code" &
            "  left outer join TSPL_CURRENCY_MASTER on TSPL_CURRENCY_MASTER.CURRENCY_CODE =TSPL_EX_COMMERCIAL_INVOICE_HEAD.CURRENCY_CODE" &
            " left outer join TSPL_SD_SALE_INVOICE_HEAD on TSPL_SD_SALE_INVOICE_HEAD.Against_Com_Inv_No=TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code " &
            " left outer join tspl_batch_item on tspl_batch_item.document_code=TSPL_SD_SALE_INVOICE_HEAD.document_code  and tspl_batch_item.Item_Code =TSPL_EX_COMMERCIAL_INVOICE_DETAIL .Item_Code  " &
            " " &
            " left outer join TSPL_ITEM_UOM_DETAIL as TSPL_ITEM_UOM_DETAIL_KG on TSPL_ITEM_UOM_DETAIL_KG.Item_Code = TSPL_EX_COMMERCIAL_INVOICE_Detail.Item_Code and TSPL_ITEM_UOM_DETAIL_KG.Uom_code = 'KG' " &
            " left outer join TSPL_ITEM_UOM_DETAIL as TBL_ITEM_STOCKING_Unit_Conv on TBL_ITEM_STOCKING_Unit_Conv.Item_Code = TSPL_EX_COMMERCIAL_INVOICE_Detail.Item_Code and TBL_ITEM_STOCKING_Unit_Conv.Uom_code = TSPL_EX_COMMERCIAL_INVOICE_Detail.Unit_Code " &
            " left outer join TSPL_ITEM_UOM_DETAIL as TSPL_ITEM_UOM_DETAIL_MT on TSPL_ITEM_UOM_DETAIL_MT.Item_Code = TSPL_EX_COMMERCIAL_INVOICE_Detail.Item_Code and TSPL_ITEM_UOM_DETAIL_MT.Uom_code = 'MT' " &
            " " &
            " where 2=2 and  TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code = '" + strDocNo + "' order by TSPL_EX_COMMERCIAL_INVOICE_DETAIL.line_no"
    ...
    Function GetTaxRateTypeDT(ByVal DocNo As String, Optional ByVal trans As SqlTransaction = Nothing) As DataTable
        ...
        qry = " select distinct * from (" & _
              " select distinct TAX1 as Tax_RateType_Name,TAX1_Rate as Tax_RateType_Rate,sum(TAX1_Amt) as Tax_RateType_Amount  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX1,TAX1_Rate " & _
              " union all " & _
              " select distinct TAX2,TAX2_Rate,sum(TAX2_Amt) as TAX2_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX2,TAX2_Rate " & _
              " union all " & _
              " select distinct TAX3,TAX3_Rate,sum(TAX3_Amt) as TAX3_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX3,TAX3_Rate " & _
              " union all " & _
              " select distinct TAX4,TAX4_Rate,sum(TAX4_Amt) as TAX4_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX4,TAX4_Rate " & _
              " union all " & _
              " select distinct TAX5,TAX5_Rate,sum(TAX5_Amt) as TAX5_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX5,TAX5_Rate " & _
              " union all " & _
              " select distinct TAX6,TAX6_Rate,sum(TAX6_Amt) as TAX6_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX6,TAX6_Rate " & _
              " union all " & _
              " select distinct TAX7,TAX7_Rate,sum(TAX7_Amt) as TAX7_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX7,TAX7_Rate " & _
              " union all " & _
              " select distinct TAX8,TAX8_Rate,sum(TAX8_Amt) as TAX8_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX8,TAX8_Rate " & _
              " union all " & _
              " select distinct TAX9,TAX9_Rate,sum(TAX9_Amt) as TAX9_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX9,TAX9_Rate " & _
              " union all " & _
              " select distinct TAX10,TAX10_Rate,sum(TAX10_Amt) as TAX1_Amt  from TSPL_EX_COMMERCIAL_INVOICE_HEAD where DOCUMENT_CODE='" & DocNo & "' group by TAX10,TAX10_Rate " & _
    ...
    Private Function SetItemWiseTax(ByVal dtAfterModify As DataTable, ByVal strShipFrm As String) As DataTable
        ...
        qry += " from TSPL_EX_COMMERCIAL_INVOICE_HEAD where Document_Code='" + strShipFrm + "' "
        qry += " union all "
    ...
    Private Sub txtReqNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtReqNo._MYValidating
        ''richa agarwal 21/04/2015
        ' If (clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmCommercialInvoiceMT) = CompairStringResult.Equal AndAlso rdbAgainstWithoutLC.IsChecked) Or (clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXCommercialInvoice) = CompairStringResult.Equal) Then
        SelectMRNItems()
        ' Else
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        Try
            ...
            gv1.Rows(IntRowNo).Cells(colTotItemWt).Value = Math.Round(clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colConvF).Value) * clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colItemWeight).Value) * dblQty, 2)
            ...
            If clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colNo_Kind_Package).Value) > 0 Then
                gv1.Rows(IntRowNo).Cells(colNo_Kind_Package).Value = Math.Round(clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colConvF).Value) * dblQty, 2)
            End If
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("EXCOMMEMPFND", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
    ...
    Private Sub txtCurrencyCode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtCurrencyCode._MYValidating
        ...
        txtCurrencyCode.Value = clsCommon.ShowSelectForm("EXCOMMCURFND", qry, "Code", "", txtCurrencyCode.Value, "CURRENCY_CODE", isButtonClicked)
        ShowCurrencyDetail()
    End Sub
    ...
    Private Sub btnDrillDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDrillDown.Click
        If clsCommon.myLen(txtReqNo.Value) > 0 Then
            clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXPorformaInvoice, txtReqNo.Value)
        Else
    ...
    Private Sub btnReverseAndUnpost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverseAndUnpost.Click
        Try
            If common.clsCommon.MyMessageBoxShow(Me, "Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsEXCommercialInvoiceHead.ReverseAndUnpost(txtDocNo.Value) Then
...
#End Region
    ...
    Private Sub btnDeliveredTo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDeliveredTo.Click
        ...
        Try
            ...
            End If
            If common.clsCommon.MyMessageBoxShow(Me, "Are you sure to deliver Shipment?", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                Dim qry = "Update TSPL_EX_COMMERCIAL_INVOICE_HEAD set Is_Delivered=1  where Document_Code='" + txtDocNo.Value + "'"
    ...
    Private Sub RadMenuItem6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem6.Click
        'PACKING lIST Ticket No : UDL/28/05/19-000299 By Prabhakar
        If clsCommon.myLen(txtDocNo.Value) <= 0 Then
        ...
        Else
            ...
            If dt.Rows.Count > 0 Then
                ...
                If clsCommon.CompairString(cmbDocType.SelectedValue, "EX") = CompairStringResult.Equal Then
                    frmCRV.funreport(CrystalReportFolder.NewSalesReports, dt, "crptExportPackingList", "Packing List", clsCommon.myCDate(dt.Rows(0)("Document_Date")))
                ElseIf clsCommon.CompairString(cmbDocType.SelectedValue, "MT") = CompairStringResult.Equal Then
                    frmCRV.funreport(CrystalReportFolder.NewSalesReports, dt, "crptExportPackingListMT", "Packing List", clsCommon.myCDate(dt.Rows(0)("Document_Date")))
                End If
                frmCRV = Nothing
    ...
    Private Sub RadMenuItem7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem7.Click
        InvoicePrint()
    End Sub
    Private Sub InvoicePrint(Optional ByVal IsPdf As Boolean = False)
        If clsCommon.myLen(txtDocNo.Value) <= 0 Then
            clsCommon.MyMessageBoxShow(Me, "No data found.", Me.Text)
        Else
            Dim dt As DataTable = funPrint(txtDocNo.Value)
    ...
    Private Sub TxtMTPONo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles TxtMTPONo._MYValidating
        SelectPurchaseItems()
    End Sub
    ...
    Function CancelData() As Boolean
        Try
            ...
            If clsCommon.MyMessageBoxShow(Me, "Are you sure to Cancel the Record?", "", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.No Then
                Exit Function
            End If
            Dim count As Integer = clsDBFuncationality.getSingleValue("select count(*)  from TSPL_SD_SALE_INVOICE_HEAD where TSPL_SD_SALE_INVOICE_HEAD.Against_Com_Inv_No ='" + txtDocNo.Value + "' ")
            If count > 0 Then
                clsCommon.MyMessageBoxShow(Me, "You can't cancelled because this documnet used in Merchant/Export Invoice")
                Exit Function
            End If

            clsEXCommercialInvoiceHead.CancelData(Me.Form_ID, txtDocNo.Value, IIf(clsCommon.CompairString(clsUserMgtCode.frmCommercialInvoiceMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
