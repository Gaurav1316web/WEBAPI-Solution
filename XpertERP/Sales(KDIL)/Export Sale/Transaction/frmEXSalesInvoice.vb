...
''updation by richa agarwal against ticket no BM00000003745(add condition in customer finder),BM00000006405,BM00000007593,BM00000007640,BM00000007686,BM00000008158,UDL/20/06/18-000192
'-BM00000003441
'=============BM00000003471
'' Work done agaist ticket no. ALF/19/07/18-000078  by Parteek
Imports common
...
Public Class frmEXSalesInvoice
...
#Region "Variables"
    ...
    'Public Const clsItemRowType.RowTypeItem As String = "Item"
    'Public Const clsItemRowType.RowTypeItem As String = "Misc"

    Public CHADetailsMandatoryOnExportSale As Boolean = False
    ...
    Dim EnableGSTRelatedfields As Double = 0
...
#End Region
    ...
    Private Sub LOCATIONRIGTHS()
        ...
        Try
            ...
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Default_LocCode) > 0 Then
                ...
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXSalesInvoice) = CompairStringResult.Equal Then
                    WhrCls = " and Location_Type='Physical'  "
                ElseIf clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesInvoiceMT) = CompairStringResult.Equal Then
    ...
    Private Sub LoadComm_Amount()
        Dim qry As String = "select '' as Code,'None' as Name union all select 'P' as Code,'Pers(%)' as Name union all select 'A' as Code,'Amount' as Name"
    ...
    Public Sub SetUserMgmtNew()
        'Ticket No : KDI/21/08/18-000417 For User Permission 
    ...
    Private Sub FrmAPInvoiceEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            CHADetailsMandatoryOnExportSale = IIf(clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Description from TSPL_FIXED_PARAMETER where Code='" & clsFixedParameterCode.CHADetailsMandatoryOnExportSale & "'")) = 0, False, True)
            ...
            EnableGSTRelatedfields = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.EnableGSTRelatedfields, clsFixedParameterCode.EnableGSTRelatedfields, Nothing))
            If EnableGSTRelatedfields = 1 Then
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXSalesInvoice) = CompairStringResult.Equal Then
                    lblExportInvoiceType.Visible = True
                    cmbExportInvoiceType.Visible = True
                Else
                    lblExportInvoiceType.Visible = False
                    cmbExportInvoiceType.Visible = False
                End If
            Else
                lblExportInvoiceType.Visible = False
                cmbExportInvoiceType.Visible = False
    ...
    Sub SetMultiCurrencyVisibility()
        Try
            ...
            Else
                ...
                Throw New Exception("Do multicurrency on for Export Sales Invoice.")
            End If
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    ...
    Sub LoadBlankGrid()
        ...
        ''ALF/09/04/19-000097
        'If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesInvoiceMT) = CompairStringResult.Equal Then
        ...
        repoRequition.HeaderText = "Export Commercial No"
            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colTotTaxAmt) Then
                        ...
                        UpdateAllTotals()
                    End If
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colSchemeApplicable) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colUnit) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal) Then
                        If (e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colHeadDiscamt) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse e.Column Is gv1.Columns(colDisPer) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal)) Then
        ...
        Try
            ...
            If (clsCommon.myLen(clsCommon.myCdbl(gv1.CurrentRow.Cells(colICode).Value)) > 0) Then
                ...
                If isBatchAvtive = "1" Then
                    ...
                    frm.strCurrDocType = clsCommon.myCstr(cmbDocType.SelectedValue)
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If CHADetailsMandatoryOnExportSale = True Then
                If clsCommon.myLen(fndCHA_Code.Value) <= 0 Then
                    RadPageView1.SelectedPage = RadPageViewPage1
                    clsCommon.MyMessageBoxShow(Me, "Select CHA Name", Me.Text)
                    fndCHA_Code.Focus()
                    fndCHA_Code.Select()
                    Errorcontrol.SetError(txtCHA_Name, "Select CHA Name")
                    Return False
                Else
                    Errorcontrol.ResetError(txtCHA_Name)
                End If
            Else
                fndCHA_Code.MendatroryField = False
            End If


            If EnableGSTRelatedfields = 1 Then
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXSalesInvoice) = CompairStringResult.Equal Then
                    If clsCommon.CompairString(cmbExportInvoiceType.SelectedValue, "") = CompairStringResult.Equal Then
                        RadPageView1.SelectedPage = RadPageViewPage5
                        clsCommon.MyMessageBoxShow(Me, "Select Export Invoice Type", Me.Text)
                        cmbExportInvoiceType.Focus()
                        cmbExportInvoiceType.Select()
                        Errorcontrol.SetError(cmbExportInvoiceType, "Select Export Invoice Type")
                        Return False
                    Else
                        Errorcontrol.ResetError(cmbExportInvoiceType)
                    End If
                End If
            End If
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
                    If clsCommon.CompairString(strItemType, cboItemType.SelectedValue) <> CompairStringResult.Equal Then
                        ...
                        Throw New Exception("Fill item " + strIName + " is not of type " + strItemType + " at row no. " + clsCommon.myCstr(ii + 1) + "")
                    End If
                    ...
                    If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(strSchemeItem, "No") = CompairStringResult.Equal Then
                        ...
                        ''richa agarwal 11 March,2019 to check if item is batch wise then qty is entered or not UDL/11/03/19-000277
            ...
            clsEXCommercialInvoiceHead.IsValidCustomer(arrReqNo, txtVendorNo.Value)
            clsEXCommercialInvoiceHead.IsValidDocumentType(arrReqNo, cmbDocType.SelectedValue)
    ...
    Sub SaveData(ByVal ChekPostBtn As Boolean)
        Dim obj As New clsEXSaleInvoiceHead
        Try
            If (AllowToSave()) Then

                obj = New clsEXSaleInvoiceHead
                ...
                obj.ExportInvoiceType = clsCommon.myCstr(cmbExportInvoiceType.SelectedValue)
                ...
                obj.Final_Gross_Wt = clsCommon.myCdbl(TxtFinalGrossWeight.Value)
                ...
                ''richa agarwal 05/08/2015
                obj.VehicleNo = clsCommon.myCstr(txtVehicleNumber.Text)
                obj.GRNo = clsCommon.myCstr(TxtGrNo.Text)
                If clsCommon.myLen(TxtGrNo.Text) > 0 Then
                    obj.GR_Date = TxtGRDate.Value
                End If
                obj.WayBillNo = clsCommon.myCstr(TxtWayBillNo.Text)
                If clsCommon.myLen(TxtWayBillNo.Text) > 0 Then
                    obj.WayBillDate = TxtWayBillDate.Value
                End If
                obj.Transport_Code = clsCommon.myCstr(FndTransporter.Value)
                obj.Transporter_Name = clsCommon.myCstr(lbltransporterName.Text)
                ''---------------------
                ...
                obj.Import_Export_No = txtIm_Ex_No.Text
                ...
                obj.LUT_No = clsCommon.myCstr(txtLUTNo.Text)
                obj.Arr = New List(Of clsEXSaleInvoiceDetail)
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsEXSaleInvoiceDetail()
                ...
                '====================notify party=================
                obj.Arr_Notify = New List(Of clsEXSaleInvoiceNotifyDetail)

                For Each grow As GridViewRowInfo In gv_Notify_Party.Rows
                    Dim objtr As New clsEXSaleInvoiceNotifyDetail()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Try
            ...
            obj = clsEXSaleInvoiceHead.GetData(strCode, "'T','R'", arrLoc, NavTyep, IIf(clsCommon.CompairString(clsUserMgtCode.frmSalesInvoiceMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                txtLUTNo.Text = obj.LUT_No
                txtVendorPONo.Text = obj.MT_Against_PO
                ''richa KDI/15/03/18-000127 fetch po date 
                txtVendorPODate.Text = clsCommon.myCstr(obj.MT_Against_PO_Date)
                ''----------
                cmbExportInvoiceType.SelectedValue = obj.ExportInvoiceType
                If clsCommon.myLen(obj.MT_Against_PO_Date) > 0 Then
                ...
                ''richa agarwal 05/08/2015
                txtVehicleNumber.Text = obj.VehicleNo
                ...
                Else
                    TxtWayBillDate.Value = clsCommon.GETSERVERDATE()
                End If
                FndTransporter.Value = obj.Transport_Code
                ...
                txtIm_Ex_No.Text = obj.Import_Export_No
                ...
                If clsERPFuncationality.GetEInvoiceStatus(txtDate.Value) = True Then 'AndAlso clsCommon.CompairString(EInvoiceType, "BB") = CompairStringResult.Equal
                    btnReverseAndUnpost.Enabled = False
                    If obj.Status = ERPTransactionStatus.Approved Then
                        btnCancel.Enabled = True
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsEXSaleInvoiceDetail In obj.Arr
                        gv1.Rows.AddNew()
                        ...
                        If clsCommon.myLen(objTr.PI_CODE) > 0 Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXCommercialInvoiceDetail.GetBalanceSRNQty(objTr.PI_CODE, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable))
                        End If
                ...
                If obj.Arr_Notify IsNot Nothing AndAlso obj.Arr_Notify.Count > 0 Then
                    For Each objtr As clsEXSaleInvoiceNotifyDetail In obj.Arr_Notify
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                If (clsEXSaleInvoiceHead.PostData(MyBase.Form_ID, txtDocNo.Value, arrLoc)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsEXSaleInvoiceHead.DeleteData(txtDocNo.Value, MyBase.Form_ID)) Then
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        Dim qry As String = "select Document_Code as Code,CONVERT(varchar(10), Document_Date,103)+' '+ CONVERT(varchar(5), Document_Date,114) as Date,(case when document_type='MT' then 'Merchant Trade' else 'Export Sale' end) as [Document type],Customer_Code as [Customer Code], Customer_Name as Customer,TSPL_SD_SALE_INVOICE_HEAD.Comments,Total_Amt as Amount,case when TSPL_SD_SALE_INVOICE_HEAD.Status=0 then 'Pending' else 'Approved' end as [Status],Against_Com_Inv_No as [Export Commercial No] from TSPL_SD_SALE_INVOICE_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALE_INVOICE_HEAD.Customer_Code  "

        Dim whrClas As String = ""
        If clsCommon.CompairString(clsUserMgtCode.frmSalesInvoiceMT, FORMTYPE) = CompairStringResult.Equal Then
            whrClas = " TSPL_SD_SALE_INVOICE_HEAD.document_type='MT' "
        Else
            whrClas = " TSPL_SD_SALE_INVOICE_HEAD.document_type='EX' "
        End If

        If clsCommon.myLen(arrLoc) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas += " and Bill_To_Location in (" + arrLoc + ") and TSPL_SD_SALE_INVOICE_HEAD.Customer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(arrLoc) > 0 Then
            whrClas += " and Bill_To_Location in (" + arrLoc + ") "
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas += " and TSPL_SD_SALE_INVOICE_HEAD.Customer_Code in (" + strwherecls + ") "
        End If
        If clsCommon.myLen(whrClas) > 0 Then
            whrClas = whrClas & " AND "
        End If
        whrClas = whrClas & "  TSPL_SD_SALE_INVOICE_HEAD.trans_type='EXP'"


        LoadData(clsCommon.ShowSelectForm("EXSALEINVCOMFND", qry, "Code", whrClas, txtDocNo.Value, "TSPL_SD_SALE_INVOICE_HEAD.Document_Date", isButtonClicked, "TSPL_SD_SALE_INVOICE_HEAD.Document_Date"), NavigatorType.Current)
    End Sub
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBillToLocation._MYValidating
        ...
        txtBillToLocation.Value = clsCommon.ShowSelectForm("EXSALEINVLOCFND", qry, "Code", WhrCls, txtBillToLocation.Value, "Code", isButtonClicked)
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating
        ...
        txtShipToLocation.Value = clsCommon.ShowSelectForm("EXSALEINVSHIPFND", qry, "Code", "Ship_To_Type_Code='" & clsCommon.myCstr(txtVendorNo.Value) & "' and loc_code='" & clsCommon.myCstr(txtBillToLocation.Value) & "'", txtShipToLocation.Value, "Code", isButtonClicked)
    ...
    Sub SelectMRNItems()
        ...
        frm.Text = "Pending Commercial Invoice"
        ...
        Dim objOrderHead As New clsEXCommercialInvoiceHead
        objOrderHead.Arr = New List(Of clsEXCommercialInvoiceDetail)
        objOrderHead.Arr_Notify = New List(Of clsEXCommercialInvoiceNotifyDetail)

        If frm.ArrReturn_EX_Comm IsNot Nothing AndAlso frm.ArrReturn_EX_Comm.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn_EX_Comm(0).Document_Code) > 0 Then
                objOrderHead = clsEXCommercialInvoiceHead.GetData(frm.ArrReturn_EX_Comm(0).Document_Code, "'T','R'", NavigatorType.Current, arrLoc, frm.strExport_Merchant)
            ...
            If objOrderHead.Arr_Notify IsNot Nothing AndAlso objOrderHead.Arr_Notify.Count > 0 Then
                For Each objtr As clsEXCommercialInvoiceNotifyDetail In objOrderHead.Arr_Notify
            ...
            For ii As Integer = 0 To frm.ArrReturn_EX_Comm.Count - 1
                If clsCommon.myLen(frm.ArrReturn_EX_Comm(ii).Document_Code) > 0 Then
                    Dim strCode As String = frm.ArrReturn_EX_Comm(ii).Document_Code
                    objOrderHead = clsEXCommercialInvoiceHead.GetData(frm.ArrReturn_EX_Comm(ii).Document_Code, "'T','R'", NavigatorType.Current, arrLoc, frm.strExport_Merchant)
                    For Each obj As clsEXCommercialInvoiceDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn_EX_Comm(ii).Item_Code AndAlso obj.Scheme_Item = "N") OrElse (obj.Scheme_Code = frm.ArrReturn_EX_Comm(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            If IsValidItem(obj) Then
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = frm.ArrReturn_EX_Comm(ii).Balance_Qty 'obj.Balance_Qty

                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = frm.ArrReturn_EX_Comm(ii).Balance_Qty
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsEXCommercialInvoiceDetail In objOrderHead.Arr
    ...
    Function IsValidItem(ByVal obj As clsEXCommercialInvoiceDetail)
    ...
    Public Function funPrint(ByVal strDocNo As String) As DataTable
        ...
        Try
            ...
            '' end            
            Dim Qry As String = "select TSPL_SD_SALE_INVOICE_HEAD.LUT_No, TSPL_ITEM_MASTER.HSN_Code,TSPL_LOCATION_MASTER.GSTNO as Loc_GST_No,Loc_for_state.GST_STATE_Code as Loc_GST_StateCode,TSPL_CUSTOMER_MASTER.GSTNO as Cust_GST_No,Customer_for_state.GST_STATE_Code as Customer_StateCode,TSPL_CURRENCY_MASTER.CURRENCY_SIGN, TSPL_SD_SALE_INVOICE_HEAD.EX_Term_Code as EX_Term_Name, Loc_for_city.City_Name as Loc_City_Name,Loc_for_state .STATE_NAME as Loc_state_Name, TSPL_COMPANY_MASTER .City_Code as Comp_City_Name,Comp_for_State.STATE_NAME as Comp_State_name, TSPL_COMPANY_MASTER.Pincode as Comp_Pin_No,TSPL_COMPANY_MASTER.IECode,tspl_vendor_bank_master.bank_code,tspl_vendor_bank_master.bank_name,tspl_vendor_bank_master.add1 as Bank_add1,tspl_vendor_bank_master.add2 as Bank_add2,tspl_vendor_bank_master.add3 as Bank_add3,tspl_vendor_bank_master.country_code,tspl_vendor_bank_master.state_code as Bank_state_code,TSPL_STATE_MASTER_For_Bank.STATE_NAME as Bank_state_Name,tspl_vendor_bank_master.city_code as Bank_City_Code,TSPL_CITY_MASTER_for_bank.City_Name as Bank_City_Name,tspl_vendor_master.IFSC_Code as ifsc_code,tspl_vendor_master.account_no, tspl_purchase_order_head.MT_Beneficiary_code,TSPL_COMPANY_MASTER.IECode,TSPL_COMPANY_MASTER.Pan_No as Comp_Pan_No,  Case when len(ISNULL(TSPL_SHIP_TO_LOCATION.telphone,''))>0 and TSPL_SHIP_TO_LOCATION.telphone='(+__)__________' then '' else ' '+TSPL_SHIP_TO_LOCATION.telphone end "
            Qry += "	 as Ship_Location_Phone, TSPL_SD_SALE_INVOICE_HEAD.Incentive_Declaration,TSPL_SD_SALE_INVOICE_DETAIL.packing_instruction,TSPL_SD_SALE_INVOICE_DETAIL.Shipping_mark,TSPL_SD_SALE_INVOICE_DETAIL.specification ,tspl_item_master.itf_code as iecode,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code1 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt1 ,"
            Qry += "TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code2 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt2,"
            Qry += "TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code3 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt3,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code4 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt4,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code5 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt5,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code6 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt6,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code7 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt7,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code8 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt8,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code9 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt9,"
            Qry += " TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Code10 ,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Amt10 ,TSPL_LOCATION_MASTER.Location_Desc,TSPL_SD_SALE_INVOICE_HEAD. Total_Add_Charge,TSPL_SD_SALE_INVOICE_DETAIL.Total_Disc_Amt,'" + strSoNo + "' as SONo, '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_LOCATION_MASTER.Add1 as Location_Add1,TSPL_LOCATION_MASTER.Add1 as [Location Address],TSPL_LOCATION_MASTER.Add2 as Location_Add2,TSPL_LOCATION_MASTER.Add3 as Location_Add3,TSPL_LOCATION_MASTER.Add4 as Location_Add4,TSPL_LOCATION_MASTER.TIN_No ,Case when len(ISNULL(TSPL_LOCATION_MASTER.Phone1,''))>0 and TSPL_LOCATION_MASTER.Phone1='(+__)__________' then '' else ' '+TSPL_LOCATION_MASTER.Phone1 end "
            Qry += "   +  Case When ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'(+__)__________' Then '  '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as Location_Phone  ,TSPL_SHIP_TO_LOCATION.Pin_Code,TSPL_CITY_MASTER.STATE_CODE  ,TSPL_CITY_MASTER.City_Name,"
            ...
            Qry += " TSPL_SD_SALE_INVOICE_DETAIL.item_code as item_code,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION as PrincipleDesc,TSPL_CUSTOMER_ITEM_MAPPING.CUSTOMER_ITEM_NO, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_SD_SALE_INVOICE_DETAIL.Row_Type,case when isnull (TSPL_BATCH_ITEM.Qty,0) >0 then TSPL_BATCH_ITEM.Qty else TSPL_SD_SALE_INVOICE_DETAIL.Qty end as qty,TSPL_SD_SALE_INVOICE_DETAIL.unit_code as uom,(TSPL_SD_SALE_INVOICE_DETAIL.item_cost * TSPL_SD_SALE_INVOICE_HEAD.convrate) as itemcost,((case when isnull (TSPL_BATCH_ITEM.Qty,0) >0 then isnull( TSPL_BATCH_ITEM.Qty,0) else isnull(TSPL_SD_SALE_INVOICE_DETAIL.Qty,0) end) * TSPL_SD_SALE_INVOICE_DETAIL.item_cost * TSPL_SD_SALE_INVOICE_HEAD.convrate) as amount,TSPL_SD_SALE_INVOICE_detail.Item_Cost as Item_Rate,TSPL_SD_SALE_INVOICE_HEAD.TAX1,TSPL_SD_SALE_INVOICE_HEAD.TAX2,TSPL_SD_SALE_INVOICE_HEAD.TAX3,TSPL_SD_SALE_INVOICE_HEAD.TAX4,TSPL_SD_SALE_INVOICE_HEAD.TAX5,TSPL_SD_SALE_INVOICE_HEAD.Total_Add_Charge,TSPL_SD_SALE_INVOICE_DETAIL.Batch_No,cast(datepart(MONTH,TSPL_SD_SALE_INVOICE_DETAIL.MFG_Date) as varchar(2)) + '/' + "  ' Change by Prabhakar 17/11/2016
            ...
            Qry += " SUBSTRING(cast(datepart(YYYY,TSPL_SD_SALE_INVOICE_DETAIL.Expiry_Date) as varchar(4)),3,2) as Exp_Date,TSPL_SD_SALE_INVOICE_DETAIL.mrp,TSPL_SD_SALE_INVOICE_DETAIL.Scheme_Item,TSPL_SD_SALE_INVOICE_DETAIL.Item_Weight,(case when COALESCE(TSPL_SD_SALE_INVOICE_HEAD.Invoice_Type,'R')='T' then 'Tax Invoice' else 'Retail Invoice' end) as Invoice_Type " & colsTaxRateType & " "
            Qry += " " & QryShowStatus & " "
            Qry += " " & SerialNoColumn & " "

            Qry += "  ,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add1  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add1 end as P_Add1, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add2  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add2 end as P_Add2, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add3  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add3 end as P_Add3, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .PIN_Code   when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Pin_No  end as P_PinNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .CST    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_CST_No"
            Qry += " end as P_CstNo,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Tin_No     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .p_Tin_No   end as P_TinNo,case when coalesce(p_cust.P_GSTNo,'')='' then TSPL_CUSTOMER_MASTER  .GSTNO  when coalesce(p_cust.P_GSTNo,'')<>'' then p_cust .P_GSTNo end as P_GSTNo,case when coalesce(p_cust.P_GST_StateCode,'')='' then Customer_for_state.GST_STATE_Code  when coalesce(p_cust.P_GST_StateCode,'')<>'' then p_cust .P_GST_StateCode end as P_GST_StateCode, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Email    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Email  end as P_Email,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Fax     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Fax   end as P_Fax, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER.Lst_No when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_LST_No    end as P_LstNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Cust_Code      when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_code  "
            Qry += "  end as P_CustCode, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Customer_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_name    end as P_Cust_Name, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CITY_MASTER_Cust.City_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_City_Name    end as P_City_Name, tspl_location_master.HOAdd1 ,TSPL_LOCATION_MASTER .HOAdd2,TSPL_SD_SALE_INVOICE_HEAD.Terms_Code as TermCondition, WayBillNo,convert(varchar,WayBillDate,103) as WayBillDate,TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_No as Po_No,convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_Date,103) as Po_Date,TSPL_SD_SALE_INVOICE_HEAD.GRNo,convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.GR_Date,103) as GR_Date,TSPL_SD_SALE_INVOICE_HEAD.VehicleNo,TSPL_CITY_MASTER.City_Name as Cust_City_Name,Transporter_Name,TSPL_SD_SALE_INVOICE_DETAIL.Alter_UnitQty,TSPL_SD_SALE_INVOICE_DETAIL.RATE_UOM ,TSPL_SD_SALE_INVOICE_DETAIL.Scheme_Qty,isnull(TSPL_SD_SALE_INVOICE_HEAD.Amount_Less_Discount* TSPL_SD_SALE_INVOICE_HEAD.convrate,0) as Amount_Less_Discoun,"
            Qry += "  TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name2,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name3,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name4,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name5,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name6,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name7,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name8,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name9,TSPL_SD_SALE_INVOICE_HEAD.Add_Charge_Name10,(TSPL_SD_SALE_INVOICE_HEAD.Total_Amt * TSPL_SD_SALE_INVOICE_HEAD.convrate) as Total_Amt,case when (Dispatch_Terms )='FE' then 'Freight Extra' else  Dispatch_Terms  end  as Terms_of_delivery,stuff((select ',' + Against_Com_Inv_No from TSPL_SD_SALE_INVOICE_head where Document_Code = '" + strDocNo + "'  for xml path('')),1,1,'') as Commercial_Inv_No,(select convert(varchar,Document_Date,103) as Document_Date from TSPL_EX_COMMERCIAL_INVOICE_HEAD where Document_Code = Against_Com_Inv_No ) as Commercial_Inv_Date"
            Qry += " ,TSPL_BATCH_ITEM.Batch_No as Batch_Name "  ' Change By Prabhakar 17/11/2016
            Qry += " from TSPL_SD_SALE_INVOICE_DETAIL  "
            Qry += " " & SerialNo & " "
            Qry += " left outer join TSPL_BATCH_ITEM on TSPL_BATCH_ITEM.Item_Code =  TSPL_SD_SALE_INVOICE_DETAIL.Item_Code and TSPL_BATCH_ITEM.Document_Code=TSPL_SD_SALE_INVOICE_DETAIL.DOCUMENT_CODE "  ' Change by Prabhakar Anand 17/11/2016
            ...
            Qry += "  left outer join tspl_purchase_order_head on tspl_purchase_order_head.purchaseorder_no=TSPL_SD_SALE_INVOICE_HEAD.CUST_PO_NO"
            Qry += "	 left outer join tspl_vendor_master on tspl_vendor_master.vendor_code=tspl_purchase_order_head.MT_Beneficiary_code"
            Qry += " left outer join tspl_vendor_bank_master on tspl_vendor_bank_master.bank_code=tspl_vendor_master.bank_code"
            Qry += " left outer join TSPL_STATE_MASTER as TSPL_STATE_MASTER_For_Bank on TSPL_STATE_MASTER_For_Bank.STATE_CODE =tspl_vendor_bank_master.state_code"
            Qry += " left outer join TSPL_CITY_MASTER  as TSPL_CITY_MASTER_for_bank on TSPL_CITY_MASTER_for_bank.City_Code =tspl_vendor_bank_master.city_code"
            ...
            Qry += "  left outer join TSPL_STATE_MASTER as Comp_for_State on Comp_for_State.STATE_CODE =TSPL_COMPANY_MASTER.State "
            Qry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_SD_SALE_INVOICE_HEAD.Customer_Code   "
            Qry += " left outer join TSPL_CITY_MASTER as TSPL_CITY_MASTER_Cust on TSPL_CITY_MASTER_Cust.City_Code=TSPL_CUSTOMER_MASTER.City_Code "
            ...
            Qry += " left outer join TSPL_CITY_MASTER as Loc_for_city on Loc_for_city.City_Code =TSPL_LOCATION_MASTER.City_Code "
            Qry += " left outer join TSPL_STATE_MASTER  as Loc_for_state on Loc_for_state.STATE_CODE =TSPL_LOCATION_MASTER.State "
            Qry += " left outer join TSPL_STATE_MASTER  as Customer_for_state on Customer_for_state.STATE_CODE =TSPL_Customer_MASTER.State "
            ...
            ' Qry += " left join TSPL_ITEM_CATEGORY_LEVEL_VALUES on TSPL_EX_PI_DETAIL.PrincipleCode=TSPL_ITEM_CATEGORY_LEVEL_VALUES.CODE "
            ...
            ''======================================================
            Qry += " left join TSPL_EX_COMMERCIAL_INVOICE_HEAD on TSPL_EX_COMMERCIAL_INVOICE_HEAD.Document_Code =TSPL_SD_SALE_INVOICE_HEAD.Against_Com_Inv_No "
            Qry += " left join TSPL_EX_COMMERCIAL_INVOICE_DETAIL on TSPL_EX_COMMERCIAL_INVOICE_DETAIL.DOCUMENT_CODE =TSPL_SD_SALE_INVOICE_DETAIL.Commercial_Inv_No and TSPL_SD_SALE_INVOICE_DETAIL.Item_Code =TSPL_EX_COMMERCIAL_INVOICE_DETAIL.Item_Code"
    ...
    Private Function SetItemWiseTax(ByVal dtAfterModify As DataTable, ByVal strShipFrm As String) As DataTable
        ...
        qry += " from TSPL_SD_SALE_INVOICE_HEAD where Document_Code='" + strShipFrm + "' "
        qry += " union all "
        ...
        Try
            If e.Column.Index >= 0 Then
                ...
                ElseIf e.Column Is gv1.Columns(colDisPer) Then
                    ...
                    End If
                ElseIf e.Column Is gv1.Columns(colAmt) Then
                    If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal OrElse clsCommon.myCdbl(gv1.CurrentRow.Cells(colIsMannualAmt).Value) = 1 Then
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("EXSALEINVEMPFND", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
    ...
    Private Sub txtCurrencyCode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtCurrencyCode._MYValidating
        ...
        txtCurrencyCode.Value = clsCommon.ShowSelectForm("EXSALEINVCURFND", qry, "Code", "", txtCurrencyCode.Value, "CURRENCY_CODE", isButtonClicked)
        ShowCurrencyDetail()
    End Sub
    ...
    Private Sub btnDrillDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDrillDown.Click
        If clsCommon.myLen(txtReqNo.Value) > 0 Then
            If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesInvoiceMT) = CompairStringResult.Equal Then
                clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXCommercialInvoice, txtReqNo.Value)
            Else
                clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXCommercialInvoice, txtReqNo.Value)
            End If
...
#Region "SMS Email Setting"
    ...
    Public Function GetAtachmntPrint(ByVal strDocNo As String)
        ...
        Dim colsTaxRateType As String = GetColumnsForTaxRateType(GetTaxRateTypeDT(strDocNo))
        '' end 
        '' shivani against[BM00000007873] 
        atchqry = "  select '" + strSoNo + "' as SONo, '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_LOCATION_MASTER.Add1 as Location_Add1,TSPL_LOCATION_MASTER.Add2 as Location_Add2,TSPL_LOCATION_MASTER.Add3 as Location_Add3,TSPL_LOCATION_MASTER.TIN_No ,ISNULL(TSPL_LOCATION_MASTER.Phone1,'')+ Case When ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'' Then ', '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as Location_Phone, case when TSPL_SHIP_TO_LOCATION.Ship_To_Desc='' then TSPL_CUSTOMER_MASTER.Customer_Name else TSPL_SHIP_TO_LOCATION.Ship_To_Desc end as shipName , case when TSPL_SHIP_TO_LOCATION.add1='' then TSPL_CUSTOMER_MASTER.Add1 else TSPL_SHIP_TO_LOCATION.add1 end as ship_Add1 ,case when  TSPL_SHIP_TO_LOCATION.Add2='' then TSPL_CUSTOMER_MASTER.Add2 else TSPL_SHIP_TO_LOCATION.Add2 end as ship_add2 ,case when  TSPL_SHIP_TO_LOCATION.Add3='' then TSPL_CUSTOMER_MASTER.Add3 else TSPL_SHIP_TO_LOCATION.Add3 end as ship_add3 ,TSPL_SHIP_TO_LOCATION.Pin_Code,TSPL_CITY_MASTER.STATE_CODE  ,City_Name,"
...
#End Region
    ...
    Private Sub InvoicePrint(Optional ByVal IsPdf As Boolean = False)
        ...
        Else
            ...
            If dt.Rows.Count > 0 Then
                ...
                If objCommonVar.GSTApplicableDate <= clsCommon.myCDate(clsCommon.GetPrintDate(txtDate.Value, "dd/MM/yyyy")) Then
                    Dim Qry As String = GetQuery(txtDocNo.Value)
                    Qry += " where TSPL_SD_SALE_INVOICE_HEAD.Document_Code ='" + txtDocNo.Value + "'"
    ...
    Private Sub FndTransporter__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles FndTransporter._MYValidating
        FndTransporter.Value = clsCommon.ShowSelectForm("TransportNameEx", "select Transport_Id as Code,Transporter_Name as Name from TSPL_TRANSPORT_MASTER ", "Code", "", FndTransporter.Value, "Code", isButtonClicked)
        lbltransporterName.Text = clsDBFuncationality.getSingleValue("Select Transporter_Name from TSPL_TRANSPORT_MASTER where Transport_Id  ='" + FndTransporter.Value + "' ")
    End Sub
    ...
    Public Function GetQuery(Optional ByVal strDocNo As String = Nothing) As String
        ...
        Qry = "select cast(TSPL_SD_SALE_INVOICE_HEAD.BarCode_Img as image) As BarCode_Img,isnull (TSPL_SD_SALE_INVOICE_HEAD.IRN_No,'') as IRN_No,isnull (TSPL_SD_SALE_INVOICE_HEAD.Ack_No,'') as Ack_No,case when len(isnull (TSPL_SD_SALE_INVOICE_HEAD.Ack_No,'')) > 0 then convert (varchar, TSPL_SD_SALE_INVOICE_HEAD.Ack_Date,103) else ''  end as Ack_Date, case when convert(date ,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103)>=convert(date ,'" + clsCommon.myCstr(DateOfEInvoiceImplementation) + "',103) then 1 else 0 end as  IsEInvoiceApply," &
                    " TSPL_SD_SALE_INVOICE_HEAD.LUT_No,TSPL_SD_SALE_INVOICE_HEAD.Carrier as place_of_pre_carriage,TSPL_SD_SALE_INVOICE_HEAD.Pre_Carriage_By ,TSPL_SD_SALE_INVOICE_HEAD.Loading_Port,TSPL_SD_SALE_INVOICE_HEAD.Discharge_Port ,TSPL_SD_SALE_INVOICE_HEAD.Final_Destination ,TSPL_SD_SALE_INVOICE_HEAD.Final_Gross_Wt ,TSPL_SD_SALE_INVOICE_HEAD.Gross_Wt , TSPL_SD_SALE_INVOICE_HEAD.CONVRATE,TSPL_SD_SALE_INVOICE_HEAD.CURRENCY_CODE, case when  TSPL_SD_SHIPMENT_HEAD.HeadDisc_Per >0 then TSPL_SD_SHIPMENT_HEAD.HeadDisc_PerAmt else TSPL_SD_SHIPMENT_HEAD.HeadDisc_Amt end as Invoice_Disc,'" + Bank_Name + "' as Bank_Name,'" + IFSC_Code + "' as IFSC_Code,'" + BANKACCNUMBER + "' as BANKACCNUMBER ,  TSPL_SD_SALE_INVOICE_DETAIL.Line_No, TSPL_SD_SALE_INVOICE_DETAIL.Item_Net_Amt, isnull(TSPL_SD_SALE_INVOICE_head.Invoice_No_For_Supplementary,'') as Invoice_No_For_Supplementary," &
            ...
            "   TSPL_SD_SALE_INVOICE_HEAD.total_add_charge ,TSPL_SD_SALE_INVOICE_HEAD. WayBillNo, case when len( isnull(TSPL_SD_SALE_INVOICE_HEAD.WayBillNo,'')) > 0 then convert (varchar,TSPL_SD_SALE_INVOICE_HEAD.WayBillDate,103) else '' end as WayBillDate,TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ', '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ', '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER_fOR_Comp.City_Name)>0 then ', '+TSPL_CITY_MASTER_fOR_Comp.City_Name else ' ' end + case when len(TSPL_STATE_MASTER_For_Comp.STATE_NAME  )>0 then ', '+ TSPL_STATE_MASTER_For_Comp.STATE_NAME else ' ' end" &
            ...
            " ,TSPL_SD_SALE_INVOICE_HEAD.Against_Shipment_No as ShipmentNo,TSPL_SD_SALE_INVOICE_DETAIL.Alt_Qty ,TSPL_SD_SALE_INVOICE_DETAIL.Alt_UOM,convert(varchar,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) as ShipmentDate," &
             " TSPL_SD_SHIPMENT_HEAD.Against_Delivery_Code as DeliveryOrderNo,TSPL_SD_SALE_INVOICE_HEAD. Against_Com_Inv_No,TSPL_SD_SALE_INVOICE_HEAD.Terms_Code as TermCondition," &
            ...
            " case when coalesce(p_cust.P_cust_code,'')='' then     case when ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')='(+__)__________' then '' else TSPL_CUSTOMER_MASTER.Phone1 end +  Case When   ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_CUSTOMER_MASTER.Phone2 Else'' End   when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Phn    end as P_Cust_Phn,case when coalesce(p_cust.P_Pan,'')='' then TSPL_CUSTOMER_MASTER  .PAN      when coalesce(p_cust.P_Pan,'')<>'' then p_cust.P_Pan   end as P_Pan,TSPL_CUSTOMER_MASTER.Cust_Code  , " &
                "  case when isnull(TSPL_SHIP_TO_LOCATION.Ship_To_Desc,'')='' then   (case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Customer_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_name    end )  else TSPL_SHIP_TO_LOCATION.Ship_To_Desc end as Customer_Name, " &
        " case when isnull(TSPL_SHIP_TO_LOCATION.add1,'')='' then (case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add1  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add1 end )  else TSPL_SHIP_TO_LOCATION.add1 end as Cust_Add1, " &
        " case when  isnull(TSPL_SHIP_TO_LOCATION.Add2,'')='' then (case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add2  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add2 end) else TSPL_SHIP_TO_LOCATION.Add2 end as Cust_add2 ," &
        " case when  TSPL_SHIP_TO_LOCATION.Add3='' then ( case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add3  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add3 end) else TSPL_SHIP_TO_LOCATION.Add3 end as cust_add3 , " &
        " case when ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')='(+__)__________' then '' else TSPL_CUSTOMER_MASTER.Phone1 end +  Case When   ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_CUSTOMER_MASTER.Phone2 Else'' End  as Cust_Phn,TSPL_CUSTOMER_MASTER.Tin_No  as Cust_TinNo ,TSPL_CUSTOMER_MASTER.CST as Cust_CSTNo,TSPL_CUSTOMER_MASTER.Lst_No as Cust_LSTNo,TSPL_CUSTOMER_MASTER.Email as Cust_Email ,TSPL_CUSTOMER_MASTER.PIN_Code as Cust_PinCode,TSPL_CITY_MASTER.City_Name as Cust_City_Name,TSPL_CUSTOMER_MASTER.Fax as Cust_Fax,TSPL_STATE_MASTER .STATE_NAME  as Cust_State_Name,TSPL_CUSTOMER_MASTER.PAN  as Customer_Pan, " &
      " TSPL_SD_SALE_INVOICE_DETAIL.item_code + case when Scheme_Item='Y' then ' (Free Scheme)' when Scheme_Item='N' then '' end as item_code,COALESCE(TSPL_ITEM_MASTER.Item_Desc,TAC.DESCRIPTION) + case when Scheme_Item='Y' then '  ( Free Scheme Not For Sale )' when Scheme_Item='N' then '' end   as itemdesc,TSPL_SD_SALE_INVOICE_DETAIL.Qty as qty" &
           ...
           " left outer join TSPL_STATE_MASTER on TSPL_CUSTOMER_MASTER.State =TSPL_STATE_MASTER.STATE_CODE  " &
             " ) p_cust on p_cust.P_cust_code=TSPL_CUSTOMER_MASTER.Parent_Customer_No and TSPL_CUSTOMER_MASTER.Parent_Customer_YN='N' left outer join  TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code =TSPL_SD_SALE_INVOICE_HEAD .Ship_To_Location " &
    ...
    Function CancelData() As Boolean
        Try
            ...
            If strSaleReturnNo > 0 Then
                Throw New Exception("You cannot cancelled this document because its Sale Return has been created.")
            End If

            Dim strReceiptCount As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select count(receipt_no) from TSPL_RECEIPT_DETAIL where Document_No in (Select Document_No from TSPL_Customer_Invoice_Head  where against_Sale_no='" & txtDocNo.Value & "') "))
            If strReceiptCount > 0 Then
                Throw New Exception("You cannot cancelled this document because receiving has been done against its AR Invoice.")
            End If
            ...
            clsEXSaleInvoiceHead.CancelData(Me.Form_ID, txtDocNo.Value, arrLoc, NavigatorType.Current, IIf(clsCommon.CompairString(clsUserMgtCode.frmSalesInvoiceMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
        ...
        End Try
    End Function
    Private Sub loadExportInvoiceType()
        Dim dt As New DataTable
        dt.Columns.Add("Code", GetType(String))
        dt.Columns.Add("Value", GetType(String))

        dt.Rows.Add("Select", "")
        dt.Rows.Add("Export with Payment of GST", "Export with Payment of GST")
        dt.Rows.Add("Export without Payment of GST", "Export without Payment of GST")
        dt.Rows.Add("SEZ", "SEZ")
        dt.Rows.Add("Deemed Export", "Deemed Export")


        cmbExportInvoiceType.DataSource = dt
        cmbExportInvoiceType.DisplayMember = "Code"
        cmbExportInvoiceType.ValueMember = "Value"
    End Sub
End Class
