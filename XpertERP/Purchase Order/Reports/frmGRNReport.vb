    ...
    Sub GridData(ByVal FromDate As String, ByVal ToDate As String, ByVal isGRNNoSelect As Boolean, ByVal ArrGRNNo As ArrayList, ByVal isVendorSelect As Boolean, ByVal ArrVendor As ArrayList, ByVal isLocationSelect As Boolean, ByVal ArrLocation As ArrayList)
        Try
            ...
            If chkTrackingReport.Checked Then
	            ...
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 1 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter1,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 1 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData1,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 2 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter2,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 2 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData2,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 3 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter3,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 3 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData3,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 4 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter4,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 4 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData4,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 5 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter5,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 5 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData5,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 6 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter6,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 6 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData6,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 7 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter7,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 7 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData7,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 8 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter8,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 8 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData8,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 9 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter9,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 9 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData9,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 10 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter10,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 10 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData10,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 11 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter11,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 11 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData11,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 12 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter12,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 12 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData12,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 13 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter13,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 13 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData13,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 14 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter14,
                MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 14 THEN TSPL_QC_CHECK_SRN_DETAIL.InputData END) AS InputData14,
	            MAX(CASE WHEN TSPL_QC_CHECK_SRN_DETAIL.Line_No = 15 THEN TSPL_QC_LOG_SHEET_MASTER.AliasName END) AS Parameter15,
...
" left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = HEAD.comp_code where 2=2 "
    ...
    Sub SetGridFormat1()
        ...
        Dim item2 As New GridViewSummaryItem("GRN Qty", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item2)

        Dim item3 As New GridViewSummaryItem("Weighment Gross Weight", "{0:n3}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item3)

        Dim item4 As New GridViewSummaryItem("Weighment Tare Weight", "{0:n3}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item4)

        Dim item5 As New GridViewSummaryItem("Gunny Bag Weight", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item5)

        Dim item6 As New GridViewSummaryItem("Weighment Net Weight", "{0:n3}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item6)

        Dim item7 As New GridViewSummaryItem("SRN Rate", "{0:n3}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item7)

        Dim item8 As New GridViewSummaryItem("SRN Received Qty", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item8)

        Dim item9 As New GridViewSummaryItem("SRN Rejected Qty", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item9)

        Dim item10 As New GridViewSummaryItem("SRN Accepted Qty", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item10)

        Dim item11 As New GridViewSummaryItem("SRN Amount", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item11)

        Dim item12 As New GridViewSummaryItem("SRN Tax Amount", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item12)

        Dim item13 As New GridViewSummaryItem("SRN Net Amount", "{0:n2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item13)

        Dim item14 As New GridViewSummaryItem("Partial Reject Qty", "{0:n2}", GridAggregateFunction.Sum)
