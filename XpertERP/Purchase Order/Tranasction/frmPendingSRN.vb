...
#Region "Variables"
    ...
    Public ArrReturn As List(Of clsSRNDetail) = Nothing
    ...
    Const colHLocation As String = "Location"
    Const colHGENo As String = "colHGENo"
    Const colHGEDate As String = "colHGEDate"
    Const colRefrencePO As String = "colRefrencePO"
    Const colHRefTenderNo As String = "colHRefTenderNo"
    Const colHRefGrnDate As String = "colHRefGrnDate"
    Const colHRefVehicle_No As String = "colHRefVehicle_No"
    Const colHRefInvoiceNo As String = "colHRefInvoiceNo"
    Const colHRefInvoiceDate As String = "colHRefInvoiceDate"
    Const colVisualQCStatus As String = "colVisualQCStatus"
    Const colVisualQCStatusSecond As String = "colVisualQCStatusSecond"
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            '' ENd
            Dim qry As String = "select CAST(0 as bit) as Sel,code,max(Final.Tax_Group) as Tax_Group,max(TSPL_TAX_GROUP_MASTER.Tax_Group_Desc) as TaxGroupName,ICode,max(IName) as IName,Unit ,max(Location) as Location,MAX(TSPL_LOCATION_MASTER.Location_Desc) as LocationName," &
                " SUM(Qty* case when RI=1 then 1 else 0 end) as POQty," &
                " SUM(Qty* case when RI=-1 then 1 else 0 end) as GRNQty," &
                " SUM(Unapproved) as UnapprovedQty,SUM(FREEQty) as FreeQty," &
                " SUM((Qty *RI)- Unapproved) as PedningQty ,MAX(Rate) as Rate," &
                " MAX(Final.TAX1_Rate) as TAX1_Rate,MAX(Final.TAX2_Rate) as TAX2_Rate,MAX(Final.TAX3_Rate) as TAX3_Rate,MAX(Final.TAX4_Rate) as TAX4_Rate,MAX(Final.TAX5_Rate) as TAX5_Rate,MAX(Final.TAX6_Rate) as TAX6_Rate,MAX(Final.TAX7_Rate) as TAX7_Rate,MAX(Final.TAX8_Rate) as TAX8_Rate,MAX(Final.TAX9_Rate) as TAX9_Rate,MAX(Final.TAX10_Rate) as TAX10_Rate, MAX(Final.TAX1_Amt) as TAX1_Amt,MAX(Final.TAX2_Amt) as TAX2_Amt,MAX(Final.TAX3_Amt) as TAX3_Amt,MAX(Final.TAX4_Amt) as TAX4_Amt,MAX(Final.TAX5_Amt) as TAX5_Amt,MAX(Final.TAX6_Amt) as TAX6_Amt,MAX(Final.TAX7_Amt) as TAX7_Amt,MAX(Final.TAX8_Amt) as TAX8_Amt,MAX(Final.TAX9_Amt) as TAX9_Amt,MAX(Final.TAX10_Amt) as TAX10_Amt,Final.MRP as MRP,max(Final.Batch_No) as Batch_No,max(Final.MFG_Date) as MFG_Date,max(Final.Expiry_Date) as Expiry_Date ,max(Disc_Per) as Disc_Per,max(TransDate) as TransDate,MAX(Vendor) as Vendor,MAX(TSPL_VENDOR_MASTER.Vendor_Name) as VendorName,0 as Assessable,max(Leak_Qty) as Leak_Qty,max(Burst_Qty) as  Burst_Qty, max(Short_Qty) as Short_Qty, max(Reject_Qty) as Reject_Qty,max(Is_Mannual_Amt) as Is_Mannual_Amt,max(Amount) as Amount ,MAX(Line_No) as Line_No,max(AbatementRate) as AbatementRate,max(Bin_No) as Bin_No,max(Disc_Type) as Disc_Type, PO_ID,MRN_Id,GRN_ID,max(GENo) as GENo,max(convert(varchar, GEDate,103)) as GEDate,max(Category) as category,max(Emergency) as emergency,max(Capex_Code) as capex_code,max(Capex_SubCode) as capex_subcode,max(Against_Item_Wise_Tax_Rate) as Against_Item_Wise_Tax_Rate,max(Taxable_Amount_Per) as Taxable_Amount_Per,max(Header_Discount_Per) as Header_Discount_Per , max(TBL_PUrchaseSNR.RefrencePO) as RefrencePO,Max(Final.Item_Insurance_Apply_On) as Item_Insurance_Apply_On,Max(Final.Item_Insurance_Rate) as Item_Insurance_Rate,Max(Final.Item_Insurance_Amt) as Item_Insurance_Amt,max(RefTendorNo) as RefTendorNo , max(GrnDate) as GrnDate,max(Vehicle_No) as Vehicle_No,max(InvoiceNo) as InvoiceNo, max(InvoiceDate) as InvoiceDate,max(VisualQCStatus) as VisualQCStatus,max(VisualQCStatusSecond) as VisualQCStatusSecond from ( " &
                " select case when TSPL_GRN_HEAD.VisualQCStatus=1 then 'Ok' when TSPL_GRN_HEAD.VisualQCStatus='2' then 'Not Ok' when TSPL_GRN_HEAD.VisualQCStatus='3' then 'Partial Ok'  when TSPL_GRN_HEAD.VisualQCStatus='4' then 'On Hold' else ' ' end as [VisualQCStatus], case when TSPL_GRN_HEAD.VisualQCStatusSecond=1 then 'Ok' when TSPL_GRN_HEAD.VisualQCStatusSecond='2' then 'Not Ok' when TSPL_GRN_HEAD.VisualQCStatusSecond='3' then 'Partial Ok'  when TSPL_GRN_HEAD.VisualQCStatusSecond='4' then 'On Hold' else ' ' end as [VisualQCStatusSecond], TSPL_SRN_DETAIL.Header_Discount_Per,TSPL_SRN_DETAIL.Taxable_Amount_Per, TSPL_SRN_DETAIL.Against_Item_Wise_Tax_Rate, TSPL_SRN_DETAIL.Category,TSPL_SRN_DETAIL.Emergency,TSPL_SRN_DETAIL.Capex_Code,TSPL_SRN_DETAIL.Capex_SubCode,TSPL_SRN_DETAIL.PO_ID,TSPL_SRN_DETAIL.MRN_Id,TSPL_SRN_DETAIL.GRN_ID,TSPL_SRN_DETAIL.Bin_No,TSPL_SRN_DETAIL.SRN_No as Code,TSPL_SRN_HEAD.Vendor_Code as Vendor,TSPL_SRN_DETAIL.Item_Code as ICode,TSPL_SRN_DETAIL.Item_Desc as IName,TSPL_SRN_DETAIL.SRN_Qty as Qty,0 as Unapproved,TSPL_SRN_DETAIL .free_qty as FREEQty,TSPL_SRN_DETAIL.Unit_Code as Unit,TSPL_SRN_DETAIL.Location as Location,1 as RI,TSPL_SRN_DETAIL.Item_Cost as Rate,1 as Chk,TSPL_SRN_HEAD.Tax_Group,TSPL_SRN_DETAIL.TAX1_Rate,TSPL_SRN_DETAIL.TAX2_Rate,TSPL_SRN_DETAIL.TAX3_Rate,TSPL_SRN_DETAIL.TAX4_Rate,TSPL_SRN_DETAIL.TAX5_Rate,TSPL_SRN_DETAIL.TAX6_Rate,TSPL_SRN_DETAIL.TAX7_Rate,TSPL_SRN_DETAIL.TAX8_Rate,TSPL_SRN_DETAIL.TAX9_Rate,TSPL_SRN_DETAIL.TAX10_Rate,TSPL_SRN_DETAIL.TAX1_Amt,TSPL_SRN_DETAIL.TAX2_Amt,TSPL_SRN_DETAIL.TAX3_Amt,TSPL_SRN_DETAIL.TAX4_Amt,TSPL_SRN_DETAIL.TAX5_Amt,TSPL_SRN_DETAIL.TAX6_Amt,TSPL_SRN_DETAIL.TAX7_Amt,TSPL_SRN_DETAIL.TAX8_Amt,TSPL_SRN_DETAIL.TAX9_Amt,TSPL_SRN_DETAIL.TAX10_Amt,TSPL_SRN_DETAIL.MRP,TSPL_SRN_DETAIL.Batch_No,TSPL_SRN_DETAIL.MFG_Date,TSPL_SRN_DETAIL.Expiry_Date,TSPL_SRN_DETAIL.Disc_Per,TSPL_SRN_HEAD.SRN_Date as TransDate,TSPL_SRN_DETAIL.Assessable,TSPL_SRN_DETAIL.Leak_Qty,TSPL_SRN_DETAIL.Burst_Qty,TSPL_SRN_DETAIL.Short_Qty,TSPL_SRN_DETAIL.Rejected_Qty as Reject_Qty,TSPL_SRN_DETAIL.Is_Mannual_Amt,TSPL_SRN_DETAIL.Amount,TSPL_SRN_DETAIL.Line_No,TSPL_SRN_DETAIL.AbatementRate,TSPL_SRN_DETAIL.Disc_Type,TSPL_SRN_HEAD.GENo,TSPL_SRN_HEAD.GEDate,TSPL_SRN_DETAIL.Item_Insurance_Apply_On,TSPL_SRN_DETAIL.Item_Insurance_Rate,TSPL_SRN_DETAIL.Item_Insurance_Amt,TSPL_PURCHASE_ORDER_HEAD.RefTendorNo, TSPL_GRN_HEAD.GRN_Date  as GrnDate, TSPL_GRN_HEAD.VehicleNo as Vehicle_No, TSPL_GRN_HEAD.[Invoice/Challan_No] as InvoiceNo, tspl_grn_head.Invoice_Date as InvoiceDate " + Environment.NewLine +
            ...
            If clsCommon.CompairString(IsMerchantTrade, "MT") = CompairStringResult.Equal Then
                qry += " and TSPL_SRN_HEAD.Document_Type ='MT' " + Environment.NewLine
            Else
                qry += " and TSPL_SRN_HEAD.Document_Type ='SRN' " + Environment.NewLine
            End If
            If clsCommon.myLen(SubLocationCode) > 0 Then
                qry += " and isnull(TSPL_SRN_HEAD.Sublocation_Code,'')='" + SubLocationCode + "'"
            End If
            ...
            ''End
            qry += " union all " &
                " select '' as  VisualQCStatus,'' as VisualQCStatusSecond, 0 as Header_Discount_Per, 0 as Taxable_Amount_Per, '' as Against_Item_Wise_Tax_Rate,TSPL_PI_DETAIL.Category,TSPL_PI_DETAIL.Emergency,TSPL_PI_DETAIL.Capex_Code,TSPL_PI_DETAIL.Capex_SubCode,TSPL_PI_DETAIL.PO_ID,TSPL_PI_DETAIL.MRN_Id,TSPL_PI_DETAIL.GRN_ID, TSPL_PI_DETAIL.Bin_No,TSPL_PI_DETAIL.SRN_ID as Code,TSPL_PI_HEAD.Vendor_Code as Vendor,TSPL_PI_DETAIL.Item_Code as ICode,TSPL_PI_DETAIL.Item_Desc as IName,TSPL_PI_DETAIL.PI_Qty as Qty,0 as Unapproved,TSPL_PI_DETAIL.free_qty as FREEQty,TSPL_PI_DETAIL.Unit_Code as Unit,'' as Location,-1 as RI,TSPL_PI_DETAIL.Item_Cost as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate ,0 as  TAX1_Amt,0 as  TAX2_Amt,0 as  TAX3_Amt,0 as  TAX4_Amt,0 as  TAX5_Amt,0 as  TAX6_Amt,0 as  TAX7_Amt,0 as  TAX8_Amt,0 as  TAX9_Amt,0 as  TAX10_Amt,TSPL_PI_DETAIL.MRP as MRP,'' as Batch_No,null as MFG_Date,null as Expiry_Date,0 as Disc_Per,null as TransDate,TSPL_PI_DETAIL.Assessable, TSPL_PI_DETAIL.Leak_Qty as Leak_Qty,TSPL_PI_DETAIL.Burst_Qty as  Burst_Qty,TSPL_PI_DETAIL.Short_Qty  as Short_Qty,TSPL_PI_DETAIL.Reject_Qty as Reject_Qty,0 as Is_Mannual_Amt,0 as Amount,0 as  Line_No,TSPL_PI_DETAIL.AbatementRate,0 as Disc_Type,null as GENo,null as GEDate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt,null as RefTendorNo, null  as GrnDate,'' as Vehicle_No , ''as InvoiceNo,null as InvoiceDate from TSPL_PI_DETAIL left outer join TSPL_PI_HEAD on TSPL_PI_HEAD.PI_No=TSPL_PI_DETAIL.PI_No where TSPL_PI_HEAD.Status=1 and len(isnull(TSPL_PI_DETAIL.SRN_Id,''))>0 " &
                " union all  " &
                " select '' as  VisualQCStatus,'' as VisualQCStatusSecond, 0 as Header_Discount_Per, 0 as Taxable_Amount_Per, '' as Against_Item_Wise_Tax_Rate, TSPL_PI_DETAIL.Category,TSPL_PI_DETAIL.Emergency,TSPL_PI_DETAIL.Capex_Code,TSPL_PI_DETAIL.Capex_SubCode,TSPL_PI_DETAIL.PO_ID,TSPL_PI_DETAIL.MRN_Id,TSPL_PI_DETAIL.GRN_ID, TSPL_PI_DETAIL.Bin_No,TSPL_PI_DETAIL.SRN_ID as Code,TSPL_PI_HEAD.Vendor_Code as Vendor,TSPL_PI_DETAIL.Item_Code as ICode,TSPL_PI_DETAIL.Item_Desc as IName,0  as Qty,TSPL_PI_DETAIL.PI_Qty as Unapproved,TSPL_PI_DETAIL.free_qty as FREEQty,TSPL_PI_DETAIL.Unit_Code as Unit,'' as Location,-1 as RI,TSPL_PI_DETAIL.Item_Cost as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate ,0 as  TAX1_Amt,0 as  TAX2_Amt,0 as  TAX3_Amt,0 as  TAX4_Amt,0 as  TAX5_Amt,0 as  TAX6_Amt,0 as  TAX7_Amt,0 as  TAX8_Amt,0 as  TAX9_Amt,0 as  TAX10_Amt,TSPL_PI_DETAIL.MRP as MRP,'' as Batch_No,null as MFG_Date,null as Expiry_Date,0 as Disc_Per,null as TransDate,TSPL_PI_DETAIL.Assessable,TSPL_PI_DETAIL.Leak_Qty as Leak_Qty,TSPL_PI_DETAIL.Burst_Qty as  Burst_Qty,TSPL_PI_DETAIL.Short_Qty as Short_Qty,TSPL_PI_DETAIL.Reject_Qty as Reject_Qty,0 as Is_Mannual_Amt,0 as Amount,0 as  Line_No,TSPL_PI_DETAIL.AbatementRate,0 as Disc_Type,null as GENo,null as GEDate,'' as Item_Insurance_Apply_On,0 as Item_Insurance_Rate,0 as Item_Insurance_Amt,null as RefTendorNo, null  as GrnDate, '' as Vehicle_No, ''as InvoiceNo,null as InvoiceDate  from TSPL_PI_DETAIL left outer join TSPL_PI_HEAD on TSPL_PI_HEAD.PI_No=TSPL_PI_DETAIL.PI_No where TSPL_PI_HEAD.Status=0 and len(isnull(TSPL_PI_DETAIL.SRN_Id,''))>0 and TSPL_PI_DETAIL.PI_No not in ('" + strCurrCode + "') " &
                " )Final   left outer join (SELECT distinct TSPL_SRN_DETAIL2.SRN_No ,TSPL_SRN_DETAIL2.Item_Code, STUFF((SELECT distinct ',' + QUOTENAME(TSPL_PURCHASE_ORDER_HEAD.ReferencePO) as RefrencePO   from TSPL_SRN_DETAIL left outer join TSPL_PURCHASE_ORDER_HEAD on  TSPL_PURCHASE_ORDER_HEAD .PurchaseOrder_No =TSPL_SRN_DETAIL.PO_ID and len(isnull(TSPL_SRN_DETAIL.PO_ID,'')) >0  where TSPL_SRN_DETAIL2.SRN_No = TSPL_SRN_DETAIL.SRN_No and len(isnull (ReferencePO,'')) > 0  FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')  as RefrencePO
                from TSPL_SRN_DETAIL  as TSPL_SRN_DETAIL2  where len(isnull(TSPL_SRN_DETAIL2.PO_ID,'')) >0 )  TBL_PUrchaseSNR on TBL_PUrchaseSNR.SRN_No = Final.Code and TBL_PUrchaseSNR.Item_Code =Final .ICode left outer join TSPL_VENDOR_MASTER on TSPL_VENDOR_MASTER.Vendor_Code=final.Vendor left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=Final.Location left outer join TSPL_TAX_GROUP_MASTER on TSPL_TAX_GROUP_MASTER.Tax_Group_Code=Final.Tax_Group and TSPL_TAX_GROUP_MASTER.Tax_Group_Type='P' "

            qry += " group by Code,ICode,Unit,MRP,PO_ID,MRN_Id,GRN_ID  "
            ''RICHA AGARWAL 07/07/2015  or sum(Reject_Qty*RI)<>0  CODE TO  REJECTED QTY ON PURCHASE INVOICE
            qry += " having SUM(Chk)>0 and (SUM((Qty *RI)- Unapproved)<>0 or (SUM(Qty*RI)=0 and (sum(Leak_Qty*RI)<>0 or sum(Burst_Qty*RI)<>0  or sum(Short_Qty*RI)<>0  or sum(Reject_Qty*RI)<>0 )))"
            ''--------------------------------------
    ...
    Sub LoadHeadData()
        ...
        For Each dr As DataRow In dtAllData.Rows
            ...
            If Not arr.Contains(strCode) Then
                ...
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHLocation).Value = clsCommon.myCstr(dr("LocationName"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHGENo).Value = clsCommon.myCstr(dr("GENo"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHGEDate).Value = clsCommon.myCstr(dr("GEDate"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colRefrencePO).Value = clsCommon.myCstr(dr("RefrencePO"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHRefTenderNo).Value = clsCommon.myCstr(dr("RefTendorNo"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHRefGrnDate).Value = clsCommon.myCstr(dr("GrnDate"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHRefVehicle_No).Value = clsCommon.myCstr(dr("Vehicle_No"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHRefInvoiceNo).Value = clsCommon.myCstr(dr("InvoiceNo"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colHRefInvoiceDate).Value = clsCommon.myCstr(dr("InvoiceDate"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colVisualQCStatus).Value = clsCommon.myCstr(dr("VisualQCStatus"))
                gvHead.Rows(gvHead.RowCount - 1).Cells(colVisualQCStatusSecond).Value = clsCommon.myCstr(dr("VisualQCStatusSecond"))
            End If
        Next
    ...
    Sub LoadBlankHeadGrid()
        ...
        repoCode.FormatString = ""
        repoCode.HeaderText = "GE No"
        repoCode.Name = colHGENo
        repoCode.Width = 100
        repoCode.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repoCode)
        ...
        repoDate.FormatString = ""
        repoDate.HeaderText = "GE Date"
        repoDate.Name = colHGEDate
        repoDate.Width = 100
        repoDate.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repoDate)
        ...
        Dim repRefrencePO As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repRefrencePO.FormatString = ""
        repRefrencePO.HeaderText = "Refrence PO"
        repRefrencePO.Name = colRefrencePO
        repRefrencePO.Width = 180
        repRefrencePO.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repRefrencePO)

        repRefrencePO = New GridViewTextBoxColumn()
        repRefrencePO.FormatString = ""
        repRefrencePO.HeaderText = "Tender No"
        repRefrencePO.Name = colHRefTenderNo
        repRefrencePO.Width = 180
        repRefrencePO.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repRefrencePO)

        Dim repGrnDate As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repGrnDate = New GridViewTextBoxColumn()
        repGrnDate.FormatString = ""
        repGrnDate.HeaderText = "GRN Date"
        repGrnDate.Name = colHRefGrnDate
        repGrnDate.Width = 100
        repGrnDate.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repGrnDate)

        Dim repVehicle_No As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repVehicle_No = New GridViewTextBoxColumn()
        repVehicle_No.FormatString = ""
        repVehicle_No.HeaderText = "Vehicle No"
        repVehicle_No.Name = colHRefVehicle_No
        repVehicle_No.Width = 180
        repVehicle_No.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repVehicle_No)

        Dim repInvoiceNo As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repInvoiceNo = New GridViewTextBoxColumn()
        repInvoiceNo.FormatString = ""
        repInvoiceNo.HeaderText = "Invoice No"
        repInvoiceNo.Name = colHRefInvoiceNo
        repInvoiceNo.Width = 180
        repInvoiceNo.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repInvoiceNo)

        Dim repInvoiceDate As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repInvoiceDate = New GridViewTextBoxColumn()
        repInvoiceDate.FormatString = ""
        repInvoiceDate.HeaderText = "Invoice Date"
        repInvoiceDate.Name = colHRefInvoiceDate
        repInvoiceDate.Width = 100
        repInvoiceDate.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repInvoiceDate)

        Dim repVisualQCStatus As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repVisualQCStatus = New GridViewTextBoxColumn()
        repVisualQCStatus.FormatString = ""
        repVisualQCStatus.HeaderText = "VisualQCStatus"
        repVisualQCStatus.Name = colVisualQCStatus
        repVisualQCStatus.Width = 100
        repVisualQCStatus.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repVisualQCStatus)

        Dim repVisualQCStatusSecond As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repVisualQCStatusSecond = New GridViewTextBoxColumn()
        repVisualQCStatusSecond.FormatString = ""
        repVisualQCStatusSecond.HeaderText = "VisualQCStatusSecond"
        repVisualQCStatusSecond.Name = colVisualQCStatusSecond
        repVisualQCStatusSecond.Width = 100
        repVisualQCStatusSecond.ReadOnly = True
        gvHead.MasterTemplate.Columns.Add(repVisualQCStatusSecond)
    ...
    Sub LoadBlankGridDetail()
        ...
        repoPOID.IsVisible = True
        repoPOID.ReadOnly = True
        repoPOID.Width = 100 ''BHA/27/06/18-000100 by balwinder on 28/06/2018
        ...
        repoAppQty.HeaderText = "Used in PI"
        ...
        repoFreeQty.HeaderText = "FREE QTY"
        ...
        repoAssessable.HeaderText = "Accessable"
        ...
        repoBurstQty.Name = colDBurstQty
        ...
        repoShortQty.Name = colDShortQty
        ...
        repoRejectQty.HeaderText = "Reject Qty"
        repoRejectQty.Name = colDRejectQty
        ...
        Dim repoDisType As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoDisType.FormatString = ""
        repoDisType.HeaderText = "Discount Type"
        repoDisType.Name = colDDiscountType
        repoDisType.Width = 80
        repoDisType.Minimum = 0
        repoDisType.ReadOnly = True
        repoDisType.IsVisible = False
        repoDisType.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gv1.MasterTemplate.Columns.Add(repoDisType)

        Dim repoIsManualAmt As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoIsManualAmt.FormatString = ""
        repoIsManualAmt.HeaderText = "Is Manual Amt"
        repoIsManualAmt.Name = colDMannualAmt
        repoIsManualAmt.Width = 80
        repoIsManualAmt.Minimum = 0
        repoIsManualAmt.ReadOnly = True
        repoIsManualAmt.IsVisible = False
        repoIsManualAmt.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gv1.MasterTemplate.Columns.Add(repoIsManualAmt)
    ...
    Public Shared Function Load_discount_for_PI(ByVal SRN_No As String, ByVal item_code As String)
        Dim disc_per As Double = 0
        Try
            Dim sQuery As String = "select disc_per from TSPL_SRN_DETAIL where SRN_No='" & SRN_No & "' and item_code='" & item_code & "'"
            disc_per = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(sQuery))
        Catch ex As Exception
    ...
    Public Shared Function Load_discount_per_unit_for_PI(ByVal SRN_No As String, ByVal item_code As String)
    ...
    Sub btnOKPressed()
        ...
        ArrReturn = New List(Of clsSRNDetail)
        Dim obj As clsSRNDetail = Nothing
        For ii As Integer = 0 To gv1.RowCount - 1
            If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                obj = New clsSRNDetail()
                obj.SRN_No = clsCommon.myCstr(gv1.Rows(ii).Cells(colDCode).Value)
                ...
                obj.Freeqty = clsCommon.myCdbl(gv1.Rows(ii).Cells(colFreeQty).Value)

                obj.SRN_Qty = clsCommon.myCdbl(gv1.Rows(ii).Cells(colDOrderQty).Value)
                obj.SRNTax_Group = clsCommon.myCstr(gv1.Rows(ii).Cells(colDTaxGroup).Value)
                ...
                obj.Rejected_Qty = clsCommon.myCdbl(gv1.Rows(ii).Cells(colDRejectQty).Value)
                obj.Disc_Type = clsCommon.myCdbl(gv1.Rows(ii).Cells(colDDiscountType).Value)
                obj.Is_Mannual_Amt = clsCommon.myCdbl(gv1.Rows(ii).Cells(colDMannualAmt).Value)
                ...
                ''RICHA AGARWAL 07/07/2015 ADD CONDITION IN BELOW 
                If (obj.Balance_Qty > 0 OrElse obj.Leak_Qty > 0 OrElse obj.Burst_Qty > 0 OrElse obj.Short_Qty > 0 OrElse obj.Rejected_Qty > 0) Then
                    ArrReturn.Add(obj)
                End If
            End If
        Next
    ...
    Private Sub gvHead_ValueChanging(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.ValueChangingEventArgs) Handles gvHead.ValueChanging
        If gvHead.CurrentRow.Index >= 0 AndAlso Not IsInsideLoadData Then
            ...
            'Dim SecQc As String = clsCommon.myCstr(gvHead.CurrentRow.Cells(colVisualQCStatusSecond).Value)
            'If (clsCommon.myCBool(e.NewValue) = True) AndAlso clsCommon.CompairString(SecQc, "Not Ok") = CompairStringResult.Equal Then
            '    e.Cancel = True
            '    common.clsCommon.MyMessageBoxShow("Second qc is not ok" + clsCommon.myCstr(gvHead.CurrentRow.Index))
            '    Exit Sub
            'End If
    ...
    Sub LoadDetailData(ByVal NewVal As Boolean, ByVal strCode As String)
        ...
        If NewVal Then
            For Each dr As DataRow In dtAllData.Rows
                If clsCommon.CompairString(strCode, clsCommon.myCstr(dr("Code"))) = CompairStringResult.Equal AndAlso clsCommon.myCdbl(dr("Rate")) > 0 Then
                    gv1.Rows.AddNew()
                    ...
                    gv1.Rows(gv1.Rows.Count - 1).Cells(colFreeQty).Value = clsCommon.myCdbl(dr("FreeQty"))
                    ...
                    gv1.Rows(gv1.Rows.Count - 1).Cells(colDRejectQty).Value = clsCommon.myCdbl(dr("Reject_Qty"))
                    gv1.Rows(gv1.Rows.Count - 1).Cells(colDDiscountType).Value = clsCommon.myCdbl(dr("Disc_Type"))
                    gv1.Rows(gv1.Rows.Count - 1).Cells(colDMannualAmt).Value = clsCommon.myCdbl(dr("Is_Mannual_Amt"))
            ...
            If clsCommon.myCBool(gvHead.Rows(ii).Cells(colHSelect).Value) Then
                ...
                For jj As Integer = 0 To gv1.RowCount - 1
                    If clsCommon.myCBool(gv1.Rows(jj).Cells(colDSelect).Value) AndAlso clsCommon.CompairString(strCode, clsCommon.myCstr(gv1.Rows(jj).Cells(colDCode).Value)) = CompairStringResult.Equal Then
                        ...
                        End If
                        Dim StrLoc As String = clsCommon.myCstr(gvHead.Rows(ii).Cells(colHLocation).Value)
                        If Not arrLoc.Contains(StrLoc) Then
                            arrLoc.Add(StrLoc)
                            If arrLoc.Count > 1 Then
                                clsCommon.MyMessageBoxShow("Items of more than one Location not acceptable ")
                                Return False
                            End If
