...
#End Region
    ...
    Public Sub Print()
        Try
            ...
            Dim qry As String = "     SELECT  TSPL_CostCenter_MASTER.cost_name,  TSPL_IssueReturn_HEAD.Created_By ,TSPL_IssueReturn_HEAD.Modify_By ,   TSPL_IssueReturn_HEAD.Doc_No,TSPL_IssueReturn_HEAD.RequisitionNo, TSPL_IssueReturn_HEAD.Doc_Date,TSPL_IssueReturn_HEAD.Doc_Type, TSPL_IssueReturn_HEAD.Remarks, TSPL_IssueReturn_HEAD.Comment,  case when  TSPL_IssueReturn_HEAD.Status=0 then 'Pending' else 'Approved' end as Status, TSPL_IssueReturn_HEAD.Posting_Date, TSPL_IssueReturn_DETAIL.Item_Code,TSPL_COMPANY_MASTER.Phone1,TSPL_COMPANY_MASTER.Email,TSPL_COMPANY_MASTER.GSTReg_No,TSPL_IssueReturn_DETAIL.Item_Desc, TSPL_IssueReturn_DETAIL.Required_Qty, TSPL_IssueReturn_DETAIL.Issued_Qty_AgainstRet as returnqty,   TSPL_IssueReturn_DETAIL.Unit_code, TSPL_COMPANY_MASTER.Comp_Name, TSPL_COMPANY_MASTER.Add1, TSPL_COMPANY_MASTER.Add2,   TSPL_COMPANY_MASTER.Add3, TSPL_COMPANY_MASTER.State, TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2, loc1.Location_Desc as Fromloc,loc2.Location_Desc as Toloc, emp1.Emp_Name as IssuesTo,emp2.Emp_Name as RequestBy,"
    ...
    Private Sub fndReqNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndReqNo._MYValidating
        If clsCommon.CompairString(clsCommon.myCstr(cboDocType.SelectedValue), "Issue") = CompairStringResult.Equal Then
            ...
            If AllowOnlyOneIssueAgainstStoreRequisition = True Then
                qry = "Select Distinct AA.RequisitionNo ,AA.Date as RequisitionDate,AA.[All/Issue] from (select TSPL_REQUISITION_HEAD.Requisition_Id as RequisitionNo,Convert(varchar,TSPL_REQUISITION_HEAD.Requisition_Date ,103) as Date,Case When (All_Transfer_Issue)=0 Then 'All' Else (Case When All_Transfer_Issue=2 Then 'Issue' Else '' End) End As [All/Issue]   " &
            ...
            Else
                ...
                " ( Select MAX(Code) as RequisitionNo,MAX(Date) as Date,MAX(Unit_code) as [Unit Code],SUM(Qty* case when RI=1 then 1 else 0 end) as RequisitionQty,SUM(Qty* case when RI=-1 then 1 else 0 end) as IssueQty,SUM((Qty *RI)- Unapproved) as PendingQty ,SUM(Unapproved) as UnapprovedQty,Case When Max(All_Transfer_Issue)=0 Then 'All' Else (Case When Max(All_Transfer_Issue)=2 Then 'Issue' Else '' End) End As [All/Issue] from" &
                ...
                " Union All " &
                " select TSPL_IssueReturn_HEAD.RequisitionNo as Code,'' as Date,TSPL_IssueReturn_DETAIL.Item_Code ,TSPL_IssueReturn_DETAIL.Item_Desc ,TSPL_IssueReturn_DETAIL.Unit_code,TSPL_IssueReturn_DETAIL.Issued_Qty as Qty,0  as Unapproved ,-1 as RI,0 As All_Transfer_Issue   from TSPL_IssueReturn_HEAD Left Outer Join TSPL_IssueReturn_DETAIL on TSPL_IssueReturn_HEAD.Doc_No =TSPL_IssueReturn_DETAIL.Doc_No  where Doc_Type='Issue' and TSPL_IssueReturn_HEAD.Status=1 and len(isnull(TSPL_IssueReturn_DETAIL.Doc_No ,''))>0 " &
                " Union All " &
                " select TSPL_IssueReturn_HEAD.RequisitionNo as Code,'' as Date,TSPL_IssueReturn_DETAIL.Item_Code ,TSPL_IssueReturn_DETAIL.Item_Desc ,TSPL_IssueReturn_DETAIL.Unit_code,0 as Qty,TSPL_IssueReturn_DETAIL.Issued_Qty  as Unapproved ,-1 as RI,0 As All_Transfer_Issue   from TSPL_IssueReturn_HEAD Left Outer Join TSPL_IssueReturn_DETAIL on TSPL_IssueReturn_HEAD.Doc_No =TSPL_IssueReturn_DETAIL.Doc_No  where Doc_Type='Issue' and TSPL_IssueReturn_HEAD.Status=0 and len(isnull(TSPL_IssueReturn_DETAIL.Doc_No ,''))>0 and TSPL_IssueReturn_DETAIL.Doc_No not in ('" & txtDocNo.Value & "'))" &
                " Final group by Code having (SUM(Qty *RI)-SUM(Unapproved)) >0) AA"
            End If

            Dim whrcls As String = " [All/Issue] In ('All','Issue')"
