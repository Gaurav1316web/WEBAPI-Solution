    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If gvSchedule IsNot Nothing AndAlso gvSchedule.Rows.Count > 0 Then
                ...
                Dim prevToDate As DateTime = Nothing
                For Each gvRows As GridViewRowInfo In gvSchedule.Rows
                    ...
                    Dim dtt As DataTable = clsDBFuncationality.GetDataTable(chkQry)
            ...
            If gv1 IsNot Nothing AndAlso gv1.Rows.Count > 0 Then
                For Each gvRow1 As GridViewRowInfo In gv1.Rows
                    If gvSchedule IsNot Nothing AndAlso gvSchedule.Rows.Count > 0 Then
                        For Each gvSch As GridViewRowInfo In gvSchedule.Rows
                            If clsCommon.myCDecimal(gvRow1.Cells(colLineNo).Value) = clsCommon.myCDecimal(gvSch.Cells(colScheduleParentSNo).Value) Then
                                QtyPer += clsCommon.myCDecimal(gvSch.Cells(colScheduleQtyPer).Value)
                            End If
                        Next
                        SchItemCode = clsCommon.myCstr(gvRow1.Cells(colICode).Value)
                        If QtyPer < 100 Then
    ...
    Private Sub FrmAPInvoiceEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.Alt AndAlso e.Shift AndAlso e.Control AndAlso e.KeyCode = Keys.F8 Then
            If clsCommon.CompairString(clsCommon.myCstr(UsLock1.Status), "Approved") = CompairStringResult.Equal Then
                Dim frm As New FrmPWD(Nothing)
                frm.strType = clsFixedParameterType.TransactionModify
                frm.strCode = clsFixedParameterCode.TransactionModify
                frm.ShowDialog()
                If frm.isPasswordCorrect Then
        ...
        Try
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal AndAlso clsCommon.myLen(gv1.Rows(ii).Cells(colICode).Value) > 0 AndAlso clsCommon.myCDecimal(gv1.Rows(ii).Cells(colQty).Value) > 0 Then
                    Dim BalanceQty As Decimal = clsCommon.myCDecimal(gv1.Rows(ii).Cells(colQty).Value)
                    ...
                    If ArrSch IsNot Nothing AndAlso ArrSch.Count > 0 Then
                        For Each obj As clsItemSchedule In ArrSch
                            If BalanceQty <= obj.Quantity AndAlso parentSNO <> clsCommon.myCDecimal(gv1.Rows(ii).Cells(colLineNo).Value) Then
                                obj.Qty_Per = 100
                            End If
                            gvSchedule.Rows.AddNew()
                            ...
                            'End If
                            'dtRunningDate = dtRunningDate.AddDays(1)
                            parentSNO = clsCommon.myCDecimal(gv1.Rows(ii).Cells(colLineNo).Value)
        ...
        Try
            If gvSchedule IsNot Nothing AndAlso gvSchedule.Rows.Count > 0 Then
                ...
                For Each grow As GridViewRowInfo In gvSchedule.Rows
                    ...
                    obj.ArrSchedule.Add(objTr)
                Next
                Dim strChk As String = "select Count(1) from TSPL_SRN_TENDER_CALC Where Against_PO ='" & txtDocNo.Value & "'"
                Dim chkInt As Integer = clsCommon.myCDecimal(clsDBFuncationality.getSingleValue(strChk))
                If chkInt > 0 Then
                    clsCommon.MyMessageBoxShow(Me, "PO already exist in RAL Penalty !", Me.Text)
                Else
