    ...
    Public Sub print(ByVal StrDocNo As String)
        ...
        Try
            ...
            If ChkPost > 0 Then
               ...
               (TSPL_JOURNAL_MASTER.Source_Narration) as  Narration, (TSPL_JOURNAL_MASTER.Created_By) AS Created_By, (TSPL_JOURNAL_DETAILS.Detail_Line_No) AS Line_No,  (TSPL_JOURNAL_DETAILS.Account_code) AS  GL_Account_Code, (TSPL_JOURNAL_DETAILS.Account_Desc) AS GL_Account_Desc,  (TSPL_JOURNAL_DETAILS.Amount) AS Expr1,case when (TSPL_JOURNAL_DETAILS.Amount)<0 then -1 * (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Credit,case when (TSPL_JOURNAL_DETAILS.Amount)>=0 then (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Debit ,
               ...
               convert(varchar,( TSPL_SRN_HEAD.Posting_Date),103) AS Posting_Date, (TSPL_SRN_HEAD.SRN_Total_Amt) as PJV_Amount, isnull((TSPL_PI_REMITTANCE.Actual_Total_TDS),0)as PJV_TDS_Amount,  (TSPL_SRN_HEAD.SRN_Total_Amt) as PJV_Net_Amount, (tab2.Source_Narration) as  Narration, (tab2.Created_By) AS Created_By, (TSPL_JOURNAL_DETAILS.Detail_Line_No) AS Line_No,  (TSPL_JOURNAL_DETAILS.Account_code) AS  GL_Account_Code, (TSPL_JOURNAL_DETAILS.Account_Desc) AS GL_Account_Desc,  (TSPL_JOURNAL_DETAILS.Amount) AS Expr1,case when (TSPL_JOURNAL_DETAILS.Amount)<0 then -1 * (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Credit
               ,case when (TSPL_JOURNAL_DETAILS.Amount)>=0 then (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Debit , (TSPL_SRN_HEAD.Bill_To_Location) AS Bill_To_Location, ( TSPL_COMPANY_MASTER.Comp_Name) AS Comp_Name, (TSPL_COMPANY_MASTER.Add1) AS Add1, (TSPL_COMPANY_MASTER.Add2) as Add2, (TSPL_COMPANY_MASTER.Add3) as Add3,   (TSPL_COMPANY_MASTER.State) as State, (TSPL_COMPANY_MASTER.CST_LST) as CST_LST,(TSPL_COMPANY_MASTER.Comp_Code) AS Comp_Code, (TSPL_VENDOR_MASTER.Vendor_Name) AS Expr2, (TSPL_VENDOR_MASTER.Add1) AS Expr3, (TSPL_VENDOR_MASTER.Add2) AS Expr4,  (TSPL_VENDOR_MASTER.Add3) AS Expr5,
               ...
               left outer join TSPL_PI_REMITTANCE on TSPL_PI_HEAD.PI_No=TSPL_PI_REMITTANCE.Document_No  LEFT OUTER JOIN  TSPL_COMPANY_MASTER ON TSPL_SRN_HEAD.Comp_Code = TSPL_COMPANY_MASTER.Comp_Code CROSS APPLY  (  SELECT distinct TSPL_PI_DETAIL.SRN_Id + ',' AS [text()]   FROM TSPL_PI_DETAIL 
               ...
               case when (TSPL_SRN_HEAD.Status)=0 then 'Pending' else 'Approved'end as status,  convert(varchar,( TSPL_SRN_HEAD.Posting_Date),103) AS Posting_Date, (TSPL_SRN_DEDUCTION_SECURITY.Ded_Amt) as PJV_Amount, isnull((TSPL_PI_REMITTANCE.Actual_Total_TDS),0)as PJV_TDS_Amount,  (TSPL_SRN_DEDUCTION_SECURITY.Ded_Amt) as PJV_Net_Amount, (TABJOURNAL.Source_Narration) as  Narration, (TABJOURNAL.Created_By) AS Created_By, (TSPL_JOURNAL_DETAILS.Detail_Line_No) AS Line_No,  (TSPL_JOURNAL_DETAILS.Account_code) AS  GL_Account_Code, (TSPL_JOURNAL_DETAILS.Account_Desc) AS GL_Account_Desc, 
               (TSPL_JOURNAL_DETAILS.Amount) AS Expr1,case when (TSPL_JOURNAL_DETAILS.Amount)<0 then -1 * (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Credit,case when (TSPL_JOURNAL_DETAILS.Amount)>=0 then (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Debit , (TSPL_SRN_HEAD.Bill_To_Location) AS Bill_To_Location, ( TSPL_COMPANY_MASTER.Comp_Name) AS Comp_Name, (TSPL_COMPANY_MASTER.Add1) AS Add1, (TSPL_COMPANY_MASTER.Add2) as Add2, (TSPL_COMPANY_MASTER.Add3) as Add3,   (TSPL_COMPANY_MASTER.State) as State, (TSPL_COMPANY_MASTER.CST_LST) as CST_LST,(TSPL_COMPANY_MASTER.Comp_Code) AS Comp_Code, 
               ...
               on TSPL_VENDOR_MASTER.State_Code= TSPL_STATE_MASTER.State_Code  LEFT OUTER JOIN TSPL_PURCHASE_ORDER_HEAD ON TSPL_PI_HEAD.Against_PO=TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_No  left outer join TSPL_USER_MASTER as TSPL_USER_MASTER_Created_By on TSPL_USER_MASTER_Created_By.User_Code = TSPL_SRN_HEAD.Created_By  left outer join TSPL_USER_MASTER as TSPL_USER_MASTER_Modified_By on TSPL_USER_MASTER_Modified_By.User_Code = TSPL_SRN_HEAD.Modify_By  left outer join TSPL_PI_REMITTANCE on TSPL_PI_HEAD.PI_No=TSPL_PI_REMITTANCE.Document_No  LEFT OUTER JOIN  TSPL_COMPANY_MASTER 
               ...
               (TSPL_SRN_DEDUCTION.Ded_Amt) as PJV_Amount, isnull((TSPL_PI_REMITTANCE.Actual_Total_TDS),0)as PJV_TDS_Amount,  (TSPL_SRN_DEDUCTION.Ded_Amt) as PJV_Net_Amount, (TABJOURNAL.Source_Narration) as  Narration, (TABJOURNAL.Created_By) AS Created_By, (TSPL_JOURNAL_DETAILS.Detail_Line_No) AS Line_No,  (TSPL_JOURNAL_DETAILS.Account_code) AS  GL_Account_Code, (TSPL_JOURNAL_DETAILS.Account_Desc) AS GL_Account_Desc,  (TSPL_JOURNAL_DETAILS.Amount) AS Expr1,case when (TSPL_JOURNAL_DETAILS.Amount)<0 then -1 * (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Credit,case when (TSPL_JOURNAL_DETAILS.Amount)>=0 then (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Debit , 
               ...
               left outer join TSPL_USER_MASTER as TSPL_USER_MASTER_Created_By on TSPL_USER_MASTER_Created_By.User_Code = TSPL_SRN_HEAD.Created_By  left outer join TSPL_USER_MASTER as TSPL_USER_MASTER_Modified_By on TSPL_USER_MASTER_Modified_By.User_Code = TSPL_SRN_HEAD.Modify_By  left outer join TSPL_PI_REMITTANCE on TSPL_PI_HEAD.PI_No=TSPL_PI_REMITTANCE.Document_No  LEFT OUTER JOIN  TSPL_COMPANY_MASTER ON TSPL_SRN_HEAD.Comp_Code = TSPL_COMPANY_MASTER.Comp_Code CROSS APPLY  (SELECT distinct tspl_PI_Detail.SRN_Id + ',' AS [text()]   FROM tspl_PI_Detail left outer join TSPL_SRN_DEDUCTION  on TSPL_SRN_DEDUCTION.SRN_No = tspl_PI_Detail.SRN_Id
                ...
                (TSPL_SRN_TENDER.Penalty) as PJV_Amount, isnull((TSPL_PI_REMITTANCE.Actual_Total_TDS),0)as PJV_TDS_Amount,  (TSPL_SRN_TENDER.Penalty) as PJV_Net_Amount, (TABJOURNAL.Source_Narration) as  Narration, (TABJOURNAL.Created_By) AS Created_By, (TSPL_JOURNAL_DETAILS.Detail_Line_No) AS Line_No,  (TSPL_JOURNAL_DETAILS.Account_code) AS  GL_Account_Code, (TSPL_JOURNAL_DETAILS.Account_Desc) AS GL_Account_Desc,  (TSPL_JOURNAL_DETAILS.Amount) AS Expr1,case when (TSPL_JOURNAL_DETAILS.Amount)<0 then -1 * (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Credit,case when (TSPL_JOURNAL_DETAILS.Amount)>=0 then (TSPL_JOURNAL_DETAILS.Amount) else 0 end as Debit , (TSPL_SRN_HEAD.Bill_To_Location) AS Bill_To_Location, ( TSPL_COMPANY_MASTER.Comp_Name) AS Comp_Name, 
            ...
            from TSPL_SRN_DETAIL  as TSPL_SRN_DETAIL2  where len(isnull(TSPL_SRN_DETAIL2.PO_ID,'')) >0 )xx group by SRN_No )  TBL_PUrchaseSNR on TBL_PUrchaseSNR.SRN_No = aa.SRN_No  ) XX group by  XX.GL_Account_Code order by PJVGroup"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
