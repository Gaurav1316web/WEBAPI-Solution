...
select SRN_No,'RM Late Penalty' as Type,Item_Code,Penalty as Amount from TSPL_SRN_TENDER where isnull(Penalty,0)>0 and  SRN_No in (" + clsCommon.GetMulcallString(ListSRN_No) + ")
    ...
    Public Sub print(ByVal StrDocNo As String)
        ...
        Try
            ...
            If ChkPost > 0 Then
                qry = "	 select T1.*,TSPL_COMPANY_MASTER.Logo_Img as Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2 as Logo_Img2 from (SELECT  ROW_NUMBER() over(order by MAX(TSPL_JOURNAL_MASTER.Voucher_No)) as Rownum ," &
                    " max(TSPL_USER_MASTER_Created_By.User_Name) as Invoice_Created_By_Name,max(TSPL_USER_MASTER_Modified_By.User_Name) as Invoice_Modifiy_By_Name, " &
                  " MAX(tspl_state_master_for_location_state.GST_STATE_Code) as LOC_GST_State_Code,MAX(TSPL_LOCATION_MASTER.GSTNO) as Loc_GstInNo ,MAX(TSPL_VENDOR_MASTER.GSTFinalNo) AS Vendor_GSTIN_NO,MAX(TSPL_STATE_MASTER.GST_STATE_Code) AS Vendor_GST_StateCode,  MAX(TSPL_JOURNAL_MASTER.Modify_By) AS Modify_By,MAX(TSPL_JOURNAL_MASTER.Voucher_No) AS  PJV_No, convert(varchar,MAX(TSPL_JOURNAL_MASTER.Voucher_Date),103) as PJV_Date , MAX(TSPL_VENDOR_MASTER.Vendor_Code) AS Vendor_Code, MAX(TSPL_VENDOR_MASTER.Vendor_Name) AS Vendor_Name, MAX(TSPL_PI_HEAD.PI_No) AS  Invoice_No, MAX(TSPL_SRN_HEAD.Against_PO) AS Against_PO, MAX(TSPL_PI_HEAD.Against_PO) AS PO_No, convert(varchar,MAX(TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_Date),103) AS  PO_Date, isnull(LEFT(MAX(el.files) ,LEN(MAX(el.files ))-1),'NoFile') as SRN_No, convert(varchar,MAX(TSPL_SRN_HEAD.SRN_Date),103) AS SRN_Date , MAX(TSPL_VENDOR_INVOICE_HEAD.Vendor_Invoice_No) AS Vendor_Invoice_No, convert(varchar,MAX(TSPL_VENDOR_INVOICE_HEAD.Vendor_Invoice_Date),103) AS  Invoice_Date,  " &
                  " 	 case when MAX(TSPL_PI_HEAD.Status)=0 then 'Pending' else 'Approved'end as status, " &
                 " convert(varchar,MAX( TSPL_PI_HEAD.Posting_Date),103) AS Posting_Date, MAX(TSPL_PI_HEAD.PI_Total_Amt) as PJV_Amount, isnull(MAX(TSPL_PI_REMITTANCE.Actual_Total_TDS),0)as PJV_TDS_Amount,  MAX(TSPL_PI_HEAD.PI_Total_Amt) as PJV_Net_Amount, MAX(TSPL_JOURNAL_MASTER.Source_Narration) as  Narration, MAX(TSPL_JOURNAL_MASTER.Created_By) AS Created_By, MAX(TSPL_JOURNAL_DETAILS.Detail_Line_No) AS Line_No,  MAX(TSPL_JOURNAL_DETAILS.Account_code) AS  GL_Account_Code, MAX(TSPL_JOURNAL_DETAILS.Account_Desc) AS GL_Account_Desc, " &
                 " SUM(TSPL_JOURNAL_DETAILS.Amount) AS Expr1,case when SUM(TSPL_JOURNAL_DETAILS.Amount)<0 then -1 * SUM(TSPL_JOURNAL_DETAILS.Amount) else 0 end as Credit,case when SUM(TSPL_JOURNAL_DETAILS.Amount)>=0 then SUM(TSPL_JOURNAL_DETAILS.Amount) else 0 end as Debit ," &
                 " MAX(TSPL_SRN_HEAD.Bill_To_Location) AS Bill_To_Location, MAX( TSPL_COMPANY_MASTER.Comp_Name) AS Comp_Name, MAX(TSPL_COMPANY_MASTER.Add1) AS Add1, max(TSPL_COMPANY_MASTER.Add2) as Add2, max(TSPL_COMPANY_MASTER.Add3) as Add3,   max(TSPL_COMPANY_MASTER.State) as State, max(TSPL_COMPANY_MASTER.CST_LST) as CST_LST,MAX(TSPL_COMPANY_MASTER.Comp_Code) AS Comp_Code," &
                " max(TSPL_VENDOR_MASTER.Vendor_Name) AS Expr2, max(TSPL_VENDOR_MASTER.Add1) AS Expr3, max(TSPL_VENDOR_MASTER.Add2) AS Expr4,  max(TSPL_VENDOR_MASTER.Add3) AS Expr5, max(TSPL_VENDOR_MASTER.City_Code_Desc) as City_Code_Desc ,convert(varchar,max(TSPL_PI_HEAD.Due_Date),103) as Due_Date,convert(varchar,max(TSPL_VENDOR_INVOICE_HEAD.Vendor_Invoice_Date),103)  as Vendor_Invoice_Date, " &
                 " MAX(TSPL_PI_HEAD.Comments) AS Comments ,MAX(TSPL_PI_HEAD.Dept_Desc) AS Dept_Desc,MAX(TSPL_PI_HEAD.TransporterDesc) AS TransporterDesc ,MAX(TSPL_PI_HEAD.VehicleDesc ) AS VehicleDesc " &
                  " ,MAX(TSPL_PI_HEAD.TapalNo) as TapalNo,MAX(TSPL_PI_HEAD.DateAndTime) as DateAndTime FROM  TSPL_PI_HEAD LEFT OUTER JOIN TSPL_VENDOR_INVOICE_HEAD ON TSPL_PI_HEAD.PI_No=TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No and TSPL_VENDOR_INVOICE_HEAD.Document_Type='I' " &
                  " LEFT OUTER JOIN TSPL_JOURNAL_MASTER ON TSPL_VENDOR_INVOICE_HEAD.Document_No=TSPL_JOURNAL_MASTER.Source_Doc_No " &
                  " LEFT OUTER JOIN  TSPL_JOURNAL_DETAILS ON TSPL_JOURNAL_MASTER.Voucher_No = TSPL_JOURNAL_DETAILS.Voucher_No " &
                   " LEFT OUTER JOIN  TSPL_SRN_HEAD on TSPL_PI_HEAD.Against_SRN= TSPL_SRN_HEAD.SRN_NO " &
                    ...
                    " left outer join tspl_state_master as tspl_state_master_for_location_state on tspl_state_master_for_location_state.state_code=tspl_location_master.state left outer join TSPL_STATE_MASTER on TSPL_VENDOR_MASTER.State_Code= TSPL_STATE_MASTER.State_Code " &
                    " LEFT OUTER JOIN TSPL_PURCHASE_ORDER_HEAD ON TSPL_PI_HEAD.Against_PO=TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_No  " &
                    ...
                    " left outer join TSPL_USER_MASTER as TSPL_USER_MASTER_Modified_By on TSPL_USER_MASTER_Modified_By.User_Code = TSPL_PI_HEAD.Modify_By " &
                    " left outer join TSPL_PI_REMITTANCE on TSPL_PI_HEAD.PI_No=TSPL_PI_REMITTANCE.Document_No " &
                     " LEFT OUTER JOIN  TSPL_COMPANY_MASTER ON TSPL_PI_HEAD.Comp_Code = TSPL_COMPANY_MASTER.Comp_Code CROSS APPLY " &
                      " (SELECT distinct TSPL_PI_DETAIL.SRN_Id + ',' AS [text()]   FROM TSPL_PI_HEAD left outer join  TSPL_PI_DETAIL on TSPL_PI_HEAD.PI_No=TSPL_PI_DETAIL.PI_No  where TSPL_PI_DETAIL.PI_No='" & StrDocNo & "' FOR XML PATH(''))el(files) where TSPL_PI_HEAD.PI_No='" & StrDocNo & "' group by  TSPL_JOURNAL_DETAILS.Account_code) as t1 " &
