'KUNAL >TICKET TASK : DEVELOPED  NEW SCREEN > DATE : 15-NOV-2016

Imports common
Imports Microsoft.Office.Interop
Imports System.IO

Public Class frmNRGPBooking
    Inherits FrmMainTranScreen
#Region "Variables"

    Private MainFormId As String = clsUserMgtCode.mbtnNRGP
    Private Trans_Type As String = "Request"
...
#End Region
    ...
    Private Sub FrmAPInvoiceEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If clsCommon.CompairString(clsCommon.myCstr(MainFormId), clsCommon.myCstr(clsUserMgtCode.mbtnNRGP)) = CompairStringResult.Equal Then
                ...
                loadtype()
            ElseIf clsCommon.CompairString(clsCommon.myCstr(MainFormId), clsCommon.myCstr("")) = CompairStringResult.Equal Then
    ...
    Sub LoadBlankGrid()
        ...
        CSA_ITEM_TYPE.HeaderText = "Item Type"
        ...
        CSA_ITEM_TYPE.DataSource = cls_TSPL_NRGP_REQUEST_HEAD.LoadItemCSAType()
        ...
        TAX_PAID.DataSource = cls_TSPL_NRGP_REQUEST_HEAD.GetTaxPaid()
        ...
        BOOK_RATE_UOM.HeaderText = "UOM (Of Rate)"
    ...
    Sub funReset()
        Try
            ...
            Me.txtDesc.Text = ""
            ' txtDesc.Text = cls_TSPL_NRGP_REQUEST_HEAD.GetBookingDescrptn(Trans_Type)
            ...
            For Each grow As GridViewRowInfo In gv1.Rows
                If clsCommon.myLen(grow.Cells(colCSA_ITEM_TYPE).Value) > 0 Then
                    ...
                    If clsCommon.myLen(grow.Cells(colBOOK_QTY_UOM).Value) <= 0 Then
                        ...
                        Return False
                    End If
                    If clsCommon.CompairString(clsCommon.myCstr(MainFormId), clsCommon.myCstr("")) = CompairStringResult.Equal Then
                    ...
                    If clsCommon.myLen(grow.Cells(colBOOK_RATE_UOM).Value) <= 0 Then
                        If clsCommon.CompairString(clsUserMgtCode.mbtnNRGP, MainFormId) = CompairStringResult.Equal Then
                        ...
                        End If
                    End If
                    If clsCommon.CompairString("", MainFormId) = CompairStringResult.Equal Then
    ...
    Function SaveData(ByVal ChekBtnPost As Boolean) As Boolean
        Try
            ...
            If AllowToSave() Then
                ...
                If lbxCsaVendor.Text = "Vendor" Then
                    obj.VEN_CODE = Me.txtBatchNo.Value
                    obj.CSA_CODE = ""
                Else
                ...
                Dim obj1 As cls_TSPL_NRGP_REQUEST_DETAIL
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colCSA_ITEM_TYPE).Value)) > 0 Then
                        obj1 = New cls_TSPL_NRGP_REQUEST_DETAIL()
    ...
    Public Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            obj = New cls_TSPL_NRGP_REQUEST_HEAD()
            obj = cls_TSPL_NRGP_REQUEST_HEAD.GetData(strCode, arrLoc, NavTyep, Trans_Type)
            If obj IsNot Nothing AndAlso (clsCommon.myLen(obj.CSA_CODE) > 0 Or clsCommon.myLen(obj.VEN_CODE) > 0) Then
                ...
                If (obj.VEN_CODE.Length > 0) Then
                    Me.txtBatchNo.Value = obj.VEN_CODE
                    txtCSAName.Text = clsDBFuncationality.getSingleValue("select TSPL_VENDOR_MASTER.Vendor_Name  from TSPL_VENDOR_MASTER where Vendor_Code = '" + obj.VEN_CODE + "'")
                Else
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtCode._MYValidating
        ...
        check = cls_TSPL_NRGP_REQUEST_HEAD.CheckValidCode(Me.txtCode.Value)
        ...
        If txtCode.MyReadOnly Or isButtonClicked Then
            Dim whrCls As String = " TSPL_NRGP_REQUEST_HEAD.trans_type='" + Trans_Type + "' "

            If clsCommon.CompairString(MainFormId, clsUserMgtCode.mbtnNRGP) = CompairStringResult.Equal Then
                whrCls += " and TSPL_NRGP_REQUEST_HEAD.Location_Code in (" + arrLoc + ") "
            End If

            txtCode.Value = clsCommon.myCstr(cls_TSPL_NRGP_REQUEST_HEAD.GetFinder(whrCls, txtCode.Value, isButtonClicked))
            LoadData(txtCode.Value, NavigatorType.Current)
        Else
            funReset()
        End If
        ...
        Try
            ...
            ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
                ButtonToolTip.SetToolTip(btnSave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine + _
                                             "TSPL_NRGP_REQUEST_HEAD " + Environment.NewLine + _
                                             "TSPL_NRGP_REQUEST_DETAIL ")
                'Add Tool tip Task No- TEC/22/05/18-000245
            End If
            ...
            If e.KeyCode = Keys.F2 AndAlso gv1.CurrentColumn IsNot Nothing AndAlso gv1.CurrentColumn Is gv1.Columns(colItemCode) Then
                ...
                gv1.CurrentRow.Cells(colCSA_ITEM_TYPE).Value = cls_TSPL_NRGP_REQUEST_HEAD.GetCSAType(clsCommon.myCstr(gv1.CurrentRow.Cells(colItemCode).Value))
                isCellValueChangedOpen = False
            End If
    ...
    Private Sub txtBatchNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtBatchNo._MYValidating
        Try
            If lbxCsaVendor.Text = "Vendor" Then
                ...
                Dim whrcls As String = ""
                txtBatchNo.Value = clsCommon.ShowSelectForm("RGPVeFNDer", qry, "Code", whrcls, txtBatchNo.Value, "Code", isButtonClicked)
                txtCSAName.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Vendor_Name from TSPL_VENDOR_MASTER where Vendor_Code ='" + txtBatchNo.Value + "'"))
            ElseIf lbxCsaVendor.Text = "CSA" Then
                lblBatchNo.Text = "CSA Code"
                Me.txtBatchNo.Value = clsCustomerMaster.getFinder("CSA_Type='N'", txtBatchNo.Value, isButtonClicked) ''Change Y->N by kunal on 05/12/2016
