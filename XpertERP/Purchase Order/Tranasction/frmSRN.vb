...
#Region "Variables"
    ...
    Dim NIR_QC As Double = 0
    Dim QC_Status As String
    Dim isSavePost As Boolean = False
#End Region
            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colRate) Then
                        ...
                        End If
                    End If
                    If e.Column Is gv1.Columns(colItemInsuranceAmt) OrElse e.Column Is gv1.Columns(colItemInsurancePer) OrElse e.Column Is gv1.Columns(colInsurancePer) OrElse e.Column Is gv1.Columns(colCategoryType) OrElse e.Column Is gv1.Columns(colCapexSubCode) OrElse e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colLeakQty) OrElse e.Column Is gv1.Columns(colBurstQty) OrElse e.Column Is gv1.Columns(colShortQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colDisPerUnit) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colOrgMRNQty) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeMisc) = CompairStringResult.Equal) Then
                        If (e.Column Is gv1.Columns(colItemInsuranceAmt) OrElse e.Column Is gv1.Columns(colItemInsurancePer) OrElse e.Column Is gv1.Columns(colInsurancePer) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colLeakQty) OrElse e.Column Is gv1.Columns(colBurstQty) OrElse e.Column Is gv1.Columns(colShortQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colDisPerUnit) OrElse e.Column Is gv1.Columns(colOrgMRNQty) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeMisc) = CompairStringResult.Equal)) Then
                    ...
                    If Not isSavePost Then
                        If Math.Round(clsCommon.myCdbl(gv1.CurrentRow.Cells(colOrgMRNQty).Value), 3) < Math.Round((clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value) + clsCommon.myCdbl(gv1.CurrentRow.Cells(colLeakQty).Value) + clsCommon.myCdbl(gv1.CurrentRow.Cells(colBurstQty).Value) + IIf(is_Load_MRN, 0, clsCommon.myCdbl(gv1.CurrentRow.Cells(colShortQty).Value)) + clsCommon.myCdbl(gv1.CurrentRow.Cells(colRejectedQty).Value)), 3) Then
                            Throw New Exception("Received Quantity( " + clsCommon.myCstr(clsCommon.myCdbl(gv1.CurrentRow.Cells(colOrgMRNQty).Value)) + " ) of Item " + clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value) + "( " + clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value).Trim() + " ) should be equal to Total Quantity(Accepted+Leak+Burst+" + IIf(is_Load_MRN, "", "short+") + "Reject)(" + clsCommon.myCstr((clsCommon.myCdbl(gv1.CurrentRow.Cells(colQty).Value) + clsCommon.myCdbl(gv1.CurrentRow.Cells(colLeakQty).Value) + clsCommon.myCdbl(gv1.CurrentRow.Cells(colBurstQty).Value) + IIf(is_Load_MRN, 0, clsCommon.myCdbl(gv1.CurrentRow.Cells(colShortQty).Value)) + clsCommon.myCdbl(gv1.CurrentRow.Cells(colRejectedQty).Value))) + ").At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(gv1.CurrentRow.Index + 1)) + " ")
                        End If
                    End If
                    isCellValueChangedOpen = False
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                dblRejectQty += clsCommon.myCdbl(gv1.Rows(ii).Cells(colRejectedQty).Value)
                If clsCommon.myLen(strICode) > 0 Then
                ...
                End If
            Next
            If dblRejectQty <= 0 AndAlso clsCommon.CompairString(QC_Status, "Rejected") = CompairStringResult.Equal Then
                Throw New Exception("Please fill rejected qty as material is rejected in QC")
            End If
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBillToLocation._MYValidating
        ...
        Try
            ...
            Else
                WhrCls = " Rejected_Type='N' and (Location_Type='Physical' or Location_Type='WorkOrder')  "
            End If
            If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
 ...
 " where TSPL_SRN_HEAD.SRN_No='" + txtDocNo.Value + "'"
    ...
    Private Sub gv1_CellFormatting(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.CellFormattingEventArgs) Handles gv1.CellFormatting
        Try
            ...
            If e.Column.Index >= 0 Then
                ...
                End If
                ''=======================================================
                If clsCommon.CompairString(QC_Status, "Rejected") <> CompairStringResult.Equal Then
                    gv1.CurrentRow.Cells(colRejectedQty).ReadOnly = True
                Else
                    gv1.CurrentRow.Cells(colRejectedQty).ReadOnly = False
                End If
            End If
        Catch ex As Exception
            'common.clsCommon.MyMessageBoxShow(me,ex.Message,me.text)
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        Try
            ...
            Else
                dblShortageQty = 0
                dblRejectedQty = clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colRejectedQty).Value)
            End If
            ...
            gv1.Rows(IntRowNo).Cells(colRejectedQty).Value = Math.Round(dblRejectedQty, 2)
