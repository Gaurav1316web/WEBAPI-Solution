...
group by Route_no"
            ...
            Else
                ...
                strBaseqry = "select TSPL_MULTIPLE_DEDUCTION_detail.Vendor_Code,TSPL_MULTIPLE_DEDUCTION_detail.Vendor_Name,case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then 'A' else 'D' end Type,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_No,convert(varchar,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) as Document_Date  ,case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then TSPL_MULTIPLE_DEDUCTION_detail.amount else 0 end as Addition,case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then 0 else TSPL_MULTIPLE_DEDUCTION_detail.amount  end as Deduction,TSPL_MULTIPLE_DEDUCTION_detail.DeductionCode,TSPL_MULTIPLE_DEDUCTION_detail.Deduction_Desc ,TSPL_VLC_MASTER_HEAD.VLC_CODE_VLC_Uploader as [VLC Uploader Code]  
...
where convert(date,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) >= convert(date,('" + fromDate.Value + "'),103) and convert(date,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) <= convert(date,('" & ToDate.Value & "'),103) "
...
and TSPL_SD_SHIPMENT_HEAD.Is_CashSale='N' and TSPL_SD_SHIPMENT_HEAD.Status=1 "
                    ...
                    End If
                    If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 Then
                        strBaseqry += " and TSPL_MCC_MASTER.MCC_Code in (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ")"
                    End If
                    If txtMultiVSP.arrValueMember IsNot Nothing AndAlso txtMultiVSP.arrValueMember.Count > 0 Then
                        strBaseqry += " and TSPL_CUSTOMER_VENDOR_MAPPING.Vendor_Code in (" + clsCommon.GetMulcallString(txtMultiVSP.arrValueMember) + ")"
                    End If
                    If clsCommon.myLen(fndArea.Value) > 0 Then
                        strBaseqry += " And TSPL_MCC_MASTER.Area_Location_Code = '" + fndArea.Value + "' "
                    End If
                    ...
                    strBaseqry += " union all
SELECT TSPL_VENDOR_INVOICE_HEAD.Vendor_Code ,TSPL_VLC_MASTER_HEAD.VLC_Name as Vendor_Name,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type ='C' then 'A' else 'D' end as Type,TSPL_VENDOR_INVOICE_HEAD.Document_No,convert(varchar,TSPL_VENDOR_INVOICE_HEAD.Posting_Date,103) as Document_Date,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type ='C' then TSPL_VENDOR_INVOICE_HEAD.Document_Total else 0 end as Addition, case when TSPL_VENDOR_INVOICE_HEAD.Document_Type ='C' then 0 else TSPL_VENDOR_INVOICE_HEAD.Document_Total end AS Deduction,TSPL_DEDUCTION_MASTER.Code as DeductionCode,TSPL_DEDUCTION_MASTER.Description as Deduction_Desc,cast(TSPL_VLC_MASTER_HEAD.VLC_Code_VLC_Uploader as integer) as [VLC Uploader Code]
...
where TSPL_VENDOR_INVOICE_HEAD.RefDocType in ('CAP-MSN-CDCS','CAP-MSN','CAP-OMSN','SUS-ADJD') 
...
and  TSPL_VENDOR_INVOICE_HEAD.Posting_Date is not null "
                    ...
                    End If
                    If txtMultiVSP.arrValueMember IsNot Nothing AndAlso txtMultiVSP.arrValueMember.Count > 0 Then
                        strBaseqry += " and TSPL_VENDOR_INVOICE_HEAD.Vendor_Code in (" + clsCommon.GetMulcallString(txtMultiVSP.arrValueMember) + ")"
                    End If
                    If clsCommon.myLen(fndArea.Value) > 0 Then
                        strBaseqry += " And TSPL_MCC_MASTER.Area_Location_Code = '" + fndArea.Value + "' "
                    End If
    ...
    Sub ReportPrint()
        Try
            ...
            If TxtDeductionCode.arrValueMember IsNot Nothing AndAlso TxtDeductionCode.arrValueMember.Count > 0 Then
                ...
                strqry7 += " and TSPL_DEDUCTION_MASTER.Code in (" + clsCommon.GetMulcallString(TxtDeductionCode.arrValueMember) + ")"
            End If
            If clsCommon.myLen(fndArea.Value) > 0 Then
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "GNG") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JDH") = CompairStringResult.Equal Then
                strQry += "sum(XYZ.Reduce_Deduc_Amt) as Reduce_Deduc_Amt
                        ...
                        CASE
                          WHEN MAX(final3.Type) = 'd'  THEN sum(xyz.amt)
                          WHEN MAX(final3.Type) = 'a' THEN SUM(final3.Addition)
                        END AS 'ReduceAmts'"
            Else

                strQry += "sum(TSPL_PAYMENT_PROCESS_DEDUCTION.Reduce_Deduc_Amt) as Reduce_Deduc_Amt,
                        CASE
                          WHEN max(final3.Type) = 'd' THEN (sum(FINAL3.Deduction) - SUM(TSPL_PAYMENT_PROCESS_DEDUCTION.Reduce_Deduc_Amt))
                          WHEN max(final3.Type) = 'a' THEN sum(final3.Addition)
                        END AS 'ReduceAmt' "
            End If
            strQry += " from (
                          SELECT  FINAL2.[Vendor Code],MAX(FINAL2.[Vendor Name])[Vendor Name],MAX(FINAL2.[VLC Uploader Code])[VLC Uploader Code],"
            If AreaWiseBilling = True Then
            ...
            Else
                strQry += " max(FINAL1.MCC_Name)MCC_Name"
            End If
            strQry += " ,FINAL1.[Document Date],FINAL1.[Document No],max(FINAL1.Type)Type,sum(FINAL1.Addition)Addition,sum(FINAL1.Deduction)Deduction,
                         MAX(FINAL1.[Deduction Code])[Deduction Code],max (FINAL1.[Deduction Desc])[Deduction Desc] FROM (
                         Select  final.Vendor_Code as [Vendor Code] ,max(final.Vendor_Name) as [Vendor Name],
            ...
            End If
            strQry += "max(final.Type) as Type,final.Document_No as [Document No],final.Document_Date as [Document Date],
                        sum(final.Addition) as Addition,sum(final.Deduction) as Deduction,final.DeductionCode as [Deduction Code] ,
                        max(final.Deduction_Desc) as [Deduction Desc] From 
                        ( select "
            If AreaWiseBilling = True Then
            ...
            strQry += " TSPL_MULTIPLE_DEDUCTION_detail.Vendor_Code,TSPL_MULTIPLE_DEDUCTION_detail.Vendor_Name,
                        case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then 'A' else 'D' end Type,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_No,
                        convert(varchar,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) as Document_Date  ,case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then TSPL_MULTIPLE_DEDUCTION_detail.amount else 0 end as Addition,
                        case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then 0 else TSPL_MULTIPLE_DEDUCTION_detail.Amount end  as Deduction, 
                        ...
                        TSPL_MCC_MASTER.MCC_Name,TSPL_MULTIPLE_DEDUCTION_DETAIL.Remarks from TSPL_MULTIPLE_DEDUCTION_HEAD 
            ...
            strQry += "  where TSPL_MULTIPLE_DEDUCTION_HEAD.IsPosted=1 and convert(date,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) >= convert(date,('" + clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") + "'),103) and convert(date,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) <= convert(date,('" + clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") + "'),103) " + strQry3 + strQry1 + strQry5 + "  )Final 
                        group by final.Document_No,final.Document_Date , final.Vendor_Code ,final.DeductionCode  
                        ) FINAL1
                          GROUP BY 
                        FINAL1.[Vendor Code]"
            'If AreaWiseBilling = True Then
            ...
            strQry += " ,FINAL1.[Document Date],FINAL1.[Document No]
                        ) FINAL2 
...
WHERE convert(date,TSPL_VENDOR_INVOICE_HEAD.Posting_Date,103) >=convert(date,('" + clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") + "'),103) and convert(date,TSPL_VENDOR_INVOICE_HEAD.Posting_Date,103) <= convert(date,('" + clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") + "'),103) and final3.Type = 'D'" + strQry4 + " 
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "GNG") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JDH") = CompairStringResult.Equal Then
                strQry += " group by [VLC Uploader Code],[Deduction Code],Reduce_Deduc_Amt )XY 
                        ...
                        sum(TSPL_MILK_SRN_DETAIL.AMOUNT) AS SRN_AMOUNT,sum(TSPL_MILK_SRN_DETAIL.Qty) as [SRN Qty],
						MAX(Reduce_Deduc_Amt)Reduce_Deduc_Amt,MAX(ReduceAmt)ReduceAmt from(
                        select Customer_CODE,max(Customer_NAME)Customer_NAME,max([VLC Uploader Code])[VLC Uploader Code],max(MCC_Name)MCC_Name,max(Document_Date)Document_Date,
                        max(Document_No)Document_No,max(Type)Type,SUM(Addition)Addition,SUM(Deduction)Deduction,MAX(DeductionCode)DeductionCode,MAX(Deduction_Desc)Deduction_Desc
                        ,sum(Reduce_Deduc_Amt)Reduce_Deduc_Amt,max(ReduceAmt)ReduceAmt 
						...
						left outer join TSPL_MILK_SRN_HEAD on TSPL_MILK_SRN_HEAD.VSP_CODE=XY.Customer_CODE
                        left outer join TSPL_MILK_SRN_DETAIL on TSPL_MILK_SRN_DETAIL.DOC_CODE=TSPL_MILK_SRN_HEAD.DOC_CODE
                        WHERE convert(date,TSPL_MILK_SRN_HEAD.DOC_DATE,103) >= convert(date,('" + clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") + "'),103)
                        and convert(date,TSPL_MILK_SRN_HEAD.DOC_DATE,103) <= convert(date,('" + clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") + "'),103) group by XY.Customer_CODE )zz
    ...
    Public Sub DCSWiseDetail()
        Try
            ...
            qry = "  Select (final.Vendor_Code) as [Vendor Code] ,max(final.Vendor_Name) as [Vendor Name],(final.[VLC Uploader Code]) as [VLC Uploader Code],
                     ...
                     from ( select TSPL_MULTIPLE_DEDUCTION_detail.Vendor_Code,TSPL_MULTIPLE_DEDUCTION_detail.Vendor_Name,case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then 'A' else 'D' end Type,
                     TSPL_MULTIPLE_DEDUCTION_HEAD.Document_No,convert(varchar,TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date,103) as Document_Date  ,
                     case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then TSPL_MULTIPLE_DEDUCTION_detail.amount else 0 end as Addition,
                     case when isnull(TSPL_MULTIPLE_DEDUCTION_DETAIL.Trans_Type,'Deduction')='Addition' then 0 else TSPL_MULTIPLE_DEDUCTION_detail.amount  end as Deduction,
