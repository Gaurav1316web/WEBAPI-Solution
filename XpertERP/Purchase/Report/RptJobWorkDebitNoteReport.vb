...
Public Class RptJobWorkDebitNoteReport
    ...
    Public Sub LoadDataQuery() 'As ArrayList
        ...
        dt1 = Nothing
        dt2 = Nothing
        qry1 = "with WholeData as (Select  DISTINCT row_number() over(order by  Secondry_Tbl.date) as r ,case when DENSE_RANK() over(order by Secondry_Tbl.date)%8<>0 then DENSE_RANK() over(order by Secondry_Tbl.date)/8 else (DENSE_RANK() over(order by Secondry_Tbl.date)/8)-1 end  as Group_Date ,  Secondry_Tbl.date as [Second_Date], XX.Comp_Name,XX.Comp_Phone,XX.Comp_Address,XX.Pincode,xx.Tin_No,xx.Pan_No," & _
          ...
          "   Convert(decimal(18,3),(xx.[Net Weight] * xx.[SNF %] /    100)) As SNFKG,xx.Date_And_Time " & _
           "  From ( Select Tspl_Gate_Entry_Details.Date_And_Time," & _
           " TSPL_COMPANY_MASTER.Comp_Code,TSPL_COMPANY_MASTER.Comp_Name,TSPL_COMPANY_MASTER.Add1,TSPL_COMPANY_MASTER.Add2," & _
            ...
            " (case when isnull(TSPL_COMPANY_MASTER.Add1,'')<>'' then TSPL_COMPANY_MASTER.Add1   end   + Case when isnull(TSPL_COMPANY_MASTER.Add2,'')<>'' then ', '+ TSPL_COMPANY_MASTER.Add2 else '' end + case when isnull(TSPL_COMPANY_MASTER.State,'')<>'' then ', ' + TSPL_COMPANY_MASTER.State else '' end  ) as Comp_Address, " & _
             ...
             " LEFT OUTER JOIN TSPL_COMPANY_MASTER ON TSPL_COMPANY_MASTER.Comp_Code=Tspl_Gate_Entry_Details.Comp_Code " & _
              ...
              " and Tspl_Gate_Entry_Details.Doc_Type in ('BulkProc')  and Gate_Entry_Type='J' ) As xx " & _
           ...
           " FROM    sys.all_objects a CROSS JOIN sys.all_objects b ) as Secondry_Tbl on convert(date,Secondry_Tbl.Date,103)=convert(date,xx.Date_And_Time,103) )"
        ...
        qry1 += " select  max(Comp_Name) as Comp_Name,max(Comp_Phone) as Comp_Phone,max(Comp_Address) as Comp_Address,max(Pincode) as Pincode,max(Tin_No) as Tin_No,max(Pan_No) as Pan_No,max([Vendor Name]) as [Vendor Name],max(Vendor_Address) as Vendor_Address " & _
            ",isnull(sum([Net Weight]),0) as [Milk Received Qty (Kg)],isnull(sum(FATKG_in_Mixed),0) as [Qty of Fat (In Mixed Milk (Kg))],isnull(sum(FATKG_in_SkimMilk),0) as [Qty of Fat (In Skim Milk (Kg))],isnull(sum(FATKG_in_Others),0) as [Qty of Fat (In Others (Kg))],isnull(sum(SNFKG),0) as [Qty of SNF (Kg)], CASE WHEN DATEPART(YYYY,min(Second_Date))<>DATEPART(YYYY,dateadd(dd,7,min(Second_Date))) " & _
                   ...
                   "  else CAST(DATEPART(DD,min(Second_Date)) AS VARCHAR)+' '+DATENAME(MM,min(Second_Date))+' - '+CAST(DATEPART(DD,dateadd(dd,7,min(Second_Date))) AS VARCHAR)+' '+DATENAME(MM,dateadd(dd,7,min(Second_Date)))+' '+ DATENAME(YYYY,dateadd(dd,7,min(Second_Date)) ) END  as Date_Range,convert(varchar(15),'" + txtFromDate.Text + "',103) as From_Date,convert(varchar(15),'" + txtToDate.Text + "',103) as To_Date " & _
       ...
       " Pivot( max(Item_Cagetory_Values) for Item_Category_Code   in ( [CATEGORY RM],[BRAND],[SUB BRAND],[DESCRP],[PACK],[PACK SIZE],[CATEGORY OT],[CATEGORY FA],[P TYPE],[L TYPE],[JW],[SCRAP]) " & _
            ...
            " left join tspl_customer_vendor_mapping on tspl_customer_vendor_mapping.Cust_Code=tspl_customer_vendor_mapping.Cust_Code " & _
            " where TSPL_SCRAPSALE_HEAD.Loc_Code='" + fndLocation.Value + "' " & _
            " and tspl_customer_vendor_mapping.Vendor_Code='" + fndcustNo.Value + "' " & _
        ...
        dt2 = clsDBFuncationality.GetDataTable(qry2)
    ...
    Private Sub RptJobWorkDebitNoteReport_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Reset()
        SetUserMgmtNew()
    End Sub
End Class
