...
Public Class FrmRptVendorTransHistory
    ...
    Sub Print()
        ...
        Try
            ...
            If txtVendorGroup.arrValueMember IsNot Nothing AndAlso txtVendorGroup.arrValueMember.Count > 0 Then
                strFIlterCheck += " and XXX.Ven_Group_Code in (" + clsCommon.GetMulcallString(txtVendorGroup.arrValueMember) + ")  " '----------changed here customer is used changed to vendor
            End If
            ...
            If ChkSummary.Checked = False Then
                DocQry += "  ( Select '" + runDate + "'  as RunDate,'" + CompanyAdd + "' as CompanyAddress ,'" + Comp_Name + "' as CompanyName, '" + strFromDate + "' as FilterFromDate,'" + strToDate + "' as FilterToDate, " + Environment.NewLine & _
                          ...
                          " RTRIM(DocNo) AS [DocumentNo] ,DocType  As [Doc Type],DocTypeCode AS [Doc Type Code],xxx.Ven_Group_Code AS [Ven Group Code],xxx.Route_No AS [Route No]" + Environment.NewLine & _
            ...
            Else
                ...
                DocQry += "  ( Select '" + runDate + "'  as RunDate,'" + CompanyAdd + "' as CompanyAddress ,'" + Comp_Name + "' as CompanyName, '" + strFromDate + "' as FilterFromDate,'" + strToDate + "' as FilterToDate, " + Environment.NewLine & _
                            ...
                            " " & DocNo & " AS [DocumentNo] ,MAX(DocType)  As [Doc Type],MAX(DocTypeCode) AS [Doc Type Code],MAX(xxx.Ven_Group_Code) AS [Ven Group Code],MAX(xxx.Route_No) AS [Route No] " + Environment.NewLine & _
                            ...
                            " CASE WHEN ISNULL(MAX([Child Cust Code]),'')<>'' THEN MAX([Child Cust Code]) ELSE MAX(VenCode) END AS MainCustCode, " + Environment.NewLine & _
                            " CASE WHEN ISNULL(MAX([Child Cust Name]),'')<>'' THEN MAX([Child Cust Name])  ELSE MAX(VenName) END AS MainCustName, " + Environment.NewLine & _
                            ...
                            " MAX(BalAmt) AS BalAmt,CASE WHEN ISNULL(MAX(TSPL_VENDOR_MASTER.Parent_Vendor_Code ),'') <> '' THEN ISNULL(MAX(TSPL_VENDOR_MASTER.Parent_Vendor_Code ),'') ELSE MAX(VenCode) END AS ParentCode   "
    ...
    Private Function GetReport_id()
        ...
        If (ChkSummary.Checked = True) Then
            reportid += IIf(rbtnDocWise.Checked = True, "D", "V")
        End If
        Return reportid
    End Function
    ...
    Private Sub FunExport(ByVal exporter As EnumExportTo)
        Try
            ...
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_code='" & clsUserMgtCode.FrmRptVendorTransList & "'"))
