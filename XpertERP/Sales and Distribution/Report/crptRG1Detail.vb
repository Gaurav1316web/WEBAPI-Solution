...
Public Class frmRG1
    ...
    Private Sub btnPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            If chkRG1MRPWise.IsChecked = True Then
                printMRPWISE()
            Else
                print()
            End If
    ...
    Sub print()
        ...
        Qry += "Select TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No,0 as op,TSPL_SALE_INVOICE_HEAD .Comp_Code,  TSPL_SALE_INVOICE_HEAD.Date_Time_Removal as [Sale_Invoice_Date], TSPL_SALE_INVOICE_DETAIL.Item_Code,  TSPL_SALE_INVOICE_DETAIL.Item_Desc ," & _
              ...
              " union  all  "
        Qry += " SELECT  hd.Transfer_No , 0,hd.Comp_Code  ,hd.EntryDateTime ,  dl.Item_Code ,dl.Item_Desc ,dl.Item_Qty ," & _
        ...
        'Qry += "SELECT  hd.Document_No,0,hd.Comp_Code  ,(Leakage_Qty + Breakage_Qty + Shortage_Qty) as WHQty,0,hd.Document_Date , " & _
        ...
        Qry = Qry + " and xxxx.Item_Code in (select Item_Code  from TSPL_ITEM_MASTER where Item_Type ='F') order by xxxx.Item_Code  ,xxxx.Sale_Invoice_Date  "
        ...
        For Each dr As DataRow In dtData.Rows
            ...
            CloseBal = 0
            'CloseBal = CDec(OpenBal) - CDec(dr("Qty").ToString()) + CDec(dr("ManuQty").ToString())
            'If CloseBal < 0 Then
            '    CloseBal = CloseBal * -1
            'End If
        ...
        ElseIf rdbDetail.IsChecked Then
            CrptName = "crptRG1Details"
        ElseIf rdbDetailMRP.IsChecked Then
            CrptName = "crptRG1DetailsDateMRPwise"
        End If
    ...
    Sub printMRPWISE()
        Try
            ...
            Qry += " select 0 as WHQty,0, TSPL_ADJUSTMENT_HEADER.Comp_Code,convert(dateTIME,TSPL_ADJUSTMENT_HEADER.EntryDateTime,103) ," & _
                                ...
                                " TSPL_ADJUSTMENT_DETAIL.Item_Quantity  from TSPL_ADJUSTMENT_DETAIL inner join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_DETAIL.Adjustment_No =TSPL_ADJUSTMENT_HEADER .Adjustment_No " & _
                                " LEFT OUTER JOIN TSPL_ITEM_UOM_DETAIL ON TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ADJUSTMENT_DETAIL.Item_Code " & _
                                " where Adjustment_Type in ('BI','BD','QI') and TSPL_ADJUSTMENT_HEADER.ItemType='FM'   and " & _
            ...
            Qry += "SELECT case when TSPL_SALE_INVOICE_DETAIL.Unit_code = 'FB' " & _
                                ...
                                " TSPL_SALE_INVOICE_DETAIL.Sale_Invoice_No INNER JOIN    TSPL_LOCATION_MASTER ON TSPL_SALE_INVOICE_HEAD.Location =   TSPL_LOCATION_MASTER.Location_Code" & _
                                " where   (convert(datetime,Date_Time_Removal,103) < '" + startDate + "') and   TSPL_SALE_INVOICE_HEAD.Is_Post='Y' " & _
                                " union all "
            ...
            Qry += "Select  case when TSPL_ADJUSTMENT_DETAIL.Unit_code = 'FB' then  TSPL_ADJUSTMENT_DETAIL.mrp else " & _
                                ...
                                " TSPL_ADJUSTMENT_HEADER.Loc_Code, convert(datetime,TSPL_ADJUSTMENT_HEADER.Posting_Date,103) ,TSPL_ADJUSTMENT_DETAIL.Item_Code, " & _
                                "case when (Adjustment_Type='BI' or Adjustment_Type='QI')  then (Case When TSPL_ADJUSTMENT_DETAIL.Unit_Code='FB' Then TSPL_ADJUSTMENT_DETAIL.Item_Quantity/Conversion_Factor else TSPL_ADJUSTMENT_DETAIL.Item_Quantity End) Else Case when Adjustment_Type='BD' then (Case When TSPL_ADJUSTMENT_DETAIL.Unit_Code='FB' Then (TSPL_ADJUSTMENT_DETAIL.Item_Quantity/Conversion_Factor)*-1 Else TSPL_ADJUSTMENT_DETAIL.Item_Quantity*-1  end) end End  , " & _
                                "TSPL_ADJUSTMENT_HEADER.Comp_Code     from  TSPL_ADJUSTMENT_DETAIL inner join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_DETAIL.Adjustment_No =TSPL_ADJUSTMENT_HEADER .Adjustment_No" & _
       " LEFT OUTER JOIN TSPL_ITEM_UOM_DETAIL ON TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ADJUSTMENT_DETAIL.Item_Code  " & _
       " where   TSPL_ITEM_UOM_DETAIL.UOM_Code='FB' AND Adjustment_Type in ('BI','BD','QI') and " & _
            ...
            Qry = Qry + " and xxxx.Item_Code in (select Item_Code  from TSPL_ITEM_MASTER where Item_Type ='F') group by xxxx.Item_Code ,xxxx.MRP_Amt  "
            ...
            If chkSummary.Checked = True Then
                CrptName = "crptRG1Summary"
            Else
                CrptName = "crptRG1DetailsMRPWise"
            End If
    ...
    Private Sub chkRG1MRPWise_CheckedChanged_1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        btnExportToExcel.Visible = False
        If chkRG1MRPWise.IsChecked = True Then
    ...
    Private Sub chkRG1MRPWise_ToggleStateChanged(ByVal sender As Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles chkRG1MRPWise.ToggleStateChanged
        btnExportToExcel.Visible = False
        If chkRG1MRPWise.IsChecked = True Then
            chkSummary.Checked = False
            btnExportToExcel.Visible = True
    ...
    Private Sub rdbDetailMRP_ToggleStateChanged(ByVal sender As Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles rdbDetailMRP.ToggleStateChanged
        If chkRG1MRPWise.IsChecked = True Then
            chkSummary.Checked = False
            btnExportToExcel.Visible = True
        Else
