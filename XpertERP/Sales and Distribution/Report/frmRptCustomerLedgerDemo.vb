            ...
            If btnrefresh = True Then
                ...
                Else
                    ...
                    Else
                        ...
                        strQry = "WITH CTETemp as (" + Environment.NewLine & _
                        " Select " & IIf(AllowtoSHOWParentChildCustomer = True, " ROW_NUMBER() OVER (order BY ACode) ", "ROW_NUMBER() OVER (PARTITION BY ACode ORDER BY ACode, DocDate)") & "  as RowNo, 'Detail' as strType, '" + runDate + "' as RunDate,'" + CompanyAdd + "' as CompanyAddress ,'" + compname + "' as CompanyName, '" + strFromDate + "' as FilterFromDate,'" + strToDate + "' as FilterToDate,Parent_Master.Customer_Name as ParentName, " + Environment.NewLine & _
                       ...
                       "MAX( TSPL_LOCATION_MASTER.Location_Desc)  as LocDesc, '' as Item_Code, '' as Item_Desc, MAX(Final.Currency_Code) as Currency_Code, MAX(Final.ConvRate) as ConvRate, SUM(convert(decimal(18,2),DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " Final." & ddlCurrencyType.SelectedValue & "") & "))  -  case when DocType <>'EXC' then sum(isnull(final.EXCHANGE_GAIN_AMT,0) ) else 0 end +  case when DocType <>'EXC' then sum(isnull(final.EXCHANGE_LOSS_AMT ,0) ) else 0 end as DrAmt," + Environment.NewLine & _
