...
Imports common
Public Class FrmInvoiceswithoutreceipt
    Inherits FrmMainTranScreen
    Dim ButtonToolTip As ToolTip = New ToolTip()
    Dim userCode, companyCode As String
    Sub LoadReceipt()
        Dim strquery As String = " SELECT DISTINCT Sale_Invoice_No AS [Invoice No],convert(varchar,Sale_Invoice_Date ,103) as [Description] FROM TSPL_SALE_INVOICE_HEAD  where (Sale_Invoice_Date >= Convert(DATE,'" + dtpfrom.Value + "',103) AND Sale_Invoice_Date <= CONVERT(DATE,'" + dtpto.Value + "',103))"
        cbgReceipt.DataSource = clsDBFuncationality.GetDataTable(strquery)
        cbgReceipt.ValueMember = "Invoice No"
    ...
    Sub Print(ByVal exporter As EnumExportTo)
        ...
        Try
            ...
            Dim strquery As String = "  select TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No   as [INVOICE NO],convert(varchar,TSPL_SALE_INVOICE_HEAD.Sale_Invoice_Date ,103) as [INV.DATE],TSPL_SALE_INVOICE_HEAD.Vehicle_Code as [Vehicle Code] ,TSPL_EMPLOYEE_MASTER.Emp_Name as [SALESMAN NAME],TSPL_SALE_INVOICE_HEAD.Cust_Name  as [CUSTOMER NAME],isnull (TSPL_SALE_INVOICE_HEAD.Total_Invoice_Amt,0) as [INV. AMOUNT]   from TSPL_SALE_INVOICE_HEAD   "
            strquery += " left outer join  TSPL_EMPLOYEE_MASTER on TSPL_EMPLOYEE_MASTER.EMP_CODE=TSPL_SALE_INVOICE_HEAD.Salesman_Code   "
            strquery += "      WHERE Not EXISTS "
            strquery += " (SELECT Document_No  "
            strquery += "    FROM TSPL_RECEIPT_DETAIL "
            strquery += " WHERE TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No = TSPL_RECEIPT_DETAIL.Document_No)"
            If clsCommon.myLen(fromdate) > 0 Then
                strquery += " and convert(date,TSPL_SALE_INVOICE_HEAD.Sale_Invoice_Date,103) >= convert(date,'" + fromdate + "',103)"
            End If

            If clsCommon.myLen(todate) > 0 Then
                strquery += " and convert(date,TSPL_SALE_INVOICE_HEAD.Sale_Invoice_Date,103) <= convert(date,'" + todate + "',103)"
            End If

            If chkReceiptSelect.IsChecked AndAlso cbgReceipt.CheckedValue.Count > 0 Then
                strquery += "and TSPL_SALE_INVOICE_HEAD.Sale_Invoice_No in(" + clsCommon.GetMulcallString(cbgReceipt.CheckedValue) + ")"
            End If
            ...
            End If
            strquery += " order by TSPL_SALE_INVOICE_HEAD.Sale_Invoice_Date"
    ...
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.receiptWOTreport)
