...
Public Class frmScrapSaleReturn
...
#Region "Variables"
    ...
    'sanjay
    Const colShipmentNo As String = "ShipmentNo"
    Const colshQty As String = "shQty"
    Const colPendingQty As String = "PendingQty"
    'sanjay
...
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        'sanjay
        Dim shQty As GridViewDecimalColumn = New GridViewDecimalColumn()
        shQty = New GridViewDecimalColumn()
        shQty.FormatString = ""
        shQty.HeaderText = "Shipped Qty"
        shQty.Name = colshQty
        shQty.IsVisible = True
        shQty.Minimum = 0
        shQty.Width = 80
        shQty.ReadOnly = True
        shQty.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gv1.MasterTemplate.Columns.Add(shQty)
        ...
        pendingQty.HeaderText = "Pending Qty"
        pendingQty.Name = colPendingQty
        ...
        shippedQty.HeaderText = "Return Qty"
        ...
        gv1.MasterTemplate.Columns.Add(repoShipmentNo)
        'SANJAY
        ...
        Try
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    End If
                    If gv1.CurrentColumn Is gv1.Columns(colICode) OrElse gv1.CurrentColumn Is gv1.Columns(colQty) OrElse gv1.CurrentColumn Is gv1.Columns(colprice) OrElse gv1.CurrentColumn Is gv1.Columns(colDisPer) OrElse gv1.CurrentColumn Is gv1.Columns(colDisAmt) OrElse gv1.CurrentColumn Is gv1.Columns(colUnit) Then
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If btnSave.Text = "Update" Then
                Dim strchk As String = "select ispost from tspl_scrapsale_head_return where Document_No='" + txtDocNo.Value + "'"
    ...
    Sub SaveData()
        Try
            ...
            If (AllowToSave()) Then
                ...
                Dim obj As New ClsScrapSaleHeadReturn()
                obj.invoice_No = lblInvoiceNo.Text
                obj.Document_No = txtDocNo.Value
                obj.shipment_No = txtShipmentNo.Value
                ...
                End If
                obj.IsEinvoice = IIf(chkEInvoice.Checked, 1, 0)
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New ClsScrapSaleDetailReturn()
                    objTr.Line_No = clsCommon.myCdbl(grow.Cells(colLineNo).Value)
                    objTr.shipment_No = clsCommon.myCstr(grow.Cells(colShipmentNo).Value) 'txtShipmentNo.Value
                    ...
                    objTr.pending_Qty = clsCommon.myCdbl(grow.Cells(colPendingQty).Value)
                    ...
                    End If
                Next
                If (obj.Arr1 Is Nothing OrElse obj.Arr1.Count <= 0) Then
                ...
                If (obj.SaveData(obj, lblInvoice.Text, isNewEntry)) Then
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            'fndLocation.Enabled = False
            Dim obj As New ClsScrapSaleHeadReturn()
            obj = ClsScrapSaleHeadReturn.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_No) > 0) Then
                ...
                Else
                    chkOnHold.Checked = False
                End If
                txtShipmentNo.Value = obj.shipment_No
                lblInvoice.Text = obj.invoice_No
                ...
                Else
                    cmbSaleType.Text = "Normal Sale"
                End If
                cmbSaleType.Enabled = False
                chkEInvoice.Checked = IIf(obj.IsEinvoice, True, False)
                chkIsEwaybill.Checked = IIf(obj.IsEwayBill, True, False)
                fndGateEntryNo.Value = obj.Gate_Entry_No
                Dim isEnableDisable As Boolean = CheckInvoiceWithGateEntry(obj.shipment_No, obj.Gate_Entry_No) 'clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" select count(*) from TSPL_Sale_Return_Gate_Entry_Invoice_Wise where Gate_Entry_No ='" + obj.Gate_Entry_No + "'  and Invoice_No = '" + obj.shipment_No + "'"))
                ...
                EInvoiceType = clsERPFuncationality.GetCustomerEInvoiceTypeFromTransationTable("TSPL_SCRAPSALE_HEAD_RETURN", "Document_No", obj.Document_No, Nothing)
                ...
                If clsCommon.myLen(txtShipmentNo.Value) > 0 Then
                ...
                If obj.Arr1 IsNot Nothing AndAlso obj.Arr1.Count > 0 Then
                    For Each objTr As ClsScrapSaleDetailReturn In obj.Arr1
                        ...
                        'sanjay
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colShipmentNo).Value = objTr.shipment_No
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colshQty).Value = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select isnull(shipped_Qty,0) from TSPL_SCRAPSALE_DETAIL where shipment_No='" + objTr.shipment_No + "' and Item_Code='" + objTr.Item_Code + "' and Unit_code='" + objTr.Unit_Code + "'"))
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = objTr.pending_Qty
                        'sanjay
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (ClsScrapSaleHeadReturn.DeleteData(txtDocNo.Value)) Then
            ...
            For Each dr As DataRow In dt.Rows
                ...
                If rbtnTaxCalAutomatic.IsChecked OrElse rbtnManualTCS.IsChecked Then
                    ...
                    If clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Is_TCS  from tspl_tax_master where tax_code ='" & clsCommon.myCstr(dr("Tax_Code")) & "' ")), "Y") = CompairStringResult.Equal Then
                        If clsCommon.CompairString(cmbSaleType.Text, "Scrap Sale") = CompairStringResult.Equal Then
                                    ...
                                    If dblOutstandingAmount >= AmountToCheckCustomerOutstandingForTCSTax Then
                                        If EnableTCSRateValidityFrom01July2021 Then
                                            ...
                                            If Is_ITR_Filled_And_TCSAmountGreater50K = True Then
                                                gv2.Rows(gv2.Rows.Count - 1).Cells(colTTaxRate).Value = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ScrapTCSRateforCustomerWithPanNo, clsFixedParameterCode.ScrapTCSRateforCustomerWithPanNo, Nothing))
                                            Else
                                                gv2.Rows(gv2.Rows.Count - 1).Cells(colTTaxRate).Value = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ScrapTCSRateforCustomerWithoutPanNo, clsFixedParameterCode.ScrapTCSRateforCustomerWithoutPanNo, Nothing))
                                            End If
                                        Else
                                        ...
                                        If AmountToCheckCustomerOutstandingForTCSTax >= 0 Then
                                            ...
                                            If dblOutstandingAmount >= AmountToCheckCustomerOutstandingForTCSTax Then
                                                ...
                                                If EnableTCSRateValidityFrom01July2021 Then
                                                    Dim Is_ITR_Filled_And_TCSAmountGreater50K As Boolean = IIf(clsCommon.myCstr(clsDBFuncationality.getSingleValue(" SELECT CASE WHEN ISNULL(IsTCSGreaterthan50K,0)=1 AND ISNULL(IsITRfilledinLast2Years,0)=1 THEN 1 ELSE 0 END FROM TSPL_CUSTOMER_MASTER WHERE Cust_Code='" & fndcustNo.Value & "'")) = 1, True, False)
                                                    If Is_ITR_Filled_And_TCSAmountGreater50K = True Then
                                                        gv1.CurrentRow.Cells(clsCommon.myCstr("colTaxRate" + strII)).Value = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ScrapTCSRateforCustomerWithPanNo, clsFixedParameterCode.ScrapTCSRateforCustomerWithPanNo, Nothing))
                                                    Else
                                                        gv1.CurrentRow.Cells(clsCommon.myCstr("colTaxRate" + strII)).Value = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ScrapTCSRateforCustomerWithoutPanNo, clsFixedParameterCode.ScrapTCSRateforCustomerWithoutPanNo, Nothing))
                                                    End If
                                                Else
                                                    Dim panno As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select isnull(pan,'')+isnull(Additional3 ,'') as PanNoAdhar from tspl_customer_master where cust_code='" & fndcustNo.Value & "'"))
                                                    'If rbtnManualTCS.IsChecked = False Then
                                        ...
                                        If AmountToCheckCustomerOutstandingForTCSTax >= 0 Then
                                            ...
                                            If dblOutstandingAmount >= AmountToCheckCustomerOutstandingForTCSTax Then
                                                ...
                                                If EnableTCSRateValidityFrom01July2021 Then
                                                    Dim Is_ITR_Filled_And_TCSAmountGreater50K As Boolean = IIf(clsCommon.myCstr(clsDBFuncationality.getSingleValue(" SELECT CASE WHEN ISNULL(IsTCSGreaterthan50K,0)=1 AND ISNULL(IsITRfilledinLast2Years,0)=1 THEN 1 ELSE 0 END FROM TSPL_CUSTOMER_MASTER WHERE Cust_Code='" & fndcustNo.Value & "'")) = 1, True, False)
                                                    If Is_ITR_Filled_And_TCSAmountGreater50K = True Then
                                                        gv1.Rows(intRowNo).Cells(clsCommon.myCstr("colTaxRate" + strII)).Value = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ScrapTCSRateforCustomerWithPanNo, clsFixedParameterCode.ScrapTCSRateforCustomerWithPanNo, Nothing))
                                                    Else
                                                        gv1.Rows(intRowNo).Cells(clsCommon.myCstr("colTaxRate" + strII)).Value = clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ScrapTCSRateforCustomerWithoutPanNo, clsFixedParameterCode.ScrapTCSRateforCustomerWithoutPanNo, Nothing))
                                                    End If
                                                Else
                                                    Dim panno As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select isnull(pan,'')+isnull(Additional3 ,'') as PanNoAdhar from tspl_customer_master where cust_code='" & fndcustNo.Value & "'"))
                                                    If rbtnManualTCS.IsChecked = False Then
    ...
    Private Sub UpdateAllTotals()
        ...
        Dim dbltcstaxbaseamt As Double = 0
                ...
                Select Case (ii)
                    Case 1
                        If (clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_TAX_MASTER WHERE Tax_Code='" & clsCommon.myCstr(gv2.Rows(ii - 1).Cells(colTTaxAutCode).Value) & "' AND Is_TCS ='Y'")) > 0) Then 'AndAlso clsCommon.myCdbl(txttcstaxbaseamount.Value) > 0 AndAlso AllowtoChangeTCSBaseAmount = True Then
                            dbltcstaxbaseamt = clsCommon.myFormat(dblTaxBaseAmt1)
                            'dblTaxBaseAmt1 = clsCommon.myCdbl(txttcstaxbaseamount.Value)
                            dblTaxBaseAmt1 = clsCommon.myCdbl(dbltcstaxbaseamt)
                    ...
                    Case 2
                        If (clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_TAX_MASTER WHERE Tax_Code='" & clsCommon.myCstr(gv2.Rows(ii - 1).Cells(colTTaxAutCode).Value) & "' AND Is_TCS ='Y'")) > 0) Then ' AndAlso clsCommon.myCdbl(txttcstaxbaseamount.Value) > 0 AndAlso AllowtoChangeTCSBaseAmount = True Then
                            dbltcstaxbaseamt = clsCommon.myFormat(dblTaxBaseAmt1 + dblTaxAmt1)
                            'dblTaxBaseAmt2 = clsCommon.myCdbl(txttcstaxbaseamount.Value)
                            dblTaxBaseAmt2 = clsCommon.myCdbl(dbltcstaxbaseamt)
                    ...
                    Case 3
                        If (clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_TAX_MASTER WHERE Tax_Code='" & clsCommon.myCstr(gv2.Rows(ii - 1).Cells(colTTaxAutCode).Value) & "' AND Is_TCS ='Y'")) > 0) Then ' AndAlso clsCommon.myCdbl(txttcstaxbaseamount.Value) > 0 AndAlso AllowtoChangeTCSBaseAmount = True Then
                            dbltcstaxbaseamt = clsCommon.myFormat(dblTaxBaseAmt1 + dblTaxAmt1 + dblTaxAmt2)
                            'dblTaxBaseAmt3 = clsCommon.myCdbl(txttcstaxbaseamount.Value)
                            dblTaxBaseAmt3 = clsCommon.myCdbl(dbltcstaxbaseamt)
                    ...
                    Case 4
                        If (clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_TAX_MASTER WHERE Tax_Code='" & clsCommon.myCstr(gv2.Rows(ii - 1).Cells(colTTaxAutCode).Value) & "' AND Is_TCS ='Y'")) > 0) Then ' AndAlso clsCommon.myCdbl(txttcstaxbaseamount.Value) > 0 AndAlso AllowtoChangeTCSBaseAmount = True Then
                            dbltcstaxbaseamt = clsCommon.myFormat(dblTaxBaseAmt1 + dblTaxAmt1 + dblTaxAmt2 + dblTaxAmt3)
                            'dblTaxBaseAmt4 = clsCommon.myCdbl(txttcstaxbaseamount.Value)
                            dblTaxBaseAmt4 = clsCommon.myCdbl(dbltcstaxbaseamt)
                    ...
                    Case 5
                        If (clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_TAX_MASTER WHERE Tax_Code='" & clsCommon.myCstr(gv2.Rows(ii - 1).Cells(colTTaxAutCode).Value) & "' AND Is_TCS ='Y'")) > 0) Then ' AndAlso clsCommon.myCdbl(txttcstaxbaseamount.Value) > 0 AndAlso AllowtoChangeTCSBaseAmount = True Then
                            dbltcstaxbaseamt = clsCommon.myFormat(dblTaxBaseAmt1 + dblTaxAmt1 + dblTaxAmt2 + dblTaxAmt3 + dblTaxAmt4)
                            'dblTaxBaseAmt4 = clsCommon.myCdbl(txttcstaxbaseamount.Value)
                            dblTaxBaseAmt5 = clsCommon.myCdbl(dbltcstaxbaseamt)
                            dblTaxAmt5 = (dblTaxBaseAmt5 * clsCommon.myCdbl(gv2.Rows(gv2.Rows.Count - 1).Cells(colTTaxRate).Value)) / 100
                            dblTCSTotAmt = dblTCSTotAmt + dblTaxAmt5
                            If rbtnManualTCS.IsChecked = False Then
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            Dim qst As String = "select count(*) from tspl_scrapsale_head_return where document_No='" + txtDocNo.Value + "' and Doc_Type='S' "
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        Dim qry As String = "select document_No as Code,CONVERT(varchar(10), Return_ship_Date,103)+' '+ CONVERT(varchar(5), Return_ship_Date,114) as Date,cust_Code as [Customer Code], cust_Name as Customer,ship_Total_Amt as Amount,case when ispost='0' then 'Pending' else 'Approved' end as [Status] from tspl_scrapsale_head_return"
    ...
    Sub PostData()
        Try
            If (myMessages.postConfirm()) Then
                ...
                End If
                ''
                If (ClsScrapSaleHeadReturn.PostData(txtDocNo.Value)) Then
        ...
        Try
            ...
            Else
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 Then
                    ...
                    ClsScrapSaleHeadReturn.frmScrapSaleReturnPrint(MyBase.Form_ID, Nothing, dtpshipment.Value, txtDocNo.Value, isPrint, location)
                    ...
                    '               "TSPL_SCRAPSALE_HEAD_RETURN.NRG_No  as NrgNo,cm.CST as CstNo,TSPL_LOCATION_MASTER.Tin_No as TinNo,d.Item_Code as ItemCode," &
                    ...
                    '               " TSPL_SCRAPSALE_HEAD_RETURN.Excisable " &
                    '               "from TSPL_SCRAPINVOICE_HEAD h left outer join TSPL_SCRAPINVOICE_DETAIL d on h.invoice_No =d.invoice_No " &
                    '               "left outer join TSPL_CUSTOMER_MASTER   cm on h.cust_Code =cm.Cust_Code  left outer join tspl_company_master   on tspl_company_master.Comp_Code ='" + objCommonVar.CurrentCompanyCode + "'left outer join TSPL_SCRAPSALE_HEAD_RETURN  on h.shipment_No =TSPL_SCRAPSALE_HEAD_RETURN.shipment_No left outer join TSPL_LOCATION_MASTER on  h.Loc_Code=TSPL_LOCATION_MASTER.Location_Code left outer join TSPL_GL_SEGMENT_CODE on TSPL_GL_SEGMENT_CODE.Segment_code  =TSPL_SCRAPSALE_HEAD_RETURN.Vehicle_Id where h.invoice_No='" + clsCommon.myCstr(InvoiceNo) + "'"
                    '    dt = clsDBFuncationality.GetDataTable(qry)
                    '    '====for KDIL(by shivani)==============='
                    '    Dim Qry1 As String = "select TSPL_SCRAPSALE_HEAD_RETURN.Is_Taxable,tspl_company_master.comp_Name,TSPL_COMPANY_MASTER.Add1 as Comp_Add1, TSPL_COMPANY_MASTER.Add2 as Comp_Add2, TSPL_COMPANY_MASTER.Add3 as Comp_Add3, TSPL_COMPANY_MASTER.Email as Comp_Email, TSPL_COMPANY_MASTER.Phone1 as Comp_Phone1, TSPL_COMPANY_MASTER.Phone2 as Comp_Phone2, TSPL_COMPANY_MASTER.Pan_No as Comp_Pan_No, cast(TSPL_COMPANY_MASTER.Logo_Img as image) as Logo_Img, cast(TSPL_COMPANY_MASTER.Logo_Img2 as image) as Logo_Img2 , TSPL_COMPANY_MASTER.GSTREg_No as Comp_GSTREg_No , TSPL_COMPANY_MASTER.CINNO as Comp_CINNO, TSPL_COMPANY_MASTER.Access_Officer as Comp_Access_Officer,TSPL_SCRAPSALE_DETAIL_RETURN.TotalAmt,isnull(TSPL_SCRAPSALE_HEAD_RETURN.RoundOffAmount,0) as RoundOffAmount,cast(TSPL_SCRAPSALE_HEAD_RETURN.BarCode_Img as image) As BarCode_Img,isnull (TSPL_SCRAPSALE_HEAD_RETURN.IRN_No,'') as IRN_No,isnull (TSPL_SCRAPSALE_HEAD_RETURN.Ack_No,'') as Ack_No,case when len(isnull (TSPL_SCRAPSALE_HEAD_RETURN.Ack_No,'')) > 0 then convert (varchar, TSPL_SCRAPSALE_HEAD_RETURN.Ack_Date,103) else ''  end as Ack_Date,TSPL_SCRAPINVOICE_HEAD.EWayBillNo,convert(varchar,TSPL_SCRAPINVOICE_HEAD.EWayBillDate,103) as EWayBillDate,'' as Electronic_Ref_No, TSPL_ITEM_MASTER.HSN_Code, " &
                    '                        "FromState.GST_STATE_CODE as From_GstStateCode,FromLocation.GSTNO as From_Loc_GstinNo,Customer_State.GST_STATE_CODE as Cust_GstStateCode,Customer_State.STATE_NAME AS Cust_StateName,TSPL_CUSTOMER_MASTER.GSTNO as Cust_GstInNo,ISNULL(TSPL_SCRAPSALE_DETAIL_RETURN.DiscountAmt,0) AS DiscountAmt," &
                    ...
                    '                    "TSPL_SCRAPINVOICE_HEAD.invoice_No,TSPL_SCRAPSALE_HEAD_RETURN.Document_No,convert(varchar,TSPL_SCRAPSALE_HEAD_RETURN.Return_ship_Date,103) as Return_Date,convert(varchar,TSPL_SCRAPINVOICE_HEAD.posting_Date,103) as Invoice_Date,TSPL_SCRAPSALE_HEAD_RETURN.Loc_Code as From_Location,FromLocation.Location_Desc as [From Location Desc],(FromLocation.Add1+FromLocation.Add2+FromLocation.Add3+FromLocation.Add4)as [From Address] ," &
                    '                    "FromLocation.Pin_Code,FromLocation.TIN_No,FromLocation.CST_No,FromLocation.State as From_State,FromState.State_Name as frm_State_name  ,TSPL_SCRAPSALE_HEAD_RETURN.ToLoc_Code as To_Location,ToLocation.Location_Desc as To_Location_Desc,(ToLocation.Add1+ToLocation.Add2+ToLocation.Add3+ToLocation.Add4)as [To Address]," &
                    '                    "tspl_customer_master.Pin_Code as [To Pin Code],tspl_customer_master.TIN_No as [To TIN No],tspl_customer_master.CST as [To CST No],tspl_customer_master.Phone1 as [To phone],ToLocation.State as To_State,ToState.State_Name as To_state_name,TSPL_SCRAPSALE_DETAIL_RETURN.Item_Code,TSPL_ITEM_MASTER.Item_Desc,Shipped_Qty," &
                    '                    "TSPL_SCRAPSALE_DETAIL_RETURN.Unit_Code,Price,ItemAmt,TSPL_SCRAPSALE_HEAD_RETURN.Invoice_Type,TSPL_SCRAPSALE_HEAD_RETURN.Total_Tax_Amt,TSPL_SCRAPSALE_HEAD_RETURN.Doc_Amt,	TAX1 .Tax_Code as tax1name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax1_rate,0) as txt1Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax1_amt,0) as txt1amt, "
                    '    Qry1 += " tax2.Tax_Code as tax2name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax2_rate,0) as txt2Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax2_amt,0) as txt2amt,"
                    '    Qry1 += " tax3.Tax_Code as tax3name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax3_rate,0) as txt3Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax3_amt,0) as txt3amt, "
                    '    Qry1 += " tax4.Tax_Code as tax4name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax4_rate,0) as txt4Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax4_amt,0) as txt4amt, "
                    '    Qry1 += " tax5.Tax_Code as tax5name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax5_rate,0) as txt5Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax5_amt,0) as txt5amt, "
                    '    Qry1 += "  tax6.Tax_Code as tax6name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax6_rate,0) as txt6Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax6_amt,0) as txt6amt, "
                    '    Qry1 += "  tax7.Tax_Code as tax7name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax7_rate,0) as txt7Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax7_amt,0) as txt7amt, "
                    '    Qry1 += " tax8.Tax_Code as tax8name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax8_rate,0) as txt8Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax8_amt,0) as txt8amt,  "
                    '    Qry1 += "  tax9.Tax_Code as tax9name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax9_rate,0) as txt9Rate,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax9_amt,0) as txt9amt,'' as cin,'' as pan, '' as companyaddress,"
                    '    Qry1 += " tax10.Tax_Code as tax10name,isnull (TSPL_SCRAPSALE_HEAD_RETURN.tax10_amt,0) as txt10amt ,TSPL_SCRAPSALE_HEAD_RETURN.AddDesc1,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt1,0) as AddAmt1,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc2,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt2,0) as AddAmt2,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc3,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt3,0) as AddAmt3,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc4,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt4,0) as AddAmt4,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc5,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt5,0) as AddAmt5,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc6,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt6,0) as AddAmt6,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc7,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt7,0) as AddAmt7,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc8,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt8,0) as AddAmt8,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc9,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt9,0) as AddAmt9,"
                    '    Qry1 += " TSPL_SCRAPSALE_HEAD_RETURN.AddDesc10,isnull (TSPL_SCRAPSALE_HEAD_RETURN.AddAmt10,0) as AddAmt10,"
                    ...
                    '     " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX1_Amt ,0) as DTax1_Amt, isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX2_Amt ,0) as DTax2_Amt," &
                    '     " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX3_Amt ,0) as DTax3_Amt, isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX4_Amt ,0) as DTax4_Amt," &
                    '     " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX5_Amt ,0) as DTax5_Amt, isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX6_Amt ,0) as DTax6_Amt," &
                    '     " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX7_Amt ,0) as DTax7_Amt, isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX8_Amt ,0) as DTax8_Amt," &
                    '     " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX9_Amt ,0) as DTax9_Amt, isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX10_Amt ,0) as DTax10_Amt," &
                    '     " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX1_Rate,0) as DTax1_Rate,  isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX2_Rate,0) as DTax2_Rate, " &
                    '      " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX3_Rate,0) as DTax3_Rate,  isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX4_Rate,0) as DTax4_Rate," &
                    '       " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX5_Rate,0) as DTax5_Rate,  isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX6_Rate,0) as DTax6_Rate," &
                    '        " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX7_Rate,0) as DTax7_Rate,  isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX8_Rate,0) as DTax8_Rate," &
                    '         " isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX9_Rate,0) as DTax9_Rate,  isnull(TSPL_SCRAPSALE_DETAIL_RETURN.TAX10_Rate,0) as DTax10_Rate,"

                    '    Qry1 += "TSPL_SCRAPSALE_HEAD_RETURN.vehicle_code,TSPL_SCRAPSALE_HEAD_RETURN.Transport_code,Transporter_name,TSPL_SCRAPSALE_HEAD_RETURN.Cust_code,TSPL_SCRAPSALE_HEAD_RETURN.cust_name,tspl_customer_master.add1,tspl_customer_master.add2,tspl_customer_master.add3,tspl_city_master.city_name,TSPL_SCRAPSALE_HEAD_RETURN.Electronic_ref_No from TSPL_SCRAPSALE_HEAD_RETURN left join TSPL_SCRAPSALE_DETAIL_RETURN on TSPL_SCRAPSALE_DETAIL_RETURN.shipment_No=TSPL_SCRAPSALE_HEAD_RETURN.shipment_No left join TSPL_SCRAPINVOICE_HEAD on TSPL_SCRAPINVOICE_HEAD. shipment_No=TSPL_SCRAPSALE_HEAD_RETURN.shipment_No left join TSPL_LOCATION_MASTER as FromLocation on FromLocation.Location_Code=TSPL_SCRAPSALE_HEAD_RETURN.Loc_Code left join TSPL_LOCATION_MASTER as ToLocation on ToLocation.Location_Code=TSPL_SCRAPSALE_HEAD_RETURN.ToLoc_Code left join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_SCRAPSALE_DETAIL_RETURN.Item_Code left join TSPL_STATE_MASTER as  FromState on FromState.State_Code=FromLocation.State"
                    '    Qry1 += " left join TSPL_STATE_MASTER as  ToState on ToState.State_Code=ToLocation.State"
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_SCRAPSALE_HEAD_RETURN.tax1 "
                    '    Qry1 += "  left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_SCRAPSALE_HEAD_RETURN.tax2 "
                    '    Qry1 += " left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_SCRAPSALE_HEAD_RETURN .TAX3 "
                    '    Qry1 += "  left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_SCRAPSALE_HEAD_RETURN .tax4 "
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_SCRAPSALE_HEAD_RETURN .tax5 "
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_SCRAPSALE_HEAD_RETURN .TAX6 "
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_SCRAPSALE_HEAD_RETURN .TAX7 "
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_SCRAPSALE_HEAD_RETURN .TAX8 "
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_SCRAPSALE_HEAD_RETURN .TAX9 "
                    '    Qry1 += " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_SCRAPSALE_HEAD_RETURN .TAX10 "

                    '    Qry1 += "left outer join TSPL_TAX_MASTER as dtax1 on dtax1.tax_code =TSPL_SCRAPSALE_DETAIL_RETURN.tax1  " &
                    ' " left outer join tspl_tax_master as dtax2 on dtax2.tax_code = TSPL_SCRAPSALE_DETAIL_RETURN.tax2 " &
                    '  "left outer join tspl_tax_master as dtax3 on dtax3.Tax_Code=TSPL_SCRAPSALE_DETAIL_RETURN .TAX3  " &
                    '   " left outer join TSPL_TAX_MASTER as dtax4 on dtax4.Tax_Code= TSPL_SCRAPSALE_DETAIL_RETURN .tax4 " &
                    '    "left outer join TSPL_TAX_MASTER as dtax5 on dtax5.Tax_Code=TSPL_SCRAPSALE_DETAIL_RETURN .tax5 " &
                    '     " left outer join TSPL_TAX_MASTER as dtax6 on dtax6.Tax_Code =TSPL_SCRAPSALE_DETAIL_RETURN .TAX6  " &
                    '     "left outer join TSPL_TAX_MASTER as dtax7 on dtax7.Tax_Code =TSPL_SCRAPSALE_DETAIL_RETURN .TAX7  " &
                    '     " left outer join TSPL_TAX_MASTER as dtax8 on dtax8.Tax_Code =TSPL_SCRAPSALE_DETAIL_RETURN .TAX8  " &
                    '     " left outer join TSPL_TAX_MASTER as dtax9 on dtax9.Tax_Code =TSPL_SCRAPSALE_DETAIL_RETURN .TAX9 " &
                    '      " left outer join TSPL_TAX_MASTER as dtax10 on dtax10.Tax_Code =TSPL_SCRAPSALE_DETAIL_RETURN .TAX10 " &
                    '" left join TSPL_TRANSPORT_MASTER on TSPL_TRANSPORT_MASTER.Transport_Id=TSPL_SCRAPSALE_HEAD_RETURN.Transport_code  left join tspl_customer_master on tspl_customer_master.cust_code=TSPL_SCRAPSALE_HEAD_RETURN.cust_code "
                    '    Qry1 += " left join TSPL_STATE_MASTER as Customer_State on tspl_customer_master .State=Customer_State.STATE_CODE "
                    '    Qry1 += " left join tspl_city_master on tspl_city_master.city_code=tspl_customer_master.city_code "
                    '    Qry1 += "left outer join tspl_company_master on tspl_company_master.comp_code = TSPL_SCRAPSALE_HEAD_RETURN.Comp_Code where TSPL_SCRAPSALE_HEAD_RETURN.Document_No='" & txtDocNo.Value & "'"
    ...
    Public Function PrintMaterialSale() As String
        ...
        '                                " isnull(TSPL_SCRAPSALE_DETAIL.TAX1_Amt ,0) as DTax1_Amt, isnull(TSPL_SCRAPSALE_DETAIL.TAX2_Amt ,0) as DTax2_Amt," & _
        '                                " isnull(TSPL_SCRAPSALE_DETAIL.TAX3_Amt ,0) as DTax3_Amt, isnull(TSPL_SCRAPSALE_DETAIL.TAX4_Amt ,0) as DTax4_Amt," & _
        '                                " isnull(TSPL_SCRAPSALE_DETAIL.TAX5_Amt ,0) as DTax5_Amt, isnull(TSPL_SCRAPSALE_DETAIL.TAX6_Amt ,0) as DTax6_Amt," & _
        '                                " isnull(TSPL_SCRAPSALE_DETAIL.TAX7_Amt ,0) as DTax7_Amt, isnull(TSPL_SCRAPSALE_DETAIL.TAX8_Amt ,0) as DTax8_Amt," & _
        '                                " isnull(TSPL_SCRAPSALE_DETAIL.TAX9_Amt ,0) as DTax9_Amt, isnull(TSPL_SCRAPSALE_DETAIL.TAX10_Amt ,0) as DTax10_Amt," & _
        '                                " isnull(TSPL_SCRAPSALE_DETAIL.TAX1_Rate,0) as DTax1_Rate,  isnull(TSPL_SCRAPSALE_DETAIL.TAX2_Rate,0) as DTax2_Rate, " & _
        '                                 " isnull(TSPL_SCRAPSALE_DETAIL.TAX3_Rate,0) as DTax3_Rate,  isnull(TSPL_SCRAPSALE_DETAIL.TAX4_Rate,0) as DTax4_Rate," & _
        '                                  " isnull(TSPL_SCRAPSALE_DETAIL.TAX5_Rate,0) as DTax5_Rate,  isnull(TSPL_SCRAPSALE_DETAIL.TAX6_Rate,0) as DTax6_Rate," & _
        '                                   " isnull(TSPL_SCRAPSALE_DETAIL.TAX7_Rate,0) as DTax7_Rate,  isnull(TSPL_SCRAPSALE_DETAIL.TAX8_Rate,0) as DTax8_Rate," & _
        ...
        '        strQuery += " from TSPL_SCRAPSALE_HEAD  left join TSPL_SCRAPSALE_DETAIL on TSPL_SCRAPSALE_DETAIL.shipment_No=TSPL_SCRAPSALE_HEAD.shipment_No " & _
        ' "left join TSPL_SCRAPINVOICE_HEAD on TSPL_SCRAPINVOICE_HEAD. shipment_No=TSPL_SCRAPSALE_HEAD.shipment_No " & _
        ' "left join TSPL_SCRAPINVOICE_DETAIL on TSPL_SCRAPINVOICE_DETAIL.invoice_No=TSPL_SCRAPINVOICE_HEAD.invoice_No " & _
        '" and TSPL_SCRAPSALE_DETAIL.Item_Code=TSPL_SCRAPINVOICE_DETAIL.Item_Code " & _
        '" and TSPL_SCRAPSALE_DETAIL.Unit_code=TSPL_SCRAPINVOICE_DETAIL.Unit_code " & _
        '"left join TSPL_LOCATION_MASTER as FromLocation on FromLocation.Location_Code=TSPL_SCRAPSALE_HEAD.Loc_Code " & _
        '  "left join TSPL_LOCATION_MASTER as ToLocation on ToLocation.Location_Code=TSPL_SCRAPSALE_HEAD.ToLoc_Code " & _
        '  "left join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_SCRAPINVOICE_DETAIL.Item_Code " & _
        '  "left join TSPL_STATE_MASTER as  FromState on FromState.State_Code=FromLocation.State  " & _
        '  "left join TSPL_STATE_MASTER as  ToState on ToState.State_Code=ToLocation.State  " & _
        '   "left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_SCRAPSALE_HEAD.tax1 " & _
        '    "left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_SCRAPSALE_HEAD.tax2 " & _
        '     "left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_SCRAPSALE_HEAD .TAX3   " & _
        '     "left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_SCRAPSALE_HEAD .tax4 " & _
        '      "left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_SCRAPSALE_HEAD .tax5 " & _
        '       "left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_SCRAPSALE_HEAD .TAX6  " & _
        '       "left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_SCRAPSALE_HEAD .TAX7  " & _
        '       "left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_SCRAPSALE_HEAD .TAX8  " & _
        '       "left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_SCRAPSALE_HEAD .TAX9  " & _
        '       "left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_SCRAPSALE_HEAD .TAX10 "

        '        strQuery += "left outer join TSPL_TAX_MASTER as dtax1 on dtax1.tax_code =TSPL_SCRAPSALE_DETAIL.tax1  " & _
        '                   " left outer join tspl_tax_master as dtax2 on dtax2.tax_code = TSPL_SCRAPSALE_DETAIL.tax2 " & _
        '                    "left outer join tspl_tax_master as dtax3 on dtax3.Tax_Code=TSPL_SCRAPSALE_DETAIL .TAX3  " & _
        '                     " left outer join TSPL_TAX_MASTER as dtax4 on dtax4.Tax_Code= TSPL_SCRAPSALE_DETAIL .tax4 " & _
        '                      "left outer join TSPL_TAX_MASTER as dtax5 on dtax5.Tax_Code=TSPL_SCRAPSALE_DETAIL .tax5 " & _
        '                       " left outer join TSPL_TAX_MASTER as dtax6 on dtax6.Tax_Code =TSPL_SCRAPSALE_DETAIL .TAX6  " & _
        '                       "left outer join TSPL_TAX_MASTER as dtax7 on dtax7.Tax_Code =TSPL_SCRAPSALE_DETAIL .TAX7  " & _
        '                       " left outer join TSPL_TAX_MASTER as dtax8 on dtax8.Tax_Code =TSPL_SCRAPSALE_DETAIL .TAX8  " & _
        '                       " left outer join TSPL_TAX_MASTER as dtax9 on dtax9.Tax_Code =TSPL_SCRAPSALE_DETAIL .TAX9 " & _
        '                        " left outer join TSPL_TAX_MASTER as dtax10 on dtax10.Tax_Code =TSPL_SCRAPSALE_DETAIL .TAX10 "

        '        strQuery += "left join TSPL_TRANSPORT_MASTER on TSPL_TRANSPORT_MASTER.Transport_Id=Transport_code  " & _
        ...
        '        "LEFT JOIN TSPL_BATCH_ITEM  ON TSPL_BATCH_ITEM.Document_Code=TSPL_SCRAPINVOICE_DETAIL.invoice_No AND " & _
        '        "TSPL_BATCH_ITEM.Parent_Line_No=TSPL_SCRAPINVOICE_DETAIL.Line_No AND TSPL_BATCH_ITEM.Item_Code=TSPL_SCRAPINVOICE_DETAIL.Item_Code WHERE TSPL_SCRAPSALE_HEAD.shipment_No='" & txtDocNo.Value & "' "
        ''Return strQuery
    End Function
    ...
    End Sub
    Sub LoadSaleType()
        cmbSaleType.DataSource = GetSaleType()
        cmbSaleType.ValueMember = "Code"
        cmbSaleType.DisplayMember = "Name"
    End Sub
    ...
    Private Sub btnReverse_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverse.Click
        Try
            ...
            If clsCommon.myLen(lblInvoice.Text) <= 0 Then
            ...
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = Windows.Forms.DialogResult.Yes Then
                ...
                End If
                ''richa ERO/19/03/19-000515
                If ClsScrapSaleHeadReturn.ReverseAndUnpost(txtDocNo.Value) Then
    ...
    Sub OpenBatchItem()
        If clsCommon.myCBool(clsDBFuncationality.getSingleValue("select TSPL_ITEM_MASTER.Is_Batch_Item  from TSPL_ITEM_MASTER where TSPL_ITEM_MASTER.Item_Code ='" + clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value) + "'", Nothing)) Then
            ...
            frm.strSplTransaction = "ScrapReturn"
            frm.strShipmentNo = clsDBFuncationality.getSingleValue("select invoice_No from TSPL_SCRAPINVOICE_HEAD where shipment_No='" & txtShipmentNo.Value & "'")
    ...
    Function IsItemExistInGrid(ByVal obj As ClsScrapSaleDetail)
    ...
    Sub SelectShipmentItems()
        isInsideLoadData = True
        Dim frm As New frmPendingMISC()
        frm.CustomerCode = fndcustNo.Value
        frm.CustomerName = txtcustdesc.Text
        ...
        If frm.ArrReturn IsNot Nothing AndAlso frm.ArrReturn.Count > 0 Then
            Dim objReq As ClsScrapSaleHead = ClsScrapSaleHead.GetData(frm.ArrReturn(0).shipment_No, NavigatorType.Current)
            If objReq IsNot Nothing AndAlso clsCommon.myLen(objReq.shipment_No) > 0 Then

                'sanjay
                chkCashSale.Checked = IIf(frm.strIsCashSale = "Y", True, False)
                'sanjay

                txtShipmentNo.Value = objReq.shipment_No
                lblInvoice.Text = objReq.strInvoiceNo
                txtponumber.Text = objReq.Po_No
                txtnrg.Value = objReq.NRG_No
                fndcustNo.Value = objReq.cust_Code
                txtcustdesc.Text = objReq.cust_Name
                '  dtpshipment.Value = clsCommon.myCDate(objReq.shipment_Date)
                dtppost.Value = clsCommon.myCDate(objReq.posting_Date)
                dtpexp.Value = objReq.expship_Date
                fndLocation.Value = objReq.Loc_Code
                txtlocation.Text = objReq.Loc_Name
                TxtVehicleCode.Value = objReq.Vehicle_Id
                txtvehicle_mannual_no.Text = objReq.Vehicle_code
                txtTransporter_Code.Value = objReq.Transporter_code
                txtTransporter_desc.Text = objReq.Transporter_Name
                txtVehicleDesc.Text = ClsScrapSaleHead.GetVehicleDesc(TxtVehicleCode.Value, Nothing)
                fndShipToLocation.Value = objReq.ToLoc_Code
                lblDocAmount.Text = objReq.doc_Amt
                txtRoundOff.Text = objReq.RoundOffAmount
                ''richa agarwal 19/03/2015
                ddlInvoiceType.SelectedValue = objReq.Invoice_Type
                ''-------------
                If objReq.Inter_unit_sale = 1 Then
                    cmbSaleType.Text = "Inter Unit Sale"
                ElseIf objReq.Inter_unit_sale = 2 Then
                    cmbSaleType.Text = "Scrap Sale"
                Else
                    cmbSaleType.Text = "Normal Sale"
                End If
                chkEInvoice.Checked = IIf(objReq.Create_E_Invoice, True, False)
                chkIsEwaybill.Checked = IIf(objReq.IsEwaybill, True, False)
                '------Ravi---------
                txtVatInvNo.Text = objReq.VAT_InvoiceNo
                If objReq.Is_Scrap = "Y" Then
                    chkScrapSale.Checked = True
                Else
                    chkScrapSale.Checked = False
                End If
                chkCashSale.Checked = IIf(objReq.Is_CashSale = "Y", True, False)
                chkCashSale.Enabled = False
                ''--------------
                If objReq.CreateInvoice = 1 Then
                    chkinvoice.Checked = True
                Else
                    chkinvoice.Checked = False
                End If

                If objReq.Excisable = "Y" Then
                    chkExcisable.Checked = True
                Else
                    chkExcisable.Checked = False
                End If
                txtdescription.Text = objReq.Description
                txtref.Text = objReq.reff
                txtTaxGroup.Value = objReq.Tax_Group
                lblTaxGrpName.Text = objReq.Tax_Desc
                txtaddamt.Text = objReq.Add_Amt
                lblTotRAmt.Text = objReq.Before_Add_Amt
                lblAmtWithDiscount.Text = objReq.Discount_Base
                lblDiscountAmt.Text = objReq.Discount_Amt
                lblAmtAfterDiscount.Text = objReq.Amount_Less_Discount
                lblTaxAmt.Text = objReq.Total_Tax_Amt
                lblTotRAmt.Text = objReq.ship_Total_Amt
                lbldocamt.Text = objReq.doc_Amt
                txtTermCode.Value = objReq.Terms_Code
                txtDueDate.Value = objReq.Due_Date
                chkInterBranch.Checked = objReq.Inter_Branch
                chkTaxable.Checked = objReq.Is_Taxable
                chkTaxable.Enabled = False
                txtEWayBillNo.Text = objReq.EWayBillNo
                If objReq.EWayBillDate Is Nothing Then
                    txtEWayBillDate.Checked = False
                Else
                    txtEWayBillDate.Checked = True
                    txtEWayBillDate.Value = objReq.EWayBillDate
                End If
                txtElectronicRefNo.Text = objReq.Electronic_Ref_No

                'If (clsCommon.myLen(objReq.AddCode1) = 0) Then
                '    gvadd.Rows.AddNew()
                'End If
                If clsCommon.myLen(txtTaxGroup.Value) > 0 Then
                    SetTaxDetails()
                ...
                If (clsCommon.myLen(objReq.AddCode1) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode1
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc1
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt1
                    isCellValueChangedOpenAdd = False

                End If
                If (clsCommon.myLen(objReq.AddCode2) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode2
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc2
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt2
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode3) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode3
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc3
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt3
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode4) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode4
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc4
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt4
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode5) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode5
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc5
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt5
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode6) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode6
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc6
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt6
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode7) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode7
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc7
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt7
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode8) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode8
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc8
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt8
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode9) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode9
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc9
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt9
                    isCellValueChangedOpenAdd = False
                End If
                If (clsCommon.myLen(objReq.AddCode10) > 0) Then
                    gvadd.Rows.AddNew()
                    isCellValueChangedOpenAdd = True
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdcode).Value = objReq.AddCode10
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladddesc).Value = objReq.AddDesc10
                    gvadd.Rows(gvadd.Rows.Count - 1).Cells(coladdamt).Value = objReq.AddAmt10
                    isCellValueChangedOpenAdd = False
                End If
                gvadd.Rows.AddNew()
                ...
                lblGrossWeight.Text = clsCommon.myCstr(objReq.Total_Gross_Weight)
                lblNetWeight.Text = clsCommon.myCstr(objReq.Total_Net_Weight)

                'SANJAY START
                For ii As Integer = 0 To frm.ArrReturn.Count - 1
                    If clsCommon.myLen(frm.ArrReturn(ii).document_No) > 0 Then
                        Dim strCode As String = frm.ArrReturn(ii).document_No
                        objReq = ClsScrapSaleHead.GetData(frm.ArrReturn(ii).document_No, NavigatorType.Current)
                        For Each objTr As ClsScrapSaleDetail In objReq.Arr
                            If (objTr.Item_Code = frm.ArrReturn(ii).Item_Code AndAlso objTr.shipment_No = frm.ArrReturn(ii).document_No AndAlso objTr.Unit_Code = frm.ArrReturn(ii).Unit_Code) Then
                                gv1.Rows.AddNew()
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colShipmentNo).Value = objTr.shipment_No
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = objTr.shipped_Qty
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = objTr.shipped_Qty - clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select sum(TSPL_SCRAPSALE_detail_Return.shipped_Qty) from TSPL_SCRAPSALE_detail_Return left outer join TSPL_SCRAPSALE_head_Return on TSPL_SCRAPSALE_head_Return.Document_No=TSPL_SCRAPSALE_detail_Return.Document_No where TSPL_SCRAPSALE_head_Return.ispost=1 and TSPL_SCRAPSALE_head_Return.shipment_No='" + objTr.shipment_No + "' and TSPL_SCRAPSALE_detail_Return.Item_Code='" + objTr.Item_Code + "' and TSPL_SCRAPSALE_detail_Return.Unit_code='" + objTr.Unit_Code + "'"))
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colshQty).Value = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select isnull(shipped_Qty,0) from TSPL_SCRAPSALE_DETAIL where shipment_No='" + objTr.shipment_No + "' and Item_Code='" + objTr.Item_Code + "' and Unit_code='" + objTr.Unit_Code + "'"))
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = objTr.shipped_Qty - clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select sum(TSPL_SCRAPSALE_detail_Return.shipped_Qty) from TSPL_SCRAPSALE_detail_Return left outer join TSPL_SCRAPSALE_head_Return on TSPL_SCRAPSALE_head_Return.Document_No=TSPL_SCRAPSALE_detail_Return.Document_No where TSPL_SCRAPSALE_head_Return.ispost=1 and TSPL_SCRAPSALE_head_Return.shipment_No='" + objTr.shipment_No + "' and TSPL_SCRAPSALE_detail_Return.Item_Code='" + objTr.Item_Code + "' and TSPL_SCRAPSALE_detail_Return.Unit_code='" + objTr.Unit_Code + "'"))
                                'sanjay
                                ...
                                If objReq.ispost = 0 Then
                                    If clsCommon.myLen(objReq.TAX1) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable1).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX1)
                                    End If
                                    If clsCommon.myLen(objReq.TAX2) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable2).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX2)
                                    End If
                                    If clsCommon.myLen(objReq.TAX3) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable3).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX3)
                                    End If
                                    If clsCommon.myLen(objReq.TAX4) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable4).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX4)
                                    End If
                                    If clsCommon.myLen(objReq.TAX5) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable5).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX5)
                                    End If
                                    If clsCommon.myLen(objReq.TAX6) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable6).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX6)
                                    End If
                                    If clsCommon.myLen(objReq.TAX7) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable7).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX7)
                                    End If
                                    If clsCommon.myLen(objReq.TAX8) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable8).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX8)
                                    End If
                                    If clsCommon.myLen(objReq.TAX9) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable9).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX9)
                                    End If
                                    If clsCommon.myLen(objReq.TAX10) > 0 Then
                                        gv1.Rows(gv1.Rows.Count - 1).Cells(colTaxRecoverable10).Value = clsTaxMaster.IsTaxRecoverableAC(objReq.TAX10)
                                    End If
                                End If
                            End If
                        Next
                ...
                For i As Integer = 0 To frm.ArrReturn.Count - 1
                    If chkStr.Contains(frm.ArrReturn(i).document_No) Then
                    Else
                        chkStr.Add(frm.ArrReturn(i).document_No)
                    End If
                Next
                Dim whr As String = ""
                whr = "  where shipment_no in  (" + clsCommon.GetMulcallString(chkStr) + ")"
                Dim manualtcs As Integer = clsDBFuncationality.getSingleValue("select isnull(sum(Is_ManualTCS),0)Is_ManualTCS from tspl_scrapsale_head " + whr + " ")
                If manualtcs <> 0 Then
                    rbtnManualTCS.IsChecked = True
                ElseIf objReq.Tax_Calculation_Type = 0 Then
                    rbtnTaxCalAutomatic.IsChecked = True
                ElseIf objReq.Tax_Calculation_Type = 1 Then
                    rbtnTaxCalManual.IsChecked = True
                End If
    ...
    Private Sub txtShipmentNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtShipmentNo._MYValidating
        SelectShipmentItems()
    End Sub
    ...
    Private Sub fndGateEntryNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndGateEntryNo._MYValidating
        Try
            ...
            WhrCls = " Doc_Type = 'MISS' and POSTED = 1  and Gate_Entry_No not in ( select case when TSPL_SCRAPSALE_HEAD_Return.Gate_Entry_No is Null then '' else TSPL_SCRAPSALE_HEAD_Return.Gate_Entry_No end  from TSPL_SCRAPSALE_HEAD_Return ) "  'and POSTED = 1
            If clsCommon.myLen(fndcustNo.Value) > 0 Then
                WhrCls = WhrCls + " and TSPL_Sale_Return_Gate_Entry_Head.Customer_Code = '" + fndcustNo.Value + "'   "
            End If
            If clsCommon.myLen(fndLocation.Value) > 0 Then
                WhrCls = WhrCls + "  and TSPL_Sale_Return_Gate_Entry_Head.Location_Code= '" + fndLocation.Value + "' "
            End If
            ...
            fndGateEntryNo.Value = clsCommon.ShowSelectForm("GATE@ENTRYNO2", qry, "Gate Entry No", WhrCls, fndGateEntryNo.Value, "", isButtonClicked)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    ...
    Private Sub btnInvoiceJE_Click(sender As Object, e As EventArgs) Handles btnInvoiceJE.Click
        clsOpenJEAgainstInvoice.ShowInvoiceJEForMiscSaleReturn(txtDocNo.Value) ' lblInvoice.Text
    End Sub
    ...
    Function CancelData() As Boolean
        Try
            ...
            Dim strReceiptCount As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select receipt_no from TSPL_RECEIPT_DETAIL where Document_No in (Select Document_No from TSPL_Customer_Invoice_Head  where AgainstScrapReturn='" & txtDocNo.Value & "') "))
            ...
            If chkTaxable.Checked = True AndAlso clsERPFuncationality.GetEInvoiceStatus(dtpshipment.Value) = True AndAlso clsCommon.CompairString(EInvoiceType, "BB") = CompairStringResult.Equal Then
                ...
                EInvoiceCancelTimeValid = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" Select  isnull (DATEDIFF(hour,Posting_Date,GETDATE()),0) as PostedHours from TSPL_SCRAPSALE_HEAD_RETURN where document_no = '" + txtDocNo.Value + "'"))
                If EInvoiceCancelTimeValid >= 24 Then
                    Throw New Exception("Invoice can not be cancelled.It has been more than 24 hours.")
                End If
            End If

            ClsScrapSaleHeadReturn.CancelData(Me.Form_ID, txtDocNo.Value, NavigatorType.Current)
