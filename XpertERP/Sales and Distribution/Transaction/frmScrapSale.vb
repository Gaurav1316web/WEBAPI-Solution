...
#End Region
                        ...
                        If gv1.CurrentColumn Is gv1.Columns(colQty) Then
                            If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
                                ...
                                If clsCommon.myLen(fndLocation.Value) > 0 And clsCommon.myLen(gv1.CurrentRow.Cells(colICode).Value) > 0 And clsCommon.myLen(gv1.CurrentRow.Cells(colUnit).Value) > 0 Then
                                    stockqty = clsItemLocationDetails.getBalance(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), IIf(clsCommon.myLen(txtSubLocation.Value) > 0, txtSubLocation.Value, fndLocation.Value), txtDocNo.Value, dtpshipment.Value, Nothing, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), 0)
    ...
    Sub AddNew()
        ...
        If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "RCDFCF") <> CompairStringResult.Equal Then
            MyLabel60.Visible = True
            txtSubLocation.Visible = True
            lblSubLocation.Visible = True
            fndLocation.Value = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Default_Location from TSPL_USER_MASTER where User_Code='" + objCommonVar.CurrentUserCode + "' "))
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            ...
            If clsCommon.myLen(strICode) > 0 Then
                If clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), clsItemRowType.RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    Dim dblBalQty As Double = clsItemLocationDetails.getBalanceWithUnapproveForRMOther(strICode, IIf(clsCommon.myLen(txtSubLocation.Value) > 0, txtSubLocation.Value, fndLocation.Value), txtDocNo.Value, dtpshipment.Value, Nothing, strUOM)
    ...
    Private Sub fndcustNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndcustNo._MYValidating
        If Not clsCommon.CompairString(cmbSaleType.Text, "Select") = CompairStringResult.Equal Then
            fndcustNo.Focus()
            If clsCommon.myLen(fndLocation.Value) <= 0 Then
                Exit Sub
            End If
            ...
            cmbSaleType.Enabled = False
        Else
            clsCommon.MyMessageBoxShow(Me, "Please Select Sale Type" & fndLocation.Value)
...
left join TSPL_TAX_MASTER on TSPL_TAX_GROUP_DETAILS.Tax_Code=TSPL_TAX_MASTER.Tax_Code "
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndLocation._MYValidating
        ...
        Else
            clsCommon.MyMessageBoxShow(Me, "Please Select Sale Type", Me.Text)
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        End If
        LoadData(clsCommon.ShowSelectForm("ScrpCodFiltrFND", qry, "Code", whrClas, txtDocNo.Value, "shipment_Date desc", isButtonClicked, "shipment_Date"), NavigatorType.Current)
    End Sub
    ...
    Private Function CustomerOutstandingAmount(ByVal strCustomer As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            Dim qry As String = "Select  ( SUM(convert(decimal(18,2),OpngBal)) + SUM(convert(decimal(18,2),DrAmt)) ) -SUM(convert(decimal(18,2),CrAmt))  as BalAmt From ( " &
                   ...
                   "(" & clsSNShipmentHead.GetCustomerBaseQry(False, False, "", False, "ConvRate", strcustomerfilter, True, clsCommon.GetPrintDate(dtpshipment.Value.AddDays(1), "dd/MMM/yyyy"), "", txtDocNo.Value, False, False, True, trans, False) & "   " &
                    ...
                    Else
                        ...
                        Qry1 += " RIGHT(TSPL_SCRAPINVOICE_HEAD.shipment_No,4) as GatePass,TSPL_SCRAPINVOICE_HEAD.reff as Remark, TSPL_SCRAPINVOICE_HEAD.Vehicle_Id as VehicleNo,'1' as  CopyType" &
                                        ...
                                        "tspl_customer_master.Pin_Code as [To Pin Code],tspl_customer_master.TIN_No as [To TIN No],tspl_customer_master.CST as [To CST No],tspl_customer_master.Phone1 as [To phone],ToLocation.State as To_State,ToState.State_Name as To_state_name,TSPL_SCRAPSALE_DETAIL.Item_Code
                                                                ...
                                                                ,Shipped_Qty," &
                                        "TSPL_SCRAPSALE_DETAIL.Unit_Code, Price, ItemAmt, TSPL_SCRAPSALE_HEAD.Invoice_Type,TSPL_SCRAPSALE_HEAD.Total_Tax_Amt,TSPL_SCRAPSALE_HEAD.Doc_Amt,	TAX1 .Tax_Code As tax1name,isnull (TSPL_SCRAPSALE_DETAIL.tax1_rate,0) As txt1Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax1_amt,0) As txt1amt, "
                        Qry1 += " tax2.Tax_Code As tax2name,isnull (TSPL_SCRAPSALE_DETAIL.tax2_rate,0) As txt2Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax2_amt,0) As txt2amt,"
                        Qry1 += " tax3.Tax_Code As tax3name,isnull (TSPL_SCRAPSALE_DETAIL.tax3_rate,0) As txt3Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax3_amt,0) As txt3amt, "
                        Qry1 += " tax4.Tax_Code As tax4name,isnull (TSPL_SCRAPSALE_DETAIL.tax4_rate,0) As txt4Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax4_amt,0) As txt4amt, "
                        Qry1 += " tax5.Tax_Code As tax5name,isnull (TSPL_SCRAPSALE_DETAIL.tax5_rate,0) As txt5Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax5_amt,0) As txt5amt, "
                        Qry1 += "  tax6.Tax_Code  As tax6name,isnull (TSPL_SCRAPSALE_DETAIL.tax6_rate,0) As txt6Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax6_amt,0) As txt6amt, "
                        Qry1 += "  tax7.Tax_Code  As tax7name,isnull (TSPL_SCRAPSALE_DETAIL.tax7_rate,0) As txt7Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax7_amt,0) As txt7amt, "
                        Qry1 += " tax8.Tax_Code As tax8name,isnull (TSPL_SCRAPSALE_DETAIL.tax8_rate,0) As txt8Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax8_amt,0) As txt8amt,  "
                        Qry1 += "  tax9.Tax_Code As tax9name,isnull (TSPL_SCRAPSALE_DETAIL.tax9_rate,0) As txt9Rate,isnull (TSPL_SCRAPSALE_DETAIL.tax9_amt,0) As txt9amt,'' as cin,'' as pan, '' as companyaddress,"
                        Qry1 += " tax10.Tax_Code as tax10name,isnull (TSPL_SCRAPSALE_DETAIL.tax10_amt,0) as txt10amt ,TSPL_SCRAPSALE_HEAD.AddDesc1,isnull (TSPL_SCRAPSALE_HEAD.AddAmt1,0) as AddAmt1,"
                            ...
                            If clsERPFuncationality.GetGSTStatus(clsCommon.myCDate(dt1.Rows(0)("Invoice_Date"))) Then
                                ...
                                Else
                                    ...
                                    Else
                                        ...
                                        Else
                                            'filePath=frmCRV.funsubreportWithdt(isPDFPath,CrystalReportFolder.PurchaseOrder, dt1, clsERPFuncationality.CompanyAddresShowinFooter(), "rptMaterialSaleInvoice_NonTaxable", "ScrapnSale Invoice Non Taxable", clsCommon.myCDate(dt1.Rows(0)("Invoice_Date")), "rptCompanyAddress.rpt", "rptCustomerOutstandingErode.rpt", dtCustomerOutstanding)
                                            filePath = frmCRV.funsubreportWithdt(MyBase.Form_ID, isPDFPath, CrystalReportFolder.KwalitySalesReport, dt1, itemSummnary, "rptScrapSaleInvoice_Union", "ScrapnSale Invoice ", clsCommon.myCDate(dt1.Rows(0)("Invoice_Date")), "rptSubItemSummary.rpt", )
                                        End If
                                    End If
  ...
  "left join TSPL_STATE_MASTER as  ToState on ToState.State_Code=ToLocation.State  " &
   "left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_SCRAPSALE_HEAD.tax1 " &
    ...
    Private Sub txtSubLocation__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtSubLocation._MYValidating
        Try
            If clsCommon.myLen(clsCommon.myCstr(fndLocation.Value)) > 0 Then
                txtSubLocation.Value = clsLocation.getFinder(" (isnull(is_sub_location,'N')='Y' or isnull(Is_Section,'N')='Y') and Main_Location_Code='" + fndLocation.Value + "'", txtSubLocation.Value, isButtonClicked)
                If clsCommon.myLen(txtSubLocation.Value) > 0 Then
