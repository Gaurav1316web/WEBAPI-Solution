            ...
            If validateRecords() Then
                ...
                If btnAdd.Text = "Save" Then
                    Try
                        ...
                        clsCommon.AddColumnsForChange(coll, "CURRENCY_CODE", Me.txtCurrencyCode.Value, True)
...
#Region "Import/Export"
    ...
    Private Sub menuImport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuImport.Click
        ...
        If AllowGSTApplicable Then
            inputs = {"Tax Authority", "Description", "Tax Liability Account", "Tax Recoverable", "Tax Recoverable Account", "Tax Recover Rate", "Tax Net Payable", "Type", "Tax Recoverable Account2", "Tax Recover Rate2", "Tax Recoverable Account3", "Tax Recover Rate3", "Tax Recoverable Account4", "Tax Recover Rate4", "Tax Recoverable Account5", "Tax Recover Rate5", "Is Mandi Tax Cess", "GST Active", "Payable Control", "Deposit Control"}
        Else
            inputs = {"Tax Authority", "Description", "Tax Liability Account", "Tax Recoverable", "Tax Recoverable Account", "Tax Recover Rate", "Tax Net Payable", "Type", "Tax Recoverable Account2", "Tax Recover Rate2", "Tax Recoverable Account3", "Tax Recover Rate3", "Tax Recoverable Account4", "Tax Recover Rate4", "Tax Recoverable Account5", "Tax Recover Rate5", "Is Mandi Tax Cess"}
        End If
        ...
        If transportSql.importExcel(gv, Strs.ToArray()) Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If AllowGSTApplicable Then
                        GSTActive = clsCommon.myCstr(grow.Cells(17).Value)
                        Payable = clsCommon.myCstr(grow.Cells(18).Value)
                        Deposit = clsCommon.myCstr(grow.Cells(19).Value)
                    End If
...
#End Region
    ...
    Public Sub LoadData()
        ...
        sql = "select Tax_Liability_Account ,(case when Tax_Recoverable='Y' then 'True' else 'False' end) ,Tax_Recoverable_Account ,Tax_Recover_Rate,Tax_Net_Payable,(case when Excisable='Y' then 'True' else 'False' end),type,Tax_Recoverable_Account2 ,Tax_Recover_Rate2 ,Tax_Recoverable_Account3 ,Tax_Recover_Rate3 ,Tax_Recoverable_Account4 ,Tax_Recover_Rate4 ,Tax_Recoverable_Account5 ,Tax_Recover_Rate5,CURRENCY_CODE,ConvRate,ApplicableFrom,Is_Mandi_Tax,GSTActive,PayableControl,DepositControl,Is_MandiTaxCess  from TSPL_TAX_MASTER where Tax_Code='" + findTaxAuthority.Value + "'"
        dr = clsDBFuncationality.GetDataTable(sql)
        If dr.Rows IsNot Nothing AndAlso dr.Rows.Count > 0 Then
            ' txtDesc.Text = findTaxAuthority.Tag
