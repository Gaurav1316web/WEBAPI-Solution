...
Public Class FrmAdditionalCharges
    ...
    Public Sub Import()
        ...
        If GstApplicable Then
            StrVar = {"Code", "Description", "Account_Code", "Account_Description", "Freight Charges", "Abatement", "Specification", "Reverse_Charge_Per", "Service_Type", "SAC_Code", "RCM", "NO GST Credit"}
        Else
            StrVar = {"Code", "Description", "Account_Code", "Account_Description", "Freight Charges", "Abatement", "Specification", "Reverse_Charge_Per"}
        End If
        ...
        If transportSql.importExcel(gv, strvarlist.ToArray()) Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If (i = 0) Then
                        QRY = "INSERT Into tspl_Additional_Charges values('" + Code + "','" + Desc + "','" + userCode + "','" + connectSql.serverDate(trans) + "','" + userCode + "','" + connectSql.serverDate(trans) + "','" + companyCode + "','" + straccount + "','" + straccdesc + "','" & freight & "','" & abtment & "','" & specification & "','" & Reverse_Charge_Per & "','" & ServiceType & "','" & SACCode & "','" + strRCM + "','" + strNOGSTCredit + "')"
                        connectSql.RunSqlTransaction(trans, QRY)
                    Else
                        QRY = "UPDATE tspl_Additional_Charges set Code='" + Code + "', description='" + Desc + "',Account_Code='" + straccount + "',Account_Description='" + straccdesc + "' ,Modified_By='" + userCode + "', Modified_Date='" + connectSql.serverDate(trans) + "', Comp_Code='" + companyCode + "',FreightCharges='" & freight & "',abatement='" & abtment & "',specification='" + specification + "',Reverse_Charge_Per='" + Reverse_Charge_Per + "' ,Service_Type='" + ServiceType + "',SAC_Code='" + SACCode + "',RCM='" + strRCM + "',NO_GST_Credit = '" + strNOGSTCredit + "'  WHERE Code='" + Code + "'"
                        connectSql.RunSqlTransaction(trans, QRY)
                    End If
    ...
    Public Sub Export()
        Dim str As String
        str = "select Code As [Code],description as [Description],Account_Code,Account_Description,FreightCharges as [Freight Charges],Abatement,Specification,Reverse_Charge_Per "
        If GstApplicable Then
    ...
    Private Sub chkRCM_ToggleStateChanged(sender As Object, args As StateChangedEventArgs) Handles chkRCM.ToggleStateChanged
        If chkRCM.Checked = True AndAlso chkRCM.Enabled = True Then
            chkNoGSTCredit.Enabled = True
        Else
            chkNoGSTCredit.Checked = False
            chkNoGSTCredit.Enabled = False
