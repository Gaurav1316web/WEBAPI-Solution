    ...
    Public Sub fnd_leave1(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Else
            Try
                ...
                Else : strquery = ""
                    txtbankacc.Text = ""
                    common.clsCommon.MyMessageBoxShow("This Bank Account does not exist in Master Table")
                    fndbankacc.Value = ""
                End If
            Catch ex As Exception
                myMessages.myExceptions(ex)
...
End if
                ...
                Else : strquery = ""
                    txtwriteoff.Text = ""
                    common.clsCommon.MyMessageBoxShow("This Write Off Account does not exist in Master Table")
                    fndwriteoff.Value = ""
                End If
            Catch ex As Exception
                myMessages.myExceptions(ex)
    ...
    Public Sub fnd_leave3(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Else
            Try
                ...
                Else : strquery = ""
                    txtcredit.Text = ""
                    common.clsCommon.MyMessageBoxShow("This Credit Card Charges Account does not exist in Master Table.")
                    fndcredit.Value = ""
                End If
            Catch ex As Exception
                myMessages.myExceptions(ex)
    ...
    Public Sub SaveData()
        ...
        Try
            ...
            ElseIf ddlbanktype.Text = "Select" Or ddlbanktype.Text = "select" Then
                common.clsCommon.MyMessageBoxShow("Please select the bank type")
                ddlbanktype.Focus()
                '' Richa Againt Ticket No. BM00000003641 on 27/08/2014
            ElseIf clsCommon.myCdbl(txtFDPer.Value) > 100 Then
                common.clsCommon.MyMessageBoxShow("FD % cannot be more than 100")
                txtFDPer.Focus()
                ''---------------------------------
                '' Anubhooti 19-Dec-2014
            ElseIf clsCommon.myLen(fndtransferclearing.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow("Transfer clearing can not be left blank.")
            ...
            ElseIf clsCommon.CompairString(clsDBFuncationality.getSingleValue("Select count(*) from TSPL_BANK_MASTER WHERE isnull(TSPL_BANK_MASTER.Main_Bank_Code,'') IN  (Select BANK_CODE  from TSPL_BANK_MASTER where BANK_CODE ='" & clsCommon.myCstr(fndbank.Value) & "')", Nothing), "1") = CompairStringResult.Equal Then
                common.clsCommon.MyMessageBoxShow("Bank Code (" & fndbank.Value & ") should not be of Clearance Bank, because it is used in other banks as a main bank.")
            Else
    ...
    Public Sub funinsert()
        Try
            ...
            If Count > 0 Then
                clsCommon.MyMessageBoxShow("Default bank already mapped with another bank")
            ...
            Else
                ...
                clsDBFuncationality.ExecuteNonQuery("update tspl_bank_master set  cheque_validity_in_days=" & clsCommon.myCdbl(txtChequeValidity.Text) & ",LCCreditLimit=" & clsCommon.myCdbl(txtLCCreditLimit.Value) & ",FDPercentage=" & clsCommon.myCdbl(txtFDPer.Value) & ",Transfer_Clearing_Account='" & fndtransferclearing.Value & "',IBAN_No='" & TxtIbanno.Text & "',Swift_Code='" & txtswiftcode.Text & "',Is_Clearance_Bank='" & IIf(chkClearanceBank.Checked, "Y", "N") & "',Main_Bank_Code='" & clsCommon.myCstr(TxtMainBankCode.Value) & "' where bank_code='" & fndbank.Value.ToString() & "'")
                UseSubAccount = clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowToUseSubAccount, clsFixedParameterCode.AllowToUseSubAccount, Nothing))
                If clsCommon.CompairString(UseSubAccount, "1") = CompairStringResult.Equal AndAlso clsCommon.CompairString(ddlbanktype.Text, "Bank") = CompairStringResult.Equal Then
    ...
    Function SaveBankCheckData(ByVal strDocNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        For Each row As GridViewRowInfo In gv1.Rows
            ...
            If dtcheck.Rows.Count > 0 Then
                clsCommon.MyMessageBoxShow("Check Code " & dtcheck.Rows(0).Item("CHECK_CODE") & " already exists in bank " & dtcheck.Rows(0).Item("BANK_CODE") & "")
                Return False
            End If
            ...
            If clsCommon.myCdbl(row.Cells(colLAST_CHECK_NUMBER).Value) < clsCommon.myCdbl(row.Cells(colLAST_PRINT_CHECK_NUMBER).Value) Then
                clsCommon.MyMessageBoxShow("Please check ! last check no. " & clsCommon.myCstr(clsCommon.myCdbl(row.Cells(colLAST_CHECK_NUMBER).Value)) & " should be greater than from printed check no. " & clsCommon.myCstr(clsCommon.myCdbl(row.Cells(colLAST_PRINT_CHECK_NUMBER).Value)) & "")
                Return False
            End If
    ...
    Public Sub funfill()
        Try
            Dim strquery As String = "select bank_code,description,add1,add2,add3,add4,city,state,postal,country,contact,phone,fax,inactive,bankaccnumber,bankacc,writeoffacc,creditacc, Bank_Type ,cheque_validity_in_Days,LCCreditLimit,FDPercentage,Sub_Account,Transfer_Clearing_Account,IBAN_No,Swift_Code,Gateway_type,Default_Bank,Branch_Code,Is_Clearance_Bank,Main_Bank_Code from TSPL_Bank_MASTER where bank_code='" + fndbank.Value + "'"
            Dim dt As DataTable
            dt = clsDBFuncationality.GetDataTable(strquery)
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
    ...
    Public Sub funupdate()
        Try
            ...
            Else
                ...
                clsDBFuncationality.ExecuteNonQuery("Update TSPL_BANK_MASTER set  cheque_validity_in_days=" & clsCommon.myCdbl(txtChequeValidity.Text) & ",LCCreditLimit=" & clsCommon.myCdbl(txtLCCreditLimit.Value) & ",FDPercentage=" & clsCommon.myCdbl(txtFDPer.Value) & ",Transfer_Clearing_Account='" & fndtransferclearing.Value & "',  IBAN_No='" & TxtIbanno.Text & "',Swift_Code='" & txtswiftcode.Text & "',Is_Clearance_Bank='" & IIf(chkClearanceBank.Checked, "Y", "N") & "',Main_Bank_Code='" & clsCommon.myCstr(TxtMainBankCode.Value) & "' where bank_code='" & clsCommon.myCstr(fndbank.Value) & "'")
                UseSubAccount = clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowToUseSubAccount, clsFixedParameterCode.AllowToUseSubAccount, Nothing))
                If clsCommon.CompairString(UseSubAccount, "1") = CompairStringResult.Equal AndAlso clsCommon.CompairString(ddlbanktype.Text, "Bank") = CompairStringResult.Equal Then
        ...
        If transportSql.importExcel(gv, "Bank Code", "Description", "Address1", "Address2", "Address3", "Address4", "City", "State", "Postal", "Country", "Contact", "Phone", "Fax", "Inactive", "Bank Account Number", "Bank Account", "Write Off Account", "Credit Account", "Bank Type", "Cheque Validity In Days", "LC Credit Limit", "FD %", "Sub Account", "IBAN No", "Swift Code", "Transfer Clearing Account", "Is Clearance Bank", "Main Bank Code") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    End If
                    '' Richa Againt Ticket No. BM00000003641 on 27/08/2014
                    clsDBFuncationality.ExecuteNonQuery("update tspl_bank_master set cheque_validity_in_days=" & strChkValidity & ",LCCreditLimit=" & dblLCCreditLimit & ",FDPercentage=" & dblFDPer & ",IBAN_No='" & stribanno & "',Swift_Code='" & strswiftcode & "',Transfer_Clearing_Account='" + strTransferClearAC + "',Is_Clearance_Bank='" & strClearanceBank & "',Main_Bank_Code='" & strMainBank & "'  where bank_code='" & strcode & "'", trans)
