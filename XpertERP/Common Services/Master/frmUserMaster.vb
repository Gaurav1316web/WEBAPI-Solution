...
#Region "Page Load"
    ...
    Private Sub FrmUserMaster_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            rmImportCustomerMapping.Visibility = ElementVisibility.Hidden
            RadMenuItem1.Visibility = ElementVisibility.Hidden
        End If
        LoadBlankUserGrid()
        gvUser.Rows.AddNew()
        txt_Mob_No.Text = ""
...
#Region "Methods"
    Private Sub funfill()
        ...
        Dim str As String = "select USER_NAME ,password ,Emp_Code,Emp_Name,User_Type,Level1_Code,Level2_Code,Level3_Code,Level4_Code, ApprovalLevel,E_Mail, Default_Location, Vendor_Code, Login_Type,Cust_Code,Distributor_Retailer_Code,Segment_code,View_Milk_Receipt_Sample,Department_Head,Licence_Reserved,Mob_No from tspl_user_master where  User_Code ='" + fndUserCode.Value + "'"
        Dim dr As DataTable
    ...
    Private Sub funInsert()
        Try '
            ...
            If clsCommon.myLen(EmployeeFinder.Value) > 0 Then
                ...
                Dim result As Boolean = clsCommonFunctionality.UpdateDataTable(coll, "TSPL_USER_MASTER", OMInsertOrUpdate.Update, "User_Code='" + fndUserCode.Value + "'")
            End If
            ...
            'sanjay
            clsDBFuncationality.ExecuteNonQuery(" update TSPL_USER_MASTER set Department_Head = '" + clsCommon.myCstr(IIf(ChkDepartmentHead.Checked, 1, 0)) + "' where User_Code ='" + fndUserCode.Value + "'")
            'sanjay
            '===============================================
...
#Region "Import/Export"
    Private Sub menuExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuExport.Click
        ...
        sql = "select User_Code,User_Name,Default_Location as [Default Location],Department_Head as [Department Head] from TSPL_USER_MASTER "
    ...
    Private Sub menuImport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuImport.Click
        ...
        Me.Controls.Add(gv)
        If transportSql.importExcel(gv, "User_Code", "User_Name", "Default Location", "Department Head") Then  ' "Zone Code"
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    ElseIf grow.Cells(0).Value.ToString().Length > 12 Then
                        ...
                        Exit Sub
                    Else
                        strUserCode = grow.Cells(0).Value.ToString().ToUpper()
                    End If
                    If grow.Cells(1).Value.ToString().Length > 50 Then
                        ...
                        Exit Sub
                    Else
                        strName = grow.Cells(1).Value.ToString()
                    End If

                    Dim strDefaultLocation As String = clsCommon.myCstr(grow.Cells(2).Value)
                    If clsCommon.myLen(strDefaultLocation) > 0 Then
                        Dim Default_Location As String = clsDBFuncationality.getSingleValue("select Location_Code  from TSPL_LOCATION_MASTER  where Location_Code ='" + grow.Cells(2).Value.ToString() + "'", trans)
                        If clsCommon.CompairString(Default_Location, strDefaultLocation) = CompairStringResult.Equal Then
                            strDefaultLocation = grow.Cells(2).Value.ToString()
                        Else
                    ...
                    clsCommon.AddColumnsForChange(colll, "Department_Head", grow.Cells(3).Value.ToString())
                    'If clsCommon.myLen(strZoneCode) > 0 Then
...
#End Region
    ...
    Private Sub FillCustomerGrid(ByVal strCriteria As String)
        Try
            ...
            gvCustomer.Columns.Clear()
            If clsCommon.myLen(fndUserCode.Value) > 0 Then
                qry = "Select ROW_NUMBER() Over (order by TSPL_CUSTOMER_MASTER.Cust_Code) as [LineNo], Cast(1 as bit) as [Select], TSPL_CUSTOMER_MASTER.Cust_Code as [Customer Code], Customer_Name as [Customer Name] from TSPL_CUSTOMER_MASTER left outer join  TSPL_USER_CUSTOMER_MAPPING on  TSPL_USER_CUSTOMER_MAPPING.Cust_Code = TSPL_CUSTOMER_MASTER.Cust_Code WHERE TSPL_USER_CUSTOMER_MAPPING.User_Code='" + strCriteria + "'"
    ...
    Private Function MatchLevel() As Decimal
        Try
            ...
            Dim level As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(qry))
            If level = 1 Then
                fndCustCode.Enabled = True
                fndDisRetailerCode.Enabled = False
                fndDisRetailerCode.Value = ""
