...
Public Class rptVendorReco
    ...
    Private Sub rptVendorReco_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        'LoadVendorGroup()
        'LoadVendor()
    ...
    Sub Print(Optional ByVal BulkExport As Integer = 0)
        ...
        Try
            ...
            BaseQry = GetVendorBaseQry(rbPortrait.IsChecked, rbLandScape.IsChecked, chkAgainstSalary.Checked, strFromDate, strToDate, strFIltervendor, False)
            BaseQryOpening = GetVendorBaseQry(rbPortrait.IsChecked, rbLandScape.IsChecked, chkAgainstSalary.Checked, strFromDate, strToDate, strFIltervendor, True)
            ''-----------------------
            ...
            If chkVendorWise.IsChecked Then
                ...
                End If
                strQry += " Select VCode, MAX(VName) as VName, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code) as Vendor_Group_Code, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code_Desc) as [Vendor_Group_Desc], MAX(TSPL_VENDOR_ACCOUNT_SET.Acct_Set_Desc) as AccountSet, SUM(CrAmt)-SUM(DrAmt) as OpngBal, 0 as DrAmt, 0 as CrAmt, 0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote From ( " & IIf(clsCommon.myCstr(clsUserMgtCode.MISCreditorReport) = FormType, strtempBaseQryforopeningForMIS, BaseQryOpening) & " ) Final LEFT OUTER JOIN TSPL_VENDOR_MASTER on final.VCode=TSPL_VENDOR_MASTER.Vendor_Code Left Outer Join TSPL_VENDOR_GROUP ON TSPL_VENDOR_MASTER.Vendor_Group_Code=TSPL_VENDOR_GROUP.Ven_Group_Code left join TSPL_VENDOR_ACCOUNT_SET on TSPL_VENDOR_GROUP.Acct_Set_Code=TSPL_VENDOR_ACCOUNT_SET.Acct_set_Code LEFT OUTER JOIN TSPL_COMPANY_MASTER ON final.Comp_Code=TSPL_COMPANY_MASTER.Comp_Code where final.Document_Type in (" + clsCommon.GetMulcallString(arr) + ") and CONVERT(date,DocDate ,103)  <'" + strFromDate + "' and DocType <>'Mislleneous' " + strFIlterCheck + " GROUP BY VCode" + Environment.NewLine & _
               " UNION ALL" + Environment.NewLine & _
               " Select VCode, MAX(VName) as VName, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code) as Vendor_Group_Code, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code_Desc) as [Vendor_Group_Desc], MAX(TSPL_VENDOR_ACCOUNT_SET.Acct_Set_Desc) as AccountSet, 0 as OpngBal, SUM(DrAmt) as DrAmt, SUM(CrAmt) as CrAmt, SUM(CONVERT(DECIMAL(18,2),Purchase*" & ddlCurrencyType.SelectedValue & ")) as Purchase, SUM(CONVERT(DECIMAL(18,2),Payments*" & IIf(clsCommon.myCstr(clsUserMgtCode.MISCreditorReport) = FormType, "1", ddlCurrencyType.SelectedValue) & ")) as Payments, SUM(DrNote*" & ddlCurrencyType.SelectedValue & ") as DrNote, SUM(CrNote*" & ddlCurrencyType.SelectedValue & ") as CrNote from ( " + BaseQry + " ) Final LEFT OUTER JOIN TSPL_VENDOR_MASTER on final.VCode=TSPL_VENDOR_MASTER.Vendor_Code Left Outer Join TSPL_VENDOR_GROUP ON TSPL_VENDOR_MASTER.Vendor_Group_Code=TSPL_VENDOR_GROUP.Ven_Group_Code left join TSPL_VENDOR_ACCOUNT_SET on TSPL_VENDOR_GROUP.Acct_Set_Code=TSPL_VENDOR_ACCOUNT_SET.Acct_set_Code LEFT OUTER JOIN TSPL_COMPANY_MASTER ON final.Comp_Code=TSPL_COMPANY_MASTER.Comp_Code where final.Document_Type in (" + clsCommon.GetMulcallString(arr) + ") and CONVERT(date,DocDate ,103)  >='" + strFromDate + "' and CONVERT(date,DocDate ,103)  <='" + strToDate + "' and DocType <>'Mislleneous' " + strFIlterCheck + " GROUP BY VCode" + Environment.NewLine & _
            ...
            If rbtnDocWise.IsChecked = True AndAlso chkAgainstSalary.Checked = False Then
                ...
                " LEFT OUTER JOIN TSPL_VENDOR_MASTER on final.VCode=TSPL_VENDOR_MASTER.Vendor_Code Left Outer Join TSPL_VENDOR_GROUP ON TSPL_VENDOR_MASTER.Vendor_Group_Code=TSPL_VENDOR_GROUP.Ven_Group_Code LEFT OUTER JOIN " + Environment.NewLine & _
                 "tspl_BankReco_Detail on final.DocNo=tspl_BankReco_Detail.Document_No and tspl_BankReco_Detail.Reconciliation_Status='C' LEFT OUTER JOIN tspl_BankReco_Head on tspl_BankReco_Detail.Reconciliation_Id =tspl_BankReco_Head.Reconciliation_Id LEFT OUTER JOIN TSPL_COMPANY_MASTER ON final.Comp_Code=TSPL_COMPANY_MASTER.Comp_Code " & _
            ...
            If blnRefresh = False Then
                ...
                ' Else
                If chkNone.IsChecked = False Then
                    dtMain = clsDBFuncationality.GetDataTable(strQry)
                End If
        ...
        End Try
    End Sub
    '   Public Function GetVendorBaseQry(ByVal strPortrait As Boolean, ByVal strLandscape As Boolean, ByVal IsOnlyForAgainstSalary As Boolean) As String
    ...
    '                   ' case when TSPL_PAYMENT_HEADER.Payment_Type='RC' then 0 when TSPL_PAYMENT_HEADER.Payment_Type='PY' then TSPL_PAYMENT_DETAIL.Applied_Amount ELSE TSPL_BANK_REVERSE.Amount end as CrAmt,
    ...
    '                   '        strtempBaseQry += " UNION ALL" + Environment.NewLine & _
    '                   '" select null AS Due_Date, '' AS PO_SRN, TSPL_VENDOR_MASTER.Vendor_Code as VCode, TSPL_VENDOR_MASTER.Vendor_Name as VName,TSPL_PAYMENT_HEADER.Payment_No as DocNo, CASE WHEN TSPL_PAYMENT_HEADER.Payment_Type ='A' THEN 'IM' else 'EXC' end as [DocType],convert(date,Payment_Date, 103) as DocDate, case when ISNULL(TSPL_PAYMENT_HEADER.Is_Security,0)=1 Then 'Security' Else '' + 'Exchange Gain/Loss against Journal Entry No  '+isnull(TSPL_JOURNAL_MASTER.Voucher_No,'')    end as DocNarr,(TSPL_PAYMENT_HEADER.cheque_No+'-'+ CONVERT(VARCHAR,Cheque_Date , 103))as ChequeDetails, TSPL_PAYMENT_HEADER.CURRENCY_CODE, TSPL_PAYMENT_HEADER.ConvRate, TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT AS CrAmt,  TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT as DrAmt, 0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote, 'RV'as Document_Type, 0 as Balance_Amount, substring(TSPL_PAYMENT_HEADER.Debit_Account , len(TSPL_PAYMENT_HEADER.Debit_Account )-2,3) as account, TSPL_PAYMENT_HEADER.Payment_Post_Date as Posting_Date,'RV-TA' as GLDocType, TSPL_PAYMENT_HEADER.Comp_Code from TSPL_PAYMENT_HEADER  LEFT OUTER JOIN TSPL_Vendor_Master ON TSPL_Vendor_Master.Vendor_Code =TSPL_PAYMENT_HEADER.Vendor_Code " + Environment.NewLine & _
    ...
    '                   '" where TSPL_PAYMENT_HEADER.Posted=1 and  TSPL_PAYMENT_HEADER.Payment_Type  ='PY'" + Environment.NewLine & _
    ...
    '                   '        ''---------------------to find gain or loss amount---------------


    '                   ''richa aagwral changes done on 11 Aug,2016 against ticket no BM00000009447
    '                   'strtempBaseQry += " UNION ALL" + Environment.NewLine & _
    '                   '" select null AS Due_Date, '' AS PO_SRN, TSPL_VENDOR_MASTER.Vendor_Code as VCode, TSPL_VENDOR_MASTER.Vendor_Name as VName,TSPL_PAYMENT_HEADER.Payment_No as DocNo,'EXC' as [DocType],convert(date,Payment_Date, 103) as DocDate, case when ISNULL(TSPL_PAYMENT_HEADER.Is_Security,0)=1 Then 'Security' Else '' + 'Exchange Gain/Loss against Journal Entry No  '+isnull(TSPL_JOURNAL_MASTER.Voucher_No,'')    end as DocNarr,(TSPL_PAYMENT_HEADER.cheque_No+'-'+ CONVERT(VARCHAR,Cheque_Date , 103))as ChequeDetails, TSPL_PAYMENT_HEADER.CURRENCY_CODE, TSPL_PAYMENT_HEADER.ConvRate, TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT AS CrAmt,  TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT as DrAmt, 0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote, 'RV'as Document_Type, 0 as Balance_Amount,CASE WHEN TSPL_PAYMENT_HEADER.Payment_Type='AD' THEN ( sELECT Loc_Code  FROM TSPL_VENDOR_INVOICE_HEAD WHERE Document_No =(  sELECT TOP 1 TSPL_PAYMENT_DETAIL.Document_No  FROM TSPL_PAYMENT_HEADER PR LEFT OUTER JOIN  TSPL_PAYMENT_DETAIL ON PR.Payment_No=TSPL_PAYMENT_DETAIL.Payment_No  WHERE PR.Payment_No =TSPL_PAYMENT_HEADER.Payment_No  ))  ELSE substring(TSPL_PAYMENT_HEADER.Debit_Account , len(TSPL_PAYMENT_HEADER.Debit_Account )-2,3) END as account, TSPL_PAYMENT_HEADER.Payment_Post_Date as Posting_Date,'RV-TA' as GLDocType, TSPL_PAYMENT_HEADER.Comp_Code from TSPL_PAYMENT_HEADER  LEFT OUTER JOIN TSPL_Vendor_Master ON TSPL_Vendor_Master.Vendor_Code =TSPL_PAYMENT_HEADER.Vendor_Code " + Environment.NewLine & _
    '                   '" left outer join TSPL_BANK_MASTER on TSPL_BANK_MASTER.BANK_CODE=TSPL_PAYMENT_HEADER.Bank_Code" + Environment.NewLine & _
    '                   '" left outer join TSPL_JOURNAL_MASTER on TSPL_JOURNAL_MASTER.Source_Doc_No =TSPL_PAYMENT_HEADER.Payment_No " + Environment.NewLine & _
    ...
    '                   '" 'RV',0, CASE WHEN TSPL_PAYMENT_HEADER.Payment_Type='AD' THEN ( sELECT Loc_Code  FROM TSPL_VENDOR_INVOICE_HEAD WHERE Document_No =(  sELECT TOP 1 TSPL_PAYMENT_DETAIL.Document_No  FROM TSPL_PAYMENT_HEADER PR LEFT OUTER JOIN  TSPL_PAYMENT_DETAIL ON PR.Payment_No=TSPL_PAYMENT_DETAIL.Payment_No  WHERE PR.Payment_No =TSPL_PAYMENT_HEADER.Payment_No  ))  ELSE Case When TSPL_PAYMENT_HEADER.Payment_Type in ('AV','OA','RC') Then  TSPL_PAYMENT_HEADER.Location_GL_Code  Else '' End END as account, TSPL_PAYMENT_HEADER.Payment_Date as Posting_Date,'RV-TA' as GLDocType,TSPL_PAYMENT_HEADER.Comp_Code " + Environment.NewLine & _
    ...
    '       " Select null AS Due_Date, '' AS PO_SRN,TSPL_VENDOR_MASTER.Vendor_Code as VCode, TSPL_VENDOR_MASTER.Vendor_Name as VName, TSPL_BANK_REVERSE.Reverse_Code as DocNo," + Environment.NewLine & _
    '       " 'EXC'  as DocType  ,convert(date,TSPL_BANK_REVERSE.Reversal_Date,103) as DocDate,case when ISNULL(TSPL_PAYMENT_HEADER.Is_Security ,0)=1 Then 'Security' Else '' + 'Exchange Gain/Loss against Journal Entry No  '+isnull(TSPL_JOURNAL_MASTER.Voucher_No,'')    end as DocNarr,(rtrim(Entry_Desc) + (case when len(RTRIM(Entry_Desc))>0 and len(RTRIM(TSPL_PAYMENT_HEADER.Cheque_No))>0  then ' / ' else '' end)  + (case when LEN(ISNULL(TSPL_BANK_REVERSE.Cheque_No,''))>0 then 'Cheque No. - ' + TSPL_BANK_REVERSE.Cheque_No +  ' - '+ convert(varchar ,TSPL_PAYMENT_HEADER . Cheque_Date ,103)else '' end)) as ChequeDetails, TSPL_PAYMENT_HEADER.Currency_Code, 1 as ConvRate,  TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT AS CrAmt, TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT as DrAmt,  0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote," + Environment.NewLine & _
    '       " 'RV',0, isnull(TSPL_VENDOR_INVOICE_HEAD.Loc_Code ,'') as account, TSPL_PAYMENT_HEADER.Payment_Date as Posting_Date,'RV-TA' as GLDocType,TSPL_PAYMENT_HEADER.Comp_Code " + Environment.NewLine & _
    '       " from TSPL_BANK_REVERSE  LEFT OUTER JOIN TSPL_VENDOR_MASTER  ON TSPL_VENDOR_MASTER.Vendor_Code =TSPL_BANK_REVERSE.Vendor_Code   " + Environment.NewLine & _
    '       " LEFT OUTER JOIN TSPL_Vendor_TYPE_MASTER  ON TSPL_Vendor_TYPE_MASTER.Ven_Type_Code  = TSPL_VENDOR_MASTER.Ven_Type_Code   " + Environment.NewLine & _
    '       " left outer join TSPL_JOURNAL_MASTER on TSPL_JOURNAL_MASTER.Source_Doc_No =TSPL_BANK_REVERSE.Reverse_Code  " + Environment.NewLine & _
    '       " LEFT OUTER JOIN TSPL_PAYMENT_HEADER ON TSPL_PAYMENT_HEADER.PAYMENT_NO =TSPL_BANK_REVERSE.Document_No " + Environment.NewLine & _
    ...
    '               'strtempBaseQry = "  Select InnQuery.ACode AS ACode, InnQuery.AName AS AName,InnQuery.DocNo as DocNo,InnQuery.AgainstInvoiceNo AS AgainstInvoiceNo,InnQuery.DocDate ,InnQuery.DocType ,InnQuery.DocNarr ,InnQuery.ChequeDetails ,InnQuery.Currency_Code ,InnQuery.ConvRate,InnQuery.DrAmt ," + Environment.NewLine & _
    ...
    '               '" case when isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0)>0 AND (DocType)<>'EXC' then  (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0) else " + Environment.NewLine & _
    '               '" case when isnull((CrAmt),0)>0 then (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_RECEIPT_HEADER.EXCHANGE_GAIN_AMT ),0) else 0 end end else " + Environment.NewLine & _
    '               '" case when isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0) else (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_RECEIPT_HEADER.EXCHANGE_GAIN_AMT ),0)  end end else " + Environment.NewLine & _
    '               '" case when (RIGHT(TSPL_BANK_MASTER.BANKACC,3))=isnull(TSPL_RECEIPT_HEADER.Location_GL_Code,'') then " + Environment.NewLine & _
    '               '" case when isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then   (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0) else  (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_RECEIPT_HEADER.EXCHANGE_GAIN_AMT ),0) end else " + Environment.NewLine & _
    '               '" case when isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_RECEIPT_HEADER.EXCHANGE_LOSS_AMT),0) else  (CrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_RECEIPT_HEADER.EXCHANGE_GAIN_AMT ),0) end end end as CrAmt, " + Environment.NewLine & _
    '               '" InnQuery.SecurityDrAmt ,InnQuery.SecurityCrAmt ,InnQuery.Sales ,InnQuery.CollectionRefund,InnQuery.DrNote,InnQuery.CrNote,InnQuery.Location,InnQuery.SourceCode ,InnQuery.Item_Code,InnQuery.Item_Desc,InnQuery.Receipt_Type,InnQuery.Bank_Code,InnQuery.Cust_Type_Code ,InnQuery.Cust_Type_Desc,InnQuery.Cust_Category_Code ,InnQuery.CUST_CATEGORY_DESC    from  (" & strtempBaseQry & " ) InnQuery " + Environment.NewLine & _
    ...
    '               '"case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 AND (DocType)<>'EXC' then  (DrAmt*   " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0) else " + Environment.NewLine & _
    '               '"case when isnull((DrAmt),0)>0 then (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT ),0) else 0 end end else "
    '               ' '' "case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)  else (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  end end else " + Environment.NewLine & _
    '               'strtempBaseQry += " (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") end else " + Environment.NewLine & _
    ...
    '               '"case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)  else  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  end end end as DrAmt, " + Environment.NewLine & _
    '               '" InnQuery.CrAmt-InnQuery.DrAmt as Purchase, InnQuery.Payments, InnQuery.drNote, InnQuery.CrNote, InnQuery.Document_Type, InnQuery.Balance_Amount, InnQuery.account,InnQuery.Posting_Date, InnQuery.GLDocType, InnQuery.Comp_Code    from  (" & strtempBaseQry & " ) InnQuery " + Environment.NewLine & _
    ...
    '                   "case when (RIGHT(TSPL_BANK_MASTER.BANKACC,3))=(InnQuery.account) then " + Environment.NewLine & _
    '                   "case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 AND (DocType)<>'EXC' then  (DrAmt*   " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0) else " + Environment.NewLine & _
    '                   "case when isnull((DrAmt),0)>0 then (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT ),0) else 0 end end else " + Environment.NewLine & _
    ...
    '                   "case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then   (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0) else  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT ),0) end else " + Environment.NewLine & _
    '                   "case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)  else  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  end end end as DrAmt, " + Environment.NewLine & _
    '                   " case when InnQuery.DocType='Pur.Invoice' then InnQuery.CrAmt-InnQuery.DrAmt else 0 end as Purchase, InnQuery.Payments, InnQuery.drNote, InnQuery.CrNote, InnQuery.Document_Type, InnQuery.Balance_Amount, InnQuery.account,InnQuery.Posting_Date, InnQuery.GLDocType, InnQuery.Comp_Code    from  (" & strtempBaseQry & " ) InnQuery " + Environment.NewLine & _
    ...
    '               'If clsCommon.CompairString(FormType, clsUserMgtCode.MISCreditorReport) = CompairStringResult.Equal Then
    '               '    'strtempBaseQry += "case when InnQuery.DocType not in ('Receipt','Pur.Invoice','Reverse Payment') then case when InnQuery.DocType not in ('AP Invoice') then InnQuery.CrAmt else case when  InnQuery.DocType in ('AP Invoice') and (Select count(Document_No) from tspl_vendor_invoice_head where document_no=InnQuery.DocNo  and ISNULL(Against_VCGL  ,'')<>'')=1 then InnQuery.CrAmt else 0 end end  else 0 end as CrAmt," & _
    '               '    'strtempBaseQry += " CASE WHEN InnQuery.DocType NOT IN ('Pur.Invoice','Reverse Payment','Receipt')  THEN case when InnQuery.DocType not in ('TDS','AP Invoice') THEN InnQuery.CrAmt  *(case when (DocType) NOT IN ('EXC','Credit Note','IM') then   InnQuery.ConvRate else 1 end) when  InnQuery.DocType in ('AP Invoice') and ((Select count(Document_No) from tspl_vendor_invoice_head where document_no=InnQuery.DocNo   and ISNULL(Against_VCGL  ,'')<>'')=1 OR (Select count(Document_No) from tspl_vendor_invoice_head where document_no=InnQuery.DocNo and (isnull(Against_BulkMillkPurchaseInvoice_No,'') ='' AND isnull(Against_MillkPurchaseInvoice_No,'') ='' AND ISNULL(Against_POInvoice_No ,'')='' AND ISNULL(Against_PurchaseReturn_No  ,'')='' AND ISNULL(Against_VSPItemIssue_No,'')='') )=1) then InnQuery.CrAmt ELSE CASE WHEN InnQuery.DocType in ('TDS') and (Select ISNULL(TSPL_REMITTANCE.Document_Type,'')  from TSPL_REMITTANCE LEFT OUTER join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No = TSPL_REMITTANCE.Document_No  and  isnull(TSPL_VENDOR_INVOICE_HEAD.Against_PurchaseReturn_No,'')='' where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  )='I' or (Select ISNULL(TSPL_REMITTANCE.Document_Type,'')  from TSPL_REMITTANCE LEFT OUTER join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No = TSPL_REMITTANCE.Document_No  and  isnull(TSPL_VENDOR_INVOICE_HEAD.Against_PurchaseReturn_No,'')='' where TSPL_REMITTANCE.Document_No=InnQuery.DocNo   )='C' then (Select ISNULL (Actual_Total_TDS ,0)  from TSPL_REMITTANCE where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  ) * -1 else 0 end END ELSE 0 END as CrAmt ," & _
    '               '    '    " case when InnQuery.DocType not in ('Payment','On Account','Advance','Receipt','Reverse Payment','Adjustment') then case when InnQuery.DocType not in ('AP Invoice','TDS') then InnQuery.DrAmt WHEN InnQuery.DocType in ('TDS') and   (Select ISNULL(TSPL_REMITTANCE.Document_Type,'')  from TSPL_REMITTANCE LEFT OUTER join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No = TSPL_REMITTANCE.Document_No  and  isnull(TSPL_VENDOR_INVOICE_HEAD.Against_PurchaseReturn_No,'')='' where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  )='D' then (Select ISNULL (Actual_Total_TDS ,0)  from TSPL_REMITTANCE where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  ) * -1 else case when  InnQuery.DocType in ('AP Invoice') and (Select count(Document_No) from tspl_vendor_invoice_head where document_no=InnQuery.DocNo  and ISNULL(Against_VCGL  ,'')<>'')=1 then InnQuery.DrAmt else 0 end end else 0 end DrAmt," & _
    '               '    '    " case when InnQuery.DocType not in ('Payment','On Account','Advance','Receipt','Debit Note','Reverse Payment','Credit Note','EXC','IM') then case when InnQuery.DocType not in ('AP Invoice','TDS') then InnQuery.Purchase else  case when  InnQuery.DocType in ('AP Invoice') and (Select count(Document_No) from tspl_vendor_invoice_head where document_no=InnQuery.DocNo  and (isnull(Against_BulkMillkPurchaseInvoice_No,'') <>'' or isnull(Against_MillkPurchaseInvoice_No,'') <>'' or ISNULL(Against_POInvoice_No ,'')<>'' or ISNULL(Against_PurchaseReturn_No  ,'')<>'' or ISNULL(Against_VSPItemIssue_No,'')<>'') and ISNULL(Against_VCGL  ,'')='')=1  then InnQuery.Purchase else 0 end end else 0 end Purchase," & _
    '               '    '    " case when InnQuery.DocType  in ('Payment','On Account','Advance','Receipt','TDS','Reverse Payment','EXC','IM') then InnQuery.Payments * (case when (DocType)<>'EXC' then   InnQuery.ConvRate else 1 end) else 0 end as Payments , "
    '               '    strtempBaseQry += " CASE WHEN InnQuery.DocType NOT IN ('Pur.Invoice','Receipt')  THEN case when InnQuery.DocType not in ('TDS','AP Invoice') THEN InnQuery.CrAmt  *(case when (DocType) NOT IN ('EXC','Credit Note','IM','Reverse Payment') then   InnQuery.ConvRate else 1 end) when  InnQuery.DocType in ('AP Invoice') and (Select Document_Type  from tspl_vendor_invoice_head where document_no=InnQuery.DocNo)='C' then InnQuery.CrAmt ELSE CASE WHEN InnQuery.DocType in ('TDS') and (Select ISNULL(TSPL_REMITTANCE.Document_Type,'')  from TSPL_REMITTANCE LEFT OUTER join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No = TSPL_REMITTANCE.Document_No  and  isnull(TSPL_VENDOR_INVOICE_HEAD.Against_PurchaseReturn_No,'')='' where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  )='I' or (Select ISNULL(TSPL_REMITTANCE.Document_Type,'')  from TSPL_REMITTANCE LEFT OUTER join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No = TSPL_REMITTANCE.Document_No  and  isnull(TSPL_VENDOR_INVOICE_HEAD.Against_PurchaseReturn_No,'')='' where TSPL_REMITTANCE.Document_No=InnQuery.DocNo   )='C' then (Select ISNULL (Actual_Total_TDS ,0)  from TSPL_REMITTANCE where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  ) * -1 else 0 end END ELSE 0 END as CrAmt ," & _
    '               '      " case when InnQuery.DocType not in ('Payment','On Account','Advance','Receipt','Adjustment') then case when InnQuery.DocType not in ('AP Invoice','TDS') then InnQuery.DrAmt WHEN InnQuery.DocType in ('TDS') and   (Select ISNULL(TSPL_REMITTANCE.Document_Type,'')  from TSPL_REMITTANCE LEFT OUTER join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No = TSPL_REMITTANCE.Document_No  and  isnull(TSPL_VENDOR_INVOICE_HEAD.Against_PurchaseReturn_No,'')='' where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  )='D' then (Select ISNULL (Actual_Total_TDS ,0)  from TSPL_REMITTANCE where TSPL_REMITTANCE.Document_No=InnQuery.DocNo  ) * -1 else case when  InnQuery.DocType in ('AP Invoice') AND  (Select Document_Type  from tspl_vendor_invoice_head where document_no=InnQuery.DocNo AND ISNULL(Against_PurchaseReturn_No,'')='')='D' then InnQuery.DrAmt else 0 end end else 0 end DrAmt, " & _
    '               '      " case when InnQuery.DocType not in ('Payment','On Account','Advance','Receipt','Debit Note','Reverse Payment','Credit Note','EXC','IM') then case when InnQuery.DocType not in ('AP Invoice','TDS') then InnQuery.Purchase else case when  InnQuery.DocType in ('AP Invoice') and (Select Document_Type  from tspl_vendor_invoice_head where document_no=InnQuery.DocNo)='I'	 then InnQuery.Purchase when  InnQuery.DocType in ('AP Invoice')  and (Select Document_Type  from tspl_vendor_invoice_head where document_no=InnQuery.DocNo AND ISNULL(Against_PurchaseReturn_No,'')<>'')='D' then InnQuery.Purchase *-1  else 0 end end else 0 end Purchase, " & _
    '               '      " case when InnQuery.DocType  in ('Payment','On Account','Advance','Receipt','TDS','EXC','IM') then InnQuery.Payments * (case when (DocType)<>'EXC' then   InnQuery.ConvRate else 1 end) else 0 end as Payments , "

    '               '    '" case when InnQuery.DocType  in ('Payment','On Account','Advance','Receipt','TDS','Reverse Payment','EXC','IM') then InnQuery.Payments * (case when (DocType)<>'EXC' then   InnQuery.ConvRate else 1 end)+case when (DocType)='Reverse Payment' then  (Select isnull((PH.EXCHANGE_LOSS_AMT  ),0) from TSPL_PAYMENT_HEADER PH where PH.Payment_No =(  Select Document_No   from TSPL_BANK_REVERSE where Reverse_Code =InnQuery.DocNo )) else 0 end else 0 end as Payments , "
    '               'Else
    '               '    strtempBaseQry += " InnQuery.CrAmt,InnQuery.DrAmt,InnQuery.Purchase,InnQuery.Payments, "
    '               'End If
    ...
    '               '"case when (RIGHT(TSPL_BANK_MASTER.BANKACC,3))=(InnQuery.account) then " + Environment.NewLine & _
    '               '"case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 AND (DocType)<>'EXC' then  (DrAmt*   " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") +isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0) else " + Environment.NewLine & _
    '               '"case when isnull((DrAmt),0)>0 AND (DocType)<>'EXC' then (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ") -isnull((TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT ),0) else isnull((DrAmt),0) end end else " + Environment.NewLine & _
    '               '"case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  else (DrAmt*  " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  end end else " + Environment.NewLine & _
    '               '"case when (RIGHT(TSPL_BANK_MASTER.BANKACC,3))=isnull(TSPL_PAYMENT_HEADER.Location_GL_Code,'') then " + Environment.NewLine & _
    ...
    '               '"case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  else  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  end end end as DrAmt, " + Environment.NewLine & _
    '               '" InnQuery.CrAmt-InnQuery.DrAmt as Purchase, InnQuery.Payments, InnQuery.drNote, InnQuery.CrNote, InnQuery.Document_Type, InnQuery.Balance_Amount, InnQuery.account,InnQuery.Posting_Date, InnQuery.GLDocType, InnQuery.Comp_Code    from  (" & strtempBaseQryforopening & " ) InnQuery " + Environment.NewLine & _
    ...
    '              "case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 AND  (DocType)<>'EXC' then   (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  else case when (DocType)<>'EXC' then  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  else isnull((TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT ),0) end end else " + Environment.NewLine & _
    '              "case when isnull((TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT),0)>0 then  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  else  (DrAmt* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " InnQuery." & ddlCurrencyType.SelectedValue & "") & ")  end end end as DrAmt, " + Environment.NewLine & _
    '              " CONVERT(DECIMAL(18,2),InnQuery.CrAmt-InnQuery.DrAmt) as Purchase, InnQuery.Payments, InnQuery.drNote, InnQuery.CrNote, InnQuery.Document_Type, InnQuery.Balance_Amount, InnQuery.account,InnQuery.Posting_Date, InnQuery.GLDocType, InnQuery.Comp_Code    from  (" & strtempBaseQryforopening & " ) InnQuery " + Environment.NewLine & _
    '              "  left outer join TSPL_PAYMENT_HEADER on TSPL_PAYMENT_HEADER.Payment_No  =InnQuery.DocNo LEFT OUTER JOIN TSPL_BANK_MASTER ON TSPL_BANK_MASTER.BANK_CODE=TSPL_PAYMENT_HEADER.Bank_Code) InnQuery  "


    '               If clsCommon.CompairString(FormType, clsUserMgtCode.MISCreditorReport) = CompairStringResult.Equal Then
    '                   'strtempBaseQryforopeningForMIS = "  Select InnQuery.Due_Date, InnQuery.PO_SRN, InnQuery.VCode, InnQuery.VName, InnQuery.DocNo, InnQuery.DocType, InnQuery.DocDate, InnQuery.DocNarr, InnQuery.ChequeDetails, InnQuery.Currency_Code, InnQuery.ConvRate,  " & _
    ...
    '   End Function
    '    Public Function GetVendorBaseQry(ByVal strPortrait As Boolean, ByVal strLandscape As Boolean, ByVal IsOnlyForAgainstSalary As Boolean, ByVal strfromdate As String, ByVal strtodate As String, ByVal strvendor As String) As String
    '        Dim strtempBaseQry As String = String.Empty
    '        Try
    '            If Not IsOnlyForAgainstSalary Then
    '                Dim strQryForRMDA As String = " - (case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' and LEN(ISNULL(TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No,''))>0 then (Select sum(TSPL_TRANSFER_ORDER_HEAD.DOC_Total_Amt) from TSPL_TRANSFER_ORDER_HEAD where Status=1 and TSPL_TRANSFER_ORDER_HEAD.RMDA_Code in (select TSPL_SRN_HEAD.RMDA_No from TSPL_PI_DETAIL left outer join TSPL_PI_HEAD on TSPL_PI_HEAD.PI_No=TSPL_PI_DETAIL.PI_No left outer join TSPL_SRN_HEAD on TSPL_SRN_HEAD.SRN_No=TSPL_PI_HEAD.Against_SRN where TSPL_PI_HEAD.PI_No=TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No  )) else 0 end )"
    ...
    '                strtempBaseQry += " case when TSPL_VENDOR_INVOICE_HEAD.Document_Type IN('I','C') Then document_total else 0 end + ( case when isnull(TAX1 ,'')='WCT' THEN TAX1_Amt * -1  when isnull(TAX2 ,'')='WCT' THEN TAX2_Amt * -1 when isnull(TAX3 ,'')='WCT' THEN TAX3_Amt * -1 when isnull(TAX4 ,'')='WCT' THEN TAX4_Amt * -1 when isnull(TAX5 ,'')='WCT' THEN TAX5_Amt * -1 when isnull(TAX6 ,'')='WCT' THEN TAX6_Amt * -1 when isnull(TAX7 ,'')='WCT' THEN TAX7_Amt * -1 when isnull(TAX8 ,'')='WCT' THEN TAX8_Amt * -1 when isnull(TAX9 ,'')='WCT' THEN TAX9_Amt * -1 when isnull(TAX10 ,'')='WCT' THEN TAX10_Amt * -1 ELSE 0 END) as CrAmt, " & _
    '                    "  case when TSPL_VENDOR_INVOICE_HEAD.Document_Type IN('D') then Document_Total else 0 end as DrAmt, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='I' Then document_total Else 0 End as Purchase, 0 as Payments, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' Then document_total Else 0 End as DrNote, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' Then document_total Else 0 End as CrNote, TSPL_VENDOR_INVOICE_HEAD.Document_Type,balance_Amt as Balance_Amount, ( select   GL_Account_Code from TSPL_VENDOR_INVOICE_DETAIL where Detail_Line_No='1' and Document_No=tspl_vendor_invoice_head.Document_No  ) as account, convert(date,tspl_vendor_invoice_head.Posting_Date,103) as Posting_Date ,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' then 'AP-DN' when TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' then 'AP-CN' else 'AP-IN' end as GLDocType, tspl_vendor_invoice_head.Comp_Code from tspl_vendor_invoice_head where ISNULL(tspl_vendor_invoice_head.Posting_Date,'')<>''  and LEN(ISNULL( TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No,''))<=0   " + Environment.NewLine & _
    '                 "  and  convert(date,Invoice_Entry_Date,103)  >='" + strfromdate + "' and  convert(date,Invoice_Entry_Date,103)  <='" + strtodate + "' " + Environment.NewLine
    ...
    '                " 0 as CrAmt,case when isnull(TAX1 ,'')='WCT' THEN TAX1_Amt * -1  when isnull(TAX2 ,'')='WCT' THEN TAX2_Amt * -1 when isnull(TAX3 ,'')='WCT' THEN TAX3_Amt * -1 when isnull(TAX4 ,'')='WCT' THEN TAX4_Amt * -1 when isnull(TAX5 ,'')='WCT' THEN TAX5_Amt * -1 when isnull(TAX6 ,'')='WCT' THEN TAX6_Amt * -1 when isnull(TAX7 ,'')='WCT' THEN TAX7_Amt * -1 when isnull(TAX8 ,'')='WCT' THEN TAX8_Amt * -1 when isnull(TAX9 ,'')='WCT' THEN TAX9_Amt * -1 when isnull(TAX10 ,'')='WCT' THEN TAX10_Amt * -1 ELSE 0 END  DrAmt, " & _
    '                " Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='I' Then document_total Else 0 End as Purchase, 0 as Payments, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' Then document_total Else 0 End as DrNote, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' Then document_total Else 0 End as CrNote, TSPL_VENDOR_INVOICE_HEAD.Document_Type,balance_Amt as Balance_Amount, ( select   GL_Account_Code from TSPL_VENDOR_INVOICE_DETAIL where Detail_Line_No='1' and Document_No=tspl_vendor_invoice_head.Document_No  ) as account, convert(date,tspl_vendor_invoice_head.Posting_Date,103) as Posting_Date ,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' then 'AP-DN' when TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' then 'AP-CN' else 'AP-IN' end as GLDocType, tspl_vendor_invoice_head.Comp_Code from tspl_vendor_invoice_head where ISNULL(tspl_vendor_invoice_head.Posting_Date,'')<>''  and LEN(ISNULL( TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No,''))<=0  ) FinalWCt where (FinalWCt.CrAmt <>0 or FinalWCt.DrAmt <>0) " + Environment.NewLine & _
    '                " and  convert(date,FinalWCt.DocDate ,103)  >='" + strfromdate + "' and  convert(date,FinalWCt.DocDate ,103)  <='" + strtodate + "' "
    '                If clsCommon.myLen(strvendor) > 0 Then
    ...
    '                ' If rbPortrait.IsChecked = True Then
    '                If strPortrait = True Then
    '                    strtempBaseQry += " TSPL_PI_HEAD.Vendor_Code as VCode,TSPL_PI_HEAD.Vendor_Name as VName, TSPL_VENDOR_INVOICE_HEAD.Document_No as DocNo,'Pur.Invoice' as DocType,convert(date,TSPL_PI_HEAD.PI_Date,103) as DocDate,(TSPL_PJV_HEAD.PJV_No+' - '+'SRN No-'+TSPL_PI_HEAD.Against_SRN+ ' ,Vendor Invoice No-'+ TSPL_PI_HEAD.Vendor_Invoice_No +' Date-' +Vendor_Invoice_Date)+ (SELECT ' & Items Are- '+STUFF((SELECT ', ' + t2.Item_Desc+'('+Convert(Varchar,COnvert(Decimal(18,2),t2.Item_Cost))+' Rs)' FROM TSPL_PI_DETAIL t2 WHERE t2.PI_No = TSPL_PI_HEAD.PI_No FOR XML PATH ('')),1,2,'')) as DocNarr,'' as ChequeDetails, TSPL_VENDOR_INVOICE_HEAD.CURRENCY_CODE, TSPL_VENDOR_INVOICE_HEAD.ConvRate, (case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='I' then TSPL_VENDOR_INVOICE_HEAD.Document_Total else 0 end) as CrAmt, (case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' Then TSPL_VENDOR_INVOICE_HEAD.Document_Total else 0 end ) as DrAmt, TSPL_VENDOR_INVOICE_HEAD.Document_Type, TSPL_VENDOR_INVOICE_HEAD.Balance_Amt" + strQryForRMDA + " as Balance_Amount, ( select Loc_Segment_Code from TSPL_LOCATION_MASTER where Location_Code=TSPL_PI_HEAD.Bill_To_Location) as account, convert(date,TSPL_PI_HEAD.Posting_Date,103) as Posting_Date ,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' then 'AP-DN' when TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' then 'AP-CN' else 'AP-IN' end as GLDocType, TSPL_PI_HEAD.Comp_Code from TSPL_PI_HEAD  left outer join TSPL_PJV_HEAD on TSPL_PI_HEAD.PI_No=TSPL_PJV_HEAD.Invoice_No left outer join TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Vendor_Invoice_No=TSPL_PI_HEAD.Vendor_Invoice_No and TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No=TSPL_PI_HEAD.PI_No where TSPL_PI_HEAD.Status=1" + Environment.NewLine & _
    '                        " and  convert(date,TSPL_PI_HEAD.PI_Date ,103)  >='" + strfromdate + "' and  convert(date,TSPL_PI_HEAD.PI_Date ,103)  <='" + strtodate + "'"
    '                    If clsCommon.myLen(strvendor) > 0 Then
    ...
    '                End If
    '                strtempBaseQry += " AND TSPL_VENDOR_INVOICE_HEAD.TDS_Actual_Amount >0 then 0 else Actual_Total_TDS END as Payments, case when TSPL_REMITTANCE.Document_Type ='D' AND TSPL_VENDOR_INVOICE_HEAD.TDS_Actual_Amount >0 then -1*Actual_Total_TDS else 0 END as DrNotes, 0 as CrNotes, 'TDS',0, Case When TSPL_PAYMENT_HEADER.Payment_Type in ('AV','OA','RC') Then TSPL_PAYMENT_HEADER.Location_GL_Code Else right( Branch_GL_AC,3) End as account, Case When ISNULL(TSPL_VENDOR_INVOICE_HEAD.Posting_Date,'')<>'' Then TSPL_VENDOR_INVOICE_HEAD.Posting_Date Else TSPL_PAYMENT_HEADER.Payment_Post_Date End as Posting_Date,case when TSPL_REMITTANCE.Document_Type in ('C') then 'AP-CN' when TSPL_REMITTANCE.Document_Type in ('I') then 'AP-IN' when TSPL_REMITTANCE.Document_Type in ('D') then 'AP-DN' else case when TSPL_REMITTANCE.Document_Type in ('AV','OA','RC') then 'AP-PY' else '' end end as GLDocType, TSPL_REMITTANCE.Comp_Code from TSPL_REMITTANCE Left Outer Join TSPL_VENDOR_INVOICE_HEAD ON TSPL_REMITTANCE.Document_No=TSPL_VENDOR_INVOICE_HEAD.Document_No LEFT OUTER JOIN TSPL_PAYMENT_HEADER ON TSPL_PAYMENT_HEADER.Payment_No=TSPL_REMITTANCE.Document_No where Remit_TDS is not null   and   Branch_GL_AC  is not null  and Actual_Total_TDS<>0 AND ISNULL(TSPL_VENDOR_INVOICE_HEAD.is_For_TDS,0)<>1 AND ISNULL(TSPL_PAYMENT_HEADER.Advance_Against_Salary,0)<>1 AND ISNULL(TSPL_PAYMENT_HEADER.Is_Security,0)<>1   " + Environment.NewLine & _
    '                    " and  convert(date,TSPL_REMITTANCE.Document_Date  ,103)  >='" + strfromdate + "' and  convert(date,TSPL_REMITTANCE.Document_Date ,103)  <='" + strtodate + "'"
    '                If clsCommon.myLen(strvendor) > 0 Then
    ...
    '               " left outer join TSPL_BANK_MASTER on TSPL_BANK_MASTER.BANK_CODE=TSPL_BANK_REVERSE.Bank_Code" & _
    '               " where Source_Type ='AP' And TSPL_BANK_REVERSE.Post='P' AND TSPL_BANK_REVERSE.Vendor_Code<> '' AND TSPL_PAYMENT_HEADER.IsChkReverse='Y' AND ISNULL(TSPL_PAYMENT_HEADER.Advance_Against_Salary,0)<>1 AND ISNULL(TSPL_PAYMENT_HEADER.Is_Security,0)<>1  " & IIf(chkIncludeApplyDocument.Checked = False, " and TSPL_PAYMENT_HEADER.Payment_Type<>'AD' ", "") & " " + Environment.NewLine & _
    '               " and  convert(date,TSPL_BANK_REVERSE.Reversal_Date ,103)  >='" + strfromdate + "' and  convert(date,TSPL_BANK_REVERSE.Reversal_Date  ,103)  <='" + strtodate + "' "
    '                    If clsCommon.myLen(strvendor) > 0 Then
    ...
    '                strtempBaseQry += " UNION ALL" + Environment.NewLine & _
    '                " select null AS Due_Date, '' AS PO_SRN, VC_Code as VCode, VC_Name as VName, TSPL_VCGL_Head.Document_No as DocNo,case when TSPL_VCGL_Head.Document_Type='V' then 'Vendor' else '' end as DocType,CONVERT(Date,TSPL_VCGL_Head.Document_Date,103) as DocDate, TSPL_VCGL_Head.Remarks as DocNarr, '' as ChequeDetails, 'INR' as CURRENCY_CODE, 1 as ConvRate, (case when Amount_Type='Dr' then Amount else 0 end) as CrAmt,(case when Amount_Type='Cr' then Amount else 0 end) as DrAmt, 0 as Purchase, 0 as Payments, case when Amount_Type='Cr' then Amount else 0 end as DrNote, case when Amount_Type='Dr' then Amount else 0 end as CrNote, (case when  TSPL_VCGL_Head.Document_Type='V' then 'Vendor' else '' end ) as Document_Type ,0 as Balance_Amount, (TSPL_VCGL_Head.Location_Segment) as account, TSPL_VCGL_Head.Posting_Date as Posting_Date,'VC-GL' as GLDocType,TSPL_VCGL_Head.Comp_Code from TSPL_VCGL_Head LEFT OUTER JOIN TSPL_VENDOR_INVOICE_HEAD ON TSPL_VENDOR_INVOICE_HEAD.Against_VCGL=TSPL_VCGL_Head.Document_No where TSPL_VCGL_Head.Document_Type='v' and TSPL_VCGL_Head.Status='1' AND ISNULL(TSPL_VENDOR_INVOICE_HEAD.Against_VCGL,'')=''  " + Environment.NewLine & _
    '" and  convert(date,TSPL_VCGL_Head.Document_Date ,103)  >='" + strfromdate + "' and  convert(date,TSPL_VCGL_Head.Document_Date,103)  <='" + strtodate + "'"
    '                If clsCommon.myLen(strvendor) > 0 Then
    ...
    '                '"  select NULL AS Due_Date, '' AS PO_SRN, TSPL_Payment_Adjustment_Header.Vendor_No as VCode, TSPL_Payment_Adjustment_Header.Vendor_Name as VName, TSPL_Payment_Adjustment_Header.Adjustment_No as DocNo, 'Adjustment' as DocType,CONVERT(date,TSPL_Payment_Adjustment_Header.Adjustment_Date,103) as DocDate,'Against Vendor Invoice No. ' +TSPL_Payment_Adjustment_Header.Doc_No as DocNarr,'' as ChequeDetails, 'INR' as CURRENCY_CODE, 1 as ConvRate, 0 as CrAmt,TSPL_Payment_Adjustment_Detail.Amount  as DrAmt, 0 as Purchase, 0 as Payments, TSPL_Payment_Adjustment_Detail.Amount as DrNote, 0 as CrNote, 'PAE' as Document_Type,0 as Balance_Amount, TSPL_VENDOR_INVOICE_HEAD.Loc_Code as account, TSPL_Payment_Adjustment_Header.Post_Date as Posting_Date ,'' as GLDocType, TSPL_Payment_Adjustment_Header.Comp_Code from  TSPL_Payment_Adjustment_Detail  left outer join  TSPL_Payment_Adjustment_Header on TSPL_Payment_Adjustment_Header.Adjustment_No =TSPL_Payment_Adjustment_Detail .Adjustment_No  LEFT OUTER JOIN TSPL_VENDOR_INVOICE_HEAD ON TSPL_VENDOR_INVOICE_HEAD.Document_No  =TSPL_Payment_Adjustment_Header.Doc_No  where isnull(TSPL_Payment_Adjustment_Header.Is_Post,'') ='Y'   " + Environment.NewLine & _
    '                strtempBaseQry += "  select NULL AS Due_Date, '' AS PO_SRN, TSPL_Payment_Adjustment_Header.Vendor_No as VCode, TSPL_Payment_Adjustment_Header.Vendor_Name as VName, TSPL_Payment_Adjustment_Header.Adjustment_No as DocNo, 'Adjustment' as DocType,CONVERT(date,TSPL_Payment_Adjustment_Header.Adjustment_Date,103) as DocDate,'Against Vendor Invoice No. ' +TSPL_Payment_Adjustment_Header.Doc_No as DocNarr,'' as ChequeDetails, 'INR' as CURRENCY_CODE, 1 as ConvRate, CASE WHEN ISNULL(TSPL_VENDOR_INVOICE_HEAD.Document_Type,'')='D' THEN  TSPL_Payment_Adjustment_Detail.Amount ELSE 0 END as CrAmt, CASE WHEN ISNULL(TSPL_VENDOR_INVOICE_HEAD.Document_Type,'')<>'D' THEN  TSPL_Payment_Adjustment_Detail.Amount ELSE 0 END  as DrAmt, 0 as Purchase, 0 as Payments, TSPL_Payment_Adjustment_Detail.Amount as DrNote, 0 as CrNote, 'PAE' as Document_Type,0 as Balance_Amount, TSPL_VENDOR_INVOICE_HEAD.Loc_Code as account, TSPL_Payment_Adjustment_Header.Post_Date as Posting_Date ,'' as GLDocType, TSPL_Payment_Adjustment_Header.Comp_Code from  TSPL_Payment_Adjustment_Detail  left outer join  TSPL_Payment_Adjustment_Header on TSPL_Payment_Adjustment_Header.Adjustment_No =TSPL_Payment_Adjustment_Detail .Adjustment_No  LEFT OUTER JOIN TSPL_VENDOR_INVOICE_HEAD ON TSPL_VENDOR_INVOICE_HEAD.Document_No  =TSPL_Payment_Adjustment_Header.Doc_No  where isnull(TSPL_Payment_Adjustment_Header.Is_Post,'') ='Y' and TSPL_Payment_Adjustment_Header.Adjust_Type='P'  " + Environment.NewLine & _
    '                    " and  convert(date,TSPL_Payment_Adjustment_Header.Adjustment_Date ,103)  >='" + strfromdate + "' and  convert(date,TSPL_Payment_Adjustment_Header.Adjustment_Date,103)  <='" + strtodate + "' "
    '                If clsCommon.myLen(strvendor) > 0 Then
    ...
    '                " Select NULL as Due_Date, MAX(PO_SRN) as PO_SRN, MAX(VCode) as VCode, MAX(VName) as VName, DocNo, MAX(DocType) as DocType, MAX(DocDate) as DocDate, MAX(DocNarr) as DocNarr, MAX(ChequeDetails) as ChequeDetails, MAX(Currency_Code) as Currency_Code, MAX(ConvRate) as ConvRate, SUM(CrAmt) as CrAmt, SUM(DrAmt) DrAmt, 0 as Purchase, SUM(DrAmt)-SUM(CrAmt) as Payments, 0 as DrNote, 0 as CrNote, MAX(Document_Type) as Document_Type, SUM(Balance_Amount) as Balance_Amount, account, MAX(Posting_Date) as Posting_Date, MAX(GLDocType) as GLDocType, MAX(Comp_Code) as Comp_Code from (" + Environment.NewLine & _
    '                " select  NULL AS Due_Date,'' AS PO_SRN, tspl_payment_header.vendor_code as VCode, tspl_payment_header.vendor_name as VName, TSPL_PAYMENT_HEADER.payment_no as DocNo, case when TSPL_PAYMENT_HEADER.Payment_Type='AV' then 'Advance' when TSPL_PAYMENT_HEADER.Payment_Type='OA' then 'On Account' when TSPL_PAYMENT_HEADER.Payment_Type='PY' then 'Payment' when TSPL_PAYMENT_HEADER.Payment_Type='RC' then 'Receipt' else 'Mislleneous' end as DocType, COnvert(date,payment_date, 103) as DocDate, ((entry_desc)+(case when entry_desc='' then '' else ' - 'end)+(reference)+(case when reference='' then '' else ' - 'end)+  ( narration)) as DocNarr, ((Cheque_No) + (case when Cheque_No='' then '' else ' - ' end) +CONVERT(Varchar, Cheque_Date, 103)) as ChequeDetails, TSPL_PAYMENT_HEADER.CURRENCY_CODE, TSPL_PAYMENT_HEADER.ConvRate, case when TSPL_PAYMENT_HEADER.payment_type in ('RC') then Payment_Amount else 0 end as CrAmt, case when TSPL_PAYMENT_HEADER.payment_type IN('OA','AV') then Payment_Amount When TSPL_PAYMENT_HEADER.payment_type IN('PY') Then (Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' Then -TSPL_PAYMENT_DETAIL.Applied_Amount Else TSPL_PAYMENT_DETAIL.Applied_Amount End) else 0 end as DrAmt, TSPL_PAYMENT_HEADER.Payment_Type as Document_Type,Payment_Amount as Balance_Amount, Case When TSPL_PAYMENT_HEADER.Payment_Type in ('PY') Then TSPL_VENDOR_INVOICE_HEAD.Loc_Code When TSPL_PAYMENT_HEADER.Payment_Type in ('AV','OA','RC') Then TSPL_PAYMENT_HEADER.Location_GL_Code Else right(TSPL_BANK_MASTER.BANKACC,3) End as account, TSPL_PAYMENT_HEADER.Payment_Post_Date as Posting_Date, case when TSPL_PAYMENT_HEADER.Payment_Type in ('PY','AV','OA','RC') then 'AP-PY' else case when TSPL_PAYMENT_HEADER.Payment_Type in ('MI') then 'AP-MI' else '' end  end as GLDocType, tspl_payment_header.Comp_Code from tspl_payment_header LEFT OUTER JOIN TSPL_PAYMENT_DETAIL on TSPL_PAYMENT_DETAIL.Payment_No=TSPL_PAYMENT_HEADER.Payment_No left outer join TSPL_BANK_MASTER on tspl_payment_header.Bank_Code=TSPL_BANK_MASTER.BANK_CODE LEFT OUTER JOIN TSPL_VENDOR_INVOICE_HEAD on TSPL_VENDOR_INVOICE_HEAD.Document_No=TSPL_PAYMENT_DETAIL.Document_No Where tspl_payment_header.Payment_Type NOT IN ('MI','AD') AND (Posted='P' or Posted='1') AND ISNULL(TSPL_PAYMENT_HEADER.Advance_Against_Salary,0)<>1 AND ISNULL(TSPL_PAYMENT_HEADER.Is_Security,0)<>1   " + Environment.NewLine & _
    '                "  and  convert(date,TSPL_PAYMENT_HEADER.payment_date ,103)  >='" + strfromdate + "' and  convert(date,TSPL_PAYMENT_HEADER.payment_date,103)  <='" + strtodate + "' "
    '                If clsCommon.myLen(strvendor) > 0 Then
    ...
    '                    End If
    '                    strtempBaseQry += " ---------------------to find gain or loss amount for payment---------------" + Environment.NewLine & _
    '        " UNION ALL" + Environment.NewLine & _
    '        " Select null AS Due_Date, '' AS PO_SRN,TSPL_VENDOR_MASTER.Vendor_Code as VCode, TSPL_VENDOR_MASTER.Vendor_Name as VName, TSPL_BANK_REVERSE.Reverse_Code as DocNo," + Environment.NewLine & _
    '        " 'EXC'  as DocType  ,convert(date,TSPL_BANK_REVERSE.Reversal_Date,103) as DocDate,case when ISNULL(TSPL_PAYMENT_HEADER.Is_Security ,0)=1 Then 'Security' Else '' + 'Exchange Gain/Loss against Journal Entry No  '+isnull(TSPL_JOURNAL_MASTER.Voucher_No,'')    end as DocNarr,(rtrim(Entry_Desc) + (case when len(RTRIM(Entry_Desc))>0 and len(RTRIM(TSPL_PAYMENT_HEADER.Cheque_No))>0  then ' / ' else '' end)  + (case when LEN(ISNULL(TSPL_BANK_REVERSE.Cheque_No,''))>0 then 'Cheque No. - ' + TSPL_BANK_REVERSE.Cheque_No +  ' - '+ convert(varchar ,TSPL_PAYMENT_HEADER . Cheque_Date ,103)else '' end)) as ChequeDetails, TSPL_PAYMENT_HEADER.Currency_Code, 1 as ConvRate,  TSPL_PAYMENT_HEADER.EXCHANGE_GAIN_AMT AS CrAmt, TSPL_PAYMENT_HEADER.EXCHANGE_LOSS_AMT as DrAmt,  0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote," + Environment.NewLine & _
    '        " 'RV',0, isnull(TSPL_VENDOR_INVOICE_HEAD.Loc_Code ,'') as account, TSPL_PAYMENT_HEADER.Payment_Date as Posting_Date,'RV-TA' as GLDocType,TSPL_PAYMENT_HEADER.Comp_Code " + Environment.NewLine & _
    ...
    '                        strtempBaseQry += " and tspl_payment_header.vendor_code in (" & strvendor & ")"
    '                    End If
    '                    strtempBaseQry += " --------------- INVOICE AGAINST APPLY DOCUMENT" + Environment.NewLine & _
    '                    " UNION ALL" + Environment.NewLine & _
    '                    " SELECT NULL AS Due_Date,'' AS PO_SRN,  MAX(VCode) AS VCode,MAX(VName) as VName,DocNo AS DocNo,MAX(DocType) AS DocType,MAX(DocDate) AS DocDate,MAX(DocNarr) AS DocNarr,MAX(ChequeDetails) AS ChequeDetails  ,MAX(Currency_Code) AS Currency_Code,MAX(ConvRate) AS ConvRate,SUM(CrAmt) AS CrAmt,SUM(DrAmt) AS DrAmt,SUM(Purchase) AS Purchase,SUM(Payments) AS Payments  ,MAX(DrNote) AS DrNote,MAX(CrNote) AS CrNote, MAX(Document_Type) AS Document_Type,SUM(Balance_Amount) AS Balance_Amount,account , MAX(Posting_Date) AS Posting_Date,MAX(GLDocType) AS GLDocType,MAX(Comp_Code) AS Comp_Code FROM (select  NULL AS Due_Date,'' AS PO_SRN, tspl_payment_header.vendor_code as VCode, tspl_payment_header.vendor_name as VName, TSPL_PAYMENT_HEADER.payment_no as DocNo,CASE WHEN TSPL_PAYMENT_HEADER.Payment_Type ='AD' THEN 'IM' else null end as DocType , COnvert(date,payment_date, 103) as DocDate, ((entry_desc)+(case when entry_desc='' then '' else ' - 'end)+(reference)+(case when reference='' then '' else ' - 'end)+  ( narration)) as DocNarr,((Cheque_No) + (case when Cheque_No='' then '' else ' - ' end) +CONVERT(Varchar, Cheque_Date, 103)) as ChequeDetails, TSPL_PAYMENT_HEADER.CURRENCY_CODE, TSPL_PAYMENT_HEADER.ConvRate, 0  AS CrAmt,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type  in ('D') then TSPL_PAYMENT_DETAIL.Applied_Amount * -1 else TSPL_PAYMENT_DETAIL.Applied_Amount  end AS  DrAmt," + Environment.NewLine & _
    '                    " 0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote,TSPL_PAYMENT_HEADER.Payment_Type as Document_Type,Payment_Amount as Balance_Amount, " + Environment.NewLine & _
    '                    " TSPL_VENDOR_INVOICE_HEAD.Loc_Code as account,TSPL_PAYMENT_HEADER.Payment_Post_Date as Posting_Date, case when TSPL_PAYMENT_HEADER.Payment_Type in ('PY','AV','OA','RC') then 'AP-PY' else case when TSPL_PAYMENT_HEADER.Payment_Type in ('MI') then 'AP-MI' else '' end  end as GLDocType, tspl_payment_header.Comp_Code  " + Environment.NewLine & _
    ...
    '                        strtempBaseQry += " and tspl_payment_header.vendor_code in (" & strvendor & ")"
    '                    End If
    '                    strtempBaseQry += " )INV  GROUP BY  DocNo,account  " + Environment.NewLine & _
    '                    " ------- APPLY DOCUMENT ENTRY WHEN LOCATION IS CHANGED OF APPLIED DOCUMENT WITH DOCUMENT LOCATION " + Environment.NewLine & _
    '                    " UNION ALL " + Environment.NewLine & _
    '                    " SELECT NULL AS Due_Date,'' AS PO_SRN,  MAX(VCode) AS VCode,MAX(VName) as VName,DocNo AS DocNo,MAX(DocType) AS DocType,MAX(DocDate) AS DocDate,MAX(DocNarr) AS DocNarr,MAX(ChequeDetails) AS ChequeDetails  ,MAX(Currency_Code) AS Currency_Code,MAX(ConvRate) AS ConvRate,SUM(CrAmt) AS CrAmt,SUM(DrAmt) AS DrAmt,SUM(Purchase) AS Purchase,SUM(Payments) AS Payments  ,MAX(DrNote) AS DrNote,MAX(CrNote) AS CrNote, MAX(Document_Type) AS Document_Type,SUM(Balance_Amount) AS Balance_Amount,account , MAX(Posting_Date) AS Posting_Date,MAX(GLDocType) AS GLDocType,MAX(Comp_Code) AS Comp_Code FROM ( " + Environment.NewLine & _
    '                    " select  NULL AS Due_Date,'' AS PO_SRN, tspl_payment_header.vendor_code as VCode, tspl_payment_header.vendor_name as VName, TSPL_BANK_REVERSE.Reverse_Code as DocNo,'Reverse Payment' as DocType , Convert(date,TSPL_BANK_REVERSE.Reversal_Date, 103) as DocDate, ((entry_desc)+(case when entry_desc='' then '' else ' - 'end)+(reference)+(case when reference='' then '' else ' - 'end)+  ( narration)) as DocNarr,((TSPL_PAYMENT_HEADER.Cheque_No) + (case when TSPL_PAYMENT_HEADER.Cheque_No='' then '' else ' - ' end) +CONVERT(Varchar, Cheque_Date, 103)) as ChequeDetails, TSPL_PAYMENT_HEADER.CURRENCY_CODE, TSPL_PAYMENT_HEADER.ConvRate, case when TSPL_VENDOR_INVOICE_HEAD.Document_Type  in ('D') then TSPL_PAYMENT_DETAIL.Applied_Amount * -1 else TSPL_PAYMENT_DETAIL.Applied_Amount  end  AS CrAmt,0 AS  DrAmt, " + Environment.NewLine & _
    ...
    '    End Function
    ''richa agarwal 20 Apr, 2017
    Public Function GetVendorBaseQry(ByVal strPortrait As Boolean, ByVal strLandscape As Boolean, ByVal IsOnlyForAgainstSalary As Boolean, ByVal strfromdate As String, ByVal strtodate As String, ByVal strvendor As String, ByVal isOpening As Boolean) As String
        Dim strtempBaseQry As String = String.Empty
        Try
            If Not IsOnlyForAgainstSalary Then
           ...
           " then TSPL_VENDOR_INVOICE_HEAD.TAX10_Amt else 0 end ) else 0 end) "
                ...
                'strtempBaseQry += " case when TSPL_VENDOR_INVOICE_HEAD.Document_Type IN('I','C') Then document_total -(case when TSPL_VENDOR_INVOICE_HEAD.GSTRegistered =0 then TSPL_VENDOR_INVOICE_HEAD.Total_Tax else 0 end) else 0 end + ( case when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX1 ,'')='WCT' THEN TAX1_Amt * -1  when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX2 ,'')='WCT' THEN TAX2_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX3 ,'')='WCT' THEN TAX3_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX4 ,'')='WCT' THEN TAX4_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX5 ,'')='WCT' THEN TAX5_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX6 ,'')='WCT' THEN TAX6_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX7 ,'')='WCT' THEN TAX7_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX8 ,'')='WCT' THEN TAX8_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX9 ,'')='WCT' THEN TAX9_Amt * -1 when isnull(TSPL_VENDOR_INVOICE_HEAD.TAX10 ,'')='WCT' THEN TAX10_Amt * -1 ELSE 0 END) as CrAmt, " & _
                '   "  case when TSPL_VENDOR_INVOICE_HEAD.Document_Type IN('D') then Document_Total -(case when TSPL_VENDOR_INVOICE_HEAD.GSTRegistered =0 then TSPL_VENDOR_INVOICE_HEAD.Total_Tax else 0 end) else 0 end as DrAmt, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='I' Then document_total -(case when TSPL_VENDOR_INVOICE_HEAD.GSTRegistered =0 then TSPL_VENDOR_INVOICE_HEAD.Total_Tax else 0 end) Else 0 End as Purchase, 0 as Payments, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' Then document_total -(case when TSPL_VENDOR_INVOICE_HEAD.GSTRegistered =0 then TSPL_VENDOR_INVOICE_HEAD.Total_Tax else 0 end) Else 0 End as DrNote, Case When TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' Then document_total -(case when TSPL_VENDOR_INVOICE_HEAD.GSTRegistered =0 then TSPL_VENDOR_INVOICE_HEAD.Total_Tax else 0 end) Else 0 End as CrNote, TSPL_VENDOR_INVOICE_HEAD.Document_Type,balance_Amt as Balance_Amount, ( select   GL_Account_Code from TSPL_VENDOR_INVOICE_DETAIL where Detail_Line_No='1' and Document_No=tspl_vendor_invoice_head.Document_No  ) as account, convert(date,tspl_vendor_invoice_head.Posting_Date,103) as Posting_Date ,case when TSPL_VENDOR_INVOICE_HEAD.Document_Type='D' then 'AP-DN' when TSPL_VENDOR_INVOICE_HEAD.Document_Type='C' then 'AP-CN' else 'AP-IN' end as GLDocType, tspl_vendor_invoice_head.Comp_Code from tspl_vendor_invoice_head where ISNULL(tspl_vendor_invoice_head.Posting_Date,'')<>''  and LEN(ISNULL( TSPL_VENDOR_INVOICE_HEAD.Against_POInvoice_No,''))<=0   " + Environment.NewLine
    ...
    Private Sub rptVendorReco_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Sub PrintForVendorCurrency(Optional ByVal BulkExport As Integer = 0)
        ...
        Try
            ...
            BaseQry = GetVendorBaseQryForVendorCurrency(rbPortrait.IsChecked, rbLandScape.IsChecked, chkAgainstSalary.Checked)
            ...
            Else
                strQry += " SUM(OpngBal)+ SUM(CrAmt)-SUM(DrAmt) as [Closing] from ("
            End If
            strQry += " Select VCode, MAX(VName) as VName, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code) as Vendor_Group_Code, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code_Desc) as [Vendor_Group_Desc], MAX(TSPL_VENDOR_ACCOUNT_SET.Acct_Set_Desc) as AccountSet, SUM(CrAmt)-SUM(DrAmt) as OpngBal, 0 as DrAmt, 0 as CrAmt, 0 as Purchase, 0 as Payments, 0 as DrNote, 0 as CrNote From ( " & IIf(clsCommon.myCstr(clsUserMgtCode.MISCreditorReport) = FormType, strtempBaseQryforopeningForMIS, BaseQry) & " ) Final LEFT OUTER JOIN TSPL_VENDOR_MASTER on final.VCode=TSPL_VENDOR_MASTER.Vendor_Code Left Outer Join TSPL_VENDOR_GROUP ON TSPL_VENDOR_MASTER.Vendor_Group_Code=TSPL_VENDOR_GROUP.Ven_Group_Code left join TSPL_VENDOR_ACCOUNT_SET on TSPL_VENDOR_GROUP.Acct_Set_Code=TSPL_VENDOR_ACCOUNT_SET.Acct_set_Code LEFT OUTER JOIN TSPL_COMPANY_MASTER ON final.Comp_Code=TSPL_COMPANY_MASTER.Comp_Code where final.Document_Type in (" + clsCommon.GetMulcallString(arr) + ") and CONVERT(date,DocDate ,103)  <'" + strFromDate + "' and DocType <>'Mislleneous' " + strFIlterCheck + " GROUP BY VCode" + Environment.NewLine & _
           " UNION ALL" + Environment.NewLine & _
           " Select VCode, MAX(VName) as VName, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code) as Vendor_Group_Code, MAX(TSPL_VENDOR_MASTER.Vendor_Group_Code_Desc) as [Vendor_Group_Desc], MAX(TSPL_VENDOR_ACCOUNT_SET.Acct_Set_Desc) as AccountSet, 0 as OpngBal, SUM(DrAmt) as DrAmt, SUM(CrAmt) as CrAmt, SUM(CONVERT(DECIMAL(18,2),Purchase*" & ddlCurrencyType.SelectedValue & ")) as Purchase, SUM(CONVERT(DECIMAL(18,2),Payments*" & IIf(clsCommon.myCstr(clsUserMgtCode.MISCreditorReport) = FormType, "1", ddlCurrencyType.SelectedValue) & ")) as Payments, SUM(DrNote*" & ddlCurrencyType.SelectedValue & ") as DrNote, SUM(CrNote*" & ddlCurrencyType.SelectedValue & ") as CrNote from ( " + BaseQry + " ) Final LEFT OUTER JOIN TSPL_VENDOR_MASTER on final.VCode=TSPL_VENDOR_MASTER.Vendor_Code Left Outer Join TSPL_VENDOR_GROUP ON TSPL_VENDOR_MASTER.Vendor_Group_Code=TSPL_VENDOR_GROUP.Ven_Group_Code left join TSPL_VENDOR_ACCOUNT_SET on TSPL_VENDOR_GROUP.Acct_Set_Code=TSPL_VENDOR_ACCOUNT_SET.Acct_set_Code LEFT OUTER JOIN TSPL_COMPANY_MASTER ON final.Comp_Code=TSPL_COMPANY_MASTER.Comp_Code where final.Document_Type in (" + clsCommon.GetMulcallString(arr) + ") and CONVERT(date,DocDate ,103)  >='" + strFromDate + "' and CONVERT(date,DocDate ,103)  <='" + strToDate + "' and DocType <>'Mislleneous' " + strFIlterCheck + " GROUP BY VCode" + Environment.NewLine & _
           " ) XXX GROUP BY VCode ORDER BY VCode"
            dtCustomer = clsDBFuncationality.GetDataTable(strQry)
            ...
            Else
                ...
                " LEFT OUTER JOIN TSPL_JOURNAL_MASTER ON TSPL_JOURNAL_MASTER.Source_Doc_No=Final.DocNo " & _
                 " where final.Document_Type in (" + clsCommon.GetMulcallString(arr) + ") and CONVERT(date,DocDate ,103)  <'" + strFromDate + "' and DocType <>'Mislleneous' " + strFIlterCheck + " GROUP BY VCode" + Environment.NewLine & _
                " UNION ALL ------------------Main UNION Between [Opening And Data]------------------------------------" + Environment.NewLine & _
                 " Select Due_Date, PO_SRN, vcode, TSPL_VENDOR_MASTER.Vendor_Name as VName, DocNo, DocType, DocDate, DocNarr, ChequeDetails, Final.Currency_Code,Final.ConvRate , DrAmt as DrAmt, CrAmt as CrAmt, Purchase* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as Purchase, Payments* " & IIf(clsCommon.myCstr(clsUserMgtCode.MISCreditorReport) = FormType, "1", IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "")) & " as Payments, DrNote* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as DrNote, CrNote* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as CrNote, Purchase* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as Purchase1, Payments* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as Payments1, DrNote* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as DrNote1, CrNote* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as CrNote1, (CrAmt-DrAmt) as EffectiveAmt, final.Document_Type, Balance_Amount* " & IIf(clsCommon.myCstr(ddlCurrencyType.SelectedValue) = "1", 1, " final." & ddlCurrencyType.SelectedValue & "") & " as Balance_Amount,substring(account,LEN(account)-2,3) + ' - ' + TSPL_GL_SEGMENT_CODE.Description  as account, CONVERT(VARCHAR,final.Posting_Date,103) As Posting_Date,GLDocType, TSPL_VENDOR_MASTER.Vendor_Group_Code, TSPL_VENDOR_GROUP.Group_Desc , Case When DrAmt>0 Then 0 else 1 End as OrderdrCr, Case When tspl_BankReco_Head.Post='Y' AND tspl_BankReco_Detail.Reconciliation_Status='C' Then 'Done' Else 'Pending' End as [Reconciliation], Case When tspl_BankReco_Head.Post='Y' AND tspl_BankReco_Detail.Reconciliation_Status='C' Then CONVERT(VARCHAR,tspl_BankReco_Detail.Reconciliation_Date,103) Else '' End as [Reconciliation_Date] ,TSPL_JOURNAL_MASTER.Voucher_No AS Voucher_No   , TSPL_VENDOR_MASTER.Vendor_Account ,( SELECT top 1 TSPL_JOURNAL_DETAILS.Account_code AS Account_code    FROM TSPL_JOURNAL_DETAILS " & _
                " LEFT OUTER JOIN TSPL_VENDOR_ACCOUNT_SET ON TSPL_VENDOR_ACCOUNT_SET.Acct_Set_Code =TSPL_VENDOR_MASTER.Vendor_Account " & _
                " LEFT OUTER JOIN TSPL_GL_ACCOUNTS GL1 ON ISNULL(GL1.Account_Code,'') = ISNULL(TSPL_JOURNAL_DETAILS.Account_code ,'')  " & _
                " LEFT OUTER JOIN TSPL_GL_ACCOUNTS GL2 ON ISNULL(GL2.Account_Code,'') =  case when Final.DocType ='AP Invoice' then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Payable_Account ,'') when Final.DocType ='Advance' and (Select top 1 Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=1  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Against_Salary  ,'')  when Final.DocType ='Advance' and (Select top 1 Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=0  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Account   ,'') when Final.DocType ='On Account' and (Select top 1  Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=1  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Against_Salary  ,'')  when Final.DocType ='On Account' and (Select top 1  Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=0  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Account   ,'') when Final.DocType='TDS' then (Select Gl_Account from TSPL_TDS_DEDUCTION_HEAD where Deduction_Code= ( Select Deduction_Code from tspl_vendor_master where vendor_code=Final.Vcode)) else ISNULL(TSPL_VENDOR_ACCOUNT_SET.Payable_Account ,'') end  " & _
                " WHERE GL1.Account_Seg_Code1 = GL2.Account_Seg_Code1 AND TSPL_JOURNAL_DETAILS.Voucher_No=TSPL_JOURNAL_MASTER.Voucher_No and TSPL_JOURNAL_DETAILS.Account_code =case when Final.DocType ='AP Invoice' then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Payable_Account ,'') when Final.DocType ='Advance' and (Select top 1 Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=1  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Against_Salary  ,'')  when Final.DocType ='Advance' and (Select top 1 Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=0  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Account   ,'') when Final.DocType ='On Account' and (Select top 1  Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=1  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Against_Salary  ,'')  when Final.DocType ='On Account' and (Select top 1  Advance_Against_Salary  from TSPL_PAYMENT_HEADER where Payment_No=Final.DocNo)=0  then ISNULL(TSPL_VENDOR_ACCOUNT_SET.Advance_Account   ,'') when Final.DocType='TDS' then (Select Gl_Account from TSPL_TDS_DEDUCTION_HEAD where Deduction_Code= ( Select Deduction_Code from tspl_vendor_master where vendor_code=Final.Vcode)) else ISNULL(TSPL_VENDOR_ACCOUNT_SET.Payable_Account ,'') end ) AS JEAccount_code " & _
    ...
    Private Sub ReStoreGridVendor()
        Try
            If clsCommon.myLen(ReportID) > 0 Then
                Dim obj As clsGridLayout = New clsGridLayout()
                obj = CType(obj.GetData(ReportID + "V", "", objCommonVar.CurrentUserCode), clsGridLayout)
    ...
    Private Sub ReStoreGridVendorGrp()
        Try
            If clsCommon.myLen(ReportID) > 0 Then
                Dim obj As clsGridLayout = New clsGridLayout()
                obj = CType(obj.GetData(ReportID + "VG", "", objCommonVar.CurrentUserCode), clsGridLayout)
