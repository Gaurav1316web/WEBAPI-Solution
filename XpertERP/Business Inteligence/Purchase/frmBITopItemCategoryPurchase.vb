Imports common
Public Class frmBITopItemCategoryPurchase
    ...
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.BITopItemCategoryPurchase)
    ...
    Public Function GetQry() As String
        ...
        qry = "select Category, CONVERT(Decimal(18,2), " + strQtyOrAmt + "/" + cmbFigure.SelectedValue + ") As " + strQtyOrAmt + " from(  select top " + clsCommon.myCstr(clsCommon.myCdbl(cboTop.SelectedValue)) + "  item_category,MAX(Category_Name) as Category,sum(" + strQtyOrAmt + ")as " + strQtyOrAmt + " from ( select TSPL_ITEM_MASTER.Item_Code, TSPL_ITEM_MASTER.item_category,TSPL_Item_Category.Category_Name,TSPL_PI_DETAIL.PI_Qty as Qty,TSPL_PI_DETAIL.Amt_Less_Discount as Amt from TSPL_PI_DETAIL  left outer join TSPL_PI_HEAD on TSPL_PI_HEAD.PI_No=TSPL_PI_DETAIL.PI_No left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_PI_DETAIL.Item_Code left outer join TSPL_Item_Category on TSPL_Item_Category.Category_Code=TSPL_ITEM_MASTER.item_category  where TSPL_PI_HEAD.Status=1 and PI_Date>='" + strFromDate + "' and PI_Date<='" + strToDate + "'  )xxx group by item_category )xxxx order by " + strQtyOrAmt + " desc"
