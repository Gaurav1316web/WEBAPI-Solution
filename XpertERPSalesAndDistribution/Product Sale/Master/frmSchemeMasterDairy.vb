...
#End Region
    ...
    Sub LoadBlankItemGrid()
        ...
        repoMainICode.HeaderImage = XpertERPSalesAndDistribution.My.Resources.Resources.search4
    ...
    Function AllowToSave() As Boolean
        ...
        For ii As Integer = 0 To gvItem.Rows.Count - 1
            ...
            If clsCommon.myCBool(gvItem.Rows(ii).Cells(colISelect).Value) = True Then
                ...
                'Check UOM exist in master
                If clsCommon.myLen(strMainICode) > 0 AndAlso clsCommon.myLen(strMainUnit) > 0 Then
                    Dim UOMEXIST As Integer = 0
                    UOMEXIST = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(1) as xx from TSPL_ITEM_UOM_DETAIL where Item_Code = '" + strMainICode + "' and UOM_Code = '" + strMainUnit + "'"))
                    If UOMEXIST = 0 Then
                        strMsg = strMsg & strMainICode & " - " & strMainUnit & Environment.NewLine
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.myLen(strUnit) > 0 Then
                    Dim SCHEMEUOMEXIST As Integer = 0
                    SCHEMEUOMEXIST = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(1) as xx from TSPL_ITEM_UOM_DETAIL where Item_Code = '" + strICode + "' and UOM_Code = '" + strUnit + "'"))
                    If SCHEMEUOMEXIST = 0 Then
                        strMsg = strMsg & strICode & " - " & strUnit & Environment.NewLine
                    End If
                End If
        ...
        Try
            ...
            If (AllowToSave()) Then
                If clsCommon.myLen(strMsg) > 0 Then
                    strMsg = "UOM does not exist -" & strMsg
                ...
                obj.Quantum = CInt(clsCommon.myCdbl(txtQuantum.Value))
    ...
    Private Sub export_main_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles export_main.Click
        Try
            ...
            End If
            ListImpExpColumnsMandatory = New List(Of String)({"Scheme Code", "Inactive", "InActive Date", "Item Group"})
    ...
    Private Sub export_beneficial_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles export_beneficial.Click
        Try
            ...
            End If
            ListImpExpColumnsMandatory = New List(Of String)({"Scheme Code", "Main Item Code", "Main Unit Code", "Scheme Item", "Unit Code"})
    ...
    Private Sub Export_Criteria_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Export_Criteria.Click
        ...
        Try
            ...
            End If
            ListImpExpColumnsMandatory = New List(Of String)({"Scheme Code", "Customer Code", "Criteria"})
    ...
    Private Sub rmWholeSheetExport_Click(sender As Object, e As EventArgs) Handles rmWholeSheetExport.Click
        Try
            ...
            ListImpExpColumnsMandatory = New List(Of String)({"Scheme_Code", "Item_Group", "Cust_Code", "MainItem_Code", "Unit_Code"})
    ...
    Private Sub rmSchemeWithSlabExport_Click(sender As Object, e As EventArgs) Handles rmSchemeWithSlabExport.Click
        ...
        End If
        Try
            Dim qry As String = " select tspl_scheme_master_New.Scheme_Code,tspl_scheme_master_New.Scheme_Desc,TSPL_SCHEME_BENEFICIARY.Cust_Code,TSPL_CUSTOMER_MASTER.Customer_Name,tspl_scheme_master_New.Item_Code as Item_Group,tspl_scheme_detail_new.MainItem_Code,tspl_item_master.Item_Desc,tspl_scheme_detail_new.MainUnit_Code as MainUnit_Code,tspl_scheme_detail_new.MainQty,tspl_scheme_master_new.Scheme_Type,tspl_scheme_detail_new.Item_Code,tspl_scheme_detail_new.Unit_Code,tspl_scheme_detail_new.Qty,CONVERT(varchar, tspl_scheme_master_New.MaxlimitStart_Date,103) as Start_Date,CONVERT(varchar, tspl_scheme_master_New.MaxlimitEnd_Date,103) as End_Date " &
            ",TSPL_SCHEME_MASTER_NEW_SLAB.Min_Range " &
            ",TSPL_SCHEME_MASTER_NEW_SLAB.Value " &
           " from tspl_scheme_master_New " &
            " left outer join TSPL_SCHEME_BENEFICIARY on TSPL_SCHEME_BENEFICIARY.Scheme_Code=tspl_scheme_master_New.Scheme_Code  " &
            " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SCHEME_BENEFICIARY.Cust_Code  " &
            " left outer join tspl_scheme_detail_new on tspl_scheme_detail_new.Scheme_Code=tspl_scheme_master_New.Scheme_Code  " &
            " left outer join tspl_item_master on tspl_item_master.Item_Code=tspl_scheme_detail_new.MainItem_Code  " &
            ...
            ListImpExpColumnsMandatory = New List(Of String)({"Scheme_Code", "Item_Group", "Cust_Code", "MainItem_Code", "MainUnit_Code", "Item_Code", "Unit_Code", "Start_Date", "End_Date", "Min_Range", "Value"})
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(fndScheme.Value) <= 0 Then
                clsCommon.MyMessageBoxShow("Select Document No")
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowHistoryData(fndScheme.Value, "Scheme_Code", "TSPL_SCHEME_MASTER_NEW", "TSPL_SCHEME_DETAIL_NEW")
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    End Sub
