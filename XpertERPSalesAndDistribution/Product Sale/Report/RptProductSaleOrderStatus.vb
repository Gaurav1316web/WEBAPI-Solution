...
'========shivani Tyagi=========='
Public Class RptProductSaleOrderStatus
    Inherits FrmMainTranScreen
    Dim isInsideLoadData As Boolean = False
#Region "Functions"
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.RptProductSaleOrderStatus)
...
#End Region
    ...
    Sub Print(ByVal IsPrint As Exporter)
        Try
            ...
            End If
            Dim Qry As String = "select case when Sale .Sale_Status =0 then 'Pending' when Sale.Balance_Qty <> 0 then 'Partial' when Sale.Balance_Qty = 0 then 'Complete' end as Status1,* from(select TSPL_CUSTOMER_MASTER.Customer_Name ,    TSPL_SD_SALES_ORDER_HEAD.Customer_Code as Cust_Code ,TSPL_CUSTOMER_MASTER.cust_Group_Code,Cust_Group_Desc,   TSPL_SD_SALES_ORDER_HEAD.Bill_To_Location as Location_Code ,   TSPL_LOCATION_MASTER.Location_Desc ,    TSPL_SD_SALES_ORDER_HEAD.Document_Code As Sale_OrderNo,convert(varchar,TSPL_SD_SALES_ORDER_HEAD.Document_Date,103) as SaleOrder_Date  ,  TSPL_SD_SALES_ORDER_DETAIL.Item_Code As SO_Item_Code,    TSPL_ITEM_MASTER_SO.Item_Desc As SO_Item_Desc,    IsNull(TSPL_SD_SALES_ORDER_DETAIL.Qty, 0) As Sale_qty,    TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Ref_No   ,    TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Code As Delivery_No,    TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Code As DO_Item_Code,    TSPL_ITEM_MASTER_SO.Item_Desc As DO_Item_Desc, Convert(varchar,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Date,103) As    Delivery_Date,    IsNull(TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Qty,    0) As Delivery_qty,    IsNull(TSPL_SD_SALES_ORDER_DETAIL.Qty, 0) -    IsNull(TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Qty, 0) As  Balance_Qty,TSPL_SD_SALES_ORDER_HEAD.Status as Sale_Status,TSPL_SD_SALES_ORDER_HEAD.Discount_Amt ,TSPL_SD_SALES_ORDER_HEAD.Amount_Less_Discount ,TSPL_SD_SALES_ORDER_HEAD.Total_Tax_Amt ,TSPL_SD_SALES_ORDER_HEAD.Total_Amt"
            Qry += " from TSPL_SD_SALES_ORDER_DETAIL left join TSPL_SD_SALES_ORDER_HEAD on TSPL_SD_SALES_ORDER_HEAD.Document_Code = TSPL_SD_SALES_ORDER_DETAIL.Document_Code left join TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE on TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Ref_No = TSPL_SD_SALES_ORDER_HEAD.Document_Code left join TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE on TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE .DOCUMENT_CODE= TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.DOCUMENT_CODE and TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Code=TSPL_SD_SALES_ORDER_DETAIL.Item_Code  And      TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Scheme_Item = 'N' Left Outer Join TSPL_CUSTOMER_MASTER On TSPL_CUSTOMER_MASTER.Cust_Code =      TSPL_SD_SALES_ORDER_HEAD.Customer_Code left join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code =TSPL_CUSTOMER_MASTER.Cust_Group_Code Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_SD_SALES_ORDER_HEAD.Bill_To_Location Left Outer Join TSPL_ITEM_MASTER As TSPL_ITEM_MASTER_SO      On TSPL_ITEM_MASTER_SO.Item_Code = TSPL_SD_SALES_ORDER_DETAIL.Item_Code Left Outer Join TSPL_ITEM_MASTER As TSPL_ITEM_MASTER_DO      On TSPL_ITEM_MASTER_DO.Item_Code =      TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Code) Sale Where 2=2 "

            Qry += " and convert(date,Sale.SaleOrder_Date,103)>=convert(date,'" + fromDate.Value + "',103) and convert(date,Sale.SaleOrder_Date,103) <=convert(date,'" + ToDate.Value + "' ,103)"
            If RbPending.IsChecked = True Then
                Qry += " and Sale .Sale_Status =0"
            ElseIf chkPartial.IsChecked = True Then
                Qry += " and Sale.Balance_Qty <> 0"
            ElseIf chkComplete.IsChecked = True Then
                Qry += " and Sale.Balance_Qty = 0"
            End If
            If rbtnLocationSelect.IsChecked Then
                Qry += " and  Sale.Location_Code in (" + clsCommon.GetMulcallString(cbgLocation.CheckedValue) + ") "
            End If
            If rbtnCustomerSelect.IsChecked Then
                Qry += " and Sale.Cust_Code in (" + clsCommon.GetMulcallString(cbgCustomer.CheckedValue) + ") "
            End If
            Qry += " order by convert(date,SaleOrder_Date,103)"
            ...
            If IsPrint = Exporter.Excel Then
                clsCommon.MyExportToExcelGrid("Sale Order Status", Gv1, arrHeader, Me.Text)
            ElseIf IsPrint = Exporter.PDF Then
                clsCommon.MyExportToPDF("Sale Order Status", Gv1, arrHeader, "Sale Order Status", True)
            End If
    ...
    Sub SetGridFormationOFGV1()
        ...
        Gv1.Columns("SaleOrder_Date").IsVisible = True
        Gv1.Columns("SaleOrder_Date").Width = 100
        Gv1.Columns("SaleOrder_Date").HeaderText = "Sale Order Date"
        ...
        Gv1.Columns("Sale_qty").IsVisible = True
        Gv1.Columns("Sale_qty").Width = 150
        Gv1.Columns("Sale_qty").HeaderText = "Sale Order Qty"
        ...
        Gv1.Columns("Delivery_No").Width = 100
        ...
        Gv1.Columns("Delivery_qty").Width = 120
        ...
        Dim item6 As New GridViewSummaryItem("Delivery_qty", "{0:F2}", GridAggregateFunction.Sum)
        summaryRowItem.Add(item6)
        Dim item7 As New GridViewSummaryItem("Sale_qty", "{0:F2}", GridAggregateFunction.Sum)
    ...
    Private Sub RptProductSaleOrderStatus_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        Reset()
    End Sub
    ...
    Private Sub ExportGrid(ByVal exporter As EnumExportTo)
        Try
            ...
            arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptProductSaleOrderStatus & "'"))
            If rbtnCustomerSelect.IsChecked Then
                Dim strLocationName As String = ""
            ...
            Else
                transportSql.applyExportTemplate(Gv1, PageSetupReport_ID)
                clsCommon.MyExportToPDF("Sale Order Status", Gv1, arrHeader, "Sale Order Status", PageSetupReport_ID, objCommonVar.CurrentUserCode)
            End If
        Catch ex As Exception
