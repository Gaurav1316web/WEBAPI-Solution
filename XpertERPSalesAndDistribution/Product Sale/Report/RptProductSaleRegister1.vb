...
'========shivani Tyagi=========='
Public Class RptProductSaleRegister1
    Inherits FrmMainTranScreen
    Dim isInsideLoadData As Boolean = False
    Dim atchqry As String = ""
#Region "Functions"
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.RptProductSaleRegister1)
...
#End Region
    ...
    Private Sub RptProductSaleRegister1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        Reset()
        rbtnSend.Visibility = ElementVisibility.Collapsed
    End Sub
    Sub LoadCategory()
    ...
    Sub Print(ByVal IsPrint As Exporter)
        Try
            ...
            strSaleInvoice = "SELECT TSPL_SD_SALE_INVOICE_DETAIL.Conv_Factor as OldConvF,OrgRate,'" & fromDate.Value & "' as Fdate,'" & ToDate.Value & "' as Tdate, case when TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='PS' then 'Sale Invoice' end as Type, TSPL_SD_SALE_INVOICE_HEAD.Document_Code, " & _
                        ...
                        " where TSPL_SD_SALE_INVOICE_HEAD.Trans_Type ='PS' and " & strInvoicePosted & " TSPL_SD_SALE_INVOICE_HEAD.Document_Date  >= '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(fromDate.Value), "dd/MMM/yyyy hh:mm tt") & "' and " & _
            ...
            If rbtnCustomerGroupSelect.IsChecked Then
                strSaleInvoice += " and TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code in (" + clsCommon.GetMulcallString(cbgCustomerGroup.CheckedValue) + ") "
            End If
            ...
            strSaleReturn = "SELECT    TSPL_SD_SALE_RETURN_DETAIL.Conv_Factor as OldConvF,OrgRate,'" & fromDate.Value & "' as Fdate,'" & ToDate.Value & "' as Tdate, 'Sale Return' as Type, TSPL_SD_SALE_RETURN_HEAD.Document_Code, " & _
                          ...
                          " (case when TSPL_SD_SALE_RETURN_HEAD.ConvRate=0 then 1 else TSPL_SD_SALE_RETURN_HEAD.ConvRate end)*TSPL_SD_SALE_RETURN_DETAIL.Item_Net_Amt  * -1 as Total_BaseCurr,FOC_Item,TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code ,Cust_Group_Desc FROM  " & _
                          ...
                          " left OUTER JOIN TSPL_LOCATION_MASTER ON TSPL_LOCATION_MASTER .Location_Code  = TSPL_SD_SALE_RETURN_HEAD.Bill_To_Location left outer join tspl_state_master on tspl_state_master.state_code=tspl_customer_master.state left outer join tspl_city_master on tspl_city_master.city_code=tspl_customer_master.city_code left outer join TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code =TSPL_SD_SALE_Return_HEAD.Ship_To_Location left join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code =TSPL_CUSTOMER_MASTER.Cust_Group_Code " & _
                          "  where TSPL_SD_SALE_RETURN_HEAD.Trans_Type ='PS' and " & strReturnPosted & "  TSPL_SD_SALE_RETURN_HEAD.Document_Date  >= '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(fromDate.Value), "dd/MMM/yyyy hh:mm tt") & "' and  " & _
            ...
            End If
            If rbtnCustomerGroupSelect.IsChecked Then
                strSaleReturn += " and TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code in (" + clsCommon.GetMulcallString(cbgCustomerGroup.CheckedValue) + ") "
            End If
        ...
        If rdbSummary.IsChecked Then
            If Gv1.Rows.Count > 0 Then
                ...
                Select Case strTransType
                    Case "SD-IN"
                        clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmSaleInvoiceProductSale, strDoc)
                    Case "Product_Sale"
                        clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmSaleInvoiceProductSale, strDoc)
                    Case "Sale Return"
    ...
    Private Sub rbtnSetting_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtnSetting.Click
        Dim frm As New FrmMailSMSSettingNew2()
        frm.FormId = clsUserMgtCode.RptProductSaleRegister1
        frm.ShowDialog()
    End Sub


    Private Sub rbtnSend_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbtnSend.Click
        'Try
        ...
        '    Dim obj As clsEmailSMSSettingNew = clsEmailSMSSettingNew.GetData(clsUserMgtCode.RptProductSaleRegister1)
    ...
    Private Sub btnQuickExport_Click(sender As Object, e As EventArgs) Handles btnQuickExport.Click
        Try
            ...
            arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptProductSaleRegister1 & "'"))
            If rbtnLocationSelect.IsChecked Then
                Dim strLocationName As String = ""
                For Each StrName As String In cbgLocation.CheckedDisplayMember
                    If clsCommon.myLen(strLocationName) > 0 Then
    ...
    Private Sub rdbDetail_ToggleStateChanged(sender As Object, args As StateChangedEventArgs) Handles rdbDetail.ToggleStateChanged
        If rdbDetail.IsChecked Then
            If chkSerializeInv.Checked = False Then
    ...
    Private Sub chkSerializeInv_ToggleStateChanged(sender As Object, args As StateChangedEventArgs) Handles chkSerializeInv.ToggleStateChanged
        If chkSerializeInv.Checked Then
            RadButton1.Enabled = False
        Else
            If rdbDetail.IsChecked Then
