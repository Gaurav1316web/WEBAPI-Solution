...
#End Region
    ...
    Private Sub Load_Sales_Report(ByVal PCGroup As Boolean)
        ...
        Try
            ...
            ElseIf rdbSaleTransfer.IsChecked = True Then
            ...
            If rbnPricegroup.Checked Then
                Itemqry = " Select price_code,price_code + 'Bag' as price_code_Bag from TSPL_PRICE_COMPONENT_MAPPING union select  'Other' as price_code,'OtherBag' as price_code_Bag "
            ElseIf rbnCustgroup.Checked Then
                Itemqry = " Select cust_group_code as price_code,cust_group_code +'Bag' as price_code_Bag from tspl_Customer_Group_Master union select 'Other' as price_code,'OtherBag' as price_code_Bag "
            End If
            ...
            If dtitemName.Rows.Count > 0 Then
                For i As Integer = 0 To dtitemName.Rows.Count - 1
                    If i = 0 Then
                        ...
                        itemPivBagName += " [" + clsCommon.myCstr(dtitemName.Rows(i)("price_code_Bag")) + "] "
                        itemSumBagName += " ISNULL([" + clsCommon.myCstr(dtitemName.Rows(i)("price_code_Bag")) + "],0)"
                    Else
                        itemNames1 += ", [" + clsCommon.myCstr(dtitemName.Rows(i)("price_code")) + "] "
                        itemPivBagName += ", [" + clsCommon.myCstr(dtitemName.Rows(i)("price_code_Bag")) + "] "
                        itemSumBagName += "+" + "ISNULL([" + clsCommon.myCstr(dtitemName.Rows(i)("price_code_Bag")) + "],0)"
            ...
            If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 AndAlso rdbDispatch.IsChecked = True Then
                whr += " and TSPL_SD_SHIPMENT_DETAIL.Location In  (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
            ElseIf txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 AndAlso rdbInvoice.IsChecked = True Then
                whr += " and TSPL_SD_SALE_INVOICE_DETAIL.Location In  (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
            ElseIf txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 AndAlso rdbSaleTransfer.IsChecked = True Then
                whr += " and TSPL_SD_SALE_RETURN_DETAIL.Location In  (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
            Else
            ...
            If rbnPricegroup.Checked AndAlso rdbDispatch.IsChecked = True AndAlso rdbSaleTransfer.IsChecked = True Then
                ...
                qry += " Select Location,max(Location_Desc)Location_Desc,max(Add1)Add1,max(Add4)Add4,Document_Date,Sum(SaleQty-ReturnQty)Quantity,Sum(SaleQtyBag-ReturnQtyBag)QuantityBag,price_CodeNon,case when isnull(price_CodeNon,'')= '' then 'OtherBag' else price_CodeNon + 'Bag' end as price_CodeNonBag
					  ...
					  0 as ReturnQty,0 as ReturnQtyBag, TSPL_SD_SHIPMENT_HEAD.price_Code as price_CodeNon							   
                ...
                qry += " group by convert (date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103),TSPL_SD_SHIPMENT_HEAD.price_Code,Location
                ...
                If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_SCRAPSALE_HEAD.Loc_Code In (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
                End If
                ...
                qry += "  UNION ALL
                                    ...
                                    TSPL_SD_SALE_RETURN_HEAD.price_Code as price_CodeNon FROM TSPL_SD_SALE_RETURN_DETAIL
                ...
                qry += "	 group by convert (date,TSPL_SD_SALE_RETURN_HEAD.Document_Date,103),TSPL_SD_SALE_RETURN_HEAD.price_Code,Location)XX group by xx.Location,xx.Document_Date,
                             xx.price_CodeNon )Tab1 "
                If Not PCGroup Then
                    qry += "    PIVOT (SUM(Quantity) FOR price_CodeNon IN (" & itemNames1 & "))AS Tab2   PIVOT (SUM(QuantityBag) FOR price_CodeNonBag IN (" & itemPivBagName & " ))AS PivBag )tmp  "
                End If
            ...
            ElseIf rbnPricegroup.Checked AndAlso rdbInvoice.IsChecked = True AndAlso rdbSaleTransfer.IsChecked = True Then
                ...
                qry += " Select Location,max(Location_Desc)Location_Desc,max(Add1)Add1,max(Add4)Add4,Document_Date,Sum(SaleQty-ReturnQty)Quantity,Sum(SaleQtyBag-ReturnQtyBag)QuantityBag,price_CodeNon ,case when isnull(price_CodeNon,'')= '' then 'OtherBag' else price_CodeNon + 'Bag' end as price_CodeNonBag
					  ...
					  ,0 as ReturnQty,0 as ReturnQtyBag,TSPL_SD_SALE_INVOICE_HEAD.price_Code as price_CodeNon		   
                ...
                If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 Then
                    qry += " and TSPL_SD_SALE_INVOICE_DETAIL.Location In  (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
                End If
                qry += " " + StatusInvoice + " " + FG + " " + SFG + " " + FGSFG + " "

                qry += " group by convert (date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103),TSPL_SD_SALE_INVOICE_HEAD.price_Code,Location
                ...
                End If
                qry += " " + StatusReturn + " " + FG + " " + SFG + " " + FGSFG + " "
                qry += "	 group by convert (date,TSPL_SD_SALE_RETURN_HEAD.Document_Date,103),TSPL_SD_SALE_RETURN_HEAD.price_Code,Location
            ...
            ElseIf rbnPricegroup.Checked AndAlso rdbDispatch.IsChecked = True Then
                If Not PCGroup Then
                    qry = "   Select 'RAJASTHAN CO-OPERATIVE DAIRY FEDERATION LIMITED' as HeadName,FORMAT(CONVERT(date, Document_Date, 103), 'yyyy-MM') AS MonthNumber,max(Location)Location,'" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") + "' As FromDate,
  ...
  FROM ("
                ...
                qry += " SELECT XX.Location,MAX(XX.Location_Desc)Location_Desc,max(xx.Add1)Add1,max(xx.Add4)Add4,(xx.Document_Date)Document_Date,
                                   Cast(sum(xx.Quantity) as Decimal(10,2))Quantity,Cast(Sum(xx.QuantityBag) as Decimal(10,2))QuantityBag,
                                    case when isnull(price_CodeNon,'')= '' then 'Other' else price_CodeNon end as price_CodeNon,case when isnull(price_CodeNon,'')= '' then 'OtherBag' else price_CodeNon + 'Bag' end as price_CodeNonBag FROM (SELECT TSPL_SD_SHIPMENT_DETAIL.Location,max(TSPL_LOCATION_MASTER.Location_Desc)Location_Desc,max(TSPL_LOCATION_MASTER.Add1)Add1,max(TSPL_LOCATION_MASTER.Add4)Add4,
                                    ...
                                    Sum(TSPL_ITEM_UOM_DETAIL.Conversion_Factor*TSPL_SD_SHIPMENT_DETAIL.Qty/TSPL_ITEM_UOM_BAG.Conversion_Factor) as QuantityBag, TSPL_SD_SHIPMENT_HEAD.price_Code as price_CodeNon 
                                    FROM TSPL_SD_SHIPMENT_DETAIL
								     left outer join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.Document_Code =TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE
                                     left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SHIPMENT_HEAD.Customer_Code
                                     LEFT OUTER JOIN TSPL_PRICE_COMPONENT_MAPPING ON TSPL_PRICE_COMPONENT_MAPPING.Price_Code=TSPL_CUSTOMER_MASTER.price_CodeNon
                ...
                ElseIf rdbSaleTransfer.IsChecked Then
                ...
                If Not PCGroup Then
                    qry += "   PIVOT (SUM(Quantity) FOR price_CodeNon IN (" & itemNames1 & "))AS Tab2 PIVOT (SUM(QuantityBag) FOR price_CodeNonBag IN (" & itemPivBagName & " ))AS PivBag )tmp group by Document_Date ,Location order by MonthNumber,Document_Date    "
                End If
  ...
  FROM ("
                ...
                qry += " SELECT XX.Location,MAX(XX.Location_Desc)Location_Desc,max(xx.Add1)Add1,max(xx.Add4)Add4,(xx.Document_Date)Document_Date,
                                   Cast(sum(xx.Quantity) as Decimal(10,2))Quantity,Cast(Sum(xx.QuantityBag) as Decimal(10,2))QuantityBag,
                                    case when isnull(xx.price_CodeNon,'')= '' then 'Other' else xx.price_CodeNon end as price_CodeNon,case when isnull(price_CodeNon,'')= '' then 'OtherBag' else price_CodeNon + 'Bag' end as price_CodeNonBag FROM (SELECT TSPL_SD_SALE_INVOICE_DETAIL.Location,max(TSPL_LOCATION_MASTER.Location_Desc)Location_Desc,max(TSPL_LOCATION_MASTER.Add1)Add1,max(TSPL_LOCATION_MASTER.Add4)Add4,
                                   ...
                                   sum(TSPL_ITEM_UOM_DETAIL.Conversion_Factor*TSPL_SD_SALE_INVOICE_DETAIL.Qty/TSPL_ITEM_UOM_QTL.Conversion_Factor) as Quantity,
                                    Sum(TSPL_ITEM_UOM_DETAIL.Conversion_Factor*TSPL_SD_SALE_INVOICE_DETAIL.Qty/TSPL_ITEM_UOM_BAG.Conversion_Factor) as QuantityBag,TSPL_SD_SALE_INVOICE_HEAD.price_Code as price_CodeNon 
                                   FROM TSPL_SD_SALE_INVOICE_DETAIL
								     left outer join TSPL_SD_SALE_INVOICE_HEAD on TSPL_SD_SALE_INVOICE_HEAD.Document_Code =TSPL_SD_SALE_INVOICE_DETAIL.DOCUMENT_CODE
                                     left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALE_INVOICE_HEAD.Customer_Code
                                     LEFT OUTER JOIN TSPL_PRICE_COMPONENT_MAPPING ON TSPL_PRICE_COMPONENT_MAPPING.Price_Code=TSPL_CUSTOMER_MASTER.price_CodeNon
            ...
            ElseIf rbnPricegroup.Checked AndAlso rdbSaleReturn.IsChecked = True Then
                ...
                End If
                    ...
                    qry += " SELECT XX.Location,MAX(XX.Location_Desc)Location_Desc,max(xx.Add1)Add1,max(xx.Add4)Add4,(xx.Document_Date)Document_Date,
                                   Cast(sum(xx.Quantity) as Decimal(10,2))Quantity,Cast(Sum(xx.QuantityBag) as Decimal(10,2))QuantityBag,
                                    case when isnull(xx.price_CodeNon,'')= '' then 'Other' else xx.price_CodeNon end as price_CodeNon,case when isnull(price_CodeNon,'')= '' then 'OtherBag' else price_CodeNon + 'Bag' end as price_CodeNonBag FROM 
                                   (SELECT TSPL_SD_SALE_RETURN_DETAIL.Location,MAX(TSPL_LOCATION_MASTER.Location_Desc)Location_Desc,max(TSPL_LOCATION_MASTER.Add1)Add1,
									...
									TSPL_SD_SALE_RETURN_HEAD.price_Code as price_CodeNon								   
...
SELECT TSPL_SCRAPSALE_HEAD_RETURN.Loc_Code,max(TSPL_LOCATION_MASTER.Location_Desc)Location_Desc,max(TSPL_LOCATION_MASTER.Add1)Add1,
                ...
                If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 Then
                    qry += " AND TSPL_SCRAPSALE_HEAD_RETURN.Loc_Code In  (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
                End If
...
PIVOT (SUM(QuantityBag) FOR Cust_Group_CodeBag IN (" & itemPivBagName & ")) as PivBag
                ...
                ElseIf rbnCustgroup.Checked AndAlso rdbDispatch.IsChecked = True Then
                    If Not PCGroup Then
                        qry = "Select 'RAJASTHAN CO-OPERATIVE DAIRY FEDERATION LIMITED' as HeadName,FORMAT(CONVERT(date, Document_Date, 103), 'yyyy-MM') AS MonthNumber,
...
group by Document_Date ,Location order by MonthNumber,Document_Date   "
                    End If
...
,sum(case when price_CodeNon='GOVTCR' then Quantity else 0 end) as GOVTCRQty,sum(case when price_CodeNon='KVSS' then Quantity else 0 end) as KVSSQty,sum(case when price_CodeNon  in ('MILKUNION','GOSHALA','DCS','GOVTCR','KVSS') then 0 else Quantity end) as OtherQty,sum(Quantity) as [Total Sale],sum(QuantityBag) as [Total BagSale]
 FROM  ( " & Environment.NewLine & qry & " group by Location,Document_Date  )xxxx 	left join tspl_company_master on 1=1 order by Document_Date "
                    Else
...
,sum(case when Cust_Group_Code='GOV' then Quantity else 0 end) as GOVQty,sum(case when Cust_Group_Code='KVSS' then Quantity else 0 end) as KVSSQty,sum(case when Cust_Group_Code='AGENCY' then Quantity else 0 end) as AGENCYQty,sum(case when Cust_Group_Code='RETAIL' then Quantity else 0 end) as RETAILQty,sum(case when Cust_Group_Code='CFP' then Quantity else 0 end) as CFPQty,sum(case when Cust_Group_Code  in ('UNION','DEALER','GOSHAL','DCS','GOV','KVSS','AGENCY','RETAIL','CFP') then 0 else Quantity end) as OtherQty,sum(Quantity) as [Total Sale],sum(QuantityBag) as [Total BagSale]
 FROM (   " & Environment.NewLine & qry & " group by Location,Document_Date  )xxxx 	left join tspl_company_master on 1=1 order by Document_Date "
    ...
    Sub FormatGrid()
        ...
        If clsCommon.myLen(Gv1.Columns("MonthNumber")) > 0 Then
    ...
    Private Sub rmiExcel_Click(sender As Object, e As EventArgs) Handles rmiExcel.Click
        Try
            If Gv1.Rows.Count > 0 Then
                ...
                If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 Then
                    arrHeader.Add(" Location : " + clsCommon.GetMulcallString(txtLocation.arrValueMember))
                End If
    ...
    Private Sub rmiPDF_Click(sender As Object, e As EventArgs) Handles rmiPDF.Click
        Try
            ...
            If Gv1.Rows.Count > 0 Then
                ...
                End If
                '
                If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 > 0 Then
                    arrHeader.Add("Location : " + clsCommon.GetMulcallString(txtLocation.arrValueMember))
                End If
                arrHeader.Add(("Date Range: " + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") + " To " + clsCommon.GetPrintDate(txtToDate.Value, "dd/MM/yyyy")) + " ")
    ...
    Private Sub rptSalesReport_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ' txtLocation.arrValueMember = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Default_Location from TSPL_USER_MASTER where User_Code='" + objCommonVar.CurrentUserCode + "' "))
        rbnPricegroup.Checked = True
        rbnCustgroup.Checked = False
    End Sub
    ...
    Private Sub txtLocation__My_Click(sender As Object, e As EventArgs) Handles txtLocation._My_Click
        Try
            ...
            End If
            txtLocation.arrValueMember = clsCommon.ShowMultipleSelectForm("LocSalesMul", qry, "Code", "Name", txtLocation.arrValueMember, txtLocation.arrDispalyMember)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    End Sub
