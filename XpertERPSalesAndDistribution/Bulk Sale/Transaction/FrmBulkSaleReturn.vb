...
"from TSPL_Dispatch_BulkSale left Outer Join TSPL_Dispatch_Detail_BulkSale On TSPL_Dispatch_BulkSale .Document_No =TSPL_Dispatch_Detail_BulkSale.Document_No  " &
    ...
    Sub PrintData(ByVal StrCode As String, ByVal AgainstInv As Boolean)
        ...
        If clsCommon.myLen(txtDocNo.Value) > 0 OrElse clsCommon.myLen(StrCode) > 0 Then
            ...
            End If
            Qry += " ,Customer_Name ,Item_Desc,TSPL_SALE_RETURN_DETAIL_BULKSALE.Unit_code ,Total_Amt ," & strfield & "  TSPL_LOCATION_MASTER.add1 +case when len(TSPL_LOCATION_MASTER.add2)>0 then ', '+TSPL_LOCATION_MASTER.add2 else '' end +case when LEN(isnull(TSPL_LOCATION_MASTER.Add3,''))>0 then ', '+isnull(TSPL_LOCATION_MASTER.Add3,'') else ' ' end " &
            " + case when len(TSPL_LOCATION_MASTER.Pin_Code   )>0 then ', Pin Code - '+ cast(TSPL_LOCATION_MASTER.Pin_Code as varchar)  else ' ' end" &
            " + case when len(TSPL_LOCATION_MASTER.Email    )>0 then TSPL_LOCATION_MASTER.Email else '' end +Case when len(ISNULL(TSPL_LOCATION_MASTER.Phone1,''))>0 and TSPL_LOCATION_MASTER.Phone1='(+__)__________' then '' else ',Phone'+TSPL_LOCATION_MASTER.Phone1 end " &
            " +  Case When   ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'(+__)__________' Then ',  '+ TSPL_LOCATION_MASTER.Phone2 Else'' End  " &
            " as lOC_Address,Comp_Name ,RoundOffAmount    from TSPL_SALE_RETURN_DETAIL_BULKSALE left join TSPL_SALE_RETURN_MASTER_BULKSALE on TSPL_SALE_RETURN_MASTER_BULKSALE.Document_No =TSPL_SALE_RETURN_DETAIL_BULKSALE.Document_No left join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code = TSPL_SALE_RETURN_MASTER_BULKSALE .customer_code left join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code =TSPL_SALE_RETURN_DETAIL_BULKSALE.Item_Code  left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER .Location_Code=  TSPL_SALE_RETURN_MASTER_BULKSALE.Location_Code    left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_SALE_RETURN_MASTER_BULKSALE.comp_code" &
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtDocNo.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowTransHistoryData(txtDocNo.Value, "Document_No", "TSPL_SALE_RETURN_MASTER_BULKSALE", "TSPL_SALE_RETURN_DETAIL_BULKSALE")
        Catch ex As Exception
