...
Imports common
Public Class frmPendingDispatchTrade
...
#Region "Variables"
    ...
    Public ArrReturn As List(Of clsDispatchDetailTradeBulkSale) = Nothing
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim qry As String = "select CAST(0 as bit) as Sel,code,ICode,max(IName) as IName,max(Location) as Location,MAX(TSPL_LOCATION_MASTER.Location_Desc) as LocationName," & _
                            ...
                            "  Select TSPL_Dispatch_Detail_BulkSale_Trade.Document_No as Code,TSPL_Dispatch_BulkSale_Trade.Customer_Code as Customer ,TSPL_Dispatch_Detail_BulkSale_Trade.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName,TSPL_Dispatch_Detail_BulkSale_Trade.Qty as Qty,0 as Unapproved,TSPL_Dispatch_BulkSale_Trade.Location_Code As Location,1 as RI,TSPL_Dispatch_Detail_BulkSale_Trade.FatPer ,TSPL_Dispatch_Detail_BulkSale_Trade.SNFPer,TSPL_Dispatch_Detail_BulkSale_Trade.Rate  as Rate ,TSPL_Dispatch_Detail_BulkSale_Trade.Amount,TSPL_Dispatch_BulkSale_Trade.Document_Date ,TSPL_Dispatch_Detail_BulkSale_Trade.Unit_Code,TSPL_BulkSalePrice_MASTER.Fat_Weightage ,TSPL_BulkSalePrice_MASTER.Snf_Weightage,TSPL_Dispatch_Detail_BulkSale_Trade.Fat_KG ,TSPL_Dispatch_Detail_BulkSale_Trade.SNF_KG   from TSPL_Dispatch_Detail_BulkSale_Trade Left Outer Join TSPL_Dispatch_BulkSale_Trade on  TSPL_Dispatch_BulkSale_Trade.Document_No=TSPL_Dispatch_Detail_BulkSale_Trade.Document_No Left Outer Join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_Dispatch_Detail_BulkSale_Trade.Item_Code Left outer Join TSPL_BulkSalePrice_MASTER on TSPL_BulkSalePrice_MASTER.Price_Code =TSPL_Dispatch_BulkSale_Trade.Price_Code where TSPL_Dispatch_BulkSale_Trade.Posted=1" & _
                            " and TSPL_Dispatch_BulkSale_Trade.Document_No not in ( Select TSPL_DISPATCH_BULKSALE_TRADE_RETURN.DispatchTrade_No  from TSPL_DISPATCH_BULKSALE_TRADE_RETURN ) "
        If clsCommon.myLen(customerCode) > 0 Then
            qry += " and (len(ISNULL(TSPL_Dispatch_BulkSale_Trade.Customer_Code,''))=0 or TSPL_Dispatch_BulkSale_Trade.Customer_Code='" + customerCode + "')" + Environment.NewLine
        End If
        If clsCommon.myLen(Locationcode) > 0 Then
            qry += " and TSPL_Dispatch_BulkSale_Trade.Location_Code='" + Locationcode + "' " + Environment.NewLine
        End If
        'qry += " Union All " + Environment.NewLine & _
        '        " Select TSPL_INVOICE_DETAIL_BULKSALE.Dispatch_Code as Code,TSPL_INVOICE_MASTER_BULKSALE .Customer_Code as Customer,TSPL_INVOICE_DETAIL_BULKSALE.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName,TSPL_INVOICE_DETAIL_BULKSALE.InvoiceQty as Qty,0 as Unapproved ,'' as Location,-1 as RI,0 as Fatper,0 as SNFPer,0 as Rate,0 as Amount,null as Document_Date,TSPL_INVOICE_DETAIL_BULKSALE.Unit_Code,0 as Fat_Weightage ,0 as Snf_Weightage,0 as Fat_KG ,0 as SNF_KG from TSPL_INVOICE_DETAIL_BULKSALE left Outer Join TSPL_INVOICE_MASTER_BULKSALE on TSPL_INVOICE_MASTER_BULKSALE.Document_No=TSPL_INVOICE_DETAIL_BULKSALE.Document_No Left Outer Join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_INVOICE_DETAIL_BULKSALE.Item_Code  where TSPL_INVOICE_MASTER_BULKSALE.Posted=1 and len(isnull(TSPL_INVOICE_DETAIL_BULKSALE.Dispatch_Code,''))>0 and TSPL_INVOICE_MASTER_BULKSALE.InvoiceAgainst='" + InvoiceAgainst + "'"
    ...
    Sub btnOKPressed()
        ...
        ArrReturn = New List(Of clsDispatchDetailTradeBulkSale)
        ...
        Try
            If Not IsInsideLoadData AndAlso gvHead.CurrentRow.Index >= 0 Then
                ...
                If clsCommon.myLen(strCode) > 0 Then
                    ...
                    If gvHead.CurrentRow.Cells(colHSelect).Value = False Then
                        For Each grow As GridViewRowInfo In gvHead.Rows

                            If gvHead.CurrentRow.Index <> grow.Index Then
                                If grow.Cells(colHSelect).Value = True AndAlso clsCommon.myCDate((grow.Cells(colHDate).Value), "dd/MM/yyyy") <> clsCommon.myCDate((gvHead.CurrentRow.Cells(colHDate).Value), "dd/MM/yyyy") Then
                                    Throw New Exception("Dispatch date should be same for selected dispatch no")
                                End If
                            End If
                        Next
                    End If
        ...
        If NewVal Then
            For Each dr As DataRow In dtAllData.Rows
                If clsCommon.CompairString(strCode, clsCommon.myCstr(dr("Code"))) = CompairStringResult.Equal Then
                    ...
                    'gv1.Rows(gv1.Rows.Count - 1).Cells(colDAmount).Value = clsCommon.myCdbl(dr("PendingQty")) * clsCommon.myCdbl(dr("Rate"))
