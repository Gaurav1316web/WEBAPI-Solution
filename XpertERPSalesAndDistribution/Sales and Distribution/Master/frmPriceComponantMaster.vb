...
#End Region
    ...
    Private Function AllowToSave() As Boolean
        Try
            If (IIf(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowAutoGenerateDocNoInMaster, clsFixedParameterCode.AllowAutoGenerateDocNoInMaster, Nothing)) = "1", True, False) = False) AndAlso clsCommon.myLen(txtComponentCode.Value) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Component Code not be left blank.", Me.Text)
                txtComponentCode.Focus()
                Return False
            ElseIf clsCommon.myLen(txtDesc.Text) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Description can not be left blank.", Me.Text)
                txtDesc.Focus()
                Return False
            ElseIf chkGLAccountApplicable.Checked Then
                If clsCommon.myLen(txtGLAccountcc.Value) <= 0 Then
                    clsCommon.MyMessageBoxShow(Me, "Select GL Account.", Me.Text)
                    txtGLAccountcc.Focus()
                    Return False
                End If
            ElseIf clsCommon.myCdbl(ddlSerialNumber.SelectedValue) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Select serial no.", Me.Text)
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtComponentCode.Value) <= 0 Then
                clsCommon.MyMessageBoxShow("Select Document No")
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowHistoryData(txtComponentCode.Value, "Price_Comp_code", "TSPL_PRICE_COMPONENT_MASTER")
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    End Sub
