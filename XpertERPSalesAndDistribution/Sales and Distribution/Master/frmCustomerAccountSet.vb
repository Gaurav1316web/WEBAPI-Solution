...
Public Class frmCustomerAccountSet
    ...
    Sub SaveData()
        ...
        ElseIf (fndrecisvablecontrol.Value = "") Then
            myMessages.blankValue(Me, "Debtor Control", Me.Text)
            fndrecisvablecontrol.Focus()
        ElseIf (fndrecieptdiscount.Value = "") Then
            myMessages.blankValue(Me, "Reciept Discount", Me.Text)
            fndrecieptdiscount.Focus()
        ElseIf (fndwriteoffs.Value = "") Then
            myMessages.blankValue(Me, "Write Offs", Me.Text)
            fndwriteoffs.Focus()
            'richa  Ticket No. BM00000003087 on 08/07/2014
        ElseIf (FndSecurity.Value = "") Then
            myMessages.blankValue(Me, "Security", Me.Text)
            FndSecurity.Focus()
        ElseIf (fndCreateSecurity.Value = "") Then
            myMessages.blankValue(Me, "Create Security", Me.Text)
            fndCreateSecurity.Focus()
        ElseIf (fndBankGuarantee.Value = "") Then
            myMessages.blankValue(Me, "Bank Guarantee", Me.Text)
            fndBankGuarantee.Focus()

        ElseIf (fndAccount1.Value = "") Then
            myMessages.blankValue(Me, "Refrigerator Security", Me.Text)
            fndAccount1.Focus()
        ElseIf (fndAccount2.Value = "") Then
            myMessages.blankValue(Me, "Other Security", Me.Text)
            fndAccount2.Focus()
            '---------richa Code Ends------------------------------------
