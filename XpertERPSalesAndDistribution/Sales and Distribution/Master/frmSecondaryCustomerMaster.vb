...
#Region "Variable"
    ...
    Const ColCompetLineNo As String = "ColCompetLineNo"
    Const ColCompetCODE As String = "ColCompetCODE"
    Const ColCompetNAME As String = "ColCompetNAME"
    Const ColCompetMilk As String = "ColCompetMilk"
    Const ColCompetCurd As String = "ColCompetCurd"
    Const ColCompetOthers As String = "ColCompetOthers"
#End Region
...
#End Region
    ...
    Public Sub funNew()
        ...
        LoadBlankGrid()
        LoadBlankGridCompetitor()
        gvCompetitor.Rows.AddNew()
        Me.fndZone.Value = ""
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            If clsCommon.myLen(txtCustomerName.Text) <= 0 Then
                common.clsCommon.MyMessageBoxShow("Please fill customer name", "Retailer", MessageBoxButtons.OK)
                pageCus.SelectedPage = RadPageViewPage1
                txtCustomerName.Focus()
                Return
            End If
            If clsCommon.myLen(fndCusgrp.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow("Select Customer Group", "Retailer", MessageBoxButtons.OK)
                pageCus.SelectedPage = RadPageViewPage1
                fndCusgrp.Focus()
                Return
            End If
            If clsCommon.myLen(fndCusCategory.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow("Select Customer Catageory ", "Retailer", MessageBoxButtons.OK)
            ...
            If clsCommon.myLen(fndAccntSet.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow("Select Customer Account Set", "Retailer", MessageBoxButtons.OK)
        ...
        Try
            ...
            If clsCommon.CompairString(clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select Description from TSPL_FIXED_PARAMETER where Code='" & clsFixedParameterCode.CustomerNameUniqueOnCM & "' and Type ='" & clsFixedParameterType.CustomerNameUniqueOnCM & "'")), "1") = CompairStringResult.Equal Then
                If clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select count(*) from TSPL_SECONDARY_CUSTOMER_MASTER where Customer_Name='" & clsCommon.myCstr(txtCustomerName.Text) & "' and Cust_Code<>'" & clsCommon.myCstr(fndCustomer.Value) & "'")) > 0 Then
                    common.clsCommon.MyMessageBoxShow("Same Retailer Name is exist with another Retailer so please change Retailer name because Retailer Name is unique.")
                    Return False
                End If
            End If
            ...
            If clsCommon.myLen(clsCommon.myCstr(fndCustCurrency.Value)) > 0 Then
                ...
                If clsCommon.CompairString(accCurrCode, clsCommon.myCstr(Me.fndCustCurrency.Value)) <> CompairStringResult.Equal Then
                    clsCommon.MyMessageBoxShow("Account Set Currency and Retailer Currency must be same in case of Multicurrency.")
                    Return False
                End If
 ...
 " ) XXX LEFT OUTER JOIN TSPL_SECONDARY_CUSTOMER_MASTER ON XXX.[Customer Id]=TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code where  XXX.Document_Type in ('IN','DB','CR','RC','AV','OA','UC','SR','VGCL','AD','RF','RC'  )  and convert(date,XXX.[Document Date] ,103) <= convert(date,'03/09/2014',103) AND [Due Amount] <> 0 And [Customer Id]='" & fndCustomer.Value & "' Group By XXX.[Customer Id]  ORDER BY [Customer Id] "
    ...
    Public Sub funFill()
        Try
            ...
            'End If
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Exclamation, "Retailer")
        End Try
    End Sub
    ...
    Private Sub fndstate__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndstate._MYValidating
        ...
        txtGstState.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select tspl_state_master.GST_STATE_Code  from tspl_state_master where STATE_CODE ='" + fndstate.Value + "'"))
        txtStateName.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select  STATE_NAME  from TSPL_State_MASTER where STATE_CODE ='" + fndstate.Value + "'"))
    End Sub
    ...
    Private Sub txtPriceCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtPriceCode._MYValidating
        ...
        If objCommonVar.GSTApplicable Then
            WhrCls = ""
        Else
            WhrCls = " TSPL_TAX_GROUP_MASTER.Excisable ='Y'"
        End If
    ...
    Private Sub txtPriceCodeNon__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtPriceCodeNon._MYValidating
        ...
        If objCommonVar.GSTApplicable Then
            WhrCls = ""
        Else
            WhrCls = " TSPL_TAX_GROUP_MASTER.Excisable ='N'"
        End If
    ...
    Sub SaveSecondaryCustomerMaster()
        ...
        If obj.CUSTOMER_TYPE = "Retailer" Or obj.CUSTOMER_TYPE = "R" Then
            obj.IsDistributor = "N"
        End If
        If obj.CUSTOMER_TYPE = "Distributor" Or obj.CUSTOMER_TYPE = "D" Then
            obj.IsDistributor = "Y"
        End If
        ...
        If clsCommon.CompairString(cboBusinessType.Text, "Commercial") = CompairStringResult.Equal Then
            obj.BusinessType = "C"
        ElseIf clsCommon.CompairString(cboBusinessType.Text, "Domestic") = CompairStringResult.Equal Then
            obj.BusinessType = "D"
        End If
        ...
        Else
            common.clsCommon.MyMessageBoxShow("Retailer No can't be blank.")
            Return
        End If
        ...
        If objCommonVar.GSTApplicable = True Then
            ...
            If ChkRegistered.Checked = True AndAlso (clsCommon.myLen(txtGSTEntityNo.Text) = 0 Or clsCommon.myLen(txtGSTBlank.Text) = 0 Or clsCommon.myLen(txtGSTDegit.Text) = 0) Then
                common.clsCommon.MyMessageBoxShow("Please Enter GST Number.", "Retailer", MessageBoxButtons.OK)
                pageCus.SelectedPage = RadPageViewPage4
                Return
            End If

            Dim GSTState As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select tspl_state_master.GST_STATE_Code  from tspl_state_master where STATE_CODE ='" + fndstate.Value + "'"))

            If ChkRegistered.Checked = True Then
                If (clsCommon.myLen(txtGSTEntityNo.Text) > 0 And clsCommon.myLen(txtGSTBlank.Text) > 0 And clsCommon.myLen(txtGSTDegit.Text) > 0) Then

                    If clsERPFuncationality.ValidationGSTNO(GSTState, txtpan.Text, clsCommon.myCstr(txtGstState.Text) + clsCommon.myCstr(txtGSTPANNO.Text) + clsCommon.myCstr(txtGSTEntityNo.Text) + clsCommon.myCstr(txtGSTBlank.Text) + clsCommon.myCstr(txtGSTDegit.Text), Nothing) = False Then
                        Return
                    End If
                End If
            End If
            ...
            If (clsCommon.myLen(txtGSTEntityNo.Text) > 0 And clsCommon.myLen(txtGSTBlank.Text) > 0 And clsCommon.myLen(txtGSTDegit.Text) > 0) Then
                ...
                obj.GSTDigit = clsCommon.myCstr(txtGSTDegit.Text)
        ...
        For Each gr As GridViewRowInfo In gvCompetitor.Rows
            Dim objTr As New clsCustomerCompetitorDetail()
            objTr.Line_No = clsCommon.myCdbl(gr.Cells(ColCompetLineNo).Value)
            objTr.Cust_Code = clsCommon.myCstr(gr.Cells(ColCompetCODE).Value)
            objTr.Competitor_Code = clsCommon.myCstr(gr.Cells(ColCompetCODE).Value)
            objTr.Milk = clsCommon.myCdbl(gr.Cells(ColCompetMilk).Value)
            objTr.Curd = clsCommon.myCdbl(gr.Cells(ColCompetCurd).Value)
            objTr.Others = clsCommon.myCdbl(gr.Cells(ColCompetOthers).Value)
            If (clsCommon.myLen(gr.Cells(ColCompetCODE).Value) > 0) Then
    ...
    Sub LoadDataSecondaryCustomer(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Cust_Code) > 0) Then
                ...
                End If
                If obj.CUSTOMER_TYPE = "D" Or obj.CUSTOMER_TYPE = "Distributor" Then
                    CmbCustomerType.Text = "Distributor"
                End If

                If clsCommon.CompairString(obj.BusinessType, "C") = CompairStringResult.Equal Then
                    cboBusinessType.Text = "Commercial"
                ElseIf clsCommon.CompairString(obj.BusinessType, "D") = CompairStringResult.Equal Then
                    cboBusinessType.Text = "Domestic"
                End If
                ...
                End If
                If CmbCustomerType.Text = "Distributor" Or CmbCustomerType.Text = "D" Then
                ...
                '===============Added by sanjay
                Dim GSTState As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select tspl_state_master.GST_STATE_Code  from tspl_state_master where STATE_CODE ='" + obj.State + "'"))
                ...
                txtGSTPANNO.Text = obj.PAN

                If clsCommon.myCdbl(obj.Registered) = 0 Then
                    ChkRegistered.Checked = False
                Else
                    ChkRegistered.Checked = True
                End If
                txtGSTEntityNo.Text = obj.GSTEntity
                txtGSTBlank.Text = obj.GSTBlank
                txtGSTDegit.Text = obj.GSTDigit
                txtGstNo.Text = obj.GSTNO
                '================================================

                If obj.Arr_CompetitorDetail IsNot Nothing AndAlso obj.Arr_CompetitorDetail.Count > 0 Then
                    For Each objTr As clsCustomerCompetitorDetail In obj.Arr_CompetitorDetail
                        gvCompetitor.Rows(gvCompetitor.Rows.Count - 1).Cells(ColCompetLineNo).Value = objTr.Line_No
                        gvCompetitor.Rows(gvCompetitor.Rows.Count - 1).Cells(ColCompetCODE).Value = objTr.Competitor_Code
                        gvCompetitor.Rows(gvCompetitor.Rows.Count - 1).Cells(ColCompetNAME).Value = ClsCompetitorMaster.GetName(objTr.Competitor_Code, Nothing)
                        gvCompetitor.Rows(gvCompetitor.Rows.Count - 1).Cells(ColCompetMilk).Value = clsCommon.myCdbl(objTr.Milk)
                        gvCompetitor.Rows(gvCompetitor.Rows.Count - 1).Cells(ColCompetCurd).Value = clsCommon.myCdbl(objTr.Curd)
                        gvCompetitor.Rows(gvCompetitor.Rows.Count - 1).Cells(ColCompetOthers).Value = clsCommon.myCdbl(objTr.Others)
                        gvCompetitor.Rows.AddNew()
                    Next
                End If
    ...
    Public Sub GetCustomerType()
        ...
        Else
            ...
            dt.Rows.Add("Distributor", "Distributor")
            CmbCustomerType.DataSource = dt
            CmbCustomerType.DisplayMember = "DESCRIPTION"
            CmbCustomerType.ValueMember = "GROUP_CODE"
        End If
    ...
    Private Sub txtParentCstNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtParentCstNo._MYValidating
        ...
        If isCheckCustomerType = False Then
            ...
            If CmbCustomerType.Text = "Distributor" Then
                ...
                Dim qryDesc As String = "select Customer_name From TSPL_CUSTOMER_MASTER where cust_code='" + txtParentCstNo.Value + "'"
                Dim dt As DataTable = clsDBFuncationality.GetDataTable(qryDesc)
                If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                    txtParentCstmrNo.Text = clsCommon.myCstr(dt.Rows(0)("Customer_name"))
    ...
    Public Sub funImport()
        ...
        Dim input() As String = {}
        If objCommonVar.GSTApplicable Then
            input = {"Cust_Code", "Customer_Name", "Add1", "Add2", "Add3", "Phone1", "Phone2", "Credit_Limit", "IsDistributor", "Parent_Customer_No", "Parent_Customer_YN", "Created_By", "Created_Date", "Modified_By", "Modified_Date", "Comp_Code", "OnHold", "City_Code", "State", "Country", "Fax", "Email", "WebSite", "PIN_NO", "Contact_Person_Name", "Contact_Person_Phone", "Contact_Person_fax", "Contact_Person_Website", "Contact_Person_Email", "TAX1", "TAX1_Rate", "TAX2", "TAX2_Rate", "TAX3", "TAX3_Rate", "TAX4", "TAX4_Rate", "TAX5", "TAX5_Rate", "TAX6", "TAX6_Rate", "TAX7", "TAX7_Rate", "TAX8", "TAX8_Rate", "TAX9", "TAX9_Rate", "TAX10", "TAX10_Rate", "Remarks1", "Remarks2", "Additional1", "Additional2", "Additional3", "Payment_Code", "Service_Tax_No", "Tin_No", "Lst_No", "Form_Type", "Channel_Code", "Channel_Desc", "Credit_Customer", "Customer_Class", "Cust_Category_Code", "Zone_Code", "Route_No", "Salesman_Code", "price_group_code", "Route_Group", "Terms_Code", "Cust_Account", "TempCreditLimit", "TempCreditLimitFrom", "TempCreditLimitTo", "Tax_Group", "Alies_Name", "Agg_Made_Date", "Agg_Close_Date", "CURRENCY_CODE", "Cust_Group_Code", "CST", "ECC", "Range", "Collectorate", "PAN", "Division", "GSTNo", "BusinessType"}
        Else
            input = {"Cust_Code", "Customer_Name", "Add1", "Add2", "Add3", "Phone1", "Phone2", "Credit_Limit", "IsDistributor", "Parent_Customer_No", "Parent_Customer_YN", "Created_By", "Created_Date", "Modified_By", "Modified_Date", "Comp_Code", "OnHold", "City_Code", "State", "Country", "Fax", "Email", "WebSite", "PIN_NO", "Contact_Person_Name", "Contact_Person_Phone", "Contact_Person_fax", "Contact_Person_Website", "Contact_Person_Email", "TAX1", "TAX1_Rate", "TAX2", "TAX2_Rate", "TAX3", "TAX3_Rate", "TAX4", "TAX4_Rate", "TAX5", "TAX5_Rate", "TAX6", "TAX6_Rate", "TAX7", "TAX7_Rate", "TAX8", "TAX8_Rate", "TAX9", "TAX9_Rate", "TAX10", "TAX10_Rate", "Remarks1", "Remarks2", "Additional1", "Additional2", "Additional3", "Payment_Code", "Service_Tax_No", "Tin_No", "Lst_No", "Form_Type", "Channel_Code", "Channel_Desc", "Credit_Customer", "Customer_Class", "Cust_Category_Code", "Zone_Code", "Route_No", "Salesman_Code", "price_group_code", "Route_Group", "Terms_Code", "Cust_Account", "TempCreditLimit", "TempCreditLimitFrom", "TempCreditLimitTo", "Tax_Group", "Alies_Name", "Agg_Made_Date", "Agg_Close_Date", "CURRENCY_CODE", "Cust_Group_Code", "CST", "ECC", "Range", "Collectorate", "PAN", "Division", "BusinessType"}
        End If
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If objCommonVar.GSTApplicable = True Then
                        ...
                        If String.IsNullOrEmpty(GSTNo) Then
                            Throw New Exception("Please Enter GST No at Line No '" + LineNo + "'")
                        End If

                        GSTState = clsCommon.myCstr(GSTNo.Trim().Substring(0, 2))
                        ...
                        If clsERPFuncationality.ValidationGSTNO(GSTState, clsCommon.myCstr(grow.Cells("Pan").Value), GSTNo, trans) = False Then
                            trans.Rollback()
                            Exit Sub
                        End If
                        ...
                        clsCommon.AddColumnsForChange(coll, "GSTNO", clsCommon.myCstr(grow.Cells("GSTNo").Value))
                        clsCommon.AddColumnsForChange(coll, "GSTEntity", clsCommon.myCstr(GSTEntity))
                        clsCommon.AddColumnsForChange(coll, "GSTBlank", clsCommon.myCstr(GSTBlank))
                        clsCommon.AddColumnsForChange(coll, "GSTDigit", clsCommon.myCstr(GSTdigit))
                    End If
                    ...
                    Dim StrBusinessType As String = clsCommon.myCstr(grow.Cells("BusinessType").Value)
                    If clsCommon.CompairString(StrBusinessType, "C") = CompairStringResult.Equal OrElse clsCommon.CompairString(StrBusinessType, "D") = CompairStringResult.Equal Then
                        clsCommon.AddColumnsForChange(coll, "BusinessType", StrBusinessType)
                    Else
                        Throw New Exception("Please Enter Only C for Commercial or D for Domestic as Business Type")
                    End If
    ...
    Private Sub txtpan_Leave(sender As Object, e As EventArgs) Handles txtpan.Leave
    ...
    Private Sub txtcst_TextChanged(sender As Object, e As EventArgs) Handles txtcst.TextChanged

    End Sub

    Sub LoadBlankGridCompetitor()
        gvCompetitor.Rows.Clear()
        gvCompetitor.Columns.Clear()

        Dim repoLineNo As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoLineNo = New GridViewDecimalColumn()
        repoLineNo.FormatString = ""
        repoLineNo.HeaderText = "Line No"
        repoLineNo.Name = ColCompetLineNo
        repoLineNo.Width = 50
        repoLineNo.ReadOnly = True
        repoLineNo.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gvCompetitor.MasterTemplate.Columns.Add(repoLineNo)


        Dim repoCompetCode As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoCompetCode.FormatString = ""
        repoCompetCode.HeaderText = "Competitor Code"
        repoCompetCode.Name = ColCompetCODE
        repoCompetCode.TextImageRelation = TextImageRelation.TextBeforeImage
        repoCompetCode.Width = 100
        gvCompetitor.MasterTemplate.Columns.Add(repoCompetCode)

        Dim repoCompetName As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoCompetName.FormatString = ""
        repoCompetName.HeaderText = "Competitor Name"
        repoCompetName.Name = ColCompetNAME
        repoCompetName.ReadOnly = True
        repoCompetName.Width = 150
        gvCompetitor.MasterTemplate.Columns.Add(repoCompetName)


        Dim repoCompetMilk As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoCompetMilk = New GridViewDecimalColumn()
        repoCompetMilk.FormatString = ""
        repoCompetMilk.HeaderText = "Milk"
        repoCompetMilk.Name = ColCompetMilk
        repoCompetMilk.Width = 50
        'repoQty.Minimum = 0
        repoCompetMilk.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gvCompetitor.MasterTemplate.Columns.Add(repoCompetMilk)

        Dim repoCompetCurd As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoCompetCurd = New GridViewDecimalColumn()
        repoCompetCurd.FormatString = ""
        repoCompetCurd.HeaderText = "Curd"
        repoCompetCurd.Name = ColCompetCurd
        repoCompetCurd.Width = 50
        'repoQty.Minimum = 0
        repoCompetCurd.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gvCompetitor.MasterTemplate.Columns.Add(repoCompetCurd)

        Dim repoCompetOthers As GridViewDecimalColumn = New GridViewDecimalColumn()
        repoCompetOthers = New GridViewDecimalColumn()
        repoCompetOthers.FormatString = ""
        repoCompetOthers.HeaderText = "Others"
        repoCompetOthers.Name = ColCompetOthers
        repoCompetOthers.Width = 50
        'repoQty.Minimum = 0
        repoCompetMilk.TextAlignment = System.Drawing.ContentAlignment.MiddleRight
        gvCompetitor.MasterTemplate.Columns.Add(repoCompetOthers)

        gvCompetitor.AllowAddNewRow = False
        gvCompetitor.ShowGroupPanel = False
        gvCompetitor.AllowColumnReorder = False
        gvCompetitor.AllowRowReorder = False
        gvCompetitor.EnableSorting = False
        gvCompetitor.AddNewRowPosition = Telerik.WinControls.UI.SystemRowPosition.Bottom
        gvCompetitor.MasterTemplate.ShowRowHeaderColumn = False

    End Sub

    Private Sub gvCompetitor_CellValueChanged(sender As Object, e As GridViewCellEventArgs) Handles gvCompetitor.CellValueChanged
        Try
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    isCellValueChangedOpen = True
                    If e.Column Is gvCompetitor.Columns(ColCompetCODE) Then
                        OpenCompetitorList(False)
                    End If
                End If
    ...
    Sub OpenCompetitorList(ByVal isButtonClick As Boolean)
        gvCompetitor.CurrentRow.Cells(ColCompetCODE).Value = ClsCompetitorMaster.getFinder(clsCommon.myCstr(gvCompetitor.CurrentRow.Cells(ColCompetCODE).Value), False)
        If clsCommon.myLen(gvCompetitor.CurrentRow.Cells(ColCompetCODE).Value) > 0 Then
            gvCompetitor.CurrentRow.Cells(ColCompetNAME).Value = ClsCompetitorMaster.GetName(clsCommon.myCstr(gvCompetitor.CurrentRow.Cells(ColCompetCODE).Value), Nothing)
        End If
    End Sub

    Private Sub FrmSecondaryCustomerMaster_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        If e.KeyCode = Keys.F2 AndAlso gvCompetitor.CurrentCell IsNot Nothing Then
            isCellValueChangedOpen = True
            If gvCompetitor.CurrentColumn Is gvCompetitor.Columns(ColCompetCODE) Then
                gvCompetitor.CurrentColumn = gvCompetitor.Columns(ColCompetNAME)
                OpenCompetitorList(True)
                gvCompetitor.CurrentColumn = gvCompetitor.Columns(ColCompetCODE)
            End If
            setGridFocus()
            isCellValueChangedOpen = False
        End If
    ...
    Private Sub setGridFocus()
        Try
            Dim intCurrRow As Integer = gvCompetitor.CurrentRow.Index
            gvCompetitor.CurrentRow.Cells(ColCompetLineNo).Value = clsCommon.myCdbl(intCurrRow + 1)
            If intCurrRow = gvCompetitor.Rows.Count - 1 Then
                gvCompetitor.Rows.AddNew()
        ...
        Try
            ...
            If transportSql.importExcel(gv, "Customer Code", "Competitor Code", "Milk", "Curd", "Others") Then
                ...
                For ii As Integer = 0 To gv.Rows.Count - 1
                    Dim strCustomerCode As String = clsCommon.myCstr(gv.Rows(ii).Cells("Customer Code").Value)
                    Dim strCompetitorCode As String = clsCommon.myCstr(gv.Rows(ii).Cells("Competitor Code").Value)
                    If clsCommon.myLen(strCustomerCode) > 0 Then
                        If clsCommon.myLen(strCompetitorCode) <= 0 Then
                            Throw New Exception("Define Competitor Code for Customer Code ( " + strCustomerCode.Trim() + " ) At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + "")
                        End If

                        'Customer
                        qry = "select count(*) from TSPL_SECONDARY_CUSTOMER_MASTER where Cust_Code='" + strCustomerCode + "'"
                        check = CInt(clsDBFuncationality.getSingleValue(qry, trans))

                        If check <= 0 Then
                            Throw New Exception("Customer Code (" + strCustomerCode + ") Does Not Exist in Master At Line No. " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                        End If
                        'Competitor
                        qry = "select count(*) from TSPL_COMPETITOR_MASTER where Code='" + strCompetitorCode + "'"
                        check = CInt(clsDBFuncationality.getSingleValue(qry, trans))

                        If check <= 0 Then
                            Throw New Exception("Competitor Code (" + strCompetitorCode + ") Does Not Exist in Master At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                        End If
                        ...
                        For jj As Integer = 0 To gv.Rows.Count - 1
                            ...
                            If (clsCommon.CompairString(strCustomerCode, clsCommon.myCstr(gv.Rows(jj).Cells("Customer Code").Value)) = CompairStringResult.Equal) AndAlso (clsCommon.CompairString(strCompetitorCode, clsCommon.myCstr(gv.Rows(jj).Cells("Competitor Code").Value)) = CompairStringResult.Equal) Then
                                Throw New Exception("Duplicate Competitor code " + "( " + strCompetitorCode.Trim() + " )" + " for Customer Code ( " + strCustomerCode.Trim() + " ) At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " and  " + clsCommon.myCstr(clsCommon.myCdbl(jj + 1)) + "")
                            End If
                        Next
                    End If
                Next
                ...
                Dim DistinctcolumnName() As String = {"Customer Code"}
                ...
                For i As Integer = 0 To dtCust.Rows.Count - 1
                    ...
                    Dim dr As DataRow() = dtData.Select(" [Customer Code]='" + clsCommon.myCstr(dtCust.Rows(i)("Customer Code")) + "'  ")
                    If dr IsNot Nothing AndAlso dr.Length > 0 Then
                        dtCustData = dr.CopyToDataTable()
                        '======================================================================
                        Dim obj As New clsSecondaryCustomerMasterInfo()
                        obj.Arr_CompetitorDetail = New List(Of clsCustomerCompetitorDetail)
                        For ii As Integer = 0 To dtCustData.Rows.Count - 1
                            Dim objTr As New clsCustomerCompetitorDetail()
                            objTr.Line_No = ii + 1
                            objTr.Cust_Code = clsCommon.myCstr(dtCustData.Rows(ii)("Customer Code"))
                            objTr.Competitor_Code = clsCommon.myCstr(dtCustData.Rows(ii)("Competitor Code"))
                            objTr.Milk = clsCommon.myCdbl(dtCustData.Rows(ii)("Milk"))
                            objTr.Curd = clsCommon.myCdbl(dtCustData.Rows(ii)("Curd"))
                            objTr.Others = clsCommon.myCdbl(dtCustData.Rows(ii)("Others"))

                            obj.Arr_CompetitorDetail.Add(objTr)

                        Next

                        If (clsCustomerCompetitorDetail.SaveData(clsCommon.myCstr(dtCust.Rows(i)("Customer Code")), obj.Arr_CompetitorDetail, trans)) = False Then
                            trans.Rollback()
                        End If
    ...
    Private Sub RmiExportCompetitor_Click(sender As Object, e As EventArgs) Handles rmiExportCompetitor.Click
        Try
            Dim str As String
            str = " select TSPL_TRADER_COMPETITOR_DETAIL.Cust_Code as [Customer Code] ,TSPL_TRADER_COMPETITOR_DETAIL.Competitor_Code as [Competitor Code] 
             ,TSPL_TRADER_COMPETITOR_DETAIL.Milk as [Milk] ,TSPL_TRADER_COMPETITOR_DETAIL.Curd as [Curd] ,TSPL_TRADER_COMPETITOR_DETAIL.Others as [Others] 
             from TSPL_TRADER_COMPETITOR_DETAIL"
