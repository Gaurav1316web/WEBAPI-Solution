...
#Region "Methods"
    ...
    Private Sub funInsert()
        Try
            If (IIf(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowAutoGenerateDocNoInMaster, clsFixedParameterCode.AllowAutoGenerateDocNoInMaster, Nothing)) = "1", True, False)) Then
                Dim ChkNewEntry As String = clsDBFuncationality.getSingleValue("Select count(*) from TSPL_TRANSPORT_TYPE where TransType_Code='" & fndTransportTypeId.Value & "'")
                If ChkNewEntry = 0 Then
                    fndTransportTypeId.Value = clsERPFuncationality.GetNextCode(Nothing, clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(), "dd/MM/yyyy"), clsDocType.TransportType, "", "")
                    If clsCommon.myLen(fndTransportTypeId.Value) <= 0 Then
    ...
    Private Sub funUpdate()
        If fndTransportTypeId.Value = "" Then
            myMessages.blankValue(Me, "Transport Type", Me.Text)
        Else
            Try
...
#Region "ButtonClick Event"
    ...
    Sub SaveData()
        ...
        If (IIf(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowAutoGenerateDocNoInMaster, clsFixedParameterCode.AllowAutoGenerateDocNoInMaster, Nothing)) = "1", True, False) = False) AndAlso fndTransportTypeId.Value = "" Then
            myMessages.blankValue(Me, "Transport Type", Me.Text)
        Else
