...
'--------------------------------Last modify date - 27/11/2012-------------------------------------
'--------------------------------Last modify Time - 10:30 pm -------------------------------------
'--------------------------------Last modify date - 17 jan 2013 03:25 pm -------------------------------------
...
'by vipin for pdf work on 11/1/2013
...
Public Class FrmSaleAccountBreakageReport
    ...
    Private Sub FrmSaleAccountBreakageReport_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub FrmSaleAccountBreakageReport_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        SetUserMgmtNew()
    ...
    Sub print()
        Try
            ...
            "" + clsCommon.ReplicateDBString + "TSPL_SALE_RETURN_HEAD.Comp_Code AS Company, " + clsCommon.ReplicateDBString + "TSPL_LOCATION_MASTER.Location_Desc AS Location, " & _
                     ...
                     "" + clsCommon.ReplicateDBString + "TSPL_SALE_RETURN_HEAD.Sale_Return_Date AS [Invoice Date], " + clsCommon.ReplicateDBString + "TSPL_SALE_RETURN_HEAD.Route_No AS [Route No],  " & _
                     ...
                     "WHERE  TSPL_ITEM_UOM_DETAIL_1.UOM_Code='FB' and convert(date," + clsCommon.ReplicateDBString + "TSPL_SALE_RETURN_HEAD.Sale_Return_Date,103) >= convert(date, '" & dtpstart.Value & "',103) AND " & _
                     "convert(date," + clsCommon.ReplicateDBString + "TSPL_SALE_RETURN_HEAD.Sale_Return_Date,103) <= convert(date, '" & dtpend.Value & "',103) " & strPost & " "
            ...
            strQuery = "select Company,Location,[Customer Group],[Customer Type],Customer,Invoice, CONVERT(DATE,[Invoice Date],103) As [Invoice Date], " & _
            "[Route No],[Route Desc],[Salesman Code],[Salesman Name],Vehicle,Item,convert(decimal(18,2),MRP) as MRP,convert(decimal(18,2),MRPBottle) as MRPBottle,[Trade Margin],[Party Discount], " & _
            "[Extra Discount],[Dist Margin],[Agency Margin],[TPT and Others],[Scheme Disc],PC1,PC2,[Trade Price], " & _
            "[Sale Qty],[FOC Qty],[Gross Qty],[Sale Amount],[FOC Amount],[Gross Amount],[Trade Margin] * [Sale Qty] as TrademarginAmt, " & _
            "[Dist Margin]* [Sale Qty] as DistmarginAmt   from (" & strQuery & ") aa ORDER BY AA.[Invoice Date]"
    ...
    Private Sub ExportToExcel(ByVal exporter As EnumExportTo)
        Try
            ...
            If exporter = EnumExportTo.Excel Then
                clsCommon.MyExportToExcelGrid("Sale Account Breakage " + IIf(rdbDetail.IsChecked, " ( Detail ) ", " ( Summary ) "), GV1, arrHeader, Me.Text)
