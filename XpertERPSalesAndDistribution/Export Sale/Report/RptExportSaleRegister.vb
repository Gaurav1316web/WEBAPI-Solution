...
'========shivani Tyagi=========='
Public Class RptExportSaleRegister
    Dim isInsideLoadData As Boolean = False
    Dim atchqry As String = ""
#Region "Functions"
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.RptExportSaleRegister)
...
#End Region
    ...
    Sub Print(ByVal IsPrint As Exporter)
        Try
            ...
            strSaleInvoice = "SELECT TSPL_SD_SALE_INVOICE_DETAIL.Conv_Factor as OldConvF,OrgRate,'" & fromDate.Value & "' as Fdate,'" & ToDate.Value & "' as Tdate, case when TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='EXP' then 'Sale Invoice' end as Type, TSPL_SD_SALE_INVOICE_HEAD.Document_Code, " & _
                        ...
                        " where TSPL_SD_SALE_INVOICE_HEAD.Trans_Type ='EXP' and " & strInvoicePosted & " TSPL_SD_SALE_INVOICE_HEAD.Document_Date  >= '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(fromDate.Value), "dd/MMM/yyyy hh:mm tt") & "' and " & _
            ...
            If rdbSummary.IsChecked Then
                qry = "select  Type, Document_Code,convert(varchar,Document_Date,103)as Document_Date ,MAX(aa.Parent_Customer_No) as Parent_Customer_No, MAX(Parent_Master.Customer_Name) as ParentName,Customer_Code,aa.Customer_Name,address,city_name,state_name,telephone,aa.Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc,sum(Amount) as Amount, " & _
                "sum(Disc_Amt) as Disc_Amt,sum(Amt_Less_Discount) as Amt_Less_Discount,sum(Total_Tax_Amt) as Total_Tax_Amt,sum(TotalAmt) as TotalAmt,convert(decimal(18,2),SUM(Total_BaseCurr)) as Total_BaseCurr  " & _
            ...
            ElseIf rdbDetail.IsChecked = True Then
                qry = "select  Type, Fdate,Tdate,Document_Code,convert(varchar,Document_Date,103)as Document_Date ,MAX(aa.Parent_Customer_No) as Parent_Customer_No, MAX(Parent_Master.Customer_Name) as ParentName,Customer_Code,aa.Customer_Name,address,city_name,state_name,telephone,aa.Salesman_Code,Salesman_Name,Bill_To_Location ,Location_Desc,Item_Code,Item_Desc,cast('ITF code-'+ max(itf_code) as varchar)as Itf_code,sum(InvoiceQty) as InvoiceQty,sum(ShippedQty) as ShippedQty,Unit_Code,Rate,sum(Amount) as Amount, " & _
                "Disc_Per,sum(Disc_Amt) as Disc_Amt,sum(Amt_Less_Discount) as Amt_Less_Discount,sum(Total_Tax_Amt) as Total_Tax_Amt,sum(TotalAmt) as TotalAmt,aa.Currency_Code,ConvRate,convert(decimal(18,2),sum(Total_BaseCurr) )as Total_BaseCurr,Category  " & _
        ...
        If rdbSummary.IsChecked Then
            If Gv1.Rows.Count > 0 Then
                ...
                Select Case strTransType
                    Case "SD-IN"
                        clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXSalesInvoice, strDoc)
                    Case "Sale Return"
    ...
    Private Sub rmsetting_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rmsetting.Click
        ...
        frm.FormId = clsUserMgtCode.RptExportSaleRegister
        frm.ShowDialog()
    End Sub
    ...
    Private Sub RptExportSaleRegister_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        Reset()
    ...
    Private Sub btnQuickExport_Click(sender As Object, e As EventArgs) Handles btnQuickExport.Click
        Try
            ...
            arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptExportSaleRegister & "'"))
