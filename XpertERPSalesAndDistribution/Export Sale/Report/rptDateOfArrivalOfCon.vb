...
Imports System.IO
'========================Preeti Gupta===============================
Public Class RptDateOfArrivalOfCon
    Inherits FrmMainTranScreen
    Dim ButtonToolTip As ToolTip = New ToolTip()

    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.RptdateOfArrivalofCons)
    ...
    Public Sub Load_Report()
        Try
            ...
            sQuery = "  select Document_Type,TSPL_SD_SALE_INVOICE_DETAIL.Item_Code as [Code Of Product],TSPL_ITEM_MASTER.Item_Desc as [Name of Product], " & _
                     ...
                     "   Consignee.Ship_To_Desc as [Name Of Consignee],TSPL_SD_SALE_INVOICE_HEAD.Document_Code+'  ' + convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103)  as [Invoice No. & Date ] , convert(varchar,TSPL_SD_SALE_INVOICE_DETAIL.Qty)+' '+ Document_Type as [Quantity],TSPL_SD_SALE_INVOICE_HEAD.CURRENCY_CODE +' '+convert(varchar,TSPL_SD_SALE_INVOICE_DETAIL.Item_Cost) as [Rate In USD], convert(varchar,isnull(TSPL_SD_SALE_INVOICE_DETAIL.Qty,0)*isnull(TSPL_SD_SALE_INVOICE_DETAIL.Item_Cost,0))+' '+EX_Term_Code as [Amount In CIF/CFR/FOB USD] ," & _
                     " convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_No,103)  as [Name of Shipping Line ],TSPL_SD_SALE_INVOICE_DETAIL.Container_No as [Container No], TSPL_SD_SALE_INVOICE_HEAD.Final_Destination as [Port Of Destination] , TSPL_SD_SALE_INVOICE_HEAD.Bill_To_Location [Location Code], TSPL_LOCATION_MASTER.Location_Desc As [Location Desc] " & _
            ...
            If LocationsFinder.arrValueMember IsNot Nothing AndAlso LocationsFinder.arrValueMember.Count > 0 Then
                sQuery += "  AND TSPL_SD_SALE_INVOICE_HEAD.Bill_To_Location IN (" + clsCommon.GetMulcallString(LocationsFinder.arrValueMember) + ")"
            End If
    ...
    Sub print(ByVal exporter As EnumExportTo)
        Try
            ...
            If exporter = EnumExportTo.Excel Then
                clsCommon.MyExportToExcelGrid("Date of Arrival Consignment", gv1, arrHeader, Me.Text)
            Else
                clsCommon.MyExportToPDF("Date of Arrival Consignment", gv1, arrHeader, Me.Text, True)
            End If
        Catch ex As Exception
    ...
    Private Sub RptDateOfArrivalOfCon_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            SetUserMgmtNew()
    ...
    Private Sub RptDateOfArrivalOfCon_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub ExportGrid(ByVal exporter As EnumExportTo)
        Try
            If gv1.Rows.Count > 0 Then
                ...
                arrHeader.Add(("Date Range: " + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MM/yyyy") + " To " + clsCommon.GetPrintDate(txtToDate.Value, "dd/MM/yyyy")) + " ")
                arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
                arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptdateOfArrivalofCons & "'"))
                If TxtMultiBuyer.arrValueMember IsNot Nothing AndAlso TxtMultiBuyer.arrValueMember.Count > 0 Then
                    arrHeader.Add(" Buyer : " + clsCommon.GetMulcallStringWithComma(TxtMultiBuyer.arrValueMember))
                Else
                    arrHeader.Add((" Buyer : All"))
                ...
                End If
                If LocationsFinder.arrValueMember IsNot Nothing AndAlso LocationsFinder.arrValueMember.Count > 0 Then
                    arrHeader.Add("Location : " + clsCommon.GetMulcallStringWithComma(LocationsFinder.arrValueMember))
                Else
                ...
                Else
                    transportSql.applyExportTemplate(gv1, PageSetupReport_ID)
                    clsCommon.MyExportToPDF("Date of Arrival Consignment", gv1, arrHeader, Me.Text, PageSetupReport_ID, objCommonVar.CurrentUserCode)
                End If
            Else
    ...
    Private Sub LocationsFinder__My_Click(sender As Object, e As EventArgs) Handles LocationsFinder._My_Click
