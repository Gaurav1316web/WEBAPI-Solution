...
Public Class FrmrptTDSLedger
    ...
    Sub LoadData()
        ...
        " select 'S' as Document_Type,1 as Is_For_TDS, MAX(TSPL_TDS_DEDUCTION_HEAD.TDS_Section) as Section,  MAX(TSPL_PAYHEAD_MASTER.Deduction_Code) AS natureofDeduction, 'Employee' as Vendor,null as Name,TSPL_GENERATE_SALARY.SALARY_GENERATION_CODE as docnumber , convert(varchar,max(TSPL_PAYPERIOD_MASTER.DATE_To),103) as DocDate, " & Environment.NewLine & _
        " null as DN,sum(TSPL_GENERATE_SALARY_PAYHEADS.PAYABLE_AMOUNT ) as baseamount,sum(TSPL_GENERATE_SALARY_PAYHEADS.PAYABLE_AMOUNT ) as DEDUCTION_AMOUNT,max(TSPL_GENERATE_SALARY.LOCATION_CODE ) as Loc_Code,0 as Debit, sum(TSPL_GENERATE_SALARY_PAYHEADS.PAYABLE_AMOUNT ) as credit,MAX(TSPL_PAYHEAD_MASTER.Deduction_Code) AS Deduction_Code,'" & objCommonVar.CurrentCompanyName & "'  as Comp,null as Logo_Img,null as Logo_img2 " & Environment.NewLine & _
        " ,null as TDS_Per,'02' as DeductCode, 0 as Actual_surcharge,0 as Actual_EduCess,0 as Actual_Sec_EduCess,max(TSPL_GENERATE_SALARY.GENERATE_REMARKS ) as Description,max(TSPL_LOCATION_MASTER.location_desc) as Location_Desc " & Environment.NewLine & _
        " from TSPL_GENERATE_SALARY" & Environment.NewLine & _
        " left outer join TSPL_GENERATE_SALARY_PAYHEADS on TSPL_GENERATE_SALARY_PAYHEADS.SALARY_GENERATION_CODE =TSPL_GENERATE_SALARY.SALARY_GENERATION_CODE " & Environment.NewLine & _
        " left outer join TSPL_PAYHEAD_MASTER on TSPL_PAYHEAD_MASTER.PAY_HEAD_CODE =TSPL_GENERATE_SALARY_PAYHEADS.PAY_HEAD_CODE " & Environment.NewLine & _
        " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=TSPL_GENERATE_SALARY.LOCATION_CODE " & Environment.NewLine & _
        " LEFT OUTER JOIN TSPL_TDS_DEDUCTION_HEAD ON TSPL_TDS_DEDUCTION_HEAD.Deduction_Code=TSPL_PAYHEAD_MASTER.Deduction_Code " & Environment.NewLine & _
        " left outer join TSPL_PAYPERIOD_MASTER on TSPL_PAYPERIOD_MASTER.PAY_PERIOD_CODE=TSPL_GENERATE_SALARY.PAY_PERIOD_CODE  " & Environment.NewLine & _
        ...
        qry += "  left join TSPL_TDS_SECTION_MASTER on TSPL_TDS_SECTION_MASTER.TDS_Group=TSPL_TDS_DEDUCTION_HEAD.TDS_Section" & _
        " left outer join TSPL_TDS_PAYMENT_DETAIL on TSPL_TDS_PAYMENT_DETAIL.Against_Document_No =final.docnumber " & _
        " and TSPL_TDS_PAYMENT_DETAIL.Document_No  not in (Select Against_TDS_PAYMENT_No from TSPL_PAYMENT_HEADER where Payment_No in (select Document_No  from tspl_bank_reverse where Reverse_Document ='Payments') and isnull(TSPL_PAYMENT_HEADER.Against_TDS_PAYMENT_No,'')<>'' ) " & _
        " left outer join TSPL_TDS_PAYMENT_HEADER on TSPL_TDS_PAYMENT_HEADER.Document_No  =TSPL_TDS_PAYMENT_DETAIL.Document_No " & _
        " left outer join TSPL_VENDOR_MASTER on TSPL_VENDOR_MASTER.Vendor_Code =final.Vendor_Code " & _
