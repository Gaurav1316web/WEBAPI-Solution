...
#End Region
    ...
    '    Try
    '        clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleMCCMilkProcurement, clsUserMgtCode.MPIncentiveEntry, obj.DBT_Reco_Code, obj.Document_Date, trans)
    ...
    '        End If
    '        clsDBTMonthlyFarmerMilkDetail.saveData(obj.FATSNFPer, obj.arr, obj.Document_Code, obj.DBT_Reco_Code, obj.From_Date, trans)
    ...
    '        " Left Outer Join TSPL_MCC_MASTER on TSPL_MP_INCENTIVE_ENTRY_HEAD.DBT_Reco_Code=TSPL_MCC_MASTER.DBT_Reco_Code  where 2=2 "
    ...
    '    End If
    '    Try
    '        Dim dt As DataTable = clsDBFuncationality.GetDataTable("select top 1 Document_Date,DBT_Reco_Code from TSPL_MP_INCENTIVE_ENTRY_HEAD  where Document_Code='" + strDocNo + "'", trans)
    '        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    '            clsERPFuncationality.ValidateLocationCode(objCommonVar.CurrentCompanyCode, clsUserMgtCode.ModuleMCCMilkProcurement, clsUserMgtCode.DBTNEFTUploader, clsCommon.myCstr(dt.Rows(0)("DBT_Reco_Code")), clsCommon.myCDate(dt.Rows(0)("Document_Date")), trans)
    '        End If
    ...
    '      ,TSPL_MP_INCENTIVE_ENTRY_HEAD.DBT_Reco_Code as [MCC Code],TSPL_MCC_MASTER.MCC_Name as [MCC NAME] ,Convert(varchar,TSPL_MP_INCENTIVE_ENTRY_HEAD.From_Date,103) as [From Date],Convert(varchar,TSPL_MP_INCENTIVE_ENTRY_HEAD.To_Date,103) as [To Date]
    ...
    '      Left Outer Join TSPL_MCC_MASTER on TSPL_MCC_MASTER.DBT_Reco_Code=TSPL_MP_INCENTIVE_ENTRY_HEAD.DBT_Reco_Code  "
    ...
    '        '    For Each objtr As clsDBTMonthlyFarmerMilkDetail In obj.arr
    ...
    '    Dim arr As New Dictionary(Of String, clsDBTMonthlyFarmerMilk)
    '    Dim qry As String
    ...
    '                            Try
    '                                qry = clsDBFuncationality.getSingleValue("select DBT_Reco_Code from TSPL_MCC_MASTER  where IsDefault=1")
    ...
    '                            qry = "select Document_Code from TSPL_MP_INCENTIVE_ENTRY_HEAD where From_Date ='" + clsCommon.GetPrintDate(objtemp.IDate, "dd/MMM/yyyy") + "' and DBT_Reco_Code='" + objtemp.MCC + "'"
    '                            dt = clsDBFuncationality.GetDataTable(qry)
    '                            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
    ...
    '                        objtr.UOM = clsDBFuncationality.getSingleValue("select Uom_Code from TSPL_Mcc_UOM_DETAIL where Stocking_Unit='Y' and  DBT_Reco_Code='" & objtemp.MCC & "'")
    ...
    '                            Dim obj As clsDBTMonthlyFarmerMilk = arr.Item(key)
    '                            clsDBTMonthlyFarmerMilk.SaveData(obj, True, trans)
    '                        Next
    '                        trans.Commit()
    '                    Catch ex As Exception
    '                        trans.Rollback()
...
#End Region
    '    Public Shared Function saveData(ByVal isFATSNFPer As Integer, ByVal arrObj As List(Of clsDBTMonthlyFarmerMilkDetail), ByVal strDocNo As String, ByVal strMCCCode As String, ByVal FromDate As Date, ByVal trans As SqlTransaction) As Boolean
    '        Try
    ...
    '                                where DBT_Reco_Code='" + strMCCCode + "'"
    '                        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry, trans)
    '                        If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
