...
#Region "Variable"
    ...
    Public EventType As String = Nothing
    Public EventSource As String = Nothing
    Public History_ON As DateTime = Nothing
    Public Arr As List(Of clsDemandHistoryDetail) = Nothing
#End Region
    Public Shared Function GetDataFromBooking(ByVal DocDate As Date, ByVal Shift As String, ByVal Booth As String, ByVal trans As SqlTransaction) As List(Of clsDemandHistoryMaster)
...
ORDER BY TSPL_BOOKING_MATSER_Hist_Data.Hist_On DESC;"
        ...
        Return lstobj
    End Function
    Public Shared Function GetDataFromDemand(ByVal DocDate As Date, ByVal Shift As String, ByVal Booth As String, ByVal trans As SqlTransaction) As List(Of clsDemandHistoryMaster)
        Dim lstobj As List(Of clsDemandHistoryMaster) = Nothing
        Try
            Dim lstStr As List(Of String) = New List(Of String)
            lstobj = New List(Of clsDemandHistoryMaster)
            ...
            Dim dts As DataTable = clsDBFuncationality.GetDataTable("select distinct TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Document_No from TSPL_DEMAND_BOOKING_DETAIL_HISTORY
where TSPL_DEMAND_BOOKING_DETAIL_HISTORY.ShiftType='" + Shift + "' and Convert(date,TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Document_Date,103)='" + clsCommon.GetPrintDate(DocDate) + "' and TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Cust_Code='" + Booth + "'", trans)
...
SELECT 
    item_code, Unit_code, Qty,Operation_Type,Operation_Source,Hist_Version,Hist_By,change_timestamp,VersionedData.Cust_Code,TSPL_CUSTOMER_MASTER.Customer_Name,Document_No,TSPL_CUSTOMER_MASTER.Route_No
...
ORDER BY  Hist_Version desc,change_timestamp desc"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(strQry, trans)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim HisVersion As Integer = 0
                ...
                For Each objdr As DataRow In dt.Rows
                    ...
                    obj.Cust_Code = clsCommon.myCstr(objdr("Cust_Code"))
                    obj.Cust_Name = clsCommon.myCstr(objdr("Customer_Name"))
                    obj.ShiftType = clsCommon.myCstr(Shift)
                    obj.EventType = clsCommon.myCstr(objdr("Operation_Type"))
                    obj.EventSource = clsCommon.myCstr(objdr("Operation_Source"))
                    obj.History_By = clsCommon.myCstr(objdr("Hist_By"))
                    obj.History_ON = clsCommon.GetPrintDate(objdr("change_timestamp"), "dd/MMM/yyyy HH:mm:ss")
                    strQry = "select TSPL_DEMAND_BOOKING_DETAIL_HISTORY.TR_Code,Cust_Code,Item_Code,Unit_code,Qty,ItemNetAmount,TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Hist_By from TSPL_DEMAND_BOOKING_DETAIL_HISTORY where TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Hist_Version='" + clsCommon.myCstr(objdr("Hist_Version")) + "'   and TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Document_No ='" + obj.Document_No + "' and 
TSPL_DEMAND_BOOKING_DETAIL_HISTORY.Cust_Code='" + Booth + "'"
                    Dim dt1 As DataTable = New DataTable()
                    dt1 = clsDBFuncationality.GetDataTable(strQry, trans)
                    If (dt1 IsNot Nothing AndAlso dt1.Rows.Count > 0) Then
                        obj.Arr = New List(Of clsDemandHistoryDetail)
