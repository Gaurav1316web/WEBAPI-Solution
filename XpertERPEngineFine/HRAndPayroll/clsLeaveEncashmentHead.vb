...
#End Region
        ...
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            ...
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                ...
                For Each dr As DataRow In dt.Rows
                    ...
                    objTr.Emp_Name = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Emp_Name from TSPL_EMPLOYEE_MASTER where EMP_CODE='" + clsCommon.myCstr(objTr.Emp_Code) + "' ", trans))
                    objTr.LEAVE_CODE = clsCommon.myCstr(dr("LEAVE_CODE"))
                    objTr.LEAVE_Name = clsCommon.myCstr(clsDBFuncationality.getSingleValue("Select LEAVE_NAME from TSPL_LEAVE_MASTER where LEAVE_CODE='" + clsCommon.myCstr(objTr.LEAVE_CODE) + "' ", trans))
    ...
    Public Shared Function PostData(ByVal strDocNo As String, ByVal trans As SqlTransaction) As Boolean
        Try
            ...
            clsCommon.AddColumnsForChange(coll, "Posted_Date", dtNow)
            clsCommonFunctionality.UpdateDataTable(coll, "TSPL_Leave_Encashment_Head", OMInsertOrUpdate.Update, "TSPL_Leave_Encashment_Head.Document_Code='" + obj.Document_Code + "'", trans)
