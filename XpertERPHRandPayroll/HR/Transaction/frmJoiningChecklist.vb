'-----------------Created By Shivani Tyagi Ticket No.[BM00000003529]
...
Public Class FrmJoiningChecklist
    ...
    Private Const ReportID As String = "JOIN_CHK"



    Public Sub LoadBlankJoining()
        ...
        repochkcode.Name = colJoiningCode
        ...
        Dim joinmandatory As GridViewCheckBoxColumn = New GridViewCheckBoxColumn()
        joinmandatory.FormatString = ""
        joinmandatory.HeaderText = "Joining Mandatory"
        joinmandatory.Name = colJoiningMandatory
        joinmandatory.Width = 100
        joinmandatory.IsVisible = False
        joinmandatory.ReadOnly = False
        gv.MasterTemplate.Columns.Add(joinmandatory)
    ...
    Sub PostData()
        Try
            ...
            If clsCommon.myLen(fndaccountsetcode.Value) > 0 Then
                Applicant_Code = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("Select COUNT(*) AS Applicant_Code from TSPL_HR_JOINING_HEAD  where Applicant_Code='" + fndaccountsetcode.Value + "'"))
                If Applicant_Code > 0 Then
                    If (myMessages.postConfirm()) Then
                        SaveData()
                        If (clsJoiningCheckListHead.PostData(MyBase.Form_ID, fndaccountsetcode.Value)) Then
    ...
    '    Try
    '        qry = "select TSPL_HR_Joining_ChkList_JOB_TITLE.Chk_Code,TSPL_HR_Check_List.Chk_Description   from TSPL_HR_Joining_ChkList_JOB_TITLE left outer join TSPL_HR_REQUISITION on TSPL_HR_Joining_ChkList_JOB_TITLE.Job_Title_Code =TSPL_HR_REQUISITION.Job_Title_Code "
    '        qry += "left outer join TSPL_HR_APPLICANT_ENTRY on TSPL_HR_REQUISITION.Requisition_Code =TSPL_HR_APPLICANT_ENTRY.Requisition_Code "
    '        qry += "left outer join TSPL_HR_Check_List on TSPL_HR_Joining_ChkList_JOB_TITLE.Chk_Code =TSPL_HR_Check_List.Chk_Code "
    ...
    '                gv.CurrentRow.Cells(colJoiningCode).Value = clsCommon.myCstr(dt.Rows(i)("Chk_Code"))
    ...
    Private Sub FrmJoiningChecklist_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        LoadBlankJoining()
        isNewEntry = True
    ...
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.JOININGCHECKLIST)
    ...
    Private Function AllowToSave() As Boolean
        Try
            ...
            For Each grow As GridViewRowInfo In gv.Rows
                If CBool(grow.Cells(colReceived).Value) = True Then
                    GridRow = GridRow + 1
                End If
            Next
            If GridRow <= 0 Then
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Dim obj As clsJoiningCheckListHead = clsJoiningCheckListHead.GetData(strCode, NavTyep)
        If obj IsNot Nothing Then
            isNewEntry = False
            fndaccountsetcode.Value = obj.ApplicantCode
            Txtdate.Text = obj.Joiningdate
            ...
            UcRequisitionDetail1.RefreshData()
            Dim qry As Integer = clsDBFuncationality.getSingleValue("select Rejected  from TSPL_HR_JOINING_HEAD where APPLICANT_CODE ='" + fndaccountsetcode.Value + "'")
            If qry = 1 Then
                UsLock1.Status = obj.Rejected
                btnPost.Enabled = False
                btnRejected.Enabled = False
                btnSave.Enabled = False
            ...
            If Not IsNothing(obj.ObjList) Then
                ...
                For Each objtr As clsJoiningDetail In obj.ObjList


                    gv.Rows.AddNew()
                    gv.Rows(counter).Cells(colReceived).Value = objtr.Received
                    gv.Rows(counter).Cells(colJoiningMandatory).Value = objtr.JoiningMandatory
                    ...
                    gv.Rows(counter).Cells(colJoiningCode).Value = objtr.Chk_code
    ...
    Sub SaveData()
        ...
        Try
            If (AllowToSave()) Then
                gv.EndEdit()
                Dim objHead As clsJoiningCheckListHead
                objHead = New clsJoiningCheckListHead
                objHead.ApplicantCode = clsCommon.myCstr(fndaccountsetcode.Value)
                objHead.Joiningdate = clsCommon.myCDate(Txtdate.Value)

                Dim objList As New List(Of clsJoiningDetail)
                Dim objListAttachment As New List(Of clsJoiningAttachment)



                For Each grow As GridViewRowInfo In gv.Rows
                    Dim obj As clsJoiningDetail
                    obj = New clsJoiningDetail
                    ...
                    If clsCommon.myLen(grow.Cells(colAttachmentpath).Value) > 0 And clsCommon.myCstr(grow.Cells(colAttachmentpath).Value).ToString.Contains("\") Then
                        Dim objAttachment As clsJoiningAttachment

                        objAttachment = New clsJoiningAttachment
                        objAttachment.Transaction_ID = clsCommon.myCstr(grow.Cells(colJoiningCode).Value)
                        ...
                        objAttachment.ColTransactionId = clsCommon.myCstr(grow.Cells(colJoiningCode).Value)
                    ...
                    Else
                        objJoin.Chk_code = clsCommon.myCstr(grow.Cells(colJoiningCode).Value)
                        ...
                        objJoin.JoiningMandatory = clsCommon.myCdbl(grow.Cells(colJoiningMandatory).Value)
                        'objJoin.Attachmentid = clsCommon.myCdbl(grow.Cells(colAttachmentId).Value)
                        objList.Add(objJoin)
    ...
    Private Sub fndaccountsetcode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndaccountsetcode._MYValidating
        Try
            'Dim qry As String = "select APPLICANT_CODE As Code from TSPL_HR_OFFER_LETTER"
            'fndaccountsetcode.Value = clsCommon.ShowSelectForm("Applicant_Code", qry, "Code", "Posted=1 ", fndaccountsetcode.Value, "", isButtonClicked)
            fndaccountsetcode.Value = ClsOfferLetterHR.GetFinder(" ", fndaccountsetcode.Value, isButtonClicked)
            If clsCommon.myLen(fndaccountsetcode.Value) > 0 Then
                LoadData(fndaccountsetcode.Value, NavigatorType.Current)
    ...
    Private Sub gv_CellClick(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.GridViewCellEventArgs) Handles gv.CellClick
        Try
            If e.Column Is gv.Columns(colAttachment) Then
                ...
                End If
                If clsCommon.myCBool(gv.CurrentRow.Cells(colReceived).Value) = True And clsCommon.myCBool(gv.CurrentRow.Cells(colJoiningMandatory).Value) = True Then
                    gv.CurrentRow.Cells(colAttachmentpath).Value = openFileDialog1.FileName
    ...
    Private Sub FrmJoiningChecklist_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub RSaveLayout_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RSaveLayout.Click
    ...
    Private Sub gv_ConditionalFormattingFormShown(ByVal sender As Object, ByVal e As System.EventArgs) Handles gv.ConditionalFormattingFormShown
