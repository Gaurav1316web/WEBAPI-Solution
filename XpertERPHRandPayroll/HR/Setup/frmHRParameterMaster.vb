...
Public Class FrmHRParameterMaster
...
#Region "Functions"
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.frmHRParameterMaster)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Dim obj As ClsHRParameterMaster = ClsHRParameterMaster.GetData(strCode, NavTyep)
        If obj IsNot Nothing Then
            AddNew()
            isNewEntry = False
            txtcode.Value = obj.Parameter_Code
            txtdesp.Text = obj.Parameter_Name
    ...
    Sub SaveData()
        ...
        Try
            If AllowToSave() Then
                ...
                Dim obj As New ClsHRParameterMaster()
                obj.Parameter_Code = txtcode.Value
                obj.Parameter_Name = txtdesp.Text
                Dim qry As Integer = clsDBFuncationality.getSingleValue("select count(Parameter_Code) from TSPL_HR_PARAMETER_MASTER where Parameter_Code='" + obj.Parameter_Code + "'")
                If (qry = 0) Then
                    isNewEntry = True
                Else
                    isNewEntry = False
                End If
                If (ClsHRParameterMaster.SaveData(obj, isNewEntry)) Then
                    '                    trans.Commit()
                    clsCommon.MyMessageBoxShow(Me, "Data saved Successfully", Me.Text)
                    LoadData(obj.Parameter_Code, NavigatorType.Current)
                    btnsave.Text = "Update"
                    btndelete.Enabled = True
                Else
                    btnsave.Text = "Save"
    ...
    Private Sub DeleteData()
        Try
            ...
            If clsCommon.MyMessageBoxShow("are you sure? do you want to delete this Code ('" + txtcode.Value + "')", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Dim qry As String = "DELETE FROM TSPL_HR_PARAMETER_MASTER WHERE Parameter_Code='" + txtcode.Value + "'"
...
#Region "Events"
    ...
    Private Sub txtcode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtcode._MYValidating
        ...
        '    Dim qry As String = "select Parameter_Code As Code,Description from  TSPL_HR_PARAMETER_MASTER"
        '    txtcode.Value = clsCommon.ShowSelectForm("TSPL_HR_PARAMETER_MASTER", qry, "Code", "", txtcode.Value, "", isButtonClicked)
        '    LoadData(txtcode.Value, NavigatorType.Current)
        'End If


        Dim str As String = "select count(*) from TSPL_HR_PARAMETER_MASTER where Parameter_Code ='" + txtcode.Value + "' "
        ...
        If txtcode.MyReadOnly OrElse isButtonClicked Then
            Dim qry As String = ""
            qry = "Select Parameter_Code As [Code],Parameter_Name As [Parameter_Name] from TSPL_HR_PARAMETER_MASTER"
            txtcode.Value = clsCommon.ShowSelectForm("PAR", qry, "Code", "", txtcode.Value, "TSPL_HR_PARAMETER_MASTER.Parameter_Code", isButtonClicked)
            If clsCommon.myLen(txtcode.Value) > 0 Then
                Dim objOT As ClsHRParameterMaster
                objOT = ClsHRParameterMaster.GetData(txtcode.Value, NavigatorType.Current)
        ...
        If transportSql.importExcel(gv, "Code", "Description") Then
            ...
            Try
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.myLen(strCode) > 0 AndAlso clsDBFuncationality.getSingleValue("Select count(*) from TSPL_HR_PARAMETER_MASTER where Parameter_Code='" + strCode + "' ") > 0 Then
                        IsNewEntry = False
                    Else
                        IsNewEntry = True
                    ...
                    obj.Parameter_Code = strCode
                    obj.Parameter_Name = strDescription
                    ClsHRParameterMaster.SaveData(obj, IsNewEntry)
    ...
    Private Sub RMExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RMExport.Click
        Dim str As String
        str = "select Parameter_Code as [Code],Parameter_Name As [Description]  from TSPL_HR_PARAMETER_MASTER "
