...
Public Class FrmRelationMaster
...
#Region "Functions"
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.frmRelationMaster)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Dim obj As ClsRelationMaster = ClsRelationMaster.GetData(strCode, NavTyep)
        If obj IsNot Nothing Then
            isNewEntry = False
            AddNew()
            txtcode.Value = obj.Relation_Code
            txtdesp.Text = obj.Relation_Name
    ...
    Sub SaveData()
        ...
        Try
            If AllowToSave() Then
                ...
                Dim obj As New ClsRelationMaster()
                obj.Relation_Code = clsCommon.myCstr(txtcode.Value)
                obj.Relation_Name = clsCommon.myCstr(txtdesp.Text)
                ...
                Else
                    isNewEntry = False
                End If
                If (ClsRelationMaster.SaveData(obj, isNewEntry)) Then
                    ' trans.Commit()
                    clsCommon.MyMessageBoxShow("Data saved Successfully", Me.Text)
                    LoadData(obj.Relation_Code, NavigatorType.Current)
                    btnsave.Text = "Update"
                    btndelete.Enabled = True
                Else
                    btnsave.Text = "Save"
    ...
    Private Sub DeleteData()
        Try
            ...
            If clsCommon.MyMessageBoxShow("are you sure? do you want to delete this Code ('" + txtcode.Value + "')", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Dim qry As String = "DELETE FROM TSPL_HR_RELATION_MASTER WHERE Relation_Code='" + txtcode.Value + "'"
...
#End Region
    ...
    Private Sub FrmRelationMaster_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub FrmRelationMaster_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        SetUserMgmtNew()
        isNewEntry = True
    ...
    Private Sub txtcode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtcode._MYValidating
        Dim str As String = "select count(*) from TSPL_HR_RELATION_MASTER where Relation_Code ='" + txtcode.Value + "' "
        ...
        If txtcode.MyReadOnly OrElse isButtonClicked Then
            Dim qry As String = ""
            qry = "Select Relation_Code As [Code],Relation_Name As [Relation_Name] from TSPL_HR_RELATION_MASTER"
            txtcode.Value = clsCommon.ShowSelectForm("REL", qry, "Code", "", txtcode.Value, "TSPL_HR_RELATION_MASTER.Relation_Code", isButtonClicked)
            If clsCommon.myLen(txtcode.Value) > 0 Then
                Dim objOT As ClsRelationMaster
                objOT = ClsRelationMaster.GetData(txtcode.Value, NavigatorType.Current)
        ...
        If transportSql.importExcel(gv, "Code", "Description") Then
            ...
            Try
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.myLen(strCode) > 0 AndAlso clsDBFuncationality.getSingleValue("Select count(*) from TSPL_HR_RELATION_MASTER where Relation_Code='" + strCode + "' ") > 0 Then
                        IsNewEntry = False
                    Else
                        IsNewEntry = True
                    ...
                    obj.Relation_Code = strCode
                    obj.Relation_Name = strDescription
                    ClsRelationMaster.SaveData(obj, IsNewEntry)
    ...
    Private Sub RMExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RMExport.Click
        Dim str As String
        str = "select  Relation_Code as [Code],Relation_Name As [Description]  from TSPL_HR_RELATION_MASTER "
