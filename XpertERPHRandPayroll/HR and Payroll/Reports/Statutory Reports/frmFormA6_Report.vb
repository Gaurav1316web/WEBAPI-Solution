...
#End Region
    ...
    Private Sub frmFormA6_Report_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
    ...
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.frmFormA6_Report)
    ...
    Sub PrintData()
        Try
            ...
            Dim Qry As String = ""
            Qry = ""
            Qry += " select MAX(T3.PF_NO) AS PF_NO, T1.EMP_CODE, MAX(T3.Emp_Name) AS Emp_Name, (SELECT (ISNULL(Add1,'')+' ' +ISNULL(Add2,'')+' '+ISNULL(Add3,'')) AS 'add'  FROM TSPL_COMPANY_MASTER WHERE TSPL_COMPANY_MASTER.Comp_Code = '" + objCommonVar.CurrentCompanyCode + "') as 'Company_Address', "
            Qry += " SUM(T1.HEAD_VALUE) AS HEAD_VALUE,  "
            Qry += " SUM(T1.ACTUAL_AMOUNT) AS ACTUAL_AMOUNT, "
            Qry += " SUM(T2.PAYABLE_DAYS) AS PAYABLE_DAYS,"
            Qry += " SUM ((CASE WHEN T4.EPS_TO_EPF =0 THEN T1.ACTUAL_AMOUNT ELSE (CASE WHEN (T1.HEAD_VALUE*.0833) > 541 THEN (T1.ACTUAL_AMOUNT-541) ELSE (T1.ACTUAL_AMOUNT-(T1.HEAD_VALUE*.0833)) END)END)) AS COPF,"
            Qry += " SUM((CASE WHEN T4.EPS_TO_EPF =0 THEN 0.00 ELSE (CASE WHEN (T1.HEAD_VALUE*.0833) < 541 THEN (T1.HEAD_VALUE*.0833) ELSE 541 END)END)) AS PENSION , "
            Qry += " '" + objCommonVar.CurrentCompanyName + "' AS 'Company_Name',  '" + Max_date + "' as Max_date, '" + Min_date + "' as Min_date  "
            ...
            Qry += " WHERE (T7.DATE_FROM  BETWEEN CONVERT(DATE, '" + Min_date + "',103)  AND CONVERT(DATE, '" + Max_date + "',103))  AND  (T7.DATE_TO BETWEEN CONVERT(DATE, '" + Min_date + "',103)  AND CONVERT(DATE, '" + Max_date + "',103)) AND   T5.SUB_HEAD_TYPE  = 'EPF' and T3.ISPF =1 "
            Qry += " AND T1.EMP_CODE IN (" + clsCommon.GetMulcallString(cbgLocation.CheckedValue) + ")  group by T1.EMP_CODE "
