...
"
    ...
    Sub SetGridFormat()
        ...
        gv1.Columns("KKK Id").HeaderText = "KKK Id"
        gv1.Columns("KKK Id").Width = 500
        gv1.Columns("KKK Id").IsVisible = True
        ...
        gv1.Columns("stotal_payable").HeaderText = "Gross Amount"
        gv1.Columns("stotal_payable").IsVisible = True
        gv1.Columns("stotal_payable").FormatString = "{0:n3}"
        ...
        Try
            ...
            Dim qry As String = "  SELECT CONCAT('" + fndDeductioncode.Value + "','  ','DEDUCTION OF EMPLOYEES FOR PERIOD',' ') + CONCAT(GS.Pay_Period_Code, '-', (final.GENERATE_DATE)) AS [Pay Period]
                    ,Final.* from (SELECT Sex,Designation,Membership_id,Special_desc,
                     LIC_No, Policy_no,generate_date,kkk_id,
                     SALARY_GENERATION_CODE,EMP_CODE AS EMP_CODE,EMPLOYEE_NAME,FATHERS_NAME AS [Father Name],Birth_date AS [Date of Birth],Joining_date as [Joining Date],LOCATION_DESC AS Location, SUM(gsli) AS vgsli,sum(stotal_payable)stotal_payable FROM (SELECT ACD.SEX,ACD.Membership_id,ACD.Special_desc,
                     ACD.LIC_No,acd.Policy_no,acd.GENERATE_DATE,acd.kkk_id,
                     ACD.SALARY_GENERATION_CODE,ACD.EMP_CODE,ACD.PAY_HEAD_CODE,EMPLOYEE_NAME,FATHERS_NAME,[Working City],Birth_date,Joining_date,Designation,Department,LOCATION_DESC,DEVISION_NAME, BANK_NAME, Bank_Branch,Bank_Branch_Name,PAYMENT_MODE, ACD.PAYPERIOD_DAYS, ACD.PRESENT_DAYS, ACD.PAYABLE_DAYS, ACD.HOLIDAY_DAYS,ACD.WEEKLY_OFF_DAYS,ACD.LEAVE_DAYS ,PF_NO,ESI_NO,BANK_ACC_NO,RELIEVING_DATE,ACD.OT_HOURS, CASE WHEN  ACD.PAY_HEAD_CODE ='" + fndDeductioncode.Value + "' THEN acd.ACTUAL_AMOUNT else 0 END  AS gsli,ACD.stotal_payable FROM  ( SeleCT T2.SEX,t2.Membership_id,t2.Special_desc,
                     t2.LIC_No,t2.Policy_no,t1.GENERATE_DATE,t2.kkk_id,
                     T1.SALARY_GENERATION_CODE,T1.EMP_CODE,T2.EMP_NAME AS EMPLOYEE_NAME,T2.FATHERS_NAME,T2.WORKING_LOCATION_CODE,Working_City.City_Name as [Working City],T3.DESIGNATION_DESC AS DESIGNATION,T4.DEPARTMENT_NAME AS DEPARTMENT, T1.DEVISION_CODE,T1.LOCATION_CODE,LOC.LOCATION_DESC,Dev.DEVISION_NAME,T2.Bank_Name,T2.Bank_Branch,T2.Bank_Branch_Name,T2.PAYMENT_MODE_New as PAYMENT_MODE,T2.Birth_date,T2.Joining_date,T2.RELIEVING_DATE,T2.BRANCH_CODE,T1.PAY_HEAD_CODE , T1.ACTUAL_AMOUNT,T1.Payable_Amount,T1.PAYPERIOD_DAYS ,T1.PRESENT_DAYS ,T1.PAYABLE_DAYS,T1.HOLIDAY_DAYS,T1.WEEKLY_OFF_DAYS,T1.LEAVE_DAYS,T2.PF_NO,T2.ESI_NO,T2.BANK_ACC_NO,T1.OT_HOURS,T1.stotal_payable FROM (SELECT YEAR(t1.GENERATE_DATE)GENERATE_DATE,T1.SALARY_GENERATION_CODE,T2.EMP_CODE,T2.PAY_HEAD_CODE,T2.ACTUAL_AMOUNT,T2.Payable_Amount,T5.PAYPERIOD_DAYS,T5.PRESENT_DAYS,T5.PAYABLE_DAYS,T5.HOLIDAY_DAYS,T5.LEAVE_DAYS, (T5.PAYPERIOD_DAYS-T5.PRESENT_DAYS-T5.HOLIDAY_DAYS-T5.LEAVE_DAYS-T5.ABSENT_DAYS) as WEEKLY_OFF_DAYS,T1.LOCATION_CODE,T1.DEVISION_CODE,T2.OT_HOURS,
                     (CASE WHEN T2.PAY_HEAD_CODE = 'BASIC' THEN T2.Payable_Amount ELSE 0 END +
 CASE WHEN T2.PAY_HEAD_CODE = 'CCA' THEN T2.Payable_Amount ELSE 0 END +
 CASE WHEN T2.PAY_HEAD_CODE = 'DA' THEN T2.Payable_Amount ELSE 0 END +
 CASE WHEN T2.PAY_HEAD_CODE = 'HRA' THEN T2.Payable_Amount ELSE 0 END +
