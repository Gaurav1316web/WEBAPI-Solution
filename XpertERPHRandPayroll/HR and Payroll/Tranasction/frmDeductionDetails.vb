...
Public Class frmDeductionDetails
    ...
    Function AllowToSave() As Boolean
        ...
        Dim StrQry As String = "select TSPL_DEDUCTION_DETAIL.PAY_HEAD_CODE from TSPL_DEDUCTION_DETAIL
        ...
        'If dtEmp.Rows.Count > 0 AndAlso dtEmp.Columns.Contains("PAY_HEAD_CODE") Then
        '    For Each grow As GridViewRowInfo In gvDeduction.Rows
        '        For Each row As DataRow In dtEmp.Rows
        '            If clsCommon.myCdbl(grow.Cells(colDeductionAmount).Value) > 0 Then
        '                If clsCommon.CompairString(clsCommon.myCstr(grow.Cells(colpayHeadCode).Value), row("PAY_HEAD_CODE")) = CompairStringResult.Equal Then
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtCode.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowTransHistoryData(txtCode.Value, "DEDUCTION_CODE", "TSPL_DEDUCTION", "TSPL_DEDUCTION_DETAIL")
        Catch ex As Exception
