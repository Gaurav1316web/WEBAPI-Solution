...
Public Class RptPFForm6
    ...
    Sub LoadReportForm6()
        ...
        Qry = ""
        Qry += " SELECT max(Comp_Name) as Comp_Name,MAX(PF_NO ) AS PF_NO,max(Comp_PF_NO) as Comp_PF_NO,MAX(Emp_Name) AS Emp_Name,SUM(HEAD_VALUE) AS TotalAmountWages,SUM(ACTUAL_AMOUNT) AS TotalWorkerShare,SUM(Pension_Fund) As TotalPensionFund ,max(EPF_Rate) As EPF_Rate,max(RATE_AMOUNT) as Employee_PF_Per,SUM(COEPF_PER) AS TotalEmployerShare,MAX(Location_Code) As Location ,MAX(Add1 ) AS Add1 ,MAX(Add2) AS Add2,MAX(Location_Desc) AS Location_Desc,MAX(Address) As [Address],EMP_CODE FROM (" & _
                " select *,'" & objCommonVar.CurrentCompanyName & "' as CompName,(pension_fund + Diff)as Total from  " & _
                " (select DISTINCT *,(ACTUAL_AMOUNT-pension_fund)as Diff from " & _
                " (select TSPL_DEVISION_MASTER.DEVISION_CODE ,TSPL_DEVISION_MASTER.DEVISION_NAME ,TSPL_LOCATION_MASTER .Location_Code , Emp_Name,TSPL_LOCATION_MASTER.PF_NO as Comp_PF_NO,TSPL_EMPLOYEE_MASTER.PF_NO , " & _
                " (case when coalesce(TSPL_GENERATE_SALARY_PAYHEADS.EPF_RATE,0)>0 then (ACTUAL_AMOUNT-(CoEPS_AMT_AC10+CoEPF_AMT_AC01)) else 0 end) as VPF,(case when coalesce(TSPL_GENERATE_SALARY_PAYHEADS.EPF_RATE,0)<=0 then ACTUAL_AMOUNT else (CoEPS_AMT_AC10+CoEPF_AMT_AC01) end) as ACTUAL_AMOUNT,ROUND(case when HEAD_VALUE>PF_MAX_LM then PF_MAX_LM else HEAD_VALUE end,0)AS HEAD_VALUE,TSPL_COMPANY_MASTER.Comp_Name,TSPL_COMPANY_MASTER.Add1,TSPL_COMPANY_MASTER.Add2,TSPL_COMPANY_MASTER.City_Code , TSPL_GENERATE_SALARY.pay_period_code as FROM_PAY_PERIOD , TSPL_GENERATE_SALARY_PAYHEADS.CoEPS_AMT_AC10  as pension_fund, (TSPL_GENERATE_SALARY_PAYHEADS.ACTUAL_AMOUNT-TSPL_GENERATE_SALARY_PAYHEADS.CoEPS_AMT_AC10) as COEPF_PER, TSPL_GENERATE_SALARY_PAYHEADS.ADMIN_AMT_AC02 as ACCOEPF_PER, TSPL_GENERATE_SALARY_PAYHEADS.EDLI_AMT_AC21 as COEDLI_PER, TSPL_GENERATE_SALARY_PAYHEADS.EDLI_AMT_AC21 as ACCOEDLI_PER, TSPL_LOCATION_MASTER.Location_Desc , TSPL_LOCATION_MASTER.Add1+Case When ISNULL(TSPL_LOCATION_MASTER.Add2,'')='' Then '' else ', '+TSPL_LOCATION_MASTER.Add2+ Case When ISNULL(TSPL_LOCATION_MASTER.Add3,'')='' Then '' Else ', '+TSPL_LOCATION_MASTER.Add3+ Case When ISNULL(TSPL_STATE_MASTER.State_Name ,'')='' Then '' else '-'+CONVERT(varchar, TSPL_STATE_MASTER.State_Name) End End End As Address " & _
                " ,TSPL_GENERATE_SALARY_PAYHEADS.EPF_RATE As [EPF_Rate],ISNULL(CoEPF_Rate_Ac01,0)+ISNULL(CoEPS_RATE_AC10,0) AS RATE_AMOUNT,TSPL_EMPLOYEE_MASTER.EMP_CODE " & _
                ...
                " left join TSPL_DEVISION_MASTER on TSPL_DEVISION_MASTER.DEVISION_CODE =TSPL_GENERATE_SALARY.DEVISION_CODE   " & _
                " left join TSPL_DEVISION_MASTER as TSPL_DEVISION_MASTER_For_Employee  on TSPL_DEVISION_MASTER_For_Employee.DEVISION_CODE =TSPL_EMPLOYEE_MASTER.DEVISION_CODE   " & _
                ...
                " WHERE TSPL_GENERATE_SALARY_PAYHEADS.SUB_HEAD_TYPE='EPF' AND TSPL_GENERATE_SALARY_PAYHEADS.ACTUAL_AMOUNT>0 "
        If txtLocCode.arrValueMember IsNot Nothing AndAlso txtLocCode.arrValueMember.Count > 0 Then
        ...
        Qry += " )as d)as m) AS Final " & _
                " left join tspl_payperiod_master ppm on final.FROM_PAY_PERIOD=ppm.pay_period_code " & _
                " where ppm.DATE_FROM between '01/APR/" & txtFromYear.Value.Year & "' and '31/MAR/" & (txtToYear.Value.Year + 1) & "' " & _
