...
Public Class RptESIHalfYearly
    Inherits FrmMainTranScreen
    Dim ButtonToolTip As ToolTip = New ToolTip()
    Dim isInsideLoadData As Boolean = False
    Dim Errorcontrol As clsErrorControl = New clsErrorControl()
    Private Sub SetUserMgmtNew()
        'MyBase.SetUserMgmt(clsUserMgtCode.RptESIHalfYearly)
    ...
    Sub LoadData()
        Try
            ...
            If cbMonth.Text = "OCT-MAR" Then
                ...
                '    InnerQry += " and TSPL_GENERATE_SALARY.DEVISION_CODE  in (" + clsCommon.GetMulcallString(txtDivisionMult.arrValueMember) + ") "
                'End If
                'InnerQry += " group by Date_To,TSPL_GENERATE_SALARY_PAYHEADS.SALARY_GENERATION_CODE    )detail on head.SALARY_GENERATION_CODE=detail.SALARY_GENERATION_CODE)  AS Final"
                'InnerQry += " where convert(date,Date_Of_Challan,103)>=convert(date,'" & From_Date.ToString & "',103) and convert(date,Date_Of_Challan,103) <=convert(date,'" & To_Date.ToString & "' ,103) group by Final.Emp_Name,Final.ESI_NO,Final.Month,Final.Date_Of_Challan ,Final.PAYABLE_DAYS ,Final.Average_Daily_Wages ,Final.Joining_date ,Final.Left_Date ,Final .Still_Working,final.HEAD_VALUE ,Location,Loc_Address,ACTUAL_Amount1) xx"
                'InnerQry += " union all  select Top 1 Emp_Name,ESI_NO ,'October_Amt','October_Date'  , 0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''  from TSPL_EMPLOYEE_MASTER  union all select Top 1 Emp_Name,ESI_NO,'November_Amt','November_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''   from TSPL_EMPLOYEE_MASTER  union all  select Top 1 Emp_Name,ESI_NO,'December_Amt','December_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''   from TSPL_EMPLOYEE_MASTER  union all select Top 1 Emp_Name,ESI_NO,'January_Amt','January_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0 ,'','','','',''   from TSPL_EMPLOYEE_MASTER  union all select Top 1 Emp_Name,ESI_NO,'February_Amt','February_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''    from TSPL_EMPLOYEE_MASTER  union  all select Top 1 Emp_Name,ESI_NO,'March_Amt','March_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''    from TSPL_EMPLOYEE_MASTER  "
                'Dim TotalActual1 As String = " select sum(total_actual)as total_actual from(" + InnerQry + ")as m "
                'Dim TotalActual As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(TotalActual1))

                'Dim mainqry As String = "select '" + CompESIC + "' as Comp_ESIC_No,'" + Comp_Name + "' as comp_Name,ESI_No,Emp_Name ,No_Of_Days ,'" & Employershare & "' as Employer_Total ,Average_Daily_Wages ,'April' as From_Month,'September' as To_Month ,Joining_date,Left_Date ,Still_Working ,HEAD_VALUE ,Emp_Actual ,From_Date ,To_date ,From_Year ,To_Year,Location "
                'mainqry += " ,October_Date,November_Date,December_Date, January_Date, February_Date,March_Date    ,convert(decimal(18,2),(coalesce(October_Amt,0)+coalesce(November_Amt,0)+coalesce(December_Amt,0)+coalesce(January_Amt,0)+coalesce(February_Amt,0)+coalesce(March_Amt,0)+coalesce(Arrear,0))) as TotalAmountPaid,isnull(October_Amt,0)as October_Amt,isnull(November_Amt ,0)as November_Amt,isnull(December_Amt ,0)as December_Amt ,isnull(January_Amt  ,0)as January_Amt,isnull(March_Amt  ,0)as March_Amt,isnull(February_Amt  ,0)as  February_Amt "
                'mainqry += " ,isnull(Arrear ,0) as Arrear,Loc_Address,ACTUAL_Amount1,Employer_Name,Employer_desg,Employer_Address,Nearestcity,'" & ESIC & "' as ESIC_No from(select  * from(select  (dd.ESI_NO)as ESI_No,Emp_Name,_Month , D_Month ,Total_Amount,MAX (dd.Date_Of_Challan) as Date_Of_Challan,sum (dd.PAYABLE_DAYS )as No_Of_Days,MAX(dd.Average_Daily_Wages )as Average_Daily_Wages,MAX(dd.Joining_date ) as Joining_date ,MAX(dd.Left_Date )as Left_Date,MAX(dd.Still_Working )as Still_Working"
                'mainqry += " ,MAX (dd.HEAD_VALUE )as HEAD_VALUE,MAX(dd.Emp_Actual)as Emp_Actual,'" & From_Date.ToString & "' as From_Date,'" & To_Date.ToString & "' as To_date,'" + txtFromYear.Text + "' as From_Year,'" + txtFromYear.Text + "' as To_Year ,max(Arrear) as  Arrear,max(Location) as Location,max(Loc_Address)as Loc_Address,max(ACTUAL_Amount1)as ACTUAL_Amount1," & TotalActual & " as total_actual ,max(Employer_Name)as Employer_Name,max(Employer_desg)as Employer_desg,max(Employer_Address)as Employer_Address,max(Nearestcity)as Nearestcity,max(ESIC_No) as ESIC_No  from(" + InnerQry + ") as dd where dd.HEAD_VALUE >0  group by ESI_No, Emp_Name ,_Month,Total_Amount,D_Month) as s   Pivot(  max(total_actual) FOR [_month] IN (October_Amt,November_Amt,December_Amt, January_Amt, February_Amt,March_Amt) )AS pivott )as t Pivot( MAX(Date_Of_Challan) FOR [D_Month] IN (October_Date,November_Date,December_Date,January_Date, February_Date, March_Date) )AS pivotr "
                'Dim dtgv As New DataTable
                'dtgv = clsDBFuncationality.GetDataTable(mainqry)
                'frmCrystalReportViewer.funreport(CrystalReportFolder.HRPayroll, dtgv, "crptESIHalfYearlyForm5", "ESI HAlf Year Report")
                '=====================================
                ...
                InnerQry += "  '" & LocAddress & "' as Loc_Address, round(TSPL_GENERATE_SALARY_PAYHEADS.Co_ESI_AMT,0) as ACTUAL_Amount1 ,Employer_Name,Employer_desg,(Employer_Add1+','+ Employer_Add2 + ',' + Employer_Add3) as Employer_Address,Nearestcity,ESIC_No from TSPL_GENERATE_SALARY_PAYHEADS " &
                            ...
                            "   left outer join TSPL_DEVISION_MASTER on TSPL_DEVISION_MASTER.DEVISION_CODE  =TSPL_GENERATE_SALARY.DEVISION_CODE   " &
                             " left join TSPL_GENERATE_SALARY_ATTENDANCE on TSPL_GENERATE_SALARY_ATTENDANCE.SALARY_GENERATION_CODE=TSPL_GENERATE_SALARY.SALARY_GENERATION_CODE and  TSPL_GENERATE_SALARY_ATTENDANCE.Emp_Code=TSPL_GENERATE_SALARY_PAYHEADS.Emp_code " &
            ...
            ElseIf cbMonth.Text = "APR-SEP" Then
                ...
                'Dim InnerQry As String = " select xx.Emp_Name ,xx.ESI_NO ,xx.Month+'_Amt' as _Month,xx.Month+'_Date' as D_Month  ,xx.Emp_Actual ,xx.Date_Of_Challan ,xx.PAYABLE_DAYS ,xx.Average_Daily_Wages ,xx.Joining_date ,xx.Left_Date ,xx.Still_Working ,xx.HEAD_VALUE,17827 as Total_Amount,xx.Arrear ,xx.Location,xx.Loc_Address,xx.ACTUAL_Amount1,xx.total_actual,xx.Employer_Name,xx.Employer_desg,xx.Employer_Address,xx.Nearestcity,xx.ESIC_No "
                'InnerQry += " from(SELECT  SUM(case when SUB_HEAD_TYPE='ARREAR' THEN actual_amount ELSE 0 end) AS Arrear, Final.Emp_Name,Final.ESI_NO,Final.Month,sum( ACTUAL_AMOUNT ) as Emp_Actual,SUM(actual_amount+ACTUAL_Amount1) as total_actual,convert (varchar ,Final.Date_Of_Challan,103) as Date_Of_Challan ,Final.PAYABLE_DAYS ,Final.Average_Daily_Wages ,Final.Joining_date ,Final.Left_Date ,Final .Still_Working,final.HEAD_VALUE  ,Final.Location,final.Loc_Address,ACTUAL_Amount1,max(Employer_Name)  as Employer_Name,max(Employer_desg)as Employer_desg,max(Employer_Address)as Employer_Address,max(Nearestcity) as Nearestcity,max(ESIC_No) as ESIC_No FROM (select head.ESI_NO,head.Emp_Name,head.HEAD_VALUE,detail.Month,RATE_AMOUNT,head.ACTUAL_AMOUNT ,detail.Date_Of_Challan,head.PAYABLE_DAYS,head.Average_Daily_Wages,head.SUB_HEAD_TYPE,head.Joining_date ,head.Left_Date, case when ISNULL(head.Left_Date,'')='' then 'Yes' else 'No' end as Still_Working ,Location,Loc_Address,ACTUAL_Amount1,Employer_Name,Employer_desg,Employer_Address ,Nearestcity,ESIC_No from (select TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Add1,TSPL_COMPANY_MASTER. add2,TSPL_COMPANY_MASTER.City_Code, TSPL_EMPLOYEE_MASTER.ESI_NO,Emp_Name,Joining_date ,RELIEVING_DATE as Left_Date,RATE_AMOUNT ,ACTUAL_AMOUNT,HEAD_VALUE,SUB_HEAD_TYPE,TSPL_GENERATE_SALARY_PAYHEADS.SALARY_GENERATION_CODE,PAYABLE_DAYS,case when PAYABLE_DAYS = 0 then 0 else (HEAD_VALUE/PAYABLE_DAYS) end as Average_Daily_Wages ,Location_Desc as Location ,"
                'InnerQry += "  '" & LocAddress & "' as Loc_Address,Co_ESI_AMT as ACTUAL_Amount1 ,Employer_Name,Employer_desg,(Employer_Add1+','+ Employer_Add2 + ',' + Employer_Add3) as Employer_Address,Nearestcity,ESIC_No from TSPL_GENERATE_SALARY_PAYHEADS left join TSPL_EMPLOYEE_MASTER on TSPL_EMPLOYEE_MASTER.EMP_CODE =TSPL_GENERATE_SALARY_PAYHEADS.EMP_CODE left join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code =TSPL_EMPLOYEE_MASTER.Comp_Code   left join TSPL_GENERATE_SALARY_ATTENDANCE on TSPL_GENERATE_SALARY_ATTENDANCE.EMP_CODE =TSPL_EMPLOYEE_MASTER.EMP_CODE left join TSPL_GENERATE_SALARY on TSPL_GENERATE_SALARY.SALARY_GENERATION_CODE =TSPL_GENERATE_SALARY_ATTENDANCE.SALARY_GENERATION_CODE left join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=TSPL_GENERATE_SALARY.LOCATION_CODE left join TSPL_STATE_MASTER on TSPL_STATE_MASTER.State_Code=TSPL_LOCATION_MASTER.State left join TSPL_DEVISION_MASTER on  TSPL_DEVISION_MASTER.devision_code=TSPL_GENERATE_SALARY.devision_code where SUB_HEAD_TYPE in ('EMPESI')and ACTUAL_AMOUNT > 0 "
                'If txtLocation.arrValueMember IsNot Nothing AndAlso txtLocation.arrValueMember.Count > 0 Then
                '    InnerQry += " and TSPL_GENERATE_SALARY.Location_Code in (" + clsCommon.GetMulcallString(txtLocation.arrValueMember) + ") "
                'End If
                'If txtDivisionMult.arrValueMember IsNot Nothing AndAlso txtDivisionMult.arrValueMember.Count > 0 Then
                ...
                'InnerQry += " group by Date_To,TSPL_GENERATE_SALARY_PAYHEADS.SALARY_GENERATION_CODE    )detail on head.SALARY_GENERATION_CODE=detail.SALARY_GENERATION_CODE)  AS Final"
                'InnerQry += " where convert(date,Date_Of_Challan,103)>=convert(date,'" & From_Date.ToString & "',103) and convert(date,Date_Of_Challan,103) <=convert(date,'" & To_Date.ToString & "' ,103) group by Final.Emp_Name,Final.ESI_NO,Final.Month,Final.Date_Of_Challan ,Final.PAYABLE_DAYS ,Final.Average_Daily_Wages ,Final.Joining_date ,Final.Left_Date ,Final .Still_Working,final.HEAD_VALUE ,Location,Loc_Address,ACTUAL_Amount1) xx"
                'InnerQry += " union   select Top 1 Emp_Name,ESI_NO ,'April_Amt','April_Date'  , 0,'',0,0,'','','',0,0,0,'','',0,0,'','','','','' from TSPL_EMPLOYEE_MASTER where ESI_NO > 0 and ESI_NO <> '' union select Top 1 Emp_Name,ESI_NO,'May_Amt','May_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''  from TSPL_EMPLOYEE_MASTER where ESI_NO > 0 and ESI_NO <> '' union  select Top 1 Emp_Name,ESI_NO,'June_Amt','June_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''  from TSPL_EMPLOYEE_MASTER where ESI_NO > 0 and ESI_NO <> '' union select Top 1 Emp_Name,ESI_NO,'July_Amt','July_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''   from TSPL_EMPLOYEE_MASTER where ESI_NO > 0 and ESI_NO <> '' union select Top 1 Emp_Name,ESI_NO,'August_Amt','August_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''   from TSPL_EMPLOYEE_MASTER where ESI_NO > 0 and ESI_NO <> '' union select Top 1 Emp_Name,ESI_NO,'September_Amt','September_Date' ,0,'',0,0,'','','',0,0,0,'','',0,0,'','','','',''   from TSPL_EMPLOYEE_MASTER where ESI_NO > 0  and ESI_NO <> ''"
                'Dim TotalActual1 As String = " select sum(total_actual)as total_actual from(" + InnerQry + ")as m "
                'Dim TotalActual As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(TotalActual1))
                ...
                'mainqry += " ,convert(decimal(18,2),(coalesce(April_Amt,0)+coalesce(May_Amt,0)+coalesce(June_Amt,0)+coalesce(July_Amt,0)+coalesce(September_Amt,0)+coalesce(August_Amt,0)+coalesce(Arrear,0))) as TotalAmountPaid,isnull(April_Amt,0)as April_Amt,isnull(May_Amt ,0)as May_Amt,isnull(June_Amt ,0)as June_Amt ,isnull(July_Amt  ,0)as July_Amt,isnull(August_Amt  ,0)as August_Amt,isnull(September_Amt  ,0)as "
