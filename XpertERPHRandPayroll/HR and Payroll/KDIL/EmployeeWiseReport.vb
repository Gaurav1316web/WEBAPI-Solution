...
Public Class EmployeeWiseReport
    ...
    Const ReportID As String = "EmployeeWiseReport"
...
#End Region
    ...
    Sub LoadData()
        ...
        Dim dt As DataTable
        Dim qry12 As String = "SELECT PAY_PERIOD_CODE AS Code " _
        & " FROM TSPL_PAYPERIOD_MASTER  where POSTED=1 and DATE_FROM>=convert(date,'" & txtFromDate.Value & "',103) and DATE_TO<=convert(date,'" & txtToDate.Value & "',103) order by Date_From"
        dt = clsDBFuncationality.GetDataTable(qry12)
        Dim list As ArrayList = Nothing
        list = New ArrayList()
        For Each row As DataRow In dt.Rows
            list.Add(row("Code"))
        Next
        txtPayPeriod.arrValueMember = list
        ...
        End If
        dt = clsSalaryGeneration.GetEmployeeMISReportData(txtPayPeriod.arrValueMember, txtLocationMult.arrValueMember, txtDivisionMult.arrValueMember, PMCond, txtMultEmployee.arrValueMember, txtFromDate.Value, txtToDate.Value, False)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            ...
            For Each dr As DataColumn In dt.Columns
                ...
                ElseIf clsCommon.CompairString(dr.ColumnName, "Co_ESI_AMT") = CompairStringResult.Equal Then
                    ...
                    Dim item1 As New GridViewSummaryItem(dr.ColumnName, "{0:F2}", GridAggregateFunction.Sum)
                    summaryRowItem.Add(item1)
                ElseIf clsCommon.CompairString(dr.ColumnName, "CTC") = CompairStringResult.Equal Then
                    gv1.Columns("CTC").IsVisible = False
                    gv1.Columns("CTC").Width = 100
                    gv1.Columns("CTC").HeaderText = "CTC"
                    Dim item1 As New GridViewSummaryItem(dr.ColumnName, "{0:F2}", GridAggregateFunction.Sum)
                    summaryRowItem.Add(item1)
                Else
    ...
    Sub funReset()
        ...
        txtMultEmployee.arrValueMember = Nothing
    ...
    Private Sub ExportGrid(ByVal exporter As EnumExportTo)
        Try
            If gv1.Rows.Count > 0 Then
                ...
                arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.EmployeeWiseReport & "'"))
                ...
                End If
                If txtMultEmployee.arrValueMember IsNot Nothing AndAlso txtMultEmployee.arrValueMember.Count > 0 Then
                    arrHeader.Add("Employee : " + clsCommon.GetMulcallStringWithComma(txtMultEmployee.arrDispalyMember))
                End If
    ...
    Private Sub txtPayPeriod__My_Click(sender As Object, e As EventArgs) Handles txtPayPeriod._My_Click
        Dim qry As String = "SELECT PAY_PERIOD_CODE AS Code,(DATEDIFF(DAY,date_from,date_to)+1) as 'Total days', " _
           & " PAY_PERIOD_NAME as Name FROM TSPL_PAYPERIOD_MASTER  where POSTED=1 and DATE_FROM>=convert(date,'" & txtFromDate.Value & "',103) and DATE_TO<=convert(date,'" & txtToDate.Value & "',103) order by Date_From"
    ...
    End Sub
    Private Sub txtMultEmployee__My_Click(sender As Object, e As EventArgs) Handles txtMultEmployee._My_Click
        Dim qry As String = "select EMP_CODE as Code,Emp_Name as Name from TSPL_EMPLOYEE_MASTER where LOCATION_CODE=" & clsCommon.GetMulcallString(txtLocationMult.arrValueMember) & ""
        txtMultEmployee.arrValueMember = clsCommon.ShowMultipleSelectForm("PPMulSel", qry, "Code", "Name", txtMultEmployee.arrValueMember, txtMultEmployee.arrDispalyMember)
