...
Public Class frmMCCMaterialSaleFarmerUploader
    ...
    Public Const colTaxable As String = "colTaxable"
    ...
    Private Sub btnSelectSheet_Click(sender As Object, e As EventArgs) Handles btnSelectSheet.Click
        ...
        If rdbAgainstBulkSale.IsChecked Then
            If transportSql.importExcel(Gv1, "BILL TO LOCATION", "DATE", "FARMER NO", "FARMER NAME", "VLC CODE", "VLC NAME", "ITEM CODE", "ITEM NAME", "QTY", "UOM") Then
                If Gv1.Columns.Count > 0 Then
                    ...
                    DecCol = New GridViewDecimalColumn()
                    DecCol.Name = colRATE
                    DecCol.HeaderText = "RATE"
                    DecCol.ReadOnly = True
                    Gv1.MasterTemplate.Columns.Insert(11, DecCol)

                    DecCol = New GridViewDecimalColumn()
                    DecCol.Name = colAMOUNT
                    DecCol.HeaderText = "AMOUNT"
                    DecCol.ReadOnly = True
                    Gv1.MasterTemplate.Columns.Insert(12, DecCol)

                    TextCol = New GridViewTextBoxColumn()
                    TextCol.Name = colTaxable
                    TextCol.HeaderText = "Taxable"
                    TextCol.ReadOnly = True
                    Gv1.MasterTemplate.Columns.Insert(13, TextCol)
    ...
    Private Sub btnExportFormat_Click(sender As Object, e As EventArgs) Handles btnExportFormat.Click
        ...
        If rdbAgainstBulkSale.IsChecked Then
            qry = "select '' as [BILL TO LOCATION], '' as [DATE], '' as [FARMER NO], '' as [FARMER NAME], '' as [VLC CODE], '' as [VLC NAME], '' as [ITEM CODE], '' as [ITEM NAME], '' as [QTY], '' as UOM"
    ...
    Sub CheckAndValidate()
        ...
        If rdbAgainstBulkSale.IsChecked Then
            For i As Integer = 0 To Gv1.Rows.Count - 1
                ...
                strCellValue = clsCommon.myCstr(Gv1.Rows(i).Cells("FARMER NO").Value)
                'If clsCommon.myLen(strCellValue) <= 0 Then
                ...
                '    Gv1.Rows(i).Cells("FARMER NO").Value = clsCommon.myCstr(CustCode)
                'End If
                If clsCommon.myLen(clsDBFuncationality.getSingleValue("select MP_Code from TSPL_MP_MASTER WHERE MP_Code='" & strCellValue & "'")) <= 0 Then
                    ValidateStatus = ValidateStatus & "Farmer No not found in master" & Environment.NewLine
                End If
                ...
                Dim vlcCode As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select VLCH.VLC_Code_VLC_Uploader as [Vlc Code] from TSPL_MP_MASTER MP left join TSPL_VLC_MASTER_HEAD VLCH on MP.VLC_Code=VLCH.VLC_Code where MP.MP_Code='" & clsCommon.myCstr(Gv1.Rows(i).Cells("FARMER NO").Value) & "' and VLCH.MCC='" & clsCommon.myCstr(Gv1.Rows(i).Cells("BILL TO LOCATION").Value) & "'"))
                ...
                Dim RateFromPriceChart As Double = GetRateMccSale(clsCommon.myCstr(Gv1.Rows(i).Cells("BILL TO LOCATION").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("Item Code").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("UOM").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("Date").Value), Nothing)
                    ...
                    Else
                    Gv1.Rows(i).Cells(colRATE).Value = RateFromPriceChart
                    Gv1.Rows(i).Cells(colAMOUNT).Value = RateFromPriceChart * Gv1.Rows(i).Cells("QTY").Value
                End If
                ...
                Gv1.Rows(i).Cells(colTaxGroup).Value = clsLocationWiseTax.GetExempedDefaultTaxGroup(True, clsCommon.myCstr(Gv1.Rows(i).Cells("BILL TO LOCATION").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("FARMER NO").Value), "S", clsCommon.myCstr(Gv1.Rows(i).Cells("Date").Value))
                    Dim dt As DataTable = clsTaxGroupMaster.GetTaxDetailsByLocation(clsCommon.myCstr(Gv1.Rows(i).Cells(colTaxGroup).Value), "S", clsCommon.myCstr(Gv1.Rows(i).Cells("FARMER NO").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("BILL TO LOCATION").Value))
                    If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                        ...
                        Gv1.Rows(i).Cells(coltax1Amt).Value = (clsCommon.myCdbl(dr("TaxRate")) * clsCommon.myCdbl(Gv1.Rows(i).Cells(colAMOUNT).Value)) / 100
                ...
                '    Gv1.Rows(i).Cells(colTaxGroup).Value = clsLocationWiseTax.GetDefaultTaxGroup(clsCommon.myCstr(Gv1.Rows(i).Cells("BILL TO LOCATION").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("FARMER NO").Value), "S", clsCommon.myCstr(Gv1.Rows(i).Cells("Date").Value))
                '    Dim dt As DataTable = clsTaxGroupMaster.GetTaxDetailsByLocation(clsCommon.myCstr(Gv1.Rows(i).Cells(colTaxGroup).Value), "S", clsCommon.myCstr(Gv1.Rows(i).Cells("FARMER NO").Value), clsCommon.myCstr(Gv1.Rows(i).Cells("BILL TO LOCATION").Value))
                '    If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
                '        Dim j As Integer = 0
                '        For Each dr As DataRow In dt.Rows
                ...
                If rdbAgainstBulkSale.IsChecked Then
                    For Each grow As GridViewRowInfo In Gv1.Rows
                        If clsCommon.myCBool(grow.Cells(colIsValidated).Value) Then
                            ''dt1.Rows.Add("" + clsCommon.myCstr(grow.Cells("Date").Value) + "", "" + clsCommon.myCstr(grow.Cells("BILL TO LOCATION").Value) + "", "" + clsCommon.myCstr(grow.Cells("FARMER NO").Value) + "", "" + clsCommon.myCstr(grow.Cells("VLC CODE").Value) + "", "" + clsCommon.myCstr(grow.Cells("Item Code").Value) + "", "" + clsCommon.myCstr(grow.Cells("QTY").Value) + "", "" + clsCommon.myCstr(grow.Cells("UOM").Value) + "", " " + clsCommon.myCstr(grow.Cells("RATE").Value) + "", "" + clsCommon.myCstr(grow.Cells("AMOUNT").Value) + "", "" + clsCommon.myCstr(grow.Cells("TAXABLE").Value) + "", "" + clsCommon.myCstr(grow.Cells(colTaxGroup).Value) + "", "" + clsCommon.myCstr(grow.Cells(coltax1).Value) + "", "" + clsCommon.myCstr(grow.Cells(coltax1rate).Value) + "", "" + clsCommon.myCstr(grow.Cells(coltax1Amt).Value) + "", "" + clsCommon.myCstr(grow.Cells(coltax2).Value) + "", "" + clsCommon.myCstr(grow.Cells(coltax2rate).Value) + "", "" + clsCommon.myCstr(grow.Cells(coltax2Amt).Value) + "")
                            clsDBFuncationality.ExecuteNonQuery("Insert into Temp_table_MCC_Material_Sale_uploader values ('" + clsCommon.GetPrintDate(grow.Cells("Date").Value, "dd/MMM/yyyy") + "', '" + clsCommon.myCstr(grow.Cells("BILL TO LOCATION").Value) + "', '" + clsCommon.myCstr(grow.Cells("FARMER NO").Value) + "', '" + clsCommon.myCstr(grow.Cells("VLC CODE").Value) + "', '" + clsCommon.myCstr(grow.Cells("Item Code").Value) + "', " + clsCommon.myCstr(grow.Cells("QTY").Value) + ", '" + clsCommon.myCstr(grow.Cells("UOM").Value) + "',  " + clsCommon.myCstr(grow.Cells(colRATE).Value) + ", " + clsCommon.myCstr(grow.Cells(colAMOUNT).Value) + ", '" + clsCommon.myCstr(grow.Cells(colTaxable).Value) + "', '" + clsCommon.myCstr(grow.Cells(colTaxGroup).Value) + "', '" + clsCommon.myCstr(grow.Cells(coltax1).Value) + "', " + clsCommon.myCstr(grow.Cells(coltax1rate).Value) + ", " + clsCommon.myCstr(grow.Cells(coltax1Amt).Value) + ", '" + clsCommon.myCstr(grow.Cells(coltax2).Value) + "', " + clsCommon.myCstr(grow.Cells(coltax2rate).Value) + ", " + clsCommon.myCstr(grow.Cells(coltax2Amt).Value) + ")", trans)
                        End If
                    Next
                End If
            End If
    ...
    Sub InvoiceSaveData(ByVal trans As SqlTransaction)
        ...
        Dim strcountno As String = ""
        Dim objTr As clsMCCMaterialSaleFarmerDetail = Nothing
        ...
        Try
            ...
            For Each dr As DataRow In dtmain.Rows
                ...
                If clsCommon.CompairString(strcountno, clsCommon.myCstr(dr("SrNo"))) <> CompairStringResult.Equal Then
                    ...
                    obj.Farmer_Code = clsCommon.myCstr(dr("Customer"))
                    obj.Farmer_Name = clsCustomerMaster.GetName(clsCommon.myCstr(dr("Customer")), trans)
                    obj.Bill_To_Location = clsCommon.myCstr(dr("Location"))
                    obj.Comments = "Entry created through MCC Material Sale Farmer uploader"
                ...
                If Not (intCurrInvNo = intNextInvNo) Then
                    ...
                    '    ' obj.r = Math.Round(Math.Round(clsCommon.myCdbl(DocuAmount), 0) - clsCommon.myCdbl(DocuAmount), 2)
