...
#Region "Variables"
    ...
    Const colULocation As String = "ULocation"
    Const colUFormerCode As String = "UFarmerCode"
    Const colUDepartment As String = "UDepartment"
    Const colUdescription As String = "Udescription"
    Const ColUComment As String = "UComment"
    ...
    Const ReportID As String = "MCCMeterialSNItemGrid"
...
#End Region
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.frmMCCMaterialSaleReturnFarmer)
    ...
    Sub LoadBlankGrid()
        ...
        repoUnitName.Name = colUOMName
        repoUnitName.Width = 150
        repoUnitName.ReadOnly = True
        gv1.MasterTemplate.Columns.Add(repoUnitName)
        ...
        repoRate.HeaderText = "Price Per Unit"
        ...
        repoAmt.HeaderText = "Basic Sale Amount"
        repoAmt.Name = colAmt
        repoAmt.Width = 80
        repoAmt.Minimum = 0
        ...
        repoCommparty.Width = 0
        repoCommparty.IsVisible = False
        ...
        repoCommPartyName.Width = 0
        repoCommPartyName.IsVisible = False
                    ...
                    If e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colSchemeApplicable) OrElse e.Column Is gv1.Columns(colAmt) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colBatchNo) OrElse e.Column Is gv1.Columns(colExpiry) OrElse e.Column Is gv1.Columns(colManufactureDate) OrElse e.Column Is gv1.Columns(colUnit) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) Then
                        If (e.Column Is gv1.Columns(colQty) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse e.Column Is gv1.Columns(colDisPer) OrElse (e.Column Is gv1.Columns(colAmt) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal)) Then
                            If ((e.Column Is gv1.Columns(colQty))) Then
                                ...
                                OpenSerialItem()
                                '' Anubhooti 20-Feb-2015 (Called  setBalance())
                                setBalance()
                            End If
                        ...
                        ElseIf e.Column Is gv1.Columns(colUnit) Then
                            ...
                            '    Dim Rateqry As String = "select Item_Rate,MRP from TSPL_VENDOR_ITEM_DETAIL where Farmer_Code='" + txtVendorNo.Value + "' and item_no='" + clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value) + "' and UOM='FC' and MRP='" + clsCommon.myCstr(gv1.CurrentRow.Cells(colMRP).Value) + "' "
                    ...
                    If discountRatio > 0 Then
                        ...
                        If dr1 IsNot Nothing AndAlso dr1.Rows.Count > 0 Then
                            ...
                            For Each tdr As DataRow In dr1.Rows
                                ...
                                If clsCommon.myLen(gv1.Rows(intParentRowIndexNo).Cells(colOrderNo).Value) = 0 Then
                                    gv1.Rows.AddNew()
                                    ...
                                    'End If
                                Else
                                    strOrderCode = gv1.Rows(intParentRowIndexNo).Cells(colOrderNo).Value
                                ...
                                gv1.Rows(intRow).Cells(colOrderNo).Value = strOrderCode
                                ...
                                gv1.Rows(intRow).Cells(colUnit).Value = clsDBFuncationality.getSingleValue("select Unit_Desc from tspl_unit_master where Unit_code= '" + clsCommon.myCstr(tdr("Unit_Code")) + "'")
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) <= 0 Then
             ...
             "left outer join TSPL_CUSTOMER_ITEM_MAPPING on TSPL_ITEM_PRICE_MASTER.Item_Code=TSPL_CUSTOMER_ITEM_MAPPING.item_no and TSPL_CUSTOMER_ITEM_MAPPING.Farmer_Code='" & txtVendorNo.Value & "' " & _
    ...
    Sub OpenICodeListCurrentCalaculation(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            '    "TSPL_ITEM_CATEGORY_LEVEL_VALUES on TSPL_ITEM_CATEGORY_LEVEL_VALUES.CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values " & _
            '    "where Farmer_Code='" & txtVendorNo.Value & "' and TSPL_ITEM_MASTER.Active=1 and Is_FreshItem=0 and Product_Type not in ('MI') "
            'Else
            ...
            '        "where Farmer_Code='" & txtVendorNo.Value & "' and TSPL_ITEM_MASTER.Active=1 and Is_FreshItem=0 and Product_Type not in ('MI')) as s pivot(max(cat_value) for description in (" + pivotheader + "))t  "
            '    Else
            ...
            qry = "select * from (select a.DESCRIPTION,a.cat_value, TSPL_ITEM_MASTER.item_code as Item,TSPL_ITEM_MASTER.item_desc as [ItemDesc]," _
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("MMSFA@1it", qry)
            If Not dr Is Nothing Then
                If clsCommon.myLen(clsCommon.myCstr(dr("Item"))) > 0 Then
                    SetitemWiseTaxSetting(True, True)
                    gv1.CurrentRow.Cells(colRowType).Value = RowTypeItem
                    ...
                    gv1.CurrentRow.Cells(colPriceCOde).Value = (clsEkoPro.GetPriceMccSale(txtBillToLocation.Value, clsCommon.myCstr(dr("Item")), clsCommon.myCstr(dr("Unit")), txtDate.Value)) 'clsCommon.myCdbl(dr("BasicRate"))
                    'gv1.CurrentRow.Cells(colAbatementPer).Value = clsCommon.myCdbl(dr("abatement_rate"))
                    gv1.CurrentRow.Cells(colSchemeApplicable).Value = "No"
                    gv1.CurrentRow.Cells(colSchemeItem).Value = "No"
                    gv1.CurrentRow.Cells(colActualCost).Value = clsCommon.myCdbl(dr("BasicRate"))
    ...
    Private Sub SetBlankOfItemColumns()
        ...
        gv1.CurrentRow.Cells(colUOMName).Value = ""
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    If objCustItem IsNot Nothing Then
                        If objCustItem.Min_Rate > (clsCommon.myCdbl(gv1.Rows(ii).Cells(colAmtAfterDis).Value) / dblQty) Then
                            common.clsCommon.MyMessageBoxShow(Me, "Minimum Rate Can't be Less Then " + clsCommon.myCstr(objCustItem.Min_Rate) + " of Item " + strICode.Trim() + "( " + strIName.Trim() + " ) At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)))
                            Return False
                        End If
                    End If
                    If clsCommon.myCdbl(gv1.Rows(ii).Cells(colRate).Value) <= 0 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Rate Can't be 0 of Item " + strICode.Trim() + "( " + strIName.Trim() + " ) At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)))
                        Return False
                    End If
                    'sanjay
                    If clsCommon.myCdbl(gv1.Rows(ii).Cells(colQty).Value) <= 0 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Quantity Can't be 0 of Item " + strICode.Trim() + "( " + strIName.Trim() + " ) At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)))
                        Return False
                    End If
                    'sanjay
                    If clsCommon.myLen(strReqNo) > 0 Then
                        ...
                        If dblQty > dblPendingQty AndAlso clsCommon.CompairString(strSchemeItem, "No") = CompairStringResult.Equal Then
                            common.clsCommon.MyMessageBoxShow(Me, "Item " + strICode + "( " + strIName.Trim() + " ) Entered Quantity(" + clsCommon.myCstr(dblQty) + ") Can't be more Pending Quantity(" + clsCommon.myCstr(dblPendingQty) + ").At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                            Return False
                        End If
                        ...
                        If objCommonVar.IsDemoERP Then
                            If Not strProject Is Nothing Then
                                ...
                                If arrProjNo.Contains(strProject) And arrProjNo.Count > 0 Then
                                    common.clsCommon.MyMessageBoxShow(Me, "SO No:" + strReqNo + " and Item : " + strIName + " is not for PJC or not related to PJC At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                                    Return False
                                Else
            ...
            Else
                If clsLocation.isLocatinExcisable(txtBillToLocation.Value) Then
                    ...
                    If Not clsCommon.CompairString("E", clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry))) = CompairStringResult.Equal Then
                        common.clsCommon.MyMessageBoxShow(Me, "Tax should be excisable for excisable location", Me.Text)
                        Return False
                    End If
                End If
                Dim intx1 As Integer = clsItemMaster.isItemOfSameExempted(arrICode)
                If Not (intx1 = arrICode.Count OrElse intx1 = 0) Then
                    Throw New Exception("All item should be of Tax Exempted or not")
                Else
                    If intx1 > 0 AndAlso clsCommon.myLen(txtTaxGroup.Value) > 0 Then
    ...
    Function InvoiceType() As Boolean
        ...
        qry = "select State_Code from tspl_mp_master where MP_Code='" + txtVendorNo.Value + "'"
        'dt = clsDBFuncationality.GetDataTable(qry)

        Dim strCustState As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry))
        Dim strTinNo As String = "" 'clsCommon.myCstr(dt.Rows(0)("Tin_No"))
    ...
    Function SaveData(ByVal ChekPostBtn As Boolean) As Boolean
        Try
            ...
            If (AllowToSave(ChekPostBtn)) Then
                ...
                obj.Farmer_Code = txtVendorNo.Value
                obj.Farmer_Name = lblVendorName.Text
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    ...
                    Dim objTr As New clsMCCMaterialSaleFarmerDetail()
                    ...
                    objTr.Amount = If(clsCommon.myCdbl(grow.Cells(colAmt).Value) <= 0, objTr.Item_Cost * objTr.Qty, clsCommon.myCdbl(grow.Cells(colAmt).Value))
                ...
                Else
                    ...
                    obj.ApplicableFrom = Nothing
                End If
                obj.isTaxExempted = isTaxExempted
                '' end CurrencyConversion
                'If clsMCCMaterialSaleFarmer.checkSaveNotification(obj, Nothing) Then
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsMCCMaterialSaleFarmer()
            obj = clsMCCMaterialSaleFarmer.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                End If
                chkVendorGrossReceipt.Checked = clsVendorMaster.isGrossReceipt(obj.Farmer_Code)
                UsLock1.Status = obj.Status
                txtDocNo.Value = obj.Document_Code
                txtDate.Value = obj.Document_Date
                txtVendorNo.Value = obj.Farmer_Code
                ...
                lblVendorName.Text = obj.Farmer_Name
                ...
                lblDept.Text = clsDBFuncationality.getSingleValue("select Description as Name from TSPL_GL_SEGMENT_CODE where Seg_No='3' and Segment_code ='" + obj.Dept + "' ")
                ...
                Dim qry As String = "select MP.MP_Code,MP.MP_Name,VLCH.VLC_Code,VLCH.VLC_Name from TSPL_MP_MASTER MP left join TSPL_VLC_MASTER_HEAD VLCH on MP.VLC_Code=VLCH.VLC_Code where MP.MP_Code='" & obj.Farmer_Code & "'"
                Dim dt_vlc As DataTable = clsDBFuncationality.GetDataTable(qry)
                If dt_vlc.Rows.Count > 0 Then
                    LblVlc_Code.Text = dt_vlc.Rows(0).Item("VLC_Code")
                    LblVlc_Name.Text = dt_vlc.Rows(0).Item("VLC_Name")
                Else
                    LblVlc_Code.Text = ""
                    LblVlc_Name.Text = ""
                End If
    ...
    Private Function CreateSTN()
        ...
        If chkAutoTransfer.Checked = True Then
            If clsCommon.myLen(txtFromLoc.Value) <= 0 Then
                ...
                Return False
            End If
            If common.clsCommon.MyMessageBoxShow(Me, "Are you sure to Create Auto Transfer from location " & txtFromLoc.Value & " To " & txtBillToLocation.Value & "  ?", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Try
                    ...
                    If clsCommon.myLen(strGITLoc) <= 0 Then
                        clsCommon.MyMessageBoxShow(Me, "Pls create GIT location for " & txtBillToLocation.Value & " ")
                        trans.Rollback()
                        Return False
                    End If
                    ...
                    For ii As Integer = 0 To gv1.Rows.Count - 1
                        ...
                        If dblEnteredQty < dblBalQtyInLoc Then
                            common.clsCommon.MyMessageBoxShow(Me, "You have already balance for Item - " + txtBillToLocation.Value + Environment.NewLine + " ", Me.Text)
                            objTr.Item_Code = ""
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                If Not clsMCCMaterialSaleFarmer.checkApprovalDocument(Form_ID, txtDocNo.Value) Then
                    Throw New Exception("Approval required for this Document")
                End If
                If SaveData(True) Then
                    If (clsMCCMaterialSaleFarmer.PostData(MyBase.Form_ID, txtDocNo.Value)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsMCCMaterialSaleFarmer.DeleteData(txtDocNo.Value)) Then
                    saveCancelLog(Reason, "Delete", txtDocNo.Value, Nothing)
    ...
    Function saveCancelLog(ByVal Reason As String, ByVal Activity_Type As String, ByVal strDocNo As String, Optional ByVal trans As System.Data.SqlClient.SqlTransaction = Nothing) As Boolean
        Dim obj As New clsCancelLog
        obj.Program_Code = Form_ID
        obj.DOCUMENT_NO = clsCommon.myCstr(strDocNo)
        obj.REASON = Reason
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ...
            If clsCommon.myLen(strwherecls) = 0 Then
                qst = "select count(*) from TSPL_MCC_Sale_Farmer_Head where Document_Code='" + txtDocNo.Value + "' and  TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC'"
            Else
                qst = "select count(*) from TSPL_MCC_Sale_Farmer_Head where Document_Code='" + txtDocNo.Value + "' and  TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC' and TSPL_MCC_Sale_Farmer_Head.Farmer_Code in (" + strwherecls + ")"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        Dim qry As String = " select TSPL_MCC_Sale_Farmer_Head.Document_Code as Code,TSPL_MCC_Sale_Farmer_Head.SALE_INVOICE_NO AS [Invoice No],CONVERT(varchar(10), TSPL_MCC_Sale_Farmer_Head.Document_Date,103)+' '+ CONVERT(varchar(5), TSPL_MCC_Sale_Farmer_Head.Document_Date,114) as Date," & _
                            " TSPL_MCC_Sale_Farmer_Head.Farmer_Code as [Farmer Code],TSPL_MP_MASTER.MP_Name as [Farmer Name],TSPL_MP_MASTER.VLC_Code as [VLC Code],TSPL_VLC_MASTER_HEAD.VLC_Name as [VLC Name]," & _
                            " TSPL_VLC_MASTER_HEAD.VSP_Code as [VSP Code], TSPL_VENDOR_MASTER.Vendor_Name as [VSP Name],TSPL_MCC_Sale_Farmer_Head.Comments,Total_Amt as Amount, " & _
                            " case when TSPL_MCC_Sale_Farmer_Head.Status=0 then 'Pending' else 'Approved' end as [Status],TSPL_MCC_Sale_Farmer_Head.Direct_Dispatch as [Direct Dispatch], " & _
                            " TSPL_MCC_Sale_Farmer_Head.Bill_To_Location as [Bill To Location],TSPL_MCC_Sale_Farmer_Head.Discount_Amt as [Discount Amount],TSPL_MCC_Sale_Farmer_Head.Amount_Less_Discount as [Amt Less Discount],TSPL_MCC_Sale_Farmer_Head.Total_Tax_Amt as [Total Tax Amount], " & _
                            " TSPL_MCC_Sale_Farmer_Head.Total_Amt as [Total Amount],TSPL_MCC_Sale_Farmer_Head.EWayBillNo as [EwayBill No],TSPL_MCC_Sale_Farmer_Head.EWayBillDate as [EWayBill Date],TSPL_MCC_Sale_Farmer_Head.Electronic_Ref_No as [Electronic Ref No] " & _
                            " From TSPL_MCC_Sale_Farmer_Head " & _
                            " left join TSPL_MP_MASTER on TSPL_MCC_Sale_Farmer_Head.Farmer_Code=TSPL_MP_MASTER.MP_Code " & _
        ...
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and  TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC' and TSPL_MCC_Sale_Farmer_Head.Farmer_Code in (" + strwherecls + ") "
        ElseIf clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            whrClas = " Bill_To_Location in (" + objCommonVar.strCurrUserLocations + ") and  TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC'"
        ElseIf clsCommon.myLen(strwherecls) > 0 Then
            whrClas = " TSPL_MCC_Sale_Farmer_Head.Farmer_Code in (" + strwherecls + ") and  TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC' "
        Else
            whrClas = " TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC'"
        End If
        '-----------------------------------------------------
    ...
    Private Sub FrmAPInvoiceEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ...
        ElseIf e.Alt AndAlso e.Control AndAlso e.Shift AndAlso e.KeyCode = Keys.F11 Then
            ...
            Else
                ...
                pwd.strCode = clsFixedParameterCode.MCCMaterialSaleFarmerReverse
                pwd.strType = clsFixedParameterType.MCCMaterialSaleFarmerReverse
                pwd.ShowDialog()
    ...
    Private Sub txtTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTaxGroup._MYValidating
        ...
        Try
            ...
            End If
        Catch ex As Exception
            If (ex.Message.Contains("Vendor") Or ex.Message.Contains("Customer")) = True Then
                clsCommon.MyMessageBoxShow(Me, "Please select MP first", Me.Text)
            Else
                clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
            End If
    ...
    Private Sub txtVendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtVendorNo._MYValidating
        ...
        '-----------------------------------------------------
        Dim qry As String = ""
        txtVendorNo.Value = clsMpMaster.getFinder("VLCH.MCC='" & txtBillToLocation.Value & "'", txtVendorNo.Value, isButtonClicked)
        '-----------------------------------------------------
        qry = "select MP.MP_Code,MP.MP_Name,VLCH.VLC_Code,VLCH.VLC_Name from TSPL_MP_MASTER MP left join TSPL_VLC_MASTER_HEAD VLCH on MP.VLC_Code=VLCH.VLC_Code where MP.MP_Code='" & txtVendorNo.Value & "'"
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            lblVendorName.Text = clsCommon.myCstr(dt.Rows(0)("MP_Name"))
            ...
            LblVlc_Name.Text = clsCommon.myCstr(dt.Rows(0)("VLC_Name"))

            'txtDate.Enabled = False
            '' code commented by Panch Raj against ticket no: KDI/17/05/18-000318 after discussion with Ranjana mam
            'txtVendorNo.Enabled = False
            chkTaxable.Enabled = False
            chkRateUserCustomer.ToggleState = ToggleState.Off ''ClsUserCustomerSettings.GetUserCustomerRateSetting(txtVendorNo.Value)
            SetMultiCurrencyVisibility()
        Else
            lblVendorName.Text = ""
            txtTermCode.Value = ""
                    ...
                    For Each obj As clsPSSalesOrderDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn(ii).Item_Code AndAlso obj.Unit_code = frm.ArrReturn(ii).Unit_code AndAlso obj.Scheme_Item = "N") Then ''OrElse (obj.Scheme_Code = frm.ArrReturn(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            If IsValidItem(obj) Then
                                ...
                                'Dim dt As DataTable = clsMCCMaterialSaleFarmer.GetOriginalQty(obj.Document_Code, obj.Item_Code, obj.Unit_code, obj.Assessable, obj.MRP, Nothing)
                                'If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            ElseIf gv1.CurrentColumn Is gv1.Columns(colComplete) AndAlso UsLock1.Status = ERPTransactionStatus.Approved Then
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 AndAlso clsCommon.myLen(strICode) > 0 AndAlso intSNo > 0 AndAlso clsCommon.CompairString(strStatus, "No") = CompairStringResult.Equal Then
                    If common.clsCommon.MyMessageBoxShow("Do you want to complete the item " + clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value), Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                        If clsMCCMaterialSaleFarmerDetail.CompleteSRN(txtDocNo.Value, strICode, intSNo) Then
    ...
    Private Function GetAttachQry() As String
        ...
        ' Dim Qry As String = "  select TSPL_SD_SHIPMENT_HEAD.Bill_To_Location, TSPL_LOCATION_MASTER.Location_Desc, TSPL_SD_SHIPMENT_HEAD.Is_Taxable,TSPL_SD_SHIPMENT_HEAD.Electronic_Ref_No,TSPL_SD_SHIPMENT_HEAD.EWayBillNo, convert(varchar,TSPL_SD_SHIPMENT_HEAD.EWayBillDate,103) as EWayBillDate,  StateMasterForLocation.GST_STATE_CODE AS From_GstStateCode," & _
        '   " TSPL_LOCATION_MASTER.GSTNO as From_Loc_GstinNo, TSPL_STATE_MASTER.GST_STATE_Code AS Cust_GstStateCode,  TSPL_CUSTOMER_MASTER.GSTNO as Cust_GstInNo,    case when coalesce(p_cust.GST_STATE_CODE,'')='' then TSPL_state_Master.GST_STATE_CODE       when coalesce(p_cust.GST_STATE_CODE,'')<>'' then p_cust .GST_STATE_CODE    end as P_GST_STATE_CODE," & _
        '   " case when coalesce(p_cust.P_GSTIN_NO,'')='' then TSPL_CUSTOMER_MASTER .GSTNO  when coalesce(p_cust.P_GSTIN_NO,'')<>'' then p_cust .P_GSTIN_NO end as P_GSTIN_NO,    TSPL_ITEM_MASTER.HSN_Code,  " & _
        ...
        ' " ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX1_Amt ,0)as DTax1_Amt,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX2_Amt ,0)as DTax2_Amt," & _
        '" ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX3_Amt ,0)as DTax3_Amt,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX4_Amt ,0)as DTax4_Amt," & _
        ' "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX5_Amt ,0)as DTax5_Amt,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX6_Amt ,0)as DTax6_Amt," & _
        '  " ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX7_Amt ,0)as DTax7_Amt,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX8_Amt ,0)as DTax8_Amt," & _
        '   "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX9_Amt ,0)as DTax9_Amt,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX10_Amt ,0)as DTax10_Amt," & _
        '   "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX1_Rate,0)as DTax1_Rate,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX2_Rate,0)as DTax2_Rate," & _
        '   "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX3_Rate,0)as DTax3_Rate,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX4_Rate,0)as DTax4_Rate," & _
        '   "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX5_Rate,0)as DTax5_Rate,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX6_Rate,0)as DTax6_Rate," & _
        '   "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX7_Rate,0)as DTax7_Rate,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX8_Rate,0)as DTax8_Rate," & _
        '   "ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX9_Rate,0)as DTax9_Rate,ISNULL(TSPL_SD_SHIPMENT_DETAIL.TAX10_Rate,0)as DTax10_Rate," & _
        '  " TSPL_LOCATION_MASTER.State as loc_state_code,tspl_location_master.HOAdd1 ,tspl_location_master.HOAdd2,TSPL_SD_SHIPMENT_HEAD.HeadDisc_PerAmt,'" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,"
        ...
        ' ''
        ' Qry += "   case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add1  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add1 end as P_Add1, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add2  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add2 end as P_Add2, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add3  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add3 end as P_Add3, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .PIN_Code   when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Pin_No  end as P_PinNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .CST    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_CST_No   end as P_CstNo,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Tin_No     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .p_Tin_No   end as P_TinNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Email    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Email  end as P_Email,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Fax     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Fax   end as P_Fax, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .PAN      when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Cust_PAN    end as P_Cust_PAN, case when coalesce(p_cust.P_cust_code,'')='' then  TSPL_CUSTOMER_MASTER  .CST      when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_CST_No    end as P_CSTNo,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Lst_No     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_LST_No    end as P_LstNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Cust_Code      when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_code   end as P_CustCode, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Customer_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_name    end as P_Cust_Name,"
        ...
        ' Qry += " TSPL_SD_SHIPMENT_DETAIL.item_code as item_code, TSPL_ITEM_MASTER.Item_Desc + case when Scheme_Item ='Y' then ' (Free Scheme)' else '' end    as itemdesc, TSPL_SD_SHIPMENT_DETAIL.Row_Type,TSPL_SD_SHIPMENT_DETAIL.Qty as qty,TSPL_SD_SHIPMENT_DETAIL.unit_code as uom,TSPL_SD_SHIPMENT_DETAIL.item_cost as itemcost,TSPL_SD_SHIPMENT_DETAIL.amount as amount,TSPL_SD_SHIPMENT_HEAD.TAX1,TSPL_SD_SHIPMENT_HEAD.TAX2,TSPL_SD_SHIPMENT_HEAD.TAX3,TSPL_SD_SHIPMENT_HEAD.TAX4,TSPL_SD_SHIPMENT_HEAD.TAX5,TSPL_SD_SHIPMENT_HEAD.Total_Add_Charge "
        ' Qry += " " & QryShowStatus & " "
        ' Qry += " " & SerialNoColumn & "  "
        ...
        Qry = "select TSPL_COMPANY_MASTER.Comp_Code,TSPL_COMPANY_MASTER.Comp_Name,ISNULL(TSPL_COMPANY_MASTER.Phone1,'')+ Case When ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as Comp_Phone,TSPL_COMPANY_MASTER.Fax as Comp_Fax,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,ISNULL(tspl_company_Master.ADD1,'') as Comp_address1,ISNULL(tspl_company_Master.ADD2,'') as Comp_address2,ISNULL(tspl_company_Master.Add3,'') as Comp_address3, tspl_company_Master.GSTINNo, tspl_company_Master.Access_Officer, tspl_company_Master.CINNo, tspl_company_Master.Email as Comp_Email, tspl_company_Master.GSTReg_No as GSTReg_No, tspl_company_Master.Pan_No as Comp_Pan, tspl_company_Master.Pincode as Comp_Pin,TSPL_MCC_Sale_Farmer_Head.Is_Taxable,TSPL_MCC_Sale_Farmer_Head.Is_CashSale, TSPL_MCC_Sale_Farmer_Head.Document_Code,convert(varchar(15),TSPL_MCC_Sale_Farmer_Head.Document_Date,103) as Document_Date,TSPL_MCC_Sale_Farmer_Head.Tax_Group,TSPL_MCC_Sale_Farmer_Head.Bill_To_Location,TSPL_MCC_Sale_Farmer_Head.EWayBillNo ,convert(varchar(15),TSPL_MCC_Sale_Farmer_Head.EWayBillDate,103) as EWayBillDate ,TSPL_MCC_Sale_Farmer_Head.Electronic_Ref_No ," &
" TSPL_MCC_Sale_Farmer_Head.Farmer_Code,TSPL_MP_MASTER.MP_Name,TSPL_MCC_Sale_Farmer_Head.Payment_Terms,TSPL_MCC_Sale_Farmer_Head.Sale_Invoice_No as DocNo ,CONVERT(VARCHAR(15),TSPL_MCC_Sale_Farmer_Head.Sale_Invoice_Date,103) as Sale_Invoice_Date,TSPL_MCC_Sale_Farmer_Head.Transporter_Name,TSPL_MCC_Sale_Farmer_Head.Transporter_Name_Manual ," &
" TSPL_MCC_Sale_Farmer_Head.Vehicle_Manual_No,TSPL_MCC_Sale_Farmer_Head.VehicleNo,TSPL_MCC_Sale_Farmer_Head.Total_Amt	,TSPL_MCC_Sale_Farmer_Head.RoundOffAmount ," &
" TSPL_LOCATION_MASTER.Add1 + CASE WHEN ISNULL(TSPL_LOCATION_MASTER.Add2,'')<>'' THEN ', ' + TSPL_LOCATION_MASTER.Add2 else '' end + case WHEN ISNULL(TSPL_LOCATION_MASTER.Add3,'')<>'' THEN ', ' + TSPL_LOCATION_MASTER.Add3 else '' end + case when isnull(BILL_TO_LOCATION_STATE.STATE_NAME,'')<>'' then ', ' + BILL_TO_LOCATION_STATE.STATE_NAME else '' END AS Loc_Address," &
" TSPL_LOCATION_MASTER.Add1 as Loc_Add1,TSPL_LOCATION_MASTER.add2 as Loc_Add2,TSPL_LOCATION_MASTER.add3 as Loc_Add3," &
" BILL_TO_LOCATION_STATE.STATE_NAME as LocationState,BILL_TO_LOCATION_STATE.GST_STATE_Code as Loc_GST_StateCode,TSPL_LOCATION_MASTER.GSTNO AS From_Loc_GSTINNO," &
" TSPL_MP_MASTER.add1 AS Farmer_add1,TSPL_MP_MASTER.Add2 as Farmer_Add2,MP_CITY_MASTER.City_Name,MP_STATE_MASTER.STATE_NAME as Farmer_StateName,MP_STATE_MASTER.GST_STATE_Code as Farmer_GST_StateCode,'' AS Farmer_GSTINNO," &
" TSPL_MCC_SALE_FARMER_DETAIL.Item_Code ,TSPL_ITEM_MASTER.Item_Desc ,TSPL_ITEM_MASTER.HSN_Code,TSPL_MCC_SALE_FARMER_DETAIL.Qty,TSPL_MCC_SALE_FARMER_DETAIL.Unit_code AS uom,TSPL_MCC_SALE_FARMER_DETAIL.Amount ,TSPL_MCC_SALE_FARMER_DETAIL.Disc_Amt,TSPL_MCC_SALE_FARMER_DETAIL.Disc_Per," &
" TSPL_MCC_SALE_FARMER_DETAIL.MRP,TSPL_MCC_SALE_FARMER_DETAIL.ActualRate as ItemCost  ," &
" dtax1.Type as tax1Type,dtax2.Type as tax2Type,dtax3.Type as tax3Type,dtax4.Type as tax4Type,dtax5.Type as tax5Type,dtax6.Type as tax6Type,dtax7.Type as tax7Type,dtax8.Type as tax8Type,dtax9.Type as tax9Type,dtax10.Type as tax10Type,  ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX1_Amt ,0)as DTax1_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX2_Amt ,0)as DTax2_Amt, ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX3_Amt ,0)as DTax3_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX4_Amt ,0)as DTax4_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX5_Amt ,0)as DTax5_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX6_Amt ,0)as DTax6_Amt, ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX7_Amt ,0)as DTax7_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX8_Amt ,0)as DTax8_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX9_Amt ,0)as DTax9_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX10_Amt ,0)as DTax10_Amt,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX1_Rate,0)as DTax1_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX2_Rate,0)as DTax2_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX3_Rate,0)as DTax3_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX4_Rate,0)as DTax4_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX5_Rate,0)as DTax5_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX6_Rate,0)as DTax6_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX7_Rate,0)as DTax7_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX8_Rate,0)as DTax8_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX9_Rate,0)as DTax9_Rate,ISNULL(TSPL_MCC_SALE_FARMER_DETAIL.TAX10_Rate,0)as DTax10_Rate  " &
" from TSPL_MCC_Sale_Farmer_Head left outer join TSPL_MCC_SALE_FARMER_DETAIL on TSPL_MCC_Sale_Farmer_Head.Document_Code =TSPL_MCC_SALE_FARMER_DETAIL.DOCUMENT_CODE" &
" LEFT OUTER JOIN TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.ITEM_CODE=TSPL_MCC_SALE_FARMER_DETAIL.Item_Code " &
" left outer join TSPL_MP_MASTER on TSPL_MP_MASTER.MP_Code =TSPL_MCC_Sale_Farmer_Head.Farmer_Code" &
" LEFT OUTER JOIN TSPL_STATE_MASTER AS MP_STATE_MASTER ON MP_STATE_MASTER.STATE_CODE=TSPL_MP_MASTER.State_Code" &
" LEFT OUTER JOIN TSPL_CITY_MASTER as MP_CITY_MASTER ON MP_CITY_MASTER.City_Code=TSPL_MP_MASTER.City_code " &
" left outer join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code=TSPL_MCC_Sale_Farmer_Head.Comp_Code " &
" LEFT OUTER JOIN TSPL_LOCATION_MASTER ON TSPL_LOCATION_MASTER.Location_Code=TSPL_MCC_Sale_Farmer_Head.Bill_To_Location" &
" LEFT OUTER JOIN TSPL_STATE_MASTER AS BILL_TO_LOCATION_STATE ON BILL_TO_LOCATION_STATE.STATE_CODE=TSPL_LOCATION_MASTER.State" &
 " left outer join TSPL_TAX_MASTER as dtax1 on dtax1.tax_code =TSPL_MCC_SALE_FARMER_DETAIL.tax1   " &
 " left outer join tspl_tax_master as dtax2 on dtax2.tax_code = TSPL_MCC_SALE_FARMER_DETAIL.tax2    left outer join tspl_tax_master as dtax3 on dtax3.Tax_Code=TSPL_MCC_SALE_FARMER_DETAIL.TAX3  " &
  " left outer join TSPL_TAX_MASTER as dtax4 on dtax4.Tax_Code= TSPL_MCC_SALE_FARMER_DETAIL.tax4    left outer join TSPL_TAX_MASTER as dtax5 on dtax5.Tax_Code=TSPL_MCC_SALE_FARMER_DETAIL.tax5   left outer join TSPL_TAX_MASTER as dtax6 on dtax6.Tax_Code =TSPL_MCC_SALE_FARMER_DETAIL.TAX6   left outer join TSPL_TAX_MASTER as dtax7 on dtax7.Tax_Code =TSPL_MCC_SALE_FARMER_DETAIL.TAX7    left outer join TSPL_TAX_MASTER as dtax8 on dtax8.Tax_Code =TSPL_MCC_SALE_FARMER_DETAIL.TAX8    left outer join TSPL_TAX_MASTER as dtax9 on dtax9.Tax_Code =TSPL_MCC_SALE_FARMER_DETAIL.TAX9 left outer join TSPL_TAX_MASTER as dtax10 on dtax10.Tax_Code =TSPL_MCC_SALE_FARMER_DETAIL.TAX10 " &
  " WHERE TSPL_MCC_Sale_Farmer_Head.Document_Code='" + txtDocNo.Value + "'"
        Return Qry
    ...
    Private Sub funPrint()
        Try
            ...
            Dim IsExampted As Double = 0
            IsExampted = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select COUNT(Is_Tax_Exempted)  from TSPL_TAX_GROUP_MASTER where Tax_Group_Code='" & txtTaxGroup.Value & "' AND Tax_Group_Type='S' AND Is_Tax_Exempted=1"))
            atchqry = GetAttachQry()
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(atchqry)

            If dt.Rows.Count > 0 Then
                ' If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Is_CashSale")), "Y") = CompairStringResult.Equal Then
                'frmCrystalReportViewer.funreport(CrystalReportFolder.MilkProcurement, dt, "rptMccMaterialSaleCashMemo", "Cash Memo")
                'Else
    ...
    Private Function SetItemWiseTax(ByVal dtAfterModify As DataTable, ByVal strShipFrm As String) As DataTable
        ...
        qry += " from TSPL_MCC_Sale_Farmer_Detail where Document_Code='" + strShipFrm + "' "
        qry += " union all "
        ...
        Try
            If e.Column.Index >= 0 Then
                ...
                ElseIf e.Column Is gv1.Columns(colQty) Then
                    ...
                    End If
                ElseIf e.Column Is gv1.Columns(colUnit) Then
                    'If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal AndAlso clsCommon.myLen(gv1.CurrentRow.Cells(colOrderNo).Value) <= 0 Then
    ...
    Public Sub funrejprint()
        Try
            Dim qry As String = "SELECT     TSPL_MCC_Sale_Farmer_Head.Document_Code, TSPL_MCC_Sale_Farmer_Head.Document_Date, TSPL_MCC_Sale_Farmer_Head.Customer_Name, TSPL_MCC_Sale_Farmer_Head.Ship_To_Location, TSPL_MCC_Sale_Farmer_Head.Bill_To_Location, TSPL_MCC_Sale_Farmer_Head.RMDA_No, TSPL_MCC_Sale_Farmer_Head.RMDA_Date,TSPL_MCC_Sale_Farmer_Head.Remarks,TSPL_MCC_Sale_Farmer_Head.Description, TSPL_SRN_DETAIL.Item_Code,   TSPL_SRN_DETAIL.Item_Desc, TSPL_SRN_DETAIL.Rejected_Qty, TSPL_SRN_DETAIL.Item_Cost,TSPL_SRN_DETAIL.Unit_code,TSPL_SRN_DETAIL.Rejected_Qty*TSPL_SRN_DETAIL.Item_Cost as Amount,TSPL_COMPANY_MASTER.comp_name,TSPL_COMPANY_MASTER.Logo_Img, TSPL_COMPANY_MASTER.Logo_Img2  FROM         TSPL_MCC_Sale_Farmer_Head INNER JOIN    TSPL_SRN_DETAIL ON TSPL_MCC_Sale_Farmer_Head.Document_Code = TSPL_SRN_DETAIL.Document_Code LEFT OUTER JOIN     TSPL_COMPANY_MASTER ON TSPL_MCC_Sale_Farmer_Head.Comp_Code = TSPL_COMPANY_MASTER.Comp_Code  where TSPL_MCC_Sale_Farmer_Head.Document_Code='" + txtDocNo.Value + "'"
    ...
    Public Function GetTaxGrp(ByVal strItmType As String) As String
        Dim qry As String = "select Tax_Group  from TSPL_VENDOR_MASTER where Farmer_Code ='" + strItmType + "'"
        strItmType = clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry))
        Return strItmType
    End Function
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        Try
            ...
            'dblRate = clsEkoPro.GetRateMccSale(txtBillToLocation.Value, clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colICode).Value), clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colUnit).Value), dtpChallan.Value) 'GetConvRate(IntRowNo)
    ...
    Private Sub txtFromLoc__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtFromLoc._MYValidating
        Dim qry As String = "select Location_Code as Code,Location_Desc as Name from TSPL_LOCATION_MASTER "
        Dim WhrCls As String = " Location_Type='Physical' and location_category='MCC' "
        If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            WhrCls += "  and  Location_Code in (" + objCommonVar.strCurrUserLocations + ")"
        End If
    ...
    Private Sub btnReverseAndUnpost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverseAndUnpost.Click
        Try
            If common.clsCommon.MyMessageBoxShow(Me, "Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsMCCMaterialSaleFarmer.ReverseAndUnpost(txtDocNo.Value) Then
...
#Region "Mail SMS Setting"
    ...
    Public Function GetMailPrint(ByVal strCode As String)
        atchqry = "  select '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate, TSPL_SHIP_TO_LOCATION.Ship_To_Desc as shipName, TSPL_SHIP_TO_LOCATION.add1 as ship_Add1, TSPL_SHIP_TO_LOCATION.Add2 as ship_add2 ,TSPL_SHIP_TO_LOCATION.Add3 as ship_add3  ,TSPL_SHIP_TO_LOCATION.Pin_Code,TSPL_CITY_MASTER.STATE_CODE  ,City_Name,TSPL_EMPLOYEE_MASTER.Emp_Name as SalesManName,TSPL_MCC_Sale_Farmer_Head.Inv_No, TSPL_MCC_Sale_Farmer_Head.Dept_Desc , TSPL_MCC_Sale_Farmer_Head.Remarks ,  TSPL_MCC_Sale_Farmer_Head.Terms_Code,TSPL_MCC_Sale_Farmer_Head.VehicleNo , "
        atchqry += " TSPL_MCC_Sale_Farmer_Detail .Specification as  specification,   TSPL_MCC_Sale_Farmer_Head.Document_Code as DocNo , TSPL_MCC_Sale_Farmer_Head.Description, "
        atchqry += "  convert(varchar ,TSPL_MCC_Sale_Farmer_Head .Document_Date,103)as Document_Date , TSPL_MCC_Sale_Farmer_Head.Against_POS, TSPL_MCC_Sale_Farmer_Head.Item_Type ,  TSPL_MCC_Sale_Farmer_Head.Farmer_Code, "
        atchqry += " TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Add1 as Customer_Add1,TSPL_CUSTOMER_MASTER.add2 as customer_Add2,TSPL_CUSTOMER_MASTER.Add3 as customer_Add3 ,TSPL_CUSTOMER_MASTER.State as customer_city_State ,TSPL_CUSTOMER_MASTER.PIN_Code as Customer_Pin_Code , TSPL_MCC_Sale_Farmer_Head .Terms_Code as termscode ,TSPL_MCC_Sale_Farmer_Head .Ref_No as ref_no ,"
        atchqry += " TSPL_MCC_Sale_Farmer_Head .Comments as comments ,  TSPL_MCC_Sale_Farmer_Head .Discount_Amt as dis_amt,TSPL_MCC_Sale_Farmer_Detail .Disc_Amt  as dis_amt1,"
        atchqry += " TSPL_MCC_Sale_Farmer_Head.Amount_Less_Discount  as aftrdiscount ,TSPL_MCC_Sale_Farmer_Head .Total_Amt as Total_amount,"
        atchqry += " TSPL_MCC_Sale_Farmer_Head.Discount_Base as bfrdisc_amount,  "
        atchqry += " tax1.Tax_Code_Desc as tax1name,isnull (TSPL_MCC_Sale_Farmer_Head.tax1_amt,0) as txt1amt,  "
        atchqry += " tax2.Tax_Code_Desc as tax2name,isnull (TSPL_MCC_Sale_Farmer_Head.tax2_amt,0) as txt2amt,  "
        atchqry += " tax3.Tax_Code_Desc as tax3name,isnull (TSPL_MCC_Sale_Farmer_Head.tax3_amt,0) as txt3amt,  "
        atchqry += " tax4.Tax_Code_Desc as tax4name,isnull (TSPL_MCC_Sale_Farmer_Head.tax4_amt,0) as txt4amt,  "
        atchqry += " tax5.Tax_Code_Desc as tax5name,isnull (TSPL_MCC_Sale_Farmer_Head.tax5_amt,0) as txt5amt,  "
        atchqry += " tax6.Tax_Code_Desc as tax6name,isnull (TSPL_MCC_Sale_Farmer_Head.tax6_amt,0) as txt6amt,  "
        atchqry += " tax7.Tax_Code_Desc as tax7name,isnull (TSPL_MCC_Sale_Farmer_Head.tax7_amt,0) as txt7amt,  "
        atchqry += " tax8.Tax_Code_Desc as tax8name,isnull (TSPL_MCC_Sale_Farmer_Head.tax8_amt,0) as txt8amt,   "
        atchqry += " tax9.Tax_Code_Desc as tax9name,isnull (TSPL_MCC_Sale_Farmer_Head.tax9_amt,0) as txt9amt,  "
        atchqry += " tax10.Tax_Code_Desc as tax10name,isnull (TSPL_MCC_Sale_Farmer_Head.tax10_amt,0) as txt10amt,  "
        atchqry += " isnull(TSPL_MCC_Sale_Farmer_Head .Total_Tax_Amt,0) as total_tax_amt, TSPL_MCC_Sale_Farmer_Head.Total_Amt as DocAmt,  TSPL_COMPANY_MASTER.Comp_Name as compname,ISNULL(TSPL_COMPANY_MASTER.Phone1,'')+ Case When ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as Phone,TSPL_COMPANY_MASTER.Fax ,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,ISNULL(tspl_company_Master.ADD1,'') as address1,"
        atchqry += " TSPL_MCC_Sale_Farmer_Detail.item_code as item_code, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_MCC_Sale_Farmer_Detail.Row_Type,TSPL_MCC_Sale_Farmer_Detail.Qty as qty,TSPL_MCC_Sale_Farmer_Detail.unit_code as uom,TSPL_MCC_Sale_Farmer_Detail.item_cost as itemcost,TSPL_MCC_Sale_Farmer_Detail.amount as amount,TSPL_MCC_Sale_Farmer_Head.TAX1,TSPL_MCC_Sale_Farmer_Head.TAX2,TSPL_MCC_Sale_Farmer_Head.TAX3,TSPL_MCC_Sale_Farmer_Head.TAX4,TSPL_MCC_Sale_Farmer_Head.TAX5,TSPL_MCC_Sale_Farmer_Head.Total_Add_Charge from TSPL_MCC_Sale_Farmer_Detail  "
        atchqry += " left outer join TSPL_MCC_Sale_Farmer_Head  on TSPL_MCC_Sale_Farmer_Head.Document_Code  =TSPL_MCC_Sale_Farmer_Detail.Document_Code   "
        atchqry += " left outer join  TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code =TSPL_MCC_Sale_Farmer_Head .Ship_To_Location "
        atchqry += " left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code =TSPL_SHIP_TO_LOCATION.City_Code "
        atchqry += " left outer join TSPL_EMPLOYEE_MASTER on TSPL_EMPLOYEE_MASTER.EMP_CODE =TSPL_MCC_Sale_Farmer_Head.Salesman_Code "
        atchqry += " left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_MCC_Sale_Farmer_Head.tax1  "
        atchqry += " left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_MCC_Sale_Farmer_Head.tax2  "
        atchqry += " left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_MCC_Sale_Farmer_Head .TAX3  "
        atchqry += " left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_MCC_Sale_Farmer_Head .tax4  "
        atchqry += " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_MCC_Sale_Farmer_Head .tax5  "
        atchqry += " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_MCC_Sale_Farmer_Head .TAX6  "
        atchqry += " left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_MCC_Sale_Farmer_Head .TAX7  "
        atchqry += " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_MCC_Sale_Farmer_Head .TAX8  "
        atchqry += " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_MCC_Sale_Farmer_Head .TAX9 "
        atchqry += " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_MCC_Sale_Farmer_Head .TAX10     "
        atchqry += " left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_MCC_Sale_Farmer_Head.comp_code  "
        atchqry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_MCC_Sale_Farmer_Head.Farmer_Code   "
        atchqry += " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER .Location_Code=  TSPL_MCC_Sale_Farmer_Head.Bill_To_Location "
        atchqry += " Left Outer Join TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_MCC_Sale_Farmer_Detail.Item_Code  where 2=2 "
        atchqry += "  and  TSPL_MCC_Sale_Farmer_Head.Document_Code = '" + strCode + "'"
...
#End Region
    ...
    Private Sub SendEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        Try
            ...
            ''    Dim frmCRV As New frmCrystalReportViewer()
            ''    strRptPath = frmCRV.funreport(True, CrystalReportFolder.NewSalesReports, dt1, "crptSaleReturn", "Sales Return")
            ''    frmCRV = Nothing
            ...
            For Each strUser As String In lstUsers
                ...
                If isSendForApproval Then
                    ...
                    emailId = clsDBFuncationality.getSingleValue(qry)
                Else
                    strContactPerson = clsDBFuncationality.getSingleValue("select upper(substring(MP_Name,1,1)) + lower(substring(MP_Name,2,49)) from TSPL_MP_MASTER where MP_code ='" & strUser & "' ")
                    emailId = clsDBFuncationality.getSingleValue("select Email from TSPL_MP_MASTER where MP_code ='" & strUser & "' ")
                End If
    ...
    Private Sub SMSSENDONLY(ByVal isPost As Boolean)
        Try
            ...
            'End If
            'sanjay
            Dim strphone As String = clsDBFuncationality.getSingleValue("select Telphone from TSPL_MP_MASTER where MP_code ='" & txtVendorNo.Value & "' ")
            Dim dtContent As DataTable = clsDBFuncationality.GetDataTable("SELECT SMS_Text,Email_Text,Email_subject from TSPL_ES_Content where Form_ID='" + Form_ID + "'", Nothing)

            Dim objSMSH As New clsSMSHead()
            objSMSH.arrMobilNo = New List(Of String)()
            objSMSH.arrMobilNo.Add(clsCommon.myCstr(strphone))
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                If clsCommon.myLen(dtContent.Rows(0)("SMS_Text")) > 0 Then
                    ...
                    objSMSH.SMS_Text = objSMSH.SMS_Text.Replace(frmEMailAndSMSSetting.ContactPerson, "")
                End If
            End If

            If objSMSH.SaveData(Form_ID, objSMSH, Nothing) Then
                If Not isPost Then
    ...
    Private Sub Export_Head_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Export_Head.Click
        Dim sQuery As String = "select Document_Code as [Document Code],RANK() over(order by Document_Code desc) as [S No],Document_Date AS [Document Date],Farmer_Code as [Customer Code],Description,Remarks,Bill_To_Location as [Bill To Location]," _
        ...
        & " ,Add_Charge_Code8,Add_Charge_Amt8,Add_Charge_Code9,Add_Charge_Amt9,Add_Charge_Code10,Add_Charge_Amt10 from TSPL_MCC_Sale_Farmer_Head"
    ...
    Private Sub Export_details_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Export_details.Click
        ...
        & " Unit_code as [UOM],Qty as [Quantity],MRP,Item_Cost  as [Item Cost],Location,Remarks,Specification from TSPL_MCC_Sale_Farmer_Detail"
        transportSql.ExporttoExcel(sQuery, Me)
    End Sub
    ...
    Private Sub btnDeliveredTo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDeliveredTo.Click
        ...
        '        Dim qry = "Update TSPL_MCC_Sale_Farmer_Head set Is_Delivered=1  where Document_Code='" + txtDocNo.Value + "'"
    ...
    Private Sub ReStoreGridLayout()
        Try
            ...
            obj = CType(obj.GetData("MilkMaterialGrid", "", objCommonVar.CurrentUserCode), clsGridLayout)
    ...
    Private Sub rmExport_Click(sender As Object, e As EventArgs) Handles rmExport.Click
        Dim qry As String = "select convert(varchar,TSPL_MCC_Sale_Farmer_head.Document_Date,103) as [Doc Date],TSPL_MCC_Sale_Farmer_head.bill_to_location as [Loc Code]," & _
                             " TSPL_MCC_Sale_Farmer_head.farmer_code  as [Farmer Code],TSPL_MCC_Sale_Farmer_head.dept as [Department],TSPL_MCC_Sale_Farmer_head.[Description]," & _
                             " TSPL_MCC_Sale_Farmer_head.comments as [Comment],TSPL_MCC_Sale_Farmer_head.Tax_Group as [Tax Group] " & _
                             " ,TSPL_MCC_Sale_Farmer_Detail.Item_Code,TSPL_MCC_Sale_Farmer_Detail.Qty,TSPL_MCC_Sale_Farmer_Detail.Unit_code as [Unit Code],TSPL_MCC_Sale_Farmer_Detail.Item_Cost as [Unit Rate] " & _
                             "  from TSPL_MCC_Sale_Farmer_head left outer join TSPL_MCC_Sale_Farmer_Detail on TSPL_MCC_Sale_Farmer_head.Document_Code=TSPL_MCC_Sale_Farmer_Detail.DOCUMENT_CODE "


        transportSql.ExporttoExcel(qry, Nothing, Me)
    End Sub
    ...
    Private Sub rmImport_Click(sender As Object, e As EventArgs) Handles rmImport.Click
        ...
        IsImported = True
        If transportSql.importExcel(gv, "Doc Date", "Loc Code", "Farmer Code", "Department", "Description", "Comment", "Tax Group", "Item_Code", "Qty", "Unit Code", "Unit Rate") Then
            Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
            Try
                ...
                '=====================20/04/2017=============
                Dim lstMP As New List(Of structMP)
                Dim obj2 As structMP
                obj2 = New structMP
                For Each r As GridViewRowInfo In gv.Rows
                    Dim mpCode As String = clsCommon.myCstr(r.Cells("Farmer Code").Value)
                    Dim DocDate As Date = clsCommon.myCDate(r.Cells("Doc Date").Value)

                    Dim newmp As New structMP
                    newmp.MP_Code = mpCode
                    newmp.Doc_Date = DocDate

                    If lstMP.Contains(newmp) = True Then

                    Else
                        obj2.Doc_Date = DocDate
                        obj2.MP_Code = mpCode
                        lstMP.Add(obj2)
                    End If
                ...
                '===============================================
                For Each MP As structMP In lstMP
                    For Each grow As GridViewRowInfo In gv.Rows



                        If clsCommon.CompairString(clsCommon.myCstr(MP.MP_Code), clsCommon.myCstr(grow.Cells("Farmer Code").Value)) = CompairStringResult.Equal AndAlso clsCommon.CompairString(clsCommon.myCDate(MP.Doc_Date), clsCommon.myCDate(grow.Cells("Doc Date").Value)) = CompairStringResult.Equal Then


                            line += 1
                            obj.Document_Date = clsCommon.myCDate(grow.Cells("Doc Date").Value)
                            If clsCommon.myLen(obj.Document_Date) <= 0 Then
                                Throw New Exception("Fill Doc Date At Line No. " + clsCommon.myCstr(counter) + "")
                            End If

                            obj.Bill_To_Location = clsCommon.myCstr(grow.Cells("Loc Code").Value)
                            If obj.Bill_To_Location.Length > 12 Then
                                Throw New Exception(" Length of Loc Code can not be greater than 12 .At Line No. " + clsCommon.myCstr(counter) + "")
                            Else
                                If obj.Bill_To_Location.Length > 0 Then
                                    strName = clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM tspl_location_master Where location_code ='" & obj.Bill_To_Location & "'", trans)
                                    If strName <= 0 Then
                                        Throw New Exception("Location (" & obj.Bill_To_Location & ") does not exist in Location Master . Please make it entry first.At Line No. " + clsCommon.myCstr(counter) + "")
                                    End If
                                End If
                            End If
                            If clsCommon.myLen(Location) <= 0 Then
                                Throw New Exception("Fill Loc Code At Line No. " + clsCommon.myCstr(counter) + "")
                            End If
                            obj.Farmer_Code = clsCommon.myCstr(grow.Cells("Farmer Code").Value)
                            If obj.Farmer_Code.Length > 30 Then
                                Throw New Exception(" Length of Farmer Code can not be greater than 30. At Line No. " + clsCommon.myCstr(counter) + "")
                            Else
                                If obj.Farmer_Code.Length > 0 Then
                                    strName = clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_MP_MASTER Where MP_Code ='" & obj.Farmer_Code & "'", trans)
                                    If strName <= 0 Then
                                        Throw New Exception("Farmer (" & obj.Farmer_Code & ") does not exist in MP  Master . Please make it entry first.At Line No. " + clsCommon.myCstr(counter) + "")
                                    End If
                                End If
                            End If
                            If clsCommon.myLen(obj.Farmer_Code) <= 0 Then
                                Throw New Exception("Fill Farmer Code At Line No. " + clsCommon.myCstr(counter) + "")
                            End If

                            obj.Dept = clsCommon.myCstr(grow.Cells("Department").Value)
                            If obj.Dept.Length > 12 Then
                                Throw New Exception(" Length of Department Code can not be greater than 12 .At Line No. " + clsCommon.myCstr(counter) + "")
                            Else
                                If obj.Dept.Length > 0 Then
                                    strName = clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_DEPARTMENT_MASTER Where DEPARTMENT_CODE ='" & obj.Dept & "'", trans)
                                    If strName <= 0 Then
                                        Throw New Exception("Department (" & obj.Dept & ") does not exist in Department  Master . Please make it entry first.At Line No. " + clsCommon.myCstr(counter) + "")
                                    End If
                                End If
                            End If
                            If clsCommon.myLen(obj.Dept) <= 0 Then
                                Throw New Exception("Fill Department At Line No. " + clsCommon.myCstr(counter) + "")
                            End If

                            obj.Description = clsCommon.myCstr(grow.Cells("Description").Value)
                            If obj.Description.Length > 200 Then
                                Throw New Exception(" Length of Description can not be greater than 200.At Line No. " + clsCommon.myCstr(counter) + "")
                            End If
                            obj.Comments = clsCommon.myCstr(grow.Cells("Comment").Value)
                            If obj.Comments.Length > 200 Then
                                Throw New Exception(" Length of Comment can not be greater than 200.At Line No. " + clsCommon.myCstr(counter) + "")
                            End If
                            obj.Tax_Group = clsCommon.myCstr(grow.Cells("Tax Group").Value)

                            If obj.Tax_Group.Length > 12 Then
                                Throw New Exception(" Length of Tax Group can not be greater than 12 .At Line No. " + clsCommon.myCstr(counter) + "")
                            Else
                                If obj.Tax_Group.Length > 0 Then
                                    strName = clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_TAX_GROUP_MASTER Where Tax_Group_Code ='" & obj.Tax_Group & "'", trans)
                                    If strName <= 0 Then
                                        Throw New Exception("Tax (" & obj.Tax_Group & ") does not exist in Tax Group Master . Please make it entry first.At Line No. " + clsCommon.myCstr(counter) + "")
                                    End If
                                End If
                            End If
                            If clsCommon.myLen(obj.Tax_Group) <= 0 Then
                                Throw New Exception(" Fill Tax Group.At Line No. " + clsCommon.myCstr(counter) + "")
                            End If
                            ...
                            Dim objTr As New clsMCCMaterialSaleFarmerDetail()
                            '=======================Detail===========================
                            objTr.Line_No = clsCommon.myCstr(line)
                            objTr.Item_Code = clsCommon.myCstr(grow.Cells("Item_Code").Value)

                            objTr.Row_Type = "Item"

                            If objTr.Item_Code.Length > 50 Then
                                Throw New Exception(" Length of Item Code can not be greater than 50 .At Line No. " + clsCommon.myCstr(counter) + "")
                            Else
                                If objTr.Item_Code.Length > 0 Then
                                    strName = clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM tspl_item_master Where Item_Code ='" & objTr.Item_Code & "'", trans)
                                    If strName <= 0 Then
                                        Throw New Exception("Item (" & objTr.Item_Code & ") does not exist in Item  Master . Please make it entry first.At Line No. " + clsCommon.myCstr(counter) + "")
                                    End If
                                End If
                            End If
                            If clsCommon.myLen(objTr.Item_Code) <= 0 Then
                                Throw New Exception(" Fill Item Code .At Line No. " + clsCommon.myCstr(counter) + "")
                            End If


                            objTr.Qty = clsCommon.myCdbl(grow.Cells("Qty").Value)
                            If clsCommon.myCdbl(objTr.Qty) < 0 Then
                                Throw New Exception(" Fill Qty .At Line No. " + clsCommon.myCstr(counter) + "")
                            End If
                            objTr.Item_Cost = clsCommon.myCdbl(grow.Cells("Unit Rate").Value)
                            'If clsCommon.myLen(objTr.Item_Cost) <= 0 Then
                            '    Throw New Exception(" Fill Rate .At Line No. " + clsCommon.myCstr(counter) + "")
                            'End If

                            objTr.Unit_code = clsCommon.myCstr(grow.Cells("Unit Code").Value)
                            If objTr.Unit_code.Length > 12 Then
                                Throw New Exception(" Length of Unit Code can not be greater than 12 .At Line No. " + clsCommon.myCstr(counter) + "")
                            Else
                                If objTr.Unit_code.Length > 0 Then
                                    strName = clsDBFuncationality.getSingleValue("SELECT COUNT(*) FROM TSPL_ITEM_UOM_DETAIL Where UOM_Code ='" & objTr.Unit_code & "' and Item_Code='" + objTr.Item_Code + "'", trans)
                                    If strName <= 0 Then
                                        Throw New Exception("Item (" & objTr.Unit_code & ") for Item Code ('" + objTr.Item_Code + "')  does not exist  . Please make it entry first.At Line No. " + clsCommon.myCstr(counter) + "")
                                    End If
                                End If
                            End If
                            If clsCommon.myLen(objTr.Unit_code) <= 0 Then
                                Throw New Exception(" Fill Unit Code .At Line No. " + clsCommon.myCstr(counter) + "")
                            End If

                            objTr.Amount = objTr.Qty * objTr.Item_Cost
                            objTr.Total_Tax_Amt = objTr.Amount
                            objTr.Item_Net_Amt = objTr.Amount
                            obj.Total_Amt += objTr.Amount
                            'obj.Total_Amt = objTr.Amount
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        Try
            If clsCommon.myLen(txtDocNo.Value) > 0 Then
                ...
                End If
                If clsMCCMaterialSaleFarmer.UpdateAfterPosting(obj, txtDocNo.Value, Nothing) Then
    ...
    Private Sub RadButton238_Click(sender As Object, e As EventArgs) Handles RadButton238.Click
        Try
            ...
            If (clsCommon.MyMessageBoxShow(Me, "Delete " + clsCommon.myCstr(TxtMultiSelectFinder3.arrValueMember.Count) + "Documents." + Environment.NewLine + "Are You sure?", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question)) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Dim arr As ArrayList = TxtMultiSelectFinder3.arrValueMember
                For Each strDocNo As String In arr
                    If (clsMCCMaterialSaleFarmer.DeleteData(strDocNo)) Then
                        saveCancelLog(Reason, "Delete", strDocNo, Nothing)
                    End If
                Next
    ...
    Private Sub TxtMultiSelectFinder3__My_Click(sender As Object, e As EventArgs) Handles TxtMultiSelectFinder3._My_Click
        ...
        '-----------------------------------------------------
        '' finder query change by Panch Raj on 01-may-2018 against ticket : KDI/30/04/18-000281
        Dim qry As String = " select TSPL_MCC_Sale_Farmer_Head.Document_Code as Code,TSPL_MCC_Sale_Farmer_Head.SALE_INVOICE_NO AS [Invoice No],CONVERT(varchar(10), TSPL_MCC_Sale_Farmer_Head.Document_Date,103)+' '+ CONVERT(varchar(5), TSPL_MCC_Sale_Farmer_Head.Document_Date,114) as Date," &
                            " TSPL_MCC_Sale_Farmer_Head.Farmer_Code as [Farmer Code],TSPL_MP_MASTER.MP_Name as [Farmer Name],TSPL_MP_MASTER.VLC_Code as [VLC Code],TSPL_VLC_MASTER_HEAD.VLC_Name as [VLC Name]," &
                            " TSPL_VLC_MASTER_HEAD.VSP_Code as [VSP Code], TSPL_VENDOR_MASTER.Vendor_Name as [VSP Name],TSPL_MCC_Sale_Farmer_Head.Comments,Total_Amt as Amount, " &
                            " case when TSPL_MCC_Sale_Farmer_Head.Status=0 then 'Pending' else 'Approved' end as [Status],TSPL_MCC_Sale_Farmer_Head.Direct_Dispatch as [Direct Dispatch], " &
                            " TSPL_MCC_Sale_Farmer_Head.Bill_To_Location as [Bill To Location],TSPL_MCC_Sale_Farmer_Head.Discount_Amt as [Discount Amount],TSPL_MCC_Sale_Farmer_Head.Amount_Less_Discount as [Amt Less Discount],TSPL_MCC_Sale_Farmer_Head.Total_Tax_Amt as [Total Tax Amount], " &
                            " TSPL_MCC_Sale_Farmer_Head.Total_Amt as [Total Amount],TSPL_MCC_Sale_Farmer_Head.EWayBillNo as [EwayBill No],TSPL_MCC_Sale_Farmer_Head.EWayBillDate as [EWayBill Date],TSPL_MCC_Sale_Farmer_Head.Electronic_Ref_No as [Electronic Ref No] " &
                            " From TSPL_MCC_Sale_Farmer_Head " &
                            " left join TSPL_MP_MASTER on TSPL_MCC_Sale_Farmer_Head.Farmer_Code=TSPL_MP_MASTER.MP_Code " &
        ...
        Else
            whrClas = " TSPL_MCC_Sale_Farmer_Head.Trans_Type='MCC'"
        End If
        qry = qry + " where " + whrClas

        TxtMultiSelectFinder3.arrValueMember = clsCommon.ShowMultipleSelectForm("MMSF@Mul", qry, "Code", "Code", TxtMultiSelectFinder3.arrValueMember, TxtMultiSelectFinder3.arrDispalyMember)
    End Sub

    Private Sub RadButton239_Click(sender As Object, e As EventArgs) Handles RadButton239.Click
        Try
            If TxtMultiSelectFinder3.arrValueMember Is Nothing OrElse TxtMultiSelectFinder3.arrValueMember.Count <= 0 Then
                Throw New Exception("Please select at least one document to Reverse")
            End If
            Dim StrAllException As String = ""
            For ii As Integer = 0 To TxtMultiSelectFinder3.arrValueMember.Count - 1
                Try
                    clsMCCMaterialSaleFarmer.ReverseAndUnpost(TxtMultiSelectFinder3.arrValueMember(ii))
                Catch ex As Exception
                    StrAllException += "Error In Document no:" + TxtMultiSelectFinder3.arrValueMember(ii) + Environment.NewLine + ex.Message
                End Try
            Next
