...
Public Class rptQCAnalysisReport
    ...
    Private Sub txtRALNo__My_Click(sender As Object, e As EventArgs) Handles txtRALNo._My_Click
        If clsCommon.myLen(txtVendor.Value) <= 0 AndAlso clsCommon.myLen(txtVendor.Value) <= 0 Then
            ...
            If clsCommon.myLen(txtLocation.Value) > 0 Then
                qry1 += " where TSPL_TENDER_DETAIL.Location='" + txtLocation.Value + "'"
            End If
...
ORDER BY PivotResult.Item_Code "
    ...
    Private Sub btnPrintA4_Click(sender As Object, e As EventArgs) Handles btnPrintA4.Click
        AnalysisPrintt(True)
    End Sub
    Private Sub AnalysisPrintt(ByVal isA4Size As Boolean)
        Try
            ...
            '                      and  TSPL_QC_CHECK_SRN_DETAIL.Item_Code = '" + txtItemCode.Value + "'
            '                      and TSPL_LOCATION_MASTER.Location_Code='" + txtLocation.Value + "'"
            '    If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
            ...
            Else
                ...
                If isA4Size = True Then
                    frmCRV.funreport(MyBase.Form_ID, CrystalReportFolder.PurchaseOrder, dt, "rptQCAnalysisReportMultiple", "Analysis Report", Nothing)
                ElseIf isA4Size = False Then
                    frmCRV.funreport(MyBase.Form_ID, CrystalReportFolder.PurchaseOrder, dt, "rptQCAnalysisReportMultiple", "Analysis Report", Nothing)
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub btnAnalysisPrintVertical_Click(sender As Object, e As EventArgs)

    End Sub

    Private Sub btnAnalysisPrintVertical_Click_1(sender As Object, e As EventArgs) Handles btnAnalysisPrintVertical.Click
        Try
            AnalysisPrint(True)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        ...
        Try
            ...
            Else
                If IsPrintVertical = True Then
                    frmCRV.funreport(MyBase.Form_ID, CrystalReportFolder.PurchaseOrder, dt, "rptQCAnalysisReportVertical", "Analysis Report", Nothing)
