...
where TSPL_SPP_PRODUCTION_CONSUMPTION_DETAIL.PROD_ENTRY_CODE='" + txtCode.Value + "'"
        ...
        Try
            ...
            If Not isInsideLoadData Then
                If Not isCellValueChanged Then
                    ...
                    'If e.Column Is gvBatch.Columns(colFINAL_PROD_QtyInBag) Then
                    '    isCellValueChanged = True
                    '    gvBatch.CurrentRow.Cells(colFINAL_PROD_Qty).Value = gvBatch.CurrentRow.Cells(colFINAL_PROD_QtyInBag).Value * 1
                    '    isCellValueChanged = False
                    'End If
    ...
    Sub OpenBOMCode(ByVal isButtonClicked As Boolean)
        ...
        Dim qry As String = " Select * from (select XXX.bom_code as  Code,MAX(XXX.BOM_DATE) AS [BOM Date],max(xxx.DESCRIPTION) as DESCRIPTION,MAX(xxx.[Valid From]) as [Valid From],MAX(xxx.[Valid Upto]) as [Valid Upto],MAX(xxx.Status) as Status,MAX(xxx.[Main Item Code]) as [Main Item Code],MAX(xxx.[Item Description]) as [Item Description],MAX(xxx.Unit) as  [Unit],MAX(xxx.Quantity) as [Quantity],MAX(XXX.REVISION_NO) AS [Revision No],MAX(xxx.Conversion_Factor) as Conversion_Factor,MAX(xxx.UOM_Code) as UOM_Code  from (select  LOCATION_CODE,(PROD_ITEM_CODE) as PROD_ITEM_CODE,max(TSPL_MF_BOM_HEAD.BOM_CODE) as bom_code, MAX(TSPL_MF_BOM_HEAD.BOM_DATE) AS BOM_DATE,max(TSPL_MF_BOM_HEAD.DESCRIPTION) DESCRIPTION,MAX(TSPL_MF_BOM_HEAD.REVISION_NO) AS REVISION_NO,max(TSPL_MF_BOM_HEAD.START_DATE) as [Valid From],max(TSPL_MF_BOM_HEAD.END_DATE) as [Valid Upto],max(TSPL_MF_BOM_HEAD.Status) as Status,max(TSPL_MF_BOM_HEAD.prod_item_code) as [Main Item Code],  max(tspl_item_master.item_desc) as [Item Description],max(TSPL_MF_BOM_HEAD.prod_item_unit_code) as [Unit],max(TSPL_MF_BOM_HEAD.prod_quantity) as [Quantity], max(TSPL_ITEM_UOM_DETAIL.Conversion_Factor) as Conversion_Factor,
                            max(TSPL_ITEM_UOM_DETAIL.UOM_Code) as UOM_Code 
                            from TSPL_MF_BOM_HEAD  left outer join TSPL_MF_BOM_DETAIL on TSPL_MF_BOM_HEAD.BOM_CODE=TSPL_MF_BOM_DETAIL.BOM_CODE LEFT OUTER JOIN TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_MF_BOM_HEAD.PROD_ITEM_CODE  left outer join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_MF_BOM_HEAD.PROD_ITEM_CODE and TSPL_ITEM_UOM_DETAIL.UOM_Code='BAG' WHERE  TSPL_ITEM_MASTER.FG_for_CF=1 and TSPL_MF_BOM_HEAD.POSTED='1'   GROUP BY TSPL_MF_BOM_HEAD.LOCATION_CODE,TSPL_MF_BOM_HEAD.PROD_ITEM_CODE
                            UNION ALL 

                            Select xx.LOCATION_CODE,xx.PROD_ITEM_CODE,max(xx.bom_code) as bom_code ,mAX(xx.BOM_DATE) AS BOM_DATE,max(xx.DESCRIPTION) DESCRIPTION,MAX(xx.REVISION_NO) AS REIVISION_NO,
                            max(xx.[Valid From]) as [Valid From],max(xx.[Valid Upto]) as [Valid Upto],max(xx.STATUS) as STATUS,max(xx.[Main Item Code]) as [Main Item Code],max(xx.[Item Description]) as [Item Description],max(xx.Unit) as Unit,max(xx.Quantity) as Quantity,max(xx.Conversion_Factor) as Conversion_Factor,max(xx.UOM_Code) as UOM_Code from (
                                select  LOCATION_CODE,(PROD_ITEM_CODE) as PROD_ITEM_CODE,(TSPL_MF_BOM_HEAD.BOM_CODE) as bom_code,(TSPL_MF_BOM_HEAD.DESCRIPTION) DESCRIPTION,
                                (TSPL_MF_BOM_HEAD.BOM_DATE) AS BOM_DATE,(TSPL_MF_BOM_HEAD.REVISION_NO) AS REVISION_NO,
                                TSPL_MF_BOM_HEAD.START_DATE as [Valid From],TSPL_MF_BOM_HEAD.END_DATE as [Valid Upto],TSPL_MF_BOM_HEAD.Status,TSPL_MF_BOM_HEAD.prod_item_code as [Main Item Code],  tspl_item_master.item_desc as [Item Description],TSPL_MF_BOM_HEAD.prod_item_unit_code as [Unit],TSPL_MF_BOM_HEAD.prod_quantity as [Quantity], TSPL_ITEM_UOM_DETAIL.Conversion_Factor,TSPL_ITEM_UOM_DETAIL.UOM_Code
                                from TSPL_MF_BOM_HEAD 
                                left outer join TSPL_MF_BOM_DETAIL on TSPL_MF_BOM_HEAD.BOM_CODE=TSPL_MF_BOM_DETAIL.BOM_CODE
                                LEFT OUTER JOIN TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_MF_BOM_HEAD.PROD_ITEM_CODE 
                                ...
                                WHERE  TSPL_ITEM_MASTER.FG_for_CF=1 and TSPL_MF_BOM_HEAD.POSTED='1' and TSPL_MF_BOM_DETAIL.CONSM_ITEM_CODE='RM0006'  
                            )xx GROUP BY  xx.LOCATION_CODE,xx.PROD_ITEM_CODE
     
                                UNION ALL 
                            select xy.LOCATION_CODE,xy.PROD_ITEM_CODE,max(xy.bom_code) as bom_code ,mAX(xy.BOM_DATE) AS BOM_DATE,max(xy.DESCRIPTION) DESCRIPTION,MAX(xy.REVISION_NO) AS REIVISION_NO,max(xy.[Valid From]) as [Valid From],max(xy.[Valid Upto]) as [Valid Upto],max(xy.STATUS) as STATUS,max(xy.[Main Item Code]) as [Main Item Code],max(xy.[Item Description]) as [Item Description],max(xy.Unit) as Unit,max(xy.Quantity) as Quantity,max(xy.Conversion_Factor) as Conversion_Factor,max(xy.UOM_Code) as UOM_Code   from (
                            select  LOCATION_CODE,(PROD_ITEM_CODE) as PROD_ITEM_CODE,(TSPL_MF_BOM_HEAD.BOM_CODE) as bom_code,(TSPL_MF_BOM_HEAD.DESCRIPTION) DESCRIPTION,
                            (TSPL_MF_BOM_HEAD.BOM_DATE) AS BOM_DATE,(TSPL_MF_BOM_HEAD.REVISION_NO) AS REVISION_NO,
                            TSPL_MF_BOM_HEAD.START_DATE as [Valid From],TSPL_MF_BOM_HEAD.END_DATE as [Valid Upto],TSPL_MF_BOM_HEAD.Status,TSPL_MF_BOM_HEAD.prod_item_code as [Main Item Code],  tspl_item_master.item_desc as [Item Description],TSPL_MF_BOM_HEAD.prod_item_unit_code as [Unit],TSPL_MF_BOM_HEAD.prod_quantity as [Quantity],TSPL_MF_BOM_HEAD.revision_no as [Revision No], TSPL_ITEM_UOM_DETAIL.Conversion_Factor,TSPL_ITEM_UOM_DETAIL.UOM_Code
                            from TSPL_MF_BOM_HEAD 
                            left outer join TSPL_MF_BOM_DETAIL on TSPL_MF_BOM_HEAD.BOM_CODE=TSPL_MF_BOM_DETAIL.BOM_CODE
                            LEFT OUTER JOIN TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_MF_BOM_HEAD.PROD_ITEM_CODE 
                            left outer join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_MF_BOM_HEAD.PROD_ITEM_CODE and TSPL_ITEM_UOM_DETAIL.UOM_Code='BAG'
                            WHERE  TSPL_ITEM_MASTER.FG_for_CF=1 and TSPL_MF_BOM_HEAD.POSTED='1' and TSPL_MF_BOM_DETAIL.CONSM_ITEM_CODE='RM0001'  
                            )xY GROUP BY  xY.LOCATION_CODE,xY.PROD_ITEM_CODE
                            )XXX
                            WHERE XXX.LOCATION_CODE='" + txtLocation.Value + "' 
                            GROUP BY XXX.LOCATION_CODE,XXX.PROD_ITEM_CODE,XXX.bom_code ) xyyx "
