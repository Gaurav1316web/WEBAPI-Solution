...
Public Class frmOutgoingQCEntry
    ...
    Public Shared Function GetItemType() As DataTable
        ...
        Dim RowTypeItem2 As String = "Yes"
        Dim RowTypeItem3 As String = "No"
    ...
    Private Sub txtprodCode__My_Click(sender As Object, e As EventArgs) Handles txtprodCode._My_Click
        Dim qry As String = "select tspl_spp_production_entry_detail.PROD_ENTRY_CODE as Code ,PROD_DATE as Date from tspl_spp_production_entry
							...
							and tspl_spp_production_entry.prod_entry_code not in ( select   TSPL_PROD_QC_CHECK_PRODUCTION_ENTRY.PROD_ENTRY_CODE from  TSPL_PROD_QC_CHECK_PRODUCTION_ENTRY) "
        ...
        If txtprodCode.arrValueMember.Count > 1 Then
    ...
    '        'nature='R'
    '        'NATURE='A'
    '        If clsCommon.myCDecimal(grow.Cells(colCompiledUn).Value) >= 0 Then
    '            Dim strTempResult As String = ""
    '            For Each grow1 As GridViewRowInfo In gv1.Rows
    '                If clsCommon.CompairString(clsCommon.myCstr(grow1.Cells(colCompiledUn).Value), "No") = CompairStringResult.Equal Then
    '                    strTempResult = "No"
    '                ElseIf clsCommon.CompairString(clsCommon.myCstr(grow1.Cells(colCompiledUn).Value), "Compiled") = CompairStringResult.Equal Then
    '                    strTempResult = "Compiled"
    '                ElseIf clsCommon.CompairString(clsCommon.myCstr(grow1.Cells(colCompiledUn).Value), "Uncompiled") = CompairStringResult.Equal Then
    '                    strTempResult = "Uncompiled"
    '                ElseIf clsCommon.CompairString(clsCommon.myCstr(grow1.Cells(colCompiledUn).Value), "Yes") = CompairStringResult.Equal Then
    '                    strTempResult = "Yes"
    '                End If
    '            Next
    ...
    '            ElseIf clsCommon.CompairString(strTempResult, "Compiled") = CompairStringResult.Equal Then
    ...
    '            ElseIf clsCommon.CompairString(strTempResult, "Uncompiled") = CompairStringResult.Equal Then
    '                rbtnRej.IsChecked = True
    '                rbtnApp.IsChecked = False
    '                rbtnUD.IsChecked = False
    '                txtAccept.Text = "Rejected"
    ...
    '            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    '            ' Retrieve the value of the "Nature" field from the first row
