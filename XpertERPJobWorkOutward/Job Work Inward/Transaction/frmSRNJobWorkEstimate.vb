...
Public Class FrmSRNJobWorkEstimate
...
#Region "Varibales"
    ...
    Const colWeightDocCode As String = "colWeightDocCode"
    Const colWeightDocDate As String = "colWeightDocDate"
...
#End Region
    ...
    Private Sub FrmSRNJobWorkEstimate_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ''BHA/03/01/19-000773 by balwinder on 07/01/2019
    ...
    Sub LoadBlankGridWeighment()
        ...
        repoTxtBox.Name = colWeightDocCode
        ...
        repoDateBox.Name = colWeightDocDate
    ...
    Private Sub txtItemStructure__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtItemStructureFAT._MYValidating
        If clsCommon.myLen(txtLocation.Value) > 0 AndAlso clsCommon.myLen(lblVendorCode.Text) > 0 Then
            ...
            qry = " select  top 1 TSPL_VENDOR_FORMULA_MAPPING.Doccode,TSPL_JW_FORMULA.Code as Formula_Code, TSPL_JW_FORMULA.Description as Formula_Desc,TSPL_JW_FORMULA.Formula ,convert (varchar,TSPL_JW_FORMULA.Created_Date,103) as Formula_Date  from TSPL_VENDOR_FORMULA_MAPPING  " &
                                " left outer join TSPL_VENDOR_FORMULA_MAPPING_DETAIL on TSPL_VENDOR_FORMULA_MAPPING.DocCode = TSPL_VENDOR_FORMULA_MAPPING_DETAIL.DocCode " &
                                " left outer join TSPL_JW_FORMULA on TSPL_JW_FORMULA.Code =TSPL_VENDOR_FORMULA_MAPPING.FormulaCode " &
                                "  where TSPL_VENDOR_FORMULA_MAPPING_DETAIL.vendorCode = '" + lblVendorCode.Text + "' and TSPL_JW_FORMULA.Structure_Code = '" + txtItemStructureFAT.Value + "' and TSPL_VENDOR_FORMULA_MAPPING.Posted = 1 and  convert (date, TSPL_VENDOR_FORMULA_MAPPING.DocDate,103) <=  Convert (date,'" + txtDocumentDate.Value + "',103)  " &
                                "  order by TSPL_VENDOR_FORMULA_MAPPING.DocDate desc "
            ' "  order by convert (datetime, TSPL_VENDOR_FORMULA_MAPPING.DocDate,103) desc "
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
    ...
    Private Sub txtItemStructureSNF__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtItemStructureSNF._MYValidating
        If clsCommon.myLen(txtLocation.Value) > 0 AndAlso clsCommon.myLen(lblVendorCode.Text) > 0 Then
            ...
            qry = " select  top 1 TSPL_VENDOR_FORMULA_MAPPING.Doccode,TSPL_JW_FORMULA.Code as Formula_Code, TSPL_JW_FORMULA.Description as Formula_Desc,TSPL_JW_FORMULA.Formula ,convert (varchar,TSPL_JW_FORMULA.Created_Date,103) as Formula_Date  from TSPL_VENDOR_FORMULA_MAPPING  " &
                                " left outer join TSPL_VENDOR_FORMULA_MAPPING_DETAIL on TSPL_VENDOR_FORMULA_MAPPING.DocCode = TSPL_VENDOR_FORMULA_MAPPING_DETAIL.DocCode " &
                                " left outer join TSPL_JW_FORMULA on TSPL_JW_FORMULA.Code =TSPL_VENDOR_FORMULA_MAPPING.FormulaCode " &
                                "  where TSPL_VENDOR_FORMULA_MAPPING_DETAIL.vendorCode = '" + lblVendorCode.Text + "' and TSPL_JW_FORMULA.Structure_Code = '" + txtItemStructureSNF.Value + "' and TSPL_VENDOR_FORMULA_MAPPING.Posted = 1 and  convert (date, TSPL_VENDOR_FORMULA_MAPPING.DocDate,103) <=  Convert (date,'" + txtDocumentDate.Value + "',103) " &
                                "  order by TSPL_VENDOR_FORMULA_MAPPING.DocDate desc "
            ' "  order by convert (datetime, TSPL_VENDOR_FORMULA_MAPPING.DocDate,103) desc "
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
            If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        Try
            ...
            End If
            ''BHA/03/12/18-000738 by balwinder on 04/12/2018
            Dim BaseQry As String = "select Tspl_Gate_Entry_Details.snf_Per as GE_SNF_Per,Tspl_Gate_Entry_Details.fat_per as GE_FAT_Per , Weighment_No,Weighment_Date,Vehicle_No_Manual,TSPL_GENERAL_WEIGHMENT_DETAIL.Item_Code,tspl_item_master.Item_Desc, Net_Weight,'KG' as UOM from TSPL_GENERAL_WEIGHMENT_DETAIL " + Environment.NewLine + _
            "left outer join tspl_item_master on tspl_item_master.Item_Code=TSPL_GENERAL_WEIGHMENT_DETAIL.Item_Code" + Environment.NewLine + _
            "left join Tspl_Gate_Entry_Details on Tspl_Gate_Entry_Details.Gate_Entry_No =TSPL_GENERAL_WEIGHMENT_DETAIL.Gate_Entry_No " + Environment.NewLine + _
            "where 2=2 and TSPL_GENERAL_WEIGHMENT_DETAIL.Posted=1 and TSPL_GENERAL_WEIGHMENT_DETAIL.IsJobWork=1 and TSPL_GENERAL_WEIGHMENT_DETAIL.Location_Code='" + txtLocation.Value + "' and TSPL_GENERAL_WEIGHMENT_DETAIL.Item_Code is not null" + Environment.NewLine + _
            "and not exists (select 1 from TSPL_JWI_ESTIMATION_WEIGHMENT where TSPL_JWI_ESTIMATION_WEIGHMENT.Weighment_Code=TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_No and TSPL_JWI_ESTIMATION_WEIGHMENT.Document_NO not in ('" + txtDocumentNo.Value + "')) and Weighment_Date>='" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtFromDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "' and Weighment_Date<='" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(txtToDate.Value), "dd/MMM/yyyy hh:mm:ss tt") + "'" + Environment.NewLine
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                If dt.Rows.Count > 1 Then
                    strICode = clsCommon.ShowSelectForm("Item@JWES", qry, "Item_Code", "", "", "", True)
            ...
            qry = BaseQry
            If clsCommon.myLen(strICode) > 0 Then
                qry += " and TSPL_GENERAL_WEIGHMENT_DETAIL.Item_Code='" + strICode + "'"
            End If
            qry += " order by Weighment_Date "
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                Dim jj As Integer = 0
                Dim dtPrevious As Date = clsCommon.myCDate(dt.Rows(0)("Weighment_Date")).AddDays(-1)
                For ii As Integer = 0 To dt.Rows.Count - 1
                    Dim dtDocDate As Date = clsCommon.myCDate(dt.Rows(ii)("Weighment_Date"))
                    If Not (dtDocDate.Year = dtPrevious.Year AndAlso dtDocDate.Month = dtPrevious.Month AndAlso dtDocDate.Day = dtPrevious.Day) Then
                        ...
                        Try
                            gvSNF.Rows(gvSNF.Rows.Count - 1).Cells("MOISTURE").Value = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" Select isnull (TSPL_JW_FORMULA_DETAILS.Value,0) as Value from TSPL_JW_FORMULA_DETAILS Left Outer Join TSPL_JW_Parameter_MASTER on TSPL_JW_Parameter_MASTER.Code =TSPL_JW_FORMULA_DETAILS.Parameter_Code where TSPL_JW_FORMULA_DETAILS.code = '" + lblFormulaCodeSNF.Text + "' and IS_MOISTURE =1  "))
                    ...
                    gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colWeightDocCode).Value = clsCommon.myCstr(dt.Rows(ii)("Weighment_No"))
                    gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colWeightDocDate).Value = clsCommon.myCDate(dt.Rows(ii)("Weighment_Date"))
                    gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colWeightTankerNo).Value = clsCommon.myCstr(dt.Rows(ii)("Vehicle_No_Manual"))
    ...
    Sub calculateWeighmentFATSNFKG(ByVal intRowIndex As Integer)
        ...
        Dim objJobEst As clsJWFormulaSoln = clsJWFormulaSoln.CalculateFormula(lblFormulaCodeFAT.Text, clsCommon.myCdbl(gvWeighment.Rows(intRowIndex).Cells(colWeightFATKg).Value), clsCommon.myCdbl(gvWeighment.Rows(intRowIndex).Cells(colWeightSNFKG).Value), Nothing)
        If objJobEst IsNot Nothing Then
            ...
            End If
        End If
        objJobEst = clsJWFormulaSoln.CalculateFormula(lblFormulaCodeSNF.Text, clsCommon.myCdbl(gvWeighment.Rows(intRowIndex).Cells(colWeightFATKg).Value), clsCommon.myCdbl(gvWeighment.Rows(intRowIndex).Cells(colWeightSNFKG).Value), Nothing)
        If objJobEst IsNot Nothing Then
    ...
    Sub calculateFAT(ByVal intParentSNo As Integer)
        ...
        If RowIndex >= 0 Then
            ...
            End If
            Dim objJobEst As clsJWFormulaSoln = clsJWFormulaSoln.CalculateFormula(lblFormulaCodeFAT.Text, totalFATKG, 0, arrQC)
            If objJobEst IsNot Nothing Then
                If objJobEst.Type = 1 Then
    ...
    Sub calculateSNF(ByVal intParentSNo As Integer)
        ...
        If RowIndex >= 0 Then
            ...
            End If
            Dim objJobEst As clsJWFormulaSoln = clsJWFormulaSoln.CalculateFormula(lblFormulaCodeSNF.Text, 0, totalSNFKG, arrQC)
            If objJobEst IsNot Nothing Then
                If objJobEst.Type = 2 Then
    ...
    Sub OpenFATBOMCode(ByVal isButtonClicked As Boolean)
        ...
        If clsCommon.myLen(icode) > 0 Then
            whrCls = " isnull(TSPL_PP_BOM_HEAD.is_osp,0)<>1 and TSPL_PP_BOM_HEAD.prod_item_code='" + icode + "' and TSPL_PP_BOM_HEAD.item_category_code='" + txtItemStructureFAT.Value + "' and tspl_item_master.Structure_Code='" + txtItemStructureFAT.Value + "' and isnull(TSPL_PP_BOM_HEAD.is_post,'0')='1' "
        Else
            whrCls = " isnull(TSPL_PP_BOM_HEAD.is_osp,0)<>1 and tspl_item_master.Structure_Code='" + txtItemStructureFAT.Value + "' and TSPL_PP_BOM_HEAD.item_category_code='" + txtItemStructureFAT.Value + "' and tspl_item_master.item_type in ('F','S') and isnull(TSPL_PP_BOM_HEAD.is_post,'0')='1' "
        End If
    ...
    Sub OpenSNFBOMCode(ByVal isButtonClicked As Boolean)
        ...
        If clsCommon.myLen(icode) > 0 Then
            whrCls = " isnull(TSPL_PP_BOM_HEAD.is_osp,0)<>1 and TSPL_PP_BOM_HEAD.prod_item_code='" + icode + "' and TSPL_PP_BOM_HEAD.item_category_code='" + txtItemStructureSNF.Value + "' and tspl_item_master.Structure_Code='" + txtItemStructureSNF.Value + "' and isnull(TSPL_PP_BOM_HEAD.is_post,'0')='1' "
        Else
            whrCls = " isnull(TSPL_PP_BOM_HEAD.is_osp,0)<>1 and tspl_item_master.Structure_Code='" + txtItemStructureSNF.Value + "' and TSPL_PP_BOM_HEAD.item_category_code='" + txtItemStructureSNF.Value + "' and tspl_item_master.item_type in ('F','S') and isnull(TSPL_PP_BOM_HEAD.is_post,'0')='1' "
        End If
    ...
    Private Sub SaveData()
        Try
            If (AllowToSave()) Then
                ...
                obj.ArrWeighment = New List(Of clsJWIEstimateWeighment)
                For ii As Integer = 0 To gvWeighment.Rows.Count - 1
                    Dim objTr As New clsJWIEstimateWeighment()
                    ...
                    objTr.Weighment_Code = clsCommon.myCstr(gvWeighment.Rows(ii).Cells(colWeightDocCode).Value)
                    objTr.Weighment_Date = clsCommon.myCDate(gvWeighment.Rows(ii).Cells(colWeightDocDate).Value)
                ...
                obj.ArrFATProduction = New List(Of clsJWIEstimateFATProduction)
                For ii As Integer = 0 To gvFAT.Rows.Count - 1
                    Dim objTr As New clsJWIEstimateFATProduction()
                    ...
                    objTr.ArrQC = New List(Of clsJWIEstimateFATProductionQCParam)
                    For jj As Integer = contQCParameterStartColumnInedx To gvFAT.Columns.Count - 1
                        Dim objParam As clsJWIEstimateFATProductionQCParam = New clsJWIEstimateFATProductionQCParam()
                ...
                Next
                obj.ArrSNFProducion = New List(Of clsJWIEstimateSNFProduction)
                For ii As Integer = 0 To gvSNF.Rows.Count - 1
                    Dim objTr As New clsJWIEstimateSNFProduction()
                    ...
                    objTr.ArrQC = New List(Of clsJWIEstimateSNFProductionQCParam)
                    For jj As Integer = contQCParameterStartColumnInedx To gvSNF.Columns.Count - 1
                        Dim objParam As clsJWIEstimateSNFProductionQCParam = New clsJWIEstimateSNFProductionQCParam()
                ...
                Next
                obj.ArrRawItem = New List(Of clsJWIEstimateRawItem)
                For ii As Integer = 0 To gvRawItem.Rows.Count - 1
                    Dim objTr As New clsJWIEstimateRawItem()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            AddNew()
            Dim obj As clsJWEStimate = clsJWEStimate.GetData(strCode, NavTyep, Nothing)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_NO) > 0) Then
                ...
                For ii As Integer = 0 To obj.ArrWeighment.Count - 1
                    If ii = 0 Then
                        txtFromDate.Value = obj.ArrWeighment(ii).Weighment_Date
                    End If
                    If ii = obj.ArrWeighment.Count - 1 Then
                        txtToDate.Value = obj.ArrWeighment(ii).Weighment_Date
                    End If
                    gvWeighment.Rows.AddNew()
                    ...
                    gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colWeightDocCode).Value = obj.ArrWeighment(ii).Weighment_Code
                    gvWeighment.Rows(gvWeighment.Rows.Count - 1).Cells(colWeightDocDate).Value = obj.ArrWeighment(ii).Weighment_Date
                ...
                For ii As Integer = 0 To obj.ArrFATProduction.Count - 1
                    ...
                    For Each objTr As clsJWIEstimateFATProductionQCParam In obj.ArrFATProduction(ii).ArrQC
                        Try
                ...
                For ii As Integer = 0 To obj.ArrSNFProducion.Count - 1
                    ...
                    For Each objTr As clsJWIEstimateSNFProductionQCParam In obj.ArrSNFProducion(ii).ArrQC
                        Try
    ...
    Private Sub txtDocumentNo__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtDocumentNo._MYValidating
        Try
            Dim qst As String = "Select count(*) from TSPL_JWI_ESTIMATION_HEAD where document_No='" + txtDocumentNo.Value + "'"
    ...
    Public Sub funPrint(ByVal strDocNo As String)
        Try
            ...
            Dim qry As String = " select TSPL_JW_ESTIMATE_DETAILS.Document_No , convert(varchar,TSPL_JWI_ESTIMATION_HEAD.Document_Date,103) as Document_Date , TSPL_JWI_ESTIMATION_HEAD.JW_Location , tspl_location_master.location_Desc ,TSPL_JWI_ESTIMATION_HEAD.Vendor_code,tspl_vendor_master.Vendor_Name, TSPL_JWI_ESTIMATION_HEAD.Item_Structure, convert(date, TSPL_JWI_ESTIMATION_HEAD.Formula_Date,103) as Formula_Date , TSPL_JWI_ESTIMATION_HEAD.Formula, TSPL_JWI_ESTIMATION_HEAD.JW_Estimat, TSPL_JWI_ESTIMATION_HEAD.Created_By, convert(varchar, TSPL_JWI_ESTIMATION_HEAD.Created_Date,103) as Created_Date, TSPL_JWI_ESTIMATION_HEAD.Modified_By, convert (varchar,TSPL_JWI_ESTIMATION_HEAD.Modified_Date,103) as Modified_Date , convert(varchar, TSPL_JWI_ESTIMATION_HEAD.From_Date,103) as From_Date,convert(varchar, TSPL_JWI_ESTIMATION_HEAD.To_Date,103) as To_Date, " & _
                                " TSPL_JW_ESTIMATE_DETAILS.SRN_NO , Convert(varchar, TSPL_JW_ESTIMATE_DETAILS.SRN_Date,103) as SRN_Date , TSPL_JW_ESTIMATE_DETAILS.UOM , TSPL_JW_ESTIMATE_DETAILS.Qty , TSPL_JW_ESTIMATE_DETAILS.FAT_PER ,TSPL_JW_ESTIMATE_DETAILS. SNF_PER , TSPL_JW_ESTIMATE_DETAILS.FAT_KG ,TSPL_JW_ESTIMATE_DETAILS. SNF_KG " & _
                                " from   TSPL_JW_ESTIMATE_DETAILS left outer join TSPL_JWI_ESTIMATION_HEAD on TSPL_JW_ESTIMATE_DETAILS.Document_No = TSPL_JWI_ESTIMATION_HEAD.Document_No " & _
                                " left outer join tspl_location_master on tspl_location_master.Location_Code = TSPL_JWI_ESTIMATION_HEAD.JW_Location " & _
                                " left  outer join tspl_vendor_master on tspl_vendor_master.Vendor_Code= TSPL_JWI_ESTIMATION_HEAD.Vendor_code " & _
    ...
    Sub PostData()
        Try
            ...
            If clsCommon.myCdbl(lblFATKGWeighmentEst.Text) = 0 AndAlso clsCommon.myCdbl(lblFATKGWeighment.Text) > 0 Then
                Throw New Exception("Total Estimated FAT Kg [" + clsCommon.myCstr(lblFATKGWeighmentEst.Text) + "] but Weighment FAT KG [" + clsCommon.myCstr(lblFATKGWeighment.Text) + "]")
            End If
            If clsCommon.myCdbl(lblSNFKGWeighmentEst.Text) = 0 AndAlso clsCommon.myCdbl(lblSNFKGWeighment.Text) > 0 Then
        ...
        Try
            ...
            ElseIf e.Alt AndAlso e.Shift AndAlso e.Control And e.KeyCode = Keys.F12 Then
                ...
                ButtonToolTip.SetToolTip(btnSave, "Press Alt+S for Save/Update Trasnaction" + Environment.NewLine + _
                                        ...
                                        "TSPL_JWI_ESTIMATION_FAT_PRODUCTION" + Environment.NewLine + _
                                        "TSPL_JWI_ESTIMATION_FAT_PRODUCTION_QC_PARAMETER" + Environment.NewLine + _
                                        "TSPL_JWI_ESTIMATION_SNF_PRODUCTION" + Environment.NewLine + _
                                        "TSPL_JWI_ESTIMATION_SNF_PRODUCTION_QC_PARAMETER" + Environment.NewLine + _
                                        "TSPL_JWI_ESTIMATION_RAW_ITEM")
    ...
    Private Sub btnunpost_Click(sender As Object, e As EventArgs) Handles btnunpost.Click
        Try
            ...
            Else
                Errorcontrol.ResetError(txtDocumentNo)
            End If
            Dim TempJobWorkBilling As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select TSPL_JOBWORK_BILLING_DETAIL.Document_Code from TSPL_JOBWORK_BILLING_DETAIL where JW_EstimationNo='" + txtDocumentNo.Value + "'"))
            If clsCommon.myLen(TempJobWorkBilling) > 0 Then
    ...
    Private Sub txtDocumentDate_ValueChanged(sender As Object, e As EventArgs) Handles txtDocumentDate.ValueChanged
        ...
        'If clsCommon.myLen(txtItemStructureFAT.Value) > 0 Then
        '    If clsCommon.myLen(txtLocation.Value) > 0 AndAlso clsCommon.myLen(lblVendorCode.Text) > 0 Then
        '        Dim qry As String = " select  top 1 TSPL_VENDOR_FORMULA_MAPPING.Doccode,TSPL_JW_FORMULA.Code as Formula_Code, TSPL_JW_FORMULA.Description as Formula_Desc,TSPL_JW_FORMULA.Formula ,convert (varchar,TSPL_JW_FORMULA.Created_Date,103) as Formula_Date  from TSPL_VENDOR_FORMULA_MAPPING  " & _
        '                            " left outer join TSPL_VENDOR_FORMULA_MAPPING_DETAIL on TSPL_VENDOR_FORMULA_MAPPING.DocCode = TSPL_VENDOR_FORMULA_MAPPING_DETAIL.DocCode " & _
        '                            " left outer join TSPL_JW_FORMULA on TSPL_JW_FORMULA.Code =TSPL_VENDOR_FORMULA_MAPPING.FormulaCode " & _
        '                            "  where TSPL_VENDOR_FORMULA_MAPPING_DETAIL.vendorCode = '" + lblVendorCode.Text + "' and TSPL_JW_FORMULA.Structure_Code = '" + txtItemStructureFAT.Value + "' and TSPL_VENDOR_FORMULA_MAPPING.Posted = 1 and  convert (date, TSPL_VENDOR_FORMULA_MAPPING.DocDate,103) <=  Convert (date,'" + txtDocumentDate.Value + "',103)  " & _
        '                            "  order by TSPL_VENDOR_FORMULA_MAPPING.Doccode desc "
        '        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        '        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
        '            lblFormulaCodeFAT.Text = clsCommon.myCstr(dt.Rows(0)("Formula_Code"))
        '            lblFormulaNameFAT.Text = clsCommon.myCstr(dt.Rows(0)("Formula_Desc"))
        ...
        '                            " left outer join TSPL_VENDOR_FORMULA_MAPPING_DETAIL on TSPL_VENDOR_FORMULA_MAPPING.DocCode = TSPL_VENDOR_FORMULA_MAPPING_DETAIL.DocCode " & _
        '                            " left outer join TSPL_JW_FORMULA on TSPL_JW_FORMULA.Code =TSPL_VENDOR_FORMULA_MAPPING.FormulaCode " & _
        '                            "  where TSPL_VENDOR_FORMULA_MAPPING_DETAIL.vendorCode = '" + lblVendorCode.Text + "' and TSPL_JW_FORMULA.Structure_Code = '" + txtItemStructureSNF.Value + "' and TSPL_VENDOR_FORMULA_MAPPING.Posted = 1 and  convert (date, TSPL_VENDOR_FORMULA_MAPPING.DocDate,103) <=  Convert (date,'" + txtDocumentDate.Value + "',103) " & _
        '                            "  order by TSPL_VENDOR_FORMULA_MAPPING.Doccode desc "
    ...
    Private Sub RadButton291_Click(sender As Object, e As EventArgs) Handles RadButton291.Click
        Try
            Dim qry As String = " select TSPL_JWI_ESTIMATION_HEAD.Document_NO as DocumentNo,TSPL_JOURNAL_MASTER.Voucher_No as VoucherNo from TSPL_JWI_ESTIMATION_HEAD" + Environment.NewLine +
            " left outer join TSPL_JOURNAL_MASTER on TSPL_JOURNAL_MASTER.Source_Doc_No=TSPL_JWI_ESTIMATION_HEAD.Document_NO " + Environment.NewLine +
            ...
            If arr IsNot Nothing AndAlso arr.Count > 0 Then
                ...
                QryInsert += "select DocumentNo,VoucherNo from (" & qry & " and TSPL_JWI_ESTIMATION_HEAD.Document_NO in(" + clsCommon.GetMulcallString(arr) & ")) Rev"
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                If common.clsCommon.MyMessageBoxShow(Me, "Recreate Journal Entry of " + clsCommon.myCstr(dt.Rows.Count) + " Jobwork ESTIMATE", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                    ...
                    For ii As Integer = 0 To dt.Rows.Count - 1
                        ...
                        Try
                            Dim obj As clsJWEStimate = clsJWEStimate.GetData(strDocNo, NavigatorType.Current, trans)
                            ...
                            If CreateJVofPackingMaterialofJWInwardinJWEstimate Then
                                clsJWEStimate.CreateJournalEntry(trans, obj.Document_Date, obj.Document_NO, obj.Location_Code, "")
                            End If
