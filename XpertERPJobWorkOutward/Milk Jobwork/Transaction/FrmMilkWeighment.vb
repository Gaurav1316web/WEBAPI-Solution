...
Public Class FrmMilkWeighment
    ...
    Dim obj As clsMilkWeighment = Nothing
    ...
    Function allowToSave() As Boolean
        Try
            ...
            chk = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*) from tspl_Milk_weighment_detail where gate_entry_no='" & fndGateEntryNoBulk.Value & "' and weighment_no <>'" & fndDocNO.Value & "' "))
            If chk > 0 Then
            ...
            End If
            Dim QCDatetime As DateTime = clsCommon.myCDate(clsDBFuncationality.getSingleValue("select QC_In_Date_Time from tspl_Milk_quality_check where gate_entry_no='" & fndGateEntryNoBulk.Value & "'"), "dd/MM/yyyy hh:mm tt")
    ...
    Sub SaveData(ByVal isPostbtnClick As Boolean)
        ...
        Try
            ...
            End If
            If clsMilkWeighment.saveData(obj, trans) Then
                trans.Commit()
    ...
    Sub postData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                End If
                SaveData(True)
                If (clsMilkWeighment.postData(fndDocNO.Value, strDocType, Me.Form_ID, Nothing)) Then
    ...
    'Sub sendForQC()
    '    If clsCommon.myLen(fndDocNO.Value) > 0 Then
    '        Dim strqry As String = "Update tspl_Milk_weighment_detail set status=1, sent_to_qc_by='" & objCommonVar.CurrentUserCode & "',sent_to_qc_date='" & clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(), "dd/MMM/yyyy") & "' where Weighment_No='" & fndDocNO.Value & "'"
    ...
    Sub loadData(ByVal strWeighmentNo As String, ByVal docType As String, ByVal navType As NavigatorType)
        Try
            ...
            If clsCommon.myLen(docType) > 0 Then
                obj = clsMilkWeighment.getData(strWeighmentNo, docType, navType, False)
            Else
                obj = clsMilkWeighment.getData(strWeighmentNo, navType, False)
            End If
            ...
            If obj IsNot Nothing Then
                If clsCommon.CompairString(obj.Doc_Type, "Tanker") = CompairStringResult.Equal Then
                    ...
                    If clsCommon.myCdbl(obj.Dip_Value) = 0 Then
                        txtDipValue.Text = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select dip_value from tspl_Milk_quality_check where  isposted=1 and gate_entry_no='" & obj.Gate_Entry_No & "'"))
                    End If
                ...
                ElseIf clsCommon.CompairString(obj.Doc_Type, "Sku_Receipt") = CompairStringResult.Equal Then
                    ...
                    End If
                    If clsCommon.myCdbl(obj.Dip_Value) = 0 Then
                        obj.Dip_Value = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select dip_value from tspl_Milk_quality_check where isposted=1 and gate_entry_no='" & obj.Gate_Entry_No & "'"))
                    End If
                    If obj.status = 0 Then
                        lblStatusBulk.Text = "Not Done"
                    ElseIf obj.status = 1 Then
                ...
                Else
                    ...
                    If (clsMilkUnloading.isUnloadingDone(obj.Gate_Entry_No, Nothing) And chkBulkMilkProc.IsChecked) Then
    ...
    Private Sub fndGateEntryNoBulk__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndGateEntryNoBulk._MYValidating
        ...
        'whrCls = whrCls & " isPosted=1  " & IIf(chkBulkMilkProc.IsChecked, " and doc_type='BulkProc'", " and doc_type='MccProc'") & "  and gate_entry_no not in (select gate_entry_no from  tspl_Milk_weighment_detail where gate_entry_no<>'" & fndGateEntryNoBulk.Value & "') "
        ...
        '    End If
        'End If
        'fndGateEntryNoBulk.Value = clsMilkGateEntry.getFinder(whrCls, fndGateEntryNoBulk.Value, isButtonClicked)
        'reset()
        'If clsCommon.myLen(fndGateEntryNoBulk.Value) > 0 Then
        '    If chkBulkMilkProc.IsChecked Then
    ...
    Sub loadGateEntryDataBulk(ByVal strGateEntryNo As String)
        ...
        strWeighmentNo = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select weighment_no from tspl_Milk_weighment_detail where gate_entry_no='" & strGateEntryNo & "'"))
        ...
        Dim obj As clsMilkGateEntry = clsMilkGateEntry.getData(strGateEntryNo, "Tanker", NavigatorType.Current)
        If obj IsNot Nothing Then
            fndGateEntryNoBulk.Value = obj.Gate_Entry_No
            ...
            gvItemBulk.Rows(0).Cells(colFatKG).Value = obj.Qty_In_Kg * obj.fat_per / 100
            gvItemBulk.Rows(0).Cells(colSNFKG).Value = obj.Qty_In_Kg * obj.snf_Per / 100
            If clsDBFuncationality.getSingleValue("select count(*) from tspl_Milk_quality_check where isPosted=1 and gate_entry_no='" & obj.Gate_Entry_No & "'") > 0 Then
                lblStatusBulk.Text = "Done"
    ...
    Sub loadGateEntryDataMcc(ByVal strGateEntryNo As String)
        ...
        Dim obj As clsMilkGateEntry = clsMilkGateEntry.getData(strGateEntryNo, "Sku_Receipt", NavigatorType.Current, False)
        If obj IsNot Nothing Then
            fndGateEntryNoBulk.Value = obj.Gate_Entry_No
            lblGateEntryDateAndTimeValueBulk.Text = clsCommon.GetPrintDate(obj.Date_And_Time, "dd/MM/yyyy hh:mm:ss tt")
            lblLocationCodeBulk.Text = obj.location_Code
    ...
    Private Sub fndDocNO__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndDocNO._MYValidating
        ...
        End If
        fndDocNO.Value = clsMilkWeighment.getFinder(whrcls, fndDocNO.Value, isButtonClicked)
        If clsCommon.myLen(fndDocNO.Value) > 0 Then
            loadData(fndDocNO.Value, strDocType, NavigatorType.Current)
        Else
            reset()
    ...
    Private Sub btnReverse_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReverse.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsMilkWeighment.ReverseAndUnpost(fndDocNO.Value) Then
    ...
    Private Sub fndTankerNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndTankerNo._MYValidating
        ...
        Try
            ...
            Dim whrCls As String = String.Empty
            whrCls = whrCls & " isPosted=1  " & IIf(chkBulkMilkProc.IsChecked, " and doc_type='Tanker'", IIf(chkMccProc.IsChecked, " and doc_type='Sku_Receipt'", "")) & "  and gate_entry_no not in (select gate_entry_no from  tspl_Milk_weighment_detail where gate_entry_no<>'" & fndGateEntryNoBulk.Value & "' " & IIf(chkPendingTare.Checked, " and  isPosted=1 and Tare_Weight>0 ", "") & " union all select gate_entry_no from tspl_gate_out ) "
            If Not clsMccMaster.isCurrentUserHO() Then
                If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                    whrCls = whrCls & " and location_code in (" & objCommonVar.strCurrUserLocations & ") "
                Else
                ...
                End If
            End If
            fndGateEntryNoBulk.Value = clsMilkGateEntry.getTankerFinder(whrCls, fndTankerNo.Value)
            'reset()
            If clsCommon.myLen(fndGateEntryNoBulk.Value) > 0 Then
                If chkBulkMilkProc.IsChecked Then
                    loadGateEntryDataBulk(fndGateEntryNoBulk.Value)
    ...
    Public Sub funPrint(ByVal strDocNo As String)
        ...
        '    Qry += " Case when len(ISNULL(TSPL_LOCATION_MASTER.Phone1,''))>0 and TSPL_LOCATION_MASTER.Phone1='(+__)__________' then '' else ' '+TSPL_LOCATION_MASTER.Phone1 end  +  Case When   ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'(+__)__________' Then '  '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as Loc_Phone,tspl_Milk_weighment_detail.Weighment_No as Doc_no,TSPL_LOCATION_MASTER.Location_Desc ,convert(varchar,tspl_Milk_weighment_detail.Weighment_date,103) as Weighment_date ,tspl_Milk_weighment_detail.Gate_Entry_No ,tspl_Milk_weighment_detail.Doc_Type ,tspl_Milk_weighment_detail.Date_And_Time ,tspl_Milk_weighment_detail.Challan_No ,convert(varchar,tspl_Milk_weighment_detail.Challan_Date,103) as Challan_Date ,tspl_Milk_weighment_detail.Tanker_No ,tspl_Milk_weighment_detail.isPosted ,convert(varchar,tspl_Milk_weighment_detail.Posting_Date,103) as Posting_Date,tspl_Milk_weighment_detail.Dispatched_From_Mcc,TSPL_LOCATION_MASTER_for_MCC.Location_Desc as Mcc_Name ,tspl_Milk_weighment_detail.location_Code ,tspl_Milk_weighment_detail.Vendor_Code ,TSPL_VENDOR_MASTER.Vendor_Name ,tspl_Milk_weighment_detail.Item_Code ,TSPL_ITEM_MASTER .Item_Desc ,tspl_Milk_weighment_detail.Qty_In_Kg ,tspl_Milk_weighment_detail.Created_By ,tspl_Milk_weighment_detail.Created_Date ,tspl_Milk_weighment_detail.Modify_By ,tspl_Milk_weighment_detail.Modify_Date ,tspl_Milk_weighment_detail.comp_code ,tspl_Milk_weighment_detail.Gross_Weight ,tspl_Milk_weighment_detail.Dip_Value ,tspl_Milk_weighment_detail.Tare_Weight ,tspl_Milk_weighment_detail.Net_Weight ,case when tspl_Milk_weighment_detail.status=1 then 'Not Done' else 'Done' end as status ,tspl_Milk_weighment_detail.Weighment_Slip_No ,tspl_Milk_weighment_detail.UOM ,tspl_Milk_weighment_detail.Sent_to_QC_By ,convert(varchar,tspl_Milk_weighment_detail.Sent_To_QC_Date,103) as Sent_To_QC_Date ,tspl_Milk_weighment_detail.QC_Done_By ,tspl_Milk_weighment_detail.QC_Done_Date,TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Add1 as Comp_Add1,TSPL_COMPANY_MASTER.Add2  as Comp_Add2,TSPL_COMPANY_MASTER.Add3 as comp_add3,TSPL_COMPANY_MASTER.Tin_No as Comp_tinNo,TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2 ,TSPL_COMPANY_MASTER.Pincode as Comp_PinCode,TSPL_COMPANY_MASTER.Email as Comp_Email,Case when len(ISNULL(TSPL_COMPANY_MASTER.Phone1,''))>0 and TSPL_COMPANY_MASTER.Phone1='(+__)__________' then '' else ' '+TSPL_COMPANY_MASTER.Phone1 end "
        ...
        '    Qry += "  from tspl_Milk_weighment_detail"
        '    Qry += " left outer join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code =tspl_Milk_weighment_detail.comp_code "
        '    Qry += " left outer join TSPL_LOCATION_MASTER as TSPL_LOCATION_MASTER_for_MCC on TSPL_LOCATION_MASTER_for_MCC.Location_Code =tspl_Milk_weighment_detail.Dispatched_From_Mcc "
        '    Qry += " left outer join TSPL_VENDOR_MASTER on TSPL_VENDOR_MASTER.Vendor_Code =tspl_Milk_weighment_detail.Vendor_Code "
        '    Qry += " left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER .Item_Code =tspl_Milk_weighment_detail.Item_Code"
        '    Qry += " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code =tspl_Milk_weighment_detail.location_Code  where 2=2"

        '    Qry += " and tspl_Milk_weighment_detail.Weighment_No ='" + strDocNo + "'"
