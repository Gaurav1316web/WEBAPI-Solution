...
Public Class FrmMilkUnloading_JWO
    ...
    Dim obj As clsMilkUnloading_JOW = Nothing
    ...
    Sub SaveData(ByVal isPost As Boolean)
        ...
        Try
            ...
            If obj.isNewEntry Then
                obj.Unloading_No = clsERPFuncationality.GetNextCode(trans, clsCommon.GetPrintDate(dtpUnloadingDateTime.Value, "dd/MMM/yyyy hh:mm:ss tt"), clsDocType.UnloadingJWO, "", txtLocation.Text)
            ...
            obj.JobWorkLocation = clsCommon.myCstr(txtjobworklocation.Text)
            ...
            If clsMilkUnloading_JOW.saveData(obj, trans) Then
                trans.Commit()
                If Not isPost Then
                    If clsCommon.CompairString(btnSave.Text, "Save") = CompairStringResult.Equal Then
    ...
    Sub DeleteData()
        Dim trans As SqlTransaction = Nothing
        obj = clsMilkUnloading_JOW.getData(fndUnloadingNo.Value, NavigatorType.Current)
        ...
        Try
            If clsCommon.myLen(fndUnloadingNo.Value) > 0 Then
                ...
                If deleteConfirm() Then
                    ...
                    If clsERPFuncationality.AddToHistory(arr, clsUserMgtCode.FrmMilkUnloading, trans) Then
                        If clsMilkUnloading_JOW.deleteData(fndUnloadingNo.Value, trans) Then
                            trans.Commit()
                            reset(True)
    ...
    Sub PostData()
        ...
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                End If
                SaveData(True)
                If (clsMilkUnloading_JOW.postData(fndUnloadingNo.Value, Me.Form_ID, Nothing)) Then
    ...
    Private Sub fndGateEntryNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndGateEntryNo._MYValidating
        'fndGateEntryNo.Value = clsMilkUnloading_JOW.getGateEntryFinder("xx.[Is Posted]='Yes' and xx.GateEntryNo  not in (select TSPL_JWO_UNLOADING.Gate_Entry_No from TSPL_JWO_UNLOADING where TSPL_JWO_UNLOADING.gate_entry_no<>'" & fndGateEntryNo.Value & "' )  and ISNULL(xx.[Weighment No] ,'')<>''  and ISNULL(xx.[QC No] ,'')<>'' and xx.Accepted>0 ", fndGateEntryNo.Value, isButtonClicked)
    ...
    Function allowToSave() As Boolean
        Try
            ...
            'Dim grsWeight As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select gross_weight from TSPL_JWO_Weighment where gate_entry_no='" & fndGateEntryNo.Value & "'"))
            ...
            Dim WDate As Date = clsCommon.GetPrintDate(clsCommon.myCDate(clsDBFuncationality.getSingleValue("select weighment_date from TSPL_JWO_Weighment where gate_entry_no='" & fndGateEntryNo.Value & "'")), "dd/MMM/yyyy hh:mm:ss tt")
            Dim QDate As Date = clsCommon.GetPrintDate(clsCommon.myCDate(clsDBFuncationality.getSingleValue("select Qc_out_date_time from TSPL_JWO_QUALITY_CHECK where gate_entry_no='" & fndGateEntryNo.Value & "'")), "dd/MMM/yyyy hh:mm:ss tt")
            Dim GDate As Date = clsCommon.GetPrintDate(clsCommon.myCDate(clsDBFuncationality.getSingleValue("select date_and_time from TSPL_JWO_GATE_ENTRY where gate_entry_no='" & fndGateEntryNo.Value & "'")), "dd/MMM/yyyy hh:mm:ss tt")
            ...
            chk = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("   select count(*) from TSPL_JWO_UNLOADING where gate_entry_no='" & fndGateEntryNo.Value & "' and unloading_no <>'" & fndUnloadingNo.Value & "' "))
            If chk > 0 Then
    ...
    Sub LoadGateEntryData(ByVal strGateEntryNo As String)
        Dim strUnNo As String = clsDBFuncationality.getSingleValue("select unloading_No from TSPL_JWO_UNLOADING where gate_entry_no='" & strGateEntryNo & "'")
        If clsCommon.myLen(strUnNo) > 0 Then
            loadData(strUnNo, NavigatorType.Current)
            Exit Sub
        End If
        Dim objGt As New clsMilkGateEntry_JOW()
        reset(False)
        objGt = clsMilkGateEntry_JOW.getData(strGateEntryNo, NavigatorType.Current)
        If objGt IsNot Nothing Then
            fndGateEntryNo.Value = objGt.Gate_Entry_No
            FndTankerNo.Value = objGt.Tanker_No
            Dim strWeighmentNo As String = clsDBFuncationality.getSingleValue("select Weighment_No from TSPL_JWO_Weighment where gate_entry_no='" & objGt.Gate_Entry_No & "' ")
            If clsCommon.myLen(strWeighmentNo) > 0 Then
                txtWeighmentNo.Text = clsCommon.myCstr(strWeighmentNo)
            Else
                txtWeighmentNo.Text = ""
            End If
            Dim strQcNo As String = clsDBFuncationality.getSingleValue("select Qc_No from TSPL_JWO_QUALITY_CHECK where gate_entry_no='" & objGt.Gate_Entry_No & "' ")
            ...
            fndSubLocation.Value = ""
            lblSubLocationName.Text = ""
            txtjobworklocation.Text = objGt.JobWorkLocation
    ...
    Sub loadData(ByVal str As String, ByVal navtype As NavigatorType)
        obj = clsMilkUnloading_JOW.getData(str, navtype)
        If obj IsNot Nothing Then
            reset(False)
            fndUnloadingNo.Value = obj.Unloading_No
    ...
    Private Sub fndUnloadingNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndUnloadingNo._MYValidating
        ...
        If Not clsMccMaster.isCurrentUserHO() Then
            ...
            End If
        End If
        fndUnloadingNo.Value = clsMilkUnloading_JOW.getFinder(WhrCls, fndUnloadingNo.Value, isButtonClicked)
        If clsCommon.myLen(fndUnloadingNo.Value) > 0 Then
            loadData(fndUnloadingNo.Value, NavigatorType.Current)
        End If
    End Sub
    ...
    Private Sub FndTankerNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles FndTankerNo._MYValidating
        fndGateEntryNo.Value = clsMilkUnloading_JOW.getGateEntryFinder("xx.[Is Posted]='Yes' and xx.GateEntryNo  not in (select TSPL_JWO_UNLOADING.Gate_Entry_No from TSPL_JWO_UNLOADING where TSPL_JWO_UNLOADING.gate_entry_no<>'" & fndGateEntryNo.Value & "' )  and ISNULL(xx.[Weighment No] ,'')<>''  and ISNULL(xx.[QC No] ,'')<>'' and xx.Accepted>0 ", FndTankerNo.Value, isButtonClicked)
        If clsCommon.myLen(fndGateEntryNo.Value) > 0 Then
            LoadGateEntryData(fndGateEntryNo.Value)
        Else
            reset(False)
    ...
    Sub loadGateInQCWeighmentDateTime(Optional trans As SqlTransaction = Nothing)
        Try
            Dim qry As String = ""
            If clsCommon.myLen(fndGateEntryNo.Value) > 0 Then
                qry = " select Date_And_Time from TSPL_JWO_GATE_ENTRY where gate_entry_no='" & fndGateEntryNo.Value & "'"
            ...
            If clsCommon.myLen(txtWeighmentNo.Text) > 0 Then
                qry = " select Weighment_Date from TSPL_JWO_Weighment where weighment_no='" & txtWeighmentNo.Text & "'"
            ...
            If clsCommon.myLen(txtQCNo.Text) > 0 Then
                qry = "select Qc_In_Date_Time from TSPL_JWO_QUALITY_CHECK where QC_No='" & txtQCNo.Text & "'"
