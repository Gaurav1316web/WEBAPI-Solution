...
Public Class frmJobWorkConsumption
...
#End Region
    ...
    Function GetMilkRate(ByVal Price_Type As String, ByVal Price_Code As String) As Decimal
        ...
        If clsCommon.CompairString(Price_Type, "MCC") = CompairStringResult.Equal Then
            ...
            MilkRate = ClsAdjustments.GetMilkRate(DtMCCPrice.Rows(0).Item("Ratio"), DtMCCPrice.Rows(0).Item("Snf_Ratio"), DtMCCPrice.Rows(0).Item("Fat_Pers"), DtMCCPrice.Rows(0).Item("SNF_Pers"), DtMCCPrice.Rows(0).Item("Milk_Rate"), gv1.CurrentRow.Cells(colFATKG).Value, gv1.CurrentRow.Cells(colSNFKG).Value, gv1.CurrentRow.Cells(colQty).Value)
        ...
        ElseIf clsCommon.CompairString(Price_Type, "Bulk") = CompairStringResult.Equal Then
            ...
            MilkRate = ClsAdjustments.GetMilkRate(objBulkPrice.Fat_Weightage, objBulkPrice.Snf_Weightage, objBulkPrice.Fat_Percentage, objBulkPrice.Snf_Percentage, objBulkPrice.Standard_Rate, gv1.CurrentRow.Cells(colFATKG).Value, gv1.CurrentRow.Cells(colSNFKG).Value, gv1.CurrentRow.Cells(colQty).Value)
        Else
            MilkRate = 0
        End If
        Return MilkRate
    ...
    Function GetMilkRateImport(ByVal Price_Type As String, ByVal Price_Code As String, ByVal Fatkg As Decimal, ByVal SnfKg As Decimal, ByVal Qty As Decimal) As Decimal
        ...
        If clsCommon.CompairString(Price_Type, "MCC") = CompairStringResult.Equal Then
            DtMCCPrice = clsDBFuncationality.GetDataTable("select * from TSPL_MILK_PRICE_MASTER where Price_Code='" & Price_Code & "'")
            MilkRate = ClsAdjustments.GetMilkRate(DtMCCPrice.Rows(0).Item("Ratio"), DtMCCPrice.Rows(0).Item("Snf_Ratio"), DtMCCPrice.Rows(0).Item("Fat_Pers"), DtMCCPrice.Rows(0).Item("SNF_Pers"), DtMCCPrice.Rows(0).Item("Milk_Rate"), Fatkg, SnfKg, Qty)

        ElseIf clsCommon.CompairString(Price_Type, "Bulk") = CompairStringResult.Equal Then
            objBulkPrice = clsPriceChartBulkProc.GetData(Price_Code, NavigatorType.Current, Nothing)
            MilkRate = ClsAdjustments.GetMilkRate(objBulkPrice.Fat_Weightage, objBulkPrice.Snf_Weightage, objBulkPrice.Fat_Percentage, objBulkPrice.Snf_Percentage, objBulkPrice.Standard_Rate, Fatkg, SnfKg, Qty)
        Else
            MilkRate = 0
        End If
        Return MilkRate
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            obj = obj.GetData(strCode, AdjustmentEnum.strCostTransaction, NavTyep, Nothing, True)
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtAdjustmentNo._MYNavigator
        Try
            Dim qst As String = "select count(*) from TSPL_ADJUSTMENT_HEADER where Reference_Document in ( 'JWO-SRN-JLO','JWO-SRN-JLI','JWO-SRN-RET-JLO','JWO-SRN-RET-JLI') and Adjustment_No='" + txtAdjustmentNo.Value + "'"
    ...
    Private Sub funPrint()
        ...
        Try
            ...
            If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                ...
                frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, "crptAdjustment", "Adjustment Detail")
            Else
                If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Adjustment_Type")), "BD") = CompairStringResult.Equal Then
                    ...
                    dt = clsDBFuncationality.GetDataTable(qry)
                    frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.PaperSize10x6, "crptAdjustmentCustomIssue", "Adjustment Detail")
                ...
                Else
                    ...
                    qry += " where Reference_Document in ( 'JWO-SRN-JLO','JWO-SRN-JLI','JWO-SRN-RET-JLO','JWO-SRN-RET-JLI') and TSPL_ADJUSTMENT_HEADER.Adjustment_No='" + txtAdjustmentNo.Value + "'  " & _
                    ")xxx group by Adjustment_No,Item_Code order by Item_Desc"
                    dt = clsDBFuncationality.GetDataTable(qry)
                    frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.PaperSize10x6, "crptAdjustmentCustomReceipt", "Adjustment Detail")
                End If
            End If
        Catch ex As Exception
                ...
                If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(0)("Adjustment_Type")), "BD") = CompairStringResult.Equal And IsPreprinted = True Then
                    ...
                    Else
                        ...
                        "WHERE (TSPL_ADJUSTMENT_HEADER.Adjustment_No = '" + strAdjustmentNo + "')  " & _
                         ") xxx group by Adjustment_No,Item_Code order by Item_Desc"
                        dt = clsDBFuncationality.GetDataTable(qry)
                        frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.NA, "rptEmptyOutward", "Empty Issue Challan")
                    End If
...
"from ( " & _
                    ...
                    If IsEmpty Then
                        If IsPreprinted Then
                            frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.PaperSize10x12, "crptAdjustmentCustomReceiptGun", "Adjustment Detail")
                        Else

                            If (clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "guntur") = CompairStringResult.Equal) Then
                                frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.NA, "crptAdjustmentCustomReceiptGuntur", "Adjustment Detail")
                            Else
                                frmCrystalReportViewer.funreport(MyBase.Form_ID, CrystalReportFolder.InventoryReport, dt, EnumTecxpertPaperSize.NA, "crptAdjustmentCustomReceiptVizag", "Adjustment Detail")
    ...
    Private Sub ExporttoExcelWithSerial_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ExporttoExcelWithSerial.Click
        Try
            ...
            qryExport = " Select TSPL_ADJUSTMENT_detail.Location_Code as [Location], TSPL_ADJUSTMENT_HEADER.Adjustment_Date , TSPL_ADJUSTMENT_HEADER.Adjustment_No, TSPL_ADJUSTMENT_detail.Item_Code as [Item Code],TSPL_ADJUSTMENT_detail.unit_code as [unit code], Item_Quantity as [Quantity], Item_Cost as [Cost Adjustment],'N' as [Third Party Location], TSPL_SERIAL_ITEM.Auto_Sr_No as [Serial No],'' as [Tag No],'' as Type, case when TSPL_ADJUSTMENT_HEADER.adjustment_type='ADJ' then 'Adjustment'" & _
                        " when TSPL_ADJUSTMENT_HEADER.adjustment_type='FLG' then 'Flushing'" & _
                        " when TSPL_ADJUSTMENT_HEADER.adjustment_type='OPG' then 'Opening'" & _
                         " when TSPL_ADJUSTMENT_HEADER.adjustment_type='CLG' then 'Closing'" & _
                         " when TSPL_ADJUSTMENT_HEADER.adjustment_type='AAD' then 'Auto Adjustment'" & _
                         "  when TSPL_ADJUSTMENT_HEADER.adjustment_type='OTH' then 'Other'" & _
                         " else '' end   as [Transaction Type]   from TSPL_ADJUSTMENT_detail  left join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_HEADER.Adjustment_No =TSPL_ADJUSTMENT_detail.Adjustment_No  LEFT JOIN TSPL_SERIAL_ITEM  on  TSPL_SERIAL_ITEM.Document_Code=TSPL_ADJUSTMENT_detail.Adjustment_No and TSPL_SERIAL_ITEM.Parent_Line_No =  TSPL_ADJUSTMENT_DETAIL.Adjustment_Line_No and   TSPL_SERIAL_ITEM.Item_Code = TSPL_ADJUSTMENT_DETAIL.Item_Code  left join TSPL_VISI_MASTER on  TSPL_VISI_MASTER.Serial_No=TSPL_SERIAL_ITEM.Auto_Sr_No LEFT JOIN   TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code = TSPL_ADJUSTMENT_detail.Location_Code "
            transportSql.ExporttoExcel(qryExport, Me)
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Exclamation, "Store Adjustment")
        End Try
    ...
    Private Sub rbtnImportPosted_Click(sender As Object, e As EventArgs) Handles rbtnImportPosted.Click
        ...
        End If
    End Sub
    Private Function CaptureScreenData() As ClsAdjustments
        Dim obj As New ClsAdjustments()
        Try
            If FrmMainTranScreen.ValidateTransactionAccToFinYear("Store Adjustment", txtDate.Value) = False Then
                Return obj
    ...
    End Sub
    Function EditAndPost(ByVal objNew As ClsAdjustments) As Boolean
        Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin
        Try
            'Dim obj As New ClsAdjustments()
            ...
            isSaved = isSaved AndAlso ClsAdjustments.ReverseAndUnpost(objNew.Adjustment_No, trans)
            isSaved = isSaved AndAlso objNew.SaveData(objNew, False, "", trans)
            isSaved = isSaved AndAlso ClsAdjustments.PostData(objNew.Adjustment_No, "Store Adjustment", trans, True, VoucherNo)
            trans.Commit()
        ...
        If transportSql.importExcel(gv, "Location", "Adjustment Date", "Item Code", "unit code", "Quantity", "Item Cost", "Amount", "Third Party Location", "Adjustment Type(BD/BI/CD/CI/QD/QI)", "Description", "Batch", "Batch Mfg Date", "Batch Exp Date", "Batch Qty", "Transaction Type") Then
            Try
                ...
                If arrExistBatchItem.Count > 0 Then
                    ...
                    Next
                End If
                Dim isSaved As Boolean = obj.SaveData(obj, True, "", Nothing)
                RadMessageBox.Show("Data Transfer Completed!", Me.Text, MessageBoxButtons.OK)
            Catch ex As Exception
                myMessages.myExceptions(ex)
            End Try
...
#Region "Correction -ve Old stock"
        ...
        If transportSql.importExcel(gv, "AdjDate", "Main Location Code", "Location Code", "Inout", "Adjustment Type", "Item Code", "UOM", "Qty", "Rate", "Amount", "Fat%", "Fat KG", "SNF%", "SNF KG", "Transaction Type") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    ClsAdjustments.PostData(obj.Adjustment_No, AdjustmentEnum.strCostTransaction, trans)
                Next
                trans.Commit()
                clsCommon.ProgressBarHide()
                RadMessageBox.Show("Data Transfer Completed!", Me.Text, MessageBoxButtons.OK)
    ...
    Private Sub OutCurrentStock(ByVal Loc_Code As String, ByVal arrItem As ArrayList, ByVal Adjustment_Date As DateTime, ByVal trans As SqlTransaction)
        ...
        Try
            ...
            qryCTE = " With Stock_Closing as " & _
                     " (select tspl_inv_move_dl.Item_Code,tspl_inv_move_dl.Product_Type,tspl_inv_move_dl.Item_Type,tspl_inv_move_dl.location_code,tspl_inv_move_dl.Stock_UOM,cast(tspl_inv_move_dl.CL_QTY as decimal(28,3)) as op_qty,case when isnull(tspl_inv_move_dl.cl_qty,0)=0 then 0 else cast(tspl_inv_move_dl.cl_avg_cost/tspl_inv_move_dl.cl_qty as decimal(28,3)) end as Op_Rate,cast(tspl_inv_move_dl.CL_Avg_Cost as decimal(28,3)) as op_cost " & _
                     " ,case when isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,0)>0 and isnull(tspl_inv_move_dl.CL_QTY,0)>0 then cast((round(tspl_inv_move_dl.CL_FAT_KG,2) * 100) / (tspl_inv_move_dl.CL_QTY/TSPL_ITEM_UOM_DETAIL.Conversion_Factor) as decimal(28,3)) end as op_fat_per,cast(round(tspl_inv_move_dl.CL_FAT_KG,2) as decimal(28,3)) as op_fat_kg, " & _
                     " case when isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,0)>0 and isnull(tspl_inv_move_dl.CL_QTY,0)>0 then cast((tspl_inv_move_dl.CL_SNF_KG * 100) / (tspl_inv_move_dl.CL_QTY/TSPL_ITEM_UOM_DETAIL.Conversion_Factor) as decimal(28,3)) end as op_snf_per,cast(tspl_inv_move_dl.CL_SNF_KG as decimal(28,3)) as op_snf_kg " & _
                     " from (select TSPL_INV_MOVE_DL.Item_Code,TSPL_ITEM_MASTER.Product_Type,TSPL_ITEM_MASTER.Item_Type,TSPL_INV_MOVE_DL.Location_Code,TSPL_INV_MOVE_DL.Stock_UOM,TSPL_INV_MOVE_DL.TRANS_DATE," & _
                     " TSPL_INV_MOVE_DL.CL_QTY, TSPL_INV_MOVE_DL.CL_Avg_Cost,round(TSPL_INV_MOVE_DL.CL_FAT_KG,2) as CL_FAT_KG, round(TSPL_INV_MOVE_DL.CL_SNF_KG,2) as CL_SNF_KG " & _
                     " from TSPL_INV_MOVE_DL inner join ( " & _
                     " select Item_Code,Location_Code,max(TRANS_DATE) as TRANS_DATE from TSPL_INV_MOVE_DL where TRANS_DATE<='" + clsCommon.GetPrintDate(Adjustment_Date, "dd/MMM/yyyy") + "' " & _
                     " group by Item_Code,Location_Code) as ItemTrans on TSPL_INV_MOVE_DL.Item_Code=ItemTrans.Item_Code and TSPL_INV_MOVE_DL.Location_Code=ItemTrans.Location_Code " & _
                     " and TSPL_INV_MOVE_DL.TRANS_DATE=ItemTrans.TRANS_DATE " & _
                     " inner join TSPL_ITEM_MASTER on TSPL_INV_MOVE_DL.Item_Code=TSPL_ITEM_MASTER.Item_Code " & _
                     " where (TSPL_INV_MOVE_DL.CL_QTY<>0 or TSPL_INV_MOVE_DL.CL_Avg_Cost<>0 or TSPL_INV_MOVE_DL.CL_FAT_KG<>0 " & _
                     " or TSPL_INV_MOVE_DL.CL_SNF_KG<>0) and TSPL_INV_MOVE_DL.Location_Code='" & Loc_Code & "' and  TSPL_INV_MOVE_DL.Item_Code in (" & clsCommon.GetMulcallString(arrItem) & ")" & _
                     " ) as tspl_inv_move_dl " & _
            ...
            qryPart2 = "select final.*,tspl_item_master.item_desc,tspl_location_master.Main_Location_Code from ( " & Environment.NewLine & _
                  "select 'Out' as inout,'Quantity' as transtype,Item_Code,Product_Type,Item_Type,location_code,Stock_UOM,op_qty,0 as Op_Rate,0 as op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_qty>0 " & _
                  "union all " & _
                  "select 'In' as inout,'Quantity' as transtype,Item_Code,Product_Type,Item_Type,location_code,Stock_UOM,0-isnull(op_qty,0) as op_qty,0 as Op_Rate,0 as op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_qty<0 " & _
                  "union all " & _
                  "select 'Out' as inout,'Cost' as transtype,Item_Code,Product_Type,Item_Type,location_code,Stock_UOM,0 as op_qty,Op_Rate,op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_cost>0 " & _
                  "union all " & _
                  ...
                  "union all " & _
                  "select 'Out' as inout,'FAT/SNF' as transtype,Item_Code,Product_Type,Item_Type,location_code,Stock_UOM,0 as op_qty,0 as Op_Rate,0 as op_cost,op_fat_per,op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_fat_kg>0 " & _
                  "union all " & _
                  ...
                  "union all " & _
                  "select 'Out' as inout,'FAT/SNF' as transtype,Item_Code,Product_Type,Item_Type,location_code,Stock_UOM,0 as op_qty,0 as Op_Rate,0 as op_cost,0 as op_fat_per,0 as op_fat_kg,op_snf_per,op_snf_kg from Stock_Closing where op_snf_kg>0 " & _
                  "union all " & _
                  ...
                  ")final left outer join tspl_item_master on tspl_item_master.item_code=final.item_code  " & _
            ...
            For Each drMain As DataRow In dtMain.Rows
                ...
                If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                    ...
                    For Each dr As DataRow In dt.Rows
                        ...
                        ''detail data========================
                        objtr = New ClsAdjustmentsDetails()
                        objtr.Price_Type = "None"
    ...
    Private Sub GetOldStockData_OutEntry(ByVal Main_Loc_Code As String, ByVal Loc_Code As String, ByVal Item_Code As String, ByVal Adjustment_Date As DateTime, ByVal Account_Code As String, ByVal Item_Type As String, ByVal trans As SqlTransaction)
        ...
        Try
            ...
            ''"and tspl_inv_move_dl.item_code='" & Item_Code & "' and tspl_inv_move_dl.location_code='" & Loc_Code & "') " & Environment.NewLine & _
            qryCTE = "With Stock_Closing as " & _
                  "(select tspl_inv_move_dl.Item_Code,tspl_inv_move_dl.location_code,tspl_inv_move_dl.Stock_UOM,cast(tspl_inv_move_dl.CL_QTY as decimal(18,3)) as op_qty,case when isnull(tspl_inv_move_dl.cl_qty,0)=0 then 0 else cast(tspl_inv_move_dl.cl_avg_cost/tspl_inv_move_dl.cl_qty as decimal(18,3)) end as Op_Rate,cast(tspl_inv_move_dl.CL_Avg_Cost as decimal(18,3)) as op_cost " & _
                  ",case when isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,0)>0 and isnull(tspl_inv_move_dl.CL_QTY,0)>0 then cast((tspl_inv_move_dl.CL_FAT_KG * 100) / (tspl_inv_move_dl.CL_QTY/TSPL_ITEM_UOM_DETAIL.Conversion_Factor) as decimal(18,3)) end as op_fat_per,cast(tspl_inv_move_dl.CL_FAT_KG as decimal(18,3)) as op_fat_kg, " & _
                  "case when isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,0)>0 and isnull(tspl_inv_move_dl.CL_QTY,0)>0 then cast((tspl_inv_move_dl.CL_SNF_KG * 100) / (tspl_inv_move_dl.CL_QTY/TSPL_ITEM_UOM_DETAIL.Conversion_Factor) as decimal(18,3)) end as op_snf_per,cast(tspl_inv_move_dl.CL_SNF_KG as decimal(18,3)) as op_snf_kg " & _
            ...
            qryPart2 = "select final.*,tspl_item_master.item_desc from ( " & Environment.NewLine & _
                  "select 'Out' as inout,'Quantity' as transtype,Item_Code,location_code,Stock_UOM,op_qty,0 as Op_Rate,0 as op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_qty>0 " & _
                  "union all " & _
                  "select 'In' as inout,'Quantity' as transtype,Item_Code,location_code,Stock_UOM,0-isnull(op_qty,0) as op_qty,0 as Op_Rate,0 as op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_qty<0 " & _
                  "union all " & _
                  "select 'Out' as inout,'Cost' as transtype,Item_Code,location_code,Stock_UOM,0 as op_qty,Op_Rate,op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_cost>0 " & _
                  "union all " & _
                  "select 'In' as inout,'Cost' as transtype,Item_Code,location_code,Stock_UOM,0 as op_qty,case when op_rate<0 then 0-isnull(Op_Rate,0) else Op_Rate end as Op_Rate,0-isnull(op_cost,0) as op_cost,0 as op_fat_per,0 as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_cost<0 " & _
                  "union all " & _
                  "select 'Out' as inout,'FAT/SNF' as transtype,Item_Code,location_code,Stock_UOM,0 as op_qty,0 as Op_Rate,0 as op_cost,op_fat_per,op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_fat_kg>0 " & _
                  "union all " & _
                  "select 'In' as inout,'FAT/SNF' as transtype,Item_Code,location_code,Stock_UOM,0 as op_qty,0 as Op_Rate,0 as op_cost,case when isnull(op_fat_per,0)<0 then 0-isnull(op_fat_per,0) else op_fat_per end as op_fat_per,0-isnull(op_fat_kg,0) as op_fat_kg,0 as op_snf_per,0 as op_snf_kg from Stock_Closing where op_fat_kg<0 " & _
                  "union all " & _
                  "select 'Out' as inout,'FAT/SNF' as transtype,Item_Code,location_code,Stock_UOM,0 as op_qty,0 as Op_Rate,0 as op_cost,0 as op_fat_per,0 as op_fat_kg,op_snf_per,op_snf_kg from Stock_Closing where op_snf_kg>0 " & _
                  "union all " & _
        ...
        If transportSql.importExcel(gv, "AdjDate", "Main Location Code", "Location Code", "Inout", "Adjustment Type", "Item Code", "UOM", "Qty", "Rate", "Amount", "Fat%", "Fat KG", "SNF%", "SNF KG", "Transaction Type") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    ClsAdjustments.PostData(obj.Adjustment_No, AdjustmentEnum.strCostTransaction)
                Next
...
#End Region
    ...
    Private Sub RadMenuItem6_Click(sender As Object, e As EventArgs) Handles RadMenuItem6.Click
        Try
            Dim qryExport As String
            qryExport = " select distinct '31-Mar-2017' as AdjDate,TSPL_INV_MOVE_DL.Location_Code as [Main Location Code],TSPL_INV_MOVE_DL.Location_Code as [Location Code],'In' as Inout,'Both'as [Adjustment Type], TSPL_INV_MOVE_DL.Item_Code as [Item Code],Stock_UOM as UOM,0 as Qty,0 as Rate,0 as Amount,0 as [Fat%],0 as [Fat KG],0 as [SNF%],0 as [SNF KG],'' as [Transaction Type] " & _
                        " from (select TSPL_INV_MOVE_DL.* from TSPL_INV_MOVE_DL inner join (" & _
                        " select Item_Code,Location_Code,max(TRANS_DATE) as TRANS_DATE from TSPL_INV_MOVE_DL where TRANS_DATE<='31-Mar-2017' " & _
                        " group by Item_Code,Location_Code) as Cond on TSPL_INV_MOVE_DL.Item_Code=Cond.Item_Code and TSPL_INV_MOVE_DL.Location_Code=Cond.Location_Code " & _
                        " and TSPL_INV_MOVE_DL.TRANS_DATE=Cond.TRANS_DATE " & _
                        " ) TSPL_INV_MOVE_DL left join ( select distinct TSPL_ADJUSTMENT_DETAIL.Item_Code,TSPL_ADJUSTMENT_DETAIL.Location_Code from TSPL_ADJUSTMENT_DETAIL " & _
                        " inner join TSPL_ADJUSTMENT_HEADER on TSPL_ADJUSTMENT_DETAIL.Adjustment_No=TSPL_ADJUSTMENT_HEADER.Adjustment_No  where TSPL_ADJUSTMENT_HEADER.Adjustment_Type='AAD') as Adj on " & _
                        " TSPL_INV_MOVE_DL.Item_Code = Adj.Item_Code And TSPL_INV_MOVE_DL.Location_Code = Adj.Location_Code " & _
