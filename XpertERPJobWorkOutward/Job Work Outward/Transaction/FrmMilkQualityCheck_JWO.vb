...
Public Class FrmMilkQualityCheck_JWO
    ...
    Sub LoadGateEntryData(ByVal strGateEntryNo As String)
        ...
        isFoundQc = clsDBFuncationality.getSingleValue("select QC_No from TSPL_JWO_QUALITY_CHECK where gate_entry_no='" & strGateEntryNo & "'")
        ...
        objGt = clsMilkGateEntry_JOW.getData(strGateEntryNo, IIf(chkBulkMilkProc.IsChecked, "Tanker", "Sku_Receipt"), NavigatorType.Current)
        If objGt IsNot Nothing Then
            ...
            Dim dt As DataTable = clsDBFuncationality.GetDataTable("select * from TSPL_JWO_Weighment where gate_entry_no='" & objGt.Gate_Entry_No & "' ")
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                txtDipValue.Text = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select dip_value from TSPL_JWO_Weighment where  gate_entry_no='" & objGt.Gate_Entry_No & "'"))
            ...
            If chkMccProc.IsChecked Then
                ...
                txtDispControlSampleFAT.Text = clsMilkQualityCheck_JOW.ControlSampleFAT(objGt.Gate_Entry_No)
                txtDispControlSampleSNF.Text = clsMilkQualityCheck_JOW.ControlSampleSNF(objGt.Gate_Entry_No)
                txtRcptControlSampleFAT.Text = ""
                txtRcptControlSampleSNF.Text = ""
                If clsMilkQualityCheck_JOW.isControlSample(objGt.Gate_Entry_No) Then
    ...
    Function allowToSave() As Boolean
        Try
            ...
            If gvPaperSeal IsNot Nothing AndAlso gvPaperSeal.Rows.Count > 0 Then
                ...
                Next
            End If
            If Not clsMilkQualityCheck_JOW.chkIsGridColumnHasTag(gvParam) Then
                Throw New Exception(" Grid's Column is not being recognized, Please delete layout and try loading Document Again ")
            End If
            Dim chk As Integer = 0
            chk = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("   select count(*) from TSPL_JWO_QUALITY_CHECK where gate_entry_no='" & fndGateEntryNo.Value & "' and QC_NO <>'" & fndQcNo.Value & "' "))
            If chk > 0 Then
    ...
    Sub SaveData(ByVal isPost As Boolean)
        ...
        Try
            ...
            If obj.isNewEntry Then
                obj.QC_No = clsERPFuncationality.GetNextCode(trans, dt, clsDocType.QCJWO, "", fndLocation.Value)
            ...
            Dim i As Integer = 0
            Dim objParam As New clsMilkQcParam_JOW
            obj.arrQcParam = New List(Of clsMilkQcParam_JOW)
            For i = 0 To gvParam.Columns.Count - 1
                ...
                Else
                    objParam = New clsMilkQcParam_JOW
                ...
                End If
            Next
            If clsMilkQualityCheck_JOW.saveData(obj, trans) Then
                If (chkMccProc.IsChecked) Then
                    If gvManualSeal IsNot Nothing AndAlso gvManualSeal.Rows.Count > 0 Then
                        Dim arrMSeal As List(Of clsMilkQCManualSealDetail_JOW) = New List(Of clsMilkQCManualSealDetail_JOW)
                        Dim objMSeal As clsMilkQCManualSealDetail_JOW = Nothing
                        For i = 0 To gvManualSeal.Rows.Count - 1
                            objMSeal = New clsMilkQCManualSealDetail_JOW()
                        ...
                        Next
                        clsMilkQCManualSealDetail_JOW.SaveData(arrMSeal, trans)
                    End If
                    If gvPaperSeal IsNot Nothing AndAlso gvPaperSeal.Rows.Count > 0 Then
                        Dim arrPSeal As List(Of clsMilkQCPaperSealDetail_JOW) = New List(Of clsMilkQCPaperSealDetail_JOW)
                        Dim objPSeal As clsMilkQCPaperSealDetail_JOW = Nothing
                        For i = 0 To gvPaperSeal.Rows.Count - 1
                            objPSeal = New clsMilkQCPaperSealDetail_JOW()
                        ...
                        Next
                        clsMilkQCPaperSealDetail_JOW.SaveData(arrPSeal, trans)
                    End If
    ...
    Sub loadData(ByVal str As String, ByVal docType As String, ByVal navtype As NavigatorType)
        ...
        If clsCommon.myLen(docType) > 0 Then
            obj = clsMilkQualityCheck_JOW.getData(str, docType, navtype)
        Else
            obj = clsMilkQualityCheck_JOW.getData(str, navtype)
        End If
        ...
        If obj IsNot Nothing Then
            ...
            Dim dt As DataTable = clsDBFuncationality.GetDataTable("select * from TSPL_JWO_Weighment where gate_entry_no='" & obj.Gate_Entry_No & "' ")
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                txtWeighmentNo.Text = clsCommon.myCstr(dt.Rows(0)("Weighment_No"))
                dtpWeighmentDate.Value = clsCommon.GetPrintDate(dt.Rows(0)("Weighment_Date"), "dd/MM/yyyy")
                lblStatusValue.Text = "Done"
            ...
            If clsCommon.CompairString(obj.Doc_Type, "Sku_Receipt") = CompairStringResult.Equal Then
                ...
                Dim arrPSeal As List(Of clsMilkQCPaperSealDetail_JOW) = clsMilkQCPaperSealDetail_JOW.getData(txtChallanNo.Text)
                Dim arrMSeal As List(Of clsMilkQCManualSealDetail_JOW) = clsMilkQCManualSealDetail_JOW.getData(txtChallanNo.Text)
            ...
            If chkMccProc.IsChecked Then
                GrpControlSample.Visible = True
                txtDispControlSampleFAT.Text = clsMilkQualityCheck_JOW.ControlSampleFAT(obj.Gate_Entry_No)
                txtDispControlSampleSNF.Text = clsMilkQualityCheck_JOW.ControlSampleSNF(obj.Gate_Entry_No)
                txtRcptControlSampleFAT.Text = obj.Receipt_Control_FAT
                txtRcptControlSampleSNF.Text = obj.Receipt_Control_SNF
                If clsMilkQualityCheck_JOW.isControlSample(obj.Gate_Entry_No) Then
                    txtRcptControlSampleFAT.Enabled = True
                    txtRcptControlSampleSNF.Enabled = True
                Else
                    txtRcptControlSampleFAT.Enabled = False
    ...
    Sub PostData()
        ...
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                End If
                SaveData(True)
                If (clsMilkQualityCheck_JOW.postData(fndQcNo.Value, strDocType, Me.Form_ID, Nothing)) Then
                    If clsCommon.CompairString(str, "2") = CompairStringResult.Equal Then
                        clsDBFuncationality.ExecuteNonQuery(" update TSPL_JWO_QUALITY_CHECK set is_Param_Accepted=2 where qc_no='" & fndQcNo.Value & "'")
                        msg = "Successfully Posted"
                    Else
                        clsDBFuncationality.ExecuteNonQuery(" update TSPL_JWO_QUALITY_CHECK set is_Param_Accepted=" & IIf(clsCommon.CompairString(str, "1") = CompairStringResult.Equal, 1, 0) & " where qc_no='" & fndQcNo.Value & "'")
                        msg = "Successfully Posted"
                    End If
                Else
    ...
    Private Sub fndQcNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles fndQcNo._MYValidating
        ...
        If chkBothDoc.Checked Then
            fndQcNo.Value = clsMilkQualityCheck_JOW.getFinder(" 1=1 " & whrCls, fndQcNo.Value, isButtonClicked)
        Else
            fndQcNo.Value = clsMilkQualityCheck_JOW.getFinder(" Doc_type='" & IIf(chkMccProc.IsChecked, "Sku_Receipt", "Tanker") & "' " & whrCls, fndQcNo.Value, isButtonClicked)
        End If
    ...
    Private Sub btnReverse_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnReverse.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                If clsMilkQualityCheck_JOW.ReverseAndUnpost(fndQcNo.Value) Then
    ...
    Private Sub mnuEmailSmsSetting_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles mnuEmailSmsSetting.Click
        ...
        frm.FormId = clsUserMgtCode.frmQC_JWO
        frm.ShowDialog()
    End Sub
    ...
    Private Sub fndTankerNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndTankerNo._MYValidating
        ...
        If Not clsMccMaster.isCurrentUserHO() Then
            If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                whrcls = " and  TSPL_JWO_GATE_ENTRY.location_code in ( " & objCommonVar.strCurrUserLocations & ") "
            End If
        End If
        whrcls = whrcls & " and TSPL_JWO_GATE_ENTRY.gate_entry_no not in( select Gate_Entry_No  from TSPL_JWO_QUALITY_CHECK) "
        Dim gtNo As String = fndTankerNo.Value
        Dim dt As DataRow
        dt = clsMilkQualityCheck_JOW.getGateEntryFinder(gtNo, "TSPL_JWO_GATE_ENTRY.isPosted='1' and TSPL_JWO_GATE_ENTRY.Doc_Type='" & IIf(chkBulkMilkProc.IsChecked, "Tanker", "Sku_Receipt") & "'  " & whrcls)
