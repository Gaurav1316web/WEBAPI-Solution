...
Public Class frmJWOFormulaMaster
...
#End Region
        ...
        Try
            If (AllowToSave()) Then
                ...
                If MyBase.isModifyonPasswordFlag Then
                    If clsPasswordCheckForMasters.CheckMasterPwd(clsUserMgtCode.JWOParameterFormula, clsCommon.myCstr(objCommonVar.CurrentCompanyCode)) Then
                    Else
                        Return
                    End If
                End If
                Dim arr As New List(Of clsJWOFormula)

                Dim obj As New clsJWOFormula()
                ...
                Dim Arr2 As New List(Of clsJWOFormulaDetails)
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsJWOFormulaDetails()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsJWOFormula()
            obj = clsJWOFormula.GetData(strCode, NavTyep, Nothing) ' clsJWOFormula.GetData(strCode, NavTyep, Nothing)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Code) > 0) Then
                ...
                '====================================
                Dim count As Boolean = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*) from TSPL_JWO_FORMULA_DETAILS where Code = '" + obj.Code + "'"))
                If count = True Then
                    For Each objtr As clsJWOFormulaDetails In obj.Arr
    ...
    Private Sub txtCode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtCode._MYValidating
        Try
            Dim qst As String = "select count(*) from TSPL_JWO_FORMULA where Code='" + txtCode.Value + "'"
            ...
            If txtCode.MyReadOnly OrElse isButtonClicked Then
                LoadData(clsJWOFormula.getFinder("", txtCode.Value, isButtonClicked), NavigatorType.Current)
    ...
    Private Sub rmExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rmExport.Click
        ...
        strDetail = "  select TSPL_JWO_FORMULA.Code , TSPL_JWO_FORMULA.Description , TSPL_JWO_FORMULA.Structure_Code as [Structure Code] , TSPL_JWO_FORMULA.Formula, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =1 )  AS Parameter1 " &
                    "  ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =1 )  AS Value1, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =2 )  AS Parameter2" &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =2 )  AS Value2, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =3 )  AS Parameter3 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =3 )  AS Value3, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =4 )  AS Parameter4 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =4 )  AS Value4 , " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =5 )  AS Parameter5 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =5 )  AS Value5 , " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =6 )  AS Parameter6 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =6 )  AS Value6, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =7 )  AS Parameter7 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =7 )  AS Value7, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =8 )  AS Parameter8 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =8 )  AS Value8, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =9 )  AS Parameter9 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =9 )  AS Value9, " &
                    "  (Select Parameter_Code From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Parameter_Code  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =10 )  AS Parameter10 " &
                    " ,(Select Value From (Select ROW_NUMBER () over (order by Code,Parameter_Code ) As SNo,Code,Value  From TSPL_JWO_FORMULA_DETAILS where TSPL_JWO_FORMULA_DETAILS.Code =TSPL_JWO_FORMULA.Code) xxx where xxx.SNo =10 )  AS Value10 " &
                    "  from TSPL_JWO_FORMULA  "
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.myLen(strFormula) > 0 Then
                        ...
                        End Try
                    End If
                    If clsCommon.myLen(strcode) > 0 AndAlso clsDBFuncationality.getSingleValue("Select count(*) from TSPL_JWO_FORMULA  where Code='" + strcode + "' ", trans) > 0 Then
                        isNewEntry = False
                    Else
                        isNewEntry = True
                    End If
