...
Public Class RptJobWorktPurchaseRegisterReport
...
#End Region
    ...
    Private Sub txtLocation__My_Click(sender As Object, e As EventArgs)
        ...
        '' Dim qry As String = " select distinct xyz.Code , xyz.Name  from (select Location_Code as Code,Location_Desc as [Name] from TSPL_LOCATION_MASTER  where TSPL_LOCATION_MASTER.Is_Jobwork=1 and TSPL_LOCATION_MASTER.Location_Type='Physical' and TSPL_LOCATION_MASTER.Is_Sub_Location='Y' " & stateCond & " union  select Main_Location_Code as Code,Location_Desc as [Name] from TSPL_LOCATION_MASTER  where Location_Code in (select Location_Code  from TSPL_LOCATION_MASTER  where TSPL_LOCATION_MASTER.Is_Jobwork=1 and TSPL_LOCATION_MASTER.Location_Type='Physical' and TSPL_LOCATION_MASTER.Is_Sub_Location='Y' " & stateCond & "  ))xyz  "
        'Dim qry As String = " select distinct xyz.Code , xyz.Name  from (select Location_Code as Code,Location_Desc as [Name] from TSPL_LOCATION_MASTER  where TSPL_LOCATION_MASTER.Is_Jobwork=1 and TSPL_LOCATION_MASTER.Location_Type='Physical' and TSPL_LOCATION_MASTER.Is_Sub_Location='Y' " & stateCond & " union   select TSPL_LOCATION_MASTER.Main_Location_Code as Code,TSPL_LOCATION_MASTER_For_Main_Loc.Location_Desc as [Name] from TSPL_LOCATION_MASTER   left outer join TSPL_LOCATION_MASTER as TSPL_LOCATION_MASTER_For_Main_Loc on TSPL_LOCATION_MASTER_For_Main_Loc.Location_Code = TSPL_LOCATION_MASTER.Main_Location_Code    where TSPL_LOCATION_MASTER.Location_Code in (select Location_Code  from TSPL_LOCATION_MASTER  where TSPL_LOCATION_MASTER.Is_Jobwork=1 and TSPL_LOCATION_MASTER.Location_Type='Physical' and TSPL_LOCATION_MASTER.Is_Sub_Location='Y' " & stateCond & " ))xyz "
    ...
    Private Sub ExportGrid(ByVal exporter As EnumExportTo)
        Try
            ...
            Dim strTemp As String = ""
            arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
            arrHeader.Add("Name : " & clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.RptJobWorOutwardPurchasekRegister & "'"))
            arrHeader.Add("From Date : " + clsCommon.GetPrintDate(fromDate.Value, "dd/MM/yyyy") + " To " + clsCommon.GetPrintDate(ToDate.Value, "dd/MM/yyyy") + " ")
            If clsCommon.myLen(txtUOM.Value) > 0 Then
                arrHeader.Add("UOM : " + txtUOM.Value)
            End If
            ...
            If Not IsNothing(txtState.arrValueMember) Then
                arrHeader.Add("State : " + clsCommon.GetMulcallStringWithComma(txtState.arrDispalyMember))
            End If
            If rbtnLocationSelect.CheckState = CheckState.Checked = False Then
                For Each grow As GridViewRowInfo In gvLocation.Rows
                    If grow.Cells("Sel").Value Then
                        arrHeader.Add("Location : " + clsCommon.myCstr(grow.Cells("Code").Value))
                    End If
            ...
            End If
            If Not IsNothing(txtCustomer.arrValueMember) Then
                arrHeader.Add("Vendor : " + clsCommon.GetMulcallStringWithComma(txtCustomer.arrValueMember))
            End If
            If btnAll.CheckState = True Or btnPosted.CheckState = True Or btnUnposted.CheckState = True Then
                arrHeader.Add("Status : " + IIf(btnAll.IsChecked, "ALL", IIf(btnPosted.IsChecked, "Posted", "Unposted")))
            End If
    ...
    Function ReturnQuery() As String
        ...
        qryList = clsPurchaseInvoiceHead.ReturnQuery(False, clsCommon.GetDateWithStartTime(fromDate.Value), clsCommon.GetDateWithEndTime(ToDate.Value), Unit_Code, Stocking_Uom, True)
        ...
        If rbtnLocationSelect.IsChecked Then
            ...
            For ii As Integer = 0 To gvLocation.RowCount - 1
                If clsCommon.myCBool(gvLocation.Rows(ii).Cells("SEL").Value) Then
                    ...
                    strWhrCatg += " ((case when TSPL_LOCATION_MASTER_SubLoc.Is_Section='N' and TSPL_LOCATION_MASTER_SubLoc.Is_Sub_Location='N' then xx.[Location Code] else TSPL_LOCATION_MASTER_SubLoc.Main_Location_Code end) = '" + clsCommon.myCstr(gvLocation.Rows(ii).Cells("CODE").Value) + "') "
                    IsApplicable = True
                    Dim arr As Dictionary(Of String, Object) = gvLocation.Rows(ii).Tag
                    If arr IsNot Nothing AndAlso arr.Count > 0 Then
                        strWhrCatg += " and xx.[Sub Location] in ("
        ...
        Else
            strMCCMaterial += " and (xx.[Location Code] in (select Main_Location_Code from TSPL_LOCATION_MASTER where len(coalesce(Jobwork_Vendor,''))>0 and Is_Sub_Location='Y') or xx.[Location Code] in (select Location_Code from TSPL_LOCATION_MASTER where len(coalesce(Jobwork_Vendor,''))>0 and Is_Sub_Location='Y'))"
        End If
    ...
    Function ReturnQueryFordocumentinfodetail() As String
        ...
        Dim qryList As ArrayList
        qryList = ReturnQueryForDocumentInfo(fromDate.Value, ToDate.Value, Unit_Code, True)
        Dim strMCCMaterial As String = qryList(0)
        strPivotForFinalOuterQuery = qryList(1)
        strPivotForAddChargeFinalOutersumQuery = qryList(2)
        ...
        If rbtnLocationSelect.IsChecked Then
            ...
            For ii As Integer = 0 To gvLocation.RowCount - 1
                If clsCommon.myCBool(gvLocation.Rows(ii).Cells("SEL").Value) Then
                    ...
                    Dim arr As Dictionary(Of String, Object) = gvLocation.Rows(ii).Tag
                    If arr IsNot Nothing AndAlso arr.Count > 0 Then
                        strWhrCatg += " and xx.[Location Code] in ("
                        Dim isFirstTime As Boolean = True
                        For Each strInn As String In arr.Keys
                            If Not isFirstTime Then
                                strWhrCatg += ","
            ...
            strMCCMaterial += " and (" + strWhrCatg + ")"
        Else
            strMCCMaterial += " and xx.[Location Code] in (select Location_Code from TSPL_LOCATION_MASTER where len(coalesce(Jobwork_Vendor,''))>0 and Is_Sub_Location='Y'))"
        End If
    ...
    Public Shared Function ReturnQueryForDocumentInfo(ByVal From_Date As Date, ByVal To_Date As Date, ByVal Unit_Code As String, Optional ByVal IsAgainstJobWork As Boolean = False) As ArrayList
        Dim QryLst As New ArrayList
        Dim strCodeColumn As String = ""
        Dim strCodeColumnMax As String = ""
        Dim strCodeDescColumn As String = ""
        ...
        If IsAgainstJobWork Then
            strMCCMaterial += " and isnull(TSPL_PI_HEAD.isJobWorkOutward,0) =1 "
        End If
        ...
        If IsAgainstJobWork Then
            strMCCMaterial += " and isnull(TSPL_MILK_TRANSFER_IN.IsAgainstJobWork,0) =1 "
        End If
        ...
        strMCCMaterial += "  select * from (Select 'Bulk Purchase' as Trans_Type,0 as Line_No,0 as ConvRate ,TSPL_LOCATION_MASTER .location_Code  as  Bill_To_Location, (TSPL_LOCATION_MASTER.Add1) + ' ' + (TSPL_LOCATION_MASTER.Add2) + ' ' + (TSPL_LOCATION_MASTER.Add3) As [Location Address],TSPL_LOCATION_MASTER.State as [Location State],tspl_Bulk_milk_purchase_Invoice_head.isPosted " _
            ...
            & " TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=tspl_Bulk_milk_purchase_Invoice_head.Loc_Code left join TSPL_VENDOR_MASTER on " _
            & " TSPL_VENDOR_MASTER.Vendor_Code=tspl_Bulk_milk_purchase_Invoice_head.VENDOR_CODE left join TSPL_Bulk_MILK_SRN on TSPL_Bulk_MILK_SRN.SRN_NO=" _
            & " tspl_Bulk_milk_purchase_Invoice_DETAIL.SRN_NO   Left join tspl_Vendor_Master Parent_v on Parent_v.vendor_Code=tspl_Vendor_Master.Parent_Vendor_Code  left join TSPL_Dispatch_BulkSale_Trade on TSPL_Dispatch_BulkSale_Trade.Against_SRN_No=TSPL_Bulk_MILK_SRN.SRN_NO  left join TSPL_vendor_Invoice_Head on  TSPL_vendor_Invoice_Head.Against_bulkmillkPurchaseInvoice_No=tspl_Bulk_milk_purchase_Invoice_head.DOC_NO  left join TSPL_STATE_MASTER on tspl_State_Master.STATE_CODE=TSPL_VENDOR_MASTER.State_Code where convert(date,tspl_Bulk_milk_purchase_Invoice_head.DOC_DATE ,103) >= convert(date,('" + From_Date + "'),103) and convert(date,tspl_Bulk_milk_purchase_Invoice_head.DOC_DATE,103) <= convert(date,('" + To_Date + "'),103) "
        If IsAgainstJobWork Then
            strMCCMaterial += " and isnull(TSPL_BULK_MILK_PURCHASE_INVOICE_head.IsAgainstJobWork,0)=1 "
        End If
        strMCCMaterial += " )t "
        ...
        strMCCMaterial += " select * from (Select 'Bulk Purchase Return' as Trans_Type,0 as Line_No,0 as ConvRate ,TSPL_LOCATION_MASTER .location_Code  as  Bill_To_Location, (TSPL_LOCATION_MASTER.Add1) + ' ' + (TSPL_LOCATION_MASTER.Add2) + ' ' + (TSPL_LOCATION_MASTER.Add3) As [Location Address],TSPL_LOCATION_MASTER.State as [Location State],TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.isPosted " _
            ...
            & " TSPL_BULK_MILK_PURCHASE_RETURN_DETAIL.SRN_NO   Left join tspl_Vendor_Master Parent_v on Parent_v.vendor_Code=tspl_Vendor_Master.Parent_Vendor_Code  left join TSPL_Dispatch_BulkSale_Trade on TSPL_Dispatch_BulkSale_Trade.Against_SRN_No=TSPL_Bulk_MILK_SRN.SRN_NO  left join TSPL_vendor_Invoice_Head on  TSPL_vendor_Invoice_Head.Against_bulkmillkPurchaseInvoice_No=TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_No  left join TSPL_STATE_MASTER on tspl_State_Master.STATE_CODE=TSPL_VENDOR_MASTER.State_Code where convert(date,TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_Date ,103) >= convert(date,('" + From_Date + "'),103) and convert(date,TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_Date,103) <= convert(date,('" + To_Date + "'),103)"
        If IsAgainstJobWork Then
            strMCCMaterial += " and isnull(TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.IsAgainstJobWork,0)=1 "
        End If
        strMCCMaterial += " )t "
    ...
    Private Sub rbtnLocationAll_ToggleStateChanged(ByVal sender As System.Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles rbtnLocationAll.ToggleStateChanged, rbtnLocationSelect.ToggleStateChanged
        ...
        'rbtLocAll.Enabled = rbtnLocationSelect.IsChecked
        'rbtnLocSelect.Enabled = rbtnLocationSelect.IsChecked
    End Sub
