...
Public Class frmBullStatus
    Inherits FrmMainTranScreen
    Dim isNewEntry As Boolean = True
    Dim ErrorControl As New clsErrorControl()


    Private Sub txtCode__MYValidating_1(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtCode._MYValidating
        Dim Sqlqry As String = "select STSTUS_Code as Code,Name from TSPL_BULL_STATUS_MASTER where STSTUS_Code ='" + txtCode.Value + "'"
        ...
        If txtCode.MyReadOnly OrElse isButtonClicked Then
            ...
            Dim qry As String = "select STSTUS_Code as Code,Name from TSPL_BULL_STATUS_MASTER"
            txtCode.Value = clsCommon.ShowSelectForm("RTY", qry, "Code", whrClas, txtCode.Value, "TSPL_BULL_STATUS_MASTER.STSTUS_Code asc", isButtonClicked, Nothing)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            btnsave.Text = "Save"
            txtCode.MyReadOnly = False
            Dim obj As clsBullStatus = clsBullStatus.GetData(strCode, NavTyep)
            If obj IsNot Nothing Then
                isNewEntry = False
                txtCode.Value = obj.STSTUS_Code
        ...
        Try
            If (AllowToSave()) Then
                If MyBase.isModifyonPasswordFlag Then
                    If clsPasswordCheckForMasters.CheckMasterPwd(clsUserMgtCode.frmBullStatus, clsCommon.myCstr(objCommonVar.CurrentCompanyCode)) Then
                    Else
                        Return
                    End If
                End If
            End If
            Dim obj As New clsBullStatus()
            obj.STSTUS_Code = txtCode.Value
            obj.Name = txtname.Text.Replace("'", "`")

            If (obj.SaveData(obj, isNewEntry)) Then
                clsCommon.MyMessageBoxShow(Me, "Data save successfully.", Me.Text)
                LoadData(obj.STSTUS_Code, NavigatorType.Current)
            End If
            'End If
        Catch ex As Exception
    ...
    Private Sub DeleteData()
        Try
            ...
            If myMessages.deleteConfirm() Then
                If clsBullStatus.DeleteData(txtCode.Value) Then
                    myMessages.delete()
                    AddNew()
                End If
            End If
    ...
    Private Sub RadMenuItem2_Click(sender As Object, e As EventArgs) Handles RadMenuItem2.Click
        Dim qry As String = "select count(*) from TSPL_BULL_STATUS_MASTER"
        Dim check As Integer = clsDBFuncationality.getSingleValue(qry)

        If check > 0 Then
            qry = "select STSTUS_Code,Name from TSPL_BULL_STATUS_MASTER"
        Else
            qry = "select '' as STSTUS_Code,'' as Name"
        End If
        ...
        If transportSql.importExcel(gv_Import, "Code", "Name") Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv_Import.Rows
                    obj.STSTUS_Code = clsCommon.myCstr(grow.Cells("STSTUS_Code").Value)
                    If clsCommon.myLen(obj.STSTUS_Code) > 50 Then
                    ...
                    Dim qry As Integer = clsDBFuncationality.getSingleValue("select count(*) from TSPL_BULL_STATUS_MASTER where STSTUS_Code='" + obj.STSTUS_Code + "'")
                    isNewEntry = True
                    If qry > 0 Then
                        isNewEntry = False
                    End If
