    ...
    Sub FillData()
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.ScrapSaleRetrun) = CompairStringResult.Equal Then
            qry = "Select TSPL_SCRAPSALE_HEAD_RETURN_Cancel_data.Document_No  as [Document Id],convert(varchar,TSPL_SCRAPSALE_HEAD_RETURN_Cancel_data.Due_Date ,103) as [Document Date],  
                ...
                WHERE  convert(date,TSPL_SCRAPSALE_HEAD_RETURN_Cancel_data.Due_Date ,103) >= convert(date,'" + dtpFromDate.Value + "',103) 
                and convert(date,TSPL_SCRAPSALE_HEAD_RETURN_Cancel_data.Due_Date,103) <= convert(date,'" + dtpToDate.Value + "',103) 
                 ORDER BY TSPL_SCRAPSALE_HEAD_RETURN_Cancel_data.Due_Date,TSPL_SCRAPSALE_HEAD_RETURN_Cancel_data.Document_No  "
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.FrmVendorService) = CompairStringResult.Equal Then
            qry = "Select TSPL_VENDOR_INVOICE_HEAD_cancel_data.Document_No as [Document Id] " &
                 ...
                 " Where Convert(Date, TSPL_VENDOR_INVOICE_HEAD_cancel_data.Vendor_Invoice_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_VENDOR_INVOICE_HEAD_cancel_data.Vendor_Invoice_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_VENDOR_INVOICE_HEAD_cancel_data.Vendor_Invoice_Date, TSPL_VENDOR_INVOICE_HEAD_cancel_data.Document_No "
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.PaymentEntryNew) = CompairStringResult.Equal Then
            qry = "Select TSPL_PAYMENT_HEADER_cancel_data.Payment_No as [Document Id] " &
                 ",convert(varchar,TSPL_PAYMENT_HEADER_cancel_data.Payment_Date ,103) as [Document Date], " &
                 "TSPL_PAYMENT_HEADER_cancel_data.Location_Code as [Location Code], TSPL_LOCATION_MASTER.Location_Desc as [Location Name],TSPL_PAYMENT_HEADER_cancel_data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_PAYMENT_HEADER_cancel_data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_PAYMENT_HEADER_cancel_data.Cancel_By as [Cancelled By],convert(varchar,TSPL_PAYMENT_HEADER_cancel_data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_PAYMENT_HEADER_cancel_data   Left Outer Join TSPL_LOCATION_MASTER  On TSPL_PAYMENT_HEADER_cancel_data.Location_Code =TSPL_LOCATION_MASTER.Location_Code " &
                 " Where Convert(Date, TSPL_PAYMENT_HEADER_cancel_data.Payment_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_PAYMENT_HEADER_cancel_data.Payment_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_PAYMENT_HEADER_cancel_data.Payment_Date, TSPL_PAYMENT_HEADER_cancel_data.Payment_No "
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.PaymentAdjustmentEntry) = CompairStringResult.Equal Then
            qry = "Select TSPL_Payment_Adjustment_Header_cancel_data.Adjustment_No as [Document Id] " &
                 ",convert(varchar,TSPL_Payment_Adjustment_Header_cancel_data.Adjustment_Date ,103) as [Document Date], " &
                 "'' as [Location Code], '' as [Location Name],TSPL_Payment_Adjustment_Header_cancel_data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_Payment_Adjustment_Header_cancel_data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_Payment_Adjustment_Header_cancel_data.Cancel_By as [Cancelled By],convert(varchar,TSPL_Payment_Adjustment_Header_cancel_data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_Payment_Adjustment_Header_cancel_data  " &
                 " Where Convert(Date, TSPL_Payment_Adjustment_Header_cancel_data.Adjustment_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_Payment_Adjustment_Header_cancel_data.Adjustment_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_Payment_Adjustment_Header_cancel_data.Adjustment_Date, TSPL_Payment_Adjustment_Header_cancel_data.Adjustment_No "

        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.journalEntry) = CompairStringResult.Equal Then
            qry = "Select TSPL_JOURNAL_MASTER_Cancel_Data.Voucher_No as [Document Id] " &
                 ",convert(varchar,TSPL_JOURNAL_MASTER_Cancel_Data.Voucher_Date ,103) as [Document Date], " &
                 "'' as [Location Code], '' as [Location Name],TSPL_JOURNAL_MASTER_Cancel_Data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_JOURNAL_MASTER_Cancel_Data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_JOURNAL_MASTER_Cancel_Data.Cancel_By as [Cancelled By],convert(varchar,TSPL_JOURNAL_MASTER_Cancel_Data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_JOURNAL_MASTER_Cancel_Data  " &
                 " Where Convert(Date, TSPL_JOURNAL_MASTER_Cancel_Data.Voucher_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_JOURNAL_MASTER_Cancel_Data.Voucher_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_JOURNAL_MASTER_Cancel_Data.Voucher_Date, TSPL_JOURNAL_MASTER_Cancel_Data.Voucher_No "

        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.mbtnVCGLEntry) = CompairStringResult.Equal Then
            qry = "Select TSPL_VCGL_Head_CANCEL_DATA.Document_No as [Document Id] " &
                 ",convert(varchar,TSPL_VCGL_Head_CANCEL_DATA.Document_Date ,103) as [Document Date], " &
                 "'' as [Location Code], '' as [Location Name],TSPL_VCGL_Head_CANCEL_DATA.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_VCGL_Head_CANCEL_DATA.Created_Date,103) as [Created Date] ,'' as Description,TSPL_VCGL_Head_CANCEL_DATA.Cancel_By as [Cancelled By],convert(varchar,TSPL_VCGL_Head_CANCEL_DATA.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_VCGL_Head_CANCEL_DATA  " &
                 " Where Convert(Date, TSPL_VCGL_Head_CANCEL_DATA.Document_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_VCGL_Head_CANCEL_DATA.Document_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_VCGL_Head_CANCEL_DATA.Document_Date, TSPL_VCGL_Head_CANCEL_DATA.Document_No "


        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.mbtnStoreAdjustment) = CompairStringResult.Equal Then
            qry = "Select TSPL_ADJUSTMENT_HEADER_cancel_data.Adjustment_No as [Document Id] " &
                 ",convert(varchar,TSPL_ADJUSTMENT_HEADER_cancel_data.Adjustment_Date ,103) as [Document Date], " &
                 "TSPL_ADJUSTMENT_HEADER_cancel_data.Loc_Code as [Location Code], '' as [Location Name],TSPL_ADJUSTMENT_HEADER_cancel_data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_ADJUSTMENT_HEADER_cancel_data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_ADJUSTMENT_HEADER_cancel_data.Cancel_By as [Cancelled By],convert(varchar,TSPL_ADJUSTMENT_HEADER_cancel_data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_ADJUSTMENT_HEADER_cancel_data  " &
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.MilkCollectionMCC) = CompairStringResult.Equal Then
            qry = "Select TSPL_MILK_COLLECTION_MCC_cancel_data.Document_No as [Document Id] " &
                 ",convert(varchar,TSPL_MILK_COLLECTION_MCC_cancel_data.Document_Date ,103) as [Document Date], " &
                 "'' as [Location Code], '' as [Location Name],TSPL_MILK_COLLECTION_MCC_cancel_data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_MILK_COLLECTION_MCC_cancel_data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_MILK_COLLECTION_MCC_cancel_data.Cancel_By as [Cancelled By],convert(varchar,TSPL_MILK_COLLECTION_MCC_cancel_data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_MILK_COLLECTION_MCC_cancel_data  " &
                 " Where Convert(Date, TSPL_MILK_COLLECTION_MCC_cancel_data.Document_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_MILK_COLLECTION_MCC_cancel_data.Document_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_MILK_COLLECTION_MCC_cancel_data.Document_No, TSPL_MILK_COLLECTION_MCC_cancel_data.Document_Date "



        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmQualityCheckApprovalForSRN) = CompairStringResult.Equal Then
            qry = "Select TSPL_QC_CHECK_HEAD_Cancel_Data.Document_Code as [Document Id] " &
                 ",convert(varchar,TSPL_QC_CHECK_HEAD_Cancel_Data.Document_Date ,103) as [Document Date], " &
                 "TSPL_QC_CHECK_HEAD_Cancel_Data.Bill_To_location as [Location Code], '' as [Location Name],TSPL_QC_CHECK_HEAD_Cancel_Data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_QC_CHECK_HEAD_Cancel_Data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_QC_CHECK_HEAD_Cancel_Data.Cancel_By as [Cancelled By],convert(varchar,TSPL_QC_CHECK_HEAD_Cancel_Data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_QC_CHECK_HEAD_Cancel_Data  " &
                 " Where Convert(Date, TSPL_QC_CHECK_HEAD_Cancel_Data.Document_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_QC_CHECK_HEAD_Cancel_Data.Document_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_QC_CHECK_HEAD_Cancel_Data.Document_Code, TSPL_QC_CHECK_HEAD_Cancel_Data.Document_Date "
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmOutgoingQC) = CompairStringResult.Equal Then
            qry = "Select TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Document_Code as [Document Id] " &
                 ",convert(varchar,TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Document_Date ,103) as [Document Date], " &
                 "'' as [Location Code], '' as [Location Name],TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Created_Date,103) as [Created Date] ,'' as Description,TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Cancel_By as [Cancelled By],convert(varchar,TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA  " &
                 " Where Convert(Date, TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Document_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Document_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Document_Code, TSPL_PROD_QC_CHECK_HEAD_CANCEL_DATA.Document_Date "


        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmAssetStoreRequistion) = CompairStringResult.Equal Then
            qry = "Select TSPL_REQUISITION_HEAD_Cancel_Data.Requisition_Id as [Document Id] " &
                 ",convert(varchar,TSPL_REQUISITION_HEAD_Cancel_Data.Requisition_Date ,103) as [Document Date], " &
                 "TSPL_REQUISITION_HEAD_Cancel_Data.Location as [Location Code], '' as [Location Name],TSPL_REQUISITION_HEAD_Cancel_Data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_REQUISITION_HEAD_Cancel_Data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_REQUISITION_HEAD_Cancel_Data.Cancel_By as [Cancelled By],convert(varchar,TSPL_REQUISITION_HEAD_Cancel_Data.Cancel_On,103) as [Cancelled Date]  " &
                 " From TSPL_REQUISITION_HEAD_Cancel_Data  " &
                 " Where Convert(Date, TSPL_REQUISITION_HEAD_Cancel_Data.Requisition_Date,103) >= Convert(Date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_REQUISITION_HEAD_Cancel_Data.Requisition_Date,103) <= convert(date,'" + dtpToDate.Value + "',103)   " &
                 " ORDER BY TSPL_REQUISITION_HEAD_Cancel_Data.Requisition_Id, TSPL_REQUISITION_HEAD_Cancel_Data.Requisition_Date "
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.FAAcquisitionEntry) = CompairStringResult.Equal Then
            qry = "Select TSPL_ACQUISITION_HEAD_cancel_data.Acquisition_Code as [Document Id] " &
                 ",convert(varchar,TSPL_ACQUISITION_HEAD_cancel_data.Acquisition_Date ,103) as [Document Date], " &
                 "TSPL_ACQUISITION_HEAD_cancel_data.Loc_Code as [Location Code], '' as [Location Name],TSPL_ACQUISITION_HEAD_cancel_data.Created_By as [Created By] " &
                 ", convert(varchar,TSPL_ACQUISITION_HEAD_cancel_data.Created_Date,103) as [Created Date] ,'' as Description,TSPL_ACQUISITION_HEAD_cancel_data.Cancel_By as [Cancelled By],convert(varchar,TSPL_ACQUISITION_HEAD_cancel_data.Cancel_On,103) as [Cancelled Date]  " &
        ...
        ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.mbtnAPInvoiceEntry) = CompairStringResult.Equal Then
            qry = "Select TSPL_VENDOR_INVOICE_HEAD_Cancel_Data.Document_No  as [Document Id],convert(varchar,TSPL_VENDOR_INVOICE_HEAD_Cancel_Data.Vendor_Invoice_Date ,103) as [Document Date] 
            ...
            WHERE  convert(date,TSPL_VENDOR_INVOICE_HEAD_Cancel_Data.Vendor_Invoice_Date ,103) >= convert(date,'" + dtpFromDate.Value + "',103)  and convert(date,TSPL_VENDOR_INVOICE_HEAD_Cancel_Data.Vendor_Invoice_Date,103) <= convert(date,'" + dtpToDate.Value + "',103) 
                ORDER BY TSPL_VENDOR_INVOICE_HEAD_Cancel_Data.Document_No,TSPL_VENDOR_INVOICE_HEAD_Cancel_Data.Vendor_Invoice_Date   "
        End If
...
#End Region
    ...
    Private Sub gv1_CellDoubleClick(sender As Object, e As GridViewCellEventArgs) Handles gv1.CellDoubleClick
        Try
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmSaleReturndairy) = CompairStringResult.Equal Then
                clsDSSalesReturnHead.funsaleReturnDairyPrint(MyBase.Form_ID, "Cancel", clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value), False)
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.ReceiptEntry) = CompairStringResult.Equal Then
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.PaymentAdjustmentEntry) = CompairStringResult.Equal Then
                clsPaymentAdjustmentEntry.funPaymentAdjEntryPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value))
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.journalEntry) = CompairStringResult.Equal Then
                clsJournalEntryHeader.funJournalEntryPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value))
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.mbtnVCGLEntry) = CompairStringResult.Equal Then
                clsVCGLHead.funVCGLPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value), Nothing)
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.mbtnStoreAdjustment) = CompairStringResult.Equal Then
                ClsAdjustments.funAdjStorePrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value), False, False)
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.MilkCollectionMCC) = CompairStringResult.Equal Then
                clsMilkCollectionMCC.funCancelPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value))
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmQualityCheckApprovalForSRN) = CompairStringResult.Equal Then
                clsQualityCheckForSRNHead.funCancelPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value))

            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmAssetStoreRequistion) = CompairStringResult.Equal Then
                clsRequistionHead.funAssetSRPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value), False)
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.FAAcquisitionEntry) = CompairStringResult.Equal Then
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboTransaction.SelectedValue), clsUserMgtCode.frmOutgoingQC) = CompairStringResult.Equal Then
                Dim qry As String = ""
                Dim dt As New DataTable
                qry = " select prod_entry_code from TSPL_PROD_QC_CHECK_PRODUCTION_ENTRY_Cancel_Data where Document_Code ='" + clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value) + "'"

                dt = clsDBFuncationality.GetDataTable(qry)
                Dim arrGroupCode As New ArrayList

                For Each row As DataRow In dt.Rows
                    arrGroupCode.Add(row("prod_entry_code"))
                Next

                clsQualityCheckForSRNHead.funOutGoingQcEntryPrint(MyBase.Form_ID, True, clsCommon.myCDate(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document Date").Value), clsCommon.myCstr(gv1.Rows(gv1.CurrentCell.RowIndex).Cells("Document ID").Value), (arrGroupCode))
