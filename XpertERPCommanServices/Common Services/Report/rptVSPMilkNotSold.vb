...
Public Class rptVSPMilkNotSold
    ...
    Dim ApplyMonthlyYearly As Boolean = False
    ...
    Private Sub RptInventoryMovement_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        ApplyMonthlyYearly = IIf(clsCommon.myCdbl(clsFixedParameter.GetData(clsFixedParameterType.ApplyMonthlyYearly, clsFixedParameterCode.ApplyMonthlyYearly, Nothing)) = 1, True, False)
        If ApplyMonthlyYearly Then
            txtPaymentCycleCode.Visible = False
            MyLabel6.Visible = False
        Else
            txtPaymentCycleCode.Visible = True
        ...
        If PickAllMCC Then
            ...
            TxtMCCMultifnd.Location = New System.Drawing.Point(65, 60)
        Else
            txtMCC.Visible = True
            lblMCC.Visible = True
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        Try
            ...
            '    Exit Sub
            'End If
            If Not ApplyMonthlyYearly Then
...
TSPL_SD_SHIPMENT_HEAD.Document_Date <= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(dtpToDate.Value), "dd/MMM/yyyy HH:mm:ss tt") + "'"
    ...
    Private Sub chkDeduction_CheckStateChanged(sender As Object, e As EventArgs) Handles chkDeduction.CheckStateChanged
        Try
            ' If chkDeduction.Checked Then
            If chkDeduction.Checked Then
                If ApplyMonthlyYearly = False Then
                    If clsCommon.myLen(txtPaymentCycleCode.Value) > 0 Then
                    Else
                        Throw New Exception("Select Payment Cycle")
                    End If
                End If
                ...
                If AreaWiseBilling = True Then
                    ...
                    Qry = "Select Code from TSPL_DEDUCTION_MASTER Where Code In ( select  xx.DeductionCode from ( (Select Distinct TSPL_MULTIPLE_DEDUCTION_DETAIL.DeductionCode from TSPL_MULTIPLE_DEDUCTION_DETAIL
                    ...
                    Qry += " inner join tspl_mcc_master on tspl_mcc_master.MCC_Code=TSPL_MULTIPLE_DEDUCTION_HEAD.MCC_Code"
                    ...
                    Qry += "  And TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date >=  '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(dtpFromDate.Value), "dd/MMM/yyyy HH:mm:ss tt") + "' And TSPL_MULTIPLE_DEDUCTION_HEAD.Document_Date  <=  '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(dtpToDate.Value), "dd/MMM/yyyy HH:mm:ss tt") + "' and 
                                          TSPL_VLC_MASTER_HEAD.VSP_Code Not In(Select VSP_Code from TSPL_MILK_SRN_HEAD where Doc_Date >=  '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(dtpFromDate.Value), "dd/MMM/yyyy HH:mm:ss tt") + "'
                                          And Doc_Date  <=  '" + clsCommon.GetPrintDate(clsCommon.GetDateWithEndTime(dtpToDate.Value), "dd/MMM/yyyy HH:mm:ss tt") + "')) "
