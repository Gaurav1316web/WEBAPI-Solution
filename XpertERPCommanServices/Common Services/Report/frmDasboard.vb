...
Imports System.IO
'Ticket No-ERO/31/10/19-001080,Sanjay ,Task- Vehicle Utilization Report
'Ticket No-ERO/31/10/19-001079,Sanjay ,Task- Procurement Milk Purchase
'Ticket No-ERO/31/10/19-001078,Sanjay ,Task- Milk Sale,Product Sale
Public Class FrmDasboard
    ...
    Private Sub FrmDasboard_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            'ddlMonthProcurement.DataSource = clsProvisionEntry.LoadMonthName()
            'ddlMonthProcurement.ValueMember = "monthNumber"
            'ddlMonthProcurement.DisplayMember = "Monthname"
            'ddlMonthProcurement.SelectedValue = TempMonth
            'dtpYearProcurement.Value = TempYear
            dtpFromDate_Procurement.Value = clsCommon.GETSERVERDATE()
            dtpToDate_Procurement.Value = clsCommon.GETSERVERDATE()

            'ddlMonthMilkRec.DataSource = clsProvisionEntry.LoadMonthName()
            'ddlMonthMilkRec.ValueMember = "monthNumber"
            'ddlMonthMilkRec.DisplayMember = "Monthname"
            'ddlMonthMilkRec.SelectedValue = TempMonth
            'dtpYearMilkRec.Value = TempYear
            dtpFromDate_MilkRec.Value = clsCommon.GETSERVERDATE()
            dtpToDate_MilkRec.Value = clsCommon.GETSERVERDATE()

            'ddlMonthMilkSale.DataSource = clsProvisionEntry.LoadMonthName()
            'ddlMonthMilkSale.ValueMember = "monthNumber"
            'ddlMonthMilkSale.DisplayMember = "Monthname"
            'ddlMonthMilkSale.SelectedValue = TempMonth
            'dtpYearMilkSale.Value = TempYear
            dtpFromDate_MilkSale.Value = clsCommon.GETSERVERDATE()
            dtpToDate_MilkSale.Value = clsCommon.GETSERVERDATE()

            'ddlMonthProductSale.DataSource = clsProvisionEntry.LoadMonthName()
            'ddlMonthProductSale.ValueMember = "monthNumber"
            'ddlMonthProductSale.DisplayMember = "Monthname"
            'ddlMonthProductSale.SelectedValue = TempMonth
            'dtpYearProductSale.Value = TempYear
            dtpFromDate_ProductSale.Value = clsCommon.GETSERVERDATE()
            dtpToDate_ProductSale.Value = clsCommon.GETSERVERDATE()

            'ddlTransportMonth.DataSource = clsProvisionEntry.LoadMonthName()
            'ddlTransportMonth.ValueMember = "monthNumber"
            'ddlTransportMonth.DisplayMember = "Monthname"
            'ddlTransportMonth.SelectedValue = TempMonth
            'dtpTransportYear.Value = TempYear
            dtpFromDate_Transport.Value = clsCommon.GETSERVERDATE()
            dtpToDate_Transport.Value = clsCommon.GETSERVERDATE()

            'ddlStoreMonth.DataSource = clsProvisionEntry.LoadMonthName()
            'ddlStoreMonth.ValueMember = "monthNumber"
            'ddlStoreMonth.DisplayMember = "Monthname"
            'ddlStoreMonth.SelectedValue = TempMonth
            'dtpStoreYear.Value = TempYear
            dtpFromDate_Store.Value = clsCommon.GETSERVERDATE()
            dtpToDate_Store.Value = clsCommon.GETSERVERDATE()

            'PageSetupReport_ID = clsUserMgtCode.frmDasboard
            SetTabNmae()
            TabHideVisible()
    ...
    Private Sub TabHideVisible()
        ...
        If Not clsCommon.CompairString(objCommonVar.CurrentUserCode, "Admin") = CompairStringResult.Equal Then
            Dim str As String = "select TSPL_DASHBOARD_GROUP_PROGRAM_MAPPING.Dashboard_Code from TSPL_DASHBOARD_GROUP_PROGRAM_MAPPING " & _
               " left join TSPL_DASHBOARD_REPORT on TSPL_DASHBOARD_GROUP_PROGRAM_MAPPING.Dashboard_Code=TSPL_DASHBOARD_REPORT.code " & _
               " where TSPL_DASHBOARD_GROUP_PROGRAM_MAPPING.Group_Code in " & _
    ...
    Private Sub btnGo_Bank_Cash_Book_Click(sender As Object, e As EventArgs) Handles btnGo_Bank_Cash_Book.Click
        Try
            ...
            End If
            Dim strAddress As String = " (TSPL_COMPANY_MASTER.Add1 + case When isnull(TSPL_COMPANY_MASTER.Add2,'')='' Then '' else ', '+ TSPL_COMPANY_MASTER.Add2 End + Case When isnull(TSPL_COMPANY_MASTER.Add3,'')='' Then '' Else ', '+ TSPL_COMPANY_MASTER.Add3 end + case When isnull(TSPL_COMPANY_MASTER.City_Code,'') ='' then '' else ', '+ TSPL_COMPANY_MASTER.City_Code end+ Case When isnull(TSPL_COMPANY_MASTER.State,'')='' Then '' else ', '+ TSPL_COMPANY_MASTER.State end +  Case When isnull(TSPL_COMPANY_MASTER.Pincode,'')='' Then '' Else ', '+ TSPL_COMPANY_MASTER.Pincode  end) "
            Dim Qry As String = clsBankReco.GetQueryForTransactionOFBB(True, dtpFromDate_Back_Cash_Book.Value, dtpToDate_Bank_Cash_Book.Value, "", "", strAddress, "Y", "", "Bank Book", "", False, Nothing, IIf(chkExcludeProvisionBank.Checked = True, True, False))
            ...
            If dt IsNot Nothing OrElse dt.Rows.Count > 0 Then
                ...
                'barSeries4.VerticalAxis.LabelInterval = 2
                ...
                'Dim seriesPie As New PieSeries()
                'seriesPie.ShowLabels = True
                'Me.RadChartView2.Series.Add(seriesPie)
                'seriesPie.DataSource = dt
                'seriesPie.ValueMember = "Closing_Balance_Chart"
                'seriesPie.DisplayMember = "Bank Code"
                'Me.RadChartView2.LegendTitle = "Value In " + cboFigureInGraph_Bank_Cash_Book.SelectedValue
    ...
    Private Sub btnGo_Vehicle_Utilization_Click(sender As Object, e As EventArgs) Handles btnGo_Vehicle_Utilization.Click
        Try
            ...
            'Qry += " and TSPL_PROVISION_ENTRY.prov_month='" & ddlProvMonth.SelectedValue & "'"
            'Qry += " and TSPL_PROVISION_ENTRY.prov_year='" & Year(dtpProvYear.Value) & "'"
    ...
    Private Sub btnGo_Procurement_Click(sender As Object, e As EventArgs) Handles btnGo_Procurement.Click
        Try
            ...
            '"(select XX.MCC_Code,XX.MCC_Name as [MCC Name],COUNT(DISTINCT XX.VLC_Code) as [No of VLC] " & _
            '",COUNT(DISTINCT XX.VSP_CODE) as [No of VSP],SUM(XX.Qty_LTR) as [Qty In LTR],SUM(XX.Qty_KG) as [Qty In KG] " & _
            '",SUM(XX.AMOUNT) as [Milk Payment],ROUND((sum(XX.FAT_KG)*100)/SUM(XX.Qty_LTR),2) AS [FAT %],ROUND((sum(XX.SNF_KG)*100)/SUM(XX.Qty_LTR),2) AS [SNF %] " & _
            '",sum(FAT_KG) as [FAT KG],sum(SNF_KG) as [SNF KG],round(SUM(XX.AMOUNT)/SUM(XX.Qty),2) as [Rate] " & _
            '" from (select tspl_mcc_master.MCC_CODE,tspl_mcc_master.MCC_NAME, TSPL_VLC_MASTER_HEAD.VLC_Code " & _
            '",TSPL_MILK_SRN_HEAD.VSP_CODE,TSPL_MILK_SRN_DETAIL.Qty as Qty_LTR,TSPL_MILK_SRN_DETAIL.ACC_Qty as Qty_KG " & _
            '",TSPL_MILK_SRN_DETAIL.FAT_KG,TSPL_MILK_SRN_DETAIL.SNF_KG,TSPL_MILK_PURCHASE_INVOICE_DETAIL.Qty,TSPL_MILK_PURCHASE_INVOICE_DETAIL.AMOUNT " & _
            '" from TSPL_MILK_SRN_HEAD " & _
            '" Left Outer Join TSPL_MILK_SRN_DETAIL On TSPL_MILK_SRN_HEAD.DOC_CODE = TSPL_MILK_SRN_DETAIL.DOC_CODE Left Outer Join TSPL_MILK_PURCHASE_INVOICE_DETAIL On TSPL_MILK_PURCHASE_INVOICE_DETAIL.SRN_CODE = TSPL_MILK_SRN_HEAD.DOC_CODE Left Outer Join TSPL_MILK_PURCHASE_INVOICE_HEAD On TSPL_MILK_PURCHASE_INVOICE_HEAD.DOC_CODE = TSPL_MILK_PURCHASE_INVOICE_DETAIL.DOC_CODE  Left Outer Join TSPL_MCC_MASTER On TSPL_MCC_MASTER.MCC_Code = TSPL_MILK_SRN_HEAD.MCC_CODE " & _
            '" Left Outer Join TSPL_VLC_MASTER_HEAD On TSPL_VLC_MASTER_HEAD.VLC_Code = TSPL_MILK_SRN_HEAD.VLC_CODE Left Outer Join TSPL_MCC_ROUTE_MASTER On TSPL_MCC_ROUTE_MASTER.Route_Code = TSPL_MILK_SRN_HEAD.ROUTE_CODE  "
            'Qry += " where MONTH(cast(TSPL_MILK_SRN_HEAD.DOC_DATE as date)) =" & ddlProvMonth.SelectedValue & ""
            'Qry += " and YEAR(cast(TSPL_MILK_SRN_HEAD.DOC_DATE  as Date)) =" & Year(dtpProvYear.Value) & ""
            '"left join  (select  COUNT(TSPL_MP_MASTER.mp_code)AS MP_Count,TSPL_MCC_MASTER.MCC_CODE from TSPL_MP_MASTER LEFT JOIN TSPL_VLC_MASTER_HEAD ON TSPL_VLC_MASTER_HEAD.VLC_Code=TSPL_MP_MASTER.VLC_Code LEFT OUTER JOIN TSPL_MCC_MASTER on TSPL_MCC_MASTER.MCC_CODE = TSPL_VLC_MASTER_HEAD.MCC group by TSPL_MCC_MASTER.MCC_CODE)MP ON MP.MCC_Code=MAIN.MCC_Code " & _
            Qry += " (select XX.MCC_Code,XX.MCC_Name as [MCC Name],COUNT(DISTINCT XX.VLC_Code) as [No of VLC] ,COUNT(DISTINCT XX.Route_Code) as [No of Route] " & _
            ",ROUND(SUM(XX.Qty_LTR),2) as [Qty In LTR],ROUND(SUM(XX.Qty_KG),2) as [Qty In KG] ,SUM(XX.AMOUNT) as [Milk Payment] " & _
            ", ROUND((sum(XX.FAT_KG )/sum(Qty_KG))*100,2) as [FAT %],ROUND((sum(XX.SNF_KG)/sum(Qty_KG ))*100,2)  as [SNF %] " & _
            ",sum(FAT_KG) as [FAT KG],sum(SNF_KG) as [SNF KG],round(SUM(XX.AMOUNT)/SUM(XX.Qty),2) as [Rate]  from ( " & _
            "select TSPL_MILK_RECEIPT_HEAD.MCC_CODE , TSPL_MCC_MASTER.MCC_NAME , TSPL_VLC_MASTER_HEAD.VLC_Code,TSPL_MCC_ROUTE_MASTER.Route_Code " & _
             ", TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT_LTR as Qty_LTR,TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT As Qty_KG " & _
             ",Convert(decimal(18,2), TSPL_MILK_SAMPLE_DETAIL.FAT * TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT / 100) As FAT_KG, Convert(decimal(18,2),TSPL_MILK_SAMPLE_DETAIL.SNF * TSPL_MILK_RECEIPT_DETAIL.ACC_WEIGHT / 100) As SNF_KG, TSPL_MILK_SRN_DETAIL.Qty As Qty,Convert(decimal(18,2),TSPL_MILK_SRN_DETAIL.AMOUNT) As AMOUNT " & _
            ...
            Qry += " and convert(date, TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103) >=  convert(date,'" + dtpFromDate_Procurement.Value + "',103)  and  convert(date, TSPL_MILK_RECEIPT_HEAD.DOC_DATE,103) <= convert(date,'" + dtpToDate_Procurement.Value + "',103) "
            ...
            "  and convert(date,File_Date,103)>=convert(date,'" + dtpFromDate_Procurement.Value + "',103) and convert(date,File_Date,103) <=convert(date,'" + dtpToDate_Procurement.Value + "',103) and qty >0  " &
            "  union all  " &
            ...
            "  where 2 = 2  and  convert(date, VDUM.Document_Date,103) >= convert(date,'" + dtpFromDate_Procurement.Value + "',103) and  convert(date, VDUM.Document_Date,103) <= convert(date,'" + dtpToDate_Procurement.Value + "',103)  " &
            ...
            " where 2=2 "
            Qry += " and convert(date, TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Filter_Month,103) >=  convert(date,'" + dtpFromDate_Procurement.Value + "',103)  and  convert(date, TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Filter_Month,103) <= convert(date,'" + dtpToDate_Procurement.Value + "',103) " & _
            " group by TSPL_MCC_MASTER.MCC_Code " & _
             ")INC ON INC.MCC_Code=MAIN.MCC_Code " & _
            " left join (select TSPL_MCC_ROUTE_MASTER.MCC_Code,SUM(TSPL_PROVISION_ENTRY.Amount) AS [Freight Cost] from " & _
            " TSPL_PROVISION_ENTRY left join TSPL_MCC_ROUTE_MASTER on TSPL_MCC_ROUTE_MASTER.Route_Code=TSPL_PROVISION_ENTRY.Route_Code " & _
            " Left join (select TSPL_Primary_Vehicle_Master.vendor_code as [Transporter Code],tspl_vendor_master.vendor_name as [Transporter Name],TSPL_Primary_Vehicle_Master.mcc_code,TSPL_Primary_Vehicle_Master.vehicle_code from TSPL_Primary_Vehicle_Master left outer join tspl_vendor_master on tspl_vendor_master.vendor_code=TSPL_Primary_Vehicle_Master.vendor_code and tspl_vendor_master.form_type='PTM' left outer join tspl_mcc_master on tspl_mcc_master.mcc_code=TSPL_Primary_Vehicle_Master.mcc_code) as t1  " & _
            " on t1.vehicle_code=TSPL_MCC_ROUTE_MASTER.Vehicle_Code   " & _
            ...
            " where TSPL_PROVISION_ENTRY.isposted=1 and TSPL_PROVISION_ENTRY.Prog_code='M-Shift_End' " & _
            " and convert(date, TSPL_PROVISION_ENTRY.DOC_DATE,103) >=  convert(date,'" + dtpFromDate_Procurement.Value + "',103)  and  convert(date, TSPL_PROVISION_ENTRY.DOC_DATE,103) <= convert(date,'" + dtpToDate_Procurement.Value + "',103) " & _
            " GROUP BY TSPL_MCC_ROUTE_MASTER.MCC_Code " & _
          ")PROV ON PROV.MCC_Code=MAIN.MCC_Code "

            'and TSPL_PROVISION_ENTRY.prov_month ='" & ddlMonthProcurement.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpYearProcurement.Value) & "'" & _
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(Qry)
            ...
            'Dim FatPerAvg As Decimal = Math.Round(clsCommon.myCdbl(dt.Compute("AVG([FAT %])", " [FAT %] is not null")), 2)
            'Dim SNFPerAvg As Decimal = Math.Round(clsCommon.myCdbl(dt.Compute("AVG([SNF %])", " [SNF %] is not null")), 2)
            ...
            '    " where 2=2   "
            '   Qry += " and convert(date, TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date,103) >=  convert(date,'" + dtpFromDate_Procurement.Value + "',103)  and  convert(date, TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date,103) <= convert(date,'" + dtpToDate_Procurement.Value + "',103) "
            '   '" AND  MONTH(cast(TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date as date)) =" & ddlMonthProcurement.SelectedValue & " and YEAR(cast(TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date as Date)) =" & Year(dtpYearProcurement.Value) & " "
            '   Qry += "  ) As xx left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code=xx.[To MCC or Plant Code] " & _
            '     " Left Outer Join TSPL_ITEM_UOM_DETAIL  On TSPL_ITEM_UOM_DETAIL.Item_Code = xx.[Item Code] And TSPL_ITEM_UOM_DETAIL.UOM_Code = 'Ltr' " & _
            '     " where  isnull(CSA_Type,'N') ='N' and (isnull(GIT_Type,'N')='N' or isnull(GIT_Type,'N')='') and isnull(Is_Consumption_Location,0) =0  and isnull(Rejected_type,'N') ='N' and  isnull(TSPL_LOCATION_MASTER.Is_Jobwork,0) =0 and xx.DocType in('Bulk In','Bulk Ret')" & _
          ...
          " Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'CLR') t_CLR On t_CLR.QC_No = TSPL_QUALITY_CHECK.QC_No  " &
           ...
           " where 2=2 and TSPL_Bulk_MILK_SRN.isposted=1 and isnull(TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_No,'') <>''  "
            ...
            If dtBulk IsNot Nothing And dtBulk.Rows.Count > 0 Then
                ...
                'dtBulk.Merge(dtJobWork, True, MissingSchemaAction.Ignore)

                dt.Merge(dtBulk, True, MissingSchemaAction.Ignore)

                'Total Row
                'Dim NoofVLC1 As Decimal = clsCommon.myCdbl(dtBulk.Compute("SUM([No of VLC])", " [No of VLC] is not null"))
                'Dim NoofRoute1 As Decimal = clsCommon.myCdbl(dtBulk.Compute("SUM([No of Route])", " [No of Route] is not null"))
                'Dim NoofFarmer1 As Decimal = clsCommon.myCdbl(dtBulk.Compute("SUM([No of Farmer])", " [No of Farmer] is not null"))

                'Dim FatPerAvg1 As Decimal = Math.Round(clsCommon.myCdbl(dtBulk.Compute("AVG([FAT %])", " [FAT %] is not null")), 2)
                'Dim SNFPerAvg1 As Decimal = Math.Round(clsCommon.myCdbl(dtBulk.Compute("AVG([SNF %])", " [SNF %] is not null")), 2)
                ...
                'Dim FatPerAvgSub As Decimal = Math.Round(clsCommon.myCdbl(dt.Compute("AVG([FAT %])", "[MCC Name]='Total'")), 2)
                'Dim SNFPerAvgSub As Decimal = Math.Round(clsCommon.myCdbl(dt.Compute("AVG([SNF %])", "[MCC Name]='Total'")), 2)
            ...
            '" AND  MONTH(cast(TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date as date)) =" & ddlMonthProcurement.SelectedValue & " and YEAR(cast(TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date as Date)) =" & Year(dtpYearProcurement.Value) & " " & _
            Qry += " and convert(date, TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date,103) >=  convert(date,'" + dtpFromDate_Procurement.Value + "',103)  and  convert(date, TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date,103) <= convert(date,'" + dtpToDate_Procurement.Value + "',103) " &
            ...
            If dtJobWork IsNot Nothing And dtJobWork.Rows.Count > 0 Then
                ...
                'Total Row
                'Dim NoofVLC3 As Decimal = clsCommon.myCdbl(dtJobWork.Compute("SUM([No of VLC])", " [No of VLC] is not null"))
                'Dim NoofRoute3 As Decimal = clsCommon.myCdbl(dtJobWork.Compute("SUM([No of Route])", " [No of Route] is not null"))
                'Dim NoofFarmer3 As Decimal = clsCommon.myCdbl(dtJobWork.Compute("SUM([No of Farmer])", " [No of Farmer] is not null"))

                'Dim FatPerAvg3 As Decimal = Math.Round(clsCommon.myCdbl(dtJobWork.Compute("AVG([FAT %])", " [FAT %] is not null")), 2)
                'Dim SNFPerAvg3 As Decimal = Math.Round(clsCommon.myCdbl(dtJobWork.Compute("AVG([SNF %])", " [SNF %] is not null")), 2)
            ...
            'Dim FatPerAvg2 As Decimal = Math.Round(clsCommon.myCdbl(dt.Compute("AVG([FAT %])", "[MCC Name]='Total'")), 2)
            'Dim SNFPerAvg2 As Decimal = Math.Round(clsCommon.myCdbl(dt.Compute("AVG([SNF %])", "[MCC Name]='Total'")), 2)
            ...
            If dt IsNot Nothing OrElse dt.Rows.Count > 0 Then
                ...
                'Dim QtyInLTR As New GridViewSummaryItem("Qty In LTR", "{0:F2}", GridAggregateFunction.Sum)
                'summaryRowItem.Add(QtyInLTR)
                'Dim QtyInKG As New GridViewSummaryItem("Qty In KG", "{0:F2}", GridAggregateFunction.Sum)
                'summaryRowItem.Add(QtyInKG)
                'Dim MilkPayment As New GridViewSummaryItem("Milk Payment", "{0:F2}", GridAggregateFunction.Sum)
                'summaryRowItem.Add(MilkPayment)
                ...
                'Dim RentAmount As New GridViewSummaryItem("Rent Amount", "{0:F2}", GridAggregateFunction.Sum)
                'summaryRowItem.Add(RentAmount)
                'Dim FreightCost As New GridViewSummaryItem("Freight Cost", "{0:F2}", GridAggregateFunction.Sum)
                'summaryRowItem.Add(FreightCost)


                'gv_Procurement.MasterTemplate.SummaryRowsBottom.Add(summaryRowItem)
    ...
    Private Sub btnGo_MilkReceived_Click(sender As Object, e As EventArgs) Handles btnGo_MilkReceived.Click
        Try
            ...
            Dim Qry As String = ""
            'MCC Purchase
            'DISTINCT xx.[To MCC or Plant Code],TSPL_LOCATION_MASTER.Location_Desc as [To MCC or Plant Name],select yy.[To MCC or Plant Name] as [MCC Name],yy.[To MCC or Plant Code] as [MCC Code] ,COUNT(DISTINCT yy.[Vendor Code]) as [No of VLC],COUNT(DISTINCT yy.[Vendor Code]) as [No of VSP]
         ...
         ", Convert(decimal(18,2),isnull(t_SNF.Param_Field_Value,0)) As [SNF %] , Convert(decimal(18,2), isnull(t_CLR.Param_Field_Value,0)) As CLR" &
          ...
          " Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'CLR') t_CLR On t_CLR.QC_No = TSPL_QUALITY_CHECK.QC_No  " &
           " where 2 = 2 "
            Qry += " and convert(date, Tspl_Gate_Entry_Details.Date_And_Time,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, Tspl_Gate_Entry_Details.Date_And_Time,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" AND  MONTH(cast(Tspl_Gate_Entry_Details.Date_And_Time as date)) =" & ddlMonthMilkRec.SelectedValue & " and YEAR(cast(Tspl_Gate_Entry_Details.Date_And_Time  as Date)) =" & Year(dtpYearMilkRec.Value) & " "

            Qry += " union all  Select  Tspl_Gate_Entry_Details.fat_per as ChallanFatPer,Tspl_Gate_Entry_Details.snf_per as ChallanSNFPer, (Tspl_Gate_Entry_Details.fat_per* (Tspl_Gate_Entry_Details.Qty_In_Kg*-1))/100 as ChallanFATKG , (Tspl_Gate_Entry_Details.snf_Per *  (Tspl_Gate_Entry_Details.Qty_In_Kg*-1))/100 as ChallanSNFKG,TSPL_Bulk_Milk_SRN_Return.SRN_Return_NO, Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then 'Bulk Ret' Else 'MCC Ret' End As DocType, Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then 'Not Req' Else IsNull(TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_No, '') End As [Milk Receipt Challan No],  IsNull(Convert(varchar,TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_Date,103), '') As [Milk Receipt Challan Date], Tspl_Gate_Entry_Details.Vendor_Code As [Vendor Code],  TSPL_VENDOR_MASTER.Vendor_Name As [Vendor Name], Tspl_Gate_Entry_Details.Challan_No As [Challan No], Convert(varchar,Tspl_Gate_Entry_Details.Challan_Date,103) As [Challan Date], TSPL_Bulk_MILK_SRN.SRN_NO As [SRN No], Convert(varchar,TSPL_Bulk_MILK_SRN.SRN_Date,103) As [SRN Date], tspl_Bulk_milk_purchase_Invoice_Detail.DOC_NO As [Invoice No],  Convert(varchar,tspl_Bulk_milk_purchase_Invoice_head.DOC_DATE,103) As [Invoice Date], Tspl_Gate_Entry_Details.Tanker_No As [Tanker No],  Tspl_Gate_Entry_Details.Gate_Entry_No As [Gate Entry No], Convert(varchar,TSPL_Weighment_Detail.Weighment_date,103) As [Weighment Date],  Convert(varchar,Tspl_Gate_Entry_Details.Date_And_Time,103) As [Gate Entry Date], Tspl_Gate_Entry_Details.Date_And_Time As [Gate Entry],  TSPL_Weighment_Detail.Weighment_No As [Weighment No], TSPL_Weighment_Detail.Weighment_date, Tspl_Gate_Entry_Details.Qty_In_Kg*-1 As [Challan Qty],  TSPL_Weighment_Detail.Gross_Weight*-1 As [Gross Weight], TSPL_Weighment_Detail.Tare_Weight*-1 As [Tare Weight], Convert(varchar,TSPL_Weighment_Detail.Tare_Weight_date,103) + ' ' + Convert(varchar,TSPL_Weighment_Detail.Tare_Weight_date,108) As [Tare Date], TSPL_Weighment_Detail.Net_Weight*-1 As [Net Weight],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then '' Else Tspl_Gate_Entry_Details.Dispatched_From_Mcc End As [From MCC or Plant Code],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then '' Else TSPL_MCC_MASTER_From_Mcc.MCC_NAME End As [From MCC or Plant Name],  Tspl_Gate_Entry_Details.location_Code As [MCC or Plant Code], Tspl_Gate_Entry_Details.location_Code [To MCC or Plant Code],  Tspl_Gate_Entry_Details.Location_Desc As [To MCC or Plant Name], Tspl_Gate_Entry_Details.Item_Code As [Item Code],  TSPL_ITEM_MASTER.Item_Desc As [Item Desc], Case When IsNull(Tspl_Gate_Entry_Details.UOM, '') = '' Then TSPL_ITEM_UOM_DETAIL.UOM_Code Else Tspl_Gate_Entry_Details.UOM End As UOM,  TSPL_QUALITY_CHECK.QC_No As [QC No], Convert(varchar,TSPL_MILK_UNLOADING.Unloading_Date_Time,103) As [Unloading Date Time],  Convert(varchar,TSPL_QUALITY_CHECK.QC_In_Date_Time,103) As [QC Date Time], Case When TSPL_QUALITY_CHECK.isPosted = '0' And TSPL_QUALITY_CHECK.is_Param_Accepted = '0' Then 'Pending' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '0' Then 'Rejected' Else Case When TSPL_QUALITY_CHECK.isPosted = '0' And TSPL_QUALITY_CHECK.is_Param_Accepted = TSPL_QUALITY_CHECK.is_Param_Accepted Then 'Pending' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '1' Then 'Accepted' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '2' Then 'Accepted with Special Approval' End End End End End As STATUS,  TSPL_MILK_UNLOADING.Unloading_No As [Unloading No], TSPL_MILK_UNLOADING.Sub_location_Code As [MCC Name], TSPL_MILK_UNLOADING.Sub_location_Code As Plant, TSPL_MILK_UNLOADING.Sub_location_Code As [Silo Code],  TSPL_LOCATION_MASTER.Location_Desc As [Silo Desc], TSPL_Gate_Out.Doc_No As [Gate Out No], Convert(varchar,TSPL_Gate_Out.Doc_Date,103) As [Gate Out Date Time],  Convert(decimal(18,2),isnull(t_FAT.Param_Field_Value,0)) As [FAT %] , Convert(decimal(18,2),isnull(t_SNF.Param_Field_Value,0)) As [SNF %] , Convert(decimal(18,2), isnull(t_CLR.Param_Field_Value,0)) As CLR, TSPL_Bulk_MILK_SRN.StandardRate As [Standard Rate], Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then TSPL_Bulk_MILK_SRN.BasicRate Else TSPL_MCC_Dispatch_Challan.Transfer_Price End As [Basic Rate],  TSPL_Bulk_MILK_SRN.Incentive, TSPL_Bulk_MILK_SRN.Deduction, TSPL_Bulk_MILK_SRN.SpecialDeduction As [Special Deduction], Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.NetRate) As [Net Rate], Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.fat_Rate) Else TSPL_MCC_Dispatch_Challan.FAT_RATE End As [FAT Rate],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.SNF_Rate) Else TSPL_MCC_Dispatch_Challan.SNF_RATE End As [SNF Rate],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.FatAmt)*-1 Else (TSPL_MCC_Dispatch_Challan.FAT_RATE * TSPL_MCC_Dispatch_Challan.FAT_KG)*-1 End As [FAT Amt],  Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.SnfAmt)*-1 Else (TSPL_MCC_Dispatch_Challan.SNF_RATE * TSPL_MCC_Dispatch_Challan.SNF_KG)*-1 End As [SNF Amt],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then TSPL_Bulk_MILK_SRN.Actual_Amount*-1 Else TSPL_MCC_Dispatch_Challan.Amount*-1 End As [Total Amount Temp], 'For ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Fat_Weightage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.FAT_W) End + ' & ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Snf_Weightage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.SNF_W) End As 'FAT Weightage & SNF Weightage', 'For ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Fat_Percentage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.FAT_R) End + ' & ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Snf_Percentage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.SNF_R) End As 'FAT Ratio & SNF ratio',  TSPL_VENDOR_MASTER.Vendor_Type As [Vendor Class]     From  TSPL_MILK_TRANSFER_IN_RETURN LEFT OUTER JOIN  TSPL_MILK_TRANSFER_IN On TSPL_MILK_TRANSFER_IN.Receipt_Challan_No = TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_No  LEFT OUTER JOIN Tspl_Gate_Entry_Details ON  Tspl_Gate_Entry_Details.Gate_Entry_No=TSPL_MILK_TRANSFER_IN.Gate_Entry_no  Left Outer Join TSPL_Weighment_Detail On TSPL_Weighment_Detail.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_VENDOR_MASTER On TSPL_VENDOR_MASTER.Vendor_Code = Tspl_Gate_Entry_Details.Vendor_Code  Left Join TSPL_MCC_MASTER As TSPL_MCC_MASTER_From_Mcc On Tspl_Gate_Entry_Details.Dispatched_From_Mcc = TSPL_MCC_MASTER_From_Mcc.MCC_Code  Left Outer Join TSPL_ITEM_MASTER On TSPL_ITEM_MASTER.Item_Code = Tspl_Gate_Entry_Details.Item_Code  Left Outer Join TSPL_QUALITY_CHECK On TSPL_QUALITY_CHECK.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_MILK_UNLOADING On TSPL_MILK_UNLOADING.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_MILK_UNLOADING.Sub_location_Code  Left Outer Join TSPL_Gate_Out On TSPL_Gate_Out.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_Bulk_MILK_SRN On TSPL_Bulk_MILK_SRN.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No Left Join TSPL_Bulk_Milk_SRN_Return On TSPL_Bulk_Milk_SRN_Return.SRN_NO = TSPL_Bulk_MILK_SRN.SRN_NO  Left Outer Join TSPL_Bulk_Price_MASTER On TSPL_Bulk_Price_MASTER.Price_Code = TSPL_Bulk_MILK_SRN.Price_Code  Left Outer Join tspl_Bulk_milk_purchase_Invoice_Detail On tspl_Bulk_milk_purchase_Invoice_Detail.SRN_NO = TSPL_Bulk_MILK_SRN.SRN_NO  Left Outer Join tspl_Bulk_milk_purchase_Invoice_head On tspl_Bulk_milk_purchase_Invoice_head.DOC_NO = tspl_Bulk_milk_purchase_Invoice_Detail.DOC_NO  Left Outer Join TSPL_ITEM_UOM_DETAIL On TSPL_ITEM_UOM_DETAIL.Item_Code = Tspl_Gate_Entry_Details.Item_Code And TSPL_ITEM_UOM_DETAIL.Stocking_Unit = 'Y'  Left Outer Join TSPL_MCC_Dispatch_Challan On TSPL_MCC_Dispatch_Challan.Chalan_NO = Tspl_Gate_Entry_Details.Challan_No  Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'FAT') t_FAT On t_FAT.QC_No = TSPL_QUALITY_CHECK.QC_No  " & _
                    " Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'SNF') t_SNF On t_SNF.QC_No = TSPL_QUALITY_CHECK.QC_No  " & _
                    " Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'CLR') t_CLR On t_CLR.QC_No = TSPL_QUALITY_CHECK.QC_No  " & _
            " where 2 = 2 "
            Qry += " and convert(date, TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_Date,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_Date,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" AND  MONTH(cast(TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_Date as date)) =" & ddlMonthMilkRec.SelectedValue & " and YEAR(cast(TSPL_MILK_TRANSFER_IN_RETURN.Receipt_Challan_Return_Date  as Date)) =" & Year(dtpYearMilkRec.Value) & " "
            Qry += " union all  Select  Tspl_Gate_Entry_Details.fat_per as ChallanFatPer,Tspl_Gate_Entry_Details.snf_per as ChallanSNFPer, (Tspl_Gate_Entry_Details.fat_per* (Tspl_Gate_Entry_Details.Qty_In_Kg*-1))/100 as ChallanFATKG , (Tspl_Gate_Entry_Details.snf_Per *  (Tspl_Gate_Entry_Details.Qty_In_Kg*-1))/100 as ChallanSNFKG ,TSPL_Bulk_Milk_SRN_Return.SRN_Return_NO, 'Purchase Return'  As DocType, Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then 'Not Req' Else IsNull(TSPL_MILK_TRANSFER_IN.Receipt_Challan_No, '') End As [Milk Receipt Challan No],  IsNull(Convert(varchar,TSPL_MILK_TRANSFER_IN.Receipt_Challan_Date,103), '') As [Milk Receipt Challan Date], Tspl_Gate_Entry_Details.Vendor_Code As [Vendor Code],  TSPL_VENDOR_MASTER.Vendor_Name As [Vendor Name], Tspl_Gate_Entry_Details.Challan_No As [Challan No], Convert(varchar,Tspl_Gate_Entry_Details.Challan_Date,103) As [Challan Date], TSPL_Bulk_MILK_SRN.SRN_NO As [SRN No], Convert(varchar,TSPL_Bulk_MILK_SRN.SRN_Date,103) As [SRN Date], tSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_No As [Invoice No],  Convert(varchar,TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_Date,103) As [Invoice Date], Tspl_Gate_Entry_Details.Tanker_No As [Tanker No],  Tspl_Gate_Entry_Details.Gate_Entry_No As [Gate Entry No], Convert(varchar,TSPL_Weighment_Detail.Weighment_date,103) As [Weighment Date],  Convert(varchar,Tspl_Gate_Entry_Details.Date_And_Time,103) As [Gate Entry Date], Tspl_Gate_Entry_Details.Date_And_Time As [Gate Entry],  TSPL_Weighment_Detail.Weighment_No As [Weighment No], TSPL_Weighment_Detail.Weighment_date, (-1)* Tspl_Gate_Entry_Details.Qty_In_Kg As [Challan Qty],  (-1)* TSPL_Weighment_Detail.Gross_Weight As [Gross Weight], (-1)* TSPL_Weighment_Detail.Tare_Weight As [Tare Weight], Convert(varchar,TSPL_Weighment_Detail.Tare_Weight_date,103) As [Tare Date], (-1)* TSPL_Weighment_Detail.Net_Weight As [Net Weight],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then '' Else Tspl_Gate_Entry_Details.Dispatched_From_Mcc End As [From MCC or Plant Code],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then '' Else TSPL_MCC_MASTER_From_Mcc.MCC_NAME End As [From MCC or Plant Name],  Tspl_Gate_Entry_Details.location_Code As [MCC or Plant Code], Tspl_Gate_Entry_Details.location_Code [To MCC or Plant Code],  Tspl_Gate_Entry_Details.Location_Desc As [To MCC or Plant Name], Tspl_Gate_Entry_Details.Item_Code As [Item Code],  TSPL_ITEM_MASTER.Item_Desc As [Item Desc], Case When IsNull(Tspl_Gate_Entry_Details.UOM, '') = '' Then TSPL_ITEM_UOM_DETAIL.UOM_Code Else Tspl_Gate_Entry_Details.UOM End As UOM,  TSPL_QUALITY_CHECK.QC_No As [QC No], Convert(varchar,TSPL_MILK_UNLOADING.Unloading_Date_Time,103) As [Unloading Date Time],  Convert(varchar,TSPL_QUALITY_CHECK.QC_In_Date_Time,103) As [QC Date Time], Case When TSPL_QUALITY_CHECK.isPosted = '0' And TSPL_QUALITY_CHECK.is_Param_Accepted = '0' Then 'Pending' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '0' Then 'Rejected' Else Case When TSPL_QUALITY_CHECK.isPosted = '0' And TSPL_QUALITY_CHECK.is_Param_Accepted = TSPL_QUALITY_CHECK.is_Param_Accepted Then 'Pending' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '1' Then 'Accepted' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '2' Then 'Accepted with Special Approval' End End End End End As STATUS,  TSPL_MILK_UNLOADING.Unloading_No As [Unloading No], TSPL_MILK_UNLOADING.Sub_location_Code As [MCC Name], TSPL_MILK_UNLOADING.Sub_location_Code As Plant, TSPL_MILK_UNLOADING.Sub_location_Code As [Silo Code],  TSPL_LOCATION_MASTER.Location_Desc As [Silo Desc], TSPL_Gate_Out.Doc_No As [Gate Out No], Convert(varchar,TSPL_Gate_Out.Doc_Date,103) As [Gate Out Date Time],  Convert(decimal(18,2),t_FAT.Param_Field_Value) As [FAT %] , Convert(decimal(18,2),t_SNF.Param_Field_Value) As [SNF %] , Convert(decimal(18,2), t_CLR.Param_Field_Value) As CLR, TSPL_Bulk_MILK_SRN.StandardRate As [Standard Rate], Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then TSPL_Bulk_MILK_SRN.BasicRate Else TSPL_MCC_Dispatch_Challan.Transfer_Price End As [Basic Rate],  (-1)* TSPL_Bulk_MILK_SRN.Incentive as Incentive, TSPL_Bulk_MILK_SRN.Deduction, TSPL_Bulk_MILK_SRN.SpecialDeduction As [Special Deduction], Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.NetRate) As [Net Rate], Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.fat_Rate) Else TSPL_MCC_Dispatch_Challan.FAT_RATE End As [FAT Rate],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.SNF_Rate) Else TSPL_MCC_Dispatch_Challan.SNF_RATE End As [SNF Rate],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(decimal(18,2),(-1)*TSPL_Bulk_MILK_SRN.FatAmt) Else (-1)*(TSPL_MCC_Dispatch_Challan.FAT_RATE * TSPL_MCC_Dispatch_Challan.FAT_KG) End As [FAT Amt],  Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then Convert(decimal(18,2),(-1)*TSPL_Bulk_MILK_SRN.SnfAmt) Else (-1)*(TSPL_MCC_Dispatch_Challan.SNF_RATE * TSPL_MCC_Dispatch_Challan.SNF_KG) End As [SNF Amt],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then (-1)* TSPL_Bulk_MILK_SRN.Actual_Amount Else (-1)*TSPL_MCC_Dispatch_Challan.Amount End As [Total Amount Temp], 'For ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Fat_Weightage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.FAT_W) End + ' & ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Snf_Weightage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.SNF_W) End As 'FAT Weightage & SNF Weightage', 'For ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Fat_Percentage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.FAT_R) End + ' & ' + Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then Convert(varchar,TSPL_Bulk_Price_MASTER.Snf_Percentage) Else Convert(varchar,TSPL_MCC_Dispatch_Challan.SNF_R) End As 'FAT Ratio & SNF ratio',  TSPL_VENDOR_MASTER.Vendor_Type As [Vendor Class]     From Tspl_Gate_Entry_Details Left Outer Join TSPL_Weighment_Detail On TSPL_Weighment_Detail.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_VENDOR_MASTER On TSPL_VENDOR_MASTER.Vendor_Code = Tspl_Gate_Entry_Details.Vendor_Code  Left Join TSPL_MCC_MASTER As TSPL_MCC_MASTER_From_Mcc On Tspl_Gate_Entry_Details.Dispatched_From_Mcc = TSPL_MCC_MASTER_From_Mcc.MCC_Code  Left Outer Join TSPL_ITEM_MASTER On TSPL_ITEM_MASTER.Item_Code = Tspl_Gate_Entry_Details.Item_Code  Left Outer Join TSPL_QUALITY_CHECK On TSPL_QUALITY_CHECK.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_MILK_UNLOADING On TSPL_MILK_UNLOADING.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_MILK_UNLOADING.Sub_location_Code  Left Outer Join TSPL_Gate_Out On TSPL_Gate_Out.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_Bulk_MILK_SRN On TSPL_Bulk_MILK_SRN.Gate_Entry_No = Tspl_Gate_Entry_Details.Gate_Entry_No Left Join TSPL_Bulk_Milk_SRN_Return On TSPL_Bulk_Milk_SRN_Return.SRN_NO = TSPL_Bulk_MILK_SRN.SRN_NO  Left Outer Join TSPL_Bulk_Price_MASTER On TSPL_Bulk_Price_MASTER.Price_Code = TSPL_Bulk_MILK_SRN.Price_Code  Left Outer Join tspl_Bulk_milk_purchase_Invoice_Detail On tspl_Bulk_milk_purchase_Invoice_Detail.SRN_NO = TSPL_Bulk_MILK_SRN.SRN_NO  Left Outer Join tspl_Bulk_milk_purchase_Invoice_head On tspl_Bulk_milk_purchase_Invoice_head.DOC_NO = tspl_Bulk_milk_purchase_Invoice_Detail.DOC_NO  Left Outer Join TSPL_BULK_MILK_PURCHASE_RETURN_HEAD On TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Invoice_No  = tspl_Bulk_milk_purchase_Invoice_head.DOC_NO  Left Outer Join TSPL_BULK_MILK_PURCHASE_RETURN_DETAIL  On TSPL_BULK_MILK_PURCHASE_RETURN_DETAIL.Pur_Return_No = TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_No   Left Outer Join TSPL_ITEM_UOM_DETAIL On TSPL_ITEM_UOM_DETAIL.Item_Code = Tspl_Gate_Entry_Details.Item_Code And TSPL_ITEM_UOM_DETAIL.Stocking_Unit = 'Y'  Left Outer Join TSPL_MILK_TRANSFER_IN On TSPL_MILK_TRANSFER_IN.Gate_Entry_no = Tspl_Gate_Entry_Details.Gate_Entry_No  Left Outer Join TSPL_MCC_Dispatch_Challan On TSPL_MCC_Dispatch_Challan.Chalan_NO = Tspl_Gate_Entry_Details.Challan_No   Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'FAT') t_FAT " & _
           " On t_FAT.QC_No = TSPL_QUALITY_CHECK.QC_No   Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'SNF') t_SNF On t_SNF.QC_No = TSPL_QUALITY_CHECK.QC_No  Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'CLR') t_CLR On t_CLR.QC_No = TSPL_QUALITY_CHECK.QC_No " & _
           " where 2=2 and TSPL_Bulk_MILK_SRN.isposted=1 and isnull(TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_No,'') <>''  "
            Qry += " and convert(date, TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_Date,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_Date,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" AND  MONTH(cast(TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_Date as date)) =" & ddlMonthMilkRec.SelectedValue & " and YEAR(cast(TSPL_BULK_MILK_PURCHASE_RETURN_HEAD .Pur_Return_Date as Date)) =" & Year(dtpYearMilkRec.Value) & " "
            ...
            Qry += "  UNION ALL Select  Tspl_Gate_Entry_Details.fat_per as ChallanFatPer,Tspl_Gate_Entry_Details.snf_per as ChallanSNFPer, (Tspl_Gate_Entry_Details.fat_per* (Tspl_Gate_Entry_Details.Qty_In_Kg*-1))/100 as ChallanFATKG , (Tspl_Gate_Entry_Details.snf_Per *  (Tspl_Gate_Entry_Details.Qty_In_Kg*-1))/100 as ChallanSNFKG , TSPL_Bulk_Milk_SRN_Return.SRN_Return_NO, Case When IsNull(Tspl_Gate_Entry_Details.Doc_Type, '') = 'BulkProc' Then 'Bulk Ret' Else 'MCC Ret' End As DocType,  'Not Req'  As [Milk Receipt Challan No],  '' As [Milk Receipt Challan Date], Tspl_Gate_Entry_Details.Vendor_Code As [Vendor Code],  TSPL_VENDOR_MASTER.Vendor_Name As [Vendor Name], Tspl_Gate_Entry_Details.Challan_No As [Challan No], Convert(varchar,Tspl_Gate_Entry_Details.Challan_Date,103) As [Challan Date], TSPL_Bulk_MILK_SRN.SRN_NO As [SRN No], Convert(varchar,TSPL_Bulk_MILK_SRN.SRN_Date,103) As [SRN Date], tspl_Bulk_milk_purchase_Invoice_Detail.DOC_NO As [Invoice No],  Convert(varchar,tspl_Bulk_milk_purchase_Invoice_head.DOC_DATE,103) As [Invoice Date], Tspl_Gate_Entry_Details.Tanker_No As [Tanker No],  Tspl_Gate_Entry_Details.Gate_Entry_No As [Gate Entry No], Convert(varchar,TSPL_Weighment_Detail.Weighment_date,103) As [Weighment Date],  Convert(varchar,Tspl_Gate_Entry_Details.Date_And_Time,103) As [Gate Entry Date], Tspl_Gate_Entry_Details.Date_And_Time As [Gate Entry],  TSPL_Weighment_Detail.Weighment_No As [Weighment No], TSPL_Weighment_Detail.Weighment_date, Tspl_Gate_Entry_Details.Qty_In_Kg*-1 As [Challan Qty],  TSPL_Weighment_Detail.Gross_Weight*-1 As [Gross Weight], TSPL_Weighment_Detail.Tare_Weight*-1 As [Tare Weight], Convert(varchar,TSPL_Weighment_Detail.Tare_Weight_date,103) + ' ' + Convert(varchar,TSPL_Weighment_Detail.Tare_Weight_date,108) As [Tare Date], TSPL_Weighment_Detail.Net_Weight*-1 As [Net Weight],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then '' Else Tspl_Gate_Entry_Details.Dispatched_From_Mcc End As [From MCC or Plant Code],  Case When Tspl_Gate_Entry_Details.Doc_Type = 'BulkProc' Then '' Else TSPL_MCC_MASTER_From_Mcc.MCC_NAME End As [From MCC or Plant Name],  Tspl_Gate_Entry_Details.location_Code As [MCC or Plant Code], Tspl_Gate_Entry_Details.location_Code [To MCC or Plant Code],  Tspl_Gate_Entry_Details.Location_Desc As [To MCC or Plant Name], Tspl_Gate_Entry_Details.Item_Code As [Item Code],  TSPL_ITEM_MASTER.Item_Desc As [Item Desc], Case When IsNull(Tspl_Gate_Entry_Details.UOM, '') = '' Then TSPL_ITEM_UOM_DETAIL.UOM_Code Else Tspl_Gate_Entry_Details.UOM End As UOM,  TSPL_QUALITY_CHECK.QC_No As [QC No], Convert(varchar,TSPL_MILK_UNLOADING.Unloading_Date_Time,103) As [Unloading Date Time],  Convert(varchar,TSPL_QUALITY_CHECK.QC_In_Date_Time,103) As [QC Date Time], Case When TSPL_QUALITY_CHECK.isPosted = '0' And TSPL_QUALITY_CHECK.is_Param_Accepted = '0' Then 'Pending' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '0' Then 'Rejected' Else Case When TSPL_QUALITY_CHECK.isPosted = '0' And TSPL_QUALITY_CHECK.is_Param_Accepted = TSPL_QUALITY_CHECK.is_Param_Accepted Then 'Pending' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '1' Then 'Accepted' Else Case When TSPL_QUALITY_CHECK.isPosted = '1' And TSPL_QUALITY_CHECK.is_Param_Accepted = '2' Then 'Accepted with Special Approval' End End End End End As STATUS,  TSPL_MILK_UNLOADING.Unloading_No As [Unloading No], TSPL_MILK_UNLOADING.Sub_location_Code As [MCC Name], TSPL_MILK_UNLOADING.Sub_location_Code As Plant, TSPL_MILK_UNLOADING.Sub_location_Code As [Silo Code],  TSPL_LOCATION_MASTER.Location_Desc As [Silo Desc], TSPL_Gate_Out.Doc_No As [Gate Out No], Convert(varchar,TSPL_Gate_Out.Doc_Date,103) As [Gate Out Date Time] ,  Convert(decimal(18,2),isnull(t_FAT.Param_Field_Value,0)) As [FAT %] , Convert(decimal(18,2),isnull(t_SNF.Param_Field_Value,0)) As [SNF %] , Convert(decimal(18,2), isnull(t_CLR.Param_Field_Value,0)) As CLR, TSPL_Bulk_MILK_SRN.StandardRate As [Standard Rate], TSPL_Bulk_MILK_SRN.BasicRate As [Basic Rate],  TSPL_Bulk_MILK_SRN.Incentive, TSPL_Bulk_MILK_SRN.Deduction, TSPL_Bulk_MILK_SRN.SpecialDeduction As [Special Deduction], Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.NetRate) As [Net Rate],  Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.fat_Rate) As [FAT Rate],   Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.SNF_Rate) As [SNF Rate],  Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.FatAmt)*-1  As [FAT Amt],  Convert(decimal(18,2),TSPL_Bulk_MILK_SRN.SnfAmt)*-1  As [SNF Amt],  TSPL_Bulk_MILK_SRN.Actual_Amount*-1 As [Total Amount Temp], 'For ' +  Convert(varchar,TSPL_Bulk_Price_MASTER.Fat_Weightage)  + ' & ' + Convert(varchar,TSPL_Bulk_Price_MASTER.Snf_Weightage)  As 'FAT Weightage & SNF Weightage', 'For ' + Convert(varchar,TSPL_Bulk_Price_MASTER.Fat_Percentage)  + ' & ' +  Convert(varchar,TSPL_Bulk_Price_MASTER.Snf_Percentage)  As 'FAT Ratio & SNF ratio',  TSPL_VENDOR_MASTER.Vendor_Type As [Vendor Class]     From  " & _
              ...
              " Left Outer Join TSPL_ITEM_UOM_DETAIL On TSPL_ITEM_UOM_DETAIL.Item_Code = Tspl_Gate_Entry_Details.Item_Code And TSPL_ITEM_UOM_DETAIL.Stocking_Unit = 'Y'  Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'FAT') t_FAT On t_FAT.QC_No = TSPL_QUALITY_CHECK.QC_No   Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'SNF') t_SNF On t_SNF.QC_No = TSPL_QUALITY_CHECK.QC_No  Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'CLR') t_CLR On t_CLR.QC_No = TSPL_QUALITY_CHECK.QC_No  " & _
             " where 2=2   "
            Qry += " and convert(date, TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" AND  MONTH(cast(TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date as date)) =" & ddlMonthMilkRec.SelectedValue & " and YEAR(cast(TSPL_Bulk_Milk_SRN_Return.SRN_Return_Date as Date)) =" & Year(dtpYearMilkRec.Value) & " "
            ...
            '" ) as yy  GROUP BY yy.[To MCC or Plant Name],yy.[To MCC or Plant Code] "
            Qry += " ) as zz " & _
            "left join  (select TSPL_MCC_MASTER.MCC_Code,sum(TSPL_INCENTIVE_ENTRY_BY_SRN_DETAIL.Incentive_Amount) as Incentive_Amount " & _
            ",(sum(TSPL_INCENTIVE_ENTRY_BY_SRN_DETAIL.Rent_Amount)) as Rent_Amount " & _
            " from TSPL_INCENTIVE_ENTRY_BY_SRN_DETAIL " & _
            " LEFT JOIN TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD ON TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Doc_Code=TSPL_INCENTIVE_ENTRY_BY_SRN_DETAIL.Doc_Code " & _
            " left outer join TSPL_MCC_MASTER on TSPL_MCC_MASTER.MCC_Code=TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.MCC_Code "
            Qry += " where convert(date, TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Filter_Month,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Filter_Month,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" where MONTH(cast(TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Filter_Month as date)) =" & ddlMonthMilkRec.SelectedValue & " and YEAR(cast(TSPL_INCENTIVE_ENTRY_BY_SRN_HEAD.Filter_Month  as Date)) =" & Year(dtpYearMilkRec.Value) & "" & _
            Qry += " group by TSPL_MCC_MASTER.MCC_Code " & _
             ")INC ON INC.MCC_Code=zz.[MCC Code] " & _
            " left join (select TSPL_MCC_ROUTE_MASTER.MCC_Code,SUM(TSPL_PROVISION_ENTRY.Amount) AS [Freight Cost] from " & _
            " TSPL_PROVISION_ENTRY left join TSPL_MCC_ROUTE_MASTER on TSPL_MCC_ROUTE_MASTER.Route_Code=TSPL_PROVISION_ENTRY.Route_Code " & _
            " Left join (select TSPL_Primary_Vehicle_Master.vendor_code as [Transporter Code],tspl_vendor_master.vendor_name as [Transporter Name],TSPL_Primary_Vehicle_Master.mcc_code,TSPL_Primary_Vehicle_Master.vehicle_code from TSPL_Primary_Vehicle_Master left outer join tspl_vendor_master on tspl_vendor_master.vendor_code=TSPL_Primary_Vehicle_Master.vendor_code and tspl_vendor_master.form_type='PTM' left outer join tspl_mcc_master on tspl_mcc_master.mcc_code=TSPL_Primary_Vehicle_Master.mcc_code) as t1  " & _
            " on t1.vehicle_code=TSPL_MCC_ROUTE_MASTER.Vehicle_Code   " & _
            ...
            Qry += " where TSPL_PROVISION_ENTRY.isposted=1 and TSPL_PROVISION_ENTRY.Prog_code='M-Shift_End' and convert(date, TSPL_PROVISION_ENTRY.Doc_Date,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, TSPL_PROVISION_ENTRY.Doc_Date,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" where TSPL_PROVISION_ENTRY.isposted=1 and TSPL_PROVISION_ENTRY.Prog_code='M-Shift_End' and TSPL_PROVISION_ENTRY.prov_month ='" & ddlMonthMilkRec.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpYearMilkRec.Value) & "'" & _
            Qry += " GROUP BY TSPL_MCC_ROUTE_MASTER.MCC_Code " & _
         ...
         ", Convert(decimal(18,2),isnull(t_SNF.Param_Field_Value,0)) As [SNF %] , Convert(decimal(18,2), isnull(t_CLR.Param_Field_Value,0)) As CLR" &
          ...
          " Left Outer Join (Select TSPL_QC_Parameter_Detail.*      From TSPL_QUALITY_CHECK Left Outer Join TSPL_QC_Parameter_Detail On TSPL_QC_Parameter_Detail.QC_No = TSPL_QUALITY_CHECK.QC_No And TSPL_QC_Parameter_Detail.Param_Type = 'CLR') t_CLR On t_CLR.QC_No = TSPL_QUALITY_CHECK.QC_No  " &
           ...
           " where 2=2 and TSPL_Bulk_MILK_SRN.isposted=1 and isnull(TSPL_BULK_MILK_PURCHASE_RETURN_HEAD.Pur_Return_No,'') <>''  "
            ...
            "  where " &
            " TSPL_GENERAL_WEIGHMENT_DETAIL.Item_Code is not null and TSPL_GENERAL_WEIGHMENT_DETAIL.IsJobWork = 1 And TSPL_GENERAL_WEIGHMENT_DETAIL.Posted = 1   "
            Qry += " and convert(date, TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date,103) >=  convert(date,'" + dtpFromDate_MilkRec.Value + "',103)  and  convert(date, TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date,103) <= convert(date,'" + dtpToDate_MilkRec.Value + "',103) "
            '" AND  MONTH(cast(TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date as date)) =" & ddlMonthMilkRec.SelectedValue & " and YEAR(cast(TSPL_GENERAL_WEIGHMENT_DETAIL.Weighment_Date as Date)) =" & Year(dtpYearMilkRec.Value) & " " & _
            ...
            If dt IsNot Nothing OrElse dt.Rows.Count > 0 Then
                ...
                'gv_MilkReceived.MasterTemplate.SummaryRowsBottom.Add(summaryRowItem)
                Dim dr As DataRow() = dt.Select(" CPL is not null and CPL> 0 and [MCC Name]<>'Total' and [MCC Name]<>'Sub Total' and [MCC Name]<>'Grand Total'")
                If dr IsNot Nothing AndAlso dr.Length > 0 Then
                    Dim dtchart As DataTable = dr.CopyToDataTable()
                    Dim barSeries As New Telerik.WinControls.UI.BarSeries("CPL", "MCC Name")
                    ...
                    'barSeries.VerticalAxis.LabelInterval = 1
    ...
    Private Sub btnGo_MilkSale_Click(sender As Object, e As EventArgs) Handles btnGo_MilkSale.Click
        Try
            ...
            'Dim EndDate As String = ""
            Dim Qry As String = ""
            'StartDate = "01/" + clsCommon.myCstr(ddlMonthMilkSale.SelectedValue) + "/" + clsCommon.myCstr(Year(dtpYearMilkSale.Value))
            'EndDate = clsCommon.myCstr(ddlMonthMilkSale.SelectedValue) + "/01/" + clsCommon.myCstr(Year(dtpYearMilkSale.Value))
            Dim obj As New clsSaleRegisterParameterType
            'obj.ReportType = "Item Wise"
            obj.Unit_Code = "Ltr"
            obj.From_Date = clsCommon.myCDate(dtpFromDate_MilkSale.Value)
            'EndDate = clsDBFuncationality.getSingleValue("select EOMONTH('" + EndDate + "')")
            obj.To_Date = clsCommon.myCDate(dtpToDate_MilkSale.Value)
            ...
            '    Qry = "select isnull(TSPL_ITEM_MASTER.Alies_Name,'') as [Item],Sum(TempData.Quantity) as [Quantity In Ltr],sum(TempData.[Total FAT KG]) AS [FAT KG] " & _
            '",sum(TempData.[Total SNF KG]) AS [SNF KG],sum(TempData.[Total Sale Amount]) AS [Sale Amount],convert(decimal(18,2),sum(TempData.[Total Sale Amount])/Sum(TempData.Quantity)) AS [Ave Realisa Per Ltr] " & _
            '" from( "
            '    Qry += clsPSInvoiceHead.GetReportDataQuery(obj)
            '    Qry += ")TempData " & _
            '    " left join tspl_item_master on tspl_item_master.Item_Code=TempData.[Item Code] " & _
            '    " where isnull(TSPL_ITEM_MASTER.Is_FreshItem,0)=1 " & _
 ...
 " from (  "
  ...
  " group by isnull(TSPL_ITEM_MASTER.Alies_Name,'') order by isnull(TSPL_ITEM_MASTER.Alies_Name,'')"
            ...
            obj.Unit_Code = "Kg"
            '     Qry = "select isnull(TSPL_ITEM_MASTER.Alies_Name,'') as [Item],Sum(TempData.Quantity) as [Quantity In Kg],sum(TempData.[Total FAT KG]) AS [FAT KG] " & _
            '",sum(TempData.[Total SNF KG]) AS [SNF KG],sum(TempData.[Total Sale Amount]) AS [Sale Amount],convert(decimal(18,2),sum(TempData.[Total Sale Amount])/Sum(TempData.Quantity)) AS [Ave Realisa Per Kg] " & _
            '" from( "
            '     Qry += clsPSInvoiceHead.GetReportDataQuery(obj)
            '     Qry += ")TempData " & _
            '     " left join tspl_item_master on tspl_item_master.Item_Code=TempData.[Item Code] " & _
 ...
 " from (  "
  ...
  " group by isnull(TSPL_ITEM_MASTER.Alies_Name,'') order by isnull(TSPL_ITEM_MASTER.Alies_Name,'')"
            ...
            'dtAll.Columns("Quantity In Kg").SetOrdinal(2)
            'dtAll.Columns("FAT KG").SetOrdinal(3)
            'dtAll.Columns("SNF KG").SetOrdinal(4)
            'dtAll.Columns("Sale Amount").SetOrdinal(5)
            'dtAll.Columns("Ave Realisa Per Ltr").SetOrdinal(6)
            'dtAll.Columns("Ave Realisa Per Kg").SetOrdinal(7)
            ...
            If dtAll IsNot Nothing OrElse dtAll.Rows.Count > 0 Then
                ...
                'gv_MilkSale.MasterTemplate.SummaryRowsBottom.Add(summaryRowItem)
    ...
    Private Sub btnGo_ProductSale_Click(sender As Object, e As EventArgs) Handles btnGo_ProductSale.Click
        Try
            ...
            'Dim EndDate As String = ""
            Dim Qry As String = ""
            'StartDate = "01/" + clsCommon.myCstr(ddlMonthProductSale.SelectedValue) + "/" + clsCommon.myCstr(Year(dtpYearProductSale.Value))
            'EndDate = clsCommon.myCstr(ddlMonthProductSale.SelectedValue) + "/01/" + clsCommon.myCstr(Year(dtpYearProductSale.Value))
            Dim obj As New clsSaleRegisterParameterType
            'obj.ReportType = "Item Wise"
            obj.Unit_Code = "Ltr"
            obj.From_Date = clsCommon.myCDate(dtpFromDate_ProductSale.Value)
            'EndDate = clsDBFuncationality.getSingleValue("select EOMONTH('" + EndDate + "')")
            obj.To_Date = clsCommon.myCDate(dtpToDate_ProductSale.Value)
            obj.Other_Cond = " and xx.Status=1  "
            ...
            '     Qry += ")TempData " & _
            '     " left join tspl_item_master on tspl_item_master.Item_Code=TempData.[Item Code] " & _
            '     " where isnull(TSPL_ITEM_MASTER.Is_Ambient,0)=1 " & _
            '     " group by isnull(TSPL_ITEM_MASTER.Alies_Name,'') order by isnull(TSPL_ITEM_MASTER.Alies_Name,'')"
 ...
 " from (  "
  ...
  " group by isnull(TSPL_ITEM_MASTER.Alies_Name,'') order by isnull(TSPL_ITEM_MASTER.Alies_Name,'')"
            ...
            If dtAll IsNot Nothing OrElse dtAll.Rows.Count > 0 Then
                ...
                'gv_ProductSale.MasterTemplate.SummaryRowsBottom.Add(summaryRowItem)
    ...
    Private Sub btn_Go_Transport_cost_Click(sender As Object, e As EventArgs) Handles btn_Go_Transport_cost.Click
        Try
            ...
            'Dim StrVehicleQry As String = clsDBFuncationality.getSingleValue("DECLARE @colsScheme AS NVARCHAR(MAX),@query  AS NVARCHAR(MAX) SELECT   STUFF((SELECT distinct ',' + QUOTENAME(" & _
            '    " ISNULL(TBL_ZONE.Zone_Code,'') + '-' + ISNULL(TSPL_ROUTE_MASTER.Route_No,'')+ '-' +ISNULL(TSPL_VEHICLE_MASTER.Vehicle_Brand,'')) as Alies_Name" & _
            '    " from TSPL_PROVISION_ENTRY   inner join TSPL_DAIRYSALE_GATEPASS_MASTER on TSPL_PROVISION_ENTRY.Ref_Doc_No = TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode  left Outer join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_id = TSPL_DAIRYSALE_GATEPASS_MASTER.Vehicle_Id   left Outer Join TSPL_ROUTE_MASTER on  TSPL_ROUTE_MASTER.Route_No = TSPL_PROVISION_ENTRY.Route_Code " & _
            '    "  left outer  join  ( select TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ,sum (convert(decimal(18,2),(TSPL_DAIRYSALE_GATEPASS_Detail.qty/LtrUnit.conversion_factor)*StockUnit.conversion_factor*CurrentUnit.conversion_factor)) as Ltr_Qty from TSPL_DAIRYSALE_GATEPASS_MASTER  left join TSPL_DAIRYSALE_GATEPASS_Detail on TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode=TSPL_DAIRYSALE_GATEPASS_Detail.GPCode  left join tspl_item_uom_detail LtrUnit on LtrUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code  " & _
            '    " left join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.unit_code=LtrUnit.uom_code  left join tspl_item_uom_detail StockUnit on StockUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code  left join tspl_item_uom_detail CurrentUnit on CurrentUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code and 	CurrentUnit.uom_code=	TSPL_DAIRYSALE_GATEPASS_Detail.unit_code  where  tspl_unit_master.Ltr_type ='Y' and StockUnit.stocking_unit='Y' group by TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ) TBL_LTR_CONV " & _
            '    " on TBL_LTR_CONV.GPCode =TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode left join (select distinct Zone_Code,Route_No from tspl_customer_master) TBL_ZONE on TBL_ZONE.Route_No=TSPL_PROVISION_ENTRY.Route_Code " & _
            '    "  where TSPL_PROVISION_ENTRY.isPosted=1 and 2=2  " & _
            '    " AND  TSPL_PROVISION_ENTRY.prov_month ='" & ddlTransportMonth.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpTransportYear.Value) & "'" & _
            '    " FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')")

            'QryTemp = "DECLARE @colsScheme AS NVARCHAR(MAX),@query  AS NVARCHAR(MAX) SELECT STUFF((SELECT distinct ',' + 'max(isnull(' + QUOTENAME( ISNULL(TBL_ZONE.Zone_Code,'') + '-' + ISNULL(TSPL_ROUTE_MASTER.Route_No,'')+ '-' +ISNULL(TSPL_VEHICLE_MASTER.Vehicle_Brand,''))" & _
            '    " +',''''))' + ' as ' + QUOTENAME(ISNULL(TBL_ZONE.Zone_Code,'') + '-' + ISNULL(TSPL_ROUTE_MASTER.Route_No,'')+ '-' +ISNULL(TSPL_VEHICLE_MASTER.Vehicle_Brand,'')) as Alies_Name " & _
            '    " from TSPL_PROVISION_ENTRY   inner join TSPL_DAIRYSALE_GATEPASS_MASTER on TSPL_PROVISION_ENTRY.Ref_Doc_No = TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode  left Outer join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_id = TSPL_DAIRYSALE_GATEPASS_MASTER.Vehicle_Id   left Outer Join TSPL_ROUTE_MASTER on  TSPL_ROUTE_MASTER.Route_No = TSPL_PROVISION_ENTRY.Route_Code " & _
            '    "  left outer  join  ( select TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ,sum (convert(decimal(18,2),(TSPL_DAIRYSALE_GATEPASS_Detail.qty/LtrUnit.conversion_factor)*StockUnit.conversion_factor*CurrentUnit.conversion_factor)) as Ltr_Qty from TSPL_DAIRYSALE_GATEPASS_MASTER  left join TSPL_DAIRYSALE_GATEPASS_Detail on TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode=TSPL_DAIRYSALE_GATEPASS_Detail.GPCode  left join tspl_item_uom_detail LtrUnit on LtrUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code  " & _
            '    " left join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.unit_code=LtrUnit.uom_code  left join tspl_item_uom_detail StockUnit on StockUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code  left join tspl_item_uom_detail CurrentUnit on CurrentUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code and 	CurrentUnit.uom_code=	TSPL_DAIRYSALE_GATEPASS_Detail.unit_code  where  tspl_unit_master.Ltr_type ='Y' and StockUnit.stocking_unit='Y' group by TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ) TBL_LTR_CONV " & _
            '    " on TBL_LTR_CONV.GPCode =TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode left join (select distinct Zone_Code,Route_No from tspl_customer_master) TBL_ZONE on TBL_ZONE.Route_No=TSPL_PROVISION_ENTRY.Route_Code " & _
            ...
            '    " AND  TSPL_PROVISION_ENTRY.prov_month ='" & ddlTransportMonth.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpTransportYear.Value) & "'" & _
            '    " FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')"
            'Dim StrVehicleQryMax As String = clsDBFuncationality.getSingleValue(QryTemp)

            ''Sum
            ''Dim strGrandTotal As String = clsDBFuncationality.getSingleValue(" DECLARE @colsScheme AS NVARCHAR(MAX),@query  AS NVARCHAR(MAX) SELECT   STUFF((SELECT distinct '+' +'Sum(isnull(' + QUOTENAME( TSPL_ITEM_MASTER.Alies_Name) +',0))' +' + ' + +'Sum(isnull(' + QUOTENAME( TSPL_ITEM_MASTER.Alies_Name+'(S)') +',0))'  as Alies_Name  FROM " + ItemInUse + "   FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'') ")
            'QryTemp = "DECLARE @colsScheme AS NVARCHAR(MAX),@query  AS NVARCHAR(MAX) SELECT STUFF((SELECT distinct '+' + 'max(isnull(' + QUOTENAME( ISNULL(TBL_ZONE.Zone_Code,'') + " & _
            '    " '-' + ISNULL(TSPL_ROUTE_MASTER.Route_No,'')+ '-' +ISNULL(TSPL_VEHICLE_MASTER.Vehicle_Brand,'')) +',''''))' as Alies_Name " & _
            '    " from TSPL_PROVISION_ENTRY   inner join TSPL_DAIRYSALE_GATEPASS_MASTER on TSPL_PROVISION_ENTRY.Ref_Doc_No = TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode  left Outer join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_id = TSPL_DAIRYSALE_GATEPASS_MASTER.Vehicle_Id   left Outer Join TSPL_ROUTE_MASTER on  TSPL_ROUTE_MASTER.Route_No = TSPL_PROVISION_ENTRY.Route_Code " & _
            '    "  left outer  join  ( select TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ,sum (convert(decimal(18,2),(TSPL_DAIRYSALE_GATEPASS_Detail.qty/LtrUnit.conversion_factor)*StockUnit.conversion_factor*CurrentUnit.conversion_factor)) as Ltr_Qty from TSPL_DAIRYSALE_GATEPASS_MASTER  left join TSPL_DAIRYSALE_GATEPASS_Detail on TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode=TSPL_DAIRYSALE_GATEPASS_Detail.GPCode  left join tspl_item_uom_detail LtrUnit on LtrUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code  " & _
            '    " left join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.unit_code=LtrUnit.uom_code  left join tspl_item_uom_detail StockUnit on StockUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code  left join tspl_item_uom_detail CurrentUnit on CurrentUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code and 	CurrentUnit.uom_code=	TSPL_DAIRYSALE_GATEPASS_Detail.unit_code  where  tspl_unit_master.Ltr_type ='Y' and StockUnit.stocking_unit='Y' group by TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ) TBL_LTR_CONV " & _
            '    " on TBL_LTR_CONV.GPCode =TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode left join (select distinct Zone_Code,Route_No from tspl_customer_master) TBL_ZONE on TBL_ZONE.Route_No=TSPL_PROVISION_ENTRY.Route_Code " & _
            ...
            '    " AND  TSPL_PROVISION_ENTRY.prov_month ='" & ddlTransportMonth.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpTransportYear.Value) & "'" & _
            '    " FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')"
            'Dim StrVehicleQrysum As String = clsDBFuncationality.getSingleValue(QryTemp)
            ''Sum

            'Dim DataQuery As String = "select sum(XX.[Running KM]) as [No of KM Running Per Month] ,(case when sum(XX.[Running KM])>0 then round(sum(XX.[Freight Amount])/sum(XX.[Running KM]),2) else 0 end) as [5. Rate Per KM] ,MAX(XX.[Vehicle Capacity])  as [1. Vehicle Capacity]  ,round(sum(XX.[Vehicle Out Crate])/count(DISTINCT XX.[Gate Pass No]),2) as [2. Vehicle Capacity Utilized]  ,round(sum(XX.[Sales Value]),2) as [4. Sales Value] ,round(sum(XX.[Sales Milk/Ltr]),2) as [3. Sales In LTR] ,round(sum(XX.[Freight Amount])/sum(XX.[Sales Milk/Ltr]),2) as [CPL] " & _
            '    ",XX.Zone_Code + '-' + ISNULL(XX.Route_No,'')+ '-' +ISNULL(XX.Vehicle_Brand,'') as Description " & _
            '    " from ( " & _
            '    " select ISNULL(TBL_ZONE.Zone_Code,'') as Zone_Code,ISNULL(TSPL_VEHICLE_MASTER.Vehicle_Brand,'') as Vehicle_Brand ,ISNULL(TSPL_VEHICLE_MASTER.Vehicle_Id,'') as Vehicle_Id ,ISNULL(TSPL_ROUTE_MASTER.Route_No,'') AS Route_No ,TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode as [Gate Pass No]  ,(case when isnull (TSPL_DAIRYSALE_GATEPASS_MASTER.Closing_Km,0)>0 then isnull (TSPL_DAIRYSALE_GATEPASS_MASTER.Closing_Km,0) -isnull( Opening_Km,0)  else 0 end) as [Running KM] ,TSPL_PROVISION_ENTRY.Amount as [Freight Amount],isnull(TSPL_SD_SHIPMENT_HEAD.Amount_Less_Discount,0) as [Sales Value]  " & _
            '    " , TBL_LTR_CONV.Ltr_Qty as [Sales Milk/Ltr], TSPL_VEHICLE_MASTER.CrateCapacity as [Vehicle Capacity]  ,TSPL_DAIRYSALE_GATEPASS_MASTER.TotalCrate as [Vehicle Out Crate]   " & _
            '    " from TSPL_PROVISION_ENTRY    " & _
            '    " inner join TSPL_DAIRYSALE_GATEPASS_MASTER on TSPL_PROVISION_ENTRY.Ref_Doc_No = TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode left join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.GPCode=TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode left Outer join TSPL_VEHICLE_MASTER on TSPL_VEHICLE_MASTER.Vehicle_id = TSPL_DAIRYSALE_GATEPASS_MASTER.Vehicle_Id   left Outer Join TSPL_ROUTE_MASTER on  TSPL_ROUTE_MASTER.Route_No = TSPL_PROVISION_ENTRY.Route_Code  " & _
            '    " left join (select distinct Zone_Code,Route_No from tspl_customer_master) TBL_ZONE on TBL_ZONE.Route_No=TSPL_PROVISION_ENTRY.Route_Code " & _
            '    " left outer  join  ( select TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ,sum (convert(decimal(18,2),(TSPL_DAIRYSALE_GATEPASS_Detail.qty/LtrUnit.conversion_factor)*StockUnit.conversion_factor*CurrentUnit.conversion_factor)) as Ltr_Qty from TSPL_DAIRYSALE_GATEPASS_MASTER  left join TSPL_DAIRYSALE_GATEPASS_Detail on TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode=TSPL_DAIRYSALE_GATEPASS_Detail.GPCode  left join tspl_item_uom_detail LtrUnit on LtrUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code   " & _
            '    " left join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.unit_code=LtrUnit.uom_code  left join tspl_item_uom_detail StockUnit on StockUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code   " & _
            '    " left join tspl_item_uom_detail CurrentUnit on CurrentUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code and 	CurrentUnit.uom_code=	TSPL_DAIRYSALE_GATEPASS_Detail.unit_code  where  tspl_unit_master.Ltr_type ='Y' and StockUnit.stocking_unit='Y'  group by TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ) TBL_LTR_CONV on  TBL_LTR_CONV.GPCode = TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode  where TSPL_PROVISION_ENTRY.isPosted=1 and 2=2    " & _
            '    " AND  TSPL_PROVISION_ENTRY.prov_month ='" & ddlTransportMonth.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpTransportYear.Value) & "'" & _
            '    ")XX group by  " & _
            '    " XX.Zone_Code + '-' + ISNULL(XX.Route_No,'')+ '-' +ISNULL(XX.Vehicle_Brand,'')"

            ' Qry = "select COL," & StrVehicleQryMax & ", (" + StrVehicleQrysum + ") as [Total] from ( " & _
            '" select Description,COL,VAL from (" & DataQuery & _
            '" )tt CROSS APPLY (VALUES " & _
            '" ('1. Vehicle Capacity',[1. Vehicle Capacity])," & _
            '" ('2. Vehicle Capacity Utilized',[2. Vehicle Capacity Utilized])," & _
            '" ('3. Sales In LTR',[3. Sales In LTR])," & _
            '" ('4. Sales Value',[4. Sales Value])," & _
            '" ('5. Rate Per KM',[5. Rate Per KM])" & _
            '" )CS (COL,VAL))T" & _
            '" PIVOT (max(VAL) FOR Description IN (" & StrVehicleQry & "))PVT" & _
            '" group by COL ORDER BY COL "
            '" left join (select distinct Zone_Code,Route_No from tspl_customer_master) TBL_ZONE on TBL_ZONE.Route_No=TSPL_PROVISION_ENTRY.Route_Code " &
            'left join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_HEAD.GPCode=TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode
            Qry = "select isnull(XX.Zone_Code,'') + '-' + ISNULL(XX.Route_No,'')+ '-' +ISNULL(XX.Vehicle_Brand,'') as [Zone-Route-Vehicle] ,MAX(XX.[Vehicle Capacity])  as [Vehicle Capacity]  ,round(sum(XX.[Vehicle Out Crate])/count(DISTINCT XX.[Gate Pass No]),2) as [Vehicle Capacity Utilized] ,round(sum(XX.[Sales Milk/Ltr]),2) as [Sales In LTR] ,round(sum(XX.[Sales Value]),2) as [Sales Value]   " &
               ...
               " left join tspl_item_uom_detail CurrentUnit on CurrentUnit.item_code=TSPL_DAIRYSALE_GATEPASS_Detail.item_code and 	CurrentUnit.uom_code=	TSPL_DAIRYSALE_GATEPASS_Detail.unit_code  where  tspl_unit_master.Ltr_type ='Y' and StockUnit.stocking_unit='Y'  group by TSPL_DAIRYSALE_GATEPASS_Detail.GPCode ) TBL_LTR_CONV on  TBL_LTR_CONV.GPCode = TSPL_DAIRYSALE_GATEPASS_MASTER.GPCode  where TSPL_PROVISION_ENTRY.isPosted=1 and 2=2    "
            Qry += " and convert(date, TSPL_PROVISION_ENTRY.Doc_Date,103) >=  convert(date,'" + dtpFromDate_Transport.Value + "',103)  and  convert(date, TSPL_PROVISION_ENTRY.Doc_Date,103) <= convert(date,'" + dtpToDate_Transport.Value + "',103) "
            '" AND  TSPL_PROVISION_ENTRY.prov_month ='" & ddlTransportMonth.SelectedValue & "' and TSPL_PROVISION_ENTRY.prov_year ='" & Year(dtpTransportYear.Value) & "'" & _
            Qry += ")XX group by  " &
            ...
            '    If clsCommon.CompairString(clsCommon.myCstr(dt.Rows(j).Item("COL")), "1. Vehicle Capacity") = CompairStringResult.Equal Then
            '        dt.Rows(j).Item("Total") = DBNull.Value
            '    ElseIf clsCommon.CompairString(clsCommon.myCstr(dt.Rows(j).Item("COL")), "2. Vehicle Capacity Utilized") = CompairStringResult.Equal Then
            '        dt.Rows(j).Item("Total") = DBNull.Value
            '    ElseIf clsCommon.CompairString(clsCommon.myCstr(dt.Rows(j).Item("COL")), "5. Rate Per KM") = CompairStringResult.Equal Then
            '        dt.Rows(j).Item("Total") = DBNull.Value
            '    End If
            'Next
    ...
    Private Sub btn_StoreReport_Click(sender As Object, e As EventArgs) Handles btn_StoreReport.Click
        Try
            PageSetupReport_ID = clsUserMgtCode.frmDasboard + "StoreReport"
            Dim Qry As String = ""
            Qry = "select ROW_NUMBER () over (order by ass1.Structure_Desc ) As SNo,ass1.Structure_Desc As [Item Group] " & _
                ",COUNT(DISTINCT ass1.purchase_no) as [No of PO] " & _
                ",COUNT(DISTINCT ass1.GRN_NO) as [No of GRN] " & _
                ",COUNT(DISTINCT ass1.SRN_NO) as [No of SRN] " & _
                ",Cast(sum(value) as decimal(18,2)) as Value " & _
                ",COUNT(DISTINCT CASE WHEN POStatus='Open' then ass1.purchase_no else null end) as [Pending PO] " & _
            " from  ( SELECT  PO.PurchaseOrder_No AS purchase_no,PO.PurchaseOrder_Date AS po_date ," & _
             " CASE WHEN (PO.close_yn) = 'Y' THEN 'Close' WHEN (PO.status) = 1 THEN 'Close' when QC.Document_Code is not null and TSPL_SRN_DETAIL.SRN_No is null  and ISNULL( QC.Ok_Qty,0)=0 and isnull(QC.Reject_Qty,0)>0 then 'Open'  WHEN ((ISNULL(PO.PurchaseOrder_Qty, 0) + ISNULL(Tolerence_Qty, 0)) - ISNULL(TSPL_GRN_DETAIL.GRN_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Short_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Leak_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Burst_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Rejected_Qty, 0)) > 0 THEN 'Open'  ELSE 'Complete' END AS POStatus " & _
              " ,  TSPL_SRN_DETAIL.SRN_NO AS SRN_NO " & _
              " ,ISNULL(TSPL_GRN_DETAIL.GRN_NO, '') AS GRN_NO,CONVERT(varchar, TSPL_GRN_HEAD.GRN_Date, 103) AS GRN_Date,PO.Against_Requisition, TSPL_REQUISITION_HEAD.Requisition_Date,ISNULL(TSPL_GRN_DETAIL.GRN_Qty, 0) AS GRNQty,ISNULL(TSPL_GRN_DETAIL.Tolerence_Qty, 0) AS Tolerence_Qty,ISNULL(TSPL_MRN_DETAIL.MRN_Qty, 0) AS MRNQty, ISNULL(TSPL_SRN_DETAIL.SRN_Qty, 0) AS SRN_Qty,ISNULL(TSPL_SRN_DETAIL.Short_Qty, 0) AS Short_Qty,ISNULL(TSPL_SRN_DETAIL.Leak_Qty, 0) AS Leak_Qty,ISNULL(TSPL_SRN_DETAIL.Burst_Qty, 0) AS Burst_Qty, ISNULL(TSPL_SRN_DETAIL.Rejected_Qty, 0) AS Rejected_Qty " & _
            " , CASE WHEN COALESCE((ISNULL(ISNULL(TSPL_SRN_DETAIL.SRN_Qty, 0), 0.0)), 0) <= 0 THEN COALESCE((SELECT (ISNULL(TSPL_MRN_DETAIL.MRN_Qty, 0))), (ISNULL((ISNULL(TSPL_GRN_DETAIL.GRN_Qty, 0)), 0.0))) END AS QCQty,TSPL_SRN_DETAIL.Balance_Qty,TSPL_ITEM_MASTER.Structure_Desc,PO.value" & _
            " ,CASE WHEN (PO.close_yn) = 'Y' THEN 'Close' WHEN (PO.status) = 1 THEN 'Close' when QC.Document_Code is not null and TSPL_SRN_DETAIL.SRN_No is null  and ISNULL( QC.Ok_Qty,0)=0 and isnull(QC.Reject_Qty,0)>0 then 'Open' " & _
            " WHEN ((ISNULL(PO.PurchaseOrder_Qty, 0) + ISNULL(Tolerence_Qty, 0)) - ISNULL(TSPL_GRN_DETAIL.GRN_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Short_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Leak_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Burst_Qty, 0) + ISNULL(TSPL_SRN_DETAIL.Rejected_Qty, 0)) > 0 THEN 'Open'  ELSE 'Complete' END AS PPStatus FROM( " & _
            " select ISNULL(TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_No, '') AS PurchaseOrder_No ,tspl_purchase_order_detail.Item_Code,MAX(TSPL_PURCHASE_ORDER_HEAD.comp_code) AS comp_code,MAX(TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_Date) AS PurchaseOrder_Date ,MAX(TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_Type) AS PurchaseOrder_Type,MAX(TSPL_PURCHASE_ORDER_HEAD.Closed_By) AS Closed_By ,MAX(TSPL_PURCHASE_ORDER_HEAD.Closed_Date) AS Closed_Date,MAX(TSPL_PURCHASE_ORDER_HEAD.Vendor_Code) AS Vendor_Code ,MAX(TSPL_PURCHASE_ORDER_HEAD.Vendor_Name) AS Vendor_Name,SUM(TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty) AS PurchaseOrder_Qty ,MAX(TSPL_PURCHASE_ORDER_HEAD.close_yn) AS close_yn,MAX(TSPL_PURCHASE_ORDER_DETAIL.status) AS status " & _
             " ,MAX(tspl_purchase_order_head.Against_Requisition) AS Against_Requisition ,MAX(TSPL_PURCHASE_ORDER_DETAIL.Item_Cost)*SUM(TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_Qty) AS Value" & _
              " from tspl_purchase_order_detail LEFT OUTER JOIN TSPL_PURCHASE_ORDER_HEAD ON TSPL_PURCHASE_ORDER_HEAD.PurchaseOrder_No = TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_No GROUP BY ISNULL(TSPL_PURCHASE_ORDER_DETAIL.PurchaseOrder_No, ''),tspl_purchase_order_detail.Item_Code)PO  LEFT JOIN TSPL_GRN_DETAIL ON ISNULL(TSPL_GRN_DETAIL.PO_Id, '') = ISNULL(PO.PurchaseOrder_No, '') AND TSPL_GRN_DETAIL.Item_Code = PO.Item_Code LEFT JOIN TSPL_GRN_HEAD ON ISNULL(TSPL_GRN_HEAD.grn_no, '') = ISNULL(TSPL_GRN_DETAIL.grn_no, '') LEFT JOIN TSPL_MRN_DETAIL ON TSPL_MRN_DETAIL.Item_Code = PO.Item_Code AND ISNULL(TSPL_MRN_DETAIL.GRN_Id, '') = ISNULL(TSPL_GRN_DETAIL.grn_no, '') AND ISNULL(TSPL_GRN_DETAIL.PO_Id, '') = ISNULL(PO.PurchaseOrder_No, '') AND TSPL_GRN_DETAIL.Item_Code = PO.Item_Code left outer join (select TSPL_QC_CHECK_DETAIL.*  from TSPL_QC_CHECK_DETAIL left outer join  TSPL_QC_CHECK_HEAD on TSPL_QC_CHECK_HEAD.Document_Code = TSPL_QC_CHECK_DETAIL.Document_Code where TSPL_QC_CHECK_HEAD.Posted=1) as  QC on isnull( QC.MRN_No,'')=ISNULL( TSPL_MRN_DETAIL.MRN_No,'') and QC.Item_Code = TSPL_MRN_DETAIL.Item_Code  and ISNULL(QC.PO_No, '')=ISNULL(TSPL_MRN_DETAIL.PO_ID, '')  LEFT JOIN TSPL_SRN_DETAIL ON TSPL_SRN_DETAIL.Item_Code = PO.Item_Code AND ISNULL(PO.PurchaseOrder_No, '') = ISNULL(TSPL_SRN_DETAIL.PO_ID, '') AND ISNULL(TSPL_MRN_DETAIL.mrn_no, '') = ISNULL(TSPL_SRN_DETAIL.mrn_id, '') AND ISNULL(TSPL_GRN_DETAIL.grn_no, '') = ISNULL(TSPL_MRN_DETAIL.grn_id, '') LEFT JOIN TSPL_SRN_HEAD ON TSPL_SRN_HEAD.SRN_No = TSPL_SRN_DETAIL.SRN_no" & _
            ...
            '" AND  MONTH(cast(PO.PurchaseOrder_Date as date)) =" & ddlStoreMonth.SelectedValue & " and YEAR(cast(PO.PurchaseOrder_Date as Date)) =" & Year(dtpStoreYear.Value) & " " & _
            Qry += " and convert(date, PO.PurchaseOrder_Date,103) >=  convert(date,'" + dtpFromDate_Store.Value + "',103)  and  convert(date, PO.PurchaseOrder_Date,103) <= convert(date,'" + dtpToDate_Store.Value + "',103) "
            Qry += " and PO.PurchaseOrder_Type <>  'J' " & _
           " )ass1 where 2=2 GROUP BY ass1.Structure_Desc "
            ...
            'Dim StartDate As String = ""
            'Dim EndDate As String = ""
            'StartDate = "01/" + clsCommon.myCstr(ddlStoreMonth.SelectedValue) + "/" + clsCommon.myCstr(Year(dtpStoreYear.Value))
            'EndDate = clsCommon.myCstr(ddlStoreMonth.SelectedValue) + "/01/" + clsCommon.myCstr(Year(dtpStoreYear.Value))
            'EndDate = clsDBFuncationality.getSingleValue("select EOMONTH('" + EndDate + "')")
            Dim StartDate As Date = CDate(dtpFromDate_Store.Value)
            Dim EndDate As Date = CDate(dtpToDate_Store.Value)

            Qry = "select * from ( select TSPL_ITEM_MASTER.Item_Type,TSPL_ITEM_MASTER.Product_Type,InventroyMovement.Trans_Type,InventroyMovement.Punching_Date, InventroyMovement.InOut,TSPL_ITEM_MASTER.Structure_Code,TSPL_STRUCTURE_MASTER.Structure_Descq, (case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=3 then InventroyMovement.FIFO_Cost else case when TSPL_PURCHASE_ACCOUNTS.Costing_Method=2 then InventroyMovement.LIFO_Cost else InventroyMovement.Avg_Cost end end ) as Cost,TSPL_ITEM_MASTER.Item_Category_Struct_Code" & _
                ",TSPL_INVENTORY_SOURCE_CODE.In_Category,TSPL_INVENTORY_SOURCE_CODE.Out_Category,TSPL_INVENTORY_SOURCE_CODE.Code,(case when ISNULL(InventroyMovement.Location_Code,'')='' then InventroyMovement.Location_Code else TSPL_LOCATION_MASTER.Main_Location_Code end) as PrimaryLocation ,InventroyMovement.IS_CONSUMPTION" & _
                " from (  select Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM, MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,IS_CONSUMPTION from TSPL_INVENTORY_MOVEMENT" & _
                " union all " & _
                " select Trans_Id,Trans_Type,Source_Doc_No,Punching_Date,InOut,Location_Code,Item_Code,UOM,MRP,Stock_UOM,Stock_Qty,FIFO_Cost,LIFO_Cost,Avg_Cost,IS_CONSUMPTION from TSPL_INVENTORY_MOVEMENT_NEW ) InventroyMovement " & _
    ...
    Private Sub VUExcel_Click(sender As Object, e As EventArgs) Handles VUExcel.Click
        Try
            If gv4.Rows.Count > 0 Then
                Dim arrHeader As List(Of String) = New List(Of String)()
                arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
    ...
    Private Sub VUCSV_Click(sender As Object, e As EventArgs) Handles VUCSV.Click
        Try
            Dim sfd As SaveFileDialog = New SaveFileDialog()
            Dim filePath As String
            sfd.FileName = "Vehicle Utilization"
    ...
    Private Sub VUPDF_Click(sender As Object, e As EventArgs) Handles VUPDF.Click
        Try
            If gv4.Rows.Count > 0 Then
                Dim arrHeader As List(Of String) = New List(Of String)()
                arrHeader.Add("Company : " & objCommonVar.CurrentCompanyName)
