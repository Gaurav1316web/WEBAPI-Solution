    ...
    Public Sub LoadData(Optional ByVal strDocumentNoForDetails As String = "", Optional ByVal strVSPCodeForDetails As String = "")
        Try
            ...
            If rdbSummary.IsChecked = True AndAlso clsCommon.myLen(strVSPCodeForDetails) <= 0 AndAlso clsCommon.myLen(strDocumentNoForDetails) <= 0 Then
                qry = " select max([MCC Code]) as [MCC Code], max(case when len (TSPL_GL_SEGMENT_CODE.Description) > 0 then TSPL_GL_SEGMENT_CODE.Description else  TSPL_LOCATION_MASTER.Location_Desc end) as [MCC Name],max([VLC CODE Uploader]) as [VLC CODE Uploader], [VSP Code] as [Secretary Code],max([VSP Name]) as [Secretary Name],max([From Date]) as [From Date] ,max([To Date]) as [To Date] ,max(Doc_Date) as [Document Date],'Payment Process' as [Doc Type],Doc_No as [Document No],max(Status) as [Doc Status] ,sum ( XXXFinal.dr) as Dr, sum (XXXFinal.cr ) as Cr , sum (XXXFinal.cr ) - sum ( XXXFinal.dr) as [Payable Amt] from ( 
                        " + qry + " 
                        ) XXXFinal 
