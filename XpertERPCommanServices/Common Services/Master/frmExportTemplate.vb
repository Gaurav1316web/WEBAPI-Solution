...
#End Region
    ...
    Sub addColumn()
        Try
            If gvColumnsMain.SelectedRows.Count = 0 Then
                Exit Sub
            End If
            '' check already exist
            For Each grow As GridViewRowInfo In gvColumnTemp.Rows
                If clsCommon.CompairString(grow.Cells(colColumnName).Value, Me.gvColumnsMain.SelectedRows(0).Cells(colColumnName).Value) = CompairStringResult.Equal Then
                    clsCommon.MyMessageBoxShow("Column: " & Me.gvColumnsMain.SelectedRows(0).Cells(colColumnName).Value & " already added")
                    Exit Sub
                End If
            Next
            ...
            Me.gvColumnTemp.Rows(Me.gvColumnTemp.Rows.Count - 1).Cells(colColumnName).Value = Me.gvColumnsMain.SelectedRows(0).Cells(colColumnName).Value
            Me.gvColumnTemp.Rows(Me.gvColumnTemp.Rows.Count - 1).Cells(colColumnHeader).Value = Me.gvColumnsMain.SelectedRows(0).Cells(colColumnHeader).Value
            'gvColumnsMain.Rows.RemoveAt(Me.gvColumnsMain.SelectedRows(0).Index)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(ex.Message)
        End Try
    End Sub
    ...
    Private Sub btnBack_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBack.Click
        If gvColumnTemp.SelectedRows.Count = 0 Then
            Exit Sub
        End If
        Me.gvColumnsMain.Rows.AddNew()
        Me.gvColumnsMain.Rows(Me.gvColumnsMain.Rows.Count - 1).Cells(colColumnName).Value = Me.gvColumnTemp.SelectedRows(0).Cells(colColumnName).Value
        Me.gvColumnsMain.Rows(Me.gvColumnsMain.Rows.Count - 1).Cells(colColumnHeader).Value = Me.gvColumnTemp.SelectedRows(0).Cells(colColumnHeader).Value        
        gvColumnTemp.Rows.RemoveAt(Me.gvColumnTemp.SelectedRows(0).Index)
        RefershLineNo()
    End Sub
