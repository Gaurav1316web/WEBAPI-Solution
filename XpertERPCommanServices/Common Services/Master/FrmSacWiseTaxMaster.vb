...
Public Class FrmSacWiseTaxMaster
    Inherits FrmMainTranScreen
#Region "Variables"
    Const colLineNo As String = "colLineNo"
    Const colSAC_Code As String = "colItem_Code"
    Const colSAC_Desc As String = "colItem_Desc"
...
#End Region
    Private Sub FrmSacWiseTaxMaster_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Reset()
        gv1.Rows.AddNew()
    ...
    Private Sub SetUserMgmtNew()
        ...
        '  RadMenuItem1.Visibility = MyBase.isExport
        If MyBase.isReverse Then
    ...
    Sub LoadBlankGrid1()
        ...
        repoItemCode.HeaderText = "SAC Code"
        repoItemCode.Name = colSAC_Code
        ...
        repoItemDesc.HeaderText = "SAC Description"
        repoItemDesc.Name = colSAC_Desc
    ...
    Sub ClearAllCurrentRowFinder()
        gv1.CurrentRow.Cells(colSAC_Code).Value = ""
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        Try
            ...
            gv1.CurrentRow.Cells(colSAC_Code).Value = clsCommon.ShowSelectForm("ItemFnder@SAC_Code", qry, "Code", "", isButtonClick)
            If clsCommon.myLen(gv1.CurrentRow.Cells(colSAC_Code).Value) > 0 Then
                ADDNewRows()
                gv1.CurrentRow.Cells(colSAC_Desc).Value = ClsSACMaster.GetName(clsCommon.myCstr(gv1.CurrentRow.Cells(colSAC_Code).Value))
            End If
        Catch ex As Exception
            gv1.CurrentRow.Cells(colSAC_Code).Value = ""
            gv1.CurrentRow.Cells(colSAC_Desc).Value = ""
    ...
    Sub OpenTaxGroupList(ByVal isButtonClick As Boolean)
        Try
            ...
            gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value = clsCommon.ShowSelectForm("ItemFnder@GroupCode", qry, "Code", "", clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value), "", isButtonClick)
            ...
            qry = "select Tax_Code from TSPL_TAX_GROUP_DETAILS where Tax_Group_Code='" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value) + "'"
    ...
    Sub OpenTax1CodeList(ByVal isButtonClick As Boolean)
        Try
            ...
            gv1.CurrentRow.Cells(colTAX1_Code).Value = clsCommon.ShowSelectForm("ItemFnder@TaxCode", qry, "Code", " Tax_Group_Code= '" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value) + "'", clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX1_Code).Value), "", isButtonClick)
        Catch ex As Exception
            gv1.CurrentRow.Cells(colTAX1_Code).Value = ""
    ...
    Sub OpenTax2CodeList(ByVal isButtonClick As Boolean)
        Try
            ...
            End If
            Dim qry As String = "select Tax_Code as Code,Tax_Code_Desc as Descriiption from TSPL_TAX_GROUP_DETAILS"
            gv1.CurrentRow.Cells(colTAX2_Code).Value = clsCommon.ShowSelectForm("ItemFnder@TaxCode", qry, "Code", "Tax_Group_Code= '" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value) + "'", clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX2_Code).Value), "", isButtonClick)
        Catch ex As Exception
            gv1.CurrentRow.Cells(colTAX2_Code).Value = ""
    ...
    Sub OpenTax3CodeList(ByVal isButtonClick As Boolean)
        Try
            ...
            End If
            Dim qry As String = "select Tax_Code as Code,Tax_Code_Desc as Descriiption from TSPL_TAX_GROUP_DETAILS"
            gv1.CurrentRow.Cells(colTAX3_Code).Value = clsCommon.ShowSelectForm("ItemFnder@TaxCode", qry, "Code", "Tax_Group_Code= '" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value) + "'", clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX3_Code).Value), "", isButtonClick)
        Catch ex As Exception
            gv1.CurrentRow.Cells(colTAX3_Code).Value = ""
    ...
    Sub OpenTax4CodeList(ByVal isButtonClick As Boolean)
        Try
            ...
            End If
            Dim qry As String = "select Tax_Code as Code,Tax_Code_Desc as Descriiption from TSPL_TAX_GROUP_DETAILS"
            gv1.CurrentRow.Cells(colTAX4_Code).Value = clsCommon.ShowSelectForm("ItemFnder@TaxCode", qry, "Code", "Tax_Group_Code= '" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value) + "' ", clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX4_Code).Value), "", isButtonClick)
        Catch ex As Exception
            gv1.CurrentRow.Cells(colTAX4_Code).Value = ""
    ...
    Private Sub fndCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndCode._MYValidating
        Dim qry As String = " select HCode as Code,CONVERT(varchar, DOC_DATE,103) as [Start Date],Case when  Status=0 then 'Pending' else 'Approved' end as Status from TSPL_SAC_WISE_TAX "
        ...
        Try
            ...
            If (Not isInsideLoadData) Then
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colSAC_Code) Then
                        OpenICodeList(False)
    ...
    Function AllowToSave() As Boolean
        Try
            If gv1.Rows.Count <= 0 Then
                Dim Msg As String = "Please fill SAC first... "
                common.clsCommon.MyMessageBoxShow(Me, Msg, Me.Text)
                Return False
            End If
            If clsCommon.myLen(gv1.Rows(0).Cells(colSAC_Code).Value) <= 0 Then
                Dim Msg As String = "Please fill SAC first ... "
                common.clsCommon.MyMessageBoxShow(Me, Msg, Me.Text)
                Return False
            End If
            For ii As Integer = 0 To gv1.Rows.Count - 1
                Dim strSACCode As String = clsCommon.myCstr(gv1.Rows(ii).Cells(colSAC_Code).Value)
                ...
                If clsCommon.myLen(strSACCode) <= 0 And clsCommon.myLen(strSno) > 0 Then
                    Dim Msg As String = "SAC Can not be blank at Row No " + clsCommon.myCstr(ii + 1)
                    common.clsCommon.MyMessageBoxShow(Me, Msg, Me.Text)
                    Return False
                End If
                If clsCommon.myLen(strSACCode) > 0 And clsCommon.myLen(strSno) > 0 Then
                    Dim strValue As String = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*) from TSPL_SAC_MASTER where Code='" + strSACCode + "'"))
                    If strValue <= 0 Then
                        Dim Msg As String = " Invalid SAC at Row No " + clsCommon.myCstr(ii + 1) + " "
                        common.clsCommon.MyMessageBoxShow(Me, Msg, Me.Text)
                        Return False
                    End If
                End If
                ...
                If clsCommon.myLen(strGroupcodeChk) > 0 Then
                    Dim strValue As String = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*)  from TSPL_TAX_GROUP_DETAILS where  Tax_Group_Code = '" + strGroupcodeChk + "'"))
                    If strValue <= 0 Then
                ...
                If isNewEntry = True Then
                    If clsCommon.myLen(strSACCode) > 0 Then
                        Dim strCountDupicate As Decimal = clsCommon.myCdbl(clsDBFuncationality.getSingleValue(" 
						 select count(*) from TSPL_SAC_WISE_TAX_GROUP  inner join  TSPL_SAC_WISE_TAX on TSPL_SAC_WISE_TAX_GROUP.HCODE =TSPL_SAC_WISE_TAX.HCODE where  convert(varchar, TSPL_SAC_WISE_TAX.DOC_DATE,103) = convert(varchar, '" + clsCommon.GetPrintDate(dtpToDate.Value, "dd/MM/yyyy") + "',103) and TSPL_SAC_WISE_TAX_GROUP.SAC_CODE = '" + strSACCode + "' and TSPL_SAC_WISE_TAX_GROUP.Tax_Group_Code = '" + strTaxGroupCode + "' "))
                        If strCountDupicate > 0 Then
                            Dim Msg As String = " SAC already Exist another Document .Duplicate Entry not Possible Same Day  at Row No " + clsCommon.myCstr(ii + 1) + " "
                            common.clsCommon.MyMessageBoxShow(Me, Msg, Me.Text)
                            Return False
                        End If
                    End If
                ...
                Else
                    If clsCommon.myLen(strSACCode) > 0 Then
                        Dim strDocCode As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select TSPL_SAC_WISE_TAX.HCODE from TSPL_SAC_WISE_TAX_GROUP  inner join  TSPL_SAC_WISE_TAX on TSPL_SAC_WISE_TAX_GROUP.HCODE =TSPL_SAC_WISE_TAX.HCODE where  convert(varchar, TSPL_SAC_WISE_TAX.DOC_DATE,103) = convert(varchar, '" + clsCommon.GetPrintDate(dtpToDate.Value, "dd/MM/yyyy") + "',103) and  TSPL_SAC_WISE_TAX_GROUP.SAC_CODE = '" + strSACCode + "' and TSPL_SAC_WISE_TAX_GROUP.Tax_Group_Code = '" + strTaxGroupCode + "' "))
                        If clsCommon.myLen(strDocCode) > 0 Then
                            If clsCommon.CompairString(strDocCode, fndCode.Value) <> CompairStringResult.Equal Then
                                Dim Msg As String = " SAC already Exist another Document : '" + strDocCode + "'.Duplicate Entry not Possible Same Day  at Row No " + clsCommon.myCstr(ii + 1) + " "
                                common.clsCommon.MyMessageBoxShow(Me, Msg, Me.Text)
                                Return False
                            End If
                        End If
            ...
            Next
            For ii As Integer = 0 To gv1.Rows.Count - 1
                Dim strICode As String = clsCommon.myCstr(gv1.Rows(ii).Cells(colSAC_Code).Value)
                Dim strITaxGroupCode As String = clsCommon.myCstr(gv1.Rows(ii).Cells(colTAX_GROUP_CODE).Value)
                For jj As Integer = 0 To gv1.Rows.Count - 1
                    If jj = ii Then
                        Continue For
                    End If
                    Dim strInnerICode As String = clsCommon.myCstr(gv1.Rows(jj).Cells(colSAC_Code).Value)
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If clsCommon.myLen(strTaxCodeChk1) > 0 Then
                    Dim strValue As String = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*)  from TSPL_TAX_GROUP_DETAILS where Tax_Group_Code = '" + strGroupcodeChk + "' and Tax_Code ='" + strTaxCodeChk1 + "'"))
                    If strValue <= 0 Then
                ...
                If clsCommon.myLen(strTaxCodeChk1) > 0 Then
                    Dim strValue As String = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(*)  from TSPL_TAX_RATES where Tax_Code = '" + strTaxCodeChk1 + "' and Tax_Rate = '" + strTaxRateChk1 + "' "))
                    If strValue <= 0 Then
    ...
    Function SaveData() As Boolean
        Try
            If AllowToSave() Then
                Dim obj As New ClsSACWiseTax()
                Dim objGroup As New clsSACWiseTaxGroup()
                If isNewEntry = True Then
                    obj.HCODE = clsERPFuncationality.GetNextCode(Nothing, clsCommon.GetPrintDate(clsCommon.GETSERVERDATE(), "dd/MMM/yyyy"), clsDocType.SACWiseTax, clsDocTransactionType.SACWiseTaxMaster, "")
                ...
                obj.ArrAuth = New List(Of clsSACWiseTaxAuthority)
                For ii As Integer = 0 To gv1.RowCount - 1
                    If clsCommon.myLen(gv1.Rows(ii).Cells(colSAC_Code).Value) > 0 Then
                        ...
                        objtr.SAC_Code = clsCommon.myCstr(gv1.Rows(ii).Cells(colSAC_Code).Value)
                        ...
                        For iii As Integer = 1 To 5
                            ...
                            Dim objAuth As New clsSACWiseTaxAuthority
        ...
        Return False
    End Function
    Sub LoadDetailData(ByVal Arr As List(Of clsSACWiseTaxGroup), ByVal isAddMasterCode As Boolean)
        If Arr IsNot Nothing AndAlso Arr.Count > 0 Then
            For Each objtr As clsSACWiseTaxGroup In Arr
                gv1.Rows.AddNew()
                gv1.CurrentRow = gv1.Rows(gv1.Rows.Count - 1)

                gv1.Rows(gv1.Rows.Count - 1).Cells(colLineNo).Value = objtr.SNO
                gv1.Rows(gv1.Rows.Count - 1).Cells(colSAC_Code).Value = objtr.SAC_Code
                gv1.Rows(gv1.Rows.Count - 1).Cells(colSAC_Desc).Value = ClsSACMaster.GetName(objtr.SAC_Code)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavType As NavigatorType)
        Try
            ...
            Reset()
            Dim obj As ClsSACWiseTax = ClsSACWiseTax.GetData(strCode, NavType, Nothing)
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                If SaveData() Then
                    If ClsSACWiseTax.PostData(fndCode.Value) Then
    ...
    Private Sub btnReverse_Click(sender As Object, e As EventArgs) Handles btnReverse.Click
        Try
            If common.clsCommon.MyMessageBoxShow("Reverse and Unpost the Current Document" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                If ClsSACWiseTax.ReverseAndUnpost(fndCode.Value) Then
    ...
    Private Sub RadMenuItem2_Click(sender As Object, e As EventArgs) Handles RadMenuItem2.Click
        Try
            ...
            Dim query As String = "  select    xxxx.sac_Code,max(TSPL_SAC_MASTER.Description) as SAC_Desc,max(xxxx.Tax_Group_Code) as Tax_Group_Code,max (TSPL_TAX_GROUP_MASTER.Tax_Group_Desc) as Tax_Group_Desc,max (xxxx.[1]) as Tax1_Code,max (xxxx.[1T]) as TAX1_Rate , max (xxxx.[2]) as Tax2_Code,max (xxxx.[2T]) as TAX2_Rate ,max (xxxx.[3]) as Tax3_Code,max (xxxx.[3T]) as TAX3_Rate ,max (xxxx.[4]) as Tax4_Code,max (xxxx.[4T]) as TAX4_Rate ,max (xxxx.[5]) as Tax5_Code,max (xxxx.[5T]) as TAX5_Rate from ( select * from ( select * from ( select TSPL_sac_WISE_TAX_AUTHORITY.HCODE, TSPL_SAC_WISE_TAX_GROUP.SNO as TAX_Group_SNo,  TSPL_SAC_WISE_TAX_GROUP.SAC_Code,TSPL_SAC_WISE_TAX_GROUP.Tax_Group_Code,TSPL_SAC_WISE_TAX_AUTHORITY.SNO,convert(varchar,TSPL_SAC_WISE_TAX_AUTHORITY.SNO)+'T' as SNO2, TSPL_SAC_WISE_TAX_AUTHORITY. DCODE,TSPL_sac_WISE_TAX_AUTHORITY.Tax_Authority,TSPL_sac_WISE_TAX_AUTHORITY.TAX_Rate  from TSPL_sac_WISE_TAX_AUTHORITY  inner join TSPL_SAC_WISE_TAX_GROUP on TSPL_sac_WISE_TAX_GROUP.DCODE=TSPL_sac_WISE_TAX_AUTHORITY.DCODE and TSPL_sac_WISE_TAX_GROUP.HCODE =TSPL_sac_WISE_TAX_AUTHORITY.HCODE  )src pivot ( max(Tax_Authority) for SNO in ([1], [2],[3],[4],[5]) ) piv ) xxx pivot  ( max(TAX_Rate)   for SNO2 in ([1T], [2T],[3T],[4T],[5T])  ) piv2 ) xxxx left outer join TSPL_sac_WISE_TAX on TSPL_sac_WISE_TAX.HCODE = xxxx.HCODE  left outer join TSPL_SAC_MASTER on TSPL_SAC_MASTER.code = xxxx.SAC_Code left outer join TSPL_TAX_GROUP_MASTER on TSPL_TAX_GROUP_MASTER.Tax_Group_Code = xxxx.Tax_Group_Code    "
    ...
    '    Try
    '        If clsCommon.myLen(fndCode.Value) > 0 Then
    '            Dim query As String = "  select   max (xxxx.HCODE) as Doc_Code,max(Convert(varchar, TSPL_SAC_WISE_TAX.DOC_DATE,103)) as DOC_DATE,max(case when TSPL_SAC_WISE_TAX.Status =1 then 'Y' else 'N' end ) as Posted, xxxx.SAC_Code,max(TSPL_SAC_MASTER.Description) as SAC_Desc,max(xxxx.Tax_Group_Code) as Tax_Group_Code,max (TSPL_TAX_GROUP_MASTER.Tax_Group_Desc) as Tax_Group_Desc,max (xxxx.[1]) as Tax1_Code,max (xxxx.[1T]) as TAX1_Rate , max (xxxx.[2]) as Tax2_Code,max (xxxx.[2T]) as TAX2_Rate ,max (xxxx.[3]) as Tax3_Code,max (xxxx.[3T]) as TAX3_Rate ,max (xxxx.[4]) as Tax4_Code,max (xxxx.[4T]) as TAX4_Rate ,max (xxxx.[5]) as Tax5_Code,max (xxxx.[5T]) as TAX5_Rate from ( select * from ( select * from ( select TSPL_SAC_WISE_TAX_AUTHORITY.HCODE, TSPL_SAC_WISE_TAX_GROUP.SNO as TAX_Group_SNo,  TSPL_SAC_WISE_TAX_GROUP.SAC_Code,TSPL_SAC_WISE_TAX_GROUP.Tax_Group_Code,TSPL_SAC_WISE_TAX_AUTHORITY.SNO,convert(varchar,TSPL_SAC_WISE_TAX_AUTHORITY.SNO)+'T' as SNO2, TSPL_SAC_WISE_TAX_AUTHORITY. DCODE,TSPL_SAC_WISE_TAX_AUTHORITY.Tax_Authority,TSPL_SAC_WISE_TAX_AUTHORITY.TAX_Rate  from TSPL_SAC_WISE_TAX_AUTHORITY  inner join TSPL_SAC_WISE_TAX_GROUP on TSPL_SAC_WISE_TAX_GROUP.DCODE=TSPL_SAC_WISE_TAX_AUTHORITY.DCODE and TSPL_SAC_WISE_TAX_GROUP.HCODE =TSPL_SAC_WISE_TAX_AUTHORITY.HCODE  )src pivot ( max(Tax_Authority) for SNO in ([1], [2],[3],[4],[5]) ) piv ) xxx pivot  ( max(TAX_Rate)   for SNO2 in ([1T], [2T],[3T],[4T],[5T])  ) piv2 ) xxxx left outer join TSPL_SAC_WISE_TAX on TSPL_SAC_WISE_TAX.HCODE = xxxx.HCODE  left outer join TSPL_SAC_MASTER on TSPL_SAC_MASTER.Code = xxxx.SAC_Code left outer join TSPL_TAX_GROUP_MASTER on TSPL_TAX_GROUP_MASTER.Tax_Group_Code = xxxx.Tax_Group_Code   "
    ...
    '            ListImpExpColumnsMandatory = New List(Of String)({"Doc_Code", "DOC_DATE", "Posted", "SAC_Code", "Tax_Group_Code"})
    '            ListImpExpColumnsSuperMandatory = New List(Of String)({"Doc_Code"})
    '            transportSql.ExporttoExcel(query, " and xxxx.HCODE = '" + fndCode.Value + "'  group by xxxx.SAC_Code, xxxx.Tax_Group_Code ", " Doc_Code,Tax_Group_Code asc  ", Me, ListImpExpColumnsMandatory, ListImpExpColumnsSuperMandatory, MyBase.Form_ID)
    '        Else
    '            clsCommon.MyMessageBoxShow("Fist Select Document", "SAC Tax")
    '        End If

    '    Catch ex As Exception
    '        clsCommon.MyMessageBoxShow(Me, ex.Message, "SAC Tax")
    '    End Try
    'End Sub
    Private Sub RadMenuItem4_Click(sender As Object, e As EventArgs) Handles RadMenuItem4.Click
        ...
        Try
            ...
            If transportSql.importExcel(dgv, "SAC_Code", "SAC_Desc", "Tax_Group_Code", "Tax_Group_Desc", "Tax1_Code", "TAX1_Rate", "Tax2_Code", "TAX2_Rate", "Tax3_Code", "TAX3_Rate", "Tax4_Code", "TAX4_Rate", "Tax5_Code", "TAX5_Rate") Then
                ...
                Try
                    ...
                    For Each grow As GridViewRowInfo In dgv.Rows
                        ...
                        If clsCommon.myLen(grow.Cells("SAC_Code").Value) > 0 Then
                            Dim isValidItemcode As Double = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(Code)  from TSPL_SAC_MASTER  where Code ='" + grow.Cells("SAC_Code").Value.ToString() + "'"))
                            If isValidItemcode <= 0 Then
                                common.clsCommon.MyMessageBoxShow(Me, "Invalid SAC code.", Me.Text)
                                isError = True
                                Exit Sub
                            End If
                        End If
                        ...
                        SAC_Code = grow.Cells("SAC_Code").Value.ToString()
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colSAC_Code).Value = SAC_Code
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colSAC_Desc).Value = ClsSACMaster.GetName(SAC_Code)
    ...
    Sub OpenTax5CodeList(ByVal isButtonClick As Boolean)
        Try
            ...
            End If
            Dim qry As String = "select Tax_Code as Code,Tax_Code_Desc as Descriiption from TSPL_TAX_GROUP_DETAILS"
            gv1.CurrentRow.Cells(colTAX5_Code).Value = clsCommon.ShowSelectForm("ItemFnder@TaxCode", qry, "Code", "Tax_Group_Code= '" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX_GROUP_CODE).Value) + "' ", clsCommon.myCstr(gv1.CurrentRow.Cells(colTAX5_Code).Value), "", isButtonClick)
        Catch ex As Exception
            gv1.CurrentRow.Cells(colTAX5_Code).Value = ""
