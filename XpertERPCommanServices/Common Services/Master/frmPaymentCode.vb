...
#End Region
    ...
    ' Ticket No : VIJ/22/10/19-000051 By Prabhakar
        ...
        Try
            If fnd_paymentcode.Value <> "" Then
                ...
                If dr.Rows IsNot Nothing AndAlso dr.Rows.Count > 0 Then
                    ...
                    fndBankCode.Value = clsCommon.myCstr(dr.Rows(0)("Bank_code"))
                    LblBankName.Text = clsCommon.myCstr(clsBankMaster.GetName(fndBankCode.Value))
                    'chkDefault.Checked = IIf(obj.isDefault = 1, True, False)
                    chkDefault.Checked = If(Not IsDBNull(dr.Rows(0)("IsDefault")), clsCommon.myCBool(dr.Rows(0)("IsDefault")), False)
                End If
                btn_save.Enabled = True
    ...
    Public Sub Savedata()
        ...
        Try
            Dim qry As String = " "
            'Dim ChkNewEntry As String = clsDBFuncationality.getSingleValue("Select count(*) from tspl_payment_code where IsDefault='1' ")
            'If ChkNewEntry = 1 Then
            '    clsCommon.MyMessageBoxShow(Me, "Already Default Selected For another Payment Type", Me.Text)
            '    Exit Sub
            'End If
            If (IIf(clsCommon.myCstr(clsFixedParameter.GetData(clsFixedParameterType.AllowAutoGenerateDocNoInMaster, clsFixedParameterCode.AllowAutoGenerateDocNoInMaster, Nothing)) = "1", True, False) = False) AndAlso clsCommon.myLen(fnd_paymentcode.Value) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, " Payment Code cannot be left blank", Me.Text)
                fnd_paymentcode.Focus()
                Return
    ...
    Private Sub funInsert()
        Try
            ...
            If chkDefault.Checked = True Then
                clsDBFuncationality.ExecuteNonQuery("update tspl_payment_code set IsDefault= 0 ")
    ...
    Public Sub DeleteData()
        ...
        If clsCommon.myLen(fnd_paymentcode.Value) <= 0 Then
            common.clsCommon.MyMessageBoxShow(Me, "Please Fill the Payment Code", Me.Text)
            fnd_paymentcode.Focus()
            Return
        End If
        Try
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If strpaymentcode.Length > 12 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Payment Code length cannot be more than 12", Me.Text)
                        trans.Rollback()
                        Exit Sub
                    End If
                    If strpaymentdesc.Length > 50 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Payment Description length cannot be more than 50", Me.Text)
                        trans.Rollback()
                        Exit Sub
                    End If
                    If strpaymenttype.Length > 12 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Payment Type length cannot be more than 12", Me.Text)
                        trans.Rollback()
                        Exit Sub
                    End If
    ...
    Private Sub fndBankCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndBankCode._MYValidating
        ...
        fndBankCode.Value = clsCommon.ShowSelectForm("PayBanKFilter", Qry, "Code", strWhrcls, fndBankCode.Value, "Code", isButtonClicked)
        LblBankName.Text = clsCommon.myCstr(clsBankMaster.GetName(fndBankCode.Value))
