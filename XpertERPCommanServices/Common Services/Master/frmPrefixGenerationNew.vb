...
#End Region
    ...
    Private Sub FrmPrefixGenerationNew_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            clsDBFuncationality.ExecuteNonQuery("alter table TSPL_DOCPREFIX_MASTER drop uk_TSPL_DOCPREFIX_MASTER")
        Catch ex As Exception
        End Try
        Try
            clsDBFuncationality.ExecuteNonQuery("alter table TSPL_DOCPREFIX_MASTER drop column RouteNo")
        Catch ex As Exception
        End Try
        isLoadForm = True
    ...
    Sub LoadData()
        ...
        isLocationReadOnly = IIf(clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select (Is_Location_Wise+Is_MCC_Wise+Is_State_Wise+Is_Route_Wise) as Is_Location_Wise from TSPL_DOCUMENT_TYPE where TSPL_DOCUMENT_TYPE.Doc_Type='" + clsCommon.myCstr(txtDocument.Value) + "' ")) > 0, False, True)
            ...
            If (Not isInsideLoadData AndAlso e.RowIndex >= 0) Then
                If (Not isInsideCellValueChaged) Then
                    ...
                    ElseIf (clsCommon.CompairString(e.Column.Name, colLoaction) = CompairStringResult.Equal) Then
                        Dim qry As String = "select Is_State_Wise,Is_MCC_Wise,Is_Route_Wise from TSPL_DOCUMENT_TYPE where Doc_Type='" + clsCommon.myCstr(txtDocument.Value) + "' and ISNULL(Doc_Trans_Type,'')='" + clsCommon.myCstr(gv1.CurrentRow.Cells(colTransType).Value) + "'"
                        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtDocument.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowHistoryData(txtDocument.Value, "Doc_Type", "TSPL_DOCPREFIX_MASTER")
        Catch ex As Exception
