...
End if
                ...
                Else : strquery = ""
                    txtwriteoff.Text = ""
                    common.clsCommon.MyMessageBoxShow(Me, "This Write Off Account does not exist in Master Table", Me.Text)
                    fndwriteoff.Value = ""
                End If
            Catch ex As Exception
                myMessages.myExceptions(ex)
    ...
    Public Sub SaveData()
        Try
            ...
            ElseIf fndbankacc.Value = "" Then
                ...
                myMessages.blankValue(Me, "Bank Account", Me.Text)
                fndbankacc.Focus()
            ElseIf fndwriteoff.Value = "" Then

                myMessages.blankValue(Me, "Write Off Account", Me.Text)
                fndwriteoff.Focus()
            ElseIf fndcredit.Value = "" Then
                myMessages.blankValue(Me, "Credit Cards Charges", Me.Text)
                fndcredit.Focus()
            ...
            If clsCommon.myLen(grow.Cells(colTS_FROM).Value) > 0 Then
                ...
                For Each growIn As GridViewRowInfo In gvPP.Rows
                    If grow.Index <> growIn.Index Then
                        ...
                        If clsCommon.myCdbl(grow.Cells(colTS_FROM).Value) <= clsCommon.myCdbl(growIn.Cells(colTS_FROM).Value) And clsCommon.myCdbl(grow.Cells(colTS_TO).Value) >= clsCommon.myCdbl(growIn.Cells(colTS_FROM).Value) Then
                            If clsCommon.myCdbl(grow.Cells(colTS_FROM).Value) <= clsCommon.myCdbl(growIn.Cells(colTS_FROM).Value) And clsCommon.myCdbl(grow.Cells(colTS_TO).Value) > clsCommon.myCdbl(growIn.Cells(colTS_FROM).Value) Then
                                clsCommon.MyMessageBoxShow(Me, "Please Check Records in Row No [" & grow.Index + 1 & "] and [" & growIn.Index + 1 & "] . Both have Some Common Range", Me.Text)
                                Return False
                            End If
                ...
                If gvPP.Columns(colTS_FROM).IsVisible Then
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colTS_FROM).Value)) <= 0 And gvPP.Columns(colTS_FROM).IsVisible Then
                        myMessages.blankValue(Me, "From Value is blank at row no " & (grow.Index + 1) & "", Me.Text)
                        Return False
                    End If
                End If
                ...
                If gvPP.Columns(colTS_TO).IsVisible Then
                    If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colTS_TO).Value)) <= 0 And gvPP.Columns(colTS_TO).IsVisible Then
                        myMessages.blankValue(Me, "To Value is blank at row no " & (grow.Index + 1) & "", Me.Text)
                        Return False
                    End If
                End If

                If clsCommon.myLen(clsCommon.myCstr(grow.Cells(colCharges).Value)) <= 0 Then
                    myMessages.blankValue(Me, "Charges is blank at row no " & (grow.Index + 1) & "", Me.Text)
                    Return False
                End If
    ...
    Public Sub funinsert()
        Try
            If AllowToSave() Then
                ...
                If Count > 0 Then
                    clsCommon.MyMessageBoxShow(Me, "Default bank already mapped with another bank", Me.Text)
    ...
    Function SaveBankCheckData(ByVal strDocNo As String, Optional ByVal trans As SqlTransaction = Nothing) As Boolean
        ...
        For Each row As GridViewRowInfo In gv1.Rows
            ...
            If dtcheck.Rows.Count > 0 Then
                clsCommon.MyMessageBoxShow(Me, "Check Code " & dtcheck.Rows(0).Item("CHECK_CODE") & " already exists in bank " & dtcheck.Rows(0).Item("BANK_CODE") & "", Me.Text)
                Return False
            End If
            ...
            If clsCommon.myCdbl(row.Cells(colLAST_CHECK_NUMBER).Value) < clsCommon.myCdbl(row.Cells(colLAST_PRINT_CHECK_NUMBER).Value) Then
                clsCommon.MyMessageBoxShow(Me, "Please check ! last check no. " & clsCommon.myCstr(clsCommon.myCdbl(row.Cells(colLAST_CHECK_NUMBER).Value)) & " should be greater than from printed check no. " & clsCommon.myCstr(clsCommon.myCdbl(row.Cells(colLAST_PRINT_CHECK_NUMBER).Value)) & "", Me.Text)
                Return False
            End If
...
End if
    ...
    Public Sub fundelete()
        Try
            ...
            If clsCommon.myLen(dpt) > 0 AndAlso dpt IsNot Nothing Then
                common.clsCommon.MyMessageBoxShow(Me, "This Customer Cannot be deleted." + Environment.NewLine + "This is already in Process", Me.Text)
                Return
            End If
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(fndbank.Value) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Select Bank", Me.Text)
                Exit Sub
            End If
