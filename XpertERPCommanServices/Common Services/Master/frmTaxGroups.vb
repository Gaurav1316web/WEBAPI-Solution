...
#Region "Variables"
    ...
    Const colTaxOnBaseAmount As String = "TaxOnBaseAmount"
...
#End Region
    ...
    Private Sub findTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles findTaxGroup._MYValidating
        ...
        If findTaxGroup.MyReadOnly OrElse isButtonClicked Then
            ...
            Dim qry As String = "SELECT TAX_Group_Code as 'Tax Group',(CASE WHEN Tax_Group_Type='S' THEN 'Sales'  WHEN Tax_Group_Type='T' THEN 'Transfer'  WHEN Tax_Group_Type='H' THEN 'HR And Payroll'   ELSE 'Purchase' END) as 'Transaction Type',Tax_Group_Desc as Description,Currency_Code From TSPL_TAX_GROUP_MASTER"
            ...
            If dr IsNot Nothing Then
                ...
                ElseIf transType = "HR And Payroll" Then
    ...
    Private Sub fillControls()
        ...
        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            For Each dr1 As DataRow In dt.Rows
                ...
                dataRowInfo.Cells(colTaxOnBaseAmount).Value = clsCommon.myCstr(dr1("Tax_On_Base_Amount"))
                '' MultiCurrency
                ...
                Try
                    ...
                    If btnAdd.Text = "Save" Then
                        ...
                        If default_Type <> 0 Then
                            Dim resetQuery As String = "UPDATE TSPL_TAX_GROUP_MASTER SET default_Type = 0 WHERE default_Type = '" & default_Type & "'"
                            clsDBFuncationality.ExecuteNonQuery(resetQuery, trans)
                        ...
                        Dim col1 As New Hashtable()
                            ...
                            For Each grow As GridViewRowInfo In gvTaxGroups.Rows
                                ...
                                End If
                                clsCommon.AddColumnsForChange(coll, "Tax_On_Base_Amount", IIf(clsCommon.CompairString(clsCommon.myCstr(grow.Cells(colTaxOnBaseAmount).Value), "Yes") = CompairStringResult.Equal, "Y", "N"))
            ...
            Else
                For Each grow As GridViewRowInfo In gvTaxGroups.Rows
                    ...
                    ElseIf grow.Cells(3).Value = "No" Then
                        Dim lastFormula As String = "<html><b>" + grow.Cells(0).Value + "-</b> (Basic Amount) " + IIf(clsCommon.CompairString(clsCommon.myCstr(grow.Cells(colTaxOnBaseAmount).Value), "Yes") = CompairStringResult.Equal, "", taxableTax) + " * (Tax Rate for <b>" + grow.Cells(0).Value + "</b>)/100<br></html>"
    ...
    Private Sub MasterTemplate_CellDoubleClick(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.GridViewCellEventArgs) Handles gvTaxGroups.CellDoubleClick
        ...
        ElseIf e.Column.Name = "surtax" Then
            ...
            End If
        ElseIf e.Column.Name = colTaxOnBaseAmount Then
            If e.Row.Cells(colTaxOnBaseAmount).Value = "No" Then
                e.Row.Cells(colTaxOnBaseAmount).Value = "Yes"
                gvTaxGroups.EndEdit()
            Else
                e.Row.Cells(colTaxOnBaseAmount).Value = "No"
            End If
        End If
    End Sub
    ...
    Private Sub menuExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles menuExport.Click
        ...
        sql += " FROM TSPL_Tax_Group_Master INNER JOIN TSPL_TAX_GROUP_DETAILS ON TSPL_Tax_Group_Master.Tax_Group_Code = TSPL_TAX_GROUP_DETAILS.Tax_Group_Code AND " &
              "  TSPL_Tax_Group_Master.Tax_Group_Type = TSPL_TAX_GROUP_DETAILS.Tax_Group_Type"

        ListImpExpColumnsMandatory = New List(Of String)({"Active", "Excisable", "VAT", "STax", "Tax Group Code", "Description", "Tax Authority", "Authority Description"})
        ListImpExpColumnsSuperMandatory = New List(Of String)({"Tax Group Code"})
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    Try
                        ...
                        '' multicurrency
                        Dim coll As New Hashtable()
                        clsCommon.AddColumnsForChange(coll, "Tax_On_Base_Amount", IIf(clsCommon.CompairString(strTaxOnBaseAmt, "Yes") = CompairStringResult.Equal, "Y", "N"))
                        clsCommonFunctionality.UpdateDataTable(coll, "TSPL_TAX_GROUP_DETAILS", OMInsertOrUpdate.Update, " Tax_Group_Code='" & strTaxGrCode & "' and  Tax_Code='" + strTaxCode + "'", trans)
                        '' end of multicurrency
