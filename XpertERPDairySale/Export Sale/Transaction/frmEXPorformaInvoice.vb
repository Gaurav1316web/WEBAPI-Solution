...
''updation by richa agarwal against ticket no BM00000003745(add condition in customer finder),BM00000007838
'-BM00000003441
'=============BM00000003445
''updation by Richa Agarwal against ticket no. BM00000006182,BM00000006177 14/04/2015,BM00000006251 on 17/04/2015,BM00000006648,BM00000006655, against ticket no BM00000006589,BM00000006716,BM00000006733,BM00000006873,BM00000007523
'' work done agiats ticket no. ALF/30/08/18-000080 on 30/08/2018
Imports common
...
Imports System.Net
Imports System.IO
Public Class frmEXPorformaInvoice
...
#Region "Variables"
    ...
    Const colPacking_Instrctn As String = "PackingInstrctn"
    Const ReportID As String = "EXPORFORMAINVOICE"
...
#End Region
    ...
    Private Sub LOCATIONRIGTHS()
        ...
        Try
            ...
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Default_LocCode) > 0 Then
                ...
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXPorformaInvoice) = CompairStringResult.Equal Then
                    WhrCls = " and Location_Type='Physical'  "
                ElseIf clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmProformaInvoiceMT) = CompairStringResult.Equal Then
    ...
    Private Sub LoadTerms_of_Payment()
        Dim qry As String = "select '' as Code,'None' as Name union all select 'LC' as Code,'Letter of Credit' as Name union all select 'DA' as Code,'Document against Acceptance' as Name union all select 'DP' as Code,'Document against Payment' as Name  union all select 'AD' as Code,'Advance' as Name union all select 'OC' as Code,'On Credit' as Name"
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    ...
    Private Sub FrmAPInvoiceEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            'RadPageViewPage2.Item.Visibility = ElementVisibility.Collapsed  ''TAX GRID
            ...
            If clsCommon.CompairString(cmbDocType.SelectedValue, "MT") = CompairStringResult.Equal Then
                rmFormat2.Visibility = ElementVisibility.Hidden

            End If

            ''richa agarwal 21/04/2015
            If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmProformaInvoiceMT) = CompairStringResult.Equal Then
    ...
    Sub SetMultiCurrencyVisibility()
        Try
            ...
            Else
                ...
                Throw New Exception("Do multicurrency on for Export Porforma Invoice.")
            End If
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    ...
    Sub LoadStuffing()
        Dim qry As String = "select '1' as Code,'Done' as Name" 'select '' as Code,'None' as Name union all  union all select '0' as Code,'Not-Done' as Name
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
    ...
    Sub BlankAllControls()
        ...
        dtpAcceptance.Text = clsCommon.GETSERVERDATE(Nothing)
        ...
        txtInvNo.Text = ""
        txtComment.Text = ""
        txtComment.Text = clsEXPorformaInvoiceHead.GetProformaDescrptn()
        txtTaxGroup.Value = ""
        lblTaxGrpName.Text = ""
        txtTermCode.Value = ""
        lblTermName.Text = ""
    ...
    Sub LoadBlankGrid()
        ...
        repoSchemeApp2.Width = 100
        repoSchemeApp2.MaxLength = 100
        gv1.MasterTemplate.Columns.Add(repoSchemeApp2)

        Dim repoSchemeApp2PCK As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoSchemeApp2PCK.FormatString = ""
        repoSchemeApp2PCK.HeaderText = "Packing Instruction"
        repoSchemeApp2PCK.Name = colPacking_Instrctn
        repoSchemeApp2PCK.Width = 100
        repoSchemeApp2PCK.MaxLength = 300
        gv1.MasterTemplate.Columns.Add(repoSchemeApp2PCK)
        ...
        repoRequition.HeaderText = "Sales Order No"
    ...
    Private Sub CalculateDiscountAmount()
        ...
        If chkDiscountOnAmt.IsChecked Then
            For Each gro As GridViewRowInfo In gv1.Rows
                ...
                If clsCommon.myLen(gro.Cells(colICode).Value) > 0 And clsCommon.myCdbl(gro.Cells(ColFOC).Value) = 0 AndAlso clsCommon.myCdbl(lblAmtWithDiscount.Text) > 0 Then
    ...
    Sub OpenUOMList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            ...
            gv1.CurrentRow.Cells(colUnit).Value = clsCommon.ShowSelectForm("EXPUNTFND", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), "Code", isButtonClick)
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If clsCommon.CompairString(cmbDocType.SelectedValue, "") = CompairStringResult.Equal Then
                ...
                cmbDocType.Focus()
                cmbDocType.Select()
                Errorcontrol.SetError(cmbDocType, "Please select document type.")
                Return False
            Else
            ...
            If clsCommon.myLen(cboStuffing.SelectedValue) <= 0 Then
                clsCommon.MyMessageBoxShow(Me, "Select stuffing status for Commercial Invoice.", Me.Text)
                RadPageView1.SelectedPage = RadPageViewPage1
                cboStuffing.Select()
                Errorcontrol.SetError(cboStuffing, "Select stuffing status for Commercial Invoice.")
                Return False
            Else
                Errorcontrol.ResetError(cboStuffing)
            End If
            ...
            If Not isNewEntry AndAlso clsCommon.myLen(txtDocNo.Value) <= 0 Then
                ...
                txtDocNo.Focus()
                txtDocNo.Select()
                Errorcontrol.SetError(txtDocNo, "PI No. not found to save.")
                Return False
            Else
            ...
            Dim Mrp_Mandatory As Boolean = False
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                Mrp_Mandatory = clsCommon.myCBool(clsItemMaster.IsMRPItem(strICode, Nothing))
                ...
                If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colRowType).Value), RowTypeItem) = CompairStringResult.Equal Then
                    ...
                    'If Mrp_Mandatory AndAlso clsCommon.myCdbl(gv1.Rows(ii).Cells(colMRP).Value) <= 0 Then
                    '    gv1.CurrentRow = gv1.Rows(ii)
                    ...
                    If clsCommon.myLen(strReqNo) > 0 Then
                        ...
                        If dblQty > dblPendingQty Then
                            ...
                            common.clsCommon.MyMessageBoxShow("Item " + strICode + "( " + strIName.Trim() + " ) Entered Quantity(" + clsCommon.myCstr(dblQty) + ") Cannot more than Pending Quantity(" + clsCommon.myCstr(dblPendingQty) + ").At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                            Return False
                        End If
                    End If
                    ...
                    If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.CompairString(strSchemeItem, "No") = CompairStringResult.Equal Then
                        ...
                        For jj As Integer = ii + 1 To gv1.Rows.Count - 1
                            ...
                            If clsCommon.CompairString(strICode, strInICode) = CompairStringResult.Equal Then 'And clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(jj).Cells(colSchemeItem).Value), "No") = CompairStringResult.Equal
                                RadPageView1.SelectedPage = RadPageViewPage1
            ...
            If arrICode Is Nothing AndAlso arrICode.Count <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                gv1.Focus()
                gv1.Select()
            ...
            If chkAcceptance.Checked AndAlso clsCommon.myLen(dtpAcceptance.Text) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                dtpAcceptance.Select()
                dtpAcceptance.Focus()
                clsCommon.MyMessageBoxShow(Me, "Fill acceptance date.", Me.Text)
                Errorcontrol.SetError(dtpAcceptance, "Fill acceptance date.")
                Return False
            Else
                Errorcontrol.ResetError(dtpAcceptance)
            End If
            ''richa agarwal 14/04/2015
            If clsCommon.CompairString(cmbDocType.SelectedValue, "MT") = CompairStringResult.Equal Then
                If clsCommon.myLen(fndPaymenttermsGroup.Value) <= 0 Then
                    RadPageView1.SelectedPage = RdPaymentterms
                    fndPaymenttermsGroup.Select()
                    fndPaymenttermsGroup.Focus()
                    clsCommon.MyMessageBoxShow(Me, "Please select Payment Terms Group.", Me.Text)
                    Errorcontrol.SetError(fndPaymenttermsGroup, "Please select Payment Terms Group.")
                    Return False
                Else
                    Errorcontrol.ResetError(fndPaymenttermsGroup)
                End If
            ...
            End If
            ''------------------------
            clsEXSalesOrder.IsValidCustomer(arrReqNo, txtVendorNo.Value)
            clsEXSalesOrder.IsValidDocumentType(arrReqNo, cmbDocType.SelectedValue)
    ...
    Sub SaveData(ByVal ChekPostBtn As Boolean)
        Dim obj As New clsEXPorformaInvoiceHead()
        Try
            If (AllowToSave()) Then

                obj = New clsEXPorformaInvoiceHead()
                obj.is_Accepted = clsCommon.myCstr(IIf(chkAcceptance.Checked = True, "Y", "N"))
                obj.Accepted_Date = clsCommon.myCDate(dtpAcceptance.Text)
                obj.is_Partshipment = clsCommon.myCstr(IIf(chkPartshipment.Checked = True, "Y", "N"))
                obj.is_Transshipment = clsCommon.myCstr(IIf(chkTransshipment.Checked = True, "Y", "N"))
                ...
                obj.HeadDisc_Per = txtDiscPer.Text
                obj.Stuffing_Status = clsCommon.myCstr(cboStuffing.SelectedValue)
                ...
                obj.Airway_Line = txtairwayline.Text
                ...
                obj.MT_Payment_Terms_Group_Desc = clsCommon.myCstr(lblpaymenttermsgroup.Text)
                ...
                obj.MT_Beneficiary_Name = clsCommon.myCstr(lblBeneficiary.Text)
                obj.MT_INCOTERMS = clsCommon.myCstr(TxtINCOTERMS.Text)
                If clsCommon.myLen(TxtMTPONo.Text) > 0 Then
                    obj.MT_Against_PO = clsCommon.myCstr(TxtMTPONo.Text)
                End If
                ...
                obj.GEDate = Nothing ' txtGEDate.Value
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsEXPorformaInvoiceDetail()
                    ...
                    objTr.Packing_Inst = clsCommon.myCstr(grow.Cells(colPacking_Instrctn).Value)
                ...
                If Not isNewEntry AndAlso Not ChekPostBtn Then ''when update record but not post done By Monika '31/03/2015
                    If clsCommon.MyMessageBoxShow("Are you sure,Want to do amendment of Porforma Invoice?", "Attention", MessageBoxButtons.YesNo, RadMessageIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
                        Dim qry As String = "insert into TSPL_EX_PI_HEAD_HISTORY select * from TSPL_EX_PI_HEAD where document_code='" + txtDocNo.Value + "'"
                        clsDBFuncationality.ExecuteNonQuery(qry)

                        qry = "insert into TSPL_EX_NOTIFY_PARTY_DETAIL_HISTORY select * from TSPL_EX_NOTIFY_PARTY_DETAIL where document_code='" + txtDocNo.Value + "'"
                        clsDBFuncationality.ExecuteNonQuery(qry)

                        qry = "insert into TSPL_EX_PI_DETAIL_HISTORY select * from TSPL_EX_PI_DETAIL where document_code='" + txtDocNo.Value + "'"
                        clsDBFuncationality.ExecuteNonQuery(qry)
                    End If
                End If
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Try
            ...
            If clsCommon.myLen(arrLoc) <= 0 Then
                clsCommon.myCstr("No location rights.")
                Exit Sub
            End If
            ...
            obj = clsEXPorformaInvoiceHead.GetData(strCode, "'T','R'", NavTyep, arrLoc, IIf(clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmProformaInvoiceMT) = CompairStringResult.Equal, "MT", "EX"))
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                txtpodate.Value = obj.podate
                txtVendorNo.Value = obj.Customer_Code
                txtPONo.Text = obj.Cust_PO_No
                txtForm38.Text = obj.Form_38_No
                '--------------------Added by Preeti gupta Against ticket no [BM00000007886,ALF/08/04/19-000095,ALF/08/04/19-0000100]
                'If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmProformaInvoiceMT) = CompairStringResult.Equal Then
                '    txtDate.Enabled = True
                'Else
                txtDate.Enabled = True
                ...
                txtVendorNo.Enabled = False
                cboStuffing.SelectedValue = obj.Stuffing_Status
                ...
                chkAcceptance.Checked = clsCommon.myCBool(IIf(obj.is_Accepted = "Y", True, False))
                dtpAcceptance.Text = obj.Accepted_Date
                chkPartshipment.Checked = clsCommon.myCBool(IIf(obj.is_Partshipment = "Y", True, False))
                chkTransshipment.Checked = clsCommon.myCBool(IIf(obj.is_Transshipment = "Y", True, False))
                ...
                txtairwayline.Text = obj.Airway_Line
                ...
                lblpaymenttermsgroup.Text = clsCommon.myCstr(obj.MT_Payment_Terms_Group_Desc)
                ...
                lblBeneficiary.Text = clsCommon.myCstr(obj.MT_Beneficiary_Name)
                TxtINCOTERMS.Text = clsCommon.myCstr(obj.MT_INCOTERMS)
                TxtMTPONo.Text = clsCommon.myCstr(obj.MT_Against_PO)
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsEXPorformaInvoiceDetail In obj.Arr
                        gv1.Rows.AddNew()
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colShippingMark).Value = objTr.Shipping_Mark
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colPacking_Instrctn).Value = objTr.Packing_Inst
                        ...
                        If clsCommon.myLen(objTr.Shipment_Code) > 0 Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsCommon.myCdbl(clsEXSalesOrderDetail.GetBalancePOQty(objTr.Shipment_Code, objTr.Item_Code, obj.Document_Code, objTr.Unit_code, objTr.MRP, objTr.Assessable))
                        End If
                ...
                If obj.Arr_Notify IsNot Nothing AndAlso obj.Arr_Notify.Count > 0 Then
                    For Each objtr As clsEXPorformaInvoiceNotifyDetail In obj.Arr_Notify
    ...
    Private Sub gv2_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gv2.DoubleClick
        Try
            If rbtnTaxCalAutomatic.IsChecked Then
                ...
                Dim dblNewRate As Double = clsCommon.myCdbl(clsCommon.ShowSelectForm("TAXFND", qry, "Rate", "Tax_Code='" + clsCommon.myCstr(gv2.CurrentRow.Cells(colTTaxAutCode).Value) + "' and Tax_Type='S'", "", "", True))
    ...
    Private Sub SendSMSandEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        ...
        Try
            'Dim obj As clsEmailSMSSettingNew = clsEmailSMSSettingNew.GetData(clsUserMgtCode.frmEXPorformaInvoice)
            ...
            '        strRptPath = frmCRV.EmailAttachment(CrystalReportFolder.NewSalesReports, dt1, "crptShipment", "Proforma Detail")
            '        frmCRV = Nothing
            '    End If
            'End If
            ''---------------------------------------------------------------------------
            ...
            If dtContent IsNot Nothing AndAlso dtContent.Rows.Count > 0 Then
                ...
                '------------------------code for attchament-------------------------------------
                funPrint(txtDocNo.Value, False, True)
    ...
    Private Sub SMSSENDONLY(ByVal isPost As Boolean)
        Try
            ...
            'If clsSMSSend.SendSMS(clsUserMgtCode.frmEXPorformaInvoice, strMes, strphone) Then
            '    If Not isPost Then
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                If (clsEXPorformaInvoiceHead.PostData(MyBase.Form_ID, txtDocNo.Value, arrLoc)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsEXPorformaInvoiceHead.DeleteData(txtDocNo.Value, FORMTYPE)) Then
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        '-----------------------------------------------------
        Dim qry As String = "select Document_Code as Code,CONVERT(varchar(10), Document_Date,103)+' '+ CONVERT(varchar(5), Document_Date,114) as Date,(case when document_type='MT' then 'Merchant Trade' else 'Export Sale' end) as [Document Type],Customer_Code as [Customer Code], Customer_Name as Customer,TSPL_EX_PI_HEAD.Comments,Total_Amt as Amount,case when TSPL_EX_PI_HEAD.Status=0 then 'Pending' else 'Approved' end as [Status],Against_SO_No as [Sales Order No] from TSPL_EX_PI_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_EX_PI_HEAD.Customer_Code "

        Dim whrClas As String = ""
        If clsCommon.CompairString(clsUserMgtCode.frmProformaInvoiceMT, FORMTYPE) = CompairStringResult.Equal Then
            whrClas = " TSPL_EX_PI_HEAD.document_type='MT' "
        Else
            whrClas = " TSPL_EX_PI_HEAD.document_type='EX' "
        End If

        If clsCommon.myLen(arrLoc) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas += " and Bill_To_Location in (" + arrLoc + ") and TSPL_EX_PI_HEAD.Customer_Code in (" + strwherecls + ") "
        ...
        LoadData(clsCommon.ShowSelectForm("PIFND", qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked), NavigatorType.Current)
    End Sub
    ...
    Private Sub FrmAPInvoiceEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
            If e.KeyCode = Keys.F2 AndAlso gv1.CurrentCell IsNot Nothing AndAlso gv1.CurrentColumn Is gv1.Columns(colUnit) Then
                ...
                isCellValueChangedOpen = False
            End If
            If e.KeyCode = Keys.F2 AndAlso gv1.CurrentCell IsNot Nothing AndAlso clsCommon.myLen(gv1.CurrentRow.Cells(colOrderNo).Value) <= 0 AndAlso gv1.CurrentColumn Is gv1.Columns(colICode) Then
                isCellValueChangedOpen = True
                OpenICodeList(True)
                isCellValueChangedOpen = False
    ...
    Private Sub txtTermCode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTermCode._MYValidating
        ...
        txtTermCode.Value = clsCommon.ShowSelectForm("EXPTERMFND", qry, "Code", "", txtTermCode.Value, "Code", isButtonClicked)
        SetTermDetails()
    End Sub
    ...
    Private Sub txtTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTaxGroup._MYValidating
        ...
        'txtTaxGroup.Value = clsCommon.ShowSelectForm("TAXFND", qry, "Code", "Tax_Group_Type='S'", txtTaxGroup.Value, "Code", isButtonClicked)
    ...
    Private Sub txtVendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtVendorNo._MYValidating
        ...
        If clsCommon.myLen(strwherecls) = 0 Then
            txtVendorNo.Value = clsCommon.ShowSelectForm("EXPVDNFND", qry, "Code", " ISNULL(TSPL_CUSTOMER_MASTER.currency_code,'') not in ('','" + objCommonVar.BaseCurrencyCode + "')", txtVendorNo.Value, "Code", isButtonClicked)
        Else
            txtVendorNo.Value = clsCommon.ShowSelectForm("EXPVNDFND", qry, "Code", " ISNULL(TSPL_CUSTOMER_MASTER.currency_code,'') not in ('','" + objCommonVar.BaseCurrencyCode + "') and TSPL_CUSTOMER_MASTER.Cust_Code in (" + strwherecls + ")", txtVendorNo.Value, "Code", isButtonClicked) 'TSPL_CUSTOMER_MASTER.Status='N'
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBillToLocation._MYValidating
        ...
        txtBillToLocation.Value = clsCommon.ShowSelectForm("EXPLOCFND", qry, "Code", WhrCls, txtBillToLocation.Value, "Code", isButtonClicked)
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating
        If clsCommon.myLen(txtBillToLocation.Value) = 0 Then
            clsCommon.MyMessageBoxShow(Me, "Please select Bill To Location first", Me.Text)
            txtBillToLocation.Focus()
            txtBillToLocation.Focus()
            Errorcontrol.SetError(lblBillToLocation, "Please select Bill To Location first")
            Exit Sub
        Else
            Errorcontrol.ResetError(lblBillToLocation)
        End If
    ...
    Sub SelectMRNItems()
        ...
        frm.strDocType = "Export PI"
        frm.Text = "Pending Sales Order"
        ...
        Dim objOrderHead As clsEXSalesOrder = Nothing
        If frm.ArrReturn_EX IsNot Nothing AndAlso frm.ArrReturn_EX.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn_EX(0).Document_Code) > 0 Then
                objOrderHead = clsEXSalesOrder.GetData(frm.ArrReturn_EX(0).Document_Code, NavigatorType.Current, arrLoc, frm.strExport_Merchant)
                If objOrderHead IsNot Nothing AndAlso clsCommon.myLen(objOrderHead.Document_Code) > 0 Then
                    ...
                    If clsCommon.myLen(txtPONo.Text) <= 0 Then
                        ...
                        If clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "UDL") = CompairStringResult.Equal Then
                            If objOrderHead.Cust_Po_Date IsNot Nothing Then
                                txtpodate.Value = objOrderHead.Cust_Po_Date
                            End If
                        End If
                    End If
                    ...
                    txtPre_Carriage_By.Text = objOrderHead.Mode_Of_Transport
            ...
            If objOrderHead.Arr_Notify IsNot Nothing AndAlso objOrderHead.Arr_Notify.Count > 0 Then
                For Each objtr As clsEXSalesOrderNotifyParty In objOrderHead.Arr_Notify
            ...
            For ii As Integer = 0 To frm.ArrReturn_EX.Count - 1
                If clsCommon.myLen(frm.ArrReturn_EX(ii).Document_Code) > 0 Then
                    ...
                    '    arr.Add(strCode)
                    objOrderHead = clsEXSalesOrder.GetData(frm.ArrReturn_EX(ii).Document_Code, NavigatorType.Current, arrLoc, frm.strExport_Merchant)
                    For Each obj As clsEXSalesOrderDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn_EX(ii).Item_Code AndAlso obj.Scheme_Item = "N") OrElse (obj.Scheme_Code = frm.ArrReturn_EX(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            ...
                            If IsValidItem(obj) Then
                                ...
                                'Dim dt As DataTable = clsEXPorformaInvoiceHead.GetOriginalQty(obj.Document_Code, obj.Item_Code, obj.Unit_code, obj.Assessable, obj.MRP, Nothing)
                                'If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsEXSalesOrderDetail In objOrderHead.Arr
    ...
    Function IsValidItem(ByVal obj As clsEXSalesOrderDetail)
        ''If clsCommon.myLen(txtTaxGroup.Value) <= 0 Then
    ...
    Public Sub funPrint(ByVal strDocNo As String, Optional ByVal SecondButton As Boolean = False, Optional ByVal IsPDF As Boolean = False)
        Try
            ...
            If clsCommon.CompairString(clsCommon.myCstr(ShowStatusForSale), "1") = CompairStringResult.Equal Then
                QryShowStatus = " ,(case when TSPL_EX_PI_HEAD.status =1 then 'AUTHORIZED' else 'NOT AUTHORIZED' end) as InvStatus "
            Else
                QryShowStatus = ""
            End If
            ...
            If clsCommon.CompairString(clsCommon.myCstr(ShowSerialNoForSales), "1") = CompairStringResult.Equal Then
                ...
                SerialNo = " left outer join (select distinct Doc_No,Serial_No,Main_Item_Code,Location_Code from TSPL_MF_PRINCIPLE_RECEIPT_SERIAL_DETAIL WHERE Is_principle='1' AND ISNULL(Serial_No,'')<>'' and Doc_No in (select Doc_No from TSPL_MF_PRINCIPLE_RECEIPT_HEAD where Status='1'))aa  on TSPL_EX_PI_DETAIL.Item_Code  =AA.Main_Item_Code  ANd aa.Location_Code =TSPL_EX_PI_DETAIL.Location  "
            Else
                SerialNoColumn = " ,0 As SerialNoText "
                SerialNo = ""
            End If

            Dim qry1 As String = "select distinct TSPL_SD_SALES_ORDER_DETAIL.document_code as Order_Code "
            qry1 += " from TSPL_EX_PI_DETAIL "
            qry1 += "left outer join TSPL_SD_SALES_ORDER_DETAIL on TSPL_SD_SALES_ORDER_DETAIL.DOCUMENT_CODE=TSPL_EX_PI_DETAIL.Sales_Order_Code and TSPL_SD_SALES_ORDER_DETAIL.Item_Code=TSPL_EX_PI_DETAIL.Item_Code "
            qry1 += " where TSPL_EX_PI_DETAIL.DOCUMENT_CODE='" + strDocNo + "'"
            ...
            '' end 
            ''shivani[BM00000008124]
            Dim strPivotVar As String = ""
            'COMMENT By Prabhakar Anand
            'qry1 = "select ITEM_CATEGORY_CODE from TSPL_ITEM_CATEGORY_LEVEL  where CATEGORY_LEVEL in ('1','2') order by CATEGORY_LEVEL"
            qry1 = "select ITEM_CATEGORY_CODE from TSPL_ITEM_CATEGORY_LEVEL  where CATEGORY_LEVEL in ('1','2') and ITEM_CATEGORY_CODE not in ('BRAND','Main Group') UNION ALL SELECT 'BRAND' AS ITEM_CATEGORY_CODE UNION ALL SELECT 'Main Group' AS ITEM_CATEGORY_CODE "
            dt1 = New DataTable()
            dt1 = clsDBFuncationality.GetDataTable(qry1)
            If dt1 IsNot Nothing AndAlso dt1.Rows.Count > 0 Then
                If dt1.Rows.Count > 2 Then
                    strPivotVar = " [" + clsCommon.myCstr(dt1.Rows(0)("ITEM_CATEGORY_CODE")) + "],[" + clsCommon.myCstr(dt1.Rows(1)("ITEM_CATEGORY_CODE")) + "],[" + clsCommon.myCstr(dt1.Rows(2)("ITEM_CATEGORY_CODE")) + "]"
                Else
                    strPivotVar = " [" + clsCommon.myCstr(dt1.Rows(0)("ITEM_CATEGORY_CODE")) + "],[" + clsCommon.myCstr(dt1.Rows(1)("ITEM_CATEGORY_CODE")) + "]"
                End If


            Else
                strPivotVar = " [Brand],[Main Group]"
            End If



            'clsFixedParameter()
            ''====shivani against(BM00000007740)
            Dim Qry As String = "  select TSPL_EX_PI_HEAD.Loading_Port, TSPL_ITEM_MASTER.HSN_Code,TSPL_LOCATION_MASTER.GSTNO as Loc_GST_No,State_For_Location.GST_STATE_Code as Loc_GST_StateCode,TSPL_CUSTOMER_MASTER.GSTNO as Cust_GST_No,State_For_Customer.GST_STATE_Code as Customer_StateCode,convert(int,left(TSPL_EX_PI_HEAD.Total_Amt,len(TSPL_EX_PI_HEAD.Total_Amt)-3)) as docamt1,(Select Value from Tspl_CUstom_field_Values where Transaction_Code=TSPL_EX_PI_HEAD.Document_Code and Custom_Field_Code=(Select Top 1 Code from Tspl_Custom_Field_Head where Name='Shipment Port')) as ShipmentPort ,softaunit1.Add1 as softaadd1,softaunit1.Add2 as softaadd2 ,softaunit1.Add3 as softaadd3,(select STATE_NAME  from TSPL_LOCATION_MASTER  left join TSPL_STATE_MASTER on TSPL_STATE_MASTER.STATE_CODE = softaunit1.state where location_code='002') as softaStateName,tspl_company_Master.Comp_Name as Compname, tspl_bank_master.COUNTRY as Bank_Country,tspl_bank_master.Swift_Code as Bank_Swift_Code,tspl_bank_master.POSTAL as Bank_Pin_No,TSPL_EX_PI_HEAD.EX_Term_Code as EX_Term_Name,tspl_company_Master.Pincode as Comp_Pin_No,tspl_company_Master.City_Code as Comp_City_Name,tspl_company_Master.State as Comp_State_Code,TSPL_STATE_MASTER_For_Comp.STATE_NAME as Comp_State_Name , State_For_Location.STATE_NAME  as Location_State_Name,TSPL_EX_PI_HEAD.is_Partshipment ,TSPL_EX_PI_HEAD.is_Transshipment,TSPL_EX_PI_HEAD.comments,tspl_bank_branch_master.branch_name,tspl_bank_master.Description as Bank_Desc,tspl_bank_master.add1 as Bank_Add1,tspl_bank_master.add2 as Bank_Add2,tspl_bank_master.add3 as Bank_add3,tspl_bank_master.ADD4 as Bank_Add4,tspl_bank_master.City as Bank_city_code,TSPL_COMPANY_MASTER.Pan_No ,tspl_bank_master.state as Bank_State_code,tspl_bank_master.Postal as Bank_Postal,tspl_bank_master.CONTACT as Bank_Contact,tspl_bank_master.fax as Bank_Fax ,tspl_bank_master.BankAccNumber,tspl_bank_branch_master.IFSC_Code as bank_IFSC_Code, TSPL_EX_PI_head.Bank_Code,TSPL_EX_PI_head.Branch_code,tspl_ITEM_master.ITF_CODE as HSCode,TSPL_EX_PI_DETAIL.shipping_mark,TSPL_EX_PI_DETAIL.Specification,TSPL_EX_PI_DETAIL.Packing_inst,TSPL_CURRENCY_MASTER.CURRENCY_SIGN,"
            If clsCommon.CompairString(objCommonVar.CurrentCompanyCode, "UDL") = CompairStringResult.Equal Then
                Qry += " (case when (select count(*) from TSPL_EX_PI_HEAD_HISTORY where TSPL_EX_PI_HEAD.document_code=TSPL_EX_PI_HEAD_HISTORY.document_code)>0 then 'PROFORMA INVOICE' else 'PROFORMA INVOICE' end) as Header_Text "
            Else
                Qry += " (case when (select count(*) from TSPL_EX_PI_HEAD_HISTORY where TSPL_EX_PI_HEAD.document_code=TSPL_EX_PI_HEAD_HISTORY.document_code)>0 then 'AMENDED PROFORMA INVOICE' else 'PROFORMA INVOICE' end) as Header_Text "
            End If


            Qry += " ,TSPL_LOCATION_MASTER.Location_Desc, Total_Add_Charge,TSPL_EX_PI_DETAIL.Total_Disc_Amt,'" + strSoNo + "' as SONo, '" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_LOCATION_MASTER.Add1 as Location_Add1,TSPL_LOCATION_MASTER.Add1 as [Location Address],TSPL_LOCATION_MASTER.Add2 as Location_Add2,TSPL_LOCATION_MASTER.Add3 as Location_Add3,TSPL_LOCATION_MASTER.Add4 as Location_Add4,TSPL_LOCATION_MASTER.TIN_No ,ISNULL(TSPL_LOCATION_MASTER.Phone1,'')+ Case When ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'' Then ', '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as Location_Phone, TSPL_SHIP_TO_LOCATION.Ship_To_Desc as shipName, TSPL_SHIP_TO_LOCATION.Ship_To_Desc as shipName, TSPL_SHIP_TO_LOCATION.add1 as ship_Add1, TSPL_SHIP_TO_LOCATION.Add2 as ship_add2 ,TSPL_SHIP_TO_LOCATION.Add3 as ship_add3  ,TSPL_SHIP_TO_LOCATION.Pin_Code,TSPL_CITY_MASTER.STATE_CODE  ,TSPL_CITY_MASTER.City_Name,"
            Qry += "TSPL_EMPLOYEE_MASTER.Emp_Name as SalesManName,TSPL_EX_PI_HEAD.Inv_No, TSPL_EX_PI_HEAD.Dept_Desc , TSPL_EX_PI_HEAD.Remarks ,  TSPL_EX_PI_HEAD.Terms_Code,TSPL_TERMS_MASTER.Terms_Desc as Term_Desc,TSPL_EX_PI_HEAD.VehicleNo,TSPL_EX_PI_HEAD.Vehicle_Code,TSPL_VEHICLE_MASTER.Description as Vehicle_Desc , "
            Qry += " TSPL_EX_PI_DETAIL .Specification as  specification, TSPL_EX_PI_HEAD.Document_Code as DocNo , TSPL_EX_PI_HEAD.Description,TSPL_EX_PI_HEAD.CUST_PO_NO,convert(varchar,TSPL_EX_PI_HEAD.CUST_PO_DATE,103) as CUST_PO_DATE, "
            Qry += " TSPL_EX_PI_HEAD.Import_Export_No,TSPL_EX_PI_HEAD.Exporter_Ref_No,(case when TSPL_EX_PI_HEAD.Payment_Terms='LC' then 'Letter of Credit' else case when TSPL_EX_PI_HEAD.Payment_Terms='DA' then 'Document against Acceptance' else case when TSPL_EX_PI_HEAD.Payment_Terms='DP' then 'Document against Payment' else '' end end end) as Payment_Terms,TSPL_EX_PI_HEAD.Pre_Carriage_By,TSPL_EX_PI_HEAD.Carrier as place_of_pre_carriage,TSPL_EX_PI_HEAD.Discharge_Port,TSPL_EX_PI_HEAD.Final_Destination,TSPL_EX_PI_HEAD.Origin_Country,TSPL_EX_PI_HEAD.Final_Destination_Country,(case when TSPL_EX_PI_HEAD.EX_Term_Code='CIF' then 'Cost, Insurance & Freight' else case when TSPL_EX_PI_HEAD.EX_Term_Code='CFR' then 'Cost & Freight' else case when TSPL_EX_PI_HEAD.EX_Term_Code='FOB' then 'Free on Board'else case when TSPL_EX_PI_HEAD.EX_Term_Code='EPH' then 'EX-Works,Palwal-Haryana' else case when TSPL_EX_PI_HEAD.EX_Term_Code='C&F' then 'Cost & Freight' else TSPL_EX_PI_HEAD.EX_Term_Code end end end end end) as EX_Term_Code,(case when TSPL_EX_PI_DETAIL.Shipping_Mark='1' then 'Marked' else '' end) as Shipping_Mark, "
            Qry += " TSPL_EX_PI_HEAD.Description,convert(varchar, TSPL_EX_PI_HEAD .Document_Date,103) as Document_Date , TSPL_EX_PI_HEAD.Against_SO_No as Against_Shipment_No, TSPL_EX_PI_HEAD.Item_Type ,  TSPL_EX_PI_HEAD.Customer_Code, "
            Qry += " TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Add1 as Customer_Add1,TSPL_CUSTOMER_MASTER.add2 as customer_Add2,TSPL_CUSTOMER_MASTER.Add3 as customer_Add3 ,TSPL_CUSTOMER_MASTER.State as customer_city_State ,TSPL_CUSTOMER_MASTER.PIN_Code as Customer_Pin_Code,COALESCE(TSPL_SHIP_TO_LOCATION.CST_No,TSPL_CUSTOMER_MASTER.CST) as Cust_CST_No,COALESCE(TSPL_SHIP_TO_LOCATION.Tin_No,TSPL_CUSTOMER_MASTER.Tin_No)as Cust_Tin_No,TSPL_CUSTOMER_MASTER.Contact_Person_Name as Cust_Contact_Name,  Case when len(ISNULL(TSPL_CUSTOMER_MASTER.Contact_Person_Phone ,''))>0 and TSPL_CUSTOMER_MASTER.Contact_Person_Phone ='(+__)__________' then '' else ' '+TSPL_CUSTOMER_MASTER.Contact_Person_Phone end as Cust_Contact_Number , TSPL_EX_PI_HEAD .Terms_Code as termscode ,TSPL_EX_PI_HEAD .Ref_No as ref_no ,"
            Qry += " TSPL_EX_PI_HEAD .Comments as comments ,TSPL_EX_PI_HEAD.Status ,TSPL_EX_PI_HEAD.On_Hold ,TSPL_EX_PI_HEAD.Comp_Code ,TSPL_EX_PI_HEAD.Due_Date ,TSPL_EX_PI_HEAD.Posting_Date ,TSPL_EX_PI_HEAD.Carrier ,TSPL_EX_PI_HEAD.GENO as GRNo ,TSPL_EX_PI_HEAD.GENo ,TSPL_EX_PI_HEAD.GEDate ,TSPL_EX_PI_HEAD.Add_Charge_Code1 ,TSPL_EX_PI_HEAD.Add_Charge_Name1 ,TSPL_EX_PI_HEAD.Challan_No ,TSPL_EX_PI_HEAD.Challan_Date ,TSPL_EX_PI_HEAD.Inv_Date ,TSPL_EX_PI_HEAD.CURRENCY_CODE ,TSPL_EX_PI_HEAD.ConvRate ,TSPL_EX_PI_HEAD.ApplicableFrom ,TSPL_EX_PI_HEAD.Against_C_Form ,TSPL_EX_PI_HEAD.CFormApplied ,TSPL_EX_PI_HEAD.CFormRecd ,TSPL_EX_PI_HEAD.PROJECT_ID ,TSPL_EX_PI_HEAD.Price_code ,TSPL_EX_PI_HEAD.Route_No ,TSPL_EX_PI_HEAD.Route_Desc ,TSPL_EX_PI_HEAD.HeadDisc_Per ,TSPL_EX_PI_HEAD.HeadDisc_Amt ,TSPL_EX_PI_HEAD.TotCashDiscAmt ,  TSPL_EX_PI_HEAD .Discount_Amt as dis_amt,(case when Scheme_Item='Y' then (TSPL_EX_PI_DETAIL.Disc_Amt+TSPL_EX_PI_DETAIL.Amount) else (TSPL_EX_PI_DETAIL.Disc_Amt) end) as dis_amt1,"
            Qry += " TSPL_EX_PI_HEAD.Amount_Less_Discount  as aftrdiscount ,TSPL_EX_PI_HEAD .Total_Amt as Total_amount,"
            Qry += " TSPL_EX_PI_HEAD.Discount_Base as bfrdisc_amount,TSPL_LOCATION_MASTER.City_Code  as Location_City_Code,TSPL_LOCATION_MASTER.State as Location_State,TSPL_LOCATION_MASTER.Pin_Code as Location_Pin_Code,TSPL_LOCATION_MASTER.Country as Location_Country,TSPL_LOCATION_MASTER.Email as Location_Email,TSPL_LOCATION_MASTER.Location_Type ,TSPL_LOCATION_MASTER.Loc_Status ,TSPL_LOCATION_MASTER.Status_Date   as Location_Status_Date,TSPL_LOCATION_MASTER.Excisable as Location_Excisable,TSPL_LOCATION_MASTER.Loc_Segment_Code ,TSPL_LOCATION_MASTER.Type as Location_Type,TSPL_LOCATION_MASTER.Purchase_Tax_Group as Location_Purchase_Tax_Group,TSPL_LOCATION_MASTER.Sales_Tax_Group as Location_Sales_Tax_Group,TSPL_LOCATION_MASTER.Ecc_Number  as Location_Ecc_Number,TSPL_LOCATION_MASTER.Registration_Number as Location_Registration_Number ,TSPL_LOCATION_MASTER.Commissionerate as Location_Commissionerate ,TSPL_LOCATION_MASTER.Range_Code as Location_Range_Code ,TSPL_LOCATION_MASTER.Range_Name as Location_Range_Name ,TSPL_LOCATION_MASTER.Range_Address as Location_Range_Address,TSPL_LOCATION_MASTER.Division_Code as Location_Division_Code,TSPL_LOCATION_MASTER.Division_Name as Location_Division_Name,TSPL_LOCATION_MASTER.Division_Address as Location_Division_Address,TSPL_LOCATION_MASTER.TAN_No as Location_TAN_No,TSPL_LOCATION_MASTER.TCAN_No as Location_TCAN_No,TSPL_LOCATION_MASTER.Service_Tax_Reg_No as Location_Service_Tax_Reg_No,TSPL_LOCATION_MASTER.DutyPaid as Location_DutyPaid,TSPL_LOCATION_MASTER.Purchase_Tax_GroupIS as Location_Purchase_Tax_GroupIS,TSPL_LOCATION_MASTER.Sales_Tax_GroupIS as Location_Sales_Tax_GroupIS,TSPL_LOCATION_MASTER.Stock_Transfer_Filled_Ac as Location_Stock_Transfer_Filled_Ac,TSPL_LOCATION_MASTER.GIT_Location as Location_GIT_Location,TSPL_LOCATION_MASTER.GIT_Type as Location_GIT_Type,TSPL_LOCATION_MASTER.CST_No as Location_CST_No,TSPL_LOCATION_MASTER.Telphone as Location_PhoneNo,TSPL_LOCATION_MASTER.TAN_No as Location_FaxNo , "
            Qry += " tax1.Tax_Code_Desc as tax1name,isnull (TSPL_EX_PI_HEAD.tax1_amt,0) as txt1amt,  "
            Qry += " tax2.Tax_Code_Desc as tax2name,isnull (TSPL_EX_PI_HEAD.tax2_amt,0) as txt2amt,  "
            Qry += " tax3.Tax_Code_Desc as tax3name,isnull (TSPL_EX_PI_HEAD.tax3_amt,0) as txt3amt,  "
            Qry += " tax4.Tax_Code_Desc as tax4name,isnull (TSPL_EX_PI_HEAD.tax4_amt,0) as txt4amt,  "
            Qry += " tax5.Tax_Code_Desc as tax5name,isnull (TSPL_EX_PI_HEAD.tax5_amt,0) as txt5amt,  "
            Qry += " tax6.Tax_Code_Desc as tax6name,isnull (TSPL_EX_PI_HEAD.tax6_amt,0) as txt6amt,  "
            Qry += " tax7.Tax_Code_Desc as tax7name,isnull (TSPL_EX_PI_HEAD.tax7_amt,0) as txt7amt,  "
            Qry += " tax8.Tax_Code_Desc as tax8name,isnull (TSPL_EX_PI_HEAD.tax8_amt,0) as txt8amt,   "
            Qry += " tax9.Tax_Code_Desc as tax9name,isnull (TSPL_EX_PI_HEAD.tax9_amt,0) as txt9amt,  "
            Qry += " tax10.Tax_Code_Desc as tax10name,isnull (TSPL_EX_PI_HEAD.tax10_amt,0) as txt10amt,TSPL_EX_PI_DETAIL.TAX1_Rate,  "
            Qry += " isnull(TSPL_EX_PI_HEAD .Total_Tax_Amt,0) as total_tax_amt, TSPL_EX_PI_HEAD.Total_Amt as DocAmt, Case when len(ISNULL(TSPL_COMPANY_MASTER.Phone1,''))>0 and TSPL_COMPANY_MASTER.Phone1='(+__)__________' then '' else ' '+TSPL_COMPANY_MASTER.Phone1 end   +  Case When   ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'(+__)__________' Then '  '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as Phone,TSPL_COMPANY_MASTER.Fax,TSPL_COMPANY_MASTER.Email ,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,ISNULL(tspl_company_Master.ADD1,'') as address1,ISNULL(tspl_company_Master.ADD2,'') as address2,ISNULL(tspl_company_Master.ADD3,'') as address3,"
            Qry += " TSPL_EX_PI_DETAIL.item_code as item_code,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION as PrincipleDesc,TSPL_CUSTOMER_ITEM_MAPPING.CUSTOMER_ITEM_NO, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_EX_PI_DETAIL.Row_Type,TSPL_EX_PI_DETAIL.Qty as qty,TSPL_EX_PI_DETAIL.unit_code as uom,TSPL_EX_PI_DETAIL.item_cost as itemcost,TSPL_EX_PI_DETAIL.amount as amount,TSPL_EX_PI_HEAD.TAX1,TSPL_EX_PI_HEAD.TAX2,TSPL_EX_PI_HEAD.TAX3,TSPL_EX_PI_HEAD.TAX4,TSPL_EX_PI_HEAD.TAX5,TSPL_EX_PI_HEAD.Total_Add_Charge,TSPL_EX_PI_DETAIL.Batch_No,cast(datepart(MONTH,TSPL_EX_PI_DETAIL.MFG_Date) as varchar(2)) + '/' + "
            Qry += " SUBSTRING(cast(datepart(YYYY,TSPL_EX_PI_DETAIL.MFG_Date) as varchar(4)),3,2) as MFG_Date,"
            Qry += " cast(datepart(MONTH,TSPL_EX_PI_DETAIL.Expiry_Date) as varchar(2)) + '/' + "
            Qry += " SUBSTRING(cast(datepart(YYYY,TSPL_EX_PI_DETAIL.Expiry_Date) as varchar(4)),3,2) as Exp_Date,TSPL_EX_PI_DETAIL.mrp,TSPL_EX_PI_DETAIL.Scheme_Item,TSPL_EX_PI_DETAIL.Item_Weight,(case when COALESCE(TSPL_EX_PI_HEAD.Invoice_Type,'R')='T' then 'Tax Invoice' else 'Retail Invoice' end) as Invoice_Type " & colsTaxRateType & " "
            Qry += " " & QryShowStatus & " "
            Qry += " " & SerialNoColumn & ", Notify.customer_name as Notify_Party,Notify.add1 as Notify_add1,Notify.add2 as Notify_add2,Notify.add3 as Notify_add3,Case when len(ISNULL(Notify.Phone1,''))>0 and Notify.Phone1='(+__)__________' then '' else ' '+Notify.Phone1 end  as Notify_Phone,Notify.pin_code as Notify_pin_Code,Notify.city_name as Notify_City,Notify.state_name as Notify_state,BRAND,[MAIN GROUP] "
            Qry += " from TSPL_EX_PI_DETAIL  "
            Qry += " " & SerialNo & " "
            Qry += " left outer join TSPL_EX_PI_HEAD  on TSPL_EX_PI_HEAD.Document_Code  =TSPL_EX_PI_DETAIL.Document_Code   "
            Qry += " left outer join TSPL_CURRENCY_MASTER on TSPL_CURRENCY_MASTER.currency_code=tspl_ex_pi_head.currency_code"
            Qry += " left outer join  TSPL_SHIP_TO_LOCATION on TSPL_SHIP_TO_LOCATION.Ship_To_Code =TSPL_EX_PI_HEAD .Ship_To_Location "
            Qry += " left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code =TSPL_SHIP_TO_LOCATION.City_Code "
            Qry += " left outer join TSPL_EMPLOYEE_MASTER on TSPL_EMPLOYEE_MASTER.EMP_CODE =TSPL_EX_PI_HEAD.Salesman_Code "
            Qry += " left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_EX_PI_HEAD.tax1  "
            Qry += " left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_EX_PI_HEAD.tax2  "
            Qry += " left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_EX_PI_HEAD .TAX3  "
            Qry += " left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_EX_PI_HEAD .tax4  "
            Qry += " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_EX_PI_HEAD .tax5  "
            Qry += " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_EX_PI_HEAD .TAX6  "
            Qry += " left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_EX_PI_HEAD .TAX7  "
            Qry += " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_EX_PI_HEAD .TAX8  "
            Qry += " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_EX_PI_HEAD .TAX9 "
            Qry += " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_EX_PI_HEAD .TAX10     "
            Qry += " left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_EX_PI_HEAD.comp_code  "
            Qry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_EX_PI_HEAD.Customer_Code   "
            Qry += " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER .Location_Code=  TSPL_EX_PI_HEAD.Bill_To_Location "
            Qry += " left outer join TSPL_STATE_MASTER  as State_For_Location on State_For_Location.STATE_CODE  =TSPL_LOCATION_MASTER.state "
            Qry += " left outer join TSPL_STATE_MASTER  as State_For_Customer on State_For_Customer.STATE_CODE  =TSPL_CUSTOMER_MASTER.State "
            Qry += " left join TSPL_TERMS_MASTER on TSPL_EX_PI_HEAD.Terms_Code=TSPL_TERMS_MASTER.Terms_Code "
            Qry += " left join TSPL_VEHICLE_MASTER on TSPL_EX_PI_HEAD.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id"
            Qry += " left join TSPL_CUSTOMER_ITEM_MAPPING on TSPL_EX_PI_DETAIL.Item_Code=TSPL_CUSTOMER_ITEM_MAPPING.item_no "
            Qry += " and TSPL_EX_PI_HEAD.Customer_Code=TSPL_CUSTOMER_ITEM_MAPPING.Customer_Code "
            ...
            Qry += "  left outer join tspl_bank_master on TSPL_EX_PI_head.Bank_code=tspl_bank_master.Bank_code"
            Qry += " left outer join tspl_bank_branch_master on tspl_bank_branch_master.Bank_CODE =tspl_bank_master.Bank_code"
            Qry += "  left outer join TSPL_STATE_MASTER as  TSPL_STATE_MASTER_For_Comp on TSPL_STATE_MASTER_For_Comp.STATE_CODE =TSPL_COMPANY_MASTER .State Left Outer Join TSPL_LOCATION_MASTER as softaunit1 on softaunit1.Location_Code='002' "
            Qry += " left outer join (select  top 1 TSPL_EX_NOTIFY_PARTY_DETAIL.Document_code,TSPL_EX_NOTIFY_PARTY_DETAIL.Cust_Code,TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Add1,TSPL_CUSTOMER_MASTER.Add2,TSPL_CUSTOMER_MASTER.Add3,TSPL_CUSTOMER_MASTER.Phone1,TSPL_CUSTOMER_MASTER.PIN_Code,TSPL_CITY_MASTER.City_Name,TSPL_STATE_MASTER.STATE_NAME,TSPL_EX_NOTIFY_PARTY_DETAIL.Location_Code from TSPL_EX_NOTIFY_PARTY_DETAIL left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_EX_NOTIFY_PARTY_DETAIL.Cust_Code left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code=TSPL_CUSTOMER_MASTER.City_Code left outer join TSPL_STATE_MASTER on TSPL_STATE_MASTER.STATE_CODE=TSPL_CUSTOMER_MASTER.State where TSPL_EX_NOTIFY_PARTY_DETAIL.Document_code='" + strDocNo + "') Notify  on Notify.document_code=TSPL_EX_PI_HEAD.document_code  "

            Qry += " left join (select pvttable.BRAND,'' as [MAIN GROUP] ,Item_Code  from (select distinct TSPL_ITEM_CATEGORY_LEVEL_VALUES.Item_Category_Code,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION,TSPL_ITEM_MASTER.Item_code    from TSPL_ITEM_MASTER  left join TSPL_ITEM_MASTER_CATEGORY on TSPL_ITEM_MASTER_CATEGORY.Item_code =TSPL_ITEM_MASTER .Item_Code "
            Qry += " left outer join TSPL_ITEM_CATEGORY_LEVEL on TSPL_ITEM_CATEGORY_LEVEL.ITEM_CATEGORY_CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Category_Code and isnull(TSPL_ITEM_CATEGORY_LEVEL.form_type,'item')='Item'  "
            Qry += " left outer join TSPL_ITEM_CATEGORY_LEVEL_VALUES on TSPL_ITEM_CATEGORY_LEVEL_VALUES.CODE=TSPL_ITEM_MASTER_CATEGORY.Item_Cagetory_Values  "
            Qry += " and TSPL_ITEM_CATEGORY_LEVEL.ITEM_CATEGORY_CODE=TSPL_ITEM_CATEGORY_LEVEL_VALUES.ITEM_CATEGORY_CODE "
            Qry += " and isnull(TSPL_ITEM_CATEGORY_LEVEL_VALUES.form_type,'item')='item' and TSPL_ITEM_CATEGORY_LEVEL_VALUES.form_type=TSPL_ITEM_CATEGORY_LEVEL.form_type"
            Qry += " left outer join TSPL_ITEM_CATEGORY_STRUCT_DETAIL on TSPL_ITEM_CATEGORY_STRUCT_DETAIL.item_category_code=TSPL_ITEM_CATEGORY_LEVEL.item_category_code"
            Qry += " and TSPL_ITEM_CATEGORY_LEVEL.form_type=TSPL_ITEM_CATEGORY_STRUCT_DETAIL.form_type and TSPL_ITEM_CATEGORY_LEVEL_VALUES.item_category_code=TSPL_ITEM_CATEGORY_STRUCT_DETAIL.item_category_code "
            Qry += " and TSPL_ITEM_CATEGORY_STRUCT_DETAIL.form_type=TSPL_ITEM_CATEGORY_LEVEL_VALUES.form_type  )as d"
            Qry += " pivot (max(Description) for d.Item_Category_Code in (" + strPivotVar + ")) as pvttable) as m on m.Item_Code =TSPL_ITEM_MASTER.Item_Code and m.Item_Code=TSPL_EX_PI_DETAIL.Item_Code"
            ''======================================================
            Qry += " where 2=2 and  TSPL_EX_PI_HEAD.Document_Code = '" + strDocNo + "' order by TSPL_EX_PI_DETAIL.line_no"
            ...
            If dt.Rows.Count > 0 Then
                ...
                'If rmFormat1.ClickMode = True Then
    ...
    Function GetTaxRateTypeDT(ByVal DocNo As String, Optional ByVal trans As SqlTransaction = Nothing) As DataTable
        ...
        qry = " select distinct * from (" & _
              " select distinct TAX1 as Tax_RateType_Name,TAX1_Rate as Tax_RateType_Rate,sum(TAX1_Amt) as Tax_RateType_Amount  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX1,TAX1_Rate " & _
              " union all " & _
              " select distinct TAX2,TAX2_Rate,sum(TAX2_Amt) as TAX2_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX2,TAX2_Rate " & _
              " union all " & _
              " select distinct TAX3,TAX3_Rate,sum(TAX3_Amt) as TAX3_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX3,TAX3_Rate " & _
              " union all " & _
              " select distinct TAX4,TAX4_Rate,sum(TAX4_Amt) as TAX4_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX4,TAX4_Rate " & _
              " union all " & _
              " select distinct TAX5,TAX5_Rate,sum(TAX5_Amt) as TAX5_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX5,TAX5_Rate " & _
              " union all " & _
              " select distinct TAX6,TAX6_Rate,sum(TAX6_Amt) as TAX6_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX6,TAX6_Rate " & _
              " union all " & _
              " select distinct TAX7,TAX7_Rate,sum(TAX7_Amt) as TAX7_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX7,TAX7_Rate " & _
              " union all " & _
              " select distinct TAX8,TAX8_Rate,sum(TAX8_Amt) as TAX8_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX8,TAX8_Rate " & _
              " union all " & _
              " select distinct TAX9,TAX9_Rate,sum(TAX9_Amt) as TAX9_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX9,TAX9_Rate " & _
              " union all " & _
              " select distinct TAX10,TAX10_Rate,sum(TAX10_Amt) as TAX1_Amt  from TSPL_EX_PI_DETAIL where DOCUMENT_CODE='" & DocNo & "' group by TAX10,TAX10_Rate " & _
    ...
    Private Function SetItemWiseTax(ByVal dtAfterModify As DataTable, ByVal strShipFrm As String) As DataTable
        ...
        qry += " from TSPL_EX_PI_DETAIL where Document_Code='" + strShipFrm + "' "
        qry += " union all "
    ...
    Public Sub funrejprint()
        Try
            Dim qry As String = "SELECT TSPL_EX_PI_HEAD.Document_Code, TSPL_EX_PI_HEAD.Document_Date, TSPL_EX_PI_HEAD.Customer_Name, TSPL_EX_PI_HEAD.Ship_To_Location, TSPL_EX_PI_HEAD.Bill_To_Location, TSPL_EX_PI_HEAD.RMDA_No, TSPL_EX_PI_HEAD.RMDA_Date,TSPL_EX_PI_HEAD.Remarks,TSPL_EX_PI_HEAD.Description, TSPL_SRN_DETAIL.Item_Code,   TSPL_SRN_DETAIL.Item_Desc, TSPL_SRN_DETAIL.Rejected_Qty, TSPL_SRN_DETAIL.Item_Cost,TSPL_SRN_DETAIL.Unit_code,TSPL_SRN_DETAIL.Rejected_Qty*TSPL_SRN_DETAIL.Item_Cost as Amount,TSPL_COMPANY_MASTER.comp_name,TSPL_COMPANY_MASTER.Logo_Img, TSPL_COMPANY_MASTER.Logo_Img2  FROM TSPL_EX_PI_HEAD INNER JOIN TSPL_SRN_DETAIL ON TSPL_EX_PI_HEAD.Document_Code = TSPL_SRN_DETAIL.Document_Code LEFT OUTER JOIN     TSPL_COMPANY_MASTER ON TSPL_EX_PI_HEAD.Comp_Code = TSPL_COMPANY_MASTER.Comp_Code  where TSPL_EX_PI_HEAD.Document_Code='" + txtDocNo.Value + "'"
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        Try
            ...
            Dim dblMRPAmt As Double = clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colMRP).Value) * dblQty
            ...
            gv1.Rows(IntRowNo).Cells(colItemWeight).Value = clsItemMaster.GetItemWeightValue(gv1.Rows(IntRowNo).Cells(colICode).Value, Nothing)
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("EXPISALEFND", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
    ...
    Private Sub txtCurrencyCode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtCurrencyCode._MYValidating
        ...
        txtCurrencyCode.Value = clsCommon.ShowSelectForm("EXPICURFND", qry, "Code", "", txtCurrencyCode.Value, "CURRENCY_CODE", isButtonClicked)
        ShowCurrencyDetail()
    End Sub
    ...
    Private Sub btnDrillDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDrillDown.Click
        If clsCommon.myLen(txtReqNo.Value) > 0 Then
            If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmProformaInvoiceMT) = CompairStringResult.Equal Then
                clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmSalesOrderMT, txtReqNo.Value)
            Else
                clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXSalesOrder, txtReqNo.Value)
            End If
...
#End Region
    ...
    Private Sub btnHistory_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnHistory.Click
        ...
        Try
            ...
            If frm.isPasswordCorrect Then
                ...
                qry = "select count(*) from tspl_sd_sale_invoice_head where Against_PI_No='" + txtDocNo.Value + "'"
                ...
                qry = "select count(*) from TSPL_EX_COMMERCIAL_INVOICE_DETAIL where PI_Code='" + txtDocNo.Value + "'"
                ...
                qry = "select count(*) from TSPL_PURCHASE_ORDER_HEAD where MT_PI_No ='" + txtDocNo.Value + "'"
                check = clsDBFuncationality.getSingleValue(qry, trans)
                ...
                qry = "insert into TSPL_EX_PI_HEAD_HISTORY select * from TSPL_EX_PI_HEAD where document_code='" + txtDocNo.Value + "'"
                ...
                qry = "Update TSPL_EX_PI_HEAD set Status=0, Posting_Date='" + clsCommon.GetPrintDate(txtDate.Text, "dd/MMM/yyyy hh:mm tt") + "',modify_date='" + clsCommon.GetPrintDate(txtDate.Text, "dd/MMM/yyyy hh:mm tt") + "',Modify_By='" + objCommonVar.CurrentUserCode + "' where document_code='" + txtDocNo.Value + "'"
                clsDBFuncationality.ExecuteNonQuery(qry, trans)
                trans.Commit()
                ...
                clsCommon.MyMessageBoxShow(Me, "Amendment done successfully.", Me.Text)
            Else
                trans.Commit()
            End If
        Catch ex As Exception
    ...
    Private Sub OpenNotify(ByVal isButtonClicked As Boolean)
        ...
        Dim dr As DataRow = clsCommon.ShowSelectFormForRow("NTFNDa", qry)
    ...
    Private Sub txtBankBranchCode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBankBranchCode._MYValidating
    ...
    Private Sub cboItemType_SelectedValueChanged(sender As Object, e As EventArgs) Handles cboItemType.SelectedValueChanged
        ...
        gv1.Rows(gv1.Rows.Count - 1).Cells(colRowType).Value = RowTypeItem
    End Sub
    ''richa agarwal 14/04/2015 Against Merchant trade Changes
    Private Sub cmbDocType_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbDocType.SelectedValueChanged
        If clsCommon.CompairString(clsCommon.myCstr(cmbDocType.SelectedValue), "MT") = CompairStringResult.Equal Then
    ...
    Private Sub TxtBeneficiary__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles TxtBeneficiary._MYValidating
        Dim strwherecls As String = ""
        Dim qry As String = "select Cust_Code as Code,Customer_Name as Name,TSPL_CUSTOMER_MASTER.add1 +case when len(TSPL_CUSTOMER_MASTER.add2)>0 then ', '+TSPL_CUSTOMER_MASTER.add2 else '' end +case when LEN(isnull(TSPL_CUSTOMER_MASTER.Add3,''))>0 then ', '+isnull(TSPL_CUSTOMER_MASTER.Add3,'') else ' ' end + case when LEN(TSPL_CITY_MASTER.City_Name)>0 then ', '+TSPL_CITY_MASTER.City_Name else ' ' end + case when len(TSPL_CUSTOMER_MASTER.State )>0 then TSPL_CUSTOMER_MASTER.State else '' end  as Address,TSPL_CUSTOMER_MASTER.Terms_Code as [Term Code] , TSPL_TERMS_MASTER.Terms_Desc as [Term Description] ,Tax_Group as [Tax Group],Tax_Group_Desc as [Tax Group Description],Salesman_Code as [Salesman Code],Salesman_Desc as Salesman   "
        qry += " from TSPL_CUSTOMER_MASTER "
        qry += " left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER.City_Code=TSPL_CUSTOMER_MASTER.City_Code"
        ...
        If clsCommon.myLen(strwherecls) = 0 Then
            TxtBeneficiary.Value = clsCommon.ShowSelectForm("EXPVDNFND", qry, "Code", " ISNULL(TSPL_CUSTOMER_MASTER.currency_code,'') not in ('','" + objCommonVar.BaseCurrencyCode + "')", TxtBeneficiary.Value, "Code", isButtonClicked)
        Else
            TxtBeneficiary.Value = clsCommon.ShowSelectForm("EXPVNDFND", qry, "Code", " ISNULL(TSPL_CUSTOMER_MASTER.currency_code,'') not in ('','" + objCommonVar.BaseCurrencyCode + "') and TSPL_CUSTOMER_MASTER.Cust_Code in (" + strwherecls + ")", TxtBeneficiary.Value, "Code", isButtonClicked) 'TSPL_CUSTOMER_MASTER.Status='N'

        End If
        qry += " where 2=2 and TSPL_CUSTOMER_MASTER.Cust_Code ='" + TxtBeneficiary.Value + "'"
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            lblBeneficiary.Text = clsCommon.myCstr(dt.Rows(0)("Name"))
        Else
            lblBeneficiary.Text = ""
        End If
    ...
    Private Sub fndPaymenttermsGroup__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles fndPaymenttermsGroup._MYValidating
    ...
    Sub CancelData()
        Try
            ...
            End If
            If clsCommon.CompairString(clsUserMgtCode.frmProformaInvoiceMT, FORMTYPE) = CompairStringResult.Equal Then
                Dim strSaleReturnNo As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT COUNT(LCRequestNo) FROM TSPL_LC_REQUEST_MT WHERE Against ='Against PI' AND PurchaseInvoice_No='" & txtDocNo.Value & "' "))
                If strSaleReturnNo > 0 Then
                    Throw New Exception("You cannot cancelled this document because its LC Request has been created.")
                End If
            End If

            Dim strReceiptCount As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("SELECT count(Document_Code) FROM TSPL_EX_COMMERCIAL_INVOICE_HEAD  WHERE Against_PI_No ='" & txtDocNo.Value & "' "))
            If strReceiptCount > 0 Then
                Throw New Exception("You cannot cancelled this document because its Commercial Invoice & Packing List has been created.")
            End If
            clsEXPorformaInvoiceHead.CancelData(Me.Form_ID, txtDocNo.Value, arrLoc, NavigatorType.Current, IIf(clsCommon.CompairString(clsUserMgtCode.frmProformaInvoiceMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
