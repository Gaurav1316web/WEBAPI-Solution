...
#End Region
    ...
    Private Sub LoadBlankNotifyGrid()
        ...
        repoline1.Width = 100
        repoline1.HeaderImage = Global.XpertERPDairySale.My.Resources.Resources.search4
    ...
    Private Sub LOCATIONRIGTHS()
        ...
        Try
            ...
            If obj IsNot Nothing AndAlso clsCommon.myLen(obj.Default_LocCode) > 0 Then
                ...
                If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmEXSalesOrder) = CompairStringResult.Equal Then
                    WhrCls = " and Location_Type='Physical'  "
                ElseIf clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesOrderMT) = CompairStringResult.Equal Then
    ...
    Private Sub fndProject__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles fndProject._MYValidating
        ...
        fndProject.Value = clsCommon.ShowSelectForm("PrjFND", qry, "Code", "", fndProject.Value, "", isButtonClicked)
    ...
    Private Sub fndRouteNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtRouteNo._MYValidating
        ...
        txtRouteNo.Value = clsCommon.ShowSelectForm("RUTFND", qry, "Code", "", txtRouteNo.Value, "", isButtonClicked)
    ...
    Private Sub FrmAPInvoiceEntry_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            ''richa in case of Merchant Trade
            If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesOrderMT) = CompairStringResult.Equal Then
                RadPageViewPage2.Item.Visibility = ElementVisibility.Collapsed 'hide taxes tab
            End If
            ...
            If clsCommon.myLen(Me.Tag) > 0 Then
                LoadDataDrillDown(clsCommon.myCstr(Me.Tag), NavigatorType.Current)
            End If
    ...
    Sub SetMultiCurrencyVisibility()
        ...
        Else
            ...
            Throw New Exception("Do multicurrency on for Export Sales Order.")
        End If
    End Sub
    ...
    Sub BlankAllControls()
        ...
        txtComment.Text = clsEXSalesOrder.GetTermsAndConditions()
    ...
    Sub LoadBlankGrid()
        ...
        repoICode.HeaderImage = Global.XpertERPDairySale.My.Resources.Resources.search4
        ...
        repoUnit.HeaderImage = Global.XpertERPDairySale.My.Resources.Resources.search4
        ...
        repoQty.Name = colQty
        ''richa agarwal ALF/09/04/19-000097
        'If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesOrderMT) = CompairStringResult.Equal Then
        ...
        repoTotalBasicAmt.FormatString = "{0:n2}"
        ...
        repoRequition.HeaderText = "Quotation Number"
        ...
        Dim repoRequition11 As GridViewTextBoxColumn = New GridViewTextBoxColumn()
        repoRequition11.FormatString = ""
        'repoRequition.HeaderText = "Requition No"
        repoRequition11.HeaderText = "PO Number"
        repoRequition11.Name = colPONO
        repoRequition11.ReadOnly = True
        repoRequition11.Width = 100
        repoRequition11.IsVisible = False
        gv1.MasterTemplate.Columns.Add(repoRequition11)
                ...
                If Not isCellValueChangedOpen Then
                    ...
                    If e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colSchemeApplicable) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse e.Column Is gv1.Columns(colTotTaxAmt) OrElse e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse (e.Column Is gv1.Columns(colAmt)) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal Then
                        ...
                        If (e.Column Is gv1.Columns(colQty) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse e.Column Is gv1.Columns(colDisPer) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse (e.Column Is gv1.Columns(colAmt)) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal) Then
                            If (e.Column Is gv1.Columns(colQty) AndAlso (clsCommon.myLen(gv1.CurrentRow.Cells(colReqistionNo).Value) > 0 OrElse clsCommon.myLen(gv1.CurrentRow.Cells(colPONO).Value) > 0)) Then
                        ...
                        ElseIf e.Column Is gv1.Columns(colICode) Then
                            If clsCommon.myLen(txtVendorNo.Value) <= 0 Then
                                common.clsCommon.MyMessageBoxShow(Me, "Please select Buyer First", Me.Text)
    ...
    Sub OpenUOMList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            ...
            gv1.CurrentRow.Cells(colUnit).Value = clsCommon.ShowSelectForm("EXSUNFND", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), "Code", isButtonClick)
    ...
    Sub OpenICodeListCurrentCalaculation(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(RowTypeItem, strItemType) <> CompairStringResult.Equal Then
            SetBlankOfItemColumns()
            Exit Sub
        End If
        ''richa 
        ...
        If clsCommon.myLen(clsCommon.myCstr(icode)) > 0 Then
            ...
            gv1.CurrentRow.Cells(ColActualBalQty).Value = clsItemLocationDetails.getBalance(clsCommon.myCstr(gv1.CurrentRow.Cells(colICode).Value), txtBillToLocation.Value, txtDocNo.Value, txtDate.Value, Nothing, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), 0) 'clsCommon.myCdbl(dr("MRP"))
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        ...
        gv1.Rows(IntRowNo).Cells(colItemWeight).Value = clsItemMaster.GetItemWeightValue(clsCommon.myCstr(gv1.Rows(IntRowNo).Cells(colICode).Value), Nothing)
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If clsCommon.myLen(txtVendorNo.Value) <= 0 Then
                ...
                txtVendorNo.Focus()
                txtVendorNo.Select()
                Errorcontrol.SetError(lblVendorName, "Please select Customer")
                Return False
            Else
                Errorcontrol.ResetError(lblVendorName)
            End If
            ...
            If clsCommon.myLen(cmbDocType.SelectedValue) <= 0 Then
                ...
                common.clsCommon.MyMessageBoxShow(Me, "Please select order type", Me.Text)
                cmbDocType.Focus()
                cmbDocType.Select()
                Errorcontrol.SetError(cmbDocType, "Please select order type")
                Return False
            Else
            ...
            If Not isNewEntry AndAlso clsCommon.myLen(txtDocNo.Value) <= 0 Then
                RadPageView1.SelectedPage = RadPageViewPage1
                common.clsCommon.MyMessageBoxShow(Me, "Sales Order code Not found to save", Me.Text)
                txtDocNo.Focus()
                txtDocNo.Select()
                Errorcontrol.SetError(txtDocNo, "Sales Order code Not found to save")
                Return False
            Else
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If clsCommon.myLen(strReqNo) > 0 Then
                    If (Not clsEXSalesQuotation.IsValidVendorForRequitionItem(strReqNo, strICode, txtVendorNo.Value)) Then
                        RadPageView1.SelectedPage = RadPageViewPage1
                        gv1.CurrentRow = gv1.Rows(ii)
                        gv1.CurrentColumn = gv1.Columns(colICode)
                        common.clsCommon.MyMessageBoxShow("Buyer :" + lblVendorName.Text + " is not valid for Qutotation No:" + strReqNo + " and Item : " + strIName + " At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                        Return False
                    End If
                    If (Not clsEXSalesQuotation.IsValidDocumentTypeForRequitionItem(strReqNo, cmbDocType.SelectedValue)) Then
                        RadPageView1.SelectedPage = RadPageViewPage1
                        gv1.CurrentRow = gv1.Rows(ii)
                        gv1.CurrentColumn = gv1.Columns(colICode)
                        common.clsCommon.MyMessageBoxShow("Order Type :" + cmbDocType.SelectedValue + " is not valid for Quotation No:" + strReqNo + " and Item : " + strIName + " At Line No: " + clsCommon.myCstr(clsCommon.myCdbl(ii + 1)) + " ")
                        Return False
                    End If
            ...
            If clsCommon.myLen(txtCurrencyCode.Value) <= 0 Then
                ...
                If clsCommon.CompairString(txtCurrencyCode.Value, objCommonVar.BaseCurrencyCode) = CompairStringResult.Equal Then
                ...
                If clsCommon.myLen(txtCurrencyCode.Value) <= 0 Then
                    clsCommon.MyMessageBoxShow(Me, "Select currency for export sale", Me.Text)
    ...
    Function CFormFunction()
        Try
            ...
            If intQuarter = 1 Then
                ...
                intCForm = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select COUNT(Document_Code) from TSPL_SD_SALE_INVOICE_HEAD where trans_type='EXP' and CFormApplied=0 and CFormRecd=0 and Against_C_Form=1 AND Document_Date > ='" & strFromDate & " ' and Document_Date < ='" & strToDate & "' "))
                If intCForm > 0 Then
    ...
    Private Function SaveData(ByVal isDoAbandomentNo As Boolean) As Boolean
        ...
        Try
            If (AllowToSave()) Then
                ...
                obj.Terms_Code = txtCRTermCode.Value
                ...
                For Each grow As GridViewRowInfo In gv_Notify_Party.Rows
                    Dim objtr As New clsEXSalesOrderNotifyParty()
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Try
            ...
            obj = clsEXSalesOrder.GetData(strCode, NavTyep, arrLoc, IIf(clsCommon.CompairString(clsUserMgtCode.frmSalesOrderMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
            ...
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                txtCRTermCode.Value = obj.Terms_Code
                lblCRTermName.Text = obj.TermsName
                ...
                txtComm_Pay_name.Text = obj.Commission_Payee_name
                ...
                txtAdvance_Pers.Enabled = False
                If clsCommon.CompairString(obj.Payment_Terms, "AD") = CompairStringResult.Equal Then
                ...
                If obj.Arr IsNot Nothing AndAlso obj.Arr.Count > 0 Then
                    For Each objTr As clsEXSalesOrderDetail In obj.Arr
                        gv1.Rows.AddNew()
                        ...
                        If clsCommon.myLen(objTr.Quotation_Code) > 0 Then
                            gv1.Rows(gv1.Rows.Count - 1).Cells(colPendingQty).Value = clsEXSalesQuotationDetail.GetBalanceRequitionQty(objTr.Quotation_Code, objTr.Item_Code, obj.Document_Code)
                        End If
                ...
                If obj.Arr_Notify IsNot Nothing AndAlso obj.Arr_Notify.Count > 0 Then
                    For Each objtr As clsEXSalesOrderNotifyParty In obj.Arr_Notify
    ...
    Sub PostData()
        Try
            ...
            If (myMessages.postConfirm()) Then
                ...
                ''=========================================
                If (clsEXSalesOrder.PostData(MyBase.Form_ID, txtDocNo.Value, arrLoc)) Then
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsEXSalesOrder.DeleteData(MyBase.Form_ID, txtDocNo.Value)) Then
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        qry = "select Document_Code as Code,convert(varchar(10),Document_Date,103)  as Date,(case when salesorder_type='EX' then 'Export Sale' else 'Merchant Trade' end) as [Document Type],Customer_Code as [Customer Code], Customer_Name as Customer,Total_Amt as Amount,case when TSPL_SD_SALES_ORDER_HEAD.status='0' then 'Pending' else 'Approved' end as [Status],Bill_To_Location as [Location],(select Location_Desc  from TSPL_LOCATION_MASTER where Location_Code =Bill_To_Location ) as [Location Name] from TSPL_SD_SALES_ORDER_HEAD left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SD_SALES_ORDER_HEAD.Customer_Code "
        Dim whrClas As String = " TSPL_SD_SALES_ORDER_HEAD.trans_type='EXP' "

        If clsCommon.CompairString(clsUserMgtCode.frmSalesOrderMT, FORMTYPE) = CompairStringResult.Equal Then
            whrClas += " and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='MT' "
        Else
            whrClas += " and TSPL_SD_SALES_ORDER_HEAD.salesorder_type='EX' "
        End If

        If clsCommon.myLen(arrLoc) > 0 And clsCommon.myLen(strwherecls) > 0 Then
            whrClas += " and Bill_To_Location in (" + arrLoc + ") and TSPL_SD_SALES_ORDER_HEAD.Customer_Code in (" + strwherecls + ") "
        ...
        End If
        '-----------------------------------------------------
        LoadData(clsCommon.ShowSelectForm("EXSOFND", qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked), NavigatorType.Current)
    End Sub
    ...
    Private Sub FrmAPInvoiceEntry_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
            ...
            ElseIf e.KeyData = Keys.F2 AndAlso gv1.CurrentColumn IsNot Nothing AndAlso gv1.CurrentColumn Is gv1.Columns(colUnit) Then
                OpenUOMList(True)
                UpdateCurrentRow(gv1.CurrentRow.Index) ''-1 is for current row
                UpdateAllTotals()
            ElseIf e.KeyData = Keys.F2 AndAlso gvAC.CurrentColumn IsNot Nothing AndAlso gvAC.CurrentColumn Is gvAC.Columns(colACCode) Then
                ...
                End If
            ElseIf e.KeyData = Keys.F2 AndAlso gv_Notify_Party.CurrentColumn IsNot Nothing AndAlso gv_Notify_Party.CurrentColumn Is gv_Notify_Party.Columns(colNT_Cust_Code) Then
                isCellValueChangedOpen = True
                OpenNotify(True)
                isCellValueChangedOpen = False
            End If
    ...
    Private Sub txtTermCode__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtCRTermCode._MYValidating
        Dim qry As String = "select Terms_Code as Code,Terms_Desc as Description,No_Days as [No Of Days] from TSPL_TERMS_MASTER"
        txtCRTermCode.Value = clsCommon.ShowSelectForm("EXTERMFND", qry, "Code", "", txtCRTermCode.Value, "Code", isButtonClicked)
        SetTermDetails()
    End Sub

    Sub SetTermDetails()
        Dim qry As String = "select Terms_Code as Code,Terms_Desc as Description,No_Days as [No Of Days] from TSPL_TERMS_MASTER where Terms_Code='" + txtCRTermCode.Value + "'"
        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            lblCRTermName.Text = clsCommon.myCstr(dt.Rows(0)("Description"))
    ...
    Private Sub txtTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTaxGroup._MYValidating
        ...
        'txtTaxGroup.Value = clsCommon.ShowSelectForm("EXSOTAXFND", qry, "Code", "Tax_Group_Type='S'", txtTaxGroup.Value, "Code", isButtonClicked)
    ...
    Private Sub txtVendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtVendorNo._MYValidating
        ...
        If clsCommon.myLen(strwherecls) = 0 Then
            txtVendorNo.Value = clsCommon.ShowSelectForm("EXSCUSTFND", qry, "Code", " ISNULL(TSPL_CUSTOMER_MASTER.currency_code,'') not in ('','" + objCommonVar.BaseCurrencyCode + "')", txtVendorNo.Value, "Code", isButtonClicked)
        Else
            txtVendorNo.Value = clsCommon.ShowSelectForm("EXSCUSTFND", qry, "Code", " ISNULL(TSPL_CUSTOMER_MASTER.currency_code,'') not in ('','" + objCommonVar.BaseCurrencyCode + "') and TSPL_CUSTOMER_MASTER.Cust_Code in (" + strwherecls + ")", txtVendorNo.Value, "Code", isButtonClicked)
        ...
        If (dt IsNot Nothing AndAlso dt.Rows.Count > 0) Then
            ...
            txtCRTermCode.Value = clsCommon.myCstr(dt.Rows(0)("Term Code"))
            lblCRTermName.Text = clsCommon.myCstr(dt.Rows(0)("Term Description"))
            ...
            ''richa agarwal 04 Apr,2019 as discussed with Shruti mam
            'txtDate.Enabled = False
            ''---------
            SetMultiCurrencyVisibility()
        Else
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBillToLocation._MYValidating
        ...
        End If
        txtBillToLocation.Value = clsCommon.ShowSelectForm("EXSLOCFND", qry, "Code", WhrCls, txtBillToLocation.Value, "Code", isButtonClicked)
    ...
    Private Sub txtShipToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtShipToLocation._MYValidating
        ...
        txtShipToLocation.Value = clsCommon.ShowSelectForm("EXSHPFND", qry, "Code", "Ship_To_Type_Code='" & clsCommon.myCstr(txtVendorNo.Value) & "' and loc_code='" & clsCommon.myCstr(txtBillToLocation.Value) & "'", txtShipToLocation.Value, "Code", isButtonClicked)
    ...
    Sub SelectRequistionItems()
        ...
        frm.Text = "Pending Sales Quotation"
        frm.VendorCode = txtVendorNo.Value
        frm.strCurrCode = txtDocNo.Value
        frm.strFormType = "EXPORT SO"
        frm.strDocumentType = "EX"
        If clsCommon.CompairString(FORMTYPE, clsUserMgtCode.frmSalesOrderMT) = CompairStringResult.Equal Then
            frm.strDocumentType = "MT"
        End If
        frm.ShowDialog()
        LoadBlankGrid()
        ...
        Dim objOrderHead As clsEXSalesQuotation = Nothing
        If frm.ArrReturn_EX IsNot Nothing AndAlso frm.ArrReturn_EX.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn_EX(0).Document_Code) > 0 Then
                objOrderHead = clsEXSalesQuotation.GetData(frm.ArrReturn_EX(0).Document_Code, NavigatorType.Current, arrLoc, frm.strDocumentType)
                If objOrderHead IsNot Nothing AndAlso clsCommon.myLen(objOrderHead.Document_Code) > 0 Then
                    ...
                    ''RICHA 3 AUG,2017
                    txtDiscPer.Value = objOrderHead.HeadDisc_Per
                    txtDiscAmt.Value = objOrderHead.HeadDisc_Amt
                    If clsCommon.myCdbl(txtDiscPer.Value) > 0 Then
                        chkDiscountOnRate.IsChecked = True
                    Else
                        chkDiscountOnAmt.IsChecked = True
                    End If
                    ...
                    cboModeOfTransport.Text = objOrderHead.Mode_Of_Transport
                    cmbDocType.SelectedValue = objOrderHead.SalesOrder_Type
                    cboItemType.SelectedValue = objOrderHead.Item_Type

                    dtpPODate.Text = objOrderHead.Cust_PO_Date

                    ' If (clsCommon.myLen(txtShipToLocation.Value)) <= 0 Then
                    txtShipToLocation.Value = objOrderHead.Ship_To_Location
                    lblShipToLocation.Text = objOrderHead.ShipToLocationName
                    ' End If
                    If (clsCommon.myLen(cboItemType.SelectedValue) <= 0) Then
                        cboItemType.SelectedValue = objOrderHead.Item_Type
                    End If
                    ...
                    End If
                    If (clsCommon.myLen(txtCRTermCode.Value) <= 0) Then
                        txtCRTermCode.Value = objOrderHead.Terms_Code
                        lblCRTermName.Text = objOrderHead.TermsName
            ...
            For ii As Integer = 0 To frm.ArrReturn_EX.Count - 1
                If clsCommon.myLen(frm.ArrReturn_EX(ii).Document_Code) > 0 Then
                    ...
                    '    arr.Add(strCode)
                    objOrderHead = clsEXSalesQuotation.GetData(frm.ArrReturn_EX(ii).Document_Code, NavigatorType.Current, arrLoc, frm.strDocumentType)
                    For Each obj As clsEXSalesQuotationDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn_EX(ii).Item_Code AndAlso obj.Scheme_Item = "N") OrElse (obj.Scheme_Code = frm.ArrReturn_EX(ii).Scheme_Code AndAlso clsCommon.myLen(obj.Scheme_Code) > 0) Then
                            If IsValidItem(obj) Then
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = frm.ArrReturn_EX(ii).Balance_Qty
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsEXSalesQuotationDetail In objOrderHead.Arr
    ...
    Function IsValidItem(ByVal obj As clsEXSalesQuotationDetail)
        ''If clsCommon.myLen(txtTaxGroup.Value) <= 0 Then
    ...
    Function IsValidPOItem(ByVal obj As clsPurchaseOrderDetail)
        ...
        For ii As Integer = 0 To gv1.RowCount - 1
            ...
            If clsCommon.myLen(strReqCode) > 0 AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.Rows(ii).Cells(colICode).Value), "N") = CompairStringResult.Equal AndAlso clsCommon.CompairString(strReqCode, obj.PurchaseOrder_No) = CompairStringResult.Equal AndAlso clsCommon.CompairString(strICode, obj.Item_Code) = CompairStringResult.Equal Then
                Dim strMsg As String = "Already exist at row no:" + clsCommon.myCstr(ii + 1) + Environment.NewLine + "Order No : " + obj.PurchaseOrder_No + "  Item : " + obj.Item_Desc + Environment.NewLine + ""
                If dblMRP > 0 Then
                    strMsg = strMsg + Environment.NewLine + "MRP : " + clsCommon.myCstr(dblMRP)
                End If
                common.clsCommon.MyMessageBoxShow(strMsg)
            ...
            ElseIf (gv1.CurrentColumn Is gv1.Columns(colComplete) AndAlso (UsLock1.Status = ERPTransactionStatus.Approved)) Then
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 AndAlso clsCommon.myLen(strICode) > 0 AndAlso intSNo > 0 AndAlso clsCommon.CompairString(strStatus, "No") = CompairStringResult.Equal Then
                    If common.clsCommon.MyMessageBoxShow("Do you want to complete the item " + clsCommon.myCstr(gv1.CurrentRow.Cells(colIName).Value), Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                        If clsSNSalesOrderDetail.CompletePO(txtDocNo.Value, strICode, intSNo) Then
    ...
    Private Sub funPrint()
        ...
        Try
            ...
            Qry = " select TSPL_SD_SALES_ORDER_DETAIL.line_no,'" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_SD_SALES_ORDER_HEAD.Cust_PO_No,'" + FooterText + "' as FooterText , TSPL_SD_SALES_ORDER_HEAD.Abandonment_No, TSPL_SD_SALES_ORDER_HEAD.Dept_Desc ,TSPL_SD_SALES_ORDER_HEAD.Delivery_date ,TSPL_SD_SALES_ORDER_HEAD.Remarks ,(case when TSPL_SD_SALES_ORDER_HEAD .status =1 then TSPL_SD_SALES_ORDER_HEAD.Modify_By else '' end) as Modify_By ,TSPL_SD_SALES_ORDER_DETAIL.Landing_Cost,TSPL_SD_SALES_ORDER_HEAD.Created_By , "
            Qry += " TSPL_SD_SALES_ORDER_HEAD.Terms_Code,TSPL_SHIP_TO_LOCATION.Telphone as ShipLoc_telphone,TSPL_LOCATION_MASTER.Service_Tax_Reg_No ,TSPL_SHIP_TO_LOCATION.Ship_To_Type_Desc as ShipLoc_Name,TSPL_SHIP_TO_LOCATION.email as shipLoc_Email,TSPL_LOCATION_MASTER.cst_no as CST_LST,TSPL_LOCATION_MASTER.Registration_Number,TSPL_LOCATION_MASTER.Add1 as loc_add1 ,TSPL_LOCATION_MASTER.add4 as loc_add4,TSPL_LOCATION_MASTER.Add2 as loc_add2,TSPL_LOCATION_MASTER.Add3 as loc_add3,TSPL_LOCATION_MASTER.Email as loc_email,TSPL_LOCATION_MASTER.TAN_No as loc_Fax , TSPL_SD_SALES_ORDER_DETAIL.TAX1_Rate,TSPL_ITEM_CATEGORY_LEVEL_VALUES.DESCRIPTION as PrincipleDesc ,TSPL_COMPANY_MASTER.Add2 as comp_Add2 ,TSPL_COMPANY_MASTER.Add3 as comp_Add3 ,ISNULL(TSPL_COMPANY_MASTER.Phone1,'')+ Case When ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as Company_Phone,TSPL_COMPANY_MASTER.Email as Comp_Email,TSPL_COMPANY_MASTER.Fax as comp_Fax,ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')+ Case When ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'' Then ', '+ TSPL_CUSTOMER_MASTER.Phone2 Else'' End as Cust_Phone ,TSPL_CUSTOMER_MASTER.Email as Cust_Email,TSPL_CUSTOMER_MASTER.Fax as cust_Fax,(TSPL_SD_SALES_ORDER_DETAIL.Total_Tax_Amt /(case when TSPL_SD_SALES_ORDER_DETAIL.Amt_Less_Discount<=0 then 1 else TSPL_SD_SALES_ORDER_DETAIL.Amt_Less_Discount end)) *100 as Tax,((((TSPL_SD_SALES_ORDER_DETAIL.Total_Tax_Amt /(case when TSPL_SD_SALES_ORDER_DETAIL.Amt_Less_Discount<=0 then 1 else TSPL_SD_SALES_ORDER_DETAIL.Amt_Less_Discount end)) *100)*TSPL_SD_SALES_ORDER_DETAIL.Item_Cost/100) +TSPL_SD_SALES_ORDER_DETAIL.Item_Cost) as landing_Rate,(TSPL_SD_SALES_ORDER_DETAIL.Total_Tax_Amt /(case when TSPL_SD_SALES_ORDER_DETAIL.Amt_Less_Discount<=0 then 1 else TSPL_SD_SALES_ORDER_DETAIL.Amt_Less_Discount end) *100) as Tax_Lan,TSPL_SD_SALES_ORDER_HEAD.Mode_Of_Transport as  ModeofTransport,TSPL_SD_SALES_ORDER_DETAIL .Specification as  specification,  "
            Qry += " TSPL_SD_SALES_ORDER_HEAD.Document_Code as DocNo ,TSPL_SHIP_TO_LOCATION.add1 +case when len(TSPL_SHIP_TO_LOCATION.add2)>0 then ', '+TSPL_SHIP_TO_LOCATION.add2 else '' end +case when LEN(isnull(TSPL_SHIP_TO_LOCATION.Add3,''))>0 then ', '+isnull(TSPL_SHIP_TO_LOCATION.Add3,'') else ' ' end  + case when len(TSPL_SHIP_TO_LOCATION.State )>0 then TSPL_SHIP_TO_LOCATION.State else '' end  as Ship_address, TSPL_SHIP_TO_LOCATION.Tin_No as Ship_ToLocation_Tin_No, convert(varchar(10),TSPL_SD_SALES_ORDER_HEAD .Document_Date,103)  as po_date ,case TSPL_SD_SALES_ORDER_HEAD .SalesOrder_Type when 'L'then 'Local' when 'I' then 'Import' when 'J' then 'Job Work' when 'O' then 'Open' when 'S' then 'Specific'else 'Null' end as po_type , "
            Qry += " TSPL_SD_SALES_ORDER_HEAD.Customer_Code, TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_SD_SALES_ORDER_DETAIL.Item_Net_Amt as Total_Item_Net_amt,TSPL_SD_SALES_ORDER_DETAIL.MRP ,TSPL_SD_SALES_ORDER_DETAIL.Item_Cost, TSPL_SD_SALES_ORDER_HEAD .Terms_Code as termscode ,TSPL_SD_SALES_ORDER_HEAD .Ref_No as ref_no ,TSPL_SD_SALES_ORDER_HEAD .Comments as comments , "
            Qry += " TSPL_SD_SALES_ORDER_HEAD .Discount_Amt as dis_amt,TSPL_SD_SALES_ORDER_DETAIL .Disc_Amt  as dis_amt1,TSPL_SD_SALES_ORDER_HEAD.Amount_Less_Discount  as aftrdiscount ,TSPL_SD_SALES_ORDER_HEAD .Total_Amt as Total_amount,TSPL_SD_SALES_ORDER_HEAD.Discount_Base as bfrdisc_amount, "
            Qry += " tax1.Tax_Code_Desc as tax1name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax1_amt,0) as txt1amt, "
            Qry += " tax2.Tax_Code_Desc as tax2name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax2_amt,0) as txt2amt, "
            Qry += " tax3.Tax_Code_Desc as tax3name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax3_amt,0) as txt3amt, "
            Qry += " tax4.Tax_Code_Desc as tax4name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax4_amt,0) as txt4amt, "
            Qry += " tax5.Tax_Code_Desc as tax5name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax5_amt,0) as txt5amt, "
            Qry += " tax6.Tax_Code_Desc as tax6name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax6_amt,0) as txt6amt, "
            Qry += " tax7.Tax_Code_Desc as tax7name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax7_amt,0) as txt7amt, "
            Qry += " tax8.Tax_Code_Desc as tax8name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax8_amt,0) as txt8amt,  "
            Qry += " tax9.Tax_Code_Desc as tax9name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax9_amt,0) as txt9amt, "
            Qry += " tax10.Tax_Code_Desc as tax10name,isnull (TSPL_SD_SALES_ORDER_HEAD.tax10_amt,0) as txt10amt, "
            Qry += " isnull(TSPL_SD_SALES_ORDER_HEAD .Total_Tax_Amt,0) as total_tax_amt, TSPL_SD_SALES_ORDER_HEAD.Total_Amt as DocAmt,  TSPL_COMPANY_MASTER.Comp_Name as compname,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,ISNULL(tspl_company_Master.ADD1,'') as address1,TSPL_SD_SALES_ORDER_DETAIL.item_code as item_code, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_SD_SALES_ORDER_DETAIL.Row_Type,TSPL_SD_SALES_ORDER_DETAIL.Qty as qty,TSPL_SD_SALES_ORDER_DETAIL.scheme_item as freeitem,TSPL_SD_SALES_ORDER_DETAIL.unit_code as uom,TSPL_SD_SALES_ORDER_DETAIL.item_cost as itemcost,TSPL_SD_SALES_ORDER_DETAIL.amount as amount ,"
            ...
            Qry += " left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_SD_SALES_ORDER_HEAD.tax1 "
            Qry += " left outer join  TSPL_SHIP_TO_LOCATION on TSPL_SD_SALES_ORDER_HEAD.Ship_To_Location =TSPL_SHIP_TO_LOCATION.Ship_To_Code "

            Qry += " left outer join tspl_tax_master as tax2 on tax2.tax_code = TSPL_SD_SALES_ORDER_HEAD.tax2 "
            Qry += " left outer join tspl_tax_master as tax3 on tax3.Tax_Code=TSPL_SD_SALES_ORDER_HEAD .TAX3 "
            Qry += " left outer join TSPL_TAX_MASTER as tax4 on tax4.Tax_Code= TSPL_SD_SALES_ORDER_HEAD .tax4 "
            Qry += " left outer join TSPL_TAX_MASTER as tax5 on tax5.Tax_Code=TSPL_SD_SALES_ORDER_HEAD .tax5 "
            Qry += " left outer join TSPL_TAX_MASTER as tax6 on tax6.Tax_Code =TSPL_SD_SALES_ORDER_HEAD .TAX6 "
            Qry += " left outer join TSPL_TAX_MASTER as tax7 on tax7.Tax_Code =TSPL_SD_SALES_ORDER_HEAD .TAX7 "
            Qry += " left outer join TSPL_TAX_MASTER as tax8 on tax8.Tax_Code =TSPL_SD_SALES_ORDER_HEAD .TAX8 "
            Qry += " left outer join TSPL_TAX_MASTER as tax9 on tax9.Tax_Code =TSPL_SD_SALES_ORDER_HEAD .TAX9 "
            Qry += " left outer join TSPL_TAX_MASTER as tax10 on tax10.Tax_Code =TSPL_SD_SALES_ORDER_HEAD .TAX10    "
            Qry += " left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_SD_SALES_ORDER_HEAD.comp_code "
            Qry += " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_SD_SALES_ORDER_HEAD.Customer_Code  "
            Qry += " left outer join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER .Location_Code=  TSPL_SD_SALES_ORDER_HEAD.Bill_To_Location"
            Qry += " Left Outer Join TSPL_ITEM_MASTER ON TSPL_ITEM_MASTER.Item_Code=TSPL_SD_SALES_ORDER_DETAIL.Item_Code "
    ...
    Sub LoadBlankGridAC()
        ...
        repoACCode.HeaderImage = Global.XpertERPDairySale.My.Resources.Resources.search4
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("EXSALEFND", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
    ...
    Private Sub txtCurrencyCode__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtCurrencyCode._MYValidating
        ...
        txtCurrencyCode.Value = clsCommon.ShowSelectForm("EXCURRENCY_MASTER", qry, "Code", "", txtCurrencyCode.Value, "CURRENCY_CODE", isButtonClicked)
        ShowCurrencyDetail()
    End Sub
        ...
        If chkDiscountOnAmt.IsChecked Then
            For Each gro As GridViewRowInfo In gv1.Rows
                ...
                If clsCommon.myLen(gro.Cells(colICode).Value) > 0 Then
                    If clsCommon.myCdbl(gro.Cells(ColFOC).Value) = 0 AndAlso clsCommon.myCdbl(lblAmtWithDiscount.Text) > 0 Then
...
#Region "New Mail System"
    Private Sub RadMenuItem3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem3.Click
        ...
        frm.FormId = clsUserMgtCode.frmSNSalesOrder
        frm.ShowDialog()
    End Sub
    ...
    Private Sub SendSMSandEmail(ByVal lstUsers As List(Of String), ByVal isSendForApproval As Boolean)
        Try
            'Dim obj As clsEmailSMSSettingNew = clsEmailSMSSettingNew.GetData(clsUserMgtCode.frmEXSalesOrder)
    ...
    Private Sub SMSSENDONLY(ByVal isPost As Boolean)
        Try
            ...
            'If clsSMSSend.SendSMS(clsUserMgtCode.frmEXSalesOrder, strMes, strphone) Then
            '    If Not isPost Then
...
#End Region
    ...
    Private Sub chkclose_ToggleStateChanged(ByVal sender As System.Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles chkclose.ToggleStateChanged
        ...
        Dim qry As String = "select count(*) from TSPL_SD_SALES_ORDER_HEAD where trans_type='EXP' and document_code='" + txtDocNo.Value + "'"
        Dim check As Integer = clsDBFuncationality.getSingleValue(qry)
        If check <= 0 Then
            clsCommon.MyMessageBoxShow(Me, "No data found for closed", Me.Text)
            Return
        End If
        ...
        If clsEXSalesOrder.ClosedData(obj, txtDocNo.Value, "") Then
            If chkclose.Checked Then
                clsCommon.MyMessageBoxShow(Me, "Document closed successfully", Me.Text)
                vaddnew = "N"
                chkclose.Checked = True
                vaddnew = "Y"
                btnSave.Enabled = False
            ...
            End If
            If Not chkclose.Checked And btnSave.Enabled = False Then
                clsCommon.MyMessageBoxShow(Me, "Document opened successfully", Me.Text)
                vaddnew = "N"
                chkclose.Checked = False
                vaddnew = "Y"
    ...
    Private Sub OpenNotify(ByVal isButtonClicked As Boolean)
        ...
        Dim dr As DataRow = clsCommon.ShowSelectFormForRow("NTFND", qry)
    ...
    'Private Sub txtPO_Merchnt__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean)
    '    If clsCommon.myLen(cmbCustPO_Type.SelectedValue) > 0 Then
    '        SelectPOItems()
    '    Else
    '        cmbCustPO_Type.SelectedValue = ""
    '    End If
    'End Sub
    ...
    Sub SelectPOItems()
        ...
        frm.strCurrCode = txtDocNo.Value
        frm.strFormType = "EXPORT SO"
        'frm.PurchaseOrder_Type = clsCommon.myCstr(cmbCustPO_Type.SelectedValue)
        frm.ShowDialog()
        LoadBlankGrid()
        Dim objOrderHead As clsPurchaseOrderHead = Nothing
        If frm.ArrReturn_EX IsNot Nothing AndAlso frm.ArrReturn_EX.Count > 0 Then
            If clsCommon.myLen(frm.ArrReturn_EX(0).Document_Code) > 0 Then
                objOrderHead = clsPurchaseOrderHead.GetData(frm.ArrReturn_EX(0).Document_Code, NavigatorType.Current)
            ...
            For ii As Integer = 0 To frm.ArrReturn_EX.Count - 1
                If clsCommon.myLen(frm.ArrReturn_EX(ii).Document_Code) > 0 Then
                    ...
                    'If Not arr.Contains(strCode) Then
                    '    arr.Add(strCode)
                    objOrderHead = clsPurchaseOrderHead.GetData(frm.ArrReturn_EX(ii).Document_Code, NavigatorType.Current)
                    For Each obj As clsPurchaseOrderDetail In objOrderHead.Arr
                        If (obj.Item_Code = frm.ArrReturn_EX(ii).Item_Code) Then
                            If IsValidPOItem(obj) Then
                                gv1.Rows.AddNew()
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colLineNo).Value = gv1.Rows.Count
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colRowType).Value = RowTypeItem
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colReqistionNo).Value = ""
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colRate).Value = obj.Item_Cost
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colOrgCost).Value = obj.Item_Cost
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colUnit).Value = obj.Unit_code
                                ...
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colDisPer).Value = obj.Disc_Per
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colMRP).Value = obj.MRP
                                If clsCommon.myCdbl(obj.MRP) <= 0 Then
                                    gv1.Rows(gv1.Rows.Count - 1).Cells(colMRP).Value = obj.Item_Cost
                                End If
                                ''gv1.Rows(gv1.Rows.Count - 1).Cells(colAssessableRate).Value = obj.Assessable
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colBinNo).Value = obj.Bin_No
                                'Dim dt As DataTable = clsSNShipmentHead.GetOriginalQty(obj.Document_Code, obj.Item_Code, obj.Unit_code, obj.Assessable, obj.MRP, Nothing)
                                ...
                                '    gv1.Rows(gv1.Rows.Count - 1).Cells(colExpiry).Value = obj.Expiry_Date
                                'End If
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colSchemeApplicable).Value = "No" ' IIf(obj.Scheme_Applicable = "Y", "Yes", "No")
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colFromSchemeCode).Value = "" ' obj.Scheme_Code
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colSchemeItem).Value = "No" ' IIf(obj.Scheme_Item = "Y", "Yes", "No")
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colTotTaxAmt).Value = obj.Item_Tax
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colTotalMRP).Value = obj.Total_MRP_Amt
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colTotalBasicAmount).Value = obj.Total_Basic_Amt
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colTotalDiscountAmount).Value = obj.Total_Disc_Amt
                                ...
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colBatchNo).Value = obj.Batch_No
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colItemWeight).Value = obj.Item_Weight
                                gv1.Rows(gv1.Rows.Count - 1).Cells(colConvF).Value = clsItemMaster.GetConvertionFactor(obj.Item_Code, obj.Unit_code, Nothing)
                                'gv1.Rows(gv1.Rows.Count - 1).Cells(colTotItemWt).Value = obj.TotalItem_Weight
            ...
            If objOrderHead.Arr IsNot Nothing AndAlso objOrderHead.Arr.Count > 0 Then
                For Each objTr As clsPurchaseOrderDetail In objOrderHead.Arr
                    If objTr.Row_Type = "Misc" Then
                        ...
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colLineNo).Value = gv1.Rows.Count
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colReqistionNo).Value = ""
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colPONO).Value = objOrderHead.PurchaseOrder_No
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colRowType).Value = objTr.Row_Type
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colICode).Value = objTr.Item_Code
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colIName).Value = objTr.Item_Desc
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colConvF).Value = clsItemMaster.GetConvertionFactor(objTr.Item_Code, objTr.Unit_code, Nothing)
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colRate).Value = objTr.Item_Cost
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colUnit).Value = objTr.Unit_code
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colQty).Value = 0
                        gv1.Rows(gv1.Rows.Count - 1).Cells(colAmt).Value = objTr.Amount
    ...
    Private Sub btnDrillDown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDrillDown.Click
        If clsCommon.myLen(txtReqNo.Value) > 0 Then
            If clsCommon.CompairString(clsUserMgtCode.frmEXSalesOrder, FORMTYPE) = CompairStringResult.Equal Then
                clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXSalesQuotation, txtReqNo.Value)
            Else
                clsOpenTransactionForm.OpenTransacionForm(clsUserMgtCode.frmEXSalesQuotation, txtReqNo.Value)
            End If
    ...
    Private Sub cmbTerms_Payment_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbTerms_Payment.SelectedValueChanged
        If isPaymentLoad = True Then
            Exit Sub
        End If
    ...
    Sub LoadDataDrillDown(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Dim obj As New clsEXSalesOrder()
        Try
            isInsideLoadData = False
            BlankAllControls()
            ...
            obj = clsEXSalesOrder.GetData(strCode, NavTyep, arrLoc, "EX")
    ...
    Sub CancelData()
        Try
            ...
            If clsCommon.MyMessageBoxShow("Are you sure to Cancel the Record?", "", MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.No Then
                Exit Sub
            End If
            Dim strReceiptCount As Integer = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select COUNT(Document_Code) from TSPL_EX_PI_HEAD WHERE Against_SO_No='" & txtDocNo.Value & "' "))
            If strReceiptCount > 0 Then
                Throw New Exception("You cannot cancelled this document because its Proforma Invoice has been created.")
            End If
            clsEXSalesOrder.CancelData(Me.Form_ID, txtDocNo.Value, arrLoc, NavigatorType.Current, IIf(clsCommon.CompairString(clsUserMgtCode.frmSalesOrderMT, FORMTYPE) = CompairStringResult.Equal, "MT", "EX"))
