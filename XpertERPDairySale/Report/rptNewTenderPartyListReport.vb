...
Sum(Total_Amt * case when RI IN(1,10,4) then 1 else case when RI In(7,9) then -1 else 0 end end 
* case when convert(date,Supply_Date,103) < '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "' then 1 else 0 end) as OP,
sum(case when (RI)=6  and convert(date,Supply_Date,103) <= '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  then Total_Amt else 0 end )as Security_Amt,
sum(case when (RI)=3  and convert(date,Supply_Date,103) <= '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  then Total_Amt else 0 end )as Refund_Amt,
sum(case when (RI)=7  and convert(date,Supply_Date,103) = '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  then Total_Amt else 0 end )as RTGS_Amt,
sum(case when (RI)=15  and convert(date,Supply_Date,103) = '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  then Total_Amt else 0 end )as Cash_Amt,
sum(case when (RI)=8  and convert(date,Supply_Date,103) = '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtDate.Value.AddDays(-1)), "dd/MMM/yyyy ") & "'  then Total_Amt else 0 end )as Closing_Rec_Amt,

Sum ( case when (RI)=1 and  --((Convert(Date, Supply_Date,103) = Convert(Date, '10/Jan/2026',103))
--OR 
(Convert(Date, Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "',103) )
--)
then Total_Amt else  0 end  ) as Sale_Amt, 

Sum ( case when (RI)=11 and (Convert(Date, Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtDate.Value.AddDays(-1)), "dd/MMM/yyyy ") & "',103) )
--OR (Convert(Date, Supply_Date,103) = Convert(Date, '10/Jan/2026 ',103) 
 then Total_Amt else 0 end  ) as Closing_Sale_Amt,
Sum(case when (RI)=12 and Convert(Date, Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtDate.Value.AddDays(-1)), "dd/MMM/yyyy ") & "',103)   then Total_Amt else 0  end )as Estimated_Avg_Sale,
Sum(case when (RI)=5 and Convert(Date, Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtDate.Value.AddDays(-1)), "dd/MMM/yyyy ") & "',103)   then Total_Amt else 0  end )as PrevDay_Return_Amt,
Sum(case when (RI)=9 and Convert(Date, Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "',103)   then Total_Amt else 0  end )as Return_Amt,
--Sum(Case when (RI)=5 THEN (Total_Amt) END) AS Return_Amt,
sum(case when (RI)=10 and convert(date,Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "',103)  then Total_Amt else 0 end )as Credit_Amt,
sum(case when (RI)=4 and convert(date,Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "',103)  then Total_Amt else 0 end )as Debit_Amt
...
sum(Type * amount) Total_Amt,NULL AS AgainstScrapReturn,NULL AS Route_No,2 as RI
 ...
 IN ( Select DocNo from TSPL_BANK_GUARANTEE_MASTER where Bank_Guarantee_Type = 'RC' AND CONVERT(date, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103) 
...
and (Against_MCC_Material_Sale_Return is null or Against_MCC_Material_Sale_Return='') and (AgainstScrapReturn is null or AgainstScrapReturn='')   group by Customer_Code)Debit_Amt on Debit_Amt.Debit_Cust=xxx.Customer_Code " & Environment.NewLine & " )xxfinal  left join TSPL_COMPANY_MASTER on 1=1"
    ...
    Sub SetGridFormation(ByVal isPrint As Boolean)
        ...
        gv1.Columns("Customer_Type").HeaderText = "Customer Type"
        gv1.Columns("Phone1").HeaderText = "Distributors " & Environment.NewLine & " Mobile Number"
        gv1.Columns("Gurantee_Amount").HeaderText = "Bank " & Environment.NewLine & " Guarantee"
        gv1.Columns("Security_Amt").HeaderText = "Security"
        gv1.Columns("Closing_Bal1").HeaderText = "Closing " & Environment.NewLine & " Balance " & Environment.NewLine & clsCommon.GetPrintDate(txtDate.Value.AddDays(-2), "dd/MM/yyyy") & ""
        gv1.Columns("Debit_Amt").HeaderText = "Debit Amt."
        gv1.Columns("Credit_Amt").HeaderText = "Credit Amt. " & Environment.NewLine & " Including (Sale Return)"
        ...
        gv1.Columns("Cash_Amt").HeaderText = "Cash Amt."
