...
Public Class rptNewTenderPartyListReport
    ...
    Private Sub LoadData(isPrint As Boolean)
        Try
            ...
            End If
            Dim qry As String = "  select " & strPrintqry & " ROW_NUMBER() over (order by Customer_Code) as SNO,IsDistributor,Route_No,case when Credit_Customer='Y' then Zone_Code1 else Zone_Code end as Zone_Code1,Credit_Customer,Customer_Code,Customer_Name,xxfinal.Phone1,Guarantee_Amount,Security_Amt,Zone_Code,Closing_Bal,Sale_Amt,RTGS_Amt,(Closing_Bal+ Sale_Amt -RTGS_Amt) as Outstanding_Amt,Estimated_Avg_Sale,(Closing_Bal+ Sale_Amt -RTGS_Amt) - (Estimated_Avg_Sale) as Estimated_OS_Amt  from ( " & Environment.NewLine & "select IsDistributor,Route_No,Zone_Code1,Credit_Customer,Customer_Code,Customer_Name,Phone1,isnull(Guarantee_Amount,0)Guarantee_Amount,isnull(Security_Amt,0)Security_Amt,Zone_Code,isnull((Closing_Sale_Amt-Closing_Rec_Amt),0) as Closing_Bal,isnull(Sale_Amt,0) Sale_Amt,isnull(RTGS_Amt,0) as RTGS_Amt,isnull(Estimated_Avg_Sale,0)Estimated_Avg_Sale from ( " & Environment.NewLine & "
            select max(IsDistributor)IsDistributor,max(Route_No)Route_No,max(Zone_Code1)Zone_Code1,max(Credit_Customer)Credit_Customer,Customer_Code,max(Customer_Name)Customer_Name,max(Phone1)Phone1,  MAX(Zone_Code)Zone_Code,sum(Sale_Amt)Sale_Amt,sum(Closing_Sale_Amt)Closing_Sale_Amt,sum(Estimated_Avg_Sale)Estimated_Avg_Sale from ( " & Environment.NewLine & " select TSPL_CUSTOMER_MASTER.IsDistributor,TSPL_CUSTOMER_MASTER.Route_No,TSPL_CUSTOMER_MASTER.Zone_Code as Zone_Code1,TSPL_CUSTOMER_MASTER.Credit_Customer, TSPL_SD_SALE_INVOICE_HEAD.Customer_Code,TSPL_CUSTOMER_MASTER.Customer_Name,replace(TSPL_CUSTOMER_MASTER.Phone1,'(+91)','') as Phone1,ROUTE.Zone_Code, ( case when (Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) = Convert(Date, '" & clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(txtDate.Value.AddDays(-1)), "dd/MMM/yyyy hh:mm tt") & "',103) AND  TSPL_SD_SHIPMENT_HEAD.Shift_Type='PM')
            ...
            If rbtnCreditCustomer.IsChecked Then
                qry += " where Credit_Customer='Y' "
            ElseIf rbtnDistributor.IsChecked Then
                qry += " where IsDistributor='Y' "
            End If
