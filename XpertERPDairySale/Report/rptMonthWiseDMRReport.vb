        ...
        Try
            ...
            If rbtnDispatch.IsChecked Then
                ...
                Else
                    SecurityRate = "xxx."
                End If
                BaseQry += " select CONVERT(varchar,xxx.Document_Date,103)Document_Date," & FreshItem & " Total_Milk_Qty,Total_Milk_Amt,TCS_Milk," & SecurityRate & "Security,(Total_Milk_Amt +TCS_Milk+" & SecurityRate & "Security) as GrandTotalMilk, " & ProdItem & " Total_Prod_Amt,TCS_Prod,(Total_Prod_Amt+ TCS_Prod) as GrandTotalProd ,(Total_Milk_Amt +TCS_Milk+" & SecurityRate & "Security+ Total_Prod_Amt+ TCS_Prod ) as [Grand Total] from ( SELECT max(Route_No)Route_No,  Document_Date,  " & FreshItemName & " SUM(" & FreshItemsQty & ") As Total_Milk_Qty,SUM(" & FreshItemsAmt & ") As Total_Milk_Amt,CONVERT(DECIMAL(18,2),(SUM(" & FreshItemsAmt & ")* 0.1 )/100)  as TCS_Milk,
                CONVERT(DECIMAL(18,2),(SUM(" & FreshItemsQty & ")* 0.1)) as Security ,  " & ProdItemName & " SUM(" & ProdItemsAmt & ") As Total_Prod_Amt ,CONVERT(DECIMAL(18,2),(SUM (" & ProdItemsAmt & ")* 0.1 )/100)  as TCS_Prod FROM (  SELECT TSPL_ITEM_MASTER.Item_Code,TSPL_SD_SHIPMENT_HEAD.Customer_Code as Cust_Code,  TSPL_SD_SHIPMENT_HEAD.Route_No, CONVERT(date,TSPL_SD_SHIPMENT_HEAD." + DateFilter + ",103)Document_Date, case when (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 0 )  then  TSPL_ITEM_MASTER.Short_Description end as Fresh_Item , case when (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 0 )  then  TSPL_ITEM_MASTER.Short_Description 
