    ...
    Private Sub LoadData()
	   ...
	   WHERE CONVERT(DATE, xxxx.Document_Date, 103) >= CONVERT(DATE, '" & clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") & "',103) AND CONVERT(DATE, xxxx.Document_Date, 103) <= CONVERT(DATE,'" & clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") & "',103)"
             ...
             SELECT TSPL_SD_SALE_RETURN_DETAIL.Security_Amt*-1 as Security_Amt, 
                ...
                + '-Amt' end as Fresh_Item_Amt,TSPL_SD_SALE_RETURN_DETAIL.Unit_Code AS UOM ,isnull(TSPL_SD_SALE_RETURN_DETAIL.qty,0) AS Qty, case when (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 0 ) then isnull(TSPL_SD_SALE_RETURN_DETAIL.Qty,0) end as Fresh_Qty, case when  IsTaxable = 1 then isnull(TSPL_SD_SALE_RETURN_DETAIL.qty,0) end as Product_Qty,case when (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 1 ) then round((isnull(TSPL_SD_SALE_RETURN_DETAIL.qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.[LTR],2) when (TSPL_ITEM_MASTER.Is_Ambient = 1 and TSPL_ITEM_MASTER.IsTaxable = 1 ) then round((isnull(TSPL_SD_SALE_RETURN_DETAIL.qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.[KG],2) end as KG_QTY
               ...
               , case when isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX5,'') <> '' then  TSPL_ITEM_MASTER.Short_Description + ' ' + (TSPL_SD_SALE_RETURN_DETAIL.TAX5) + convert(varchar,convert(decimal(18,1),(TSPL_SD_SALE_RETURN_DETAIL.TAX5_Rate),103)) + '%'  end as Product_Item_Tax5, case when isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX1,'') <> '' then TSPL_SD_SALE_RETURN_DETAIL.TAX1_Amt end as Tax1_Amt, case when isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX2,'') <> '' then TSPL_SD_SALE_RETURN_DETAIL.TAX2_Amt end as Tax2_Amt, case  when isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX3,'') <> '' then TSPL_SD_SALE_RETURN_DETAIL.TAX3_Amt end as Tax3_Amt, case when isnull(TSPL_SD_SALE_RETURN_DETAIL.TAX4,'') <>'' then  TSPL_SD_SALE_RETURN_DETAIL.TAX4_Amt end as Tax4_Amt, case when isnull(TSPL_SD_SALE_RETURN_head.tax5,'') <> '' 
