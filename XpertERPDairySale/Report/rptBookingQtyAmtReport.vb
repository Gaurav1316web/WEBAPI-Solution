    ...
    Sub PrintView(ByVal printnotpad As Boolean)
        Try
            ...
            If cboDocumentType.SelectedIndex = 1 Then
                strWhrClause2 = " and convert(date, TSPL_DEMAND_BOOKING_MASTER." + strDate + ",103) >= '" + clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") + "' and  convert(date, TSPL_DEMAND_BOOKING_MASTER." + strDate + ",103) <= '" + clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") + "' and TSPL_DEMAND_BOOKING_MASTER.Posted = 1 "
            ElseIf cboDocumentType.SelectedIndex = 2 Then
                strWhrClause2 = " and convert(date, TSPL_DEMAND_BOOKING_MASTER." + strDate + ",103) >= '" + clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") + "' and  convert(date, TSPL_DEMAND_BOOKING_MASTER." + strDate + ",103) <= '" + clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") + "' and TSPL_DEMAND_BOOKING_MASTER.Posted = 0 "
            Else
                strWhrClause2 = " and convert(date, TSPL_DEMAND_BOOKING_MASTER." + strDate + ",103) >= '" + clsCommon.GetPrintDate(fromDate.Value, "dd/MMM/yyyy") + "' and  convert(date, TSPL_DEMAND_BOOKING_MASTER." + strDate + ",103) <= '" + clsCommon.GetPrintDate(ToDate.Value, "dd/MMM/yyyy") + "' "
            End If

            If txtCustomer.arrValueMember IsNot Nothing AndAlso txtCustomer.arrValueMember.Count > 0 Then
                strWhrClause2 += " and TSPL_DEMAND_BOOKING_DETAIL.Cust_Code  in (" + clsCommon.GetMulcallString(txtCustomer.arrValueMember) + ")"
            End If

            Dim ItemInUse As String = ""
            ItemInUse = " TSPL_DEMAND_BOOKING_DETAIL Left Outer Join TSPL_DEMAND_BOOKING_MASTER On TSPL_DEMAND_BOOKING_MASTER.Document_No = TSPL_DEMAND_BOOKING_DETAIL.Document_No Left Outer Join TSPL_CUSTOMER_MASTER On TSPL_CUSTOMER_MASTER.Cust_Code = TSPL_DEMAND_BOOKING_DETAIL.Cust_Code Left Outer Join TSPL_ITEM_MASTER On TSPL_ITEM_MASTER.Item_Code = TSPL_DEMAND_BOOKING_DETAIL.Item_Code Left Outer Join TSPL_VEHICLE_MASTER On TSPL_VEHICLE_MASTER.Vehicle_Id = TSPL_DEMAND_BOOKING_DETAIL.Vehicle_Code Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_DEMAND_BOOKING_MASTER.location_code left outer join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code=TSPL_CUSTOMER_MASTER.Cust_Group_Code where (TSPL_ITEM_MASTER.Alies_Name !='' or TSPL_ITEM_MASTER.Alies_Name is null) "
            ...
            Dim strGrandTotalWithoutScheme As String = clsDBFuncationality.getSingleValue(" DECLARE @colsScheme AS NVARCHAR(MAX),@query  AS NVARCHAR(MAX) SELECT   STUFF((SELECT distinct '+' +'Sum(isnull(' + QUOTENAME( " + strAliasCol_Amount + ") +',0))'      as Alies_Name  FROM " + ItemInUse + "  FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)') ,1,1,'')  ")
            ...
            qry = " select  " + DistributorQry + " " + strItem2WithSum + " , (" + strGrandTotalWithoutScheme + ") as [Grand Total] ,cast(SUM(QtyLtr) as decimal(18,2)) as [Total In Ltr] from ( select * from (select max(zzz.item_code) as item_code,zzz.Document_No, max(Document_Date) as Document_Date,Max(Created_By) as Created_By,max(Created_Date) as Created_Date,max(Modified_By) as Modified_By,max(Modified_Date) as Modified_Date ,max(Booth) as [Booth] , max([Customer Category Code]) as  [Customer Category Code]," + DistributorSubQry + " max([Customer Code]) as  [Customer Code],zzz.[VEHICLE NO],zzz.[WdName],zzz.Description,max(zzz.Description_Creat) as Description_Creat,max(zzz.Description_Pouch) as Description_Pouch,max(zzz.Description_Ltr) as Description_Ltr, max(zzz.Description_Amount) as Description_Amount
                                          , sum (Qty_Create) as Qty_Create,sum (Qty_Ltr) as Qty_Ltr,sum (Qty_Pouch) as Qty_Pouch,sum(Amount_Ltr) as Amount_Ltr,zzz.Cust_Group_Code as [Group], max(zzz.[Cust Group Desc]) as [Cust Group Desc],zzz.Zone_Code as [Zone]  ,zzz.[Route No] ,sum(qty) as qty,sum(QtyLtr) as QtyLtr from  (Select TSPL_DEMAND_BOOKING_MASTER.Document_No, Convert (varchar,TSPL_DEMAND_BOOKING_MASTER.Document_Date,103) as Document_Date,( format ( TSPL_DEMAND_BOOKING_MASTER.Created_Date, 'HH') +'.'+ format ( TSPL_DEMAND_BOOKING_MASTER.Created_Date, 'mm') ) as Created_Date_Time,TSPL_DEMAND_BOOKING_MASTER.Created_By,convert (varchar,TSPL_DEMAND_BOOKING_MASTER.Created_Date,103) as Created_Date,TSPL_DEMAND_BOOKING_MASTER.Modified_By, Convert (varchar,TSPL_DEMAND_BOOKING_MASTER.Modified_Date,103) as Modified_Date, TSPL_ITEM_MASTER.Sku_Seq, TSPL_DEMAND_BOOKING_MASTER.Location_Code, TSPL_LOCATION_MASTER.Location_Desc,isnull(TSPL_CUSTOMER_MASTER.cust_category_code,'') as [Customer Category Code], " + Distributor + " TSPL_DEMAND_BOOKING_DETAIL.Cust_Code As [Customer Code], " + strCustomerName + " As WdName, TSPL_DEMAND_BOOKING_DETAIL.Item_Code as Item_Code,TSPL_DEMAND_BOOKING_DETAIL.Unit_code as UOM,TSPL_DEMAND_BOOKING_MASTER.route_no as [Route No] , "
            If rdbEnglish.IsChecked = True Then
            ...
            qry += ",TSPL_VEHICLE_MASTER.Description [Lorry_No],TSPL_CUSTOMER_MASTER.Cust_Group_Code,TSPL_CUSTOMER_MASTER.Zone_Code ,IsNull(TSPL_VEHICLE_MASTER.Description, '''') As [VEHICLE NO], " + strCreateConv + " as Qty, TSPL_DEMAND_BOOKING_MASTER.Document_Date As [Order Date],TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Desc as [Cust Group Desc], TSPL_CUSTOMER_MASTER.OldName as Booth,(CASE WHEN TSPL_DEMAND_BOOKING_DETAIL.Qty=0 THEN 0 ELSE (TSPL_DEMAND_BOOKING_DETAIL.Qty*TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor)/coalesce(TSPL_ITEM_UOM_DETAILltr.Conversion_Factor,TSPL_ITEM_UOM_DETAILKG.Conversion_Factor) END) AS QtyLtr , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILCREATE.Conversion_Factor,0)) as Qty_Create
                                          , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILLTR.Conversion_Factor,0)) as Qty_Ltr
                                          , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILPOUCH.Conversion_Factor,0)) as Qty_Pouch
                                          , ItemNetAmount as Amount_Ltr From TSPL_DEMAND_BOOKING_DETAIL Left Outer Join TSPL_DEMAND_BOOKING_MASTER On TSPL_DEMAND_BOOKING_MASTER.Document_No = TSPL_DEMAND_BOOKING_DETAIL.Document_No Left Outer Join TSPL_CUSTOMER_MASTER On TSPL_CUSTOMER_MASTER.Cust_Code = TSPL_DEMAND_BOOKING_DETAIL.Cust_Code Left Outer Join TSPL_ITEM_MASTER On TSPL_ITEM_MASTER.Item_Code = TSPL_DEMAND_BOOKING_DETAIL.Item_Code Left Outer Join TSPL_VEHICLE_MASTER On TSPL_VEHICLE_MASTER.Vehicle_Id = TSPL_DEMAND_BOOKING_DETAIL.Vehicle_Code Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_DEMAND_BOOKING_MASTER.location_code left outer join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code=TSPL_CUSTOMER_MASTER.Cust_Group_Code Left Outer Join (Select Cust_Code As [Distributor Code],Customer_Name As [Distributor Name],Cust_Group_Code from TSPL_CUSTOMER_MASTER) As Distributor ON Distributor.[Distributor Code]=TSPL_CUSTOMER_MASTER.Distributor_Code " &
                                  " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILLTR on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILLTR.Item_Code and TSPL_ITEM_UOM_DETAILLTR.UOM_Code='LTR'  left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILKG on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILKG.Item_Code and TSPL_ITEM_UOM_DETAILKG.UOM_Code='KG' left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILCREATE on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILCREATE.Item_Code and TSPL_ITEM_UOM_DETAILCREATE.UOM_Code='CRATE'  join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILPOUCH on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILPOUCH.Item_Code and TSPL_ITEM_UOM_DETAILPOUCH.UOM_Code='Pouch'  left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILUOM on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILUOM.Item_Code and TSPL_DEMAND_BOOKING_DETAIL.Unit_code =TSPL_ITEM_UOM_DETAILUOM.UOM_Code " &
                                  "  where 2=2   " + strWhrClause2 + " )zzz  group by zzz.Document_No,zzz.[VEHICLE NO] ,zzz.WdName,zzz.Description,zzz.[Customer Category Code],zzz.Cust_Group_Code,zzz.Zone_Code,zzz.[Route No]	) as s 
            ...
            qry = qry + " Union All 
                          select " + UnionQry + "'' as  [Customer Code],N'" + strTotalRow + "' as [Customer Name],max([route no])[route no], " + strItem2WithSum + " , (" + strGrandTotalWithoutScheme + ") as [Grand Total] ,cast(SUM(QtyLtr) as decimal(18,2)) as [Total In Ltr] from ( select * from (select max(zzz.item_code) as item_code,zzz.Document_No, max(Document_Date) as Document_Date,Max(Created_By) as Created_By,max(Created_Date) as Created_Date,max(Modified_By) as Modified_By,max(Modified_Date) as Modified_Date ,max(Booth) as [Booth] , max([Customer Category Code]) as  [Customer Category Code], max([Customer Code]) as  [Customer Code],zzz.[VEHICLE NO],zzz.[WdName],zzz.Description,max(zzz.Description_Creat) as Description_Creat,max(zzz.Description_Pouch) as Description_Pouch,max(zzz.Description_Ltr) as Description_Ltr, max(zzz.Description_Amount) as Description_Amount
                                          , sum (Qty_Create) as Qty_Create,sum (Qty_Ltr) as Qty_Ltr,sum (Qty_Pouch) as Qty_Pouch,sum(Amount_Ltr) as Amount_Ltr,zzz.Cust_Group_Code as [Group], max(zzz.[Cust Group Desc]) as [Cust Group Desc],zzz.Zone_Code as [Zone]  ,zzz.[Route No] ,sum(qty) as qty,sum(QtyLtr) as QtyLtr from  (Select TSPL_DEMAND_BOOKING_MASTER.Document_No, Convert (varchar,TSPL_DEMAND_BOOKING_MASTER.Document_Date,103) as Document_Date,( format ( TSPL_DEMAND_BOOKING_MASTER.Created_Date, 'HH') +'.'+ format ( TSPL_DEMAND_BOOKING_MASTER.Created_Date, 'mm') ) as Created_Date_Time,TSPL_DEMAND_BOOKING_MASTER.Created_By,convert (varchar,TSPL_DEMAND_BOOKING_MASTER.Created_Date,103) as Created_Date,TSPL_DEMAND_BOOKING_MASTER.Modified_By, Convert (varchar,TSPL_DEMAND_BOOKING_MASTER.Modified_Date,103) as Modified_Date, TSPL_ITEM_MASTER.Sku_Seq, TSPL_DEMAND_BOOKING_MASTER.Location_Code, TSPL_LOCATION_MASTER.Location_Desc,isnull(TSPL_CUSTOMER_MASTER.cust_category_code,'') as [Customer Category Code], TSPL_DEMAND_BOOKING_DETAIL.Cust_Code As [Customer Code], " + strCustomerName + " As WdName, TSPL_DEMAND_BOOKING_DETAIL.Item_Code as Item_Code,TSPL_DEMAND_BOOKING_DETAIL.Unit_code as UOM,TSPL_DEMAND_BOOKING_MASTER.route_no as [Route No] , "
            If rdbEnglish.IsChecked = True Then
                qry += " TSPL_ITEM_MASTER.Alies_Name As [Description] ,
                                            TSPL_ITEM_MASTER.Alies_Name+'_Crate' As [Description_Creat]
                                          ,TSPL_ITEM_MASTER.Alies_Name+'_Pouch' As [Description_Pouch]
            ...
            qry += " TSPL_VEHICLE_MASTER.Description [Lorry_No],TSPL_CUSTOMER_MASTER.Cust_Group_Code,TSPL_CUSTOMER_MASTER.Zone_Code ,IsNull(TSPL_VEHICLE_MASTER.Description, '''') As [VEHICLE NO], " + strCreateConv + " as Qty, TSPL_DEMAND_BOOKING_MASTER.Document_Date As [Order Date],TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Desc as [Cust Group Desc], TSPL_CUSTOMER_MASTER.OldName as Booth,(CASE WHEN TSPL_DEMAND_BOOKING_DETAIL.Qty=0 THEN 0 ELSE (TSPL_DEMAND_BOOKING_DETAIL.Qty*TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor)/coalesce(TSPL_ITEM_UOM_DETAILltr.Conversion_Factor,TSPL_ITEM_UOM_DETAILKG.Conversion_Factor) END) AS QtyLtr , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILCREATE.Conversion_Factor,0)) * 0 as Qty_Create
                                          , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILLTR.Conversion_Factor,0)) as Qty_Ltr
                                          , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILPOUCH.Conversion_Factor,0)) * 0 as Qty_Pouch
                                          , ItemNetAmount * 0 as Amount_Ltr  From TSPL_DEMAND_BOOKING_DETAIL Left Outer Join TSPL_DEMAND_BOOKING_MASTER On TSPL_DEMAND_BOOKING_MASTER.Document_No = TSPL_DEMAND_BOOKING_DETAIL.Document_No Left Outer Join TSPL_CUSTOMER_MASTER On TSPL_CUSTOMER_MASTER.Cust_Code = TSPL_DEMAND_BOOKING_DETAIL.Cust_Code Left Outer Join TSPL_ITEM_MASTER On TSPL_ITEM_MASTER.Item_Code = TSPL_DEMAND_BOOKING_DETAIL.Item_Code Left Outer Join TSPL_VEHICLE_MASTER On TSPL_VEHICLE_MASTER.Vehicle_Id = TSPL_DEMAND_BOOKING_DETAIL.Vehicle_Code Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_DEMAND_BOOKING_MASTER.location_code left outer join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code=TSPL_CUSTOMER_MASTER.Cust_Group_Code " &
                                  "  left Outer Join (Select TSPL_DEMAND_BOOKING_DETAIL.Document_No,Sum (isnull(Qty,0)) as Scheme_Qty  from TSPL_DEMAND_BOOKING_DETAIL Group by TSPL_DEMAND_BOOKING_DETAIL.Document_No ) TBL_SCHEME_VALUE    On    TBL_SCHEME_VALUE.Document_No = TSPL_DEMAND_BOOKING_MASTER.Document_No " &
                                  " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILLTR on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILLTR.Item_Code and TSPL_ITEM_UOM_DETAILLTR.UOM_Code='LTR'  left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILKG on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILKG.Item_Code and TSPL_ITEM_UOM_DETAILKG.UOM_Code='KG' left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILCREATE on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILCREATE.Item_Code and TSPL_ITEM_UOM_DETAILCREATE.UOM_Code='CRATE'  join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILPOUCH on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILPOUCH.Item_Code and TSPL_ITEM_UOM_DETAILPOUCH.UOM_Code='Pouch'  left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILUOM on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILUOM.Item_Code and TSPL_DEMAND_BOOKING_DETAIL.Unit_code =TSPL_ITEM_UOM_DETAILUOM.UOM_Code " &
                                  "  where 2=2   " + strWhrClause2 + " )zzz  group by zzz.Document_No,zzz.[VEHICLE NO] ,zzz.WdName,zzz.Description,zzz.[Customer Category Code],zzz.Cust_Group_Code,zzz.Zone_Code,zzz.[Route No] 	) as s 
            ...
            qry += " TSPL_VEHICLE_MASTER.Description [Lorry_No],TSPL_CUSTOMER_MASTER.Cust_Group_Code,TSPL_CUSTOMER_MASTER.Zone_Code ,IsNull(TSPL_VEHICLE_MASTER.Description, '''') As [VEHICLE NO], " + strCreateConv + " as Qty, TSPL_DEMAND_BOOKING_MASTER.Document_Date As [Order Date],TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Desc as [Cust Group Desc], TSPL_CUSTOMER_MASTER.OldName as Booth,(CASE WHEN TSPL_DEMAND_BOOKING_DETAIL.Qty=0 THEN 0 ELSE (TSPL_DEMAND_BOOKING_DETAIL.Qty*TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor)/coalesce(TSPL_ITEM_UOM_DETAILltr.Conversion_Factor,TSPL_ITEM_UOM_DETAILKG.Conversion_Factor) END) AS QtyLtr , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILCREATE.Conversion_Factor,0)) * 0 as Qty_Create
                                          , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILLTR.Conversion_Factor,0)) * 0 as Qty_Ltr
                                          , convert (decimal(18,2), TSPL_DEMAND_BOOKING_DETAIL.Qty * TSPL_ITEM_UOM_DETAILUOM.Conversion_Factor / nullif ( TSPL_ITEM_UOM_DETAILPOUCH.Conversion_Factor,0)) * 0 as Qty_Pouch
                                          , ItemNetAmount as Amount_Ltr  From TSPL_DEMAND_BOOKING_DETAIL Left Outer Join TSPL_DEMAND_BOOKING_MASTER On TSPL_DEMAND_BOOKING_MASTER.Document_No = TSPL_DEMAND_BOOKING_DETAIL.Document_No Left Outer Join TSPL_CUSTOMER_MASTER On TSPL_CUSTOMER_MASTER.Cust_Code = TSPL_DEMAND_BOOKING_DETAIL.Cust_Code Left Outer Join TSPL_ITEM_MASTER On TSPL_ITEM_MASTER.Item_Code = TSPL_DEMAND_BOOKING_DETAIL.Item_Code Left Outer Join TSPL_VEHICLE_MASTER On TSPL_VEHICLE_MASTER.Vehicle_Id = TSPL_DEMAND_BOOKING_DETAIL.Vehicle_Code Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_DEMAND_BOOKING_MASTER.location_code left outer join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code=TSPL_CUSTOMER_MASTER.Cust_Group_Code " &
                                  " left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILLTR on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILLTR.Item_Code and TSPL_ITEM_UOM_DETAILLTR.UOM_Code='LTR'  left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILKG on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILKG.Item_Code and TSPL_ITEM_UOM_DETAILKG.UOM_Code='KG' left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILCREATE on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILCREATE.Item_Code and TSPL_ITEM_UOM_DETAILCREATE.UOM_Code='CRATE'  join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILPOUCH on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILPOUCH.Item_Code and TSPL_ITEM_UOM_DETAILPOUCH.UOM_Code='Pouch'  left join TSPL_ITEM_UOM_DETAIL TSPL_ITEM_UOM_DETAILUOM on TSPL_DEMAND_BOOKING_DETAIL.Item_Code=TSPL_ITEM_UOM_DETAILUOM.Item_Code and TSPL_DEMAND_BOOKING_DETAIL.Unit_code =TSPL_ITEM_UOM_DETAILUOM.UOM_Code " &
                                  "  where 2=2   " + strWhrClause2 + " )zzz group by zzz.Document_No,zzz.[VEHICLE NO] ,zzz.WdName,zzz.Description,zzz.[Customer Category Code],zzz.Cust_Group_Code,zzz.Zone_Code,zzz.[Route No]	) as s 
