    ...
    Public Function returnQry() As String
        ...
        ElseIf rbtnSummary.IsChecked Then
            strqry = " SELECT distinct Short_Description
						...
						left join ( select Item_Code,Report_UOM,UOM_Code from TSPL_ITEM_UOM_DETAIL where Report_UOM = 1 ) It on It.Item_Code =TSPL_ITEM_MASTER.Item_Code 
                        Left Outer Join TSPL_LOCATION_MASTER On TSPL_LOCATION_MASTER.Location_Code = TSPL_DEMAND_BOOKING_MASTER.location_code 
                        left outer join TSPL_CUSTOMER_GROUP_MASTER on TSPL_CUSTOMER_GROUP_MASTER.Cust_Group_Code=TSPL_CUSTOMER_MASTER.Cust_Group_Code  where  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date ,103)>=convert(date,'" + fromDate.Value + "',103) AND convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date,103)<=convert(date,'" + ToDate.Value + "',103) " + whrh + " order by Short_Description desc"
        End If
    ...
    Private Sub PrintBoothBill_Click(sender As Object, e As EventArgs) Handles PrintBoothBill.Click
        Try
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                If rbtnDetail.IsChecked Then
                    Dim query As String = "SELECT (CONVERT(DECIMAL(18, 2), (Amount / ((QtyLtr * ConversionFactor) / CF)))) AS RateLtr, (Sku_Seq) AS Sku_Seq, (Item_Desc) AS Item_Desc,  (Amount) AS Amount, (QtyLtr) AS QtyLtr, 
                    ...
                    MAX(ConversionFactor) AS ConversionFactor,  MAX(CF) AS CF,  MAX(Report_UOM) AS Report_UOM,(Route_No)Route_No,(Cust_Code)Cust_Code FROM (SELECT TSPL_DEMAND_BOOKING_MASTER.Route_No,
                    ...
                    AND convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date,103)<=convert(date,'" + ToDate.Value + "',103) " + Whr + " )xx group by Route_No, Cust_Code, Item_Code )xxx order by Sku_Seq"
