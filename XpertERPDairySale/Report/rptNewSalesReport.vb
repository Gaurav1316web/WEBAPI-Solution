        ...
        Try
           ...
           JOIN ( SELECT Route_No,MAX(Start_Date) AS Max_Start_Date FROM TSPL_DISTRIBUTOR_ROUTE_CUSTOMER  LEFT JOIN  TSPL_DISTRIBUTOR_ROUTE  ON TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.code = TSPL_DISTRIBUTOR_ROUTE.code
            ...
            If rbtnRouteAndCustomer.IsChecked Then
                strITEM += " Select * from ( "
            End If
            strITEM += "SELECT max(TSPL_ITEM_MASTER.Short_Description) as  Fresh_Item " & qry & " " & whrcls & " and  (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 0 ) or ( 2 = 2  " & whrcls & " and (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 1 and Is_CrateType = 1)) group by TSPL_ITEM_MASTER.Item_Code,Sku_Seq,Print_Sequence"
            If rbtnRouteAndCustomer.IsChecked Then
                strITEM += " Union "
                strITEM += " Select Short_Description As Fresh_Item,Sku_Seq,Print_Sequence,Alies_Name2,Alies_Name3 from TSPL_ITEM_MASTER Where Is_DisplayDemand=1 And TSPL_ITEM_MASTER.Is_FreshItem = 1 "
                strITEM += ")qwerty "
            End If
            strITEM += " ORDER BY Sku_Seq"
            dtFreshItem = clsDBFuncationality.GetDataTable(strITEM)

            strITEM = ""
            If rbtnRouteAndCustomer.IsChecked Then
                strITEM += " Select * from ( "
            End If
            strITEM += "SELECT max(TSPL_ITEM_MASTER.Short_Description) as  Product_Item " & qry & " " & whrcls & "  and TSPL_ITEM_MASTER.Is_Ambient = 1  group by TSPL_ITEM_MASTER.Item_Code,Sku_Seq,Print_Sequence"
            If rbtnRouteAndCustomer.IsChecked Then
                strITEM += " Union "
                strITEM += " Select Short_Description As Fresh_Item,Sku_Seq,Print_Sequence,Alies_Name2,Alies_Name3 from TSPL_ITEM_MASTER Where Is_DisplayDemand=1 And TSPL_ITEM_MASTER.Is_Ambient = 1 "
                strITEM += ")qwerty "
            End If
            strITEM += " ORDER BY Sku_Seq"
            dtProductItem = clsDBFuncationality.GetDataTable(strITEM)
         ...
         when TSPL_DEMAND_BOOKING_DETAIL.TAX5 = 'TCS' then TAX5_Amt when TSPL_DEMAND_BOOKING_DETAIL.TAX6 = 'TCS' then TAX6_Amt when TSPL_DEMAND_BOOKING_DETAIL.TAX7 = 'TCS' then TAX7_Amt when TSPL_DEMAND_BOOKING_DETAIL.TAX8 = 'TCS' then TAX8_Amt when TSPL_DEMAND_BOOKING_DETAIL.TAX9 = 'TCS' then TAX9_Amt when TSPL_DEMAND_BOOKING_DETAIL.TAX10 = 'TCS' then TAX10_Amt else 0 END ) AS Amount, case when TSPL_ITEM_MASTER.Is_Ambient = 1  then TSPL_ITEM_MASTER.Short_Description  end AS Product_Item
		        ,case when TSPL_ITEM_MASTER.Is_Ambient = 1  then TSPL_ITEM_MASTER.Short_Description + 'Amt' end AS Product_Item_Amt, 0 AS Receipt_Amount "
                If rbtnRouteAndCustomer.IsChecked Then
                    Q1 += " ,TSPL_CUSTOMER_MASTER.Credit_Customer,case when Credit_Customer='N' then TSPL_DEMAND_BOOKING_MASTER.Route_No else TSPL_CUSTOMER_MASTER.Cust_Code end as GrpColumn,case when Credit_Customer='N' then TSPL_ROUTE_MASTER.Route_Desc else TSPL_CUSTOMER_MASTER.Customer_Name end as GrpColumnName "
                End If
...
left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code = dist.Cust_Code left Outer join TSPL_ROUTE_MASTER On TSPL_ROUTE_MASTER.Route_No=TSPL_CUSTOMER_MASTER.Route_No "
                ...
                If rbtnRouteAndCustomer.IsChecked Then
                    qry += Q1 + " left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code = TSPL_DEMAND_BOOKING_DETAIL.Cust_Code 
left Outer join TSPL_ROUTE_MASTER On TSPL_ROUTE_MASTER.Route_No=TSPL_CUSTOMER_MASTER.Route_No " + Q3
                Else
                    qry += Q1 + Q2 + Q3
                End If
            ...
            Else
                ...
                case when (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 0 ) or (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 1 and Is_CrateType = 1) then isnull(TSPL_SD_SHIPMENT_DETAIL.Qty,0) end as Fresh_Qty, case when TSPL_ITEM_MASTER.Is_Ambient = 1 then isnull(TSPL_SD_SHIPMENT_DETAIL.qty,0) end as Product_Qty,round((isnull(TSPL_SD_SHIPMENT_DETAIL.qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.[KG],2) as KG_QTY ,round((isnull(TSPL_SD_SHIPMENT_DETAIL.qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.[KG],2) as KG_QTY1
                ,round((isnull(TSPL_SD_SHIPMENT_DETAIL.qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.[LTR],2) as LTR_QTY ,case when TSPL_ITEM_MASTER.Is_Ambient = 1  then isnull(TSPL_SD_SHIPMENT_DETAIL.Amount,0)end as Product_Amount
		        ,case when (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 0 ) or (TSPL_ITEM_MASTER.Is_FreshItem = 1 and TSPL_ITEM_MASTER.IsTaxable = 1 and Is_CrateType = 1) then isnull(TSPL_SD_SHIPMENT_DETAIL.Amount,0)end as Fresh_Amount,TSPL_SD_SHIPMENT_DETAIL.Amount, case when TSPL_ITEM_MASTER.Is_Ambient = 1  then TSPL_ITEM_MASTER.Short_Description  end AS Product_Item
		        ,case when TSPL_ITEM_MASTER.Is_Ambient = 1  then TSPL_ITEM_MASTER.Short_Description + 'Amt' end AS Product_Item_Amt, 0 AS Receipt_Amount
           ...
           JOIN ( SELECT Route_No,MAX(Start_Date) AS Max_Start_Date FROM TSPL_DISTRIBUTOR_ROUTE_CUSTOMER  LEFT JOIN  TSPL_DISTRIBUTOR_ROUTE  ON TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.code = TSPL_DISTRIBUTOR_ROUTE.code
            ...
            ElseIf rbtnRouteAndCustomer.IsChecked Then
                ...
                strBase = "select  0 As Days,Document_Date,Shift_Type,Item_Code,GrpColumn as RouteCustCode,Max(GrpColumnName)GrpColumnName,max(case when Credit_Customer='Y' then Customer_Name else Route_Desc end) as RouteCustName,max(Item_Sub_Group_Type) as Item_Sub_Group_Type, max(Fresh_Item) as Fresh_Item,max(Fresh_Item_Amt) as Fresh_Item_Amt,max(UOM) as UOM,sum(Qty) as Qty,sum(isnull(Fresh_Qty,0)) as Fresh_Qty,sum(isnull(Product_Qty,0)) as Product_Qty,sum(isnull(KG_QTY,0)) as KG_QTY,sum(isnull(	KG_QTY1,0)) as KG_QTY1, sum(isnull(LTR_QTY,0)) as LTR_QTY  ,sum(Product_Amount) as Product_Amount	, sum(Fresh_Amount) as Fresh_Amount	  ,sum(Amount) as Amount, max(Product_Item) as Product_Item,max(Product_Item_Amt) as Product_Item_Amt,sum(isnull(Receipt_Amount,0)) as Receipt_Amount	,Credit_Customer,Max(Cust_Code)Cust_Code 
