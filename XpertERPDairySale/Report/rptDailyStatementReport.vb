            ...
            If rbtnDistributorWise.IsChecked Then
               ...
               where Item_Code is  not null group by  XXXFirst.Cust_Code,Credit_Customer, XXXFirst.Item_Code,	XXXFirst.Sku_Seq "
                ...
                left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code " & whrcls & " ) YY group by  yy.Cust_Code,yy.Item_Code,Credit_Customer ,Sku_Seq ) XX GROUP BY xx.Cust_Code,Credit_Customer,Customer_Name, xx.Item_Code,	xx.Sku_Seq ) YYY  ) xxx
                 left join ( Select  Cust_Code,isnull(SUM(PAmt),0) AS PAmt1,Item_Code from ( Select TSPL_CUSTOMER_MASTER.Cust_Code, 
               ...
               Left Join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No = TSPL_DEMAND_BOOKING_master.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code left outer join TSPL_CUSTOMER_MASTER as TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code
                WHERE  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date) = CONVERT(DATE, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103) and Is_Ambient = 1 and TSPL_CUSTOMER_MASTER1.Credit_Customer = 'N') XXXFirst 
                 group by  XXXFirst.Cust_Code,XXXFirst.Item_Code  ) xx on xx.Cust_Code = xxx.Cust_Code and xxx.Item_Code  = xx.Item_Code order by xxx.Customer_Name		  "
                ...
                ElseIf rbtnProduct.IsChecked Then
                   ...
                   Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code " & whrcls & "  and Credit_Customer = 'Y'  ) XX group by  xx.Cust_Code,xx.Item_Code ) xx on xx.Cust_Code = XXXFirst.Cust_Code  and xx.Item_Code = XXXFirst.Item_Code Group By  XXXFirst.Item_Code ,XXXFirst.Sku_Seq ) YY "
                    dtDepartmentSale = clsDBFuncationality.GetDataTable(DeptQry)
                    query = "select *,isnull(TotalAmt_ItemWise,0)PAmt, isnull(TotalAmt_ItemWise,0) + isnull(MAmt,0) as [Total Amount], 'Qty' as QtyCol from ( select case when yyy.Credit_Customer='Y'   then isnull(Credit_CustomerQty,0) else isnull(Final_Qty,0) end as final,case when  yyy.Credit_Customer='Y' and Customer_Name4='I-DPT' then Final_Qty else 0 end as InDepartment,* from  ( Select  '" + objCommonVar.CurrentUser + "'  UserName ,Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,max(Customer_Name)Customer_Name, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,SUM(MAmt) AS MAmt,
                   (Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty    from ( " & Environment.NewLine & " Select  TSPL_CUSTOMER_MASTER1.Credit_Customer,  CASE WHEN TSPL_CUSTOMER_MASTER1.Credit_Customer = 'y' THEN  CASE WHEN TSPL_ITEM_MASTER.Is_FreshItem = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.LTR, 2) ELSE 0 END  END AS Credit_CustomerQty,TSPL_CUSTOMER_MASTER.Cust_Code,coalesce(TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Customer_Name) as Customer_Name  ,TSPL_DEMAND_BOOKING_MASTER.ShiftType,
                   ...
                   Left Join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No = TSPL_DEMAND_BOOKING_master.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code left outer join TSPL_CUSTOMER_MASTER AS TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code " & whrcls & "  and TSPL_CUSTOMER_MASTER1.Credit_Customer = 'N' )  XXXFirst  where Item_Code is  not null group by  XXXFirst.Cust_Code,Credit_Customer, XXXFirst.Item_Code,XXXFirst.Sku_Seq  "
                    query += " union all " & Environment.NewLine & " SELECT max(UserName)UserName,Cust_Code,max(Customer_Name1) as Customer_Name1,max(Customer_Name4)Customer_Name4 , max(Customer_Name)Customer_Name , sum(Final_Qty)Final_Qty , Sku_Seq,max(phone2)phone2 , max(phone1)phone1 , max(Circle_No)Circle_No , max(Comp_Name)Comp_Name , max(City_Code)City_Code , max(state)State , Item_Code, max(Add1)Add1 , max(Add2)Add2 , max(Pincode)Pincode , max(fax)fax , max(date)date , max(Short_Description)Short_Description , max(Unit_Desc)Unit_Desc , sum(TotalLtr_ItemWise)TotalLtr_ItemWise , sum(TotalAmt_ItemWise)TotalAmt_ItemWise , sum(MAmt)MAmt , Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty FROM ( select   '' as UserName, Cust_Code, max(yy.Customer_Name) as Customer_Name1,' IN-DPT'  as Customer_Name4,max(Customer_Name)+ ' IN-DPT' as Customer_Name,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,'' as State,Item_Code,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date, 
                    max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as MAmt, (Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from ( " & Environment.NewLine & " select  CASE WHEN TSPL_CUSTOMER_MASTER.Credit_Customer = 'y' THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0)), 2)  END ELSE 0 END AS Credit_CustomerQty,dist.Cust_Code, TSPL_CUSTOMER_MASTER1.Customer_Name, TSPL_DEMAND_BOOKING_DETAIL.Item_Code ,Sku_Seq,TSPL_CUSTOMER_MASTER.Credit_Customer,TSPL_ITEM_MASTER.Short_Description from TSPL_DEMAND_BOOKING_DETAIL left outer join TSPL_DEMAND_BOOKING_master on TSPL_DEMAND_BOOKING_master.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code  left join ( select  TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Route_No,max(TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Cust_Code)Cust_Code 
                    from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Route_No ) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No  left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code left outer join TSPL_CUSTOMER_MASTER AS TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =dist.Cust_Code left outer join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No=TSPL_DEMAND_BOOKING_master.Route_No left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code " & whrcls & " ) YY  group by  yy.Cust_Code,yy.Item_Code,Credit_Customer ,Sku_Seq 
                     ) XX GROUP BY xx.Cust_Code,Credit_Customer,Customer_Name, xx.Item_Code,	xx.Sku_Seq ) YYY ) xxx  order by xxx.Customer_Name "
                End If
            End If
