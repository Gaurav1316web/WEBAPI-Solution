    ...
    Private Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btnDetail.Click
        ...
        Try
            BaseQry = " Select Credit_Customer,  CASE WHEN Credit_Customer = 'y' THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.KG, 2)  ELSE  CASE WHEN TSPL_ITEM_MASTER.Is_FreshItem = 1 THEN 
                        ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.LTR, 2) ELSE 0 END END ELSE 0 END AS Credit_CustomerQty,dist.Cust_Code,coalesce(TSPL_CUSTOMER_MASTER.Customer_Name_Hindi,TSPL_CUSTOMER_MASTER.Customer_Name) as Customer_Name  ,TSPL_DEMAND_BOOKING_MASTER.ShiftType,
                       ...
                       ,(CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=0   and Credit_Customer='N' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as PAmt from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No 
	                    ...
	                    left join ( select  Route_No,max(Cust_Code)Cust_Code from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by Route_No) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No  Left Join TSPL_ROUTE_MASTER on TSPL_DEMAND_BOOKING_MASTER.Route_No = TSPL_ROUTE_MASTER.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code left outer join tspl_company_master on 2 = 2
                        left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code WHERE  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date) = CONVERT(DATE, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103)  )"

            If rbtnDistributorWise.IsChecked Then
                DeptQry += "  Select * from  ( Select   '" + objCommonVar.CurrentUser + "' as UserName,'Department Sale' as Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,'Department Sale' AS Customer_Name, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc
		        ,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,sum(ProdQ) as ProdQ ,SUM(MAmt) AS MAmt,SUM(PAmt) AS PAmt,(sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], max(Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty from (    " & Environment.NewLine & " " & BaseQry & " XXXFirst 
                where Item_Code is  not null   and Credit_Customer = 'Y'  Group By  XXXFirst.Item_Code ,XXXFirst.Sku_Seq )YY order by Cust_Code "

                dtDepartmentSale = clsDBFuncationality.GetDataTable(DeptQry)
                query = " select  '" & dtDepartmentSale.Rows.Count & "' as DeptCount,  case when yyy.Credit_Customer='Y'   then isnull(Credit_CustomerQty,0) else isnull(Final_Qty,0) end as final,case when  yyy.Credit_Customer='Y' and Customer_Name4='I-DPT' then Final_Qty else 0 end as InDepartment,* from  ( Select  '" + objCommonVar.CurrentUser + "' as UserName ,Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,max(Customer_Name)Customer_Name, sum(Final_Qty)Final_Qty,
                Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,sum(ProdQ) as ProdQ ,SUM(MAmt) AS MAmt,SUM(PAmt) AS PAmt,
                (sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], (Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty    from ( " & BaseQry & "   XXXFirst 
             where Item_Code is  not null group by  XXXFirst.Cust_Code,Credit_Customer, XXXFirst.Item_Code,	XXXFirst.Sku_Seq "

                query += " union all " & Environment.NewLine & "select '' as UserName, Cust_Code, max(yy.Customer_Name) as Customer_Name1,' IN-DPT'  as Customer_Name4,Customer_Name+ ' IN-DPT' as Customer_Name,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,Item_Code,'' as State,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date, max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as ProdQ,0 as MAmt,0 as Pamt,0 as [Total Amount],
                (Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from ( select CASE WHEN Credit_Customer = 'y' THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.KG, 2) ELSE CASE WHEN TSPL_ITEM_MASTER.Is_FreshItem = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.LTR, 2)   ELSE 0   END END ELSE 0 END AS Credit_CustomerQty,dist.Cust_Code,
                coalesce(TSPL_CUSTOMER_MASTER.Customer_Name_Hindi,TSPL_CUSTOMER_MASTER.Customer_Name) as Customer_Name  ,TSPL_DEMAND_BOOKING_DETAIL.Item_Code ,Sku_Seq,Credit_Customer,TSPL_ITEM_MASTER.Short_Description from TSPL_DEMAND_BOOKING_DETAIL left outer join TSPL_DEMAND_BOOKING_master on TSPL_DEMAND_BOOKING_master.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code  left join ( select  Route_No,max(Cust_Code)Cust_Code 
                from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by Route_No) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No  left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code left outer join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No=TSPL_DEMAND_BOOKING_master.Route_No left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code 
                left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code WHERE  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date) = CONVERT(DATE, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103)  ) YY 
             where Item_Code is  not null group by  yy.Cust_Code,yy.Customer_Name,yy.Item_Code,Credit_Customer ,Sku_Seq ) YYY  order by Cust_Code "


            End If

            If rbtnRouteWise.IsChecked Then
                query = " select  case when yyy.Credit_C    ustomer='Y'   then Credit_CustomerQty else Final_Qty end as final,case when  yyy.Credit_Customer='Y' and route_desc4='I-DPT' then Final_Qty else 0 end as InDepartment,* from  (Select   '" + objCommonVar.CurrentUser + "' as UserName,route_no,max(route_desc)route_desc1,max(route_desc)route_desc4,max(route_desc)route_desc, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc
   ...
   select '' as UserName,route_no, max(yy.Route_Desc) as route_desc1,' IN-DPT'  as Route_Desc4,Route_Desc+ ' IN-DPT' as route_desc,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,Item_Code,'' as State,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date, max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as ProdQ,0 as MAmt,0 as Pamt,0 as [Total Amount], (Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from (select CASE 
	...
	left outer join TSPL_DEMAND_BOOKING_master on TSPL_DEMAND_BOOKING_master.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                If rbtnDistributorWise.IsChecked Then
                    frmCRV.funsubreportWithdt(CrystalReportFolder.KwalitySalesReport, dt, dtDepartmentSale, "crptDailyStatementDistributorWiseDetailcredit", "Daily Statement Distributor Wise", "rptsubDailyStatementDepartSale")
                Else
