    ...
    Private Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btnDetail.Click
        ...
        Try
            ...
            If rbtnDistributorWise.IsChecked Then
                DeptQry += "  Select   '" + objCommonVar.CurrentUser + "' as UserName,'Department Sale' as Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,'Department Sale' AS Customer_Name, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc
		        ,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,SUM(MAmt) AS MAmt,SUM(PAmt) AS PAmt,(sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], max(Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty,isnull(sum(PAmt),0)PAmt,isnull(sum(PAmt),0) + ISNULL(sum(MAmt),0) as [Total Amount] from (    " & Environment.NewLine & " " & BaseQry & "  Left Join TSPL_ROUTE_MASTER on TSPL_DEMAND_BOOKING_MASTER.Route_No = TSPL_ROUTE_MASTER.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code   " & whrcls & " and  Credit_Customer = 'Y'  ) XXXFirst 
                ...
                Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,SUM(MAmt) AS MAmt, (Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty    from ( " & BaseQry & " left join ( select  Route_No,max(Cust_Code)Cust_Code from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by Route_No) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No 
