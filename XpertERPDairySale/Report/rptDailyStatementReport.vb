...
Public Class rptDailyStatementReport
    ...
    Private Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btnDetail.Click
        ...
        If rbtnDistributorWise.IsChecked Then
            ...
            Route = "max(Route_No)Route_No,max(Route_Desc)Route_Desc,"
            GroupCls = " XXXFirst.Cust_Code,Credit_Customer, XXXFirst.Item_Code,XXXFirst.Sku_Seq "
        ElseIf rbtnDisRouteWise.IsChecked Then
            OrderBy = "Customer_Name,Route_No"
            Route = " Route_No,max(Route_Desc)Route_Desc ,"
            GroupCls = "  XXXFirst.Route_No, XXXFirst.Cust_Code,Credit_Customer, XXXFirst.Item_Code,XXXFirst.Sku_Seq "
        End If
        Try
            If rbtnDistributorWise.IsChecked OrElse rbtnDisRouteWise.IsChecked Then
                If rbtnMilkType.IsChecked Then

                    BaseQry = "  THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.KG, 2)  ELSE  CASE WHEN TSPL_ITEM_MASTER.Is_FreshItem = 1 THEN 
                ...
                left join (  Select  Cust_Code,SUM(PAmt) AS PAmt,Item_Code from ( Select TSPL_CUSTOMER_MASTER.Cust_Code,Credit_Customer, (CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=0   and Credit_Customer='Y' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as PAmt,TSPL_ITEM_MASTER.Item_Code from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No 
                Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code  WHERE  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date) = CONVERT(DATE, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103) and Is_Ambient = 1 and Credit_Customer = 'Y'  ) XX group by  xx.Cust_Code,xx.Item_Code  ) xx on xx.Cust_Code = XXXFirst.Cust_Code and xx.Item_Code = XXXFirst.Item_Code Group By  XXXFirst.Item_Code ,XXXFirst.Sku_Seq ) YY  "

                    dtDepartmentSale = clsDBFuncationality.GetDataTable(DeptQry)

                    query = " select *," & dtDepartmentSale.Rows.Count & " AS DepCount,isnull(PAmt1,0)PAmt, isnull(PAmt1,0) + isnull(MAmt,0) as [Total Amount] from (  select ROW_NUMBER() OVER(PARTITION BY " & OrderBy & " ORDER BY Customer_Name) AS SNO, case when yyy.Credit_Customer='Y'   then isnull(Credit_CustomerQty,0) else isnull(Final_Qty,0) end as final,case when  yyy.Credit_Customer='Y' and Customer_Name4='I-DPT' then Final_Qty else 0 end as InDepartment,* from ( Select " & Route & "  '" + objCommonVar.CurrentUser + "' as UserName ,Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,max(Customer_Name)Customer_Name, sum(Final_Qty)Final_Qty,
                Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,SUM(MAmt) AS MAmt, (Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty from ( Select TSPL_ROUTE_MASTER.Route_Desc ,dist.Route_No, TSPL_CUSTOMER_MASTER1.Credit_Customer,  CASE WHEN TSPL_CUSTOMER_MASTER1.Credit_Customer = 'y' " & BaseQry & " (CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=1 and TSPL_CUSTOMER_MASTER1.Credit_Customer='N' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as MAmt from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No 
                Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left outer join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.Unit_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_code left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code 
                ...
                Left Join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No = TSPL_DEMAND_BOOKING_master.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code  	left outer join TSPL_CUSTOMER_MASTER AS TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code
                " & whrcls & " and TSPL_CUSTOMER_MASTER1.Credit_Customer = 'N' )  XXXFirst 
                where Item_Code is  not null group by  " & GroupCls & ""

                    query += " union all " & Environment.NewLine & " Select " & Route & " max(UserName)UserName,Cust_Code,max(Customer_Name1) as Customer_Name1,max(Customer_Name4)Customer_Name4 , max(Customer_Name)Customer_Name , sum(Final_Qty)Final_Qty , Sku_Seq,max(phone2)phone2 , max(phone1)phone1 , max(Circle_No)Circle_No , max(Comp_Name)Comp_Name , max(City_Code)City_Code , max(state)State , Item_Code, max(Add1)Add1 , max(Add2)Add2 , max(Pincode)Pincode , max(fax)fax , max(date)date , max(Short_Description)Short_Description , max(Unit_Desc)Unit_Desc , sum(TotalLtr_ItemWise)TotalLtr_ItemWise , sum(TotalAmt_ItemWise)TotalAmt_ItemWise , sum(MAmt)MAmt , Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty FROM
                ( select " & Route & " '' as UserName, Cust_Code, max(XXXFirst.Customer_Name) as Customer_Name1,' IN-DPT'  as Customer_Name4,max(Customer_Name)+ ' IN-DPT' as Customer_Name,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,Item_Code,'' as State,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date, max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as ProdQ,0 as MAmt,0 as Pamt,0 as [Total Amount],
                (Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from (  select  TSPL_ROUTE_MASTER.Route_Desc,dist.Route_No, CASE WHEN TSPL_CUSTOMER_MASTER.Credit_Customer = 'y' THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.KG, 2) ELSE CASE WHEN TSPL_ITEM_MASTER.Is_FreshItem = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.LTR, 2)   ELSE 0   END END ELSE 0 END AS Credit_CustomerQty,dist.Cust_Code,
                ...
                left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code " & whrcls & " ) XXXFirst group by " & GroupCls & "  "

                    query += " union all " & Environment.NewLine & " select " & Route & " '' as UserName, Cust_Code, max(XXXFirst.Customer_Name) as Customer_Name1,' IN-DPT'  as Customer_Name4,max(Customer_Name)+ ' IN-DPT' as Customer_Name,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,Item_Code,'' as State,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date, max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as ProdQ,0 as MAmt,0 as Pamt,0 as [Total Amount],
                (Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from ( select TSPL_ROUTE_MASTER.Route_Desc,dist.Route_No, 0 AS Credit_CustomerQty,dist.Cust_Code,
				...
				left outer join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No=TSPL_DEMAND_BOOKING_master.Route_No left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code 
                left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code " & whrcls & " ) XXXFirst group by  " & GroupCls & " ) XX "
                    If rbtnDistributorWise.IsChecked Then
                        query += "GROUP BY xx.Cust_Code,Credit_Customer,Customer_Name, xx.Item_Code,xx.Sku_Seq "
                    ElseIf rbtnDisRouteWise.IsChecked Then
                        query += "GROUP BY xx.Route_No, xx.Cust_Code,Credit_Customer,Customer_Name, xx.Item_Code,xx.Sku_Seq "
                    End If
                    query += "  ) YYY  ) xxx left join ("
                    If rbtnDistributorWise.IsChecked Then
                        query += "Select  Cust_Code,isnull(SUM(PAmt),0) AS PAmt1 from ( Select  "
                    ElseIf rbtnDisRouteWise.IsChecked Then
                        query += "Select Route_No, Cust_Code,isnull(SUM(PAmt),0) AS PAmt1 from ( Select TSPL_ROUTE_MASTER.Route_No, "
                    End If
                    query += "  TSPL_CUSTOMER_MASTER.Cust_Code, 
                       (CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=0   and TSPL_CUSTOMER_MASTER1.Credit_Customer='N' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as PAmt from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No 
                        Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left join ( select  Route_No,max(Cust_Code)Cust_Code from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by Route_No) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No 
               Left Join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No = TSPL_DEMAND_BOOKING_master.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code left outer join TSPL_CUSTOMER_MASTER as TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code
                WHERE  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date) = CONVERT(DATE, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103) and Is_Ambient = 1 and TSPL_CUSTOMER_MASTER1.Credit_Customer = 'N') XXXFirst "

                    If rbtnDisRouteWise.IsChecked Then
                        query += "  group by XXXFirst.Route_No, XXXFirst.Cust_Code ) xx on xx.Route_No = xxx.Route_No and xx.Cust_Code = xxx.Cust_Code "
                    ElseIf rbtnDistributorWise.IsChecked Then
                        query += "  group by  XXXFirst.Cust_Code ) xx on xx.Cust_Code = xxx.Cust_Code "
                    End If
                    query += "  and xxx.Customer_Name4 <> ' IN-DPT' and xxx.SNO = 1 order by xxx.Customer_Name "

                ElseIf rbtnProduct.IsChecked Then
                    DeptQry = " Select *,ISNULL(MAmt,0) as [Total Amount] from  ( Select '" + objCommonVar.CurrentUser + "' as UserName,'Department Sale' as Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,'Department Sale' AS Customer_Name, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,XXXFirst.Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc
		            ,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,SUM(MAmt) AS MAmt,(sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], max(Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty, isnull(sum(PAmt),0)PAmt from (  Select Credit_Customer,  CASE WHEN Credit_Customer = 'y' THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) ), 2)   END ELSE 0 END AS Credit_CustomerQty,TSPL_CUSTOMER_MASTER.Cust_Code,coalesce(TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Customer_Name) as Customer_Name  ,TSPL_DEMAND_BOOKING_MASTER.ShiftType,
                    case when TSPL_ITEM_MASTER.Is_Ambient = 1 then round((isnull(TSPL_DEMAND_BOOKING_DETAIL.Qty,0)),2) else 0 end as Final_Qty ,case when TSPL_ITEM_MASTER.Is_FreshItem = 1 then	round((isnull(TSPL_DEMAND_BOOKING_DETAIL.Qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.KG,2) else 0 end as Kg_Qty , tspl_company_master.State,tspl_company_master.City_Code,tspl_company_master.Circle_No,tspl_company_master.Phone1,tspl_company_master.Phone2,   tspl_company_master.Comp_Name ,tspl_company_master.Add1,tspl_company_master.Add2,tspl_company_master.Pincode,tspl_company_master.Fax,'" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "' as Date, TSPL_ITEM_MASTER.Sku_Seq , TSPL_DEMAND_BOOKING_DETAIL.Item_Code,TSPL_ITEM_MASTER.Short_Description as Short_Description,TSPL_UNIT_MASTER.Unit_Desc,TSPL_DEMAND_BOOKING_DETAIL.TotalLtr_ItemWise,TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount
                    ,TSPL_DEMAND_BOOKING_DETAIL.TotalCrates_ItemWise ,(CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=0 THEN TSPL_DEMAND_BOOKING_DETAIL.Qty ELSE 0 END) as ProdQ,(CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=1 and Credit_Customer='Y' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as MAmt from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No  Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left outer join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.Unit_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_code left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code  left outer join tspl_company_master 
                    on 2 = 2 left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code   Left Join TSPL_ROUTE_MASTER on TSPL_DEMAND_BOOKING_MASTER.Route_No = TSPL_ROUTE_MASTER.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code  " & whrcls & " and  Credit_Customer = 'Y' ) XXXFirst left join (  Select  Cust_Code,SUM(PAmt) AS PAmt,Item_Code from ( Select TSPL_CUSTOMER_MASTER.Cust_Code,Credit_Customer, (CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=0   and Credit_Customer='Y' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as PAmt, TSPL_ITEM_MASTER.Item_Code from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No 
                   Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code " & whrcls & "  and Credit_Customer = 'Y'  ) XX group by  xx.Cust_Code,xx.Item_Code ) xx on xx.Cust_Code = XXXFirst.Cust_Code  and xx.Item_Code = XXXFirst.Item_Code Group By  XXXFirst.Item_Code ,XXXFirst.Sku_Seq ) YY "
                    dtDepartmentSale = clsDBFuncationality.GetDataTable(DeptQry)
                    query = "select *," & dtDepartmentSale.Rows.Count & " AS DepCount,isnull(TotalAmt_ItemWise,0)PAmt, isnull(TotalAmt_ItemWise,0) + isnull(MAmt,0) as [Total Amount], 'Qty' as QtyCol from ( select case when yyy.Credit_Customer='Y'   then isnull(Credit_CustomerQty,0) else isnull(Final_Qty,0) end as final,case when  yyy.Credit_Customer='Y' and Customer_Name4='I-DPT' then Final_Qty else 0 end as InDepartment,* from  ( Select " & Route & " '" + objCommonVar.CurrentUser + "'  UserName ,Cust_Code,max(Customer_Name)Customer_Name1,max(Customer_Name)Customer_Name4,max(Customer_Name)Customer_Name, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State,Item_Code, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,SUM(MAmt) AS MAmt,
                   (Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty    from ( " & Environment.NewLine & " Select TSPL_ROUTE_MASTER.Route_Desc ,dist.Route_No,  TSPL_CUSTOMER_MASTER1.Credit_Customer,  CASE WHEN TSPL_CUSTOMER_MASTER1.Credit_Customer = 'y' THEN  CASE WHEN TSPL_ITEM_MASTER.Is_FreshItem = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.LTR, 2) ELSE 0 END  END AS Credit_CustomerQty,TSPL_CUSTOMER_MASTER.Cust_Code,coalesce(TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Customer_Name) as Customer_Name  ,TSPL_DEMAND_BOOKING_MASTER.ShiftType,
                    case when TSPL_ITEM_MASTER.Is_Ambient = 1 then round((isnull(TSPL_DEMAND_BOOKING_DETAIL.Qty,0)),2) end as Final_Qty,tspl_company_master.State,tspl_company_master.City_Code,tspl_company_master.Circle_No,tspl_company_master.Phone1,tspl_company_master.Phone2, tspl_company_master.Comp_Name,tspl_company_master.Add1,tspl_company_master.Add2,tspl_company_master.Pincode,tspl_company_master.Fax , '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "' as Date, TSPL_ITEM_MASTER.Sku_Seq , TSPL_DEMAND_BOOKING_DETAIL.Item_Code,TSPL_ITEM_MASTER.Short_Description as Short_Description,TSPL_UNIT_MASTER.Unit_Desc,TSPL_DEMAND_BOOKING_DETAIL.TotalLtr_ItemWise,TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ,TSPL_DEMAND_BOOKING_DETAIL.TotalCrates_ItemWise ,(CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=0 THEN TSPL_DEMAND_BOOKING_DETAIL.Qty ELSE 0 END) as ProdQ,(CASE WHEN TSPL_ITEM_MASTER.Is_Milk_Pouch=1 and TSPL_CUSTOMER_MASTER1.Credit_Customer='N' THEN TSPL_DEMAND_BOOKING_DETAIL.ItemNetAmount ELSE 0 END) as MAmt 
                   from TSPL_DEMAND_BOOKING_MASTER  Left outer join TSPL_DEMAND_BOOKING_DETAIL On TSPL_DEMAND_BOOKING_MASTER.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No  Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code left outer join TSPL_UNIT_MASTER on TSPL_UNIT_MASTER.Unit_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_code left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code  left outer join tspl_company_master on 2 = 2 left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code  left join ( select  Route_No,max(Cust_Code)Cust_Code from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by Route_No) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No 
                   Left Join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No = TSPL_DEMAND_BOOKING_master.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =dist.Cust_Code left outer join TSPL_CUSTOMER_MASTER AS TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code " & whrcls & "  and TSPL_CUSTOMER_MASTER1.Credit_Customer = 'N' )  XXXFirst  where Item_Code is  not null group by " & GroupCls & " "

                    query += " union all " & Environment.NewLine & " SELECT  " & Route & " max(UserName)UserName,Cust_Code,max(Customer_Name1) as Customer_Name1,max(Customer_Name4)Customer_Name4 , max(Customer_Name)Customer_Name , sum(Final_Qty)Final_Qty , Sku_Seq,max(phone2)phone2 , max(phone1)phone1 , max(Circle_No)Circle_No , max(Comp_Name)Comp_Name , max(City_Code)City_Code , max(state)State , Item_Code, max(Add1)Add1 , max(Add2)Add2 , max(Pincode)Pincode , max(fax)fax , max(date)date , max(Short_Description)Short_Description , max(Unit_Desc)Unit_Desc , sum(TotalLtr_ItemWise)TotalLtr_ItemWise , sum(TotalAmt_ItemWise)TotalAmt_ItemWise , sum(MAmt)MAmt , Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty FROM ( select " & Route & "  '' as UserName, Cust_Code, max(XXXFirst.Customer_Name) as Customer_Name1,' IN-DPT'  as Customer_Name4,max(Customer_Name)+ ' IN-DPT' as Customer_Name,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,'' as State,Item_Code,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date, 
                    max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as MAmt, (Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from ( " & Environment.NewLine & " select TSPL_ROUTE_MASTER.Route_Desc ,dist.Route_No,  CASE WHEN TSPL_CUSTOMER_MASTER.Credit_Customer = 'y' THEN CASE WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0)), 2)  END ELSE 0 END AS Credit_CustomerQty,dist.Cust_Code, TSPL_CUSTOMER_MASTER1.Customer_Name, TSPL_DEMAND_BOOKING_DETAIL.Item_Code ,Sku_Seq,TSPL_CUSTOMER_MASTER.Credit_Customer,TSPL_ITEM_MASTER.Short_Description from TSPL_DEMAND_BOOKING_DETAIL left outer join TSPL_DEMAND_BOOKING_master on TSPL_DEMAND_BOOKING_master.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code  left join ( select  TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Route_No,max(TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Cust_Code)Cust_Code 
                    from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Route_No ) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No  left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code left outer join TSPL_CUSTOMER_MASTER AS TSPL_CUSTOMER_MASTER1 on TSPL_CUSTOMER_MASTER1.Cust_Code =dist.Cust_Code left outer join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No=TSPL_DEMAND_BOOKING_master.Route_No left join TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code   and TSPL_ITEM_UOM_DETAIL.UOM_Code=TSPL_DEMAND_BOOKING_DETAIL.Unit_Code left join (  SELECT * FROM ( select item_code,uom_code,conversion_factor from TSPL_ITEM_UOM_DETAIL) I  PIVOT (Max(conversion_factor) FOR uom_code IN ( [KG],[LTR] )) P ) I ON TSPL_DEMAND_BOOKING_DETAIL.Item_Code = I.item_code " & whrcls & " ) XXXFirst  group by " & GroupCls & ""
                    If rbtnDistributorWise.IsChecked Then
                        query += " ) XX GROUP BY xx.Cust_Code,Credit_Customer,Customer_Name, xx.Item_Code,	xx.Sku_Seq ) YYY ) xxx  order by xxx.Customer_Name "
                    ElseIf rbtnDisRouteWise.IsChecked Then
                        query += ") XX GROUP BY xx.Route_No, xx.Cust_Code,Credit_Customer,Customer_Name, xx.Item_Code,	xx.Sku_Seq ) YYY ) xxx  order by xxx.Customer_Name "
                    End If
                End If
            End If
