    ...
    Private Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btnDetail.Click
        ...
        Try
            ...
            If rbtnDistributorWise.IsChecked Then
		        ...
		        ,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,sum(ProdQ) as ProdQ ,SUM(MAmt) AS MAmt,SUM(PAmt) AS PAmt,(sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], max(Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty from (    " & Environment.NewLine & " " & BaseQry & "  Left Join TSPL_ROUTE_MASTER on TSPL_DEMAND_BOOKING_MASTER.Route_No = TSPL_ROUTE_MASTER.Route_No left outer join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_DEMAND_BOOKING_DETAIL.Cust_Code   WHERE  convert(date,TSPL_DEMAND_BOOKING_MASTER.Document_Date) = CONVERT(DATE, '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "', 103)  ) XXXFirst 
                ...
                (sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], (Credit_Customer)Credit_Customer,isnull(cast(sum(Credit_CustomerQty) as decimal(18,2)),0) Credit_CustomerQty    from ( " & BaseQry & " left join ( select  Route_No,max(Cust_Code)Cust_Code from TSPL_DISTRIBUTOR_ROUTE_CUSTOMER group by Route_No) dist on dist.Route_No = TSPL_DEMAND_BOOKING_MASTER.Route_No 
               ...
               where Item_Code is  not null group by  XXXFirst.Cust_Code,Credit_Customer, XXXFirst.Item_Code,	XXXFirst.Sku_Seq "
                ...
                coalesce(TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Customer_Name) as Customer_Name  ,TSPL_DEMAND_BOOKING_DETAIL.Item_Code ,Sku_Seq,Credit_Customer,TSPL_ITEM_MASTER.Short_Description from TSPL_DEMAND_BOOKING_DETAIL left outer join TSPL_DEMAND_BOOKING_master on TSPL_DEMAND_BOOKING_master.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code  left join ( select  Route_No,max(Cust_Code)Cust_Code 
