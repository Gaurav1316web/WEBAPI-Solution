...
Public Class rptDailyStatementReport
    ...
    Private Sub btnDetail_Click(sender As Object, e As EventArgs) Handles btnDetail.Click
        ...
        from (  " & Environment.NewLine & " " & BaseQry & " and Credit_Customer = 'Y'  Group By  XXXFirst.Item_Code ,XXXFirst.Sku_Seq   order by Group_Cust  "
                ...
                '    query += " group by  XXXFirst.Route_No, XXXFirst.Cust_Code,	XXXFirst.Item_Code ,XXXFirst.Sku_Seq  order by XXXFirst.Route_No ASC "

            End If

            If rbtnRouteWise.IsChecked Then
                query = " select  case when yyy.Credit_Customer='Y'   then Credit_CustomerQty else Final_Qty end as final,case when  yyy.Credit_Customer='Y' and route_desc4='In Department' then Final_Qty else 0 end as InDepartment,* from  (Select   '" + objCommonVar.CurrentUser + "' as UserName,route_no,max(route_desc)route_desc1,max(route_desc)route_desc4,max(route_desc)route_desc, sum(Final_Qty)Final_Qty, Sku_Seq,max(Phone2)Phone2,max(Phone1)Phone1,max(Circle_No)Circle_No, max(Comp_Name)Comp_Name,max(City_Code)City_Code,max(State)State, max(Add1)Add1,max(Add2)Add2,max(Pincode)Pincode,max(Fax)Fax,max(Date)Date,Item_Code,max(Short_Description) AS Short_Description,max(Unit_Desc) as Unit_Desc
		,sum(TotalLtr_ItemWise) as TotalLtr_ItemWise,sum(ItemNetAmount) as TotalAmt_ItemWise,sum(ProdQ) as ProdQ ,SUM(MAmt) AS MAmt,SUM(PAmt) AS PAmt,(sum(isnull(MAmt,0))+sum(isnull(PAmt,0))) as [Total Amount], max(Credit_Customer)Credit_Customer,cast(sum(Credit_CustomerQty) as decimal(18,2)) Credit_CustomerQty
        from (  
 ...
 case when TSPL_ITEM_MASTER.Is_Ambient = 1 then round((isnull(TSPL_DEMAND_BOOKING_DETAIL.Qty,0) *isnull(TSPL_ITEM_UOM_DETAIL.Conversion_Factor,1))/I.KG,2) 
   ...
   where Item_Code is  not null    group by  XXXFirst.Route_No, 	XXXFirst.Item_Code ,XXXFirst.Sku_Seq 

   union all

   select '' as UserName,route_no, max(yy.Route_Desc) as route_desc1,' In Department'  as Route_Desc4,Route_Desc+' In Department' as route_desc,0 as Final_Qty,Sku_Seq,'' as Phone2,'' as Phone1,'' as Circle_No,'' as Comp_Name,'' as City_Code,'' as State,'' as Add1,'' as Add2,'' as Pincode,'' as fax,'' as date,Item_Code, max(Short_Description)Short_Description,'' as Unit_Desc,0 as TotalLtr_ItemWise,0 as TotalAmt_ItemWise,0 as ProdQ,0 as MAmt,0 as Pamt,0 as [Total Amount], max(Credit_Customer)Credit_Customer,Sum(Credit_CustomerQty)Credit_CustomerQty from (select CASE 
        WHEN Credit_Customer = 'y' THEN 
            CASE 
                WHEN TSPL_ITEM_MASTER.Is_Ambient = 1 THEN 
                    ROUND((ISNULL(TSPL_DEMAND_BOOKING_DETAIL.Qty, 0) * ISNULL(TSPL_ITEM_UOM_DETAIL.Conversion_Factor, 1)) / I.KG, 2) 
	...
	left outer join TSPL_DEMAND_BOOKING_master on TSPL_DEMAND_BOOKING_master.Document_No=TSPL_DEMAND_BOOKING_DETAIL.Document_No
	        Left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_DEMAND_BOOKING_DETAIL.Item_Code 
					...
					left outer join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No=TSPL_DEMAND_BOOKING_master.Route_No
					 ...
					 group by  yy.Route_no,yy.Route_Desc,yy.Item_Code ,Sku_Seq)YYY  order by  Route_No"
            End If
