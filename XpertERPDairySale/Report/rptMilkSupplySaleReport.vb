        ...
        Try
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JPR") = CompairStringResult.Equal Then
                If clsCommon.GetPrintDate(txtFromDate.Value, "dd-MMM-yyyy") <> clsCommon.GetPrintDate(txtToDate.Value, "dd-MMM-yyyy") Then
                    whrcls = " And Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" & txtFromDate.Value & "', 103)  and   convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= CONVERT(DATE, '" & txtToDate.Value.AddDays(+1) & "', 103)"
                Else
                    whrcls = " And Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" & txtFromDate.Value & "', 103)  and   convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= CONVERT(DATE, '" & txtToDate.Value & "', 103)"
                End If
            Else
                whrcls = " And Convert(Date, TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) >= Convert(Date, '" & txtFromDate.Value & "', 103)  and   convert(date,TSPL_SD_SHIPMENT_HEAD.Supply_Date,103) <= CONVERT(DATE, '" & txtToDate.Value & "', 103)"
            End If
            ...
            ElseIf rbtnBothShift.IsChecked Then
                If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JPR") = CompairStringResult.Equal Then
                    If clsCommon.GetPrintDate(txtFromDate.Value, "dd-MMM-yyyy") = clsCommon.GetPrintDate(txtToDate.Value, "dd-MMM-yyyy") Then
                        whrcls += " and Shift_Type = 'PM' "
                    Else
                        whrcls += "  AND ((CONVERT(DATE, TSPL_SD_SHIPMENT_HEAD.Supply_Date, 103) = '" + clsCommon.GetPrintDate(txtFromDate.Value, "dd/MMM/yyyy") + "'  AND TSPL_SD_SHIPMENT_HEAD.Shift_Type IN ('PM')) 
        OR (CONVERT(DATE, TSPL_SD_SHIPMENT_HEAD.Supply_Date, 103) = '" + clsCommon.GetPrintDate(txtToDate.Value, "dd/MMM/yyyy") + "' AND TSPL_SD_SHIPMENT_HEAD.Shift_Type IN ('AM', 'PM')) 
        OR (CONVERT(DATE, TSPL_SD_SHIPMENT_HEAD.Supply_Date, 103) = '" + clsCommon.GetPrintDate(txtToDate.Value.AddDays(+1), "dd/MMM/yyyy") + "'  AND TSPL_SD_SHIPMENT_HEAD.Shift_Type = 'AM') )"
                    End If
                End If
            End If
            ...
            BaseQry = " Select TSPL_ITEM_MASTER.Item_Code ,Sku_Seq, TSPL_CUSTOMER_MASTER.Cust_Code ,TSPL_CUSTOMER_MASTER.Customer_Name, TSPL_SD_SALE_INVOICE_HEAD.Route_No, TSPL_SD_SALE_INVOICE_HEAD.Route_Desc,"
            If rbtnDetail.IsChecked Then
            ...
            End If
            BaseQry += "  TSPL_ITEM_MASTER.Item_Desc, TSPL_SD_SALE_INVOICE_DETAIL.Item_Net_Amt As Amount, TSPL_ITEM_MASTER.Short_Description + ' (' +  I.UOM_Code + ')' as Short_Description,isnull((TSPL_SD_SALE_INVOICE_DETAIL.Qty * isnull((TSPL_ITEM_UOM_DETAIL.Conversion_Factor),1)) /(I.Conversion_Factor),0) As Report_UOM_Qty
            ...
            where 2 = 2   and TSPL_SD_SALE_INVOICE_HEAD.Status = 1 " & whrcls & " order by Supply_Date, TSPL_SD_SHIPMENT_HEAD.Shift_Type, Sku_Seq "
            ...
            If dt.Rows.Count > 0 Then
                ...
                PropertyGroupDescription4.SortOrder = SortOrder.Ascending
