...
from 
  ...
  left outer join TSPL_LOCATION_MASTER on TSPL_SD_SALE_INVOICE_HEAD.Bill_To_Location = TSPL_LOCATION_MASTER.Location_Code"
    ...
    Private Sub btnPrintMultipleInvoice_Click(sender As Object, e As EventArgs) Handles btnPrintMultipleInvoice.Click
        PrintMultipleInvoice(Me.Form_ID, Me.Text, Nothing, False)
    End Sub

    Sub PrintMultipleInvoice(ByVal strFormID As String, ByVal strFormText As String, ByVal strInvoice As String, ByVal isCancelOrDelete As String)
        Try
            ...
            Else
                ...
                If strIgstInv IsNot Nothing AndAlso strIgstInv.Count > 0 Then
                    If clsCommon.MyMessageBoxShow("Are you sure to print IGST Invoice ?", strFormText, MessageBoxButtons.YesNo) = DialogResult.Yes Then
            ...
            If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "GNG") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JSL") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrComp_Code1, "NAG") = CompairStringResult.Equal Then
                If strInvoiceno.Count > 0 Then
                    If common.clsCommon.MyMessageBoxShow(" Do you want to print Invoice", strFormText, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then

                        Dim dtDocdate As Date?
                        dtDocdate = Nothing
                        Dim StrSql As String = Nothing
                        StrSql = "Select Document_Code,Document_Date,Customer_Code,Bill_To_Location,is_taxable,Tax_Group from "
                        If clsCommon.CompairString(isCancelOrDelete, "Cancel") = CompairStringResult.Equal Then
                            StrSql &= " TSPL_SD_SALE_INVOICE_HEAD_Cancel_Data "
                        ElseIf clsCommon.CompairString(isCancelOrDelete, "Delete") = CompairStringResult.Equal Then
                            StrSql &= " TSPL_SD_SALE_INVOICE_HEAD_Delete_Data "
                        Else
                            StrSql &= " TSPL_SD_SALE_INVOICE_HEAD "
                        End If
                        StrSql &= " where Document_Code in (" & clsCommon.GetMulcallString(strInvoiceno) & ")"
                        ...
                        Else
                            dtDocdate = clsCommon.myCDate(gv1.Rows(0).Cells(1).Value)
                        End If
                        Dim strCust As String = Nothing
                        If clsCommon.CompairString(isCancelOrDelete, "Cancel") = CompairStringResult.Equal OrElse clsCommon.CompairString(isCancelOrDelete, "Delete") = CompairStringResult.Equal Then
                            If clsCommon.myLen(clsCommon.myCstr(dt1.Rows(0)("Customer_Code"))) > 0 Then
                                strCust = clsCommon.myCstr(dt1.Rows(0)("Customer_Code"))
                            End If
                        Else
                            If clsCommon.myLen(clsCommon.myCstr(gv1.Rows(0).Cells(2).Value)) > 0 Then
                                strCust = clsCommon.myCstr(gv1.Rows(0).Cells(2).Value)
                            End If
                        End If

                        qry = objMultPrintInvoice.PrintInvoiceForAll(clsCommon.GetMulcallString(strInvoiceno), dtDocdate, strCust, Nothing, isCancelOrDelete)
                        ...
                        If clsCommon.CompairString(objCommonVar.CurrComp_Code1, "GNG") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrComp_Code1, "JSL") = CompairStringResult.Equal OrElse clsCommon.CompairString(objCommonVar.CurrComp_Code1, "NAG") = CompairStringResult.Equal Then
                            ...
                            If clsCommon.myCstr(dt.Rows(0)("TAX1")) = "IGST" OrElse clsCommon.myCstr(dt.Rows(0)("TAX2")) = "IGST" OrElse clsCommon.myCstr(dt.Rows(0)("TAX3")) = "IGST" OrElse clsCommon.myCstr(dt.Rows(0)("TAX4")) = "IGST" OrElse clsCommon.myCstr(dt.Rows(0)("TAX5")) = "IGST" OrElse clsCommon.myCstr(dt.Rows(0)("TAX6")) = "IGST" Then
                                frmCRV.funsubreportWithdt(strFormID, CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "crptTaxableNonTaxableInvoiceIGST", "Bill of Supply", dtDocdate, "rptCompanyAddress.rpt", "FreshHeader.rpt", clsERPFuncationality.CompanyAddresInvoiceHeader())
                            Else
                                frmCRV.funsubreportWithdt(strFormID, CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "crptTaxableNonTaxableInvoiceGNG1", "Bill of Supply", dtDocdate, "rptCompanyAddress.rpt", "FreshHeader.rpt", clsERPFuncationality.CompanyAddresInvoiceHeader())
                            End If
            ...
            Else
                If strInvoiceno.Count > 0 Then
                    If common.clsCommon.MyMessageBoxShow(" Do you want to print Invoice", strFormText, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                        Dim TSPL_SD_SALE_INVOICE_HEAD As String = Nothing
                        Dim TSPL_SD_SALE_INVOICE_DETAIL As String = Nothing
                        Dim TSPL_SD_SHIPMENT_DETAIL As String = Nothing
                        Dim TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL As String = Nothing
                        Dim TSPL_DAIRYSALE_GATEPASS_MASTER As String = Nothing
                        If clsCommon.CompairString(isCancelOrDelete, "Cancel") = CompairStringResult.Equal Then
                            TSPL_SD_SALE_INVOICE_HEAD = " TSPL_SD_SALE_INVOICE_HEAD_Cancel_Data As TSPL_SD_SALE_INVOICE_HEAD "
                            TSPL_SD_SALE_INVOICE_DETAIL = " TSPL_SD_SALE_INVOICE_DETAIL_Cancel_Data As TSPL_SD_SALE_INVOICE_DETAIL "
                            TSPL_SD_SHIPMENT_DETAIL = " TSPL_SD_SHIPMENT_DETAIL_Cancel_Data As TSPL_SD_SHIPMENT_DETAIL "
                            TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL = " TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL_Cancel_Data As TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL "
                            TSPL_DAIRYSALE_GATEPASS_MASTER = " TSPL_DAIRYSALE_GATEPASS_MASTER_Cancel_Data As TSPL_DAIRYSALE_GATEPASS_MASTER "
                        ElseIf clsCommon.CompairString(isCancelOrDelete, "Delete") = CompairStringResult.Equal Then
                            TSPL_SD_SALE_INVOICE_HEAD = " TSPL_SD_SALE_INVOICE_HEAD_Delete_Data As TSPL_SD_SALE_INVOICE_HEAD "
                            TSPL_SD_SALE_INVOICE_DETAIL = " TSPL_SD_SALE_INVOICE_DETAIL_Delete_Data As TSPL_SD_SALE_INVOICE_DETAIL "
                            TSPL_SD_SHIPMENT_DETAIL = " TSPL_SD_SHIPMENT_DETAIL_Delete_Data As TSPL_SD_SHIPMENT_DETAIL "
                            TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL = " TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL_Delete_Data As TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL "
                            TSPL_DAIRYSALE_GATEPASS_MASTER = " TSPL_DAIRYSALE_GATEPASS_MASTER_Delete_Data As TSPL_DAIRYSALE_GATEPASS_MASTER "
                        Else
                            TSPL_SD_SALE_INVOICE_HEAD = " TSPL_SD_SALE_INVOICE_HEAD "
                            TSPL_SD_SALE_INVOICE_DETAIL = " TSPL_SD_SALE_INVOICE_DETAIL "
                            TSPL_SD_SHIPMENT_DETAIL = " TSPL_SD_SHIPMENT_DETAIL "
                            TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL = " TSPL_DAIRYSALE_GATEPASS_SHIPMENT_DETAIL "
                            TSPL_DAIRYSALE_GATEPASS_MASTER = " TSPL_DAIRYSALE_GATEPASS_MASTER "
                        End If
...
left join TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code='UDP'"
                        Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
                        If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                            frmCRV.funsubreportWithdt(strFormID, CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "crptMultipleInvoicePrint", "Bill of Supply", "rptCompanyAddress.rpt", "FreshHeader.rpt", clsERPFuncationality.CompanyAddresInvoiceHeader())
                        End If
                    Else
                        frmCRV = Nothing
                    End If
