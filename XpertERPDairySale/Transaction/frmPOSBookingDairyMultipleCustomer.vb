...
Public Class frmPOSBookingDairyMultipleCustomer
        ...
        Try
            ...
            If ShowItemLocationWiseonBooking = 0 Then
                If clsCommon.myLen(txtDocNo.Value) > 0 Then
                    qry = "select * from (select  distinct Line_No,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,TSPL_STRUCTURE_MASTER.Structure_Descq as CSA_TYPE ,TSPL_BOOKING_DETAIL.Unit_code as UOM_Code,0 as Total_Qty,Sku_Seq,isnull(tspl_customer_master.Cust_Code,'') as Cust_Code, isnull(Booking_Qty,0) as Qty  from TSPL_BOOKING_DETAIL left outer join TSPL_ITEM_MASTER on TSPL_BOOKING_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code LEFT OUTER JOIN TSPL_CUSTOMER_MASTER  ON TSPL_BOOKING_DETAIL.Cust_Code=TSPL_CUSTOMER_MASTER.Cust_Code LEFT OUTER JOIN TSPL_LOCATION_MASTER  ON TSPL_BOOKING_DETAIL.Loc_Code =TSPL_LOCATION_MASTER.Location_Code LEFT OUTER JOIN TSPL_ROUTE_MASTER  ON TSPL_ROUTE_MASTER.Route_No   =TSPL_CUSTOMER_MASTER.Route_No LEFT OUTER JOIN TSPL_VEHICLE_MASTER  ON TSPL_VEHICLE_MASTER.Vehicle_Id  =TSPL_BOOKING_DETAIL.Vehicle_Code left outer join TSPL_STRUCTURE_MASTER on TSPL_ITEM_MASTER.Structure_Code=TSPL_STRUCTURE_MASTER.Structure_Code  where isnull(TSPL_BOOKING_DETAIL.Scheme_Item,'N')<>'Y' and isnull(TSPL_BOOKING_DETAIL.FOC_Item,'0')<>'1' and  Document_No='" & txtDocNo.Value & "' union all " & _
                        "select  1 as Line_No ,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,TSPL_STRUCTURE_MASTER.Structure_Descq as CSA_TYPE,TSPL_ITEM_UOM_DETAIL.UOM_Code,0.00 as Total_Qty ,Sku_Seq,'' as Cust_Code, 0.00 as Qty from tspl_item_master left outer join TSPL_ITEM_UOM_DETAIL on tspl_item_master.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1  left outer join TSPL_STRUCTURE_MASTER on TSPL_ITEM_MASTER.Structure_Code=TSPL_STRUCTURE_MASTER.Structure_Code where " & strItem & "  and tspl_item_master.Item_Code not in (select TSPL_BOOKING_DETAIL.Item_Code from TSPL_BOOKING_DETAIL where Document_No='" & txtDocNo.Value & "' ) " & _
                        ") as t pivot(max(Qty) for Cust_Code in (" & strtotalShort & ") ) as pivot1 order by Sku_Seq asc"
                Else
                    qry = "select * from (select  1 as Line_No ,tspl_item_master.Item_Code,TSPL_STRUCTURE_MASTER.Structure_Descq as CSA_TYPE ,TSPL_ITEM_UOM_DETAIL.UOM_Code,0.00 as Total_Qty,Sku_Seq,'' as Cust_Code, 0.00 as Qty  from tspl_item_master left outer join TSPL_ITEM_UOM_DETAIL on tspl_item_master.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1 left outer join TSPL_STRUCTURE_MASTER on TSPL_ITEM_MASTER.Structure_Code=TSPL_STRUCTURE_MASTER.Structure_Code where " & strItem & " ) as t pivot(max(Qty) for Cust_Code in (" & strtotalShort & ") ) as pivot1 order by Sku_Seq asc"
                End If
            Else
                If clsCommon.myLen(txtDocNo.Value) > 0 Then
                    qry = "select * from (select  distinct Line_No,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,TSPL_STRUCTURE_MASTER.Structure_Descq as CSA_TYPE ,TSPL_BOOKING_DETAIL.Unit_code as UOM_Code,0.00 as Total_Qty,Sku_Seq,isnull(tspl_customer_master.Cust_Code,'') as Cust_Code, isnull(Booking_Qty,0) as Qty  from TSPL_BOOKING_DETAIL left outer join TSPL_ITEM_MASTER on TSPL_BOOKING_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code LEFT OUTER JOIN TSPL_CUSTOMER_MASTER  ON TSPL_BOOKING_DETAIL.Cust_Code=TSPL_CUSTOMER_MASTER.Cust_Code LEFT OUTER JOIN TSPL_LOCATION_MASTER  ON TSPL_BOOKING_DETAIL.Loc_Code =TSPL_LOCATION_MASTER.Location_Code LEFT OUTER JOIN TSPL_ROUTE_MASTER  ON TSPL_ROUTE_MASTER.Route_No   =TSPL_CUSTOMER_MASTER.Route_No LEFT OUTER JOIN TSPL_VEHICLE_MASTER  ON TSPL_VEHICLE_MASTER.Vehicle_Id  =TSPL_BOOKING_DETAIL.Vehicle_Code left outer join TSPL_STRUCTURE_MASTER on TSPL_ITEM_MASTER.Structure_Code=TSPL_STRUCTURE_MASTER.Structure_Code  where isnull(TSPL_BOOKING_DETAIL.Scheme_Item,'N')<>'Y' and isnull(TSPL_BOOKING_DETAIL.FOC_Item,'0')<>'1' and Document_No='" & txtDocNo.Value & "' union all " & _
                        "select  1 as Line_No ,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,TSPL_STRUCTURE_MASTER.Structure_Descq as CSA_TYPE,TSPL_ITEM_UOM_DETAIL.UOM_Code,0.00 as Total_Qty ,Sku_Seq,'' as Cust_Code, 0.00 as Qty from tspl_item_master left outer join TSPL_ITEM_UOM_DETAIL on tspl_item_master.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1 left outer join TSPL_Location_ItemMAPPING on TSPL_ITEM_MASTER.Item_Code=TSPL_Location_ItemMAPPING.Item_code left outer join TSPL_STRUCTURE_MASTER on TSPL_ITEM_MASTER.Structure_Code=TSPL_STRUCTURE_MASTER.Structure_Code   where " & strItem & " and TSPL_Location_ItemMAPPING.Location_Code='" & txtLocation.Value & "' and tspl_item_master.Item_Code not in (select TSPL_BOOKING_DETAIL.Item_Code from TSPL_BOOKING_DETAIL where Document_No='" & txtDocNo.Value & "' ) " & _
                        ") as t pivot(max(Qty) for Cust_Code     in (" & strtotalShort & ") ) as pivot1 order by Sku_Seq asc"
                Else
                    qry = "select * from (select  1 as Line_No ,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,TSPL_STRUCTURE_MASTER.Structure_Descq as CSA_TYPE ,TSPL_ITEM_UOM_DETAIL.UOM_Code,0.00 as Total_Qty,Sku_Seq,'' as Cust_Code, 0.00 as Qty  from tspl_item_master left outer join TSPL_ITEM_UOM_DETAIL on tspl_item_master.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1 left outer join TSPL_Location_ItemMAPPING on TSPL_ITEM_MASTER.Item_Code=TSPL_Location_ItemMAPPING.Item_code left outer join TSPL_STRUCTURE_MASTER on TSPL_ITEM_MASTER.Structure_Code=TSPL_STRUCTURE_MASTER.Structure_Code where " & strItem & " and TSPL_Location_ItemMAPPING.Location_Code='" & txtLocation.Value & "') as t pivot(max(Qty) for Cust_Code in (" & strtotalShort & ") ) as pivot1 order by Sku_Seq asc"
                End If
            End If
                ...
                If ii >= 7 Then
                    ...
                    If clsCommon.myLen(txtDocNo.Value) > 0 Then
                        qry = "select delivery_no,do_posted,Vehicle_Code,Number,Total_Qty,DocumentAmount,tspl_booking_detail.Posted,tspl_booking_detail.Booking_Status,tspl_booking_detail.Performance_Invoice_no from tspl_booking_detail left outer join TSPL_VEHICLE_MASTER ON " & _
                            " tspl_booking_detail.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id  where Document_No='" & txtDocNo.Value & "' and cust_code='" & strCustomer & "'"
        ...
        Try
            ...
            If (AllowToSave()) Then
                ...
                If (obj.SaveData(obj, isNewEntry, trans)) = True Then
                    ...
                    For ii As Integer = 7 To gv1.Columns.Count - 1
                        Dim objBookingCust As clsBookingTemp = TryCast(gv1.Columns(ii).Tag, clsBookingTemp)
                        If (clsCommon.myCdbl(gv1.Rows(0).Cells(ii).Tag)) > 0 Then
                            ...
                            qry = "Update TSPL_BOOKING_DETAIL set DocumentAmount=" & gv1.Rows(0).Cells(ii).Tag & ",Performance_Invoice_no='" & strPerformaInvoiceNo & "' where   Document_No='" & obj.Document_No & "' and Cust_Code='" & objBookingCust.CustCOde & "' and    Loc_Code='" & clsCommon.myCstr(txtLocation.Value) & "' and isnull(Scheme_Item,'N')='N' and isnull(FOC_Item,0)=0 "
                            clsDBFuncationality.ExecuteNonQuery(qry, trans)
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 Then
                    ...
                    For ii As Integer = 7 To gv1.Columns.Count - 1
                        ...
                        If dblTotalAmt > 0 AndAlso (strBookingStatus = 1 OrElse strBookingStatus = 3) Then
                            qry = "Update TSPL_BOOKING_DETAIL set Booking_Status=4 where Cust_Code='" & objBooking.CustCOde & "' and Document_No='" + txtDocNo.Value + "' "
                            clsDBFuncationality.ExecuteNonQuery(qry, trans)
                        End If
                    Next
    ...
    Public Sub funPrint(ByVal strDocNo As String)
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                frmCrystalReportViewer.funsubreportWithdt(MyBase.Form_ID, CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "rptDSBoking", "Performa Invoice", "rptCompanyAddress.rpt")
            End If
