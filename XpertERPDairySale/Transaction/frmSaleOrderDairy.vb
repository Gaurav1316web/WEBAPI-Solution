...
Public Class frmSaleOrderDairy
...
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        BOOK_QTY_UOM.HeaderImage = My.Resources.search4
        ...
        repoShipparty.HeaderImage = My.Resources.search4
        ...
        BOOK_RATE_UOM.HeaderImage = My.Resources.search4
                    ...
                    If e.Column Is gv1.Columns(colUnit) OrElse e.Column Is gv1.Columns(colUnitALter) OrElse e.Column Is gv1.Columns(colUnitRate) OrElse e.Column Is gv1.Columns(ColCommParty) OrElse e.Column Is gv1.Columns(colCommRate) OrElse e.Column Is gv1.Columns(colRTRate) OrElse e.Column Is gv1.Columns(colICodeGrp) OrElse e.Column Is gv1.Columns(colTAX_PAID) OrElse e.Column Is gv1.Columns(colShipParty) OrElse e.Column Is gv1.Columns(colLocationCode) OrElse e.Column Is gv1.Columns(colMRP) OrElse e.Column Is gv1.Columns(colSchemeApplicable) OrElse e.Column Is gv1.Columns(colHeaDDisPer) OrElse e.Column Is gv1.Columns(colTotTaxAmt) OrElse e.Column Is gv1.Columns(colICode) OrElse e.Column Is gv1.Columns(colQty) OrElse e.Column Is gv1.Columns(colManualRate) OrElse e.Column Is gv1.Columns(colSpecification) OrElse e.Column Is gv1.Columns(colRemarks) OrElse e.Column Is gv1.Columns(colDisPer) OrElse (e.Column Is gv1.Columns(colHeadDiscamt)) OrElse (e.Column Is gv1.Columns(colAmt)) AndAlso clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colRowType).Value), RowTypeMisc) = CompairStringResult.Equal Then
                        ...
                        ElseIf e.Column Is gv1.Columns(colUnitALter) Then
                            If clsCommon.myLen(gv1.CurrentRow.Cells(colUnit).Value) = 0 Then
                                common.clsCommon.MyMessageBoxShow(Me, "Please select Main Unit First", Me.Text)
                                isCellValueChangedOpen = False
                                Exit Sub
                            End If
    ...
    Sub OpenUOMAlterList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            ...
            Else
                'gv1.CurrentRow.Cells(colUnitRate).Value = Nothing
                If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), clsCommon.myCstr(gv1.CurrentRow.Cells(colUnitALter).Value)) = CompairStringResult.Equal Then
                    common.clsCommon.MyMessageBoxShow(Me, "Main Unit and Alternate unit should not be same.", Me.Text)
                    gv1.CurrentRow.Cells(colUnitALter).Value = Nothing
                    isCellValueChangedOpen = False
                End If
    ...
    Sub OpenItemGroupList(ByVal isButtonClick As Boolean)
        If clsCommon.myLen(txtReqNo.Value) <= 0 Then
            common.clsCommon.MyMessageBoxShow(Me, "Please select Booking First", Me.Text)
            isCellValueChangedOpen = False
            Exit Sub
    ...
    Sub OpenItemList(ByVal isButtonClick As Boolean)
        ...
        Dim whrCls As String
        If clsCommon.myLen(txtReqNo.Value) = 0 Then
            whrCls = "Is_FreshItem=0 and Product_Type not in ('MI') and Item_Type in ('F','T') and Is_Serial_Item=0"
    ...
    Private Sub CalDiffrate(ByVal XR As Integer, ByVal CellChanged As Boolean, ByVal strUnit As String)
        ...
        Try
            ...
            If CellChanged Then
                ...
                Else
                    clsCommon.MyMessageBoxShow(Me, "Please create Price chart or check location mapped with price chart.", Me.Text)
    ...
    Sub OpenICodeListCurrentCalaculation(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("SOD@3eIto", qry)
            If Not dr Is Nothing Then
                If clsCommon.myLen(clsCommon.myCstr(dr("Item"))) > 0 Then
                    SetitemWiseTaxSetting(True, True)
                    gv1.CurrentRow.Cells(colRowType).Value = RowTypeItem
    ...
    Private Sub UpdateCurrentRow(ByVal IntRowNo As Integer)
        ...
        "and Basic_Price='" & clsCommon.myCdbl(gv1.Rows(IntRowNo).Cells(colRate).Value) & "'  order by TSPL_SCHEME_MASTER_NEW.scheme_code desc "
            ...
            TotalItem_WeightMetric = TotalItem_Weight * clsWeightConversionInfo.GetWeightConverionFactor(strICode, wt_unit, gv1.Rows(IntRowNo).Cells(colBOOK_QTY_UOM).Value, Nothing)
        End If
        '''' end 
    ...
    Function AllowToSave() As Boolean
        Try
            ...
            If Not isNewEntry AndAlso clsCommon.myLen(txtDocNo.Value) <= 0 Then
                common.clsCommon.MyMessageBoxShow(Me, "Pucahse Order code Not found to save", Me.Text)
                txtDocNo.Focus()
                Return False
            End If
            ...
            If chkclose.Checked = True AndAlso IsRemarksMandatory = True AndAlso clsCommon.myLen(txtCloseRemarks.Value) = 0 Then
                common.clsCommon.MyMessageBoxShow(Me, "Please enter Remarks for Closing Order", Me.Text)
                txtCloseRemarks.Focus()
                Return False
            End If
            If chkCommApply.Checked = True AndAlso clsCommon.myCdbl(lblCommAmt.Text) = 0 Then
                common.clsCommon.MyMessageBoxShow(Me, "Please enter Commission amount.", Me.Text)
                Return False
            End If
            If txtCustPODate.Checked Then
                If clsCommon.GetDateWithStartTime(clsCommon.myCDate(txtCustPODate.Value)) > clsCommon.GetDateWithStartTime(clsCommon.myCDate(txtDeliveryDate.Value)) Then
                    common.clsCommon.MyMessageBoxShow(Me, "Please enter Delivery Date greater or equal  than Customer Po Date.", Me.Text)
                    Return False
                End If
                ...
                If clsCommon.GetDateWithStartTime(clsCommon.myCDate(txtCustPODate.Value)) > clsCommon.GetDateWithStartTime(clsCommon.myCDate(txtDate.Value)) Then
                    common.clsCommon.MyMessageBoxShow(Me, "Please enter Document Date greater or equal  than Customer Po Date.", Me.Text)
                    Return False
                End If
            Else
                If clsCommon.myLen(txtPONo.Text) > 0 Then
                    common.clsCommon.MyMessageBoxShow(Me, "Please enter Customer Po Date.", Me.Text)
                    Return False
                End If
                'If clsCommon.GetDateWithStartTime(clsCommon.myCDate(txtCustPODate.Value)) > clsCommon.GetDateWithStartTime(clsCommon.myCDate(txtDate.Value)) Then
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If clsCommon.myLen(strICode) > 0 Then
                    ...
                    If clsCommon.myLen(strAlterUOM) <= 0 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Please enter Alter UOM for " + strIName + ". At Line No" + clsCommon.myCstr(ii + 1))
                        Return False
                    End If
                    If clsCommon.myLen(strRateUOM) <= 0 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Please enter Rate UOM for " + strIName + ". At Line No" + clsCommon.myCstr(ii + 1))
                        Return False
                    End If
                    If chkCommApply.Checked = True AndAlso clsCommon.myLen(strCommParty) <= 0 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Please enter Commission party for " + strIName + ". At Line No" + clsCommon.myCstr(ii + 1))
                        Return False
                    End If
                End If
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        End If
        '-----------------------------------------------------
        LoadData(clsCommon.ShowSelectForm("DeliveryCodeFnd", qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked), NavigatorType.Current)
        btnCopy.Enabled = False
    End Sub
        ...
        Try
           ...
           "where  TSPL_RECEIPT_HEADER.Posted='Y'  and Receipt_Type='P'  and SecurityDeposit='N'  and Cust_Code='" & strCustomer & "' ) xxx "
            ...
            If dblAmt < lblTotRAmt1.Text And UsLock1.Status = ERPTransactionStatus.Pending Then
                Dim dblNewCredtitLimit = dblAmt - lblTotRAmt1.Text
                common.clsCommon.MyMessageBoxShow(Me, "Please increase credit limit " + clsCommon.myCstr(dblNewCredtitLimit) + " for customer " + txtVendorNo.Value)
                Return False
            End If
    ...
    '            QryShowStatus = " ,(case when TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.status =1 then 'AUTHORIZED' else 'NOT AUTHORIZED' end) as SOStatus "
    '        Else
    '            QryShowStatus = ""
    '        End If
    ...
    '        Qry = " select TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.line_no,'" + clsCommon.GetPrintDate(clsCommon.GETSERVERDATE, "dd/MM/yyyy") + "' as RunDate,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Cust_PO_No,'" + FooterText + "' as FooterText , TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Abandonment_No, TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Dept_Desc ,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Delivery_date ,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Remarks ,(case when TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .status =1 then TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Modify_By else '' end) as Modify_By ,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Landing_Cost,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Created_By , "
    '        Qry += " TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Terms_Code,TSPL_SHIP_TO_LOCATION.Telphone as ShipLoc_telphone,TSPL_LOCATION_MASTER.Service_Tax_Reg_No ,TSPL_SHIP_TO_LOCATION.Ship_To_Type_Desc as ShipLoc_Name,TSPL_SHIP_TO_LOCATION.email as shipLoc_Email,TSPL_LOCATION_MASTER.cst_no as CST_LST,TSPL_LOCATION_MASTER.Registration_Number,TSPL_LOCATION_MASTER.Add1 as loc_add1 ,TSPL_LOCATION_MASTER.add4 as loc_add4,TSPL_LOCATION_MASTER.Add2 as loc_add2,TSPL_LOCATION_MASTER.Add3 as loc_add3,TSPL_LOCATION_MASTER.Email as loc_email,TSPL_LOCATION_MASTER.TAN_No as loc_Fax , TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.TAX1_Rate,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.PrincipleDesc ,TSPL_COMPANY_MASTER.Add2 as comp_Add2 ,TSPL_COMPANY_MASTER.Add3 as comp_Add3 ,ISNULL(TSPL_COMPANY_MASTER.Phone1,'')+ Case When ISNULL(TSPL_COMPANY_MASTER.Phone2,'')<>'' Then ', '+ TSPL_COMPANY_MASTER.Phone2 Else'' End as Company_Phone,TSPL_COMPANY_MASTER.Email as Comp_Email,TSPL_COMPANY_MASTER.Fax as comp_Fax,ISNULL(TSPL_CUSTOMER_MASTER.Phone1,'')+ Case When ISNULL(TSPL_CUSTOMER_MASTER.Phone2,'')<>'' Then ', '+ TSPL_CUSTOMER_MASTER.Phone2 Else'' End as Cust_Phone ,TSPL_CUSTOMER_MASTER.Email as Cust_Email,TSPL_CUSTOMER_MASTER.Fax as cust_Fax,(TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Total_Tax_Amt /(case when TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Amt_Less_Discount<=0 then 1 else TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Amt_Less_Discount end)) *100 as Tax,((((TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Total_Tax_Amt /(case when TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Amt_Less_Discount<=0 then 1 else TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Amt_Less_Discount end)) *100)*TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Cost/100) +TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Cost) as landing_Rate,(TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Total_Tax_Amt /(case when TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Amt_Less_Discount<=0 then 1 else TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Amt_Less_Discount end) *100) as Tax_Lan,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Mode_Of_Transport as  ModeofTransport,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE .Specification as  specification,  "
    '        Qry += " TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Code as DocNo ,TSPL_SHIP_TO_LOCATION.add1 +case when len(TSPL_SHIP_TO_LOCATION.add2)>0 then ', '+TSPL_SHIP_TO_LOCATION.add2 else '' end +case when LEN(isnull(TSPL_SHIP_TO_LOCATION.Add3,''))>0 then ', '+isnull(TSPL_SHIP_TO_LOCATION.Add3,'') else ' ' end  + case when len(TSPL_SHIP_TO_LOCATION.State )>0 then TSPL_SHIP_TO_LOCATION.State else '' end  as Ship_address, TSPL_SHIP_TO_LOCATION.Tin_No as Ship_ToLocation_Tin_No, convert(varchar(10),TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Document_Date,103)  as po_date ,case TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .SalesOrder_Type when 'L'then 'Local' when 'I' then 'Import' when 'J' then 'Job Work' when 'O' then 'Open' when 'S' then 'Specific'else 'Null' end as po_type , "
    '        Qry += " TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Customer_Code, TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Net_Amt as Total_Item_Net_amt,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.MRP ,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Item_Cost, TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Terms_Code as termscode ,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Ref_No as ref_no ,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Comments as comments , "
    '        Qry += " TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Discount_Amt as dis_amt,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE .Disc_Amt  as dis_amt1,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Amount_Less_Discount  as aftrdiscount ,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Total_Amt as Total_amount,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Discount_Base as bfrdisc_amount, "
    ...
    '        Qry += " isnull(TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE .Total_Tax_Amt,0) as total_tax_amt, TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Total_Amt as DocAmt,  TSPL_COMPANY_MASTER.Comp_Name as compname,TSPL_COMPANY_MASTER.Logo_Img,TSPL_COMPANY_MASTER.Logo_Img2,ISNULL(tspl_company_Master.ADD1,'') as address1,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.item_code as item_code, TSPL_ITEM_MASTER.Item_Desc   as itemdesc, TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Row_Type,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Qty as qty,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.scheme_item as freeitem,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.unit_code as uom,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.item_cost as itemcost,TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.amount as amount ,"
    '        Qry += " TSPL_LOCATION_MASTER.TIN_No ,ISNULL(TSPL_LOCATION_MASTER.Phone1,'')+ Case When ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'' Then ', '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as Location_Phone ,(Select Add1+Case When ISNULL(Add2,'')='' Then '' else ', '+Add2+ Case When ISNULL(Add3,'')='' Then '' Else ', '+Add3+ Case When ISNULL(Pin_Code,'')='' Then '' else '-'+CONVERT(varchar, Pin_Code) End End End from TSPL_LOCATION_MASTER Where TSPL_LOCATION_MASTER.Location_Code = TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Bill_To_Location ) as [BillToAddress],TSPL_CUSTOMER_MASTER.add1 +case when len(TSPL_CUSTOMER_MASTER.add2)>0 then ', '+TSPL_CUSTOMER_MASTER.add2 else '' end +case when LEN(isnull(TSPL_CUSTOMER_MASTER.Add3,''))>0 then ', '+isnull(TSPL_CUSTOMER_MASTER.Add3,'') else ' ' end  + case when len(TSPL_CUSTOMER_MASTER.State )>0 then TSPL_CUSTOMER_MASTER.State else '' end  as Cust_address,TSPL_CUSTOMER_MASTER.Lst_No as cust_Lst_No,TSPL_CUSTOMER_MASTER.Tin_No as Cust_Tin_No  ,TSPL_CUSTOMER_MASTER.CST as Cust_Cst,"
    '        Qry += " (Select Add1+Case When ISNULL(Add2,'')='' Then '' else ', '+Add2+ Case When ISNULL(Add3,'')='' Then '' Else ', '+Add3+ Case When ISNULL(Pin_Code,'')='' Then '' else '-'+CONVERT(varchar, Pin_Code) End End End from TSPL_LOCATION_MASTER Where TSPL_LOCATION_MASTER.Location_Code = TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Ship_To_Location ) as [ShipToAddress], TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.TAX1,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.TAX2,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.TAX3,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.TAX4,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.TAX5,TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Total_Add_Charge "
    '        Qry += "" & QryShowStatus & ""
    '        Qry += " from TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE "
    '        Qry += " left outer join TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE  on TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Document_Code  =TSPL_DELIVERY_ORDER_DETAIL_PRODUCTSALE.Document_Code  "
    '        Qry += " left outer join TSPL_TAX_MASTER as tax1 on tax1.tax_code =TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.tax1 "
    '        Qry += " left outer join  TSPL_SHIP_TO_LOCATION on TSPL_DELIVERY_ORDER_HEAD_PRODUCTSALE.Ship_To_Location =TSPL_SHIP_TO_LOCATION.Ship_To_Code "
...
#Region "New Mail System"
    ...
    Private Sub btnsend_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsend.Click
        Try
            ...
            If Not (common.clsCommon.MyMessageBoxShow(Me, "Send E-Mail/SMS Of Respective Delivery Order No. " + txtDocNo.Value + "" + Environment.NewLine + "Are you sure?", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.Yes) Then
                Return
            End If
            LoadData(txtDocNo.Value, NavigatorType.Current)
...
#End Region
    ...
    Private Sub chkclose_ToggleStateChanged(ByVal sender As System.Object, ByVal args As Telerik.WinControls.UI.StateChangedEventArgs) Handles chkclose.ToggleStateChanged
        ...
        If chkclose.Checked Then
            If chkclose.Checked = True AndAlso IsRemarksMandatory = True AndAlso clsCommon.myLen(txtCloseRemarks.Value) = 0 Then
                common.clsCommon.MyMessageBoxShow(Me, "Please enter Remarks for Closing Order")
                chkclose.Checked = False
                txtCloseRemarks.Focus()
                Return
            End If
        ...
        If clsPSSalesOrder.ClosedData(obj, txtDocNo.Value, txtCloseRemarks.Value) Then
            If chkclose.Checked Then
                clsCommon.MyMessageBoxShow(Me, "Sale Order No. " + txtDocNo.Value + " Is Closed Successfully", Me.Text)
                vaddnew = "N"
                chkclose.Checked = True
                vaddnew = "Y"
                btnSave.Enabled = False
            ...
            End If
            If Not chkclose.Checked And btnSave.Enabled = False Then
                clsCommon.MyMessageBoxShow(Me, "Sale Order No. " + txtDocNo.Value + " Is Opened Successfully", Me.Text)
                vaddnew = "N"
                chkclose.Checked = False
                vaddnew = "Y"
                If UsLock1.Status = ERPTransactionStatus.Approved Then
