...
Public Class frmPOSBookingDairyMultipleDistributor
...
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        Try
            ...
            qry = "SELECT TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code as Customer_Code,'' as Vehicle_Id,'' as Route_No  " & _
                   " FROM TSPL_SECONDARY_CUSTOMER_MASTER " & _
                   " left outer join TSPL_POS_GROUP_MASTER ON TSPL_POS_GROUP_MASTER.GROUP_CODE= TSPL_SECONDARY_CUSTOMER_MASTER.POS_Type " & _
                   " where TSPL_SECONDARY_CUSTOMER_MASTER.Parent_Customer_No='" & txtCustomer.Value & "' and coalesce(TSPL_SECONDARY_CUSTOMER_MASTER.is_close,0)=0  " & _
                   " order by TSPL_POS_GROUP_MASTER.[level],TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code "
            ...
            If dt.Rows.Count > 0 Then
                ...
                If ShowItemLocationWiseonBooking = 0 Then
                    If clsCommon.myLen(txtDocNo.Value) > 0 Then
                        qry = "select max(line_no) as Line_No,Item_Code,max(Item_Desc) as Item_Desc,max(CSA_TYPE) as CSA_TYPE,max(UOM_Code) as UOM_Code,0 as Total_Qty,max(Sku_Seq) as Sku_Seq," & strCustCodeString & "  from (select  distinct Line_No,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,tspl_item_master.CSA_TYPE ,TSPL_POS_BOOKING_DETAIL.Unit_code as UOM_Code,0 as Total_Qty,Sku_Seq,isnull(TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code,'') as Distributor_Code, isnull(Booking_Qty,0) as Qty  from TSPL_POS_BOOKING_DETAIL left outer join TSPL_ITEM_MASTER on TSPL_POS_BOOKING_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code LEFT OUTER JOIN TSPL_SECONDARY_CUSTOMER_MASTER as TSPL_CUSTOMER_MASTER  ON TSPL_POS_BOOKING_DETAIL.Cust_Code=TSPL_CUSTOMER_MASTER.Cust_Code LEFT OUTER JOIN TSPL_LOCATION_MASTER  ON TSPL_POS_BOOKING_DETAIL.Loc_Code =TSPL_LOCATION_MASTER.Location_Code left Outer Join TSPL_SECONDARY_CUSTOMER_MASTER on TSPL_SECONDARY_CUSTOMER_MASTER.Parent_Customer_No=TSPL_CUSTOMER_MASTER.Cust_Code left Outer Join TSPL_POS_BOOKING_MATSER on   TSPL_POS_BOOKING_MATSER.Main_Booking_No=TSPL_POS_BOOKING_DETAIL.Main_Booking_No and TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code=TSPL_POS_BOOKING_MATSER.Distributor_Code and TSPL_POS_BOOKING_MATSER.Document_No=TSPL_POS_BOOKING_DETAIL.Document_No  where isnull(TSPL_POS_BOOKING_DETAIL.Scheme_Item,'N')<>'Y' and isnull(TSPL_POS_BOOKING_DETAIL.FOC_Item,'0')<>'1' and  TSPL_POS_BOOKING_MATSER.Main_Booking_No='" & txtDocNo.Value & "' union all " & _
                            "select  1 as Line_No ,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,tspl_item_master.CSA_TYPE,TSPL_ITEM_UOM_DETAIL.UOM_Code,0.00 as Total_Qty ,Sku_Seq,'' as Cust_Code, 0.00 as Qty from tspl_item_master left outer join TSPL_ITEM_UOM_DETAIL on tspl_item_master.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1  where " & strItem & "  and tspl_item_master.Item_Code not in (select TSPL_POS_BOOKING_DETAIL.Item_Code from TSPL_POS_BOOKING_MATSER left Outer join TSPL_POS_BOOKING_DETAIL on  TSPL_POS_BOOKING_MATSER.Main_Booking_No=TSPL_POS_BOOKING_DETAIL.Main_Booking_No and TSPL_POS_BOOKING_MATSER.Document_No=TSPL_POS_BOOKING_DETAIL.Document_No where TSPL_POS_BOOKING_MATSER.Main_Booking_No='" & txtDocNo.Value & "' ) " & _
                ...
                Else
                    If clsCommon.myLen(txtDocNo.Value) > 0 Then
                        qry = "select max(line_no) as Line_No,Item_Code,max(Item_Desc) as Item_Desc,max(CSA_TYPE) as CSA_TYPE,max(UOM_Code) as UOM_Code,0 as Total_Qty,max(Sku_Seq) as Sku_Seq," & strCustCodeString & "  from (select  distinct Line_No,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,tspl_item_master.CSA_TYPE ,TSPL_POS_BOOKING_DETAIL.Unit_code as UOM_Code,0 as Total_Qty,Sku_Seq,isnull(TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code,'') as Distributor_Code, isnull(Booking_Qty,0) as Qty  from TSPL_POS_BOOKING_DETAIL left outer join TSPL_ITEM_MASTER on TSPL_POS_BOOKING_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code LEFT OUTER JOIN TSPL_SECONDARY_CUSTOMER_MASTER as TSPL_CUSTOMER_MASTER  ON TSPL_POS_BOOKING_DETAIL.Cust_Code=TSPL_CUSTOMER_MASTER.Cust_Code LEFT OUTER JOIN TSPL_LOCATION_MASTER  ON TSPL_POS_BOOKING_DETAIL.Loc_Code =TSPL_LOCATION_MASTER.Location_Code left Outer Join TSPL_SECONDARY_CUSTOMER_MASTER on TSPL_SECONDARY_CUSTOMER_MASTER.Parent_Customer_No=TSPL_CUSTOMER_MASTER.Cust_Code left Outer Join TSPL_POS_BOOKING_MATSER on   TSPL_POS_BOOKING_MATSER.Main_Booking_No=TSPL_POS_BOOKING_DETAIL.Main_Booking_No and TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code=TSPL_POS_BOOKING_MATSER.Distributor_Code and TSPL_POS_BOOKING_MATSER.Document_No=TSPL_POS_BOOKING_DETAIL.Document_No  where isnull(TSPL_POS_BOOKING_DETAIL.Scheme_Item,'N')<>'Y' and isnull(TSPL_POS_BOOKING_DETAIL.FOC_Item,'0')<>'1' and  TSPL_POS_BOOKING_MATSER.Main_Booking_No='" & txtDocNo.Value & "' union all " & _
                            "select  1 as Line_No ,tspl_item_master.Item_Code,tspl_item_master.Item_Desc,tspl_item_master.CSA_TYPE,TSPL_ITEM_UOM_DETAIL.UOM_Code,0.00 as Total_Qty ,Sku_Seq,'' as Distributor_Code, 0.00 as Qty from tspl_item_master left outer join TSPL_ITEM_UOM_DETAIL on tspl_item_master.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1 left outer join TSPL_Location_ItemMAPPING on TSPL_ITEM_MASTER.Item_Code=TSPL_Location_ItemMAPPING.Item_code   where " & strItem & " and TSPL_Location_ItemMAPPING.Location_Code='" & txtLocation.Value & "' and tspl_item_master.Item_Code not in (select TSPL_POS_BOOKING_DETAIL.Item_Code from TSPL_POS_BOOKING_DETAIL where Main_Booking_No='" & txtDocNo.Value & "' ) " & _
                            ") as t pivot(sum(Qty) for Distributor_Code     in (" & strtotalShort & ") ) as pivot1 group by Item_Code  order by Sku_Seq asc"
                    Else
                ...
                For ii As Integer = 0 To gv1.Columns.Count - 1
                    ...
                    If ii >= 7 Then
                        ...
                        qry = "select TSPL_SECONDARY_CUSTOMER_MASTER.Customer_Name,TSPL_ROUTE_MASTER.vehicle_code,TSPL_VEHICLE_MASTER.Vehicle_No,Number,TSPL_CUSTOMER_MASTER." & _
                              " Zone_Code,TSPL_CUSTOMER_MASTER.Route_No from TSPL_SECONDARY_CUSTOMER_MASTER LEFT OUTER JOIN TSPL_CUSTOMER_MASTER ON TSPL_SECONDARY_CUSTOMER_MASTER.Parent_Customer_No=TSPL_CUSTOMER_MASTER.Cust_Code left outer join TSPL_ROUTE_MASTER on TSPL_CUSTOMER_MASTER.Route_No=TSPL_ROUTE_MASTER.Route_No  left outer join " & _
                              " TSPL_VEHICLE_MASTER on TSPL_ROUTE_MASTER.vehicle_code=TSPL_VEHICLE_MASTER.Vehicle_Id  where  TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code='" & clsCommon.myCstr(gv1.Columns(ii).Name) & "'"
                        ...
                        If clsCommon.myLen(txtDocNo.Value) > 0 Then
                            qry = "select TSPL_POS_BOOKING_DETAIL.Document_No,delivery_no,do_posted,Vehicle_Code,Number,Total_Qty,DocumentAmount,TSPL_POS_BOOKING_DETAIL.Posted,TSPL_POS_BOOKING_DETAIL.Booking_Status,TSPL_POS_BOOKING_DETAIL.Performance_Invoice_no from TSPL_POS_BOOKING_MATSER left Outer Join TSPL_POS_BOOKING_DETAIL on TSPL_POS_BOOKING_MATSER.Main_Booking_No=TSPL_POS_BOOKING_DETAIL.Main_Booking_No and TSPL_POS_BOOKING_MATSER.Document_No=TSPL_POS_BOOKING_DETAIL.Document_No left outer join TSPL_VEHICLE_MASTER ON " & _
                                " TSPL_POS_BOOKING_DETAIL.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id  where TSPL_POS_BOOKING_MATSER.Main_Booking_No='" & txtDocNo.Value & "' and TSPL_POS_BOOKING_MATSER.Distributor_Code='" & strCustomer & "'  and TSPL_POS_BOOKING_DETAIL.FOC_Item=0"
        ...
        Try
            If clsCommon.myLen(txtDocNo.Value) > 0 Then
                If e.Column.Index >= 6 Then
                    ...
                    'Dim strVehicle = clsDBFuncationality.getSingleValue("select Vehicle_Code from TSPL_POS_BOOKING_DETAIL where Document_No ='" & txtDocNo.Value & "' and cust_code='" & objBookingCust.CustCOde & "'")
        ...
        If blnSaveTotalQTy = False Then
            If gv1.CurrentColumn.Index > 6 AndAlso intChangeColumn > 0 Then
                ...
                If clsCommon.myLen(txtDocNo.Value) > 0 And objBookingCust.Booking_Status = 2 Then
                    objBookingCust.Booking_Status = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select  Booking_Status from TSPL_POS_BOOKING_DETAIL where Main_Booking_No='" & txtDocNo.Value & "'  and Cust_Code='" & strCustCode & "' and Document_No='" + strBookingNO + "'"))
                End If
    ...
    Sub CalculateItemRate(ByVal intRow As Integer, ByVal intColumn As Integer)
        ...
        For ii As Integer = 0 To gv1.Rows.Count - 1
            If clsCommon.myCdbl(gv1.Rows(ii).Cells(intColumn).Value) > 0 Then
                ...
                qry = "SELECT TSPL_SECONDARY_CUSTOMER_MASTER.Customer_Name,vehicle_code,TSPL_VEHICLE_MASTER.Vehicle_No,TSPL_SECONDARY_CUSTOMER_MASTER.Zone_Code,TSPL_CUSTOMER_MASTER.Route_No,Number FROM TSPL_SECONDARY_CUSTOMER_MASTER LEFT JOIN " & _
                        " TSPL_CUSTOMER_MASTER ON TSPL_CUSTOMER_MASTER.Cust_Code=TSPL_SECONDARY_CUSTOMER_MASTER.Parent_Customer_No " & _
                       ...
                       " WHERE TSPL_SECONDARY_CUSTOMER_MASTER.Cust_Code='" & txtCustomer.Value & "'"
                ...
                Dim objBookingItem As clsBookingTemp = New clsBookingTemp()
                Dim Price_code As String = clsDBFuncationality.getSingleValue("select price_CodeNon from TSPL_SECONDARY_CUSTOMER_MASTER where cust_code='" & txtCustomer.Value & "'")
                objBookingItem.PriceCode = Price_code
                ...
                qry = " SELECT TSPL_POS_COMMISSION_MAPPING.ITEM_PRICE_ID Item_Price_ID,TSPL_ITEM_PRICE_MASTER.Item_Code,UOM,Start_Date, " & _
                        " TSPL_POS_COMMISSION_MAPPING.BASIC_RATE Item_Basic_Price,Item_Basic_Net,Price_Code,Item_Selling_Price  " & _
                        " FROM TSPL_POS_COMMISSION_MAPPING JOIN TSPL_ITEM_PRICE_MASTER ON TSPL_ITEM_PRICE_MASTER.Item_Price_ID=TSPL_POS_COMMISSION_MAPPING.ITEM_PRICE_ID " & _
                        " left  outer join  TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_PRICE_MASTER.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and  " & _
                        " TSPL_ITEM_PRICE_MASTER.UOM=TSPL_ITEM_UOM_DETAIL.UOM_Code   WHERE GROUP_CODE='CNF' AND  Start_Date<='" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  " & _
                        " and (End_Date >= '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  or TSPL_ITEM_PRICE_MASTER.End_date is null) "
                dt = clsDBFuncationality.GetDataTable(qry)
                If dt.Rows.Count > 0 Then
    ...
    Sub SaveData()
        ...
        Dim qry As String = String.Empty
        Dim obj As New clsPOSBookingEntryDistributor()
        '' start here
                ...
                For Each grow As GridViewRowInfo In gv1.Rows
                    If clsCommon.myCdbl(grow.Cells(ii).Value) > 0 Then
                        ...
                        If clsCommon.myLen(objBookingCust.PerformaInvoiceBookingNo) = 0 Then
                            qry = "select Customer_Name,vehicle_code,TSPL_VEHICLE_MASTER.Vehicle_No,Zone_Code,TSPL_SECONDARY_CUSTOMER_MASTER.Route_No from TSPL_SECONDARY_CUSTOMER_MASTER left outer join TSPL_ROUTE_MASTER on TSPL_SECONDARY_CUSTOMER_MASTER.Route_No=TSPL_ROUTE_MASTER.Route_No left outer join TSPL_VEHICLE_MASTER on TSPL_ROUTE_MASTER.vehicle_code=TSPL_VEHICLE_MASTER.Vehicle_Id where Cust_Code='" & txtCustomer.Value & "'"
                            Dim dt1 As DataTable = clsDBFuncationality.GetDataTable(qry)
                            If (dt1 IsNot Nothing AndAlso dt1.Rows.Count > 0) Then
                                strVehicleCode = clsCommon.myCstr(dt1.Rows(0)("vehicle_code"))
                                strRoute = clsCommon.myCstr(dt1.Rows(0)("Route_No"))
                            ...
                            'End If
                        Else
                            strVehicleCode = clsDBFuncationality.getSingleValue("select Vehicle_Code from TSPL_POS_BOOKING_DETAIL where Main_Booking_No ='" & txtDocNo.Value & "' and cust_code='" & txtCustomer.Value & "'")
                        End If
                       ...
                       " FROM TSPL_POS_COMMISSION_MAPPING JOIN TSPL_ITEM_PRICE_MASTER ON TSPL_ITEM_PRICE_MASTER.Item_Price_ID=TSPL_POS_COMMISSION_MAPPING.ITEM_PRICE_ID " & _
                       " left  outer join  TSPL_ITEM_UOM_DETAIL on TSPL_ITEM_PRICE_MASTER.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and  " & _
                       " TSPL_ITEM_PRICE_MASTER.UOM=TSPL_ITEM_UOM_DETAIL.UOM_Code   WHERE TSPL_POS_COMMISSION_MAPPING.group_code='" + txtCustGrp.Value + "' AND  Start_Date<='" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  " & _
                       " and (End_Date >= '" & clsCommon.GetPrintDate(txtDate.Value, "dd/MMM/yyyy") & "'  or TSPL_ITEM_PRICE_MASTER.End_date is null) "

                        dt = clsDBFuncationality.GetDataTable(qry)
                        If dt.Rows.Count > 0 Then
                            dblSellingRate = clsCommon.myCdbl(dt.Rows(0).Item("Item_Basic_Price")) ''selling_price
        ...
        Try
            ...
            obj.Arr = New List(Of clsPOSBookingEntryDistributorDetail)
            Dim objTr As New clsPOSBookingEntryDistributorDetail()
            ...
            If (AllowToSave()) Then
                If btnSave.Text = "Save" Then
                    MainBookingNo = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select max(isnull(Main_Booking_No,'')) as 'Main_Booking_No' from TSPL_POS_BOOKING_MATSER", trans))
                ...
                For ii As Integer = 7 To gv1.Columns.Count - 1
                    ...
                    If dblBookAmount > 0 Then
                        For Each grow As GridViewRowInfo In gv1.Rows
                            objTr = New clsPOSBookingEntryDistributorDetail
                            ...
                            If objTr.Booking_Qty > 0 Then
                                ...
                                Dim strBookingStatus As Integer = 0
                                If clsCommon.myLen(txtDocNo.Value) > 0 Then
                                    strBookingStatus = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select  Booking_Status from TSPL_POS_BOOKING_DETAIL where Document_No='" & txtDocNo.Value & "'  and Cust_Code='" & objTr.Cust_Code & "'", trans))
                                End If
                        ...
                        If (obj.SaveData(obj, isNewEntry, trans)) = True Then
                            ...
                            Dim BookingNoQry As String = "select isnull(Document_No,'') as 'Booking_No' from TSPL_POS_BOOKING_MATSER where Main_Booking_No='" + clsCommon.myCstr(obj.Main_Booking_No) + "' and Distributor_Code='" + clsCommon.myCstr(objBookingCust.CustCOde) + "'"
    ...
    Private Function CustomerOutstandingAmount(ByVal strCustomer As String, ByVal dblTotal As Double, ByVal Trans As SqlTransaction, ByVal strDoc As String, ByVal blnBeforSave As Boolean, Optional ByVal intColumn As Integer = 0) As Boolean
        ...
        Try
          ...
          "select  sum(amount) as OutStandingAmt,1 as RI from TSPL_BANK_GUARANTEE_MASTER where Type='Customer' and vendor_code='" & strCustomer & "' and Bank_Guarantee_Type='RT' ) xxx "
            ...
            Else
                qry = "select sum(case when RI=1 then 1 else -1  end *  OutStandingAmt) from ( " & _
           "select SUM(isnull (isnull(TSPL_POS_BOOKING_DETAIL.Booking_Qty,0) * isnull(TSPL_POS_BOOKING_DETAIL.Item_Rate,0) ,0) ) as OutStandingAmt , 1 as RI from TSPL_POS_BOOKING_DETAIL " & _
           "where  TSPL_POS_BOOKING_DETAIL.Cust_Code='" & strCustomer & "'  and TSPL_POS_BOOKING_DETAIL.Document_No not in ('" & strDoc & "')  " & _
           " union all " & _
          "select isnull(SUM(isnull(TSPL_RECEIPT_DETAIL.Applied_Amount,0) ),0) as OutStandingAmt ,-1 as RI  from   " & _
          "TSPL_Customer_Invoice_Head left outer join  TSPL_RECEIPT_DETAIL on TSPL_Customer_Invoice_Head.Document_No=TSPL_RECEIPT_DETAIL.Document_No  left outer join TSPL_RECEIPT_HEADER on TSPL_RECEIPT_HEADER.Receipt_No=TSPL_RECEIPT_DETAIL.Receipt_No " & _
          "where  TSPL_RECEIPT_HEADER.Posted='Y'  and Against_Sale_No <> ''  and Customer_Code='" & strCustomer & "' " & _
                            ...
                            If clsCommon.myCdbl(dblCustTotalQty) > 0 Then
                                ...
                                If chkCreateDO.Checked = False OrElse (chkCreateDO.Checked = True AndAlso clsCommon.myLen(objBookingCust.DeliveryNo) = 0) Then
                                    ...
                                    For Each grow As GridViewRowInfo In gv1.Rows
                                        ...
                                        qry = "Update TSPL_POS_BOOKING_DETAIL set DO_Qty=" & objTr.Qty & ",Booking_Qty=" & objTr.Qty & ",Total_Qty=" & dblTotal_Qty & " where item_code='" & objTr.Item_Code & "' and Scheme_Item<>'Y' and vehicle_code='" & obj.Lorry_No & "' and  Document_No='" & txtDocNo.Value & "' and Cust_Code='" & obj.Customer_Code & "' and    Loc_Code='" & obj.Location_Code & "' and Sampling =" & obj.Sampling & ""
                                        clsDBFuncationality.ExecuteNonQuery(qry, trans)
                                    ...
                                    If (obj.Arr IsNot Nothing OrElse obj.Arr.Count > 0) Then
                                        If blnRatezero = False Then
                                            ...
                                            If (obj.SaveData(obj, isNewEntry, trans)) Then
                                                qry = "Update TSPL_POS_BOOKING_DETAIL set DO_Posted=" & obj.Status & ", Delivery_No='" & obj.Document_No & "',DocumentAmount=" & obj.Total_Amt & " where Document_No='" & obj.Booking_No & "' and    Loc_Code='" & obj.Location_Code & "' and vehicle_code='" & obj.Lorry_No & "' and Sampling=" & obj.Sampling & ""
                                                clsDBFuncationality.ExecuteNonQuery(qry, trans)
                                                qry = "Update TSPL_POS_BOOKING_MATSER set CreateDO_Automatic=1 where Document_No='" & obj.Booking_No & "' "
                                                clsDBFuncationality.ExecuteNonQuery(qry, trans)
                                                DOCreated = True
    ...
    Private Sub LoadDataForPivot()
        ...
        dtExtraColumn = clsDBFuncationality.GetDataTable("select distinct  tspl_item_master.short_description from TSPL_POS_BOOKING_DETAIL left join tspl_item_master on tspl_item_master.item_code = TSPL_POS_BOOKING_DETAIL.item_code where Document_No ='" & txtDocNo.Value & "'  ")


        strPivotForInternalquery = " select STUFF(a.strr,1,1,'') from (select(select distinct + ',['+ tspl_item_master.short_description+']' from TSPL_POS_BOOKING_DETAIL"
        strPivotForInternalquery += " left join tspl_item_master on tspl_item_master.item_code = TSPL_POS_BOOKING_DETAIL.item_code where Document_No ='" & txtDocNo.Value & "'  for xml path ('')) as strr)as a "
        strPivotForInternal = clsDBFuncationality.getSingleValue(strPivotForInternalquery, Nothing)

        strPivotForOuterquery = " (select(select distinct + ',sum(isnull(['+ tspl_item_master.short_description+'],0)) as '+'['+ tspl_item_master.short_description+']' from TSPL_POS_BOOKING_DETAIL"
        strPivotForOuterquery += " left join tspl_item_master on tspl_item_master.item_code = TSPL_POS_BOOKING_DETAIL.item_code where Document_No ='" & txtDocNo.Value & "'  for xml path ('')) as strr)"
        strPivotForOuter = clsDBFuncationality.getSingleValue(strPivotForOuterquery, Nothing)

        StrToatlQuery = "select +','+STUFF(a.strr,1,1,'') from (select(select distinct + '+sum(isnull(['+ tspl_item_master.short_description+'],0))' from TSPL_POS_BOOKING_DETAIL"
        StrToatlQuery += " left join tspl_item_master on tspl_item_master.item_code = TSPL_POS_BOOKING_DETAIL.item_code where Document_No ='" & txtDocNo.Value & "'  for xml path ('')) as strr)as a"
        StrToatl = clsDBFuncationality.getSingleValue(StrToatlQuery, Nothing)


        '================================================================end here=============================================================================
        Dim qry As String = " select tspl_vehicle_master.Number as Vechicle_Name,TSPL_ROUTE_MASTER.Route_Desc, tspl_item_master.short_description,TSPL_POS_BOOKING_MATSER.Document_No,TSPL_POS_BOOKING_MATSER.Document_Date ,TSPL_POS_BOOKING_MATSER.Location_code,TSPL_LOCATION_MASTER.Location_Desc ,TSPL_POS_BOOKING_DETAIL.Cust_Code ,TSPL_CUSTOMER_MASTER.Customer_Name,TSPL_CUSTOMER_MASTER.Route_No ,TSPL_POS_BOOKING_DETAIL.Vehicle_Code ,TSPL_POS_BOOKING_DETAIL.DocumentAmount ,TSPL_POS_BOOKING_DETAIL.Item_Code,TSPL_ITEM_MASTER.Item_Desc , TSPL_POS_BOOKING_DETAIL.Booking_Qty,isnull(TSPL_POS_BOOKING_DETAIL.Scheme_item,'N') as 'Scheme_item' from TSPL_POS_BOOKING_DETAIL "
        qry += " left join TSPL_POS_BOOKING_MATSER on TSPL_POS_BOOKING_MATSER.Document_No =TSPL_POS_BOOKING_DETAIL.Document_No"
        qry += " left join TSPL_LOCATION_MASTER on TSPL_LOCATION_MASTER.Location_Code =TSPL_POS_BOOKING_MATSER.Location_code"
        qry += " left join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER.Cust_Code =TSPL_POS_BOOKING_DETAIL.Cust_Code "
        qry += " left join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code =TSPL_POS_BOOKING_DETAIL.Item_Code "
        qry += " left join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No =TSPL_CUSTOMER_MASTER.Route_No"
        qry += " left join tspl_vehicle_master on tspl_vehicle_master.vehicle_id=TSPL_POS_BOOKING_DETAIL.vehicle_code"
        qry += " where TSPL_POS_BOOKING_MATSER.Document_No='" & txtDocNo.Value & "' and Booking_Status not in ('2','5')"
    ...
    Public Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        ...
        Try
            ...
            Dim obj As New clsPOSBookingEntryDistributor
            obj = clsPOSBookingEntryDistributor.GetData(strCode, NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Main_Booking_No) > 0) Then
                ...
                lblCustGrp.Text = clsDBFuncationality.getSingleValue("SELECT [Description] FROM [TSPL_POS_GROUP_MASTER] where Group_Code='" + txtCustGrp.Value + "'")
                lblLocation.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Location_Desc from TSPL_LOCATION_MASTER where Location_Code='" + txtLocation.Value + "'"))
                lblCustomerName.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue(" select Customer_Name as Name from TSPL_SECONDARY_CUSTOMER_MASTER where Cust_Code='" + txtCustomer.Value + "'"))
                ...
                If obj.Posted = 1 Then
                    ...
                    Dim DOStatus = clsDBFuncationality.getSingleValue("select top 1  Main_Booking_No from TSPL_POS_BOOKING_DETAIL where DO_Posted <> 4 and Main_Booking_No='" & txtDocNo.Value & "'")
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            Dim qry As String = "select count(*) from TSPL_POS_BOOKING_MATSER where Main_Booking_No='" + txtDocNo.Value + "' and comp_code='" + objCommonVar.CurrentCompanyCode + "'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtDocNo._MYValidating
        Try
            ' Dim qry As String = "select distinct TSPL_POS_BOOKING_MATSER.Document_No as DocumentNo,convert(varchar(12),TSPL_POS_BOOKING_MATSER.Document_date,103) as Document_date,case when Posted=1 then 'posted' else 'Unposted' end as Posted from TSPL_POS_BOOKING_MATSER"
            Dim qry As String = "select distinct TSPL_POS_BOOKING_MATSER.Main_Booking_No as DocumentNo,convert(varchar(12),TSPL_POS_BOOKING_MATSER.Document_date,103) as Document_date,(select isnull((Select distinct '['+TSPL_LOCATION_MASTER.Location_Desc +']  ' from TSPL_POS_BOOKING_DETAIL left outer join TSPL_LOCATION_MASTER on TSPL_POS_BOOKING_DETAIL.Loc_Code=TSPL_LOCATION_MASTER.Location_Code where TSPL_POS_BOOKING_DETAIL.Main_Booking_No=TSPL_POS_BOOKING_MATSER.Main_Booking_No   for xml path('')),'')  ) as Location  ,case when Posted=1 then 'posted' else 'Unposted' end as Posted from TSPL_POS_BOOKING_MATSER"
            Dim whrClas As String = " TSPL_POS_BOOKING_MATSER.comp_code='" + objCommonVar.CurrentCompanyCode + "'"
            Reset()
        ...
        Try
            ...
            If (myMessages.deleteConfirm()) Then
                If clsCancelLog.CheckForReasonOnDelete() Then
                    ...
                    End If
                End If
                If (clsPOSBookingEntryDistributor.DeleteData(txtDocNo.Value)) Then
    ...
    Private Sub ExportToExcel()
        Try
            Dim strCreatedBy As String = clsDBFuncationality.getSingleValue("Select Created_By from TSPL_POS_BOOKING_MATSER where Main_Booking_No='" + txtDocNo.Value + "'")
    ...
    Sub Post()
        Try
            If (myMessages.postConfirm()) Then
                If clsCommon.myLen(txtDocNo.Value) > 0 Then
                    Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
                    Dim qry = "Update TSPL_POS_BOOKING_MATSER set Posted=1, " & _
                    ...
                    For ii As Integer = 7 To gv1.Columns.Count - 1
                        ...
                        If dblTotalAmt > 0 Then
                            qry = "Update TSPL_POS_BOOKING_DETAIL set Booking_Status=4 where Cust_Code='" & objBooking.CustCOde & "' and Main_Booking_No='" + txtDocNo.Value + "' "
                            clsDBFuncationality.ExecuteNonQuery(qry, trans)
                        End If
                    Next
                        ...
                        For Each grow As GridViewRowInfo In gv.Rows
                            ...
                            If clsCommon.myLen(grow.Cells("Customer Code").Value) > 0 AndAlso clsCommon.myLen(grow.Cells("Location Code").Value) > 0 Then
                                ...
                                For ii As Integer = 3 To gv.Columns.Count - 1
                                    ...
                                    End If
                                Next
                                qry = "Update TSPL_POS_BOOKING_DETAIL set DocumentAmount=" & dblTotal & " where   Document_No='" & obj.Document_No & "' and Cust_Code='" & clsCommon.myCstr(grow.Cells("Customer Code").Value) & "' and    Loc_Code='" & clsCommon.myCstr(grow.Cells("Location Code").Value) & "' and Sampling=" & clsCommon.myCdbl(grow.Cells("Sampling").Value) & ""
                                clsDBFuncationality.ExecuteNonQuery(qry, trans)
                            End If
                        Next
                        trans.Commit()
    ...
    Private Sub txtCustGrp__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtCustGrp._MYValidating
        Dim qry As String = " SELECT GROUP_CODE AS Code,DESCRIPTION Description,LEVEL FROM TSPL_POS_GROUP_MASTER "
        Dim WhCls As String = " LEVEL>1 "
        txtCustGrp.Value = clsCommon.ShowSelectForm("CUSGRP_CODE", qry, "Code", WhCls, txtCustGrp.Value, " [Level]", isButtonClicked)
        lblCustGrp.Text = clsDBFuncationality.getSingleValue("  SELECT DESCRIPTION FROM TSPL_POS_GROUP_MASTER  where GROUP_CODE='" + txtCustGrp.Value + "'")
...
#End Region
    ...
    Public Sub funPrint(ByVal strDocNo As String)
        ...
        Try
            ...
            Qry += " where TSPL_POS_BOOKING_MATSER.Document_No ='" + strDocNo + "'"
    ...
    Public Function GetQuery() As String
        Dim Qry As String = " select TSPL_COMPANY_MASTER.CINNo as CompCinNo,TSPL_COMPANY_MASTER.logo_Img,TSPL_COMPANY_MASTER.add1 +case when len(TSPL_COMPANY_MASTER.add2)>0 then ' , '+TSPL_COMPANY_MASTER.add2 else '' end +case when LEN(isnull(TSPL_COMPANY_MASTER.Add3,''))>0 then ' , '+isnull(TSPL_COMPANY_MASTER.Add3,'') else ' ' end  + case when LEN(TSPL_COMPANY_MASTER.tin_no)>0 then ' , TIN No '+TSPL_COMPANY_MASTER.tin_no else ' ' end as Comp_Add_GP,TSPL_COMPANY_MASTER.CE_Division as GP_Division,TSPL_COMPANY_MASTER.ServiceTax_Reg_No +case when len(TSPL_COMPANY_MASTER.Ecc_No)>0 then ''+TSPL_COMPANY_MASTER.Ecc_No else '' end as GP_ECC_No,TSPL_COMPANY_MASTER.CE_Range as GP_CE_Range,TSPL_COMPANY_MASTER.Comp_Name AS CompName ," & _
                            ...
                            " TSPL_POS_BOOKING_DETAIL.item_code,tspl_item_master.item_desc,TSPL_POS_BOOKING_DETAIL.booking_Qty,TSPL_POS_BOOKING_DETAIL.unit_Code,TSPL_POS_BOOKING_DETAIL.item_Rate,TSPL_POS_BOOKING_DETAIL.documentAmount,TSPL_POS_BOOKING_DETAIL.vehicle_code,tspl_vehicle_master.Number as Vechicle_Name,TSPL_POS_BOOKING_MATSER.Document_No,TSPL_POS_BOOKING_MATSER.Document_Date,TSPL_POS_BOOKING_DETAIL.cust_code,TSPL_CUSTOMER_MASTER.customer_name " & _
                            " from TSPL_POS_BOOKING_DETAIL  left join TSPL_POS_BOOKING_MATSER on TSPL_POS_BOOKING_MATSER.Document_No =TSPL_POS_BOOKING_DETAIL.Document_No " & _
                            " left outer join TSPL_COMPANY_MASTER on  tspl_company_Master.Comp_Code = TSPL_POS_BOOKING_MATSER.comp_code " & _
                            ...
                            " left join tspl_location_master on TSPL_LOCATION_MASTER .Location_Code =TSPL_POS_BOOKING_MATSER.location_code " & _
                            " left join TSPL_STATE_MASTER on TSPL_STATE_MASTER .STATE_CODE =tspl_location_master.State " & _
                            " left join tspl_item_master on tspl_item_master.item_code=TSPL_POS_BOOKING_DETAIL.item_code " & _
                            " left join tspl_vehicle_master on tspl_vehicle_master.vehicle_id=TSPL_POS_BOOKING_DETAIL.vehicle_code" & _
                            " left join TSPL_CUSTOMER_MASTER on TSPL_CUSTOMER_MASTER .cust_code=TSPL_POS_BOOKING_DETAIL.cust_code "
        Return Qry
    End Function
    ' KUNAL > TICKET : BM00000009943 > DATE : 5-OCT-2016
    Private Sub btnCopyOrder_Click(sender As Object, e As EventArgs) Handles btnCopyOrder.Click
        Try
            ...
            Dim qry As String = "select distinct TSPL_POS_BOOKING_MATSER.Document_No as DocumentNo,convert(varchar(12),TSPL_POS_BOOKING_MATSER.Document_date,103) as Document_date,(select isnull((Select distinct '['+TSPL_LOCATION_MASTER.Location_Desc +']  ' from TSPL_POS_BOOKING_DETAIL left outer join TSPL_LOCATION_MASTER on TSPL_POS_BOOKING_DETAIL.Loc_Code=TSPL_LOCATION_MASTER.Location_Code where TSPL_POS_BOOKING_DETAIL.Document_No=TSPL_POS_BOOKING_MATSER.Document_No   for xml path('')),'')  ) as Location  ,case when Posted=1 then 'posted' else 'Unposted' end as Posted from TSPL_POS_BOOKING_MATSER"
            Dim whrClas As String = " TSPL_POS_BOOKING_MATSER.comp_code='" + objCommonVar.CurrentCompanyCode + "'"
            Reset()
    ...
    Private Sub txtCustomer__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtCustomer._MYValidating
        If clsCommon.myLen(txtCustGrp.Value) = 0 Then
            ...
            Exit Sub
        End If
        Dim qry As String = "SELECT Cust_Code AS Code,Customer_Name Name FROM TSPL_SECONDARY_CUSTOMER_MASTER JOIN TSPL_POS_GROUP_MASTER ON TSPL_POS_GROUP_MASTER.GROUP_CODE=TSPL_SECONDARY_CUSTOMER_MASTER.POS_Type  "
        Dim WhrCls As String = " GROUP_CODE='" + txtCustGrp.Value + "'"
        txtCustomer.Value = clsCommon.ShowSelectForm("Customer", qry, "Code", WhrCls, txtCustomer.Value, "Code", isButtonClicked)
        lblCustomerName.Text = clsDBFuncationality.getSingleValue(" select Customer_Name as Name from TSPL_SECONDARY_CUSTOMER_MASTER where Cust_Code='" + txtCustomer.Value + "'")
        If clsCommon.myLen(txtCustomer.Value) > 0 AndAlso clsCommon.myLen(txtLocation.Value) > 0 Then
            LoadBlankGrid()
        End If
    End Sub
End Class
