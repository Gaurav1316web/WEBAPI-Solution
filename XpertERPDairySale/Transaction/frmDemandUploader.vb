                ...
                Try
                    ...
                    If (Not String.IsNullOrEmpty(clsCommon.myCstr(gv1.Rows(dblrows).Cells(2).Value))) Then
                        If (ValidateBooth(gv1.Rows(dblrows).Cells(2).Value)) Then
                            ...
                            Dim LocationCode As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Location_Code from TSPL_Route_Master where Route_No='" + RouteNO + "' "))
                            If clsCommon.myLen(LocationCode) <= 0 Then
                                Throw New Exception("Location Code not mapped in Route Master for Route no [" + RouteNO + "]")
                            End If
                            Dim City_Code As String = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select City_Code from TSPL_ROUTE_MASTER where Route_No='" + RouteNO + "'"))
                            If clsCommon.myLen(City_Code) <= 0 Then
                                Throw New Exception("City Code not mapped in Route Master for Route no [" + RouteNO + "]")
                            End If
                                        ...
                                        If clsCommon.CompairString(clsCommon.myCstr(obj1.IsFreshAmbient), "Fresh") = CompairStringResult.Equal Then
                                            If clsCommon.CompairString(clsCommon.myCstr(obj1.Unit_code), "Crate") = CompairStringResult.Equal Then
                                                ...
                                                If AllowRouteWiseDemandEntryInDecimal Then
                                                    If clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select count(AllowEntryInDecimal) from TSPL_Route_Master where Route_No='" & RouteNO & "' and  AllowEntryInDecimal=1 ")) = 0 Then
