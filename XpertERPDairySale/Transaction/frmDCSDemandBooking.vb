    ...
    Public Sub LoadDCS(ByVal RouteNo As String)
        Try
            If clsCommon.myLen(RouteNo) > 0 Then
                ...
                Dim StrQry As String = "select VLC_Code_VLC_Uploader,VLC_Name,VSP_Code from TSPL_VLC_master_Head 
left outer join tspl_vendor_master on tspl_vendor_master.Vendor_Code=TSPL_VLC_master_Head.VSP_Code
where Route_Code= '" + RouteNo + "'"
                If clsCommon.myLen(objCommonVar.strCurrUserZones) > 0 Then
                    StrQry += " and TSPL_VENDOR_MASTER.zone_code in (" + objCommonVar.strCurrUserZones + ")"
    ...
    Private Sub btnHistory_Click(sender As Object, e As EventArgs) Handles btnHistory.Click
        Try
            If clsCommon.myLen(txtDocNo.Value) <= 0 Then
                ...
                Exit Sub
            End If
            clsERPFuncationalityOLD.ShowTransHistoryData(txtDocNo.Value, "Document_No", "TSPL_DCS_DEMAND_BOOKING_MASTER", "TSPL_DCS_DEMAND_BOOKING_DETAIL")
        Catch ex As Exception
            Throw New Exception(ex.Message)
        End Try
    End Sub
