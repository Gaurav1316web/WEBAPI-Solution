...
Imports common
Public Class frmPendingGatePassDairySale
#Region "Variables"
    ...
    Public ArrReturn As List(Of clsGatePassDairySaleDetail) = Nothing
...
#End Region
    ...
    Private Sub frmPendingDelivery_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'End If
        '-------------------------------------
        Dim qry As String = "select CAST(0 as bit) as Sel,code,ICode,max(IName) as IName,(Scheme_Code) as 'Scheme Code',Unit,max(Location) as Location,MAX(TSPL_LOCATION_MASTER.Location_Desc) as LocationName,max(TransDate) as TransDate , " & _
        ...
        qry += " select TSPL_GATEPASS_DETAIL_DAIRYSALE.Line_No,TSPL_GATEPASS_DETAIL_DAIRYSALE.Scheme_Code,TSPL_GATEPASS_DETAIL_DAIRYSALE.Document_No as Code,TSPL_GATEPASS_MASTER_DAIRYSALE.Customer_Code as Vendor, " & _
        "TSPL_GATEPASS_DETAIL_DAIRYSALE.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName, TSPL_GATEPASS_DETAIL_DAIRYSALE.Qty  as Qty, " & _
        "0 as Unapproved,TSPL_GATEPASS_DETAIL_DAIRYSALE.Unit_Code as Unit,TSPL_GATEPASS_MASTER_DAIRYSALE.Location_Code as Location, " & _
        "1 as RI,TSPL_GATEPASS_DETAIL_DAIRYSALE.Rate as Rate,1 as Chk,Document_Date as TransDate, " & _
        "TSPL_GATEPASS_DETAIL_DAIRYSALE.Price_code,TSPL_GATEPASS_DETAIL_DAIRYSALE.Price_Date,TSPL_GATEPASS_DETAIL_DAIRYSALE.Conv_Factor,TSPL_GATEPASS_DETAIL_DAIRYSALE.MRP from TSPL_GATEPASS_DETAIL_DAIRYSALE " & _
        ...
        "left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_GATEPASS_DETAIL_DAIRYSALE.Item_Code " & _
        "where  TSPL_GATEPASS_MASTER_DAIRYSALE.Short_Close='N' and isnull(TSPL_GATEPASS_DETAIL_DAIRYSALE.Scheme_Item,'N')<>'Y' and isnull(TSPL_GATEPASS_DETAIL_DAIRYSALE.FOC_Item,'0')<>'1'  and TSPL_GATEPASS_MASTER_DAIRYSALE.OnHold='N'   and TSPL_GATEPASS_MASTER_DAIRYSALE.Delivery_Code not in (select isnull(Against_Delivery_Code,'') from TSPL_SD_SHIPMENT_HEAD) " + Environment.NewLine 'TSPL_GATEPASS_MASTER_DAIRYSALE.Posted=1  and
        If clsCommon.myLen(VendorCode) > 0 Then
            qry += " and TSPL_GATEPASS_MASTER_DAIRYSALE.Customer_Code='" + VendorCode + "'"
        End If
        If clsCommon.myLen(LocationCode) > 0 Then
            qry += " and TSPL_GATEPASS_MASTER_DAIRYSALE.Location_Code='" + LocationCode + "'"
        End If
        ...
        qry += " select TSPL_SD_SHIPMENT_DETAIL.Line_No,TSPL_SD_SHIPMENT_DETAIL.Scheme_Code,TSPL_SD_SHIPMENT_DETAIL.GatePass_No as Code,TSPL_SD_SHIPMENT_Head.Customer_Code as Vendor,TSPL_SD_SHIPMENT_DETAIL.Item_Code as ICode,'' as IName,isnull(TSPL_SD_SHIPMENT_DETAIL.Qty,0) as Qty,0 as Unapproved,TSPL_SD_SHIPMENT_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,null as TransDate, " + Environment.NewLine
        qry += " TSPL_SD_SHIPMENT_DETAIL.Price_code,TSPL_SD_SHIPMENT_DETAIL.Price_Date,TSPL_SD_SHIPMENT_DETAIL.Conv_Factor,TSPL_SD_SHIPMENT_DETAIL.MRP "
        qry += " from TSPL_SD_SHIPMENT_DETAIL " + Environment.NewLine
        ...
        qry += " where TSPL_SD_SHIPMENT_Head.Status=1 and Scheme_Item='N' and TSPL_SD_SHIPMENT_Head.Trans_Type='PS' and len(isnull(TSPL_SD_SHIPMENT_DETAIL.GatePass_No,''))>0   " + Environment.NewLine
        qry += " union all   " + Environment.NewLine
        qry += " select TSPL_SD_SHIPMENT_DETAIL.Line_No,TSPL_SD_SHIPMENT_DETAIL.Scheme_Code,TSPL_SD_SHIPMENT_DETAIL.GatePass_No as Code,TSPL_SD_SHIPMENT_Head.Customer_Code as Vendor,TSPL_SD_SHIPMENT_DETAIL.Item_Code as ICode,'' as IName,0 as Qty,isnull(TSPL_SD_SHIPMENT_DETAIL.Qty,0) as Unapproved,TSPL_SD_SHIPMENT_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,null as TransDate, " + Environment.NewLine
        qry += " TSPL_SD_SHIPMENT_DETAIL.Price_code,TSPL_SD_SHIPMENT_DETAIL.Price_Date,TSPL_SD_SHIPMENT_DETAIL.Conv_Factor,TSPL_SD_SHIPMENT_DETAIL.MRP "
        qry += " from TSPL_SD_SHIPMENT_DETAIL " + Environment.NewLine
        qry += " left outer join TSPL_SD_SHIPMENT_Head on TSPL_SD_SHIPMENT_Head.Document_Code=TSPL_SD_SHIPMENT_DETAIL.Document_Code" + Environment.NewLine
        qry += " where TSPL_SD_SHIPMENT_Head.Status=0 and Scheme_Item='N' and TSPL_SD_SHIPMENT_Head.Trans_Type='PS'  and len(isnull(TSPL_SD_SHIPMENT_DETAIL.GatePass_No,''))>0 and TSPL_SD_SHIPMENT_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
        ...
        '    qry += "" + StrCondition1 + "  "
        'End If
        qry += " group by Scheme_Code,Code,ICode,Unit having SUM(Chk)>0 and SUM(Qty *RI) <>0  order by Code,max(Line_No)"
    ...
    Sub btnOKPressed()
        ...
        ArrReturn = New List(Of clsGatePassDairySaleDetail)
        Dim obj As clsGatePassDairySaleDetail = Nothing
        For ii As Integer = 0 To gv1.RowCount - 1
            If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                obj = New clsGatePassDairySaleDetail()
