...
'' work done agaist ticket no. BHA/31/10/18-000653

Public Class frmSaleInvoiceDairy
...
#End Region
    Public Sub SetUserMgmtNew()
        ...
        ''MyBase.SetUserMgmt(clsUserMgtCode.frmSaleInvoicedairy)
    ...
    Public Shared Function GetInvoiceType() As DataTable
        ...
        If Not objCommonVar.GSTApplicable Then
            dr = dt.NewRow()
            dr("Code") = "E"
            dr("Name") = "Excise"
            dt.Rows.Add(dr)
                ...
                If clsCommon.CompairString(clsCommon.myCstr(gv1.CurrentRow.Cells(colSchemeApplicable).Value), "Yes") = CompairStringResult.Equal Then
                    ...
                    If objD IsNot Nothing AndAlso objD.Arr.Count > 0 Then
                        For Each objtr As clsSchemeApplyOnDairy In objD.Arr
                            ...
                            Dim MainSaleOrderCode As Double = gv1.Rows(Index).Cells(colOrderNo).Value
                            '-------------------------------------------------------------
    ...
    Sub SaveData(ByVal ChekPostBtn As Boolean)
        Try
            If (AllowToSave()) Then
                ...
                obj.Is_Taxable = IIf(ddlInvoiceType.SelectedValue = "T", 1, 0)
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Try
            ...
            Dim obj As New clsPSInvoiceHead()
            obj = clsPSInvoiceHead.GetData(strCode, "'T','R','E'", NavTyep)
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.Document_Code) > 0) Then
                ...
                Else
                    ...
                    If dblPost = 0 Then
                        common.clsCommon.MyMessageBoxShow(Me, "Please Post this Shipment No " & txtReqNo.Value & " before posting invoice ")
                        btnPost.Enabled = False
                        btnDelete.Enabled = False
                    End If
                End If
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtDocNo._MYNavigator
        Try
            ...
            End If
            qst = "select count(*) from TSPL_SD_SALE_INVOICE_HEAD where Document_Code='" + txtDocNo.Value + "'  and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='PS'  and Invoice_Type in ('T','R','E') " + strcondition + " "
    ...
    Private Sub txtTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTaxGroup._MYValidating
        ...
        If clsERPFuncationality.GetGSTStatus(txtDate.Value) = True And objCommonVar.GSTActiveTaxGroup Then
            ...
            txtTaxGroup.Value = clsCommon.ShowSelectForm("Shipmentfndid", qry, "Code", "Tax_Group_Type='S' AND Active=1", txtTaxGroup.Value, "Code", isButtonClicked)
            SetTaxDetails()
        Else
            Dim qry As String = "select Tax_Group_Code as Code,Tax_Group_Desc as Description from TSPL_TAX_GROUP_MASTER "
    ...
    Public Function GetQuery() As String
        ...
        Qry += " as Comp_Address, TSPL_LOCATION_MASTER .Add1 as Loc_Add1,TSPL_LOCATION_MASTER.Add2 as Loc_ADd2,TSPL_LOCATION_MASTER.Add3  as Loc_Add3,TSPL_LOCATION_MASTER.Pin_Code as Loc_Pin_Code,TSPL_LOCATION_MASTER.TIN_No as Loc_TinNo,Case when ISNULL(TSPL_LOCATION_MASTER.Phone1,'')='(+__)__________' then '' else TSPL_LOCATION_MASTER.Phone1 end +  Case When   ISNULL(TSPL_LOCATION_MASTER.Phone2,'')<>'(+__)__________' Then ', '+ TSPL_LOCATION_MASTER.Phone2 Else'' End as  Loc_Phn,TSPL_LOCATION_MASTER.Email as Loc_Email,( case when TSPL_SD_SALE_INVOICE_head.Invoice_Type='R' then 'Retail Invoice' else 'Tax Invoice' end) as Invoice_Type,case when len(isnull(Alternate_UOM ,''))>0 then ((Qty *TSPL_ITEM_UOM_DETAIL.Conversion_Factor)/Uom_Detail.Conversion_Factor) else null end  as Alternet_Qty,"
        Qry += " TSPL_SD_SALE_INVOICE_DETAIL .Alternate_UOM,    TSPL_SD_SALE_INVOICE_DETAIL.Scheme_Qty ,TSPL_SD_SALE_INVOICE_DETAIL.Scheme_Item_UOM + case when Scheme_Item='Y' then ' (Free Scheme)' when Scheme_Item='N' then ''end as Scheme_Item_UOM  , TSPL_SD_SALE_INVOICE_HEAD.Discount_Base,case when len(isnull(TSPL_SD_SALE_INVOICE_HEAD.Road_Permit_No,''))>0 then TSPL_SD_SALE_INVOICE_HEAD.Road_Permit_No else TSPL_SD_SALE_INVOICE_HEAD.GRNo end AS Dis_Doc_No, TSPL_SD_SHIPMENT_HEAD.Description , TSPL_COMPANY_MASTER .State as Comp_State, TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_No as Buyer_order_no,case when TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_No <> '' then convert(varchar,TSPL_SD_SALE_INVOICE_HEAD.Cust_PO_Date,103) else '' end  as Buyer_order_date,case when (TSPL_SD_SHIPMENT_HEAD.Dispatch_Terms )='FE' then 'Freight Extra' else  TSPL_SD_SHIPMENT_HEAD.Dispatch_Terms  end  as Terms_of_delivery,TSPL_SD_SALE_INVOICE_HEAD.Document_Code as InvoiceNo,TSPL_SD_SALE_INVOICE_HEAD.Document_Date as Date_Time_Invoice,convert(varchar ,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103) as InvoiceDate "
        ...
        Qry += "   case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add1  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add1 end as P_Add1, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add2  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add2 end as P_Add2, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Add3  when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_add3 end as P_Add3, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .PIN_Code   when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Pin_No  end as P_PinNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .CST    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_CST_No   end as P_CstNo,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Tin_No     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .p_Tin_No   end as P_TinNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Email    when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Email  end as P_Email,case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Fax     when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_Fax   end as P_Fax, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER.Lst_No when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_LST_No    end as P_LstNo, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Cust_Code      when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_code   end as P_CustCode, case when coalesce(p_cust.P_cust_code,'')='' then TSPL_CUSTOMER_MASTER  .Customer_Name       when coalesce(p_cust.P_cust_code,'')<>'' then p_cust .P_cust_name    end as P_Cust_Name,"
        ...
        Qry += " ,TSPL_SD_SALE_INVOICE_DETAIL.mrp, TSPL_SD_SALE_INVOICE_DETAIL.amount as amount,TSPL_SD_SALE_INVOICE_DETAIL.Amt_Less_Discount,TSPL_SD_SALE_INVOICE_DETAIL.unit_code as uom,TSPL_SD_SALE_INVOICE_DETAIL.RATE_UOM as RATE_UOM"
        ...
        Qry += "left outer join TSPL_ITEM_UOM_DETAIL as Uom_Detail  on Uom_Detail.Item_Code =TSPL_ITEM_MASTER.Item_Code  and   Uom_Detail.UOM_Code =TSPL_SD_SALE_INVOICE_DETAIL.Alternate_UOM "
        Qry += "  left outer join TSPL_CITY_MASTER on TSPL_CITY_MASTER .City_Code =TSPL_CUSTOMER_MASTER.City_Code "
        ...
        Qry += "  left outer join TSPL_CITY_MASTER   as TSPL_CITY_MASTER_For_Location on  TSPL_CITY_MASTER_For_Location.City_Code =TSPL_LOCATION_MASTER.City_Code "
        Qry += " left outer join TSPL_STATE_MASTER  as TSPL_STATE_MASTER_For_State on TSPL_STATE_MASTER_For_State.STATE_CODE =TSPL_LOCATION_MASTER.state "
        Qry += " left outer join TSPL_CHAPTER_HEAD on TSPL_CHAPTER_HEAD.Chapter_Head_Code =TSPL_ITEM_MASTER.Cheapter_Heads"
        Qry += " left outer join tspl_state_master as Location_State on Location_State.state_code=tspl_location_master.state"
    ...
    Public Sub funPrint(ByVal strDocNo As String)
        ...
        Try
            ...
            Qry = "Select * from (" & Qry & ") XXX LEFT OUTER JOIN (Select '1' as COL1, 1 as COL2,  'ORIGINAL COPY' as CopyType1 UNION Select '1' as COL1, 2 as COL2,  'DUPLICATE COPY' as CopyType1 UNION Select '1' as COL1, 3 as COL2,  'TRIPLICATE COPY' as CopyType1"

            If Not AllowThreeFormatByDefault Then
                Qry += " UNION Select '1' as COL1, 4 as COL2,  'QUADRUPLICATE COPY' as CopyType1"
            End If
            Qry += " ) YYY ON YYY.COL1=XXX.CopyType ORDER BY YYY.COL2"
            ...
            Else
                ...
                Else
                    If dt.Rows.Count > 0 Then
                        frmCRV.funsubreportWithdt(MyBase.Form_ID, CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "rptProductSaleInvoice", "Retail Invoice", "rptCompanyAddress.rpt")
                    End If
                End If
                frmCRV = Nothing
            End If
    ...
    Public Sub funPrintChallan(ByVal strDocNo As String)
        ...
        Try
            ...
            Else
                If dt.Rows.Count > 0 Then
                    frmCRV.funsubreportWithdt(MyBase.Form_ID, CrystalReportFolder.KwalitySalesReport, dt, clsERPFuncationality.CompanyAddresShowinFooter(), "rptProductSaleInvoice_Challan", "Challan", "rptCompanyAddress.rpt")
                End If
            End If
            frmCRV = Nothing
...
#End Region
    ...
    Private Sub btnSendEmailSMS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSendEmailSMS.Click
        Try
            ...
            If Not (common.clsCommon.MyMessageBoxShow(Me, "Send E-Mail/SMS Of Respective Sale Invoice No. " + txtDocNo.Value + "" + Environment.NewLine + "Are you sure?", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.Yes) Then
                Return
            End If
            LoadData(txtDocNo.Value, NavigatorType.Current)
            Dim lstUsers As New List(Of String)
