Imports common
Public Class frmPendingSaleInvoiceDairy
#Region "Variables"
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        qry += " where  TSPL_SD_SALE_RETURN_HEAD.Status=1 and Scheme_Item='N' and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0   " + Environment.NewLine
        qry += " union all   " + Environment.NewLine
        qry += " select TSPL_SD_SALE_RETURN_DETAIL.Line_No,TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code as Code,TSPL_SD_SALE_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_RETURN_DETAIL.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,isnull(TSPL_SD_SALE_RETURN_DETAIL.Qty,0) + isnull(DamageQty,0) as Unapproved,TSPL_SD_SALE_RETURN_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date" + Environment.NewLine
        qry += " ,Scheme_Code from TSPL_SD_SALE_RETURN_DETAIL " + Environment.NewLine
        qry += " left outer join TSPL_SD_SALE_RETURN_HEAD on TSPL_SD_SALE_RETURN_HEAD.Document_Code=TSPL_SD_SALE_RETURN_DETAIL.Document_Code" + Environment.NewLine
        qry += " where  TSPL_SD_SALE_RETURN_HEAD.Status=0 and Scheme_Item='N' and TSPL_SD_SALE_RETURN_HEAD.Trans_Type='PS' and len(isnull(TSPL_SD_SALE_RETURN_DETAIL.Invoice_Code,''))>0 and TSPL_SD_SALE_RETURN_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
        ...
        If dtAllData Is Nothing OrElse dtAllData.Rows.Count <= 0 Then
            common.clsCommon.MyMessageBoxShow(Me, "No item found for vendor " + VendorName + "", Me.Text)
            Me.Close()
        End If
        LoadHeadData()
        LoadBlankGridDetail()
