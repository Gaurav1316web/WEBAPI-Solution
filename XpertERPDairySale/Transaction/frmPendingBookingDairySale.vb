...
Imports common
Public Class frmPendingBookingDairySale
#Region "Variables"
    ...
    Public FreshAndAmbiet As String = Nothing

    Public DairyProformaInvoice As Boolean = False
    Public ProformaInvoiceNo As String = Nothing

    Public ArrReturn As List(Of clsDeliveryNoteFreshSaleDetail) = Nothing
    Dim dtAllData As DataTable = Nothing

    Public ArrProformaReturn As List(Of clsBookingProformaInvoiceDetail) = Nothing
...
#End Region
    ...
    Private Sub frmPendingDelivery_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'End If
        '-------------------------------------
        If clsCommon.CompairString(DairyProformaInvoice, "True") = CompairStringResult.Equal Then
            qry = "select CAST(0 as bit) as Sel,code,ICode,max(IName) as IName,MAX(Unit) AS Unit,max(Location) as Location,MAX(TSPL_LOCATION_MASTER.Location_Desc) as LocationName,max(TransDate) as TransDate , " & _
             ...
             "MAX(MRP) as MRP,max(final.Price_Code) as Price_Code,max(price_date) as price_date,max(Conv_Factor) as Conv_Factor,max(ProformaInvoiceNo) as ProformaInvoiceNo  from ( " + Environment.NewLine
            ...
            "IPM.Start_Date as Price_Date,TSPL_ITEM_UOM_DETAIL.Conversion_Factor as Conv_Factor, isnull(IPM.Item_Basic_Net,0) as MRP,TSPL_BOOKING_DETAIL.Performance_Invoice_no  as ProformaInvoiceNo from " & _
            ...
            "where   Booking_Qty > 0 and TSPL_BOOKING_MATSER.posted=1 AND Loc_Code='" & LocCode & "' AND Cust_Code='" & VendorCode & "' and TSPL_BOOKING_MATSER.From_Screen_code='" & clsUserMgtCode.frmDairyBookingCustomer & "' and TSPL_ITEM_MASTER.IsTaxable='" & TaxCode & "' and TSPL_BOOKING_MATSER.TRANSACTION_TYPE='" & FreshAndAmbiet & "'"

            qry += " union all " + Environment.NewLine

            qry += " select TSPL_DAIRY_PROFORMA_INVOICE_detail.Line_No,TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Booking_No as Code, TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Customer_Code as Vendor,TSPL_DAIRY_PROFORMA_INVOICE_detail.Item_Code as ICode"
            qry += " ,'' as IName, isnull(TSPL_DAIRY_PROFORMA_INVOICE_detail.Qty,0) as Qty,0 as Unapproved, TSPL_ITEM_UOM_DETAIL.UOM_Code   as Unit, TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Bill_To_Location as Location,-1 as RI,0 as Rate,0 as Chk"
            qry += " ,null as TransDate, TSPL_DAIRY_PROFORMA_INVOICE_detail.Price_code,TSPL_DAIRY_PROFORMA_INVOICE_detail.Price_Date,0 AS Conv_Factor, TSPL_DAIRY_PROFORMA_INVOICE_detail.MRP,TSPL_DAIRY_PROFORMA_INVOICE_HEAD.BookingProformaNo  from TSPL_DAIRY_PROFORMA_INVOICE_detail "
            qry += " left outer join TSPL_DAIRY_PROFORMA_INVOICE_HEAD on TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Document_Code=TSPL_DAIRY_PROFORMA_INVOICE_detail.DOCUMENT_CODE left outer join TSPL_ITEM_UOM_DETAIL on TSPL_DAIRY_PROFORMA_INVOICE_detail.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1 "
            qry += "  where TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Status=1  and  TSPL_DAIRY_PROFORMA_INVOICE_detail.Location='" & LocCode & "' and TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Customer_Code='" & VendorCode & "'"
            qry += " union all"
            qry += " select TSPL_DAIRY_PROFORMA_INVOICE_detail.Line_No,TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Booking_No as Code, TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Customer_Code as Vendor,TSPL_DAIRY_PROFORMA_INVOICE_detail.Item_Code as ICode,'' as IName, isnull(TSPL_DAIRY_PROFORMA_INVOICE_detail.Qty,0) as Qty,0 as Unapproved"
            qry += " , TSPL_ITEM_UOM_DETAIL.UOM_Code   as Unit, TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Bill_To_Location as Location,-1 as RI,0 as Rate,0 as Chk,null as TransDate, TSPL_DAIRY_PROFORMA_INVOICE_detail.Price_code,TSPL_DAIRY_PROFORMA_INVOICE_detail.Price_Date,0 AS Conv_Factor, TSPL_DAIRY_PROFORMA_INVOICE_detail.MRP,TSPL_DAIRY_PROFORMA_INVOICE_HEAD.BookingProformaNo  from TSPL_DAIRY_PROFORMA_INVOICE_detail "
            qry += " left outer join TSPL_DAIRY_PROFORMA_INVOICE_HEAD on TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Document_Code=TSPL_DAIRY_PROFORMA_INVOICE_detail.DOCUMENT_CODE left outer join TSPL_ITEM_UOM_DETAIL on TSPL_DAIRY_PROFORMA_INVOICE_detail.Item_Code=TSPL_ITEM_UOM_DETAIL.Item_Code and TSPL_ITEM_UOM_DETAIL.Default_UOM=1  "
            qry += " where TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Status=0  and  TSPL_DAIRY_PROFORMA_INVOICE_detail.Location='" & LocCode & "' and TSPL_DAIRY_PROFORMA_INVOICE_HEAD.Customer_Code='" & VendorCode & "' "
    ...
    Sub btnOKPressed()
        ...
        If clsCommon.CompairString(DairyProformaInvoice, True) = CompairStringResult.Equal Then
            ArrProformaReturn = New List(Of clsBookingProformaInvoiceDetail)
            Dim obj As clsBookingProformaInvoiceDetail = Nothing
            For ii As Integer = 0 To gv1.RowCount - 1
                If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                    obj = New clsBookingProformaInvoiceDetail()
                    ...
                    obj.OrgRate = clsCommon.myCdbl(gv1.Rows(ii).Cells(colDRate).Value)
                    ...
                    obj.Amount = obj.Qty * obj.OrgRate
                    If (obj.Balance_Qty > 0) Then
                        ArrProformaReturn.Add(obj)
                    End If
                End If
            Next
        ...
        If NewVal Then
            For Each dr As DataRow In dtAllData.Rows
                If clsCommon.CompairString(strCode, clsCommon.myCstr(dr("Code"))) = CompairStringResult.Equal Then
                    ...
                    End If
                    ProformaInvoiceNo = clsCommon.myCstr(dr("ProformaInvoiceNo"))
