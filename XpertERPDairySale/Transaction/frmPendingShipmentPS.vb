Imports common
Public Class frmPendingShipmentPS
#Region "Variables"
    ...
    Public ArrReturn As List(Of clsPSShipmentHeadDetail) = Nothing
...
#End Region
    ...
    Private Sub FrmPendingRequistion_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        qry += " select TSPL_SD_SHIPMENT_DETAIL.Line_No,TSPL_SD_SHIPMENT_DETAIL.Document_Code as Code,TSPL_SD_SHIPMENT_HEAD.Customer_Code as Vendor,TSPL_SD_SHIPMENT_DETAIL.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName,TSPL_SD_SHIPMENT_DETAIL.Row_Type as IType, TSPL_SD_SHIPMENT_DETAIL.Qty  as Qty,0 as Unapproved,TSPL_SD_SHIPMENT_DETAIL.Unit_Code as Unit,TSPL_SD_SHIPMENT_DETAIL.Location as Location,1 as RI,TSPL_SD_SHIPMENT_DETAIL.Item_Cost as Rate,1 as Chk,TSPL_SD_SHIPMENT_HEAD.Tax_Group,TSPL_SD_SHIPMENT_DETAIL.TAX1_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX2_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX3_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX4_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX5_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX6_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX7_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX8_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX9_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX10_Rate,TSPL_SD_SHIPMENT_DETAIL.MRP ,TSPL_SD_SHIPMENT_DETAIL.Disc_Per,Document_Date as TransDate,TSPL_SD_SHIPMENT_DETAIL.Assessable  ,TSPL_SD_SHIPMENT_DETAIL.Batch_No ,TSPL_SD_SHIPMENT_DETAIL.Expiry_Date ,TSPL_SD_SHIPMENT_DETAIL.MFG_Date " + Environment.NewLine
        ...
        qry += " where TSPL_SD_SHIPMENT_DETAIL.Status=0 and Scheme_Item='N' and TSPL_SD_SHIPMENT_HEAD.Trans_Type='PS'  and TSPL_SD_SHIPMENT_HEAD.Status=1 " + Environment.NewLine
        If clsCommon.myLen(VendorCode) > 0 Then
            qry += " and TSPL_SD_SHIPMENT_HEAD.Customer_Code='" + VendorCode + "'"
        End If
        qry += " union all " + Environment.NewLine
        qry += " select  TSPL_SD_SALE_INVOICE_DETAIL.Line_No,TSPL_SD_SALE_INVOICE_DETAIL.Shipment_Code as Code,TSPL_SD_SALE_INVOICE_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_INVOICE_DETAIL.Item_Code as ICode,'' as IName,'' as IType,isnull(TSPL_SD_SALE_INVOICE_DETAIL.Qty,0) as Qty,0 as Unapproved,TSPL_SD_SALE_INVOICE_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_INVOICE_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date" + Environment.NewLine
        ...
        qry += " where TSPL_SD_SALE_INVOICE_HEAD.Status=1 and Scheme_Item='N' and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='PS' and len(isnull(TSPL_SD_SALE_INVOICE_DETAIL.Shipment_Code,''))>0   " + Environment.NewLine
        qry += " union all   " + Environment.NewLine
        qry += " select  TSPL_SD_SALE_INVOICE_DETAIL.Line_No,TSPL_SD_SALE_INVOICE_DETAIL.Shipment_Code as Code,TSPL_SD_SALE_INVOICE_HEAD.Customer_Code as Vendor,TSPL_SD_SALE_INVOICE_DETAIL.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,isnull(TSPL_SD_SALE_INVOICE_DETAIL.Qty,0) as Unapproved,TSPL_SD_SALE_INVOICE_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,MRP as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SALE_INVOICE_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date" + Environment.NewLine
        qry += "  ,Scheme_Code from TSPL_SD_SALE_INVOICE_DETAIL " + Environment.NewLine
        qry += " left outer join TSPL_SD_SALE_INVOICE_HEAD on TSPL_SD_SALE_INVOICE_HEAD.Document_Code=TSPL_SD_SALE_INVOICE_DETAIL.Document_Code" + Environment.NewLine
        qry += " where TSPL_SD_SALE_INVOICE_HEAD.Status=0 and Scheme_Item='N' and TSPL_SD_SALE_INVOICE_HEAD.Trans_Type='PS' and len(isnull(TSPL_SD_SALE_INVOICE_DETAIL.Shipment_Code,''))>0 and TSPL_SD_SALE_INVOICE_DETAIL.DOCUMENT_CODE not in ('" + strCurrCode + "')  " + Environment.NewLine
    ...
    Sub btnOKPressed()
        ArrReturn = New List(Of clsPSShipmentHeadDetail)
        Dim obj As clsPSShipmentHeadDetail = Nothing
        For ii As Integer = 0 To gv1.RowCount - 1
            If (clsCommon.myCBool(gv1.Rows(ii).Cells(colDSelect).Value)) Then
                obj = New clsPSShipmentHeadDetail()
        ...
        If Not IsInsideLoadData Then
            If gvHead.CurrentColumn Is gvHead.Columns(colHSelect) Then
                ...
                If clsCommon.CompairString(strVendorCode, VendorCode) = CompairStringResult.Equal Then
                    ...
                    End If
                Else
                    common.clsCommon.MyMessageBoxShow("Shipment's Customer should be `" + VendorName)
                    e.Cancel = True
                End If
            End If
        End If
