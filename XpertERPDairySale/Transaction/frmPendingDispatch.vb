...
#End Region
    ...
    Private Sub frmPendingDispatch_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub frmPendingDispatch_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
            ...
            If gvHead.CurrentColumn Is gvHead.Columns(colHSelect) Then
                For Each grow As GridViewRowInfo In gvHead.Rows
                    If grow.Cells(colHSelect).Value = True Then
                        common.clsCommon.MyMessageBoxShow(Me, "Only one Invoice can select at a time.", Me.Text)
                        e.Cancel = True
                        Exit Sub
                    End If
                Next
    ...
    Sub LoadData()
        Try
            ...
            qry += " select TSPL_SD_SHIPMENT_DETAIL.Line_No,TSPL_SD_SHIPMENT_DETAIL.Document_Code as Code,TSPL_SD_SHIPMENT_HEAD.Customer_Code as Vendor,TSPL_SD_SHIPMENT_DETAIL.Item_Code as ICode,TSPL_ITEM_MASTER.Item_Desc as IName,TSPL_SD_SHIPMENT_DETAIL.Row_Type as IType, TSPL_SD_SHIPMENT_DETAIL.Qty  as Qty,0 as Unapproved,TSPL_SD_SHIPMENT_DETAIL.Unit_Code as Unit,TSPL_SD_SHIPMENT_DETAIL.Location as Location,1 as RI,TSPL_SD_SHIPMENT_DETAIL.Item_Cost as Rate,1 as Chk,TSPL_SD_SHIPMENT_HEAD.Tax_Group,TSPL_SD_SHIPMENT_DETAIL.TAX1_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX2_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX3_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX4_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX5_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX6_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX7_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX8_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX9_Rate,TSPL_SD_SHIPMENT_DETAIL.TAX10_Rate,TSPL_SD_SHIPMENT_DETAIL.MRP ,TSPL_SD_SHIPMENT_DETAIL.Disc_Per,TSPL_SD_SHIPMENT_HEAD.Document_Date as TransDate,TSPL_SD_SHIPMENT_DETAIL.Assessable ,TSPL_SD_SHIPMENT_DETAIL.Batch_No ,TSPL_SD_SHIPMENT_DETAIL.Expiry_Date ,TSPL_SD_SHIPMENT_DETAIL.MFG_Date 
 ,Scheme_Code,TSPL_SD_SHIPMENT_HEAD.Route_No ,CASE WHEN Shift_type = 'AM' THEN 'Morning' ELSE 'Evening' END AS Shift_type from TSPL_SD_SHIPMENT_DETAIL 
 ...
 left outer join TSPL_ITEM_MASTER on TSPL_ITEM_MASTER.Item_Code=TSPL_SD_SHIPMENT_DETAIL.Item_Code "
            qry += " where TSPL_SD_SHIPMENT_DETAIL.Status=0 and Scheme_Item='N'  and TSPL_SD_SHIPMENT_HEAD.Screen_Type='DS' and TSPL_SD_SHIPMENT_HEAD.Status=1 
 and convert(date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) AND TSPL_SD_SHIPMENT_HEAD.Is_Taxable='" & Is_Taxable & "' and  TSPL_SD_SHIPMENT_HEAD.Sale_Invoice_No='' or TSPL_SD_SHIPMENT_HEAD.Sale_Invoice_No is null
 "
            If clsCommon.myLen(VendorCode) > 0 Then
                qry += " and TSPL_SD_SHIPMENT_HEAD.Customer_Code='" + VendorCode + "'"
            End If
            qry += " union all " + Environment.NewLine
            qry += " select TSPL_SD_SHIPMENT_RETURN_DETAIL.Line_No,TSPL_SD_SHIPMENT_RETURN_DETAIL.Shipment_Code as Code,TSPL_SD_SHIPMENT_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SHIPMENT_RETURN_DETAIL.Item_Code as ICode,'' as IName,'' as IType,isnull(TSPL_SD_SHIPMENT_RETURN_DETAIL.Qty,0) + isnull(TSPL_SD_SHIPMENT_RETURN_DETAIL.DamageQty,0) as Qty,0 as Unapproved,TSPL_SD_SHIPMENT_RETURN_DETAIL.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SHIPMENT_RETURN_DETAIL.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date
 ,Scheme_Code, TSPL_SD_SHIPMENT_RETURN_HEAD.Route_No,'' as Shift_type from TSPL_SD_SHIPMENT_RETURN_DETAIL 
 left outer join TSPL_SD_SHIPMENT_RETURN_HEAD on TSPL_SD_SHIPMENT_RETURN_HEAD.Document_Code=TSPL_SD_SHIPMENT_RETURN_DETAIL.Document_Code 
 left outer join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_RETURN_DETAIL.Shipment_Code=TSPL_SD_SHIPMENT_HEAD.Document_Code 
 where  TSPL_SD_SHIPMENT_RETURN_HEAD.Status=1 and Scheme_Item='N'    and TSPL_SD_SHIPMENT_RETURN_HEAD.Screen_Type='DS' and len(isnull(TSPL_SD_SHIPMENT_RETURN_DETAIL.Shipment_Code,''))>0   
 and convert(date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103) AND TSPL_SD_SHIPMENT_RETURN_HEAD.Is_Taxable=" & Is_Taxable & " " + Environment.NewLine

            qry += " union all   " + Environment.NewLine
            qry += " select TSPL_SD_SHIPMENT_RETURN_Detail.Line_No,TSPL_SD_SHIPMENT_RETURN_Detail.Shipment_Code as Code,TSPL_SD_SHIPMENT_RETURN_HEAD.Customer_Code as Vendor,TSPL_SD_SHIPMENT_RETURN_Detail.Item_Code as ICode,'' as IName,'' as IType,0 as Qty,isnull(TSPL_SD_SHIPMENT_RETURN_Detail.Qty,0)  + isnull(TSPL_SD_SHIPMENT_RETURN_Detail.DamageQty,0)  as Unapproved,TSPL_SD_SHIPMENT_RETURN_Detail.Unit_code as Unit,'' as Location,-1 as RI,0 as Rate,0 as Chk,'' as Tax_Group,0 as TAX1_Rate,0 as TAX2_Rate,0 as TAX3_Rate,0 as TAX4_Rate,0 as TAX5_Rate,0 as TAX6_Rate,0 as TAX7_Rate,0 as TAX8_Rate,0 as TAX9_Rate,0 as TAX10_Rate,mrp as MRP,0 as Disc_Per,null as TransDate,isnull(TSPL_SD_SHIPMENT_RETURN_Detail.Assessable,0)as Assessable, '' as  Batch_No ,null as Expiry_Date ,null as MFG_Date
 ,Scheme_Code,TSPL_SD_SHIPMENT_RETURN_HEAD.Route_No,'' as Shift_type from TSPL_SD_SHIPMENT_RETURN_Detail 
 left outer join TSPL_SD_SHIPMENT_RETURN_HEAD on TSPL_SD_SHIPMENT_RETURN_HEAD.Document_Code=TSPL_SD_SHIPMENT_RETURN_Detail.Document_Code
 left outer join TSPL_SD_SHIPMENT_HEAD on TSPL_SD_SHIPMENT_RETURN_Detail.Shipment_Code=TSPL_SD_SHIPMENT_HEAD.Document_Code 
 where  TSPL_SD_SHIPMENT_RETURN_HEAD.Status=0 and Scheme_Item='N'    and TSPL_SD_SHIPMENT_RETURN_HEAD.Screen_Type='DS' and len(isnull(TSPL_SD_SHIPMENT_RETURN_Detail.Shipment_Code,''))>0 and TSPL_SD_SHIPMENT_RETURN_Detail.DOCUMENT_CODE not in ('')  
 and convert(date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) > =convert(date,'" + tp_FromDate.Value + "',103) and convert(date,TSPL_SD_SHIPMENT_HEAD.Document_Date,103) <= convert(date,'" + tp_ToDate.Value + "',103)  AND TSPL_SD_SHIPMENT_RETURN_HEAD.Is_Taxable=" & Is_Taxable & " " + Environment.NewLine
            ...
            qry += " group by Code,ICode,Unit,MRP " & IIf(ShowMulMRPOfSameItemOnDairyBookingCustomer = True, " ,Rate", "") & " having SUM(Chk)>0 and SUM(Qty *RI) <>0 and SUM((Qty *RI)- Unapproved)>0 order by Code,max(Line_No)"
