...
#End Region
    ...
    Sub OpenItemGroupList(ByVal isButtonClick As Boolean)
        ...
        gv1.CurrentRow.Cells(colICodeGrp).Value = clsCommon.ShowSelectForm("DS-BOItemGrpFndr", Qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colICodeGrp).Value), "Code", isButtonClick)
        SetBlankOfItemGrpColumns()
    End Sub
    Sub OpenItemList(ByVal isButtonClick As Boolean)
        Dim whrCls As String = "Product_Type not in ('MI')  and Item_Type in ('F','T','S') and Is_Serial_Item=0"
        ...
        If CheckItemtaxType() = False Then
            Exit Sub
        End If
        'Dim intCurrentTaxType = clsCommon.myCdbl(clsDBFuncationality.getSingleValue("select Is_Tax_Exempted from TSPL_ITEM_MASTER where Item_Code='" & gv1.CurrentRow.Cells(colICode).Value & "' "))
        'Dim intFirstTaxType As Integer = CheckItemtaxType()
        'Dim strCurrentTaxtype As String = ""
        'Dim strFirstTaxType As String = ""
        'If intCurrentTaxType <> intFirstTaxType Then
        '    If intCurrentTaxType = 0 Then
        '        strCurrentTaxtype = "None"
        '    ElseIf intCurrentTaxType = 1 Then
        '        strCurrentTaxtype = "Tax Exempted"
        '    ElseIf intCurrentTaxType = 2 Then
        '        strCurrentTaxtype = "Excisable"
        '    End If
        '    If intFirstTaxType = 0 Then
        '        strFirstTaxType = "None"
        '    ElseIf intFirstTaxType = 1 Then
        '        strFirstTaxType = "Tax Exempted"
        '    ElseIf intFirstTaxType = 2 Then
        '        strFirstTaxType = "Excisable"
        '    End If
        '    common.clsCommon.MyMessageBoxShow("First Item tax type is " & strFirstTaxType & "  and Current Item Type is " & strCurrentTaxtype & "  ")
        ...
        Dim dblRate As Double = ItemPrice(gv1.CurrentRow.Cells(colICode).Value, gv1.CurrentRow.Cells(colUnit).Value)
        gv1.CurrentRow.Cells(colRate).Value = dblRate
        SetBlankOfItemGrpColumns()
    End Sub
    Function ItemPrice(ByVal strItem As String, ByVal strUnit As String) As Double
        Dim qry = " Select RowNo, Item_Price_ID, XXXE.Item_Code, UOM, Start_Date, Item_Basic_Price,Item_Basic_Net,Price_Code from ( " & _
       ...
       "TSPL_ITEM_PRICE_MASTER.Price_Code='" & txtPriceCode.Text & "' and UOM='" & strUnit & "' and TSPL_ITEM_PRICE_MASTER.item_code='" & strItem & "' AND Location_Code='" & txtBillToLocation.Value & "'  " & _
       ") XXXE WHERE RowNo=1  "
    ...
    Sub OpenItemUOMList(ByVal isButtonClick As Boolean)
        ...
        gv1.CurrentRow.Cells(colUnit).Value = clsCommon.ShowSelectForm("DS-BOItemUOMFndr", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), "Code", isButtonClick)
    End Sub
    ...
    Sub OpenUOMList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.myLen(strICode) > 0 Then
            Dim qry As String = "select UOM_Code as Code,UOM_Description as [Description] from TSPL_ITEM_UOM_DETAIL"
            Dim whrCls As String = "Item_Code='" + strICode + "'"
            gv1.CurrentRow.Cells(colUnit).Value = clsCommon.ShowSelectForm("DS-BOUOMFndr", qry, "Code", whrCls, clsCommon.myCstr(gv1.CurrentRow.Cells(colUnit).Value), "Code", isButtonClick)
    ...
    Sub OpenICodeList(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            If clsCommon.myLen(pivotheader) <= 0 Then
             ...
             "where TSPL_ITEM_MASTER.Active=1 and Is_FreshItem=0 and Product_Type not in ('MI') )xxx Where  Tax_group='" & txtTaxGroup.Value & "'   " & strPriceCondition & "  " & strTaxRate & " " & _
    ...
    Sub OpenICodeListCurrentCalaculation(ByVal isButtonClick As Boolean)
        ...
        If clsCommon.CompairString(strItemType, RowTypeItem) = CompairStringResult.Equal Then
            ...
            Dim dr As DataRow = clsCommon.ShowSelectFormForRow("BD@Ite", qry)
            If Not dr Is Nothing Then
                If clsCommon.myLen(clsCommon.myCstr(dr("Item"))) > 0 Then
                    SetitemWiseTaxSetting(True, True)
                    gv1.CurrentRow.Cells(colRowType).Value = RowTypeItem
        ...
        Try
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                ...
                If (clsCommon.myLen(strICode) > 0 OrElse clsCommon.myLen(strICodeGrp) > 0) Then
                    ...
                    '    common.clsCommon.MyMessageBoxShow("Please enter Booked Rate UOM for " + strIName + ". At Line No" + clsCommon.myCstr(ii + 1))
                    '    Return False
                    'End If
    ...
    Private Sub gv2_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gv2.DoubleClick
        Try
            If True Then
                ...
                Dim dblNewRate As Double = clsCommon.myCdbl(clsCommon.ShowSelectForm("DSBOTaxfnd", qry, "Rate", "Tax_Code='" + clsCommon.myCstr(gv2.CurrentRow.Cells(colTTaxAutCode).Value) + "' and Tax_Type='S'", "", "", True))
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtDocNo._MYValidating
        ...
        ''End If
        '-----------------------------------------------------
        LoadData(clsCommon.ShowSelectForm("DSBookingOrderNoFndd", qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked), NavigatorType.Current)
        btnCopy.Enabled = False
    End Sub
    ...
    Private Sub txtTaxGroup__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtTaxGroup._MYValidating
        ...
        txtTaxGroup.Value = clsCommon.ShowSelectForm("DSBOTaxGroupfndd", qry, "Code", "Tax_Group_Type='S'", txtTaxGroup.Value, "Code", isButtonClicked)
        SetTaxDetails()
    ...
    Private Sub txtVendorNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtVendorNo._MYValidating
        ...
        '-------richa 30/07/2014 Ticket No. BM00000003242---------
        If clsCommon.myLen(strwherecls) = 0 Then
            txtVendorNo.Value = clsCommon.ShowSelectForm("DS-BOCustFndr", qry, "Code", "TSPL_CUSTOMER_MASTER.Status='N' and (Parent_Customer_No='' or Parent_Customer_No is null)", txtVendorNo.Value, "Code", isButtonClicked)
        Else
            txtVendorNo.Value = clsCommon.ShowSelectForm("DS-BOCustFndr", qry, "Code", "TSPL_CUSTOMER_MASTER.Status='N' and (Parent_Customer_No='' or Parent_Customer_No is null) and TSPL_CUSTOMER_MASTER.Cust_Code in (" + strwherecls + ")", txtVendorNo.Value, "Code", isButtonClicked)
    ...
    Private Sub txtBillToLocation__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtBillToLocation._MYValidating
        ...
        End If
        txtBillToLocation.Value = clsCommon.ShowSelectForm("DS-BOLocFndr", qry, "Code", WhrCls, txtBillToLocation.Value, "Code", isButtonClicked)
        lblBillToLocation.Text = clsCommon.myCstr(clsDBFuncationality.getSingleValue("select Location_Desc from TSPL_LOCATION_MASTER where Location_Code='" + txtBillToLocation.Value + "'"))
        'Dim frm As New frmMandatoryFieldChecker
    ...
    Private Sub txtSalesman__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtSalesman._MYValidating
        ...
        txtSalesman.Value = clsCommon.ShowSelectForm("DS-BOSalesmanFndr", qry, "Code", whrcls, txtSalesman.Value, "Code", isButtonClicked)
...
#Region "New Mail System"
    ...
    Private Sub btnSendForApproval_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSendForApproval.Click
        Try
            ...
            If Not (common.clsCommon.MyMessageBoxShow(Me, "Send E-Mail/SMS Of Respective Booking No. " + txtDocNo.Value + "" + Environment.NewLine + "Are you sure?", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.Yes) Then
                Return
            End If
            LoadData(txtDocNo.Value, NavigatorType.Current)
