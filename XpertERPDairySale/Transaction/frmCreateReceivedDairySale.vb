...
#End Region
    ...
    Private Function GetCrateQty(ByVal strVehicle As String) As Double
        Dim dblCrateQty As Double = 0
        dblCrateQty = clsDBFuncationality.getSingleValue("SELECT SUM(CrateQty) AS CrateQty FROM ( " &
        "select Bill_To_Location,Customer_Code,Customer_Name,Vehicle_Code,TSPL_VEHICLE_MASTER.Description as VehicleNo ,CrateQty,CrateComments from " &
        "TSPL_SD_SALE_INVOICE_HEAD LEFT OUTER JOIN TSPL_CUSTOMER_MASTER ON TSPL_SD_SALE_INVOICE_HEAD.Customer_Code=TSPL_CUSTOMER_MASTER.Cust_Code " &
        "left  outer join TSPL_VEHICLE_MASTER on TSPL_SD_SALE_INVOICE_HEAD.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id " &
        "where CrateQty > 0  AND " &
        "Bill_To_Location='" & fndLocation.Value & "' and TSPL_SD_SALE_INVOICE_HEAD.Vehicle_Code='" & strVehicle & "' and convert(date,TSPL_SD_SALE_INVOICE_HEAD.Document_Date,103)= '" & clsCommon.GetPrintDate(txtInvoiceDate.Value, "dd/MMM/yyyy") & "'  " &
        "UNION ALL " &
        "SELECT Location_Code,Customer_Code,Customer_Name,TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Vehicle_Code,TSPL_VEHICLE_MASTER.Description as VehicleNo ,-1 * CrateQtyRecd,Remarks FROM " &
        "TSPL_CRATE_RECEIVED_HEAD_FRESHSALE LEFT OUTER JOIN TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE ON " &
        "TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Document_No=TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE.Document_No  LEFT OUTER JOIN " &
        "TSPL_CUSTOMER_MASTER ON TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE.Customer_Code=TSPL_CUSTOMER_MASTER.Cust_Code " &
        "left  outer join TSPL_VEHICLE_MASTER on TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id " &
        " where " &
        "Location_Code='" & fndLocation.Value & "' and TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Vehicle_Code='" & strVehicle & "'  and convert(date,TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.invoice_date,103)= '" & clsCommon.GetPrintDate(txtInvoiceDate.Value, "dd/MMM/yyyy") & "'  " &
    ...
    Sub FillGrid()
        ...
        End If
        Qry = "SELECT Customer_Code,MAX(Customer_Name) as  Customer_Name,Vehicle_Code,VehicleNo,SUM(Crate) AS Crate,SUM(jaali) AS jaali,SUM(Box) AS Box,sum(shippedCAN) as CAN FROM ( " &
            "select 0 as posted, TSPL_SD_SHIPMENT_HEAD.Bill_To_Location,TSPL_SD_SHIPMENT_HEAD.Customer_Code,Customer_Name,TSPL_SD_SHIPMENT_HEAD.Vehicle_Code,TSPL_VEHICLE_MASTER.Description as VehicleNo ,TSPL_SD_SHIPMENT_HEAD.Route_No,TSPL_ROUTE_MASTER.Route_Desc ,  TSPL_SD_SHIPMENT_HEAD.Crate,TSPL_SD_SHIPMENT_HEAD.jaali ,TSPL_SD_SHIPMENT_HEAD.Box  as Box,TSPL_SD_SHIPMENT_HEAD.shippedCAN  from TSPL_SD_SHIPMENT_HEAD " &
            "LEFT OUTER JOIN TSPL_CUSTOMER_MASTER ON TSPL_SD_SHIPMENT_HEAD.Customer_Code=TSPL_CUSTOMER_MASTER.Cust_Code  " &
            "left  outer join TSPL_VEHICLE_MASTER on TSPL_SD_SHIPMENT_HEAD.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id  " &
        ...
        End If
        Qry += " UNION ALL "
        Qry += " select 0 as posted, TSPL_SD_SALE_RETURN_HEAD.Bill_To_Location,TSPL_SD_SALE_RETURN_HEAD.Customer_Code,Customer_Name,TSPL_SD_SALE_RETURN_HEAD.Vehicle_Code, " &
        "TSPL_VEHICLE_MASTER.Description as VehicleNo  ,TSPL_SD_SALE_RETURN_HEAD.Route_No,TSPL_ROUTE_MASTER.Route_Desc , -1 * TSPL_SD_SALE_RETURN_HEAD.CrateMan  as Crate,-1 * TSPL_SD_SALE_RETURN_HEAD.jaali as Jaali ,-1 * TSPL_SD_SALE_RETURN_HEAD.box as Box,TSPL_SD_SALE_RETURN_HEAD.shippedCAN  from TSPL_SD_SALE_RETURN_HEAD  " &
       " LEFT OUTER JOIN TSPL_CUSTOMER_MASTER ON TSPL_SD_SALE_RETURN_HEAD.Customer_Code=TSPL_CUSTOMER_MASTER.Cust_Code  left  outer join " &
        " TSPL_VEHICLE_MASTER on TSPL_SD_SALE_RETURN_HEAD.Vehicle_Code=TSPL_VEHICLE_MASTER.Vehicle_Id  " &
        " left join TSPL_ROUTE_MASTER on TSPL_ROUTE_MASTER.Route_No =TSPL_SD_SALE_RETURN_HEAD.Route_No " &
        ...
        QryForCustomerOpening += " Opening.Customer_Code ,sum(Opening.OpencrateQty*Type) as OpencrateQty,sum(Opening.OpenJaaliQty*Type ) as OpenJaaliQty,sum(Opening.OpenBoxQty *Type)  as OpenBoxQty,sum(Opening.OpenCanQty *Type)  as OpenCanQty,sum(Opening.CrateQtyRecd *Type) as CrateQtyRecd,sum(Opening.JaaliQtyRecd*Type ) as JaaliQtyRecd,sum(Opening.BoxQtyRecd*Type ) as BoxQtyRecd,sum(Opening.CanQtyRecd*Type ) as CanQtyRecd,sum(Opening.CrateOutQty*Type ) as CrateOutQty,sum(Opening.jaaliOutQty*Type ) as jaaliOutQty ,sum(Opening.boxOutQty*Type ) as boxOutQty,sum(Opening.CanOutQty*Type ) as CanOutQty,sum(Opening.CrateAdjQty*Type ) as CrateAdjQty,sum(Opening.JaaliAdjQty*Type ) as JaaliAdjQty,sum(Opening.BoxAdjQty *Type) as  BoxAdjQty ,sum(Opening.CanAdjQty *Type) as  canAdjQty from " &
                   " (" &
                    ...
                    " 0  as BoxAdjQty," &
                      ...
                      " from TSPL_CRATE_RECEIVED_detail_FRESHSALE" &
                    " left join TSPL_CRATE_RECEIVED_HEAD_FRESHSALE on TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Document_No =TSPL_CRATE_RECEIVED_detail_FRESHSALE.Document_No " &
                    " where TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Type ='O' " &
                    " union all" &
                    " select TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Document_Date as Document_Date,TSPL_CRATE_RECEIVED_detail_FRESHSALE.Vehicle_Code ,TSPL_CRATE_RECEIVED_detail_FRESHSALE.Customer_Code  ,-1 as Type,TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Type as Type1  ," &
                    " isnull(TSPL_CRATE_RECEIVED_detail_FRESHSALE.CrateQtyRecd,0) + isnull(TSPL_CRATE_RECEIVED_detail_FRESHSALE.Adjustment,0)  as OpencrateQty," &
                    " isnull(TSPL_CRATE_RECEIVED_detail_FRESHSALE.JaaliQtyRecd,0) + isnull(TSPL_CRATE_RECEIVED_detail_FRESHSALE.jaaliAdjustment,0)    as OpenjaaliQty ," &
                    ...
                    " 0  as BoxAdjQty," &
                      ...
                      " from TSPL_CRATE_RECEIVED_detail_FRESHSALE" &
                    " left join TSPL_CRATE_RECEIVED_HEAD_FRESHSALE on TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Document_No =TSPL_CRATE_RECEIVED_detail_FRESHSALE.Document_No " &
                    " where TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Type ='I' " &
                    " )as Opening WHERE convert(date,Document_Date,103)<(convert(date,'" + txtDate.Value + "',103))"
        'If chkCustomerWise.Checked Then
        '    QryForCustomerOpening += " group by Customer_Code " + Environment.NewLine '----------Qry for Branch opening'
        'Else
...
" left join tspl_vehicle_master on tspl_vehicle_master.vehicle_id=pp.vehicle_code where 2=2  "
        Dim qryfinal As String = " With CTETemp as (" &
                   " Select convert(varchar,Doc_Date,103) as Doc_Date,Vehicle_Code,Vehicle_Name,Customer_Code,Customer_Name, OpencrateQty, OpenJaaliQty, OpenBoxQty,OpenCanQty, CrateQtyRecd, JaaliQtyRecd, BoxQtyRecd ,CanQtyRecd,CrateOutQty," &
                   " jaaliOutQty,boxOutQty  ,CanOutQty, CrateAdjQty , JaaliAdjQty  , BoxAdjQty ,CanAdjQty"
        qryfinal += ",SUM(CrateQtyClosing) OVER (Partition BY Customer_Code ORDER BY Customer_Code, Doc_Date) as CrateQtyClosing, " &
              " SUM(JaaliQtyClosing) OVER (Partition BY Customer_Code ORDER BY Customer_Code, Doc_Date) as JaaliQtyClosing, " &
              " SUM(BoxQtyClosing) OVER (Partition BY Customer_Code ORDER BY Customer_Code, Doc_Date) as BoxQtyClosing," &
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As Object, ByVal e As System.EventArgs, ByVal isButtonClicked As Boolean) Handles txtDocNo._MYValidating
        Try
            Qry = "SELECT Document_No as Code, FORMAT(Document_Date, 'dd/MM/yyyy')  as Date,Route_code,case when Type='I' then'IN'else'OUT' end as [I/O Type],TotalCrateQty, Location_Code as Location,Posted FROM TSPL_CRATE_RECEIVED_HEAD_FRESHSALE "
            Dim whrClas As String = ""
            If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
                whrClas = " Location_Code in (" + objCommonVar.strCurrUserLocations + ")"
            End If
            LoadData(clsCommon.ShowSelectForm("Docfnd", Qry, "Code", whrClas, txtDocNo.Value, "Code", isButtonClicked, "Document_Date"), NavigatorType.Current)
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
        End Try
    End Sub
        ...
        If transportSql.importExcel(gv, "Location Code", "Route No", "Type I/O", "Cust Code", "Date", "Shift", "Crate Qty") Then
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    'Counter += 1
                    Lineno += 1
                    Dim strUnique As String = strLocation & "_" & strRoute & "_" & strdate & "_" & strshift & "_" & strTypeIO
                    Try
...
where TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Location_Code='" & strLocation & "' and TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Route_code='" & strRoute & "' and TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.ShiftType='" & strshift & "' and convert(date,TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Document_Date,103)='" & clsCommon.GetPrintDate(strdate, "dd/MMM/yyyy") & "' and TSPL_CRATE_RECEIVED_HEAD_FRESHSALE.Type='" & strTypeIO & "' and TSPL_CRATE_RECEIVED_DETAIL_FRESHSALE.Customer_Code='" & strCustCode & "'"
                ...
                Else
                    ...
                    Try
                        For Each grow As GridViewRowInfo In gv.Rows
                            clsCommon.ProgressBarPercentUpdate((Counter + 1) * 100 / (gv.Rows.Count), "Updating .... " & clsCommon.myCstr(Counter + 1) & "/" & clsCommon.myCstr(gv.Rows.Count) & "")
                            Dim strLocation As String = clsCommon.myCstr(grow.Cells("Location Code").Value)
                            Dim strRoute As String = clsCommon.myCstr(grow.Cells("Route No").Value)
                            Dim strTypeIO As String = clsCommon.myCstr(grow.Cells("Type I/O").Value)
                            Dim strCustCode As String = clsCommon.myCstr(grow.Cells("Cust Code").Value)
...
left join TSPL_DISTRIBUTOR_ROUTE on TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Code=TSPL_DISTRIBUTOR_ROUTE.Code
 ...
 and TSPL_DISTRIBUTOR_ROUTE.Start_Date<='" & clsCommon.GetPrintDate(obj.Document_Date) & "' and TSPL_DISTRIBUTOR_ROUTE_CUSTOMER.Route_No='" & obj.Route_code & "' and 2=(Case when TSPL_DISTRIBUTOR_ROUTE.End_Date is null then 2 else (Case when TSPL_DISTRIBUTOR_ROUTE.End_Date>='" & clsCommon.GetPrintDate(obj.Document_Date) & "' then 2 else 3 end) end) order by Start_Date desc"
                            ...
                            End If
                            objTr.CrateQty = 0
                            objTr.CrateQtyRecd = dblCrateQty
                            objTr.Balance = 0
                            objTr.Remarks = ""
                            objTr.OutQty = 0
                            objTr.Adjustment = 0
