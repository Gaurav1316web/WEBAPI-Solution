...
#End Region
    ...
    Sub LoadReportType()
        ...
        cboReportType.SelectedValue = "ItemType"
    End Sub
    ...
    Private Sub RadChartView1_Drill(sender As Object, e As UI.DrillEventArgs) Handles RadChartView1.Drill
        Try
            ...
            If clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "ItemType") = CompairStringResult.Equal Then
                If Not arrDrillDownBack.Contains("ItemType") Then
                    arrDrillDownBack.Add("ItemType")
                End If
                cboReportType.SelectedValue = "ItemWise"
                cboReportType.Tag = strSelectedValue
                Load_Report()
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "ItemWise") = CompairStringResult.Equal Then
                cboReportType.Tag = Nothing
            End If
        Catch ex As Exception
            clsCommon.MyMessageBoxShow(Me, ex.Message, Me.Text)
    ...
    Private Sub btnBack_Click(sender As Object, e As EventArgs) Handles btnBack.Click
        Try
            ...
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "ItemWise") = CompairStringResult.Equal AndAlso arrDrillDownBack.Contains("ItemType") Then
                arrDrillDownBack.Remove("ItemType")
                cboReportType.SelectedValue = "ItemType"
                cboReportType.Tag = Nothing
                Load_Report()
            End If
    ...
    Public Sub Load_Report()
        Try
            ...
            qry = "Select isnull(TSPL_ITEM_MASTER.Alies_Name,'') as Alies_Name,Case When isnull(TSPL_ITEM_MASTER.Is_Ambient,0) =1 Then 'Ambient' else 'Fresh' end as ItemType,xxxx.* from ( " + clsPSInvoiceHead.GetReportDataQuery(obj) +
            ")xxxx left join tspl_item_master on tspl_item_master.Item_Code=xxxx.[Item Code] where (isnull(TSPL_ITEM_MASTER.Is_Ambient,0) =1 or isnull(TSPL_ITEM_MASTER.Is_FreshItem,0) =1)"
            If clsCommon.myLen(cboReportType.Tag) > 0 Then
                qry = "select * from (" + qry + ")x where ItemType='" + clsCommon.myCstr(cboReportType.Tag) + "' "
            End If


            Dim sQuery As String = ""
            If clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "ItemType") = CompairStringResult.Equal Then
                sQuery = "Select ItemType as GrpCode,Convert(Decimal(18,2),sum([Sale Amount] )) As [Sale Amount],Sum(Quantity) As Quantity from ( " + qry + ")xxxxx Group by ItemType"
            ElseIf clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "ItemWise") = CompairStringResult.Equal Then
                sQuery = "Select Alies_Name as GrpCode,Convert(Decimal(18,2),sum([Sale Amount] )) As [Sale Amount],Sum(Quantity) As Quantity from ( " + qry + ")xxxxx Group by Alies_Name "
            End If
            ...
            '    sQuery = "select [MCC Code] as GrpCode,max([MCC Name]) as GrpName " + strColumn2 + ",sum( [Milk Weight]) as [Milk Weight],sum([FAT(KG)]) as [FAT(KG)],sum([SNF(KG)]) as [SNF(KG)],sum([SRN Amount]) as [SRN Amount] from (" + qry + " )xxxxxx Group by [MCC Code]" + strColumn2AfterGrp + " order by [MCC Code]" + strColumn2AfterGrp + ""
            'ElseIf clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "Route") = CompairStringResult.Equal Then
            '    sQuery = "select [Route Code] as GrpCode,max([Route Name]) as GrpName " + strColumn2 + ",sum( [Milk Weight]) as [Milk Weight],sum([FAT(KG)]) as [FAT(KG)],sum([SNF(KG)]) as [SNF(KG)],sum([SRN Amount]) as [SRN Amount] from (" + qry + " )xxxxxx Group by [Route Code]" + strColumn2AfterGrp + " order by [Route Code]" + strColumn2AfterGrp + ""
            'ElseIf clsCommon.CompairString(clsCommon.myCstr(cboReportType.SelectedValue), "VLC") = CompairStringResult.Equal Then
            '    sQuery = "select [VLC Code] as GrpCode,max([VLC Name]) as GrpName " + strColumn2 + ",sum([Milk Weight]) as [Milk Weight],sum([FAT(KG)]) as [FAT(KG)],sum([SNF(KG)]) as [SNF(KG)],sum([SRN Amount]) as [SRN Amount] from (" + qry + " )xxxxxx Group by [VLC Code]" + strColumn2AfterGrp + " order by [VLC Code]" + strColumn2AfterGrp + ""
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                qry = ""
                If clsCommon.myLen(cboReportType.Tag) > 0 Then
                    qry += "-> Item Type [" + clsCommon.myCstr(cboReportType.Tag) + "]"
                End If
                ...
                If arrRow IsNot Nothing AndAlso arrRow.Count > 0 Then
                    ...
                    For Each strVal As String In arrRow
                        ...
                        barSeries.CategoryMember = "GrpCode"
                ...
                If (dtNew.Rows.Count + arrRow.Count) > 50 Then
                    IntWidth = 200 + (300 * (dtNew.Rows.Count + arrRow.Count))
                ElseIf (dtNew.Rows.Count + arrRow.Count) > 40 Then
                    IntWidth = 200 + (250 * (dtNew.Rows.Count + arrRow.Count))
                ElseIf (dtNew.Rows.Count + arrRow.Count) > 30 Then
                    IntWidth = 200 + (200 * (dtNew.Rows.Count + arrRow.Count))
                ElseIf (dtNew.Rows.Count + arrRow.Count) > 20 Then
                    IntWidth = 200 + (150 * (dtNew.Rows.Count + arrRow.Count))
                ElseIf (dtNew.Rows.Count + arrRow.Count) > 10 Then
                    IntWidth = 200 + (100 * (dtNew.Rows.Count + arrRow.Count))
                ElseIf (dtNew.Rows.Count + arrRow.Count) > 0 Then
                    IntWidth = 200 + (50 * (dtNew.Rows.Count + arrRow.Count))
                End If
