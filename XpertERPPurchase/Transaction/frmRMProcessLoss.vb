...
Public Class frmRMProcessLoss
    ...
    Sub View()
        ...
        If gv1.Rows.Count >= 0 Then
            ...
            view.ColumnGroups(2).Rows(0).ColumnNames.Add(gv1.Columns(colopQty).Name)
            view.ColumnGroups(2).Rows(0).ColumnNames.Add(gv1.Columns(colopAmt).Name)


            view.ColumnGroups.Add(New GridViewColumnGroup("Received"))
            view.ColumnGroups(3).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(3).Rows(0).ColumnNames.Add(gv1.Columns(colRecQty).Name)
            view.ColumnGroups(3).Rows(0).ColumnNames.Add(gv1.Columns(colRecAmt).Name)

            view.ColumnGroups.Add(New GridViewColumnGroup("Issue In Production"))
            view.ColumnGroups(4).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(4).Rows(0).ColumnNames.Add(gv1.Columns(colIssProdQty).Name)
            view.ColumnGroups(4).Rows(0).ColumnNames.Add(gv1.Columns(colIssProdAmt).Name)

            view.ColumnGroups.Add(New GridViewColumnGroup("Other Issue"))
            view.ColumnGroups(5).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(5).Rows(0).ColumnNames.Add(gv1.Columns(colOtherIssQty).Name)
            view.ColumnGroups(5).Rows(0).ColumnNames.Add(gv1.Columns(colOtherIssAmt).Name)

            view.ColumnGroups.Add(New GridViewColumnGroup("Stock Transfer"))
            view.ColumnGroups(6).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(6).Rows(0).ColumnNames.Add(gv1.Columns(colStockTransferQty).Name)
            view.ColumnGroups(6).Rows(0).ColumnNames.Add(gv1.Columns(colStockTransferAmt).Name)

            view.ColumnGroups.Add(New GridViewColumnGroup("Closing Stock"))
            view.ColumnGroups(7).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(7).Rows(0).ColumnNames.Add(gv1.Columns(colClQty).Name)
            view.ColumnGroups(7).Rows(0).ColumnNames.Add(gv1.Columns(colClAmt).Name)

            view.ColumnGroups.Add(New GridViewColumnGroup("Process Loss"))
            view.ColumnGroups(8).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(8).Rows(0).ColumnNames.Add(gv1.Columns(colPLQty).Name)
            view.ColumnGroups(8).Rows(0).ColumnNames.Add(gv1.Columns(colPLper).Name)



            view.ColumnGroups.Add(New GridViewColumnGroup(""))
            view.ColumnGroups(9).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(9).Rows(0).ColumnNames.Add(gv1.Columns(colRate).Name)

            view.ColumnGroups.Add(New GridViewColumnGroup("Closing Stock"))
            view.ColumnGroups(10).Rows.Add(New GridViewColumnGroupRow())
            view.ColumnGroups(10).Rows(0).ColumnNames.Add(gv1.Columns(colFinalClStock).Name)
...
where CONVERT(DATE,PROD_DATE,103)>= convert(date,'" + clsCommon.GetPrintDate(Slot1FD) + "' ,103) and  CONVERT(DATE,PROD_DATE,103)<= convert(date,'" + clsCommon.GetPrintDate(Slot3TD) + "' ,103))    
                     ...
                     where Punching_Date < '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(Slot1FD), "dd/MMM/yyyy hh:mm:ss tt") + "' group by xxx.Item_Code,xxx.Location_Code 
...
where CONVERT(DATE,PROD_DATE,103)>= convert(date,'" + clsCommon.GetPrintDate(Slot1FD) + "' ,103) and  CONVERT(DATE,PROD_DATE,103)<= convert(date,'" + clsCommon.GetPrintDate(Slot3TD) + "' ,103))   
                     and ( ((case when Is_Section='N' and Is_Sub_Location='N' then Location_Code else Main_Location_Code end) = '" + clsCommon.myCstr(txtLoc.Value) + "') )) xxx 
                     where Punching_Date>= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(Slot1FD), "dd/MMM/yyyy hh:mm:ss tt") + "' and Punching_Date<= '" + clsCommon.GetPrintDate(clsCommon.GetDateWithStartTime(Slot3TD), "dd/MMM/yyyy hh:mm:ss tt") + "' 
                     union  all 
                    ...
                    ,0 as FATAmount
                    ,0 as SNFAmount
                     ,In_Category,Out_Category,(case when ConvertedUnitp.processLoss_Uom=1 then ConvertedUnitp.UOM_Code else ConvertedUnits.UOM_Code end) as CONuom from (SELECT 0 as Trans_Id,null as Trans_Type,null as Trans_Type_Name, null as Source_Doc_No, thedate as Punching_Date,'In' as InOutView,'I' as InOut,TSPL_LOCATION_MASTER.Location_Code as Location_Code,TSPL_LOCATION_MASTER.Location_Desc as [Loc Desp],null as [LocAddress],null as SourceCode,null as SourceName,null as SourceType ,TSPL_ITEM_MASTER.Item_Type,null as Item_Type_Name,null as Item_Group,null as Group_Description,null as [FINISHFOOD],null as [RAWMATERIAL],null as [OTHER],null as [PACKINGMAT],null as [FIXEDASST],null as [MAKE],null as [KV],null as [FINISHFOODDESC],null as [RAWMATERIALDESC],null as [OTHERDESC],null as [PACKINGMATDESC],null as [FIXEDASSTDESC],null as [MAKEDESC],null as [KVDESC],TSPL_ITEM_MASTER.Item_Code,TSPL_ITEM_MASTER.Item_Desc,null as Item_Category_Struct_Code,TSPL_ITEM_UOM_DETAIL.UOM_Code as Stock_UOM ,null as  itf_code ,0 as Stock_Qty,0 as Balance_QTYKG,0 as Rate,0 as Cost,0 as Balance_FAT, 0 as Balance_SNF ,null as In_Category,null as Out_Category,TSPL_ITEM_MASTER.Product_Type   FROM ExplodeDates( '" + clsCommon.GetPrintDate(Slot1FD) + "','" + clsCommon.GetPrintDate(Slot3TD) + "') as d,TSPL_ITEM_MASTER,TSPL_LOCATION_MASTER,TSPL_ITEM_UOM_DETAIL where 2=2  and TSPL_ITEM_MASTER.Item_Code in (select distinct TSPL_MF_BOM_DETAIL.CONSM_ITEM_CODE from TSPL_SPP_PRODUCTION_ENTRY_DETAIL 
left outer join TSPL_SPP_PRODUCTION_ENTRY on TSPL_SPP_PRODUCTION_ENTRY.PROD_ENTRY_CODE=TSPL_SPP_PRODUCTION_ENTRY_DETAIL.PROD_ENTRY_CODE
left outer join TSPL_MF_BOM_HEAD on TSPL_MF_BOM_HEAD.BOM_CODE=TSPL_SPP_PRODUCTION_ENTRY_DETAIL.BOM_CODE
left outer join TSPL_MF_BOM_DETAIL on TSPL_MF_BOM_DETAIL.BOM_CODE=TSPL_MF_BOM_HEAD.BOM_CODE
where CONVERT(DATE,PROD_DATE,103)>= convert(date,'" + clsCommon.GetPrintDate(Slot1FD) + "' ,103) and  CONVERT(DATE,PROD_DATE,103)<= convert(date,'" + clsCommon.GetPrintDate(Slot3TD) + "' ,103))   and ( ((case when Is_Section='N' and Is_Sub_Location='N' then Location_Code else Main_Location_Code end) = '" + clsCommon.myCstr(txtLoc.Value) + "') )  and TSPL_ITEM_UOM_DETAIL.Stocking_Unit='Y' and TSPL_ITEM_UOM_DETAIL.Item_Code=TSPL_ITEM_MASTER.Item_Code) Items
...
left join (select TSPL_ITEM_QC_PARAMETER_MASTER.Item_Code,TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range as Fat_Per  from TSPL_ITEM_QC_PARAMETER_MASTER  left join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where TSPL_PARAMETER_MASTER.Type='FAT') Fat on Items.Item_Code=Fat.Item_Code  left join  (select TSPL_ITEM_QC_PARAMETER_MASTER.Item_Code,TSPL_ITEM_QC_PARAMETER_MASTER.Actual_Range as SNF_Per  from TSPL_ITEM_QC_PARAMETER_MASTER  left join TSPL_PARAMETER_MASTER on TSPL_PARAMETER_MASTER.Code=TSPL_ITEM_QC_PARAMETER_MASTER.Code  where TSPL_PARAMETER_MASTER.Type='SNF') as SNF on Items.Item_Code=SNF.Item_Code where 2=2  )xxxxxx Group by  Item_Code,Location_Code,Punching_Date )xxxxxxx left outer join tspl_location_master on tspl_location_master.Location_Code=xxxxxxx.Location_code where Punching_Date is not null )x )final where Location_Code= '" + clsCommon.myCstr(txtLoc.Value) + "'
                     AND Punching_Date= '" + clsCommon.GetPrintDate((Slot1FD), "dd/MM/yyyy") + "'
