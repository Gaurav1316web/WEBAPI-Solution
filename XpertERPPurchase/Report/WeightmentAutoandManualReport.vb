...
Public Class Weightment_Auto_and_Manual_Report
    ...
    Private Sub gv1_DoubleClick(sender As Object, e As EventArgs) Handles gv1.DoubleClick
        Try
            ...
            Dim strqry As String = ""
            'TemplateGridview = gv1
            '        If rbtWeightment.Checked = True AndAlso chkIncludeQC.Checked AndAlso strcolm2 = "Visual QC" Then
            '            strqry = " 	 Select ROW_NUMBER() OVER (PARTITION BY TSPL_GRN_HEAD.Bill_To_Location ORDER BY TSPL_GRN_HEAD.Bill_To_Location) AS SrNo,TSPL_GRN_HEAD.Bill_To_Location as Location ,tspl_grn_head.VehicleNo [Vehicle NO],
            'tspl_grn_head.Vendor_Code[Vendor Code],tspl_grn_head.Vendor_Name[Vendor Name],tspl_grn_head.GRN_No[GRN No],tspl_grn_head.GRN_Date[GRN Date],
            'CASE WHEN TSPL_GRN_HEAD.VisualQCStatus=1 THEN 'APPROVED' else 'PENDING' end as Visual_QC from tspl_grn_head
            ' where TSPL_GRN_HEAD.VisualQCStatus=1 AND convert(date,TSPL_GRN_HEAD.GRN_Date,103)>=convert(date,('" + txtFromDate.Value + "'),103) 
            '         and convert(date,TSPL_GRN_HEAD.GRN_Date,103)<=convert(date,('" + txtToDate.Value + "'),103)  
            'and TSPL_GRN_HEAD.Bill_To_Location=('" + strloc + "') "
            '        ElseIf rbtWeightment.Checked = True AndAlso chkIncludeQC.Checked AndAlso strcolm2 = "NIR QC" Then
            '            strqry = " Select ROW_NUMBER() OVER (PARTITION BY TSPL_MRN_HEAD.Bill_To_Location ORDER BY TSPL_MRN_HEAD.Bill_To_Location) AS SrNo,TSPL_MRN_HEAD.Bill_To_Location as Location,
            '                        Document_No as [NIR No],Document_Date as [NIR Date],TSPL_MRN_HEAD.MRN_No as [MRN No],TSPL_MRN_HEAD.MRN_Date as [MRN Date],TSPL_MRN_HEAD.VehicleNo,TSPL_MRN_HEAD.Vendor_Code[Vendor Code],TSPL_MRN_HEAD.Vendor_Name[Vendor Name],case when TSPL_NIR_QC.QC_Status=1 then 'APPROVED' else 'Pending' end as [QC Status] from TSPL_NIR_QC
            '                        left outer join TSPL_MRN_HEAD ON TSPL_MRN_HEAD.MRN_No = TSPL_NIR_QC.MRN_No
            '                        where  convert(date,TSPL_NIR_QC.Document_Date,103)>=convert(date,('" + txtFromDate.Value + "'),103) 
            '                         and convert(date,TSPL_NIR_QC.Document_Date,103)<=convert(date,('" + txtToDate.Value + "'),103)  
            '                and TSPL_MRN_HEAD.Bill_To_Location=('" + strloc + "') "
            '        ElseIf rbtWeightment.Checked = True AndAlso chkIncludeQC.Checked AndAlso strcolm2 = "WET QC" Then
            '            strqry = "  Select ROW_NUMBER() OVER (PARTITION BY TSPL_QC_CHECK_HEAD.Bill_To_Location ORDER BY TSPL_QC_CHECK_HEAD.Bill_To_Location) AS SrNo,TSPL_QC_CHECK_HEAD.Bill_To_Location as Location,TSPL_QC_CHECK_HEAD.Document_Code as [WET QC No],Document_Date as [WET QC Date],tspl_grn_head.VehicleNo [Vehicle NO],
            'tspl_grn_head.Vendor_Code[Vendor Code],tspl_grn_head.Vendor_Name[Vendor Name],tspl_grn_head.GRN_No[GRN No],tspl_grn_head.GRN_Date[GRN Date],TSPL_QC_CHECK_HEAD.QC_Status as [QC Status] from TSPL_QC_CHECK_HEAD
            '        left outer join TSPL_GRN_HEAD ON TSPL_GRN_HEAD.GRN_No = TSPL_QC_CHECK_HEAD.Gate_Entry_No
            '        where  convert(date,TSPL_QC_CHECK_HEAD.Document_Date,103)>=convert(date,('" + txtFromDate.Value + "'),103) 
            '         and convert(date,TSPL_QC_CHECK_HEAD.Document_Date,103)<=convert(date,('" + txtToDate.Value + "'),103)  
            'and TSPL_QC_CHECK_HEAD.Bill_To_Location=('" + strloc + "') "
            If rbtWeightment.Checked = True AndAlso chkIncludeQC.Checked Then
			 ...
			 ,tspl_grn_head.GRN_No[GRN No],CASE WHEN TSPL_GRN_HEAD.VisualQCStatus=1 THEN 'APPROVED' else 'PENDING' end as Visual_QC,
            ...
            If dt Is Nothing OrElse dt.Rows.Count > 0 Then
                ...
                For ii As Integer = 0 To gv2.Columns.Count - 1
                    gv2.Columns(ii).ReadOnly = True
                Next
                ' If rbtWeightment.Checked = True AndAlso chkIncludeQC.Checked AndAlso strcolm2 <> "NIR QC" AndAlso strcolm2 <> "Visual QC" AndAlso strcolm2 <> "WET QC" Then
                summaryDrill()
                ' End If
                'summaryDrill()
