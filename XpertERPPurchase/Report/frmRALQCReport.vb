...
Public Class frmRALQCReport
    ...
    Private Sub btnGo_Click(sender As Object, e As EventArgs) Handles btnGo.Click
        Try
            Dim strQry As String = "select [RAL Code],(Select SUM(Qty) from TSPL_TENDER_DETAIL Where DocumentCode=xx.[RAL Code]) As [RAL Qty],SUM(ISNULL([GRN Qty],0)) as [GRN Qty], SUM(ISNULL([SRN Qty],0)) as [SRN Qty], SUM([Visual QC]) as [Visual QC],SUM([NIR QC]) AS [NIR QC],COUNT([WET/Chemical QC])As [WET/Chemical QC] from (" & ReturnBaseQry(False) & ")xx group by [RAL Code]"
...
Inner Join TSPL_TENDER_HEADER On TSPL_TENDER_HEADER.DocumentCode=TSPL_TENDER_DETAIL.DocumentCode"
    ...
    Private Sub ExportToExcel(ByVal exporter As EnumExportTo, ByVal gv As RadGridView)
        Try
            ...
            If exporter = EnumExportTo.Excel Then
                clsCommon.MyExportToExcelGrid("RAL QC Report", gv, arrHeader, Me.Text)
            Else
    ...
    Private Sub gv1_CellDoubleClick(sender As Object, e As GridViewCellEventArgs) Handles gv1.CellDoubleClick
        Try
            Dim strQry As String = "Select [RAL Code],[Vendor Code],[Vendor Name],[Vehicle No],[Item Code],[Item Desc],[GRN No],[GRN Qty],[Gross Weight],[Tare Weight],[Net Weight],[SRN No],[SRN Qty],[Visual QC Status],[NIR QC Status],[WET/Chemical QC] from (" & ReturnBaseQry(True) & ")xyz"
            Dim dt As DataTable = clsDBFuncationality.GetDataTable(strQry)
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ReportDetailsGrid()
