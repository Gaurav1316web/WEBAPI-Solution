...
#Region "Variables"
    ...
    Const ReportID As String = "CashFlowRpt"
...
#End Region
        ...
        Try
            ...
            Dim BaseQry As String = " select TSPL_GL_ACCOUNTS.Account_Balance,TSPL_JOURNAL_DETAILS.Account_code,TSPL_GL_ACCOUNTS.Description as AccountName,TSPL_ACCOUNT_MAIN_GL_ACCOUNT.Main_GL_Account as Main_GL_Account,TSPL_ACCOUNT_MAIN_GL_ACCOUNT.main_gl_Account_desc as MainGLName,TSPL_ACCOUNT_Sub_GROUPS.Account_Sub_Group_Code as SubGrpCode,TSPL_ACCOUNT_Sub_GROUPS.Account_Sub_Group_desc as SubGrpdesc,TSPL_CASH_FLOW_PERFORMA.Group_Code as GrpCode,TSPL_CASH_FLOW_PERFORMA.Group_Name as GrpName, Amount,TSPL_CASH_FLOW_PERFORMA.S_No,TSPL_CASH_FLOW_PERFORMA.Main_Particular,TSPL_CASH_FLOW_PERFORMA.type as typ,TSPL_CASH_FLOW_PERFORMA.Particular,TSPL_CASH_FLOW_PERFORMA.Type,TSPL_CASH_FLOW_PERFORMA.Note,TSPL_ACCOUNT_GROUPS.Account_Group_Code,TSPL_ACCOUNT_GROUPS.Account_Group_Desc " & strExtraColumn & " from TSPL_JOURNAL_DETAILS " + Environment.NewLine + _
                " inner join TSPL_GL_ACCOUNTS as TabRollupName on TabRollupName.Account_Code=" + Environment.NewLine + _
                                     ...
                                     " inner join TSPL_CASH_FLOW_PERFORMA on TSPL_CASH_FLOW_PERFORMA.Group_Code= TSPL_ACCOUNT_MAIN_GROUPS.Account_Main_Group_Code "
            If settSelectGLInCashFlowPerforma Then
                BaseQry += " inner join TSPL_CASH_FLOW_PERFORMA_GL_MAIN on TSPL_CASH_FLOW_PERFORMA_GL_MAIN.Main_GL_Account=TSPL_ACCOUNT_MAIN_GL_ACCOUNT.Main_GL_Account and TSPL_CASH_FLOW_PERFORMA.S_No=TSPL_CASH_FLOW_PERFORMA_GL_MAIN.SNo "
            End If
            ...
            If rbtnNA.IsChecked OrElse rbtnTreeView.IsChecked Then
                sQuery = "select palp.S_No,palp.Main_Particular,palp.Particular,GrpCode,GrpName,case when palp.Formula='' then null else Formula end as Formula,subqry.* from " _
                       & " TSPL_CASH_FLOW_PERFORMA palp Left join(" + Replace(fnlQty, "order by S_No", "") + ") subqry on subqry.s_no=palp.s_no order by palp.s_no"
    ...
    Sub SetGridFormation(ByVal arr As Dictionary(Of String, clsTempBalanceSheet))
        Dim qry As String = "select * from TSPL_CASH_FLOW_PERFORMA where Font_Style>'1.000000'"
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                If exporter = EnumExportTo.Excel Then
                    clsCommon.MyExportToExcelGrid("Profit and Loss", gv1, arrHeader, Me.Text, True)
                ElseIf exporter = EnumExportTo.PDF Then
                    clsCommon.MyExportToPDF("Profit and Loss", gv1, arrHeader, Me.Text, PageSetupReport_ID, objCommonVar.CurrentUserCode)
                End If
        ...
        Try
            ...
            ElseIf ChkGLAccount.IsChecked Then
                ...
                If clsCommon.myLen(strACode) > 0 Then
                    ...
                    frm.boolPrevWithoutOpening = True
