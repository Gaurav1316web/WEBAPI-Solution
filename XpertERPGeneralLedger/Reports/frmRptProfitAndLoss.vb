...
'==============modify by Preeti gupta Against Ticket No[BM00000007734]
Imports common
Imports System.IO
...
#End Region
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt(clsUserMgtCode.rptProfitAndLoss)
    ...
    Private Sub FrmRptSales_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ElseIf ReportLevel = 3 Then
            rbtnSubGroup.IsChecked = True
            rbtnGLMainAccount.IsChecked = True
            Dim arr As New ArrayList()
    ...
    Sub GetReportGridId()
        ...
        If rbtnMainGroup.IsChecked = True Then
            VarID += "_M"
        End If
        If rbtnGroup.IsChecked = True Then
            VarID += "_G"
        End If
        ...
        Try
            ...
            Dim BaseQry As String = " select TSPL_GL_ACCOUNTS.Account_Balance,TSPL_JOURNAL_DETAILS.Account_code,TSPL_GL_ACCOUNTS.Description as AccountName,TSPL_ACCOUNT_MAIN_GL_ACCOUNT.Main_GL_Account as Main_GL_Account,TSPL_ACCOUNT_MAIN_GL_ACCOUNT.main_gl_Account_desc as MainGLName,TSPL_ACCOUNT_Sub_GROUPS.Account_Sub_Group_Code as SubGrpCode,TSPL_ACCOUNT_Sub_GROUPS.Account_Sub_Group_desc as SubGrpdesc,TSPL_PROFIT_AND_LOSS_PERFORMA.Group_Code as GrpCode,TSPL_PROFIT_AND_LOSS_PERFORMA.Group_Name as GrpName, Amount,TSPL_PROFIT_AND_LOSS_PERFORMA.S_No,TSPL_PROFIT_AND_LOSS_PERFORMA.Main_Particular,TSPL_PROFIT_AND_LOSS_PERFORMA.type as typ,TSPL_PROFIT_AND_LOSS_PERFORMA.Particular,TSPL_PROFIT_AND_LOSS_PERFORMA.Type,TSPL_PROFIT_AND_LOSS_PERFORMA.Note,TSPL_ACCOUNT_GROUPS.Account_Group_Code,TSPL_ACCOUNT_GROUPS.Account_Group_Desc " & strExtraColumn & " from TSPL_JOURNAL_DETAILS " + Environment.NewLine + _
                " inner join TSPL_GL_ACCOUNTS as TabRollupName on TabRollupName.Account_Code=" + Environment.NewLine + _
                                     ...
                                     " inner join TSPL_PROFIT_AND_LOSS_PERFORMA on TSPL_PROFIT_AND_LOSS_PERFORMA.Group_Code= TSPL_ACCOUNT_MAIN_GROUPS.Account_Main_Group_Code "
            If settSelectGLInProftAndLossPerforma Then
                BaseQry += " inner join TSPL_PROFIT_AND_LOSS_PERFORMA_GL_MAIN on TSPL_PROFIT_AND_LOSS_PERFORMA_GL_MAIN.Main_GL_Account=TSPL_ACCOUNT_MAIN_GL_ACCOUNT.Main_GL_Account and TSPL_PROFIT_AND_LOSS_PERFORMA.S_No=TSPL_PROFIT_AND_LOSS_PERFORMA_GL_MAIN.SNo "
            End If
            ...
            If rbtnMainGroup.IsChecked OrElse chkTreeView.Checked Then
            ...
            ElseIf rbtnGroup.IsChecked Then
            ...
            ElseIf rbtnSubGroup.IsChecked Then
            ...
            ElseIf rbtnGLMainAccount.IsChecked Then
            ...
            ElseIf rbtnGLAccount.IsChecked Then
            ...
            If chkTreeView.Checked Then
                sQuery = "select palp.S_No,palp.Main_Particular,palp.Particular,GrpCode,GrpName,case when palp.Formula='' then null else Formula end as Formula,subqry.* from " _
                       & " TSPL_PROFIT_AND_LOSS_PERFORMA palp Left join(" + Replace(fnlQty, "order by S_No", "") + ") subqry on subqry.s_no=palp.s_no order by palp.s_no"
                ...
                For Each dr As DataRow In dtTotal.Select("Formula is not null", "S_No ASC")
                    Dim Formula As String = "" ''BHA/17/10/18-000631 by balwinder on 23/10/2018
                ...
                dt.Rows.Clear()
                fnlQty = "select Account_Balance,MAX(S_No) as S_No, Main_Particular ,Particular ,max(Note) as Note"
                If rbtnMainGroup.IsChecked Then
                    fnlQty += ",GrpCode as GrpCode,(max(isnull(GrpName,''))) as GrpName"
                ElseIf rbtnGroup.IsChecked Then
                    fnlQty += ",GrpCode as GrpCode,(max(isnull(GrpName,''))) as GrpName,Account_Group_code as [Account Group Code] ,(max(isnull(Account_group_desc,''))) as [Account Group Desc]"
                ElseIf rbtnSubGroup.IsChecked Then
                    fnlQty += ",GrpCode as GrpCode,(max(isnull(GrpName,''))) as GrpName,Account_Group_code as [Account Group Code] ,(max(isnull(Account_group_desc,''))) as [Account Group Desc],SubGrpCode as [Sub Group Code],max( SubGrpDesc ) as [Sub Grp Desc]"
                ElseIf rbtnGLMainAccount.IsChecked Then
                    fnlQty += ",GrpCode as GrpCode,(max(isnull(GrpName,''))) as GrpName,Account_Group_code as [Account Group Code] ,(max(isnull(Account_group_desc,''))) as [Account Group Desc],SubGrpCode as [Sub Group Code],max( SubGrpDesc ) as [Sub Grp Desc],Main_GL_Account as [Main GL Account],max( MainGLName) as [Main GL Name] "
                ElseIf rbtnGLAccount.IsChecked Then
                    fnlQty += ",GrpCode as GrpCode,(max(isnull(GrpName,''))) as GrpName,Account_Group_code as [Account Group Code] ,(max(isnull(Account_group_desc,''))) as [Account Group Desc],SubGrpCode as [Sub Group Code],max( SubGrpDesc ) as [Sub Grp Desc],Main_GL_Account as [Main GL Account],max(MainGLName) as [Main GL Name],Account_code as [GL Account],max(AccountName) as [GL Account Name] "
                End If
                If arr IsNot Nothing AndAlso arr.Count > 0 Then
                    For Each strkey As KeyValuePair(Of String, clsTempBalanceSheet) In arr
                        fnlQty += ",sum(" + strkey.Key + ")* typ as " + strkey.Key
                    Next
                End If
                fnlQty += ", sum(Amount)* typ  as Amt ,(typ*  sum(Amount) )  as AmtActual  from(" + BaseQry + ") Final group by Main_Particular,typ,Particular,Account_Balance "
                If rbtnMainGroup.IsChecked Then
                    fnlQty += ",GrpCode"
                ElseIf rbtnGroup.IsChecked Then
                    fnlQty += ",GrpCode,Account_Group_Code"
                ElseIf rbtnSubGroup.IsChecked Then
                    fnlQty += ",GrpCode,Account_Group_Code,SubGrpCode"
                ElseIf rbtnGLMainAccount.IsChecked Then
                    fnlQty += ",GrpCode,Account_Group_Code,SubGrpCode,Main_GL_Account"
                ElseIf rbtnGLAccount.IsChecked Then
                    fnlQty += ",GrpCode,Account_Group_Code,SubGrpCode,Main_GL_Account,Account_code"
                End If
                fnlQty += " order by S_No"
                Dim dtRawData As DataTable = clsDBFuncationality.GetDataTable(fnlQty)
                Dim SNo As Integer = -1
                Dim strMainGrpCode As String = Nothing
                Dim strGrpCode As String = Nothing
                Dim strSubGrpCode As String = Nothing
                Dim strMainGLCode As String = Nothing
                Dim strGLCode As String = Nothing

                Dim intMainGrpVersion As Integer = 0
                Dim intGrpVersion As Integer = 0
                Dim intSubGrpVersion As Integer = 0
                Dim intMainGLVersion As Integer = 0
                Dim intGLVersion As Integer = 0

                If dtRawData IsNot Nothing AndAlso dtRawData.Rows.Count > 0 Then
                    For ii As Integer = 0 To dtRawData.Rows.Count - 1
                        If rbtnMainGroup.IsChecked Then
                            If Not (SNo = clsCommon.myCdbl(dtRawData.Rows(ii)("S_No")) OrElse clsCommon.CompairString(strMainGrpCode, clsCommon.myCstr(dtRawData.Rows(ii)("GrpCode"))) = CompairStringResult.Equal) Then
                                SNo = clsCommon.myCdbl(dtRawData.Rows(ii)("S_No"))
                                strMainGrpCode = clsCommon.myCstr(dtRawData.Rows(ii)("GrpCode"))
                                intMainGrpVersion = clsCommon.myCdbl(dtRawData.Rows(ii)("Main_Particular"))
                                AddNewRowForTree(dt, dtRawData.Rows(ii), clsCommon.myCstr(dtRawData.Rows(ii)("GrpName")), clsCommon.myCstr(intMainGrpVersion))
                                If arr IsNot Nothing AndAlso arr.Count > 0 Then
                                    For Each strkey As KeyValuePair(Of String, clsTempBalanceSheet) In arr
                        ...
                        ElseIf rbtnGroup.IsChecked Then
                            If Not (SNo = clsCommon.myCdbl(dtRawData.Rows(ii)("S_No")) OrElse clsCommon.CompairString(strMainGrpCode, clsCommon.myCstr(dtRawData.Rows(ii)("GrpCode"))) = CompairStringResult.Equal) Then
                                ...
                                AddNewRowForTree(dt, dtRawData.Rows(ii), clsCommon.myCstr(dtRawData.Rows(ii)("GrpName")), clsCommon.myCstr(intMainGrpVersion))
                            End If
                            If Not clsCommon.CompairString(strGrpCode, clsCommon.myCstr(dtRawData.Rows(ii)("Account Group Code"))) = CompairStringResult.Equal Then
                                strGrpCode = clsCommon.myCstr(dtRawData.Rows(ii)("Account Group Code"))
                                intGrpVersion += 1
                                AddNewRowForTree(dt, dtRawData.Rows(ii), clsCommon.myCstr(dtRawData.Rows(ii)("Account Group Desc")), clsCommon.myCstr(intMainGrpVersion) + "." + clsCommon.myCstr(intGrpVersion))
                                If arr IsNot Nothing AndAlso arr.Count > 0 Then
                                    For Each strkey As KeyValuePair(Of String, clsTempBalanceSheet) In arr
                                        dt.Rows(dt.Rows.Count - 1)(strkey.Value.ColumnName) = clsCommon.myCdbl(dtRawData.Rows(ii)(strkey.Value.ColumnName))
                                    Next
                        ...
                        ElseIf rbtnSubGroup.IsChecked Then
                            ...
                            If Not clsCommon.CompairString(strSubGrpCode, clsCommon.myCstr(dtRawData.Rows(ii)("Sub Group Code"))) = CompairStringResult.Equal Then
                                ...
                                intSubGrpVersion += 1
                                AddNewRowForTree(dt, dtRawData.Rows(ii), clsCommon.myCstr(dtRawData.Rows(ii)("Sub Grp Desc")), clsCommon.myCstr(intMainGrpVersion) + "." + clsCommon.myCstr(intGrpVersion) + "." + clsCommon.myCstr(intSubGrpVersion))
                                If arr IsNot Nothing AndAlso arr.Count > 0 Then
                                    For Each strkey As KeyValuePair(Of String, clsTempBalanceSheet) In arr
                                        dt.Rows(dt.Rows.Count - 1)(strkey.Value.ColumnName) = clsCommon.myCdbl(dtRawData.Rows(ii)(strkey.Value.ColumnName))
                                    Next
                        ...
                        ElseIf rbtnGLMainAccount.IsChecked Then
                            ...
                            If Not clsCommon.CompairString(strMainGLCode, clsCommon.myCstr(dtRawData.Rows(ii)("Main GL Account"))) = CompairStringResult.Equal Then
                                ...
                                intMainGLVersion += 1
                                AddNewRowForTree(dt, dtRawData.Rows(ii), clsCommon.myCstr(dtRawData.Rows(ii)("Main GL Name")), clsCommon.myCstr(intMainGrpVersion) + "." + clsCommon.myCstr(intGrpVersion) + "." + clsCommon.myCstr(intSubGrpVersion) + "." + clsCommon.myCstr(intMainGLVersion))
                                If arr IsNot Nothing AndAlso arr.Count > 0 Then
                                    For Each strkey As KeyValuePair(Of String, clsTempBalanceSheet) In arr
                                        dt.Rows(dt.Rows.Count - 1)(strkey.Value.ColumnName) = clsCommon.myCdbl(dtRawData.Rows(ii)(strkey.Value.ColumnName))
                                    Next
                        ...
                        ElseIf rbtnGLAccount.IsChecked Then
                            ...
                            If Not clsCommon.CompairString(strMainGLCode, clsCommon.myCstr(dtRawData.Rows(ii)("GL Account"))) = CompairStringResult.Equal Then
                                strGLCode = clsCommon.myCstr(dtRawData.Rows(ii)("GL Account"))
                                intGLVersion += 1
                                AddNewRowForTree(dt, dtRawData.Rows(ii), clsCommon.myCstr(dtRawData.Rows(ii)("GL Account Name")), clsCommon.myCstr(intMainGrpVersion) + "." + clsCommon.myCstr(intGrpVersion) + "." + clsCommon.myCstr(intSubGrpVersion) + "." + clsCommon.myCstr(intMainGLVersion) + "." + clsCommon.myCstr(intGLVersion))
                                If arr IsNot Nothing AndAlso arr.Count > 0 Then
                                    For Each strkey As KeyValuePair(Of String, clsTempBalanceSheet) In arr
                                        dt.Rows(dt.Rows.Count - 1)(strkey.Value.ColumnName) = clsCommon.myCdbl(dtRawData.Rows(ii)(strkey.Value.ColumnName))
                                    Next
    ...
    '                        intGrpCodeVersion = clsCommon.myCdbl(dtRawData.Rows(ii)("Main_Particular"))
    ...
    Sub SetGridFormation(ByVal arr As Dictionary(Of String, clsTempBalanceSheet))
        Dim qry As String = "select * from TSPL_PROFIT_AND_LOSS_PERFORMA where Font_Style>'1.000000'"
        ...
        ElseIf chkTreeView.Checked Then
            If chkLocationWise.Checked Then
                view.ColumnGroups.Add(New GridViewColumnGroup("", "GrpName"))
                view.ColumnGroups("GrpName").Rows.Add(New GridViewColumnGroupRow())
                view.ColumnGroups("GrpName").Rows(0).ColumnNames.Add(gv1.Columns("GrpName").Name)
            ...
            'descriptor1.GroupNames.Add("Particular", System.ComponentModel.ListSortDirection.Ascending)
            'gv1.GroupDescriptors.Add(descriptor1)
            'descriptor1 = New GroupDescriptor()
            'descriptor1.GroupNames.Add("Main_Particular", System.ComponentModel.ListSortDirection.Ascending)
            'gv1.GroupDescriptors.Add(descriptor1)
            'descriptor1 = New GroupDescriptor()
            'descriptor1.GroupNames.Add("Particular", System.ComponentModel.ListSortDirection.Ascending)
            'gv1.GroupDescriptors.Add(descriptor1)
            'descriptor1 = New GroupDescriptor()
            'descriptor1.GroupNames.Add("Sub Group Code", System.ComponentModel.ListSortDirection.Ascending)
            'gv1.GroupDescriptors.Add(descriptor1)
            'descriptor1 = New GroupDescriptor()
            'descriptor1.GroupNames.Add("Main GL Account", System.ComponentModel.ListSortDirection.Ascending)
            'gv1.GroupDescriptors.Add(descriptor1)
        End If
    ...
    Private Sub gv1_ViewCellFormatting(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.CellFormattingEventArgs) Handles gv1.ViewCellFormatting
        Try
            If rbtnMainGroup.IsChecked Or chkTreeView.Checked Then
    ...
    Private Sub gv1_CellDoubleClick(ByVal sender As System.Object, ByVal e As Telerik.WinControls.UI.GridViewCellEventArgs) Handles gv1.CellDoubleClick
        'Try
        '    If rbtnMainGroup.IsChecked Then
        '        Dim dr() As DataRow = DtGroup.Select("S_No=" & gv1.CurrentRow.Cells("S_No").Value)
        '        Dim str As String = ""
        '        If dr.Length > 0 Then
        '            str = clsCommon.myLen(gv1.CurrentRow.Cells("GrpCode").Value) > 0 And IsNothing(dr(0)("Formula"))
        '        Else
        '            str = clsCommon.myLen(gv1.CurrentRow.Cells("GrpCode").Value) > 0
        '        End If
        '        If str Then 'clsCommon.myLen(gv1.CurrentRow.Cells("GrpCode").Value) > 0 And IIf(dr.Length > 0, IsNothing(dr(0)("Formula")), "") Then
        '            Dim frm As New frmRptProfitAndLoss
        '            frm.ReportLevel = 1
        ...
        '            For Each row As String In cbgLocSeg.CheckedValue
        '                frm.arrGlgroup.Add(row)
        '            Next
        '            frm.Show()
        '        End If
        '    ElseIf rbtnGroup.IsChecked Then
        '        If clsCommon.myLen(gv1.CurrentRow.Cells("RollupCode").Value) > 0 Then
        '            Dim frm As New frmRptProfitAndLoss
        '            frm.ReportLevel = 2
        '            frm.rbtnYearly.IsChecked = Me.rbtnYearly.IsChecked
        '            frm.rbtnHalfYearly.IsChecked = Me.rbtnHalfYearly.IsChecked
        ...
        '            frm.txtFiscalYear.Value = Me.txtFiscalYear.Value
        '            frm.lblFiscalYear.Text = Me.lblFiscalYear.Text
        '            frm.strCurrentRollupCode = clsCommon.myCstr(gv1.CurrentRow.Cells("RollupCode").Value)
        ...
        '            If chkLocationWise.IsChecked AndAlso cbgLocSeg.CheckedValue.Count > 0 Then
        ...
        Try
            ...
            ElseIf rbtnGLAccount.IsChecked Then
                ...
                If clsCommon.myLen(strACode) > 0 Then
                    ...
                    frm.RadLabel7.Visible = True
                    frm.txtFromDate.Visible = True
                    frm.boolPrevWithoutOpening = True ''BHA/19/06/19-000905 by balwinder on 19/06/2019
    ...
    Private Sub btnBack_Click(sender As Object, e As EventArgs) Handles btnBack.Click
        Try
            ...
            ElseIf rbtnGLAccount.IsChecked Then
                ...
                rbtnGLMainAccount.IsChecked = True
                RefreshData()
            ElseIf rbtnMainGroup.IsChecked Then
    ...
    Private Sub btnGridPDF_Click(sender As Object, e As EventArgs) Handles btnGridPDF.Click
        Try
            ...
            doc.DocumentName = "ProfitAndLoss"

            doc.MiddleHeader = clsDBFuncationality.getSingleValue("select program_name from tspl_program_Master where program_cODE='" & clsUserMgtCode.rptProfitAndLoss & "'")
