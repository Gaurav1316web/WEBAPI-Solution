...
#End Region
    ...
    Private Sub GLTrans_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'done by stuti on 30/12/2016 against udl points
        ...
        clsCommonFunctionality.CreateOrAlterTable("TSPL_JOURNAL_DETAILS", coll, "Primary Key (Voucher_No,PK_Id)")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_JOURNAL_Details_History", coll)
        coll = New Dictionary(Of String, String)()
        coll.Add("IND_AS", "integer NOT NULL default 0")
        clsCommonFunctionality.CreateOrAlterTable("TSPL_JOURNAL_MASTER", coll)

        Try
            ERPStartDate = clsCommon.myCDate(clsFixedParameter.GetData(clsFixedParameterType.ERPStartDate, clsFixedParameterCode.ERPStartDate, Nothing))
...
#Region "ButtonEvents"
        ...
        Try
            ...
            If arrAccountCode IsNot Nothing AndAlso arrAccountCode.Count > 0 Then
                If clsCommon.GetDateWithStartTime(dtVoucher.Value) >= clsCommon.GetDateWithStartTime(ERPStartDate) Then
                    Dim qry As String = "select Account_Code from TSPL_GL_ACCOUNTS where Account_Code in (" + clsCommon.GetMulcallString(arrAccountCode) + ") and ControlAccount<>'N'"
    ...
    Public Sub authorisedata()
        ...
        Else
            ...
            Dim strQuery As String = "  SELECT TSPL_JOURNAL_MASTER.Source_Narration, TSPL_JOURNAL_MASTER.Source_Type, TSPL_JOURNAL_MASTER.CustVend_Code,TSPL_JOURNAL_MASTER.CustVend_Name, TSPL_JOURNAL_MASTER.Voucher_No, TSPL_JOURNAL_MASTER.Voucher_Date,     case when  TSPL_JOURNAL_MASTER.CustVend_Code='' then   TSPL_JOURNAL_MASTER.Voucher_Desc else TSPL_JOURNAL_MASTER.Voucher_Desc+'  for ' +TSPL_JOURNAL_MASTER.CustVend_Code+' - '+TSPL_JOURNAL_MASTER.CustVend_Name end as Voucher_Desc, " & _
                  "   TSPL_JOURNAL_MASTER.Source_Code,TSPL_JOURNAL_MASTER.Source_Desc, TSPL_JOURNAL_MASTER.Source_doc_No,convert(varchar(15),TSPL_JOURNAL_MASTER.Source_Doc_Date,103) as Source_Doc_Date,TSPL_JOURNAL_MASTER.Posting_Date, TSPL_JOURNAL_MASTER.Total_Debit_Amt, " & _
                  "   TSPL_JOURNAL_MASTER.Total_Credit_Amt, TSPL_JOURNAL_DETAILS.Account_code, TSPL_JOURNAL_DETAILS.Account_Desc,  " & _
                  "   TSPL_JOURNAL_DETAILS.Amount ,case when Amount>=0 then  Amount else 0 end as DrAmt,case when Amount<0 then -1 * Amount else 0 end as CrAmt, TSPL_JOURNAL_DETAILS.Description, TSPL_JOURNAL_DETAILS.Reference,  " & _
                  "   TSPL_JOURNAL_DETAILS.Posting_Date AS [Dtline PostDt], TSPL_JOURNAL_DETAILS.Detail_Line_No, " & _
                  " TSPL_JOURNAL_MASTER.Created_By, Case When Authorized='A' Then TSPL_JOURNAL_MASTER.Modify_By else '' End as Modify_By, TSPL_COMPANY_MASTER.Comp_Name ,TSPL_COMPANY_MASTER.Logo_Img ,TSPL_COMPANY_MASTER.Logo_Img2 ,(SELECT     CASE WHEN TSPL_JOURNAL_MASTER.Authorized = 'N' THEN 'OPEN' WHEN TSPL_JOURNAL_MASTER.Authorized = 'A' THEN 'Authorize' END" & _
                   "  AS Expr1) AS Status, TSPL_JOURNAL_MASTER.Remarks ,TSPL_COST_CENTRE_FINANCIAL.Cost_Center_Fin_Code ,TSPL_COST_CENTRE_FINANCIAL.Cost_Center_Fin_Name, TSPL_HIRERACHY_LEVEL_MASTER.Hirerachy_Code,TSPL_HIRERACHY_LEVEL_MASTER.Description as HIRERACHY_LEVEL " & _
                  "   FROM         TSPL_JOURNAL_MASTER INNER JOIN TSPL_JOURNAL_DETAILS ON TSPL_JOURNAL_MASTER.Journal_No = TSPL_JOURNAL_DETAILS.Journal_No  " & _
                  "  AND  TSPL_JOURNAL_MASTER.Voucher_No = TSPL_JOURNAL_DETAILS.Voucher_No  left outer join  TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code  =TSPL_JOURNAL_MASTER.Comp_Code   left outer join TSPL_COST_CENTRE_FINANCIAL on TSPL_COST_CENTRE_FINANCIAL.Cost_Center_Fin_Code=TSPL_JOURNAL_DETAILS.Cost_Centre_Code " & _
    ...
    Sub PrintData(ByVal StrCode As String, ByVal FormID As String)
        ...
        Else
            ...
            Dim strQuery As String = "  SELECT TSPL_JOURNAL_MASTER.Source_Narration, TSPL_JOURNAL_MASTER.Source_Type, TSPL_JOURNAL_MASTER.CustVend_Code,TSPL_JOURNAL_MASTER.CustVend_Name, TSPL_JOURNAL_MASTER.Voucher_No, TSPL_JOURNAL_MASTER.Voucher_Date,     case when  TSPL_JOURNAL_MASTER.CustVend_Code='' then   TSPL_JOURNAL_MASTER.Voucher_Desc else TSPL_JOURNAL_MASTER.Voucher_Desc+'  for ' +TSPL_JOURNAL_MASTER.CustVend_Code+' - '+TSPL_JOURNAL_MASTER.CustVend_Name end as Voucher_Desc, " & _
                    ...
                    "   FROM         TSPL_JOURNAL_MASTER INNER JOIN TSPL_JOURNAL_DETAILS ON TSPL_JOURNAL_MASTER.Journal_No = TSPL_JOURNAL_DETAILS.Journal_No  " & _
                    "  AND  TSPL_JOURNAL_MASTER.Voucher_No = TSPL_JOURNAL_DETAILS.Voucher_No  left outer join  TSPL_COMPANY_MASTER on TSPL_COMPANY_MASTER.Comp_Code  =TSPL_JOURNAL_MASTER.Comp_Code   left outer join TSPL_COST_CENTRE_FINANCIAL on TSPL_COST_CENTRE_FINANCIAL.Cost_Center_Fin_Code=TSPL_JOURNAL_DETAILS.Cost_Centre_Code " & _
                    " left outer join TSPL_HIRERACHY_LEVEL_MASTER on TSPL_HIRERACHY_LEVEL_MASTER.Hirerachy_Code=TSPL_JOURNAL_DETAILS.Hirerachy_Code where TSPL_JOURNAL_MASTER.Voucher_No = '" + StrCode + "'  order by Detail_Line_No "
...
#Region "AutoNumber"
    ...
    Public Function fnAutoGenerateNo(ByVal trans As SqlTransaction, ByVal TranDate As Date, ByVal isGLJE As Boolean, ByVal strLocationCode As String, ByVal isLocationCodeisSegment As Boolean, Optional ByVal isMilkSRN As Boolean = False) As String
        ...
        ElseIf isMilkSRN Then
            strJournalNo = clsERPFuncationality.GetNextCode(trans, TranDate, clsDocType.JournalEntry, clsDocTransactionType.JournalEntryMilkSRN, strLocationCode, isLocationCodeisSegment)
        Else
...
#Region "Function"
        ...
        Try
            ...
            If clsCommon.myLen(txtLocation.Value) > 0 Then
                ...
                If clsCommon.myLen(whrcls) > 0 Then
                    finalWhr = " ( " + whrcls + " ) and "
                End If
                finalWhr += "  ((Account_Code like '%" + txtLocation.Value + "' and ControlAccount ='N' ) or (Account_Code in (select Account_Code from TSPL_CONTROL_ACC_MAPPING where Account_Code like '%" + txtLocation.Value + "')))"
                whrcls = finalWhr
            Else
    ...
    Private Sub funInsert(ByVal ChekPostBtn As Boolean)
        ...
        Try
            ...
            Dim qry As String = "Update TSPL_JOURNAL_MASTER set Segment_Code='" + txtLocation.Value + "',MonthlyReverse='" & IIf(chkMonthly.Checked, 1, 0) & "',ProgramCode='" & Program_Code & "',IND_AS='" & IIf(chkIndAS.Checked, 1, 0) & "' where Voucher_No='" + StrVoucher + "' "
    ...
    Public Function funUpdate(Optional ByVal transNew As SqlTransaction = Nothing) As String
        ...
        Try
            ...
            Dim qry As String = "Update TSPL_JOURNAL_MASTER set Segment_Code='" & txtLocation.Value & "',IND_AS='" & IIf(chkIndAS.Checked, 1, 0) & "',MonthlyReverse='" & IIf(chkMonthly.Checked, 1, 0) & "' where Voucher_No='" & fndVoucher.Value & "' "
            clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Public Sub funFill()
        ...
        Try
            sql = " SELECT TSPL_JOURNAL_MASTER.Voucher_No, TSPL_JOURNAL_MASTER.Voucher_Date, TSPL_JOURNAL_MASTER.Source_Code, " & _
                         ...
                         "  TSPL_JOURNAL_MASTER.Source_Type, TSPL_JOURNAL_MASTER.CustVend_Code, TSPL_JOURNAL_MASTER.CustVend_Name,  " & _
                         ...
                         " TSPL_JOURNAL_DETAILS.Cost_Centre_Code as [Cost Centre],TSPL_JOURNAL_MASTER.Ind_As " & _
...
#End Region
    ...
    Private Sub btnUnpostTransaction_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUnpostTransaction.Click
        clsCommon.MyMessageBoxShow("Do this work by Reverse Journal Entry")
        'If clsCommon.myLen(fndVoucher.Value) > 0 Then
        '    Dim qry As String = "select 1 from TSPL_JOURNAL_MASTER where Voucher_No='" + fndVoucher.Value + "' and Authorized='A'"
        '    Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
        '    If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
...
"lbexit:" + Environment.NewLine + _
    ...
    Private Sub btnExportblank_Click(sender As Object, e As EventArgs) Handles btnExportblank.Click
        Try
            Dim strCmd As String = "select  '' as [Detail Line No] , '' as [Voucher Description] , '' as [Voucher Date],'' as [Account Code],'' as [Account Name],0 as [Debit],0 as [Credit],'' as [Description] "
            If SettingCostCenter Then
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.GetDateWithStartTime(clsCommon.myCDate(EntryDate)) >= clsCommon.GetDateWithStartTime(ERPStartDate) Then
                        qry = "select TSPL_GL_ACCOUNTS.Account_Code from TSPL_GL_ACCOUNTS WHERE TSPL_GL_ACCOUNTS.Account_Code='" & AccCode & "' and TSPL_GL_ACCOUNTS.ControlAccount='N' "
                        STRACCOUNT = clsCommon.myCstr(clsDBFuncationality.getSingleValue(qry, trans))
                        If clsCommon.myLen(STRACCOUNT) <= 0 Then
