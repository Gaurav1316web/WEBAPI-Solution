...
Public Class FrmFiscalYearEndProcess
        ...
        Try
            ...
            If clsCommon.MyMessageBoxShow(Me, "Start the finanacial year end process" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Try
                    ...
                    'qry = " update TSPL_LOCK_LOCATION set Is_Locked = 1, End_Date = (select End_Date from TSPL_Fiscal_Year_Master where Fiscal_Code = '" + objCommonVar.CurrFiscalYear + "') "
                    'clsDBFuncationality.ExecuteNonQuery(qry, trans)
    ...
    Private Sub RadButton2_Click(sender As Object, e As EventArgs) Handles RadButton2.Click
        Try
            ...
            Dim trans As SqlTransaction = clsDBFuncationality.GetTransactin()
            Try
                Dim qry As String = "disable TRIGGER dbo.TRG_JD_FiscaYearEndNoUpdateNoDelete ON dbo.TSPL_JOURNAL_DETAILS"
                clsDBFuncationality.ExecuteNonQuery(qry, trans)

                qry = "disable TRIGGER dbo.TRG_JM_FiscaYearEndNoUpdateNoDelete ON dbo.TSPL_JOURNAL_MASTER "
                clsDBFuncationality.ExecuteNonQuery(qry, trans)

                qry = "delete from TSPL_JOURNAL_DETAILS where Voucher_No in ( select Voucher_No from TSPL_JOURNAL_MASTER where Source_Code='GL-JE' and Transaction_Type='X' and convert(date,Source_Doc_Date,103)=(select convert(date, End_Date,103) from TSPL_Fiscal_Year_Master where Fiscal_Code='" + TxtFinder1.Value + "') and Voucher_Desc like 'Fiscal Year End for%')"
                clsDBFuncationality.ExecuteNonQuery(qry, trans)
