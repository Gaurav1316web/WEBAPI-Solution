...
Imports common
Public Class FrmUserGroupMapping
    ...
    Private Sub FrmUserGroupMapping_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        End If
    End Sub
    Private Sub FrmUserGroupMapping_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
    ...
    Private Sub fun_gridfill()
        ...
        Try
            ...
            Dim strQuery As String = "select Group_Code, Group_Desc from TSPL_USER_GROUP_MASTER"
        ...
        Try
            For i As Integer = 0 To dgv_Groupmapping.Rows.Count - 1
                If CBool(dgv_Groupmapping.Rows(i).Cells(2).Value = True) Then
                    connectSql.RunSp("SP_TSPL_USER_GROUP_MAPPING_INSERT", New SqlParameter("@User_Code", fndUser_Name.Value), New SqlParameter("@Group_Code", dgv_Groupmapping.Rows(i).Cells(0).Value), New SqlParameter("@created_by", userCode), New SqlParameter("@created_date", connectSql.serverDate()), New SqlParameter("Modify_By", userCode), New SqlParameter("Modify_Date", connectSql.serverDate()), New SqlParameter("@Comp_Code", companyCode))
                End If
            Next
            Dim strUser_Code As String = "select User_Code from TSPL_USER_GROUP_MAPPING where User_Code='" + fndUser_Name.Value + "'"
            Dim dr As DataTable
            dr = clsDBFuncationality.GetDataTable(strUser_Code)
    ...
    Private Sub funfill()
        Dim strQuery As String = "select Group_Code as [Group Code] from TSPL_USER_GROUP_MAPPING WHERE User_Code='" + fndUser_Name.Value + "'"
            ...
            For j As Integer = 0 To dt.Rows.Count - 1
                ...
                For i As Integer = 0 To dgv_Groupmapping.Rows.Count - 1
                    If dgv_Groupmapping.Rows(i).Cells(0).Value = strcode Then
                        ...
                        'Else
                        '    dgv_Groupmapping.Rows(i).Cells(2).Value = False
                    End If
                Next
            Next
        End If
    ...
    Private Sub RadMenuItem_Export_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadMenuItem_Export.Click
        Dim strQuery As String = "select d.User_Code as [User Code],d.Group_Code as [Group Code],m.Group_Desc as [Group Desc] from TSPL_USER_GROUP_MAPPING d join TSPL_USER_GROUP_MASTER m on m.Group_Code = d.Group_Code"

        ListImpExpColumnsMandatory = New List(Of String)({"User Code", "Group Code"})
        ListImpExpColumnsSuperMandatory = New List(Of String)({"User Code", "Group Code"})
        transportSql.ExporttoExcel(strQuery, "", "", Me, ListImpExpColumnsMandatory, ListImpExpColumnsSuperMandatory, MyBase.Form_ID)
    End Sub
            ...
            Try
                ...
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.myLen(strg_code) > 0 Then
                        GroupCode = clsDBFuncationality.getSingleValue("Select Group_Code from TSPL_USER_GROUP_MASTER Where Group_Code='" + strg_code + "'", trans)
                        If clsCommon.myLen(GroupCode) <= 0 Then
                            Throw New Exception("Group Code '" + strg_code + "' at line " + LineNo + " does not exist .")
                        End If
                    Else
                        Throw New Exception("Group Code can not be blank at line " + LineNo + " .")
                    End If

                    connectSql.RunSpTransaction(trans, "SP_TSPL_USER_GROUP_MAPPING_INSERT", New SqlParameter("@User_Code", UserCode), New SqlParameter("@Group_Code", GroupCode), New SqlParameter("@Created_By", UserCode), New SqlParameter("@Created_Date", connectSql.serverDate(trans)), New SqlParameter("@Modify_By", UserCode), New SqlParameter("@Modify_Date", connectSql.serverDate(trans)), New SqlParameter("@Comp_Code", companyCode))
    ...
    Private Sub fun_Update()
        For i As Integer = 0 To dgv_Groupmapping.Rows.Count - 1
            Dim strUser_Code As String = "delete from TSPL_USER_GROUP_MAPPING where User_Code='" + fndUser_Name.Value + "'"
            connectSql.RunSql(strUser_Code)
        Next
        For i As Integer = 0 To dgv_Groupmapping.Rows.Count - 1
            If CBool(dgv_Groupmapping.Rows(i).Cells(2).Value = True) Then
                connectSql.RunSp("SP_TSPL_USER_GROUP_MAPPING_INSERT", New SqlParameter("@User_Code", fndUser_Name.Value), New SqlParameter("@Group_Code", dgv_Groupmapping.Rows(i).Cells(0).Value), New SqlParameter("@created_by", userCode), New SqlParameter("@created_date", connectSql.serverDate()), New SqlParameter("Modify_By", userCode), New SqlParameter("Modify_Date", connectSql.serverDate()), New SqlParameter("@Comp_Code", companyCode))
