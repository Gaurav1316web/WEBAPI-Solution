...
Imports common
Public Class FrmCattleColorMaster
    ...
    Private Sub SetUserMgmtNew()
        ''MyBase.SetUserMgmt("CattleColor")
    ...
    Private Sub AddNew()
        ...
        txtCode.Focus()
    End Sub
    Private Sub FrmCattleColorMaster_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SetUserMgmtNew()
        isNewEntry = True
    ...
    Sub LoadData(ByVal strCode As String, ByVal NavTyep As NavigatorType)
        Dim obj As clsCattleColor = clsCattleColor.GetData(strCode, NavTyep)
        If obj IsNot Nothing Then
            isNewEntry = False
            txtCode.Value = obj.Cattle_Color_Code
            txtDesc.Text = obj.Cattle_Color_Name
    ...
    Sub SaveData()
        Try
            If AllowToSave() Then
                Dim obj As New clsCattleColor()
                obj.Cattle_Color_Code = txtCode.Value
                obj.Cattle_Color_Name = txtDesc.Text
                Dim qry As Integer = clsDBFuncationality.getSingleValue("select count(Cattle_Color_Code) from TSPL_CATTLE_COLOR_MASTER WHERE Cattle_Color_Code='" + obj.Cattle_Color_Code + "'")
                If (qry = 0) Then
                    isNewEntry = True
                Else
                    isNewEntry = False
                End If
                If (clsCattleColor.SaveData(obj, isNewEntry)) Then
                    clsCommon.MyMessageBoxShow("Data saved Successfully", Me.Text)
                    LoadData(obj.Cattle_Color_Code, NavigatorType.Current)
                    btnsave.Text = "Update"
                    btndelete.Enabled = True
                Else
                    btnsave.Text = "Save"
    ...
    Private Sub DeleteData()
        Try
            ...
            If clsCommon.MyMessageBoxShow("Are you sure? do you want to delete this Code ('" + txtcode.Value + "')", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                Dim qry As String = "DELETE FROM TSPL_CATTLE_COLOR_MASTER WHERE Cattle_Color_Code='" + txtCode.Value + "'"
    ...
    Private Sub txtCode__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtCode._MYValidating
        Dim str As String = "select count(*) from TSPL_CATTLE_COLOR_MASTER where Cattle_Color_Code ='" + txtCode.Value + "' "
        ...
        If txtcode.MyReadOnly OrElse isButtonClicked Then
            Dim qry As String = ""
            qry = "Select Cattle_Color_Code As [Code],Cattle_Color_Name As [Name] from TSPL_CATTLE_COLOR_MASTER "
            txtCode.Value = clsCommon.ShowSelectForm("TSPL_CATTLE_COLOR_MASTER", qry, "Code", "", txtCode.Value, "TSPL_CATTLE_COLOR_MASTER.Cattle_Color_Code", isButtonClicked)
            If clsCommon.myLen(txtcode.Value) > 0 Then
                Dim objOT As clsCattleColor
                objOT = clsCattleColor.GetData(txtCode.Value, NavigatorType.Current)
    ...
    Private Sub RadMenuItem3_Click(sender As Object, e As EventArgs) Handles RadMenuItem3.Click
        Dim str As String
        str = " select Cattle_Color_Code as [Code],Cattle_Color_Name As [Description]  from TSPL_CATTLE_COLOR_MASTER "
    ...
    Private Sub FrmCattleColorMaster_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        ...
        If transportSql.importExcel(gv, "Code", "Description") Then
            ...
            Try
                For Each grow As GridViewRowInfo In gv.Rows
                    ...
                    If clsCommon.myLen(strCode) > 0 AndAlso clsDBFuncationality.getSingleValue("Select count(*) from TSPL_CATTLE_COLOR_MASTER where Cattle_Color_Code='" + strCode + "' ") > 0 Then
                        IsNewEntry = False
                    Else
                        IsNewEntry = True
                    ...
                    obj.Cattle_Color_Code = strCode
                    obj.Cattle_Color_Name = strDescription
                    clsCattleColor.SaveData(obj, IsNewEntry)
