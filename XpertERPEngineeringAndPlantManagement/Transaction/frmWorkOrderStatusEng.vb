...
Public Class frmWorkOrderStatusEng
...
#End Region
    ...
    Sub LoadData()
        ...
        '        whrCls = " tspl_parameter_range_master.Loc_Code in (" & objCommonVar.strCurrUserLocations & ") "
        '    End If
        'End If
        'whrCls = " tspl_parameter_range_master.Loc_Code ='" & fndLoc.Value & "' and vendor_Class='" & cmbVendorClass.SelectedValue & "'"
        Try
            gv.Rows.Clear()
            ' gv.Rows.AddNew()
            LoadBlankGrid()
            isNewEntry = True
            '  Dim qry As String = "select count(*) from tspl_parameter_range_master " & IIf(clsCommon.myLen(whrCls) > 0, " where " & whrCls, "")
            ' Dim check As Integer = clsDBFuncationality.getSingleValue(qry)

            'If check > 0 Then
            '  qry = "select tspl_parameter_range_master.code,tspl_parameter_range_master.lower_range,tspl_parameter_range_master.loc_code,TSPL_LOCATION_MASTER.location_desc,tspl_parameter_range_master.upper_range,tspl_parameter_range_master.value,tspl_parameter_range_master.effective_date,tspl_parameter_range_master.Condition_value,tspl_parameter_range_master.Status,tspl_parameter_master.description,tspl_parameter_master.nature,tspl_parameter_range_master.Vendor_Class,tspl_parameter_range_master.IsReject from tspl_parameter_range_master left outer join tspl_parameter_master on tspl_parameter_range_master.code=tspl_parameter_master.code  left outer join TSPL_LOCATION_MASTER  on TSPL_LOCATION_MASTER.Location_Code=tspl_parameter_range_master.loc_code " & IIf(clsCommon.myLen(whrCls) > 0, " where " & whrCls, "")
    ...
    Private Sub frmWorkOrderStatusEng_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub frmWorkOrderStatusEng_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        LoadProcurementType()
    ...
    Sub LoadBlankGrid()
        ...
        repocode.HeaderImage = My.Resources.search4
        ...
        repoValue.HeaderImage = My.Resources.search4
    ...
    Private Sub gv_CellValueChanged(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.GridViewCellEventArgs) Handles gv.CellValueChanged
        If Not isLoadData Then '------when on loaddata then it should not run
            If Not isValueChanged Then
                ...
                If clsCommon.CompairString(e.Column.Name, colCode) = CompairStringResult.Equal AndAlso (Not TypeOf e.Row Is GridViewFilteringRowInfo) Then
                    ...
                    ElseIf (clsCommon.CompairString(gv.CurrentRow.Cells(colNature).Value, "B") = CompairStringResult.Equal) Then
                        ...
                        '    gv.CurrentRow.Cells(colStatus).ReadOnly = True
                    End If
                ElseIf clsCommon.CompairString(e.Column.Name, colCondtionValue) = CompairStringResult.Equal AndAlso (Not TypeOf e.Row Is GridViewFilteringRowInfo) Then
                ...
                ElseIf clsCommon.CompairString(e.Column.Name, colLocCode) = CompairStringResult.Equal AndAlso (Not TypeOf e.Row Is GridViewFilteringRowInfo) Then
                    OpenLocation()
    ...
    Private Sub gv_EditorRequired(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.EditorRequiredEventArgs) Handles gv.EditorRequired
    ...
    Private Sub gv_UserDeletingRow(ByVal sender As Object, ByVal e As Telerik.WinControls.UI.GridViewRowCancelEventArgs) Handles gv.UserDeletingRow
        ...
        If common.clsCommon.MyMessageBoxShow(Me, "Delete The Current Row." + Environment.NewLine + "It will delete the record permanently " + "Are you sure?", Me.Text, MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.No Then
            e.Cancel = True
            Exit Sub
        Else
