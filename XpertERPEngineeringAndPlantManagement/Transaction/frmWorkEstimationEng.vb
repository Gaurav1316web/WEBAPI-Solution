...
'New Screen,  Added by Sanjay 30/07/2020
Public Class frmWorkEstimationEng
...
#End Region
    ...
    Sub LoadBlankGrid()
        ...
        repoOrderNo.HeaderImage = My.Resources.search4
        ...
        Try
            ...
            For ii As Integer = 0 To gv1.Rows.Count - 1
                    ...
                    If clsCommon.myLen(strICode) > 0 AndAlso clsCommon.myCdbl(gv1.Rows(ii).Cells(colRate).Value) <= 0 Then
                        gv1.CurrentRow = gv1.Rows(ii)
                    clsCommon.MyMessageBoxShow(Me, "Unit Cost should not be 0(zero) at row no. " + clsCommon.myCstr(ii + 1) + ".")
                    Return False
                    End If
    ...
    Sub SaveData(ByVal ChekBtnPost As Boolean)
        Try
            ...
            If (AllowToSave()) Then
                Dim obj As New clsWorkEstimationHead()
                'obj.Is_Open_PO = CInt(clsCommon.myCdbl(IIf(chkOpenPO.Checked = True, 1, 0)))
                obj.WorkEstimation_Id = txtReqNo.Value
                obj.WorkEstimation_Date = txtDate.Value
                obj.Requisition_Id = txtRequisition.Value
                ...
                '    End If
                'End If
                obj.ArrTr = New List(Of clsWorkEstimationDetail)
                For Each grow As GridViewRowInfo In gv1.Rows
                    Dim objTr As New clsWorkEstimationDetail()
                ...
                If (obj.SaveData(obj, isNewEntry)) Then
                    ...
                    txtReqNo.Value = obj.WorkEstimation_Id
                    ''BM00000008148 approval work 16/10/2015
                    ...
                    LoadData(obj.WorkEstimation_Id, NavigatorType.Current)
                End If
    ...
    Sub LoadData(ByVal strDocumentNo As String, ByVal navType As common.NavigatorType)
        Try
            ...
            Dim obj As New clsWorkEstimationHead()
            obj = clsWorkEstimationHead.GetData(strDocumentNo, navType, Nothing, "N")
            If (obj IsNot Nothing AndAlso clsCommon.myLen(obj.WorkEstimation_Id) > 0) Then
                ...
                txtReqNo.Value = obj.WorkEstimation_Id
                txtDate.Value = obj.WorkEstimation_Date
                txtRequisition.Value = obj.Requisition_Id
                ...
                If clsCommon.myLen(Me.fndcapexsubcode.Value) > 0 Then
                    ...
                    lbl_rebudgetamt.Text = clsCapexBudget.GetReBudget(Me.fndcapexsubcode.Value, obj.WorkEstimation_Id, Nothing)
                    lbl_rebudgetamtwithtolerence.Text = clsCapexBudget.GetReBudgetWithTolerence(Me.fndcapexsubcode.Value, obj.WorkEstimation_Id, Nothing)
                End If
                ...
                gv1.Rows.Clear()
                For Each objTr As clsWorkEstimationDetail In obj.ArrTr
                    gv1.Rows.AddNew()
                ...
                ''For Custom Fields
                If MyBase.customFieldTabProperty = ElementVisibility.Visible Then
                    UcCustomFields1.LoadData(obj.WorkEstimation_Id)
                End If
                clsCustomFieldGrid.FillDataInGrid(obj.WorkEstimation_Id, MyBase.Form_ID, gv1)
                ''End of For Custom Fields
                UcAttachment1.LoadData(obj.WorkEstimation_Id)
    ...
    '---------------------Richa Code Ends Here-------------------------
    Sub LoadDataRequisition(ByVal strDocumentNo As String)
        Try
            isInsideLoadData = True
            BlankAllControls()
            LoadBlankGrid()
            ...
            obj = clsRequistionHead.GetData(strDocumentNo, NavigatorType.Current, Nothing, "N", clsUserMgtCode.WorkRequisitionEng)
    ...
    Private Sub txtDocNo__MYNavigator(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal NavType As common.NavigatorType) Handles txtReqNo._MYNavigator
        Try
            ...
            vaddnew = "Y"
            Dim qst As String = "select count(*) from TSPL_WORK_ESTIMATION_HEAD where Is_Internal='N'"
    ...
    Private Sub txtDocNo__MYValidating(ByVal sender As System.Object, ByVal e As System.EventArgs, ByVal isButtonClicked As System.Boolean) Handles txtReqNo._MYValidating
        ...
        '    whrClas = whrClas & " and TSPL_WORK_ESTIMATION_HEAD.Created_By='" + clsCommon.myCstr(objCommonVar.CurrentUserCode) + "'"
        'End If
        'If clsCommon.myLen(objCommonVar.strCurrUserLocations) > 0 Then
        '    whrClas = whrClas & " and Location in (" + objCommonVar.strCurrUserLocations + ")"
        'End If

        LoadData(clsCommon.ShowSelectForm("WEReqfndNo", qry, "Code", whrClas, txtReqNo.Value, "WorkEstimation_Date desc", isButtonClicked), NavigatorType.Current)
    End Sub
    ...
    Private Sub frmWorkEstimationEng_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
    ...
    Private Sub btnUnpost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUnpost.Click
        Try
            If clsCommon.myLen(txtReqNo.Value) > 0 Then
                Dim qry As String = "select 1 from TSPL_WORK_ESTIMATION_HEAD where WorkEstimation_Id='" + txtReqNo.Value + "' and Status=1"
                Dim dt As DataTable = clsDBFuncationality.GetDataTable(qry)
                If dt Is Nothing OrElse dt.Rows.Count <= 0 Then
                    Throw New Exception("Transaction status should be posted.")
                End If
                'Use this code for Work order check
                ...
                If clsCommon.MyMessageBoxShow(Me, "Unpost the current transaction" + Environment.NewLine + "Are you sure", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    qry = "update TSPL_WORK_ESTIMATION_HEAD set Status=0,Posting_Date=null where WorkEstimation_Id='" + txtReqNo.Value + "'"
    ...
    Private Sub chkprclose_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chkprclose.CheckedChanged
        ...
        '    Dim response = MsgBox("Are you sure want to close Work Estimation?", MsgBoxStyle.YesNo, "Attention")
    ...
    Private Sub btnMailSendemail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnMailSendemail.Click
        ...
        Try
            ...
            If Not (common.clsCommon.MyMessageBoxShow(Me, "Send E-Mail/SMS Of Respective Purchase Requisition No. " + txtReqNo.Value + "" + Environment.NewLine + "Are you sure?", Me.Text, MessageBoxButtons.YesNo, RadMessageIcon.Question) = DialogResult.Yes) Then
                Return
            End If
            LoadData(txtReqNo.Value, NavigatorType.Current)
    ...
    Private Sub TxtRequisition__MYValidating(sender As Object, e As EventArgs, isButtonClicked As Boolean) Handles txtRequisition._MYValidating
        Try
            'Dim qry As String = "select Requisition_Id as Code,Requisition_Date as Date,Status from TSPL_REQUISITION_HEAD "
            'Dim WhrCls As String = " From_Screen_Code='" + clsUserMgtCode.WorkRequisitionEng + "'"

            'txtRequisition.Value = clsCommon.ShowSelectForm("PURESTFND", qry, "Code", WhrCls, txtRequisition.Value, "Code", isButtonClicked)
            ...
            whrClas += " and TSPL_REQUISITION_HEAD.Status=1 and TSPL_REQUISITION_HEAD.From_Screen_Code='" + clsUserMgtCode.WorkRequisitionEng + "'"
            whrClas += " and not exists (select 1 from TSPL_WORK_ESTIMATION_HEAD where TSPL_REQUISITION_HEAD.Requisition_Id=TSPL_WORK_ESTIMATION_HEAD.Requisition_Id) "
            'If ShowIndentBasedOnCreatedUser AndAlso Not (MyBase.isPostFlag) Then
            ...
            '    whrClas = whrClas & " and Location in (" + objCommonVar.strCurrUserLocations + ")"
            'End If
            txtRequisition.Value = clsCommon.ShowSelectForm("WERufindNo", qry, "Code", whrClas, txtRequisition.Value, "Requisition_Date desc", isButtonClicked)
            If clsCommon.myLen(txtRequisition.Value) > 0 Then
